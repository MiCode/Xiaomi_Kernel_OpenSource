...
#define POWER_CONSUMPTION_WORKAROUND
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		#if defined(POWER_CONSUMPTION_WORKAROUND)
		msleep(2);
		regmap_update_bits(cs35l45->regmap, CS35L45_REFCLK_INPUT, CS35L45_PLL_FORCE_EN_MASK, 0);
		#endif
		break;
	default:
		dev_err(cs35l45->dev, "Invalid event = 0x%x\n", event);
			       ...
			       struct snd_soc_dai *dai)
{
	...
	#if defined(POWER_CONSUMPTION_WORKAROUND)
	regmap_update_bits(cs35l45->regmap, CS35L45_REFCLK_INPUT, CS35L45_PLL_FORCE_EN_MASK, CS35L45_PLL_FORCE_EN_MASK);
	msleep(1);
	#endif
