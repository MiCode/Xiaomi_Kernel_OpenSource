...
int fsm_misc_send_apr(void __user *buf)
{
	...
	if (afe.param_id == CAPI_V2_PARAM_FSADSP_CALIB && header[4] == 0) {
		fsm_reset_re25_data();
	}
...
int fsm_get_dev_info(int *arg)
{
	...
	for (idx = 0; idx < dev_info.dev_count; idx++) {
		fsm_dev = fsm_get_fsm_dev_by_id(idx);
		if (fsm_dev == NULL) {
			continue;
		}
		dev_info.addr[idx] = fsm_dev->addr;
		...
		size_t count, loff_t *offset)
{
	...
	fsm_misc = filp->private_data;
	if (fsm_misc->addr == 0 || fsm_misc->index < 0)
		return -ENODEV;
	do {
