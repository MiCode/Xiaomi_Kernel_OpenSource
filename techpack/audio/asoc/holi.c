				...
				int count)
{
    ...
    if (((MSM_BACKEND_DAI_SECONDARY_MI2S_RX == dailink[i].id)
        ...
        && !strcmp(dailink[i].name, LPASS_BE_SEC_MI2S_TX))) {
			...
			//dailink[i].codecs = NULL; // clear codecs config
			//dailink[i].num_codecs = 0; // clear codecs config
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev,"%s[ZS]start %d\n", __func__, ret);
	if (!pdev->dev.of_node) {
		dev_err(&pdev->dev,
	...
	dev_info(&pdev->dev,"%s[ZS]populate_snd_card_dailinks_return_value %d\n", __func__, ret);
	...
	dev_info(&pdev->dev,"%s[ZS]devm_snd_soc_register_card_return_value %d\n", __func__, ret);
	if (ret == -EPROBE_DEFER) {
		if (codec_reg_done)
			ret = -EINVAL;
		goto err;
