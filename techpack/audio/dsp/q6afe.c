...
#if defined(CONFIG_SND_SOC_FS19XX)
			...
			struct cal_type_data *cal_type, u16 port_id)
{
	...
	list_for_each_safe(ptr, next,
		...
		if (cal_utils_is_cal_stale(cal_block))
			continue;
		pr_err("%s: port id: 0x%x, dev_acdb_id: %d\n", __func__,
		...
		if (afe_top->path == path) {
			if (this_afe.dev_acdb_id[afe_port_index] > 0) {
				if (afe_top->acdb_id ==
					...
					pr_err("%s: top_id:%x acdb_id:%d afe_port_id:0x%x\n",
				...
				pr_err("%s: top_id:%x acdb_id:%d afe_port:0x%x\n",
				   ...
				   int cal_type_index)
{
	...
	*topology_id = (u32)afe_top_info->topology;
	...
	pr_err("%s: port_id = 0x%x acdb_id = %d topology_id = 0x%x cal_type_index=%d ret=%d\n",
...
int afe_send_port_island_mode(u16 port_id)
{
	...
	pr_err("%s: AFE set island mode 0x%x  enable for port 0x%x ret %d\n",
			    ...
			    struct afe_ttp_config *ttp_cfg)
{
	...
	/* Start SW MAD module */
	mad_type = afe_port_get_mad_type(port_id);
	pr_err("%s: port_id 0x%x, mad_type %d\n", __func__, port_id,
		 mad_type);
	if (mad_type != MAD_HW_NONE && mad_type != MAD_SW_AUDIO) {
		if (!afe_has_config(AFE_CDC_REGISTERS_CONFIG) ||
			!afe_has_config(AFE_SLIMBUS_SLAVE_CONFIG)) {
...
static int q6afe_send_pkt_inband(struct fsm_afe *afe,
		uint8_t *param, uint32_t param_size)
{
	...
	if (afe->op_set) {
		...
		if (packed_param_data == NULL)
			return -ENOMEM;
		ret = q6common_pack_pp_params(packed_param_data, &afe->param_hdr, param,
						  &packed_data_size);
		if (ret) {
		...
		ret = q6afe_set_params(afe->port_id, index, NULL, packed_param_data,
					   packed_data_size);
		kfree(packed_param_data);
	} else {
	...
	if (ret) {
		pr_err("%s module:%x, %s param: %x failed\n", __func__, afe->module_id,
				(afe->op_set ? "set" : "get"), afe->param_id);
	}
	...
	if (afe->op_set) { // set params
		...
		if (packed_data_size > cal_block->map_data.map_size) {
			pr_err("%s: packed size:%d must less than map size: %d\n", __func__,
					packed_data_size, cal_block->map_data.map_size);
			return -EINVAL;
		}
		cal_block->cal_data.size = packed_data_size;
		ret = q6afe_set_params(afe->port_id, index, &afe->mem_hdr,
				NULL, packed_data_size);
	} else { // get params
		ret = q6afe_get_params(afe->port_id, &afe->mem_hdr, &afe->param_hdr);
	}
	if (ret) {
		pr_err("%s module:%x, %s param: %x failed", __func__, afe->module_id,
				(afe->op_set ? "set" : "get"), afe->param_id);
	}

	return ret;
}
EXPORT_SYMBOL(q6audio_get_port_id);
