...
#if IS_ENABLED(CONFIG_ISPV3)
/**
 * _cam_req_mgr_check_ispv3_device_linked()
 *
 * @brief    : check ispv3 device is presented in this link
 * @link     : crm link that links the rt devices
 *
 * @return   : true for ispv3 presented in this link
 *
 */
bool _cam_req_mgr_check_ispv3_device_linked(struct cam_req_mgr_core_link *link)
{
	bool ispv3_connected = false;
	int  i = 0;
	for (; i < link->num_devs; i++) {
		if (link->l_dev[i].dev_info.dev_id == CAM_REQ_MGR_DEVICE_ISPV3){
			ispv3_connected = true;
			break;
		}
	}

	return ispv3_connected;
}
#endif
...
#if IS_ENABLED(CONFIG_ISPV3)
	/* trigger full recovery if ispv3_key_reg_bad_state is true */
	if (link->ispv3_key_reg_bad_state &&
		_cam_req_mgr_check_ispv3_device_linked(link)) {
		struct cam_req_mgr_core_session *session = NULL;
		struct cam_req_mgr_message       msg;
		int pd;

		// reset to false
		link->ispv3_key_reg_bad_state = FALSE;
		...
		/* Notify all devices in the link about the error */
		__cam_req_mgr_send_evt(0, CAM_REQ_MGR_LINK_EVT_ERR, CRM_KMD_ERR_FATAL, link);
		...
		msg.session_hdl             = session->session_hdl;
		msg.u.err_msg.error_type    = CAM_REQ_MGR_ERROR_TYPE_FULL_RECOVERY;
		msg.u.err_msg.request_id    = link->req.apply_data[result_idx][pd].req_id;
		msg.u.err_msg.link_hdl      = link->link_hdl;
		msg.u.err_msg.resource_size = 0;
		msg.u.err_msg.error_code    = CAM_REQ_MGR_CSID_RECOVERY_OVERFLOW_ERROR; // fatal as xxx

		CAM_DBG(CAM_CRM, "Failed for device: %s while applying request: %lld",
				dev->dev_info.name, link->req.apply_data[result_idx][pd].req_id);

		cam_req_mgr_notify_message(&msg,
				V4L_EVENT_CAM_REQ_MGR_ERROR,
				V4L_EVENT_CAM_REQ_MGR_EVENT);} else if (rc < 0) {
#else
		if (rc < 0)	{
#endif
...
#if IS_ENABLED(CONFIG_ISPV3)
	/* check and set the ispv3 key registers stats on this link */
	if (add_req->ispv3_key_reg_bad_state) {
		link->ispv3_key_reg_bad_state = TRUE;
		CAM_ERR(CAM_CRM, "link handle %d , ispv3 key reg status %d", link->link_hdl, link->ispv3_key_reg_bad_state);
	}
#endif
