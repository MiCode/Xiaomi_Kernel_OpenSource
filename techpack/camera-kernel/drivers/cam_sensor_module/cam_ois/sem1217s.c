...
int32_t i2c_read_data(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr, uint32_t length, uint8_t *data)
{
	int32_t rc = 0;
	int32_t i = 0;
...
int32_t i2c_write_data(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr, uint32_t length, uint8_t* data, uint32_t delay)
{
	...
	for (i = 0; i < length && i < 256; i+=4) {
		CAM_DBG(CAM_OIS, "[SEM1217S] Write addr 0x%04x = 0x%02x 0x%02x 0x%02x 0x%02x", w_data[i].reg_addr, data[i], data[i+1], data[i+2], data[i+3]);
	}

	return rc;
}

int32_t load_fw_buff(
