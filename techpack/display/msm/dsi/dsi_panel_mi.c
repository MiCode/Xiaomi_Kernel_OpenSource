...
static void dynamic_elvss_delayed_work(struct work_struct *work)
{
	struct dsi_panel_mi_cfg *mi_cfg = container_of(work,
				struct dsi_panel_mi_cfg, dynamic_elvss_delayed_work.work);
	...
	if (rc)
		pr_err("Failed to send DSI_CMD_SET_MI_DYNAMIC_ELVSS_ON command\n");
	else
		pr_debug("dynamic elvss on\n");
exit:
				...
				struct device_node *of_node)
{
	...
	if (mi_cfg->hbm_51_ctrl_flag) {
		...
		if (mi_cfg->dynamic_elvss_enabled) {
			...
			INIT_DELAYED_WORK(&mi_cfg->dynamic_elvss_delayed_work, dynamic_elvss_delayed_work);
			rc = utils->read_u32(of_node,
				"mi,mdss-dsi-panel-dynamic-elvss-on-delay", &mi_cfg->dynamic_elvss_on_delay);
			if (rc)
				mi_cfg->dynamic_elvss_on_delay = 150;
		}
	}
	...
	if (mi_cfg->fod_dimlayer_enabled) {
		...
		rc = utils->read_u32(of_node, "mi,mdss-panel-before-fod-hbm-on-60Hz-delay",
			&mi_cfg->before_fod_hbm_on_60Hz_delay);
		if (rc) {
			mi_cfg->before_fod_hbm_on_60Hz_delay = 0;
		}
		pr_info("before_fod_hbm_on_60Hz_delay=%d\n", mi_cfg->before_fod_hbm_on_60Hz_delay);
	}
...
int dsi_panel_update_greenish_gamma_setting(struct dsi_panel *panel)
{
	...
	if (rc) {
		...
		for (i = 0; i <= 15; i++) {
			...
			pr_info("DSI_CMD_SET_MI_GAMMA_B7 %d line tx_buf %x %x %x\n", i,
			...
			int doze_brightness, bool need_panel_lock)
{
	...
	if (mi_cfg->in_aod) {
		if (mi_cfg->dynamic_elvss_enabled && doze_brightness)
			cancel_delayed_work(&mi_cfg->dynamic_elvss_delayed_work);
