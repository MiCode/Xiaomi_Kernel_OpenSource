...
exit:
	...
	if (mi_cfg->dynamic_elvss_enabled) {
		schedule_delayed_work(&mi_cfg->dynamic_elvss_delayed_work,
				msecs_to_jiffies(mi_cfg->dynamic_elvss_on_delay));
	}
...
error:
	...
	if (panel->mi_cfg.gamma_update_flag && !panel->mi_cfg.gamma_cfg.read_done) {
