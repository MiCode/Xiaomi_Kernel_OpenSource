...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	notifier_data.refresh_rate = 120;
	notifier_data.id = 1;
	pr_info("[%s]: dsi panel power on\n", __func__);
...
int dsi_panel_enable(struct dsi_panel *panel)
{
	...
	pr_info("[%s]: dsi panel send DSI_CMD_SET_ON\n", __func__);
	if (panel->cur_mode->timing.refresh_rate == 120) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DISP_BC_120HZ);
		if (rc)
			pr_err("[%s][%s] failed to send DSI_CMD_SET_DISP_BC_120HZ cmd, rc=%d\n",
			 ...
			 struct dsi_display_mode *adj_mode)
{
	...
	if (adj_mode->timing.refresh_rate == 120) {
		...
		if (rc)
			pr_err("[%s][%s] failed to send DSI_CMD_SET_DISP_BC_60HZ cmd, rc=%d\n",
					__func__, panel->name, rc);
		else {
			...
			bool enable)
{
	...
	if (enable) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DISP_DIMMING_ON);
		if (rc)
			pr_err("[%s][%s] failed to send DSI_CMD_SET_DISP_DIMMING_ON cmd, rc=%d\n",
					__func__, panel->name, rc);
		else
			panel->bl_config.dimming_enable = true;
	} else {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DISP_DIMMING_OFF);
		if (rc)
			pr_err("[%s][%s] failed to send DSI_CMD_SET_DISP_DIMMING_Off cmd, rc=%d\n",
					__func__, panel->name, rc);
		else
