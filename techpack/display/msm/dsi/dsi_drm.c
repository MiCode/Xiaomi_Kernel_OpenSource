...
static void dsi_bridge_pre_enable(struct drm_bridge *bridge)
{
	...
	atomic_set(&c_bridge->display->panel->esd_recovery_pending, 0);
  	if (atomic_read(&prim_panel_is_on)) {
		...
		pr_err("skip set display config for video panel in fpc\n");
		return;
	}
...
int dsi_bridge_interface_enable(int timeout)
{
	...
	pr_info("[%s]: panel early resume by fp\n", __func__);
	if (timeout > 0)
...
static void dsi_bridge_enable(struct drm_bridge *bridge)
{
	...
	//		(c_bridge->display->panel->dsi_refresh_flag == 120)) {
...
static void dsi_bridge_post_disable(struct drm_bridge *bridge)
{
	...
	SDE_ATRACE_END("dsi_bridge_post_disable");
	drm_notifier_call_chain(DRM_EVENT_BLANK, &g_notify_data);
	pr_err("%s: prim_panel_is_on false\n", __func__);
