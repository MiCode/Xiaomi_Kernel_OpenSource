	...
	if (hbm_overlay) {
		...
		if (dsi_display->panel->mi_cfg.fod_hbm_layer_enabled == false) {
			...
			if (skip == false) {
				...
				if (dsi_display->panel->mi_cfg.before_fod_hbm_on_60Hz_delay != 0
					&& dsi_display->panel->cur_mode->timing.refresh_rate == 60) {

					usleep_range(dsi_display->panel->mi_cfg.before_fod_hbm_on_60Hz_delay,
						dsi_display->panel->mi_cfg.before_fod_hbm_on_60Hz_delay + 10);
				}

				sde_connector_hbm_ctl(connector, DISPPARAM_HBM_FOD_ON);
				if (dsi_display->panel->mi_cfg.delay_after_fod_hbm_on)
					sde_encoder_wait_for_event(c_conn->encoder, MSM_ENC_VBLANK);
...
int sde_connector_pre_hbm_ctl(struct drm_connector *connector)
{
	int ret;
	/* close dimming */
	ret = dsi_display_hbm_set_disp_param(connector, DISPPARAM_DIMMING_OFF);
	return ret;
}
