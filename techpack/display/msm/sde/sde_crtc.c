		...
		list_for_each_entry(encoder, &dev->mode_config.encoder_list, head) {
			...
			if (sde_encoder_is_dsi_display(encoder)) {
				...
				if (c_bridge && c_bridge->display && c_bridge->display->panel) {
					...
					if (panel->mi_cfg.idle_time && panel->mi_cfg.idle_fps
							&& panel->cur_mode
							&& (panel->cur_mode->timing.refresh_rate !=
								panel->mi_cfg.idle_fps)) {
						idle_time = panel->mi_cfg.idle_time;
...
{
	int ret = 0, exit_idle = 1;
	struct drm_event event, idle_event;
	...
	if (gcrtc) {
		...
		if (dsi_display && dsi_display->is_prim_display && dsi_display->panel) {
			...
			if (!dsi_display->panel->mi_cfg.panel_max_frame_rate) {
