	...
	struct cam_isp_hw_done_event_data *done_next_req)
{
	...
	for (i = 0; i < done->num_handles; i++) {
		...
		if (!req_isp->bubble_detected) {
			// QC: Signal fence for early bufdones got during reapply of a bubble req.
			if(req_isp->num_acked != 0 && req_isp->early_bufdone_flag == false) {
				for (i = 0; i < req_isp->num_fence_map_out; i++) {
					if (req_isp->sync_enabled_id[i] == 1){
						rc = cam_sync_signal(req_isp->fence_map_out[i].sync_id,
							CAM_SYNC_STATE_SIGNALED_SUCCESS,
							CAM_SYNC_COMMON_EVENT_SUCCESS);
						req_isp->sync_enabled_id[i] = 0;
						req_isp->fence_map_out[i].sync_id = -1;
						CAM_DBG(CAM_ISP,
							"Sync early bufdones:req %lld res 0x%x fd 0x%x, ctx %u",
							req->request_id,
							req_isp->fence_map_out[j].resource_handle,
							req_isp->fence_map_out[j].sync_id,ctx->ctx_id);
						req_isp->early_bufdone_flag = true;
					}
				}
			}
			...
			CAM_DBG(CAM_ISP,
				...
				bubble_state, req_isp->bubble_report, req_isp->num_acked, ctx->ctx_id);
			continue;
		}
	...
	uint32_t status, uint32_t event_cause)
{
	...
	for (i = 0; i < req_isp->num_deferred_acks; i++) {
		...
		CAM_DBG(CAM_ISP, "ctx %d num_acked %d req %lld",
			ctx->ctx_id, req_isp->num_acked, req->request_id);
	}
	...
	bool defer_buf_done)
{
	...
	for (i = 0; i < done->num_handles; i++) {
		...
		if (defer_buf_done) {
			...
			req_isp->num_acked++;
			CAM_DBG(CAM_ISP,
				"num_acked %d buf done with bubble state %d recovery %d",
				req_isp->num_acked, bubble_state, req_isp->bubble_report);
				/* Process deferred buf_done acks */
	...
	uint32_t bubble_state)
{
	...
	if (list_empty(&ctx->active_req_list)) {
		...
		if (!list_empty(&ctx->wait_req_list)) {
			...
			req_isp = (struct cam_isp_ctx_req *) req->req_priv;
			CAM_DBG(CAM_ISP, "done->num_handles %d", done->num_handles);
			...
			for (i = 0; i < done->num_handles; i++) {
				for (j = 0; j < req_isp->num_fence_map_out; j++) {
					CAM_DBG(CAM_ISP, "req %lld done 0x%x res 0x%x"
						"ctx %u num_acked %d"
						"num_fence_map_out %d",
						req->request_id,
						done->resource_handle[i],
					...
					if (done->resource_handle[i] ==
						req_isp->fence_map_out[j].resource_handle) {
						req_isp->num_acked++;
						// QC: save the fence map out index for signalling
						// fence during re-apply.
						req_isp->sync_enabled_id[j]++;
						CAM_INFO(CAM_ISP,
							"ctx %u num_acked %d, index %d",
							ctx->ctx_id,
							req_isp->num_acked, i);
						break;
					}
	...
	void *evt_data)
{
	...
	// QC: Before moving request to active list, reset sync_enabled_id
	//   so that we dont signal early bufdones for request for which
	//   bubble is not detected yet, but eventually result in bubble.
	for(i=0; i < req_isp->num_fence_map_out; i++) {
		req_isp->sync_enabled_id[i] = 0;
	}
	...
	enum cam_isp_ctx_activated_substate next_state)
{
	...
	// QC :reset sync_enable_id before appply for early bufdones
	for(i = 0; i <  req_isp->num_fence_map_out; i++) {
		req_isp->sync_enabled_id[i] = 0;
	}
	req_isp->early_bufdone_flag = false;
	...
	struct cam_context *ctx, struct cam_req_mgr_apply_request *apply)
{
	...
	if (rc)
		CAM_WARN(CAM_ISP, "Apply failed in Substate[%s], rc %d",
			__cam_isp_ctx_substate_val_to_type(
			ctx_isp->substate_activated), rc);
	...
	struct cam_context *ctx, struct cam_config_dev_cmd *cmd)
{
	...
	CAM_DBG(CAM_ISP, "req Id %lld num_entry: %d, num fence out: %d, num fence in: %d for ctx %d",
		req->request_id, req_isp->num_cfg, req_isp->num_fence_map_out,
		req_isp->num_fence_map_in,ctx->ctx_id);
	...
	uint32_t evt_id, void *evt_data)
{
	...
	CAM_DBG(CAM_ISP, "Enter: State %d, Substate[%s], evt id %d ctx id %d",
