...
extern struct mutex custom_gpio2_powernum_mutex;
int custom_gpio2_powernum;
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_CUSTOM_GPIO2:
			if (no_gpio) {
				CAM_ERR(CAM_SENSOR, "request gpio failed,but still do power up");
			}
			if (!gpio_num_info) {
			...
			if(power_setting->seq_type == SENSOR_CUSTOM_GPIO2){
			    CAM_INFO(CAM_SENSOR,
			        "custom_gpio2_powernum init: %d",custom_gpio2_powernum);
			    mutex_lock(&custom_gpio2_powernum_mutex);
			    custom_gpio2_powernum++;
			    mutex_unlock(&custom_gpio2_powernum_mutex);
			    CAM_INFO(CAM_SENSOR,
			        "custom_gpio2_powernum has been ++ : %d",custom_gpio2_powernum);
			}
	...
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		...
		case SENSOR_CUSTOM_GPIO2:
			...
			if (!gpio_num_info->valid[pd->seq_type])
				continue;
			if(pd->seq_type == SENSOR_CUSTOM_GPIO2){
			    CAM_INFO(CAM_SENSOR,
			        "powerdown custom_gpio2_powernum begin: %d",custom_gpio2_powernum);
			    mutex_lock(&custom_gpio2_powernum_mutex);
			    custom_gpio2_powernum--;
			    mutex_unlock(&custom_gpio2_powernum_mutex);
			    CAM_INFO(CAM_SENSOR,
			        "powerdown custom_gpio2_powernum has been -- : %d",custom_gpio2_powernum);
			    if(custom_gpio2_powernum){
			        CAM_INFO(CAM_SENSOR,
			            "powerdown custom_gpio2_powernum > 0 is :%d do not down, continue",custom_gpio2_powernum);
			        continue;
			    }
			}
			cam_res_mgr_gpio_set_value(
