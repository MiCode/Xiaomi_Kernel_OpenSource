...
void RamWrite32A(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr, uint32_t data, uint32_t delay)
{
	...
	CAM_ERR(CAM_OIS, "[OISDEBUG] write addr 0x%08x = 0x%08x", addr, data);
}
...
//	CAM_ERR(CAM_OIS, "OIS %s :  write addr 0x%08x = 0x%08x", o_ctrl->ois_name, addr, data);
}
...
void RamRead32A(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr, uint32_t* data)
{
	...
	CAM_ERR(CAM_OIS, "[OISDEBUG] read addr 0x%04x = 0x%08x", addr, *data);
}
...
void CntWrt(struct cam_ois_ctrl_t *o_ctrl, uint8_t *data, uint32_t length, uint32_t delay)
{
	...
	for (i = 0; i < (length -2) && i < 256; i+=4) {
		CAM_ERR(CAM_OIS, "[OISDEBUG] write addr 0x%04x = 0x%02x 0x%02x 0x%02x 0x%02x", addr, data[i+2], data[i+3], data[i+4], data[i+5]);
	}
}
...
void CntRd(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr, uint8_t *data, uint32_t length)
{
	...
	rc = camera_io_dev_read_seq(&o_ctrl->io_master_info,
		...
		length);
	for (i = 0; i < length; i++) {
		CAM_ERR(CAM_OIS, "[OISDEBUG] read addr 0x%04x[%d] = 0x%02x", addr, i, data[i]);
	}
	if (rc) {
...
uint8_t FlashMultiRead(struct cam_ois_ctrl_t *o_ctrl, uint8_t SelMat, uint32_t UlAddress, uint32_t *PulData , uint8_t UcLength)
{
	...
	for( i=0 ; i < UcLength ; i++ ){
		...
		CAM_ERR(CAM_OIS, "Read Data[%d] = 0x%08x", i, PulData[i]);
	}
	IOWrite32A(o_ctrl, FLASHROM_FLAMODE, 0x00000002, 0);
...
//		return 10;	// USR MAT,INF_MAT2もAccessしない
	...
	IOWrite32A(o_ctrl, FLASHROM_FLA_ADR, ((uint32_t)SelMat << 16) | ( SetAddress & 0x000010, 0), 0);
	// page write Start
	IOWrite32A(o_ctrl, FLASHROM_FLAMODE , 0x00000000, 0);
...
//	msleep(5);
	...
	for (i = 0; i < 16; i++) {
		...
		CAM_ERR(CAM_OIS, "Write Data[%d] = %08x", i , PulData[i] );
	}
	do {
		if (UlCnt++ > 100) {
			ans = 2;
...
uint32_t CheckFwValid(struct cam_ois_ctrl_t *o_ctrl, uint8_t CurrentFwVersion)
{
	...
	CAM_ERR(CAM_OIS, "is_fw_valid %d, fw_version 0x%x", is_fw_valid, fw_version);
...
uint32_t DrvOffAdj(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_ERR(CAM_OIS, "[OISDEBUG] CheckDrvOffAdj result %d", ans);
	if (ans == 1) {
		CAM_ERR(CAM_OIS, "[OISDEBUG] ready to update prog fw");
...
uint8_t CoreResetwithoutMC128(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_ERR(CAM_OIS, "[OISDEBUG] read from addr 0x%04x =  0x%x", ROMINFO, UlReadVal);
...
//--------------------------------------------------------------------------------
	...
	do {
		...
		if(UlCnt++ > 10) {
			IOWrite32A(o_ctrl, FLASHROM_FLAMODE , 0x00000002, 0);
			CAM_ERR(CAM_OIS, "[OISDEBUG] CheckSum loop timeout");
	...
	for(i = 0;i < 8; i++) {
		CAM_ERR(CAM_OIS, "ReadData[%d] = 0x%x, fw_data[%d] = 0x%x",
	...
	//	RamWrite32A(o_ctrl, 0xF00A, 0x00000000, 0);	// FromCmd.Addrの設定
		...
		for(i = 1;i < (total_bytes / BURST_LENGTH_FC) ; i++)
		{
			...
			if (UcOddEvn == 0)
				...
				data[1] = 0x09;
			UlNum = 2;
			CAM_ERR(CAM_OIS, "[OISDEBUG] step 1 write mem at %d", i);
		...
		CAM_ERR(CAM_OIS, "[OISDEBUG] step 1 done");
		...
		for (i = 0; i < Remainder; i++)
		{
			...
			if (UcOddEvn == 0)
				...
				data[1] = 0x09;
			UlNum = 2;
			CAM_ERR(CAM_OIS, "[OISDEBUG] step 2 write mem at %d", i);
			for(j=0 ; j < BURST_LENGTH_FC; j++) {
				data[UlNum++] = *NcFromVal++;
			}
			UlCnt = 0;
		...
		CAM_ERR(CAM_OIS, "[OISDEBUG] step 2 done");
		UlCnt = 0;
		do {
	//		msleep(4);
			msleep(1);
	...
	if (rc) {
		...
		for (i = 1; i< (total_bytes / 64); i++)
		{
			...
			if (UcOddEvn == 0)
				...
				data[1] = 0x09;
			CAM_ERR(CAM_OIS, "[%d] UcOddEvn=%d, data[1]= %d", i, data[1], NcFromVal);
		...
		{	/* write magic code */
			...
			data[1] = 0x08;
//			data[1] = 0x09;
			CAM_ERR(CAM_OIS, "[%d]UcOddEvn= %d, data[1]= %d", 0, data[1], NcFromVal1st );
...
uint8_t MatVerify(struct cam_ois_ctrl_t *o_ctrl, uint32_t FwChecksum, uint32_t FwChecksumSize)
{
	...
	CAM_ERR(CAM_OIS, "UlReadVal = %x", UlReadVal);
	if (UlReadVal != 0x0A)
		return( 0x53 );
	...
	if (rc) {
		...
		for (i = 0; i < total_bytes;) {
			for (cnt = 0; cnt < write_stride && i < total_bytes;
				...
				CAM_ERR(CAM_OIS, "[OISDEBUG] prog byte[%d] addr 0x%04x = 0x%04x", i, fw_addr, *ptr);
			}
