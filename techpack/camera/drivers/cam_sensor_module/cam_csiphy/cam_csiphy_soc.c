...
#ifdef CONFIG_MI7250_CAMERA
        	//Add by xiaomi baichunyu@xiaomi.com
        	if ( (get_hw_version_major() == 9) || (get_hw_version_major() == 0x29) ) {
			csiphy_dev->ctrl_reg->csiphy_2ph_combo_mode_reg =
                	csiphy_2ph_v1_2_combo_mode_reg_xiaomi;
			CAM_INFO(CAM_CSIPHY, "hw_version_major= %d ,qcom combo patch merge.",
                                get_hw_version_major());
        	}else{
			CAM_INFO(CAM_CSIPHY, "hw_version_major= %d ,qcom combo patch not merge.",
                                get_hw_version_major());
        	}
        	//Add by xiaomi baichunyu@xiaomi.com
#endif
