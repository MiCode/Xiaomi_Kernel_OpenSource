...
/* add for wingtec hardware id set begin*/
			wingtec_set_camera_hardware_info(memptr);
/* add for wingtec hardware id set end*/
...
/* add for wingtec camera hardware info set begin*/
int wingtec_set_camera_hardware_info(uint8_t *eeprom_data)
{
        char sensor_name[HARDWARE_MAX_ITEM_LONGTH];
        char sensor_vendor[HARDWARE_MAX_ITEM_LONGTH];
        char pstr_temp[HARDWARE_MAX_ITEM_LONGTH];
        char raw_str[] = "_mipi_raw_";

        memset(sensor_name, 0, sizeof(sensor_name));
        memset(sensor_vendor, 0, sizeof(sensor_vendor));
        memset(pstr_temp, 0, sizeof(pstr_temp));
        if(!eeprom_data)
        {
                CAM_ERR(CAM_EEPROM,"eeprom data is NULL!");
                return -1;
        }
        //read sensor id to distinguish sensor name
        switch (eeprom_data[12])
        {
                case 0x52:// main s5khm2
                        sprintf(sensor_name, "%s" , "s5khm2");
                        if (eeprom_data[1] == 0x01)
                        {
                                sprintf(sensor_vendor, "%s" , "sunny");
                                sprintf(pstr_temp, "%s%s%s%s", sensor_name, sensor_vendor, raw_str, "i");
                        }
                        else if (eeprom_data[1] == 0x10)
                        {
                                sprintf(sensor_vendor, "%s" , "aac");
                                sprintf(pstr_temp, "%s%s%s%s", sensor_name, sensor_vendor, raw_str, "ii");
                        }
#ifdef CONFIG_WT_QGKI
                        hardwareinfo_set_prop(HARDWARE_BACK_CAM, pstr_temp);
                        hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID, sensor_vendor);
#endif
                        CAM_DBG(CAM_EEPROM,"sensor_name %s vendor %s",pstr_temp,sensor_vendor);
                        break;
                case 0x46:// front ov16a1q
                        sprintf(sensor_name, "%s" , "ov16a1q");
                        if (eeprom_data[1] == 0x01)
                        {
                                sprintf(sensor_vendor, "%s" , "sunny");
                                sprintf(pstr_temp, "%s%s%s%s", sensor_name, sensor_vendor, raw_str, "i");
...
#ifdef CONFIG_WT_QGKI
                        hardwareinfo_set_prop(HARDWARE_FRONT_CAM, pstr_temp);
                        hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, sensor_vendor);
#endif
                        CAM_DBG(CAM_EEPROM,"sensor_name %s vendor %s",pstr_temp,sensor_vendor);
                        break;
                case 0x58:// ultra imx355
                        sprintf(sensor_name, "%s" , "imx355");
                        if (eeprom_data[1] == 0x06)
                        {
                                sprintf(sensor_vendor, "%s" , "qtech");
                                sprintf(pstr_temp, "%s%s%s%s", sensor_name, sensor_vendor, raw_str, "ii");
                        }
                        else if (eeprom_data[1] == 0x10)
                        {
...
#ifdef CONFIG_WT_QGKI
                        hardwareinfo_set_prop(HARDWARE_BACK_WIDE_CAM, pstr_temp);
                        hardwareinfo_set_prop(HARDWARE_BACK_WIDE_CAM_MOUDULE_ID, sensor_vendor);
#endif
                        CAM_DBG(CAM_EEPROM,"sensor_name %s vendor %s",pstr_temp,sensor_vendor);
                        break;
                case 0x44:// macro gc02m1
                        sprintf(sensor_name, "%s" , "gc02m1");
                        if (eeprom_data[1] == 0x07)
                        {
                                sprintf(sensor_vendor, "%s" , "ofilm");
                                sprintf(pstr_temp, "%s%s%s%s", sensor_name, sensor_vendor, raw_str, "i");
                        }
                        else if (eeprom_data[1] == 0x10)
                        {
...
#ifdef CONFIG_WT_QGKI
                        hardwareinfo_set_prop(HARDWARE_BACK_MACRO_CAM, pstr_temp);
                        hardwareinfo_set_prop(HARDWARE_BACK_MACRO_CAM_MOUDULE_ID, sensor_vendor);
#endif
                        CAM_DBG(CAM_EEPROM,"sensor_name %s vendor %s",pstr_temp,sensor_vendor);
                        break;
                case 0x57:// main s5kgw3
                        sprintf(sensor_name, "%s" , "s5kgw3");
                        if (eeprom_data[1] == 0x07)
                        {
                                sprintf(sensor_vendor, "%s" , "ofilm");
                                sprintf(pstr_temp, "%s%s%s%s", sensor_name, sensor_vendor, raw_str, "i");
                        ...
                        break;
                default:
                        CAM_ERR(CAM_EEPROM,"eeprom data read unknown id 0x%x",eeprom_data[12]);
                        break;
        }
        return 0;
}
/* add for wingtec camera hardware info set end*/
