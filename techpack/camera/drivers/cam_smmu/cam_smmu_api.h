 ...
 * @param discard_iova_end   : iova addr end upto where should not be used
 */
struct cam_smmu_region_info {
	dma_addr_t iova_start;
	size_t iova_len;
	dma_addr_t discard_iova_start;
	dma_addr_t discard_iova_end;
};
 ...
 * @param discard_iova_end: End address of io space to discard
 *
 * @return Status of operation. Negative in case of error. Zero otherwise.
 */
int cam_smmu_get_io_region_info(int32_t smmu_hdl,
	dma_addr_t *iova, size_t *len,
	dma_addr_t *discard_iova_start, dma_addr_t *discard_iova_end);
