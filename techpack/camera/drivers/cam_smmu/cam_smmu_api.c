	...
	dma_addr_t *discard_iova_start, dma_addr_t *discard_iova_end)
{
	...
	if (!iova || !len || !discard_iova_start || !discard_iova_end ||
	...
	*discard_iova_end =
		iommu_cb_set.cb_info[idx].io_info.discard_iova_end;

	CAM_DBG(CAM_SMMU,
		"I/O area for hdl = %x start addr = %pK len = %zu discard [%pK %pK]",
		smmu_hdl, *iova, *len,
		*discard_iova_start, *discard_iova_end);
	mutex_unlock(&iommu_cb_set.cb_info[idx].lock);
	...
	bool dis_delayed_unmap, struct cam_dma_buff_info **mapping_info)
{
	...
	if (region_id == CAM_SMMU_REGION_SHARED) {
		...
		*len_ptr = (size_t)buf->size;
		...
		/* This check can be avoided once tested throroughly */
		if (iommu_cb_set.cb_info[idx].discard_iova_start) {
			dma_addr_t start_addr = *paddr_ptr;
			dma_addr_t end_addr = *paddr_ptr + *len_ptr;
			dma_addr_t discard_iova_start =
				iommu_cb_set.cb_info[idx].discard_iova_start;
			dma_addr_t discard_iova_end =
				iommu_cb_set.cb_info[idx].discard_iova_end;

			if (((start_addr >= discard_iova_start) &&
				(start_addr < discard_iova_end)) ||
				((end_addr > discard_iova_start) &&
				(end_addr < discard_iova_end)) ||
				((start_addr < discard_iova_start) &&
				(end_addr > discard_iova_end))) {
				CAM_ERR(CAM_SMMU,
					"[%s] : Incorrect map [0x%x 0x%x] inside [0x%x 0x%x]",
					iommu_cb_set.cb_info[idx].name,
					start_addr, end_addr,
					discard_iova_start, discard_iova_end);
				rc = -EINVAL;
	...
	dma_addr_t *discard_iova_start, dma_addr_t *discard_iova_end)
{
	...
	if (!discard_iova_start || !discard_iova_end)
		return -EINVAL;
	...
	*discard_iova_end = 0;
	...
	CAM_INFO(CAM_UTIL, "Discard region [0x%x 0x%x]",
		discard_iova[0], discard_iova[1]);

	*discard_iova_start = discard_iova[0];
	*discard_iova_end = discard_iova[1];
	...
	struct cam_context_bank_info *cb)
{
	...
	if (cb->io_support) {
		...
		if ((cb->discard_iova_start !=
			...
			of_node_put(mem_map_node);
			return -EINVAL;
		} else if (cb->discard_iova_start && cb->discard_iova_end) {
			if ((cb->discard_iova_start <=
