...
$(TARGET_FACTORY_BUILD_CONFIG): \
	$(info $(shell if [ $(FACTORY_BUILD) = 1 ]; then \
		...
		echo "FACTORY_BUILD: add CONFIG_MTK_ULTRASND_PROXIMITY=y to $(KERNEL_CONFIG_FILE)"; \
		echo "CONFIG_MTK_ULTRASND_PROXIMITY=y" >> $(KERNEL_CONFIG_FILE); \
		...
		echo "Not FACTORY_BUILD: remove CONFIG_MTK_ULTRASND_PROXIMITY=y from $(KERNEL_CONFIG_FILE)"; \
		sed -i '/CONFIG_MTK_ULTRASND_PROXIMITY=y/d' $(KERNEL_CONFIG_FILE); \
	fi))
...
$(INSTALLED_KERNEL_TARGET): check-mtk-config
