...
static int trace_start(struct tracectx *t)
{
	...
	clk_set_parent(t->coresight_clk, t->pll_p);
	clk_set_rate(t->coresight_clk, 144000000);
...
static void ptm_dump(void)
{
	...
	for (; length; length--)
		printk(KERN_CONT "%08x",
				cpu_to_be32(etb_readl(t, ETB_READMEM)));
	printk(KERN_INFO "\n--- ETB buffer end ---\n");
...
#define clk_readl(reg)  __raw_readl((u32)reg_clk_base + (reg))
#define clk_writel(value, reg) __raw_writel(value, (u32)reg_clk_base + (reg))
...
static int funnel_and_tpiu_init(struct tracectx *t)
{
	...
	funnel_writel(t, FUNNEL_MINIMUM_HOLD_TIME(0) |
			...
			FUNNEL_CTRL);
	/*
	 * Assign CPU0-3 funnel priority, we give CPU0 highest priority
	 * because TEGRA use CPU0 for IRQ handling only.
	 */
...
static int etb_init(struct tracectx *t)
{
	...
	t->last_etb = devm_kzalloc(t->dev, t->etb_depth * sizeof(*t->last_etb),
					GFP_KERNEL);
	if (NULL == t->last_etb) {
		dev_dbg(t->dev, "failes to allocate memory to hold ETB\n");
		return -ENOMEM;
	}
...
static void tegra_ptm_enter_resume(void)
{
	...
	if (trace_isrunning(&tracer)) {
		/*
		 * On Tegra, LP0 will cut off VDD_CORE and only in that case
		 * we need initialized the TPIU and FUNNEL.
		 */
...
void tegra_ptm_enter_stop(void)
{
	if (trace_isrunning(&tracer)) {
		/*
		 * On Tegra, LP0 will cut off VDD_CORE and only in that case
...
static int __devinit ptm_probe(struct platform_device *dev)
{
	struct tracectx *t = &tracer;
	int ret = 0;
	int i;
	...
	for (i = 0; i < dev->num_resources; i++) {
		...
		if (NULL == res)
			goto out;
		tmp = devm_ioremap_nocache(&dev->dev, res->start,
				resource_size(res));
		if (NULL == tmp)
			goto out;

		if (0 == strncmp("ptm", res->name, 3))
			t->ptm_regs[t->ptm_regs_count++] = tmp;
...
static int __init ptm_init(void)
{
	...
	if (retval) {
		printk(KERN_ERR "Failed to probe ptm\n");
		return retval;
	}
