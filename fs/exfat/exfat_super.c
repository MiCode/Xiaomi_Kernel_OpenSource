...
static int exfat_fill_inode(struct inode *inode, FILE_ID_T *fid)
{
	...
	exfat_time_fat2unix(sbi, (struct timespec_compat *)&inode->i_mtime, &info.ModifyTimestamp);
	exfat_time_fat2unix(sbi, (struct timespec_compat *)&inode->i_ctime, &info.CreateTimestamp);
	exfat_time_fat2unix(sbi, (struct timespec_compat *)&inode->i_atime, &info.AccessTimestamp);
...
#endif
{
	...
	exfat_time_unix2fat(sbi, (struct timespec_compat *)&inode->i_mtime, &info.ModifyTimestamp);
	exfat_time_unix2fat(sbi, (struct timespec_compat *)&inode->i_ctime, &info.CreateTimestamp);
	exfat_time_unix2fat(sbi, (struct timespec_compat *)&inode->i_atime, &info.AccessTimestamp);
...
static int exfat_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (sbi->fs_info.vol_type != EXFAT) {
		error = -EINVAL;
		//snprintf(buf, "cp%d", sbi->options.codepage);
