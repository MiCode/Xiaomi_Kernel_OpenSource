...
s32 ffsMountVol(struct super_block *sb)
{
	...
	pr_debug("[EXFAT] trying to mount...\n");
	...
	pr_debug("[EXFAT] mounted successfully\n");
...
s32 ffsUmountVol(struct super_block *sb)
{
	...
	pr_debug("[EXFAT] trying to unmount...\n");
	...
	if (p_fs->dev_ejected) {
		pr_debug("[EXFAT] unmounted with media errors. "
			"device's already ejected.\n");
		return FFS_MEDIAERR;
	}

	pr_debug("[EXFAT] unmounted successfully\n");
...
s32 ffsTruncateFile(struct inode *inode, u64 old_size, u64 new_size)
{
	...
	if (fid->size != old_size) {
		pr_err("[EXFAT] truncate : can't skip it because of "
...
void exfat_free_cluster(struct super_block *sb, CHAIN_T *p_chain, s32 do_relse)
{
	...
	if (p_chain->size <= 0) {
		pr_err("[EXFAT] free_cluster : skip free-req clu:%u, "
...
#endif
		if (ret == -EOPNOTSUPP) {
			pr_debug("discard not supported by device, disabling");
			opts->discard = 0;
		}
	}
#endif /* CONFIG_EXFAT_DISCARD */
...
s32 sector_read(struct super_block *sb, sector_t sec, struct buffer_head **bh, s32 read)
{
	...
	if ((sec >= (p_fs->PBR_sector+p_fs->num_sectors)) && (p_fs->num_sectors > 0)) {
		pr_debug("[EXFAT] sector_read: out of range error! (sec = %llu)\n", (unsigned long long)sec);
		fs_error(sb);
		return ret;
	}
...
s32 sector_write(struct super_block *sb, sector_t sec, struct buffer_head *bh, s32 sync)
{
	...
	if (sec >= (p_fs->PBR_sector+p_fs->num_sectors) && (p_fs->num_sectors > 0)) {
		pr_debug("[EXFAT] sector_write: out of range error! (sec = %llu)\n", (unsigned long long)sec);
		fs_error(sb);
		return ret;
	}

	if (bh == NULL) {
		pr_debug("[EXFAT] sector_write: bh is NULL!\n");
		fs_error(sb);
		return ret;
	}
...
s32 multi_sector_read(struct super_block *sb, sector_t sec, struct buffer_head **bh, s32 num_secs, s32 read)
{
	...
	if (((sec+num_secs) > (p_fs->PBR_sector+p_fs->num_sectors)) && (p_fs->num_sectors > 0)) {
		pr_debug("[EXFAT] multi_sector_read: out of range error! (sec = %llu, num_secs = %d)\n",
...
s32 multi_sector_write(struct super_block *sb, sector_t sec, struct buffer_head *bh, s32 num_secs, s32 sync)
{
	...
	if ((sec+num_secs) > (p_fs->PBR_sector+p_fs->num_sectors) && (p_fs->num_sectors > 0)) {
		pr_debug("[EXFAT] multi_sector_write: out of range error! (sec = %llu, num_secs = %d)\n",
		       (unsigned long long)sec, num_secs);
		fs_error(sb);
		return ret;
	}
	if (bh == NULL) {
		pr_debug("[EXFAT] multi_sector_write: bh is NULL!\n");
		fs_error(sb);
		return ret;
	}
