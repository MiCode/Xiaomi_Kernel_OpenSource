...
int yaffs_check_ff(u8 *buffer, int n_bytes)
{
	...
	if ((((unsigned long)buffer)&0x3) || (n_bytes&0x3)) {
		...
		while (n_bytes--) {
			if (*buffer != 0xFF) {
				if (ret)
					pr_err("yaffsdebug checkFF %.4x:%.2x\n", (int)(buffer - (u8 *)tmp), *buffer);
				else
					pr_err("%.4x:%.2x\n", (int)(buffer - (u8 *)tmp), *buffer);
				ret = 0;
		...
		while (n_bytes--) {
			if (*tmp != 0xFFFFFFFF) {
				if (ret) {
					pr_err("yaffsdebug checkFF - %.4x:%.8x\n",
...
void mtk_dump_word(void *p, int count, unsigned int offset, int notskipseq)
{
	...
	if (((unsigned long)p%4) || (count%4)) {
		pr_err("mtk_dump_word error p:%p count:%d\n", p, count);
		return;
	}
...
static int yaffs_check_chunk_erased(struct yaffs_dev *dev, int nand_chunk)
{
	...
	if (!yaffs_check_ff(data, dev->data_bytes_per_chunk) ||
		...
		retval = YAFFS_FAIL;
/*power loss log*/
		pr_err("yaffsdebug write CheckChunk chunk:%d addr:0x%x chunkUsed:%d\n",
...
int yaffs_check_alloc_available(struct yaffs_dev *dev, int n_chunks)
{
	...
	ret = (dev->n_free_chunks > (reserved_chunks + n_chunks));
	return ret;
}
	...
	do {
		...
		if (!bi->skip_erased_check) {
			...
			if (erased_ok != YAFFS_OK) {
				pr_err("yaffsdebug write chunknotclean chunk:%d addr:0x%x\n",
		...
		if (write_ok != YAFFS_OK) {
			pr_err("yaffsdebug write writechunk error chunk:%d\n", chunk);
			goto writeerror;
		}
		...
		if (!bi->skip_erased_check) {
			...
			if (write_ok != YAFFS_OK)
				pr_err("yaffsdebug write verifychunk error chunk:%d\n", chunk);
#if defined(YAFFS_MVG_TEST_DEBUG_WRITECHECK)
...
void yaffs_set_obj_name(struct yaffs_obj *obj, const YCHAR *name)
{
	...
	if (name && strnlen(name, YAFFS_SHORT_NAME_LENGTH + 1) <=
...
static int yaffs_gc_block(struct yaffs_dev *dev, int block, int whole_block)
{
	...
	buffer = yaffs_get_temp_buffer(dev, __LINE__);
		...
		if (yaffs_check_chunk_bit(dev, block, dev->gc_chunk)) {
			...
			if (object && !yaffs_skip_verification(dev)) {
				...
				if (old_chunk != matching_chunk)
					yaffs_trace(YAFFS_TRACE_ERROR,
							...
							old_chunk, matching_chunk, tags.obj_id,
							tags.chunk_id);
			...
			if (!object) {
				yaffs_trace(YAFFS_TRACE_ERROR,
						...
						old_chunk, tags.obj_id, tags.chunk_id,
						tags.n_bytes);
			}
...
void yaffs_link_fixup(struct yaffs_dev *dev, struct yaffs_obj *hard_list)
{
	...
	while (hard_list) {
		...
		if (in) {
			 ...
			 * Got a problem... hardlink to a non-existent object
			 */
