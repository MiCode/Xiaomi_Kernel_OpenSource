			     ...
			     u8 *data, struct yaffs_ext_tags *tags)
{
	...
	if (dev->param.inband_tags || (data && !tags))
		...
		if (retval) {
			pr_err("yaffsdebug mtdiferror %s retval:%d unfixed:%d fixed:%d chunk:%d addr:0x%llx\n",
				(retval == -EUCLEAN)?"fix ecc":"unfix ecc",
				retval, dev->n_ecc_unfixed, dev->n_ecc_fixed, nand_chunk, addr);
#ifdef YAFFS_MVG_TEST_DEBUG_LOG
			if (retval != -EUCLEAN) {
				pr_err("dump checksum_BCH chunk:%d addr:0x%x\n", nand_chunk, addr);
	...
	if (retval) {
		...
		*seq_number = 0;
		...
		if (t.ecc_result == YAFFS_ECC_RESULT_UNFIXED) {
			pr_err("yaffsdebug : unfixed ecc error when scan block, seqnum:0x%x\n", t.seq_number);
			*seq_number = 0;
			*state = YAFFS_BLOCK_STATE_EMPTY;
		}
	}
