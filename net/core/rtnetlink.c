...
{	if (rtnl_instance.flag) {
		...
		if (strncmp(rtnl_instance.task->comm, "RfxSender", 9) == 0) {
			if (rtnl_instance.count == 3) {
				ccci_md_force_assert(MD_SYS1, MD_FORCE_ASSERT_BY_AP_Q0_BLOCKED, "TX", 3);
			} else if (rtnl_instance.count == 5) {
...
void __rtnl_unlock(void)
{
	...
	if (rtnl_instance.end - rtnl_instance.start > 4000000000)
