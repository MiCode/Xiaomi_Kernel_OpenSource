...
drop:
	...
	printk(KERN_ERR "ADDLOG %s:%d dev:%s state:%lu  , dropped:%d,qlen:%u,id:0x%x", __func__, __LINE__, skb->dev->name, skb->dev->state, atomic_read(&skb->dev->rx_dropped), qlen, ip_hdr(skb)->id);
	kfree_skb(skb);
	return NET_RX_DROP;
}
...
skip_classify:
	if (pfmemalloc && !skb_pfmemalloc_protocol(skb)){
		printk(KERN_ERR "ADDLOG %s:%d dev:%s state:%lu  , dropped:%d", __func__, __LINE__, skb->dev->name, skb->dev->state, atomic_read(&skb->dev->rx_dropped));
		goto drop;
	}
