...
/**
 * struct mem_buf_lend_ioctl_arg2: An request to allocate memory from another
 * VM to other VMs.
 * @dma_buf_fd: The fd of the dma-buf that will be exported to another VM.
 * @nr_acl_entries: The number of ACL entries in @acl_list.
 * @acl_list: An array of structures, where each structure specifies a VMID
 ...
 * import IOCTL command with this argument.
 */
struct mem_buf_lend_ioctl_arg2 {
	__u32 dma_buf_fd;
	__u32 nr_acl_entries;
	__u64 acl_list;
	__u64 memparcel_hdl;
...
#define MEM_BUF_IOC_LEND2		_IOWR(MEM_BUF_IOC_MAGIC, 3,\
					      struct mem_buf_lend_ioctl_arg2)

#define MEM_BUF_VALID_FD_FLAGS (O_CLOEXEC | O_ACCMODE)
/**
 * struct mem_buf_retrieve_ioctl_arg: A request to import memory from another
 * VM as a dma-buf
 ...
 * import IOCTL command with this argument.
 */
struct mem_buf_retrieve_ioctl_arg2 {
	__u32 sender_vm_fd;
	__u32 nr_acl_entries;
	__u64 acl_list;
	__u64 memparcel_hdl;
...
#define MEM_BUF_IOC_RETRIEVE2		_IOWR(MEM_BUF_IOC_MAGIC, 4,\
					      struct mem_buf_retrieve_ioctl_arg2)
 ...
 * import IOCTL command with this argument.
 */
struct mem_buf_reclaim_ioctl_arg2 {
	__u64 memparcel_hdl;
	__u32 dma_buf_fd;
	__u32 reserved0;
	__u64 reserved1;
...
#define MEM_BUF_IOC_RECLAIM2		_IOWR(MEM_BUF_IOC_MAGIC, 3,\
					      struct mem_buf_reclaim_ioctl_arg2)
