...
/**
 * mitee_client_open_context() - Open a TEE context
 ...
 */
struct tee_context *
mitee_client_open_context(struct tee_context *start,
			int (*match)(struct tee_ioctl_version_data *,
				     const void *),
			const void *data, struct tee_ioctl_version_data *vers);

/**
 * mitee_client_close_context() - Close a TEE context
 * @ctx:	TEE context to close
 *
 * Note that all sessions previously opened with this context will be
 * closed when this function is called.
 */
void mitee_client_close_context(struct tee_context *ctx);

/**
 * mitee_client_get_version() - Query version of TEE
 * @ctx:	TEE context to TEE to query
 * @vers:	Pointer to version data
 */
void mitee_client_get_version(struct tee_context *ctx,
			    struct tee_ioctl_version_data *vers);

/**
 * mitee_client_open_session() - Open a session to a Trusted Application
 * @ctx:	TEE context
 ...
 */
int mitee_client_open_session(struct tee_context *ctx,
			    struct tee_ioctl_open_session_arg *arg,
			    struct tee_param *param);

/**
 * mitee_client_close_session() - Close a session to a Trusted Application
 * @ctx:	TEE Context
 * @session:	Session id
 *
 ...
 */
int mitee_client_close_session(struct tee_context *ctx, u32 session);

/**
 * mitee_client_invoke_func() - Invoke a function in a Trusted Application
 * @ctx:	TEE Context
 ...
 */
int mitee_client_invoke_func(struct tee_context *ctx,
			   struct tee_ioctl_invoke_arg *arg,
			   struct tee_param *param);
