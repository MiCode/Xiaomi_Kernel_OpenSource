 ...
 * Read-modify of AUD_CONFIG (Eg. for enabling audio) causes the "channels"
 * field to be updated as 0xy binary resulting in bad audio.
...
void had_build_channel_allocation_map(struct snd_intelhad *intelhaddata)
{
	...
	struct cea_channel_speaker_allocation *ca = &channel_allocations[0];
	for (i = 0; i < ARRAY_SIZE(channel_allocations); i++) {
		if (spk_mask == ca[i].spk_mask) {
			for (c = 0; c < ca[i].channels; c++) {
				chmap->map[c] = spk_to_chmap(
					ca[i].speakers[(MAX_SPEAKERS - 1)-c]);
			}
			chmap->channels = ca[i].channels;
