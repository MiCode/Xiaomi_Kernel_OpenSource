...
{
	switch (aud_blk) {
	case MT6895_MEMIF_DAI:
	case MT6895_MEMIF_MOD_DAI:
		return dai_memif_rate_transform(dev, rate);
	case MT6895_DAI_PCM_1:
	case MT6895_DAI_PCM_2:
		return pcm_rate_transform(dev, rate);
	default:
		return mt6895_general_rate_transform(dev, rate);
	}
}
...
EXPORT_SYMBOL(mt6895_enable_dc_compensation);
...
EXPORT_SYMBOL(mt6895_set_lch_dc_compensation);
...
EXPORT_SYMBOL(mt6895_set_rch_dc_compensation);
...
EXPORT_SYMBOL(mt6895_adda_dl_gain_control);
