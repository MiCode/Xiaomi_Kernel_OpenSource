...
static int mtk_set_src_1_param(struct mtk_base_afe *afe, int id)
{
	...
	if (rate_in > rate_out) {
		...
		regmap_update_bits(afe->regmap, AFE_GENERAL1_ASRC_2CH_CON2,
				   CHSET_IIR_STAGE_MASK_SFT,
				   iir_stage << CHSET_IIR_STAGE_SFT);
		/* CHSET_IIR_EN */
		regmap_update_bits(afe->regmap, AFE_GENERAL1_ASRC_2CH_CON2,
				   CHSET_IIR_EN_MASK_SFT,
				   0x1 << CHSET_IIR_EN_SFT);
	} else {
		/* CHSET_IIR_EN off */
		regmap_update_bits(afe->regmap, AFE_GENERAL1_ASRC_2CH_CON2,
				   CHSET_IIR_EN_MASK_SFT,
			    ...
			    int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		/* CHSET_STR_CLR */
		regmap_update_bits(afe->regmap, reg,
				   GSRC_CON0_CHSET_STR_CLR_MASK_SFT,
				   0x1 << GSRC_CON0_CHSET_STR_CLR_SFT);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		...
		/* CHSET_STR_CLR */
		regmap_update_bits(afe->regmap, reg,
				   GSRC_CON0_CHSET_STR_CLR_MASK_SFT, 0x0);
		break;
	default:
		break;
	}
				  ...
				  struct snd_soc_dapm_widget *sink)
{
	...
	if (strcmp(w->name, HW_SRC_1_EN_W_NAME) == 0)
		src_priv = afe_priv->dai_priv[MT6895_DAI_SRC_1];
	else
		src_priv = afe_priv->dai_priv[MT6895_DAI_SRC_2];
				 ...
				 struct snd_soc_dai *dai)
{
	...
	unsigned int rate_reg = mt6895_rate_transform(afe->dev, rate, id);
...
int mt6895_dai_src_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6895_dai_set_priv(afe, MT6895_DAI_SRC_1,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;

	ret = mt6895_dai_set_priv(afe, MT6895_DAI_SRC_2,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;
