				...
				int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6833_mck_enable(afe, tdm_priv->bck_id, tdm_priv->bck_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt6833_mck_disable(afe, tdm_priv->bck_id);
		break;
	default:
		break;
	}
				...
				int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6833_mck_enable(afe, tdm_priv->mclk_id, tdm_priv->mclk_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		tdm_priv->mclk_rate = 0;
		mt6833_mck_disable(afe, tdm_priv->mclk_id);
		break;
	default:
		break;
	}
...
static struct mtk_afe_tdm_priv *init_tdm_priv_data(struct mtk_base_afe *afe)
{
	...
	tdm_priv->bck_id = MT6833_I2S4_BCK;
	tdm_priv->mclk_id = MT6833_I2S4_MCK;
...
int mt6833_dai_tdm_register(struct mtk_base_afe *afe)
{
	...
	afe_priv->dai_priv[MT6833_DAI_TDM] = tdm_priv;
