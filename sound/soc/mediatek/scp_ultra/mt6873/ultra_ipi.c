...
static void ultra_ipi_recv_handler(int id,
				 void *prdata,
				 void *data,
				 unsigned int len);
static void ultra_ipi_ack_handler(int id,
				void *prdata,
				void *data,
				unsigned int len);
		      ...
		      bool (*ipi_tx_ack_call)(unsigned int, unsigned int))
{
	...
	mtk_ipi_register(&scp_ipidev, IPI_IN_AUDIO_ULTRA_SND_0,
			(void *)ultra_ipi_recv_handler, NULL,
			 &ultra_ipi_receive);
	if (0)
		mtk_ipi_register(&scp_ipidev, /*IPI_IN_AUDIO_ULTRA_SND_ACK_1*/1,
				(void *)ultra_ipi_ack_handler, NULL,
				&ultra_ipi_send_ack);
#endif
		  ...
		  unsigned int need_ack)
{
	...
	// if (!ultra_ipi_send_info()) {
		// pr_info("SCP is off, bypass send ipi id(%d)\n", msg_id);
		// return false;
	// }
	// if (ultra_service_GetScpRecoverStatus() == true) {
	...
	memset(&ipi_data.payload[0], 0,
		sizeof(unsigned int) * ULTRA_IPI_SEND_BUFFER_LENGTH);
