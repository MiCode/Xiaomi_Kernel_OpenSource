...
#define DUMP_ULTRA_PCM_DATA_PATH "/data/vendor/audiohal"
...
enum { /* dump_data_t */
	DUMP_PCM_IN = 0,
	DUMP_PCM_OUT = 1,
	ELLIPTIC_IPI_DATA = 2,//add for elliptic data flow
	NUM_DUMP_DATA,
};
...
static void ultra_elliptic_data_routine(struct work_struct *ws)
{
	if (1) {
		struct dump_work_t *dump_work = NULL;
		uint32_t rw_idx = 0;
...
static int ultra_dump_kthread(void *data)
{
	...
	while (b_enable_stread && !kthread_should_stop()) {
		...
		case ELLIPTIC_IPI_DATA:{
			pr_info("ELLIPTIC_IPI_DATA ");
			...
			while (size > 0) {
				 ...
				 */
					//test call elliptic push
				// ret = elliptic_data_push(
					// ELLIPTIC_ALL_DEVICES,
					// (const char *)pcm_dump->decode_pcm,
					// writedata,//ELLIPTIC_MSG_BUF_SIZE,
					// ELLIPTIC_DATA_PUSH_FROM_KERNEL);
					//use this to test the time issue?
					//seems also fail;
				if (ret != 0)
					pr_info("[ELUS] : failed to push apr payload");
				size -= writedata;
				pcm_dump++;
			}
			break;
		...
		case DUMP_PCM_OUT: {
			...
			while (size > 0) {
				...
				if (!IS_ERR(fp_pcm_out)) {
					...
					pr_info("fp_pcm_in not exist return\n");
					break;
				}
...
void audio_ipi_client_ultra_init(void)
{
	...
	dump_workqueue[ELLIPTIC_IPI_DATA] =
			create_workqueue("elliptic_data");
	if (dump_workqueue[ELLIPTIC_IPI_DATA] == NULL)
		pr_notice("dump_workqueue[ELLIPTIC_IPI_DATA] = %p\n",
				dump_workqueue[ELLIPTIC_IPI_DATA]);
	AUD_ASSERT(dump_workqueue[ELLIPTIC_IPI_DATA] != NULL);
	...
	INIT_WORK(&dump_work[ELLIPTIC_IPI_DATA].work,
			ultra_elliptic_data_routine);
