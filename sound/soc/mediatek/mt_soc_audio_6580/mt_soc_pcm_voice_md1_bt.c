...
#endif
	daibt_attribute.mDAI_BT_MODE = (sample_rate == 8000) ? Soc_Aud_DATBT_MODE_Mode8K
...
static int mtk_voice_bt1_prepare(struct snd_pcm_substream *substream)
{
	...
	SetDaiBtEnable(true);
	voice_bt1Pcm.mPcmModeWidebandSel = (runtimeStream->rate == 8000) ?
...
static int mtk_voice_bt_probe(struct platform_device *pdev)
{
	...
	return snd_soc_register_platform(&pdev->dev, &mtk_soc_voice_bt_platform);
}
