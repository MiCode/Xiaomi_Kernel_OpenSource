...
static void StopAudioCaptureHardware(struct snd_pcm_substream *substream)
{
	...
	mt_afe_disable_memory_path(Soc_Aud_Digital_Block_I2S_IN_ADC);

	if (mt_afe_get_memory_path_state(Soc_Aud_Digital_Block_I2S_IN_ADC) == false)
		SetI2SAdcEnable(false);

	mt_afe_disable_memory_path(Soc_Aud_Digital_Block_MEM_VUL);

	mt_afe_set_irq_state(Soc_Aud_IRQ_MCU_MODE_IRQ2_MCU_MODE, false);

	/* here to turn off digital part */
	mt_afe_set_connection(Soc_Aud_InterCon_DisConnect, Soc_Aud_InterConnectionInput_I03,
		      Soc_Aud_InterConnectionOutput_O09);
	mt_afe_set_connection(Soc_Aud_InterCon_DisConnect, Soc_Aud_InterConnectionInput_I04,
...
static void StartAudioCaptureHardware(struct snd_pcm_substream *substream)
{
	...
	if (mt_afe_get_memory_path_state(Soc_Aud_Digital_Block_I2S_IN_ADC) == false) {
		mt_afe_enable_memory_path(Soc_Aud_Digital_Block_I2S_IN_ADC);
		SetI2SAdcEnable(true);
	} else
		mt_afe_enable_memory_path(Soc_Aud_Digital_Block_I2S_IN_ADC);

	mt_afe_set_connection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I03,
		      Soc_Aud_InterConnectionOutput_O09);
	mt_afe_set_connection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I04,
		      Soc_Aud_InterConnectionOutput_O10);
	...
	mt_afe_set_sample_rate(Soc_Aud_Digital_Block_MEM_VUL, substream->runtime->rate);
	mt_afe_enable_memory_path(Soc_Aud_Digital_Block_MEM_VUL);
...
static snd_pcm_uframes_t mtk_capture_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	afe_ul1_spinlock_lock();
	if (mt_afe_get_memory_path_state(Soc_Aud_Digital_Block_MEM_VUL) == true) {
...
static void SetVULBuffer(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *hw_params)
{
	...
	mt_afe_set_reg(AFE_VUL_BASE, pblock->pucPhysBufAddr, 0xffffffff);
	mt_afe_set_reg(AFE_VUL_END, pblock->pucPhysBufAddr + (pblock->u4BufferSize - 1), 0xffffffff);
...
static int mtk_capture_pcm_open(struct snd_pcm_substream *substream)
{
	...
	mt_afe_main_clk_on();
	mt_afe_adc_clk_on();
	VUL_Control_context = get_mem_control_t(Soc_Aud_Digital_Block_MEM_VUL);
	...
	if (0) {
		...
		set_sramstate(SRAM_STATE_CAPTURE);
...
static int mtk_capture_pcm_close(struct snd_pcm_substream *substream)
{
	...
	if (mCaptureUseSram == true) {
		clear_sramstate(SRAM_STATE_CAPTURE);
...
static int mtk_capture_alsa_start(struct snd_pcm_substream *substream)
{
	...
	set_memif_substream(Soc_Aud_Digital_Block_MEM_VUL, substream);
				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (Vul_Block->u4BufferSize <= 0) {
		msleep(50);
		pr_err("Vul_Block->u4BufferSize = %d\n", Vul_Block->u4BufferSize);
		return 0;
	}
	...
	PRINTK_AUD_UL1
		...
		Vul_Block->u4DataRemained, Vul_Block->u4DMAReadIdx, Vul_Block->u4WriteIdx);

	if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize) {
		if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx) {
			PRINTK_AUD_UL1("%s 1, read_size:%zu, DataRemained:%x, DMA_Read_Ptr:0x%zu, DMAReadIdx:%x\n",
		...
		uint32_t size_1 = Vul_Block->u4BufferSize - DMA_Read_Ptr;
...
static int mtk_afe_capture_probe(struct snd_soc_platform *platform)
{
	...
	afe_allocate_mem_buffer(platform->dev, Soc_Aud_Digital_Block_MEM_VUL,
				   UL1_MAX_BUFFER_SIZE);
	Capture_dma_buf = afe_get_mem_buffer(Soc_Aud_Digital_Block_MEM_VUL);
...
MODULE_DEVICE_TABLE(of, mt_soc_pcm_capture_of_ids);
