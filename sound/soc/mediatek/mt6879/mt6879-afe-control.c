...
{
	switch (aud_blk) {
	case MT6879_MEMIF_DAI:
	case MT6879_MEMIF_MOD_DAI:
		return dai_memif_rate_transform(dev, rate);
	case MT6879_DAI_PCM_1:
	case MT6879_DAI_PCM_2:
		return pcm_rate_transform(dev, rate);
	default:
		return mt6879_general_rate_transform(dev, rate);
	}
}
...
EXPORT_SYMBOL(mt6879_enable_dc_compensation);
...
EXPORT_SYMBOL(mt6879_set_lch_dc_compensation);
...
EXPORT_SYMBOL(mt6879_set_rch_dc_compensation);
...
EXPORT_SYMBOL(mt6879_adda_dl_gain_control);
