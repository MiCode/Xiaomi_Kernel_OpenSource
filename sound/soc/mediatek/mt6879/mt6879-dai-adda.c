...
static int mtk_adda_ul_src_dmic(struct mtk_base_afe *afe, int id)
{
	...
	switch (id) {
	case MT6879_DAI_ADDA:
	case MT6879_DAI_AP_DMIC:
		reg = AFE_ADDA_UL_SRC_CON0;
		break;
	case MT6879_DAI_ADDA_CH34:
	case MT6879_DAI_AP_DMIC_CH34:
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap, AFE_ADDA_MTKAIF_RX_CFG0,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6879_DAI_ADDA);
		}
		break;
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap,
					   ...
					   MTKAIF_RXIF_VOICE_MODE_MASK_SFT,
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6879_DAI_ADDA_CH34);
		}
...
static bool is_adda_mtkaif_need_phase_delay(struct mt6879_afe_private *afe_priv)
{
	if (mt6879_afe_gpio_is_prepared(MT6879_AFE_GPIO_DAT_MISO0_ON) &&
	    afe_priv->mtkaif_chosen_phase[0] < 0) {
		AUDIO_AEE("adda mtkaif miso0 calib fail");
		return false;
	}

	if (mt6879_afe_gpio_is_prepared(MT6879_AFE_GPIO_DAT_MISO1_ON) &&
...
/* mtkaif dmic */
static const char *const mt6879_adda_off_on_str[] = {
	"Off", "On"
};

static const struct soc_enum mt6879_adda_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6879_adda_off_on_str),
			    mt6879_adda_off_on_str),
};

static int mt6879_adda_dmic_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6879_adda_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6879_adda6_only_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6879_adda6_only_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_adda_controls[] = {
	...
	SOC_ENUM_EXT("MTKAIF_DMIC", mt6879_adda_enum[0],
		     mt6879_adda_dmic_get, mt6879_adda_dmic_set),
	SOC_ENUM_EXT("MTKAIF_ADDA6_ONLY", mt6879_adda_enum[0],
		     mt6879_adda6_only_get, mt6879_adda6_only_set),
};
...
int mt6879_dai_adda_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6879_dai_set_priv(afe, MT6879_DAI_ADDA,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	ret = mt6879_dai_set_priv(afe, MT6879_DAI_ADDA_CH34,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	/* ap dmic priv share with adda */
	afe_priv->dai_priv[MT6879_DAI_AP_DMIC] =
		afe_priv->dai_priv[MT6879_DAI_ADDA];
	afe_priv->dai_priv[MT6879_DAI_AP_DMIC_CH34] =
		afe_priv->dai_priv[MT6879_DAI_ADDA_CH34];
