...
static int mtk_is_i2s_low_power(int i2s_num)
{
	...
	i2s_bit_shift = i2s_num - MT6879_DAI_I2S_0;
	if (i2s_bit_shift < 0 || i2s_bit_shift > MT6879_DAI_I2S_MAX_NUM) {
...
/* low jitter control */
static const char *const mt6879_i2s_hd_str[] = {
	"Normal", "Low_Jitter"
};

static const struct soc_enum mt6879_i2s_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6879_i2s_hd_str),
			    mt6879_i2s_hd_str),
};

static int mt6879_i2s_hd_get(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6879_i2s_hd_set(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_dai_i2s_controls[] = {
	SOC_ENUM_EXT(MTK_AFE_I2S0_KCONTROL_NAME, mt6879_i2s_enum[0],
		     mt6879_i2s_hd_get, mt6879_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2S1_KCONTROL_NAME, mt6879_i2s_enum[0],
		     mt6879_i2s_hd_get, mt6879_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2S2_KCONTROL_NAME, mt6879_i2s_enum[0],
		     mt6879_i2s_hd_get, mt6879_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2S3_KCONTROL_NAME, mt6879_i2s_enum[0],
		     mt6879_i2s_hd_get, mt6879_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2S5_KCONTROL_NAME, mt6879_i2s_enum[0],
		     mt6879_i2s_hd_get, mt6879_i2s_hd_set),
			    ...
			    int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6879_afe_gpio_request(afe, true, i2s_priv->id, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt6879_afe_gpio_request(afe, false, i2s_priv->id, 0);
		break;
	default:
		break;
	}
			  ...
			  int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (strcmp(w->name, APLL1_W_NAME) == 0)
			mt6879_apll1_enable(afe);
		else
			mt6879_apll2_enable(afe);
		break;
	case SND_SOC_DAPM_POST_PMD:
		if (strcmp(w->name, APLL1_W_NAME) == 0)
			mt6879_apll1_disable(afe);
		else
			mt6879_apll2_disable(afe);
		break;
	default:
		break;
	}
			     ...
			     int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6879_mck_enable(afe, i2s_priv->mclk_id, i2s_priv->mclk_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		i2s_priv->mclk_rate = 0;
		mt6879_mck_disable(afe, i2s_priv->mclk_id);
		break;
	default:
		break;
	}
				    ...
				    struct snd_soc_dapm_widget *sink)
{
	...
	i2s_need_apll = mt6879_get_apll_by_rate(afe, i2s_priv->rate);
		...
		struct snd_soc_dai *dai)
{
	...
	unsigned int rate_reg = mt6879_rate_transform(afe->dev,
				rate, dai->id);
...
static const struct mtk_afe_i2s_priv mt6879_i2s_priv[DAI_I2S_NUM] = {
	[DAI_I2S0] = {
		.id = MT6879_DAI_I2S_0,
		.mclk_id = MT6879_I2S0_MCK,
		.share_property_name = "i2s0-share",
		.share_i2s_id = -1,
	},
	[DAI_I2S1] = {
		.id = MT6879_DAI_I2S_1,
		.mclk_id = MT6879_I2S1_MCK,
		.share_property_name = "i2s1-share",
		.share_i2s_id = -1,
	},
	[DAI_I2S2] = {
		.id = MT6879_DAI_I2S_2,
		.mclk_id = MT6879_I2S2_MCK,
		.share_property_name = "i2s2-share",
		.share_i2s_id = -1,
	},
	[DAI_I2S3] = {
		.id = MT6879_DAI_I2S_3,
		.mclk_id = MT6879_I2S3_MCK,
		.share_property_name = "i2s3-share",
		.share_i2s_id = -1,
	},
	[DAI_I2S5] = {
		.id = MT6879_DAI_I2S_5,
		.mclk_id = MT6879_I2S5_MCK,
		.share_property_name = "i2s5-share",
		.share_i2s_id = -1,
	},
};

int mt6879_dai_i2s_get_share(struct mtk_base_afe *afe)
{
	struct mt6879_afe_private *afe_priv = afe->platform_priv;
	...
	for (i = 0; i < DAI_I2S_NUM; i++) {
		i2s_priv = afe_priv->dai_priv[mt6879_i2s_priv[i].id];
		property_name = mt6879_i2s_priv[i].share_property_name;
...
int mt6879_dai_i2s_set_priv(struct mtk_base_afe *afe)
{
	int i;
	int ret;

	for (i = 0; i < DAI_I2S_NUM; i++) {
		ret = mt6879_dai_set_priv(afe, mt6879_i2s_priv[i].id,
...
int mt6879_dai_i2s_register(struct mtk_base_afe *afe)
{
	...
	ret = mt6879_dai_i2s_set_priv(afe);
	if (ret)
		return ret;

	/* parse share i2s */
	ret = mt6879_dai_i2s_get_share(afe);
	if (ret)
		return ret;
