				  ...
				  struct snd_soc_dapm_widget *sink)
{
	...
	if (strcmp(w->name, HW_SRC_1_EN_W_NAME) == 0)
		src_priv = afe_priv->dai_priv[MT6853_DAI_SRC_1];
	else
		src_priv = afe_priv->dai_priv[MT6853_DAI_SRC_2];
				 ...
				 struct snd_soc_dai *dai)
{
	...
	unsigned int rate_reg = mt6853_rate_transform(afe->dev, rate, id);
...
int mt6853_dai_src_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6853_dai_set_priv(afe, MT6853_DAI_SRC_1,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;

	ret = mt6853_dai_set_priv(afe, MT6853_DAI_SRC_2,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;
