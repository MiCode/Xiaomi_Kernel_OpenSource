...
{
    printk("mtk_capture_tdm_prepare substream->rate = %d  substream->channels = %d \n", substream->runtime->rate, substream->runtime->channels);
    EnableApll1(true);
    EnableApll2(true);
...
static snd_pcm_uframes_t mtk_capture_pcm_pointer(struct snd_pcm_substream *substream)
{
    ...
    PRINTK_AUD_UL1("mtk_capture_tdm_pointer Awb_Block->u4WriteIdx;= 0x%x \n", Awb_Block->u4WriteIdx);
    if (TDM_VUL_Control_context->interruptTrigger == 1)
    {
        ...
        if (HW_Cur_ReadIdx == 0)
        {
            PRINTK_AUD_UL1("[Auddrv] mtk_awb_tdm_pointer  HW_Cur_ReadIdx ==0 \n");
        ...
        PRINTK_AUD_UL1("[Auddrv] mtk_awb_tdm_pointer =0x%x HW_memory_index = 0x%x\n", HW_Cur_ReadIdx, HW_memory_index);
                                     ...
                                     struct snd_pcm_hw_params *hw_params)
{
    ...
    int ret = 0;
    printk("mtk_capture_tdm_hw_params \n");
    ...
    {
        printk("mtk_capture_tdm_hw_params Capture_dma_buf->area\n");
//        runtime->dma_bytes = params_buffer_bytes(hw_params);
    ...
    {
        printk("mtk_capture_tdm_hw_params snd_pcm_lib_malloc_pages\n");
...
static int mtk_capture_pcm_hw_free(struct snd_pcm_substream *substream)
{
    if (Capture_dma_buf->area)
    {
        printk("mtk_capture_tdm_hw_free dma_buf->area \n");

        return 0;
    }
    else
    {
        printk("mtk_capture_tdm_hw_free snd_pcm_lib_free_pages \n");
...
static int mtk_capture_pcm_open(struct snd_pcm_substream *substream)
{
    ...
    if (ret < 0)
    {
        printk("snd_tdm_hw_constraint_integer failed\n");
    }

    printk("mtk_capture_tdm_open runtime rate = %d channels = %d \n", runtime->rate, runtime->channels);
    if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
    {
        printk("SNDRV_tdm_STREAM_CAPTURE mtkalsa_capture_constraints\n");
    }
    else
    {
    ...
    if (ret < 0)
    {
        printk("mtk_capture_tdm_close\n");
        mtk_capture_pcm_close(substream);
        return ret;
    }
    printk("mtk_capture_tdm_open return\n");
    return 0;
}
...
{
    printk("mtk_capture_tdm_trigger cmd = %d\n", cmd);
                                ...
                                void __user *dst, snd_pcm_uframes_t count)
{
    ...
    PRINTK_AUD_UL1("mtk_capture_tdm_copy pos = %lucount = %lu \n ", pos, count);
...
static int mtk_asoc_capture_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
    printk("mtk_asoc_capture_tdm_new \n");
    return 0;
}
...
//    Capture_dma_buf =  Get_Mem_Buffer(Soc_Aud_Digital_Block_MEM_VUL);
    ...
    printk("mtk_afe_capture_probe TDM UL dma_alloc_coherent\n");
