...
static struct snd_dma_buffer *mod_adci2s_dma_buf  = NULL;
...
void StartAudiomod_adci2sAnalogHardware(void);
void StopAudiomod_adci2sAnalogHardware(void);
...
static void StopAudiomod_adci2sHardware(struct snd_pcm_substream *substream)
{
    printk("StopAudiomod_adci2sHardware \n");
    ...
    SetConnection(Soc_Aud_InterCon_DisConnect, Soc_Aud_InterConnectionInput_I14, Soc_Aud_InterConnectionOutput_O09);
...
static void StartAudiomod_adci2sHardware(struct snd_pcm_substream *substream)
{
    printk("StartAudiomod_adci2sHardware \n");
    ...
    SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I14, Soc_Aud_InterConnectionOutput_O09);
...
{
    //printk("mtk_mod_adci2s_pcm_prepare substream->rate = %d  substream->channels = %d \n", substream->runtime->rate, substream->runtime->channels);
    return 0;
}
...
static int mtk_mod_adci2s_alsa_stop(struct snd_pcm_substream *substream)
{
    ...
    printk("mtk_mod_adci2s_alsa_stop \n");
...
static snd_pcm_uframes_t mtk_mod_adci2s_pcm_pointer(struct snd_pcm_substream *substream)
{
    ...
    PRINTK_AUD_UL1("mtk_mod_adci2s_pcm_pointer Awb_Block->u4WriteIdx;= 0x%x \n", Awb_Block->u4WriteIdx);
    if (VUL_Control_context->interruptTrigger == 1)
    {
        ...
        PRINTK_AUD_UL1("[Auddrv] mtk_awb_pcm_pointer =0x%x HW_memory_index = 0x%x\n", HW_Cur_ReadIdx, HW_memory_index);
                                        ...
                                        struct snd_pcm_hw_params *hw_params)
{
    ...
    int ret = 0;
    printk("mtk_mod_adci2s_pcm_hw_params \n");

    dma_buf->dev.type = SNDRV_DMA_TYPE_DEV;
    dma_buf->dev.dev = substream->pcm->card->dev;
    dma_buf->private_data = NULL;
    printk("mod_adci2s_dma_buf = %p mod_adci2s_dma_buf->area = %p\n", mod_adci2s_dma_buf, mod_adci2s_dma_buf->area);

    if (mod_adci2s_dma_buf->area)
    {
        printk("mtk_mod_adci2s_pcm_hw_params mod_adci2s_dma_buf->area\n");
    ...
    {
        printk("mtk_mod_adci2s_pcm_hw_params snd_pcm_lib_malloc_pages\n");
        ret =  snd_pcm_lib_malloc_pages(substream, params_buffer_bytes(hw_params));
    }
    printk("mtk_mod_adci2s_pcm_hw_params dma_bytes = %zu dma_area = %p dma_addr = 0x%lx\n",
...
{
    printk("mtk_mod_adci2s_pcm_hw_free \n");
    if (mod_adci2s_dma_buf->area)
    {
        return 0;
    }
...
static int mtk_mod_adci2s_pcm_open(struct snd_pcm_substream *substream)
{
    ...
    memcpy((void *)(&(runtime->hw)), (void *)&mtk_mod_adci2s_hardware , sizeof(struct snd_pcm_hardware));
    ...
    printk("mtk_mod_adci2s_pcm_open runtime rate = %d channels = %d \n", runtime->rate, runtime->channels);
    if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
    {
        printk("SNDRV_PCM_STREAM_CAPTURE mtkalsa_mod_adci2s_constraints\n");
    }
    else
    {
    ...
    if (ret < 0)
    {
        printk("mtk_mod_adci2s_pcm_close\n");
        mtk_mod_adci2s_pcm_close(substream);
        return ret;
    }
    printk("mtk_mod_adci2s_pcm_open return\n");
    return 0;
}
...
{
    printk("mtk_mod_adci2s_alsa_start \n");
...
{
    printk("mtk_mod_adci2s_pcm_trigger cmd = %d\n", cmd);
                                   ...
                                   void __user *dst, snd_pcm_uframes_t count)
{
    ...
    PRINTK_AUD_UL1("mtk_mod_adci2s_pcm_copy pos = %lucount = %lu \n ", pos, count);
    ...
    if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize)
    {
        ...
        if (copy_to_user((void __user *)Read_Data_Ptr, (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr), read_size))
        {
            ...
            printk("AudDrv_MEMIF_Read Fail 1 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%zu,read_size:%zu", Read_Data_Ptr, Vul_Block->pucVirtBufAddr, Vul_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
            return 0;
        }
    ...
    else
    {
        ...
        if (copy_to_user((void __user *)(Read_Data_Ptr + size_1), (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr), size_2))
        {
            ...
            printk("AudDrv_MEMIF_Read Fail 3 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x , DMA_Read_Ptr:0x%zu, read_size:%zu", Read_Data_Ptr, Vul_Block->pucVirtBufAddr, Vul_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
            return read_count << 2;
        }
...
static int mtk_asoc_mod_adci2s_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
    printk("mtk_asoc_mod_adci2s_pcm_new \n");
    return 0;
}
...
static int mtk_afe_mod_adci2s_probe(struct snd_soc_platform *platform)
{
    printk("mtk_afe_mod_adci2s_probe\n");
