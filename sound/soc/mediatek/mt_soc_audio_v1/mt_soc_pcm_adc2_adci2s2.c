...
static struct snd_dma_buffer *adc2_adci2s2_dma_buf  = NULL;
...
void StartAudioadc2_adci2s2AnalogHardware(void);
void StopAudioadc2_adci2s2AnalogHardware(void);
...
static void StopAudioadc2_adci2s2Hardware(struct snd_pcm_substream *substream)
{
    printk("StopAudioadc2_adci2s2Hardware \n");
...
static void StartAudioadc2_adci2s2Hardware(struct snd_pcm_substream *substream)
{
    printk("StartAudioadc2_adci2s2Hardware \n");
...
{
    printk("mtk_adc2_adci2s2_pcm_prepare substream->rate = %d  substream->channels = %d \n", substream->runtime->rate, substream->runtime->channels);
    return 0;
}
...
static int mtk_adc2_adci2s2_alsa_stop(struct snd_pcm_substream *substream)
{
    ...
    printk("mtk_adc2_adci2s2_alsa_stop \n");
...
static snd_pcm_uframes_t mtk_adc2_adci2s2_pcm_pointer(struct snd_pcm_substream *substream)
{
    ...
    PRINTK_AUD_UL2("mtk_adc2_adci2s2_pcm_pointer vul2_Block->u4WriteIdx;= 0x%x \n", vul2_Block->u4WriteIdx);
    if (VUL2_Control_context->interruptTrigger == 1)
    {
        ...
        if (HW_Cur_ReadIdx == 0)
        {
            PRINTK_AUD_UL2("[Auddrv] mtk_adc2_adci2s2_pcm_pointer  HW_Cur_ReadIdx ==0 \n");
        ...
        PRINTK_AUD_UL2("[Auddrv] mtk_adc2_adci2s2_pcm_pointer =0x%x HW_memory_index = 0x%x\n", HW_Cur_ReadIdx, HW_memory_index);
                                     ...
                                     struct snd_pcm_hw_params *hw_params)
{
    ...
    int ret = 0;
    printk("mtk_adc2_adci2s2_pcm_hw_params \n");

    dma_buf->dev.type = SNDRV_DMA_TYPE_DEV;
    dma_buf->dev.dev = substream->pcm->card->dev;
    dma_buf->private_data = NULL;
    printk("adc2_adci2s2_dma_buf = %p adc2_adci2s2_dma_buf->area = %p\n", adc2_adci2s2_dma_buf, adc2_adci2s2_dma_buf->area);

    if (adc2_adci2s2_dma_buf->area)
    {
        printk("mtk_adc2_adci2s2_pcm_hw_params adc2_adci2s2_dma_buf->area\n");
    ...
    {
        printk("mtk_adc2_adci2s2_pcm_hw_params snd_pcm_lib_malloc_pages\n");
        ret =  snd_pcm_lib_malloc_pages(substream, params_buffer_bytes(hw_params));
    }
    printk("mtk_adc2_adci2s2_pcm_hw_params dma_bytes = %zu dma_area = %p dma_addr = 0x%lx\n",
...
{
    printk("mtk_adc2_adci2s2_pcm_hw_free \n");
    if (adc2_adci2s2_dma_buf->area)
    {
        return 0;
    }
...
static int mtk_adc2_adci2s2_pcm_open(struct snd_pcm_substream *substream)
{
    ...
    memcpy((void *)(&(runtime->hw)), (void *)&mtk_adc2_adci2s2_hardware , sizeof(struct snd_pcm_hardware));
    ...
    printk("mtk_adc2_adci2s2_pcm_open runtime rate = %d channels = %d \n", runtime->rate, runtime->channels);
    ...
    if (ret < 0)
    {
        printk("mtk_adc2_adci2s2_pcm_close\n");
        mtk_adc2_adci2s2_pcm_close(substream);
        return ret;
    }
    printk("mtk_adc2_adci2s2_pcm_open return\n");
    return 0;
}
...
{
    printk("mtk_adc2_adci2s2_alsa_start \n");
...
{
    printk("mtk_adc2_adci2s2_pcm_trigger cmd = %d\n", cmd);
                                ...
                                void __user *dst, snd_pcm_uframes_t count)
{
    ...
    PRINTK_AUD_UL2("mtk_adc2_adci2s2_pcm_copy pos = %lucount = %lu \n ", pos, count);
    ...
    {
        PRINTK_AUD_UL2("mtk_adc2_adci2s2_pcm_copy u4DataRemained=%x > u4BufferSize=%x" , Vul_Block->u4DataRemained, Vul_Block->u4BufferSize);
        Vul_Block->u4DataRemained = 0;
    ...
    PRINTK_AUD_UL2("mtk_adc2_adci2s2_pcm_copy finish0, read_count:%x, read_size:%x, u4DataRemained:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x \r\n",
    ...
    if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize)
    {
        ...
        {
            printk("mtk_adc2_adci2s2_pcm_copy 1, read_size:%zu, DataRemained:%x, DMA_Read_Ptr:0x%zu, DMAReadIdx:%x \r\n",
        ...
        if (copy_to_user((void __user *)Read_Data_Ptr, (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr), read_size))
        {
            ...
            printk("mtk_adc2_adci2s2_pcm_copy Fail 1 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%zu,read_size:%zu", Read_Data_Ptr, Vul_Block->pucVirtBufAddr, Vul_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
            return 0;
        }
        ...
        PRINTK_AUD_UL2("mtk_adc2_adci2s2_pcm_copy finish1, copy size:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x, u4DataRemained:%x \r\n",
    ...
    else
    {
        ...
        if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx)
        {
            ...
            printk("mtk_adc2_adci2s2_pcm_copy 2, read_size1:%x, DataRemained:%x, DMA_Read_Ptr:0x%zu, DMAReadIadx:%x \r\n",
        ...
        if (copy_to_user((void __user *)Read_Data_Ptr, (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr), size_1))
        {
            ...
            printk("mtk_adc2_adci2s2_pcm_copy Fail 2 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%zu,read_size:%zu",
        ...
        PRINTK_AUD_UL2("mtk_adc2_adci2s2_pcm_copy finish2, copy size_1:%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x, u4DataRemained:%x \r\n",
                       size_1, Vul_Block->u4DMAReadIdx, Vul_Block->u4WriteIdx, Vul_Block->u4DataRemained);

        if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx)
        {

            printk("mtk_adc2_adci2s2_pcm_copy 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:0x%zu, DMAReadIdx:%x \r\n",
        ...
        if (copy_to_user((void __user *)(Read_Data_Ptr + size_1), (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr), size_2))
        {
            ...
            printk("mtk_adc2_adci2s2_pcm_copy Fail 3 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x , DMA_Read_Ptr:0x%zu, read_size:%zu", Read_Data_Ptr, Vul_Block->pucVirtBufAddr, Vul_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
            return read_count << 2;
        }
        ...
        PRINTK_AUD_UL2("mtk_adc2_adci2s2_pcm_copy finish3, copy size_2:%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x u4DataRemained:%x \r\n",
...
static int mtk_asoc_adc2_adci2s2_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
    printk("mtk_asoc_adc2_adci2s2_pcm_new \n");
    return 0;
}
...
static int mtk_afe_adc2_adci2s2_probe(struct snd_soc_platform *platform)
{
    printk("mtk_afe_adc2_adci2s2_probe\n");
