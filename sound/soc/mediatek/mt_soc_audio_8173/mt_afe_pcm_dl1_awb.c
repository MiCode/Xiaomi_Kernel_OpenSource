 ...
 */
static int mt_pcm_dl1_awb_close(struct snd_pcm_substream *substream);
...
static struct snd_pcm_hardware mt_pcm_dl1_awb_hardware = {
...
static struct snd_pcm_hw_constraint_list mt_pcm_dl1_awb_constraints_rates = {
...
static int mt_pcm_dl1_awb_hw_params(struct snd_pcm_substream *substream,
...
static int mt_pcm_dl1_awb_hw_free(struct snd_pcm_substream *substream)
{
	pr_debug("%s\n", __func__);
...
static int mt_pcm_dl1_awb_start(struct snd_pcm_substream *substream)
{
	pr_debug("%s\n", __func__);
	mt_afe_add_ctx_substream(MT_AFE_MEM_CTX_AWB, substream);
	mt_pcm_dl1_awb_start_audio_hw(substream);
	return 0;
}

static int mt_pcm_dl1_awb_stop(struct snd_pcm_substream *substream)
{
	pr_debug("%s\n", __func__);
	mt_pcm_dl1_awb_stop_audio_hw(substream);
	mt_afe_reset_dma_buffer(MT_AFE_MEM_CTX_AWB);
	mt_afe_remove_ctx_substream(MT_AFE_MEM_CTX_AWB);
	return 0;
}

static int mt_pcm_dl1_awb_trigger(struct snd_pcm_substream *substream, int cmd)
{
	pr_debug("%s cmd=%d\n", __func__, cmd);
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		return mt_pcm_dl1_awb_start(substream);
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		return mt_pcm_dl1_awb_stop(substream);
	}
	return -EINVAL;
}

static snd_pcm_uframes_t mt_pcm_dl1_awb_pointer(struct snd_pcm_substream *substream)
{
	return mt_afe_update_hw_ptr(MT_AFE_MEM_CTX_AWB);
}

static struct snd_pcm_ops mt_pcm_dl1_awb_ops = {
	.open = mt_pcm_dl1_awb_open,
	.close = mt_pcm_dl1_awb_close,
	.ioctl = snd_pcm_lib_ioctl,
	.hw_params = mt_pcm_dl1_awb_hw_params,
	.hw_free = mt_pcm_dl1_awb_hw_free,
	.prepare = mt_pcm_dl1_awb_prepare,
	.trigger = mt_pcm_dl1_awb_trigger,
...
static const struct of_device_id mt_pcm_dl1_awb_dt_match[] = {
	{.compatible = "mediatek," MT_SOC_DL1_AWB_PCM,},
	{}
};

MODULE_DEVICE_TABLE(of, mt_pcm_dl1_awb_dt_match);

static struct platform_driver mt_pcm_dl1_awb_driver = {
	.driver = {
		   .name = MT_SOC_DL1_AWB_PCM,
		   .owner = THIS_MODULE,
		   .of_match_table = mt_pcm_dl1_awb_dt_match,
		   },
	.probe = mt_pcm_dl1_awb_dev_probe,
	.remove = mt_pcm_dl1_awb_dev_remove,
};

module_platform_driver(mt_pcm_dl1_awb_driver);

MODULE_DESCRIPTION("AFE PCM DL1 AWB platform driver");
MODULE_LICENSE("GPL");
