				...
				int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6873_mck_enable(afe, tdm_priv->bck_id, tdm_priv->bck_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt6873_mck_disable(afe, tdm_priv->bck_id);
		break;
	default:
		break;
	}
				...
				int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6873_mck_enable(afe, tdm_priv->mclk_id, tdm_priv->mclk_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		tdm_priv->mclk_rate = 0;
		mt6873_mck_disable(afe, tdm_priv->mclk_id);
		break;
	default:
		break;
	}
				    ...
				    struct snd_soc_dapm_widget *sink)
{
	...
	/* which apll */
	cur_apll = mt6873_get_apll_by_name(afe, source->name);
				...
				int freq)
{
	...
	apll = mt6873_get_apll_by_rate(afe, freq);
				 ...
				 struct snd_soc_dai *dai)
{
	...
	if (tdm_priv->bck_rate > tdm_priv->mclk_rate)
		dev_err(afe->dev, "%s(), bck_rate > mclk_rate rate\n",
			__func__);

	if (tdm_priv->mclk_rate % tdm_priv->bck_rate != 0)
		dev_err(afe->dev, "%s(), bck cannot generate\n",
			__func__);
				  ...
				  int clk_id, unsigned int freq, int dir)
{
	...
	if (!tdm_priv) {
		dev_err(afe->dev, "%s(), tdm_priv == NULL\n", __func__);
		return -EINVAL;
	}

	if (dir != SND_SOC_CLOCK_OUT) {
		dev_err(afe->dev, "%s(), dir != SND_SOC_CLOCK_OUT\n", __func__);
		return -EINVAL;
	}
