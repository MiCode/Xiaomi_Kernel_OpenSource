...
#define AWINIC_CODE_VERSION "V0.0.5-V1.0.4"	/* "code version"-"excel version" */
...
int aw_check_sum(struct aw_bin *bin, int bin_num)
{
	...
	pr_info("aw_bin_parse p_check_sum = %p\n", p_check_sum);
	...
	pr_info("aw_bin_parse bin_num=%d, check_sum = 0x%x, sum_data = 0x%x\n",
		bin_num, check_sum, sum_data);
	if (sum_data != check_sum) {
		p_check_sum = NULL;
		DBG("aw_bin_parse check sum or check bin data len error\n");
...
int aw_check_data_version(struct aw_bin *bin, int bin_num)
{
	...
	DBG("aw_bin_parse Unrecognized this bin data version\n");
	return -4;
}
...
int aw_check_register_num_v1(struct aw_bin *bin, int bin_num)
{
	...
	if (parse_register_num != check_register_num) {
		p_check_sum = NULL;
		DBG("aw_bin_parse register num is error\n");
...
int aw_check_dsp_reg_num_v1(struct aw_bin *bin, int bin_num)
{
	...
	if (parse_dsp_reg_num != check_dsp_reg_num) {
		p_check_sum = NULL;
		DBG("aw_bin_parse dsp reg num is error\n");
...
int aw_check_soc_app_num_v1(struct aw_bin *bin, int bin_num)
{
	 ...
	 *(p_check_sum+1),
	 *(p_check_sum));*/
	parse_soc_app_num = GET_32_DATA(*(p_check_sum + 7),
					*(p_check_sum + 6),
					*(p_check_sum + 5), *(p_check_sum + 4));
	check_soc_app_num = bin->header_info[bin_num].bin_data_len - 8;
	pr_info
	    ("aw_bin_parse bin_num=%d, parse_soc_app_num = 0x%x, check_soc_app_num = 0x%x\n",
	     bin_num, parse_soc_app_num, check_soc_app_num);
	if (parse_soc_app_num != check_soc_app_num) {
		p_check_sum = NULL;
		DBG("aw_bin_parse soc app num is error\n");
		DBG("aw_bin_parse bin_num=%d, parse_soc_app_num = 0x%x, check_soc_app_num = 0x%x\n", bin_num, parse_soc_app_num, check_soc_app_num);
		return -7;
	}
	bin->header_info[bin_num].reg_num = parse_soc_app_num;
	bin->header_info[bin_num].download_addr =
	    GET_32_DATA(*(p_check_sum + 3), *(p_check_sum + 2),
			*(p_check_sum + 1), *(p_check_sum + 0));
	bin->header_info[bin_num].valid_data_len =
	    bin->header_info[bin_num].bin_data_len - 8;
	p_check_sum = NULL;
	bin->header_info[bin_num].valid_data_addr =
	    bin->header_info[bin_num].valid_data_addr + 8;
	return 0;
}
				      ...
				      struct aw_bin *bin)
{
	...
	pr_info("aw_bin_parse enter multi bin branch -- %s\n", __func__);
	if (!bin_serial_num) {
...
int aw_get_multi_bin_header_1_0_0(struct aw_bin *bin)
{
	...
	for (i = 0; i < bin_num; i++) {
		pr_info("aw_bin_parse enter multi bin for is %d\n", i);
...
int aw_parse_bin_header_1_0_0(struct aw_bin *bin)
{
	...
	bin_data_type = GET_32_DATA(*(bin->p_addr + 11),
				    *(bin->p_addr + 10),
				    *(bin->p_addr + 9), *(bin->p_addr + 8));
	pr_info("aw_bin_parse bin_data_type 0x%x\n", bin_data_type);
	...
	case DATA_TYPE_SOC_APP:
		...
		pr_info("aw_bin_parse enter single bin branch\n");
		bin->single_bin_parse_num += 1;
		pr_info("%s bin->single_bin_parse_num is %d\n", __func__,
	...
	case DATA_TYPE_MULTI_BINS:
		...
		pr_info("aw_bin_parse enter multi bin branch\n");
		bin->multi_bin_parse_num += 1;
		pr_info("%s bin->multi_bin_parse_num is %d\n", __func__,
		...
		break;
	default:
		DBG("aw_bin_parse Unrecognized this bin data type\n");
		return -2;
	}
	return 0;
}
...
static int aw_check_bin_header_version(struct aw_bin *bin)
{
	...
	pr_info("aw_bin_parse header_version 0x%x\n", header_version);
	...
	case HEADER_VERSION_1_0_0:
		...
		return ret;
	default:
		DBG("aw_bin_parse Unrecognized this bin header version \n");
		return -1;
	}
}
...
int aw_parsing_bin_file(struct aw_bin *bin)
{
	...
	if (!bin) {
		pr_info("aw_bin_parse bin is NULL\n");
		return -8;
	}
	...
	if (ret < 0) {
		DBG_ERR("aw_bin_parse parse header info error\n");
		return ret;
	}
	bin->p_addr = NULL;

	pr_info("bin->all_bin_parse_num is %d\n",
		bin->all_bin_parse_num);
	pr_info("bin->multi_bin_parse_num is %d\n",
		bin->multi_bin_parse_num);
	pr_info("bin->single_bin_parse_num is %d\n",
		bin->single_bin_parse_num);
