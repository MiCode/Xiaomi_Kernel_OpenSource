				...
				int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6785_mck_enable(afe, tdm_priv->bck_id, tdm_priv->bck_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt6785_mck_disable(afe, tdm_priv->bck_id);
		break;
	default:
		break;
	}
				...
				int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6785_mck_enable(afe, tdm_priv->mclk_id, tdm_priv->mclk_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		tdm_priv->mclk_rate = 0;
		mt6785_mck_disable(afe, tdm_priv->mclk_id);
		break;
	default:
		break;
	}
				    ...
				    struct snd_soc_dapm_widget *sink)
{
	...
	/* which apll */
	cur_apll = mt6785_get_apll_by_name(afe, source->name);
				...
				int freq)
{
	...
	apll = mt6785_get_apll_by_rate(afe, freq);
	apll_rate = mt6785_get_apll_rate(afe, apll);
...
int mt6785_dai_tdm_register(struct mtk_base_afe *afe)
{
	...
	tdm_priv->bck_id = MT6785_I2S4_BCK;
	tdm_priv->mclk_id = MT6785_I2S4_MCK;

	afe_priv->dai_priv[MT6785_DAI_TDM] = tdm_priv;
	return 0;
}
