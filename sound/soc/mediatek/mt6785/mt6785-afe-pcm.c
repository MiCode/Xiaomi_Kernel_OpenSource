...
static const struct snd_pcm_hardware mt6785_afe_hardware = {
...
static int mt6785_fe_startup(struct snd_pcm_substream *substream,
...
void mt6785_fe_shutdown(struct snd_pcm_substream *substream,
			struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
int mt6785_fe_trigger(struct snd_pcm_substream *substream, int cmd,
		      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
static int mt6785_memif_fs(struct snd_pcm_substream *substream,
			   unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	return mt6785_rate_transform(afe->dev, rate, id);
}

static int mt6785_get_dai_fs(struct mtk_base_afe *afe,
			     int dai_id, unsigned int rate)
{
	return mt6785_rate_transform(afe->dev, rate, dai_id);
}

static int mt6785_irq_fs(struct snd_pcm_substream *substream, unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);

	return mt6785_general_rate_transform(afe->dev, rate);
}

int mt6785_get_memif_pbuf_size(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;

	if ((runtime->period_size * 1000) / runtime->rate > 10)
		return MT6785_MEMIF_PBUF_SIZE_256_BYTES;
	else
		return MT6785_MEMIF_PBUF_SIZE_32_BYTES;
}


#if defined(CONFIG_SND_SOC_MTK_AUDIO_DSP)
int mt6785_fe_prepare(struct snd_pcm_substream *substream,
		      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_pcm_runtime * const runtime = substream->runtime;
...
/* FE DAIs */
static const struct snd_soc_dai_ops mt6785_memif_dai_ops = {
	.startup	= mt6785_fe_startup,
	.shutdown	= mt6785_fe_shutdown,
	.hw_params	= mtk_afe_fe_hw_params,
	.hw_free	= mtk_afe_fe_hw_free,
#if defined(CONFIG_SND_SOC_MTK_AUDIO_DSP)
	.prepare	= mt6785_fe_prepare,
#else
	.prepare	= mtk_afe_fe_prepare,
#endif
	.trigger	= mt6785_fe_trigger,
};
...
static struct snd_soc_dai_driver mt6785_memif_dai_driver[] = {
	/* FE DAIs: memory intefaces to CPU */
	{
		...
		.ops = &mt6785_memif_dai_ops,
	},
	{
		.name = "DL12",
		.id = MT6785_MEMIF_DL12,
		.playback = {
			.stream_name = "DL12",
			.channels_min = 1,
			.channels_max = 2,
...
/* kcontrol */
static int mt6785_irq_cnt1_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6785_PRIMARY_MEMIF];
	return 0;
}

static int mt6785_irq_cnt1_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6785_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6785_PRIMARY_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6785_irq_cnt2_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6785_RECORD_MEMIF];
	return 0;
}

static int mt6785_irq_cnt2_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6785_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6785_RECORD_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6785_deep_irq_cnt_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6785_DEEP_MEMIF];
	return 0;
}

static int mt6785_deep_irq_cnt_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6785_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6785_DEEP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6785_voip_rx_irq_cnt_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6785_VOIP_MEMIF];
	return 0;
}

static int mt6785_voip_rx_irq_cnt_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6785_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6785_VOIP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6785_deep_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_deep_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_fast_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_fast_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6785_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6785_FAST_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6785_primary_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_primary_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_voip_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_voip_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_record_xrun_assert_get(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6785_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6785_RECORD_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6785_record_xrun_assert_set(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	afe_priv->xrun_assert[MT6785_RECORD_MEMIF] = xrun_assert;
	return 0;
}

static int mt6785_echo_ref_xrun_assert_get(struct snd_kcontrol *kcontrol,
					   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6785_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6785_ECHO_REF_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6785_echo_ref_xrun_assert_set(struct snd_kcontrol *kcontrol,
					   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	dev_info(afe->dev, "%s(), xrun_assert %d\n", __func__, xrun_assert);
	afe_priv->xrun_assert[MT6785_ECHO_REF_MEMIF] = xrun_assert;
	return 0;
}

static int mt6785_sram_size_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
					  ...
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	int memif_num = MT6785_BARGEIN_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
#if defined(CONFIG_SND_SOC_MTK_AUDIO_DSP)
static int mt6785_adsp_primary_mem_get(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_primary_mem_set(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_deepbuffer_mem_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_deepbuffer_mem_set(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_voip_mem_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_voip_mem_set(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_playback_mem_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_playback_mem_set(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_call_final_mem_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_call_final_mem_set(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_ktv_mem_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_ktv_mem_set(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_offload_mem_get(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_offload_mem_set(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_capture_mem_get(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_capture_mem_set(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_ref_mem_set(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_a2dp_mem_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adsp_a2dp_mem_set(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_mmap_dl_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_mmap_dl_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6785_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6785_MMAP_DL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6785_mmap_ul_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_mmap_ul_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6785_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6785_MMAP_UL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6785_mmap_ion_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6785_mmap_ion_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	mtk_get_ion_buffer();
	return 0;
...
static int mt6785_dl_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_dl_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int mt6785_ul_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_ul_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static const struct snd_kcontrol_new mt6785_pcm_kcontrols[] = {
	SOC_SINGLE_EXT("Audio IRQ1 CNT", SND_SOC_NOPM, 0, 0x3ffff, 0,
		       mt6785_irq_cnt1_get, mt6785_irq_cnt1_set),
	SOC_SINGLE_EXT("Audio IRQ2 CNT", SND_SOC_NOPM, 0, 0x3ffff, 0,
		       mt6785_irq_cnt2_get, mt6785_irq_cnt2_set),
	SOC_SINGLE_EXT("deep_buffer_irq_cnt", SND_SOC_NOPM, 0, 0x3ffff, 0,
		       mt6785_deep_irq_cnt_get, mt6785_deep_irq_cnt_set),
	SOC_SINGLE_EXT("voip_rx_irq_cnt", SND_SOC_NOPM, 0, 0x3ffff, 0,
		       mt6785_voip_rx_irq_cnt_get, mt6785_voip_rx_irq_cnt_set),
	SOC_SINGLE_EXT("deep_buffer_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_deep_scene_get, mt6785_deep_scene_set),
	SOC_SINGLE_EXT("record_xrun_assert", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_record_xrun_assert_get,
		       mt6785_record_xrun_assert_set),
	SOC_SINGLE_EXT("echo_ref_xrun_assert", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_echo_ref_xrun_assert_get,
		       mt6785_echo_ref_xrun_assert_set),
	SOC_SINGLE_EXT("fast_play_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_fast_scene_get, mt6785_fast_scene_set),
	SOC_SINGLE_EXT("primary_play_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_primary_scene_get, mt6785_primary_scene_set),
	...
	SOC_SINGLE_EXT("adsp_primary_sharemem_scenario",
		       ...
		       mt6785_adsp_primary_mem_get,
		       mt6785_adsp_primary_mem_set),
	SOC_SINGLE_EXT("adsp_deepbuffer_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_adsp_deepbuffer_mem_get,
		       mt6785_adsp_deepbuffer_mem_set),
	SOC_SINGLE_EXT("adsp_voip_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_adsp_voip_mem_get,
		       mt6785_adsp_voip_mem_set),
	SOC_SINGLE_EXT("adsp_playback_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_adsp_playback_mem_get,
		       mt6785_adsp_playback_mem_set),
	SOC_SINGLE_EXT("adsp_call_final_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_adsp_call_final_mem_get,
		       mt6785_adsp_call_final_mem_set),
	SOC_SINGLE_EXT("adsp_ktv_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_adsp_ktv_mem_get,
		       mt6785_adsp_ktv_mem_set),
	SOC_SINGLE_EXT("adsp_offload_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_adsp_offload_mem_get,
		       mt6785_adsp_offload_mem_set),
	SOC_SINGLE_EXT("adsp_capture_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_adsp_capture_mem_get,
		       mt6785_adsp_capture_mem_set),
	...
	SOC_SINGLE_EXT("adsp_a2dp_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_adsp_a2dp_mem_get,
		       mt6785_adsp_a2dp_mem_set),
#endif
	...
	SOC_SINGLE_EXT("mmap_record_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6785_mmap_ul_scene_get, mt6785_mmap_ul_scene_set),
	...
	SOC_SINGLE_EXT("aaudio_dl_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6785_dl_mmap_fd_get,
		       mt6785_dl_mmap_fd_set),
	SOC_SINGLE_EXT("aaudio_ul_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6785_ul_mmap_fd_get,
		       mt6785_ul_mmap_fd_set),
};
...
static const struct snd_soc_dapm_widget mt6785_memif_widgets[] = {
	/* inter-connections */
...
static const struct snd_soc_dapm_route mt6785_memif_routes[] = {
	{"UL1", NULL, "UL1_CH1"},
	{"UL1", NULL, "UL1_CH2"},
	{"UL1", NULL, "UL1_CH3"},
	{"UL1", NULL, "UL1_CH4"},
...
static const struct mtk_base_irq_data irq_data[MT6785_IRQ_NUM] = {
	[MT6785_IRQ_0] = {
		.id = MT6785_IRQ_0,
...
static const int memif_irq_usage[MT6785_MEMIF_NUM] = {
	/* TODO: verify each memif & irq */
	[MT6785_MEMIF_DL1] = MT6785_IRQ_0,
	[MT6785_MEMIF_DL2] = MT6785_IRQ_1,
	[MT6785_MEMIF_DL3] = MT6785_IRQ_2,
	[MT6785_MEMIF_DL4] = MT6785_IRQ_3,
	[MT6785_MEMIF_DL5] = MT6785_IRQ_4,
	[MT6785_MEMIF_DL6] = MT6785_IRQ_5,
	[MT6785_MEMIF_DL7] = MT6785_IRQ_6,
	[MT6785_MEMIF_DL8] = MT6785_IRQ_7,
	[MT6785_MEMIF_DL12] = MT6785_IRQ_8,
	[MT6785_MEMIF_DAI] = MT6785_IRQ_9,
	[MT6785_MEMIF_MOD_DAI] = MT6785_IRQ_10,
	[MT6785_MEMIF_DAI2] = MT6785_IRQ_11,
	[MT6785_MEMIF_VUL12] = MT6785_IRQ_12,
	[MT6785_MEMIF_VUL2] = MT6785_IRQ_13,
	[MT6785_MEMIF_AWB] = MT6785_IRQ_14,
	[MT6785_MEMIF_AWB2] = MT6785_IRQ_15,
	[MT6785_MEMIF_VUL3] = MT6785_IRQ_16,
	[MT6785_MEMIF_VUL4] = MT6785_IRQ_17,
	[MT6785_MEMIF_VUL5] = MT6785_IRQ_18,
	[MT6785_MEMIF_VUL6] = MT6785_IRQ_19,
	[MT6785_MEMIF_HDMI] = MT6785_IRQ_31,
};

static bool mt6785_is_volatile_reg(struct device *dev, unsigned int reg)
{
...
static const struct regmap_config mt6785_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,

	.volatile_reg = mt6785_is_volatile_reg,
...
static irqreturn_t mt6785_afe_irq_handler(int irq_id, void *dev)
{
...
static int mt6785_afe_runtime_suspend(struct device *dev)
{
...
static int mt6785_afe_runtime_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	int ret;
...
static int mt6785_set_memif_sram_mode(struct device *dev,
...
static int mt6785_set_sram_mode(struct device *dev,
				enum mtk_audio_sram_mode sram_mode)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	/* set memif sram mode */
	mt6785_set_memif_sram_mode(dev, sram_mode);
...
static const struct mtk_audio_sram_ops mt6785_sram_ops = {
...
const struct snd_soc_platform_driver mt6785_afe_pcm_platform = {
	...
	.probe = mt6785_afe_pcm_platform_probe,
};

static ssize_t mt6785_debugfs_read(struct file *file, char __user *buf,
				    size_t count, loff_t *pos)
{
	...
	n += scnprintf(buffer + n, size - n,
		       "CLK_CFG_6 = 0x%x\n",
		       get_cksys_reg(CLK_CFG_6));
	n += scnprintf(buffer + n, size - n,
		       "CLK_AUDDIV_0 = 0x%x\n",
		       get_cksys_reg(CLK_AUDDIV_0));
	n += scnprintf(buffer + n, size - n,
		       "CLK_AUDDIV_1 = 0x%x\n",
		       get_cksys_reg(CLK_AUDDIV_1));
	n += scnprintf(buffer + n, size - n,
		       "CLK_AUDDIV_2 = 0x%x\n",
		       get_cksys_reg(CLK_AUDDIV_2));
	n += scnprintf(buffer + n, size - n,
		       "CLK_AUDDIV_3 = 0x%x\n",
		       get_cksys_reg(CLK_AUDDIV_3));

	n += scnprintf(buffer + n, size - n,
		       "AP_PLL_CON5 = 0x%x\n",
		       get_apmixed_reg(AP_PLL_CON5));
	n += scnprintf(buffer + n, size - n,
		       "APLL1_CON1 = 0x%x\n",
		       get_apmixed_reg(APLL1_CON1));
	n += scnprintf(buffer + n, size - n,
		       "APLL1_CON2 = 0x%x\n",
		       get_apmixed_reg(APLL1_CON2));
	n += scnprintf(buffer + n, size - n,
		       "APLL2_CON1 = 0x%x\n",
		       get_apmixed_reg(APLL2_CON1));
	n += scnprintf(buffer + n, size - n,
		       "APLL2_CON2 = 0x%x\n",
		       get_apmixed_reg(APLL2_CON2));
	...
	n += scnprintf(buffer + n, size - n,
		       "AFE_VUL_END = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_3RD_DAC_DL_SDM_AUTO_RESET_CON,
		    &value);
	n += scnprintf(buffer + n, size - n,
...
static const struct mtk_afe_debug_cmd mt6785_debug_cmds[] = {
	MTK_AFE_DBG_CMD("write_reg", mtk_afe_debug_write_reg),
	{}
};

static const struct file_operations mt6785_debugfs_ops = {
	.open = mtk_afe_debugfs_open,
	.write = mtk_afe_debugfs_write,
	.read = mt6785_debugfs_read,
};

static const struct snd_soc_component_driver mt6785_afe_pcm_component = {
	.name = "mt6785-afe-pcm-dai",
};

static int mt6785_dai_memif_register(struct mtk_base_afe *afe)
{
	...
	dai->dai_drivers = mt6785_memif_dai_driver;
	dai->num_dai_drivers = ARRAY_SIZE(mt6785_memif_dai_driver);

	dai->controls = mt6785_pcm_kcontrols;
	dai->num_controls = ARRAY_SIZE(mt6785_pcm_kcontrols);
	dai->dapm_widgets = mt6785_memif_widgets;
	dai->num_dapm_widgets = ARRAY_SIZE(mt6785_memif_widgets);
	dai->dapm_routes = mt6785_memif_routes;
	dai->num_dapm_routes = ARRAY_SIZE(mt6785_memif_routes);
	return 0;
}

typedef int (*dai_register_cb)(struct mtk_base_afe *);
static const dai_register_cb dai_register_cbs[] = {
	mt6785_dai_adda_register,
	mt6785_dai_i2s_register,
	mt6785_dai_hw_gain_register,
	mt6785_dai_src_register,
	mt6785_dai_pcm_register,
	mt6785_dai_tdm_register,
	mt6785_dai_hostless_register,
	mt6785_dai_memif_register,
};

static int mt6785_afe_pcm_dev_probe(struct platform_device *pdev)
{
	int ret, i;
#if !defined(CONFIG_FPGA_EARLY_PORTING) || defined(FORCE_FPGA_ENABLE_IRQ)
	int irq_id;
	...
	struct mt6785_afe_private *afe_priv;
	struct resource *res;
	struct device *dev;
	...
	ret = mtk_audio_sram_init(dev, afe->sram, &mt6785_sram_ops);
	if (ret)
		return ret;

	/* init memif */
	afe->memif_size = MT6785_MEMIF_NUM;
	...
	afe->memif[MT6785_DEEP_MEMIF].ack = mtk_sp_clean_written_buffer_ack;

	mutex_init(&afe->irq_alloc_lock);	/* needed when dynamic irq */

	/* init irq */
	afe->irqs_size = MT6785_IRQ_NUM;
	...
	/* others */
	afe->mtk_afe_hardware = &mt6785_afe_hardware;
	afe->memif_fs = mt6785_memif_fs;
	afe->irq_fs = mt6785_irq_fs;
	afe->get_dai_fs = mt6785_get_dai_fs;
	afe->get_memif_pbuf_size = mt6785_get_memif_pbuf_size;

	afe->runtime_resume = mt6785_afe_runtime_resume;
	afe->runtime_suspend = mt6785_afe_runtime_suspend;

	afe->request_dram_resource = mt6785_afe_dram_request;
	afe->release_dram_resource = mt6785_afe_dram_release;

	/* debugfs */
	afe->debug_cmds = mt6785_debug_cmds;
...
static int mt6785_afe_pcm_dev_remove(struct platform_device *pdev)
{
	struct mtk_base_afe *afe = platform_get_drvdata(pdev);

	pm_runtime_disable(&pdev->dev);
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt6785_afe_runtime_suspend(&pdev->dev);
...
static const struct of_device_id mt6785_afe_pcm_dt_match[] = {
	{ .compatible = "mediatek,mt6785-sound", },
	{},
};
MODULE_DEVICE_TABLE(of, mt6785_afe_pcm_dt_match);

static int mt6785_afe_suspend(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	dev_info(afe->dev, "%s()\n", __func__);

	mt6785_afe_suspend_clock(afe);

	return 0;
}

static int mt6785_afe_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	dev_info(afe->dev, "%s()\n", __func__);

	mt6785_afe_resume_clock(afe);

	return 0;
}

static const struct dev_pm_ops mt6785_afe_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(mt6785_afe_suspend,
				mt6785_afe_resume)
	SET_RUNTIME_PM_OPS(mt6785_afe_runtime_suspend,
			   mt6785_afe_runtime_resume, NULL)
};

static struct platform_driver mt6785_afe_pcm_driver = {
	.driver = {
		   .name = "mt6785-audio",
		   .of_match_table = mt6785_afe_pcm_dt_match,
#ifdef CONFIG_PM
		   .pm = &mt6785_afe_pm_ops,
#endif
	},
	.probe = mt6785_afe_pcm_dev_probe,
	.remove = mt6785_afe_pcm_dev_remove,
};

module_platform_driver(mt6785_afe_pcm_driver);

MODULE_DESCRIPTION("Mediatek ALSA SoC AFE platform driver for 6785");
