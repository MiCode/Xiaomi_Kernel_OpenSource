...
/*
 *  mt6789-afe-clk.c  --  Mediatek 6885 afe clock ctrl
 *
...
int mt6789_afe_dram_request(struct device *dev) { return 0; }
int mt6789_afe_dram_release(struct device *dev) { return 0; }
int mt6789_apll1_enable(struct mtk_base_afe *afe) { return 0; }
void mt6789_apll1_disable(struct mtk_base_afe *afe) {}
int mt6789_apll2_enable(struct mtk_base_afe *afe) { return 0; }
void mt6789_apll2_disable(struct mtk_base_afe *afe) {}
int mt6789_get_apll_rate(struct mtk_base_afe *afe, int apll) { return 0; }
int mt6789_get_apll_by_rate(struct mtk_base_afe *afe, int rate) { return 0; }
int mt6789_get_apll_by_name(struct mtk_base_afe *afe, const char *name) { return 0; }
int mt6789_mck_enable(struct mtk_base_afe *afe, int mck_id, int rate) { return 0; }
void mt6789_mck_disable(struct mtk_base_afe *afe, int mck_id) {}
int mt6789_set_audio_int_bus_parent(struct mtk_base_afe *afe, int clk_id) { return 0; }
#else
...
int mt6789_set_audio_int_bus_parent(struct mtk_base_afe *afe,
				    int clk_id)
{
...
int mt6789_afe_enable_clock(struct mtk_base_afe *afe)
{
	struct mt6789_afe_private *afe_priv = afe->platform_priv;
	int ret = 0;
	...
	ret = mt6789_set_audio_int_bus_parent(afe, CLK_CLK26M);
...
void mt6789_afe_disable_clock(struct mtk_base_afe *afe)
{
	...
	mt6789_set_audio_int_bus_parent(afe, CLK_CLK26M);
	clk_disable_unprepare(afe_priv->clk[CLK_MUX_AUDIOINTBUS]);
	clk_disable_unprepare(afe_priv->clk[CLK_MUX_AUDIO]);
	clk_disable_unprepare(afe_priv->clk[CLK_INFRA_AUDIO_26M]);
	clk_disable_unprepare(afe_priv->clk[CLK_INFRA_SYS_AUDIO]);
...
int mt6789_afe_dram_request(struct device *dev)
{
...
int mt6789_afe_dram_release(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	struct mt6789_afe_private *afe_priv = afe->platform_priv;
#if !defined(SKIP_SB_SMC)
...
int mt6789_apll1_enable(struct mtk_base_afe *afe)
{
	struct mt6789_afe_private *afe_priv = afe->platform_priv;
	int ret;
...
int mt6789_apll2_enable(struct mtk_base_afe *afe)
{
	struct mt6789_afe_private *afe_priv = afe->platform_priv;
	int ret;
...
int mt6789_get_apll_rate(struct mtk_base_afe *afe, int apll)
{
	return (apll == MT6789_APLL1) ? 180633600 : 196608000;
}

int mt6789_get_apll_by_rate(struct mtk_base_afe *afe, int rate)
{
	return ((rate % 8000) == 0) ? MT6789_APLL2 : MT6789_APLL1;
}

int mt6789_get_apll_by_name(struct mtk_base_afe *afe, const char *name)
{
...
static const struct mt6789_mck_div mck_div[MT6789_MCK_NUM] = {
	[MT6789_I2S0_MCK] = {
...
int mt6789_mck_enable(struct mtk_base_afe *afe, int mck_id, int rate)
{
	struct mt6789_afe_private *afe_priv = afe->platform_priv;
