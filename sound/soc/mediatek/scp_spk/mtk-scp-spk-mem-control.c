...
int mtk_scp_spk_allocate_tcm_iv_buf(void)
{
	...
	if (ret < 0) {
		dev_err(scp_spk->dev, "%s(), send error:0%d\n",
			__func__, ret);
		return -1;
	}
	...
	if (ipi_msg.param1) {
		scp_spk_iv_tcm_buf.phy_addr =
			SCP_TCM_PHY_ADDR +
			(ipi_msg.param1 & 0xFFFFFFF0) + 0x10;
		scp_spk_iv_tcm_buf.vir_addr =
			(unsigned char *)(SCP_TCM +
					  (ipi_msg.param1 & 0xFFFFFFF0) +
					  0x10);
		scp_spk_iv_tcm_buf.size = ipi_msg.param2;

		dev_info(scp_spk->dev,
			 "%s(), spk tcm iv addr:0x%x, area:0x%x, bytes:%d, SCP_TCM:0x%x\n",
			 __func__,
			 scp_spk_iv_tcm_buf.phy_addr,
			 scp_spk_iv_tcm_buf.vir_addr,
			 scp_spk_iv_tcm_buf.size, SCP_TCM);
