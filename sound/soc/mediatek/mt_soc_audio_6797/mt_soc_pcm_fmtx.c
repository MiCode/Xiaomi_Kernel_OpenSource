	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) { /* copy once */
			if (!access_ok(VERIFY_READ, data_w_ptr, copy_size)) {
				PRINTK_AUD_FMTX("[mtk_pcm_fmtx_copy] 0ptr invalid data_w_ptr=0x%x, size=%d",
						(kal_uint32)data_w_ptr, copy_size);
				PRINTK_AUD_FMTX("[mtk_pcm_fmtx_copy] u4BufferSize=%d, u4DataRemained=%d",
						Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
			} else {
				PRINTK_AUD_FMTX("memcpy Idx= %p data_w_ptr = %p copy_size = 0x%x\n",
			...
			PRINTK_AUD_FMTX("[%s] finish1, size:%x, WriteIdx:%x, ReadIdx=%x, Remained:%x, count=%x \r\n",
			...
			if (!access_ok(VERIFY_READ, data_w_ptr, size_1)) {
				pr_warn("[mtk_pcm_fmtx_copy] 1ptr invalid data_w_ptr=%p, size_1=%d", data_w_ptr,
				       size_1);
				pr_warn("[mtk_pcm_fmtx_copy] u4BufferSize=%d, u4DataRemained=%d",
				       Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
			} else {

				PRINTK_AUD_FMTX("[%s]mcmcpy dx= %x data_w_ptr = %p size_1 = %x\n",
						__func__,
			...
			if (!access_ok(VERIFY_READ, data_w_ptr + size_1, size_2)) {
				PRINTK_AUD_FMTX("[mtk_pcm_fmtx_copy] 2ptr invalid data_w_ptr=%x, size_1=%d, size_2=%d",
						(kal_uint32)data_w_ptr, size_1, size_2);
				PRINTK_AUD_FMTX("[mtk_pcm_fmtx_copy] u4BufferSize=%d, u4DataRemained=%d",
						Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
			} else {

				PRINTK_AUD_FMTX("[%s]mcmcpy Idx= %x data_w_ptr+size_1 = %p size_2 = %x\n",
			...
			PRINTK_AUD_FMTX("[%s] finish2, copy size:%x, WriteIdx:%x,ReadIdx=%x Remained:%x \r\n",
					__func__, copy_size, Afe_Block->u4WriteIdx, Afe_Block->u4DMAReadIdx,
