...
/*
 * mt6855-afe-clk.h  --  Mediatek 6855 afe clock ctrl definition
 *
 * Copyright (c) 2021 MediaTek Inc.
 *  Author: Tina Tsai <tina.tsai@mediatek.com>
 */

#ifndef _MT6855_AFE_CLOCK_CTRL_H_
#define _MT6855_AFE_CLOCK_CTRL_H_
...
int mt6855_init_clock(struct mtk_base_afe *afe);
int mt6855_afe_enable_clock(struct mtk_base_afe *afe);
void mt6855_afe_disable_clock(struct mtk_base_afe *afe);

int mt6855_afe_dram_request(struct device *dev);
int mt6855_afe_dram_release(struct device *dev);

int mt6855_apll1_enable(struct mtk_base_afe *afe);
void mt6855_apll1_disable(struct mtk_base_afe *afe);

int mt6855_apll2_enable(struct mtk_base_afe *afe);
void mt6855_apll2_disable(struct mtk_base_afe *afe);

int mt6855_get_apll_rate(struct mtk_base_afe *afe, int apll);
int mt6855_get_apll_by_rate(struct mtk_base_afe *afe, int rate);
int mt6855_get_apll_by_name(struct mtk_base_afe *afe, const char *name);

extern void aud_intbus_mux_sel(unsigned int aud_idx);

/* these will be replaced by using CCF */
int mt6855_mck_enable(struct mtk_base_afe *afe, int mck_id, int rate);
void mt6855_mck_disable(struct mtk_base_afe *afe, int mck_id);
