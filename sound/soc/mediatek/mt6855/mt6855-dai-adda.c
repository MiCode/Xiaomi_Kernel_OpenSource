...
static int mtk_adda_ul_src_dmic(struct mtk_base_afe *afe, int id)
{
	...
	switch (id) {
	case MT6855_DAI_ADDA:
	case MT6855_DAI_AP_DMIC:
			     ...
			     int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6855_afe_gpio_request(afe, true, MT6855_DAI_ADDA, 1);
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap, AFE_ADDA_MTKAIF_RX_CFG0,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6855_DAI_ADDA);
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6855_afe_gpio_request(afe, false, MT6855_DAI_ADDA, 1);
...
static bool is_adda_mtkaif_need_phase_delay(struct mt6855_afe_private *afe_priv)
{
	if (mt6855_afe_gpio_is_prepared(MT6855_AFE_GPIO_DAT_MISO0_ON) &&
	    afe_priv->mtkaif_chosen_phase[0] < 0) {
		AUDIO_AEE("adda mtkaif miso0 calib fail");
		return false;
	}

	if (mt6855_afe_gpio_is_prepared(MT6855_AFE_GPIO_DAT_MISO1_ON) &&
			     ...
			     int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6855_afe_gpio_request(afe, true, MT6855_DAI_ADDA, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6855_afe_gpio_request(afe, false, MT6855_DAI_ADDA, 0);
		break;
	default:
		break;
	}
...
/* mtkaif dmic */
static const char *const mt6855_adda_off_on_str[] = {
	"Off", "On"
};

static const struct soc_enum mt6855_adda_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6855_adda_off_on_str),
			    mt6855_adda_off_on_str),
};

static int mt6855_adda_dmic_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6855_adda_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6855_adda6_only_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6855_adda6_only_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_adda_controls[] = {
	...
	SOC_ENUM_EXT("MTKAIF_DMIC", mt6855_adda_enum[0],
		     mt6855_adda_dmic_get, mt6855_adda_dmic_set),
	SOC_ENUM_EXT("MTKAIF_ADDA6_ONLY", mt6855_adda_enum[0],
		     mt6855_adda6_only_get, mt6855_adda6_only_set),
};
...
int mt6855_dai_adda_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6855_dai_set_priv(afe, MT6855_DAI_ADDA,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	/* ap dmic priv share with adda */
	afe_priv->dai_priv[MT6855_DAI_AP_DMIC] =
		afe_priv->dai_priv[MT6855_DAI_ADDA];
