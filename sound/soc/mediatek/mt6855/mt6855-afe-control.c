...
{
	switch (aud_blk) {
	case MT6855_MEMIF_DAI:
	case MT6855_MEMIF_MOD_DAI:
		return dai_memif_rate_transform(dev, rate);
	case MT6855_DAI_PCM_1:
	case MT6855_DAI_PCM_2:
		return pcm_rate_transform(dev, rate);
	default:
		return mt6855_general_rate_transform(dev, rate);
	}
}
...
EXPORT_SYMBOL(mt6855_enable_dc_compensation);
...
EXPORT_SYMBOL(mt6855_set_lch_dc_compensation);
...
EXPORT_SYMBOL(mt6855_set_rch_dc_compensation);
...
EXPORT_SYMBOL(mt6855_adda_dl_gain_control);
...
EXPORT_SYMBOL(mt6855_set_adda_predistortion);
