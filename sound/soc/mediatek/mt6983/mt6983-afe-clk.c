...
/*
 *  mt6983-afe-clk.c  --  Mediatek 6983 afe clock ctrl
 *
...
int mt6983_init_clock(struct mtk_base_afe *afe) { return 0; }
int mt6983_afe_enable_clock(struct mtk_base_afe *afe) { return 0; }
void mt6983_afe_disable_clock(struct mtk_base_afe *afe) {}
int mt6983_afe_dram_request(struct device *dev) { return 0; }
int mt6983_afe_dram_release(struct device *dev) { return 0; }
int mt6983_apll1_enable(struct mtk_base_afe *afe) { return 0; }
void mt6983_apll1_disable(struct mtk_base_afe *afe) {}
int mt6983_apll2_enable(struct mtk_base_afe *afe) { return 0; }
void mt6983_apll2_disable(struct mtk_base_afe *afe) {}
int mt6983_get_apll_rate(struct mtk_base_afe *afe, int apll) { return 0; }
int mt6983_get_apll_by_rate(struct mtk_base_afe *afe, int rate) { return 0; }
int mt6983_get_apll_by_name(struct mtk_base_afe *afe, const char *name) { return 0; }
int mt6983_mck_enable(struct mtk_base_afe *afe, int mck_id, int rate) { return 0; }
void mt6983_mck_disable(struct mtk_base_afe *afe, int mck_id) {}
int mt6983_set_audio_int_bus_parent(struct mtk_base_afe *afe, int clk_id) { return 0; }
#else
...
int mt6983_set_audio_int_bus_parent(struct mtk_base_afe *afe,
				    int clk_id)
{
	...
	if (clk_id < 0) {
		dev_err(afe->dev, "%s clk id = %d is negative value\n",
			__func__, clk_id);
		return -EINVAL;
	}
...
int mt6983_afe_enable_clock(struct mtk_base_afe *afe)
{
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int ret = 0;
	...
	ret = mt6983_set_audio_int_bus_parent(afe, CLK_CLK26M);
...
int mt6983_afe_dram_release(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	struct arm_smccc_res res;
...
int mt6983_apll1_enable(struct mtk_base_afe *afe)
{
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int ret;
...
int mt6983_apll2_enable(struct mtk_base_afe *afe)
{
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int ret;
...
int mt6983_get_apll_rate(struct mtk_base_afe *afe, int apll)
{
	return (apll == MT6983_APLL1) ? 180633600 : 196608000;
}

int mt6983_get_apll_by_rate(struct mtk_base_afe *afe, int rate)
{
	return ((rate % 8000) == 0) ? MT6983_APLL2 : MT6983_APLL1;
}

int mt6983_get_apll_by_name(struct mtk_base_afe *afe, const char *name)
{
...
static const struct mt6983_mck_div mck_div[MT6983_MCK_NUM] = {
	[MT6983_I2S0_MCK] = {
...
{
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int apll = mt6983_get_apll_by_rate(afe, rate);
	int apll_clk_id = apll == MT6983_APLL1 ?
	...
	if (mck_id < 0) {
		dev_err(afe->dev, "%s mck id = %d is negative value\n",
			__func__, mck_id);
		return -EINVAL;
	}
...
void mt6983_mck_disable(struct mtk_base_afe *afe, int mck_id)
{
	...
	if (mck_id == MT6983_I2S5_MCK) /* i2s5 */
		div_clk_id = mck_div[mck_id].div_msb_clk_id;
	else
		div_clk_id = mck_div[mck_id].div_clk_id;
	...
	if (m_sel_id < 0) {
		dev_err(afe->dev, "%s(), m_sel_id = %d < 0\n",
				__func__, m_sel_id);
		return;
	}
...
int mt6983_init_clock(struct mtk_base_afe *afe)
{
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int i = 0;
