...
static int mt6983_mt6338_headset_init(struct snd_soc_component *component)
{
	struct snd_soc_card *card = &mt6983_mt6338_soc_card;

	return mt6338_accdet_init(component, card);
}

static struct snd_soc_aux_dev mt6983_mt6338_headset_dev = {
	.dlc = COMP_EMPTY(),
	.init = mt6983_mt6338_headset_init,
};
#endif
static int mt6983_mt6338_dev_probe(struct platform_device *pdev)
{
	...
	card->dev = &pdev->dev;
#if IS_ENABLED(CONFIG_SND_SOC_MT6338_ACCDET)
	mt6983_mt6338_headset_dev.dlc.of_node =
		of_parse_phandle(pdev->dev.of_node,
				"mediatek,headset-codec", 0);
	if (mt6983_mt6338_headset_dev.dlc.of_node) {
		card->aux_dev = &mt6983_mt6338_headset_dev;
		card->num_aux_devs = 1;
	} else
		dev_err(&pdev->dev,
			"Property 'mediatek,headset-codec' missing/invalid\n");
	mt6338_accdet_set_drvdata(card);
#endif
