...
{
	if (strstr(name, "DPTX"))
		return MT6983_DAI_TDM_DPTX;
	else
		return MT6983_DAI_TDM;
}
				...
				int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6983_mck_enable(afe, tdm_priv->bck_id, tdm_priv->bck_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt6983_mck_disable(afe, tdm_priv->bck_id);
		break;
	default:
		break;
	}
				...
				int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6983_mck_enable(afe, tdm_priv->mclk_id, tdm_priv->mclk_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		tdm_priv->mclk_rate = 0;
		mt6983_mck_disable(afe, tdm_priv->mclk_id);
		break;
	default:
		break;
	}
				...
				int freq)
{
	...
	apll = mt6983_get_apll_by_rate(afe, freq);
	apll_rate = mt6983_get_apll_rate(afe, apll);
						   ...
						   int id)
{
	...
	if (id == MT6983_DAI_TDM_DPTX)
		tdm_priv->mclk_multiple = 256;
	else
		tdm_priv->mclk_multiple = 128;

	tdm_priv->bck_id = MT6983_I2S4_BCK;
	tdm_priv->mclk_id = MT6983_I2S4_MCK;
...
int mt6983_dai_tdm_register(struct mtk_base_afe *afe)
{
	...
	tdm_priv = init_tdm_priv_data(afe, MT6983_DAI_TDM);
	if (!tdm_priv)
		return -ENOMEM;

	tdm_dptx_priv = init_tdm_priv_data(afe, MT6983_DAI_TDM_DPTX);
	if (!tdm_dptx_priv)
		return -ENOMEM;

	afe_priv->dai_priv[MT6983_DAI_TDM] = tdm_priv;
	afe_priv->dai_priv[MT6983_DAI_TDM_DPTX] = tdm_dptx_priv;
