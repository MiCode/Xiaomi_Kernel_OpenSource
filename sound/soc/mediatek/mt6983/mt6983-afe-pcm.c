...
/*
 *  Mediatek ALSA SoC AFE platform driver for 6983
 *
...
#include "mt6983-afe-gpio.h"
#include "mt6983-interconnection.h"
...
static int mt6983_fe_startup(struct snd_pcm_substream *substream,
...
void mt6983_fe_shutdown(struct snd_pcm_substream *substream,
			struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
int mt6983_fe_trigger(struct snd_pcm_substream *substream, int cmd,
		      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
static int mt6983_memif_fs(struct snd_pcm_substream *substream,
			   unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	return mt6983_rate_transform(afe->dev, rate, id);
}

static int mt6983_get_dai_fs(struct mtk_base_afe *afe,
			     int dai_id, unsigned int rate)
{
	return mt6983_rate_transform(afe->dev, rate, dai_id);
}

static int mt6983_irq_fs(struct snd_pcm_substream *substream, unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	return mt6983_general_rate_transform(afe->dev, rate);
}

int mt6983_get_memif_pbuf_size(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;

	if ((runtime->period_size * 1000) / runtime->rate > 10)
		return MT6983_MEMIF_PBUF_SIZE_256_BYTES;
	else
		return MT6983_MEMIF_PBUF_SIZE_32_BYTES;
}

/* FE DAIs */
static const struct snd_soc_dai_ops mt6983_memif_dai_ops = {
	.startup	= mt6983_fe_startup,
	.shutdown	= mt6983_fe_shutdown,
	.hw_params	= mtk_afe_fe_hw_params,
	.hw_free	= mtk_afe_fe_hw_free,
	.prepare	= mtk_afe_fe_prepare,
	.trigger	= mt6983_fe_trigger,
};
...
static struct snd_soc_dai_driver mt6983_memif_dai_driver[] = {
	/* FE DAIs: memory intefaces to CPU */
	{
		...
		.ops = &mt6983_memif_dai_ops,
	},
	{
		.name = "DL12",
		.id = MT6983_MEMIF_DL12,
		.playback = {
			.stream_name = "DL12",
			.channels_min = 1,
			.channels_max = 2,
...
/* kcontrol */
static int mt6983_irq_cnt1_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6983_PRIMARY_MEMIF];
	return 0;
}

static int mt6983_irq_cnt1_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6983_PRIMARY_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6983_irq_cnt2_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6983_RECORD_MEMIF];
	return 0;
}

static int mt6983_irq_cnt2_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6983_RECORD_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6983_deep_irq_cnt_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6983_DEEP_MEMIF];
	return 0;
}

static int mt6983_deep_irq_cnt_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6983_DEEP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6983_voip_rx_irq_cnt_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6983_VOIP_MEMIF];
	return 0;
}

static int mt6983_voip_rx_irq_cnt_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6983_VOIP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6983_deep_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6983_deep_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6983_fast_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6983_fast_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6983_FAST_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6983_primary_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6983_primary_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6983_voip_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6983_voip_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6983_record_xrun_assert_get(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6983_RECORD_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6983_record_xrun_assert_set(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	afe_priv->xrun_assert[MT6983_RECORD_MEMIF] = xrun_assert;
	return 0;
}

static int mt6983_echo_ref_xrun_assert_get(struct snd_kcontrol *kcontrol,
					   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6983_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6983_ECHO_REF_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6983_echo_ref_xrun_assert_set(struct snd_kcontrol *kcontrol,
					   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	dev_info(afe->dev, "%s(), xrun_assert %d\n", __func__, xrun_assert);
	afe_priv->xrun_assert[MT6983_ECHO_REF_MEMIF] = xrun_assert;
	return 0;
}

static int mt6983_sram_size_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6983_vow_barge_in_irq_id_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	int memif_num = MT6983_BARGE_IN_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUDIO_DSP)
static int mt6983_adsp_ref_mem_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6983_adsp_ref_mem_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6983_adsp_mem_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6983_adsp_mem_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mt6983_pcm_kcontrols[] = {
	...
	SOC_SINGLE_EXT("adsp_ref_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6983_adsp_ref_mem_get,
		       mt6983_adsp_ref_mem_set),
			     ...
			     int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (strstr(w->name, "UL4") && channels > 2) {
			...
			mtk_set_cm(afe, CM2, rate, 0x7, false, channels);
			mtk_enable_cm(afe, CM2, 1);
		}
		break;
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (mux == CM1_MUX_VUL9_8CH || mux == CM1_MUX_VUL9_2CH) {
			...
			if (mux == CM1_MUX_VUL9_8CH)
				mtk_enable_cm_bypass(afe, CM1, 0x0, 0x0);//cm, cm
		...
		mtk_set_cm(afe, CM1, rate, 0x7, false, channels);
		mtk_enable_cm(afe, CM1, 1);
		break;
	case SND_SOC_DAPM_PRE_PMD:
...
static const struct snd_kcontrol_new memif_ul1_ch3_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("ADDA_UL_CH3", AFE_CONN9,
				    I_ADDA_UL_CH3, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("ETDM_CH1", AFE_CONN9_1,
				    I_ETDM_IN_CH1, 1, 0),
};
...
static const struct snd_kcontrol_new memif_ul1_ch4_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("ADDA_UL_CH4", AFE_CONN10,
				    I_ADDA_UL_CH4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("ETDM_CH2", AFE_CONN10_1,
				    I_ETDM_IN_CH2, 1, 0),
};
...
static const struct snd_kcontrol_new memif_ul2_ch1_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("DL11_CH4", AFE_CONN5_2,
				    I_DL11_CH4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("DL11_CH5", AFE_CONN5_2,
				    I_DL11_CH5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("DL11_CH6", AFE_CONN5_2,
				    I_DL11_CH6, 1, 0),
	...
	SOC_DAPM_SINGLE_AUTODISABLE("SRC_1_OUT_CH1", AFE_CONN5_1,
				    I_SRC_1_OUT_CH1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("HW_SRC_3_OUT_CH1", AFE_CONN5_2,
				    I_SRC_3_OUT_CH1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("GAIN1_OUT_CH1", AFE_CONN5,
				    I_GAIN1_OUT_CH1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("ETDM_CH1", AFE_CONN5_1,
				    I_ETDM_IN_CH1, 1, 0),
};
...
static const struct snd_kcontrol_new memif_ul2_ch2_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("DL11_CH4", AFE_CONN6_2,
				    I_DL11_CH4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("DL11_CH5", AFE_CONN6_2,
				    I_DL11_CH5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("DL11_CH6", AFE_CONN6_2,
				    I_DL11_CH6, 1, 0),
	...
	SOC_DAPM_SINGLE_AUTODISABLE("SRC_1_OUT_CH2", AFE_CONN6_1,
				    I_SRC_1_OUT_CH2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("HW_SRC_3_OUT_CH2", AFE_CONN6_2,
				    I_SRC_3_OUT_CH2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("GAIN1_OUT_CH2", AFE_CONN6,
				    I_GAIN1_OUT_CH2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("ETDM_CH2", AFE_CONN6_1,
				    I_ETDM_IN_CH2, 1, 0),
};
...
static const struct snd_kcontrol_new memif_ul5_ch1_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("ADDA_UL_CH1", AFE_CONN44,
				    I_ADDA_UL_CH1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("ADDA_UL_CH2", AFE_CONN44,
				    I_ADDA_UL_CH2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("ADDA_UL_CH3", AFE_CONN44,
				    I_ADDA_UL_CH3, 1, 0),
};

static const struct snd_kcontrol_new memif_ul5_ch2_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("ADDA_UL_CH1", AFE_CONN45,
				    I_ADDA_UL_CH1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("ADDA_UL_CH2", AFE_CONN45,
				    I_ADDA_UL_CH2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("ADDA_UL_CH3", AFE_CONN45,
				    I_ADDA_UL_CH3, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("ADDA_UL_CH4", AFE_CONN45,
				    I_ADDA_UL_CH4, 1, 0),
};
...
static const struct snd_soc_dapm_widget mt6983_memif_widgets[] = {
	/* inter-connections */
...
static const struct snd_soc_dapm_route mt6983_memif_routes[] = {
	{"UL1", NULL, "UL1_CH1"},
	{"UL1", NULL, "UL1_CH2"},
	{"UL1", NULL, "UL1_CH3"},
	{"UL1", NULL, "UL1_CH4"},
	...
	{"UL1_CH1", "ADDA_UL_CH3", "ADDA_UL_Mux"},
	...
	{"UL1_CH3", "ADDA_UL_CH3", "ADDA_UL_Mux"},
	...
	{"UL1_CH1", "ETDM_CH3", "ETDM Capture"},
	{"UL1_CH2", "ETDM_CH4", "ETDM Capture"},
	{"UL1_CH3", "ETDM_CH1", "ETDM Capture"},
	{"UL1_CH4", "ETDM_CH2", "ETDM Capture"},
	...
	{"UL2_CH1", "DL11_CH5", "Hostless_UL2 UL"},
	{"UL2_CH1", "DL11_CH6", "Hostless_UL2 UL"},
	...
	{"UL2_CH2", "DL11_CH5", "Hostless_UL2 UL"},
	{"UL2_CH2", "DL11_CH6", "Hostless_UL2 UL"},
	...
	{"UL2_CH1", "ETDM_CH1", "ETDM Capture"},
	{"UL2_CH2", "ETDM_CH2", "ETDM Capture"},
	...
	{"UL2_CH1", "HW_SRC_3_OUT_CH1", "HW_SRC_3_Out"},
	{"UL2_CH2", "HW_SRC_3_OUT_CH2", "HW_SRC_3_Out"},
	...
	{"UL5_CH1", "ADDA_UL_CH2", "ADDA_UL_Mux"},
	{"UL5_CH1", "ADDA_UL_CH3", "ADDA_CH34_UL_Mux"},
	{"UL5_CH2", "ADDA_UL_CH1", "ADDA_UL_Mux"},
	{"UL5_CH2", "ADDA_UL_CH2", "ADDA_UL_Mux"},
	{"UL5_CH2", "ADDA_UL_CH3", "ADDA_CH34_UL_Mux"},
	{"UL5_CH2", "ADDA_UL_CH4", "ADDA_CH34_UL_Mux"},
...
static const struct mtk_base_memif_data memif_data[MT6983_MEMIF_NUM] = {
	[MT6983_MEMIF_DL1] = {
		.name = "DL1",
		.id = MT6983_MEMIF_DL1,
...
static const struct mtk_base_irq_data irq_data[MT6983_IRQ_NUM] = {
	[MT6983_IRQ_0] = {
		.id = MT6983_IRQ_0,
...
static const int memif_irq_usage[MT6983_MEMIF_NUM] = {
	/* TODO: verify each memif & irq */
	[MT6983_MEMIF_DL1] = MT6983_IRQ_0,
	[MT6983_MEMIF_DL2] = MT6983_IRQ_1,
	[MT6983_MEMIF_DL3] = MT6983_IRQ_2,
	[MT6983_MEMIF_DL4] = MT6983_IRQ_3,
	[MT6983_MEMIF_DL5] = MT6983_IRQ_4,
	[MT6983_MEMIF_DL6] = MT6983_IRQ_5,
	[MT6983_MEMIF_DL7] = MT6983_IRQ_6,
	[MT6983_MEMIF_DL8] = MT6983_IRQ_7,
	[MT6983_MEMIF_DL9] = MT6983_IRQ_8,
	[MT6983_MEMIF_DL12] = MT6983_IRQ_9,
	[MT6983_MEMIF_DAI] = MT6983_IRQ_10,
	[MT6983_MEMIF_MOD_DAI] = MT6983_IRQ_11,
	[MT6983_MEMIF_DAI2] = MT6983_IRQ_12,
	[MT6983_MEMIF_VUL12] = MT6983_IRQ_13,
	[MT6983_MEMIF_VUL2] = MT6983_IRQ_14,
	[MT6983_MEMIF_AWB] = MT6983_IRQ_15,
	[MT6983_MEMIF_AWB2] = MT6983_IRQ_16,
	[MT6983_MEMIF_VUL3] = MT6983_IRQ_17,
	[MT6983_MEMIF_VUL4] = MT6983_IRQ_18,
	[MT6983_MEMIF_VUL5] = MT6983_IRQ_19,
	[MT6983_MEMIF_VUL6] = MT6983_IRQ_20,
	[MT6983_MEMIF_HDMI] = MT6983_IRQ_31,
	[MT6983_MEMIF_DL11] = MT6983_IRQ_24,
	[MT6983_MEMIF_VUL8] = MT6983_IRQ_22,
	[MT6983_MEMIF_VUL9] = MT6983_IRQ_23,
};

static bool mt6983_is_volatile_reg(struct device *dev, unsigned int reg)
{
...
static const struct regmap_config mt6983_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,

	.volatile_reg = mt6983_is_volatile_reg,
...
static irqreturn_t mt6983_afe_irq_handler(int irq_id, void *dev)
{
...
static int mt6983_afe_runtime_suspend(struct device *dev)
{
...
static int mt6983_afe_runtime_resume(struct device *dev)
{
	...
	ret = mt6983_afe_enable_clock(afe);
	if (ret)
		return ret;
...
static int mt6983_afe_pcm_copy(struct snd_pcm_substream *substream,
			       ...
			       mtk_sp_copy_f sp_copy)
{
	...
	mt6983_set_audio_int_bus_parent(afe, CLK_TOP_MAINPLL_D4_D4);
...
static int mt6983_set_memif_sram_mode(struct device *dev,
...
static int mt6983_set_sram_mode(struct device *dev,
				enum mtk_audio_sram_mode sram_mode)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	/* set memif sram mode */
	mt6983_set_memif_sram_mode(dev, sram_mode);
...
static const struct mtk_audio_sram_ops mt6983_sram_ops = {
...
static const struct snd_soc_component_driver mt6983_afe_component = {
	.name = AFE_PCM_NAME,
...
static ssize_t mt6983_debug_read_reg(char *buffer, int size, struct mtk_base_afe *afe)
{
	...
	n += scnprintf(buffer + n, size - n,
			   "AFE_VUL_END = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_3RD_DAC_DL_SDM_AUTO_RESET_CON, &value);
	n += scnprintf(buffer + n, size - n,
	...
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_02_01, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_02_01 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_04_03, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_04_03 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_06_05, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_06_05 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_08_07, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_08_07 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_10_09, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_10_09 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_12_11, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_12_11 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_14_13, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_14_13 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_16_15, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_16_15 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_18_17, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_18_17 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_20_19, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_20_19 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_22_21, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_22_21 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_24_23, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_24_23 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_26_25, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_26_25 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_28_27, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_28_27 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA_ULCF_CFG_30_29, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA_ULCF_CFG_30_29 = 0x%x\n", value);
	...
	regmap_read(afe->regmap, AFE_BT_SECURITY0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_BT_SECURITY0 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_BT_SECURITY1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_BT_SECURITY1 = 0x%x\n", value);
	...
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN57 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN58, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN58 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN59, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN59 = 0x%x\n", value);
	...
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN57_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN58_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN58_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN59_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN59_1 = 0x%x\n", value);
	...
	regmap_read(afe->regmap, AFE_ADDA6_UL_SRC_MON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA6_UL_SRC_MON0 = 0x%x\n", value);
	...
	n += scnprintf(buffer + n, size - n,
			   "FPGA_CFG4 = 0x%x\n", value);
	regmap_read(afe->regmap, FPGA_CFG5, &value);
	n += scnprintf(buffer + n, size - n,
			   "FPGA_CFG5 = 0x%x\n", value);
	regmap_read(afe->regmap, FPGA_CFG6, &value);
	n += scnprintf(buffer + n, size - n,
			   "FPGA_CFG6 = 0x%x\n", value);
	regmap_read(afe->regmap, FPGA_CFG7, &value);
	n += scnprintf(buffer + n, size - n,
			   "FPGA_CFG7 = 0x%x\n", value);
	regmap_read(afe->regmap, FPGA_CFG8, &value);
	n += scnprintf(buffer + n, size - n,
			   "FPGA_CFG8 = 0x%x\n", value);
	regmap_read(afe->regmap, FPGA_CFG9, &value);
	n += scnprintf(buffer + n, size - n,
			   "FPGA_CFG9 = 0x%x\n", value);
	regmap_read(afe->regmap, FPGA_CFG10, &value);
	n += scnprintf(buffer + n, size - n,
			   "FPGA_CFG10 = 0x%x\n", value);
	regmap_read(afe->regmap, FPGA_CFG11, &value);
	n += scnprintf(buffer + n, size - n,
			   "FPGA_CFG11 = 0x%x\n", value);
	regmap_read(afe->regmap, FPGA_CFG12, &value);
	n += scnprintf(buffer + n, size - n,
			   "FPGA_CFG12 = 0x%x\n", value);
	regmap_read(afe->regmap, FPGA_CFG13, &value);
	n += scnprintf(buffer + n, size - n,
			   "FPGA_CFG13 = 0x%x\n", value);
	...
	regmap_read(afe->regmap, AFE_DL10_CON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL10_CON0 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_DL10_BASE_MSB, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL10_BASE_MSB = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_DL10_BASE, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL10_BASE = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_DL10_CUR_MSB, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL10_CUR_MSB = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_DL10_CUR, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL10_CUR = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_DL10_END_MSB, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL10_END_MSB = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_DL10_END, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL10_END = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_DL10_RCH_MON, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL10_RCH_MON = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_DL10_LCH_MON, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL10_LCH_MON = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_VUL7_2_CON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_VUL7_2_CON0 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_VUL7_2_BASE_MSB, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_VUL7_2_BASE_MSB = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_VUL7_2_BASE, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_VUL7_2_BASE = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_VUL7_2_CUR_MSB, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_VUL7_2_CUR_MSB = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_VUL7_2_CUR, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_VUL7_2_CUR = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_VUL7_2_END_MSB, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_VUL7_2_END_MSB = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_VUL7_2_END, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_VUL7_2_END = 0x%x\n", value);
	...
	n += scnprintf(buffer + n, size - n,
			   "AFE_CM2_MON = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON0 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON3, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON3 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON4, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON4 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON5, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON5 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON6, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON6 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON7, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON7 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON8, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON8 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON9, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON9 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON10, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON10 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON12, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON12 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GENERAL3_ASRC_2CH_CON13, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GENERAL3_ASRC_2CH_CON13 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_TOP_CON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_TOP_CON0 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_UL_SRC_CON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_UL_SRC_CON0 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_UL_SRC_CON1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_UL_SRC_CON1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_SRC_DEBUG, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_SRC_DEBUG = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_SRC_DEBUG_MON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_SRC_DEBUG_MON0 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_UL_SRC_MON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_UL_SRC_MON0 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_UL_SRC_MON1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_UL_SRC_MON1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_PROXIMITY_CON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_PROXIMITY_CON0 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_02_01, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_02_01 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_04_03, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_04_03 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_06_05, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_06_05 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_08_07, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_08_07 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_10_09, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_10_09 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_12_11, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_12_11 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_14_13, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_14_13 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_16_15, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_16_15 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_18_17, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_18_17 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_20_19, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_20_19 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_22_21, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_22_21 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_24_23, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_24_23 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_26_25, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_26_25 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_28_27, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_28_27 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_ULCF_CFG_30_29, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_ULCF_CFG_30_29 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_IIR_COEF_02_01, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_IIR_COEF_02_01 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_IIR_COEF_04_03, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_IIR_COEF_04_03 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_IIR_COEF_06_05, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_IIR_COEF_06_05 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_IIR_COEF_08_07, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_IIR_COEF_08_07 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_ADDA7_IIR_COEF_10_09, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_ADDA7_IIR_COEF_10_09 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN80_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN80_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN81_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN81_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN82_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN82_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN83_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN83_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN60, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN60 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN61, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN61 = 0x%x\n", value);
	...
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN61_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN62, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN62 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN63, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN63 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN64, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN64 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN65, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN65 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN66, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN66 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN67, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN67 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN68, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN68 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN69, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN69 = 0x%x\n", value);
	...
	n += scnprintf(buffer + n, size - n,
			   "ETDM_OUT1_MON = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN70, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN70 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN71, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN71 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN72, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN72 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN73, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN73 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN74, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN74 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN75, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN75 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN76, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN76 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN77, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN77 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN78, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN78 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN79, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN79 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN80, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN80 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN81, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN81 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN82, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN82 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN83, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN83 = 0x%x\n", value);
	...
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN66_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN67_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN67_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN68_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN68_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN69_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN69_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN70_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN70_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN71_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN71_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN72_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN72_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN73_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN73_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN74_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN74_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN75_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN75_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN76_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN76_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN77_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN77_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN78_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN78_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN79_1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN79_1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN0_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN0_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN1_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN1_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN2_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN2_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN3_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN3_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN4_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN4_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN5_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN5_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN6_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN6_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN7_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN7_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN8_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN8_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN9_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN9_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN10_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN10_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN11_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN11_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN12_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN12_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN13_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN13_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN14_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN14_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN15_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN15_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN16_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN16_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN17_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN17_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN18_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN18_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN19_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN19_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN20_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN20_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN21_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN21_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN22_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN22_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN23_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN23_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN24_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN24_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN25_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN25_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN26_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN26_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN27_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN27_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN28_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN28_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN29_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN29_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN30_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN30_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN31_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN31_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN32_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN32_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN33_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN33_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN34_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN34_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN35_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN35_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN36_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN36_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN37_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN37_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN38_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN38_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN39_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN39_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN40_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN40_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN41_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN41_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN42_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN42_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN43_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN43_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN44_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN44_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN45_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN45_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN46_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN46_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN47_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN47_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN48_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN48_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN49_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN49_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN50_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN50_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN51_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN51_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN52_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN52_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN53_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN53_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN54_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN54_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN55_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN55_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN56_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN56_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN57_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN57_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN58_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN58_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN59_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN59_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN60_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN60_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN61_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN61_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN62_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN62_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN63_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN63_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN64_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN64_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN65_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN65_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN66_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN66_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN67_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN67_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN68_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN68_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN69_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN69_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN70_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN70_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN71_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN71_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN72_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN72_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN73_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN73_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN74_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN74_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN75_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN75_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN76_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN76_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN77_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN77_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN78_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN78_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN79_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN79_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN80_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN80_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN81_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN81_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN82_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN82_2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_CONN83_2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_CONN83_2 = 0x%x\n", value);
	...
	regmap_read(afe->regmap, AFE_TINY_CONN8, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_TINY_CONN8 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_TINY_CONN9, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_TINY_CONN9 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_TINY_CONN10, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_TINY_CONN10 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_TINY_CONN11, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_TINY_CONN11 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_TINY_CONN12, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_TINY_CONN12 = 0x%x\n", value);
	...
	regmap_read(afe->regmap, ETDM_COWORK_SEC_CON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "ETDM_COWORK_SEC_CON0 = 0x%x\n", value);
	regmap_read(afe->regmap, ETDM_COWORK_SEC_CON1, &value);
	n += scnprintf(buffer + n, size - n,
			   "ETDM_COWORK_SEC_CON1 = 0x%x\n", value);
	regmap_read(afe->regmap, ETDM_COWORK_SEC_CON3, &value);
	n += scnprintf(buffer + n, size - n,
			   "ETDM_COWORK_SEC_CON3 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GAIN3_CON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GAIN3_CON0 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GAIN3_CON1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GAIN3_CON1 = 0x%x\n", value);
	...
	n += scnprintf(buffer + n, size - n,
			   "AFE_GAIN3_CON3 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GAIN3_CUR, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GAIN3_CUR = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GAIN4_CON0, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GAIN4_CON0 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GAIN4_CON1, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GAIN4_CON1 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GAIN4_CON2, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GAIN4_CON2 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GAIN4_CON3, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GAIN4_CON3 = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_GAIN4_CUR, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_GAIN4_CUR = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_VUL8_CON0, &value);
	n += scnprintf(buffer + n, size - n,
	...
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL11_BASE = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_DL11_CUR_MSB, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL11_CUR_MSB = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_DL11_CUR, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL11_CUR = 0x%x\n", value);
	regmap_read(afe->regmap, AFE_DL11_END_MSB, &value);
	n += scnprintf(buffer + n, size - n,
			   "AFE_DL11_END_MSB = 0x%x\n", value);
...
static const struct mtk_afe_debug_cmd mt6983_debug_cmds[] = {
	MTK_AFE_DBG_CMD("write_reg", mtk_afe_debug_write_reg),
	{}
};

static const struct file_operations mt6983_debugfs_ops = {
	.open = mtk_afe_debugfs_open,
	.write = mtk_afe_debugfs_write,
	.read = mt6983_debugfs_read,
};
#endif

static int mt6983_dai_memif_register(struct mtk_base_afe *afe)
{
	...
	dai->dai_drivers = mt6983_memif_dai_driver;
	dai->num_dai_drivers = ARRAY_SIZE(mt6983_memif_dai_driver);

	dai->controls = mt6983_pcm_kcontrols;
	dai->num_controls = ARRAY_SIZE(mt6983_pcm_kcontrols);
	dai->dapm_widgets = mt6983_memif_widgets;
	dai->num_dapm_widgets = ARRAY_SIZE(mt6983_memif_widgets);
	dai->dapm_routes = mt6983_memif_routes;
	dai->num_dapm_routes = ARRAY_SIZE(mt6983_memif_routes);
	return 0;
}

typedef int (*dai_register_cb)(struct mtk_base_afe *);
static const dai_register_cb dai_register_cbs[] = {
	mt6983_dai_adda_register,
	mt6983_dai_i2s_register,
	mt6983_dai_hw_gain_register,
	mt6983_dai_src_register,
	mt6983_dai_pcm_register,
	mt6983_dai_tdm_register,
	mt6983_dai_etdm_register,
	mt6983_dai_hostless_register,
	mt6983_dai_memif_register,
};

static int mt6983_afe_pcm_dev_probe(struct platform_device *pdev)
{
	int ret, i;
#if !defined(CONFIG_FPGA_EARLY_PORTING) || defined(FORCE_FPGA_ENABLE_IRQ)
	int irq_id;
	...
	struct mt6983_afe_private *afe_priv;
	struct resource *res;
	struct device *dev;
	...
	ret = mtk_audio_sram_init(dev, afe->sram, &mt6983_sram_ops);
	if (ret)
		return ret;
	...
	ret = devm_request_irq(dev, irq_id, mt6983_afe_irq_handler,
			       IRQF_TRIGGER_NONE,
	...
	/* others */
	afe->mtk_afe_hardware = &mt6983_afe_hardware;
	afe->memif_fs = mt6983_memif_fs;
	afe->irq_fs = mt6983_irq_fs;
	afe->get_dai_fs = mt6983_get_dai_fs;
	afe->get_memif_pbuf_size = mt6983_get_memif_pbuf_size;

	afe->runtime_resume = mt6983_afe_runtime_resume;
	afe->runtime_suspend = mt6983_afe_runtime_suspend;

	afe->request_dram_resource = mt6983_afe_dram_request;
...
#if IS_ENABLED(CONFIG_DEBUG_FS)
	/* debugfs */
	afe->debug_cmds = mt6983_debug_cmds;
	afe->debugfs = debugfs_create_file("mtksocaudio",
					   S_IFREG | 0444, NULL,
					   afe, &mt6983_debugfs_ops);
#endif
	/* register component */
	ret = devm_snd_soc_register_component(&pdev->dev,
					      &mt6983_afe_component,
					      afe->dai_drivers,
					      afe->num_dai_drivers);
	if (ret) {
...
static int mt6983_afe_pcm_dev_remove(struct platform_device *pdev)
{
	struct mtk_base_afe *afe = platform_get_drvdata(pdev);

	pm_runtime_disable(&pdev->dev);
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt6983_afe_runtime_suspend(&pdev->dev);
...
static const struct of_device_id mt6983_afe_pcm_dt_match[] = {
	{ .compatible = "mediatek,mt6983-sound", },
	{},
};
MODULE_DEVICE_TABLE(of, mt6983_afe_pcm_dt_match);

static const struct dev_pm_ops mt6983_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt6983_afe_runtime_suspend,
			   mt6983_afe_runtime_resume, NULL)
};

static struct platform_driver mt6983_afe_pcm_driver = {
	.driver = {
		   .name = "mt6983-audio",
		   .of_match_table = mt6983_afe_pcm_dt_match,
#if IS_ENABLED(CONFIG_PM)
		   .pm = &mt6983_afe_pm_ops,
#endif
	},
	.probe = mt6983_afe_pcm_dev_probe,
	.remove = mt6983_afe_pcm_dev_remove,
};

module_platform_driver(mt6983_afe_pcm_driver);

MODULE_DESCRIPTION("Mediatek ALSA SoC AFE platform driver for 6983");
