...
static const struct soc_enum Btcvsd_Loopback_Enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(btcvsd_loopback_usage),
			    btcvsd_loopback_usage),
};
...
static int btcvsd_tx_timestamp_set(struct snd_kcontrol *kcontrol,
...
static int Btcvsd_Loopback_Control_Set(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.enumerated.item[0] >
	    ARRAY_SIZE(btcvsd_loopback_usage)) {
...
mtk_pcm_btcvsd_tx_pointer(struct snd_pcm_substream *substream)
{
...
static int
mtk_asoc_pcm_btcvsd_tx_platform_probe(struct snd_soc_platform *platform)
{
	pr_debug("%s\n", __func__);
...
static struct snd_pcm_ops mtk_btcvsd_tx_ops = {
	...
	.copy_user = mtk_pcm_btcvsd_tx_copy,
};
