...
/*
 *  Mediatek ALSA SoC AFE platform driver for 6835
 *
...
static ssize_t mt6835_debug_read_reg(char *buffer, int size, struct mtk_base_afe *afe);

static const struct snd_pcm_hardware mt6835_afe_hardware = {
...
static int mt6835_fe_startup(struct snd_pcm_substream *substream,
...
void mt6835_fe_shutdown(struct snd_pcm_substream *substream,
			struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
int mt6835_fe_trigger(struct snd_pcm_substream *substream, int cmd,
		      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
static int mt6835_memif_fs(struct snd_pcm_substream *substream,
			   unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	return mt6835_rate_transform(afe->dev, rate, id);
}

static int mt6835_get_dai_fs(struct mtk_base_afe *afe,
			     int dai_id, unsigned int rate)
{
	return mt6835_rate_transform(afe->dev, rate, dai_id);
}

static int mt6835_irq_fs(struct snd_pcm_substream *substream, unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	return mt6835_general_rate_transform(afe->dev, rate);
}

int mt6835_get_memif_pbuf_size(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;

	if ((runtime->period_size * 1000) / runtime->rate > 10)
		return MT6835_MEMIF_PBUF_SIZE_256_BYTES;
	else
		return MT6835_MEMIF_PBUF_SIZE_32_BYTES;
}

/* FE DAIs */
static const struct snd_soc_dai_ops mt6835_memif_dai_ops = {
	.startup        = mt6835_fe_startup,
	.shutdown       = mt6835_fe_shutdown,
	.hw_params      = mtk_afe_fe_hw_params,
	.hw_free        = mtk_afe_fe_hw_free,
	.prepare        = mtk_afe_fe_prepare,
	.trigger        = mt6835_fe_trigger,
};
...
static struct snd_soc_dai_driver mt6835_memif_dai_driver[] = {
	/* FE DAIs: memory intefaces to CPU */
	{
		...
		.ops = &mt6835_memif_dai_ops,
	},
	{
		.name = "DL12",
		.id = MT6835_MEMIF_DL12,
		.playback = {
			.stream_name = "DL12",
			.channels_min = 1,
			.channels_max = 2,
...
/* kcontrol */
static int mt6835_irq_cnt1_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6835_PRIMARY_MEMIF];
	return 0;
}

static int mt6835_irq_cnt1_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6835_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6835_PRIMARY_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6835_irq_cnt2_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6835_RECORD_MEMIF];
	return 0;
}

static int mt6835_irq_cnt2_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6835_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6835_RECORD_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6835_deep_irq_cnt_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6835_DEEP_MEMIF];
	return 0;
}

static int mt6835_deep_irq_cnt_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6835_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6835_DEEP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6835_voip_rx_irq_cnt_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6835_VOIP_MEMIF];
	return 0;
}

static int mt6835_voip_rx_irq_cnt_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6835_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6835_VOIP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6835_deep_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_deep_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_fast_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_fast_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6835_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6835_FAST_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6835_primary_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_primary_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_voip_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_voip_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_record_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6835_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6835_RECORD_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6835_record_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	afe_priv->xrun_assert[MT6835_RECORD_MEMIF] = xrun_assert;
	return 0;
}

static int mt6835_echo_ref_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6835_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6835_ECHO_REF_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6835_echo_ref_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	dev_info(afe->dev, "%s(), xrun_assert %d\n", __func__, xrun_assert);
	afe_priv->xrun_assert[MT6835_ECHO_REF_MEMIF] = xrun_assert;
	return 0;
}

static int mt6835_sram_size_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_vow_barge_in_irq_id_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	int memif_num = MT6835_BARGE_IN_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUDIO_DSP)
static int mt6835_adsp_ref_mem_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_adsp_ref_mem_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_adsp_mem_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_adsp_mem_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_mmap_dl_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_mmap_dl_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6835_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6835_MMAP_DL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6835_mmap_ul_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_mmap_ul_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6835_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6835_MMAP_UL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6835_mmap_ion_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6835_mmap_ion_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_dl_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_dl_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int mt6835_ul_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6835_ul_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
static const struct snd_kcontrol_new mt6835_pcm_kcontrols[] = {
	...
	SOC_SINGLE_EXT("adsp_ref_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6835_adsp_ref_mem_get,
		       mt6835_adsp_ref_mem_set),
	...
	SOC_SINGLE_EXT("mmap_play_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6835_mmap_dl_scene_get, mt6835_mmap_dl_scene_set),
	SOC_SINGLE_EXT("mmap_record_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6835_mmap_ul_scene_get, mt6835_mmap_ul_scene_set),
	...
	SOC_SINGLE_EXT("aaudio_dl_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6835_dl_mmap_fd_get,
		       mt6835_dl_mmap_fd_set),
	SOC_SINGLE_EXT("aaudio_ul_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6835_ul_mmap_fd_get,
		       mt6835_ul_mmap_fd_set),
};
...
static const struct snd_soc_dapm_widget mt6835_memif_widgets[] = {
	/* inter-connections */
...
static const struct snd_soc_dapm_route mt6835_memif_routes[] = {
	{"UL1", NULL, "UL1_CH1"},
	{"UL1", NULL, "UL1_CH2"},
	{"UL1", NULL, "UL1_CH3"},
	{"UL1", NULL, "UL1_CH4"},
...
static const struct mtk_base_irq_data irq_data[MT6835_IRQ_NUM] = {
	[MT6835_IRQ_0] = {
		.id = MT6835_IRQ_0,
...
static const int memif_irq_usage[MT6835_MEMIF_NUM] = {
	/* TODO: verify each memif & irq */
	[MT6835_MEMIF_DL1] = MT6835_IRQ_0,
	[MT6835_MEMIF_DL2] = MT6835_IRQ_1,
	[MT6835_MEMIF_DL3] = MT6835_IRQ_2,
	[MT6835_MEMIF_DL4] = MT6835_IRQ_3,
	[MT6835_MEMIF_DL5] = MT6835_IRQ_4,
	[MT6835_MEMIF_DL6] = MT6835_IRQ_5,
	[MT6835_MEMIF_DL7] = MT6835_IRQ_6,
	[MT6835_MEMIF_DL8] = MT6835_IRQ_7,
	[MT6835_MEMIF_DL9] = MT6835_IRQ_8,
	[MT6835_MEMIF_DL12] = MT6835_IRQ_9,
	[MT6835_MEMIF_DAI] = MT6835_IRQ_10,
	[MT6835_MEMIF_MOD_DAI] = MT6835_IRQ_11,
	[MT6835_MEMIF_DAI2] = MT6835_IRQ_12,
	[MT6835_MEMIF_VUL12] = MT6835_IRQ_13,
	[MT6835_MEMIF_VUL2] = MT6835_IRQ_14,
	[MT6835_MEMIF_AWB] = MT6835_IRQ_15,
	[MT6835_MEMIF_AWB2] = MT6835_IRQ_16,
	[MT6835_MEMIF_VUL3] = MT6835_IRQ_17,
	[MT6835_MEMIF_VUL4] = MT6835_IRQ_18,
	[MT6835_MEMIF_VUL5] = MT6835_IRQ_19,
	[MT6835_MEMIF_VUL6] = MT6835_IRQ_20,
	[MT6835_MEMIF_VUL7] = MT6835_IRQ_21,
};

static bool mt6835_is_volatile_reg(struct device *dev, unsigned int reg)
{
...
static const struct regmap_config mt6835_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,

	.volatile_reg = mt6835_is_volatile_reg,
...
static irqreturn_t mt6835_afe_irq_handler(int irq_id, void *dev)
{
...
static int mt6835_afe_runtime_suspend(struct device *dev)
{
...
skip_regmap:
	mt6835_afe_sram_release(afe);
	mt6835_afe_disable_clock(afe);
	return 0;
}

static int mt6835_afe_runtime_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	int ret;
	...
	ret = mt6835_afe_enable_clock(afe);
	if (ret)
		return ret;
	mt6835_afe_sram_request(afe);
...
static int mt6835_afe_pcm_copy(struct snd_pcm_substream *substream,
			       ...
			       mtk_sp_copy_f sp_copy)
{
	...
	mt6835_set_audio_int_bus_parent(afe, CLK_TOP_MAINPLL_D4_D4);

	ret = sp_copy(substream, channel, hwoff, buf, bytes);

	mt6835_set_audio_int_bus_parent(afe, CLK_CLK26M);

	return ret;
}

static int mt6835_set_memif_sram_mode(struct device *dev,
...
static int mt6835_set_sram_mode(struct device *dev,
				enum mtk_audio_sram_mode sram_mode)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	/* set memif sram mode */
	mt6835_set_memif_sram_mode(dev, sram_mode);
...
static const struct mtk_audio_sram_ops mt6835_sram_ops = {
	.set_sram_mode = mt6835_set_sram_mode,
};
				    ...
				    char *buf, loff_t offset, size_t size)
{
	...
	read_size = mt6835_debug_read_reg(buffer, AFE_SYS_DEBUG_SIZE, afe);
...
static int mt6835_afe_component_probe(struct snd_soc_component *component)
{
	...
	if (component) {
		...
		mt6835_add_misc_control(component);
...
static const struct snd_soc_component_driver mt6835_afe_component = {
	.name = AFE_PCM_NAME,
	.probe = mt6835_afe_component_probe,
				   ...
				   size_t count, loff_t *pos)
{
	...
	n = mt6835_debug_read_reg(buffer, size, afe);
...
static const struct mtk_afe_debug_cmd mt6835_debug_cmds[] = {
	MTK_AFE_DBG_CMD("write_reg", mtk_afe_debug_write_reg),
	{}
};

static const struct file_operations mt6835_debugfs_ops = {
	.open = mtk_afe_debugfs_open,
	.write = mtk_afe_debugfs_write,
	.read = mt6835_debugfs_read,
};
#endif

static int mt6835_dai_memif_register(struct mtk_base_afe *afe)
{
	...
	dai->dai_drivers = mt6835_memif_dai_driver;
	dai->num_dai_drivers = ARRAY_SIZE(mt6835_memif_dai_driver);

	dai->controls = mt6835_pcm_kcontrols;
	dai->num_controls = ARRAY_SIZE(mt6835_pcm_kcontrols);
	dai->dapm_widgets = mt6835_memif_widgets;
	dai->num_dapm_widgets = ARRAY_SIZE(mt6835_memif_widgets);
	dai->dapm_routes = mt6835_memif_routes;
	dai->num_dapm_routes = ARRAY_SIZE(mt6835_memif_routes);
	return 0;
}

typedef int (*dai_register_cb)(struct mtk_base_afe *);
static const dai_register_cb dai_register_cbs[] = {
	mt6835_dai_adda_register,
	mt6835_dai_i2s_register,
	mt6835_dai_hw_gain_register,
	mt6835_dai_src_register,
	mt6835_dai_pcm_register,
	//      mt6835_dai_tdm_register,
	mt6835_dai_hostless_register,
	mt6835_dai_memif_register,
};

static int mt6835_afe_pcm_dev_probe(struct platform_device *pdev)
{
	int ret, i;
#if !defined(CONFIG_FPGA_EARLY_PORTING) || defined(FORCE_FPGA_ENABLE_IRQ)
	int irq_id;
	...
	struct mt6835_afe_private *afe_priv;
	struct resource *res;
	struct device *dev;
	...
	ret = mtk_audio_sram_init(dev, afe->sram, &mt6835_sram_ops);
	if (ret)
		return ret;
	...
	afe->memif_size = MT6835_MEMIF_NUM;
	...
	afe->memif[MT6835_DEEP_MEMIF].ack = mtk_sp_clean_written_buffer_ack;
	afe->memif[MT6835_FAST_MEMIF].fast_palyback = 1;

	mutex_init(&afe->irq_alloc_lock);       /* needed when dynamic irq */

	/* init irq */
	afe->irqs_size = MT6835_IRQ_NUM;
	...
	ret = devm_request_irq(dev, irq_id, mt6835_afe_irq_handler,
			       IRQF_TRIGGER_NONE,
	...
	/* others */
	afe->mtk_afe_hardware = &mt6835_afe_hardware;
	afe->memif_fs = mt6835_memif_fs;
	afe->irq_fs = mt6835_irq_fs;
	afe->get_dai_fs = mt6835_get_dai_fs;
	afe->get_memif_pbuf_size = mt6835_get_memif_pbuf_size;

	afe->runtime_resume = mt6835_afe_runtime_resume;
	afe->runtime_suspend = mt6835_afe_runtime_suspend;

	afe->request_dram_resource = mt6835_afe_dram_request;
	afe->release_dram_resource = mt6835_afe_dram_release;

	afe->copy = mt6835_afe_pcm_copy;

#if IS_ENABLED(CONFIG_DEBUG_FS)
	/* debugfs */
	afe->debug_cmds = mt6835_debug_cmds;
	afe->debugfs = debugfs_create_file("mtksocaudio",
					   S_IFREG | 0444, NULL,
					   afe, &mt6835_debugfs_ops);
#endif
	/* register component */
	ret = devm_snd_soc_register_component(&pdev->dev,
					      &mt6835_afe_component,
					      afe->dai_drivers,
					      afe->num_dai_drivers);
	if (ret) {
...
static int mt6835_afe_pcm_dev_remove(struct platform_device *pdev)
{
	struct mtk_base_afe *afe = platform_get_drvdata(pdev);

	pm_runtime_disable(&pdev->dev);
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt6835_afe_runtime_suspend(&pdev->dev);
...
static const struct of_device_id mt6835_afe_pcm_dt_match[] = {
	{ .compatible = "mediatek,mt6835-sound", },
	{},
};
MODULE_DEVICE_TABLE(of, mt6835_afe_pcm_dt_match);

static const struct dev_pm_ops mt6835_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt6835_afe_runtime_suspend,
			   mt6835_afe_runtime_resume, NULL)
};

static struct platform_driver mt6835_afe_pcm_driver = {
	.driver = {
		.name = "mt6835-audio",
		.of_match_table = mt6835_afe_pcm_dt_match,
#if IS_ENABLED(CONFIG_PM)
		.pm = &mt6835_afe_pm_ops,
#endif
	},
	.probe = mt6835_afe_pcm_dev_probe,
	.remove = mt6835_afe_pcm_dev_remove,
};

module_platform_driver(mt6835_afe_pcm_driver);

MODULE_DESCRIPTION("Mediatek ALSA SoC AFE platform driver for 6835");
