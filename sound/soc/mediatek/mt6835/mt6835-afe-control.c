...
{
	switch (aud_blk) {
	case MT6835_MEMIF_DAI:
	case MT6835_MEMIF_MOD_DAI:
		return dai_memif_rate_transform(dev, rate);
	case MT6835_DAI_PCM_1:
	case MT6835_DAI_PCM_2:
		return pcm_rate_transform(dev, rate);
	default:
		return mt6835_general_rate_transform(dev, rate);
	}
}
			...
			int priv_size, const void *priv_data)
{
	...
	if (id < 0) {
		dev_err(afe->dev, "%s(), negative id %d\n", __func__, id);
		return -EINVAL;
	}
...
EXPORT_SYMBOL(mt6835_enable_dc_compensation);
...
EXPORT_SYMBOL(mt6835_set_lch_dc_compensation);
...
EXPORT_SYMBOL(mt6835_set_rch_dc_compensation);
...
EXPORT_SYMBOL(mt6835_adda_dl_gain_control);
...
int mt6835_set_adda_predistortion(int hp_impedance)
{
	...
	if (hp_impedance == 0) {
		...
		regmap_write(local_afe->regmap, AFE_ADDA_PREDIS_CON0, 0);
		regmap_write(local_afe->regmap, AFE_ADDA_PREDIS_CON1, 0);
		goto exit;
	}
...
exit:
	regmap_read(local_afe->regmap, AFE_ADDA_PREDIS_CON0, &read_predis_con0);
	regmap_read(local_afe->regmap, AFE_ADDA_PREDIS_CON1, &read_predis_con1);

	dev_info(local_afe->dev, "%s(), AFE_ADDA_PREDIS_CON0=0x%x, AFE_ADDA_PREDIS_CON1=0x%x\n",
...
EXPORT_SYMBOL(mt6835_set_adda_predistortion);
