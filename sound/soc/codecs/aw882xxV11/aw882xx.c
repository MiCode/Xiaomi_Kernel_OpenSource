...
#define AW882XX_DRIVER_VERSION "v1.11.0.1"
...
#if 0
static void extcon_parse_cmdline(struct aw882xx *aw882xx)
{
	char *ruby = NULL, *rubypro = NULL, *rubyplus = NULL;
	const char *sku = get_hw_sku();
	aw882xx->product_name = RUBY;
	ruby = strnstr(sku, "ruby", strlen(sku));
	rubypro = strnstr(sku, "rubypro", strlen(sku));
	rubyplus = strnstr(sku, "rubyplus", strlen(sku));

	if (rubyplus)
		aw882xx->product_name = RUBYPLUS;
	else if (rubypro)
		aw882xx->product_name = RUBYPRO;
	else if (ruby)
		aw882xx->product_name = RUBY;
	aw_dev_info(aw882xx->dev, "product_name = %d, ruby = %d, rubypro = %d, rubyplus = %d\n", aw882xx->product_name, ruby ? 1 : 0, rubypro ? 1 : 0, rubyplus ? 1 : 0);
}
#endif
// audio add end
/******************************************************
...
static int aw882xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		if (gpio_is_valid(aw882xx->spk_sw_gpio)) {
			...
			aw_dev_info(aw882xx->dev, "[spk_sw_gpio] set gpio pull low");
		}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (3 == ucontrol->value.integer.value[0]) {
		if (gpio_is_valid(aw882xx->spk_sw_gpio)) {
			gpio_direction_output(aw882xx->spk_sw_gpio, 1);
			aw_dev_info(codec->dev, "[spk_sw_gpio] set gpio pull high");
		}
	} else {
		if (gpio_is_valid(aw882xx->spk_sw_gpio)) {
			gpio_direction_output(aw882xx->spk_sw_gpio, 0);
			aw_dev_info(codec->dev, "[spk_sw_gpio] set gpio pull low");
		}
	}
...
static void aw882xx_request_firmware(struct work_struct *work)
{
	...
	char *acf_bin_name=ACF_BIN_NAME;
	switch (aw882xx->product_name)
	{
	case BAILU:
		acf_bin_name = ACF_BIN_NAME_M9B;
		aw_dev_info(aw882xx->dev, "load M9B[BAILU] firmware! -> [%s]",acf_bin_name);
		break;
	case COROT:
		acf_bin_name = ACF_BIN_NAME_M12;
		aw_dev_info(aw882xx->dev, "load M12[COROT] firmware! -> [%s]",acf_bin_name);
		break;
	case ARISTOTLE:
		acf_bin_name = ACF_BIN_NAME_M12A;
		aw_dev_info(aw882xx->dev, "load M12A[ARISTOTLE] firmware! -> [%s]",acf_bin_name);
		break;
	default:
		acf_bin_name = ACF_BIN_NAME;
		aw_dev_info(aw882xx->dev, "load M9B[default] firmware! -> [%s]",acf_bin_name);
		break;
	}
				...
				int num_dai)
{
	...
	if ((dai_drv != NULL) && (num_dai > 0))
		for (i = 0; i < num_dai; i++) {
			...
			aw_dev_info(aw882xx->dev, "dai_drv[i].playback.stream_name [%s]", dai_drv[i].playback.stream_name);
			aw_dev_info(aw882xx->dev, "dai_drv[i].capture.stream_name [%s]", dai_drv[i].capture.stream_name);
	...
	struct device_node *np)
{
	...
	if (aw882xx->spk_sw_gpio < 0) {
		aw_dev_err(aw882xx->dev, "no spk_sw_gpio provided.");
	} else {
...
static int aw882xx_gpio_request(struct aw882xx *aw882xx)
{
...
#if IS_ENABLED(CONFIG_SND_SMARTPA_AW882XX_GPIO_CONFIG_V11)
	...
	if (gpio_is_valid(aw882xx->spk_sw_gpio)) {
		...
		if (ret) {
			aw_dev_err(aw882xx->dev, "spk_sw_gpio request failed");
		}
	}
				...
				const struct i2c_device_id *id)
{
	...
	*/ 
    ...
    mtk_spk_set_type(MTK_SPK_AW_AW882XX);
	/*codec register*/
