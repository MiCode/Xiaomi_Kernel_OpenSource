						...
						uint32_t *data_buf, int num)
{
	...
	if (ret) {
		aw_dev_dbg(aw_dev->dev, "get dev list failed");
		return ret;
	}
	...
	list_for_each(pos, dev_list) {
		...
		if (local_dev->channel < num) {
			...
			aw_dev_dbg(local_dev->dev, "channel num[%d] overflow buf num[%d]",
...
static int aw_cali_store_cali_re(struct aw_device *aw_dev, int32_t re)
{
	...
	if ((re > aw_dev->re_range.re_min) && (re < aw_dev->re_range.re_max)) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "write re to nvram failed!");
			return ret;
		}
	} else {
		aw_dev_dbg(aw_dev->dev, "invalid cali re %d!", re);
		return -EINVAL;
	}
...
int aw_cali_get_cali_re(struct aw_cali_desc *cali_desc)
{
	...
	if (ret < 0) {
		...
		aw_dev_dbg(aw_dev->dev, "get re failed");
		return ret;
	}
...
int aw_cali_read_cali_re_from_dsp(struct aw_cali_desc *cali_desc, uint32_t *re)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "set cali re error");
		return ret;
	}
...
int aw_cali_svc_get_ra(struct aw_cali_desc *cali_desc)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read ra error");
		return ret;
	}
					...
					uint32_t *re)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp read re error");
		return ret;
	}

	ret = aw_dev->ops.aw_dsp_read(aw_dev, ra_desc->dsp_reg, &ra, ra_desc->data_type);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp read ra error");
		return ret;
	}
...
static int aw_cali_svc_get_devs_r0(struct aw_device *aw_dev, int32_t *r0_buf, int num)
{
	...
	list_for_each (pos, dev_list) {
		...
		if (local_dev->channel < num) {
			...
			if (ret) {
				aw_dev_dbg(local_dev->dev, "get r0 failed!");
				return ret;
			}
			cnt++;
		} else {
			aw_dev_dbg(aw_dev->dev, "channel num[%d] overflow buf num[%d] ",
						 local_dev->channel, num);
		}
	}
	return cnt;
					...
					uint32_t *f0)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read f0 failed");
		return ret;
	}
...
static int aw_cali_svc_get_devs_f0(struct aw_device *aw_dev, int32_t *f0_buf, int num)
{
	...
	list_for_each (pos, dev_list) {
		...
		if (local_dev->channel < num) {
			...
			if (ret) {
				aw_dev_dbg(local_dev->dev, "get f0 failed!");
				return ret;
			}
			cnt++;
		} else {
					...
					uint32_t *q)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read q failed");
		return ret;
	}
...
int aw_cali_svc_get_dev_te(struct aw_cali_desc *cali_desc, int32_t *te)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read temperature failed");
		return ret;
	}
...
static int aw_cali_svc_get_devs_te(struct aw_device *aw_dev, int32_t *te_buf, int num)
{
	...
	list_for_each (pos, dev_list) {
		...
		if (local_dev->channel < num) {
			...
			if (ret) {
				aw_dev_dbg(local_dev->dev, "get temperature failed!");
				return ret;
			}
			cnt++;
		} else {
...
static int aw_cali_svc_del_max_min_ave_algo(uint32_t *data, int data_size)
{
	...
	if ((data_size - AW_CALI_DATA_SUM_RM) == 0) {
		aw_pr_dbg("data_size id :%d less than 2", data_size);
		return -EINVAL;
	}
...
static int aw_cali_svc_cali_init_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "syst_check failed");
		return ret;
	}

	ret = aw_dev_get_dsp_status(aw_dev);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp status error");
		return ret;
	}

	ret = aw_dev_get_hmute(aw_dev);
	if (ret == 1) {
		aw_dev_dbg(aw_dev->dev, "mute staus");
		return -EINVAL;
	}
...
static int aw_cali_svc_get_cali_cfg(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp read reg0x%x error", desc->actampth_reg);
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp read reg0x%x error", desc->noiseampth_reg);
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp read reg0x%x error", desc->ustepn_reg);
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp read reg0x%x error", desc->alphan_reg);
		return ret;
	}
				...
				struct cali_cfg cali_cfg)
{
	...
	ret = aw_dev->ops.aw_dsp_write(aw_dev,
			desc->actampth_reg, cali_cfg.data[0], desc->actampth_data_type);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp write reg0x%x error", desc->actampth_reg);
		return ret;
	}
	...
	ret = aw_dev->ops.aw_dsp_write(aw_dev,
			desc->noiseampth_reg, cali_cfg.data[1], desc->noiseampth_data_type);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp write reg0x%x error", desc->noiseampth_reg);
		return ret;
	}
	...
	ret = aw_dev->ops.aw_dsp_write(aw_dev,
			desc->ustepn_reg, cali_cfg.data[2], desc->ustepn_data_type);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp write reg0x%x error", desc->ustepn_reg);
		return ret;
	}
	...
	ret = aw_dev->ops.aw_dsp_write(aw_dev,
			desc->alphan_reg, cali_cfg.data[3], desc->alphan_data_type);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp write reg0x%x error", desc->alphan_reg);
		return ret;
	}
...
static int aw_cali_svc_get_smooth_cali_re(struct aw_device *aw_dev)
{
	...
	if (dsp_re < aw_dev->re_range.re_min || dsp_re > aw_dev->re_range.re_max) {
		...
		if (aw_dev->cali_desc.cali_check_st) {
			...
			if (ret < 0) {
				aw_dev_dbg(aw_dev->dev, "write re failed");
			}
		}
...
static int aw_cali_svc_cali_en(struct aw_device *aw_dev, bool cali_en)
{
	...
	if (cali_en) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "get cali cfg failed");
			aw_dev_dsp_enable(aw_dev, true);
			return ret;
		}
		set_cfg.data[0] = 0;
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "set cali cfg failed");
						...
						int type, bool enable)
{
	...
	if (enable) {
		...
		if (type == CALI_TYPE_RE) {
			...
			aw_dev_dbg(aw_dev->dev, "type:%d unsupported", type);
			return -EINVAL;
		}
		...
		ret = aw_dev->ops.aw_reg_read(aw_dev,
					desc->reg, &reg_val);
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "read reg 0x%x failed", desc->reg);
			return ret;
		}
		...
		ret = aw_dev->ops.aw_reg_write(aw_dev,
					desc->reg, reg_val);
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "write reg 0x%x failed", desc->reg);
			return ret;
		}
	} else {
		/*reset dsp vol*/
					...
					bool noise_enable)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read dsp reg 0x%x failed", desc->dsp_reg);
		return ret;
	}
	...
	ret = aw_dev->ops.aw_dsp_write(aw_dev,
			desc->dsp_reg, reg_val, desc->data_type);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "write dsp reg 0x%x failed", desc->dsp_reg);
		return ret;
	}
...
static int aw_cali_svc_cali_f0_en(struct aw_device *aw_dev, bool f0_enable)
{
	...
	if (f0_enable) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "run dsp volume error, ret=%d", ret);
			return ret;
		}
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "write white noise error, ret=%d", ret);
					...
					int type, unsigned int flag, bool is_enable)
{
	...
	if (is_enable) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "init check failed");
			return ret;
		}
...
static int aw_cali_svc_dev_cali_re(struct aw_device *aw_dev, unsigned int flag)
{
	...
	if (ret < 0)
		aw_dev_dbg(aw_dev->dev, "cali re failed");
...
static int aw_cali_svc_devs_get_cali_re(struct list_head *dev_list)
{
	...
	list_for_each(pos, dev_list) {
		...
		if (ret < 0) {
			aw_dev_dbg(local_dev->dev, "get re failed");
			return ret;
		}
	}
...
static int aw_cali_svc_set_devs_re_str(struct aw_device *aw_dev, const char *re_str)
{
	...
	for (i = 0 ; i < dev_num; i++) {
		...
		if (ret <= 0) {
			aw_dev_dbg(aw_dev->dev, "unsupported str: %s", re_str);
			return -EINVAL;
		}
		len += snprintf(str_data, sizeof(str_data), "dev[%d]:%d ", i, re_data[i]);
		if (len > strlen(re_str)) {
			aw_dev_dbg(aw_dev->dev, "%s: unsupported", re_str);
			return -EINVAL;
		}
	}
	...
	list_for_each (pos, dev_list) {
		...
		if (local_dev->channel < AW_DEV_CH_MAX) {
			...
			if (ret < 0) {
				aw_dev_dbg(local_dev->dev, "store cali re failed");
				return ret;
			}
			cnt++;
		}
...
static int aw_cali_svc_dev_cali_f0_q(struct aw_device *aw_dev, unsigned int flag)
{
	...
	if (ret < 0)
		aw_dev_dbg(aw_dev->dev, "get f0 q failed");
...
static int aw_cali_svc_devs_get_cali_f0_q(struct list_head *dev_list)
{
	...
	list_for_each(pos, dev_list) {
		...
		if (ret < 0) {
			aw_dev_dbg(local_dev->dev, "get f0 q failed");
			return ret;
		}
	}
...
static int aw_cali_svc_get_dev_cali_val(struct aw_device *aw_dev, int type, uint32_t *data_buf)
{
	...
	case GET_Q_TYPE:
		*data_buf = aw_dev->cali_desc.q;
		break;
	default:
		aw_dev_dbg(aw_dev->dev, "type:%d not support", type);
		return -EINVAL;
	}
	...
	list_for_each(pos, dev_list) {
		...
		if (local_dev->channel < num) {
			...
			case GET_Q_TYPE:
				...
				break;
			default:
				aw_dev_dbg(local_dev->dev, "type:%d not support", type);
				return -EINVAL;
			}
			cnt++;
		} else {
...
static int aw_cali_svc_cali_cmd(struct aw_device *aw_dev, int cali_cmd, bool is_single, unsigned int flag)
{
	switch (cali_cmd) {
		...
		default : {
			aw_dev_dbg(aw_dev->dev, "unsupported cmd %d", cali_cmd);
			return -EINVAL;
		}
	}
	return 0;
...
static int aw_cali_svc_get_cmd_form_str(struct aw_device *aw_dev, const char *buf)
{
	...
	if (i == CALI_STR_MAX) {
		aw_dev_dbg(aw_dev->dev, "supported cmd [%s]!", buf);
		return -EINVAL;
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read buf %s failed", buf);
		return ret;
	}

	if (time < 1000) {
		aw_dev_dbg(aw_dev->dev, "time:%d is too short, no set", time);
		return -EINVAL;
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (is_single_cali) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "store cali re failed!");
			return ret;
		}
	} else {
		ret = aw_cali_svc_set_devs_re_str(aw_dev, buf);
		if (ret <= 0) {
			aw_pr_dbg("set re str %s failed", buf);
			return -EPERM;
		}
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "cali f0 failed");
		return ret;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = aw_cali_svc_cali_f0_q(aw_dev, is_single_cali, CALI_OPS_NOISE);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "cali f0 q failed");
		return ret;
	}
	...
	if (is_single_cali) {
		...
		ret = aw_cali_svc_get_devs_cali_val(aw_dev, GET_F0_TYPE, f0, AW_DEV_CH_MAX);
		if (ret <= 0) {
			aw_dev_dbg(aw_dev->dev, "get f0 failed");
			return -EINVAL;
		}

		ret = aw_cali_svc_get_devs_cali_val(aw_dev, GET_Q_TYPE, q, AW_DEV_CH_MAX);
		if (ret <= 0) {
			aw_dev_dbg(aw_dev->dev, "get q failed");
			return -EINVAL;
		}
					...
					struct class_attribute *attr, const char *buf, size_t len)
{
	...
	ret = kstrtoint(buf, 0, &time);
	if (ret < 0) {
		aw_pr_dbg("read buf %s failed", buf);
		return ret;
	}

	if (time < 1000) {
		aw_pr_dbg("time:%d is too short, no set", time);
		return -EINVAL;
	}
...
static ssize_t aw_cali_class_cali_f0_q_show(struct  class *class, struct class_attribute *attr, char *buf)
{
	...
	ret = aw_cali_svc_cali_f0_q(local_dev, is_single_cali, CALI_OPS_NOISE);
	if (ret < 0) {
		aw_dev_dbg(local_dev->dev, "cali f0 q failed");
		return ret;
	}
	...
	if (ret <= 0) {
		aw_dev_dbg(local_dev->dev, "get q failed");
		return -EINVAL;
	}
...
static ssize_t aw_class_re_range_show(struct  class *class, struct class_attribute *attr, char *buf)
{
	...
	if (ret <= 0) {
		aw_dev_dbg(local_dev->dev, "get re range failed");
		return -EINVAL;
	}
...
static void aw_cali_class_attr_init(struct aw_device *aw_dev)
{
	...
	if (aw_dev->channel != 0) {
		aw_dev_dbg(aw_dev->dev, "class node already register");
		return;
	}

	ret = class_register(&aw_cali_class);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "error creating class node");
		return;
	}

	ret = class_create_file(&aw_cali_class, &class_attr_cali_time);
	if (ret)
		aw_dev_dbg(aw_dev->dev, "creat class_attr_cali_time fail");

	ret = class_create_file(&aw_cali_class, &class_attr_re25_calib);
	if (ret)
		aw_dev_dbg(aw_dev->dev, "creat class_attr_re25_calib fail");

	ret = class_create_file(&aw_cali_class, &class_attr_f0_calib);
	if (ret)
		aw_dev_dbg(aw_dev->dev, "creat class_attr_f0_calib fail");


	ret = class_create_file(&aw_cali_class, &class_attr_f0_q_calib);
	if (ret)
		aw_dev_dbg(aw_dev->dev, "creat class_attr_f0_q_calib fail");

	ret = class_create_file(&aw_cali_class, &class_att_re_range);
	if (ret)
		aw_dev_dbg(aw_dev->dev, "creat class_att_re_range fail");
}
			...
			unsigned int cmd, unsigned long arg)
{
	...
	if (!data_ptr) {
		aw_dev_dbg(aw_dev->dev, "malloc failed !");
		return -ENOMEM;
	}
	...
	switch (cmd) {
		...
		default:{
			aw_dev_dbg(aw_dev->dev, "unsupported  cmd %d", cmd);
			ret = -EINVAL;
		} break;
	}
...
static ssize_t aw_cali_misc_read(struct file *filp, char __user *buf, size_t size, loff_t *pos)
{
	...
	case CALI_STR_SHOW_R0: {
		...
		if (ret <= 0) {
			aw_dev_dbg(aw_dev->dev, "get r0 failed");
			return -EINVAL;
		} else {
			for (i = 0; i < ret; i++)
	...
	case CALI_STR_SHOW_CALI_F0: {
		...
		if (ret <= 0) {
			aw_dev_dbg(aw_dev->dev, "get cali f0 failed");
			return -EINVAL;
		} else {
			for (i = 0; i < ret; i++)
				len += snprintf(local_buf+len, sizeof(local_buf)-len,
	...
	case CALI_STR_SHOW_TE: {
		...
		if (ret <= 0) {
			aw_dev_dbg(aw_dev->dev, "get te failed");
			return -EINVAL;
		} else {
			for (i = 0; i < ret; i++)
				len += snprintf(local_buf+len, sizeof(local_buf)-len,
	...
	case CALI_STR_VER: {
		if (aw_dev->ops.aw_get_version) {
			...
			if (len < 0) {
				aw_dev_dbg(aw_dev->dev, "get version failed");
				return -EINVAL;
			}
			len += snprintf(local_buf+len, sizeof(local_buf) - len, "\n");
		} else {
			aw_dev_dbg(aw_dev->dev, "get version is NULL");
			return -EINVAL;
		}
	} break;
	case CALI_STR_SHOW_RE_RANGE: {
		...
		if (ret <= 0) {
			aw_dev_dbg(aw_dev->dev, "get re range failed");
			return -EINVAL;
		} else {
			for (i = 0; i < ret; i++) {
				len += snprintf(local_buf+len, sizeof(local_buf)-len,
	...
	default: {
		if (g_msic_wr_flag == CALI_STR_NONE) {
			...
			aw_dev_dbg(aw_dev->dev, "unsupported flag [%d]", g_msic_wr_flag);
	...
	if (copy_to_user((void __user *)buf, local_buf, len)) {
		aw_dev_dbg(aw_dev->dev, "copy_to_user error");
		g_msic_wr_flag = CALI_STR_NONE;
		return -EFAULT;
	}
...
static int aw_cali_misc_switch_dev(struct file *filp, struct aw_device *aw_dev, char *cmd_buf)
{
	...
	if (dev_select_num >= AW_DEV_CH_MAX) {
		aw_dev_dbg(aw_dev->dev, "unsupport str [%s]", cmd_buf);
		return -EINVAL;
	}
	...
	aw_dev_dbg(aw_dev->dev, " unsupport [%s]", cmd_buf);
	return -EINVAL;
}
...
static ssize_t aw_cali_misc_write(struct file *filp, const char __user *buf, size_t size, loff_t *pos)
{
	...
	if (kernel_buf == NULL) {
		aw_dev_dbg(aw_dev->dev, "kzalloc failed !");
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "upported cmd [%s]!", kernel_buf);
		ret = -EINVAL;
		goto exit;
	}
	...
	default: {
		aw_dev_dbg(aw_dev->dev, "unsupported [%s]!", kernel_buf);
		ret = -EINVAL;
	} break;
	};
...
static int aw_cali_misc_init(struct aw_device *aw_dev)
{
	...
	if (g_misc_dev == NULL) {
		...
		if (ret) {
			aw_dev_dbg(aw_dev->dev, "misc register fail: %d\n", ret);
