...
static int aw_get_msg_id(int dev_ch, uint32_t *msg_id)
{
	...
	case AW_DEV_CH_SEC_R:
		*msg_id = AFE_MSG_ID_MSG_1;
		break;
	default:
		pr_debug("%s: can not find msg num, channel %d ", __func__, dev_ch);
		return -EINVAL;
	}
...
static int aw_mtk_write_data_to_dsp(int msg_id, void *data, int size)
{
	...
	if (!dsp_data) {
		pr_debug("%s: kzalloc dsp_msg error\n", __func__);
		return -ENOMEM;
	}
...
static int aw_mtk_set_spin_angle(struct aw_device *aw_dev, uint32_t spin_angle)
{
	...
	if (ret)
		aw_dev_dbg(aw_dev->dev, "write data to dsp failed");
...
static int aw_mtk_set_mixer_en(struct aw_device *aw_dev, uint32_t msg_id, int32_t is_enable)
{
	...
	if (!dsp_msg) {
		aw_dev_dbg(aw_dev->dev, "kzalloc dsp_msg error");
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, " write data failed");
		kfree(dsp_msg);
		dsp_msg = NULL;
		return ret;
	}
					...
					uint32_t spin_angle)
{
	...
	if (ret)
		aw_dev_dbg(aw_dev->dev, "write spin angle to dsp failed");
	else
...
static int aw_qcom_get_spin_angle(uint32_t *spin_angle, int size)
{
	...
	if (ret)
		pr_debug("%s: get spin angle failed\n", __func__);
	else
...
static int aw_set_adsp_spin_angle(struct aw_device *aw_dev, uint32_t spin_angle)
{
	if (spin_angle >= AW_SPIN_MAX) {
		aw_dev_dbg(aw_dev->dev, "spin_angle:%d not support",
				spin_angle);
		return -EINVAL;
	}
...
static int aw_set_mixer_en(struct aw_device *aw_dev, int32_t is_enable)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "get msg_num failed");
		return ret;
	}
...
#endif
	if (ret)
		aw_dev_dbg(aw_dev->dev, "set mixer status failed");
...
int aw_check_spin_mode(struct aw_spin_desc *spin_desc)
{
	...
	list_for_each(pos, dev_list) {
		...
		if (g_spin_mode != spin_mode) {
			aw_pr_dbg("dev[%d] spin mode:%d not equal g_spin_mode:%d, check failed",
					...
					uint32_t spin_angle)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_pa->dev, "set rx failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_pa->dev, "set tx failed");
		return ret;
	}
...
static int aw_set_reg_spin_angle(struct aw883xx *aw883xx, uint32_t spin_angle)
{
	...
	if (spin_angle >= ARRAY_SIZE(aw_spin)) {
		aw_dev_dbg(aw883xx->dev, "spin_angle:%d not support",
					spin_angle);
		return -EINVAL;
	}
	...
	ret = aw_dev_get_list_head(&dev_list);
	if (ret) {
		aw_dev_dbg(aw883xx->dev, "get dev list failed");
		return ret;
	}
	...
	list_for_each(pos, dev_list) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw883xx->dev, "set channal mode failed");
			return ret;
		}
	}
...
static int aw_set_spin_angle(struct aw883xx *aw883xx, uint32_t spin_angle)
{
	...
	default:
		aw_pr_dbg("unsupported spin mode:%d", g_spin_mode);
		return -EINVAL;
	}
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (aw883xx->pstream == AW883XX_STREAM_OPEN) {
		...
		if (ret < 0)
			aw_dev_dbg(aw883xx->dev, "set spin error, ret=%d\n",
				ret);
	} else {
			...
			struct snd_ctl_elem_info *uinfo)
{
	...
	if (aw883xx == NULL) {
		aw_pr_dbg("get struct aw883xx failed");
		return -EINVAL;
	}
...
static int aw_spin_control_create(struct aw883xx *aw883xx)
{
	...
	if (aw_spin_control == NULL) {
		aw_dev_dbg(aw883xx->codec->dev, "kcontrol malloc failed!");
		return -ENOMEM;
	}
					...
					struct device_node *np)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "get spin_data failed, close spin function");
		return ret;
	}
	...
	if  (ret != AW_SPIN_MAX) {
		aw_dev_dbg(aw_dev->dev, "unsupported str:%s, close spin function",
				str_data);
		return -EINVAL;
	}
	...
	if (spin_count != ARRAY_SIZE(aw_spin)) {
		aw_dev_dbg(aw_dev->dev, "get spin_data failed, spin_count:%d", spin_count);
		return -EINVAL;
	}
