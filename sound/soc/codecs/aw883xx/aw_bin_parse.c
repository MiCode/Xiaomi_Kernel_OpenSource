...
pr_debug("AWINIC_BIN_ERR %s,line= %d,"fmt, __func__, __LINE__, ##arg);\
} while (0)
#else
		...
		uint8_t *data, uint32_t data_len, struct aw_prof_desc *prof_desc)
{
	...
	if (aw_bin == NULL) {
		aw_dev_dbg(aw_dev->dev, "kzalloc aw_bin failed");
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "parse bin failed");
		...
		if ((aw_dev->i2c->adapter->nr == cfg_dde[i].dev_bus) &&
			...
			if (cfg_dde[i].data_type == ACF_SEC_TYPE_MONITOR) {
				ret = aw_monitor_parse_fw(&aw_dev->monitor_desc,
						...
						cfg_dde[i].data_size);
				if (ret < 0) {
					aw_dev_dbg(aw_dev->dev, "parse monitor failed");
					return ret;
					}
			} else {
				if (cfg_dde[i].dev_profile >= AW_PROFILE_MAX) {
					aw_dev_dbg(aw_dev->dev, "dev_profile [%d] overflow",
				...
				if (ret < 0) {
					aw_dev_dbg(aw_dev->dev, "parse failed");
					return ret;
				}
				sec_num++;
			}
		...
		struct aw_cfg_hdr *prof_hdr, struct aw_all_prof_info *all_prof_info)
{
	...
	if (sec_num == 0) {
		aw_dev_dbg(aw_dev->dev, "get dev default type failed, get num[%d]", sec_num);
		return -EINVAL;
	}
				...
				struct aw_all_prof_info all_prof_info)
{
	...
	if (!prof_info->count) {
		aw_dev_dbg(aw_dev->dev, "no profile data");
		return -EPERM;
	}
	...
	if (prof_info->prof_desc == NULL) {
		aw_dev_dbg(aw_dev->dev, "prof_desc kzalloc failed");
		return -ENOMEM;
	}
		...
		if (prof_desc[i].prof_st == AW_PROFILE_OK) {
			...
			if ((sec_desc[AW_DATA_TYPE_REG].data != NULL) &&
				...
				if (num >= prof_info->count) {
					aw_dev_dbg(aw_dev->dev, "get scene num[%d] overflow count[%d]",
						num, prof_info->count);
					return -ENOMEM;
				}
...
static int aw_dev_create_prof_name_list_v_1_0_0_0(struct aw_device *aw_dev)
{
	...
	if (prof_desc == NULL) {
		aw_dev_dbg(aw_dev->dev, "prof_desc is NULL");
		return -EINVAL;
	}
	...
	if (prof_info->prof_name_list == NULL) {
		aw_dev_dbg(aw_dev->dev, "prof_name_list devm_kzalloc failed");
		return -ENOMEM;
	}
...
static int aw_get_dde_type_info(struct aw_device *aw_dev, struct aw_container *aw_cfg)
{
	...
	if (!(dev_num || default_num)) {
		aw_dev_dbg(aw_dev->dev, "can't find scene");
		return -EINVAL;
	}
...
static int aw_get_default_scene_count_v_1_0_0_0(struct aw_device *aw_dev,struct aw_container *aw_cfg,
						uint32_t *scene_num)
{
	int i;
	struct aw_cfg_hdr *cfg_hdr = (struct aw_cfg_hdr *)aw_cfg->data;
...
static int aw_dev_parse_scene_count_v_1_0_0_0(struct aw_device *aw_dev,
							struct aw_container *aw_cfg, uint32_t *count)
{
	int ret;
	...
	if (aw_dev->prof_info.prof_type == AW_DEV_TYPE_ID) {
		...
		aw_dev_dbg(aw_dev->dev, "unsupported prof_type[%x]",
...
static int aw_dev_parse_data_by_sec_type_v_1_0_0_0(struct aw_device *aw_dev, struct aw_cfg_hdr *prof_hdr,
							struct aw_cfg_dde_v_1_0_0_0 *cfg_dde, int *cur_scene_id)
{
	int ret;
	struct aw_prof_info *prof_info = &aw_dev->prof_info;
	...
	case ACF_SEC_TYPE_MUTLBIN:
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "parse multi bin failed");
			return ret;
		}
	...
	case ACF_SEC_TYPE_MONITOR:
		return aw_monitor_parse_fw(&aw_dev->monitor_desc,
						...
						cfg_dde->data_size);
	default:
		aw_pr_dbg("unsupported SEC_TYPE [%d]", cfg_dde->data_type);
		return -EINVAL;
	}
		...
		struct aw_cfg_hdr *prof_hdr)
{
	...
	if (cur_scene_id == 0) {
		aw_dev_dbg(aw_dev->dev, "get dev default type failed, get num[%d]", cur_scene_id);
		return -EINVAL;
	}
		...
		struct aw_cfg_hdr *cfg_hdr)
{
	...
	if (aw_dev->prof_info.prof_type == AW_DEV_TYPE_ID) {
		...
		aw_dev_dbg(aw_dev->dev, "prof type matched failed, get num[%d]",
			aw_dev->prof_info.prof_type);
		return -EINVAL;
	}
...
static int aw_dev_load_cfg_by_hdr_v_1_0_0_0(struct aw_device *aw_dev, struct aw_container *aw_cfg)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "get scene count failed");
		return ret;
	}
	...
	prof_info->prof_desc = devm_kzalloc(aw_dev->dev,
					...
					GFP_KERNEL);
	if (prof_info->prof_desc == NULL) {
		aw_dev_dbg(aw_dev->dev, "prof_desc devm_kzalloc failed");
		return -ENOMEM;
	}

	ret = aw_dev_parse_by_hdr_v_1_0_0_0(aw_dev, cfg_hdr);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, " failed");
		return ret;
	}

	ret = aw_dev_create_prof_name_list_v_1_0_0_0(aw_dev);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "create prof name list failed");
		return ret;
	}
...
int aw_dev_cfg_load(struct aw_device *aw_dev, struct aw_container *aw_cfg)
{
	...
	case AW_CFG_HDR_VER_0_0_0_1:
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "hdr_cersion[0x%x] parse failed",
	...
	case AW_CFG_HDR_VER_1_0_0_0:
		...
		break;
	default:
		aw_pr_dbg("unsupported hdr_version [0x%x]", cfg_hdr->a_hdr_version);
		return -EINVAL;
	}
...
static int aw_dev_check_cfg_by_hdr(struct aw_container *aw_cfg)
{
	...
	if (cfg_hdr->a_id != ACF_FILE_ID) {
		aw_pr_dbg("not acf type file");
		return -EINVAL;
	}

	hdr_ddt_len = cfg_hdr->a_hdr_offset + cfg_hdr->a_ddt_size;
	if (hdr_ddt_len > aw_cfg->len) {
		aw_pr_dbg("hdrlen with ddt_len [%d] overflow file size[%d]",
	...
	if (act_data != aw_cfg->len) {
		aw_pr_dbg("act_data[%d] not equal to file size[%d]!",
			act_data, aw_cfg->len);
		return -EINVAL;
	}
	...
	for (i = 0; i < cfg_hdr->a_ddt_num; i++) {
		...
		if (end_data_offset > aw_cfg->len) {
			aw_pr_dbg("a_ddt_num[%d] end_data_offset[%d] overflow file size[%d]",
		...
		if (act_crc8 != cfg_dde[i].data_crc) {
			aw_pr_dbg("a_ddt_num[%d] crc8 check failed, act_crc8:0x%x != data_crc 0x%x",
...
int aw_dev_load_acf_check(struct aw_container *aw_cfg)
{
	...
	if (aw_cfg == NULL) {
		aw_pr_dbg("aw_prof is NULL");
		return -ENOMEM;
	}

	if (aw_cfg->len < sizeof(struct aw_cfg_hdr)) {
		aw_pr_dbg("cfg hdr size[%d] overflow file size[%d]",
...
char *aw_dev_get_prof_name(struct aw_device *aw_dev, int index)
{
	...
	if ((index >= aw_dev->prof_info.count) || (index < 0)) {
		aw_dev_dbg(aw_dev->dev, "index[%d] overflow count[%d]",
...
{
	if ((index >= aw_dev->prof_info.count) || (index < 0)) {
		aw_dev_dbg(aw_dev->dev, "%s: index[%d] overflow count[%d]\n",
