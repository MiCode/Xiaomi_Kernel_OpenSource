						...
						unsigned int *vol)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read voltage failed!");
		return ret;
	}

	if (desc->int_bit == 0) {
		aw_dev_dbg(aw_dev->dev, "desc->int_bit:%d unsupported", desc->int_bit);
		return -EINVAL;
	}
...
static int aw_monitor_get_chip_temperature(struct aw_device *aw_dev, int *temp)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "get temperature failed!");
		return ret;
	}
			...
			int *data, int data_type)
{
	...
	if (psy) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "get data failed");
			return -EINVAL;
		}
		*data = prop.intval;
		aw_dev_dbg(aw_dev->dev, "get system data: %d", *data);
	} else {
		aw_dev_dbg(aw_dev->dev, "no struct power supply name : %s", name);
		return -EINVAL;
	}
...
static int aw_monitor_get_system_temperature(struct aw_device *aw_dev, int *temp)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "get system temperature failed!");
		return ret;
	}
...
static int aw_monitor_set_system_temperature(struct aw_device *aw_dev, int sys_temp)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "set system temperature failed!");
		return ret;
	}
...
static int aw_monitor_get_system_voltage(struct aw_device *aw_dev, unsigned int *vol)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "get system voltage failed!");
		return ret;
	}
			...
			struct aw_monitor_trace *data_trace)
{
	...
	if (table_info.aw_table == NULL) {
		aw_dev_dbg(aw_dev->dev, "table_info.aw_table is null");
		return -EINVAL;
	}
					...
					uint32_t aplha)
{
	...
	if (monitor->first_entry == AW_FIRST_ENTRY) {
		...
		if (monitor->samp_count == 0) {
			aw_dev_dbg(aw_dev->dev, "monitor->samp_count:%d unsupported", monitor->samp_count);
			return -EINVAL;
		}
				...
				uint16_t ipeak)
{
	...
	ret = aw_dev->ops.aw_reg_read(aw_dev, desc->reg, &reg_val);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read ipeak failed");
		return;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "write ipeak failed");
		return;
	}
...
static void aw_monitor_set_gain(struct aw_device *aw_dev, uint16_t gain)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read volume failed");
		return;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "set gain failed");
		return;
	}
...
static int aw_monitor_vmax_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "no iis signal");
		return ret;
	}

	ret = aw_dev_get_dsp_status(aw_dev);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "dsp not work");
		return ret;
	}
						...
						uint32_t vmax)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "vmax_check fail, ret=%d", ret);
		return;
	}
...
static void aw_monitor_check_sysint(struct aw_device *aw_dev)
{
	...
	if (ret < 0)
		aw_dev_dbg(aw_dev->dev, "get_sysint fail, ret=%d", ret);

	if (sysint & (desc->intst_mask)) {
		desc->sysint_st = sysint;
		aw_dev_dbg(aw_dev->dev, "check sysint fail, reg=0x%04x", sysint);
	}
}
...
void aw_hw_monitor_work_func(struct work_struct *work)
{
	struct aw_device *aw_dev  = container_of(work,
					...
					uint8_t *data, uint32_t data_len)
{
	...
	if (monitor_hdr->temp_offset > data_len) {
		aw_dev_dbg(aw_dev->dev, "temp_offset[%d] overflow file size[%d]!",
			monitor_hdr->temp_offset, data_len);
		return -ENOMEM;
	}

	if (monitor_hdr->vol_offset > data_len) {
		aw_dev_dbg(aw_dev->dev, "vol_offset[%d] overflow file size[%d]!",
	...
	if (temp_size > data_len) {
		aw_dev_dbg(aw_dev->dev, "temp_size:[%d] overflow file size[%d]!",
			temp_size, data_len);
		return -ENOMEM;
	}
	...
	if (vol_size > data_len) {
		aw_dev_dbg(aw_dev->dev, "vol_size:[%d] overflow file size[%d]!",
			vol_size, data_len);
		return -ENOMEM;
	}
				...
				uint8_t *data, uint32_t data_len)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "check monitor failed");
		return ret;
	}
				...
				uint8_t *data, uint32_t data_len)
{
	...
	if (monitor_desc == NULL || data == NULL) {
		pr_debug("monitor_desc or data is NULL");
		return -EINVAL;
	}
	...
	default:
		aw_dev_dbg(aw_dev->dev, "cfg version:0x%x unsupported",
...
static int aw_monitor_real_time_update_monitor(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "failed to read %s", AW883XX_MONITOR_NAME);
	...
	if (aw_monitor_cnt == NULL) {
		aw_dev_dbg(aw_dev->dev, "alloc failed!");
		release_firmware(cont);
		return ret;
	}
	...
	if (ret < 0)
		aw_dev_dbg(aw_dev->dev, "parse monitor firmware failed!");
...
void aw_monitor_init(struct aw_monitor_desc *monitor_desc)
{
	...
	if (ret < 0)
		aw_dev_dbg(aw_dev->dev, "error creating sysfs attr files");
}
