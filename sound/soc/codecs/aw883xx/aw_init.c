					...
					bool *is_enable, uint8_t *temp_flag)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw883xx->dev, "read hardware monitor status failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw883xx->dev, "read hardware temp switch failed");
		return ret;
	}
...
static int aw_pid_2049_cali_get_iv_st(struct aw_device *aw_dev)
{
	...
	aw_dev_dbg(aw883xx->dev, "IV data abnormal, please check");
...
static int aw_pid_2049_dsp_fw_check(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < AW_FW_CHECK_PART; i++) {
		...
		if (ret  < 0) {
			aw_dev_dbg(aw_dev->dev, "dsp read failed");
			return ret;
		}
		...
		if (dsp_val != bin_val) {
			aw_dev_dbg(aw_dev->dev, "check failed, addr[0x%x], read[0x%x] != bindata[0x%x]",
		...
		if ((addr - base_addr) > dsp_fw_desc->len) {
			aw_dev_dbg(aw_dev->dev, "check failed, addr[0x%x] too large", addr);
			return -EINVAL;
		}
	}
...
static int aw883xx_dev_init(struct aw883xx *aw883xx)
{
	...
	if (aw_pa == NULL) {
		aw_dev_dbg(aw883xx->dev, "dev kalloc failed");
		return -ENOMEM;
	}
...
int aw883xx_init(struct aw883xx *aw883xx)
{
	if (aw883xx->chip_id == AW883XX_PID_2049) {
		...
		aw_dev_dbg(aw883xx->dev, "unsupported device");
		return -EINVAL;
	}
}
