...
static int aw_dev_set_dsp_crc32(struct aw_device *aw_dev)
{
	...
	if (crc_data_len > crc_dsp_cfg->len) {
		aw_dev_dbg(aw_dev->dev, "crc data len :%d > cfg_data len:%d",
			crc_data_len, crc_dsp_cfg->len);
		return ret;
	}

	if (crc_data_len % 4 != 0) {
		aw_dev_dbg(aw_dev->dev, "The crc data len :%d unsupport", crc_data_len);
		return ret;
	}
	...
	ret = aw_dev->ops.aw_dsp_write(aw_dev, desc->dsp_reg, crc_value,
						desc->data_type);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "set dsp crc value failed");
		return ret;
	}
...
static int aw_dev_dsp_crc_check_enable(struct aw_device *aw_dev, bool flag)
{
	...
	if (flag) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "enable dsp crc failed");
			return ret;
		}
	} else {
		ret = aw_dev->ops.aw_reg_write_bits(aw_dev, dsp_crc_desc->ctl_reg,
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "close dsp crc failed");
			return ret;
		}
	}
...
static int aw_dev_dsp_st_check(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < AW_DSP_ST_CHECK_MAX; i++) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "read reg0x%x failed", desc->reg);
			continue;
		}

		if ((reg_val & (~desc->dsp_mask)) != desc->dsp_check) {
			aw_dev_dbg(aw_dev->dev, "check dsp st fail,reg_val:0x%04x", reg_val);
			ret= -EINVAL;
			continue;
		} else {
...
static int aw_dev_dsp_crc32_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "check crc32 fail");
		return ret;
	}
...
static int aw_dev_get_icalk(struct aw_device *aw_dev, int16_t *icalk)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "reg read failed");
		return ret;
	}
			...
			unsigned int addr, uint32_t dsp_data, unsigned char data_type)
{
	...
	if (data_type == AW_DSP_16_DATA) {
		...
		aw_dev_dbg(aw_dev->dev, "data type[%d] unsupported", data_type);
		return -EINVAL;
	}

	addr_offset = (addr - aw_dev->dsp_mem_desc.dsp_cfg_base_addr) * 2;
	if (addr_offset > crc_dsp_cfg->len) {
		aw_dev_dbg(aw_dev->dev, "addr_offset[%d] > crc_dsp_cfg->len[%d]",
...
static int aw_dev_vsense_select(struct aw_device *aw_dev, int *vsense_select)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read vsense_reg_val failed");
		return ret;
	}
...
static int aw_dev_set_vcalb(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read vcalb_adj failed");
		return ret;
	}
	...
	if (vsense_select == AW_DEV_VDSEL_VSENSE) {
		...
		aw_dev_dbg(aw_dev->dev, "unsupport vsense status");
		return -EINVAL;
	}

	if ((vcalk == 0) || (desc->iscal_factor == 0)) {
		aw_dev_dbg(aw_dev->dev, "vcalk:%d or desc->iscal_factor:%d unsupported",
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "write vcalb failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "modify dsp cfg failed");
		return ret;
	}
...
static int aw_dev_get_cali_f0_delay(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "read cali delay failed, ret=%d", ret);
		return ret;
	}
...
int aw_dev_get_int_status(struct aw_device *aw_dev, uint16_t *int_status)
{
	...
	if (ret < 0)
		aw_dev_dbg(aw_dev->dev, "read interrupt reg fail, ret=%d", ret);
	else
		*int_status = reg_val;
...
int aw_dev_get_iis_status(struct aw_device *aw_dev)
{
	...
	if ((reg_val & desc->pll_check) == desc->pll_check)
		...
		aw_dev_dbg(aw_dev->dev, "check pll lock fail,reg_val:0x%04x", reg_val);
...
static int aw_dev_mode1_pll_check(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < AW_DEV_SYSST_CHECK_MAX; i++) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "mode1 iis signal check error");
...
static int aw_dev_mode2_pll_check(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < AW_DEV_SYSST_CHECK_MAX; i++) {
		ret = aw_dev_get_iis_status(aw_dev);
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "mode2 iis signal check error");
			usleep_range(AW_2000_US, AW_2000_US + 10);
		} else {
			break;
		}
	...
	if (ret == 0) {
		...
		for (i = 0; i < AW_DEV_SYSST_CHECK_MAX; i++) {
			...
			if (ret < 0) {
				aw_dev_dbg(aw_dev->dev, "mode2 switch to mode1, iis signal check error");
				usleep_range(AW_2000_US, AW_2000_US + 10);
			} else {
				break;
			}
...
int aw_dev_syspll_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "mode2 check iis failed");
			return ret;
		}
	}
...
int aw_dev_sysst_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0)
		aw_dev_dbg(aw_dev->dev, "check fail");
...
static int aw_dev_sysint_check(struct aw_device *aw_dev)
{
	...
	if (reg_val & (desc->intst_mask)) {
		aw_dev_dbg(aw_dev->dev, "pa stop check fail:0x%04x", reg_val);
		ret = -EINVAL;
	}
...
void aw_dev_dsp_enable(struct aw_device *aw_dev, bool dsp)
{
	...
	if (dsp) {
		...
		if (ret < 0)
			aw_dev_dbg(aw_dev->dev, "enable dsp failed");
	} else {
		ret = aw_dev->ops.aw_reg_write_bits(aw_dev, desc->reg,
					desc->mask, desc->disable);
		if (ret < 0)
			aw_dev_dbg(aw_dev->dev, "disable dsp failed");
	}
...
void aw_dev_memclk_select(struct aw_device *aw_dev, unsigned char flag)
{
	...
	if (flag == AW_DEV_MEMCLK_PLL) {
		...
		if (ret < 0)
			aw_dev_dbg(aw_dev->dev, "memclk select pll failed");

	} else if (flag == AW_DEV_MEMCLK_OSC) {
		ret = aw_dev->ops.aw_reg_write_bits(aw_dev, desc->reg,
					desc->mask, desc->osc_clk);
		if (ret < 0)
			aw_dev_dbg(aw_dev->dev, "memclk select OSC failed");
	} else {
		aw_dev_dbg(aw_dev->dev, "unknown memclk config, flag=0x%x", flag);
	}
...
static int aw_dev_get_vmax(struct aw_device *aw_dev, unsigned int *vmax)
{
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "get vmax failed");
		return ret;
	}
				...
				uint8_t *data, uint32_t len)
{
	...
	if (data_len % 2 != 0) {
		aw_dev_dbg(aw_dev->dev, "data len:%d unsupported",
				data_len);
		return -EINVAL;
	}
					...
					uint8_t *data, uint32_t len)
{
	...
	if (len && (data != NULL)) {
		...
		aw_dev_dbg(aw_dev->dev, "reg data is null or len is 0");
		return -EPERM;
	}
			...
			uint8_t *data, uint32_t len)
{
	...
	if (len && (data != NULL)) {
		...
		aw_dev_dbg(aw_dev->dev, "dsp firmware data is null or len is 0");
		return -EPERM;
	}
			...
			uint8_t *data, uint32_t size)
{
	...
	if (crc_dsp_cfg->data == NULL) {
		...
		if (!crc_dsp_cfg->data) {
			aw_dev_dbg(aw_dev->dev, "error allocating memory");
			return -ENOMEM;
		}
			...
			uint8_t *data, uint32_t len)
{
	...
	if (len && (data != NULL)) {
		...
		aw_dev_dbg(aw_dev->dev, "dsp config data is null or len is 0");
		return -EPERM;
	}
...
static int aw_dev_sram_check(struct aw_device *aw_dev)
{
	...
	if (AW_DSP_ODD_NUM_BIT_TEST != reg_val) {
		aw_dev_dbg(aw_dev->dev, "check reg 0x40 odd bit failed, read[0x%x] does not match write[0x%x]",
	...
	if (AW_DSP_EVEN_NUM_BIT_TEST != reg_val) {
		aw_dev_dbg(aw_dev->dev, "check reg 0x40 even bit failed, read[0x%x] does not match write[0x%x]",
	...
	if (AW_DSP_EVEN_NUM_BIT_TEST != reg_val) {
		aw_dev_dbg(aw_dev->dev, "check dsp fw addr failed, read[0x%x] does not match write[0x%x]",
						reg_val, AW_DSP_EVEN_NUM_BIT_TEST);
		goto error;
	}
	...
	aw_dev->ops.aw_i2c_read(aw_dev, dsp_mem_desc->dsp_mdat_reg, &reg_val);
	if (AW_DSP_ODD_NUM_BIT_TEST != reg_val) {
		aw_dev_dbg(aw_dev->dev, "check dsp cfg failed, read[0x%x] does not match write[0x%x]",
						reg_val, AW_DSP_ODD_NUM_BIT_TEST);
		goto error;
	}
...
int aw_dev_fw_update(struct aw_device *aw_dev, bool up_dsp_fw_en, bool force_up_en)
{
	...
	if (aw_dev->fw_status == AW_DEV_FW_FAILED) {
		aw_dev_dbg(aw_dev->dev, "fw status[%d] error", aw_dev->fw_status);
		return -EPERM;
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "update reg failed");
		return ret;
	}
	...
	if (up_dsp_fw_en) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "check sram failed");
			goto error;
		}
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "update dsp fw failed");
			goto error;
		}
	}
	...
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "update dsp cfg failed");
		goto error;
	}
...
int aw_dev_dsp_check(struct aw_device *aw_dev)
{
	...
	if (aw_dev->dsp_cfg == AW_DEV_DSP_BYPASS) {
		...
		for (i = 0; i < AW_DEV_DSP_CHECK_MAX; i++) {
			...
			if (ret < 0) {
				aw_dev_dbg(aw_dev->dev, "dsp wdt status error=%d", ret);
				usleep_range(AW_2000_US, AW_2000_US + 10);
			} else {
				return 0;
			}
		...
		aw_dev_dbg(aw_dev->dev, "unknown dsp cfg=%d", aw_dev->dsp_cfg);
		return -EINVAL;
	}
...
static void aw_dev_cali_re_update(struct aw_cali_desc *cali_desc)
{
	...
	if (aw_dev->cali_desc.cali_re < aw_dev->re_range.re_max &&
		...
		aw_dev_dbg(aw_dev->dev, "cali_re:%d out of range, no set",
...
int aw_device_start(struct aw_device *aw_dev)
{
	...
	if (aw_dev->dsp_cfg == AW_DEV_DSP_WORK) {
		...
		if (aw_dev->dsp_crc_st != AW_DSP_CRC_OK) {
			...
			if (ret < 0) {
				aw_dev_dbg(aw_dev->dev, "dsp crc check failed");
				aw_dev_reg_dump(aw_dev);
				goto crc_check_fail;
			}
		}
		...
		if (ret < 0) {
			aw_dev_dbg(aw_dev->dev, "check dsp status failed");
			aw_dev_reg_dump(aw_dev);
			goto dsp_check_fail;
		}
	}else {
...
int aw_device_init(struct aw_device *aw_dev, struct aw_container *aw_cfg)
{
	...
	if (aw_dev == NULL || aw_cfg == NULL) {
		aw_pr_dbg("aw_dev is NULL or aw_cfg is NULL");
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		...
		aw_dev_dbg(aw_dev->dev, "aw_dev acf parse failed");
		return -EINVAL;
	}
	...
	ret = aw_dev_fw_update(aw_dev, AW_FORCE_UPDATE_ON,
			AW_DSP_FW_UPDATE_ON);
	if (ret < 0) {
		aw_dev_dbg(aw_dev->dev, "fw update failed");
		return ret;
	}
