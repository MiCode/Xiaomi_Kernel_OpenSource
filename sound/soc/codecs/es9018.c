...
static int es9018_set_coefficient(struct es9018_priv *es9018)
{
	...
	/* set the coefficient stage1 */
	for (i = 0; i < ES9018_COEFFS_STAGE1_COUNT; i++) {
		/* set the coefficient address */
...
static int es9018_set_mclk_rate(struct es9018_priv *es9018, unsigned long rate_bclk)
{
	...
	for (m = 4; m <= 16; m <<= 1) {
		...
		struct clk *clk_mclk = es9018->clk_mclk;

		if (rate_mclk == clk_round_rate(clk_mclk, rate_mclk)) {
			ret = clk_set_rate(clk_mclk, rate_mclk);
			if (ret == 0)
				break;
		}
	}
...
static int es9018_probe(struct snd_soc_codec *codec)
{
	...
	/* keep the chip in reset state until the bias turn on */
	...
	/* keep mute gpio in the inactive state */
	...
	if (gpio_is_valid(es9018->mute_gpio)) {
		...
		/* mute gpio is optinal */
		dev_info(codec->dev, "Failed to parse mute-gpio(%d)\n", ret);
	}

	/* keep switch gpio in the inactive state */
	...
	if (gpio_is_valid(es9018->switch_gpio)) {
		...
		/* switch gpio is optinal */
		dev_info(codec->dev, "Failed to parse switch-gpio(%d)\n", ret);
	}

	/* initialize the clock and regulator(all optional) */
	es9018->clk_mclk = clk_get(codec->dev, "mclk");
	if (IS_ERR(es9018->clk_mclk)) {
		dev_info(codec->dev, "mclk can't be found\n");
		es9018->clk_mclk = NULL;
	}
...
static int es9018_set_bias_on(struct snd_soc_codec *codec)
{
	...
	if (es9018->reg_dvdd) {
		...
		if (ret < 0) {
			...
			goto enable_dvdd_err;
		}
	}

	/* enable the clock */
	if (es9018->clk_mclk) {
		ret = clk_prepare_enable(es9018->clk_mclk);
		if (ret < 0) {
			dev_err(codec->dev, "Failed to enable mclk(%d)\n", ret);
			goto enable_mclk_err;
		}
	}
...
bus_access_err:
	...
	if (es9018->clk_mclk)
		clk_disable_unprepare(es9018->clk_mclk);
enable_mclk_err:
	if (es9018->reg_dvdd)
		regulator_disable(es9018->reg_dvdd);
enable_dvdd_err:
	if (es9018->reg_dvcc)
		regulator_disable(es9018->reg_dvcc);
enable_dvcc_err:
	if (es9018->reg_avccr)
		regulator_disable(es9018->reg_avccr);
enable_avccr_err:
	if (es9018->reg_avccl)
		regulator_disable(es9018->reg_avccl);
enable_avccl_err:
	if (es9018->reg_vcca)
		regulator_disable(es9018->reg_vcca);
enable_vcca_err:
	es9018_mute(es9018, false);
	return ret;
}
...
static void es9018_set_bias_off(struct snd_soc_codec *codec)
{
	...
	/* disable regulators in sequence */
	if (es9018->reg_dvdd) {
...
static int es9018_get_php_impedance(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct msm8974_asoc_mach_data *mach_data = snd_soc_card_get_drvdata(codec->card);
			...
			struct snd_soc_dai *codec_dai)
{
	...
	if (es9018->clk_mclk) {
		...
		rate_mclk = clk_get_rate(es9018->clk_mclk);
	} else
		rate_mclk = es9018->rate_mclk;
