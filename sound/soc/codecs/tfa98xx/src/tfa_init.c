...
static enum Tfa98xx_Error tfa9912_specific(struct tfa_device *tfa)
{
	...
	if (tfa->rev == 0x1a13) {
		...
		/* -----  version 1.41  ----- */
		reg_write(tfa, 0x00, 0x0255);
		reg_write(tfa, 0x01, 0x838a);
		reg_write(tfa, 0x02, 0x2dc8);
		reg_write(tfa, 0x05, 0x762a);
		reg_write(tfa, 0x22, 0x543c);
		reg_write(tfa, 0x26, 0x0100);
		reg_write(tfa, 0x51, 0x0000);
		reg_write(tfa, 0x52, 0x551c);
		reg_write(tfa, 0x53, 0x003e);
		reg_write(tfa, 0x61, 0x000c);
		reg_write(tfa, 0x63, 0x0a96);
		reg_write(tfa, 0x65, 0x0a82);
		reg_write(tfa, 0x66, 0x0701);
		reg_write(tfa, 0x6c, 0x00d5);
		reg_write(tfa, 0x70, 0x26f8);
		reg_write(tfa, 0x71, 0x3074);
		reg_write(tfa, 0x75, 0x4484);
		reg_write(tfa, 0x76, 0x72ea);
		reg_write(tfa, 0x83, 0x0716);
		reg_write(tfa, 0x89, 0x0013);
		reg_write(tfa, 0xb0, 0x4c08);
		reg_write(tfa, 0xc6, 0x004e);
		/* ----- generated code end   ----- */
...
static enum Tfa98xx_Error tfa9874_specific(struct tfa_device *tfa)
{
	...
	switch (tfa->rev) {
		...
		case 0x0c74:
			/* ----- generated code start ----- */
			/* V1.14 */
			reg_write(tfa, 0x02, 0x22c8);
			reg_write(tfa, 0x52, 0x57dc);
			reg_write(tfa, 0x53, 0x003e);
			reg_write(tfa, 0x56, 0x0400);
			reg_write(tfa, 0x61, 0x0110);
			reg_write(tfa, 0x6f, 0x00a5);
			reg_write(tfa, 0x70, 0x07f8);
			reg_write(tfa, 0x73, 0x0047);
			reg_write(tfa, 0x74, 0x5098);
			reg_write(tfa, 0x75, 0x8d28);
			reg_write(tfa, 0x80, 0x0000);
			reg_write(tfa, 0x83, 0x0799);
			reg_write(tfa, 0x84, 0x0081);
			/* ----- generated code end   ----- */
			break;
		default:
			pr_info("\nWarning: Optimal settings not found for device with revid = 0x%x \n", tfa->rev);
...
static enum Tfa98xx_Error tfa9888_specific(struct tfa_device *tfa)
{
	...
	/* --------- Version v1 ---------- */
	if (tfa->rev == 0x2c88) {
		reg_write(tfa, 0x00, 0x164d);
		reg_write(tfa, 0x01, 0x828b);
		reg_write(tfa, 0x02, 0x1dc8);
		reg_write(tfa, 0x0e, 0x0080);
		reg_write(tfa, 0x20, 0x089e);
		reg_write(tfa, 0x22, 0x543c);
		reg_write(tfa, 0x23, 0x0006);
		reg_write(tfa, 0x24, 0x0014);
		reg_write(tfa, 0x25, 0x000a);
		reg_write(tfa, 0x26, 0x0100);
		reg_write(tfa, 0x28, 0x1000);
		reg_write(tfa, 0x51, 0x0000);
		reg_write(tfa, 0x52, 0xfafe);
		reg_write(tfa, 0x70, 0x3ee4);
		reg_write(tfa, 0x71, 0x1074);
		reg_write(tfa, 0x83, 0x0014);
		/* ----- generated code end   ----- */
	} else {
		pr_info("Warning: Optimal settings not found for device with revid = 0x%x \n", tfa->rev);
...
static enum Tfa98xx_Error tfa9891_specific(struct tfa_device *tfa)
{
	...
	/* ----- generated code start ----- */
	/* -----  version 18.0 ----- */
	reg_write(tfa, 0x09, 0x025d);
	reg_write(tfa, 0x10, 0x0018);
	reg_write(tfa, 0x22, 0x0003);
	reg_write(tfa, 0x25, 0x0001);
	reg_write(tfa, 0x46, 0x0000);
	reg_write(tfa, 0x55, 0x3ffb);
	/* ----- generated code end   ----- */
...
static enum Tfa98xx_Error tfa9894_specific(struct tfa_device *tfa)
{
	...
	if (tfa->rev == 0x0a94) {
		/* V36 */
		/* ----- generated code start ----- */
		reg_write(tfa, 0x00, 0xa245);
		reg_write(tfa, 0x02, 0x51e8);
		reg_write(tfa, 0x52, 0xbe17);
		reg_write(tfa, 0x57, 0x0344);
		reg_write(tfa, 0x61, 0x0033);
		reg_write(tfa, 0x71, 0x00cf);
		reg_write(tfa, 0x72, 0x34a9);
		reg_write(tfa, 0x73, 0x3808);
		reg_write(tfa, 0x76, 0x0067);
		reg_write(tfa, 0x80, 0x0000);
		reg_write(tfa, 0x81, 0x5715);
		reg_write(tfa, 0x82, 0x0104);
		/* ----- generated code end   ----- */
	} else if (tfa->rev == 0x1a94) {
		/* V14 */
		/* ----- generated code start ----- */
		reg_write(tfa, 0x00, 0xa245);
		reg_write(tfa, 0x01, 0x15da);
		reg_write(tfa, 0x02, 0x5288);
		reg_write(tfa, 0x52, 0xbe17);
		reg_write(tfa, 0x53, 0x0dbe);
		reg_write(tfa, 0x56, 0x05c3);
		reg_write(tfa, 0x57, 0x0344);
		reg_write(tfa, 0x61, 0x0032);
		reg_write(tfa, 0x71, 0x00cf);
		reg_write(tfa, 0x72, 0x34a9);
		reg_write(tfa, 0x73, 0x38c8);
		reg_write(tfa, 0x76, 0x0067);
		reg_write(tfa, 0x80, 0x0000);
		reg_write(tfa, 0x81, 0x5799);
		reg_write(tfa, 0x82, 0x0104);
		/* ----- generated code end ----- */
