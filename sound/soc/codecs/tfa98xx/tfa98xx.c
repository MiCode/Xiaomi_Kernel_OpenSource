...
#define NXP_DEBUG_DUMP_REGISTER_LEVEL	2
...
#define TFA98XX_VERSION		"2.10.2_r2"
...
static DEFINE_MUTEX(tfa_mutex_lock);
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	dev_info(tfa98xx->dev, "loaded %s - size: %zu\n", fw_name,
					cont ? cont->size : 0);

	/* load container file once. */
	mutex_lock(&tfa_mutex_lock);
	if (NULL == container) {
		container = kzalloc(cont->size, GFP_KERNEL);
		if (!container) {
			mutex_unlock(&tfa_mutex_lock);
		...
		dev_info(tfa98xx->dev, "FW: %.2s%.2s\n", container->version, container->subversion);
		dev_info(tfa98xx->dev, "FW: %.8s\n", container->customer);
		dev_info(tfa98xx->dev, "FW: %.8s\n", container->application);
		dev_info(tfa98xx->dev, "FW: %.8s\n", container->type);
		dev_info(tfa98xx->dev, "FW: %d ndev\n", container->ndev);
		dev_info(tfa98xx->dev, "FW: %d nprof\n", container->nprof);
	...
	if (tfa98xx->flags & TFA98XX_FLAG_DSP_START_ON_MUTE) {
		...
		dev_info(tfa98xx->dev, "tfa98xx interrupt enable.\n");
		return;
	}
...
static void tfa98xx_dbg_show_speaker_info(void)
{
	int dev = 0;
	unsigned char bytes[LSMODEL_MAX_WORDS*3];
	int data[LSMODEL_MAX_WORDS+1] = { 0 };
	unsigned int stateSize = 0;
	int SpkTemp = 0;
	int old = 0;

	for (dev = 0; dev < tfa98xx_cnt_max_device(); dev++) {
		old = handles_local[dev].in_use;
		handles_local[dev].in_use = 1;
		if ((tfa98xx_dev_family(dev) == 1) && (tfa98xx_cf_enabled(dev))) {
			memset(bytes, 0x00, sizeof(bytes));
			memset(data, 0x00, sizeof(data));
			if (tfa98xx_dsp_get_state_info(dev, bytes, &stateSize) == Tfa98xx_Error_Ok) {
				tfa98xx_convert_bytes2data(3 * stateSize, bytes, data);
				SpkTemp = data[3] / (1 << (23 - SPKRBST_TEMPERATURE_EXP));
				pr_debug("[0x%02x] speaker temp: %d\n", handles_local[dev].slave_address/2, SpkTemp);
			}
		}
		handles_local[dev].in_use = old;
	}
}
...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	for (dev = 0; dev < tfa98xx_cnt_max_device(); dev++) {
		tfa98xx = tfa98xx_devices[dev];

		if (tfa98xx == NULL)
			break;

		/* if it is input device, we will find anohter one. */
		if (tfa98xx->input)
			continue;
		...
		if (tfa98xx_dev_family(tfa98xx->handle) == 1) {
			 ...
			 */
			ret = regmap_read(tfa98xx->regmap, TFA98XX_STATUSREG, &val);
			if (ret < 0) {
				/* if reading register is failed, we will try to read next device.*/
				pr_err("%s  read register error (%d)", __func__, ret);
				continue;
			}
			pr_debug("[0x%02x] SYS_STATUS0: 0x%04x\n", tfa98xx->i2c->addr, val);
		...
		if (tfa98xx_dev_family(tfa98xx->handle) == 2) {
			...
			pr_info("tfa98xx_monitor() SYS_STATUS0: 0x%04x\n", val);
			if ((TFA98XX_STATUS_FLAGS0_ACS & val) ||
				(TFA98XX_STATUS_FLAGS0_WDS & val)) {
				tfa98xx->dsp_init = TFA98XX_DSP_INIT_RECOVER;

				if (TFA98XX_STATUS_FLAGS0_ACS & val)
					pr_err("ERROR: ACS\n");
				if (TFA98XX_STATUS_FLAGS0_WDS & val)
					pr_err("ERROR: WDS\n");

				queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->init_work, 0);
			}

			if (TFA98XX_STATUS_FLAGS0_SPKS & val)
				pr_err("ERROR: SPKS\n");
	...
	/* show speaker temperature. */
	tfa98xx_dbg_show_speaker_info();
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (no_start == 0) {
		...
		case 0x92: /* tfa9891 */
			...
			break;
		case 0x97:
			dev_info(&i2c->dev, "TFA9897 detected\n");
			tfa98xx->flags |= TFA98XX_FLAG_DSP_START_ON_MUTE;
			tfa98xx->flags |= TFA98XX_FLAG_SKIP_INTERRUPTS;
			tfa98xx->flags |= TFA98XX_FLAG_TFA9897_FAM_DEV;
			break;
		case 0x12:
			dev_info(&i2c->dev, "TFA9895 detected\n");
			tfa98xx->flags |= TFA98XX_FLAG_DSP_START_ON_MUTE;
			tfa98xx->flags |= TFA98XX_FLAG_SKIP_INTERRUPTS;
			tfa98xx->flags |= TFA98XX_FLAG_TFA9890_FAM_DEV;
			break;
		default:
			dev_info(&i2c->dev, "Unsupported device revision (0x%x)\n", reg & 0xff);
			return -EINVAL;
		}
	}
