		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		//switch off PLL force enable when power down PA if it was on
		regmap_update_bits(cs35l41->regmap, CS35L41_PLL_CLK_CTRL,
					CS35L41_PLL_FORCE_EN_MASK, 0);
		pr_debug("----CSPL: %s, switch off PLL force enable!\n", __func__);
		break;
	default:
				...
				int dir)
{
	...
	//switch on PLL force enable when first time DAI open
	//this should be done before PLL setting
...
/* Add run-time mixer control for fast use case switch */
	...
	regmap_update_bits(cs35l41->regmap,
			CS35L41_VPBR_CFG, 0xF << 8 | 0x1F << 0, 3 << 8 | 0xC << 0);
#endif
...
#ifdef CONFIG_SND_SOC_CS35L41_J10
    dev_info(cs35l41->dev, "%s: ignore Set device name to match codec driver XM \n", __func__);
#else
	/* Set device name to match codec driver XM */
	ret = dev_set_name(cs35l41->dev, "cs35l41");
	if (ret < 0) {
		dev_err(cs35l41->dev, "[CSPL] Set codec name error[%d]\n", ret);
		return ret;
	}
#endif
