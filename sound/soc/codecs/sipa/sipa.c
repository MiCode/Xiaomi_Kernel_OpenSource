...
{
pr_err("[  err][%s] %s: read chipid begin\r\n", LOG_FLAG, __func__);
#ifdef DISTINGUISH_CHIP_TYPE
	...
	sipa_dev_t *si_pa)
{
	...
	pr_err("[  err][%s] %s: distinguish_chip_type\r\n", LOG_FLAG, __func__);
	...
	const struct i2c_device_id *id)
{
	...
	pr_err("[  err][%s] %s: client->addr:%02x\r\n", LOG_FLAG, __func__, client->addr);
	if (client->addr > 0x7f) {
	...
	pr_err("[  err][%s] %s: si_pa->chip_type:%d\r\n", LOG_FLAG, __func__, si_pa->chip_type);
	if (CHIP_TYPE_SIA8100X != si_pa->chip_type) {
		/* load firmware */
		pr_err("[  err][%s] %s: sipa_param_load_fw\r\n", LOG_FLAG, __func__);
...
static int sipa_probe(struct platform_device *pdev)
{
	...
	pr_err("[ err][%s] %s: read si_pa_disable_pin:%d \r\n", LOG_FLAG, __func__, disable_pin);
	...
	pr_err("[ err][%s] %s: si_pa->chip_type=%d si_pa->rst_pin=%d si_pa->mute_mode=%d si_pa->owi_pin=%d si_pa->disable_pin=%d si_pa->pa_status=%d\r\n", LOG_FLAG, __func__, si_pa->chip_type, si_pa->rst_pin, si_pa->mute_mode, si_pa->owi_pin, si_pa->disable_pin, si_pa->pa_status);
