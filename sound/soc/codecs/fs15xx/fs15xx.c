...
/*awinic pa multiplex fsm add*/
static struct aw87xx_dev_t* aw87xx_dev = NULL;
/*awinic pa multiplex fsm add*/
...
static int fs15xx_pinctrl_init(struct platform_device *pdev, fs15xx_dev_t *fs15xx)
{
...
#if defined(FS15XX_ID_DETECT_SUPPORT)
	...
	state_name = "fs15xx_id_active";
	state = pinctrl_lookup_state(ctrl, state_name);
	if (IS_ERR(state)) {
		ret = PTR_ERR(state);
		pr_err("lookup state: %s failed\n", state_name);
	} else {
		fs15xx->fs15xx_id_active = state;
	}
#endif
...
#if defined(FS15XX_ADP_BOOST_SUPPORT)
...
#else
	fs15xx->fs15xx_mod_default = NULL;
#endif
	pr_info("fs15xx done");
...
/*awinic pa multiplex fsm add*/
static int aw87xx_ext_amp_power_on(int enable) {
	struct aw87xx_dev_t *aw87xx = aw87xx_dev;
	int count;
	unsigned long flags;

	if (aw87xx_dev == NULL)
		return -EINVAL;
	if (!gpio_is_valid(aw87xx->aw_cmd_gpio)) {
		pr_err("invalid gpio\n");
		return -EINVAL;
	}
	pr_info("mode: %d\n", aw87xx_dev->aw87xx_mode);

	count = aw87xx_dev->aw87xx_mode;
	spin_lock_irqsave(&aw87xx->aw87xx_lock, flags);
	if (enable) {
		while (count > 0) { // count of pulse
		gpio_set_value(aw87xx->aw_cmd_gpio, 0);
		udelay(5); // 0.75us <
		gpio_set_value(aw87xx->aw_cmd_gpio, 1);
		udelay(5); // < 10us
		count--;
		}
	} else {
	...
	spin_unlock_irqrestore(&aw87xx->aw87xx_lock, flags);
...
int ext_amp_poweron(int enable)
{
	...
	struct aw87xx_dev_t* aw87xx = aw87xx_dev;
	if(aw87xx == NULL) {
		ret = fs15xx_ext_amp_set(enable);
	} else {
		aw87xx_ext_amp_power_on(enable);
		ret = 1;
		pr_info("ext amp use is aw87xx\n");
	}

	return ret;
}
EXPORT_SYMBOL(ext_amp_poweron);
/*awinic pa multiplex fsm add*/
...
/*awinic pa multiplex fsm add*/
static int use_aw87xx_ext_amp(fs15xx_dev_t *fs15xx)
{
	pr_info("use aw87xx ext amp comming ..... \n");

	if (aw87xx_dev == NULL) {
		aw87xx_dev = kzalloc(sizeof(struct aw87xx_dev_t), GFP_KERNEL);
		if (aw87xx_dev == NULL) {
			pr_err("allocate aw87xx dev memery failed\n");
			return -ENOMEM;
		}
	}

	aw87xx_dev->aw_id_gpio = fs15xx->gpio_id;
	aw87xx_dev->aw_cmd_gpio = fs15xx->gpio_cmd;
	aw87xx_dev->aw87xx_mode = AW87XX_DEFAULT_MODE;
	pr_info("use aw87xx ext amp end ..... \n");
...
static int fs15xx_check_pa_type(fs15xx_dev_t *fs15xx)
{
	...
	pinctrl_select_state(fs15xx->pinctrl, fs15xx->fs15xx_id_active);
	udelay(1);
	gpio_direction_input(fs15xx->gpio_id);
	level = gpio_get_value(fs15xx->gpio_id);
	pinctrl_select_state(fs15xx->pinctrl, fs15xx->fs15xx_id_default);
#else
	...
	if (level == 0) {
          #if IS_ENABLED(CONFIG_LCT_AUDIO_INFO)
		lct_audio_info_set_pa_name(FS15XX_DRV_NAME, strlen(FS15XX_DRV_NAME));
          #endif
		pr_info("fs15xx detected\n");
		return 0;
/*awinic pa multiplex fsm add*/
	} else {
		if (!(use_aw87xx_ext_amp(fs15xx))) {
                  #if IS_ENABLED(CONFIG_LCT_AUDIO_INFO)
			lct_audio_info_set_pa_name(AW87XX_DEV_NAME, strlen(AW87XX_DEV_NAME));
                  #endif
			pr_info("aw87xx detected\n");
		}
/*awinic pa multiplex fsm add*/
	}
...
static int fs15xx_reset(fs15xx_dev_t *fs15xx)
{
	unsigned long flags;
	int ret;
...
#endif
#ifdef FS15XX_ID_DETECT_SUPPORT
