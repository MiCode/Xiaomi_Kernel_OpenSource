					...
					struct snd_ctl_elem_value *ucontrol)
{
		...
		if (pTAS2559->spk_id_gpio_p)
			ucontrol->value.integer.value[0] = spk_id_get(pTAS2559->spk_id_gpio_p);
		return 0;
}
...
static int tas2559_edge_a_get(struct snd_kcontrol *pKcontrol,
			struct snd_ctl_elem_value *pValue)
{
#ifdef KCONTROL_CODEC
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(pKcontrol);
	...
	pValue->value.integer.value[0] = pTAS2559->mnLeftEdge;
...
static int tas2559_edge_a_put(struct snd_kcontrol *pKcontrol,
			struct snd_ctl_elem_value *pValue)
{
#ifdef KCONTROL_CODEC
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(pKcontrol);
	...
	dev_dbg(pTAS2559->dev, "%s, edge %d\n", __func__, edge);
	pTAS2559->mnLeftEdge = pValue->value.integer.value[0];
	tas2559_update_edge(pTAS2559, DevA);

	mutex_unlock(&pTAS2559->codec_lock);
	return 0;
}

static int tas2559_edge_b_get(struct snd_kcontrol *pKcontrol,
			struct snd_ctl_elem_value *pValue)
{
#ifdef KCONTROL_CODEC
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(pKcontrol);
	...
	pValue->value.integer.value[0] = pTAS2559->mnRightEdge;
...
static int tas2559_edge_b_put(struct snd_kcontrol *pKcontrol,
			struct snd_ctl_elem_value *pValue)
{
#ifdef KCONTROL_CODEC
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(pKcontrol);
...
#endif
	struct tas2559_priv *pTAS2559 = snd_soc_codec_get_drvdata(codec);
	int edge = pValue->value.integer.value[0];

	mutex_lock(&pTAS2559->codec_lock);

	dev_dbg(pTAS2559->dev, "%s, edge %d\n", __func__, edge);
	pTAS2559->mnRightEdge = pValue->value.integer.value[0];
	tas2559_update_edge(pTAS2559, DevB);
...
static const struct snd_kcontrol_new tas2559_snd_controls[] = {
	...
	SOC_ENUM_EXT("TAS2559 ClassD Edge", classd_edge_enum[0],
		tas2559_edge_a_get, tas2559_edge_a_put),
	SOC_ENUM_EXT("TAS2560 ClassD Edge", classd_edge_enum[0],
		tas2559_edge_b_get, tas2559_edge_b_put),
};
