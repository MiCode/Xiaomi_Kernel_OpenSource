...
extern int lct_audio_info_set_pa_name(char *pa_name, int count);
#endif
			...
			const struct i2c_device_id *id)
{
	...
	if(fsm_dev->id == 0) {
		...
		fsm_vbat_monitor_init();
		// fsm_codec_register(&i2c->dev, fsm_dev->id);
	}

#ifdef CONFIG_LCT_AUDIO_INFO
	strcpy(lct_audio_pa_info, dev_name(&i2c->dev));
	strcat(lct_audio_pa_info, "_fsm1599_pa");
