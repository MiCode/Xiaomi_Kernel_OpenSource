...
#define AUDIO_JACK_WAKE_LOCK_TIME         1000
...
static void rt5659_jack_detect_work(struct work_struct *work)
{
	...
	wake_lock_timeout(&rt5659->jack_wake_lock, msecs_to_jiffies(AUDIO_JACK_WAKE_LOCK_TIME));
	pr_debug("%s: hold wakelock to ensure jack is detected.\n", __func__);
	val = rt5659_check_jd_status(rt5659->codec);
...
static const struct snd_kcontrol_new sto2_adc_switch =
	SOC_DAPM_SINGLE("Switch", RT5659_DUMMY_4, 1, 1,
		0);
...
static int rt5659_sto2_adc_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (val == RT5659_SCLK_SRC_PLL1) {
			...
			snd_soc_update_bits(codec, RT5659_CLK_DET, 0x10, 0x10);
		...
		break;
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_update_bits(codec, RT5659_CLK_DET, 0x10, 0x0);
...
static const struct snd_soc_dapm_widget rt5659_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SWITCH("Stereo2 ADC MIX", SND_SOC_NOPM, 0, 0,
		&sto2_adc_switch),
	...
	SND_SOC_DAPM_SUPPLY_S("ADC Stereo2 Filter", 4, RT5659_PWR_DIG_2,
		RT5659_PWR_ADC_S2F_BIT, 0, rt5659_sto2_adc_event,
	...
	SND_SOC_DAPM_PGA("PDMLR", SND_SOC_NOPM, 0, 0, NULL, 0),
...
static const struct snd_soc_dapm_route rt5659_dapm_routes[] = {
	...
	{ "PDMLR", NULL, "PDML" },
	{ "PDMLR", NULL, "PDMR" },
	{ "Stereo2 ADC MIX", "Switch", "PDMLR" },
	{ "Stereo2 ADC MIX", NULL, "ADC Stereo2 Filter" },
	{ "Stereo2 ADC LR", NULL, "Stereo2 ADC MIX" },
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	case RT5659_AIF3:
		...
		snd_soc_update_bits(codec, RT5659_I2S3_SDP,
					RT5659_I2S_MS_MASK, RT5659_I2S_MS_M);
		snd_soc_update_bits(codec, RT5659_PWR_DIG_1, RT5659_PWR_I2S3, RT5659_PWR_I2S3);
...
static int rt5659_probe(struct snd_soc_codec *codec)
{
	...
	/* for I2S3(NXP) */
	snd_soc_update_bits(codec, RT5659_MONO_ADC_MIXER, 0x20, 0x20);
	snd_soc_write(codec, RT5659_TEST_MODE_CTRL_1, 0x1002);
	snd_soc_update_bits(codec, RT5659_DUMMY_3, 0x40, 0x40);
	/* SPO is not used */
	...
	dev_info(codec->dev, "rt5659 codec registed.\n");
...
static int rt5659_parse_dt(struct rt5659_priv *rt5659, struct device_node *np)
{
	...
	rt5659->pdata.gpio_ldo = of_get_named_gpio(np, "ldo-gpio", 0);
	rt5659->pdata.gpio_reset = of_get_named_gpio(np, "rst-gpio", 0);
	rt5659->pdata.hs_gpio = of_get_named_gpio(np, "int-gpio", 0);
	rt5659->pdata.hs_irq = irq_of_parse_and_map(np, 0);
...
static void rt5659_calibrate_handler(struct work_struct *work)
{
	...
	if (rt5659->pdata.hs_irq) {
		pr_info("%s request irq \n", __func__);
		ret = request_threaded_irq(rt5659->pdata.hs_irq, NULL, rt5659_irq,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING
			| IRQF_ONESHOT, "rt5659", rt5659);
		if (ret)
			pr_err("Failed to reguest IRQ: %d\n", ret);
		pr_debug("%s: set irq wake.\n", __func__);
		irq_set_irq_wake(rt5659->pdata.hs_irq, 1);
	}
		    ...
		    const struct i2c_device_id *id)
{
	...
	if (gpio_request(rt5659->pdata.hs_gpio, "hs_gpio")) {
		pr_err("int-gpio:%d request fail.\n",
						rt5659->pdata.hs_gpio);
	}
	gpio_direction_input(rt5659->pdata.hs_gpio);

	if (hardware_id == 0) {
		regulator_1v8 = regulator_get(&i2c->dev, "reg-1v8");
		if (IS_ERR(regulator_1v8))
			dev_err(&i2c->dev, "Fail to get regulator_1v8\n");
		else if (regulator_set_voltage(regulator_1v8, 1800000, 1800000))
			dev_err(&i2c->dev, "Fail to set regulator_1v8 1.8v\n");
		else if (regulator_enable(regulator_1v8))
			dev_err(&i2c->dev, "Fail to enable regulator_1v8\n");
		pr_info("for p1 and before build, hardware_id = %d.\n", hardware_id);
	} else {
		pr_info("for p2 and later build, hardware_id = %d.\n", hardware_id);
	}
	...
	if (gpio_is_valid(rt5659->pdata.gpio_ldo)) {
		if (gpio_request(rt5659->pdata.gpio_ldo, "rt5659-35"))
			dev_err(&i2c->dev, "Fail gpio_request gpio_ldo\n");
		else if (gpio_direction_output(rt5659->pdata.gpio_ldo, 1))
			dev_err(&i2c->dev, "Fail gpio_direction gpio_ldo\n");
	} else {
		dev_err(&i2c->dev, "gpio_ldo(%d) is not valid gpio_reset.\n",
				rt5659->pdata.gpio_ldo);
	}

	if (gpio_is_valid(rt5659->pdata.gpio_reset)) {
		if (gpio_request(rt5659->pdata.gpio_reset, "rt5659-33"))
		...
		dev_err(&i2c->dev, "gpio_reset(%d) is not valid gpio_reset.\n",
				rt5659->pdata.gpio_reset);
	}
	...
	if (val != DEVICE_ID) {
		...
		dev_info(&i2c->dev, "rt5659 device ID is %x.\n", val);
	}

	regmap_write(rt5659->regmap, RT5659_RESET, 0);
#if 0 /* do it in delayed_work */
	rt5659_calibrate(rt5659);
#endif
	...
	wake_lock_init(&rt5659->jack_wake_lock, WAKE_LOCK_SUSPEND, "jack_wakelock");
	INIT_DELAYED_WORK(&rt5659->calibrate_work, rt5659_calibrate_handler);

	dev_info(&i2c->dev, "<%s> <%d>: gpio=%d, irq=%d.\n", __func__, __LINE__,
			rt5659->pdata.hs_gpio, rt5659->pdata.hs_irq);

	schedule_delayed_work(&rt5659->calibrate_work,
			msecs_to_jiffies(10));

#if 0 /* request irq after calibration finished */
	if (rt5659->pdata.hs_irq) {
		ret = request_threaded_irq(rt5659->pdata.hs_irq, NULL, rt5659_irq,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING
			| IRQF_ONESHOT, "rt5659", rt5659);
...
static int rt5659_i2c_remove(struct i2c_client *i2c)
{
	...
	wake_lock_destroy(&rt5659->jack_wake_lock);
