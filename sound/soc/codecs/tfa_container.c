...
static char *filename = "seltech_mono.cnt";
...
int tfaContWritePatch(struct tfa98xx *tfa98xx)
{
	...
	return -EINVAL; // patch not in the list
}
...
int tfaContWriteFilterbank(struct tfa98xx *tfa98xx, struct nxpTfaFilter *filter)
{
	...
	for (biquad_index = 0; biquad_index < 10; biquad_index++) {
		if (filter[biquad_index].enabled) {
			error = tfa98xx_dsp_biquad_set_coeff(tfa98xx, biquad_index + 1, // start @1
						    sizeof(filter[biquad_index].biquad.bytes),
						    filter[biquad_index].biquad.bytes);
		} else {
			error = tfa98xx_dsp_biquad_disable(tfa98xx, biquad_index+1);
		}
...
/*
 * write vstep attenuation to de device
 */
...
/*
 * write vstep attenuation to the device
 */
...
//	tfaContShowHeader(tfa98xx, hdr);

	switch (type) {
	case volstepHdr:
		pr_debug("volstep\n");
		err = tfaContWriteVstepAttenuation(tfa98xx, (struct nxpTfaVolumeStep2File *)hdr);
		break;
	default:
		pr_err("Header is of unknown type: 0x%x\n", type);
		return -EINVAL;
...
int tfaContWriteProfile(struct tfa98xx *tfa98xx, int profile, int vstep)
{
	...
	/* mute */
	if (tfa98xx->profile_current != profile ) { /* profile switch so mute first */
		tfaRunMuteAmplifier(tfa98xx); // this will wait for SWS
	}

	tfa98xx->profile_current = profile;	// record current profile
	tfa98xx->vstep = tfa98xx->profiles[profile].vstep;	// record current volume step
...
/*
 * write volume attenuation from vstep file in the profilelist to the target
 */
int tfaContWriteVolumeAttenuation(struct tfa98xx *tfa98xx, int profile, int vstep)
{
	...
	/* process the list and write all files  */
	if (prof->list[0].type == dscFile) {
		file = (struct nxpTfaFileDsc *)(prof->list[0].offset + base);
		if (tfaContWriteAttenuation(tfa98xx, file)) {
			return -EINVAL;
		}
	}
