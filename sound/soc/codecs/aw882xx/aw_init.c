...
static void aw_pid_2113_efver_check(struct aw_device *aw_dev)
{
	...
	if ((tem == AW_PID_2113_DEFAULT_CFG1) || (tem == AW_PID_2113_DEFAULT_CFG2))
		aw_dev->frcpwm_en = AW_FRCPWM_DISABLE;
	else
		aw_dev->frcpwm_en = AW_FRCPWM_ENABLE;

	aw_dev_info(aw_dev->dev, "temh is 0x%04x, teml is 0x%04x, tem is 0x%04x, frcpwm_en is %d",
				temh, teml, tem, aw_dev->frcpwm_en);
}
