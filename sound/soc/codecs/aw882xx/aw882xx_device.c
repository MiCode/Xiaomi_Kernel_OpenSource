...
static int aw_dev_reg_fw_update(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < data_len; i += 2) {
		...
		if(reg_addr == aw_dev->dither_desc.reg) {
 		...
 		if(reg_addr == aw_dev->pwmdelay_desc.reg) {
 			aw_dev->pwmdelaya_val = reg_val & (~aw_dev->pwmdelay_desc.pwmdela_reg_mask);
 			aw_dev->pwmdelayb_val = reg_val & (~aw_dev->pwmdelay_desc.pwmdelb_reg_mask);
 			aw_dev_info(aw_dev->dev, "pwmdela=0x%04x", aw_dev->pwmdelaya_val);
 			aw_dev_info(aw_dev->dev, "pwmdelb=0x%04x", aw_dev->pwmdelayb_val);
 		}
...
static void aw_dev_set_pwm_delay(struct aw_device *aw_dev, bool pwmdel)
{
	int i = 0;
	unsigned int pwmdela_val =aw_dev->pwmdelaya_val>>AW_DEV_PWMDEL_SHIFT;
	unsigned int pwmdelb_val =aw_dev->pwmdelayb_val;
	struct aw_pwmdelay_desc *pwmdelay_desc = &aw_dev->pwmdelay_desc;

	aw_dev_dbg(aw_dev->dev, "enter,pwmdel:%d", pwmdel);

	if (pwmdelay_desc->reg == AW_REG_NONE)
		return;

	if(pwmdel) {
		aw_dev_dbg(aw_dev->dev, "PWMDELA:0x%x PWMDELB:0x%x", aw_dev->pwmdelaya_val, aw_dev->pwmdelayb_val);
	}

	if(pwmdel) {
		if(pwmdelb_val >= pwmdela_val) {
			for(i=pwmdela_val + 1; i<=pwmdelb_val; i++) {
				aw_dev->ops.aw_i2c_write_bits(aw_dev, pwmdelay_desc->reg,
						pwmdelay_desc->pwmdelb_reg_mask,
						i);
				aw_dev_dbg(aw_dev->dev, "PWMDELB:0x%x", i);
				usleep_range(AW_1000_US, AW_1000_US + 10);
			}
		}else {
			for(i=pwmdelb_val + 1; i<=pwmdela_val; i++) {
				aw_dev->ops.aw_i2c_write_bits(aw_dev, pwmdelay_desc->reg,
						pwmdelay_desc->pwmdela_reg_mask,
						i<<AW_DEV_PWMDEL_SHIFT);
				aw_dev_dbg(aw_dev->dev, "PWMDELA:0x%x", i<<AW_DEV_PWMDEL_SHIFT);
				usleep_range(AW_1000_US, AW_1000_US + 10);
			}
		}
		...
		if(pwmdelb_val >= pwmdela_val) {
			aw_dev->ops.aw_i2c_write_bits(aw_dev, pwmdelay_desc->reg,
					pwmdelay_desc->pwmdelb_reg_mask,
					aw_dev->pwmdelaya_val);
		}else {
			aw_dev->ops.aw_i2c_write_bits(aw_dev, pwmdelay_desc->reg,
					pwmdelay_desc->pwmdela_reg_mask,
					aw_dev->pwmdelayb_val<<AW_DEV_PWMDEL_SHIFT);
		}
	}
...
int aw882xx_device_start(struct aw_device *aw_dev)
{
	...
	aw_dev_set_pwm_delay(aw_dev, false);
	...
	aw_dev_set_pwm_delay(aw_dev, true);
