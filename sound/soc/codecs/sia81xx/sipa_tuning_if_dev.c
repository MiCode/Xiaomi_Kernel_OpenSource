...
#define DEVICE_NAME "sipa_tuning_if"
...
enum {
	CMD_SIPA_TUNING_IF_ADD_CAL_ID = _IOW(0x10, 0xE0, uint32_t),
	CMD_SIPA_TUNING_IF_RM_CAL_ID = _IOW(0x10, 0xE1, uint32_t),
	CMD_SIPA_TUNING_IF_PICK_CMD = _IOR(0x10, 0xE2, uint64_t),
	CMD_SIPA_TUNING_IF_TEST = _IOWR(0x10, 0xE3, uint32_t),
};
...
static unsigned long record_cal_module(struct file *filp,
...
/*************************************************************************
* tuning interface provide for sipa_socket.c
* ************************************************************************/
...
static unsigned long sipa_tuning_if_dev_type_open(uint32_t cal_id)
{
	struct cal_module_unit *module = NULL;

	module = is_cal_id_exist(cal_id);
	if (NULL == module) {
		pr_info("[ info][%s] %s: cal_module id %d first record\r\n",
			LOG_FLAG, __func__, cal_id);

		module = (struct cal_module_unit *)record_cal_module(
		TUNING_FD_DUMMY, cal_id, SIXTH_SIPA_RX_MODULE);
		if (NULL == module) {
			pr_err("[  err][%s] %s: err !!! record_cal_module fail, "
				"cal_id = %u \r\n",
				LOG_FLAG, __func__, cal_id);
			return 0;
		}
...
static int sipa_tuning_if_dev_type_close(unsigned long handle)
{
...
static int sipa_tuning_if_dev_type_read(unsigned long handle,
	uint32_t mode_id, uint32_t param_id, uint32_t size, uint8_t *payload)
{
	...
	mutex_lock(&module->tuning_lock);

	ret = write_payload_to_cal_module(module,
	...
	ret = read_payload_from_cal_module(module,
		&reply_pid, size, payload);
	if (0 != ret) {

		pr_err("[  err][%s] %s: read_payload_from_cal_module ret = %d !! \r\n",
			LOG_FLAG, __func__, ret);
	...
	mutex_unlock(&module->tuning_lock);
...
static int sipa_tuning_if_dev_type_write(unsigned long  handle,
	uint32_t mode_id, uint32_t param_id, uint32_t size, uint8_t *payload)
{
	int ret = 0, timer = TIMEOUT_MS;
	struct cal_module_unit *module = NULL;
...
/*************************************************************************
* end - tuning interface provide for sipa_socket.c
* ************************************************************************/
...
static ssize_t sipa_tuning_if_dev_write(struct file *fp,
...
static long sipa_tuning_if_dev_unlocked_ioctl(struct file *fp,
	unsigned int cmd, unsigned long arg)
{
	switch (cmd) {
	case CMD_SIPA_TUNING_IF_ADD_CAL_ID:
	{
#if 0
		uint32_t *args;
		...
		ret = record_cal_module(fp, cal_id, module_id);
		if (0 != ret) {
	...
	case CMD_SIPA_TUNING_IF_RM_CAL_ID:
	{
	...
	case CMD_SIPA_TUNING_IF_PICK_CMD:
	{
		struct cal_module_unit *module = NULL;
...
static int sipa_tuning_if_dev_open(struct inode *inode, struct file *fp)
{
...
static int sipa_tuning_if_dev_close(struct inode *inode, struct file *fp)
{
	pr_info("[ info][%s] %s: run !! \r\n", LOG_FLAG, __func__);
...
static struct file_operations sipa_tuning_if_ops = {
	.owner = THIS_MODULE,
	.open = sipa_tuning_if_dev_open,
	.release = sipa_tuning_if_dev_close,
	.read = sipa_tuning_if_dev_read,
	.write = sipa_tuning_if_dev_write,
	.unlocked_ioctl = sipa_tuning_if_dev_unlocked_ioctl,
	.compat_ioctl = sipa_tuning_if_dev_unlocked_ioctl,
};

static struct miscdevice sipa_tuning_if_misc_dev = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = DEVICE_NAME,
	.fops = &sipa_tuning_if_ops,
};
...
static int open_tuning_if_dev(void)
{
	...
	int ret = misc_register(&sipa_tuning_if_misc_dev);
	if (ret != 0) {
	   ret_error = ret ;
...
static void close_tuning_if_dev(void)
{
	...
	misc_deregister(&sipa_tuning_if_misc_dev);

	return;
}

static int sipa_tuning_if_init(void)
{
	pr_info("[ info][%s] %s: run !! \r\n", LOG_FLAG, __func__);
...
static void sipa_tuning_if_exit(void)
{
	pr_info("[ info][%s] %s: run !! \r\n", LOG_FLAG, __func__);
...
/* provide for sipa_socket.c */
struct sipa_cal_opt tuning_if_opt = {
	.init = sipa_tuning_if_init,
	.exit = sipa_tuning_if_exit,
	.open = sipa_tuning_if_dev_type_open,
	.close = sipa_tuning_if_dev_type_close,
	.read = sipa_tuning_if_dev_type_read,
	.write = sipa_tuning_if_dev_type_write,
};
