				 ...
				 struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		//configure PLL only when open playback stream to avoid pop sound
				...
				struct cs35l41_platform_data *pdata)
{
 ...
 //       printk("zhanglei debug %regid %08x reg revid %08x\n",regid,reg_revid);
	...
	cs35l41->irq_gpio = devm_gpiod_get_optional(cs35l41->dev, "irq", GPIOD_IN);
	if (IS_ERR(cs35l41->irq_gpio)) {
		ret = PTR_ERR(cs35l41->irq_gpio);
		cs35l41->irq_gpio = NULL;
		if (ret == -EBUSY) {
			dev_info(cs35l41->dev, "irq line busy, assuming shared irq\n");
		} else {
			dev_err(cs35l41->dev, "Failed to get irq GPIO, error: %d\n", ret);
			goto err;
		}
	}

	if (cs35l41->irq_gpio) {
		cs35l41->irq = gpiod_to_irq(cs35l41->irq_gpio);
		dev_info(cs35l41->dev, "gpiod_to_irq num: %d\n", cs35l41->irq);
	}
	...
	ret = devm_request_threaded_irq(cs35l41->dev, cs35l41->irq, NULL,
			...
			"cs35l41", cs35l41);
      printk("-----cs35l41------ debug for ret= %d,cs35l41->dev=%d,cs35l41->irq=%d,cs35l41_irq=%d\n", ret,cs35l41->dev,cs35l41->irq,cs35l41_irq);
