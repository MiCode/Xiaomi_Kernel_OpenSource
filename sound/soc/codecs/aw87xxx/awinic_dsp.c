...
static int aw_dsp_set_afe_rx_module_enable(void *buf, int size)
{
	...
	return aw_mtk_write_data_to_dsp(AW_MSG_ID_RX_SET_ENABLE, buf, size, channel);
}
static int aw_dsp_get_afe_rx_module_enable(void *buf, int size)
{
	int32_t channel = 0;
	return aw_mtk_read_data_from_dsp(AW_MSG_ID_RX_SET_ENABLE, buf, size, channel);
}

int aw_dsp_set_afe_module_en(int enable)
{
	int ret;
...
int aw_dsp_get_afe_module_en(int *status)
{
	int ret;
...
int aw_dev_set_afe_module_en(int enable)
{
	return aw_dsp_set_afe_module_en(enable);
}
int aw_dev_get_afe_module_en(int *status)
{
	return aw_dsp_get_afe_module_en(status);
}
