...
static int rpmb_open(struct inode *inode, struct file *file)
{
	...
	MSG(ERR, "%s,  rpmb kzalloc memory done!!!\n", __func__);
#endif
	return 0;
}
...
long rpmb_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if ((param.data_len + param.addr * 256)
		...
		param.data_len > RPMB_IOC_MAX_BYTES){
                MSG(ERR, "%s, copy from user failed param.data_len=%d,RPMB_IOC_MAX_BYTES=%d\n",param.data_len,RPMB_IOC_MAX_BYTES,__func__);
		return -EINVAL;
		}
	...
	case RPMB_IOCTL_SOTER_WRITE_DATA:
		if (get_user(user_arg, arg_p)) {
                 MSG(ERR, "%s, xinan ISEE rpmb get counter failed \n",__func__);
			ret = -1;
			goto end;
		}
