...
static int get_entropy_callback(void *ctx, void *buf)
{
	      ...
	      unsigned long long reseed_interval)
{
	...
	if (!memcmp(entropy_pool, ctx->prev_hw_drbg_block,
			Q_HW_DRBG_BLOCK_BYTES)) {
		memset(entropy_pool, 0, Q_HW_DRBG_BLOCK_BYTES);
		return FIPS140_PRNG_ERR;
	}

	memcpy(ctx->prev_hw_drbg_block, entropy_pool,
			Q_HW_DRBG_BLOCK_BYTES);
...
fips_drbg_final(struct fips_drbg_ctx_s *ctx)
{
	...
	ctx->get_entropy_callback     = NULL;
	ctx->get_entropy_callback_ctx = NULL;
