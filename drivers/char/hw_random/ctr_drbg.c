...
#define CYPHER_OUT_LEN_64	(64)

static uint8_t df_initial_k[16] =
			"\x0\x1\x2\x3\x4\x5\x6\x7\x8\x9\xa\xb\xc\xd\xe\xf";

static void _crypto_cipher_test_complete(struct crypto_async_request *req,
				int err)
{
	struct msm_ctr_tcrypt_result_s *res;
...
static int ctr_aes_init(struct ctr_drbg_ctx_s *ctx)
{
	...
	if (IS_ERR_OR_NULL(ctx->aes_ctx.tfm)) {
		pr_err("%s: qcom-ecb(aes) failed, ctx->aes_ctx.tfm %pK\n",
					__func__, ctx->aes_ctx.tfm);
	...
	ctx->aes_ctx.req = ablkcipher_request_alloc(ctx->aes_ctx.tfm,
							GFP_KERNEL);
	if (IS_ERR_OR_NULL(ctx->aes_ctx.req)) {
	...
	if (IS_ERR_OR_NULL(ctx->df_aes_ctx.tfm)) {
		pr_err("%s: qcom-ecb(aes) failed\n", __func__);
		ctx->df_aes_ctx.tfm = crypto_alloc_ablkcipher("ecb(aes)", 0, 0);
		if (IS_ERR_OR_NULL(ctx->df_aes_ctx.tfm)) {
			pr_err("%s: ecb(aes) failed\n", __func__);
			status = -ENOMEM;
			goto clr_output;
		}
	}
	...
	ctx->df_aes_ctx.req = ablkcipher_request_alloc(ctx->df_aes_ctx.tfm,
							GFP_KERNEL);
	if (IS_ERR_OR_NULL(ctx->df_aes_ctx.req)) {
	...
	memset(&ctx->df_aes_ctx.input, 0, sizeof(ctx->df_aes_ctx.input));
	memset(&ctx->df_aes_ctx.output, 0, sizeof(ctx->df_aes_ctx.output));
...
update(struct ctr_drbg_ctx_s *ctx, const uint8_t *data, size_t data_len)
{
	...
	for (i = 0; i < 2; ++i) {
		...
		default:
			pr_debug("ablkcipher_encrypt ret %d err %d\n",
				rc, ctx->aes_ctx.result.err);
			break;
		}
	...
	if (data_len > 0)
		pr_debug("in update, data_len = %zu\n", data_len);
					...
					size_t         entropy_len_bits)
{
	...
	update_rv = block_cipher_df(ctx, (uint8_t *)entropy,
				(entropy_len_bits / 8), seed_material, 32);
	...
	rc = crypto_ablkcipher_setkey(ctx->aes_ctx.tfm,
				ctx->seed.key_V.key, AES128_KEY_SIZE);
	if (rc) {
		memset(seed_material, 0, 32);
		pr_debug("set-key in Instantiate failed, returns %d\n", rc);
		return CTR_DRBG_GENERAL_ERROR;
	}
			...
			unsigned long long reseed_interval)
{
	...
	if (((nonce_len_bits / 8) + (entropy_len_bits / 8)) > 32) {
		pr_debug("entropy_len_bits + nonce_len_bits is too long\n");
		pr_debug("nonce len: %zu, entropy: %zu\n",
			nonce_len_bits, entropy_len_bits);
		return CTR_DRBG_INVALID_ARG_ERR1;
	}

	if (reseed_interval > (1ULL << 48))
		return CTR_DRBG_INVALID_ARG_ERR2;
	...
	if (update_rv != CTR_DRBG_SUCCESS) {
		pr_debug("block_cipher_df failed, returns %d\n", update_rv);
		memset(seed_material, 0, 32);
		return CTR_DRBG_GENERAL_ERROR;
	}
			...
			size_t len_bits)
{
	...
	for (i = 0; i < total_blocks; ++i) {
		...
		if (((len_bits % 128) != 0) && (i == (total_blocks - 1))) {
			...
			memcpy(ctx->prev_drn, ctx->aes_ctx.output.virt_addr,
					16);
			rv = 0;
...
/*
 * Zeroizes the context structure. In some future implementation it
 ...
 */
static enum ctr_drbg_status_t df_bcc_func(struct ctr_drbg_ctx_s *ctx,
		uint8_t *key,
		uint8_t *input,
		uint32_t input_size,
		uint8_t *output)
		...
		uint8_t *output, uint32_t output_size)
{
	...
	uint8_t           temp[32];
	uint32_t          out_len = 0;
	uint8_t           siv_string[CYPHER_OUT_LEN_64];
	uint8_t          *p_s_string = NULL;
	int               rc;
	struct scatterlist sg_in, sg_out;

	if (output_size > CYPHER_OUT_LEN_64)
		return CTR_DRBG_INVALID_ARG;
	...
	if (ret_val != CTR_DRBG_SUCCESS) {
		pr_debug("df_bcc_func failed, returned %d\n", ret_val);
		goto out;
	}
	...
	out_len = 0;
	rc = crypto_ablkcipher_setkey(ctx->df_aes_ctx.tfm, temp,
				AES128_KEY_SIZE);
	if (rc) {
		pr_debug("crypto_ablkcipher_setkey API failed: %d\n", rc);
		goto out;
