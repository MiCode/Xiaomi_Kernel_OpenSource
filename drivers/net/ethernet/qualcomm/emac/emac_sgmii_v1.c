...
static int emac_sgmii_v1_init(struct emac_adapter *adpt)
{
	...
	emac_reg_write_all(sgmii->base, physical_coding_sublayer_programming,
	...
	emac_reg_write_all(sgmii->base, pll_setting, ARRAY_SIZE(pll_setting));
	emac_reg_write_all(sgmii->base, cdr_setting, ARRAY_SIZE(cdr_setting));
