				      ...
				      const struct macsec_secy *secy)
{
	...
	for (i = 0; i < ATL_MACSEC_MAX_SECY; i++) {
		if (hw->macsec_cfg.atl_secy[i].sw_secy == secy) {
			return i;
			break;
		}
	}
...
static int atl_update_secy(struct atl_hw *hw, int secy_idx)
{
	...
	unsigned int sc_idx = atl_secy->sc_idx;
	int ret = 0;
...
static int atl_mdo_add_secy(struct macsec_context *ctx)
{
	...
	hw->macsec_cfg.atl_secy[secy_idx].sc_idx = to_hw_sc_idx(secy_idx, sc_sa);
...
static int atl_mdo_upd_txsa(struct macsec_context *ctx)
{
	...
	memcpy(nic->hw.macsec_cfg.atl_secy[secy_idx].tx_sa_key[ctx->sa.assoc_num],
...
static int atl_macsec_apply_rxsc_cfg(struct atl_hw *hw, const int rxsc_idx)
{
	const struct macsec_rx_sc *rx_sc =
		hw->macsec_cfg.atl_rxsc[rxsc_idx].sw_rxsc;
	const struct macsec_secy *secy =
		hw->macsec_cfg.atl_rxsc[rxsc_idx].sw_secy;
	int ret = 0;
	int i;
	...
	for (i = 0; i < MACSEC_NUM_AN; i++) {
		if (rx_sc->sa[i]) {
			ret = atl_update_rxsa(
				hw, hw->macsec_cfg.atl_rxsc[rxsc_idx].hw_sc_idx,
				secy, rx_sc->sa[i],
				hw->macsec_cfg.atl_rxsc[rxsc_idx].rx_sa_key[i],
				i);
			if (ret)
				return ret;
		}
...
void atl_macsec_check_txsa_expiration(struct atl_nic *nic)
{
	...
	for (i = 0; i < ATL_MACSEC_MAX_SA; i++) {
		if (egress_sa_expired & BIT(i)) {
			...
			if (!netif_running(hw->macsec_cfg.atl_secy[secy_idx].sw_secy->netdev)) {
