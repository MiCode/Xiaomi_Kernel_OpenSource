...
/* IRIS regulators for Pronto bear hardware */
static struct vregs_info iris_vregs_pronto_bear[] = {
	{"qcom,iris-vddxo",  VREG_NULL_CONFIG, 1800000, 0,
		1800000, 10000,  NULL},
	{"qcom,iris-vddrfa", VREG_NULL_CONFIG, 1300000, 0,
		1300000, 100000, NULL},
...
/* WCNSS regulators for Pronto bear hardware */
static struct vregs_info pronto_vregs_pronto_bear[] = {
					...
					int is_pronto_light)
{
	...
	case WCNSS_PRONTO_HW:
		if (is_pronto_light) {
			wcnss_vregs_off(iris_vregs_pronto_bear,
				ARRAY_SIZE(iris_vregs_pronto_bear));
		} else {
				...
				int is_pronto_light)
{
	...
	case WCNSS_PRONTO_HW:
		if (is_pronto_light) {
			ret = wcnss_vregs_on(dev, iris_vregs_pronto_bear,
					ARRAY_SIZE(iris_vregs_pronto_bear));
		} else {
					...
					int is_pronto_light)
{
	...
	case WCNSS_PRONTO_HW:
		if (is_pronto_light) {
			wcnss_vregs_off(pronto_vregs_pronto_bear,
				ARRAY_SIZE(pronto_vregs_pronto_bear));
		} else {
				...
				int is_pronto_light)
{
	...
	case WCNSS_PRONTO_HW:
		if (is_pronto_light) {
			ret = wcnss_vregs_on(dev, pronto_vregs_pronto_bear,
					ARRAY_SIZE(pronto_vregs_pronto_bear));
		} else {
		...
		enum wcnss_opcode on, int *iris_xo_set)
{
	...
	if (on) {
		...
		wcnss_iris_vregs_off(hw_type, cfg->is_pronto_light);
		wcnss_core_vregs_off(hw_type, cfg->is_pronto_light);
	}
