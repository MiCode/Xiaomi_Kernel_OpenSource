...
#define WLAN_DOMESTIC_TYPE 1
#define WLAN_OVERSEAS_TYPE 2
#define WLAN_DEFAULT_TYPE  WLAN_DOMESTIC_TYPE

#define HQ_HW_INFO_FILENAME "/sys/class/huaqin/interface/hw_info/pcba_config"
#define HQ_HW_INFO_LENGTH 32

#define HWINFO_CN_STR           "CN"
#define HWINFO_IN_STR           "IN"
#define HWINFO_GLOBAL_STR       "GLOBAL"
#define HWINFO_CN_LEN           10
#define HWINFO_IN_LEN           10
#define HWINFO_GLOBAL_LEN       14
...
#define NVBIN_FILE_OVERSEAS "wlan/prima/WCNSS_qcom_wlan_nv_overseas.bin"
...
static struct {
 ...
 static inline int get_wlan_nv_type (void)
 {
	 int ret_wlan_nv_type = WLAN_DEFAULT_TYPE;
	 struct file *fp = NULL;
	 mm_segment_t old_fs;
	 int ret = 0;
	 loff_t pos;
	 ...
	 len = HQ_HW_INFO_LENGTH - 1;
	 pbuf = kmalloc (HQ_HW_INFO_LENGTH, GFP_KERNEL);
	 if (NULL == pbuf) {
		pr_err ("wcnss %s: get_wlan_nv_type pbuf is null ", __func__);
		return WLAN_DEFAULT_TYPE;
	 }

	 fp  = filp_open (HQ_HW_INFO_FILENAME, O_RDONLY, 0);
	 if (IS_ERR (fp)) {
		pr_err ("wcnss %s: get_wlan_nv_type filp_open fail ", __func__);
		kfree (pbuf);
		return WLAN_DEFAULT_TYPE;
	 }

	 pr_err ("wcnss %s: open wlan nv file success ", __func__);

	 pos = 0;
	 memset (pbuf, 0, len);
	 old_fs = get_fs ();
	 set_fs (KERNEL_DS);
	 ret = vfs_read (fp, pbuf, len, &pos);
	 filp_close (fp, NULL);
	 filp_close (fp, NULL);
	 set_fs (old_fs);

	 if (ret < 0) {
		pr_err ("wcnss %s: get_wlan_nv_type vfs_read fail", __func__);
		kfree (pbuf);
		return WLAN_DEFAULT_TYPE;
	 }

	 pr_err ("wcnss %s: get_wlan_nv_type vfs_read : %s", __func__, pbuf);

	 if (NULL != strstr (pbuf, HWINFO_CN_STR)) {
		pr_err ("wcnss %s: hdd CN ", __func__);
		ret_wlan_nv_type = WLAN_DOMESTIC_TYPE;
	 } else if (NULL != strstr (pbuf, HWINFO_IN_STR)) {
		pr_err ("wcnss %s: hdd IN ", __func__);
		ret_wlan_nv_type = WLAN_OVERSEAS_TYPE;
	 } else if (NULL != strstr (pbuf, HWINFO_GLOBAL_STR)) {
		pr_err ("wcnss %s: hdd GLOBAL ", __func__);
		ret_wlan_nv_type = WLAN_OVERSEAS_TYPE;
	 } else {
		pr_err ("wcnss %s: hdd NO MATCH ALL , set default type ", __func__);
		ret_wlan_nv_type = WLAN_DEFAULT_TYPE;

	 }


	 kfree (pbuf);
	 pr_err ("wcnss %s: wlan_nv_type %d ", __func__, ret_wlan_nv_type);
	 return ret_wlan_nv_type;
}
...
static void wcnss_nvbin_dnld (void)
{
	...
	down_read (&wcnss_pm_sem);
		...
		wlan_nv_type = get_wlan_nv_type ();

		if (WLAN_DOMESTIC_TYPE == wlan_nv_type) {
		  ret = request_firmware (&nv, NVBIN_FILE, dev);
		} else {
		  ret = request_firmware (&nv, NVBIN_FILE_OVERSEAS, dev);
		}

		if (ret) {
			pr_err ("wcnss: %s: request_firmware_overseas failed for %s (ret = %d)\n",
