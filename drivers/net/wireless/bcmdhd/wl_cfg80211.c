...
static struct bcm_cfg80211 *g_bcm_cfg;
...
bcm_struct_cfgdev *wl_cfg80211_create_iface(struct wiphy *wiphy,
...
static s32 wl_notify_roaming_status(struct bcm_cfg80211 *cfg,
				    bcm_struct_cfgdev *cfgdev,
				    const wl_event_msg_t *e, void *data);
static s32 wl_notify_scan_status(struct bcm_cfg80211 *cfg,
				 bcm_struct_cfgdev *cfgdev,
				 const wl_event_msg_t *e, void *data);
...
#endif				/* WL_SCHED_SCAN */
#ifdef PNO_SUPPORT
static s32 wl_notify_pfn_status(struct bcm_cfg80211 *cfg,
				bcm_struct_cfgdev *cfgdev,
				const wl_event_msg_t *e, void *data);
#endif				/* PNO_SUPPORT */
#ifdef GSCAN_SUPPORT
static s32 wl_notify_gscan_event(struct bcm_cfg80211 *wl,
				 bcm_struct_cfgdev *cfgdev,
				 const wl_event_msg_t *e, void *data);
...
#ifdef WLTDLS
static s32 wl_tdls_event_handler(struct bcm_cfg80211 *cfg,
				 bcm_struct_cfgdev *cfgdev,
				 const wl_event_msg_t *e, void *data);
#endif				/* WLTDLS */
/*
...
static void *wl_read_prof(struct bcm_cfg80211 *cfg, struct net_device *ndev,
			  s32 item);
...
static void wl_update_hidden_ap_ie(struct wl_bss_info *bi, u8 *ie_stream,
				   u32 *ie_size, bool roam);
...
#ifdef WL11U
bcm_tlv_t *wl_cfg80211_find_interworking_ie(u8 *parse, u32 len);
static s32
wl_cfg80211_add_iw_ie(struct bcm_cfg80211 *cfg, struct net_device *ndev,
...
static s32 wl_inform_single_bss(struct bcm_cfg80211 *cfg,
				struct wl_bss_info *bi, bool roam);
...
static s32 wl_config_ifmode(struct bcm_cfg80211 *cfg, struct net_device *ndev,
			    s32 iftype);
static void wl_init_conf(struct wl_conf *conf);
static s32 wl_cfg80211_handle_ifdel(struct bcm_cfg80211 *cfg,
...
int get_roam_channel_list(int target_chan,
			  chanspec_t *channels, const wlc_ssid_t *ssid,
			  int ioctl_ver);
...
static chanspec_t wl_chspec_to_legacy(chanspec_t chspec)
{
	...
	if (CHSPEC_IS20(chspec)) {
		...
		WL_ERR(("wl_chspec_to_legacy: unable to convert chanspec %s (0x%04X) " "to pre-11ac format\n", wf_chspec_ntoa(chspec, chanbuf), chspec));
		return INVCHANSPEC;
	}
 ...
 * Returns INVCHANSPEC on error
 */
static chanspec_t wl_chspec_host_to_driver(chanspec_t chanspec)
{
	if (ioctl_version == 1) {
 ...
 * Returns INVCHANSPEC on error
 */
static chanspec_t wl_chspec_driver_to_host(chanspec_t chanspec)
{
...
int wl_cfg80211_hex_str_to_bin(unsigned char *data, int dlen, char *str)
{
	int count, slen;
	int hvalue;
	char tmp[3] = { 0 };
...
static void wl_validate_wps_ie(char *wps_ie, s32 wps_ie_len, bool *pbc)
{
...
static chanspec_t wl_cfg80211_get_shared_freq(struct wiphy *wiphy)
{
	...
	err = wldev_ioctl(dev, WLC_GET_BSSID, &bssid, sizeof(bssid), false);
	if (err) {
	...
	*(u32 *) cfg->extra_buf = htod32(WL_EXTRA_BUF_MAX);
	err = wldev_ioctl(dev, WLC_GET_BSS_INFO, cfg->extra_buf,
...
static bcm_struct_cfgdev *wl_cfg80211_add_monitor_if(char *name)
{
#if defined(WL_ENABLE_P2P_IF) || defined(WL_CFG80211_P2P_DEV_IF)
...
static bcm_struct_cfgdev *wl_cfg80211_add_virtual_iface(struct wiphy *wiphy,
#if defined(WL_CFG80211_P2P_DEV_IF)
							const char *name,
#else
							char *name,
							...
							*params)
{
	...
	if (cfg->p2p_supported && (wlif_type != -1)) {
		...
		if (cfg->p2p && !cfg->p2p->on
		    && strnstr(name, WL_P2P_INTERFACE_PREFIX, IFNAMSIZ)) {
			p2p_on(cfg) = true;
		...
		strlcpy(cfg->p2p->vir_ifname, name, IFNAMSIZ);
		...
		if (!cfg->wlfc_on && !disable_proptx) {
			...
			if (!enabled && dhd->op_mode != DHD_FLAG_HOSTAP_MODE &&
				...
				dhd_wlfc_init(dhd);
				err =
				    wldev_ioctl(primary_ndev, WLC_UP, &up,
						sizeof(s32), true);
				if (err < 0)
		...
		err =
		    wl_cfgp2p_ifadd(cfg, &cfg->p2p->int_addr, htod32(wlif_type),
				    chspec);
		if (unlikely(err)) {
		...
		if (timeout > 0 && !wl_get_p2p_status(cfg, IF_ADDING)
			 ...
			 */
			strlcpy(cfg->if_event_info.name, name, IFNAMSIZ);
			new_ndev =
			    wl_cfg80211_allocate_if(cfg, event->ifidx,
						    cfg->p2p->vir_ifname,
						    event->mac, event->bssidx);
			if (new_ndev == NULL)
				goto fail;

			wl_to_p2p_bss_ndev(cfg, P2PAPI_BSSCFG_CONNECTION) =
			    new_ndev;
			wl_to_p2p_bss_bssidx(cfg, P2PAPI_BSSCFG_CONNECTION) =
			    event->bssidx;
			...
			if (wl_cfg80211_register_if(cfg, event->ifidx, new_ndev)
			    != BCME_OK) {
				wl_cfg80211_remove_if(cfg, event->ifidx,
						      new_ndev);
				goto fail;
			}
			...
			if (mode != WL_MODE_AP)
				wldev_iovar_setint(new_ndev, "buf_key_b4_m4",
						   1);
...
wl_cfg80211_del_virtual_iface(struct wiphy *wiphy, bcm_struct_cfgdev *cfgdev)
{
	...
	if (cfg->p2p_supported) {
		...
		if (cfg->p2p->vif_created) {
			...
			wl_add_remove_pm_enable_work(cfg, FALSE,
						     WL_HANDLER_DEL);
			...
			if (wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP) {
				DHD_OS_WAKE_LOCK_CTRL_TIMEOUT_CANCEL((dhd_pub_t
								      *) (cfg->
									  pub));
			}
		}
				 ...
				 struct vif_params *params)
{
	...
	if (ap) {
		...
		if (cfg->p2p_supported && cfg->p2p->vif_created) {
			WL_DBG(("p2p_vif_created (%d) p2p_on (%d)\n",
				cfg->p2p->vif_created, p2p_on(cfg)));
			...
			wl_cfgp2p_ifchange(cfg, &cfg->p2p->int_addr,
	...
	if (ibss) {
		...
		wl_set_mode_by_netdev(cfg, ndev, mode);
		err =
		    wldev_ioctl(ndev, WLC_SET_INFRA, &infra, sizeof(s32),
				true);
		if (err < 0) {
...
s32 wl_cfg80211_notify_ifadd(int ifidx, char *name, uint8 *mac, uint8 bssidx)
{
	...
	if (ifadd_expected) {
		...
		strlcpy(if_event_info->name, name, IFNAMSIZ);
...
s32 wl_cfg80211_notify_ifdel(int ifidx, char *name, uint8 *mac, uint8 bssidx)
{
				    ...
				    struct net_device *ndev)
{
	...
	if (p2p_is_on(cfg) && cfg->p2p->vif_created) {
		...
		if (cfg->scan_request && (cfg->escan_info.ndev == ndev)) {
			...
			wl_notify_escan_complete(cfg, cfg->escan_info.ndev,
						 true, false);
		}
		...
		wl_clr_drv_status(cfg, CONNECTED,
				  wl_to_p2p_bss_ndev(cfg, type));
	...
	if (n_channels > 0) {
		for (i = 0; i < n_channels; i++) {
			...
			params->channel_list[j] =
			    wl_chspec_host_to_driver(params->channel_list[j]);
			j++;
		}
	} else {
	...
	if (!cfg->p2p_supported || !p2p_scan(cfg)) {
		...
		if (ndev == bcmcfg_to_prmry_ndev(cfg)
			...
			struct ieee80211_channel
			    tmp_channel_list[CH_MAX_2G_CHANNEL];
			...
			for (i = 0; i < request->n_channels; i++) {
				...
				if (j > CH_MAX_2G_CHANNEL) {
					WL_ERR(("Index %d exceeds max 2.4GHz channels %d" " and previous 5G connected channel\n", j, CH_MAX_2G_CHANNEL));
					break;
				}
				bcopy(request->channels[i],
				      &tmp_channel_list[j],
		...
		if (request && request->n_channels) {
			...
			if (num_chans == SOCIAL_CHAN_CNT && ((default_chan_list
							      [0] ==
							      SOCIAL_CHAN_1)
							     &&
							     (default_chan_list
		...
		err =
		    wl_cfgp2p_escan(cfg, ndev, cfg->active_scan, num_chans,
...
static s32
wl_do_escan(struct bcm_cfg80211 *cfg, struct wiphy *wiphy,
		   ...
		   struct cfg80211_ssid *this_ssid)
{
	...
	if (remain_on_channel_ndev) {
		...
		wl_notify_escan_complete(cfg, remain_on_channel_ndev, true,
					 true);
	}
	...
	mod_timer(&cfg->scan_timeout,
		  jiffies + msecs_to_jiffies(WL_SCAN_TIMER_INTERVAL_MS));
	if (request) {		/* scan bss */
		ssids = request->ssids;
		p2p_ssid = false;
		...
		if (p2p_ssid) {
			...
			if (!cfg->p2p_supported || !p2p_scan(cfg)) {
				...
				interworking_ie = wl_cfg80211_find_interworking_ie((u8 *)request->ie,
													  request->ie_len);

				if (interworking_ie != NULL) {
					err =
					    wl_cfg80211_add_iw_ie(cfg, ndev,
								  bssidx,
								  VNDR_IE_CUSTOM_FLAG,
								  interworking_ie->id,
...
#endif				/* WL11U */
				err =
				    wl_cfgp2p_set_management_ie(cfg, ndev,
								bssidx,
								VNDR_IE_PRBREQ_FLAG,
								(u8 *) request->
								ie,
...
#define SCAN_EBUSY_RETRY_LIMIT 10
	if (err == -EBUSY) {
		if (busy_count++ > SCAN_EBUSY_RETRY_LIMIT) {
			...
			ret = wldev_ioctl(ndev, WLC_GET_BSSID,
...
static chanspec_t
channel_to_chanspec(struct wiphy *wiphy, struct net_device *dev, u32 channel,
		    u32 bw_cap)
{
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
	u8 *buf = NULL;
...
void
wl_cfg80211_ibss_vsie_set_buffer(vndr_ie_setbuf_t *ibss_vsie,
				 int ibss_vsie_len)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
						   ...
						   char *name)
{
	...
	strlcpy(event->name, name, IFNAMSIZ);
					    ...
					    u8 *mac_addr, const char *name)
{
	 ...
	 */
	 ret = wl_cfg80211_add_del_bss(cfg, primary_ndev,
					   bsscfg_idx, iface_type, 0,
					   addr);
	if (ret < 0) {
		return NULL;
	}
	...
	if (memcmp(addr, event->mac, ETH_ALEN) != 0) {
		ret =
		    wldev_iovar_setbuf_bsscfg(primary_ndev, "cur_etheraddr",
					      addr, ETH_ALEN, cfg->ioctl_buf,
					      WLC_IOCTL_MAXLEN, event->bssidx,
					      &cfg->ioctl_buf_sync);
		if (unlikely(ret)) {
...
s32 wl_cfg80211_del_iface(struct wiphy *wiphy, bcm_struct_cfgdev *cfgdev)
{
	...
	ret = wl_cfg80211_add_del_bss(cfg, ndev,
					   bsscfg_idx, iface_type, true,
					   NULL);
	if (ret < 0) {
...
bcm_cfg80211_del_ibss_if(struct wiphy *wiphy, bcm_struct_cfgdev *cfgdev)
{
	...
	memset(&cfg->if_event_info, 0, sizeof(cfg->if_event_info));
	err =
	    wldev_iovar_setbuf(primary_ndev, "aibss_ifdel", &cfg->ibss_if_addr,
		      ...
		      struct cfg80211_ibss_params *params)
{
	...
	if (wl_get_drv_status(cfg, CONNECTED, dev)) {
		...
		if (!params->bssid
			...
			&& (memcmp(params->ssid, ssid->SSID, ssid->SSID_len) ==
			    0) && (*channel == cfg->channel))) {
			WL_ERR(("Connection already existed to " MACDBG "\n",
				MAC2STRDBG((u8 *)
					   wl_read_prof(cfg, dev,
							WL_PROF_BSSID))));
			return -EISCONN;
		}
	...
	if (chan) {
		...
		bw_cap = param[0];
		chanspec =
		    channel_to_chanspec(wiphy, dev, cfg->channel, bw_cap);
	}
	/*
	...
	if (params->bssid) {
		memcpy(&join_params.params.bssid, params->bssid,
		       ETHER_ADDR_LEN);
		err =
		    wldev_ioctl(dev, WLC_SET_DESIRED_BSSID,
...
wl_set_set_cipher(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	if (is_wps_conn(sme)) {
		...
		if (pval == AES_ENABLED) {
			wpa2_ie =
			      bcm_parse_tlvs((u8 *) sme->ie, sme->ie_len,
					     DOT11_MNG_RSN_ID);
			if ((wpa2_ie != NULL)
			    && (wl_cfg80211_get_rsn_capa(wpa2_ie, rsn_cap) ==
				0)) {
		    ...
		    struct cfg80211_connect_params *sme)
{
...
#endif
	bzero(&bssid, sizeof(bssid));
	err = wldev_ioctl(dev, WLC_GET_BSSID, &bssid, ETHER_ADDR_LEN,
				 false);
	if (!wl_get_drv_status(cfg, CONNECTED, dev) &&
	    err == 0) {
		if (!ETHER_ISNULLADDR(&bssid)) {
			...
			WL_DBG(("drv status CONNECTED is not set, but connected in FW!" MACDBG "/n", MAC2STRDBG(bssid.octet)));
	...
	if (p2p_is_on(cfg) && (dev != bcmcfg_to_prmry_ndev(cfg))) {
		...
		/* find the RSN_IE */
		wpa2_ie = bcm_parse_tlvs((u8 *) sme->ie, sme->ie_len,
					      DOT11_MNG_RSN_ID);
		if (wpa2_ie != NULL) {
			WL_DBG((" WPA2 IE is found\n"));
		}
		/* find the WPA_IE */
		wpa_ie = wl_cfgp2p_find_wpaie((u8 *) sme->ie,
						   sme->ie_len);
		if (wpa_ie != NULL) {
		...
		if (wpa_ie != NULL || wpa2_ie != NULL) {
			wpaie =
			    (wpa_ie != NULL) ? (u8 *) wpa_ie : (u8 *) wpa2_ie;
			wpaie_len =
	...
	if (dhd->roam_env_detection) {
		...
		if (is_roamtrig_reset && is_roam_env_ok) {
			...
			err =
			    wldev_ioctl(dev, WLC_SET_ROAM_TRIGGER, roam_trigger,
	...
	if (chan) {
		...
		WL_DBG(("channel (%d), center_req (%d), %d channels\n",
			cfg->channel, chan->center_freq, chan_cnt));
	} else {
#ifdef ROAM_CHANNEL_CACHE
		wlc_ssid_t ssid;
		int band;
		...
		chan_cnt =
		    get_roam_channel_list(cfg->channel, chanspec_list, &ssid,
					  ioctl_version);
#else
		cfg->channel = 0;
#endif				/* ROAM_CHANNEL_CACHE */
	...
	ext_join_params->scan.nprobes = chan_cnt ?
	    (ext_join_params->scan.active_time /
	     WL_SCAN_JOIN_PROBE_INTERVAL_MS) : -1;
...
#ifdef ROAM_CHANNEL_CACHE
		if (use_chan_cache) {
			memcpy(ext_join_params->assoc.chanspec_list,
			       chanspec_list, sizeof(chanspec_t) * chan_cnt);
			for (i = 0; i < chan_cnt; i++) {
				w_count +=
				    snprintf(chan_ptr + w_count,
					     sizeof(chan_info) - w_count, "%d",
					     wf_chspec_ctlchan(chanspec_list
							       [i]));
				if (i != chan_cnt - 1) {
					w_count +=
					    snprintf(chan_ptr + w_count,
			...
			ext_join_params->assoc.chanspec_list[0] =
			    wl_chspec_host_to_driver(ext_join_params->assoc.
						     chanspec_list[0]);
	...
	err =
	    wldev_iovar_setbuf_bsscfg(dev, "join", ext_join_params,
				      join_params_size, cfg->ioctl_buf,
				      WLC_IOCTL_MAXLEN, bssidx,
				      &cfg->ioctl_buf_sync);
		       ...
		       u16 reason_code)
{
	...
	if (act || wl_get_drv_status(cfg, CONNECTING, dev)) {
		...
		if (wl_get_drv_status(cfg, CONNECTING, dev)) {
			...
			wl_send_event(dev, WLC_E_SET_SSID, WLC_E_STATUS_ABORT,
				      0);
		} else {
...
int wl_cfg80211_enable_roam_offload(struct net_device *dev, bool enable)
{
	int err;
						    ...
						    struct key_params *params))
{
	...
	key.index = key_idx;
	swap_key_from_BE(&key);
	err = wldev_ioctl(dev, WLC_GET_KEY, &key, sizeof(key), false);
	if (err) {
		return err;
	}
	swap_key_to_BE(&key);
	...
	u.index = key.index;
	err = wldev_ioctl(dev, WLC_GET_KEY_SEQ, &u, sizeof(u), false);
	if (err) {
		return err;
	}
	params.seq = u.tsc;
				   ...
				   u8 *mac, struct station_info *sinfo)
{
	...
	if (wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP) {
		...
		WL_INFORM(("STA %s : idle time : %d sec, connected time :%d ms\n", bcm_ether_ntoa((const struct ether_addr *) mac, eabuf), sinfo->inactive_time, sta->idle * 1000));
#endif
		...
		if (cfg->roam_offload) {
			struct ether_addr bssid;
			err =
			    wldev_ioctl(dev, WLC_GET_BSSID, &bssid,
					ETHER_ADDR_LEN, false);
			if (err) {
				WL_ERR(("Failed to get current BSSID\n"));
			} else {
				if (memcmp(mac, &bssid.octet, ETHER_ADDR_LEN) !=
				    0) {
					/* roaming is detected */
					err =
					    wl_cfg80211_delayed_roam(cfg, dev,
								     &bssid);
					if (err)
						WL_ERR(("Failed to handle the delayed roam, " "err=%d", err));
					mac = (u8 *) bssid.octet;
				}
			}
		}
		...
		if (err) {
			...
			if (maxrxpktglom != rxpktglom) {
				...
				err =
				    wldev_iovar_setbuf(dev, "bus:maxtxpktglom",
						       (char *) &maxrxpktglom,
						       4, cfg->ioctl_buf,
						       WLC_IOCTL_MAXLEN, NULL);
				if (err < 0) {
		      ...
		      struct cfg80211_pmksa *pmksa)
{
	...
	WL_DBG(("set_pmksa,IW_PMKSA_ADD - PMKID: %pM =\n",
		&cfg->pmk_list->pmkids.pmkid[cfg->pmk_list->pmkids.npmkid -
					     1].BSSID));
	for (i = 0; i < WPA2_PMKID_LEN; i++) {
		WL_DBG(("%02x\n",
			cfg->pmk_list->pmkids.pmkid[cfg->pmk_list->pmkids.
						    npmkid - 1].PMKID[i]));
	}
...
#endif				/* WL_CFG80211_P2P_DEV_IF */
{
	...
	memcpy(&cfg->remain_on_chan, channel,
	       sizeof(struct ieee80211_channel));
...
static void wl_cfg80211_afx_handler(struct work_struct *work)
{
	...
	if (afx_instance != NULL && cfg->afx_hdl->is_active) {
		if (cfg->afx_hdl->is_listen && cfg->afx_hdl->my_listen_chan) {
			ret = wl_cfgp2p_discover_listen(cfg, cfg->afx_hdl->my_listen_chan, (100 * (1 + (RANDOM32() % 3))));	/* 100ms ~ 300ms */
		} else {
			ret = wl_cfgp2p_act_frm_search(cfg, cfg->afx_hdl->dev,
						       cfg->afx_hdl->bssidx,
						       cfg->afx_hdl->
						       peer_listen_chan, NULL);
		}
...
static s32
wl_cfg80211_af_searching_channel(struct bcm_cfg80211 *cfg,
				 struct net_device *dev)
{
...
#ifdef WL11U
static bool
wl_cfg80211_check_DFS_channel(struct bcm_cfg80211 *cfg,
			      wl_af_params_t *af_params, void *frame,
			      u16 frame_len)
{
		...
		if (!wl_cfgp2p_is_p2p_action(frame, frame_len)) {
			...
			for_each_bss(bss_list, bi, i) {
				...
				if (CHSPEC_IS5G(chanspec)
				    &&
				    ((bi->ctl_ch ? bi->
				      ctl_ch : CHSPEC_CHANNEL(chanspec))
				     == af_params->channel)) {
			      ...
			      u16 action_frame_len, s32 bssidx)
{
	...
	if (!wl_cfg80211_check_DFS_channel
	    (cfg, af_params, action_frame->data, action_frame->len)) {
	...
	if (config_af_params.search_channel) {
		/* initialize afx_hdl */
		if (wl_cfgp2p_find_idx(cfg, dev, &cfg->afx_hdl->bssidx) !=
		    BCME_OK) {
			WL_ERR(("Find p2p index from dev(%p) failed\n", dev));
			goto exit;
		}
	...
	ack =
	    wl_cfgp2p_tx_action_frame(cfg, dev, af_params,
				      bssidx) ? false : true;
	...
	if (ack && config_af_params.extra_listen &&
		...
		if (extar_listen_time > 50) {
			wl_set_drv_status(cfg, WAITING_NEXT_ACT_FRM_LISTEN,
					  dev);
			WL_DBG(("Wait more time! actual af time:%d,"
				"calculated extar listen:%d\n",
				af_params->dwell_time, extar_listen_time));
			if (wl_cfgp2p_discover_listen
			    (cfg, cfg->af_sent_channel,
			...
			wl_clr_drv_status(cfg, WAITING_NEXT_ACT_FRM_LISTEN,
					  dev);
		}
	}
#endif				/* WL_CFG80211_SYNC_GON */
	...
	if (ieee80211_is_mgmt(mgmt->frame_control)) {
		if (ieee80211_is_probe_resp(mgmt->frame_control)) {
			...
			struct maclist *assoc_maclist =
			    (struct maclist *) mac_buf;
			if (!bcmp
			    ((const uint8 *) BSSID_BROADCAST,
			     (const struct ether_addr *) mgmt->da,
			     ETHER_ADDR_LEN)) {
				assoc_maclist->count =
				    MAX_NUM_OF_ASSOCIATED_DEV;
				err =
				    wldev_ioctl(dev, WLC_GET_ASSOCLIST,
			...
			enum nl80211_channel_type channel_type)
{
	...
	if (chan->band == IEEE80211_BAND_5GHZ) {
		...
		if (err) {
			if (err != BCME_UNSUPPORTED) {
				...
				err =
				    wldev_iovar_getint(dev, "mimo_bw_cap",
						       &bw_cap);
				if (err) {
	...
	if (wf_chspec_valid(chspec)) {
		...
		if (fw_chspec != INVCHANSPEC) {
			err = wldev_iovar_setint(dev, "chanspec",
						      fw_chspec);
			if (err == BCME_BADCHAN) {
...
#ifdef WL_CFG80211_VSDB_PRIORITIZE_SCAN_REQUEST
struct net_device *wl_cfg80211_get_remain_on_channel_ndev(struct bcm_cfg80211
							  *cfg)
{
	struct net_info *_net_info, *next;
	list_for_each_entry_safe(_net_info, next, &cfg->net_list, list) {
		if (_net_info->ndev &&
		    test_bit(WL_STATUS_REMAINING_ON_CHANNEL,
			     &_net_info->sme_state))
...
wl_validate_wpa2ie(struct net_device *dev, bcm_tlv_t *wpa2ie, s32 bssidx)
{
	...
	len -=
	     (WPA_IE_SUITE_COUNT_LEN + (WPA_SUITE_LEN * suite_count));
	if (len <= 0)
		return BCME_BADLEN;
	...
	if (len >=
		...
		err =
		    wldev_iovar_setint_bsscfg(dev, "wme_bss_disable",
...
static s32
wl_cfg80211_bcn_validate_sec(struct net_device *dev,
			     struct parsed_ies *ies, u32 dev_role, s32 bssidx)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
	...
	if (dev_role == NL80211_IFTYPE_P2P_GO && (ies->wpa2_ie)) {
		...
		if (ies->wps_ie) {
			cfg->ap_info->wps_ie =
			    kmemdup(ies->wps_ie, ies->wps_ie_len, GFP_KERNEL);
		}
	}
				      ...
				      u32 dev_role, s32 bssidx)
{
	...
	if (info->beacon_interval) {
		err = wldev_ioctl(dev, WLC_SET_BCNPRD,
				       &info->beacon_interval, sizeof(s32),
				       true);
		if (err < 0) {
	...
	if (info->dtim_period) {
		err = wldev_ioctl(dev, WLC_SET_DTIMPRD,
				       &info->dtim_period, sizeof(s32),
				       true);
		if (err < 0) {
	...
	if ((info->ssid) && (info->ssid_len > 0) &&
		...
		if (dev_role == NL80211_IFTYPE_AP) {
			...
			memset(cfg->hostapd_ssid.SSID, 0x00,
			       DOT11_MAX_SSID_LEN);
			memcpy(cfg->hostapd_ssid.SSID, info->ssid,
			       info->ssid_len);
...
static s32 wl_cfg80211_parse_ies(u8 *ptr, u32 len, struct parsed_ies *ies)
{
	s32 err = BCME_OK;
	...
	/* find the WPSIE */
	ies->wps_ie = wl_cfgp2p_find_wpsie(ptr, len);
	if (ies->wps_ie != NULL) {
		WL_DBG(("WPSIE in beacon \n"));
		ies->wps_ie_len =
		    ies->wps_ie->length + WPA_RSN_IE_TAG_FIXED_LEN;
	} else {
		WL_ERR(("No WPSIE in beacon \n"));
	}

	/* find the RSN_IE */
	ies->wpa2_ie = bcm_parse_tlvs(ptr, len, DOT11_MNG_RSN_ID);
	if (ies->wpa2_ie != NULL) {
	...
	/* find the WPA_IE */
	ies->wpa_ie = wl_cfgp2p_find_wpaie(ptr, len);
	if (ies->wpa_ie != NULL) {
		WL_DBG((" WPA found\n"));
...
static s32
wl_cfg80211_bcn_bringup_ap(struct net_device *dev,
			   struct parsed_ies *ies, u32 dev_role, s32 bssidx)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
	struct wl_join_params join_params;
	...
	if (dev_role == NL80211_IFTYPE_P2P_GO) {
		...
		if (!is_bssup && (ies->wpa2_ie != NULL)) {
			...
			err =
			    wldev_ioctl(dev, WLC_SET_INFRA, &infra,
					sizeof(s32), true);
			if (err < 0) {
			...
			err = wl_cfgp2p_bss(cfg, dev, bssidx, 1);
			if (err < 0) {
...
#ifdef MAX_GO_CLIENT_CNT
			err =
			    wldev_iovar_setint_bsscfg(dev, "bss_maxassoc",
						      bss_maxassoc, bssidx);
			if (unlikely(err)) {
...
#ifdef DISABLE_11H_SOFTAP
		err =
		    wldev_ioctl(dev, WLC_SET_SPECT_MANAGMENT, &spect,
				sizeof(s32), true);
		if (err < 0) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0))
s32
wl_cfg80211_parse_ap_ies(struct net_device *dev,
...
s32
wl_cfg80211_set_ies(struct net_device *dev,
				   ...
				   struct parsed_ies *ies, s32 bssidx)
{
	...
	if (ies->wps_ie) {
		if (cfg->ap_info->wps_ie &&
		    memcmp(cfg->ap_info->wps_ie, ies->wps_ie,
			   ies->wps_ie_len)) {
		...
		if ((ies->wpa_ie != NULL || ies->wpa2_ie != NULL)) {
			if (!cfg->ap_info->security_mode) {
				...
				if (ies->wpa_ie != NULL) {
				    ...
				    if (memcmp
					(cfg->ap_info->rsn_ie, ies->wpa2_ie,
					 ies->wpa2_ie->len +
					 WPA_RSN_IE_TAG_FIXED_LEN)) {
					update_bss = true;
					kfree(cfg->ap_info->rsn_ie);
			...
			if (update_bss) {
				...
				if (wl_validate_wpa2ie
				    (dev, ies->wpa2_ie, bssidx) < 0
				    || wl_validate_wpaie(dev, ies->wpa_ie,
							 bssidx) < 0) {
					return BCME_ERROR;
				}
			   ...
			   u8 *mac, struct station_parameters *params)
{
	...
	if (!(params->sta_flags_set & BIT(NL80211_STA_FLAG_AUTHORIZED))) {
		err =
		    wldev_ioctl(dev, WLC_SCB_DEAUTHORIZE, (void *) mac,
				ETH_ALEN, true);
		if (err)
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0))
static s32
wl_cfg80211_start_ap(struct wiphy *wiphy,
		     struct net_device *dev, struct cfg80211_ap_settings *info)
{
	...
	err = wl_cfg80211_set_channel(wiphy, dev,
						   dev->ieee80211_ptr->preset_chandef.chan, NL80211_CHAN_HT20);
	...
	err = wl_cfg80211_bcn_set_params(info, dev, dev_role, bssidx);
	if (err < 0) {
	...
	/* Parse IEs */
	err = wl_cfg80211_parse_ap_ies(dev, &info->beacon, &ies);
	if (err < 0) {
	...
	err = wl_cfg80211_bcn_bringup_ap(dev, &ies, dev_role, bssidx);
	if (err < 0) {
	...
	err = wl_cfg80211_set_mac_acl(wiphy, dev, info->acl);
	if (err < 0) {
	...
	/* Set IEs to FW */
	err = wl_cfg80211_set_ies(dev, &info->beacon, bssidx);
	if (err < 0)
		WL_ERR(("Set IEs failed \n"));
...
static s32
wl_cfg80211_change_beacon(struct wiphy *wiphy,
			  struct net_device *dev,
			  struct cfg80211_beacon_data *info)
{
	s32 err = BCME_OK;
	...
	/* Parse IEs */
	err = wl_cfg80211_parse_ap_ies(dev, info, &ies);
	if (err < 0) {
	...
	/* Set IEs to FW */
	err = wl_cfg80211_set_ies(dev, info, bssidx);
	if (err < 0) {
		WL_ERR(("Set IEs failed \n"));
		goto fail;
	}
	...
	if (dev_role == NL80211_IFTYPE_AP) {
		...
		/* Enable Probe Req filter, WPS-AP certification 4.2.13 */
		if ((dev_role == NL80211_IFTYPE_AP) && (ies.wps_ie != NULL)) {
			wl_validate_wps_ie((char *) ies.wps_ie, ies.wps_ie_len,
					   &pbc);
			   ...
			   struct beacon_parameters *info)
{
	...
	/* find the SSID */
	ssid_ie = bcm_parse_tlvs((u8 *) &info->head[ie_offset],
	...
	if (ssid_ie != NULL) {
		if (dev_role == NL80211_IFTYPE_AP) {
			...
			cfg->hostapd_ssid.SSID_len =
			    MIN(ssid_ie->len, DOT11_MAX_SSID_LEN);
...
static bool
is_ssid_in_list(struct cfg80211_ssid *ssid, struct cfg80211_ssid *ssid_list,
		int count)
{
	int i;
			     ...
			     struct cfg80211_sched_scan_request *request)
{
	...
	for (i = 0; i < request->n_match_sets && ssid_cnt < MAX_PFN_LIST_COUNT;
		...
		if (ssid->ssid_len) {
			...
			if (is_ssid_in_list
			    (ssid, hidden_ssid_list, request->n_ssids)) {
	...
	if (ssid_cnt) {
		ret =
		     dhd_dev_pno_set_for_ssid(dev, ssids_local, ssid_cnt,
					      pno_time, pno_repeat,
					      pno_freq_expo_max, NULL,
					      0);
		if (ret < 0) {
				   ...
				   int idx, struct survey_info *info)
{
	...
	if (!idx) {
		...
		val = 1;
		err =
		    wldev_ioctl(ndev, WLC_UP, (void *) &val, sizeof(val),
				true);
		if (err < 0) {
	...
	survey =
	    (struct wl_dump_survey *) kzalloc(sizeof(struct wl_dump_survey),
					      GFP_KERNEL);
	if (unlikely(!survey)) {
			 ...
			 struct regulatory_request *request)
{
	...
	WL_ERR(("Set country code %c%c from %s\n",
		...
		((request->initiator ==
		  NL80211_REGDOM_SET_BY_COUNTRY_IE) ? " 11d AP" : "User")));

	ret = wldev_set_country(bcmcfg_to_prmry_ndev(cfg), request->alpha2,
						 false,
						 (request->initiator ==
						  NL80211_REGDOM_SET_BY_USER ? true : false)));
	if (ret < 0) {
...
static s32 wl_setup_wiphy(struct wireless_dev *wdev,
			  struct device *sdiofunc_dev, void *context)
{
	s32 err = 0;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0))
...
static bool wl_is_linkdown(struct bcm_cfg80211 *cfg, const wl_event_msg_t *e)
{
	...
	if (event == WLC_E_DEAUTH_IND ||
		...
		WL_ERR(("Link down Reason : WLC_E_%s reason = %d status = %d\n",
			wl_dbg_estr[event], ntoh32(e->reason),
			ntoh32(e->status)));
			    ...
			    const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_DISASSOC_IND || event == WLC_E_DEAUTH_IND
		...
		WL_ERR(("event %s(%d) status %d reason %d\n",
			bcmevent_get_name(event), event, ntoh32(e->status),
			reason));
	}
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 2, 0)) && !defined(WL_CFG80211_STA_EVENT)
	WL_DBG(("Enter \n"));
	...
	wldev_iovar_getbuf_bsscfg(ndev, "cur_etheraddr",
				  NULL, 0, cfg->ioctl_buf, WLC_IOCTL_SMLEN,
				  bsscfgidx, &cfg->ioctl_buf_sync);
	...
	err = wldev_ioctl(ndev, WLC_GET_CHANNEL, &ci, sizeof(ci), false);
	if (err) {
		kfree(body);
		return err;
	}
			      ...
			      const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_JOIN || event == WLC_E_START ||
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
		chan =
		    ieee80211_get_channel(bcmcfg_to_wiphy(cfg), cfg->channel);
#endif
		if (wl_get_drv_status(cfg, CONNECTED, ndev)) {
			...
			WL_INFORM(("IBSS BSSID is changed from " MACDBG " to "
				   MACDBG "\n", MAC2STRDBG(cur_bssid),
				   MAC2STRDBG((u8 *) &e->addr)));
			wl_get_assoc_ies(cfg, ndev);
			wl_update_prof(cfg, ndev, NULL, (void *) &e->addr,
				       WL_PROF_BSSID);
			wl_update_bss_info(cfg, ndev, false);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
			cfg80211_ibss_joined(ndev, (s8 *) &e->addr, chan,
					     GFP_KERNEL);
#else
			...
			wl_set_drv_status(cfg, CONNECTED, ndev);
			active = true;
			wl_update_prof(cfg, ndev, NULL, (void *) &active,
				       WL_PROF_ACT);
		}
	...
	if (wl_get_mode_by_netdev(cfg, ndev) == WL_MODE_AP) {
		...
		if (wl_is_linkup(cfg, e, ndev)) {
			...
			if (wl_get_drv_status(cfg, CONNECTED, ndev)) {
				scb_val_t scbval;
				u8 *curbssid =
				    wl_read_prof(cfg, ndev, WL_PROF_BSSID);
				s32 reason = 0;
				if (event == WLC_E_DEAUTH_IND
				    || event == WLC_E_DISASSOC_IND)
				...
				reason =
				    (reason ==
				     WLAN_REASON_UNSPECIFIED) ? 0 : reason;

				WL_ERR(("link down if %s may call cfg80211_disconnected. " "event : %d, reason=%d from " MACDBG "\n", ndev->name, event, ntoh32(e->reason), MAC2STRDBG((u8 *) (&e->addr))));
				if (!cfg->roam_offload &&
				    memcmp(curbssid, &e->addr,
					   ETHER_ADDR_LEN) != 0) {
					WL_ERR(("BSSID of event is not the connected BSSID" "(ignore it) cur: " MACDBG " event: " MACDBG "\n", MAC2STRDBG(curbssid), MAC2STRDBG((u8 *) (&e->addr))));
					return 0;
				}
				wl_clr_drv_status(cfg, CONNECTED, ndev);
				if (!wl_get_drv_status
				    (cfg, DISCONNECTING, ndev)) {
					...
					wl_clr_drv_status(cfg, DISCONNECTING,
							  ndev);
				}
				...
				if ((memcmp
				    ...
				    ||
				    (memcmp
				     (&e->addr, connect_req_bssid,
				      ETHER_ADDR_LEN) == 0))
					/* In case this event comes while associating another AP */
#endif				/* ESCAN_RESULT_PATCH */
					if (!wl_get_drv_status
					    (cfg, DISCONNECTING, ndev))
						wl_bss_connect_done(cfg, ndev,
								    e, data,
								    false);
			}
	...
	if (datalen) {
		...
		if (evt_data->version == ROAM_EXP_EVENT_VERSION) {
			...
			if (ndev) {
				...
				WL_ERR(("SSID is %s\n", ssid->SSID));
				wl_update_prof(cfg, ndev, NULL, ssid,
					       WL_PROF_SSID);
			} else {
				WL_ERR(("NULL ndev!\n"));
			}
		} else {
			WL_ERR(("Version mismatch %d, expected %d",
				evt_data->version, ROAM_EXP_EVENT_VERSION));
		}
	}
	return BCME_OK;
}
					...
					const wl_event_msg_t *e, void *data)
{
	...
	if (datalen) {
		wl_rssi_monitor_evt_t *evt_data =
		    (wl_rssi_monitor_evt_t *) data;
		if (evt_data->version == RSSI_MONITOR_VERSION) {
			...
			WL_ERR(("Version mismatch %d, expected %d",
				evt_data->version, RSSI_MONITOR_VERSION));
		}
	}
	return BCME_OK;
}
			      ...
			      size_t *join_params_size)
{
	...
	if (ch != 0) {
		...
		n_channels =
		    get_roam_channel_list(ch, join_params->params.chanspec_list,
		...
		*join_params_size += WL_ASSOC_PARAMS_FIXED_SIZE +
		...
		join_params->params.chanspec_list[0] =
		    wl_chspec_host_to_driver(join_params->params.
					     chanspec_list[0]);

		join_params->params.chanspec_num =
		    htod32(join_params->params.chanspec_num);
#endif				/* ROAM_CHANNEL_CACHE */
		WL_DBG(("join_params->params.chanspec_list[0]= %X, %d channels\n", join_params->params.chanspec_list[0], join_params->params.chanspec_num));
	}
}
			      ...
			      bool roam)
{
	...
	*(u32 *) cfg->extra_buf = htod32(WL_EXTRA_BUF_MAX);
	...
	if (!bss) {
		...
		band =
		    (channel <=
		     CH_MAX_2G_CHANNEL) ? IEEE80211_BAND_2GHZ :
		    IEEE80211_BAND_5GHZ;
		    ...
		    const wl_event_msg_t *e, void *data)
{
	...
	wl_get_assoc_ies(cfg, ndev);
	wl_update_prof(cfg, ndev, NULL, (void *) (e->addr.octet),
		       WL_PROF_BSSID);
	curbssid = wl_read_prof(cfg, ndev, WL_PROF_BSSID);
	...
	if (wl_get_drv_status(cfg, CONNECTING, ndev)) {
		...
		if (completed) {
			...
			wl_update_prof(cfg, ndev, NULL,
				       (void *) (e->addr.octet), WL_PROF_BSSID);
			curbssid = wl_read_prof(cfg, ndev, WL_PROF_BSSID);
			wl_update_bss_info(cfg, ndev, false);
			wl_update_pmklist(ndev, cfg->pmk_list, err);
			wl_set_drv_status(cfg, CONNECTED, ndev);
...
#ifdef BT_WIFI_HANDOVER
static s32
wl_notify_bt_wifi_handover_req(struct bcm_cfg80211 *cfg,
			       bcm_struct_cfgdev *cfgdev,
			       const wl_event_msg_t *e, void *data)
{
	struct net_device *ndev = NULL;
		      ...
		      const wl_event_msg_t *e, void *data)
{
	...
	case WLC_E_PFN_BSSID_NET_LOST:
		...
		if (len) {
			ptr =
			    dhd_dev_hotlist_scan_event(ndev, data,
						       &send_evt_bytes,
						       HOTLIST_LOST);
			if (ptr) {
				wl_cfgvendor_send_hotlist_event(wiphy, ndev,
	...
	case WLC_E_PFN_GSCAN_FULL_RESULT:
		ptr =
		    dhd_dev_process_full_gscan_result(ndev, data,
						      &send_evt_bytes);
		if (ptr) {
			wl_cfgvendor_send_async_event(wiphy, ndev,
	...
	case WLC_E_PFN_SSID_EXT:
		ptr =
		    dhd_dev_process_epno_result(ndev, data, event,
						&send_evt_bytes);
		if (ptr) {
			wl_cfgvendor_send_async_event(wiphy, ndev,
...
void
wl_stop_wait_next_action_frame(struct bcm_cfg80211 *cfg,
			       struct net_device *ndev)
{
	if (wl_get_drv_status_all(cfg, FINDING_COMMON_CHANNEL)) {
		if (timer_pending(&cfg->p2p->listen_timer)) {
		...
		if (cfg->afx_hdl != NULL) {
			if (cfg->afx_hdl->dev != NULL) {
				wl_clr_drv_status(cfg, SCANNING,
						  cfg->afx_hdl->dev);
			...
			const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_ACTION_FRAME_RX) {
		wldev_iovar_getbuf_bsscfg(ndev, "cur_etheraddr",
					  NULL, 0, cfg->ioctl_buf,
					  WLC_IOCTL_SMLEN, bsscfgidx,
					  &cfg->ioctl_buf_sync);

		err =
		    wldev_ioctl(ndev, WLC_GET_BSSID, &bssid, ETHER_ADDR_LEN,
				false);
		if (err < 0)
		...
		if (wl_cfgp2p_is_pub_action(&mgmt_frame[DOT11_MGMT_HDR_LEN],
			...
			if (sd_act_frm
				...
				if (cfg->next_af_subtype == sd_act_frm->action) {
					...
					wl_clr_drv_status(cfg,
							  WAITING_NEXT_ACT_FRM,
							  ndev);

					/* Stop waiting for next AF. */
					wl_stop_wait_next_action_frame(cfg,
								       ndev);
				}
			}
			(void) sd_act_frm;
		...
		if (prbreq_ies.wps_ie != NULL) {
			wl_validate_wps_ie((char *) prbreq_ies.wps_ie,
		...
		if (event == WLC_E_P2P_PROBREQ_MSG) {
			 ...
			 * GO-NEG Phase
			 */
			if (cfg->p2p && wl_get_p2p_status(cfg, GO_NEG_PHASE)) {
	...
	cfg80211_rx_mgmt(cfgdev, freq, 0, mgmt_frame, mgmt_frame_len,
			 GFP_ATOMIC);
#else
...
static void wl_init_escan_result_buf(struct bcm_cfg80211 *cfg)
{
...
static s32 wl_create_event_handler(struct bcm_cfg80211 *cfg)
{
	...
	PROC_START(wl_event_handler, cfg, &cfg->event_tsk, 0,
		   "wl_event_handler");
				    ...
				    bool aborted, bool fw_abort)
{
	...
	if (cfg->escan_info.ndev != ndev) {
		WL_ERR(("ndev is different %p %p, name%s!=%s\n", cfg->escan_info.ndev, ndev, cfg->escan_info.ndev->name, ndev->name));
		err = BCME_ERROR;
		return err;
	}
...
static s32 wl_escan_handler(struct bcm_cfg80211 *cfg,
			    bcm_struct_cfgdev *cfgdev,
			    const wl_event_msg_t *e, void *data)
{
	s32 err = BCME_OK;
	...
	if (!ndev
		...
		WL_ERR(("escan is not ready ndev %p drv_status 0x%x e_type %d e_states %d name%s\n", ndev, wl_get_drv_status(cfg, SCANNING, ndev), ntoh32(e->event_type), ntoh32(e->status), ndev->name));
		goto exit;
	}
	...
	if (status == WLC_E_STATUS_PARTIAL) {
		...
		if (!
		    (bcmcfg_to_wiphy(cfg)->
		     interface_modes & BIT(NL80211_IFTYPE_ADHOC))) {
		...
		if (wl_get_drv_status_all(cfg, FINDING_COMMON_CHANNEL)) {
			...
			if (p2p_dev_addr
				...
				if ((channel > MAXCHANNEL) || (channel <= 0))
					...
					WL_ERR(("ACTION FRAME SCAN : Peer "
						MACDBG " found,"
						" channel : %d\n",
						MAC2STRDBG(cfg->afx_hdl->
							   tx_dst_addr.octet),
						channel));
			...
			if (scan_req_match(cfg)) {
				...
				if ((cfg->p2p->search_state !=
				     WL_P2P_DISC_ST_SCAN)
				    && (bi->flags & WL_BSS_FLAGS_FROM_BEACON))
					goto exit;
				p2p_ie =
				     wl_cfgp2p_find_p2pie(((u8 *) bi) +
							  bi->ie_offset,
							  bi->ie_length);
				if (p2p_ie == NULL) {
			...
			for (i = 0; i < list->count; i++) {
				...
				if (!bcmp
				    ...
				    (CHSPEC_BAND
				     (wl_chspec_driver_to_host(bi->chanspec))
				     ==
				     CHSPEC_BAND(wl_chspec_driver_to_host
						 (bss->chanspec)))
					...
					if (!
					    (bss->
					     flags & WL_BSS_FLAGS_FROM_BEACON)
					    && (bi->
						flags &
					...
					WL_DBG(("%s(" MACDBG
						...
						bss->SSID,
						MAC2STRDBG(bi->BSSID.octet), i,
						bss->RSSI, bss->flags, bi->RSSI,
						bi->flags));
					...
					if (dtoh32(bss->length) != bi_length) {
						...
						if (list->buflen - prev_len +
						    bi_length >
						    ESCAN_BUF_SIZE) {
							WL_ERR(("Buffer is too small: keep the" " previous result of this AP\n"));
							/* Only update RSSI */
							bss->RSSI = bi->RSSI;
							bss->flags |= (bi->flags
								       &
		...
		if (wl_get_drv_status_all(cfg, FINDING_COMMON_CHANNEL)) {
			...
			if (cfg->afx_hdl->peer_chan == WL_INVALID)
				complete(&cfg->act_frm_scan);
		} else if ((likely(cfg->scan_request))
			   || (cfg->sched_scan_running)) {
...
static void wl_cfg80211_concurrent_roam(struct bcm_cfg80211 *cfg, int enable)
{
	...
	if (enable && connected_cnt > 1) {
		for_each_ndev(cfg, iter, next) {
			...
			err = wldev_iovar_getint(iter->ndev, "roam_off",
						      (s32 *) &iter->roam_off);
			if (err != BCME_OK) {
			...
			err =
			     wldev_iovar_setint(iter->ndev, "roam_off",
						1);
			if (err != BCME_OK) {
				    ...
				    enum wl_status state, bool set)
{
	...
	if (set) {
		if (state == WL_STATUS_CONNECTED) {
			...
			if (mode == WL_MODE_AP) {
				if (wl_add_remove_eventmsg
				    (primary_dev, WLC_E_P2P_PROBREQ_MSG, false))
			...
			if (cfg->vsdb_mode || _net_info->pm_block) {
				...
				for_each_ndev(cfg, iter, next) {
					...
					err =
					    wldev_ioctl(iter->ndev, WLC_GET_PM,
							&iter->pm,
							sizeof(iter->pm),
							false);
					WL_DBG(("%s:power save %s\n",
				...
				for_each_ndev(cfg, iter, next) {
					err =
					     wldev_ioctl(iter->ndev, WLC_SET_PM,
							 &pm, sizeof(pm),
							 true);
					if (err != 0) {
						if (err == -ENODEV)
			...
			if (cfg->vsdb_mode) {
				...
				frameburst = 0;
				if (wldev_ioctl
				    (primary_dev, WLC_SET_FAKEFRAG, &frameburst,
		...
		if (state == WL_STATUS_CONNECTED) {
			...
			wl_update_prof(cfg, _net_info->ndev, NULL, &chan,
				       WL_PROF_CHAN);
			wl_cfg80211_determine_vsdb_mode(cfg);
			for_each_ndev(cfg, iter, next) {
				if (iter->pm_restore && iter->pm) {
					...
					iter->pm_restore = 0;
					wl_cfg80211_update_power_mode(iter->
								      ndev);
				}
			}
	...
	while (down_interruptible(&tsk->sema) == 0) {
		...
		while ((e = wl_deq_event(cfg))) {
			...
			if (WL_IS_P2P_DEV_EVENT(e) && (cfg->p2p_wdev)) {
				...
				ndev =
				    dhd_idx2net((struct dhd_pub *) (cfg->pub),
						e->emsg.ifidx);
				if (ndev)
			...
			if (e->etype < WLC_E_LAST && cfg->evt_handler[e->etype]) {
				cfg->evt_handler[e->etype] (cfg, cfgdev,
							    &e->emsg, e->edata);
			} else {
...
s32 wl_cfg80211_up(void *para)
{
	...
	err = wldev_ioctl(bcmcfg_to_prmry_ndev(cfg), WLC_GET_VERSION, &val,
					   sizeof(int), false);
	if (err < 0) {
...
#error "Both DUAL_STA and DUAL_STA_STATIC_IF can't be enabled together"
	...
	wl_cfg80211_create_iface(cfg->wdev->wiphy, NL80211_IFTYPE_STATION, NULL,
				 "wlan%d");
...
#ifdef PCIE_FULL_DONGLE
		dhd_tdls_update_peer_info(ndev, TRUE,
					  (uint8 *) &e->addr.octet[0]);
#endif				/* PCIE_FULL_DONGLE */
		msg = " TDLS PEER CONNECTED ";
		break;
	case WLC_E_TDLS_PEER_DISCONNECTED:
#ifdef PCIE_FULL_DONGLE
		dhd_tdls_update_peer_info(ndev, FALSE,
					  (uint8 *) &e->addr.octet[0]);
#endif				/* PCIE_FULL_DONGLE */
				 ...
				 u8 *peer, enum nl80211_tdls_operation oper)
{
	...
	case NL80211_TDLS_TEARDOWN:
		...
		/* auto mode off */
		ret =
		    dhd_tdls_enable(dev, true, false,
				    (struct ether_addr *) peer);
		if (ret < 0)
			return ret;
		break;
	...
	if (info.mode) {
		ret =
		    wldev_iovar_setbuf(dev, "tdls_endpoint", &info,
				       sizeof(info), cfg->ioctl_buf,
			       ...
			       enum wl_management_type type)
{
	...
	if (ndev != NULL) {
		...
		if (pktflag)
			ret =
			    wl_cfgp2p_set_management_ie(cfg, ndev, bssidx,
							pktflag, buf, len);
	}
      exit:
	return ret;
...
static bool wl_cfg80211_valid_chanspec_p2p(chanspec_t chanspec)
{
	bool valid = false;
	char chanbuf[CHANSPEC_STR_LEN];
			     ...
			     int *channel)
{
	...
	while (retry--) {
		...
		ret =
		    wldev_ioctl(ndev, WLC_GET_CHANNEL_SEL, &chosen,
				sizeof(chosen), false);
		...
		WL_INFORM(("attempt = %d, ret = %d, chosen = %d\n",
			   (CHAN_SEL_RETRY_COUNT - retry), ret,
			   dtoh32(chosen)));
	}
...
wl_cfg80211_get_best_channels(struct net_device *dev, char *cmd, int total_len)
{
	...
	ret =
	    wl_cfg80211_get_best_channel(ndev, (void *) buf, CHANSPEC_BUF_SIZE,
					 &channel);
	if (ret < 0) {
	...
	snprintf(pos, total_len, "%04d ", channel);
	pos += 5;
		    ...
		    size_t count, loff_t *ppos)
{
	...
	params = &tbuf[0];
	colon = strnchr(params, strlen(params), '\n');
	if (colon != NULL)
		*colon = '\0';
	while ((token = strsep(&params, " ")) != NULL) {
		...
		if (*token == '\0')
			continue;
		colon = strnchr(token, strlen(token), ':');
		if (colon != NULL) {
			*colon = ' ';
		}
		...
		if (tokens == 2) {
			for (i = 0; i < ARRAYSIZE(sublogname_map); i++) {
				if (!strncmp
				    (sublog, sublogname_map[i].sublogname,
...
{
	wldev_iovar_getbuf_bsscfg(bcmcfg_to_prmry_ndev(cfg), "cur_etheraddr",
				  NULL, 0, cfg->ioctl_buf, WLC_IOCTL_SMLEN, 0,
				  &cfg->ioctl_buf_sync);
...
bcm_tlv_t *wl_cfg80211_find_interworking_ie(u8 *parse, u32 len)
{
	...
	while ((ie =
		bcm_parse_tlvs(parse, (u32) len, DOT11_MNG_INTERWORKING_ID))) {
		return (bcm_tlv_t *) ie;
	}
	return NULL;
}
		      ...
		      uint8 data_len)
{
	...
	strlcpy(ie_setbuf->cmd, "add", VNDR_IE_CMD_LEN);
	ie_setbuf->cmd[VNDR_IE_CMD_LEN - 1] = '\0';

	/* Buffer contains only 1 IE */
	iecount = htod32(1);
	memcpy((void *) &ie_setbuf->ie_buffer.iecount, &iecount, sizeof(int));
	memcpy((void *) &ie_setbuf->ie_buffer.ie_list[0].pktflag, &pktflag,
	       sizeof(uint32));
	...
	memcpy((uchar *) &ie_setbuf->ie_buffer.ie_list[0].ie_data.data[0],
	       data, data_len);
			...
			const struct cfg80211_acl_data *acl)
{
	...
	if (macmode == MACLIST_MODE_DISABLED) {
		ret =
		     wl_android_set_ap_mac_list(cfgdev, macmode, NULL);
		if (ret != 0)
	...
	/* set the list */
	ret = wl_android_set_ap_mac_list(cfgdev, macmode, list);
	if (ret != 0)
		WL_ERR(("%s : Setting MAC list failed error=%d\n", __FUNCTION__,
			ret));
