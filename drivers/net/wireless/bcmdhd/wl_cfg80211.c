...
#if defined(WL_SELF_MANAGED_REGDOM) && (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0))	
	wdev->wiphy->regulatory_flags |= REGULATORY_WIPHY_SELF_MANAGED;
#elif defined(WL_SELF_MANAGED_REGDOM) && (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 0, 0))
        wdev->wiphy->regulatory_flags |= REGULATORY_WIPHY_SELF_MANAGED;
...
wl_cfg80211_net_attach(struct net_device *primary_ndev)
{
	...
	for (i = 0; i < DHD_NUM_STATIC_IFACES; i++) {
		snprintf(iname, sizeof(iname), WL_STATIC_IFNAME_PREFIX, i+2);
		if (wl_cfg80211_register_static_if(cfg, NL80211_IFTYPE_AP,
