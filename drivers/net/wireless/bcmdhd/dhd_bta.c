...
int dhd_bta_docmd(dhd_pub_t *pub, void *cmd_buf, uint cmd_len)
{
	...
	len = (uint) cmd->plen + HCI_CMD_PREAMBLE_SIZE;
	if (len > cmd_len) {
...
int dhd_bta_tx_hcidata(dhd_pub_t *pub, void *data_buf, uint data_len)
{
	...
	len = (uint) ltoh16(data->dlen) + HCI_ACL_DATA_PREAMBLE_SIZE;
	if (len > data_len) {
...
void dhd_bta_doevt(dhd_pub_t *dhdp, void *data_buf, uint data_len)
{
	...
	case HCI_Command_Complete:{
			...
			case HCI_Read_Data_Block_Size:{
					read_data_block_size_evt_parms_t *parms2
					    ...
					    parms->parms;
					dhdp->maxdatablks =
					    ltoh16_ua((uint8 *) &parms2->data_block_num);
					break;
				}
			}
			break;
	...
	case HCI_Flush_Occurred:{
			...
			dhd_bta_flush_hcidata(dhdp,
					      ltoh16_ua((uint8 *) &evt_parms->handle));
			break;
		}
	default:
		break;
