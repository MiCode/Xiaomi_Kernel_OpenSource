...
uint dhd_prot_hdrlen(dhd_pub_t *dhd, void *PKTBUF)
{
	...
	hdrlen =
	    BDC_HEADER_LEN + (((struct bdc_header *) PKTBUF)->dataOffset * 4);
#endif
	return hdrlen;
}
...
int dhd_prot_attach(dhd_pub_t *dhd)
{
	...
	cdc = (dhd_prot_t *) DHD_OS_PREALLOC(dhd, DHD_PREALLOC_PROT,
							sizeof(dhd_prot_t));
	if (!cdc) {
...
dhd_get_hostreorder_pkts(void *osh, struct reorder_info *ptr, void **pkt,
			 uint32 *pkt_count, void **pplast, uint8 start,
			 uint8 end)
{
	void *plast = NULL, *p;
	uint32 pkt_cnt = 0;
			     ...
			     uint32 *pkt_count)
{
	...
	DHD_REORDER(("flow_id %d, flags 0x%02x, idx(%d, %d, %d)\n", flow_id,
		     flags, reorder_info_buf[WLHOST_REORDERDATA_CURIDX_OFFSET],
	...
	*pkt = NULL;
	...
	if (flags & WLHOST_REORDERDATA_NEW_HOLE) {
		DHD_REORDER(("%s: new hole, so cleanup pending buffers\n",
			     __FUNCTION__));
		if (ptr->pend_pkts) {
			dhd_get_hostreorder_pkts(dhd->osh, ptr, pkt, &cnt,
						 &plast, ptr->exp_idx,
						 ptr->exp_idx);
			ptr->pend_pkts = 0;
		}
		...
		*pkt_count = cnt;
		...
		if ((exp_idx == ptr->exp_idx) && (cur_idx != ptr->exp_idx)) {
			...
			*pkt_count = cnt;
			...
			DHD_REORDER(("%s:, flow %d, both moved, cur %d(%d), exp %d(%d)\n", __FUNCTION__, flow_id, ptr->cur_idx, cur_idx, ptr->exp_idx, exp_idx));
