...
/* Firmware requested operation mode */
	...
	DHD_FLAG_CONCURR_SINGLE_CHAN_MODE =
	    (DHD_FLAG_STA_MODE | DHD_FLAG_P2P_MODE),
...
extern int dhd_os_get_image_block_seek(char *buf, int len, void *image, int offset);
...
#if defined(KEEP_ALIVE)
extern int dhd_dev_start_mkeep_alive(dhd_pub_t *dhd_pub, u8 mkeep_alive_id,
				     u8 *ip_pkt, u16 ip_pkt_len,
...
extern int dhd_wl_ioctl_get_intiovar(dhd_pub_t *dhd_pub, char *name,
				     uint *pval, int cmd, uint8 set,
				     int ifidx);
...
extern int dhd_iovar(dhd_pub_t *pub, int ifidx, char *name, char *cmd_buf,
...
#define FW_SUPPORTED(dhd, capa) ((strnstr(dhd->fw_capabilities, #capa, WLC_IOCTL_SMLEN) != NULL))
...
void dhd_tdls_update_peer_info(struct net_device *dev, bool connect_disconnect,
			       uint8 *addr);
