...
bool pkt_is_dhcp(osl_t *osh, void *p)
{
	...
	src_port =
	    ntoh16(*(uint16 *) (pt + ETHER_TYPE_LEN + IPV4_OPTIONS_OFFSET));
...
static tdata_psh_info_t *_tdata_psh_info_pool_deq(tcpack_sup_module_t *
						  tcpack_sup_mod)
{
				     ...
				     tcpack_sup_module_t *tcpack_sup_mod)
{
	...
	for (i = 0; i < TCPDATA_PSH_INFO_MAXNUM; i++)
		_tdata_psh_info_pool_enq(tcpack_sup_mod,
					 &tdata_psh_info_pool[i]);
					...
					tcpack_sup_module_t *tcpack_sup_mod)
{
	...
	for (i = 0; i < tcpack_sup_mod->tcpdata_info_cnt; i++) {
		tcpdata_info_t *tcpdata_info =
		    &tcpack_sup_mod->tcpdata_info_tbl[i];
...
int dhd_tcpack_suppress_set(dhd_pub_t *dhdp, uint8 mode)
{
	...
	if (mode == TCPACK_SUP_OFF) {
		...
		MFREE(dhdp->osh, dhdp->tcpack_sup_module,
...
inline int dhd_tcpack_check_xmit(dhd_pub_t *dhdp, void *pkt)
{
	...
	for (i = 0; i < tbl_cnt; i++) {
		if (tcpack_info_tbl[i].pkt_in_q == pkt) {
			...
			bzero(&tcpack_info_tbl[tbl_cnt - 1],
			      sizeof(tcpack_info_t));
					 ...
					 uint8 *tcp_hdr, uint32 tcp_ack_num)
{
	...
	for (i = 0; i < tcpack_sup_mod->tcpdata_info_cnt; i++) {
		tcpdata_info_t *tcpdata_info_tmp =
		    &tcpack_sup_mod->tcpdata_info_tbl[i];
		DHD_TRACE(("%s %d: data info[%d], IP addr " IPV4_ADDR_STR " "
			   IPV4_ADDR_STR " TCP port %d %d\n", __FUNCTION__,
			   __LINE__, i,
			   IPV4_ADDR_TO_STR(ntoh32_ua
					    (tcpdata_info_tmp->src_ip_addr)),
			   IPV4_ADDR_TO_STR(ntoh32_ua
					    (tcpdata_info_tmp->dst_ip_addr)),
	...
	while ((tdata_psh_info = tcpdata_info->tdata_psh_info_head)) {
		if (IS_TCPSEQ_GE(tcp_ack_num, tdata_psh_info->end_seq)) {
			DHD_TRACE(("%s %d: PSH ACKED! %u >= %u\n",
				   __FUNCTION__, __LINE__, tcp_ack_num,
...
bool dhd_tcpack_suppress(dhd_pub_t *dhdp, void *pkt)
{
	...
	DHD_TRACE(("%s %d: TCP ACK with zero DATA length"
		   ...
		   IPV4_ADDR_TO_STR(ntoh32_ua
				    (&new_ip_hdr[IPV4_DEST_IP_OFFSET])),
	...
	if (dhd_tcpdata_psh_acked
	    (dhdp, new_ip_hdr, new_tcp_hdr, new_tcp_ack_num)) {
	...
	for (i = 0; i < tcpack_sup_mod->tcpack_info_cnt; i++) {
		...
		DHD_TRACE(("%s %d: oldpkt %p[%d], IP addr " IPV4_ADDR_STR " "
			   IPV4_ADDR_STR " TCP port %d %d\n", __FUNCTION__,
			   __LINE__, oldpkt, i,
			   IPV4_ADDR_TO_STR(ntoh32_ua
					    (&old_ip_hdr[IPV4_SRC_IP_OFFSET])),
			   IPV4_ADDR_TO_STR(ntoh32_ua
					    (&old_ip_hdr[IPV4_DEST_IP_OFFSET])),
		...
		if (memcmp(&new_ip_hdr[IPV4_SRC_IP_OFFSET],
		    ...
		    || memcmp(&new_tcp_hdr[TCP_SRC_PORT_OFFSET],
			      &old_tcp_hdr[TCP_SRC_PORT_OFFSET],
			      TCP_PORT_LEN * 2))
			continue;
		...
		if (IS_TCPSEQ_GT(new_tcp_ack_num, old_tcpack_num)) {
			...
			if (new_ip_hdr_len == old_ip_hdr_len &&
				...
				tack_tbl.cnt[6]++;
#endif				/* DEBUG_COUNTER && DHDTCPACK_SUP_DBG */
				DHD_TRACE(("%s %d: lenth mismatch %d != %d || %d != %d" " ACK %u -> %u\n", __FUNCTION__, __LINE__, new_ip_hdr_len, old_ip_hdr_len, new_tcp_hdr_len, old_tcp_hdr_len, old_tcpack_num, new_tcp_ack_num));
			}
			...
			DHD_TRACE(("%s %d: ACK number reverse old %u(0x%p) new %u(0x%p)\n", __FUNCTION__, __LINE__, old_tcpack_num, oldpkt, new_tcp_ack_num, pkt));
		}
		dhd_os_tcpackunlock(dhdp);
		goto exit;
	}
...
bool dhd_tcpdata_info_get(dhd_pub_t *dhdp, void *pkt)
{
	...
	if (IP_VER(ip_hdr) != IP_VER_4 || IPV4_PROT(ip_hdr) != IP_PROT_TCP) {
		DHD_TRACE(("%s %d: Not IPv4 nor TCP! ip ver %d, prot %d\n",
			   __FUNCTION__, __LINE__, IP_VER(ip_hdr),
			   IPV4_PROT(ip_hdr)));
		goto exit;
	}
	...
	while (i < tcpack_sup_mod->tcpdata_info_cnt) {
		...
		DHD_TRACE(("%s %d: data info[%d], IP addr " IPV4_ADDR_STR " "
			   ...
			   __LINE__, i,
			   IPV4_ADDR_TO_STR(ntoh32_ua
					    (tdata_info_tmp->src_ip_addr)),
			   IPV4_ADDR_TO_STR(ntoh32_ua
					    (tdata_info_tmp->dst_ip_addr)),
		...
		if (now_in_ms - tdata_info_tmp->last_used_time >
		    TCPDATA_INFO_TIMEOUT) {
			tdata_psh_info_t *tdata_psh_info_tmp;
			tcpdata_info_t *last_tdata_info;

			while ((tdata_psh_info_tmp =
				tdata_info_tmp->tdata_psh_info_head)) {
				tdata_info_tmp->tdata_psh_info_head =
				    tdata_psh_info_tmp->next;
			...
			last_tdata_info =
			    &tcpack_sup_mod->tcpdata_info_tbl[tcpack_sup_mod->
							      tcpdata_info_cnt];
			if (i < tcpack_sup_mod->tcpdata_info_cnt) {
				ASSERT(last_tdata_info != tdata_info_tmp);
				bcopy(last_tdata_info, tdata_info_tmp,
	...
	if (tcpdata_info == NULL) {
		...
		if (i >= TCPDATA_INFO_MAXNUM) {
			DHD_TRACE(("%s %d: tcp_data_info_tbl FULL! %d %d"
				   " IP addr " IPV4_ADDR_STR " " IPV4_ADDR_STR
				   " TCP port %d %d\n", __FUNCTION__, __LINE__,
				   i, tcpack_sup_mod->tcpdata_info_cnt,
				   IPV4_ADDR_TO_STR(ntoh32_ua
						    (&ip_hdr
						     [IPV4_SRC_IP_OFFSET])),
				   IPV4_ADDR_TO_STR(ntoh32_ua
		 ...
		 */
		DHD_TRACE(("%s %d: Add data info to tbl[%d]: IP addr "
			   IPV4_ADDR_STR " " IPV4_ADDR_STR " TCP port %d %d\n",
			   __FUNCTION__, __LINE__,
			   tcpack_sup_mod->tcpdata_info_cnt,
			   IPV4_ADDR_TO_STR(ntoh32_ua
					    (&ip_hdr[IPV4_SRC_IP_OFFSET])),
			   IPV4_ADDR_TO_STR(ntoh32_ua
					    (&ip_hdr[IPV4_DEST_IP_OFFSET])),
			   ntoh16_ua(&tcp_hdr[TCP_SRC_PORT_OFFSET]),
			   ntoh16_ua(&tcp_hdr[TCP_DEST_PORT_OFFSET])));
