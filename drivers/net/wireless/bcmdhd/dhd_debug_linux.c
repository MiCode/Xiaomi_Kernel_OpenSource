...
static void
debug_data_send(dhd_pub_t *dhdp, int ring_id, const void *data,
...
static void dbg_ring_poll_worker(struct work_struct *work)
{
	...
	while (rlen > 0) {
		...
		hdr =
		    (dhd_dbg_ring_entry_t *) ((void *) hdr + ENTRY_LENGTH(hdr));
	}
...
int
dhd_os_dbg_register_urgent_notifier(dhd_pub_t *dhdp,
		     ...
		     int flags, int time_intval, int threshold)
{
	...
	ret =
	    dhd_dbg_set_configuration(dhdp, ring_id, log_level, flags,
				      threshold);
	if (ret) {
	...
	if (ring_id == FW_VERBOSE_RING_ID || ring_id == FW_EVENT_RING_ID) {
		...
		if (dhd_wl_ioctl_get_intiovar
		    (dhdp, "rte_timesync", &ms, WLC_GET_VAR, FALSE, 0))
	...
	if (time_intval == 0 || log_level == 0) {
		...
		ring_info->interval =
		    msecs_to_jiffies(time_intval * MSEC_PER_SEC);
...
int dhd_os_suppress_logging(dhd_pub_t *dhdp, bool suppress)
{
	...
	max_log_level =
	    MAX(os_priv[FW_VERBOSE_RING_ID].log_level,
	...
	if (max_log_level == SUPPRESS_LOG_LEVEL) {
		...
		ret =
		    dhd_iovar(dhdp, 0, "logtrace", (char *) &enable,
			      sizeof(enable), 1);
...
int
dhd_os_get_ring_status(dhd_pub_t *dhdp, int ring_id,
...
int
dhd_os_push_push_ring_data(dhd_pub_t *dhdp, int ring_id, void *data,
			   int32 data_len)
{
	int ret = BCME_OK, i;
...
int dhd_os_dbg_attach(dhd_pub_t *dhdp)
{
	...
	ret =
	    dhd_dbg_attach(dhdp, dhd_os_dbg_pullreq, dhd_os_dbg_urgent_notifier,
			   os_priv);
	if (ret)
		MFREE(dhdp->osh, os_priv,
		      sizeof(*os_priv) * DEBUG_RING_ID_MAX);
