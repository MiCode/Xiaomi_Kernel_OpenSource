...
static int dhd_msgbuf_set_ioctl(dhd_pub_t *dhd, int ifidx, uint cmd,
				void *buf, uint len, uint8 action);
static int dhdmsgbuf_cmplt(dhd_pub_t *dhd, uint32 id, uint32 len, void *buf,
			   void *retbuf);
...
static void dhd_prot_ringstatus_process(dhd_pub_t *dhd, void *buf,
					uint16 msglen);
static void dhd_prot_genstatus_process(dhd_pub_t *dhd, void *buf,
				       uint16 msglen);
static void *dhd_alloc_ring_space(dhd_pub_t *dhd, msgbuf_ring_t *ring,
				  uint16 msglen, uint16 *alloced);
static int dhd_fillup_ioct_reqst_ptrbased(dhd_pub_t *dhd, uint16 len, uint cmd,
					  void *buf, int ifidx);
...
static void dhdmsgbuf_dmaxfer_compare(dhd_pub_t *dhd, void *buf,
				      uint16 msglen);
static void dhd_prot_process_flow_ring_create_response(dhd_pub_t *dhd,
						       void *buf,
						       uint16 msglen);
static void dhd_prot_process_flow_ring_delete_response(dhd_pub_t *dhd,
						       void *buf,
						       uint16 msglen);
static void dhd_prot_process_flow_ring_flush_response(dhd_pub_t *dhd,
						      void *buf, uint16 msglen);
...
static uint16 dhd_msgbuf_rxbuf_post_ctrlpath(dhd_pub_t *dhd, bool event_buf,
					     uint32 max_to_post);
...
static msgbuf_ring_t *prot_ring_attach(dhd_prot_t *prot, char *name,
				       uint16 max_item, uint16 len_item,
				       uint16 ringid);
static void *prot_get_ring_space(msgbuf_ring_t *ring, uint16 nitems,
				 uint16 *alloced);
static void dhd_set_dmaed_index(dhd_pub_t *dhd, uint8 type, uint16 ringid,
				uint16 new_index);
static uint16 dhd_get_dmaed_index(dhd_pub_t *dhd, uint8 type, uint16 ringid);
static void prot_ring_write_complete(dhd_pub_t *dhd, msgbuf_ring_t *ring,
				     void *p, uint16 len);
static void prot_upd_read_idx(dhd_pub_t *dhd, msgbuf_ring_t *ring);
static uint8 *prot_get_src_addr(dhd_pub_t *dhd, msgbuf_ring_t *ring,
				uint16 *available_len);
static void prot_store_rxcpln_read_idx(dhd_pub_t *dhd, msgbuf_ring_t *ring);
static void prot_early_upd_rxcpln_read_idx(dhd_pub_t *dhd,
					   msgbuf_ring_t *ring);
...
static INLINE uint32 dhd_pktid_map_reserve(dhd_pktid_map_handle_t *handle,
					   void *pkt);
static INLINE void dhd_pktid_map_save(dhd_pktid_map_handle_t *handle,
				      void *pkt, uint32 nkey,
 ...
 */
static void dhd_pktid_map_fini(dhd_pktid_map_handle_t *handle)
{
	void *osh;
	int nkey;
	dhd_pktid_map_t *map;
...
static void dhd_pktid_map_clear(dhd_pktid_map_handle_t *handle)
{
	void *osh;
	int nkey;
	dhd_pktid_map_t *map;
...
#ifdef DHD_USE_STATIC_IOCTLBUF
			if (locker->buf_type == BUFF_TYPE_IOCTL_RX)
				PKTFREE_STATIC(osh, (ulong *) locker->pkt,
					       FALSE);
			else
				PKTFREE(osh, (ulong *) locker->pkt, FALSE);
#else
...
int dhd_prot_attach(dhd_pub_t *dhd)
{
	...
	/* Ring Allocations */
	/* 1.0   H2D    TXPOST ring */
	prot->h2dring_txp_subn = prot_ring_attach(prot, "h2dtxp",
	...
	/* 2.0   H2D    RXPOST ring */
	prot->h2dring_rxp_subn = prot_ring_attach(prot, "h2drxp",
	...
	prot->h2dring_ctrl_subn = prot_ring_attach(prot, "h2dctrl",
	...
	prot->d2hring_tx_cpln = prot_ring_attach(prot, "d2htxcpl",
	...
	prot->d2hring_rx_cpln = prot_ring_attach(prot, "d2hrxcpl",
	...
	prot->d2hring_ctrl_cpln = prot_ring_attach(prot, "d2hctrl",
	...
	prot->retbuf.va =
	    DMA_ALLOC_CONSISTENT(dhd->osh, IOCT_RETBUF_SIZE, DMA_ALIGN_LEN,
				 &alloced, &prot->retbuf.pa,
				 &prot->retbuf.dmah);
	...
	prot->d2h_dma_scratch_buf.va =
	    DMA_ALLOC_CONSISTENT(dhd->osh, DMA_D2H_SCRATCH_BUF_LEN,
				 DMA_ALIGN_LEN, &alloced,
	...
	ASSERT(MODX((unsigned long) prot->d2h_dma_scratch_buf.va, DMA_ALIGN_LEN)
	       == 0);
	bzero(prot->d2h_dma_scratch_buf.va, DMA_D2H_SCRATCH_BUF_LEN);
	OSL_CACHE_FLUSH((void *) prot->d2h_dma_scratch_buf.va,
			DMA_D2H_SCRATCH_BUF_LEN);

	/* PKTID handle INIT */
	prot->pktid_map_handle =
	    NATIVE_TO_PKTID_INIT(dhd->osh, MAX_PKTID_ITEMS);
...
int dhd_prot_init_index_dma_block(dhd_pub_t *dhd, uint8 type, uint32 length)
{
	uint alloced = 0;
...
int dhd_prot_init(dhd_pub_t *dhd)
{
	...
	dhd_bus_cmn_writeshared(dhd->bus, &prot->d2h_dma_scratch_buf.pa,
				...
				DNGL_TO_HOST_DMA_SCRATCH_BUFFER, 0);
	dhd_bus_cmn_writeshared(dhd->bus, &prot->d2h_dma_scratch_buf_len,
				sizeof(prot->d2h_dma_scratch_buf_len),
				DNGL_TO_HOST_DMA_SCRATCH_BUFFER_LEN, 0);
	...
	if (DMA_INDX_ENAB(dhd->dma_d2h_ring_upd_support)) {
		dhd_bus_cmn_writeshared(dhd->bus,
					&prot->d2h_dma_writeindx_buf.pa,
...
dhd_prot_print_metadata(dhd_pub_t *dhd, void *ptr, int len)
{
	...
	while (len > TLV_HDR_LEN) {
		...
		case WLFC_CTL_TYPE_FIFO_CREDITBACK:
			bcm_print_bytes("METADATA FIFO_CREDITBACK", tlv_v,
					tlv_l);
			break;
...
dhd_prot_packet_get(dhd_pub_t *dhd, uint32 pktid, uint8 buf_type)
{
	...
	uint32 pa_len;
	PKTBUF =
	    PKTID_TO_NATIVE(dhd->prot->pktid_map_handle, pktid, pa, pa_len,
			    buf_type);
	if (PKTBUF) {
...
static int BCMFASTPATH dhd_msgbuf_rxbuf_post(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;
	int16 fillbufs;
	uint16 cnt = 64;
...
static int BCMFASTPATH dhd_prot_rxbufpost(dhd_pub_t *dhd, uint16 count)
{
	void *p;
	...
	for (i = 0; i < alloced; i++) {
		...
		physaddr =
		    DMA_MAP(dhd->osh, PKTDATA(dhd->osh, p), pktlen, DMA_RX, p,
			    0);
		if (PHYSADDRISZERO(physaddr)) {
			if (RING_WRITE_PTR(ring) < alloced - i)
				RING_WRITE_PTR(ring) =
				    RING_MAX_ITEM(ring) - alloced + i;
			else
		...
		rxbuf_post->cmn_hdr.request_id =
		    htol32(NATIVE_TO_PKTID
			   (dhd->prot->pktid_map_handle, p, physaddr, pktlen,
			    DMA_RX, BUFF_TYPE_DATA_RX));
		...
		rxbuf_post->data_buf_addr.high_addr =
		    htol32(PHYSADDRHI(physaddr));
		...
		if (prot->rx_metadata_offset) {
			...
			rxbuf_post->metadata_buf_addr.high_addr =
			    htol32(PHYSADDRHI(physaddr));
			rxbuf_post->metadata_buf_addr.low_addr =
			    htol32(PHYSADDRLO(physaddr));
		} else {
	...
	if (alloced > 0)
		prot_ring_write_complete(dhd, prot->h2dring_rxp_subn, msg_start,
					 alloced);

	return alloced;
}

static int dhd_prot_rxbufpost_ctrl(dhd_pub_t *dhd, bool event_buf)
{
	void *p;
	uint16 pktsz;
	...
	pktlen = PKTLEN(dhd->osh, p);
	physaddr =
	    DMA_MAP(dhd->osh, PKTDATA(dhd->osh, p), pktlen, DMA_RX, p, 0);
	if (PHYSADDRISZERO(physaddr)) {
...
static uint16
dhd_msgbuf_rxbuf_post_ctrlpath(dhd_pub_t *dhd, bool event_buf,
			       uint32 max_to_post)
{
	uint32 i = 0;
	int32 ret_val;
...
static int dhd_msgbuf_rxbuf_post_ioctlresp_bufs(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;
	uint16 retcnt = 0;
...
static int dhd_msgbuf_rxbuf_post_event_bufs(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;
...
int BCMFASTPATH dhd_prot_process_msgbuf_rxcpl(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;
	...
	while (TRUE) {
		...
		src_addr =
		    prot_get_src_addr(dhd, prot->d2hring_rx_cpln, &src_len);
		if (src_addr == NULL)
			break;
		...
		if (dhd_prot_process_msgtype
		    (dhd, prot->d2hring_rx_cpln, src_addr,
		     src_len) != BCME_OK) {
...
dhd_prot_update_txflowring(dhd_pub_t *dhd, uint16 flow_id, void *msgring_info)
{
	...
	DHD_TRACE(("flow %d, write %d read %d \n\n", flow_id,
		   RING_WRITE_PTR(ring), RING_READ_PTR(ring)));

	/* Need more logic here, but for now use it directly */
	dhd_bus_schedule_queue(dhd->bus, flow_id, TRUE);
}

int BCMFASTPATH dhd_prot_process_msgbuf_txcpl(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;
	...
	while (TRUE) {
		...
		src_addr =
		    prot_get_src_addr(dhd, prot->d2hring_tx_cpln, &src_len);
		if (src_addr == NULL)
			break;
		...
		if (dhd_prot_process_msgtype
		    (dhd, prot->d2hring_tx_cpln, src_addr,
		     src_len) != BCME_OK) {
...
int BCMFASTPATH dhd_prot_process_ctrlbuf(dhd_pub_t *dhd)
{
	...
	while (TRUE) {
		...
		uint16 src_len;
		src_addr =
		    prot_get_src_addr(dhd, prot->d2hring_ctrl_cpln, &src_len);
		...
		OSL_PREFETCH(src_addr);
		if (dhd_prot_process_msgtype
		    (dhd, prot->d2hring_ctrl_cpln, src_addr,
		     src_len) != BCME_OK) {
...
dhd_prot_process_msgtype(dhd_pub_t *dhd, msgbuf_ring_t *ring, uint8 *buf,
			 uint16 len)
{
	dhd_prot_t *prot = dhd->prot;
...
void dhd_msgbuf_d2h_check_cmplt(msgbuf_ring_t *ring, void *msg)
{
	uint32 tries;
	uint32 *marker =
	    (uint32 *) msg + RING_LEN_ITEMS(ring) / sizeof(uint32) - 1;
...
static int BCMFASTPATH
dhd_process_msgtype(dhd_pub_t *dhd, msgbuf_ring_t *ring, uint8 *buf,
		    uint16 len)
{
	uint16 pktlen = len;
	uint16 msglen;
...
dhd_prot_ringstatus_process(dhd_pub_t *dhd, void *buf, uint16 msglen)
{
	...
	DHD_ERROR(("ring status: request_id %d, status 0x%04x, flow ring %d, w_offset %d \n", ring_status->cmn_hdr.request_id, ring_status->compl_hdr.status, ring_status->compl_hdr.flow_ring_id, ring_status->write_idx));
...
dhd_prot_ioctcmplt_process(dhd_pub_t *dhd, void *buf, uint16 msglen)
{
	...
	dhd_bus_update_retlen(dhd->bus, sizeof(ioctl_comp_resp_msg_t), pkt_id,
			      status, resp_len);
...
dhd_prot_txstatus_process(dhd_pub_t *dhd, void *buf, uint16 msglen)
{
	...
	if (pkt) {
		...
		if (dhd->prot->tx_metadata_offset && txstatus->metadata_len) {
			...
			ptr =
			    PKTDATA(dhd->osh,
				    pkt) - (dhd->prot->tx_metadata_offset);
			bcm_print_bytes("txmetadata", ptr,
					txstatus->metadata_len);
			dhd_prot_print_metadata(dhd, ptr,
						txstatus->metadata_len);
		}
#endif				/* DHD_DBG_SHOW_METADATA */
...
dhd_prot_rxcmplt_process(dhd_pub_t *dhd, void *buf, uint16 msglen)
{
	...
	DHD_GENERAL_LOCK(dhd, flags);
	pkt =
	    dhd_prot_packet_get(dhd, ltoh32(rxcmplt_h->cmn_hdr.request_id),
				BUFF_TYPE_DATA_RX);
	DHD_GENERAL_UNLOCK(dhd, flags);
	...
	DHD_INFO(("id 0x%04x, offset %d, len %d, idx %d, phase 0x%02x, pktdata %p, metalen %d\n", ltoh32(rxcmplt_h->cmn_hdr.request_id), data_offset, ltoh16(rxcmplt_h->data_len), rxcmplt_h->cmn_hdr.if_id, rxcmplt_h->cmn_hdr.flags, PKTDATA(dhd->osh, pkt), ltoh16(rxcmplt_h->metadata_len)));
...
int BCMFASTPATH dhd_prot_txdata(dhd_pub_t *dhd, void *PKTBUF, uint8 ifidx)
{
	...
	if (!dhd_bus_is_txmode_push(dhd->bus)) {
		...
		flowid = (uint16) DHD_PKTTAG_FLOWID((DHD_PKTTAG_fr_t *)
						    PKTTAG(PKTBUF));
	...
	txdesc->flag2 =
	    (txdesc->flag2 & ~(BCMPCIE_PKT_FLAGS2_FORCELOWRATE_MASK <<
			       BCMPCIE_PKT_FLAGS2_FORCELOWRATE_SHIFT)) |
	    ((dhcp_pkt & BCMPCIE_PKT_FLAGS2_FORCELOWRATE_MASK)
	     << BCMPCIE_PKT_FLAGS2_FORCELOWRATE_SHIFT);
	...
	physaddr =
	    DMA_MAP(dhd->osh, PKTDATA(dhd->osh, PKTBUF), pktlen, DMA_TX, PKTBUF,
		    0);
	...
	if (prot->tx_metadata_offset && (headroom >= prot->tx_metadata_offset)) {
		...
		meta_physaddr = DMA_MAP(dhd->osh, PKTDATA(dhd->osh, PKTBUF),
					prot->tx_metadata_offset, DMA_RX,
					PKTBUF, 0);
		...
		txdesc->metadata_buf_addr.high_addr =
		    htol32(PHYSADDRHI(meta_physaddr));
		txdesc->metadata_buf_addr.low_addr =
		    htol32(PHYSADDRLO(meta_physaddr));
	} else {
...
#else
	prot_ring_write_complete(dhd, msg_ring, txdesc,
				 DHD_FLOWRING_DEFAULT_NITEMS_POSTED_H2D);
#endif
...
int BCMFASTPATH
dhd_prot_hdrpull(dhd_pub_t *dhd, int *ifidx, void *pkt, uchar *buf,
		 uint *len)
{
	return 0;
}

static void BCMFASTPATH dhd_prot_return_rxbuf(dhd_pub_t *dhd, uint16 rxcnt)
{
	dhd_prot_t *prot = dhd->prot;
		   ...
		   int len)
{
	...
	if (action & WL_IOCTL_ACTION_SET) {
		ret =
		    dhd_msgbuf_set_ioctl(dhd, ifidx, ioc->cmd, buf, len,
					 action);
	} else {
		ret =
		    dhdmsgbuf_query_ioctl(dhd, ifidx, ioc->cmd, buf, len,
					  action);
		if (ret > 0)
			ioc->used = ret;
	}
	...
	if (prot->dmaxfer.srcmem.va && prot->dmaxfer.destmem.va) {
		if (memcmp(prot->dmaxfer.srcmem.va,
			...
			bcm_print_bytes("XFER SRC: ",
					prot->dmaxfer.srcmem.va,
					prot->dmaxfer.len);
			bcm_print_bytes("XFER DEST: ", prot->dmaxfer.destmem.va,
					prot->dmaxfer.len);
		} else {
...
dhdmsgbuf_dmaxfer_req(dhd_pub_t *dhd, uint len, uint srcdelay, uint destdelay)
{
	...
	dmap->host_input_buf_addr.low =
	    htol32(PHYSADDRLO(prot->dmaxfer.srcmem.pa));
	dmap->host_ouput_buf_addr.high =
	    htol32(PHYSADDRHI(prot->dmaxfer.destmem.pa));
	dmap->host_ouput_buf_addr.low =
	    htol32(PHYSADDRLO(prot->dmaxfer.destmem.pa));
...
static int
dhdmsgbuf_query_ioctl(dhd_pub_t *dhd, int ifidx, uint cmd, void *buf, uint len,
		      uint8 action)
{
	dhd_prot_t *prot = dhd->prot;
	...
	if (cmd == WLC_GET_VAR && buf) {
		if (!strcmp((char *) buf, "bcmerrorstr")) {
			strlcpy((char *) buf, bcmerrorstr(dhd->dongle_error),
				BCME_STRLEN);
			goto done;
	...
	ret =
	    dhd_fillup_ioct_reqst_ptrbased(dhd, (uint16) len, cmd, buf, ifidx);
...
dhdmsgbuf_cmplt(dhd_pub_t *dhd, uint32 id, uint32 len, void *buf, void *retbuf)
{
	...
	if (ioct_resp.resp_len != 0) {
		DHD_GENERAL_LOCK(dhd, flags);
		pkt =
		    dhd_prot_packet_get(dhd, ioct_resp.cmn_hdr.request_id,
					BUFF_TYPE_IOCTL_RX);
		DHD_GENERAL_UNLOCK(dhd, flags);

		DHD_INFO(("ioctl ret buf %p retlen %d status %x \n", pkt,
			  retlen, ioct_resp.compl_hdr.status));
		/* get ret buf */
		if ((buf) && (pkt)) {
...
static int
dhd_msgbuf_set_ioctl(dhd_pub_t *dhd, int ifidx, uint cmd, void *buf, uint len,
		     uint8 action)
{
	dhd_prot_t *prot = dhd->prot;
...
static void *BCMFASTPATH
dhd_alloc_ring_space(dhd_pub_t *dhd, msgbuf_ring_t *ring, uint16 nitems,
		     uint16 *alloced)
{
	void *ret_buf;
	uint16 r_index = 0;
	...
	if (ret_buf == NULL) {
		...
		if (DMA_INDX_ENAB(dhd->dma_d2h_ring_upd_support)) {
			r_index =
			    dhd_get_dmaed_index(dhd, H2D_DMA_READINDX,
						ring->idx);
			ring->ringstate->r_offset = r_index;
		} else
...
static int
dhd_fillup_ioct_reqst_ptrbased(dhd_pub_t *dhd, uint16 len, uint cmd, void *buf,
			       int ifidx)
{
	dhd_prot_t *prot = dhd->prot;
	...
	ioct_rqst =
	    (ioctl_req_msg_t *) dhd_alloc_ring_space(dhd,
						     prot->h2dring_ctrl_subn,
						     DHD_FLOWRING_DEFAULT_NITEMS_POSTED_H2D,
						     &alloced);
	if (ioct_rqst == NULL) {
	...
	ioct_rqst->host_input_buf_addr.high =
	    htol32(PHYSADDRHI(prot->ioctbuf.pa));
	ioct_rqst->host_input_buf_addr.low =
	    htol32(PHYSADDRLO(prot->ioctbuf.pa));
	/* copy ioct payload */
	...
	DHD_CTL(("submitted IOCTL request request_id %d, cmd %d, output_buf_len %d, tx_id %d\n", ioct_rqst->cmn_hdr.request_id, cmd, ioct_rqst->output_buf_len, ioct_rqst->trans_id));
...
void pktid_map_uninit(void *pktid_map_handle)
{
		...
		for (ix = 0; ix < MAX_PKTID_ITEMS; ix++) {
			if (!bcm_mwbmap_isfree(handle->mwbmap_hdl, ix)) {
				...
				DMA_UNMAP(osh, handle->pktid_list[ix + 1].pa,
					  (uint) handle->pktid_list[ix +
								    1].pa_len,
					  handle->pktid_list[ix + 1].dma, 0, 0);
				PKTFREE(osh,
					(unsigned long *) handle->pktid_list[ix
									     +
									     1].native,
					TRUE);
			}
...
uint32 BCMFASTPATH
pktid_map_unique(void *pktid_map_handle, void *pkt, dmaaddr_t physaddr,
		 uint32 physlen, uint32 dma)
{
	uint32 id;
	pktid_map_t *handle = (pktid_map_t *) pktid_map_handle;
	...
	if (handle == NULL) {
		printf
		    ("%s:%d: Error !!! pktid_map_unique called without initing pktid_map\n",
		     __FUNCTION__, __LINE__);
		return 0;
	}
	...
	if (id == BCM_MWBMAP_INVALID_IDX) {
		printf("%s:%d: bcm_mwbmap_alloc failed. Free Count = %d\n",
		       __FUNCTION__, __LINE__,
		       bcm_mwbmap_free_cnt(handle->mwbmap_hdl));
		return 0;
	}
...
void *BCMFASTPATH
pktid_get_packet(void *pktid_map_handle, uint32 id, dmaaddr_t *physaddr,
		 uint32 *physlen)
{
	void *native = NULL;
	pktid_map_t *handle = (pktid_map_t *) pktid_map_handle;
	if (handle == NULL) {
		printf
		    ("%s:%d: Error !!! pktid_get_packet called without initing pktid_map\n",
		     __FUNCTION__, __LINE__);
		return NULL;
	}
	...
	if (bcm_mwbmap_isfree(handle->mwbmap_hdl, (id - 1))) {
		printf
		    ("%s:%d: Error !!!. slot (%d/0x%04x) free but the app is using it.\n",
				       ...
				       uint16 ringid)
{
	...
	ring->ring_base.va =
	    DMA_ALLOC_CONSISTENT(prot->osh, size, DMA_ALIGN_LEN, &alloced,
				 &ring->ring_base.pa, &ring->ring_base.dmah);

	if (ring->ring_base.va == NULL)
		goto fail;
	ring->ringmem->base_addr.high_addr =
	    htol32(PHYSADDRHI(ring->ring_base.pa));
	ring->ringmem->base_addr.low_addr =
	    htol32(PHYSADDRLO(ring->ring_base.pa));
...
fail:
	...
	DMA_FREE_CONSISTENT(prot->osh, ring->ring_base.va, size,
			    ring->ring_base.pa, NULL);
...
prot_get_ring_space(msgbuf_ring_t *ring, uint16 nitems, uint16 *alloced)
{
	...
	ring_avail_cnt =
	    CHECK_WRITE_SPACE(RING_READ_PTR(ring), RING_WRITE_PTR(ring),
			      RING_MAX_ITEM(ring));
	...
	*alloced = MIN(nitems, ring_avail_cnt);
	...
	ret_ptr =
	    (char *) HOST_RING_BASE(ring) +
	    (RING_WRITE_PTR(ring) * RING_LEN_ITEMS(ring));
...
static void BCMFASTPATH
prot_ring_write_complete(dhd_pub_t *dhd, msgbuf_ring_t *ring, void *p,
			 uint16 nitems)
{
	dhd_prot_t *prot = dhd->prot;
 ...
 */
static void
dhd_set_dmaed_index(dhd_pub_t *dhd, uint8 type, uint16 ringid,
		    uint16 new_index)
{
	dhd_prot_t *prot = dhd->prot;
				...
				uint16 *available_len)
{
	...
	DHD_TRACE(("%s: h2d_dma_readindx_buf %p, d2h_dma_writeindx_buf %p\n",
		   __FUNCTION__,
		   (uint32 *) (dhd->prot->h2d_dma_readindx_buf.va),
	...
	if (DMA_INDX_ENAB(dhd->dma_d2h_ring_upd_support)) {
		...
		d2h_w_index =
		    dhd_get_dmaed_index(dhd, D2H_DMA_WRITEINDX, ring->idx);
		ring->ringstate->w_offset = d2h_w_index;
	} else
		dhd_bus_cmn_readshared(dhd->bus,
				       &(RING_WRITE_PTR(ring)), RING_WRITE_PTR,
				       ring->idx);
	...
	*available_len = READ_AVAIL_SPACE(w_ptr, r_ptr, depth);
	...
	ret_addr =
	    (char *) ring->ring_base.va + (r_ptr * ring->ringmem->len_items);

	/* update read pointer */
	if ((ring->ringstate->r_offset + *available_len) >=
	    ring->ringmem->max_item)
...
prot_early_upd_rxcpln_read_idx(dhd_pub_t *dhd, msgbuf_ring_t *ring)
{
	...
	if (DMA_INDX_ENAB(dhd->dma_h2d_ring_upd_support))
		dhd_set_dmaed_index(dhd, D2H_DMA_READINDX,
				    ring->idx,
				    (uint16) prot->rx_cpln_early_upd_idx);
	else
		dhd_bus_cmn_writeshared(dhd->bus,
					&(prot->rx_cpln_early_upd_idx),
					sizeof(uint16), RING_READ_PTR,
					ring->idx);
}
...
dhd_prot_flow_ring_create(dhd_pub_t *dhd, flow_ring_node_t *flow_ring_node)
{
	...
	msgbuf_flow_info = prot_ring_attach(prot, "h2dflr",
							  H2DRING_TXPOST_MAX_ITEM,
							  H2DRING_TXPOST_ITEMSIZE,
							  BCMPCIE_H2D_TXFLOWRINGID +
	...
	flow_create_rqst =
	    (tx_flowring_create_request_t *) dhd_alloc_ring_space(dhd,
								  prot->
								  h2dring_ctrl_subn,
								  DHD_FLOWRING_DEFAULT_NITEMS_POSTED_H2D,
								  &alloced);
	...
	flow_create_rqst->flow_ring_id =
	    htol16((uint16) flow_ring_node->flowid);
	...
	flow_create_rqst->flow_ring_ptr.low_addr =
	    msgbuf_flow_info->ringmem->base_addr.low_addr;
	flow_create_rqst->flow_ring_ptr.high_addr =
	    msgbuf_flow_info->ringmem->base_addr.high_addr;
	flow_create_rqst->max_items = htol16(H2DRING_TXPOST_MAX_ITEM);
	flow_create_rqst->len_item = htol16(H2DRING_TXPOST_ITEMSIZE);
	bcm_ether_ntoa((struct ether_addr *) flow_ring_node->flow_info.da,
		       eabuf);
	DHD_ERROR(("%s Send Flow create Req msglen flow ID %d for peer %s prio %d ifindex %d\n", __FUNCTION__, flow_ring_node->flowid, eabuf, flow_ring_node->flow_info.tid, flow_ring_node->flow_info.ifindex));
	...
	if (DMA_INDX_ENAB(dhd->dma_h2d_ring_upd_support))
		dhd_set_dmaed_index(dhd, H2D_DMA_WRITEINDX,
				    msgbuf_flow_info->idx,
				    (uint16) RING_WRITE_PTR(msgbuf_flow_info));
	else
		dhd_bus_cmn_writeshared(dhd->bus,
					&(RING_WRITE_PTR(msgbuf_flow_info)),
					sizeof(uint16), RING_WRITE_PTR,
					msgbuf_flow_info->idx);
	DHD_GENERAL_UNLOCK(dhd, flags);
...
static void
dhd_prot_process_flow_ring_create_response(dhd_pub_t *dhd, void *buf,
					   uint16 msglen)
{
	tx_flowring_create_response_t *flow_create_resp =
	    (tx_flowring_create_response_t *) buf;

	DHD_ERROR(("%s Flow create Response status = %d Flow %d\n",
		   __FUNCTION__, flow_create_resp->cmplt.status,
		   flow_create_resp->cmplt.flow_ring_id));
...
void dhd_prot_print_info(dhd_pub_t *dhd, struct bcmstrbuf *strbuf)
{
	...
	if (dhd_bus_is_txmode_push(dhd->bus)) {
		...
		dhd_prot_print_flow_ring(dhd, dhd->prot->h2dring_txp_subn,
					 strbuf);
	}
...
dhd_prot_flow_ring_delete(dhd_pub_t *dhd, flow_ring_node_t *flow_ring_node)
{
	...
	DHD_GENERAL_LOCK(dhd, flags);
	flow_delete_rqst =
	    (tx_flowring_delete_request_t *) dhd_alloc_ring_space(dhd,
								  prot->
								  h2dring_ctrl_subn,
								  DHD_FLOWRING_DEFAULT_NITEMS_POSTED_H2D,
								  &alloced);
...
static void
dhd_prot_process_flow_ring_delete_response(dhd_pub_t *dhd, void *buf,
					   uint16 msglen)
{
	tx_flowring_delete_response_t *flow_delete_resp =
	    (tx_flowring_delete_response_t *) buf;
...
int dhd_prot_flow_ring_flush(dhd_pub_t *dhd, flow_ring_node_t *flow_ring_node)
{
	...
	DHD_GENERAL_LOCK(dhd, flags);
	flow_flush_rqst =
	    (tx_flowring_flush_request_t *) dhd_alloc_ring_space(dhd,
								 prot->
								 h2dring_ctrl_subn,
								 DHD_FLOWRING_DEFAULT_NITEMS_POSTED_H2D,
								 &alloced);
...
static void
dhd_prot_process_flow_ring_flush_response(dhd_pub_t *dhd, void *buf,
					  uint16 msglen)
{
...
dhd_rxchain_frame(dhd_pub_t *dhd, void *pkt, uint ifidx)
{
	...
	if (rxchain->pkt_count == 0) {
		...
		if (PKT_CTF_CHAINABLE(dhd, ifidx, eh, prio, rxchain->h_sa,
			...
			/* Keep a copy of ptr to ether_da, ether_sa and prio */
			rxchain->h_da =
			    ((struct ether_header *) eh)->ether_dhost;
			rxchain->h_sa =
			    ((struct ether_header *) eh)->ether_shost;
			rxchain->h_prio = prio;
			rxchain->ifidx = ifidx;
			rxchain->pkt_count++;
		}
...
#endif				/* DHD_RX_CHAINING */
static void dhd_prot_ring_clear(msgbuf_ring_t *ring)
{
	uint16 size;
	DHD_TRACE(("%s\n", __FUNCTION__));
...
void dhd_prot_clear(dhd_pub_t *dhd)
{
	...
	if (prot->d2h_dma_scratch_buf.va) {
		OSL_CACHE_INV((void *) prot->d2h_dma_scratch_buf.va,
			      DMA_D2H_SCRATCH_BUF_LEN);
		bzero(prot->d2h_dma_scratch_buf.va, DMA_D2H_SCRATCH_BUF_LEN);
		OSL_CACHE_FLUSH((void *) prot->d2h_dma_scratch_buf.va,
				DMA_D2H_SCRATCH_BUF_LEN);
	...
	if (prot->h2d_dma_writeindx_buf.va) {
		OSL_CACHE_INV((void *) prot->h2d_dma_writeindx_buf.va,
			      prot->h2d_dma_writeindx_buf_len);
		bzero(prot->h2d_dma_writeindx_buf.va,
		      prot->h2d_dma_writeindx_buf_len);
		OSL_CACHE_FLUSH((void *) prot->h2d_dma_writeindx_buf.va,
				prot->h2d_dma_writeindx_buf_len);
	}
	...
	if (prot->d2h_dma_readindx_buf.va) {
		OSL_CACHE_INV((void *) prot->d2h_dma_readindx_buf.va,
			      prot->d2h_dma_readindx_buf_len);
		bzero(prot->d2h_dma_readindx_buf.va,
		      prot->d2h_dma_readindx_buf_len);
		OSL_CACHE_FLUSH((void *) prot->d2h_dma_readindx_buf.va,
				prot->d2h_dma_readindx_buf_len);
	}
	...
	if (prot->d2h_dma_writeindx_buf.va) {
		OSL_CACHE_INV((void *) prot->d2h_dma_writeindx_buf.va,
			      prot->d2h_dma_writeindx_buf_len);
		bzero(prot->d2h_dma_writeindx_buf.va,
		      prot->d2h_dma_writeindx_buf_len);
		OSL_CACHE_FLUSH((void *) prot->d2h_dma_writeindx_buf.va,
				prot->d2h_dma_writeindx_buf_len);
	}
