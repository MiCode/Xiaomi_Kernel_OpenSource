	     ...
	     uint8 type)
{
	...
	pktp_id = (uint8) pktpool_register(pktp);
	if (pktp_id == PKTPOOL_INVALID_ID) {
		return BCME_ERROR;
	}
...
int pktpool_dbg_dump(pktpool_t *pktp)
{
	...
	for (i = 0; i < pktp->dbg_qlen; i++) {
		...
		printf("%d, p: 0x%x dur:%lu us state:%d\n", i,
		       pktp->dbg_q[i].p, pktp->dbg_q[i].dur / 100,
		       PKTPOOLSTATE(pktp->dbg_q[i].p));
	}
	...
	for (i = 0; i < pktp->dbg_qlen; i++) {
		...
		if (pktp->dbg_q[i].p == p) {
			...
			if (cycles >= pktp->dbg_q[i].cycles)
				pktp->dbg_q[i].dur =
				    cycles - pktp->dbg_q[i].cycles;
			else
				pktp->dbg_q[i].dur =
				    (((uint32) -1) - pktp->dbg_q[i].cycles) +
				    cycles + 1;
