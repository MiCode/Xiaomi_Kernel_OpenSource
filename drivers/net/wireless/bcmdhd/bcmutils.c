...
/* count segments of a chained packet */
uint BCMFASTPATH pktsegcnt_war(osl_t *osh, void *p)
{
	uint cnt;
	uint8 *pktdata;
	...
	for (cnt = 0; p; p = PKTNEXT(osh, p)) {
		...
		if (len > 128) {
			...
			if (((uintptr) pktdata & ~0x7ff) !=
			    ((uintptr) (pktdata + len) & ~0x7ff))
				cnt++;
...
char *bcm_ipv6_ntoa(void *ipv6, char *buf)
{
	...
	for (i = 0; i < IPV6_ADDR_LEN / 2; i++) {
		if ((uint8 *) (a + i) == a4) {
			snprintf(p, 16, ":%u.%u.%u.%u", a4[0], a4[1], a4[2],
				 a4[3]);
			break;
		} else if (i == i_max) {
			*p++ = ':';
...
bool BCMFASTPATH pktgetdscp(uint8 *pktdata, uint pktlen, uint8 *dscp)
{
	...
	if (eh->ether_type == HTON16(ETHER_TYPE_IP)) {
		...
		*dscp = IP_DSCP46(ip_body);
		...
		if (pktlen >=
		    sizeof(struct ethervlan_header) + IPV4_MIN_HEADER_LEN
 ...
 */
bcm_tlv_t *bcm_parse_tlvs_min_bodylen(void *buf, int buflen, uint key,
				      int min_bodylen)
{
...
void
bcm_bprhex(struct bcmstrbuf *b, const char *msg, bool newline, uint8 *buf,
	   int len)
{
	int i;
...
bcm_tlv_t *bcm_find_vendor_ie(void *tlvs, int tlvs_len, const char *voui,
...
/* calculate a * b + c */
void
bcm_uint64_multiple_add(uint32 *r_high, uint32 *r_low, uint32 a, uint32 b,
			uint32 c)
{
...
/* calculate a / b */
void bcm_uint64_divide(uint32 *r, uint32 a_high, uint32 a_low, uint32 b)
{
	...
	while (a1 != 0) {
		r0 += (0xffffffff / b) * a1;
		bcm_uint64_multiple_add(&a1, &a0, ((0xffffffff % b) + 1) % b,
					a1, a0);
	}
...
void bcm_bitprint32(const uint32 u32)
{
	int i;
...
uint32 BCMFASTPATH bcm_mwbmap_alloc(struct bcm_mwbmap *mwbmap_hdl)
{
	...
	for (wordix = 0; wordix < mwbmap_p->wmaps; ++wordix) {
		...
		if (bitmap != 0U) {
			...
			MWBMAP_DBG(("Lvl1: bitix<%02u> wordix<%02u>: %08x ^ %08x = %08x wfree %d", bitix, wordix, *bitmap_p, bitmap, (*bitmap_p) ^ bitmap, count));
			...
			*bitmap_p ^= bitmap;
			...
			MWBMAP_DBG(("Lvl2: bitix<%02u> wordix<%02u>: %08x ^ %08x = %08x ifree %d", bitix, wordix, *bitmap_p, bitmap, (*bitmap_p) ^ bitmap, mwbmap_p->ifree));
...
void bcm_mwbmap_force(struct bcm_mwbmap *mwbmap_hdl, uint32 bitix)
{
	...
	*bitmap_p ^= bitmap;	/* mark as in use */
	...
	MWBMAP_DBG(("Lvl1: bitix<%02lu> wordix<%02u>: %08x ^ %08x = %08x wfree %d", BCM_MWBMAP_MODOP(bitix), wordix, *bitmap_p, bitmap, (*bitmap_p) ^ bitmap, count));
...
void BCMFASTPATH bcm_mwbmap_free(struct bcm_mwbmap *mwbmap_hdl, uint32 bitix)
{
	...
	*bitmap_p |= bitmap;	/* mark as available */
...
#if defined(BCM_MWBMAP_DEBUG)
	{
		...
		count = mwbmap_p->wd_count[bitix];
		MWBMAP_ASSERT(count ==
			      bcm_cntsetbits(mwbmap_p->id_bitmap[bitix]));
#endif				/*  ! BCM_MWBMAP_USE_CNTSETBITS */
...
void bcm_mwbmap_show(struct bcm_mwbmap *mwbmap_hdl)
{
	...
	printf("mwbmap_p %p wmaps %u imaps %u ifree %d total %u\n", mwbmap_p,
	       mwbmap_p->wmaps, mwbmap_p->imaps, mwbmap_p->ifree,
	       mwbmap_p->total);
...
#endif				/* ! BCM_MWBMAP_USE_CNTSETBITS */
		printf("\tIDMAP:%2u. 0x%08x %02u\t", ix,
		       mwbmap_p->id_bitmap[ix], count);
...
#if defined(BCM_MWBMAP_USE_CNTSETBITS)
				count =
				    bcm_cntsetbits(mwbmap_p->id_bitmap
						   [idmap_ix]);
#else				/* ! BCM_MWBMAP_USE_CNTSETBITS */
...
bool id16_map_audit(void *id16_map_hndl)
{
	...
	ASSERT((id16_map->stack_idx > 0)
	       && (id16_map->stack_idx < id16_map->total));
	for (idx = 0; idx <= id16_map->stack_idx; idx++) {
		ASSERT(id16_map->stack[idx] >= id16_map->start);
		ASSERT(id16_map->stack[idx] <
		       (id16_map->start + id16_map->total));

#if defined(BCM_DBG) && defined(BCM_DBG_ID16)
		if (id16_map->dbg) {
			uint16 val16 = id16_map->stack[idx];
			if (((id16_map_dbg_t *) (id16_map->dbg))->
			    avail[val16] != TRUE) {
				insane |= 1;
...
#ifdef BCMDRIVER
void
dll_pool_detach(void *osh, dll_pool_t *pool, uint16 elems_max,
		uint16 elem_size)
{
	uint32 mem_size;
...
dll_pool_t *dll_pool_init(void *osh, uint16 elems_max, uint16 elem_size)
{
	...
	mem_size = sizeof(dll_pool_t) + (elems_max * elem_size);
	dll_pool_p = (dll_pool_t *) MALLOC(osh, mem_size);
	if (dll_pool_p == NULL) {
		printf
		    ("dll_pool_init: elems_max<%u> elem_size<%u> malloc failure\n",
		     elems_max, elem_size);
		ASSERT(0);
		return dll_pool_p;
	}
