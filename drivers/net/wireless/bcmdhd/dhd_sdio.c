...
static void dhdsdio_tune_fifoparam(struct dhd_bus *bus)
{
	...
	/* Update watermark */
	if (wm > 0) {
		bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_WATERMARK, wm,
				 &err);
	...
	if (on == TRUE) {
		...
		if (err == 0) {
			...
			DHD_TRACE(("%s: SBSDIO_FUNC1_CHIPCLKCSR : 0x%x\n",
				   __FUNCTION__, csr));
			if (!err
			    && ((csr & SBSDIO_HT_AVAIL) != SBSDIO_HT_AVAIL)) {
...
int dhd_bus_txdata(struct dhd_bus *bus, void *pkt)
{
	...
	if (dhd_deferred_tx || bus->fcstate || pktq_len(&bus->txq)
		...
		if (!deq_ret) {
			...
			{
#ifdef DHDTCPACK_SUPPRESS
				if (dhd_tcpack_check_xmit(bus->dhd, pkt) ==
				    BCME_ERROR) {
					DHD_ERROR(("%s %d: tcpack_suppress ERROR!!! Stop using\n", __FUNCTION__, __LINE__));
					dhd_tcpack_suppress_set(bus->dhd,
								TCPACK_SUP_OFF);
				}
#endif				/* DHDTCPACK_SUPPRESS */
...
#ifdef PROP_TXSTATUS
			wlfc_enabled =
			    (dhd_wlfc_flowcontrol(bus->dhd, ON, FALSE) !=
			     WLFC_UNSUPPORTED);
#endif
	...
	*new_pkt = NULL;
	...
	if (PKTHEADROOM(osh, pkt) < head_padding) {
		...
		if (last_chained_pkt && bus->blocksize != 0 &&
			...
			chain_tail_padding =
			    modulo > 0 ? (bus->blocksize - modulo) : 0;
		}
#ifdef DHDENABLE_TAILPAD
	...
	if (alloc_new_pkt) {
		...
		if (last_chained_pkt && bus->blocksize != 0 &&
		    (cur_total_len > (int) bus->blocksize
		     || prev_chain_total_len > 0)) {
		...
		bcopy(PKTDATA(osh, pkt), PKTDATA(osh, tmp_pkt),
		      PKTLEN(osh, pkt));
		*new_pkt = tmp_pkt;
		pkt = tmp_pkt;
	}
			 ...
			 bool free_pkt)
{
	...
	for (i = 0; i < num_pkt; i++) {
		...
		pkt_len =
		    dhdsdio_txpkt_preprocess(bus, pkt, chan, bus->tx_seq + i,
					     total_len, last_pkt, &pad_pkt_len,
					     &new_pkt);
		if (pkt_len <= 0)
			goto done;
		if (new_pkt) {
...
static uint dhdsdio_sendfromq(dhd_bus_t *bus, uint maxframes)
{
	...
	for (cnt = 0; (cnt < maxframes) && DATAOK(bus);) {
		...
		if (bus->txglom_enable) {
			num_pkt =
			    MIN((uint32) DATABUFCNT(bus),
				(uint32) bus->txglomsize);
			num_pkt = MIN(num_pkt, ARRAYSIZE(pkts));
		}
		num_pkt = MIN(num_pkt, pktq_mlen(&bus->txq, tx_prec_map));
		for (i = 0; i < num_pkt; i++) {
			pkts[i] =
			    pktq_mdeq(&bus->txq, ~bus->flowcontrol, &prec_out);
			if (!pkts[i]) {
		...
		if (i == 0)
			break;
		if (dhdsdio_txpkt(bus, SDPCM_DATA_CHANNEL, pkts, i, TRUE) !=
		    BCME_OK)
			dhd->tx_errors++;
		else
...
int dhd_bus_txctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	*(((uint16 *) frame) + 1) = htol16(~msglen);
	...
	return ret ? err_nodevice >=
	    ERROR_BCME_NODEVICE_MAX ? -ETIMEDOUT : -EIO : 0;
}
...
void dhd_bus_dump(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
	...
	bcm_bprintf(strbuf, " multi4 %u multi6 %u icmp6 %u multiother %u\n",
		    bus->wake_counts.rx_multi_ipv4,
		    bus->wake_counts.rx_multi_ipv6, bus->wake_counts.rx_icmpv6,
		    bus->wake_counts.rx_multi_other);
	bcm_bprintf(strbuf, " icmp6_ra %u, icmp6_na %u, icmp6_ns %u\n",
		    bus->wake_counts.rx_icmpv6_ra,
		    bus->wake_counts.rx_icmpv6_na,
...
static int dhdsdio_checkdied(dhd_bus_t *bus, char *data, uint size)
{
	...
	if ((sdpcm_shared.flags & (SDPCM_SHARED_ASSERT | SDPCM_SHARED_TRAP)) ==
		...
		if (sdpcm_shared.flags & SDPCM_SHARED_TRAP) {
			...
			addr =
			    sdpcm_shared.console_addr + OFFSETOF(hnd_cons_t,
								 log);
			rv =
...
static int dhdsdio_mem_dump(dhd_bus_t *bus)
{
	...
	while (size) {
		...
		ret =
		     dhdsdio_membytes(bus, FALSE, start, databuf, read_size);
		if (ret) {
	...
	*bcmerror = 0;
	...
	if (bus->sih->chip == BCM4330_CHIP_ID) {
		uart_enab = CC_PLL_CHIPCTRL_SERIAL_ENAB;
		   ...
		   bus->sih->chip == BCM43342_CHIP_ID || 0) {
		if (enable) {
		...
		int val_size)
{
	...
	bcmerror =
			 bcm_iovar_lencheck(vi, arg, len, IOV_ISSET(actionid));
	if (bcmerror != 0)
		goto exit;
...
void dhd_txglom_enable(dhd_pub_t *dhdp, bool enable)
{
	...
	if (enable) {
		...
		bcm_mkiovar("bus:rxglom", (void *) &rxglom, 4, buf,
			    sizeof(buf));
		ret =
		    dhd_wl_ioctl_cmd(dhdp, WLC_SET_VAR, buf, sizeof(buf), TRUE,
				     0);
		if (ret >= 0)
...
static uint8 dhdsdio_rxglom(dhd_bus_t *bus, uint8 rxseq, int pkt_wake)
{
	uint16 dlen, totlen;
	uint8 *dptr, num = 0;
...
static bool dhdsdio_pr94636_WAR(dhd_bus_t *bus)
{
	 ...
	 */
	 bcmerror =
	     bcmsdh_rwdata(bus->sdh, FALSE, l_addr, (uint8 *) &l_data[0],
			   20);
	if (bcmerror) {
	...
	*finished = FALSE;
	...
	for (rxseq = bus->rx_seq, rxleft = maxframes;
		...
		if (TXCTLOK(bus) && bus->ctrl_frame_stat
			 ...
			 */
			bus->dotxinrx =
			    (bus->dhd->tcpack_sup_mode ==
			     TCPACK_SUP_DELAYTX) ? FALSE : TRUE;
#endif
		}
		...
		dhd_os_sdunlock(bus->dhd);
		dhd_rx_frame(bus->dhd, ifidx, pkt, pkt_count, chan, pkt_wake,
			     &bus->wake_counts);
		pkt_wake = 0;
		dhd_os_sdlock(bus->dhd);
	}
...
static bool dhdsdio_probe_malloc(dhd_bus_t *bus, osl_t *osh, void *sdh)
{
	...
	if (bus->dhd->maxctl) {
		...
		bus->rxbuf =
		     DHD_OS_PREALLOC(bus->dhd, DHD_PREALLOC_RXBUF,
				     bus->rxblen);
		if (!bus->rxbuf) {
...
static int dhdsdio_suspend(void *context)
{
	...
	if ((!ret) && (bus->dhd->up)) {
		if (wait_event_timeout(bus->bus_sleep, bus->sleeping, wait_time)
		    == 0) {
			if (!bus->sleeping) {
				return 1;
			}
...
dhd_bcmsdh_send_buf(dhd_bus_t *bus, uint32 addr, uint fn, uint flags,
		    uint8 *buf, uint nbytes, void *pkt,
		    bcmsdh_cmplt_fn_t complete, void *handle, int max_retry)
{
	int ret;
	int i = 0;
	int retries = 0;
...
#ifdef PROP_TXSTATUS
	wlfc_enabled =
	    (dhd_wlfc_cleanup_txq(dhdp, NULL, 0) != WLFC_UNSUPPORTED);
#endif
	if (!wlfc_enabled) {
#ifdef DHDTCPACK_SUPPRESS
		/* Clean tcp_ack_info_tbl in order to prevent access to flushed pkt,
