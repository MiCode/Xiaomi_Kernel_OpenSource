...
void *dhd_deferred_work_init(void *dhd_info)
{
	...
	work =
	    (struct dhd_deferred_wq *) kzalloc(sizeof(struct dhd_deferred_wq),
					       flags);
	...
	fifo_size =
	    is_power_of_2(fifo_size) ? fifo_size :
			   ...
			   event_handler_t event_handler, u8 priority)
{
	...
	if (priority == DHD_WORK_PRIORITY_HIGH) {
		status =
		    kfifo_in_spinlocked(deferred_wq->prio_fifo, &deferred_event,
					DEFRD_EVT_SIZE,
					&deferred_wq->work_lock);
	} else {
		status =
		    kfifo_in_spinlocked(deferred_wq->work_fifo, &deferred_event,
					DEFRD_EVT_SIZE,
					&deferred_wq->work_lock);
	}
...
dhd_get_scheduled_work(struct dhd_deferred_wq *deferred_wq,
		       struct dhd_deferred_event_t *event)
{
	int status = 0;
