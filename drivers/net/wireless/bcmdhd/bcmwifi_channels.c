...
char *wf_chspec_ntoa(chanspec_t chspec, char *buf)
{
	...
	if (CHSPEC_IS20(chspec)) {
		...
		bw = wf_chspec_bw_str[(chspec & WL_CHANSPEC_BW_MASK) >>
		...
		snprintf(buf, CHANSPEC_STR_LEN, "%s%d/%s%s", band, ctl_chan, bw,
			 sb);
#else
		/* ctl sideband string instead of BW for 40MHz */
		if (CHSPEC_IS40(chspec)) {
			sb = CHSPEC_SB_UPPER(chspec) ? "u" : "l";
			snprintf(buf, CHANSPEC_STR_LEN, "%s%d%s", band,
				 ctl_chan, sb);
		} else {
			snprintf(buf, CHANSPEC_STR_LEN, "%s%d/%s", band,
				 ctl_chan, bw);
		...
		/* 80+80 */
		uint chan1 =
		    (chspec & WL_CHANSPEC_CHAN1_MASK) >>
		    WL_CHANSPEC_CHAN1_SHIFT;
		uint chan2 =
		    (chspec & WL_CHANSPEC_CHAN2_MASK) >>
		    WL_CHANSPEC_CHAN2_SHIFT;

		/* convert to channel number */
		chan1 =
		    (chan1 < WF_NUM_5G_80M_CHANS) ? wf_5g_80m_chans[chan1] : 0;
		chan2 =
		    (chan2 < WF_NUM_5G_80M_CHANS) ? wf_5g_80m_chans[chan2] : 0;

		/* Outputs a max of CHANSPEC_STR_LEN chars including '\0'  */
		snprintf(buf, CHANSPEC_STR_LEN, "%d/80+80/%d-%d", ctl_chan,
			 chan1, chan2);
	}
	...
	if (CHSPEC_IS2G(chanspec)) {
		...
		if (chspec_bw == WL_CHANSPEC_BW_8080) {
			...
			if (chspec_bw == WL_CHANSPEC_BW_20) {
				...
				for (i = 0; i < num_ch; i++) {
					if (chspec_ch ==
					    (uint) LOWER_20_SB(center_ch[i])
					    || chspec_ch ==
					    (uint) UPPER_20_SB(center_ch[i]))
						break;	/* match found */
				}
...
{
	return (char *)
	    wf_chspec_bw_str[(CHSPEC_BW(chspec) >> WL_CHANSPEC_BW_SHIFT)];
}
...
wf_chspec_get8080_chspec(uint8 primary_20mhz, uint8 chan0, uint8 chan1)
{
	...
	chanspec = ((seg0 << WL_CHANSPEC_CHAN1_SHIFT) |
		    ...
		    WL_CHANSPEC_BW_8080 | WL_CHANSPEC_BAND_5G);
...
chanspec_t wf_chspec_primary80_chspec(chanspec_t chspec)
{
	...
	if (CHSPEC_IS80(chspec)) {
		...
		chspec80 =
		    (WL_CHANSPEC_BAND_5G | WL_CHANSPEC_BW_80 | sb |
		     center_chan);
