	...
	list_for_each_entry_safe(_net_info, next, &cfg->net_list, list) {
		...
		case 2:
			...
			if (cfg->state_notifier &&
				...
				cfg->state_notifier(cfg, _net_info, status,
						    false);
			break;
		case 4:
	...
	list_for_each_entry_safe(_net_info, next, &cfg->net_list, list) {
		if (ndev && (_net_info->ndev == ndev)) {
			...
			case 1:
				...
				if (cfg->state_notifier &&
				    test_bit(status, &(cfg->interrested_state)))
					cfg->state_notifier(cfg, _net_info,
							    status, true);
				break;
			case 2:
				clear_bit(status, &_net_info->sme_state);
...
static inline struct wl_profile *wl_get_profile_by_netdev(struct bcm_cfg80211
							  *cfg,
							  struct net_device
							  *ndev)
{
...
static inline struct net_info *wl_get_netinfo_by_netdev(struct bcm_cfg80211
							*cfg,
							struct net_device *ndev)
{
	struct net_info *_net_info, *next;
...
extern s32 wl_cfg80211_notify_ifadd(int ifidx, char *name, uint8 *mac,
				    uint8 bssidx);
extern s32 wl_cfg80211_notify_ifdel(int ifidx, char *name, uint8 *mac,
				    uint8 bssidx);
extern s32 wl_cfg80211_notify_ifchange(int ifidx, char *name, uint8 *mac,
				       uint8 bssidx);
...
extern void wl_cfg80211_ibss_vsie_set_buffer(vndr_ie_setbuf_t *ibss_vsie,
					     int ibss_vsie_len);
