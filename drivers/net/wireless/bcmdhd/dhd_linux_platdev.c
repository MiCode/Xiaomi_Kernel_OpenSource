...
static int wifi_ctrlfunc_register_drv(void)
{
	...
	dev1 =
	    bus_find_device(&platform_bus_type, NULL, WIFI_PLAT_NAME,
			    wifi_platdev_match);
	dev2 =
	    bus_find_device(&platform_bus_type, NULL, WIFI_PLAT_NAME2,
			    wifi_platdev_match);
...
{
	dhd_wifi_platdata =
	    (bcmdhd_wifi_platdata_t *) (pdev->dev.platform_data);
...
int dhd_wifi_platform_register_drv(void)
{
	...
	if (cfg_multichip) {
		dev =
		    bus_find_device(&platform_bus_type, NULL, WIFI_PLAT_EXT,
				    wifi_platdev_match);
		if (dev == NULL) {
				...
				do {
					...
					if (err) {
						...
						err =
						    ...
						    (adapter, TRUE);
						if (err) {
							DHD_ERROR(("failed to enumerate bus %s, " "%d retry left\n", adapter->name, retry));
...
static int dhd_wifi_platform_load_sdio(void)
{
	...
	for (i = 0; i < dhd_wifi_platdata->num_adapters; i++) {
		...
		do {
			...
			err =
			    wifi_platform_set_power(adapter, TRUE,
						    WIFI_TURNON_DELAY);
			if (err) {
				/* WL_REG_ON state unknown, Power off forcely */
				wifi_platform_set_power(adapter, FALSE,
