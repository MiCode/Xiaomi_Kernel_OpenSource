...
{
	cfg->p2p = kzalloc(sizeof(struct p2p_info), GFP_KERNEL);
	if (!cfg->p2p) {
...
s32 wl_cfgp2p_set_firm_p2p(struct bcm_cfg80211 *cfg)
{
	...
	ret = wldev_iovar_setbuf_bsscfg(ndev, "p2p_da_override", &null_eth_addr,
					sizeof(null_eth_addr), cfg->ioctl_buf,
					WLC_IOCTL_MAXLEN, 0,
					&cfg->ioctl_buf_sync);
 ...
 */
s32
wl_cfgp2p_ifchange(struct bcm_cfg80211 *cfg, struct ether_addr *mac,
...
wl_cfgp2p_ifidx(struct bcm_cfg80211 *cfg, struct ether_addr *mac, s32 *index)
{
	...
	ret =
	    wldev_iovar_getbuf_bsscfg(dev, "p2p_if", mac, sizeof(*mac), getbuf,
				      sizeof(getbuf), wl_to_p2p_bss_bssidx(cfg,
...
s32
wl_cfgp2p_set_p2p_mode(struct bcm_cfg80211 *cfg, u8 mode, u32 channel,
		       u16 listen_ms, int bssidx)
{
	...
	ret = wldev_iovar_setbuf_bsscfg(dev, "p2p_state", &discovery_mode,
					sizeof(discovery_mode), cfg->ioctl_buf,
			   ...
			   const u8 *ie, u32 ie_len)
{
	...
	ret =
	    wldev_iovar_setint_bsscfg(wl_to_p2p_bss_ndev
				      (cfg, P2PAPI_BSSCFG_DEVICE), "wsec",
				      AES_ENABLED, wl_to_p2p_bss_bssidx(cfg,
									P2PAPI_BSSCFG_DEVICE));
	if (unlikely(ret < 0)) {
...
#define P2PAPI_SCAN_AF_SEARCH_DWELL_TIME_MS 100
	...
	if (search_state == WL_P2P_DISC_ST_SEARCH) {
		 ...
		 */
		wl_cfgp2p_set_p2p_mode(cfg, WL_P2P_DISC_ST_SEARCH, 0, 0,
				       bssidx);
		/* use null ssid */
		ssid.SSID_len = 0;
			 ...
			 struct ether_addr *tx_dst_addr)
{
	...
	ret = wl_cfgp2p_escan(cfg, ndev, true, chan_cnt,
			      ...
			      WL_SCAN_ACTION_START, bssidx, NULL,
			      p2p_scan_purpose);
...
wl_cfgp2p_parse_vndr_ies(u8 *parse, u32 len, struct parsed_vndr_ies *vndr_ies)
{
	...
	while (ie) {
		...
		if (ie->id == DOT11_MNG_VS_ID) {
			...
			memcpy(&parsed_info->vndrie, vndrie,
			       sizeof(vndr_ie_t));
...
s32
wl_cfgp2p_set_management_ie(struct bcm_cfg80211 *cfg, struct net_device *ndev,
	...
	if (vndr_ie_len > mgmt_ie_buf_len) {
		...
		if (mgmt_ie_buf != NULL) {
			...
			for (i = 0; i < old_vndr_ies.count; i++) {
				...
				del_add_ie_buf_len =
				    wl_cfgp2p_vndr_ie(cfg, curr_ie_buf, pktflag,
 ...
 * Returns 0 if success.
 */
s32 wl_cfgp2p_clear_management_ie(struct bcm_cfg80211 *cfg, s32 bssidx)
{
		  ...
		  const s8 *add_del_cmd)
{
	...
	strlcpy(hdr.cmd, add_del_cmd, VNDR_IE_CMD_LEN);
 ...
 */
s32
wl_cfgp2p_find_idx(struct bcm_cfg80211 *cfg, struct net_device *ndev,
		   s32 *bssidx)
{
	u32 i;
	...
	if (wl_get_p2p_status(cfg, LISTEN_EXPIRED) == 0) {
		...
		if (wl_get_drv_status_all(cfg, REMAINING_ON_CHANNEL) ||
			...
			if (ndev && (ndev->ieee80211_ptr != NULL)) {
				...
				if (cfgdev)
					cfg80211_remain_on_channel_expired
					    (cfgdev, cfg->last_roc_id,
					     &cfg->remain_on_chan,
					     cfg->remain_on_chan_type,
					     GFP_KERNEL);
...
void wl_cfgp2p_listen_expired(unsigned long data)
{
	...
	wl_cfg80211_event(cfg->p2p_net ? cfg->p2p_net :
			  wl_to_p2p_bss_ndev(cfg, P2PAPI_BSSCFG_DEVICE), &msg,
			  NULL);
#else
 ...
 */
s32
wl_cfgp2p_discover_listen(struct bcm_cfg80211 *cfg, s32 channel,
			  u32 duration_ms)
{
#define EXTRA_DELAY_TIME	100
	...
	ret =
	    wl_cfgp2p_set_p2p_mode(cfg, WL_P2P_DISC_ST_LISTEN, channel,
				   (u16) duration_ms, wl_to_p2p_bss_bssidx(cfg,
									   P2PAPI_BSSCFG_DEVICE));
 ...
 */
s32
wl_cfgp2p_action_tx_complete(struct bcm_cfg80211 *cfg,
			  ...
			  wl_af_params_t *af_params, s32 bssidx)
{
	...
	evt_ret =
	     wl_cfg80211_apply_eventbuffer(bcmcfg_to_prmry_ndev(cfg), cfg,
					   &buf);
	if (evt_ret < 0)
		return evt_ret;
	...
	ret =
	    wldev_iovar_setbuf_bsscfg(dev, "actframe", af_params,
				      sizeof(*af_params), cfg->ioctl_buf,
				      WLC_IOCTL_MAXLEN, bssidx,
				      &cfg->ioctl_buf_sync);
...
s32
wl_cfgp2p_bss(struct bcm_cfg80211 *cfg, struct net_device *ndev,
...
s32
wl_cfgp2p_set_p2p_noa(struct bcm_cfg80211 *cfg, struct net_device *ndev,
		      char *buf, int len)
{
	s32 ret = -1;
...
s32
wl_cfgp2p_get_p2p_noa(struct bcm_cfg80211 *cfg, struct net_device *ndev,
		      char *buf, int buf_len)
{
	...
	if (cfg->p2p && cfg->p2p->vif_created) {
		if (cfg->p2p->noa.desc[0].count || cfg->p2p->ops.ops) {
			_buf[0] = 1;	/* noa index */
			_buf[1] = (cfg->p2p->ops.ops ? 0x80 : 0) | (cfg->p2p->ops.ctw & 0x7f);	/* ops + ctw */
			len += 2;
			if (cfg->p2p->noa.desc[0].count) {
				noa_desc = (wifi_p2p_noa_desc_t *) &_buf[len];
				noa_desc->cnt_type =
				    cfg->p2p->noa.desc[0].count;
				noa_desc->duration =
				    cfg->p2p->noa.desc[0].duration;
				noa_desc->interval =
				    cfg->p2p->noa.desc[0].interval;
...
s32
wl_cfgp2p_set_p2p_ps(struct bcm_cfg80211 *cfg, struct net_device *ndev,
		     char *buf, int len)
{
	int ps, ctw;
	int ret = -1;
	...
	if (cfg->p2p && cfg->p2p->vif_created) {
		...
		if ((legacy_ps != -1)
			...
			ret =
			    wldev_ioctl(dev, WLC_SET_PM, &legacy_ps,
...
s32 wl_cfgp2p_register_ndev(struct bcm_cfg80211 *cfg)
{
	...
	net = alloc_etherdev(sizeof(struct bcm_cfg80211 *));
	if (!net) {
...
int wl_cfgp2p_start_p2p_device(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	...
	ret =
	    wl_cfgp2p_enable_discovery(cfg, bcmcfg_to_prmry_ndev(cfg), NULL, 0);
	if (unlikely(ret < 0)) {
