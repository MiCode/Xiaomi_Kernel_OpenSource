		   ...
		   uint8 *byte)
{
	...
	if (rw) {		/* CMD52 Write */
		if (func == 0) {
			...
			if (sd->func[func]) {
				sdio_claim_host(sd->func[func]);
				sdio_writeb(sd->func[func], *byte, regaddr,
					    &err_ret);
				sdio_release_host(sd->func[func]);
			}
		}
		...
		if (sd->func[func]) {
			...
			if (func == 0) {
				*byte =
				    sdio_f0_readb(sd->func[func], regaddr,
						  &err_ret);
			} else {
				*byte =
				    sdio_readb(sd->func[func], regaddr,
					       &err_ret);
			}
			sdio_release_host(sd->func[func]);
		}
		   ...
		   uint addr, uint32 *word, uint nbytes)
{
	...
	if (rw) {		/* CMD52 Write */
		if (nbytes == 4) {
			...
			sdio_writew(sd->func[func], (*word & 0xFFFF), addr,
				    &err_ret);
		} else {
		...
		if (nbytes == 4) {
			...
			*word =
			    sdio_readw(sd->func[func], addr, &err_ret) & 0xFFFF;
		} else {
			sd_err(("%s: Invalid nbytes: %d\n", __FUNCTION__,
				nbytes));
		}
...
static SDIOH_API_RC
sdioh_request_packet_chain(sdioh_info_t *sd, uint fix_inc, uint write,
			   uint func, uint addr, void *pkt)
{
	...
	while (pnext != NULL) {
		...
		while (pnext != NULL && ttl_len < max_req_size) {
			...
			sg_set_buf(&sd->sg_list[sg_count++], pdata,
				   sg_data_size);
			...
			uint addr, uint8 *buf, uint len)
{
	...
	if (err_ret)
		sd_err(("%s: %s FAILED %p, addr=0x%05x, pkt_len=%d, ERR=%d\n",
			__FUNCTION__, (write) ? "TX" : "RX", buf, addr, len,
			err_ret));
	else
...
sdioh_request_buffer(sdioh_info_t *sd, uint pio_dma, uint fix_inc, uint write,
		     uint func, uint addr, uint reg_width, uint buf_len,
		     uint8 *buffer, void *pkt)
{
	...
	if (pkt) {
		...
		if (PKTNEXT(sd->osh, pkt))
			return sdioh_request_packet_chain(sd, fix_inc, write,
							  func, addr, pkt);
	...
	if (((ulong) buffer & DMA_ALIGN_MASK) == 0
		...
		return sdioh_buffer_tofrom_bus(sd, fix_inc, write, func, addr,
					       buffer, buf_len);
	...
	tmppkt =
	    PKTGET_STATIC(sd->osh, buf_len + DEFAULT_SDIO_F2_BLKSIZE,
			  write ? TRUE : FALSE);
	if (tmppkt == NULL) {
	...
	if (sd->func[0]) {
		if (stage == 0) {
			...
			/* sdio_reset_comm() - has been fixed in latest kernel/msm.git for Linux
			   ...
			   patch for it
			 */
			ret = sdio_reset_comm(sd->func[0]->card);
			if (ret) {
