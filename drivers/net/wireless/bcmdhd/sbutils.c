...
static void sb_write_sbreg(si_info_t *sii, volatile uint32 *sbr, uint32 v)
{
	...
	if (BUSTYPE(sii->pub.bustype) == PCMCIA_BUS) {
		...
		BCM_REFERENCE(dummy);
		W_REG(sii->osh, (volatile uint32 *)sbr, (uint16)(v & 0xffff));
		dummy = R_REG(sii->osh, sbr);
		BCM_REFERENCE(dummy);
		W_REG(sii->osh, ((volatile uint32 *)sbr + 1), (uint16)((v >> 16) & 0xffff));
	} else
		W_REG(sii->osh, sbr, v);
...
static uint32 _sb_coresba(si_info_t *sii)
{
	...
	case PCI_BUS:
		sbaddr =
		    OSL_PCI_READ_CONFIG(sii->osh, PCI_BAR0_WIN,
					sizeof(uint32));
		break;
 ...
 * NULL.
 */
uint32 *sb_corereg_addr(si_t *sih, uint coreidx, uint regoff)
{
	uint32 *r = NULL;
	bool fast = FALSE;
	si_info_t *sii = SI_INFO(sih);
	...
	for (i = 0; i < sii->numcores; i++) {
		...
		if (sii->pub.socirev > SONICS_2_2)
			bcm_bprintf(b, "sbimerrlog 0x%x sbimerrloga 0x%x\n",
				    sb_corereg(sih, si_coreidx(&sii->pub),
					       SBIMERRLOG, 0, 0),
				    sb_corereg(sih, si_coreidx(&sii->pub),
					       SBIMERRLOGA, 0, 0));
		...
		bcm_bprintf(b,
			    ...
			    R_SBREG(sii, &sb->sbtmstatelow), R_SBREG(sii,
								     &sb->
								     sbtmstatehigh),
			    R_SBREG(sii, &sb->sbidhigh), R_SBREG(sii,
								 &sb->
								 sbimstate),
			    R_SBREG(sii, &sb->sbimconfiglow), R_SBREG(sii,
								      &sb->
								      sbimconfighigh));
	}
