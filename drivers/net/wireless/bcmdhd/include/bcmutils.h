...
#define bcm_strcpy_s(dst, noOfElements, src)            strlcpy((dst), (src), strlen(dst))
#define bcm_strncpy_s(dst, noOfElements, src, count)    strlcpy((dst), (src), ((count) + 1))
#define bcm_strcat_s(dst, noOfElements, src)            strlcat((dst), (src), strlen(src) + 1)
...
/* rx completion list */
	...
	extern rxcpl_info_t *bcm_rxcpllist_end(rxcpl_info_t *ptr,
					       uint32 *count);
...
#endif
#endif				/* setbit */
	extern void set_bitrange(void *array, uint start, uint end,
				 uint maxbit);
...
#define bcm_valid_tlv(elt, buflen) (\
	...
	extern bcm_tlv_t *bcm_parse_tlvs_min_bodylen(void *buf, int buflen,
						     uint key, int min_bodylen);

	extern bcm_tlv_t *bcm_parse_ordered_tlvs(void *buf, int buflen,
						 uint key);

	extern bcm_tlv_t *bcm_find_vendor_ie(void *tlvs, int tlvs_len,
					     const char *voui, uint8 *type,
					     int type_len);

	extern uint8 *bcm_write_tlv(int type, const void *data, int datalen,
				    uint8 *dst);
	extern uint8 *bcm_write_tlv_safe(int type, const void *data,
					 int datalen, uint8 *dst,
					 int dst_maxlen);

	extern uint8 *bcm_copy_tlv(const void *src, uint8 *dst);
	extern uint8 *bcm_copy_tlv_safe(const void *src, uint8 *dst,
					int dst_maxlen);
 ...
 */
	extern int bcm_xtlv_buf_init(bcm_xtlvbuf_t *tlv_buf, uint8 *buf,
				     uint16 len, bcm_xtlv_opts_t opts);
	...
	extern int bcm_xtlv_put_data(bcm_xtlvbuf_t *tbuf, uint16 type,
				     const void *data, uint16 dlen);
	extern int bcm_xtlv_put_8(bcm_xtlvbuf_t *tbuf, uint16 type,
				  const int8 data);
	extern int bcm_xtlv_put_16(bcm_xtlvbuf_t *tbuf, uint16 type,
				   const int16 data);
	extern int bcm_xtlv_put_32(bcm_xtlvbuf_t *tbuf, uint16 type,
				   const int32 data);
	extern int bcm_unpack_xtlv_entry(uint8 **buf, uint16 xpct_type,
					 uint16 xpct_len, void *dst,
					 bcm_xtlv_opts_t opts);
	extern int bcm_pack_xtlv_entry(uint8 **buf, uint16 *buflen,
				       uint16 type, uint16 len, void *src,
				       bcm_xtlv_opts_t opts);
	extern int bcm_xtlv_size(const bcm_xtlv_t *elt, bcm_xtlv_opts_t opts);

/* callback for unpacking xtlv from a buffer into context. */
	typedef int (bcm_xtlv_unpack_cbfn_t) (void *ctx, uint8 *buf,
...
/* unpack a set of tlvs from the buffer using provided xtlv desc */
	extern int bcm_unpack_xtlv_buf_to_mem(void *buf, int *buflen,
					      xtlv_desc_t *items,
					      bcm_xtlv_opts_t opts);

/* pack a set of tlvs into buffer using provided xtlv desc */
	extern int bcm_pack_xtlv_buf_from_mem(void **buf, uint16 *buflen,
					      xtlv_desc_t *items,
					      bcm_xtlv_opts_t opts);
 ...
 */
	typedef bool(*bcm_pack_xtlv_next_info_cbfn_t) (void *ctx,
						       uint16 *tlv_id,
						       uint16 *tlv_len);
...
/* generic datastruct to help dump routines */
	...
	extern void bcm_bprhex(struct bcmstrbuf *b, const char *msg,
			       bool newline, uint8 *buf, int len);

	extern void bcm_inc_bytes(uchar *num, int num_bytes, uint8 amount);
	extern int bcm_cmp_bytes(const uchar *arg1, const uchar *arg2,
				 uint8 nbytes);
	extern void bcm_print_bytes(const char *name, const uchar *cdata,
				    int len);
...
/* calculate a * b + c */
	extern void bcm_uint64_multiple_add(uint32 *r_high, uint32 *r_low,
					    uint32 a, uint32 b, uint32 c);
/* calculate a / b */
	extern void bcm_uint64_divide(uint32 *r, uint32 a_high, uint32 a_low,
				      uint32 b);
...
#define B6(n) B4(n), B4(n + 1), B4(n + 1), B4(n + 2)
	...
	static INLINE uint32	/* Uses table _CSBTBL for fast counting of 1's in a u32 */
	 bcm_cntsetbits(const uint32 u32) {
		...
		return (_CSBTBL[p[0]] + _CSBTBL[p[1]] + _CSBTBL[p[2]] +
			_CSBTBL[p[3]]);
	}
...
/* Incarnate a multiword bitmap based small index allocator */
	extern struct bcm_mwbmap *bcm_mwbmap_init(osl_t *osh,
						  uint32 items_max);
...
/* Force an index at a specified position to be in use */
	extern void bcm_mwbmap_force(struct bcm_mwbmap *mwbmap_hdl,
				     uint32 bitix);

/* Free a previously allocated index back into the multiword bitmap allocator */
	extern void bcm_mwbmap_free(struct bcm_mwbmap *mwbmap_hdl,
				    uint32 bitix);
...
/* Determine whether an index is inuse or free */
	extern bool bcm_mwbmap_isfree(struct bcm_mwbmap *mwbmap_hdl,
				      uint32 bitix);
 ...
 */
	extern void *id16_map_init(osl_t *osh, uint16 total_ids,
				   uint16 start_val16);
...
/* End - Simple 16bit Id Allocator. */
	...
	extern void bcm_uint64_right_shift(uint32 *r, uint32 a_high,
					   uint32 a_low, uint32 b);
...
/* Elements managed in a double linked list */
	...
	dll_pool_t *dll_pool_init(void *osh, uint16 elems_max,
				  uint16 elem_size);
