...
*/
static void *_dhd_wlfc_hanger_create(osl_t *osh, int max_items)
{
	int i;
...
static int _dhd_wlfc_hanger_delete(osl_t *osh, void *hanger)
{
...
static int
_dhd_wlfc_hanger_poppkt(void *hanger, uint32 slot_id, void **pktout,
...
static int _dhd_wlfc_enque_afq(athost_wl_status_info_t *ctx, void *p)
{
	...
	if (entry_idx < WLFC_MAC_DESC_TABLE_SIZE)
		...
		entry =
		    &ctx->destination_entries.interfaces[entry_idx -
...
static int
_dhd_wlfc_deque_afq(athost_wl_status_info_t *ctx, uint16 hslot, uint8 hcnt,
		    uint8 prec, void **pktout)
{
	wlfc_mac_descriptor_t *entry;
	struct pktq *pq;
	struct pktq_prec *q;
	...
	if (hslot < WLFC_MAC_DESC_TABLE_SIZE)
		...
		entry =
		    &ctx->destination_entries.interfaces[hslot -
							 WLFC_MAC_DESC_TABLE_SIZE];
	else
		entry = &ctx->destination_entries.other;
	...
	if (!b) {
		...
		DHD_INFO(("%s: out of order, seq(%d), head_seq(%d)\n",
			  __FUNCTION__, hcnt,
			  WL_TXSTATUS_GET_FREERUNCTR(DHD_PKTTAG_H2DTAG
						     (PKTTAG(q->head)))));
		     ...
		     uint16 htodseq, bool skip_wlfc_hdr)
{
	...
	if (tim_signal) {
		tim_signal_len =
		    TLV_HDR_LEN + WLFC_CTL_VALUE_LEN_PENDING_TRAFFIC_BMP;
	}
	...
	if (WLFC_GET_REUSESEQ(dhdp->wlfc_mode)) {
		...
		memcpy(&wlh[TLV_HDR_LEN + WLFC_CTL_VALUE_LEN_PKTTAG],
		       &wl_seqinfo, WLFC_CTL_VALUE_LEN_SEQ);
	}
...
static wlfc_mac_descriptor_t *_dhd_wlfc_find_table_entry(athost_wl_status_info_t *ctx, void *p)
{
	int i;
...
_dhd_wlfc_prec_drop(dhd_pub_t *dhdp, int prec, void *p, bool bPktInQ)
{
	...
	ctx->pkt_cnt_in_drv[DHD_PKTTAG_IF(PKTTAG(p))][DHD_PKTTAG_FIFO
						      (PKTTAG(p))]--;
	ctx->stats.pktout++;
...
_dhd_wlfc_prec_enq_with_drop(dhd_pub_t *dhdp, struct pktq *pq, void *pkt,
			     int prec, bool qHead, uint8 current_seq)
{
	void *p = NULL;
...
_dhd_wlfc_enque_suppressed(athost_wl_status_info_t *ctx, int prec, void *p)
{
	 ...
	 */
	if (_dhd_wlfc_prec_enq_with_drop
	    (ctx->dhdp, &entry->psq, p, ((prec << 1) + 1), FALSE,
	...
	*slot = hslot;
	...
	if (header_needed) {
		if (WLFC_GET_AFQ(dhdp->wlfc_mode)) {
			hslot =
			    (uint) (entry - &ctx->destination_entries.nodes[0]);
		} else {
		...
		if (WLFC_GET_REORDERSUPP(dhdp->wlfc_mode)) {
			...
			gen =
			    WL_TXSTATUS_GET_GENERATION(DHD_PKTTAG_H2DTAG
						       (PKTTAG(p)));
		} else {
			_dhd_wlfc_hanger_get_genbit(ctx->hanger, p, hslot,
						    &gen);
		}
				      ...
				      bool only_no_credit)
{
	...
	*ac_credit_spent = ((prec == AC_COUNT)
			    && !ctx->bcmc_credit_supported) ? 0 : 1;
		...
		if (entry->occupied
			...
			*needs_hdr = 0;
			...
			if (p != NULL) {
				...
				*entry_out = entry;
				ctx->pkt_cnt_in_q[DHD_PKTTAG_IF(PKTTAG(p))]
				    [prec]--;
				 ...
				 */
				_dhd_wlfc_traffic_pending_check(ctx, entry,
								prec);
				return p;
			}
		}
...
_dhd_wlfc_enque_delayq(athost_wl_status_info_t *ctx, void *pktbuf, int prec)
{
	...
	if (pktbuf != NULL) {
		 ...
		 */
		if (_dhd_wlfc_prec_enq_with_drop
		    (ctx->dhdp, &entry->psq, pktbuf, (prec << 1), FALSE,
		     WLFC_SEQCOUNT(entry, prec))
		    == FALSE) {
			WLFC_DBGMESG(("D"));
			ctx->stats.delayq_full_error++;
...
static void
_dhd_wlfc_hanger_free_pkt(athost_wl_status_info_t *wlfc, uint32 slot_id,
			  uint8 pkt_state, int pkt_txstatus)
{
	wlfc_hanger_t *hanger;
	...
	if (item->pkt) {
		if ((item->pkt_state & WLFC_HANGER_PKT_STATE_TXCOMPLETE) &&
			...
			int ret =
			    _dhd_wlfc_hanger_poppkt(wlfc->hanger, slot_id, &p,
						    TRUE);
			BCM_REFERENCE(ret);
			BCM_REFERENCE(pkt);
			...
			if (!(item->pkt_state & WLFC_HANGER_PKT_STATE_TXSTATUS)) {
				/* cleanup case */
				wlfc_mac_descriptor_t *entry =
				    _dhd_wlfc_find_table_entry(wlfc, p);
			...
			wlfc->stats.pktout++;
			dhd_txcomplete((dhd_pub_t *) wlfc->dhdp, p,
				       item->pkt_txstatus);
		...
		while (p) {
			if (fn == NULL || (*fn) (p, arg)) {
				...
				if (q_type == Q_TYPE_PSQ) {
					if (!WLFC_GET_AFQ(dhdp->wlfc_mode)
					    && (prec & 1)) {
						_dhd_wlfc_hanger_remove_reference
						    (ctx->hanger, p);
					}
					ctx->pkt_cnt_in_q[DHD_PKTTAG_IF
							  (PKTTAG(p))][prec >>
								       1]--;
					ctx->pkt_cnt_per_ac[prec >> 1]--;
					...
					if (!(prec & 1)) {
						 ...
						 * dhd_tcpack_check_xmit() and dhd_txcomplete().
						 */
						_dhd_wlfc_pushheader(ctx, p,
								     FALSE, 0,
								     0, 0, 0,
								     TRUE);
#ifdef DHDTCPACK_SUPPRESS
						if (dhd_tcpack_check_xmit
						    (dhdp, p) == BCME_ERROR) {
							DHD_ERROR(("%s %d: tcpack_suppress ERROR!!!" " Stop using it\n", __FUNCTION__, __LINE__));
							dhd_tcpack_suppress_set
							    (dhdp,
							     TCPACK_SUP_OFF);
						}
#endif				/* DHDTCPACK_SUPPRESS */
				...
				PKTSETLINK(p, NULL);
				if (dir) {
					ctx->pkt_cnt_in_drv[DHD_PKTTAG_IF
							    (PKTTAG(p))][prec >>
									 1]--;
					ctx->stats.pktout++;
					dhd_txcomplete(dhdp, p, FALSE);
...
static void _dhd_wlfc_cleanup_txq(dhd_pub_t *dhd, f_processpkt_t fn, void *arg)
{
	...
	while ((pkt = head)) {
		...
		wlfc->pkt_cnt_in_drv[DHD_PKTTAG_IF(PKTTAG(pkt))][DHD_PKTTAG_FIFO
								 (PKTTAG
								  (pkt))]--;
		wlfc->stats.pktout++;
	...
	for (i = 0; i < total_entries; i++) {
		if (table[i].occupied) {
			...
			if ((fn == NULL)
				...
				table[i].occupied = 0;
				if (table[i].transit_count
				    || table[i].suppr_transit_count) {
					DHD_ERROR(("%s: table[%d] transit(%d), suppr_tansit(%d)\n", __FUNCTION__, i, table[i].transit_count, table[i].suppr_transit_count));
				}
			}
		}
	}
...
_dhd_wlfc_mac_entry_update(athost_wl_status_info_t *ctx,
			   wlfc_mac_descriptor_t *entry, uint8 action,
			   ...
			   f_processpkt_t fn, void *arg)
{
	...
	if ((action == eWLFC_MAC_ENTRY_ACTION_ADD)
		...
		if (action == eWLFC_MAC_ENTRY_ACTION_ADD) {
			dhd_pub_t *dhdp = (dhd_pub_t *) (ctx->dhdp);
			pktq_init(&entry->psq, WLFC_PSQ_PREC_COUNT,
				  WLFC_PSQ_LEN);
			if (WLFC_GET_AFQ(dhdp->wlfc_mode)) {
				pktq_init(&entry->afq, WLFC_AFQ_PREC_COUNT,
					  WLFC_PSQ_LEN);
			}
			...
			if (entry->next == NULL) {
				...
				if (ctx->active_entry_head) {
					entry->prev =
					    ctx->active_entry_head->prev;
					ctx->active_entry_head->prev->next =
					    entry;
...
#ifdef LIMIT_BORROW
static int
_dhd_wlfc_borrow_credit(athost_wl_status_info_t *ctx, int highest_lender_ac,
			int borrower_ac, bool bBorrowAll)
{
	...
	for (lender_ac = 0; lender_ac <= highest_lender_ac; lender_ac++) {
		if (!bBorrowAll) {
			borrow_limit =
			    ctx->Init_FIFO_credit[lender_ac] /
	...
	for (i = 0; i <= 4; i++) {
		if (ctx->Init_FIFO_credit[i] != ctx->FIFO_credit[i]) {
			DHD_ERROR(("%s: credit[i] is not returned, (%d %d)\n",
				   __FUNCTION__, ctx->Init_FIFO_credit[i],
				   ctx->FIFO_credit[i]));
		}
	}
		...
		if (rc == BCME_OK) {
			...
			if (gen != commit_info->mac_entry->generation) {
				...
				if (!commit_info->mac_entry->suppressed) {
					commit_info->mac_entry->suppressed =
					    TRUE;
				}
...
static int
_dhd_wlfc_compressed_txstatus_update(dhd_pub_t *dhd, uint8 *pkt_info,
				     uint8 len, void **p_mac)
{
	...
	while (count < len) {
		if (WLFC_GET_AFQ(dhd->wlfc_mode)) {
			ret =
			    _dhd_wlfc_deque_afq(wlfc, hslot, hcnt, fifo_id,
						&pktbuf);
		} else {
			ret =
			    _dhd_wlfc_hanger_poppkt(wlfc->hanger, hslot,
						    &pktbuf, FALSE);
			if (!pktbuf) {
				_dhd_wlfc_hanger_free_pkt(wlfc, hslot,
							  WLFC_HANGER_PKT_STATE_TXSTATUS,
							  -1);
				goto cont;
			}
		}
		...
		if (!WLFC_GET_AFQ(dhd->wlfc_mode)) {
			...
			uint32 delta;
			old_t =
			    ((wlfc_hanger_t *) (wlfc->hanger))->items[hslot].
			    push_time;
		...
		if ((status_flag == WLFC_CTL_PKTFLAG_D11SUPPRESS) ||
			...
			WL_TXSTATUS_SET_GENERATION(DHD_PKTTAG_H2DTAG
						   (PKTTAG(pktbuf)), gen);

			if (WLFC_GET_REUSESEQ(dhd->wlfc_mode)) {
				WL_SEQ_SET_FROMDRV(DHD_PKTTAG_H2DSEQ
						   (PKTTAG(pktbuf)),
						   seq_fromfw);
				WL_SEQ_SET_NUM(DHD_PKTTAG_H2DSEQ
					       (PKTTAG(pktbuf)), seq_num);
			}
			...
			if (ret != BCME_OK) {
				...
				_dhd_wlfc_prec_drop(dhd, (fifo_id << 1) + 1,
						    pktbuf, FALSE);
			} else {
				if (!WLFC_GET_AFQ(dhd->wlfc_mode)) {
					/* Mark suppressed to avoid a double free
					   during wlfc cleanup
					 */
					_dhd_wlfc_hanger_mark_suppressed
					    (wlfc->hanger, hslot, gen);
				}
			}
		} else {
			...
			if (!WLFC_GET_AFQ(dhd->wlfc_mode)) {
				...
				wlfc->pkt_cnt_in_drv[DHD_PKTTAG_IF
						     (PKTTAG(pktbuf))]
...
static void
_dhd_wlfc_add_requested_entry(athost_wl_status_info_t *wlfc,
			      wlfc_mac_descriptor_t *entry)
{
	int i;
	...
	if (i == wlfc->requested_entry_count) {
		...
		ASSERT(wlfc->requested_entry_count <=
		       (WLFC_MAC_DESC_TABLE_SIZE - 1));
		wlfc->requested_entry[wlfc->requested_entry_count++] = entry;
	}
}

static void
_dhd_wlfc_remove_requested_entry(athost_wl_status_info_t *wlfc,
				 wlfc_mac_descriptor_t *entry)
{
	int i;
...
_dhd_wlfc_mac_table_update(dhd_pub_t *dhd, uint8 *value, uint8 type)
{
	...
	if (type == WLFC_CTL_TYPE_MACDESC_ADD) {
		existing_index =
		    _dhd_wlfc_find_mac_desc_id_from_mac(dhd, &value[2]);
		if ((existing_index != WLFC_MAC_DESC_ID_INVALID)
		    && (existing_index != table_index)
		    && table[existing_index].occupied) {
			/*
...
static void
_dhd_wlfc_reorderinfo_indicate(uint8 *val, uint8 len, uchar *info_buf,
			       uint *info_len)
{
	if (info_len) {
...
int dhd_wlfc_enable(dhd_pub_t *dhd)
{
...
#endif
		dhd->wlfc_state =
		    MALLOC(dhd->osh, sizeof(athost_wl_status_info_t));
	...
	if (!WLFC_GET_AFQ(dhd->wlfc_mode)) {
		wlfc->hanger =
		    _dhd_wlfc_hanger_create(dhd->osh, WLFC_HANGER_MAXITEMS);
		if (wlfc->hanger == NULL) {
	...
	_dhd_wlfc_mac_entry_update(wlfc, &wlfc->destination_entries.other,
				   eWLFC_MAC_ENTRY_ACTION_ADD, 0xff, 0, NULL,
				   NULL, NULL);
...
int dhd_wlfc_resume(dhd_pub_t *dhd)
{
	...
	bcm_mkiovar("tlv", (char *) &tlv, 4, (char *) iovbuf, sizeof(iovbuf));
	if (dhd_wl_ioctl_cmd
	    (dhd, WLC_SET_VAR, (char *) iovbuf, sizeof(iovbuf), TRUE, 0) < 0) {
		DHD_ERROR(("%s: failed to set bdcv2 tlv signaling to 0x%x\n",
			   __FUNCTION__, tlv));
		return -EPERM;
	}
			   ...
			   uchar *reorder_info_buf, uint *reorder_info_len)
{
	...
	if (dhd->proptxstatus_mode != WLFC_ONLY_AMPDU_HOSTREORDER) {
		if (!dhd->wlfc_state
		    || (dhd->proptxstatus_mode == WLFC_FCMODE_NONE)) {
			dhd_os_wlfc_unblock(dhd);
	...
	if (remainder) {
		while ((processed < (WLFC_MAX_PENDING_DATALEN * 2))
			...
			if (type == WLFC_CTL_TYPE_HOST_REORDER_RXPKTS)
				_dhd_wlfc_reorderinfo_indicate(value, len,
							       reorder_info_buf,
							       reorder_info_len);

			if (wlfc == NULL) {
				ASSERT(dhd->proptxstatus_mode ==
				       WLFC_ONLY_AMPDU_HOSTREORDER);

				if (type != WLFC_CTL_TYPE_HOST_REORDER_RXPKTS &&
				    type != WLFC_CTL_TYPE_TRANS_ID)
					DHD_INFO(("%s():%d dhd->wlfc_state is NULL yet!" " type %d remainder %d processed %d\n", __FUNCTION__, __LINE__, type, remainder, processed));
				continue;
			}
			...
			bool need_toggle_host_if)
{
	...
	if (dhdp->proptxstatus_module_ignore) {
		if (pktbuf) {
			...
			_dhd_wlfc_pushheader(ctx, pktbuf, FALSE, 0, 0, htod, 0,
					     FALSE);
	...
	if (pktbuf) {
		...
		if (rc) {
			_dhd_wlfc_prec_drop(ctx->dhdp, (ac << 1), pktbuf,
					    FALSE);
		} else {
			ctx->stats.pktin++;
			ctx->pkt_cnt_in_drv[DHD_PKTTAG_IF(PKTTAG(pktbuf))]
			    [ac]++;
		}
	}
	...
	for (ac = AC_COUNT; ac >= 0; ac--) {
		...
		while (FALSE == dhdp->proptxstatus_txoff) {
			...
			if (no_credit && (ac < AC_COUNT) && (tx_map >= rx_map)) {
				...
				lender =
				    _dhd_wlfc_borrow_credit(ctx, ac - 1, ac,
							    FALSE);
				if (lender != -1) {
					no_credit = FALSE;
				}
...
#ifdef LIMIT_BORROW
				if (lender != -1) {
					_dhd_wlfc_return_credit(ctx, lender,
								ac);
				}
#endif
				break;
...
int dhd_wlfc_txcomplete(dhd_pub_t *dhd, void *txp, bool success)
{
	...
	if (!success || dhd->proptxstatus_txstatus_ignore) {
		wlfc_mac_descriptor_t *entry =
		    _dhd_wlfc_find_table_entry(wlfc, txp);

		WLFC_DBGMESG(("At: %s():%d, bus_complete() failure for %p, htod_tag:0x%08x\n", __FUNCTION__, __LINE__, txp, DHD_PKTTAG_H2DTAG(PKTTAG(txp))));
		if (!WLFC_GET_AFQ(dhd->wlfc_mode)) {
			_dhd_wlfc_hanger_poppkt(wlfc->hanger,
						WL_TXSTATUS_GET_HSLOT
						(DHD_PKTTAG_H2DTAG
						 (PKTTAG(txp))), &pout, TRUE);
			ASSERT(txp == pout);
		}
		...
		wlfc->pkt_cnt_in_drv[DHD_PKTTAG_IF(PKTTAG(txp))][DHD_PKTTAG_FIFO
								 (PKTTAG
								  (txp))]--;
		wlfc->stats.pktout++;
		PKTFREE(wlfc->osh, txp, TRUE);
		...
		if (WLFC_GET_AFQ(dhd->wlfc_mode)) {
			...
			int hslot =
			    WL_TXSTATUS_GET_HSLOT(DHD_PKTTAG_H2DTAG
						  (PKTTAG(txp)));
			_dhd_wlfc_hanger_free_pkt(wlfc, hslot,
						  WLFC_HANGER_PKT_STATE_TXCOMPLETE,
						  -1);
		}
	}
...
int dhd_wlfc_init(dhd_pub_t *dhd)
{
	...
	/* query caps */
	ret =
	    bcm_mkiovar("wlfc_mode", (char *) &mode, 4, iovbuf,
			sizeof(iovbuf));
	if (ret > 0) {
		ret =
...
int dhd_wlfc_dump(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
	...
	bcm_bprintf(strbuf,
		    ...
		    wlfc->Init_FIFO_credit[2], wlfc->FIFO_credit[2],
		    wlfc->stats.send_pkts[2], wlfc->stats.drop_pkts[4],
		    wlfc->stats.drop_pkts[5], wlfc->stats.ooo_pkts[2],
		    wlfc->Init_FIFO_credit[3], wlfc->FIFO_credit[3],
		    wlfc->stats.send_pkts[3], wlfc->stats.drop_pkts[6],
		    wlfc->stats.drop_pkts[7], wlfc->stats.ooo_pkts[3],
		    wlfc->Init_FIFO_credit[4], wlfc->FIFO_credit[4],
		    wlfc->stats.send_pkts[4], wlfc->stats.drop_pkts[8],
		    wlfc->stats.drop_pkts[9], wlfc->stats.ooo_pkts[4]);
	...
	for (i = 0; i < WLFC_MAX_IFNUM; i++) {
		if (interfaces[i].occupied) {
			...
			bcm_bprintf(strbuf,
				    "INTERFACE[%d].PSQ(len,state,credit),(trans,supp_trans)"
				    "= (%d,%s,%d),(%d,%d)\n", i,
	...
	for (i = 0; i < WLFC_MAC_DESC_TABLE_SIZE; i++) {
		if (mac_table[i].occupied) {
			...
			bcm_bprintf(strbuf,
				    "MAC_table[%d].PSQ(len,state,credit),(trans,supp_trans)"
				    "= (%d,%s,%d),(%d,%d)\n", i,
				    mac_table[i].psq.len,
				    ((mac_table[i].state ==
				      WLFC_STATE_OPEN) ? " OPEN" : "CLOSE"),
		    ...
		    wlfc->stats.fifo_credits_back[5]);
	{
		...
		for (i = 0;
		     i <
		     sizeof(wlfc->stats.fifo_credits_sent) / sizeof(uint32);
		     i++) {
		...
		for (i = 0;
		     i <
		     sizeof(wlfc->stats.fifo_credits_back) / sizeof(uint32);
		     i++) {
	...
	bcm_bprintf(strbuf,
		    ...
		    wlfc->stats.pktout, wlfc->stats.pktdropped,
	...
	bcm_bprintf(strbuf, "wlfc- cleanup(txq,psq,fw) = (%d,%d,%d)\n",
		    wlfc->stats.cleanup_txq_cnt,
		    wlfc->stats.cleanup_psq_cnt, wlfc->stats.cleanup_fw_cnt);
	...
	for (i = 0; i < WLFC_MAX_IFNUM; i++) {
		bcm_bprintf(strbuf,
			    ...
			    i, wlfc->pkt_cnt_in_q[i][0],
			    wlfc->pkt_cnt_in_q[i][1], wlfc->pkt_cnt_in_q[i][2],
			    wlfc->pkt_cnt_in_q[i][3], wlfc->pkt_cnt_in_q[i][4]);
	}
	bcm_bprintf(strbuf, "\n");
...
int dhd_wlfc_set_module_ignore(dhd_pub_t *dhd, int val)
{
	...
	if ((bool) val != dhd->proptxstatus_module_ignore) {
		...
		dhd->proptxstatus_txstatus_ignore =
		    dhd->proptxstatus_module_ignore;
	...
	if (bChanged) {
		...
		bcm_mkiovar("tlv", (char *) &tlv, 4, iovbuf, sizeof(iovbuf));
		if (dhd_wl_ioctl_cmd
		    (dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) < 0) {
			DHD_ERROR(("%s: failed to set bdcv2 tlv signaling to 0x%x\n", __FUNCTION__, tlv));
		} else {
