				...
				wlfc_mac_descriptor_t * entry, int prec)
{
	...
	if (((entry->state == WLFC_STATE_CLOSE)
		...
		if (!(entry->ac_bitmap & (1 << prec)))
			DHD_TRACE(("ac_bitmap %d %u\n", prec,
				   entry->ac_bitmap));
		return 1;
	}
	return 0;
	...
	*needs_hdr = 1;
	...
	for (i = 0; i < total_entries; i++) {
		...
		if (entry->occupied && !entry->deleting) {
				...
				if (p == NULL) {
					if (entry->suppressed == TRUE) {
						if ((entry->
							...
							DHD_TRACE(("suppress %u %u\n", entry->suppress_count, entry->suppr_transit_count));
							return NULL;
						}
					}
					/* De-Q from delay Q */
			...
			if (entry->occupied)
				DHD_TRACE(("occupied %d %u %u\n", prec,
					   entry->occupied, entry->deleting));
		}
	}
	return NULL;
}
...
void dhd_wlfc_qmon_tx(void *state, void *pktbuf)
{
	...
	if (!ctx) {
		wlfc_mac_descriptor_t *entry =
		    _dhd_wlfc_find_table_entry(ctx, pktbuf);
		if (entry)
...
static uint8 dhd_wlfc_find_mac_desc_id_from_mac(dhd_pub_t * dhdp, uint8 * ea)
{
	...
	if (ea != NULL) {
		for (table_index = 0; table_index < WLFC_MAC_DESC_TABLE_SIZE;
		     table_index++) {
			if ((memcmp(ea, &table[table_index].ea[0],
					ETHER_ADDR_LEN) == 0)
...
void dhd_wlfc_cleanup(dhd_pub_t * dhd, ifpkt_cb_t fn, int arg)
{
	...
	for (i = 0; i < total_entries; i++) {
		if (table[i].occupied
			...
			if (fn == NULL)
				table[i].occupied = 0;
			if (arg == table[i].interface_id) {
				table[i].suppressed = FALSE;
				table[i].transit_count = 0;
				table[i].suppress_count = 0;
				table[i].suppr_transit_count = 0;
			}
		}
	}
