...
static int wl_cfgvendor_send_cmd_reply(struct wiphy *wiphy,
				       struct net_device *dev, const void *data,
				       int len)
{
	struct sk_buff *skb;
				    ...
				    wl_vendor_event_t event)
{
	...
	while (total > 0) {
		...
		/* Alloc the SKB for vendor_event */
		skb =
		    cfg80211_vendor_event_alloc(wiphy, malloc_len, event,
						kflags);
		if (!skb) {
			WL_ERR(("skb alloc failed"));
			return -ENOMEM;
		...
		while (cache && iter_cnt_to_send) {
			...
			/* Push the data to the skb */
			nla_append(skb, cnt * sizeof(wifi_gscan_result_t),
				   ptr);
						...
						const void *data, int len)
{
	...
	while (iter) {
		num_results_iter =
		    (mem_needed -
		     (int32) GSCAN_BATCH_RESULT_HDR_LEN) /
		    (int32) sizeof(wifi_gscan_result_t);
		if (num_results_iter <= 0
		    || ((iter->tot_count - iter->tot_consumed) >
			num_results_iter)) {
			break;
		}
...
static int wl_cfgvendor_gscan_anqpo_config(struct wiphy *wiphy,
					   ...
					   const void *data, int len)
{
	int err = BCME_ERROR, rem, type, hs_list_size = 0, malloc_size, i =
	    0, j, k, num_oi, oi_len;
	...
	nla_for_each_attr(iter, data, len, rem) {
		...
		case GSCAN_ATTRIBUTE_ANQPO_HS_LIST:
			if (hs_list_size > 0) {
				hs_list =
				    kmalloc(hs_list_size *
					    sizeof(wifi_passpoint_network),
					    GFP_KERNEL);
				if (hs_list == NULL) {
	...
	anqpo_hs_list =
	    (wl_anqpo_pfn_hs_list_t *) kmalloc(malloc_size, GFP_KERNEL);
	...
	if ((hs_list_size > 0) && (NULL != hs_list)) {
		...
		for (i = 0; i < hs_list_size; i++, src_hs++, dst_hs++) {
			...
			memcpy(dst_hs->realm.data, src_hs->realm,
			       dst_hs->realm.length);
			memcpy(dst_hs->plmn.mcc, src_hs->plmn,
			       ANQPO_MCC_LENGTH);
			memcpy(dst_hs->plmn.mnc, src_hs->plmn,
			       ANQPO_MCC_LENGTH);
			for (j = 0; j < ANQPO_MAX_PFN_HS; j++) {
				...
				if (0 != src_hs->roamingConsortiumIds[j]) {
					...
					rcid =
					    (char *) &src_hs->
					    roamingConsortiumIds[j];
					for (k = 0; k < ANQPO_MAX_OI_LENGTH;
					     k++)
						if (0 != rcid[k])
							oi_len++;

					dst_hs->rc.oi[j].length = oi_len;
					memcpy(dst_hs->rc.oi[j].data, rcid,
					       oi_len);
				}
			}
	...
	err = wldev_iovar_setbuf(bcmcfg_to_prmry_ndev(cfg), "anqpo_pfn_hs_list",
				 anqpo_hs_list, malloc_size, iovar_buf,
					  ...
					  const void *data, int len)
{
	...
	if (dhd_dev_set_lazy_roam_cfg(bcmcfg_to_prmry_ndev(cfg), &roam_param) <
	    0) {
		WL_ERR(("Could not set batch cfg\n"));
		err = -EINVAL;
	}
				       ...
				       const void *data, int len)
{
	...
	nla_for_each_attr(iter, data, len, tmp2) {
		...
		case GSCAN_ATTRIBUTE_BSSID_PREF_LIST:
			if (!num)
				return -EINVAL;
			bssid_pref = create_bssid_pref_cfg(num);
			if (bssid_pref == NULL) {
	...
	if (!bssid_pref) {
		...
		if (flush) {
			bssid_pref = create_bssid_pref_cfg(0);
			if (bssid_pref == NULL) {
				WL_ERR(("%s: Can't malloc memory\n",
					__FUNCTION__));
				err = -ENOMEM;
...
void wl_cfgvendor_rtt_evt(void *ctx, void *rtt_data)
{
	...
	if (list_empty(rtt_cache_list)) {
		skb =
		    cfg80211_vendor_event_alloc(wiphy, 100,
	...
	list_for_each_entry(rtt_header, rtt_cache_list, list) {
		...
		nla_put_u32(skb, RTT_ATTRIBUTE_RESULTS_COMPLETE, complete);
		rtt_nl_hdr =
		    nla_nest_start(skb, RTT_ATTRIBUTE_RESULTS_PER_TARGET);
		if (!rtt_nl_hdr) {
			WL_ERR(("rtt_nl_hdr is NULL\n"));
			break;
		}
		nla_put(skb, RTT_ATTRIBUTE_TARGET_MAC, ETHER_ADDR_LEN,
			&rtt_header->peer_mac);
		nla_put_u32(skb, RTT_ATTRIBUTE_RESULT_CNT,
			    rtt_header->result_cnt);
					    ...
					    const void *data, int len)
{
	...
	if (strncmp
	    ((char *) data, BRCM_VENDOR_SCMD_CAPA,
	     strlen(BRCM_VENDOR_SCMD_CAPA)) == 0) {
		err =
		    wldev_iovar_getbuf(bcmcfg_to_prmry_ndev(cfg), "cap", NULL,
					...
					const void *data, int len)
{
	...
	err =
	    wldev_iovar_getbuf(bcmcfg_to_prmry_ndev(cfg), "radiostat", NULL, 0,
	...
	err =
	    wldev_iovar_getbuf(bcmcfg_to_prmry_ndev(cfg), "wme_counters", NULL,
	...
	bzero(iovar_buf, WLC_IOCTL_MAXLEN);
	output =
	    (char *) iface + sizeof(wifi_iface_stat) +
					  ...
					  const void *data, int len)
{
	...
	nla_for_each_attr(iter, data, len, rem) {
		...
		case DEBUG_ATTRIBUTE_RING_NAME:
			strlcpy(ring_name, nla_data(iter),
	...
	ret =
	    dhd_os_start_logging(dhd_pub, ring_name, log_level, flags,
					 ...
					 const void *data, int len)
{
	...
	if (buf_len > 0 && user_buf) {
		...
		ret =
		    dhd_os_get_socram_dump(bcmcfg_to_prmry_ndev(cfg), &mem_buf,
					   &buf_len);
		if (ret) {
					...
					const void *data, int len)
{
	...
	ret =
	    dhd_os_get_version(bcmcfg_to_prmry_ndev(cfg), dhd_ver, &buf_ptr,
			       buf_len);
	if (ret < 0) {
...
static void wl_cfgvendor_dbg_ring_send_evt(void *ctx,
					   const int ring_id, const void *data,
					   const uint32 len,
					   const dhd_dbg_ring_status_t
					   ring_status)
{
					  ...
					  const void *data, int len)
{
	...
	ret =
	    dhd_dev_start_mkeep_alive(dhd_pub, mkeep_alive_id, ip_pkt,
...
int wl_cfgvendor_attach(struct wiphy *wiphy, dhd_pub_t *dhd)
{
	...
	dhd_os_dbg_register_callback(FW_EVENT_RING_ID,
				     wl_cfgvendor_dbg_ring_send_evt);
