...
static wl_pfn_gscan_ch_bucket_cfg_t *dhd_pno_gscan_create_channel_list(dhd_pub_t *dhd,
...
static int
_dhd_pno_gscan_cfg(dhd_pub_t *dhd, wl_pfn_gscan_cfg_t *pfncfg_gscan_param,
		   int size)
{
	int err = BCME_OK;
	NULL_CHECK(dhd, "dhd is NULL", err);
	...
	err =
	    dhd_iovar(dhd, 0, "pfn_gscan_cfg", (char *) pfncfg_gscan_param,
		      size, 1);
	if (err < 0) {
...
static int _dhd_pno_suspend(dhd_pub_t *dhd)
{
	...
	_pno_state = PNO_GET_PNOSTATE(dhd);
	err =
	    dhd_iovar(dhd, 0, "pfn_suspend", (char *) &suspend,
		      sizeof(suspend), 1);
	if (err < 0) {
	     ...
	     dhd_pno_mode_t mode)
{
	...
	if (mode & DHD_PNO_GSCAN_MODE) {
		...
		if (_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) {
			...
			if ((_params->params_legacy.pno_repeat != 0) ||
				...
				pfn_param.repeat =
				    (uchar) (_params->params_legacy.pno_repeat);
				pfn_param.exp =
				    (uchar) (_params->params_legacy.
					     pno_freq_expo_max);
			}
		}
	...
	if (mode == DHD_PNO_BATCH_MODE) {
		...
		err =
		    dhd_iovar(dhd, 0, "pfnmem", (char *) &_tmp, sizeof(_tmp),
			      1);
		if (err < 0) {
	...
	err =
	    dhd_iovar(dhd, 0, "pfn_set", (char *) &pfn_param,
		      sizeof(pfn_param), 1);
	if (err < 0) {
		DHD_ERROR(("%s : failed to execute pfn_set %d\n", __FUNCTION__,
			   err));
	...
	list_for_each_entry_safe(siter, snext,
		...
		while (phead != NULL) {
			...
			list_for_each_entry_safe(iter, next,
				...
				nleftsize -= nreadsize;
				bp += nreadsize =
				    snprintf(bp, nleftsize, "%s", AP_END_MARKER);
				nleftsize -= nreadsize;
				list_del(&iter->list);
	...
	int pno_freq_expo_max, uint16 *channel_list, int nchan)
{
	...
	if (tot_nchan > 0 && channel_list) {
		...
		if (err < 0) {
			...
			for (i = 0; i < tot_nchan; i++)
				_params->params_legacy.chan_list[i] =
				    _chan_list[i];
		}
	}
#endif				/* GSCAN_SUPPORT */
...
#endif				/* GSCAN_SUPPORT */
	err = _dhd_pno_set(dhd, _params, DHD_PNO_LEGACY_MODE);
	if (err < 0) {
		DHD_ERROR(("failed to set call pno_set (err %d) in firmware\n",
			   err));
		goto exit;
	}
	err = _dhd_pno_add_ssid(dhd, ssid_list, nssid);
	if (err < 0) {
	...
	if (tot_nchan > 0) {
		err = _dhd_pno_cfg(dhd, _chan_list, tot_nchan);
		if (err < 0) {
	...
	if (_pno_state->pno_status == DHD_PNO_DISABLED) {
		err = _dhd_pno_enable(dhd, PNO_ON);
		if (err < 0)
		      ...
		      struct dhd_pno_batch_params *batch_params)
{
	...
	if (batch_params->band == WLC_BAND_2G
		...
		err = _dhd_pno_get_channels(dhd,
					    &_params->params_batch.chan_list
					    [batch_params->nchan], &rem_nchan,
	...
	if (_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) {
		...
		err = _dhd_pno_add_ssid(dhd, &_params2->params_legacy.ssid_list,
	...
	err = _dhd_pno_set(dhd, _params, DHD_PNO_BATCH_MODE);
	if (err < 0) {
		...
		if (info) {
			...
			if (err < 0) {
				...
				*len = 0;
				...
				ptr =
				    (uint32 *) kmalloc(mem_needed, GFP_KERNEL);
				if (!ptr) {
		    ...
		    void *buf, bool flush)
{
	...
	case DHD_PNO_BATCH_SCAN_CFG_ID:
		{
			gscan_batch_params_t *ptr =
			    (gscan_batch_params_t *) buf;
			_params->params_gscan.bestn = ptr->bestn;
			_params->params_gscan.mscan = ptr->mscan;
			_params->params_gscan.buffer_threshold =
			    ptr->buffer_threshold;
		}
		break;
	case DHD_PNO_GEOFENCE_SCAN_CFG_ID:
		{
			...
			if (!_params->params_gscan.nbssid_hotlist)
				INIT_LIST_HEAD(&_params->params_gscan.
					       hotlist_bssid_list);
			...
			for (i = 0, bssid_ptr = ptr->bssid; i < ptr->nbssid;
				...
				if (!_pno_bssid) {
					DHD_ERROR(("_pno_bssid is NULL, cannot kalloc %zd bytes", sizeof(struct dhd_pno_bssid)));
					err = BCME_NOMEM;
					goto exit;
				}
				memcpy(&_pno_bssid->macaddr,
				       &bssid_ptr->macaddr, ETHER_ADDR_LEN);

				flags =
				    (int8) bssid_ptr->rssi_reporting_threshold;
			...
			_params->params_gscan.lost_ap_window =
			    ptr->lost_ap_window;
		}
		break;
...
static int
dhd_pno_set_for_gscan(dhd_pub_t *dhd,
		      struct dhd_pno_gscan_params *gscan_params)
{
	int err = BCME_OK;
	int mode, i = 0, k;
	...
	ch_bucket = dhd_pno_gscan_create_channel_list(dhd, _pno_state,
									_chan_list,
									&tot_num_buckets,
									&num_buckets_to_fw);
	if (!ch_bucket)
	...
	if ((_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) &&
		...
		err = _dhd_pno_add_ssid(dhd, &params_legacy->ssid_list,
								 params_legacy->nssid);
		if (err < 0) {
			DHD_ERROR(("failed to add ssid list (err %d) in firmware\n", err));
			goto exit;
	...
	err = _dhd_pno_set(dhd, _params, DHD_PNO_GSCAN_MODE);
	if (err < 0) {
		DHD_ERROR(("failed to set call pno_set (err %d) in firmware\n",
			   err));
		goto exit;
	...
	for (i = 0, k = 0; i < tot_num_buckets; i++) {
		if (ch_bucket[i].bucket_end_index != CHANNEL_BUCKET_EMPTY_INDEX) {
			...
			pfn_gscan_cfg_t->channel_bucket[k].
	...
	tot_nchan =
	    pfn_gscan_cfg_t->channel_bucket[num_buckets_to_fw -
					    1].bucket_end_index + 1;
	...
	err = _dhd_pno_gscan_cfg(dhd, pfn_gscan_cfg_t, gscan_param_size);
	if (err < 0) {
	...
	if (gscan_params->nbssid_hotlist) {
		...
		wl_pfn_bssid_t *ptr;
		p_pfn_bssid =
		    (wl_pfn_bssid_t *) kzalloc(sizeof(wl_pfn_bssid_t) *
		...
		DHD_PNO(("nhotlist %d\n", gscan_params->nbssid_hotlist));
		list_for_each_entry_safe(iter, next,
					 &gscan_params->hotlist_bssid_list,
					 list) {
		...
		err =
		    _dhd_pno_add_bssid(dhd, p_pfn_bssid,
				       gscan_params->nbssid_hotlist);
		if (err < 0) {
	...
	*num_buckets_to_fw = 0;
	...
	for (i = 0; i < _params->params_gscan.nchannel_buckets && nchan; i++) {
		if (!gscan_buckets[i].band) {
			ch_cnt =
			    MIN(gscan_buckets[i].num_channels, (uint8) nchan);
			num_channels += ch_cnt;
		...
		if (nchan) {
			...
			*num_buckets_to_fw = *num_buckets_to_fw + 1;
			DHD_PNO(("end_idx  %d freq_mult - %d\n",
				 ch_bucket[legacy_bucket_idx].bucket_end_index,
				 ch_bucket
				 [legacy_bucket_idx].bucket_freq_multiple));
		}
	}
	return ch_bucket;
}
...
static int dhd_pno_stop_for_gscan(dhd_pub_t *dhd)
{
	...
	if (_pno_state->pno_params_arr[INDEX_OF_GSCAN_PARAMS].params_gscan.
	    mscan) {
		/* retrieve the batching data from firmware into host */
		err = dhd_wait_batch_results_complete(dhd);
		if (err != BCME_OK)
...
int dhd_pno_initiate_gscan_request(dhd_pub_t *dhd, bool run, bool flush)
{
	...
	if (run) {
		...
		if (flush) {
			mutex_lock(&_pno_state->pno_mutex);
			dhd_pno_reset_cfg_gscan(params, _pno_state,
						GSCAN_FLUSH_ALL_CFG);
			mutex_unlock(&_pno_state->pno_mutex);
		}
...
int dhd_pno_enable_full_scan_result(dhd_pub_t *dhd, bool real_time_flag)
{
	...
	if (_pno_state->pno_mode & DHD_PNO_GSCAN_MODE) {
		if (old_flag != gscan_params->send_all_results_flag) {
			...
			err = _dhd_pno_gscan_cfg(dhd, &gscan_cfg,
										  sizeof
										  (wl_pfn_gscan_cfg_t));
			if (err < 0) {
...
static int _dhd_pno_get_gscan_batch_from_fw(dhd_pub_t *dhd)
{
	...
	while (plbestnet->status != PFN_COMPLETE) {
		...
		for (i = 0, count = 0; i < plbestnet->count; i++, count++) {
			...
			if (TIME_DIFF_MS(timestamp, plnetinfo->timestamp) >
			    timediff) {
		...
		for (i = 0; i < num_scans_in_cur_iter; i++) {
			...
			for (j = 0; j < nAPs_per_scan[i]; j++, plnetinfo++) {
				...
				memcpy(result->ssid, plnetinfo->pfnsubnet.u.SSID,
				       plnetinfo->pfnsubnet.SSID_len);
				result->ssid[plnetinfo->pfnsubnet.SSID_len] =
				    '\0';
				memcpy(&result->macaddr,
				       &plnetinfo->pfnsubnet.BSSID,
				       ETHER_ADDR_LEN);

				DHD_PNO(("\tSSID : "));
				DHD_PNO(("\n"));
				DHD_PNO(("\tBSSID: %02x:%02x:%02x:%02x:%02x:%02x\n", result->macaddr.octet[0], result->macaddr.octet[1], result->macaddr.octet[2], result->macaddr.octet[3], result->macaddr.octet[4], result->macaddr.octet[5]));
				DHD_PNO(("\tchannel: %d, RSSI: %d, timestamp: %d ms\n", plnetinfo->pfnsubnet.channel, plnetinfo->RSSI, plnetinfo->timestamp));
...
_dhd_pno_get_for_batch(dhd_pub_t *dhd, char *buf, int bufsize, int reason)
{
	...
	if (buf && bufsize) {
		...
		if (!list_empty
	      ...
	      convert_format:
		err =
		    _dhd_pno_convert_format(dhd, &_params->params_batch, buf,
					    bufsize);
		if (err < 0) {
...
int dhd_pno_get_for_batch(dhd_pub_t *dhd, char *buf, int bufsize, int reason)
{
...
#endif
	{
		if (!(_pno_state->pno_mode & DHD_PNO_BATCH_MODE)) {
			...
			snprintf(pbuf, 56, "%s", RESULTS_END_MARKER);
			err = strlen(buf);
			goto exit;
		}
			...
			struct dhd_pno_hotlist_params *hotlist_params)
{
	...
	if (hotlist_params->band == WLC_BAND_2G
		...
		/* get a valid channel list based on band B or A */
		err = _dhd_pno_get_channels(dhd,
					    &_params->params_hotlist.chan_list
					    [hotlist_params->nchan], &rem_nchan,
					    hotlist_params->band, FALSE);
		if (err < 0) {
	...
	err = _dhd_pno_set(dhd, _params, DHD_PNO_HOTLIST_MODE);
	if (err < 0) {
		DHD_ERROR(("%s : failed to set call pno_set (err %d) in firmware\n", __FUNCTION__, err));
		goto exit;
	}
				  ...
				  uint32 event, int *size)
{
	...
	*size = 0;
	...
	if (event == WLC_E_PFN_NET_FOUND || event == WLC_E_PFN_NET_LOST) {
		...
		results =
		    (dhd_epno_results_t *) kmalloc(mem_needed, GFP_KERNEL);
		if (!results) {
		...
		for (i = 0; i < count; i++) {
			...
			results[i].flags =
			    (event ==
			     WLC_E_PFN_NET_FOUND) ? WL_PFN_SSID_EXT_FOUND :
			...
			DHD_PNO(("ssid - %s bssid %02x:%02x:%02x:%02x:%02x:%02x " "ch %d rssi %d flags %d\n", ssid, bssid->octet[0], bssid->octet[1], bssid->octet[2], bssid->octet[3], bssid->octet[4], bssid->octet[5], results[i].channel, results[i].rssi, results[i].flags));
		}
	}
	*size = mem_needed;
	return results;
...
void *dhd_pno_process_anqpo_result(dhd_pub_t *dhd, const void *data,
				   uint32 event, int *size)
{
	...
	if (event == WLC_E_PFN_NET_FOUND) {
		mem_needed =
		    OFFSETOF(wifi_gscan_full_result_t,
			     ie_data) + bi->ie_length + OFFSETOF(wl_event_gas_t,
								 data) +
		    gas_data->data_len + sizeof(int);
		result =
		    (wifi_gscan_full_result_t *) kmalloc(mem_needed, GFP_KERNEL);
		if (NULL == result) {
		...
		memcpy((uint8 *) result +
		       ...
		       gas_data, OFFSETOF(wl_event_gas_t,
					  data) + gas_data->data_len);
				  ...
				  int *send_evt_bytes, hotlist_type_t type)
{
	...
	gscan_hotlist_cache =
	    (gscan_results_cache_t *) kmalloc(malloc_size, GFP_KERNEL);
	...
	for (i = 0; i < results->count; i++, plnetinfo++) {
		...
		hotlist_found_array->channel =
		    wf_channel2mhz(plnetinfo->pfnsubnet.channel,
				   (plnetinfo->pfnsubnet.channel <=
				    CH_MAX_2G_CHANNEL ? WF_CHAN_FACTOR_2_4_G :
				    WF_CHAN_FACTOR_5_G));
		...
		memcpy(&hotlist_found_array->macaddr,
		       &plnetinfo->pfnsubnet.BSSID, ETHER_ADDR_LEN);
