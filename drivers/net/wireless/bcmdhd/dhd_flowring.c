...
void /* Initialize a flow ring's queue */ dhd_flow_queue_init(dhd_pub_t *dhdp,
							      flow_queue_t *queue, int max)
{
...
int dhd_flow_rings_init(dhd_pub_t *dhdp, uint32 num_flow_rings)
{
	...
	flowid_allocator = id16_map_init(dhdp->osh,
					 num_flow_rings - FLOW_RING_COMMON,
					 FLOWID_RESERVED);
	...
	flow_ring_table =
	    (flow_ring_table_t *) MALLOC(dhdp->osh, flow_ring_table_sz);
...
fail:
	...
	if (flow_ring_table != NULL) {
		for (idx = 0; idx < num_flow_rings; idx++) {
			if (flow_ring_table[idx].lock != NULL)
				dhd_os_spin_lock_deinit(dhdp->osh,
							flow_ring_table
							[idx].lock);
		}
	...
	if (dhdp->flow_ring_table != NULL) {
		...
		for (idx = 0; idx < dhdp->num_flow_rings; idx++) {
			if (flow_ring_table[idx].active) {
				dhd_bus_clean_flow_ring(dhdp->bus,
							&flow_ring_table[idx]);
			}
			...
			dhd_os_spin_lock_deinit(dhdp->osh,
						flow_ring_table[idx].lock);
			flow_ring_table[idx].lock = NULL;
		}

		/* Destruct the flow ring table */
		flow_ring_table_sz =
		    dhdp->num_flow_rings * sizeof(flow_ring_table_t);
		MFREE(dhdp->osh, flow_ring_table, flow_ring_table_sz);
	}
...
dhd_flowid_find(dhd_pub_t *dhdp, uint8 ifindex, uint8 prio, char *sa, char *da)
{
	...
	if (DHD_IF_ROLE_STA(if_flow_lkup[ifindex].role)) {
		...
		if (dhdp->peer_tbl.tdls_peer_count && !(ETHER_ISMULTI(da)) &&
			...
			while (cur != NULL) {
				if (!memcmp
				    (cur->flow_info.da, da, ETHER_ADDR_LEN)) {
					DHD_FLOWID_UNLOCK(dhdp->flowid_lock,
							  flags);
					return cur->flowid;
				}
				cur = cur->next;
...
/* Allocate Flow ID */
static INLINE uint16
dhd_flowid_alloc(dhd_pub_t *dhdp, uint8 ifindex, uint8 prio, char *sa,
		 char *da)
{
	...
	fl_hash_node =
	    (flow_hash_info_t *) MALLOC(dhdp->osh, sizeof(flow_hash_info_t));
	if (fl_hash_node == NULL) {
		DHD_ERROR(("%s: fl_hash_node alloc failed  \n", __FUNCTION__));
		return FLOWID_INVALID;
	}
	memcpy(fl_hash_node->flow_info.da, da,
	       sizeof(fl_hash_node->flow_info.da));
	...
	if (DHD_IF_ROLE_STA(if_flow_lkup[ifindex].role)) {
		...
		if (dhdp->peer_tbl.tdls_peer_count &&
			...
			if (cur) {
				...
				if_flow_lkup[ifindex].fl_hash[hash] =
				    fl_hash_node;
			}
		} else
#endif				/* WLTDLS */
...
dhd_flowid_update(dhd_pub_t *dhdp, uint8 ifindex, uint8 prio, void *pktbuf)
{
	...
	if (dhd_flowid_lookup
	    (dhdp, ifindex, prio, eh->ether_shost, eh->ether_dhost,
	     &flowid) != BCME_OK) {
		return BCME_ERROR;
	}
...
void dhd_flowid_free(dhd_pub_t *dhdp, uint8 ifindex, uint16 flowid)
{
	...
	for (hashix = 0; hashix < DHD_FLOWRING_HASH_SIZE; hashix++) {
		...
		if (cur) {
			...
			if (found) {
				if (!prev) {
					if_flow_lkup[ifindex].fl_hash[hashix] =
					    cur->next;
				} else {
					prev->next = cur->next;
				}
...
void dhd_flow_rings_delete_for_peer(dhd_pub_t *dhdp, uint8 ifindex, char *addr)
{
	...
	for (id = 0; id < dhdp->num_flow_rings; id++) {
		if (flow_ring_table[id].active &&
		    (flow_ring_table[id].flow_info.ifindex == ifindex) &&
		    (!memcmp
		     (flow_ring_table[id].flow_info.da, addr, ETHER_ADDR_LEN))
		    && (flow_ring_table[id].status !=
			FLOW_RING_STATUS_DELETE_PENDING)) {
			DHD_INFO(("%s: deleting flowid %d\n", __FUNCTION__,
				  flow_ring_table[id].flowid));
...
int dhd_flow_prio_map(dhd_pub_t *dhd, uint8 *map, bool set)
{
	uint8 iovbuf[24];
	if (!set) {
		bcm_mkiovar("bus:fl_prio_map", NULL, 0, (char *) iovbuf,
			    sizeof(iovbuf));
		if (dhd_wl_ioctl_cmd
		    (dhd, WLC_GET_VAR, iovbuf, sizeof(iovbuf), FALSE, 0) < 0) {
	...
	bcm_mkiovar("bus:fl_prio_map", (char *) map, 4, (char *) iovbuf,
		    sizeof(iovbuf));
