...
#else
static inline int dhd_read_macaddr(struct dhd_info *dhd)
{
	return 0;
}
#endif
...
#else
static inline int dhd_write_macaddr(struct ether_addr *mac)
{
	return 0;
}
#endif
...
static uint32 maxdelay, tspktcnt, maxdelaypktno;
...
static int dhd_get_pkt_ether_type(dhd_pub_t *dhd, void *skb, uint8 **data_ptr,
				  int *len_ptr, uint16 *et_ptr,
				  bool *snap_ptr);

static int dhd_convert_dhcp_broadcast_ack_to_unicast(dhd_pub_t *pub,
						     void *pktbuf, int ifidx);
...
#ifdef SAR_SUPPORT
static int dhd_sar_callback(struct notifier_block *nfb, unsigned long action,
			    void *data)
{
	dhd_info_t *dhd =
	    (dhd_info_t *) container_of(nfb, struct dhd_info, sar_notifier);
	char iovbuf[32];
	s32 sar_enable;
	s32 txpower;
	int ret;
	...
	if (data) {
		...
		bcm_mkiovar("qtxpower", (char *) &txpower, 4, iovbuf,
			    sizeof(iovbuf));
		ret = dhd_wl_ioctl_cmd(&dhd->pub, WLC_SET_VAR, iovbuf,
		...
		if (dhd->sar_enable == sar_enable)
			return NOTIFY_DONE;
		bcm_mkiovar("sar_enable", (char *) &sar_enable, 4, iovbuf,
			    sizeof(iovbuf));
		ret = dhd_wl_ioctl_cmd(&dhd->pub, WLC_SET_VAR, iovbuf,
							  sizeof(iovbuf), TRUE, 0);
		if (ret < 0)
...
static void dhd_if_del_sta_list(dhd_if_t *ifp)
{
	...
	list_for_each_entry_safe(sta, next, &ifp->sta_list, list) {
		...
		if (ifp->fwdh) {
			...
			fwder_deassoc(ifp->fwdh, (uint16 *) (sta->ea.octet),
				      (wofa_t) sta);
		}
#endif				/* BCM_GMAC3 */
		list_del(&sta->list);
	...
	list_for_each_entry_safe(sta, next, &ifp->sta_list, list) {
		if (!memcmp(sta->ea.octet, ea, ETHER_ADDR_LEN)) {
			...
			if (ifp->fwdh) {	/* Found a sta, remove from WOFA forwarder. */
				ASSERT(ISALIGNED(ea, 2));
				fwder_deassoc(ifp->fwdh, (uint16 *) ea,
					      (wofa_t) sta);
			}
#endif				/* BCM_GMAC3 */
			list_del(&sta->list);
...
static bool _turn_on_arp_filter(dhd_pub_t *dhd, int op_mode)
{
	bool _apply = FALSE;
...
void dhd_enable_packet_filter(int value, dhd_pub_t *dhd)
{
	...
	if (dhd_pkt_filter_enable && (!value || (dhd_support_sta_mode(dhd)
						 && !dhd->dhcp_in_progress))) {
		for (i = 0; i < dhd->pktfilter_count; i++) {
#ifndef GAN_LITE_NAT_KEEPALIVE_FILTER
			if (value && (i == DHD_ARP_FILTER_NUM) &&
			    !_turn_on_arp_filter(dhd, dhd->op_mode)) {
				DHD_TRACE(("Do not turn on ARP white list pkt filter:" "val %d, cnt %d, op_mode 0x%x\n", value, i, dhd->op_mode));
				continue;
			}
...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	if (dhd->up) {
		if (value && dhd->in_suspend) {
			 ...
			 */
			bcn_li_dtim =
			    dhd_get_suspend_bcn_li_dtim(dhd, &dtim_period,
							&bcn_interval);
			...
			if (FW_SUPPORTED(dhd, ndoe)) {
				...
				nd_ra_filter = 1;
				bcm_mkiovar("nd_ra_filter_enable",
					    (char *) &nd_ra_filter, 4, iovbuf,
					    sizeof(iovbuf));
				ret =
				     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
						      sizeof(iovbuf), TRUE,
			...
			dhd_iovar(dhd, 0, "lpas", (char *) &lpas, sizeof(lpas),
				  1);
...
int dhd_ifidx2hostidx(dhd_info_t *dhd, int ifidx)
{
	int i = DHD_MAX_IFS;
...
static void dhd_ifadd_event_handler(void *handle, void *event_info, u8 event)
{
	...
	if (FW_SUPPORTED((&dhd->pub), ap)
	    && !(DHD_IF_ROLE_STA(if_event->event.role))) {
		char iovbuf[WLC_IOCTL_SMLEN];
		uint32 var_int = 1;

		memset(iovbuf, 0, sizeof(iovbuf));
		bcm_mkiovar("ap_isolate", (char *) &var_int, 4, iovbuf,
			    sizeof(iovbuf));
		dhd_wl_ioctl_cmd(&dhd->pub, WLC_SET_VAR, iovbuf,
				 sizeof(iovbuf), TRUE, ifidx);
	}
#endif				/* PCIE_FULL_DONGLE */
done:
...
static void dhd_set_multicast_list(struct net_device *dev)
{
	...
	dhd_deferred_schedule_work(dhd->dhd_deferred_wq,
				   (void *) dhd->iflist[ifidx],
...
void dhd_tx_dump(osl_t *osh, void *pkt)
{
	uint8 *dump_data;
	uint16 protocol;
...
int BCMFASTPATH dhd_sendpkt(dhd_pub_t *dhdp, int ifidx, void *pktbuf)
{
	...
	if (PKTLEN(dhdp->osh, pktbuf) >= ETHER_HDR_LEN) {
		...
		if (ntoh16(eh->ether_type) == ETHER_TYPE_IP) {
			...
			source_port =
			    (pktdata[udp_port_pos] << 8) | pktdata[udp_port_pos
								   + 1];
			dest_port =
			    (pktdata[udp_port_pos + 2] << 8) |
			    pktdata[udp_port_pos + 3];
	 ...
	 */
	ret =
	    dhd_flowid_update(dhdp, ifidx,
			      dhdp->flow_prio_map[(PKTPRIO(pktbuf))], pktbuf);
	if (ret != BCME_OK) {
	...
	if (dhd_wlfc_is_supported(dhdp)) {
		...
		if (ETHER_ISMULTI(eh->ether_dhost))
			...
			DHD_PKTTAG_SETFIFO(PKTTAG(pktbuf),
...
#ifdef PROP_TXSTATUS
	{
		if (dhd_wlfc_commit_packets
		    (dhdp, (f_commitpkt_t) dhd_bus_txdata, dhdp->bus, pktbuf,
...
int BCMFASTPATH dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	if (skb_headroom(skb) < dhd->pub.hdrlen + htsfdlystat_sz) {
		...
		skb2 =
		    skb_realloc_headroom(skb, dhd->pub.hdrlen + htsfdlystat_sz);
	...
	if (ifp->wmf.wmf_enable && (ntoh16(eh->ether_type) == ETHER_TYPE_IP) &&
		...
		ucast_convert = dhd->pub.wmf_ucast_upnp
		...
		if (ucast_convert) {
			...
			sdu_clone =
				     PKTDUP(dhd->pub.osh, pktbuf);
				if (sdu_clone == NULL) {
					DHD_IF_STA_LIST_UNLOCK(ifp, flags);
					DHD_PERIM_UNLOCK_TRY(DHD_FWDER_UNIT
							     (dhd), TRUE);
					DHD_OS_WAKE_UNLOCK(&dhd->pub);
					return WMF_NOP;
				}
				dhd_wmf_forward(ifp->wmf.wmfh, sdu_clone, 0,
						sta, 1);
			}
...
#endif				/* defined(DHD_IGMP_UCQUERY) || defined(DHD_UCAST_UPNP) */
		{
			 ...
			 */
			ret =
			    dhd_wmf_packets_handle(&dhd->pub, pktbuf, NULL,
						   ifidx, 0);
			switch (ret) {
			case WMF_TAKEN:
			case WMF_DROP:
	     ...
	     int pkt_wake, wake_counts_t *wcp)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if (dhdp->block_ping) {
			if (dhd_l2_filter_block_ping(dhdp, pktbuf, ifidx) ==
			    BCME_OK) {
		...
		if (ifp->wmf.wmf_enable && (ETHER_ISMULTI(eh->ether_dhost))) {
			...
			sta =
			    dhd_find_sta(dhdp, ifidx, (void *) eh->ether_shost);
			ret =
			    dhd_wmf_packets_handle(dhdp, pktbuf, sta, ifidx, 1);
			switch (ret) {
			case WMF_TAKEN:
		...
		if ((DHD_IF_ROLE_AP(dhdp, ifidx)
		     || DHD_IF_ROLE_P2PGO(dhdp, ifidx)) && (!ifp->ap_isolate)) {
			eh = (struct ether_header *) PKTDATA(dhdp->osh, pktbuf);
			if (ETHER_ISUCAST(eh->ether_dhost)) {
				if (dhd_find_sta
				    (dhdp, ifidx, (void *) eh->ether_dhost)) {
	...
	|| defined(DHD_WAKE_STATUS)
		...
		if (protocol != ETHER_TYPE_BRCM && protocol == ETHER_TYPE_IP) {
			...
			udp_port_pos = ETHER_HDR_LEN + ip_header_len;
			source_port =
			    (dump_data[udp_port_pos] << 8) |
			    dump_data[udp_port_pos + 1];
			dest_port =
			    (dump_data[udp_port_pos + 2] << 8) |
			    dump_data[udp_port_pos + 3];
			if (source_port == 0x0044 || dest_port == 0x0044) {
				dump_hex =
				    (dump_data[udp_port_pos + 249] << 8) |
		...
		if (ntoh16(skb->protocol) == ETHER_TYPE_BRCM) {
			...
			if (dhd_rx_suspend_again(skb) != 0) {
				if (skb->dev->ieee80211_ptr
				    && skb->dev->ieee80211_ptr->ps == false)
					tout_rx =
					    CUSTOM_DHCP_LOCK_xTIME *
					    DHD_PACKET_TIMEOUT_MS;
				else
					tout_rx = DHD_PACKET_TIMEOUT_MS;
			}
#ifdef PROP_TXSTATUS
			dhd_wlfc_save_rxpath_ac_time(dhdp,
						     (uint8) PKTPRIO(skb));
#endif				/* PROP_TXSTATUS */
...
#define RXF_WATCHDOG_TIME 250	/* BARK_TIME(1000) /  */
	...
	if (dhd_rxf_prio > 0) {
		...
		param.sched_priority =
		    (dhd_rxf_prio <
		     MAX_RT_PRIO) ? dhd_rxf_prio : (MAX_RT_PRIO - 1);
		setScheduler(current, SCHED_FIFO, &param);
	}
...
static int dhd_ethtool(dhd_info_t *dhd, void *uaddr)
{
	...
	case ETHTOOL_GDRVINFO:
		...
		if (copy_from_user(&info, uaddr, sizeof(info)))
			return -EFAULT;
		strlcpy(drvname, info.driver, sizeof(info.driver));
		...
		if (strcmp(drvname, "?dhd") == 0) {
			...
			strlcpy(info.version, EPI_VERSION_STR,
				sizeof(info.version));
...
static bool dhd_check_hang(struct net_device *net, dhd_pub_t *dhdp, int error)
{
	...
	if ((error == -ETIMEDOUT) || (error == -EREMOTEIO) ||
		...
		DHD_ERROR(("%s: Event HANG send up due to  re=%d te=%d e=%d s=%d\n", __FUNCTION__, dhdp->rxcnt_timeout, dhdp->txcnt_timeout, error, dhdp->busstate));
		net_os_send_hang_message(net);
		return TRUE;
	}
	return FALSE;
		      ...
		      void *data_buf)
{
...
#ifdef BCM_FD_AGGR
		bcmerror =
		    dhd_fdaggr_ioctl(pub, ifidx, (wl_ioctl_t *) ioc, data_buf,
				     buflen);
#else
	...
	bcmerror =
	    dhd_wl_ioctl(pub, ifidx, (wl_ioctl_t *) ioc, data_buf, buflen);
...
static int dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (dhd->pub.hang_was_sent) {
		...
		DHD_OS_WAKE_LOCK_CTRL_TIMEOUT_ENABLE(&dhd->pub,
	...
	if (is_compat_task()) {
		...
		if (copy_from_user
		    (&compat_ioc, ifr->ifr_data, sizeof(compat_wl_ioctl_t))) {
...
#endif				/* CONFIG_COMPAT */
	{
		...
		/* To differentiate between wl and dhd read 4 more byes */
		if ((copy_from_user
		     (&ioc.driver, (char *) ifr->ifr_data + sizeof(wl_ioctl_t),
		      sizeof(uint)) != 0)) {
			bcmerror = BCME_BADADDR;
			goto done;
		}
...
static int dhd_interworking_enable(dhd_pub_t *dhd)
{
	...
	bcm_mkiovar("interworking", (char *) &enable, sizeof(enable), iovbuf,
		    sizeof(iovbuf));

	ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE,
					  0);
	if (ret < 0) {
	...
	if (ret == BCME_OK) {
		...
		bcm_mkiovar("wnm", (char *) &cap, sizeof(cap), iovbuf,
			    sizeof(iovbuf));
...
int
dhd_event_ifadd(dhd_info_t *dhdinfo, wl_event_data_if_t *ifevent, char *name,
		uint8 *mac)
{

#ifdef WL_CFG80211
	if (wl_cfg80211_notify_ifadd(ifevent->ifidx, name, mac, ifevent->bssidx)
	    == BCME_OK)
		return BCME_OK;
#endif
	...
	if (ifevent->ifidx > 0) {
		...
		strlcpy(if_event->name, name, IFNAMSIZ);
...
int
dhd_event_ifdel(dhd_info_t *dhdinfo, wl_event_data_if_t *ifevent, char *name,
		uint8 *mac)
{
	dhd_if_event_t *if_event;

#ifdef WL_CFG80211
	if (wl_cfg80211_notify_ifdel(ifevent->ifidx, name, mac, ifevent->bssidx)
	    == BCME_OK)
		return BCME_OK;
#endif				/* WL_CFG80211 */
 ...
 */
struct net_device *dhd_allocate_if(dhd_pub_t *dhdpub, int ifidx, char *name,
...
int dhd_init_logstrs_array(dhd_event_log_t *temp)
{
...
dhd_pub_t *dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	dhd =
	    wifi_platform_prealloc(adapter, DHD_PREALLOC_DHD_INFO,
				   sizeof(dhd_info_t));
	if (dhd == NULL) {
	...
	get_customized_country_code(dhd->adapter,
				    dhd->pub.dhd_cspec.country_abbrev,
	...
	if (iface_name[0]) {
		int len;
		char ch;
		strlcpy(if_name, iface_name, IFNAMSIZ);
	...
	dhd->pub.wlfc_state =
	    MALLOC(dhd->pub.osh, sizeof(athost_wl_status_info_t));
	...
	cpufreq_register_notifier(&dhd->freq_trans,
...
{
	if (strnstr(dhdinfo->fw_path, "_apsta", PATH_MAX) != NULL)
		return DHD_FLAG_HOSTAP_MODE;
	if (strnstr(dhdinfo->fw_path, "_p2p", PATH_MAX) != NULL)
		return DHD_FLAG_P2P_MODE;
	if (strnstr(dhdinfo->fw_path, "_ibss", PATH_MAX) != NULL)
		return DHD_FLAG_IBSS_MODE;
	if (strnstr(dhdinfo->fw_path, "_mfg", PATH_MAX) != NULL)
...
bool dhd_update_fw_nv_path(dhd_info_t *dhdinfo)
{
	...
	if (fw && fw[0] != '\0') {
		...
		strlcpy(dhdinfo->fw_path, fw, sizeof(dhdinfo->fw_path));
	...
	if (nv && nv[0] != '\0') {
		...
		strlcpy(dhdinfo->nv_path, nv, sizeof(dhdinfo->nv_path));
...
int dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	if (dhd->pub.busstate == DHD_BUS_DOWN && dhd_update_fw_nv_path(dhd)) {
		...
		ret =
		    dhd_bus_download_firmware(dhd->pub.bus, dhd->pub.osh,
					      dhd->fw_path, dhd->nv_path);
		if (ret < 0) {
...
#ifdef PCIE_FULL_DONGLE
	{
		...
		ret = dhd_flow_rings_init(&dhd->pub,
							 num_flowrings);
		if (ret != BCME_OK) {
			DHD_PERIM_UNLOCK(dhdp);
			return ret;
		     ...
		     struct ether_addr *mac)
{
	...
	if (dhd->tdls_enable == tdls_on)
		goto auto_mode;
	bcm_mkiovar("tdls_enable", (char *) &tdls, sizeof(tdls), iovbuf,
		    sizeof(iovbuf));
...
auto_mode:
	...
	bcm_mkiovar("tdls_auto_op", (char *) &tdls_auto_op,
		    sizeof(tdls_auto_op), iovbuf, sizeof(iovbuf));
	...
	if (tdls_auto_op) {
		...
		bcm_mkiovar("tdls_rssi_high", (char *) &tdls_rssi_high, 4,
			    iovbuf, sizeof(iovbuf));
		ret =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
				      TRUE, 0);
		if (ret < 0) {
		...
		bcm_mkiovar("tdls_rssi_low", (char *) &tdls_rssi_low, 4, iovbuf,
			    sizeof(iovbuf));
...
uint32 dhd_get_concurrent_capabilites(dhd_pub_t *dhd)
{
	...
	if (!FW_SUPPORTED(dhd, p2p)) {
		...
		ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, buf, sizeof(buf),
					    FALSE, 0);
		if (ret < 0) {
...
int dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (!ret) {
		...
		bcm_mkiovar("cur_etheraddr", (void *) &ea_addr, ETHER_ADDR_LEN,
			    buf, sizeof(buf));
		ret =
		    dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, buf, sizeof(buf), TRUE,
				     0);
		if (ret < 0) {
	...
	bcm_mkiovar("cap", 0, 0, dhd->fw_capabilities,
		    sizeof(dhd->fw_capabilities));

	ret =
	     dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, dhd->fw_capabilities,
			      sizeof(dhd->fw_capabilities), FALSE, 0);
	if (ret < 0) {
	...
	if ((!op_mode && dhd_get_fw_mode(dhd->info) == DHD_FLAG_HOSTAP_MODE) ||
		...
		bcm_mkiovar("cur_etheraddr", (void *) iovbuf, ETHER_ADDR_LEN,
			    buf, sizeof(buf));
		ret =
		    dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, buf, sizeof(buf), TRUE,
				     0);
...
#endif
#ifdef MAX_AP_CLIENT_CNT
		bcm_mkiovar("maxassoc", (char *) &max_assoc, 4, iovbuf,
			    sizeof(iovbuf));
		ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
							  TRUE, 0);
		if (ret < 0) {
		...
		if ((!op_mode
		     && dhd_get_fw_mode(dhd->info) == DHD_FLAG_P2P_MODE)
		...
		if (dhd->op_mode != DHD_FLAG_IBSS_MODE && concurrent_mode) {
#if defined(ARP_OFFLOAD_SUPPORT)
			arpoe = 1;
#endif
		...
		if (dhd->op_mode & DHD_FLAG_P2P_MODE) {
			bcm_mkiovar("apsta", (char *) &apsta, 4, iovbuf,
				    sizeof(iovbuf));
			ret =
			     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
					      sizeof(iovbuf), TRUE, 0);
	...
	/* Set Listen Interval */
	bcm_mkiovar("assoc_listen", (char *) &listen_interval, 4, iovbuf,
		    sizeof(iovbuf));
	ret =
	     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE,
			      0);
	if (ret < 0)
	...
	if (ret < 0)
		DHD_ERROR(("%s: roam scan period set failed %d\n", __FUNCTION__,
			   ret));
	if ((dhd_wl_ioctl_cmd
	     (dhd, WLC_SET_ROAM_DELTA, roam_delta, sizeof(roam_delta), TRUE,
	      0)) < 0)
	...
	if (roam_trigger[0] == WL_AUTO_ROAM_TRIGGER) {
		...
		if (dhd_wl_ioctl_cmd
		    (dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE,
		     0) == BCME_OK)
	...
	if (ret < 0) {
		if (ret != BCME_NOTDOWN) {
			...
			bcm_mkiovar("lpc", (char *) &lpc, 4, iovbuf,
				    sizeof(iovbuf));
			ret =
			    dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
					     sizeof(iovbuf), TRUE, 0);
	...
	bcm_mkiovar("bus:credall", (char *) &credall, 4, iovbuf,
		    sizeof(iovbuf));
	dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
#endif
	...
	if (glom != DEFAULT_GLOM_VALUE) {
		...
		bcm_mkiovar("bus:txglom", (char *) &glom, 4, iovbuf,
			    sizeof(iovbuf));
		dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
				 TRUE, 0);
	}
	...
	bcm_mkiovar("assoc_retry_max", (char *) &retry_max, 4, iovbuf,
		    sizeof(iovbuf));
	dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
#if defined(AP) && !defined(WLP2P)
	/* Turn off MPC in AP mode */
	...
	if (ap_fw_loaded == TRUE) {
		dhd_wl_ioctl_cmd(dhd, WLC_SET_DTIMPRD, (char *) &dtim,
				 sizeof(dtim), TRUE, 0);
	}
#endif
...
#endif
	if (ampdu_ba_wsize != 0) {
		bcm_mkiovar("ampdu_ba_wsize", (char *) &ampdu_ba_wsize, 4,
			    iovbuf, sizeof(iovbuf));
		ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
							  TRUE, 0);
		if (ret < 0) {
	...
	if (ampdu_mpdu != 0 && (ampdu_mpdu <= ampdu_ba_wsize)) {
		bcm_mkiovar("ampdu_mpdu", (char *) &ampdu_mpdu, 4, iovbuf,
			    sizeof(iovbuf));
		ret =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
				      TRUE, 0);
	...
	if (ampdu_release != 0 && (ampdu_release <= ampdu_ba_wsize)) {
		bcm_mkiovar("ampdu_release", (char *) &ampdu_release, 4, iovbuf,
			    sizeof(iovbuf));
		ret =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
				      TRUE, 0);
	...
	bcm_mkiovar("buf_key_b4_m4", (char *) &buf_key_b4_m4, 4, iovbuf,
		    sizeof(iovbuf));
	ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE,
					  0);
	if (ret < 0) {
	...
	bcm_mkiovar("event_msgs", eventmask, WL_EVENTING_MASK_LEN, iovbuf,
		    sizeof(iovbuf));
	ret =
	     dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, iovbuf, sizeof(iovbuf), FALSE,
			      0);
	if (ret < 0) {
	...
	bcm_mkiovar("event_msgs_ext", (char *) eventmask_msg, msglen, iov_buf,
		    sizeof(iov_buf));
	ret2 =
	    dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, iov_buf, sizeof(iov_buf), FALSE,
			     0);
	...
	ret =
	     dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, buf, sizeof(buf), FALSE,
			      0);
	if (ret < 0)
...
#ifndef DISABLE_11N
	bcm_mkiovar("ampdu_hostreorder", (char *) &hostreorder, 4, iovbuf,
		    sizeof(iovbuf));
	ret2 = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE,
					  0);
	if (ret2 < 0) {
	...
	if (FW_SUPPORTED(dhd, ap)) {
		...
		bcm_mkiovar("ap_isolate", (char *) &wl_ap_isolate, 4, iovbuf,
			    sizeof(iovbuf));
		ret =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
				      TRUE, 0);
			  ...
			  int cmd, uint8 set, int ifidx)
{
	...
	if (bcm_mkiovar(name, NULL, 0, iovbuf, sizeof(iovbuf))) {
		ret =
		    dhd_wl_ioctl_cmd(dhd_pub, cmd, iovbuf, sizeof(iovbuf), set,
				     ifidx);
		if (!ret) {
			  ...
			  int cmd, uint8 set, int ifidx)
{
	...
	/* memset(iovbuf, 0, sizeof(iovbuf)); */
	if (bcm_mkiovar
	    (name, (char *) &lval, sizeof(lval), iovbuf, sizeof(iovbuf))) {
		ret =
		    dhd_wl_ioctl_cmd(dhd_pub, cmd, iovbuf, sizeof(iovbuf), set,
				     ifidx);
		if (ret) {
...
static void dhd_inet6_work_handler(void *dhd_info, void *event_data, u8 event)
{
	struct ipv6_work_info_t *ndo_work =
	    (struct ipv6_work_info_t *) event_data;
	...
	case NETDEV_UP:
		...
		ret =
		    dhd_ndo_add_ip(pub, &ndo_work->ipv6_addr[0],
				   ndo_work->if_idx);
		if (ret < 0) {
				       ...
				       unsigned long event, void *ptr)
{
	...
	ndo_info = (struct ipv6_work_info_t *)
	    kzalloc(sizeof(struct ipv6_work_info_t), GFP_ATOMIC);
	if (!ndo_info) {
...
void dhd_detach(dhd_pub_t *dhdp)
{
...
#ifdef PROP_TXSTATUS
#ifdef WLFC_STATE_PREALLOC
	MFREE(dhd->pub.osh, dhd->pub.wlfc_state,
	...
	dhd->new_freq = NULL;
	cpufreq_unregister_notifier(&dhd->freq_trans,
				    CPUFREQ_TRANSITION_NOTIFIER);
#endif
	...
	if (dhdp) {
		...
		for (i = 0; i < ARRAYSIZE(dhdp->reorder_bufs); i++) {
			if (dhdp->reorder_bufs[i]) {
				...
				buf_size +=
				    ((ptr->max_idx + 1) * sizeof(void *));
				DHD_REORDER(("free flow id buf %d, maxidx is %d, buf_size %d\n", i, ptr->max_idx, buf_size));

				MFREE(dhdp->osh, dhdp->reorder_bufs[i],
				      buf_size);
		...
		if (dhd &&
		    dhd != (dhd_info_t *) dhd_os_prealloc(dhdp,
...
static void __exit dhd_module_exit(void)
{
	dhd_module_cleanup();
...
static int __init dhd_module_init(void)
{
	...
	if (firmware_path[0] != '\0') {
		strlcpy(fw_bak_path, firmware_path, MOD_PARAM_PATHLEN);
		fw_bak_path[MOD_PARAM_PATHLEN - 1] = '\0';
	}

	if (nvram_path[0] != '\0') {
		strlcpy(nv_bak_path, nvram_path, MOD_PARAM_PATHLEN);
	...
	do {
		...
		if (!err) {
			...
			strlcpy(firmware_path, fw_bak_path, MOD_PARAM_PATHLEN);
			firmware_path[MOD_PARAM_PATHLEN - 1] = '\0';
			strlcpy(nvram_path, nv_bak_path, MOD_PARAM_PATHLEN);
...
static int
dhd_reboot_callback(struct notifier_block *this, unsigned long code,
		    void *unused)
{
	DHD_TRACE(("%s: code = %ld\n", __FUNCTION__, code));
	if (code == SYS_RESTART) {
...
int dhd_os_d3ack_wait(dhd_pub_t *pub, uint *condition, bool *pending)
{
	dhd_info_t *dhd = (dhd_info_t *) (pub->info);
	int timeout;
...
void *dhd_os_open_image(char *filename)
{
	 ...
	 */
	if (IS_ERR(fp)) {
		DHD_ERROR(("%s: filename:%s, fp error:%d\n", __FUNCTION__, filename, PTR_ERR(fp)));
		fp = NULL;
	}
...
int dhd_os_get_image_block_seek(char *buf, int len, void *image, int seek)
{
...
#ifdef DHDTCPACK_SUPPRESS
void dhd_os_tcpacklock(dhd_pub_t *pub)
{
	dhd_info_t *dhd;
...
void dhd_os_tcpackunlock(dhd_pub_t *pub)
{
	dhd_info_t *dhd;
		       ...
		       bool kmalloc_if_fail)
{
	...
	buf =
	    (uint8 *) wifi_platform_prealloc(dhdpub->info->adapter, section,
					     size);
...
void dhd_sendup_log(dhd_pub_t *dhdp, void *data, int data_len)
{
	...
	if (p) {
		ASSERT(ISALIGNED
		       ((uintptr) PKTDATA(dhdp->osh, p), sizeof(uint32)));
...
int
dhd_dev_pno_set_for_ssid(struct net_device *dev, wlc_ssid_ext_t *ssids_local,
...
int
dhd_dev_pno_set_for_hotlist(struct net_device *dev,
			      ...
			      wlc_roam_exp_params_t *roam_param)
{
	...
	DHD_ERROR(("a_band_boost_factor %d a_band_penalty_factor %d cur_bssid_boost %d\n", roam_param->a_band_boost_factor, roam_param->a_band_penalty_factor, roam_param->cur_bssid_boost));
	...
	err =
	    dhd_iovar(&(dhd->pub), 0, "roam_exp_params", (char *) &roam_exp_cfg,
				     ...
				     uint32 flush)
{
	...
	bssid_pref->flags = (flush
			     && !bssid_pref->count) ? ROAM_EXP_CLEAR_BSSID_PREF
	    : 0;
	len = sizeof(wl_bssid_pref_cfg_t);
	len += (bssid_pref->count - 1) * sizeof(wl_bssid_pref_list_t);
	err =
	    dhd_iovar(&(dhd->pub), 0, "roam_exp_bssid_pref",
		      (char *) bssid_pref, len, 1);
	if (err != BCME_OK) {
				...
				uint32 len, uint32 flush)
{
	...
	if (blacklist) {
		err =
		    dhd_wl_ioctl_cmd(&(dhd->pub), WLC_SET_MACLIST,
				     (char *) blacklist, len, TRUE, 0);
		if (err != BCME_OK) {
	...
	macmode = (flush
		   && !blacklist) ? WLC_MACMODE_DISABLED : WLC_MACMODE_DENY;
	err =
	    dhd_wl_ioctl_cmd(&(dhd->pub), WLC_SET_MACMODE, (char *) &macmode,
			       ...
			       uint32 flush)
{
	...
	err =
	    dhd_iovar(&(dhd->pub), 0, "roam_exp_ssid_whitelist",
		      (char *) ssid_whitelist, len, 1);
	if (err != BCME_OK) {
		DHD_ERROR(("%s : Failed to execute roam_exp_bssid_pref %d\n",
			   __FUNCTION__, err));
	}
				   ...
				   int *send_evt_bytes)
{
	...
	return (dhd_pno_process_anqpo_result
		(&dhd->pub, data, event, send_evt_bytes));
}
#endif				/* GSCAN_SUPPORT */
...
int dhd_dev_cfg_rand_mac_oui(struct net_device *dev, uint8 *oui)
{
	...
	if (ETHER_ISMULTI(oui)) {
		...
		DHD_ERROR(("Random MAC OUI to be used - %02x:%02x:%02x\n",
			   rand_mac_oui[0], rand_mac_oui[1], rand_mac_oui[2]));
	}
	return BCME_OK;
}
...
int dhd_set_rand_mac_oui(dhd_pub_t *dhd)
{
	...
	if (ETHER_ISNULLADDR(&cfg.macaddr))
		...
		cfg.flags =
		    (WL_PFN_MAC_OUI_ONLY_MASK | WL_PFN_SET_MAC_UNASSOC_MASK);
...
int
dhd_dev_rtt_cancel_cfg(struct net_device *dev, struct ether_addr *mac_list,
		       int mac_cnt)
{
	dhd_info_t *dhd = *(dhd_info_t **) netdev_priv(dev);
...
int
dhd_dev_rtt_unregister_noti_callback(struct net_device *dev,
				     dhd_rtt_compl_noti_fn noti_fn)
{
	dhd_info_t *dhd = *(dhd_info_t **) netdev_priv(dev);
...
int
dhd_dev_start_mkeep_alive(dhd_pub_t *dhd_pub, u8 mkeep_alive_id, u8 *ip_pkt,
			  u16 ip_pkt_len, u8 *src_mac, u8 *dst_mac,
			  u32 period_msec)
{
	...
	pbuf = kzalloc(TEMP_BUF_SIZE, GFP_KERNEL);
	if (pbuf == NULL) {
	...
	pmac_frame = kzalloc(FRAME_SIZE, GFP_KERNEL);
	if (pmac_frame == NULL) {
	...
	res = dhd_wl_ioctl_cmd(dhd_pub, WLC_GET_VAR, pbuf, TEMP_BUF_SIZE,
						FALSE, 0);
	if (res < 0) {
...
int dhd_dev_stop_mkeep_alive(dhd_pub_t *dhd_pub, u8 mkeep_alive_id)
{
	char *pbuf;
	const char *str;
	...
	bcm_mkiovar("mkeep_alive", &mkeep_alive_id, sizeof(mkeep_alive_id),
		    pbuf, TEMP_BUF_SIZE);
	res =
			 dhd_wl_ioctl_cmd(dhd_pub, WLC_GET_VAR, pbuf, TEMP_BUF_SIZE, FALSE,
					  0);
	if (res < 0) {
		DHD_ERROR(("%s: Get mkeep_alive failed (error=%d)\n",
			   __FUNCTION__, res));
	...
	if (dtoh32(mkeep_alive_pktp->period_msec != 0)) {
		...
		mkeep_alive_pktp =
		    (wl_mkeep_alive_pkt_t *) (pbuf + str_len + 1);
		...
		res =
		    dhd_wl_ioctl_cmd(dhd_pub, WLC_SET_VAR, pbuf, buf_len, TRUE,
				     0);
	} else {
...
#if defined(SOFTAP)
	if (strnstr(fw, "apsta", PATH_MAX) != NULL) {
...
static ssize_t
dhd_debugfs_write(struct file *file, const char __user *ubuf, size_t count,
		  loff_t *ppos)
{
	loff_t pos = *ppos;
	size_t ret;
...
static void dhd_dbg_create(void)
{
	if (g_dbgfs.debugfs_dir) {
		g_dbgfs.debugfs_mem =
		    debugfs_create_file("mem", 0644, g_dbgfs.debugfs_dir, NULL,
					&dhd_dbg_state_ops);
	}
}
...
uint32 dhd_get_htsf(dhd_info_t *dhd, int ifidx)
{
	...
	if (dhd->htsf.coef) {
		...
		delta_us =
		    (baseval -
		     (((baseval - baseval2) * dhd->htsf.coefdec2)) / 10);
...
static void dhd_dump_latency(void)
{
	...
	printf("%08X %08X %08X %08X \t%d %d %d   %d\n", maxdelayts.t1,
	       maxdelayts.t2, maxdelayts.t3, maxdelayts.t4,
	       maxdelayts.t2 - maxdelayts.t1, maxdelayts.t3 - maxdelayts.t2,
	       maxdelayts.t4 - maxdelayts.t3, maxdelayts.t4 - maxdelayts.t1);
}
...
static int dhd_ioctl_htsf_get(dhd_info_t *dhd, int ifidx)
{
	...
	printf(" TSF_h=%04X lo=%08X Calc:htsf=%08X, coef=%d.%d%d delta=%d ",
	       tsf_buf.high, tsf_buf.low, s2, dhd->htsf.coef,
	       dhd->htsf.coefdec1, dhd->htsf.coefdec2, s2 - tsf_buf.low);
...
void htsf_update(dhd_info_t *dhd, void *data)
{
	static ulong cur_cycle, prev_cycle;
	...
	if (hfactor) {
		htsf =
		    ((cyc_delta * 10) / (hfactor * 10 + dec1)) + prev_tsf.low;
...
static void dhd_mem_dump_to_file(void *handle, void *event_info, u8 event)
{
	dhd_info_t *dhd = handle;
		       ...
		       uint32 size)
{
	...
	if (dhd_ver) {
		...
		strlcpy(*buf, strnstr(info_string, "Firmware: ", MOD_PARAM_INFOLEN), size);
	}
	return ret;
}
...
dhd_get_pkt_ether_type(dhd_pub_t *pub, void *pktbuf,
		       uint8 **data_ptr, int *len_ptr, uint16 *et_ptr,
		       bool *snap_ptr)
{
		    ...
		    uint8 **data_ptr, int *len_ptr, uint8 *prot_ptr)
{
	...
	if ((iph_frag & IPV4_FRAG_MORE)
	    || (iph_frag & IPV4_FRAG_OFFSET_MASK) != 0) {
...
static
int dhd_convert_dhcp_broadcast_ack_to_unicast(dhd_pub_t *pub, void *pktbuf,
					      int ifidx)
{
	dhd_sta_t *stainfo;
...
int dhd_l2_filter_block_ping(dhd_pub_t *pub, void *pktbuf, int ifidx)
{
	...
	if (dhd_get_pkt_ip_type(pub, pktbuf, (uint8 **) &icmph, &udpl, &prot)
	    != 0)
		return BCME_ERROR;
