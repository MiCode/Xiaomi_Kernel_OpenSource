...
struct dhd_pno_gscan_params {
	...
	struct dhd_pno_gscan_channel_bucket
	 channel_bucket[GSCAN_MAX_CH_BUCKETS];
...
extern int dhd_pno_set_for_hotlist(dhd_pub_t *dhd,
				   wl_pfn_bssid_t *p_pfn_bssid,
				   struct dhd_pno_hotlist_params
				   *hotlist_params);
...
extern int dhd_pno_initiate_gscan_request(dhd_pub_t *dhd, bool run,
					  bool flush);
extern int dhd_pno_enable_full_scan_result(dhd_pub_t *dhd,
					   bool real_time_flag);
...
extern void *dhd_handle_hotlist_scan_evt(dhd_pub_t *dhd,
...
extern void *dhd_pno_process_anqpo_result(dhd_pub_t *dhd, const void *data,
					  uint32 event, int *size);
