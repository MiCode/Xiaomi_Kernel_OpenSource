...
int
dhd_wl_ioctl(dhd_pub_t *dhd_pub, int ifidx, wl_ioctl_t *ioc, void *buf,
	     int len)
{
	int ret = BCME_ERROR;
	    ...
	    int val_size)
{
	...
	case IOV_SVAL(IOV_TCPACK_SUPPRESS):{
			bcmerror =
			    dhd_tcpack_suppress_set(dhd_pub, (uint8) int_val);
			break;
		}
#endif				/* DHDTCPACK_SUPPRESS */
#ifdef DHD_WMF
	case IOV_GVAL(IOV_WMF_BSS_ENAB):{
			...
			if (dhd_iovar_parse_bssidx
			    (dhd_pub, (char *) name, &bssidx,
			     &val) != BCME_OK) {
	...
	case IOV_SVAL(IOV_WMF_BSS_ENAB):{
			...
			if (int_val) {
				/* Enable WMF */
				if (dhd_wmf_instance_add(dhd_pub, bssidx) !=
				    BCME_OK) {
...
#ifdef SHOW_EVENTS
static void
wl_show_host_event(dhd_pub_t *dhd_pub, wl_event_msg_t *event,
		   void *event_data, void *raw_event_ptr, char *eventmask)
{
	...
	case WLC_E_TRACE:
		{
			dhd_dbg_trace_evnt_handler(dhd_pub, event_data,
						   raw_event_ptr, datalen);
		}
#endif				/* SHOW_LOGTRACE */
			...
			bcm_dngl_event_msg_t *dngl_event, size_t pktlen)
{
	...
	default:
		DHD_ERROR(("%s:Unknown DNGL Event Type:%d", __FUNCTION__,
			   type));
		if (p && DHD_EVENT_ON()) {
		  ...
		  wl_event_msg_t *event, void **data_ptr, void *raw_event)
{
	...
	case WLC_E_IF:
		{
...
#ifdef PROP_TXSTATUS
			{
				...
				WLFC_DBGMESG(("WLC_E_IF: idx:%d, action:%s, iftype:%s, " "[%02x:%02x:%02x:%02x:%02x:%02x]\n", ifevent->ifidx, ((ifevent->opcode == WLC_E_IF_ADD) ? "ADD" : "DEL"), ((ifevent->role == 0) ? "STA" : "AP "), ea[0], ea[1], ea[2], ea[3], ea[4], ea[5]));
				(void) ea;
			...
			if (ifevent->ifidx > 0 && ifevent->ifidx < DHD_MAX_IFS) {
				if (ifevent->opcode == WLC_E_IF_ADD) {
					...
					dhd_event_ifdel(dhd_pub->info, ifevent,
							event->ifname,
							event->addr.octet);
			...
			*ifidx = hostidx;
			...
			dhd_event(dhd_pub->info, (char *) pvt_data, evlen,
				  *ifidx);
			break;
		}
	...
	case WLC_E_DISASSOC_IND:
		...
		if (type != WLC_E_LINK) {
			...
			uint8 role =
			    dhd_flow_rings_ifindex2role(dhd_pub, ifindex);
...
void dhd_arp_offload_set(dhd_pub_t *dhd, int arp_mode)
{
	...
	iovar_len =
	    bcm_mkiovar("arp_ol", (char *) &arp_mode, 4, iovbuf,
			sizeof(iovbuf));
	if (!iovar_len) {
...
void dhd_arp_offload_enable(dhd_pub_t *dhd, int arp_enable)
{
	...
	iovar_len =
	    bcm_mkiovar("arpoe", (char *) &arp_enable, 4, iovbuf,
			sizeof(iovbuf));
	if (!iovar_len) {
		DHD_ERROR(("%s: Insufficient iovar buffer size %zu \n",
			   __FUNCTION__, sizeof(iovbuf)));
...
void dhd_aoe_arp_clr(dhd_pub_t *dhd, int idx)
{
	...
	ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, iov_len, TRUE, idx);
	if (ret < 0)
...
void dhd_aoe_hostip_clr(dhd_pub_t *dhd, int idx)
{
	...
	iov_len =
	    bcm_mkiovar("arp_hostip_clear", 0, 0, iovbuf, sizeof(iovbuf));
	if (!iov_len) {
		DHD_ERROR(("%s: Insufficient iovar buffer size %zu \n",
			   __FUNCTION__, sizeof(iovbuf)));
		return;
...
int dhd_ndo_enable(dhd_pub_t *dhd, int ndo_enable)
{
	...
	iov_len =
	    bcm_mkiovar("ndoe", (char *) &ndo_enable, 4, iovbuf,
			sizeof(iovbuf));
	if (!iov_len) {
		DHD_ERROR(("%s: Insufficient iovar buffer size %zu \n",
			   __FUNCTION__, sizeof(iovbuf)));
...
bool dhd_is_associated(dhd_pub_t *dhd, void *bss_buf, int *retval)
{
	...
	ret =
	    dhd_wl_ioctl_cmd(dhd, WLC_GET_BSSID, (char *) &bssid,
			     ETHER_ADDR_LEN, FALSE, 0);
...
int
dhd_get_suspend_bcn_li_dtim(dhd_pub_t *dhd, int *dtim_period,
			    int *bcn_interval)
{
	...
	ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_BCNPRD,
						bcn_interval, sizeof(*bcn_interval), FALSE,
						0);
	if (ret < 0) {
	...
	ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_DTIMPRD,
	...
	if (((*dtim_period) * (*bcn_interval) * bcn_li_dtim) >
		...
		bcn_li_dtim =
		    (allowed_skip_dtim_cnt !=
		     0) ? allowed_skip_dtim_cnt : NO_DTIM_SKIP;
	}
	...
	DHD_ERROR(("%s beacon=%d bcn_li_dtim=%d DTIM=%d Listen=%d\n",
		   __FUNCTION__, *bcn_interval, bcn_li_dtim, *dtim_period,
 ...
 */
int
wl_iw_parse_ssid_list_tlv(char **list_str, wlc_ssid_ext_t *ssid, int max,
			  int *bytes_left)
{
	char *str;
	int idx = 0;
...
int wl_iw_parse_ssid_list(char **list_str, wlc_ssid_t *ssid, int idx, int max)
{
	...
	for (str = *list_str; str != NULL; str = ptr) {
		...
		if (idx < max) {
			...
			strlcpy((char *) ssid[idx].SSID, str,
				sizeof(ssid[idx].SSID));
