				     ...
				     int total_len)
{
	...
	if (ret_now != suspend_flag) {
		ret = net_os_set_suspend(dev, ret_now, 1);
		if (!ret)
				      ...
				      int total_len)
{
	...
	if (suspend_flag != 0)
		suspend_flag = 1;
	ret = net_os_set_suspend(dev, suspend_flag, 0);
	if (!ret)
...
wls_parse_batching_cmd(struct net_device *dev, char *command, int total_len)
{
	...
	if (!strncmp(pos, PNO_BATCHING_SET, strlen(PNO_BATCHING_SET))) {
		...
		while ((token = strsep(&pos, PNO_PARAMS_DELIMETER)) != NULL) {
			...
			if (*token == '\0')
				continue;
			delim = strnchr(token, total_len, PNO_PARAM_VALUE_DELLIMETER);
			if (delim != NULL)
				*delim = ' ';
			...
			if (!strncmp
				...
				if (tokens != 1) {
					err = BCME_ERROR;
					DHD_ERROR(("%s : invalid format for channel" " <> params\n", __FUNCTION__));
					goto exit;
				}
			    ...
			    if (!strncmp
				(param, PNO_PARAM_RTT, strlen(PNO_PARAM_RTT))) {
				batch_params.rtt =
				    simple_strtol(value, NULL, 0);
				DHD_PNO(("rtt : %d\n", batch_params.rtt));
			} else {
				    ...
				    int total_len)
{
	...
	if ((cmd_tlv_temp->prefix == PNO_TLV_PREFIX) &&
		...
		nssid = wl_iw_parse_ssid_list_tlv(&str_ptr, ssids_local,
									   MAX_PFN_LIST_COUNT,
									   &tlv_size_left);
		if (nssid <= 0) {
			   ...
			   struct maclist *maclist)
{
	...
	if (macmode != MACLIST_MODE_DISABLED) {
		...
		ret = wldev_ioctl(dev, WLC_SET_MACLIST, maclist,
				       sizeof(int) +
		...
		ret = wldev_ioctl(dev, WLC_GET_ASSOCLIST, assoc_maclist,
				       sizeof(mac_buf), false);
		if (ret != 0) {
...
wl_android_set_mac_address_filter(struct net_device *dev, const char *str)
{
	...
	for (i = 0; i < list->count; i++) {
		strlcpy(eabuf, strsep((char **) &str, " "),
			ETHER_ADDR_STR_LEN);
		ret = bcm_ether_atoe(eabuf, &list->ea[i]);
		if (!ret) {
	...
	/* set the list */
	ret = wl_android_set_ap_mac_list(dev, macmode, list);
	if (ret != 0)
				       ...
				       int total_len)
{
	...
	strlcpy(vndr_ie->cmd, "add", VNDR_IE_CMD_LEN);
	vndr_ie->cmd[VNDR_IE_CMD_LEN - 1] = '\0';

	/* Set the IE count - the buffer contains only 1 IE */
	iecount = htod32(1);
	memcpy((void *) &vndr_ie->vndr_ie_buffer.iecount, &iecount,
	       sizeof(s32));
...
wl_android_set_roampref(struct net_device *dev, char *command, int total_len)
{
	...
	*pref++ = 0;
	...
	error =
	    wldev_iovar_setbuf(dev, "join_pref", buf, total_bytes, smbuf,
			       sizeof(smbuf), NULL);
	if (error) {
...
wl_android_set_miracast(struct net_device *dev, char *command, int total_len)
{
	...
	case MIRACAST_MODE_SOURCE:
		...
		ret =
		    wldev_ioctl(dev, WLC_GET_BCNPRD, &val, sizeof(int), false);
		if (!ret && val > 100) {
			config.param = 0;
...
int wl_keep_alive_set(struct net_device *dev, char *extra, int total_len)
{
	...
	res = wldev_ioctl(dev, WLC_SET_VAR, buf, buf_len, TRUE);
	if (res < 0) {
...
#define PRIVATE_COMMAND_MAX_LEN	8192
	...
	/* CUSTOMER_SET_COUNTRY feature is define for only GGSM model */
		...
		bytes_written =
		    wl_keep_alive_set(net, command + skip,
