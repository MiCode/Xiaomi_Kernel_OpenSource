...
#ifdef CONFIG_PARTIALRESUME
		wifi_process_partial_resume(pch->adapter,
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 0))
	error = pci_module_init(&dhdpcie_driver);
	if (!error)
		return 0;
...
int dhdpcie_detach(dhdpcie_info_t *pch)
{
	...
	if (pch) {
		...
		if (!dhd_download_fw_on_driverload)
			pci_load_and_free_saved_state(pch->dev,
						      &pch->default_state);
...
int dhdpcie_get_resource(dhdpcie_info_t *dhdpcie_info)
{
	...
	do {
		...
		dhdpcie_info->tcm =
		    (volatile char *) REG_MAP(bar1_addr, DONGLE_TCM_MAP_SIZE);
		...
		DHD_TRACE(("%s:Phys addr : reg space = %p base addr 0x"
			   PRINTF_RESOURCE " \n", __FUNCTION__,
			   dhdpcie_info->regs, bar0_addr));
		DHD_TRACE(("%s:Phys addr : tcm_space = %p base addr 0x"
			   PRINTF_RESOURCE " \n", __FUNCTION__,
			   dhdpcie_info->tcm, bar1_addr));
...
int dhdpcie_init(struct pci_dev *pdev)
{
	...
	do {
		...
		adapter =
		    dhd_wifi_platform_get_adapter(PCI_BUS, pdev->bus->number,
						  PCI_SLOT(pdev->devfn));
		if (adapter != NULL)
		...
		/* Bus initialization */
		bus =
		    dhdpcie_bus_attach(osh, dhdpcie_info->regs,
				       dhdpcie_info->tcm);
		if (!bus) {
		...
		if (bus->intr) {
			...
			bus->pollrate = 1;
			DHD_INFO(("%s: PCIe interrupt function is NOT registered " "due to polling mode\n", __FUNCTION__));
		}
