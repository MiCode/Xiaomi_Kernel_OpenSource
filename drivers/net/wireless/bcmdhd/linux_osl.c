...
osl_t *osl_attach(void *pdev, uint bustype, bool pkttag, void **osl_cmn)
{
#else
osl_t *osl_attach(void *pdev, uint bustype, bool pkttag)
{
	...
	if (osl_cmn == NULL || *osl_cmn == NULL) {
		osh->cmn = kmalloc(sizeof(osl_cmn_t), flags);
		if (!osh->cmn) {
			kfree(osh);
			return NULL;
		}
...
int osl_static_mem_init(osl_t *osh, void *adapter)
{
	...
	if (!bcm_static_buf && adapter) {
		bcm_static_buf =
		     (bcm_static_buf_t *) wifi_platform_prealloc(adapter, 3,
								 STATIC_BUF_SIZE
								 +
...
static struct sk_buff *osl_alloc_skb(osl_t *osh, unsigned int len)
{
	...
	gfp_t flags = (in_interrupt() || irqs_disabled()) ? GFP_ATOMIC : GFP_KERNEL;
 ...
 */
void BCMFASTPATH osl_pkt_tofwder(osl_t *osh, void *skbs, int skb_cnt)
{
 ...
 */
void BCMFASTPATH osl_pkt_frmfwder(osl_t *osh, void *skbs, int skb_cnt)
{
...
void *BCMFASTPATH osl_pkt_frmnative(osl_t *osh, void *pkt)
{
	...
	for (nskb = (struct sk_buff *) pkt; nskb; nskb = nskb->next) {
		atomic_add(PKTISCHAINED(nskb) ? PKTCCNT(nskb) : 1,
			   &osh->cmn->pktalloced);
...
void *osl_pktget_static(osl_t *osh, uint len)
{
	...
	if (len <= DHD_SKB_1PAGE_BUFSIZE) {
		...
		if (i != STATIC_PKT_1PAGE_NUM) {
			...
			spin_unlock_irqrestore(&bcm_static_skb->osl_pkt_lock,
					       flags);
#else
#ifdef NET_SKBUFF_DATA_USES_OFFSET
			skb_set_tail_pointer(skb, len);
...
void osl_pktclear_static(osl_t *osh)
{
	int i;
#if defined(BCMPCIE)
	unsigned long flags;
...
osl_dma_map(osl_t *osh, void *va, uint size, int direction, void *p,
	...
	if (dmah != NULL) {
		...
		for (skb = (struct sk_buff *) p; skb != NULL;
			...
			if (skb_is_nonlinear(skb)) {
				nsegs =
				    skb_to_sgvec(skb, sg, 0, PKTLEN(osh, skb));
				ASSERT((nsegs > 0)
				       && (totsegs + nsegs <= MAX_DMA_SEGS));
...
{
	if (size > 0)
		dma_sync_single_for_device(OSH_NULL, virt_to_dma(OSH_NULL, va),
					   size, DMA_TX);
}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 36)
	p = pskb_copy((struct sk_buff *) skb, GFP_ATOMIC);
...
void osl_flag_set(osl_t *osh, uint32 mask)
{
	osh->flags |= mask;
}

bool osl_is_flag_set(osl_t *osh, uint32 mask)
{
