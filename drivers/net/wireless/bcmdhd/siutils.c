		...
		uint bustype, void *sdh, char **vars, uint *varsz)
{
	...
	/* alloc si_info_t */
	sii = MALLOCZ(osh, sizeof(si_info_t));
	if (sii == NULL) {
		 ...
		 uint32 savewin, uint *origidx, void *regs)
{
	...
	if (sii->pub.cccaps & CC_CAP_PMU) {
		if (AOB_ENAB(&sii->pub)) {
			...
			sii->pub.pmucaps =
			    R_REG(sii->osh, &pmu->pmucapabilities);
			si_setcoreidx(&sii->pub, SI_CC_IDX);
		} else
			sii->pub.pmucaps =
			    R_REG(sii->osh, &cc->pmucapabilities);
	...
	for (i = 0; i < sii->numcores; i++) {
		...
		SI_VMSG(("CORE[%d]: id 0x%x rev %d base 0x%x regs 0x%p\n",
			 i, cid, crev, cores_info->coresba[i],
			 cores_info->regs[i]));
			      ...
			      uint *varsz)
{
	...
	if ((sih->chip == BCM4358_CHIP_ID) ||
	    ...
	    (sih->chip == BCM43562_CHIP_ID) || (sih->chip == BCM4358_CHIP_ID)) {
		si_chipid_fixup(sih);
	}
	...
	if (CHIP_HOSTIF_PCIE(&(sii->pub))) {
		 ...
		 */
		if ((sflags & (SISF_SDRENABLE | SISF_TCMPROT)) ==
		    SISF_SDRENABLE) {
			disable_arm_irq();
			while (1) {
				hnd_cpu_wait(sih);
...
void *si_d11_switch_addrbase(si_t *sih, uint coreunit)
{
...
uint si_pmu_corereg(si_t *sih, uint32 idx, uint regoff, uint mask, uint val)
{
	...
	if (mask != 0 && sih->pmurev >= 22 &&
		...
		pmustatus_offset =
		    AOB_ENAB(sih) ? OFFSETOF(pmuregs_t,
					     pmustatus) : OFFSETOF(chipcregs_t,
								   pmustatus);
 ...
 * NULL.
 */
uint32 *si_corereg_addr(si_t *sih, uint coreidx, uint regoff)
{
	if (CHIPTYPE(sih->socitype) == SOCI_SB)
...
static uint si_slowclk_src(si_info_t *sii)
{
	...
	if (sii->pub.ccrev < 6) {
		if ((BUSTYPE(sii->pub.bustype) == PCI_BUS) &&
		    (OSL_PCI_READ_CONFIG
		     (sii->osh, PCI_GPIO_OUT,
		      sizeof(uint32)) & PCI_CFG_GPIO_SCS))
			return SCC_SS_PCI;
		else
			return SCC_SS_XTAL;
...
static uint32 si_pcie_survive_perst(si_t *sih, uint32 mask, uint32 val)
{
	si_info_t *sii;
...
static void si_watchdog_reset(si_t *sih)
{
	si_info_t *sii = SI_INFO(sih);
	uint32 i;
...
void
si_survive_perst_war(si_t *sih, bool reset, uint32 sperst_mask,
		     uint32 sperst_val)
{
#ifdef SURVIVE_PERST_ENAB
	...
	if (reset) {
		...
		if (bar0win_after != bar0win) {
			...
			OSL_PCI_WRITE_CONFIG(sii->osh, PCI_BAR0_WIN,
					     sizeof(uint32), bar0win);
		}
	}
	if (sperst_mask) {
