...
static int
dhd_rtt_convert_results_to_host(rtt_report_t *rtt_report, uint8 *p_data,
				uint16 tlvid, uint16 len);

static wifi_rate_t dhd_rtt_convert_rate_to_host(uint32 ratespec);
...
uint rate_mcs2rate(uint mcs, uint nss, uint bw, int sgi)
{
	...
	if (mcs == 32) {
		...
		rate =
		    (rate * wl_mcs_info[mcs].coding_q) /
		    wl_mcs_info[mcs].coding_d;
...
int rate_rspec2rate(uint32 rspec)
{
	int rate = -1;
	...
	if (RSPEC_ISLEGACY(rspec)) {
		...
		if (mcs == 32) {
			rate =
			    rate_mcs2rate(mcs, 1, WL_RSPEC_BW_40MHZ,
					  RSPEC_ISSGI(rspec));
		} else {
			uint nss = 1 + (mcs / 8);
			mcs = mcs % 8;
			rate =
			    rate_mcs2rate(mcs, nss, RSPEC_BW(rspec),
					  RSPEC_ISSGI(rspec));
		}
...
static rtt_reason_t
ftm_get_statusmap_info(wl_proxd_status_t id,
		       const ftm_status_map_host_entry_t *p_table,
		       uint32 num_entries)
{
	int i;
...
*/
static const ftm_strmap_entry_t *ftm_get_strmap_info(int32 id,
...
*/
static const char *ftm_cmdid_to_str(uint16 cmdid)
{
...
static const char *ftm_tmu_value_to_logstr(wl_proxd_tmu_t tmu)
{
	return ftm_map_id_to_str((int32) tmu,
				 &ftm_tmu_value_loginfo[0],
				 ARRAYSIZE(ftm_tmu_value_loginfo));
}

static const ftm_strmap_entry_t *ftm_get_event_type_loginfo(wl_proxd_event_type_t event_type)
{
...
static const char *ftm_session_state_value_to_logstr(wl_proxd_session_state_t
						     state)
{
	return ftm_map_id_to_str((int32) state,
				 &ftm_session_state_value_loginfo[0],
...
*/
static int
rtt_do_get_ioctl(dhd_pub_t *dhd, wl_proxd_iov_t *p_proxd_iov,
		 uint16 proxd_iovsize, ftm_subcmd_info_t *p_subcmd_info)
{
	...
	if (tlvs_len > 0 && p_subcmd_info->handler) {
		...
		status =
		    bcm_unpack_xtlv_buf(p_proxd_iov, (uint8 *) p_iovresp->tlvs,
					tlvs_len, BCM_XTLV_OPTION_ALIGN32,
					p_subcmd_info->handler);
	}
exit:
	return status;
}

static wl_proxd_iov_t *rtt_alloc_getset_buf(wl_proxd_method_t method,
					    wl_proxd_session_id_t session_id,
					    wl_proxd_cmd_t cmdid,
					    uint16 tlvs_bufsize,
					    uint16 *p_out_bufsize)
{
			   ...
			   wl_proxd_session_id_t session_id)
{
	...
	p_proxd_iov =
	    rtt_alloc_getset_buf(method, session_id, p_subcmd_info->cmdid, 0,
				 &proxd_iovsize);
	...
	status =
	    rtt_do_get_ioctl(dhd, p_proxd_iov, proxd_iovsize, p_subcmd_info);
...
*/
static int
dhd_rtt_common_set_handler(dhd_pub_t *dhd,
			   ...
			   wl_proxd_session_id_t session_id)
{
	...
	proxd_iovsize = 0;
	p_proxd_iov = rtt_alloc_getset_buf(method, session_id, p_subcmd_info->cmdid, 0, &proxd_iovsize);	/* no TLV */
	if (p_proxd_iov == NULL)
		return BCME_NOMEM;

	/* no TLV to pack, simply issue a set-proxd iovar */
	ret =
	    dhd_iovar(dhd, 0, "proxd", (void *) p_proxd_iov, proxd_iovsize, 1);
#ifdef RTT_DEBUG
	if (ret != BCME_OK) {
...
static int
rtt_handle_config_options(wl_proxd_session_id_t session_id,
...
static int
rtt_handle_config_general(wl_proxd_session_id_t session_id,
			  wl_proxd_tlv_t **p_tlv, uint16 *p_buf_space_left,
		...
		if (p_config_param_info != NULL) {
			...
			if (ret != BCME_OK) {
				DHD_ERROR(("%s bad TLV ID : %d\n",
					   __FUNCTION__,
					   p_config_param_info->tlvid));
				break;
			}

			ret =
			    bcm_pack_xtlv_entry((uint8 **) p_tlv,
						p_buf_space_left,
						p_config_param_info->tlvid,
						src_data_size, p_src_data,
						BCM_XTLV_OPTION_ALIGN32);
			if (ret != BCME_OK) {
...
static int
dhd_rtt_start_session(dhd_pub_t *dhd, wl_proxd_session_id_t session_id,
		      bool start)
{
	ftm_subcmd_info_t subcmd_info;
...
dhd_rtt_ftm_config(dhd_pub_t *dhd, wl_proxd_session_id_t session_id,
		   ftm_config_category_t catagory, void *ftm_configs,
		   int ftm_cfg_cnt)
{
	ftm_subcmd_info_t subcmd_info;
	wl_proxd_tlv_t *p_tlv;
	...
	p_proxd_iov =
	    rtt_alloc_getset_buf(WL_PROXD_METHOD_FTM, session_id,
				 subcmd_info.cmdid, FTM_IOC_BUFSZ,
				 &proxd_iovsize);
	...
	if (catagory == FTM_CONFIG_CAT_OPTIONS) {
		ret =
		    rtt_handle_config_options(session_id, &p_tlv,
					      &buf_space_left,
					      (ftm_config_options_info_t *)
					      ftm_configs, ftm_cfg_cnt);
	} else if (catagory == FTM_CONFIG_CAT_GENERAL) {
		ret =
		    rtt_handle_config_general(session_id, &p_tlv,
					      &buf_space_left,
					      (ftm_config_param_info_t *)
					      ftm_configs, ftm_cfg_cnt);
	}
...
chanspec_t dhd_rtt_convert_to_chspec(wifi_channel_info_t channel)
{
	...
	case WIFI_CHAN_WIDTH_80:
		...
		break;
	default:
		DHD_ERROR(("doesn't support this bandwith : %d",
			   channel.width));
		bw = -1;
		break;
	}
...
int dhd_rtt_stop(dhd_pub_t *dhd, struct ether_addr *mac_list, int mac_cnt)
{
	...
	for (i = 0; i < mac_cnt; i++) {
		for (j = 0; j < rtt_status->rtt_config.rtt_target_cnt; j++) {
			if (!bcmp
			    (&mac_list[i],
			     &rtt_status->rtt_config.target_info[j].addr,
			     ETHER_ADDR_LEN)) {
				rtt_status->rtt_config.target_info[j].disable =
				    TRUE;
			}
		}
	}
...
static int dhd_rtt_start(dhd_pub_t *dhd)
{
	...
	if (rtt_target->burst_period) {
		ftm_params[ftm_param_cnt].data_intvl.intvl = htol32(rtt_target->burst_period);	/* ms */
		ftm_params[ftm_param_cnt].data_intvl.tmu =
...
int
dhd_rtt_register_noti_callback(dhd_pub_t *dhd, void *ctx,
			       dhd_rtt_compl_noti_fn noti_fn)
{
	int err = BCME_OK;
...
static wifi_rate_t dhd_rtt_convert_rate_to_host(uint32 rspec)
{
				...
				uint16 tlvid, uint16 len)
{
	...
	if (avg_dist == 0xffffffff) {	/* report 'failure' case */
		DHD_RTT((">\tavg_dist=-1m, burst_num=%d, valid_measure_cnt=%d\n", ltoh16_ua(&p_data_info->burst_num), p_data_info->num_valid_rtt));	/* in a session */
		avg_dist = FTM_INVALID;
	} else {
		DHD_RTT((">\tavg_dist=%d.%04dm, burst_num=%d, valid_measure_cnt=%d num_ftm=%d\n", avg_dist >> 8,	/* 1/256m units */
			 ((avg_dist & 0xff) * 625) >> 4, ltoh16_ua(&p_data_info->burst_num), p_data_info->num_valid_rtt, p_data_info->num_ftm));	/* in a session */
	}
	/* show 'avg_rtt' sample */
	p_sample = &p_data_info->avg_rtt;
	DHD_RTT((">\tavg_rtt sample: rssi=%d rtt=%d%s std_deviation =%d.%d ratespec=0x%08x\n", (int16) ltoh16_ua(&p_sample->rssi), ltoh32_ua(&p_sample->rtt.intvl), ftm_tmu_value_to_logstr(ltoh16_ua(&p_sample->rtt.tmu)), ltoh16_ua(&p_data_info->sd_rtt) / 10, ltoh16_ua(&p_data_info->sd_rtt) % 10, ltoh32_ua(&p_sample->ratespec)));
	...
	if (proxd_status == WL_PROXD_E_REMOTE_FAIL) {
		...
		DHD_RTT((">\tretry_after: %d%s\n",
			 ...
			 ftm_tmu_value_to_logstr(ltoh16_ua
						 (&p_data_info->u.retry_after.
						  tmu))));
	} else {
		/* burst duration */
		p_time.intvl = ltoh32_ua(&p_data_info->u.retry_after.intvl);
...
dhd_rtt_event_handler(dhd_pub_t *dhd, wl_event_msg_t *event, void *event_data)
{
	...
	DHD_RTT(("receive '%s' event: version=0x%x len=%d method=%d sid=%d tlvs_len=%d\n", p_loginfo->text, version, ltoh16(p_event->len), ltoh16(p_event->method), ltoh16(p_event->sid), tlvs_len));
	rtt_target_info =
	...
	case WL_PROXD_EVENT_BURST_END:
		...
		if (is_new) {
			...
			rtt_results_header =
			    kzalloc(sizeof(rtt_results_header_t), GFP_KERNEL);
			...
			list_add_tail(&rtt_results_header->list,
				      &rtt_status->rtt_results_cache);
		}
		if (tlvs_len > 0) {
			...
			ret =
			    bcm_unpack_xtlv_buf((void *) &(rtt_result->report),
						(uint8 *) &p_event->tlvs[0],
						tlvs_len,
						BCM_XTLV_OPTION_ALIGN32,
			...
			rtt_result->report.ftm_num =
			    rtt_target_info->num_frames_per_burst;
			...
			list_add_tail(&rtt_result->list,
				      &rtt_results_header->result_list);
	...
	case WL_PROXD_EVENT_SESSION_END:
		...
		if (idx < rtt_status->rtt_config.rtt_target_cnt) {
			...
			list_for_each_entry(iter, &rtt_status->noti_fn_list,
					    list) {
				iter->noti_fn(iter->ctx,
					      &rtt_status->rtt_results_cache);
			}
			...
			if (!list_empty(&rtt_status->rtt_results_cache)) {
				...
				list_for_each_entry_safe(entry, next,
					...
					list_del(&entry->list);
					/* Iterate rtt_result list */
					list_for_each_entry_safe(rtt_result,
								 next2,
								 &entry->
								 result_list,
								 list) {
...
int dhd_rtt_deinit(dhd_pub_t *dhd)
{
	...
	if (!list_empty(&rtt_status->noti_fn_list)) {
		list_for_each_entry_safe(iter, iter2, &rtt_status->noti_fn_list,
					 list) {
			list_del(&iter->list);
			kfree(iter);
		}
	...
	if (!list_empty(&rtt_status->rtt_results_cache)) {
		list_for_each_entry_safe(rtt_header, next,
					 &rtt_status->rtt_results_cache, list) {
			list_del(&rtt_header->list);
			list_for_each_entry_safe(rtt_result, next2,
						 &rtt_header->result_list,
						 list) {
				list_del(&rtt_result->list);
				kfree(rtt_result);
			}
