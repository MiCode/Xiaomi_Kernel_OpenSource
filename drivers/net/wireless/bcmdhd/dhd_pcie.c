...
const bcm_iovar_t dhdpcie_iovars[] = {
	...
	,
	{"bar0secwinreg", IOV_BAR0_SECWIN_REG, 0, IOVT_BUFFER,
	 2 * sizeof(int32)}
	,
...
void dhdpcie_bus_reg_unmap(osl_t *osh, ulong addr, int size)
{
 ...
 */
dhd_bus_t *dhdpcie_bus_attach(osl_t *osh, volatile char *regs,
			      volatile char *tcm)
{
	dhd_bus_t *bus;
...
static bool dhdpcie_dongle_attach(dhd_bus_t *bus)
{
	...
	bus->def_intmask =
	    PCIE_MB_D2H_MB_MASK | PCIE_MB_TOPCIE_FN0_0 | PCIE_MB_TOPCIE_FN0_1;
	...
	if (bus) {
		...
		if ((bus->sih->buscorerev == 2) || (bus->sih->buscorerev == 6)
			...
			si_corereg(bus->sih, bus->sih->buscoreidx,
				   PCIMailBoxMask, bus->def_intmask, 0);
		}
	}
	DHD_TRACE(("%s Exit\n", __FUNCTION__));
}
...
void dhdpcie_bus_release(dhd_bus_t *bus)
{
	...
	if (bus) {
		...
		if (bus->dhd) {
			...
			dhd_detach(bus->dhd);
			dhdpcie_bus_release_dongle(bus, osh, dongle_isolation,
						   TRUE);
			dhd_free(bus->dhd);
			bus->dhd = NULL;
		}
		...
		if (bus->regs) {
			dhdpcie_bus_reg_unmap(osh, (ulong) bus->regs,
					      DONGLE_REG_MAP_SIZE);
			bus->regs = NULL;
		}
		if (bus->tcm) {
			dhdpcie_bus_reg_unmap(osh, (ulong) bus->tcm,
					      DONGLE_TCM_MAP_SIZE);
			bus->tcm = NULL;
		}
...
void
dhdpcie_bus_release_dongle(dhd_bus_t *bus, osl_t *osh, bool dongle_isolation,
			   bool reset_flag)
{
...
/* 32 bit config write */
void
dhdpcie_bus_cfg_write_dword(dhd_bus_t *bus, uint32 addr, uint32 size,
			    uint32 data)
{
...
#if defined(CONFIG_ARCH_MSM) && defined(CONFIG_64BIT)
void dhdpcie_bus_cfg_set_bar1_win(dhd_bus_t *bus, uint32 data)
{
...
int dhdpcie_download_code_array(struct dhd_bus *bus)
{
	...
	while (downloded_len < dlarray_size) {
		...
		bcmerror =
		    dhdpcie_bus_membytes(bus, TRUE, offset, (uint8 *) memptr,
					 len);
		downloded_len += len;
		if (bcmerror) {
			DHD_ERROR(("%s: error %d on writing %d membytes at 0x%08x\n", __FUNCTION__, bcmerror, MEMBLOCK, offset));
...
static int dhdpcie_bus_readconsole(dhd_bus_t *bus)
{
	...
	rv = dhdpcie_bus_membytes(bus, FALSE, addr, (uint8 *) &c->log,
					  sizeof(c->log));
	if (rv < 0)
		return rv;
	...
	rv =
	     dhdpcie_bus_membytes(bus, FALSE, addr, c->buf, c->bufsize);
	if (rv < 0)
		return rv;
...
static int dhdpcie_checkdied(dhd_bus_t *bus, char *data, uint size)
{
	...
	bcm_bprintf(&strbuf,
		    ...
		    pciedev_shared->msgtrace_addr,
		    pciedev_shared->console_addr);
	...
	if ((bus->pcie_sh->flags & (PCIE_SHARED_ASSERT | PCIE_SHARED_TRAP)) ==
		...
		if (bus->pcie_sh->flags & PCIE_SHARED_TRAP) {
			...
			addr =
			    bus->pcie_sh->console_addr + OFFSETOF(hnd_cons_t,
								  log);
			rv = dhdpcie_bus_membytes(bus, FALSE, addr,
						  (uint8 *) &console_ptr,
						  sizeof(console_ptr));
			if (rv < 0)
				goto printbuf;
			...
			addr =
			    bus->pcie_sh->console_addr + OFFSETOF(hnd_cons_t,
								  log.idx);
			rv =
			     dhdpcie_bus_membytes(bus, FALSE, addr,
						  (uint8 *) &console_index,
			...
			if (console_size > CONSOLE_BUFFER_MAX ||
			    !console_buffer)
				goto printbuf;
			rv = dhdpcie_bus_membytes(bus, FALSE, console_ptr,
...
static int dhdpcie_mem_dump(dhd_bus_t *bus)
{
	...
	while (size) {
		...
		ret =
		     dhdpcie_bus_membytes(bus, FALSE, start, databuf,
					  read_size);
		if (ret) {
 ...
 */
static int
dhdpcie_bus_membytes(dhd_bus_t *bus, bool write, ulong address, uint8 *data,
		     uint size)
{
	int bcmerror = 0;
	uint dsize;
		...
		while (size) {
			if (size >= sizeof(uint64) && little_endian) {
				...
				if (is_64bit_unaligned) {
					...
					dhdpcie_bus_wtcm32(bus, address,
							   *((uint32 *) data));
					data += 4;
					size -= 4;
					address += 4;
...
#endif
					dhdpcie_bus_wtcm64(bus, address,
							   *((uint64 *) data));
			} else {
				dsize = sizeof(uint8);
			...
			if (size >= sizeof(uint64) && little_endian) {
				...
				if (is_64bit_unaligned) {
					DHD_INFO(("%s: read unaligned %lx\n",
						  __FUNCTION__, address));
					*(uint32 *) data =
					    dhdpcie_bus_rtcm32(bus, address);
					data += 4;
					size -= 4;
					address += 4;
					is_64bit_unaligned = (address & 0x7);
...
#endif
					*(uint64 *) data =
					    dhdpcie_bus_rtcm64(bus, address);
			} else {
				dsize = sizeof(uint8);
...
dhd_bus_schedule_queue(struct dhd_bus *bus, uint16 flow_id, bool txs)
{
...

	{
		...
		txp = dhd_flow_queue_dequeue(bus->dhd, queue);
		...
		while (txp != NULL) {
			...
			if (ret != BCME_OK) {	/* may not have resources in flow ring */
				...
				DHD_FLOWRING_UNLOCK(flow_ring_node->lock,
						    flags);
...
int BCMFASTPATH dhd_bus_txdata(struct dhd_bus *bus, void *txp, uint8 ifidx)
{
	...
	if (!bus->txmode_push) {
		...
		if (txp_pend) {
			DHD_FLOWRING_LOCK(flow_ring_node->lock, flags);
			ret = dhd_flow_queue_enqueue(bus->dhd, queue,
										txp_pend);
			if (ret != BCME_OK) {
				DHD_FLOWRING_UNLOCK(flow_ring_node->lock,
						    flags);
...
void
dhd_bus_update_retlen(dhd_bus_t *bus, uint32 retlen, uint32 pkt_id,
		      uint16 status, uint32 resp_len)
{
	bus->rxlen = retlen;
...
int dhd_bus_console_in(dhd_pub_t *dhd, uchar *msg, uint msglen)
{
	...
	val = htol32(0);
	rv =
	     dhdpcie_bus_membytes(bus, TRUE, addr, (uint8 *) &val,
				  sizeof(val));
	if (rv < 0)
		goto done;
	...
	rv =
	     dhdpcie_bus_membytes(bus, TRUE, addr, (uint8 *) msg, msglen);
	if (rv < 0)
		goto done;
...
dhd_bus_rx_frame(struct dhd_bus *bus, void *pkt, int ifidx, uint pkt_count,
		 int pkt_wake)
{
	dhd_rx_frame(bus->dhd, ifidx, pkt, pkt_count, 0, pkt_wake,
		     &bus->wake_counts);
}
...
uint8 dhdpcie_bus_rtcm8(dhd_bus_t *bus, ulong offset)
{
	...
	data = R_REG(bus->dhd->osh,
		     (volatile uint8 *) (bus->tcm +
					 dhd_bus_cmn_check_offset(bus,
								  offset)));
#else
	data =
	    *(volatile uint8 *) (bus->tcm +
				 dhd_bus_cmn_check_offset(bus, offset));
#endif
	return data;
}
			...
			uint16 ringid)
{
	...
	case DNGL_TO_HOST_DMA_SCRATCH_BUFFER:
		...
		dhdpcie_bus_membytes(bus, TRUE, tcm_offset,
				     (uint8 *) &long_data, len);
	...
	case DNGL_TO_HOST_DMA_SCRATCH_BUFFER_LEN:
		...
		dhdpcie_bus_wtcm32(bus, tcm_offset,
	...
	case RING_LEN_ITEMS:
		...
		dhdpcie_bus_wtcm16(bus, tcm_offset,
...
dhd_bus_cmn_readshared(dhd_bus_t *bus, void *data, uint8 type, uint16 ringid)
{
	...
	case HTOD_MB_DATA:
		*(uint32 *) data =
		    LTOH32(dhdpcie_bus_rtcm32(bus, bus->h2d_mb_data_ptr_addr));
		break;
	case DTOH_MB_DATA:
		*(uint32 *) data =
		    LTOH32(dhdpcie_bus_rtcm32(bus, bus->d2h_mb_data_ptr_addr));
		break;
		 ...
		 void *params, int plen, void *arg, int len, bool set)
{
	...
	bcmerror =
	    dhdpcie_bus_doiovar(bus, vi, actionid, name, params, plen, arg, len,
				val_size);
...
int dhd_buzzz_dump_cntrs3(char *p, uint32 *core, uint32 *ovhd, uint32 *log)
{
	...
	for (ctr = 0U; ctr < 3; ctr++) {
		...
		bytes += snprintf(p + bytes, 56, "%12u ", delta[ctr]);
	}
...
int dhd_buzzz_dump_cntrs6(char *p, uint32 *core, uint32 *ovhd, uint32 *log)
{
	...
	{			/* 32bit cyccnt */
		...
		bytes += snprintf(p + bytes, 56, "%12u ", delta);
		cyccnt = delta;
	}
	...
	{			/* Extract the 4 cnts: cpi, exc, sleep and lsu */
		...
		for (i = 0; i < 4; i++) {
			...
			if (delta.u8[i] >= ovhd[i + 1])
				...
				delta.u8[i] = 0;
			bytes += snprintf(p + bytes, 56, "%4u ", delta.u8[i]);
		}
	...
	{			/* Extract the foldcnt from arg0 */
		...
		if (delta >= ovhd[5])
			...
			delta = 0;
		bytes += snprintf(p + bytes, 56, "%4u ", delta);
		foldcnt = delta;
	}
	...
	if (instrcnt > 0xFFFFFF00)
		bytes += snprintf(p + bytes, 56, "[%10s] ", "~");
	else
		bytes += snprintf(p + bytes, 56, "[%10u] ", instrcnt);
	return bytes;
}
...
int dhd_buzzz_dump_log(char *p, uint32 *core, uint32 *log, buzzz_t *buzzz)
{
	...
	case 0:
		bytes += snprintf(p + bytes, sizeof(fmt), fmt[arg0.klog.id]);
		break;
	case 1:
		{
			uint32 arg1 = *log++;
			bytes += snprintf(p + bytes, sizeof(fmt), fmt[arg0.klog.id], arg1);
			break;
		}
	default:
...
void dhd_buzzz_dump(buzzz_t *buzzz_p, void *buffer_p, char *p)
{
	...
	if (buzzz_p->wrap) {
		part2 =
		    ((uint32) buzzz_p->end - (uint32) buzzz_p->cur) / log_sz;
...
int dhd_buzzz_dump_dngl(dhd_bus_t *bus)
{
	...
	buzzz_p = MALLOC(bus->dhd->osh, sizeof(buzzz_t));
	if (buzzz_p == NULL) {
	...
	if (buzzz_p)
		MFREE(bus->dhd->osh, buzzz_p, sizeof(buzzz_t));
	if (buffer_p)
		MFREE(bus->dhd->osh, buffer_p, buzzz_p->buffer_sz);
...
static int
dhdpcie_bus_doiovar(dhd_bus_t *bus, const bcm_iovar_t *vi, uint32 actionid,
		    const char *name, void *params, int plen, void *arg,
		    int len, int val_size)
{
	int bcmerror = 0;
	int32 int_val = 0;
	int32 int_val2 = 0;
	...
	if (plen >= (int) sizeof(int_val) * 3)
		bcopy((void *) ((uintptr) params + 2 * sizeof(int_val)),
		      &int_val3, sizeof(int_val3));
	...
	case IOV_GVAL(IOV_BAR0_SECWIN_REG):
		{
			...
			dhdpcie_bus_cfg_write_dword(bus, PCIE2_BAR0_CORE2_WIN,
						    sizeof(uint32), base);
			bar0 = (uchar *) bus->regs;
	...
	case IOV_SVAL(IOV_PCIECOREREG):
		si_corereg(bus->sih, bus->sih->buscoreidx, int_val, ~0,
			   int_val2);
		break;
	case IOV_GVAL(IOV_SBREG):
		{
			...
			int_val =
			    si_corereg(bus->sih, coreidx, (addr & 0xFFF), 0, 0);
	...
	case IOV_SVAL(IOV_SBREG):
		{
			...
			si_corereg(bus->sih, coreidx, (addr & 0xFFF), ~0,
				   sdreg.value);
	...
	case IOV_GVAL(IOV_PCIECOREREG):
		int_val =
		    si_corereg(bus->sih, bus->sih->buscoreidx, int_val, 0, 0);
		bcopy(&int_val, arg, sizeof(int_val));
		break;
	...
	case IOV_SVAL(IOV_PCIE_DMAXFER):
		bcmerror =
		    dhdpcie_bus_dmaxfer_req(bus, int_val, int_val2, int_val3);
		break;
	...
	case IOV_GVAL(IOV_MEMBYTES):
		{
			...
			DHD_INFO(("%s: Request to %s %d bytes at address 0x%08x\n dsize %d ", __FUNCTION__, (set ? "write" : "read"), size, address, dsize));
	...
	case IOV_GVAL(IOV_DMA_RINGINDICES):
		{
			...
			d2h_support =
			    DMA_INDX_ENAB(bus->dhd->
					  dma_d2h_ring_upd_support) ? 1 : 0;
			h2d_support =
			    DMA_INDX_ENAB(bus->dhd->
					  dma_h2d_ring_upd_support) ? 1 : 0;
		...
		if (bus->dhd->busstate == DHD_BUS_DOWN) {
			if ((int_val > 3) || (int_val < 0)) {
				...
				bus->dhd->dma_d2h_ring_upd_support =
				    (int_val & 1) ? TRUE : FALSE;
				bus->dhd->dma_h2d_ring_upd_support =
				    (int_val & 2) ? TRUE : FALSE;
			}
		} else {
	...
	case IOV_SVAL(IOV_FLOW_PRIO_MAP):
		int_val =
		    (int32) dhd_update_flow_prio_map(bus->dhd, (uint8) int_val);
		bcopy(&int_val, arg, sizeof(int_val));
		break;
...
void dhd_bus_set_suspend_resume(dhd_pub_t *dhdp, bool state)
{
...
int dhdpcie_bus_suspend(struct dhd_bus *bus, bool state)
{
	...
	if (state) {
		...
		timeleft =
		    dhd_os_d3ack_wait(bus->dhd, &bus->wait_for_d3_ack,
				      &pending);
		...
		if (bus->wait_for_d3_ack == 1) {
			...
			if (dhd_os_check_wakelock_all(bus->dhd)) {
				...
				dhdpcie_bus_intr_disable(bus);
				rc = dhdpcie_pci_suspend_resume(bus->dev,
								state);
			}
		} else if (timeleft == 0) {
			DHD_ERROR(("%s: resumed on timeout\n", __FUNCTION__));
		...
		bus->suspended = FALSE;
		if (dhdpcie_bus_cfg_read_dword(bus, PCI_VENDOR_ID, 4) ==
		    PCIE_LINK_DOWN) {
...
static int
dhdpcie_bus_dmaxfer_req(struct dhd_bus *bus, uint32 len, uint32 srcdelay,
			uint32 destdelay)
{
	if (bus->dhd == NULL) {
		DHD_ERROR(("bus not inited\n"));
	...
	if (enter) {
		...
		if (cr4_regs == NULL) {	/* no CR4 present on chip */
			...
			if (bus->sih->chip == BCM43602_CHIP_ID) {
				W_REG(bus->pcie_mb_intr_osh,
				      cr4_regs + ARMCR4REG_BANKIDX, 5);
				W_REG(bus->pcie_mb_intr_osh,
				      cr4_regs + ARMCR4REG_BANKPDA, 0);
				W_REG(bus->pcie_mb_intr_osh,
				      cr4_regs + ARMCR4REG_BANKIDX, 7);
				W_REG(bus->pcie_mb_intr_osh,
				      cr4_regs + ARMCR4REG_BANKPDA, 0);
			}
...
static int dhdpcie_bus_write_vars(dhd_bus_t *bus)
{
	...
	if (bus->vars) {
		...
		/* Write the vars list */
		bcmerror =
		    dhdpcie_bus_membytes(bus, TRUE, varaddr, vbuffer, varsize);
		...
		bcmerror =
		    dhdpcie_bus_membytes(bus, FALSE, varaddr, nvram_ularray,
					 varsize);
		if (bcmerror) {
...
void dhd_bus_dump(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
	...
	bcm_bprintf(strbuf, " unicast %u muticast %u broadcast %u arp %u\n",
		    ...
		    dhdp->bus->wake_counts.rx_bcast,
		    dhdp->bus->wake_counts.rx_arp);
...
static void dhd_bus_gen_devmb_intr(struct dhd_bus *bus)
{
	...
	if (bus->db1_for_mb) {
		...
		si_corereg(bus->sih, bus->sih->buscoreidx, PCIH2D_DB1, ~0,
			   0x12345678);
	} else {
...
void dhd_bus_ringbell(struct dhd_bus *bus, uint32 value)
{
	if ((bus->sih->buscorerev == 2) || (bus->sih->buscorerev == 6) ||
	    (bus->sih->buscorerev == 4)) {
		si_corereg(bus->sih, bus->sih->buscoreidx, PCIMailBoxInt,
			   PCIE_INTB, PCIE_INTB);
	} else {
		/* this is a pcie core register, not the config regsiter */
		DHD_INFO(("writing a door bell to the device\n"));
		si_corereg(bus->sih, bus->sih->buscoreidx, PCIH2D_MailBox, ~0,
			   0x12345678);
	}
}

static void dhd_bus_ringbell_fast(struct dhd_bus *bus, uint32 value)
{
...
dhd_mb_ring_t dhd_bus_get_mbintr_fn(struct dhd_bus *bus)
{
	if ((bus->sih->buscorerev == 2) || (bus->sih->buscorerev == 6) ||
		...
		bus->pcie_mb_intr_addr =
		    si_corereg_addr(bus->sih, bus->sih->buscoreidx,
				    PCIMailBoxInt);
...
bool BCMFASTPATH dhd_bus_dpc(struct dhd_bus *bus)
{
	...
	if ((bus->sih->buscorerev == 6) || (bus->sih->buscorerev == 4) ||
		...
		newstatus =
		    si_corereg(bus->sih, bus->sih->buscoreidx, PCIMailBoxInt, 0,
			       0);
...
static void dhdpcie_send_mb_data(dhd_bus_t *bus, uint32 h2d_mb_data)
{
	...
	if (cur_h2d_mb_data != 0) {
		...
		while ((i++ < 100) && cur_h2d_mb_data) {
			OSL_DELAY(10);
			dhd_bus_cmn_readshared(bus, &cur_h2d_mb_data,
					       HTOD_MB_DATA, 0);
		}
		if (i >= 100)
	...
	dhd_bus_cmn_writeshared(bus, &h2d_mb_data, sizeof(uint32),
				HTOD_MB_DATA, 0);
...
static int dhdpcie_readshared(dhd_bus_t *bus)
{
	...
	rv = dhdpcie_bus_membytes(bus, FALSE, addr, (uint8 *) sh,
				       sizeof(pciedev_shared_t));
	if (rv < 0) {
		DHD_ERROR(("Failed to read PCIe shared struct,"
			   "size read %d < %d\n", rv,
			   (int) sizeof(pciedev_shared_t)));
		return rv;
	}
	...
	if (sh->flags & PCIE_SHARED_DMA_INDEX) {
		...
		DHD_ERROR(("%s: Host support DMAing indices: H2D:%d - D2H:%d. FW supports it\n", __FUNCTION__, (DMA_INDX_ENAB(bus->dhd->dma_h2d_ring_upd_support) ? 1 : 0), (DMA_INDX_ENAB(bus->dhd->dma_d2h_ring_upd_support) ? 1 : 0)));
	...
	/* get ring_info, ring_state and mb data ptrs and store the addresses in bus structure */
	{
		...
		rv = dhdpcie_bus_membytes(bus, FALSE, sh->rings_info_ptr,
								   (uint8 *) &ring_info,
								   sizeof(ring_info_t));
		if (rv < 0)
			return rv;
		...
		if (DMA_INDX_ENAB(bus->dhd->dma_h2d_ring_upd_support)) {
			w_init = dhd_prot_init_index_dma_block(bus->dhd,
							       ...
							       max_sub_queues);
			r_init =
			    dhd_prot_init_index_dma_block(bus->dhd,
	...
	/* Ring state mem ptr info */
	{
		...
		if (bus->txmode_push) {
			...
			for (j = 0;
			     j <
			     (bus->max_sub_queues -
			      BCMPCIE_H2D_COMMON_MSGRINGS); i++, j++) {
				bus->ring_sh[i].ring_state_w = h2d_w_idx_ptr;
				bus->ring_sh[i].ring_state_r = h2d_r_idx_ptr;
				...
				DHD_INFO(("FLOW Rings h2d w/r : idx %d write %x read %x \n", i, bus->ring_sh[i].ring_state_w, bus->ring_sh[i].ring_state_r));
			}
		}
	}
}
...
static int dhdpcie_cc_nvmshadow(dhd_bus_t *bus, struct bcmstrbuf *b)
{
	...
	if (chipcregs->sromcontrol & SRC_PRESENT) {
		...
		sprom_size =
		    (1 << (2 * ((chipcregs->sromcontrol & SRC_SIZE_MASK)
				>> SRC_SIZE_SHIFT))) * 1024;
	...
	if (chipcregs->sromcontrol & SRC_OTPPRESENT) {
		...
		if (((chipcregs->otplayout & OTPL_WRAP_TYPE_MASK) >>
			...
			otp_size =
			    otp_size_65nm[(chipcregs->capabilities &
					   CC_CAP_OTPSIZE)
...
uint8 BCMFASTPATH dhd_bus_is_txmode_push(dhd_bus_t *bus)
{
...
uint32 dhd_bus_max_h2d_queues(struct dhd_bus *bus, uint8 *txpush)
{
	if (bus->txmode_push)
		*txpush = 1;
	else
