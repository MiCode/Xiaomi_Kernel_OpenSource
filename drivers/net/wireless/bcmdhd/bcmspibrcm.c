...
extern SDIOH_API_RC sdioh_query_device(sdioh_info_t *sd)
{
...
extern void sdioh_dwordmode(sdioh_info_t *sd, bool set)
{
	...
	int status;
	status =
	     sdioh_request_byte(sd, SDIOH_READ, SPI_FUNC_0, SPID_STATUS_ENABLE,
				&reg);
	if (status != SUCCESS) {
	...
	status =
	     sdioh_request_byte(sd, SDIOH_WRITE, SPI_FUNC_0, SPID_STATUS_ENABLE,
				&reg);
	if (status != SUCCESS) {
		sd_err(("%s: Failed to set dwordmode in gSPI\n", __FUNCTION__));
		return;
...
*/
	...
	case IOV_SVAL(IOV_RESP_DELAY_ALL):
		...
		if (si->resp_delay_all)
			...
			if (bcmspi_card_regwrite
			    (si, SPI_FUNC_0, SPID_RESPONSE_DELAY, 1,
		...
		if (bcmspi_card_regwrite
		    (si, SPI_FUNC_0, SPID_STATUS_ENABLE, 1, int_val)
		    != SUCCESS) {
			sd_err(("%s: Unable to set response delay.\n",
				__FUNCTION__));
			bcmerror = BCME_SDIO_ERROR;
...
sdioh_cis_read(sdioh_info_t *sd, uint func, uint8 *cisd, uint32 length)
{
	...
	status =
	    bcmspi_card_bytewrite(sd, SDIO_FUNC_1, SBSDIO_FUNC1_SBADDRLOW,
				  &data);
	if (status == SUCCESS) {
		data = (bar0 >> 16) & SBSDIO_SBADDRMID_MASK;
		status =
		    bcmspi_card_bytewrite(sd, SDIO_FUNC_1,
					  SBSDIO_FUNC1_SBADDRMID, &data);
	} else {
	...
	for (count = 0; count < length / 2; count++) {
		if (bcmspi_card_regread(sd, SDIO_FUNC_1, offset, 2, &cis_byte) <
		    0) {
		   ...
		   uint8 *byte)
{
	...
	status =
			 bcmspi_cmd_issue(sd, sd->sd_use_dma, cmd_arg, &data, 1);
	if (status != SUCCESS) {
		spi_unlock(sd);
		return status;
	}
...
extern SDIOH_API_RC
sdioh_request_buffer(sdioh_info_t *sd, uint pio_dma, uint fix_inc, uint rw,
		     ...
		     uint8 *buffer, void *pkt)
{
	...
	while (buflen > 0) {
		...
		if (bcmspi_card_buf
		    (sd, rw, func, fifo, addr, len,
		     (uint32 *) buffer) != SUCCESS) {
...
static int bcmspi_prog_resp_delay(sdioh_info_t *sd, int func, uint8 resp_delay)
{
	...
	bcmspi_card_byterewrite(sd, SPI_FUNC_0, SPID_RESPONSE_DELAY,
				resp_delay);
		...
		sd_trace(("Packet length = %d\n", sd->dwordmode ?
			  ((dstatus & STATUS_F2_PKT_LEN_MASK) >>
			   (STATUS_F2_PKT_LEN_SHIFT -
			    2)) : ((dstatus & STATUS_F2_PKT_LEN_MASK) >>
				   STATUS_F2_PKT_LEN_SHIFT)));
	}
...
static int get_client_blocksize(sdioh_info_t *sd)
{
	...
	status = bcmspi_card_regread(sd, 0, SPID_F1_INFO_REG,
					  8, regdata);
	if (status != SUCCESS) {
		return status;
	}
...
static int bcmspi_client_init(sdioh_info_t *sd)
{
	...
	/* check to see if the response delay needs to be programmed properly */
	{
		...
		bcmspi_card_regread(sd, 0, SPID_RESP_DELAY_F1, 1,
				    &f1_respdelay);
		if ((f1_respdelay == 0) || (f1_respdelay == 0xFF)) {
			...
			sd_trace(("resp delay for funcs f0(%d), f1(%d), f2(%d), f3(%d)\n", GSPI_F0_RESP_DELAY, GSPI_F1_RESP_DELAY, GSPI_F2_RESP_DELAY, GSPI_F3_RESP_DELAY));
			ret_val =
			    bcmspi_card_regwrite(sd, SPI_FUNC_0,
						 SPID_RESP_DELAY_F0, 1,
						 GSPI_F0_RESP_DELAY);
			if (ret_val != SUCCESS) {
...
static int bcmspi_set_highspeed_mode(sdioh_info_t *sd, bool hsmode)
{
	...
	status = bcmspi_card_regread(sd, 0, SPID_CONFIG,
						  4, &regdata);
	...
	if (hsmode == TRUE) {
		...
		if (regdata & HIGH_SPEED_MODE) {
			...
			sd_trace(("Writing %08x to device at %08x\n", regdata,
				  SPID_CONFIG));
			status =
			     bcmspi_card_regwrite(sd, 0, SPID_CONFIG, 4,
						  regdata);
			if (status != SUCCESS) {
				return status;
			}
...
static bool bcmspi_host_device_init_adapt(sdioh_info_t *sd)
{
	...
	if (sd->wordlen == 4) {
		status =
		     bcmspi_card_regread(sd, 0, SPID_TEST_READ, 4,
					 &regdata);
		if (status != SUCCESS)
			return FALSE;
		if (regdata == TEST_RO_DATA_32BIT_LE) {
			...
			sd_err(("Spid power was left on but signature read failed." " Value read = 0x%x\n", regdata));
			return FALSE;
		}
	} else {
		sd->wordlen = 2;
...
#define CTRL_REG_DEFAULT	0x00010430	/* according to the host m/c */
		...
		for (i = 0; i < INIT_ADAPT_LOOP; i++) {
			if ((regdata == 0xfdda7d5b) || (regdata == 0xfdda7d5a)) {
				...
				status =
				     bcmspi_card_regread(sd, 0, SPID_TEST_READ,
							 4,
							 &regdata);
				if (status != SUCCESS)
					return FALSE;
		...
		if (regdata == TEST_RO_DATA_32BIT_LE) {
			...
			sd_err(("Stale spid reg values read as it was kept powered. Value read =" "0x%x\n", regdata));
			return FALSE;
		}
	}
...
#define RW_PATTERN2	0x4B5B6B7B
	...
	status =
	     bcmspi_card_regwrite(sd, 0, SPID_TEST_RW, 4, regdata);
	if (status != SUCCESS)
		return FALSE;
	regdata = 0;
	status =
		bcmspi_card_regread(sd, 0, SPID_TEST_RW, 4, &regdata);
	if (status != SUCCESS)
		return FALSE;
...
/* Read device reg */
static int
bcmspi_card_regread(sdioh_info_t *sd, int func, uint32 regaddr, int regsize,
		    uint32 *data)
{
	int status;
	...
	cmd_arg = SFIELD(cmd_arg, SPI_REG_ADDR, regaddr);
	cmd_arg =
	    SFIELD(cmd_arg, SPI_LEN, regsize == BLOCK_SIZE_F2 ? 0 : regsize);

	sd_trace(("%s: RD cmd_arg=0x%x func=%d regaddr=0x%x regsize=%d\n",
		  __FUNCTION__, cmd_arg, func, regaddr, regsize));
	status =
	     bcmspi_cmd_issue(sd, sd->sd_use_dma, cmd_arg, data,
					  regsize);
	if (status != SUCCESS)
		return status;
...
static int
bcmspi_card_regread_fixedaddr(sdioh_info_t *sd, int func, uint32 regaddr,
			      int regsize, uint32 *data)
{
...
static int
bcmspi_card_regwrite(sdioh_info_t *sd, int func, uint32 regaddr, int regsize,
		     uint32 data)
{
	int status;
	uint32 cmd_arg, dstatus;
		 ...
		 uint32 *data, uint32 datalen)
{
	...
	if ((GFIELD(cmd_arg, SPI_RW_FLAG) == 0)) {
		...
		case 0:
			if (sd->resp_delay_new)
				...
				resp_delay =
				    sd->resp_delay_all ? F0_RESPONSE_DELAY : 0;
			break;
		case 1:
			if (sd->resp_delay_new)
		...
		case 2:
			if (sd->resp_delay_new)
				...
				resp_delay =
				    sd->resp_delay_all ? F2_RESPONSE_DELAY : 0;
			break;
		default:
			ASSERT(0);
			break;
	...
	if (sd->wordlen == 4) {	/* 32bit spid */
		sd->card_dstatus =
		    SPISWAP_WD4(*(uint32 *) &spi_inbuf[dstatus_idx]);
	} else if (sd->wordlen == 2) {	/* 16bit spid */
		sd->card_dstatus =
		    SPISWAP_WD2(*(uint32 *) &spi_inbuf[dstatus_idx]);
	} else {
		...
		uint32 addr, int nbytes, uint32 *data)
{
		...
		if (write) {
			...
			if ((dstatus & STATUS_F2_RX_READY) == 0) {
				...
				while (retries-- && !enable) {
					OSL_DELAY(WAIT_F2RXFIFORDY_DELAY *
						  1000);
					bcmspi_card_regread(sd, SPI_FUNC_0,
							    SPID_STATUS_REG, 4,
							    &dstatus);
	...
	if ((func == 2) && (fifo == 1)) {
		sd_data(("%s: %s func %d, %s, addr 0x%x, len %d bytes, r_cnt %d t_cnt %d\n", __FUNCTION__, write ? "Wr" : "Rd", func, "INCR", addr, nbytes, sd->r_cnt, sd->t_cnt));
	}

	sd_trace(("%s cmd_arg = 0x%x\n", __FUNCTION__, cmd_arg));
	sd_data(("%s: %s func %d, %s, addr 0x%x, len %d bytes, r_cnt %d t_cnt %d\n", __FUNCTION__, write ? "Wd" : "Rd", func, "INCR", addr, nbytes, sd->r_cnt, sd->t_cnt));
