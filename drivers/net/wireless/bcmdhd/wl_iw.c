...
#define DAEMONIZE(a) { daemonize(); \
	do { if (a) \
		strlcpy(current->comm, a, MIN(sizeof(current->comm), (strlen(a) + 1))); \
	} while (0); \
}
...
static int dev_wlc_ioctl(struct net_device *dev, int cmd, void *arg, int len)
{
	...
	strlcpy(ifr.ifr_name, dev->name, IFNAMSIZ);
...
static int dev_wlc_intvar_set(struct net_device *dev, char *name, int val)
{
	...
	val = htod32(val);
	len =
	    bcm_mkiovar(name, (char *) (&val), sizeof(val), buf, sizeof(buf));
	ASSERT(len);
...
#if WIRELESS_EXT > 12
static int
wl_iw_set_leddc(struct net_device *dev,
...
static int
wl_iw_set_vlanmode(struct net_device *dev,
		   struct iw_request_info *info,
		   union iwreq_data *wrqu, char *extra)
{
	int mode = *(int *) extra;
...
static int
wl_iw_set_pm(struct net_device *dev,
	       ...
	       union iwreq_data *cwrq, char *extra)
{
	...
	cap[0] = 0;
	err =
	     dev_wlc_ioctl(dev, WLC_GET_PHYTYPE, &phytype,
			   sizeof(phytype));
	if (err < 0)
		goto done;
	err =
	     dev_wlc_ioctl(dev, WLC_GET_BANDLIST, band, sizeof(band));
	if (err < 0)
		goto done;
	...
	switch (phytype) {
	case WLC_PHY_TYPE_A:
		strlcpy(cap, "a", sizeof(cap));
		break;
	case WLC_PHY_TYPE_B:
		strlcpy(cap, "b", sizeof(cap));
		break;
	case WLC_PHY_TYPE_LP:
	case WLC_PHY_TYPE_G:
		if (band[0] >= 2)
			strlcpy(cap, "abg", sizeof(cap));
		else
			strlcpy(cap, "bg", sizeof(cap));
		break;
	case WLC_PHY_TYPE_N:
		if (band[0] >= 2)
			strlcpy(cap, "abgn", sizeof(cap));
		else
			strlcpy(cap, "bgn", sizeof(cap));
		break;
	}
      done:
		...
		struct iw_point *dwrq, char *extra)
{
	...
	error = dev_wlc_intvar_get(dev, "nmode", &nmode);
	if (error)
		return error;
	error =
	...
	if (error)
		return error;
	if (nmode == 1
	    && ((phytype == WLC_PHY_TYPE_SSN) || (phytype == WLC_PHY_TYPE_LCN)
		|| (phytype == WLC_PHY_TYPE_LCN40))) {
		error = dev_wlc_intvar_get(dev, "mimo_bw_cap", &bw_cap);
		if (error)
			return error;
		error = dev_wlc_intvar_get(dev, "sgi_tx", &sgi_tx);
		if (error)
			return error;
		error =
	      ...
	      struct iw_request_info *info, struct sockaddr *awrq, char *extra)
{
	...
	error =
	     dev_wlc_ioctl(dev, WLC_REASSOC, awrq->sa_data, ETHER_ADDR_LEN);
	if (error) {
		WL_ERROR(("%s: WLC_REASSOC failed (%d).\n", __FUNCTION__,
			  error));
		return error;
			     ...
			     struct iw_request_info *info, wl_bss_info_t *bi)
{
	...
	if (bi->ie_length) {
		...
		ie = bcm_parse_tlvs(ptr, ptr_len, DOT11_MNG_MDIE_ID);
		if (ie) {
			iwe.cmd = IWEVGENIE;
			iwe.u.data.length = ie->len + 2;
			event =
		...
		ie = bcm_parse_tlvs(ptr, ptr_len, DOT11_MNG_WPA_ID);
		while (ie) {
			/* look for WPS IE */
	       ...
	       struct iw_request_info *info, struct iw_point *dwrq, char *extra)
{
	...
	for (i = 0; i < list->count && i < IW_MAX_AP; i++) {
		...
		if (bi->rateset.count) {
			...
			for (j = 0;
			     j < bi->rateset.count && j < IW_MAX_BITRATES;
			     j++) {
				iwe.u.bitrate.value =
				    (bi->rateset.rates[j] & 0x7f) * 500000;
				value =
				    IWE_STREAM_ADD_VALUE(info, event, value,
							 end, &iwe,
							 IW_EV_PARAM_LEN);
			}
			event = value;
	       ...
	       struct iw_request_info *info, struct iw_point *dwrq, char *extra)
{
	...
	strlcpy(extra, iw->nickname, IW_ESSID_MAX_SIZE);
	dwrq->length = strlen(extra) + 1;

	return 0;
}

static int wl_iw_set_rate(struct net_device *dev,
			  struct iw_request_info *info,
...
static int wl_iw_get_rate(struct net_device *dev,
			  struct iw_request_info *info,
			  struct iw_param *vwrq, char *extra)
{
	int error, rate;
		...
		struct iw_param *vwrq, char *extra)
{
	...
	error =
	    dev_wlc_intvar_set(dev, "qtxpower",
			       (int) (bcm_mw_to_qdbm(txpwrmw)));
	return error;
}
		    ...
		    struct iw_point *dwrq, char *extra)
{
	 ...
	 */
		...
		for (j = 0; j < (WSEC_MAX_PSK_LEN / 2); j++) {
			snprintf(charptr, WSEC_MAX_PSK_LEN + 1, "%02x", iwe->key[j]);
			charptr += 2;
		}
		...
		error =
		    dev_wlc_ioctl(dev, WLC_SET_WSEC_PMK, &pmk, sizeof(pmk));
		if (error)
			return error;
	}
		...
		struct iw_param *vwrq, char *extra)
{
	...
	if (iwpmksa->cmd == IW_PMKSA_REMOVE) {
		...
		{
			uint j;
			WL_TRACE(("wl_iw_set_pmksa,IW_PMKSA_REMOVE - PMKID: %s = ", bcm_ether_ntoa(&pmkidptr->pmkid[0].BSSID, eabuf)));
			for (j = 0; j < WPA2_PMKID_LEN; j++)
				WL_TRACE(("%02x ",
					  pmkidptr->pmkid[0].PMKID[j]));
			WL_TRACE(("\n"));
		}
	...
	if (iwpmksa->cmd == IW_PMKSA_ADD) {
		...
		bcopy(&iwpmksa->pmkid[0],
		      &pmkid_array[pmkid_list.pmkids.npmkid].PMKID,
		      WPA2_PMKID_LEN);
		{
			uint j;
			uint k;
	...
	WL_TRACE(("\n"));
	dev_wlc_bufvar_set(dev, "pmkid_info", (char *) &pmkid_list,
			   sizeof(pmkid_list));
	return 0;
}
#endif				/* WIRELESS_EXT > 17 */
		  ...
		  struct iw_param *vwrq, char *extra)
{
	...
	case IW_AUTH_KEY_MGMT:
		...
		if (val & (WPA_AUTH_PSK | WPA_AUTH_UNSPECIFIED)) {
			...
			if (paramval &
			    (IW_AUTH_KEY_MGMT_FT_802_1X |
			     IW_AUTH_KEY_MGMT_FT_PSK))
				val |= WPA2_AUTH_FT;
...
static int wl_iw_iscan(iscan_info_t *iscan, wlc_ssid_t *ssid, uint16 action)
{
	...
	if (!err) {
		...
		(void) dev_iw_iovar_setbuf(iscan->dev, "iscan", params,
					   params_size, iscan->ioctlbuf,
					   WLC_IOCTL_SMLEN);
	}
...
static uint32 wl_iw_iscan_get(iscan_info_t *iscan)
{
	...
	(void) dev_iw_iovar_getbuf(iscan->dev,
				   "iscanresults",
				   &list,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 7, 0))
	iscan->kthread =
	    kthread_run(_iscan_sysioc_thread, iscan, "iscan_sysioc");
