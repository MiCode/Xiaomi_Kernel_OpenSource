	...
	while (eromptr < eromlim) {
		...
		if ((nmw + nsw == 0)) {
			...
			if (cid != GMAC_COMMON_4706_CORE_ID
			    && cid != NS_CCB_CORE_ID && cid != PMU_CORE_ID
			    && cid != GCI_CORE_ID)
				continue;
		}
...
void *ai_setcoreidx(si_t *sih, uint coreidx)
{
	...
	case SI_BUS:
		...
		if (!cores_info->wrappers[coreidx] && (wrap != 0)) {
			cores_info->wrappers[coreidx] =
			    REG_MAP(wrap, SI_CORE_SIZE);
	...
	case PCI_BUS:
		...
		if (PCIE_GEN2(sii))
			OSL_PCI_WRITE_CONFIG(sii->osh, PCIE2_BAR0_WIN2, 4,
					     wrap);
		else
 ...
 * NULL.
 */
uint32 *ai_corereg_addr(si_t *sih, uint coreidx, uint regoff)
{
	uint32 *r = NULL;
	bool fast = FALSE;
	si_info_t *sii = SI_INFO(sih);
...
void ai_core_reset(si_t *sih, uint32 bits, uint32 resetbits)
{
	...
	while (R_REG(sii->osh, &ai->resetctrl) != 0 && --loop_counter != 0) {
		/* ensure there are no pending backplane operations */
		SPINWAIT(((dummy =
			   R_REG(sii->osh, &ai->resetstatus)) != 0), 300);
	...
	for (i = 0; i < sii->numcores; i++) {
		...
		bcm_bprintf(b,
			    ...
			    "intstatus 0x%x config 0x%x itcr 0x%x\n", R_REG(osh,
									    &ai->
									    ioctrlset),
			    R_REG(osh, &ai->ioctrlclear), R_REG(osh,
								&ai->ioctrl),
			    R_REG(osh, &ai->iostatus), R_REG(osh,
							     &ai->ioctrlwidth),
			    R_REG(osh, &ai->iostatuswidth), R_REG(osh,
								  &ai->
								  resetctrl),
			    R_REG(osh, &ai->resetstatus), R_REG(osh,
								&ai->
								resetreadid),
			    R_REG(osh, &ai->resetwriteid), R_REG(osh,
								 &ai->
								 errlogctrl),
			    R_REG(osh, &ai->errlogdone), R_REG(osh,
							       &ai->
							       errlogstatus),
			    R_REG(osh, &ai->errlogaddrlo), R_REG(osh,
								 &ai->
								 errlogaddrhi),
			    R_REG(osh, &ai->errlogid), R_REG(osh,
							     &ai->errloguser),
			    R_REG(osh, &ai->errlogflags), R_REG(osh,
								&ai->intstatus),
			    R_REG(osh, &ai->config), R_REG(osh, &ai->itcr));
	}
}
#endif
