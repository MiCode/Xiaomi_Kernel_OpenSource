...
#define ASSERT(exp)
#endif
inline void *MALLOCZ(void *o, size_t s)
{
	BCM_REFERENCE(o);
	return calloc(1, s);
}

inline void MFREE(void *o, void *p, size_t s)
{
	BCM_REFERENCE(o);
	BCM_REFERENCE(s);
	free(p);
...
{
	return ((opts & BCM_XTLV_OPTION_ALIGN32) ?
		ALIGN_SIZE(dlen + BCM_XTLV_HDR_SIZE, 4)
		: (dlen + BCM_XTLV_HDR_SIZE));
}

bcm_xtlv_t *bcm_next_xtlv(bcm_xtlv_t *elt, int *buflen, bcm_xtlv_opts_t opts)
{
	int sz;
#ifdef BCMDBG
...
int
bcm_xtlv_buf_init(bcm_xtlvbuf_t *tlv_buf, uint8 *buf, uint16 len,
...
int
bcm_xtlv_put_data(bcm_xtlvbuf_t *tbuf, uint16 type, const void *data,
		  uint16 dlen)
{
	bcm_xtlv_t *xtlv;
	int size;
...
int bcm_xtlv_put_8(bcm_xtlvbuf_t *tbuf, uint16 type, const int8 data)
{
	bcm_xtlv_t *xtlv;
	int size;
...
int bcm_xtlv_put_16(bcm_xtlvbuf_t *tbuf, uint16 type, const int16 data)
{
	bcm_xtlv_t *xtlv;
	int size;
...
int bcm_xtlv_put_32(bcm_xtlvbuf_t *tbuf, uint16 type, const int32 data)
{
	bcm_xtlv_t *xtlv;
	int size;
 ...
 */
int
bcm_unpack_xtlv_buf(void *ctx, uint8 *tlv_buf, uint16 buflen,
		    bcm_xtlv_opts_t opts, bcm_xtlv_unpack_cbfn_t *cbfn)
{
	...
	while (sbuflen >= (int) BCM_XTLV_HDR_SIZE) {
		...
		res = cbfn(ctx, ptlv->data, type, len);
		if (res != BCME_OK)
			break;
		tlv_buf += size;
	}
			   ...
			   bcm_xtlv_opts_t opts)
{
	...
	while (items->type != 0) {
		...
		res = bcm_pack_xtlv_entry(&ptlv,
								   buflen,
								   items->type,
								   items->len,
								   items->ptr,
								   opts);
		if ((items->len > 0) && res != BCME_OK) {
			break;
		}
		items++;
	}
			   ...
			   bcm_xtlv_opts_t opts)
{
	...
	elt =
	    bcm_valid_xtlv((bcm_xtlv_t *) tlv_buf, *buflen,
			   opts) ? (bcm_xtlv_t *) tlv_buf : NULL;
	if (!elt || !items) {
		res = BCME_BADARG;
		return res;
	}
