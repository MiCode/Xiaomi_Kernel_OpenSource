...
extern int dhd_os_get_image_block_seek(char *buf, int len, void *image, int seek);
extern void dhd_os_close_image(void *image);

#define BCM_DBG pr_debug
#define BCM_ERROR pr_err
...
int dhd_wifi_init_gpio(void)
{
	...
	/* get wlan_reg_on */
	wl_reg_on = of_get_named_gpio(np, "wifi_reg_on", 0);
	if (wl_reg_on >= 0) {
		...
		gpio_wl_reg_on = 31;
		printk("%s: force gpio_wl_reg_on:%d.\n", __FUNCTION__,
		       gpio_wl_reg_on);
	}

	/* get host_wake irq */
	wl_host_wake = of_get_named_gpio(np, "wifi_host_wake", 0);
	if (wl_host_wake >= 0) {
		printk("%s: wl_host_wake:%d.\n", __FUNCTION__, wl_host_wake);
		brcm_wake_irq = gpio_to_irq(wl_host_wake);
	} else {
		printk("%s: wl_host_wake:%d.\n", __FUNCTION__, wl_host_wake);
		brcm_wake_irq = gpio_to_irq(30);
	}
...
extern void smdc_mmc_card_detect(int val);
static int dhd_wlan_set_carddetect(int val)
{
	smdc_mmc_card_detect(val);
	return 0;
}
...
void dhd_wifi_init_mac_addr(void)
{
	char mac_tmp[18];
	void *filep;
	int len = 0;
	...
	memset(mac_tmp, 0, 18);
	filep = dhd_os_open_image(WIFI_NVRAM_FILE_NAME);
	if (!filep)
		BCM_ERROR("%s: open error\n", __FUNCTION__);

	len = dhd_os_get_image_block_seek((char *)mac_tmp, 6, filep, WIFI_MAC_OFFSET);
	if (len < 6)
		BCM_ERROR("MAC address read error len=%d\n", len);
	for (i = 0; i < 6; i++) {
		brcm_mac_addr[i] = (u8)mac_tmp[i];
	}

	/* if wifi MAC is invaild, just do forced conversion for normal use */
	if (brcm_mac_addr[0] & 0x03) {
		BCM_ERROR("%s: bad mac address, first byte is %2x\n", __FUNCTION__, brcm_mac_addr[0]);
		brcm_mac_addr[0] &= 0xFC;
		BCM_ERROR("%s: mac address after conversion, first byte is %2x\n", __FUNCTION__, brcm_mac_addr[0]);
	}
