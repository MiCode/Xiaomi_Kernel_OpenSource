...
void add_roam_cache(wl_bss_info_t *bi)
{
	...
	roam_cache[n_roam_cache].chanspec =
	    (channel <=
	     CH_MAX_2G_CHANNEL ? band2G : band5G) | band_bw | channel;
			  ...
			  const wlc_ssid_t *ssid, int ioctl_ver)
{
	...
	if (target_chan) {
		...
		channels[n++] = (target_chan & WL_CHANSPEC_CHAN_MASK) |
		    (target_chan <=
		     CH_MAX_2G_CHANNEL ? band2G : band5G) | band_bw;
	...
	for (i = 0; i < n_roam_cache; i++) {
		...
		bool is_2G =
		    ioctl_ver == 1 ? LCHSPEC_IS2G(ch) : CHSPEC_IS2G(ch);
		bool is_5G =
		    ioctl_ver == 1 ? LCHSPEC_IS5G(ch) : CHSPEC_IS5G(ch);
		...
		if ((roam_cache[i].ssid_len == ssid->SSID_len) &&
		    ...
		    (memcmp(roam_cache[i].ssid, ssid->SSID, ssid->SSID_len) ==
		     0)) {
			/* match found, add it */
			WL_DBG(("%s: channel = %s\n", __FUNCTION__,
...
void update_roam_cache(struct bcm_cfg80211 *cfg, int ioctl_ver)
{
	...
	error = wldev_iovar_getbuf(dev, "roamscan_channels", 0, 0,
				   (void *) &channel_list,
				   sizeof(channel_list), NULL);

	WL_DBG(("%d AP, %d cache item(s), err=%d\n", n_roam_cache,
		channel_list.n, error));
	...
	for (i = 0; i < n_roam_cache; i++) {
		...
		if ((roam_cache[i].ssid_len == ssid.SSID_len) &&
			...
			/* match found, add it */
			/* XXX: JIRA:SW4349-173 : 80p80 Support Required */
			ch = CHSPEC_CHANNEL(ch) | (is_2G ? band2G : band5G) |
			    band_bw;
...
void wl_update_roamscan_cache_by_band(struct net_device *dev, int band)
{
	...
	for (i = 0; i < chanlist_before.n; i++) {
		...
		bool is_2G =
		    ioctl_ver == 1 ? LCHSPEC_IS2G(chspec) : CHSPEC_IS2G(chspec);
		bool is_5G =
		    ioctl_ver == 1 ? LCHSPEC_IS5G(chspec) : CHSPEC_IS5G(chspec);
