			...
			struct msm_rpmstats_private_data *prvdata)
{
	...
	for (m = 0; m < 2; m++) {
		...
		for (n = 0; n < prvdata->subsystem_num_records; n++) {
			data_subsystem.subsystem_name = msm_subsystem_stats_read_long_register(reg_subsystem, n,
					offsetof(struct msm_rpm_subsystem_stats_data, subsystem_name));
			data_subsystem.status = msm_subsystem_stats_read_long_register(reg_subsystem, n,
					offsetof(struct msm_rpm_subsystem_stats_data, status));
			data_subsystem.count = msm_subsystem_stats_read_long_register(reg_subsystem, n,
...
void system_sleep_status_print_enabled(void)
{
	...
	for (m = 0; m < 2; m++) {
		...
		for (n = 0; n < SUBSYSTEM_STATS_NUM_REC; n++) {
			subsystem_name = msm_subsystem_stats_read_long_register(reg_subsystem, n,
					offsetof(struct msm_rpm_subsystem_stats_data, subsystem_name));
			subsystem_status = msm_subsystem_stats_read_long_register(reg_subsystem, n,
					offsetof(struct msm_rpm_subsystem_stats_data, status));
			subsystem_count = msm_subsystem_stats_read_long_register(reg_subsystem, n,
					offsetof(struct msm_rpm_subsystem_stats_data, count));
			type[4] = 0;
