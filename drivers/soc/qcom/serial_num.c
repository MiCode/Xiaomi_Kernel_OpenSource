...
static void sn_create_proc(void)
{
	...
	entry = proc_create("serial_num", 0555 /* default mode */,
			NULL /* parent dir */, &sn_fops);
	entry = proc_create("fuse_state", 0555 /* default mode */,
...
static int sn_fuse_probe(struct platform_device *pdev)
{
	...
	sn_create_proc();
	dev_info(dev, "serial num: %x debugcpuid\n", sn);
	dev_info(drvdata->dev, "fuse state: 0x%x,0x%x,0x%x debugfuse\n", fuse_state1, fuse_state2, fuse_state3);
