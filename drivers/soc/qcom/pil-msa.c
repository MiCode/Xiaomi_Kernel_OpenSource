...
static int pil_msa_wait_for_mba_ready(struct q6v5_data *drv)
{
	...
	val = pil_is_timeout_disabled() ? 0 : pbl_mba_boot_timeout_ms * 1000;
...
int __pil_mss_deinit_image(struct pil_desc *pil, bool err_path)
{
	...
	s32 status;
	u64 val = pil_is_timeout_disabled() ? 0 : pbl_mba_boot_timeout_ms * 1000;
				  ...
				  size_t size)
{
	...
	s32 status;
	int ret;
	u64 val = pil_is_timeout_disabled() ? 0 : modem_auth_timeout_ms * 1000;
	struct device *dma_dev = drv->mba_mem_dev_fixed ?: &drv->mba_mem_dev;
