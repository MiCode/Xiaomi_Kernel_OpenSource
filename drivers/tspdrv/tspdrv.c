	...
	case TSPDRV_SET_RUNTIME_RECORD_FLAG:
		{
			long nRecordFlag;
			if (0 !=
			    copy_from_user((void *)&nRecordFlag,
					   (const void __user *)arg,
					   sizeof(long))) {
				/* Error copying the data */
	...
	case TSPDRV_SET_RUNTIME_RECORD_BUF_SIZE:
		{
			...
			if (0 !=
			    copy_from_user((void *)&nRecorderBufSize,
					   (const void __user *)arg,
					   sizeof(long))) {
				/* Error copying the data */
	...
	case TSPDRV_SET_DEVICE_PARAMETER:
		{
			...
			if (0 !=
			    copy_from_user((void *)&deviceParam,
					   (const void __user *)arg,
					   sizeof(deviceParam))) {
				/* Error copying the data */
			...
			case VIBE_KP_CFG_FREQUENCY_PARAM6:
				if (0 >
				    ImmVibeSPI_ForceOut_SetFrequency
