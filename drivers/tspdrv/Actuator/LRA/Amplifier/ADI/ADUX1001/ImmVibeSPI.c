...
{
	if (0 !=
	    down_timeout(&g_hSemaphoreAmpDisableEnable,
			 msecs_to_jiffies(AMP_ENABLE_DISABLE_TIMEOUT_MS))) {
		DbgOut((DBL_ERROR,
			"AmpDisableWorkQueueHandler: down_timeout timed out.\n"));
		return;
	}
		...
		if (g_nTrailingNegativeForce == 0
			...
			if (g_workqueueStruct) {
				...
				queue_delayed_work(g_workqueueStruct,
						   &g_AmpDisableHandler,
						   msecs_to_jiffies
						   (WAIT_TIME_BEFORE_DISABLE_AMP_MS
						    -
						    (5 *
						     g_nTrailingNegativeForce)));
			}
		}
...
static void AmpEnable(VibeUInt8 nActuatorIndex)
{
	...
	if (0 !=
	    down_timeout(&g_hSemaphoreAmpDisableEnable,
			 msecs_to_jiffies(AMP_ENABLE_DISABLE_TIMEOUT_MS))) {
		DbgOut((DBL_ERROR,
			"ImmVibeSPI_ForceOut_AmpEnable: down_timeout timed out.\n"));
	}
...
static int calibrate_adux1001(struct adux1001_calib_data *calib_data)
{
	...
	g_i2cBuf[0] = 0x1C;
	i2c_transfer(g_pTheClient->adapter, msg, 2);
	while ((calibrationTimeoutCycles--) > 0
	       && ((g_i2cBuf[0] & 0x01) == 0x01)) {
...
*/
	if (0 > get_calibdata(&g_calib_data)) {
		...
		if (store_calibdata(&g_calib_data) < 0) {
			DbgOut((DBL_ERROR,
				"ADUX1001 storing calibration data failed.\n"));
		}
	}
#endif
...
*/
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_SetSamples(VibeUInt8
							nActuatorIndex,
							VibeUInt16
							nOutputSignalBitDepth,
							VibeUInt16
							nBufferSizeInBytes,
							VibeInt8 *
							pForceOutputBuffer)
{
	...
	if ((0 == nActuatorIndex) && (8 == nOutputSignalBitDepth)
	    && (1 == nBufferSizeInBytes)) {
		...
		if (force == 0) {
			...
			if (force >= 10) {	/* 127*.08, to ensure we stay out of the unstable zone */
				...
				unsigned int duty =
				    ((unsigned int)force *
				     (unsigned int)PWM_PERIOD) >> 7;
				pwm_config(g_pPWMDev, (PWM_PERIOD - duty),
					   PWM_PERIOD);
			} else {	/* 0 < nForce < 10 */
				 ...
				 */
				pwm_config(g_pPWMDev,
					   PWM_PERIOD - PWM_BRAKING_MAGNITUDE,
					   PWM_PERIOD);
			}
		}
	} else
...
*/
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_SetFrequency(VibeUInt8
							  nActuatorIndex,
							  VibeUInt16
							  nFrequencyParameterID,
							  VibeUInt32
							  nFrequencyParameterValue)
{
...
*/
IMMVIBESPIAPI VibeStatus ImmVibeSPI_Device_GetName(VibeUInt8 nActuatorIndex,
						   char *szDevName, int nSize)
{
	...
	sprintf(szDevName, "%s %d %s", szDeviceName, VIBE_EDITION_LEVEL,
		g_szFWVersion);
