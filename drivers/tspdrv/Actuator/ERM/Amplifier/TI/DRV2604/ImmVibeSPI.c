			 ...
			 const struct i2c_device_id *id)
{
	...
	do {
		drv2604_write_reg_val(ERM_autocal_sequence,
	...
	} while (((status & DIAG_RESULT_MASK) == AUTO_CAL_FAILED)
		 && (nCalibrationCount < MAX_AUTOCALIBRATION_ATTEMPT));

	/* Check result */
	if ((status & DIAG_RESULT_MASK) == AUTO_CAL_FAILED) {
		DbgOut((DBL_ERROR,
			"drv2604 auto-calibration failed after %d attempts.\n",
			nCalibrationCount));
	} else {
...
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_Initialize(void)
{
	...
	if (adapter) {
		...
		if (client) {
			...
			DbgOut((DBL_VERBOSE,
				"drv2605: Cannot create new device.\n"));
			return VIBE_E_FAIL;
		}
