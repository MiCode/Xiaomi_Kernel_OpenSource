...
static ssize_t gpio_jingshang_get_sat_1p1_en(struct device *device,
		struct device_attribute *attr, char *buf)
{
	return gpio_jingshang_get_general(device, JINGSHANG_GPIO_SAT_1P1_EN, buf);
}

static ssize_t gpio_jingshang_set_sat_1p1_en(struct device *device,
                              struct device_attribute *attr,
                              const char *buf, size_t count)
{
	return gpio_jingshang_set_general(device, JINGSHANG_GPIO_SAT_1P1_EN, buf, count);
}

static DEVICE_ATTR(sat_1p1_en, JINGSHANG_DEV_MODE_MSK, gpio_jingshang_get_sat_1p1_en, gpio_jingshang_set_sat_1p1_en);
	...
	const char* gpio_name)
{
	...
	if(idx == JINGSHANG_GPIO_SIM_CTL0) {
   ...
   /*	if (tt_voltage_type == TT_SOC_VDD) {
		...
		if (tt_voltage_type == TT_SOC_VDD) {
			if(jingshang_data->data) {
				gpio_num = jingshang_data->data[JINGSHANG_GPIO_SAT_1P1_EN].gpio_num;
