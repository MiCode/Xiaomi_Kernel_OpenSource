...
static ssize_t sdc_status_show_pin(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int len = 0;
	unsigned int bufLen = PAGE_SIZE;
	...
	sdc_status = mtk_gpio_get_in(pctl->chip, pctl->dbg_start + 1);
...
static ssize_t sdc_status_store_pin(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	pr_debug("[pinctrl] Err: Don't support!\n");
	return -EINVAL;
}

static DEVICE_ATTR(sdc_status, 0664, sdc_status_show_pin, sdc_status_store_pin);
