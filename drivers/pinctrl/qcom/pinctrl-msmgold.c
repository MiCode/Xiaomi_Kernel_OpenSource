...
static const struct pinctrl_pin_desc msmgold_pins[] = {
...
static const struct msm_function msmgold_functions[] = {
...
static const struct msm_pingroup msmgold_groups[] = {
...
static const struct msm_pinctrl_soc_data msmgold_pinctrl = {
	.pins = msmgold_pins,
	.npins = ARRAY_SIZE(msmgold_pins),
	.functions = msmgold_functions,
	.nfunctions = ARRAY_SIZE(msmgold_functions),
	.groups = msmgold_groups,
	.ngroups = ARRAY_SIZE(msmgold_groups),
	.ngpios = 134,
};

static int msmgold_pinctrl_probe(struct platform_device *pdev)
{
	return msm_pinctrl_probe(pdev, &msmgold_pinctrl);
}

static const struct of_device_id msmgold_pinctrl_of_match[] = {
	{ .compatible = "qcom,msmgold-pinctrl", },
	{ },
};

static struct platform_driver msmgold_pinctrl_driver = {
	.driver = {
		.name = "msmgold-pinctrl",
		.owner = THIS_MODULE,
		.of_match_table = msmgold_pinctrl_of_match,
	},
	.probe = msmgold_pinctrl_probe,
	.remove = msm_pinctrl_remove,
};

static int __init msmgold_pinctrl_init(void)
{
	return platform_driver_register(&msmgold_pinctrl_driver);
}
arch_initcall(msmgold_pinctrl_init);

static void __exit msmgold_pinctrl_exit(void)
{
	platform_driver_unregister(&msmgold_pinctrl_driver);
}
module_exit(msmgold_pinctrl_exit);

MODULE_DESCRIPTION("QTI msmgold pinctrl driver");
MODULE_LICENSE("GPL v2");
MODULE_DEVICE_TABLE(of, msmgold_pinctrl_of_match);
