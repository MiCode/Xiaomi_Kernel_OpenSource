...
int lm3533_init(struct lm3533 *lm3533)
{
	...
	{
		//bl
		lm3533_write(lm3533, 0x10, 0);
		lm3533_write(lm3533, 0x1a, 0x02);
		lm3533_write(lm3533, 0x1f, 0x12);
		lm3533_write(lm3533, 0x27, 0x01);//only bank A
		lm3533_write(lm3533, 0x2c, 0x0e);
		if(pbl_data != NULL)
		{
			lm3533_write(lm3533, 0x14, pbl_data->pwm);
		}
		//button_bl
		lm3533_write(lm3533, 0x11, 0);
		lm3533_write(lm3533, 0x16, 0);
		lm3533_write(lm3533, 0x21, 0x0);
		lm3533_write(lm3533, 0x26, 0x4); //1x gain
		lm3533_write(lm3533, 0x27, 0x05); //bank A&C
		dev_err(lm3533->dev, "In %s\n",__func__);
	}
	else
	{
