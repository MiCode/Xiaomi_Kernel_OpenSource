...
static int mt6377_get_cali_data(struct device *dev, struct pmic_tz_data *tz_data)
{
	size_t len = 0;
	unsigned short *efuse_buff;
	struct nvmem_cell *cell_1;
	...
	cell_1 = devm_nvmem_cell_get(dev, "mt6377_e_data");
	if (IS_ERR(cell_1)) {
		dev_info(dev, "Error: Failed to get nvmem cell %s\n",
			"mt6377_e_data");
		return PTR_ERR(cell_1);
	...
	tz_data->adc_cali_en = (efuse_buff[0] & BIT(0));
	if (tz_data->adc_cali_en == 0)
		goto out;

	cali_data[0].o_vts = (efuse_buff[1] & GENMASK(7, 0))
		| (efuse_buff[1] & GENMASK(12, 8));
	cali_data[1].o_vts = (efuse_buff[2] & GENMASK(7, 0))
		| (efuse_buff[2] & GENMASK(12, 8));
	cali_data[2].o_vts = (efuse_buff[3] & GENMASK(7, 0))
		| (efuse_buff[3] & GENMASK(12, 8));
	cali_data[3].o_vts = (efuse_buff[4] & GENMASK(7, 0))
		| (efuse_buff[4] & GENMASK(12, 8));

	tz_data->degc_cali = ((efuse_buff[0] & GENMASK(6, 1)) >> 1);
	tz_data->o_slope_sign = ((efuse_buff[0] & BIT(8)) >> 8);
	tz_data->o_slope = ((efuse_buff[0] & GENMASK(14, 9)) >> 9);
	tz_data->id = ((efuse_buff[0] & BIT(15)) >> 15);
...
out:
	for (i = 0; i < tz_data->sensor_num; i++)
		dev_info(dev, "[pmic6377_debug] tz_id=%d, o_vts = 0x%x\n", i, cali_data[i].o_vts);

	dev_info(dev, "[pmic6377_debug] degc_cali= 0x%x\n", tz_data->degc_cali);
	dev_info(dev, "[pmic6377_debug] adc_cali_en        = 0x%x\n", tz_data->adc_cali_en);
	dev_info(dev, "[pmic6377_debug] o_slope        = 0x%x\n", tz_data->o_slope);
	dev_info(dev, "[pmic6377_debug] o_slope_sign        = 0x%x\n", tz_data->o_slope_sign);
	dev_info(dev, "[pmic6377_debug] id        = 0x%x\n", tz_data->id);
	kfree(efuse_buff);
...
static struct pmic_tz_cali_data mt6377_cali_data[] = {
	[0] = {
		.cali_factor = 1863,
		.o_vts = 1600,
		.iio_chan_name = "pmic6377_ts1",
	},
	[1] = {
		.cali_factor = 1863,
		.o_vts = 1600,
		.iio_chan_name = "pmic6377_ts2",
	},
	[2] = {
		.cali_factor = 1863,
		.o_vts = 1600,
		.iio_chan_name = "pmic6377_ts3",
	},
	[3] = {
		.cali_factor = 1863,
		.o_vts = 1600,
		.iio_chan_name = "pmic6377_ts4",
	}
};
...
static struct pmic_tz_data mt6377_pmic_tz_data = {
	.degc_cali = 50,
	.adc_cali_en = 0,
	.o_slope = 0,
	.o_slope_sign = 0,
	...
	.sensor_num = 4,
	.pullup_volt = 1840,
	.cali_data = mt6377_cali_data,
	.get_cali_data = mt6377_get_cali_data,
};
...
static const struct of_device_id pmic_temp_of_match[] = {
	...
	{
		.compatible = "mediatek,mt6377-pmic-temp",
		.data = (void *)&mt6377_pmic_tz_data,
	},
	{},
};
