...
struct mtk_i2c {
	...
	bool hw_init_pre;		/* i2c hw init before resume */
	bool hw_init_flag;		/* every i2c bus need hw init after resume */
			    ...
			    struct i2c_msg msgs[], int num)
{
	...
	if (i2c->hw_init_pre || i2c->hw_init_flag) {
		mtk_i2c_init_hw(i2c);
		if (!i2c->hw_init_pre)
			i2c->hw_init_flag = false;
	}
...
static int mtk_i2c_suspend(struct device *dev)
{
	...
	i2c->hw_init_pre = true;
	i2c->hw_init_flag = true;
...
static int mtk_i2c_resume(struct device *dev)
{
	...
	i2c->hw_init_pre = false;

	return 0;
}

static const struct dev_pm_ops mtk_i2c_pm = {
	.suspend = mtk_i2c_suspend,
	.resume = mtk_i2c_resume,
};
#endif
