...
static int mt_i2c_do_transfer(struct mt_i2c *i2c)
{
	...
	if (speed_hz > MAX_FS_MODE_SPEED)
		/*  i2c does not allow slave to hold the SCL line low when the
		 *   transmission rate over 400k. it will reduce the real
		 *   transmission rate.
		 */
	...
	if (isDMA == true && (!i2c->is_ccu_trig)) {
		...
		if (i2c->op == I2C_MASTER_RD) {
			i2c_writel_dma(I2C_DMA_INT_FLAG_NONE,
				i2c, OFFSET_INT_FLAG);
			if (i2c->dev_comp->dma_ver == DMA_CONFIG_VERSION1)
...
static int mt_i2c_probe(struct platform_device *pdev)
{
	...
	if (i2c->dev_comp->dma_support == DMA_SUPPORT_8G_MEMORY) {
		if (dma_set_mask(&pdev->dev, DMA_BIT_MASK(33))) {
			dev_info(&pdev->dev, "dma_set_mask return error.\n");
			return -EINVAL;
		}
	} else if (i2c->dev_comp->dma_support == DMA_SUPPORT_64G_MEMORY) {
