...
static int msm_11ad_probe(struct platform_device *pdev)
{
	...
	ctx->pcie_path = of_icc_get(&pdev->dev, NULL);
	if (IS_ERR(ctx->pcie_path)) {
		rc = PTR_ERR(ctx->pcie_path);
		if (rc != -EPROBE_DEFER) {
			dev_err(ctx->dev,
				"Unable to read bus-scaling from DT\n");
		}
		rc = -EINVAL;
		goto out_module;
	}
...
static int ops_bus_request(void *handle, u32 kbps /* KBytes/Sec */)
{
	...
	/* TODO: use new icc API to complete bw voting */
	rc = icc_set_bw(ctx->pcie_path, 500, 800);
	if (rc)
		dev_err(ctx->dev,
