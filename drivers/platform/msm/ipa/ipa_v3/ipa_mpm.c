...
struct ipa_mpm_context {
	...
	struct mutex mutex;
	int ipa_mpm_pcie_clock;
	int ipa_mpm_ipa_clock;
	...
	int probe_id)
{
	...
	if (vote == CLK_ON) {
		if ((!ipa_mpm_ctx->ipa_mpm_pcie_clock)
			...
			ipa_mpm_ctx->ipa_mpm_pcie_clock = 1;
		...
		if ((ipa_mpm_ctx->ipa_mpm_pcie_clock) &&
			...
			ipa_mpm_ctx->ipa_mpm_pcie_clock = 0;
		}
...
static void ipa_mpm_vote_unvote_ipa_clk(enum ipa_mpm_clk_vote_type vote)
{
	if (vote == CLK_ON) {
		...
		if ((ipa_mpm_ctx->ipa_mpm_ipa_clock) &&
			...
			ipa_mpm_ctx->ipa_mpm_ipa_clock = 0;
