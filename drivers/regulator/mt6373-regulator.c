...
static int mt6373_regulator_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT6373_MAX_REGULATOR; i++) {
		config.driver_data = &mt6373_regulators[i];
		rdev = devm_regulator_register(&pdev->dev, &mt6373_regulators[i].desc, &config);
		if (IS_ERR(rdev)) {
			ret = PTR_ERR(rdev);
			dev_err(&pdev->dev, "failed to register %s, ret=%d\n",
				mt6373_regulators[i].desc.name, ret);
			continue;
		}
		irq = platform_get_irq_byname_optional(pdev, mt6373_regulators[i].desc.name);
		if (irq < 0)
			continue;
		ret = devm_request_threaded_irq(&pdev->dev, irq, NULL,
						mt6373_oc_irq,
						IRQF_TRIGGER_HIGH,
						mt6373_regulators[i].desc.name,
						rdev);
		if (ret) {
