...
#define pm8008_debug(reg, message, ...) \
	pr_info("%s: " message, (reg)->rdesc.name, ##__VA_ARGS__)
...
static const struct regmap_config pm8008_regmap_config = {
        .reg_bits = 16,
        .val_bits = 8,
};
...
static int pm8008_regulator_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	regmap = devm_regmap_init_i2c(client, &pm8008_regmap_config);
	if (IS_ERR(regmap)) {
		pr_err("PM8008 failed to allocate regmap\n");
		return PTR_ERR(regmap);
	}

	rc = pm8008_parse_regulator(regmap, &client->dev);
	if (rc < 0) {
...
#if 0 //Test Code
	/*val[0] = 0;
	rc = pm8008_write(regmap,0x4546,&val[0],1);
	if (rc < 0)
		pr_err("failed to read 0x%04x\n", 0x4546);

	val[1] = 0xF0;
	rc = pm8008_write(regmap,0x4540,&val[1],1);
	if (rc < 0)
		pr_err("failed to read 0x%04x\n", 0x4540);

	val[2] = 0x0a;
	rc = pm8008_write(regmap,0x4541,&val[2],1);
	if (rc < 0)
		pr_err("failed to read 0x%04x\n", 0x4541);

	val[3] = 0x80;
	rc = pm8008_write(regmap,0x4546,&val[3],1);
	if (rc < 0)
		pr_err("failed to read 0x%04x\n", 0x4546);


	rc = regmap_bulk_read(regmap, 0x4540, val, 2);
	if (rc < 0)
		pr_err("failed to read 0x%04x\n", 0x4540);

	pr_info("zyk read 0x100~0x105 %d %d %d %d %d",val[0],val[1],val[2],val[3],val[4]);
	*/
#endif
}
...
static int pm8008_pmic_chip_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	int rc = 0;
	unsigned char val[5] = {0};
	...
	chip->regmap = devm_regmap_init_i2c(client, &pm8008_regmap_config);
	if (IS_ERR(regmap)) {
		pr_err("PM8008 failed to allocate regmap\n");
		return PTR_ERR(regmap);
	}
/*	ppinctrl = devm_pinctrl_get(&client->dev);
	if (IS_ERR(ppinctrl)) {
		pr_err("%s : Cannot find pm8008 pinctrl!\n", __func__);
		return -EINVAL;
	}
...
#if 0 //debug code for read chip id
	rc = regmap_bulk_read(chip->regmap, 0x100, val, 5);
	if (rc < 0)
		pr_err("failed to read 0x%04x\n", 0x100);
...
static int pm8008_chip_remove(struct i2c_client *client)
{
	struct pm8008_chip *chip = i2c_get_clientdata(client);
	int rc;
...
MODULE_DEVICE_TABLE(i2c, pm8008_regulator_i2c_device_id);

static struct i2c_driver pm8008_regulator_i2c_driver = {
	.driver		= {
		.name		= "qcom,pm8008-regulator",
	.owner		= THIS_MODULE,
		.of_match_table	= pm8008_regulator_match_table,
	},
	.probe		= pm8008_regulator_i2c_probe,
	.id_table	= pm8008_regulator_i2c_device_id,
};
MODULE_DEVICE_TABLE(i2c, pm8008_regulator_i2c_device_id);
module_i2c_driver(pm8008_regulator_i2c_driver);
...
static const struct i2c_device_id pm8008_chip_i2c_device_id[] = {
	{ "qcom,pm8008-chip", 0 },
	{ },
};

static struct i2c_driver pm8008_chip_i2c_driver = {
	.driver		= {
		.name		= "qcom,pm8008-chip",
		.owner		= THIS_MODULE,
		.of_match_table	= pm8008_chip_match_table,
	},
	.probe		= pm8008_pmic_chip_i2c_probe,
	.remove		= pm8008_chip_remove,
	.id_table	= pm8008_chip_i2c_device_id,
};
MODULE_DEVICE_TABLE(i2c, pm8008_chip_i2c_device_id);
module_i2c_driver(pm8008_chip_i2c_driver);

MODULE_DESCRIPTION("MTK Port PM8008 PMIC Regulator Driver");
MODULE_LICENSE("GPL v2");
MODULE_AUTHOR("zhaiyankun@xiaomi.com");
