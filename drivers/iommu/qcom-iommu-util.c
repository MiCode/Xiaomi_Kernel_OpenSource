...
/*
 * Returns the IOVA range that will be mapped by the client driver that uses
 * the fastmap DMA allocator. The range is generated by inspecting both the
 * qcom,iommu-dma-addr-pool and qcom,iommu-geometry properties. There are a few
 * cases that can arise, and here is how they are interpreted:
 *
 * Case 1:
 *
 * Neither qcom,iommu-dma-addr-pool or qcom,iommu-geometry is present. In this
 * case, we do not know what ranges the client will use, so consider the range
 * as the entire fastmap IOVA range [0, 4 GB).
 *
 * Case 2:
 *
 * qcom,iommu-dma-addr-pool is present, but qcom,iommu-geometry is not present.
 * In this case, we know the DMA range, but we do not know if the client may
 * use the IOMMU APIs directly to map memory outside of the DMA range, so
 * use the entire fastmap IOVA range [0, 4 GB).
 *
 * Case 3:
 *
 * qcom,iommu-geometry is present, but qcom,iommu-dma-addr-pool is not present.
 * In this case, we do not know if the client will use the DMA APIs, which may
 * allocate IOVAs outside of the range specified in qcom,iommu-geometry,
 * so use the entire fastmap IOVA range [0, 4 GB).
 *
 * Case 4:
 *
 * Both qcom,iommu-geometry and qcom,iommu-dma-addr-pool are present. In this
 * case, we need to specify the range as the union of both ranges, so the range
 * becomes: [min(dma_base, geometry_base),
