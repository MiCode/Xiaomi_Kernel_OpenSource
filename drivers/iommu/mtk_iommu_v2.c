				...
				unsigned int larbid, unsigned int portid)
{
	...
	for (i = 0; i < MTK_IOVA_DOMAIN_COUNT; i++) {
		if (mtk_domain_array[i].port_mask[larbid] &
			...
			pr_notice("%s, %d, larb%d, port%d belonging to group%d\n",
				  __func__, __LINE__,
				  larbid, portid, domain_id);
#endif
			break;
		}
	}
...
static int mtk_dump_debug_reg_info(struct mtk_iommu_data *data)
{
	...
	pr_notice("m4u dump debug register\n");
	for (i = 0; i < MTK_IOMMU_DEBUG_REG_NR; i++) {
		pr_notice("0x%x = 0x%x\n", REG_MMU_DBG(i),
			  readl_relaxed(base + REG_MMU_DBG(i)));
	}
...
static int mtk_dump_rs_sta_info(struct mtk_iommu_data *data, int mmu)
{
	...
	if (!data ||
	    mmu >= MTK_MMU_NUM_OF_IOMMU(data->m4uid)) {
		pr_notice("%s, %d, invalid data, mmu:%d\n",
			  __func__, __LINE__, mmu);
		return -1;
	}
	base = data->base;

	pr_notice("m4u dump RS status register -- mmu%d\n", mmu);
	for (i = 0; i < MTK_IOMMU_RS_COUNT; i++) {
		pr_notice("0x%x = 0x%x\n", REG_MMU_RS_STA(mmu, i),
			  readl_relaxed(base + REG_MMU_RS_STA(mmu, i)));
	}
...
int mtk_dump_reg_for_hang_issue(unsigned int m4u_id)
{
	...
	if (!data || data->base == 0) {
		pr_notice("base is NULL\n");
		return 0;
	}
	base = data->base;
	mtk_switch_secure_debug_func(m4u_id, 1);
	...
	/* dump five times*/
	for (cnt = 0; cnt < 5; cnt++) {
		pr_notice("REG_MMU_STA(0x08)	 = 0x%x\n",
			  readl_relaxed(base + REG_MMU_STA));
	...
	mtk_switch_secure_debug_func(m4u_id, 0);
	return 0;
}
...
int mtk_iommu_dump_reg(int m4u_id, unsigned int start, unsigned int end)
{
	...
	for (i = start; i < end; i += 16) {
		pr_info("0x%x=0x%x, 0x%x=0x%x, 0x%x=0x%x, 0x%x=0x%x\n",
			(i + 4 * 0), readl_relaxed(base + i + 4 * 0),
			(i + 4 * 1), readl_relaxed(base + i + 4 * 1),
			(i + 4 * 2), readl_relaxed(base + i + 4 * 2),
			(i + 4 * 3), readl_relaxed(base + i + 4 * 3));
	}
...
static irqreturn_t mtk_iommu_isr(int irq, void *dev_id)
{
	...
	for (i = 0; i < MTK_MMU_NUM_OF_IOMMU(m4uid); i++) {
		if (int_state_l2 & (F_INT_MMU_MAIN_MSK(i) |
	...
	if (int_state_l2 & F_INT_L2_TABLE_WALK_FAULT) {
		...
		fault_iova = (u64)(regval & F_MMU_FAULT_VA_BIT31_12) |
				(u64)((regval & F_MMU_FAULT_VA_B34_32) << 23);
		layer = regval & 1;
		mmu_aee_print(
	...
	if (int_state & F_INT_TRANSLATION_FAULT(slave_id)) {
		...
		if (WARN_ON(mtk_iommu_get_larb_port(
				int_id, &fault_larb,
				&fault_port)))
			return 0;
		...
		fault_pa |= (u64)((regval &
					F_MMU_FAULT_PA_B34_32) << 26);
		pr_notice("fault_pa=%llx\n", fault_pa);
#ifdef IOMMU_DESIGN_OF_BANK
		if (m4uid >= APU_IOMMU_INDEX) {
			fault_port = M4U_PORT_L21_APU_FAKE_DATA;
			fault_larb = 21;
			is_vpu = true;
		} else {
			is_vpu = false;
		...
		if (report_iommu_fault(domain, data->dev, fault_iova,
			...
			dev_err_ratelimited(
				data->dev,
				"iommu fault type=0x%x iova=0x%llx pa=0x%llx larb=%d port=%d layer=%d %s\n",
			...
			struct mtk_iommu_data *data, struct device *dev)
{
	...
	pr_notice("%s, %d, dev:%s allocated the %d group:%p, domain:%p start:0x%llx, end:0x%llx\n",
...
static int mtk_iommu_reg_backup(struct mtk_iommu_data *data)
{
	...
	reg->dummy = readl_relaxed(base +
					   REG_MMU_DUMMY);
	if (g_secure_status) {
		mtk_iommu_atf_call(IOMMU_ATF_SECURITY_BACKUP,
...
static int mtk_iommu_reg_restore(struct mtk_iommu_data *data)
{
	...
	if (g_secure_status) {
		mtk_iommu_atf_call(IOMMU_ATF_SECURITY_RESTORE,
				   data->m4uid, MTK_IOMMU_BANK_COUNT);
