...
int dmabuf_rbtree_dbg_add_cb(const struct dma_buf *dmabuf, void *priv)
{
	...
	node = dmabuf_rbtree_dbg_add_return(dmabuf, priv);
...
static unsigned long dmabuf_dbg_rbtree_clear(struct dump_fd_data *fddata)
{
			       ...
			       struct seq_file *s)
{
	...
	/* heap is NULL, try all heap to dump, matched heap will dump & return 0 */
...
static int dma_heap_buf_dump_cb(const struct dma_buf *dmabuf, void *priv)
{
	...
	dmabuf_dump(s, "inode:%-8d size:%-8zu count:%-2ld cache_sg:%-1d exp:%s\tname:%s\n",
...
static struct dump_fd_data *
dmabuf_rbtree_add_all(struct dma_heap *heap,
		      struct seq_file *s)
{
	struct task_struct *p;
	...
	for_each_process(p) {
		...
		if (res)
			dmabuf_dump(s, "[E] %s#%d pid:%d(%s) res:%d\n",
...
static void dmabuf_rbtree_dump_stats(struct dump_fd_data *fddata)
{
...
static void dmabuf_rbtree_dump_all(struct dma_heap *heap, unsigned long flag, struct seq_file *s)
{
	...
	dmabuf_dump(s, "\n");
	if (!(flag & HEAP_DUMP_RB_SKIP_BUFFER))
				 ...
				 int flag)
{
	if (!heap) {
		...
		dma_heap_default_show(NULL, s,
				      flag | HEAP_DUMP_HEAP_SKIP_POOL);
		return;
	}
...
{
	dmabuf_rbtree_dump_all(NULL, flag | HEAP_DUMP_RB_SKIP_BUFFER, s);
}
