						...
						int bits_per_word)
{
	...
	/* Sync */
	//pk_log("w--liuyang", (unsigned char *)xfer.tx_buf, len);
	ret = spi_sync(msg.spi, &msg);
	//pk_log("r---liuyang", (unsigned char *)xfer.rx_buf, len);
...
{
	int host_req, mcu_req, mcu_resp, nstandby;
	struct bcm_spi_priv *priv;
	bool skip_sanity;
	...
	ret = request_irq(spi->irq, bcm_irq_handler,
		IRQF_TRIGGER_RISING | IRQF_ONESHOT, "ttyBCM", priv);
	if (ret) {
