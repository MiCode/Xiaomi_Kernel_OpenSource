...
#define PMIC_KEYS_REGS(_deb_reg, _deb_mask, _intsel_reg, _intsel_mask)         \
	{                                                                      \
		.deb_reg = _deb_reg, .deb_mask = _deb_mask,                    \
		.intsel_reg = _intsel_reg, .intsel_mask = _intsel_mask,        \
	}
...
static const struct pmic_regs mt6397_regs = {
	.pwrkey_regs =
		PMIC_KEYS_REGS(MT6397_CHRSTATUS, 0x8, MT6397_INT_RSV, 0x10),
	.homekey_regs =
		PMIC_KEYS_REGS(MT6397_OCSTATUS2, 0x10, MT6397_INT_RSV, 0x8),
	.pmic_rst_reg = MT6397_TOP_RST_MISC,
	.indie_release_irq_mask = 0,
};

static const struct pmic_regs mt6323_regs = {
	.pwrkey_regs = PMIC_KEYS_REGS(MT6323_CHRSTATUS, 0x2,
				      MT6323_INT_MISC_CON, 0x10),
	.homekey_regs =
		PMIC_KEYS_REGS(MT6323_CHRSTATUS, 0x4, MT6323_INT_MISC_CON, 0x8),
	.pmic_rst_reg = MT6323_TOP_RST_MISC,
	.indie_release_irq_mask = 0,
};

static const struct pmic_regs mt6392_regs = {
	.pwrkey_regs = PMIC_KEYS_REGS(MT6392_CHRSTATUS, 0x2,
				      MT6392_INT_MISC_CON, 0x10),
	.homekey_regs =
		PMIC_KEYS_REGS(MT6392_CHRSTATUS, 0x4, MT6392_INT_MISC_CON, 0x8),
...
static int mtk_pmic_key_setup(struct mtk_pmic_keys *keys, const char *propname,
			      struct pmic_keys_info *info, bool wakeup)
{
	int ret;
	...
	ret = devm_request_threaded_irq(
		keys->dev, info->irq, NULL, mtk_pmic_keys_irq_handler_thread,
		IRQF_ONESHOT | IRQF_TRIGGER_HIGH, "mtk-pmic-keys", info);
	if (ret) {
		dev_info(keys->dev, "Failed to request IRQ: %d: %d\n",
		info->irq, ret);
		return ret;
	}
...
static const struct of_device_id of_pmic_keys_match_tbl[] = {
	{
		.compatible = "mediatek,mt6397-keys", .data = &mt6397_regs,
	},
	{
		.compatible = "mediatek,mt6323-keys", .data = &mt6323_regs,
	},
	{
		.compatible = "mediatek,mt6392-keys", .data = &mt6392_regs,
	},
	{
		/* sentinel */
	} };
...
static int mtk_pmic_keys_probe(struct platform_device *pdev)
{
	...
	if (!res) {
		dev_info(&pdev->dev, "no IRQ resource\n");
		return -ENODEV;
	}
	...
	if (!input_dev) {
		dev_info(&pdev->dev, "[PMICKEYS] input allocate device fail.\n");
		return -ENOMEM;
	}
	...
	ret = mtk_pmic_key_setup(keys, "mediatek,pwrkey-code", &keys->pwrkey,
				 true);
	if (ret)
		goto out_dispose_irq;

	ret = mtk_pmic_key_setup(keys, "mediatek,homekey-code", &keys->homekey,
				 false);
	if (ret)
		goto out_dispose_irq;
