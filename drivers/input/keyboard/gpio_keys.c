...
static int gpio_keys_probe(struct platform_device *pdev)
{
 ...
 err_pinctrl:
	if (ddata->key_pinctrl) {
		set_state =
		pinctrl_lookup_state(ddata->key_pinctrl, "tlmm_gpio_key_suspend");
		if (IS_ERR(set_state))
			pr_err("cannot get gpiokey pinctrl sleep state\n");
		else
...
static int gpio_keys_suspend(struct device *dev)
{
	...
	if (ddata->key_pinctrl) {
		...
		if (ret) {
			pr_err("failed to put the pin in suspend state\n");
			return ret;
		}
	}
...
static int gpio_keys_resume(struct device *dev)
{
	...
	if (ddata->key_pinctrl) {
		error = gpio_keys_pinctrl_configure(ddata, false);
		if (error) {
			pr_err("failed to put the pin in resume state\n");
			return error;
		}
	}
