    ...
    if (evdata->data && mdata) {
        ...
        if (event == MI_DRM_EARLY_EVENT_BLANK) {
            if(*blank == MI_DRM_BLANK_UNBLANK) {
                dbgprint(ALERT_LEVEL, "keyboard_drm_notifier_callback keyboard early resume");
                flush_workqueue(mdata->event_wq);
                queue_work(mdata->event_wq, &mdata->resume_work);
            } else if (*blank == MI_DRM_BLANK_POWERDOWN) {
                ret = Nanosic_state_write(SCREEN_OFF);
                dbgprint(ALERT_LEVEL, "keyboard_drm_notifier_callback keyboard early suspend: write screen off:%d\n", ret);
            }
        } else if (event == MI_DRM_EVENT_BLANK) {
            if (*blank == MI_DRM_BLANK_UNBLANK) {
                ret = Nanosic_state_write(SCREEN_ON);
                dbgprint(ALERT_LEVEL, "keyboard_drm_notifier_callback keyboard resume: write screen on:%d\n", ret);
            } else if (*blank == MI_DRM_BLANK_POWERDOWN) {
