...
static int sar_parse_dt(struct device *dev)
{
	...
	sar_int_gpio = of_get_named_gpio(np, "virtualsar,irq-gpio", 0);
	if (sar_int_gpio == 85) {
...
{
	struct virtualsar_data *sdata = dev_id;
	unsigned long irqflags = 0;

	spin_lock_irqsave(&sdata->irq_lock, irqflags);
	//sar_irq_disable(sdata);
	input_data = gpio_get_value(sar_int_gpio);
	/*BSP.SYS 2022.07.14 add for input virtual sar keyevent start*/
	if (0 == input_data)
		gpio_key.code = 249;
	else
		gpio_key.code = 250;
	/*BSP.SYS 2022.07.14 add for input virtual sar keyevent end*/
...
static s8 sar_request_irq(struct virtualsar_data *sdata)
{
	s32 ret = -1;
...
static s8 sar_request_io_port(struct virtualsar_data *sdata)
{
	s32 ret = 0;
...
static s8 sar_request_input_dev(struct virtualsar_data *sdata)
{
	s8 ret = -1;
	...
	/*BSP.SYS 2022.07.14 add for input virtual sar keyevent start*/
	input_set_capability(sdata->input_dev, EV_KEY, 249);
	input_set_capability(sdata->input_dev, EV_KEY, 250);
	/*BSP.SYS 2022.07.14 add for input virtual sar keyevent end*/
	return 0;
}
...
{
	int ret = 0;
	struct virtualsar_data *sdata;//fusb30x_chip* chip;
