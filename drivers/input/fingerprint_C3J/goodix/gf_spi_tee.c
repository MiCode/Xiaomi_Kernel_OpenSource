...
static void freq_release(struct work_struct *work)
{
	...
	if (atomic_read(&boosted) == 1) {
		...
		update_userlimit_cpu_freq(CPU_KIR_FP, cluster_num,
					freq_to_set);
		atomic_dec(&boosted);
	}
}
...
static int gf_probe(struct spi_device *spi)
{
	...
	*/
	...
	if (NULL == proc_entry) {
		pr_err("GF5658 Couldn't create proc entry!");
		return -ENOMEM;
	} else {
		pr_err("GF5658 Create proc entry success!");
	}
