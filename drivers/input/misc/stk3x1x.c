...
#define DRIVER_VERSION  "3.8.0 20141208"
...
union stk_ges_operation stk_ges_op[10] = {
	...
	static int PS_THDL = 0;
	static int PS_THDH = 0;
...
static int calibration_psnesor(struct stk3x1x_data *ps_data);
#endif
...
static int32_t stk3x1x_init_all_reg(struct stk3x1x_data *ps_data)
{
	...
	ret = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_LEDCTRL_REG, 0xbf);
	if (ret < 0) {
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
	}
...
#ifdef MP_CALIBRATION
static int calibration_get_crosstalk_ps(struct stk3x1x_data *ps_data)
{
	int16_t reading, flag_reg, count = 0;
	...
	while (count < 5) {
		...
		if (flag_reg < 0) {
			printk(KERN_INFO "%s: read flag error %d\n", __func__, flag_reg);
			return -EPERM;
		}
		...
		reading = stk3x1x_get_ps_reading(ps_data);
		if (reading < 0) {
			printk(KERN_INFO "%s: read ps error %d\n", __func__, reading);
			return -EPERM;
		}

		printk(KERN_INFO "%s: read ps data[%d] = %d\n", __func__, count, reading);
		count = count + 1;
		sum = sum + reading;
	}

	crosstalk = sum/5;
	printk(KERN_INFO "%s: crosstalk = %d\n", __func__, crosstalk);
	return crosstalk;
...
static int calibration_psnesor(struct stk3x1x_data *ps_data)
{
	...
	int ps_val;
	msleep(60);
	ps_val = calibration_get_crosstalk_ps(ps_data);

	ps_data->cali_ct = ps_val;

	ps_data->ps_thd_h = ps_data->cali_ct + ps_data->stk_ht_n_ct;
	ps_data->ps_thd_l = ps_data->cali_ct + ps_data->stk_lt_n_ct;
	PS_THDH = ps_data->ps_thd_h + 200;
	PS_THDL = ps_data->ps_thd_l + 150;
...
#endif	/* #ifdef STK_TUNE0 */
#if 1
static ssize_t stk_show_ps_threshold(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);
...
static ssize_t stk_store_ps_threshold(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	...
	printk(KERN_INFO "stk PS prox_threshold -> %ld\n", val);

	ps_data->ps_thd_h = val;
...
static ssize_t stk_show_ps_hsyteresis_threshold(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);
...
static ssize_t stk_store_ps_hsyteresis_threshold(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	/* If proximity work, then ALS must be enable */
	...
	printk(KERN_INFO "stk PS prox_hsyteresis_threshold -> %ld\n", val);

	ps_data->ps_thd_l = val;
...
static ssize_t stk_show_ps_cali_update_flag(struct device *dev,
				struct device_attribute *attr, char *buf)
{
...
static ssize_t stk_store_ps_cali_update_flag(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	CALI_FLAG = 0;
...
static ssize_t stk_show_ps_raw_adc(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);
	uint32_t reading;
...
static struct device_attribute als_enable_attribute = __ATTR(enable_als_sensor, 0664, stk_als_enable_show, stk_als_enable_store);
...
#endif
#if 1
static struct device_attribute dev_attr_cali_param_1 = __ATTR(cali_param_1, 0664, stk_show_ps_threshold, stk_store_ps_threshold);
static struct device_attribute dev_attr_cali_param_2 = __ATTR(cali_param_2, 0664, stk_show_ps_hsyteresis_threshold, stk_store_ps_hsyteresis_threshold);
static struct device_attribute dev_attr_cali_update = __ATTR(cali_update, 0664, stk_show_ps_cali_update_flag, stk_store_ps_cali_update_flag);
static struct device_attribute dev_attr_raw_adc = __ATTR(raw_adc, 0664, stk_show_ps_raw_adc, NULL);
#endif
...
static const struct attribute_group stk_attr_group = {
		.attrs = stk_attributes,
};

static ssize_t stk_ps_self_caliberate(struct sensors_classdev *sensors_cdev)
{
	struct stk3x1x_data *data = container_of(sensors_cdev, struct stk3x1x_data, ps_cdev);
	int ret;
	ret = calibration_psnesor(data);
	if (ret < 0) {
		printk("PS calibration fali %s\n", __func__);
		return ret;
	} else
		CALI_FLAG = 1;
	return ret;
                        ...
                        const struct i2c_device_id *id)
{
	...
	err = sysfs_create_group(&client->dev.kobj, &stk_attr_group);
	if (err) {
	...
	ps_data->ps_cdev.sensors_self_test = stk_ps_self_caliberate;
