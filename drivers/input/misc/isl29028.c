...
#define ISL29028_DEVICE_ID_MASK				0xF8
#define ISL29028_WRONG_DEVICE_ID			0xC0
...
{
	DUMP_REGISTER(data, ISL29028_ID);
	...
	DUMP_REGISTER(data, ISL29028_ALSIR_TH2);
...
static int update_device(struct isl29028_data *data)
{
	...
	error = i2c_smbus_write_byte_data(client, ISL29028_CONFIGURE,
					get_reg_config(data->prox_period,
					als_active(data),
					prox_active(data),
					data->als_highrange));
	if (error < 0) {
	...
	if (prox_active(data))
		error = i2c_smbus_write_byte_data(client,
					ISL29028_INTERRUPT,
					ISL29028_ACTIVE_INTR);
	else
		error = i2c_smbus_write_byte_data(client,
					ISL29028_INTERRUPT,
					ISL29028_INACTIVE_INTR);
	if (error < 0) {
...
static irqreturn_t isl29028_interrupt(int irq, void *dev)
{
	...
	if (prox_active(data)) {
		...
		input_report_abs(data->prox_input, ABS_MISC, prox);
...
{
	struct isl29028_data *data =
		container_of(to_delayed_work(work), struct isl29028_data,
			 delayed_work);
...
static int setup_prox_input(struct isl29028_data *data)
{
	...
	input_set_abs_params(data->prox_input, ABS_MISC,
			ISL29028_PROX_MISC_MIN, ISL29028_PROX_MISC_MAX, 0,
			0);
...
static int isl29028_get_deviceid(struct isl29028_data *data)
{
	int error = 0;
	unsigned char id;

	error = i2c_smbus_read_byte_data(data->client, ISL29028_ID);
	if (error < 0)
		return error;

	id = (unsigned char)error;
	id &= ISL29028_DEVICE_ID_MASK;
	if (id == ISL29028_WRONG_DEVICE_ID)
		return -ENODEV;
			  ...
			  const struct i2c_device_id *id)
{
	...
	error = isl29028_get_deviceid(data);
	if (error < 0) {
