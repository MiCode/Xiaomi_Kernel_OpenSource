...
static int aw8624_haptic_stop_delay(struct aw8624 *aw8624)
{
	...
	pr_err("%s do not enter standby automatically, reg_val:0x%02x\n", __func__,reg_val);
...
static int16_t aw8624_haptic_effect_strength(struct aw8624 *aw8624)
{
	...
	aw8624->level = (aw8624->play.vmax_mv - 0x3FFF) / 128;
	if( aw8624->level < 0x1E)
		aw8624->level = 0x1E; /*30*/
#endif
	VIB_DEBUG("aw8624->level =0x%x\n", aw8624->level);
	return 0;
}
					 ...
					 unsigned char flag)
{
	...
	if (aw8624->effect_id > aw8624->info.effect_id_boundary)
		return 0;
	VIB_DEBUG("effect_id =%d activate_mode =%d\n", aw8624->effect_id,
...
static int aw8624_rtp_trim_lra_calibration(struct aw8624 *aw8624)
{
	...
	VIB_DEBUG("microsecond:%ld  theory_time = %d\n", aw8624->microsecond,
		  theory_time);
...
static int aw8624_rtp_osc_calibration(struct aw8624 *aw8624)
{
	...
	while (1) {
		...
		if (osc_int_state & AW8624_BIT_SYSINT_DONEI) {
			...
			pr_info
			    ("%s playback done aw8624->rtp_cnt= %d \n",
			     __func__, aw8624->rtp_cnt);
			break;
		}
		...
		if (aw8624->microsecond > OSC_CALIBRATION_T_LENGTH) {
			pr_info
			    ("%s time out aw8624->rtp_cnt %d osc_int_state %02x\n",
...
static void aw8624_rtp_work_routine(struct work_struct *work)
{
	...
	if ((aw8624->effect_id < aw8624->info.effect_id_boundary) &&
		...
		return;
	VIB_DEBUG("effect_id =%d state=%d\n", aw8624->effect_id, aw8624->state);
	...
	if (aw8624->state) {
		...
		VIB_DEBUG("aw8624->rtp_file_num =%d\n", aw8624->rtp_file_num);
...
static int aw8624_haptic_f0_calibration(struct aw8624 *aw8624)
{
	...
	if (aw8624_haptic_get_f0(aw8624)) {
		...
		VIB_DEBUG("f0_cali_step=%d\n", f0_cali_step);
		VIB_DEBUG("f0_limit=%d\n", (int)f0_limit);
		VIB_DEBUG("aw8624->info.f0_pre=%d\n", (int)aw8624->info.f0_pre);
...
static int aw8624_init_f0_calibration(void *arg) {
	struct aw8624 *aw8624 = (struct aw8624*) arg;
	mutex_lock(&aw8624->lock);
...
static int aw8624_haptic_init(struct aw8624 *aw8624)
{
	...
	/* f0 calibration */
#ifndef DISABLE_POWERUP_F0_CALI

	kthread_run(aw8624_init_f0_calibration, aw8624, "f0_calibration");
#endif
...
static void aw8624_vibrator_work_routine(struct work_struct *work)
{
	...
	VIB_FUNC_ENTER();
	VIB_DEBUG("state=%d activate_mode=%d duration=%d\n", aw8624->state,
...
static irqreturn_t aw8624_irq(int irq, void *data)
{
	...
	aw8624_i2c_read(aw8624, AW8624_REG_SYSINT, &reg_val);
	VIB_DEBUG("reg SYSINT=0x%x\n", reg_val);
	aw8624_i2c_read(aw8624, AW8624_REG_DBGSTAT, &dbg_val);
	VIB_DEBUG("reg DBGSTAT=0x%x\n", dbg_val);
	...
	if (reg_val & AW8624_BIT_SYSINT_FF_AEI) {
		...
		if (aw8624->rtp_init) {
			while ((!aw8624_haptic_rtp_get_fifo_afi(aw8624)) &&
			       (aw8624->play_mode == AW8624_HAPTIC_RTP_MODE)
			       && !atomic_read(&aw8624->exit_in_irq)) {
				mutex_lock(&aw8624->rtp_lock);
				VIB_DEBUG
			   ...
			   struct device_node *np)
{
	...
	rc = of_property_read_u32(np, "mtk,play-rate-us", &tmp);
	if (!rc)
	...
	for_each_available_child_of_node(np, child_node) {
		...
		rc = of_property_count_elems_of_size(child_node,
						     ...
						     sizeof(u8));
		if (rc < 0) {
			VIB_DEBUG("Count mtk,wf-pattern property failed !\n");
		} else if (rc == 0) {
			VIB_DEBUG("mtk,wf-pattern has no data\n");
		}
		...
		rc = of_property_read_u8_array(child_node, "mtk,wf-pattern",
					       ...
					       effect->pattern_length);
		if (rc < 0) {
			VIB_DEBUG("Read mtk,wf-pattern property failed !\n");
		}
		...
		rc = of_property_read_u32(child_node, "mtk,wf-play-rate-us",
					  &tmp);
		if (rc < 0)
			VIB_DEBUG("Read mtk,wf-play-rate-us failed !\n");
		else
			effect->play_rate_us = tmp;
		VIB_DEBUG("effect->play_rate_us=%d \n", effect->play_rate_us);

		rc = of_property_read_u32(child_node, "mtk,wf-repeat-count",
					  &tmp);
		if (rc < 0) {
			VIB_DEBUG("Read  mtk,wf-repeat-count failed !\n");
		} else {
		...
		rc = of_property_read_u32(child_node, "mtk,wf-s-repeat-count",
					  &tmp);
		if (rc < 0) {
			VIB_DEBUG("Read  mtk,wf-s-repeat-count failed !\n");
		} else {
	...
	VIB_DEBUG("aw8624->effects_count: %d\n", aw8624->effects_count);
	VIB_DEBUG("aw8624->effect_id_boundary: %d\n",
		  aw8624->info.effect_id_boundary);
	VIB_DEBUG("aw8624->effect_max: %d\n", aw8624->info.effect_max);
	VIB_DEBUG("aw8624->info.cont_drv_lvl: %d\n", aw8624->info.cont_drv_lvl);
	VIB_DEBUG("aw8624->info.cont_drv_lvl_ov: %d\n",
		  aw8624->info.cont_drv_lvl_ov);
	VIB_DEBUG("aw8624->info.gain_flag: %d\n", aw8624->info.gain_flag);
	for (i = 0; i < 3; i++)
		for (j = 0; j < 5; j++)
	...
	VIB_DEBUG("aw8624->info.parameter1: 0x%x\n", aw8624->info.parameter1);
	return 0;
}
					...
					struct ff_effect *old)
{
	...
	VIB_DEBUG("aw8624->effect_type= 0x%x\n", aw8624->effect_type);
	mutex_lock(&aw8624->lock);
	...
	if (aw8624->effect_type == FF_CONSTANT) {
		...
		VIB_DEBUG("Unsupported effect type: %d\n", effect->type);
	}
	mutex_unlock(&aw8624->lock);
	VIB_DEBUG("aw8624->effect_type= 0x%x\n", aw8624->effect_type);
	return 0;
...
static int select_pin_ctl(struct aw8624 *aw8624, const char *name)
{
	...
	for (i = 0; i < ARRAY_SIZE(aw8624->pinctrl_state); i++) {
		...
		if (!strncmp(n, name, strlen(n))) {
			...
			if (rc)
				VIB_DEBUG("cannot select '%s'\n", name);
			else
				VIB_DEBUG("Selected '%s'\n", name);
			goto exit;
		}
	}

	rc = -EINVAL;
	VIB_DEBUG("'%s' not found\n", name);
...
static DEVICE_ATTR(osc_save, S_IWUSR | S_IRUGO, NULL, aw8624_osc_save_store);
