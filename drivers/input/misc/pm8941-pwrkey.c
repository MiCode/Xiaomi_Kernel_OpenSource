...
/*add by xiao*/
#define POWERKEY_DEBUG 1
#define S2_TIMER_1			0x40
#define S2_TIMER_2			0x41
...
static irqreturn_t pm8941_pwrkey_irq(int irq, void *_data)
{
	...
	unsigned int sts,sts2,sts3,sts4;
	int error;
	u64 elapsed_us;
...
#endif
#ifdef POWERKEY_DEBUG
          	error = regmap_read(pwrkey->regmap,
			pwrkey->pon_pbs_baseaddr + PON_PULL_CTL, &sts2);
		if(error)
        	{
			dev_err(pwrkey->dev,"xiaoms pm8941_pwrkey_irq : can't get the PON_PULL_CTL register\n");
		}
    		dev_err(pwrkey->dev,"xiaoms pm8941_pwrkey_irq : PON_PULL_CTL register is 0x%x------\n",sts2);
		dev_err(pwrkey->dev,"xiaoms pm8941_pwrkey_irq : pwrkey->regmap=0x%x,pwrkey->pon_pbs_baseaddr=0x%x------\n",pwrkey->regmap,pwrkey->pon_pbs_baseaddr);

        	error = regmap_read(pwrkey->regmap,
			pwrkey->pon_pbs_baseaddr + S2_TIMER_1, &sts3);
		if(error)
        	{
			dev_err(pwrkey->dev,"xiaoms pm8941_pwrkey_irq : can't get the S2_TIMER_1 register\n");
        	}
        	dev_err(pwrkey->dev,"xiaoms pm8941_pwrkey_irq : S2_TIMER_1 register is 0x%x------\n",sts3);

        	error = regmap_read(pwrkey->regmap,
			pwrkey->pon_pbs_baseaddr + S2_TIMER_2, &sts4);
		if(error)
        	{
			dev_err(pwrkey->dev,"xiaoms pm8941_pwrkey_irq : can't get the S2_TIMER_2 register\n");
        	}
        	dev_err(pwrkey->dev,"xiaoms pm8941_pwrkey_irq : S2_TIMER_2 register is 0x%x------\n",sts4);
#endif
		return IRQ_HANDLED;
	}
...
static int pm8941_pwrkey_probe(struct platform_device *pdev)
{
	...
	if(error)
	{	
		dev_err(pwrkey->dev,"xiaoms pm8941_pwrkey_probe : can't get the PON_PULL_CTL register\n"); 
	}
	dev_dbg(&pdev->dev,"xiaoms pm8941_pwrkey_probe : PON_PULL_CTL register is 0x%0x------\n",sts2); 
