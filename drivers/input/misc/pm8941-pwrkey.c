...
static irqreturn_t pm8941_pwrkey_irq(int irq, void *_data)
{
    ...
    if (!strcmp(pwrkey->data->name,"pmic_pwrkey_resin_bark")){
		...
		if(sts){
			...
			pr_err("pwrkey_resin_bark_irq trigger, start D&R task info");
			if(mi_display_pwrkey_cb != NULL) {
				pr_err("%s: pmic_pwrkey_resin_bark \n", __func__);
				mi_display_pwrkey_cb(PMIC_PWRKEY_RESIN_BARK_TRIGGER);
			}
			console_loglevel = 15;
			pr_err("------ collect D&R-state processes info before long comb key ------\n");
	...
	pr_info("%s: pwrkey || resin event, KEY_POWER state: %d\n", __func__, sts);
	if((pwrkey->code == KEY_POWER || pwrkey->code == KEY_VOLUMEDOWN) && !sts){
		if(mi_display_pwrkey_cb != NULL){
