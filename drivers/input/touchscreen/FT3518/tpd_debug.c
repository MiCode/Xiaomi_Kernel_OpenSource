...
void tpd_debug_set_time(void)
{
	...
	tpd_last_2_int_time[1] = (t.tv_sec & 0xFFF) * 1000000 + t.tv_nsec/1000;
			...
			int cal_x, int cal_y, int p, int down)
{
	...
	if ((tpd_down_status == 0 && down == 1) ||
		...
		trace_MET_touch("EV_KEY",
				t.tv_sec, t.tv_nsec, "BTN_TOUCH", down);
		tpd_down_status = !tpd_down_status;
	}
	if (tpd_down_status) {
		trace_MET_touch("EV_ABS", t.tv_sec, t.tv_nsec, "X", raw_x);
		trace_MET_touch("EV_ABS", t.tv_sec, t.tv_nsec, "Y", raw_y);
	}
}
			...
			int cal_x, int cal_y, int p, int down)
{
	...
	if (down == TPD_TYPE_INT_DOWN) {
		...
		snprintf(buffer, tpd_log_line_buffer,
			 ...
			 (t.tv_sec & 0xFFF), t.tv_nsec/1000, raw_x / 1000);
	} else if (down && tpd_down_status == 0) {
		snprintf(buffer, tpd_log_line_buffer,
			 "[%5lu.%06lu][tpd_em_log]"
