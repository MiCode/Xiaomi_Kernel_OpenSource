...
static int tp_data_dump_proc_show(struct seq_file *file, void* data)
{
    ...
    int *diffdata = NULL;
       ...
       FTS_TEST_ERROR("file->read_pos = %lld", file->read_pos);
       FTS_TEST_ERROR("file->file->f_pos = %lld", file->file->f_pos);
    FTS_TEST_FUNC_ENTER();

    fts_write_reg(0xEE,1);//disable Auto Clb
    ...
    fts_read_reg(FACTORY_REG_CHX_NUM, &tx_num);
    fts_read_reg(FACTORY_REG_CHY_NUM, &rx_num);
    //seq_printf(file,"Tx:%2d\n", tx_num);
    //seq_printf(file,"Rx:%2d\n", rx_num);

    //get rawdata
    rawdata = (int *)kzalloc(tx_num * rx_num * sizeof(int), GFP_KERNEL);
    if (IS_ERR_OR_NULL(rawdata)) {
        FTS_TEST_ERROR("malloc rawdata memory fail");
        ret = -ENOMEM;
        goto exit;
    }
    ret = lct_tp_get_rawdata(rawdata, tx_num * rx_num * 2);
    if (ret < 0) {
        FTS_TEST_ERROR("lct_tp_get_rawdata() failed, ret = %d", ret);
        goto err_get_rawdata_fail;
    }

    //get diffdata
    diffdata = (int *)kzalloc(tx_num * rx_num * sizeof(int), GFP_KERNEL);
    if (IS_ERR_OR_NULL(diffdata)) {
        FTS_TEST_ERROR("malloc diffdata memory fail");
        ret = -ENOMEM;
        goto err_get_rawdata_fail;
    }
    ret = lct_tp_get_diffdata(diffdata, tx_num * rx_num * 2);
    if (ret < 0) {
        FTS_TEST_ERROR("lct_tp_get_rawdata() failed, ret = %d", ret);
        goto err_get_diffdata_fail;
    }
    ...
    for (row = 0; row < tx_num; row++) {
       ...
       //ret = ( 10 + 11 + (tx_num * rx_num * 6 + tx_num) * 2 );

err_get_diffdata_fail:
    if (IS_ERR_OR_NULL(diffdata))
        kfree(diffdata);
err_get_rawdata_fail:
    if (IS_ERR_OR_NULL(rawdata))
        kfree(rawdata);
exit:
    enter_work_mode();
...
{
       FTS_TEST_ERROR("file->f_pos = %lld", file->f_pos);
...
static int fts_tp_data_dump_proc_init(void) {
    ...
    if (IS_ERR_OR_NULL(tp_data_dump_proc)) {
