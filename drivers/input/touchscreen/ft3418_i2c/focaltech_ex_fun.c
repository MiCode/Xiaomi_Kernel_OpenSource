...
int lct_get_lockdown_info(void)
{
	...
	snprintf(tp_lockdown_info_buf, 64 , "%02x%02x%02x%02x%02x%02x%02x%02x\n",
...
int lct_fts_get_tpfwver(const char *cmd)
{
	...
	if ((fwver == 0xFF) || (fwver == 0x00)){
		...
		num_read_chars = snprintf(buf, 64, "[Vendor]samsung,[FW]0x%02x,[IC]ft3418\n", fwver);
...
int lct_fts_tp_set_screen_angle_callback(int angle)
{
	u8 val;
	int ret = -EIO;
	...
	if (ret < 0)
		FTS_ERROR("write FTS_SET_ANGLE, err\n");
...
#if LCT_TP_WORK_EN
int lct_fts_tp_work_callback(bool en)
{
	if (en){
	...
	FTS_INFO("%s Touchpad", en?"Enable":"Disable");
	return 0;
}
#endif
...
int lct_fts_tp_palm_callback(bool en)
{
	...
	FTS_INFO("%s PALM", en ? "Disable" : "Enable");
...
int lct_create_procfs(struct fts_ts_data *ts_data)
{
	...
	ret = init_lct_tp_info("[Vendor]samsung,[FW]unknow,[IC]ft3418\n", NULL);
	if(ret) {
	...
	if (ret < 0) {
		FTS_ERROR("init_lct_tp_grip_area Failed!");
		goto err_init_lct_tp_grip_area_fail;

	} else {
		FTS_INFO("init_lct_tp_grip_area Succeeded!");
...
#if LCT_TP_WORK_EN
		ret = init_lct_tp_work(lct_fts_tp_work_callback);
		if (ret < 0) {
				FTS_ERROR("init_lct_tp_work Failed!");
				goto err_init_lct_tp_work_fail;
		} else {
					FTS_INFO("init_lct_tp_work Succeeded!");
		}
#endif

#if LCT_TP_PALM_EN
		ret = init_lct_tp_palm(lct_fts_tp_palm_callback);
		if (ret < 0) {
			FTS_ERROR("init_lct_tp_palm Failed!");
			goto err_init_lct_tp_palm_fail;

		} else {
			FTS_INFO("init_lct_tp_palm Succeeded!");
