...
int fts_rawdata_differ(u8 raw_diff, char *buf)
{
	...
	buffer = kzalloc(buffer_length, GFP_KERNEL);
	if (buffer == NULL) {
...
ssize_t tp_differ_proc_read(struct file *file, char __user *buf,
...
ssize_t tp_rawdata_proc_read(struct file *file, char __user *buf,
		 size_t count, loff_t *off)
{
	int cnt = 0;
...
int fts_tp_differ_proc(void)
{
	...
	if (tp_differ_proc == NULL) {
		FTS_TEST_ERROR(
		"[focal] %s() - ERROR: create fts_tp_differ proc()  failed.",
		  __func__);
		ret = -1;
	}
