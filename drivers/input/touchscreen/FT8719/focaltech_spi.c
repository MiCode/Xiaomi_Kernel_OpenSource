...
static int fts_boot_write(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
	...
	u8 *rxbuf = NULL;
	u32 txlen = 0;
	u32 txlen_need = CD_PACKAGE_BUFLEN + SPI_HEADER_LENGTH + datalen;
...
err_boot_write:
	if (data && (txlen_need > SPI_BUF_LENGTH)) {
		if (txbuf) {
			kfree(txbuf);
			txbuf = NULL;
		}
	...
	if (data && datalen) {
		...
		if (ctrl & DATA_CRC_EN) {
			ret = rdata_check(rxbuf + CD_PACKAGE_BUFLEN,
							  txlen - CD_PACKAGE_BUFLEN);
			if (ret < 0) {
				FTS_INFO("read data(addr:%x) crc check incorrect,in %s mode",
						 cmdaddr, ts_data->fw_is_running ? "fw" : "boot");
				goto boot_read_err;
			}
		}
	}
