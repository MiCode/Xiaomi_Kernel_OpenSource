...
static struct attribute *tracking_attributes[] = {
	&dev_attr_track_frame.attr, &dev_attr_track_cnt.attr, NULL,
};

static struct attribute_group tracking_attr_group = { .attrs =
	tracking_attributes,
};
...
int ist30xx_init_tracking_sysfs(struct ist30xx_data *data)
{
	...
	ist30xx_tracking_dev =
	device_create(ist30xx_class, NULL, 0, NULL, "tracking");
	dev_set_drvdata(ist30xx_tracking_dev, data);

	/* /sys/class/touch/tracking/... */
	if (sysfs_create_group
		(&ist30xx_tracking_dev->kobj, &tracking_attr_group))
