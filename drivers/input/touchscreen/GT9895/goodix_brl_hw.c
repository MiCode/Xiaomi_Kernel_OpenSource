...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (on) {
		...
		if (cd->pinctrl) {
			...
			if (ret < 0) {
				ts_err("Failed to select rst active state, ret:%d", ret);
			}
		}
#endif
			 ...
			 struct goodix_ts_event *ts_event)
{
	...
	ts_debug("event_handler spi read data:%*ph",
		pre_read_len,
		pre_buf);
	ts_debug("spi read addr: %x", misc->touch_data_addr);
