...
int goodix_parse_cfg_bin(struct goodix_cfg_bin *cfg_bin)
{
	...
	if (cfg_bin->bin_data_len != cfg_bin->head.bin_len) {
		ts_err("cfg_bin len error,cfg_bin len:%d\n,"
			CFG_TAG "len of firmware:%d\n"
	...
	for (i = 0; i < cfg_bin->head.pkg_num; i++) {
		...
		if (i == cfg_bin->head.pkg_num - 1) {
			...
			cfg_bin->cfg_pkgs[i].pkg_len = cfg_bin->bin_data_len
								- offset1;
		} else {
			offset1 = cfg_bin->bin_data[TS_CFG_BIN_HEAD_LEN +
						 i * TS_CFG_OFFSET_LEN] +
			...
			if (offset2 <= offset1) {
				ts_err("offset error,pkg:%d, offset1:%d,"
					CFG_TAG "offset2:%d"
					, i, offset1, offset2);
				r = -EINVAL;
				goto exit;
...
int goodix_cfg_bin_proc(void *data)
{
	...
	if (!r) {
		...
		if (r != -EBUS) {
			ts_err("get reg and cfg from cfg_bin FAILED,"
				CFG_TAG "update fw then retry");
				...
				struct goodix_cfg_bin *cfg_bin)
{
	...
	for (i = 0; i < cfg_bin->head.pkg_num; i++) {
		...
		if (!strncmp(cfg_bin->cfg_pkgs[i].cnst_info.ic_type, "nanjing",
			...
			ts_err("get ic type FAILED,"
				CFG_TAG "unknown ic type from cfg_bin:%s",
				cfg_bin->cfg_pkgs[i].cnst_info.ic_type);
		...
		r = goodix_i2c_read(ts_dev, addr,
				&temp_sensor_id, 1);
		if (r < 0) {
			ts_err("read sensor id FAILED,I2C ERROR, pkg:%d,"
				CFG_TAG "sensor_id reg:0x%02x", i, addr);
			goto exit;
		} else {
			...
			!= 0)
				temp_sensor_id &=
			      cfg_bin->cfg_pkgs[i].reg_info.sensor_id.reserved1;

			if (temp_sensor_id !=
			cfg_bin->cfg_pkgs[i].cnst_info.sensor_id) {
				ts_err("pkg:%d, sensor id contrast FAILED,"
					CFG_TAG "reg:0x%02x", i, addr);
				ts_err("sensor_id from i2c:%d, sensor_id of cfg"
					CFG_TAG "bin:%d", temp_sensor_id,
				cfg_bin->cfg_pkgs[i].cnst_info.sensor_id);
				continue;
			}
		}
		...
		if (!addr || !cfg_bin->cfg_pkgs[i].cnst_info.fw_mask[0]) {
			...
			r = goodix_i2c_read(ts_dev, addr, temp_fw_mask,
						sizeof(temp_fw_mask));
			if (r < 0) {
				ts_err("read fw_mask FAILED, I2C ERROR,pkg:"
				CFG_TAG "%d, fw_mask reg:0x%02x", i, addr);
				goto exit;
			} else if (strncmp(temp_fw_mask,
				cfg_bin->cfg_pkgs[i].cnst_info.fw_mask,
				sizeof(temp_fw_mask))) {
				ts_err("pkg:%d, fw_mask contrast FAILED,"
					CFG_TAG "reg:0x%02x,", i, addr);
		...
		if (read_len <= 0 || read_len > TS_CFG_BLOCK_PID_LEN) {
			ts_err("pkg:%d, hw_pid len ERROR, len:%d", i, read_len);
			continue;
		}
	...
	if (!normal_pkg) {
		...
		if (!strncmp(normal_pkg->cnst_info.ic_type, "nanjing",
			...
			ts_err("get ic type FAILED, unknown ic type from"
				CFG_TAG "cfg_bin:%s"
	...
	if (high_sense_pkg) {
		...
		if (!ts_dev->highsense_cfg) {
			...
			if (!ts_dev->highsense_cfg) {
				ts_err("Failed to alloc"
					CFG_TAG " memory for high sense cfg");
				return -ENOMEM;
			}
