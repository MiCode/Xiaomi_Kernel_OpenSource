...
extern struct mutex edge_inhibition_mutex; /* lock for edge_inhibition */
...
int gt1x_update_prepare(char *filename)
{
	...
	if (filename == NULL) {
		...
		sprintf(filepath,"%s%s","/sdcard/",filename);
		GTP_INFO("Firmware:%s",filepath);
		/*add tom  */
