...
struct mutex edge_inhibition_mutex; /* lock for edge_inhibition */
...
static ssize_t edge_inhibition_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
	...
	if (len > 8) {
		GTP_ERROR("Too much data");
		return -EFAULT;
	}

	memset(edge_config,0,sizeof(edge_config));
	ret = copy_from_user(edge_config, buff, len);
	if (ret) {
		GTP_ERROR("copy_from_user failed.");
		return -EPERM;
	}
...
s32 edge_inhibition_init(void)
{
#ifdef EDGE_INHIBITION
	struct proc_dir_entry *proc_entry = NULL;
	mutex_init(&edge_inhibition_mutex);
