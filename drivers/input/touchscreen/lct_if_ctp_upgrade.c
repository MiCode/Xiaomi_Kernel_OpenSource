...
static struct proc_dir_entry *g_ctp_if_upgrade_version_proc;

 #define CTP_IF_UPGRADE_OLD_VERSION_PROC_FILE "ctp_if_upgrade_old_version"

static struct proc_dir_entry *g_ctp_if_upgrade_old_version_proc;


static CTP_UPGRADE_FUNC PIFCtpupdateFunc;
static CTP_READ_VERSION PIFCtpreadverFunc;
static CTP_READ_VERSION PIFCtpreadOldverFunc;
...
{
	char tmp_data[64] = {0};
	//printk("[elan]ctp_upgrade_proc_write len = %d\n",len);
	if (copy_from_user(tmp_data, buf, size))
    {
        printk("copy_from_user() fail.\n");
        return -EFAULT;
    }
	//printk("[elan]ctp_upgrade_proc_write %s\n",tmp_data);
