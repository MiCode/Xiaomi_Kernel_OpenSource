...
int ndt_write_tp_info(unsigned char keycode)
{
	unsigned char reg_addr = IIC_TP_INFO;
	unsigned char reg_data = keycode;
	int ret;

	if (g_ndt_client == NULL) {
		pr_err("%s ndt press not init\n", __func__);
		return -EINVAL;
	}
	ret = ndt_write_register(reg_addr, &reg_data, 1);
	if (ret < 0)
		pr_err("%s error\n", __func__);
	else
		pr_info("%s write value: %u success\n", __func__, keycode);
	return ret;
}
