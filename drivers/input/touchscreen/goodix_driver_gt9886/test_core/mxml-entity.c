	...
	for (i = 0; i < global->num_entity_cbs; i++)
		if (cb == global->entity_cbs[i]) {
			...
			if (i < global->num_entity_cbs)
				memmove(global->entity_cbs + i,
					global->entity_cbs + i + 1,
					(global->num_entity_cbs -
					i)*sizeof(global->entity_cbs[0]));
			return;
	}
}
...
int  _mxml_entity_cb(const char *name)/* O - Unicode value or -1 */
{					/* I - Entity name */
	int diff;		/* Difference between names */
	int curent;		/* Current entity in search */
	...
	while ((last - first) > 1) {
		curent = (first + last) / 2;
		diff = strcmp(name, entities[curent].name);
		if (diff == 0)
			return entities[curent].val;
		else if (diff < 0)
			last = curent;
		else
			first = curent;
