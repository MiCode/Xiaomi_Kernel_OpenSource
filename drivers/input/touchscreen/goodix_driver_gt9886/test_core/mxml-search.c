...
mxmlFindElement(mxml_node_t *node,	/* I - Current node */
		...
		int descend)
{				/* I - Descend into tree - @code MXML_DESCEND@, @code MXML_NO_DESCEND@, or @code MXML_DESCEND_FIRST@ */
	...
	while (node != NULL) {
		...
		if (node->type == MXML_ELEMENT &&
			...
			temp = mxmlElementGetAttr(node, attr);
			if (temp != NULL) {
...
{				/* I - Path to element */
	...
	while (*path) {
		...
		pathsep = strnchr(path, '/', strlen(path));
		if (pathsep == NULL)
		...
		node = mxmlFindElement(node, node, element, NULL, NULL, descend);
		if (node == NULL)
			return NULL;
	}
