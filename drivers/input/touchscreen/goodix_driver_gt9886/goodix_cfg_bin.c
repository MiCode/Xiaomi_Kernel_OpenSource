...
extern void goodix_sync_setup(struct goodix_ts_core *core_data);
...
int goodix_get_lockdowninfo(struct goodix_ts_core *ts_core)
{
	...
	if (ts_core->lockdown_info[0] != 0x53) {
		ts_err("CSOT + Goodix error lockdown only in P1.1, fix it");
		ts_core->lockdown_info[0] = 0x53;
		ts_core->lockdown_info[1] = 0x42;
		ts_core->lockdown_info[2] = 0x32;
		ts_core->lockdown_info[3] = 0x01;
		ts_core->lockdown_info[4] = 0x74;
		ts_core->lockdown_info[5] = 0x02;
		ts_core->lockdown_info[6] = 0x32;
		ts_core->lockdown_info[7] = 0x00;
	}
...
static int goodix_cfg_bin_proc(struct goodix_ts_core *core_data)
{
	...
	ts_dev->chip_version.sensor_id = 1;
