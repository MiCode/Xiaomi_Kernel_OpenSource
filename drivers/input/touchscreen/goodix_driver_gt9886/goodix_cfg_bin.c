...
int goodix_cfg_bin_proc(void *data)
{
	...
	goodix_ts_register_notifier(&core_data->ts_notifier);
		...
		add_hw_component_info(HWCOMPONENT_NAME_TOUCH, HWCOMPONENT_KEY_IC, "Goodix");
		tp_maker = kzalloc(20, GFP_KERNEL);
		if (tp_maker == NULL)
			ts_err("fail to alloc vendor name memory\n");
		else {
			strlcpy(tp_maker, update_hw_component_touch_module_info(goodix_core_data->lockdown_info[0]), 20);
