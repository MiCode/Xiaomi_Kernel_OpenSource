...
#ifdef CONFIG_TOUCHSCREEN_GOODIX_REGULATOR
	core_data->avdd_source = devm_regulator_get(dev, "vtouch-source");
	if (IS_ERR_OR_NULL(core_data->avdd_source)) {
		r = PTR_ERR(core_data->avdd_source);
		ts_err("Failed to get regulator avdd:%d", r);
		core_data->avdd_source = NULL;
		return r;
	}
...
int goodix_ts_power_on(struct goodix_ts_core *core_data)
{
	...
	if (core_data->avdd_source) {
		r = regulator_enable(core_data->avdd_source);
		if (!r) {
			ts_info("regulator enable avdd source SUCCESS");
		} else {
			ts_err("Failed to enable avdd source power:%d", r);
			return r;
		}
	}
...
int goodix_ts_power_off(struct goodix_ts_core *core_data)
{
	...
	if (core_data->avdd_source) {
		r = regulator_disable(core_data->avdd_source);
		if (!r) {
			ts_info("regulator disable avdd source SUCCESS");
		} else {
			ts_err("Failed to disable avdd source power:%d", r);
			return r;
		}
	}
#else
