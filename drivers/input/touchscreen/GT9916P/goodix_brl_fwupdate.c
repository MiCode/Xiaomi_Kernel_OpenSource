...
struct update_info_t update_bra = {
	...
	MISCTL_REG_BRA,	    WATCH_DOG_REG_BRA,	       GOODIX_CFG_ID_ADDR_BRA,
	ENABLE_MISCTL_BRA,
};
...
struct update_info_t update_brb = {
	...
	MISCTL_REG_BRB,	   WATCH_DOG_REG_BRB,	  GOODIX_CFG_ID_ADDR_BRB,
	ENABLE_MISCTL_BRB,
};
...
struct update_info_t update_brd = {
	...
	MISCTL_REG_BRD,	   WATCH_DOG_REG_BRD,	  GOODIX_CFG_ID_ADDR_BRD,
	ENABLE_MISCTL_BRD,
};
...
static int goodix_fw_version_compare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	if (fwu_ctrl->ic_config && fwu_ctrl->ic_config->len > 0) {
		...
		goodix_reg_read(config_id_reg, (u8 *)&ic_cfg_id,
				sizeof(ic_cfg_id));
...
static int goodix_load_isp(struct firmware_data *fw_data)
{
	...
	r = goodix_reg_write_confirm(isp_ram_reg, (u8 *)fw_isp->data,
				     fw_isp->size);
	if (r < 0) {
...
static ssize_t goodix_sysfs_result_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct fw_update_ctrl *fw_ctrl = &goodix_fw_update_ctrl;
