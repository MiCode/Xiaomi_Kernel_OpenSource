...
{
	u32 temp_check_sum = 0;
	int temp_position = 0;
	int i = 0;

	/* calculate checksum */
	board_data->edge_filter_params.length = 12;
	temp_check_sum += board_data->edge_filter_params.length;
	for (i = 0; i < 6; i++) {
		temp_check_sum += board_data->edge_filter_params.parameters[i];
	}
	board_data->edge_filter_params.check_sum = -temp_check_sum;

	/* cmd parameters */
	board_data->edge_filter_params_cmd_length = 2 +
		board_data->edge_filter_params.length + 4;
	...
	/* add parameters */
	for (i = 2; i < (2 + board_data->edge_filter_params.length); i += 2) {
		temp_position = (i >> 1) - 1;
		board_data->edge_filter_params_cmd[i] =
			(board_data->edge_filter_params.parameters[temp_position] >> 8) & 0xFF;
		board_data->edge_filter_params_cmd[i + 1] =
			(board_data->edge_filter_params.parameters[temp_position]) & 0xFF;
	}
	...
	ts_info("board_data->edge_filter_params_cmd: %*ph",
		board_data->edge_filter_params_cmd_length,
		board_data->edge_filter_params_cmd);
						...
						struct goodix_ts_board_data *board_data)
{
	...
	/* edge filter parameter */
	r = of_property_read_u16_array(node, "goodix,edge-filter-parameter",
				 board_data->edge_filter_params.parameters, 6);
	if (r) {
		ts_err("Invalid goodix,edge-filter-parameter");
		return -EINVAL;
	}
	ts_info("edge_filter_params values: %d, %d, %d, %d, %d, %d",
			board_data->edge_filter_params.parameters[0],
			board_data->edge_filter_params.parameters[1],
			board_data->edge_filter_params.parameters[2],
			board_data->edge_filter_params.parameters[3],
			board_data->edge_filter_params.parameters[4],
			board_data->edge_filter_params.parameters[5]);
