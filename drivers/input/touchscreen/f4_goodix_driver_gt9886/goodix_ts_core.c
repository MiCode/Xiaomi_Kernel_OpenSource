	...
	for (i = 0; i < Touch_Mode_NUM; i++) {
		...
		case Touch_UP_THRESHOLD:
			temp_value =
			    ...
			    [Touch_UP_THRESHOLD][SET_CUR_VALUE];
			goodix_game_value &= 0xFC;
			goodix_game_value |= temp_value;
			break;
		case Touch_Tolerance:
			temp_value =
			    xiaomi_touch_interfaces.touch_mode[Touch_Tolerance]
			    [SET_CUR_VALUE];
		...
		case Touch_Edge_Filter:
			temp_value =
			    xiaomi_touch_interfaces.touch_mode
			    [Touch_Edge_Filter][SET_CUR_VALUE];
		...
		case Touch_Panel_Orientation:
			...
			temp_value =
			    xiaomi_touch_interfaces.touch_mode
			    [Touch_Panel_Orientation][SET_CUR_VALUE];
			if (temp_value == 3)
				temp_value = 2;
	...
	if (mode < Touch_Mode_NUM && mode > 0) {
		...
		for (i = 0; i < Touch_Mode_NUM; i++) {
			xiaomi_touch_interfaces.touch_mode[i][SET_CUR_VALUE] =
			    xiaomi_touch_interfaces.touch_mode[i]
			    [GET_DEF_VALUE];
			gtp_set_cur_value(mode,
					  xiaomi_touch_interfaces.touch_mode
					  [mode][SET_CUR_VALUE]);
