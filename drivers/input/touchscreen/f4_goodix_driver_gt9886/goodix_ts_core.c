...
int goodix_ts_suspend(struct goodix_ts_core *core_data)
{
	struct goodix_ext_module *ext_module;
	struct goodix_ts_device *ts_dev = core_data->ts_dev;
	u8 state_data[3] = {0x70, 0x00, 0x90};
	int r = 0;
	ts_info("Suspend start");

	r = goodix_i2c_write(ts_dev, GSX_REG_GESTURE, state_data, 3);
	ts_info("disable palm sensor Enhanced Mode in suspend");
	if (r < 0) {
		ts_err("%s Failed to disable palm sensor Enhanced Mode in suspend: %d\n", __func__, r);
	}
...
int goodix_ts_resume(struct goodix_ts_core *core_data)
{
	struct goodix_ext_module *ext_module;
	struct goodix_ts_device *ts_dev = core_data->ts_dev;
	u8 state_data[3] = {0x70, 0x01, 0x8F};
	int r = 0;
...
out:
	...
	if (core_data->palm_sensor_enabled) {
		r = goodix_i2c_write(ts_dev, GSX_REG_GESTURE, state_data, 3);
		ts_info("enable palm sensor Enhanced Mode in resume");
		if (r < 0) {
			ts_err("%s Failed to enable palm sensor Enhanced Mode in resume: %d\n", __func__, r);
		}
	}
...
static int gtp_set_cur_value(int gtp_mode, int gtp_value)
{
	...
	u8 temp_value = 0;
	u16 edge_filter_corner_size = 250;
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < Touch_Mode_NUM; i++) {
		...
		case Touch_Edge_Filter:
			...
			if (xiaomi_touch_interfaces.touch_mode[Touch_Game_Mode][SET_CUR_VALUE]) {
				...
				edge_filter_corner_size =
					dev->board_data->edge_filter_corner_size[2];
			}
			dev->board_data->edge_filter_params.parameters[5] = edge_filter_corner_size;
#else
	...
	if (xiaomi_touch_interfaces.touch_mode[Touch_Game_Mode][SET_CUR_VALUE]) {
		...
		ts_info("set cur write values:0x%x, 0x%x, 0x%x", state_data[0], state_data[1], state_data[2]);
		ret = goodix_i2c_write(dev, GTP_GAME_CMD_ADD, state_data, 3);
		if (ret < 0) {
...
static int gtp_palm_sensor_write(int value)
{
	...
	if (goodix_core_data) {
		...
		if (value) {
			state_data[1] = 0x01;
			state_data[2] = 0x8F;
			ret = goodix_i2c_write(goodix_core_data->ts_dev, GSX_REG_GESTURE, state_data, 3);
			ts_info("enable palm sensor Enhanced Mode");
			if (ret < 0) {
				ts_err("%s Failed to enable palm sensor Enhanced Mode: %d\n", __func__, ret);
			}
		} else {
			state_data[1] = 0x00;
			state_data[2] = 0x90;
			ret = goodix_i2c_write(goodix_core_data->ts_dev, GSX_REG_GESTURE, state_data, 3);
			ts_info("disable palm sensor Enhanced Mode");
			if (ret < 0) {
				ts_err("%s Failed to disable palm sensor Enhanced Mode: %d\n", __func__, ret);
			}
		}
	} else
...
#ifdef USE_NEW_VERSION_EDGE_FILTER
	gtp_set_cur_value(Touch_Active_MODE, 3);
#endif
