...
#else
	for (i = 0; i < tx_num; i++) {
		printk("Ch/Tx_%02d:  ", i + 1);
		for (j = 0; j < rx_num; j++) {
			printk("%5d, ", data[i * rx_num + j]);
		}
		printk("\n");
	}

	if (key) {
		printk("Ch/Tx_%02d:  ", tx_num + 1);
		for (i = tx_num * rx_num; i < node_num; i++) {
			printk("%5d, ", data[i]);
		}
		printk("\n");
...
/* N19A code for HQ-359934 by p-huangyunbiao at 2023/12/19 end */
	...
	snprintf(filepath, FILE_NAME_LENGTH, "%s%s", FTS_INI_DIR_PATH,
		 file_name);
...
/* N19A code for HQ-359934 by p-huangyunbiao at 2023/12/19 start */
#else
	FTS_INFO("not support kernel_write interface to write files");
#endif
/* N19A code for HQ-359934 by p-huangyunbiao at 2023/12/19 end */
	FTS_TEST_FUNC_EXIT();
	return 0;
...
static int fts_ito_test_show(struct seq_file *file, void *data)
{
	...
	char test_result_bmp[ITO_TEST_ITEM] = { 0 };
	...
	for (i = FTS_SPI_TEST; i <= FTS_SHORT_CIRCUIT_TEST; i++)
	...
	if ((fts_ftest->test_item[FTS_SPI_TEST].testresult) == RESULT_PASS) {
...
static const struct proc_ops ito_test_ops = {
	.proc_open = fts_ito_test_open,
	.proc_read = seq_read,
	.proc_write = fts_ito_test_write,
...
static int lct_tp_get_diffdata(int *data, int byte_num)
{
	...
	/* read diffdata */
	ret = read_rawdata(tdata, 0x01, 0xAD, FACTORY_REG_RAWDATA_ADDR,
			   byte_num, data);
	if (ret < 0) {
...
static int tp_data_dump_proc_open(struct inode *inode, struct file *file)
{
	...
	ret = fts_enter_test_environment(1); //upgrade fw
	if (ret < 0) {
		FTS_ERROR("enter test environment fail");
	}
	...
	ret = fts_enter_test_environment(0); //upgrade fw
	if (ret < 0) {
		FTS_ERROR("enter test environment fail");
	}
	goto exit;
...
int fts_tp_data_dump_proc_init(void)
{
	...
	lct_tp_data_dump_p->tp_data_dump_proc = proc_create(
		FTS_PROC_TP_DATA_DUMP, 0444, NULL, &tp_data_dump_proc_fops);
	if (IS_ERR_OR_NULL(lct_tp_data_dump_p->tp_data_dump_proc)) {
		FTS_TEST_ERROR("ERROR: create /proc/%s failed.",
			       FTS_PROC_TP_DATA_DUMP);
