 ...
 *
 * Copyright (c) 2012-2023, FocalTech Systems, Ltd., all rights reserved.
 *
...
/* N19A code for HQ-364908 by p-huangyunbiao at 2024/1/8 start */
	panel_proximity_mode_set_by_ft8725(fts_proximity_data.mode);
...
static int ps_batch(int flag, int64_t sampling_period_ns,
		    int64_t max_batch_report_ns)
{
	return 0;
}
...
/*N19A code for HQ-359826 by chenyushuai at 2023/12/20 start*/
	if (regvalue != 0x00) {
		...
		fts_write_reg(buf_addr, buf_value);
		ps_touch_event = PS_FAR_AWAY;
	}
/*N19A code for HQ-359826 by chenyushuai at 2023/12/20 end*/
	if (regvalue != old_proximity_value) {
		tpd_notifier_call_chain(ps_touch_event, NULL);
		FTS_INFO("old_proximity_value is %d", old_proximity_value);
