...
#if IS_ENABLED(CONFIG_DRM_PANEL_N19A_36_02_0A_DSC_VDO)
...
static int fts_drm_panel_notifier_callback(struct notifier_block *self,
					   unsigned long event, void *data);
#elif IS_ENABLED(FTS_MSM_DRM_NOTIFY)
static int fts_drm_notifier_callback(struct notifier_block *self,
				     unsigned long event, void *data);
#elif IS_ENABLED(FTS_FB_NOTIFY)
...
#elif IS_ENABLED(FTS_MTK_DRM_NOTIFY)
static int fts_mtk_drm_notifier_callback(struct notifier_block *nb,
					 unsigned long event, void *data);
#endif
...
#if FTS_PSENSOR_ENABLE
	if ((fts_proximity_readdata(ts_data) != 0) &&
	    (ts_data->suspended == 1) && (tp_sensor_flag == 1)) {
		return 1;
	}
#endif
		...
		for (i = 0; i < max_touch_num; i++) {
			...
			events[i].x =
				...
				((touch_buf[FTS_TOUCH_OFF_XL + base] & 0xFF)
				 << 3) +
				((touch_buf[FTS_TOUCH_OFF_PRE + base] & 0x70) >>
				 4);
			events[i].y =
					   ...
					   unsigned long event, void *data)
{
	...
	if (!(event == DRM_PANEL_EARLY_EVENT_BLANK ||
		...
		//FTS_LOG("event(%lu) not need to process\n", event);
		return 0;
	}
...
/*******************************************************
Description:
	focaltech touchscreen driver early suspend function.
...
/*******************************************************
Description:
	focaltech touchscreen driver late resume function.
...
static int fts_notifier_callback_init(struct fts_ts_data *ts_data)
{
	...
	ts_data->drm_panel_notif.notifier_call = fts_drm_panel_notifier_callback;
	if (active_panel) {
	...
	if (ret) {
		FTS_ERROR("[FB]Unable to register notifierier: %d", ret);
	}
#elif IS_ENABLED(FTS_EARLYSUSPEND_NOTIFY)
...
#elif IS_ENABLED(FTS_MTK_DRM_NOTIFY)
	FTS_INFO("init notifier with disp_notifier_register");
	ts_data->disp_notifier.notifier_call = fts_mtk_drm_notifier_callback;
	ret = mtk_disp_notifier_register("focal ic Touch",
				&ts_data->disp_notifier);
	if (ret)
		FTS_ERROR(
			"[DRM]Failed to register disp notifier client:%d", ret);
#endif
...
static int fts_notifier_callback_exit(struct fts_ts_data *ts_data)
{
	...
	if (active_panel) {
		if (drm_panel_notifier_unregister(active_panel,
						  &ts_data->drm_panel_notif))
			FTS_ERROR("Error occurred while unregistering drm_panel_notifier.\n");
	}
#elif IS_ENABLED(FTS_MSM_DRM_NOTIFY)
...
static u8 fts_panel_vendor_read(void)
{
	...
	char str_tmp[3] = { 0 };
	if (fts_data) {
		str_tmp[0] = fts_data->lockdowninfo[0];
		str_tmp[1] = fts_data->lockdowninfo[1];
...
static u8 fts_panel_display_read(void)
{
	...
	char str_tmp[3] = { 0 };
	if (fts_data) {
		str_tmp[0] = fts_data->lockdowninfo[2];
		str_tmp[1] = fts_data->lockdowninfo[3];
		sscanf(str_tmp, "%x", &num_tmp);
		return num_tmp;
	} else {
		return 0;
...
static u8 fts_panel_color_read(void)
{
	...
	char str_tmp[3] = { 0 };
	if (fts_data) {
		str_tmp[0] = fts_data->lockdowninfo[4];
		str_tmp[1] = fts_data->lockdowninfo[5];
		sscanf(str_tmp, "%x", &num_tmp);
		return num_tmp;
	} else {
		return 0;
...
/*
 * focaltech_touch_vendor_read - read vendor ic name
 *
...
/* N19A code for HQ-348496 by p-huangyunbiao at 2024/1/31 start */
	...
	if (chosen) {
		...
		FTS_INFO("%s tp_lockdown_info : %s\n", __func__, tp_lockdown_info);
	}
	strlcpy(ts_data->lockdowninfo, tp_lockdown_info, LOCKDOWN_INFO_LENGTH);
...
static int fts_ts_resume(struct device *dev)
{
	...
	if ((fts_proximity_resume() == 0) &&
	    (fts_data->fts_proximity_suspend_flag == 1)) {
		if (ts_data->gesture_support) {
...
static int fts_set_cur_value(int mode, int value)
{
	...
	if (mode < Touch_Mode_NUM && mode >= 0) {
		...
		xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE] = value;
		FTS_INFO(
			"fts_set_cur_value mode = %d, value = %d\n", mode,
			xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE]);
		if (mode == 0) {
			FTS_INFO("fts_set_cur_value mode = %d, value = %d\n",
				 mode,
...
static int fts_get_mode_value(int mode, int value_type)
{
	...
	if (mode < Touch_Mode_NUM && mode >= 0) {
		...
		FTS_INFO(
			"fts_get_mode_value mode = %d , value = %d\n", mode,
			xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE]);
	} else
		FTS_INFO("%s ,don't support\n", __func__);
	return value;
...
static int fts_ts_probe(struct spi_device *spi)
{
	...
	if (ret) {
		FTS_ERROR("fts_touch_in %s fail ret = %d\n", __func__, ret);
	}

	ret = tpd_register_psenable_callback();
	if (ret) {
		FTS_ERROR("tpd_in %s fail ret = %d\n", __func__, ret);
	}
	/*N19A code for HQ-359826 by chenyushuai at 2023/12/20 end*/	
...
/*N19A code for HQ-369142 by p-xielihui at 2024/1/29 start*/
	...
	if ((panel_id == FT8725_HUAXING_PRIVATE) || (panel_id == FT8725_HUAXING_COMMON))
		FTS_INFO("It is FT8725 touch IC\n");
	else {
		FTS_ERROR("not FT8725 touch IC\n");
		return -ENODEV;
	}
#endif
/*N19A code for HQ-369142 by p-xielihui at 2024/1/29 end*/
