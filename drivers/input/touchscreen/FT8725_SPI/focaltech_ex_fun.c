...
struct proc_dir_entry *fts_proc_touchscreen_dir;
			       ...
			       size_t count, loff_t *ppos)
{
	...
	case PROC_READ_DATA_DIRECT:
		writelen = buflen - 1;
		if (writelen >= FTS_MAX_COMMMAND_LENGTH) {
			FTS_ERROR("cmd(PROC_READ_DATA_DIRECT) length(%d) fail",
				  writelen);
			goto proc_write_err;
		}
		memcpy(proc->cmd, writebuf + 1, writelen);
			      ...
			      size_t count, loff_t *ppos)
{
	...
	case PROC_READ_DATA:
		...
		ret = fts_read(proc->cmd, proc->cmd_len, readbuf,
			       num_read_chars);
		if (ret < 0) {
	...
	case PROC_READ_DATA_DIRECT:
		num_read_chars = buflen;
		ret = fts_spi_transfer_direct(proc->cmd, proc->cmd_len, readbuf,
					      num_read_chars);
		if (ret < 0) {
	...
	case PROC_GET_DRIVER_INFO:
		if (buflen >= 64) {
			...
			snprintf(&readbuf[32], buflen - 32, "%s",
				 FTS_DRIVER_VERSION);
		}
		break;
...
proc_read_err:
	if ((num_read_chars > 0) &&
	    copy_to_user(buff, readbuf, num_read_chars)) {
			  ...
			  int *eof, void *data)
{
	...
	case PROC_GET_DRIVER_INFO:
		if (buflen >= 64) {
			num_read_chars = buflen;
			readbuf[0] = ts_data->bus_type;
			snprintf(&readbuf[16], buflen - 16, "%s",
				 FTS_DRIVER_VERSION);
		}
		break;
...
int fts_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
	struct ftxxxx_proc *proc = &ts_data->proc;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
	proc->proc_entry = proc_create_data(PROC_NAME, 0777, NULL,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
	ts_data->proc_ta.proc_entry = proc_create_data(
		"fts_ta", 0777, NULL, &fts_procta_fops, ts_data);
...
/* fts_pen node */
static ssize_t fts_pen_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	int count = 0;
	struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
static ssize_t fts_pen_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	int value = 0;
	struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
/* 		if (2 == lcm_supply) {
			strcpy(fwname, "focaltech_ft8720_hx.ini");
			FTS_INFO("fwname:%s.\n", fwname);
		} else if (3 == lcm_supply) {
			strcpy(fwname, "focaltech_ft8725_hx.ini");
			FTS_INFO("fwname:%s.\n", fwname);
		} else if (4 == lcm_supply) {
			strcpy(fwname, "focaltech_ft8725_2_hx.ini");
			FTS_INFO("fwname:%s.\n", fwname);
		} */
...
{
	fts_proc_touchscreen_dir =
		proc_mkdir(FTS_PROC_TOUCHSCREEN_FOLDER, NULL);
	if (fts_proc_touchscreen_dir == NULL) {
		printk(KERN_ERR "%s: proc_touchpanel_dir file create failed!\n",
		       __func__);
	} else {
		fts_proc_ctp_openshort_test_file =
			proc_create(FTS_PROC_CTP_OPENSHORT_TEST_FILE,
				    (S_IWUSR | S_IRUGO),
				    fts_proc_touchscreen_dir,
				    &fts_proc_ctp_openshort_test_fops);
		...
		fts_proc_lockdown_info_file = proc_create(
			PROC_CTP_LOCKDOWN_INFO_FILE, (S_IWUSR | S_IRUGO),
			fts_proc_touchscreen_dir, &fts_proc_lockdown_info_fops);
		if (fts_proc_ctp_openshort_test_file == NULL) {
			printk(KERN_ERR
			       "%s: proc_ctp_openshort_test_file create failed!\n",
			       __func__);
