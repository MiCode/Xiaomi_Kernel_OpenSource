 ...
 *
 * Copyright (c) 2012-2023, Focaltech Ltd. All rights reserved.
 *
...
#define FTS_8725_FW_NAME "focaltech_8725_fw"
#define FTS_8725_2_FW_NAME "focaltech_8725_2_fw"
...
struct upgrade_setting_nf upgrade_setting_list[] = {
	{ 0x87, 0x19, 0, (64 * 1024), (128 * 1024), 0x00, 0x02, 8, 1, 1, 1, 0,
	  0 },
	{ 0x86, 0x22, 0, (64 * 1024), (128 * 1024), 0x00, 0x02, 8, 1, 1, 0, 0,
	  0 },
	{ 0x87, 0x56, 0, (88 * 1024), 32766, 0xA5, 0x01, 8, 0, 2, 0, 1, 0 },
	{ 0x80, 0x09, 0, (88 * 1024), 32766, 0xA5, 0x01, 8, 0, 2, 0, 1, 0 },
	{ 0x86, 0x32, 0, (64 * 1024), (128 * 1024), 0xA5, 0x01, 12, 0, 1, 0, 0,
	  0 },
	{ 0x86, 0x42, 0, (64 * 1024), (128 * 1024), 0xA5, 0x01, 12, 0, 1, 0, 0,
	  0 },
	{ 0x87, 0x20, 0, (88 * 1024), (128 * 1024), 0xA5, 0x01, 8, 0, 2, 0, 1,
	  0 },
	{ 0x87, 0x22, 0, (88 * 1024), (128 * 1024), 0xA5, 0x01, 8, 0, 2, 0, 1,
	  0 },
	{ 0x82, 0x01, 0, (96 * 1024), (128 * 1024), 0xA5, 0x01, 8, 0, 2, 0, 0,
	  0 },
	{ 0xF0, 0xC6, 0, (84 * 1024), (128 * 1024), 0xA5, 0x01, 8, 0, 2, 0, 1,
	  0 },
	{ 0x56, 0x62, 0, (128 * 1024), (128 * 1024), 0xA5, 0x01, 8, 0, 4, 0, 0,
	  5 },
	{ 0x82, 0x05, 0, (120 * 1024), (128 * 1024), 0xA5, 0x01, 8, 0, 2, 0, 0,
	  0 },
	{ 0x87, 0x25, 0, (88 * 1024), (128 * 1024), 0xA5, 0x01, 8, 0, 2, 0, 1,
	  0 },
};
...
static int fts_ecc_cal_tp(u32 ecc_saddr, u32 ecc_len, u16 *ecc_value)
{
	...
	if (upg->setting_nf->ecc_delay) {
		...
		if (i >= FTS_ECC_FINISH_TIMEOUT) {
			FTS_ERROR("wait ecc finish timeout,ecc_finish=%x",
				  value[0]);
			return -EIO;
		}
	}
...
/* 	if (2 == lcm_supply) {
	...
	} else if (3 == lcm_supply) {
		sprintf(Tp_name, "HX,FT8725,FW:0x0%d", fwver);
	} else if (4 == lcm_supply) {
		sprintf(Tp_name, "HX,FT8725_2,FW:0x0%d", fwver);
	} */
...
#if defined(KERNEL_RW_FUNC_GKI2_0_SUPPORT)
...
/* N19A code for HQ-359934 by p-huangyunbiao at 2023/12/19 start */
#else
	FTS_INFO("not support kernel_read interface reads file");
	return -EINVAL;
#endif
/* N19A code for HQ-359934 by p-huangyunbiao at 2023/12/19 end */
}
...
int fts_enter_test_environment(bool test_state)
{
	...
	for (i = 0; i < 3; i++) {
		FTS_INFO("fw download times:%d", i + 1);
		ret = fts_fw_write_start(upg->fw + app_offset, upg->fw_length,
					 true);
		if (0 == ret)
			break;
	}
...
static int fts_fw_resume(bool need_reset, enum FW_TYPE fw_type)
{
	...
	if (FTS_FW_REQUEST_SUPPORT) {
		FTS_INFO("chip id is 0x%x\n", fts_data->ic_info.ids.chip_idl);
	...
	if ((fw_type == FW_GESTURE) ||
	    ((fw_type == FW_AUTO) && upg->ts_data->gesture_bmode &&
	     upg->ts_data->suspended)) {
...
/* 	if (2 == lcm_supply) {
		...
		snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin", FTS_8725_FW_NAME,
			 upg->module_info->vendor_name);
	} else if (4 == lcm_supply) {
		snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin",
			 FTS_8725_2_FW_NAME, upg->module_info->vendor_name);
	} */
...
/* N19A code for HQ-379605 by p-huangyunbiao at 2024/3/28 start */
		sprintf(tp_version_info,
			"[Vendor]Huaxing [TP-IC]:FT8725, [FW]0x%x\n",
			fw_version);
/* N19A code for HQ-379605 by p-huangyunbiao at 2024/3/28 end */
	}
}
/* N19A code for HQ-369421 by p-huangyunbiao at 2024/2/2 start */
#if IS_ENABLED(CONFIG_DRM_PANEL_N19A_42_03_0B_DSC_VDO)
extern void (*fts_fwresume_work_callback)(void);
#endif
...
/* N19A code for HQ-369421 by p-huangyunbiao at 2024/2/2 start */
#if IS_ENABLED(CONFIG_DRM_PANEL_N19A_42_03_0B_DSC_VDO)
	fts_fwresume_work_callback = fts_fwresume_work;
#endif
/* N19A code for HQ-369421 by p-huangyunbiao at 2024/2/2 end */
	return 0;
}
...
/* N19A code for HQ-369421 by p-huangyunbiao at 2024/2/2 start */
#if IS_ENABLED(CONFIG_DRM_PANEL_N19A_42_03_0B_DSC_VDO)
	fts_fwresume_work_callback = NULL;
#endif
/* N19A code for HQ-369421 by p-huangyunbiao at 2024/2/2 end */
...
/* 	if (2 == lcm_supply) {
		...
		seq_printf(m, "[Vendor]Huaxing, [TP-IC]: FT8725,[FW]0x%x\n",
			   fw_version);
	} else if (4 == lcm_supply) {
		seq_printf(m, "[Vendor]Huaxing, [TP-IC]: FT8725_2,[FW]0x%x\n",
			   fw_version);
	} */

	seq_printf(m, "[Vendor]%s, [TP-IC]: FT8725,[FW]0x%x\n",
				"huaxing", fw_version);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops fts_tp_lockdown_info_fops = {
...
static ssize_t fts_aftersale_selftest_write(struct file *file,
...
int32_t fts_extra_proc_init(void)
{
	FTS_proc_tp_info_entry =
		proc_create(FTS_TP_INFO, 0444, NULL, &fts_tp_info_fops);
	...
	FTS_proc_tp_lockdown_info_entry = proc_create(
		FTS_TP_LOCKDOWN_INFO, 0444, NULL, &fts_tp_lockdown_info_fops);
	...
	FTS_proc_tp_selftest_entry = proc_create(FTS_TP_SELFTEST, 0777, NULL,
						 &fts_aftersale_test_ops);
