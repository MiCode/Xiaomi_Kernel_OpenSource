...
#define FTS_KEY_DOUBLE_CLICK 143

/******************************************************************************
* Private constant and macro definitions using #define
*****************************************************************************/
#define KEY_GESTURE_DOUBLECLICK FTS_KEY_DOUBLE_CLICK
...
/* N19A code for HQ-348470 by p-xielihui at 2024/1/2 start */
	panel_gesture_mode_set_by_ft8725(ts_data->gesture_support);
		       ...
		       unsigned int code, int value)
{
	if (type == EV_SYN && code == SYN_CONFIG) {
		if (value == WAKEUP_OFF) {
			...
			FTS_INFO("gesture disabled:%d", gesture_support);
		} else if (value == WAKEUP_ON) {
			fts_data->gesture_support = true;
			gesture_support = true;
			FTS_INFO("gesture enabled:%d", gesture_support);
		}
/* N19A code for HQ-348470 by p-xielihui at 2024/1/2 start */
		panel_gesture_mode_set_by_ft8725(gesture_support);
/* N19A code for HQ-348470 by p-xielihui at 2024/1/2 end */
	}
	return 0;
}
