...
static int syna_tcm_resume(struct device *dev)
{
	...
	if (tcm_hcd->host_download_mode) {
		...
		if (tcm_hcd->id_info.mode == MODE_APPLICATION && tcm_hcd->app_status == APP_STATUS_OK && !tcm_hcd->need_reset) {
			tcm_hcd->update_watchdog(tcm_hcd, true);
			goto mod_resume;
		}
#ifdef RESET_ON_RESUME
		tcm_hcd->need_reset = false;
...
static int __init syna_tcm_module_init(void)
{
	...
	if (IS_ERR_OR_NULL(g_lcd_id)){
		...
		if (strstr(g_lcd_id, "tianma nt36672a") != NULL) {
			LOGV("This is [Vendor]tianma [IC]nt36672a LCM!\n");
			goto err_lcd;
		} else if (strstr(g_lcd_id, "boe td4320") != NULL) {
			LOGV("LCM is right!\n");
		} else {
			LOGV("Unknown LCM!\n");
			goto err_lcd;
