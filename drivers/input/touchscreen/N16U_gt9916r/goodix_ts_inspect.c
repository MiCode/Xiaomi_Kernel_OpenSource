...
static int goodix_init_testlimits(struct goodix_ts_test *ts_test)
{
	...
	else if (ts_core->lockdown_info[1] == 0x42)
		sprintf(limit_file, "%s_test_limits_%d_CSOT.csv", GOODIX_TEST_FILE_NAME,
			ts_core->fw_version.sensor_id);
	else
 		sprintf(limit_file, "%s_test_limits_%d_TM.csv", GOODIX_TEST_FILE_NAME,
			ts_core->fw_version.sensor_id);
...
for (i = 0; i < data_size; i++) {
		avg_val = 0;
		for (j = 0; j < TOTAL_FRAME_NUM; j++)
			avg_val += ts_test->accord_arr[j].data[i];
		avg_val = avg_val / TOTAL_FRAME_NUM;
		if (avg_val > ts_test->test_params.deviation_limits[i]) {
...
static int goodix_save_data(struct goodix_ts_test *ts_test)
{
	...
	if (ts_test->test_result[GTP_SELFNOISE_TEST]) {
		ret = goodix_save_self_data(ts_test, ts_test->self_noisedata.data,
...
static int auto_test_result_show(struct seq_file *m, void *v)
{
	...
	bytes += sprintf(&data[bytes], "test_resule:%s\n", info->result);
	ret = fs_write(data, bytes);
	if (ret < 0)
		ts_err("fs_write failed");
