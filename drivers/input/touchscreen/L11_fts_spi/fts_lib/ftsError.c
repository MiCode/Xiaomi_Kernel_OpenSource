...
int errorHandler(u8 *event, int size)
{
	...
	if (info != NULL && event != NULL && size > 1
		...
		case EVT_TYPE_ERROR_ESD:
			...
			if (res < OK) {
				logError(1,
					 "%s %s: Error performing powercycle ERROR %08X\n",
					 tag, __func__, res);
			}
	...
	logError(1,
		 "%s %s: event Null or not correct size! ERROR %08X\n",
