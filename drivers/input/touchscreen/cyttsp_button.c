...
static int __devinit cyttsp_button_probe(struct i2c_client *client,
		const struct i2c_device_id *id)
{
	...
	register_glove_mode_notifier(&data->glove_mode_notif);
...
static int __devexit cyttsp_button_remove(struct i2c_client *client)
{
	struct cyttsp_button_data *data = i2c_get_clientdata(client);
	const struct cyttsp_button_platform_data *pdata = data->pdata;

	unregister_glove_mode_notifier(&data->glove_mode_notif);
...
static struct i2c_driver cyttsp_button_driver = {
	...
	.remove		= __devexit_p(cyttsp_button_remove),
