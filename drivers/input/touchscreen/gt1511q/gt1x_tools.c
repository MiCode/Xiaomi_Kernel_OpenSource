...
static void set_tool_node_name(char *procname)
{
	...
	snprintf(procname, 40, "gmnode%02d%02d%02d", v0, v1, v2);
}
...
static ssize_t gt1x_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	...
	ret = copy_from_user(&temp_data, &buff[CMD_HEAD_LENGTH], 1);
	if (ret) {
		GTP_ERROR("copy_from_user failed.");
	}
...
static ssize_t gt1x_tool_read(struct file *filp, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (cmd_head.wr % 2) {
		...
		*ppos += sizeof(IC_TYPE);
		...
		progress_buf[0] = update_info.progress >> 8;
		progress_buf[1] = update_info.progress & 0xff;
		progress_buf[2] = update_info.max_progress >> 8;
		progress_buf[3] = update_info.max_progress & 0xff;
