...
/* add nova tp selftest by zhangchaofan start, 2018-09-10*/
#include "../lct_ctp_selftest.h"
/* add nova tp selftest by zhangchaofan end, 2018-09-10*/
...
/* modify nvt_test dir by zhangchaofan start, 2018-11-30*/
...
/* modify nvt_test dir by zhangchaofan end, 2018-11-30*/
...
static int32_t nvt_save_rawdata_to_csv(int32_t *rawdata, uint8_t x_ch, uint8_t y_ch, const char *file_path, uint32_t offset)
{
	...
	org_fs = get_fs();
	set_fs(KERNEL_DS);
	fp = filp_open(file_path, O_RDWR | O_CREAT, 0664);	//modify file_open permission by zhangchaofan , 2018-11-14
	if (fp == NULL || IS_ERR(fp)) {
		...
		return -1;	//modify by zhangchaofan if open fail return error, 2018-11-14
	}
	...
	for (j = 0; j < y_ch; j++) {
		for (i = 0; i < x_ch; i++) {
			if (RecordResult[j * x_ch + i] != 0) {
				isPass = false;
				LOGV("(x=%d, y=%d) failed!\n", i, j);	//add debug for tp by zhangchaofan 2018-11-07
				break;
			}
		}
	}
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	//add  begin debug for tp by zhangchaofan 2018-11-07
	...
	//add  end debug for tp by zhangchaofan 2018-11-07
...
static int lct_nova_tp_selftest(void)
{
	...
	if ((TestResult_FWMutual == 0)&& (TestResult_FW_CC == 0) && (TestResult_Noise == 0)&& (TestResult_Short ==0) && (TestResult_Open == 0)){
		...
		NVT_LOG("FWMutual=%d, FW_CC=%d, Noise=%d, Short=%d, Open=%d, Fail!\n",
			TestResult_FWMutual, TestResult_FW_CC, TestResult_Noise, TestResult_Short, TestResult_Open);	//add  debug for tp by zhangchaofan 2018-11-07
		NVT_LOG("Fail\n");
                return 1;
	}
}
