...
static ssize_t fwu_sysfs_guest_serialization_show(struct device *dev,
		struct device_attribute *attr, char *buf);
#endif
...
static struct device_attribute attrs[] = {
	...
	__ATTR(guestserialization, S_IRUGO,
			fwu_sysfs_guest_serialization_show,
			synaptics_rmi4_store_error),
#ifdef SYNA_TDDI
...
static enum flash_area fwu_go_nogo(void)
{
	...
	for (ii = 0; ii < config_id_size; ii++) {
		printk("fwu->img.ui_config.data[%d]=%x, fwu->config_id[%d]=%x\n", ii, fwu->img.ui_config.data[ii], ii, fwu->config_id[ii]);
...
static int fwu_do_reflash(void)
{
	...
	if (retval < 0)
		return retval;
	printk("%s: Configuration programmed\n", __func__);
...
static int fwu_do_read_customer_serialization_data(void)
{
	...
	fwu->config_area = PM_CONFIG_AREA;
	fwu->config_size = fwu->blkcount.pm_config * fwu->block_size;
