 ...
 * 0 -- disable (default)
 * 1 -- read all the report and cmd&response
 * 2 -- only print the cmd&reponse and identify report
 * 3 -- skip the report (repord ID > REPORT_TOUCH), it is mainly for production
 */
...
static void syna_print_xfer_data(struct syna_tcm_hcd *tcm_hcd, unsigned char *data, unsigned int length, unsigned int wr_rd)
{
	...
	offset += cnt;
	LOGI(tcm_hcd->pdev->dev.parent, "%s", print_buf);
...
static int parse_dt(struct device *dev, struct syna_tcm_board_data *bdata)
{
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			LOGE(dev,
					"Failed to read synaptics, power-on-state property, retval=%d\n", retval);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			LOGE(dev,
					"Failed to read synaptics, power-delay-ms property, retval=%d\n", retval);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			LOGE(dev,
					"Failed to read synaptics, reset-on-state property, retval=%d\n", retval);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			LOGE(dev,
					"Failed to read synaptics, reset-active-ms property, retval=%d\n", retval);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			LOGE(dev,
					"Unable to read synaptics, reset-delay-ms property, retval=%d\n", retval);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			LOGE(dev,
					"Unable to read synaptics, byte-delay-us property, retval=%d\n", retval);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			LOGE(dev,
					"Unable to read synaptics, block-delay-us property, retval=%d\n", retval);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			LOGE(dev,
					"Unable to read synaptics, spi-mode property, retval=%d\n", retval);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			LOGE(dev,
					"Unable to read synaptics, ubl-max-freq property, retval=%d\n", retval);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			LOGE(dev,
					"Unable to read synaptics, ubl-byte-delay-us property, retval=%d\n", retval);
			return retval;
		} else {
		...
		unsigned short addr, unsigned char *data, unsigned int length)
{
	...
	if (retval < 0) {
		LOGE(&spi->dev,
				"Failed to allocate memory, retval=%d\n", retval);
		goto exit;
	}
		...
		unsigned short addr, unsigned char *data, unsigned int length)
{
	...
	if (retval < 0) {
		LOGE(&spi->dev,
				"Failed to copy write data, retval=%d\n", retval);
		goto exit;
	}
		...
		unsigned int length)
{
	...
	if (retval == 0) {
		retval = length;
		syna_print_xfer_data(tcm_hcd, data, length, SYNA_SPI_TRANSFER_READ);
	} else {
		LOGE(&spi->dev,
				"Failed to complete SPI transfer, error = %d\n",
				retval);
		...
		unsigned int length)
{
	...
	if (retval == 0) {
		retval = length;
		syna_print_xfer_data(tcm_hcd, data, length, SYNA_SPI_TRANSFER_WRITE);
	} else {
		LOGE(&spi->dev,
				"Failed to complete SPI transfer, error = %d\n",
				retval);
...
static int syna_tcm_spi_probe(struct spi_device *spi)
{
	...
	if (retval < 0) {
		LOGE(&spi->dev,
				"Failed to set up SPI protocol driver, retval=%d\n", retval);
		return retval;
	}
	...
	if (retval < 0) {
		LOGE(&spi->dev,
				"Failed to add platform device, retval=%d\n", retval);
		return retval;
	}
