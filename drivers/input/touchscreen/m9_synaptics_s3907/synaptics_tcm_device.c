...
static void device_capture_touch_report(unsigned int count)
{
	...
	case REPORT_TOUCH:
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to allocate memory for device_hcd->report.buf, retval=%d\n", retval);
			report = false;
			goto exit;
		}
		idx = 4;
	...
	if (size) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to copy touch report data, retval=%d\n", retval);
			report = false;
			goto exit;
		} else {
			offset += size;
...
static int device_capture_touch_report_config(unsigned int count)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to allocate memory for tcm_hcd->config.buf, retval=%d\n", retval);
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to copy touch report config data, retval=%d\n", retval);
		UNLOCK_BUFFER(tcm_hcd->config);
		return retval;
	}
		...
		size_t count, loff_t *f_pos)
{
	...
	if (device_hcd->raw_mode) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to allocate memory for device_hcd->resp.buf, retval=%d\n", retval);
		...
		size_t count, loff_t *f_pos)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to allocate memory for device_hcd->out.buf, retval=%d\n", retval);
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to write command 0x%02x, retval=%d\n",
				device_hcd->out.buf[0], retval);
		UNLOCK_BUFFER(device_hcd->resp);
		UNLOCK_BUFFER(device_hcd->out);
		goto exit;
	}
	...
	if (count && device_hcd->out.buf[0] == CMD_SET_TOUCH_REPORT_CONFIG) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to capture touch report config, retval=%d\n", retval);
		}
	}
...
static int device_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (rmidev_major_num) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to register char device, retval=%d\n", retval);
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to allocate char device, retval=%d\n", retval);
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to add char device, retval=%d\n", retval);
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to create class, retval=%d\n", retval);
...
/* 	LOGE(tcm_hcd->pdev->dev.parent, "-----enter-----%s---irq_gpio = %d\n", __func__, bdata->irq_gpio);
	if (bdata->irq_gpio >= 0) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to export GPIO, retval=%d\n", retval);
		} else {
			retval = gpio_export_link(&tcm_hcd->pdev->dev,
					"attn", bdata->irq_gpio);
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
						"Failed to export GPIO link, retval=%d\n", retval);
			}
		}
	} */
