...
static int recovery_parse_romboot_ihex(void)
{
	...
	for (record = 0; record < recovery_hcd->ihex_records; record++) {
		...
		if (retval != 7) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to read ihex record, retval=%d\n", retval);
			return -EINVAL;
		}
		...
		if (type == 0x00) {
			...
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
						"Failed to add data entry for data0, retval=%d\n", retval);
				return retval;
			}
			...
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
						"Failed to add data entry for data1, retval=%d\n", retval);
				return retval;
			}
		...
		unsigned int in_size)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to write command CMD_SPI_MASTER_WRITE_THEN_READ_EXTENDED, retval=%d\n", retval);
		goto exit;
	}
...
static int recovery_flash_romboot_status(struct syna_tcm_hcd *tcm_hcd)
{
	...
	for (idx = 0; idx < STATUS_CHECK_RETRY; idx++) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to write JEDEC_READ_STATUS, retval=%d\n", retval);
			return retval;
		}
...
static int recovery_flash_romboot_erase(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to write JEDEC_WRITE_ENABLE, retval=%d\n", retval);
		return retval;
	}
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to write JEDEC_CHIP_ERASE, retval=%d\n", retval);
		return retval;
	}
		...
		unsigned char *image_buf, unsigned int image_size)
{
	...
	for (idx = 0; idx < pages; idx++) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to get correct status, retval=%d\n", retval);
			return retval;
		}
...
static void recovery_do_romboot_recovery(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to parse ihex data, retval=%d\n", retval);
		goto do_program_exit;
	}
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to program chip, retval=%d\n", retval);
		goto do_program_exit;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to do recovery, retval=%d\n", retval);
		goto exit;
	}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to copy ihex data, retval=%d\n", retval);
...
static int recovery_device_reset(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to write F$35 command, retval=%d\n", retval);
		return retval;
	}
...
static int recovery_add_padding(unsigned int *words)
{
	...
	while (padding) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to add data entry, retval=%d\n", retval);
			return retval;
		}
...
static int recovery_parse_f35_ihex(void)
{
	...
	for (record = 0; record < recovery_hcd->ihex_records; record++) {
		...
		if (type == 0x00) {
			...
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
						"Failed to add padding, retval=%d\n", retval);
				return retval;
			}
...
static int recovery_check_status(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to read status, retval=%d\n", retval);
		return retval;
	}
...
static int recovery_write_flash(void)
{
	...
	while (entries_written < recovery_hcd->data_entries) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to copy chunk data, retval=%d\n", retval);
			return retval;
		}
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to write chunk data, retval=%d\n", retval);
			return retval;
		}
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to get no error recovery mode status, retval=%d\n", retval);
		return retval;
	}
...
static int recovery_poll_erase_completion(void)
{
	...
	do {
		...
		do {
			...
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
						"Failed to read command status, retval=%d\n", retval);
				return retval;
			}
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to read flash status, retval=%d\n", retval);
			return retval;
		}
...
exit:
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to get erase completion, retval=%d\n", retval);
	}
...
static int recovery_erase_flash(void)
{
	...
	if (recovery_hcd->f35_addr.command_base) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to wait for erase completion, retval=%d\n", retval);
			return retval;
		}
	} else {
...
static int recovery_set_up_recovery_mode(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				...
				STR(CMD_REBOOT_TO_ROM_BOOTLOADER), retval);
		return retval;
	}
...
static int recovery_do_f35_recovery(void)
{
	...
	if (recovery_hcd->set_up_recovery_mode) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to set up recovery mode, retval=%d\n", retval);
			return retval;
		}
	}
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to read PDT entry, retval=%d\n", retval);
		return retval;
	}
	...
	if (IS_NOT_FW_MODE(tcm_hcd->id_info.mode)) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to run application firmware, retval=%d\n", retval);
			return retval;
		}
	}
