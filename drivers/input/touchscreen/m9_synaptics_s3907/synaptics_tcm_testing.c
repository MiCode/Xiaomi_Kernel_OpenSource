...
#define DEFAULT_SEQ_FILE_SIZE				200 * 1024
...
\
	...
	if (retval < 0) { \
		LOGE(tcm_hcd->pdev->dev.parent, \
				"Failed to do "#t_name" test, retval=%d\n", retval); \
		goto exit; \
	} \
\
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
			"Failed to copy report data, retval=%d\n", retval);
	} else {
		retval = readlen;
	}
...
static int testing_load_testlimits(enum test_code testcode, unsigned int gapdiff)
{
	...
	switch (tcm_hcd->lockdown_info[0]) {
		case M9_TOUCH_PANEL_MAKER:
			sprintf(limit_file_name, "%s", SYNA_TCM_TESTING_LIMITS_FILE_NAME);
			break;
		case M9_SECOND_TOUCH_PANEL_MAKER:
			sprintf(limit_file_name, "%s", SYNA_TCM_SECOND_TESTING_LIMITS_FILE_NAME);
			break;
		default:
			LOGE(tcm_hcd->pdev->dev.parent,
				"error can not load csv for touch\n");
			break;
	}
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to request limit file:%s, retval=%d\n",
				limit_file_name, retval);
		goto exit;
	}
	...
	if (dest_buffer0) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to allocate memory for PT%d buf, retval=%d\n", testcode, retval);
			goto exit;
		}
	...
	if (dest_buffer1) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to read limts data for %s [PT%d] from csv file, retval=%d\n", test_name1, testcode, retval);
			goto exit;
		}
	}
...
static int testing_result_save_csv(char *buf, unsigned int size)
{
	...
	if ((buf != NULL) && size > 0) {
		...
		retval = fs_write(buf, size);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to write data to file, retval=%d\n", retval);
		}
	}
	return retval;
}
...
static int testing_run_prod_test_item(enum test_code test_code)
{
	...
	if (tcm_hcd->features.dual_firmware &&
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to run production test firmware, retval=%d\n", retval);
			return retval;
		}
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to allocate memory for testing_hcd->out.buf, retval=%d\n", retval);
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				...
				STR(CMD_PRODUCTION_TEST), retval);
...
static void testing_copy_resp_to_output(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to allocate memory for testing_hcd->output.buf, retval=%d\n", retval);
		UNLOCK_BUFFER(testing_hcd->output);
		return;
	}
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to copy test resp data, retval=%d\n", retval);
		UNLOCK_BUFFER(testing_hcd->output);
		return;
	}
...
static void testing_standard_frame_output(bool image_only)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to copy header data, retval=%d\n", retval);
		UNLOCK_BUFFER(testing_hcd->output);
		return;
	}
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to copy test data, retval=%d\n", retval);
		UNLOCK_BUFFER(testing_hcd->resp);
		UNLOCK_BUFFER(testing_hcd->output);
		return;
	}
...
static int testing_pt01_trx_trx_short(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to load PT%d limits from csv file, retval=%d\n", TEST_PT1_TRX_TRX_SHORTS, retval);
		retval = -EINVAL;
		goto exit;
	}
...
static int testing_pt03_trx_ground(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to load PT%d limits from csv file, retval=%d\n", TEST_PT3_TRX_GROUND_SHORTS, retval);
		retval = -EINVAL;
		goto exit;
	}
...
static int testing_pt18_hybrid_abs_raw(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to load PT%d limits from csv file, retval=%d\n", TEST_PT18_HYBRID_ABS_RAW, retval);
		retval = -EINVAL;
		goto exit;
	}
	if ((limits_size != frame_size) ||
...
static int testing_reset_open(void)
{
	...
	if (tcm_hcd->id_info.mode == MODE_APPLICATION_FIRMWARE) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to enter bootloader mode, retval=%d\n", retval);
			return retval;
		}
	} else {
...
static int testing_pt22_trans_raw_cap(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to load PT%d limits from csv file, retval=%d\n", TEST_PT22_TRANS_RAW_CAP, retval);
		retval = -EINVAL;
		goto exit;
	}
...
static int testing_self_test(char *buf)
{
	...
	/* PT1 Test (TEST_PT1_TRX_TRX_SHORTS) */
	...
	/* PT03 Test (TEST_PT3_TRX_GROUND_SHORTS) */
	...
	/* PT22 Test (TEST_PT22_TRANS_RAW_CAP) */
	...
	/* PT10 Test (TEST_PT10_DELTA_NOISE) */
	...
	retval = testing_pt05_full_raw();
	if (retval < 0) {
		goto exit;
	}
	result_flag |= (testing_hcd->result) ? (1<<TEST_PT5_FULL_RAW_CAP) : 0;
	offset = testing_save_output(save_buf, offset, NULL);

	/* PT18 Test (TEST_PT18_HYBRID_ABS_RAW) */
	...
	cnt = snprintf(result_info, RESULT_INFO_LEN,
					"Panel Test Result = %s: PT1 = %s  PT3 = %s  PT22 = %s  GAP_DIFF = %s  PT10 = %s  PT18 = %s  PT5 = %s\n",
					(testing_hcd->result) ? "pass" : "fail",
					...
					(result_flag & (1<<TEST_PT5_FULL_RAW_CAP)) ? "pass":"fail");
...
static int auto_test_result_show(struct seq_file *m, void *v)
{
	...
	struct syna_tcm_hcd *tcm_hcd = m->private;
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
			"Failed to do self_test, retval=%d\n", retval);
		goto exit;
	}
...
{
	return single_open_size(file, auto_test_result_show, PDE_DATA(inode), DEFAULT_SEQ_FILE_SIZE);
}
...
static int testing_xiaomi_self_test(char *buf)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
			"Failed to do self_test, testing_hcd->result = %d, retval=%d\n",
			testing_hcd->result, retval);
		goto exit;
	}
...
static int testing_xiaomi_report_data(int report_type, char *buf)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
			"Failed to read RID %d data, retval=%d\n", report_type, retval);
		goto exit;
	}
...
static void testing_report(void)
{
	...
	if (testing_hcd->report_index == 0) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to allocate memory for testing_hcd->report.buf, retval=%d\n", retval);
			UNLOCK_BUFFER(testing_hcd->report);
			return;
		}
	}
...
static int testing_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to create sysfs bin file, retval=%d\n", retval);
		goto err_sysfs_create_bin_file;
	}

	proc_entry = proc_create_data("auto_test", 0660, NULL, &auto_test_ops, tcm_hcd);
...
{
	remove_proc_entry("auto_test", NULL);
