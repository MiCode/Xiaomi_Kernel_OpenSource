...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (on) {
		...
		ret = regulator_enable(cd->pmavdd);
		if (ret) {
			regulator_disable(cd->avdd);
			regulator_disable(cd->iovdd);
			ts_err("Failed to enable pmavdd:%d", ret);
			return ret;
		}
	...
	ret = regulator_disable(cd->pmavdd);
	if (ret)
		ts_err("Failed to disable pmavdd:%d", ret);
