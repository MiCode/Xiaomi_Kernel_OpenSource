...
*******************************************************************************/
static s32 seg_read_chip_reg(PST_TP_DEV p_dev, u16 addr, u8 *p_buf,
				u16 buf_len)
{
	struct device *dev;
	...
	for (i = 0; i < packages; i++) {
		...
		ret =
			hw_ops->read_trans(ts_dev, tmp_addr,
						(u8 *) &p_buf[tmp_addr -
						start_addr], send_len);
		if (ret < 0) {
			ret = 0;
		...
		if (ret == 0) {
			/*board_print_error("segment read error!\n");*/
			return 0;
		}
			      ...
			      u16 buf_len)
{
	...
	for (i = 0; i < packages; i++) {
		...
		hw_ops = ts_dev->hw_ops;
		ret =
			hw_ops->write_trans(ts_dev, tmp_addr,
					(u8 *) &p_buf[tmp_addr -
					start_addr],
					write_len);
		if (ret < 0) {
...
extern s32 board_enter_sleep_mode(IN PST_TP_DEV p_dev)
{
	...
	for (i = 0; i < retry_cnt; i++) {
		ret =
			chip_reg_write(p_dev, p_dev->cmd_set.sleep_cmd.addr,
				...
				u8 b_filter_data, ptr32 p_cur_data)
{
	...
	if ((p_dev->chip_type == TP_NORMANDY)
		...
		for (i = 0; i < retry_cnt; i++) {
			ret =
				sync_read_rawdata(p_dev->rawdata_addr,
					...
					u8 b_filter_data, ptr32 p_cur_data)
{
	...
	if ((p_dev->chip_type == TP_NORMANDY)
		...
		for (i = 0; i < retry_cnt; i++) {
			...
			ret =
				sync_read_rawdata(p_dev->diffdata_addr,
...
extern u16 board_get_volt_value(PST_TP_DEV p_dev, u8 adc_id,
				u16 aver_len)
{
	return 1;
}

extern s32 board_set_power_volt(PST_TP_DEV p_dev, u16 volt,
				u16 aver_len, u8 cnt)
{
	return 1;
}
