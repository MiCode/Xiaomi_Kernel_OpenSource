...
{				/* I - Node to delete */
	...
	*next;			/* Next node */
	...
	for (curr = node->child; curr; curr = next) {
		/*
		 * Get the next node...
		 */
		next = curr->child;
...
int /* O - Reference count */ mxmlGetRefCount(mxml_node_t *node)
{				/* I - Node */
	/*
	 * Range check input...
	 */
...
int /* O - New reference count */ mxmlRelease(mxml_node_t *node)
{				/* I - Node */
	if (node) {
		if ((--node->ref_count) <= 0) {
			mxmlDelete(node);
...
int /* O - New reference count */ mxmlRetain(mxml_node_t *node)
{				/* I - Node */
	if (node)
		return (++node->ref_count);
	else
...
{				/* I - Node */
		...
		if (node->value.element.num_attrs) {
			for (i = 0; i < node->value.element.num_attrs; i++) {
				...
				if (node->value.element.attrs[i].value)
					free(node->value.element.attrs[i].
						value);
			}
	...
	case MXML_CUSTOM:
		if (node->value.custom.data && node->value.custom.destroy)
			(*(node->value.custom.destroy)) (node->value.custom.
							 data);
		break;
	default:
		break;
...
{				/* I - Node type */
	 ...
	 */
	node = calloc(1, sizeof(mxml_node_t));
	if (node == NULL) {
