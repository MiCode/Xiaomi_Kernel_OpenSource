...
extern s32 parse_key_data(PST_TP_DEV p_tp_dev, u8 *cfg)
{
	...
	key_start_addr =
		parse_addr_from_str(p_tp_dev, KEY_START_ADDR, cfg);
	sen_as_key = parse_cfg_value(p_tp_dev, SEN_AS_KEY_ADDR, cfg);
	key_en = parse_cfg_value(p_tp_dev, KEY_EN, cfg);

	ret =
		get_sc_drv_sen_key_num(p_tp_dev, key_start_addr, key_en,
				sen_as_key, key_com_port_num,
				max_key_num);
	return ret;
}
				...
				PST_TEST_PARAM p_test_param)
{
	...
	obj_set =
		mxmlFindElement(top, top, "OBJ_SETTING", NULL, NULL,
				MXML_DESCEND);

	if ((obj_set == NULL) || (obj_set->child == NULL)) {
		board_print_error
			("can not find OBJ_SETTING in order!\n");
		return 0;
	}

	parent_xnode =
		mxmlFindElement(obj_set, obj_set, "powerVolt", NULL, NULL,
				MXML_DESCEND);
	...
	/*find chip type*/
	parent_xnode =
		mxmlFindElement(obj_set, obj_set, "tpMainType", NULL, NULL,
				MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error
			("can not find tpMainType in order!\n");
		return 0;
	}
	...
	parent_xnode =
		mxmlFindElement(parent_xnode, obj_set, "tpSubType", NULL,
				NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
	...
	chip_ini =
		mxmlFindElement(top, top, "chipIni", NULL, NULL,
				MXML_DESCEND);
	...
	ret =
		header_file_init_dev(p_tp_dev, p_test_param->test_cfg,
				&p_test_param->cfg_len);
	/*init device with ini content
	//    ret = init_dev_from_order_content((u8*)chip_ini->child->value.opaque,p_tp_dev,
	//        p_test_param->test_cfg, &p_test_param->cfg_len);
	*/
	return ret;
}
...
extern s32 parse_order_config(PST_TEST_PARAM p_test_param)
{
	...
	ret =
		alloc_mem_in_heap((void **)&p_test_param->test_cfg,
				TP_MAX_CFG_LEN);
	if (ret == 0) {
	...
	parent_xnode =
		mxmlFindElement(top, top, "chipConfigProc", NULL, NULL,
				MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error
	...
	/*init test config */
	child_xnode =
		mxmlFindElement(parent_xnode, parent_xnode, "config", NULL,
				NULL, MXML_DESCEND);
	if ((child_xnode != NULL) && (child_xnode->child != NULL)) {
		ret =
			hexstr_to_array(p_test_param->test_cfg,
	...
	child_xnode =
		mxmlFindElement(parent_xnode, parent_xnode, "option", NULL,
			NULL, MXML_DESCEND);
	if ((child_xnode != NULL) && (child_xnode->child != NULL)) {
...
extern s32 parse_test_item_id(PST_TEST_PARAM p_test_param)
{
	...
	parent_xnode =
		mxmlFindElement(top, top, "testItemSet", NULL, NULL,
				MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
	...
	ret =
		alloc_mem_in_heap((void **)&p_test_param->
				p_test_item_id_set,
				MAX_TEST_NUM *
	...
	/*find test item*/
	test_item =
		mxmlFindElement(parent_xnode, parent_xnode, "testItem",
			NULL, NULL, MXML_DESCEND);
	while ((test_item != NULL) && (test_item->child != NULL)) {
		/*test item is select or not*/
		is_select =
			mxmlFindElement(test_item, test_item, "bSelected",
					NULL, NULL, MXML_DESCEND);
		if ((is_select != NULL) && (is_select->child != NULL)
			...
			if ((test_id != NULL)
				&& (test_id->child != NULL)) {
				itest_id =
					str_to_int(test_id->child->value.
						opaque);
				if (is_mach_test_item(itest_id) != 1) {
					test_item =
						mxmlFindElement(test_item,
							parent_xnode,
							"testItem",
							NULL, NULL,
							MXML_NO_DESCEND);
				...
				board_print_debug("test id :%d\n",
						itest_id);
				p_test_id_set[test_num].test_id =
					str_to_int(test_id->child->value.
						opaque);
		...
		/*find next test item*/
		test_item =
			mxmlFindElement(test_item, parent_xnode, "testItem",
				...
				p_raw_test_param)
{
	...
	parent_xnode =
		mxmlFindElement(top, top, "needCheckNodeParam", NULL, NULL,
				MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
	...
	ret =
		alloc_mem_in_heap((void **)&p_raw_test_param->
				p_need_check_node,
				...
				u16 data_len)
{
	...
	/*init need check key*/
	ret =
		alloc_mem_in_heap((void **)&p_need_check_key->p_data_buf,
			data_len);
	if (ret == 0) {
		board_print_error
	...
	child_xnode =
		mxmlFindElement(parent_xnode, top, "needCheckKey", NULL,
				NULL, MXML_DESCEND);
	if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
		board_print_debug
	...
	p_need_check_key->len =
		hexstr_to_array(p_need_check_key->p_data_buf, tmp_value,
			data_len);
	return ret;
}
				...
				u16 data_len)
{
	...
	i = 0;
	parent_xnode =
		mxmlFindElement(special_xnode, special_xnode, "node", NULL,
				NULL, MXML_DESCEND);
	while ((special_xnode != NULL)
		...
		i++;
		parent_xnode =
			mxmlFindElement(parent_xnode, special_xnode, "node",
					NULL, NULL, MXML_DESCEND);
	}
...
extern s32 parse_uniform_limit(PST_RAWDATA_TEST_PARAM p_raw_test_param)
{
	...
	if (ret == 0) {
		board_print_error
			("uniform limit malloc memory error!\n");
		return 0;
	}
	child_xnode =
				...
				PST_SUB_TEST_ITEM_ID_SET p_sub_id_set)
{
	...
	test_item =
		mxmlFindElement(top, top, "rawdataTestParam", NULL, NULL,
				MXML_DESCEND);
	...
	ret =
		alloc_mem_in_heap((void **)&p_test_param->p_raw_test_param,
	...
	/*special test node*/
	ret =
		parse_special_node(p_raw_test_param,
...
extern s32 parse_short_test_param(PST_TEST_PARAM p_test_param)
{
	...
	ret =
		alloc_mem_in_heap((void **)&p_test_param->p_short_param,
...
extern s32 parse_version_test_param(PST_TEST_PARAM p_test_param)
{
	...
	p_ver_param =
		(PST_VERSION_PARAM) p_test_param->p_version_test_param;
...
extern s32 parse_flash_test_param(PST_TEST_PARAM p_test_param)
{
	...
	p_flash_param =
	    (PST_FLASH_TEST_PARAM) p_test_param->p_flash_test_param;
	p_flash_param->config_arr = NULL;
	p_flash_param->delay_time = 0;

	test_item =
	    mxmlFindElement(top, top, "flashTestParam", NULL, NULL,
			    MXML_DESCEND);
	if ((test_item == NULL) || (test_item->child == NULL)) {
		board_print_error
	...
	parent_xnode =
	    mxmlFindElement(test_item, test_item, "option", NULL, NULL,
			    MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error
		    ("can not find flash option,order error!\n");
		return 0;
	}
...
static s32 parse_ic_name(PST_TEST_PARAM p_test_param)
{
	...
	parent_xnode =
		mxmlFindElement(obj_set, obj_set, "chipName", NULL, NULL,
			MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
...
extern s32 release_order_config(PST_TEST_PARAM p_test_param)
{
	...
	if (p_test_param->test_cfg != NULL) {
		ret =
			free_mem_in_heap_p((void **)&p_test_param->
					test_cfg);
	}
...
extern s32 release_flash_test_param(PST_TEST_PARAM p_test_param)
{
	...
	if (p_flash_param != NULL) {
		if (p_flash_param->config_arr != NULL) {
			ret =
				free_mem_in_heap((void *)p_flash_param->
						config_arr);
		}
			...
			u32 *p_len)
{
	...
	/*read decode order*/
	ret = read_file_decoder(p_tp_dev, pp_buf);
	if (ret == 0) {
