...
static int /* O  - Character value or EOF on error */ mxml_get_entity(mxml_node_t *parent,	/* I  - Parent node */
	...
	while ((ch = (*getc_cb) (p, &encoding)) != EOF) {
			...
			*bufptr = '\0';
			...
			case MXML_CUSTOM:
				if (global->custom_load_cb) {
					...
					node =
						mxmlNewCustom(parent, NULL, NULL);
		...
		if (ch == '<') {
			...
			*bufptr = '\0';
			 ...
			 */
			ch = mxml_get_entity(parent, p, &encoding,
						getc_cb);
			if (ch == EOF)
				goto error;
...
{				/* I - Global data */
	...
	*next;			/* Next node */
	...
	for (curr = node; curr; curr = next) {
		...
		case MXML_ELEMENT:
			col =
				mxml_write_ws(curr, p, cb, MXML_WS_BEFORE_OPEN, col,
					putc_cb);
