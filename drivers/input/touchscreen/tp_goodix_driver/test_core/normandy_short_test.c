...
static s32 wait_status(PST_TP_DEV p_dev, u16 sta_addr, u8 right_sta,
			u16 time_out);
static s32 send_short_param(ptr32 p_data);
static s32 wait_short_end(PST_TP_DEV p_dev);

static u16 calc_short_gnd_vdd(u16 diff_code, s32 avdd,
			u8 *p_short_opt);
...
/*********test program*************/
	/* (1)Judge whether exist short code in current touch panel chip firmware
	//  We can read test parameter to judge.There is no short code in TP_GT9P now.
	*/
...
static s32 run_short_code(IN ptr32 p_data)
{
	...
	for (i = 0; i < retry_cnt; i++) {
		...
		/*wait short ready*/
		ret =
			wait_status(p_dev, NORM_SHORT_STA_ADDR,
...
static s32 analyse_short_res(IN_OUT ptr32 p_data)
{
	...
	if (0x08 == (short_flag & 0x08)) {
		...
		for (i = 0; i < data_len_tmp; i += 2) {
			...
			/*calculate short_r*/
			short_r =
				calc_short_gnd_vdd(diff_code_tmp,
	...
	for (i = 0; i < retry_cnt; i++) {
		...
		ret =
			chip_reg_read(p_dev, NORM_SHORT_DRVSELFCODE_ADDR,
				p_tmp_data_arr, data_len_tmp);
		if (1 == ret) {
			break;
		}
	...
	for (i = 0; i < retry_cnt; i++) {
		...
		*/
		ret =
			chip_reg_read(p_dev, NORM_SHORT_NUMBER_ADDR, chn_short_cnt,
				3);
		if (1 == ret) {
			break;
		}
		...
		*/
		...
		for (i = 0; i < chn_short_cnt[0]; i++) {
			...
			for (j = 0; j < retry_cnt; j++) {
				ret =
					chip_reg_read(p_dev, chn_short_mess_addr,
						p_tmp_data_arr, data_len_tmp);
				if (1 == ret) {
					break;
				}
			...
			*/
			chn_id_tmp1 =
				(p_tmp_data_arr[0] << 8) + p_tmp_data_arr[1];
			{
				/*Tx*/
