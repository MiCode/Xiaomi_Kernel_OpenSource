...
extern u32 decrypt_generate_crc32(u32 *crc32_table, u8 *buf, u32 len)
{
	...
	for (i = 0; i < len; ++i) {
		...
		old_crc32 =
		    ((old_crc32 >> 8) & 0xFFFFFF) ^ crc32_table[t];
	}
	/*old_crc32 = ~old_crc32;*/
...
extern s32 read_file_decoder(PST_TP_DEV p_tp_dev, s8 **file_content)
{
	...
	u32 crc32_table[256];
	char order_name[64];
	s8 ver_date[VECTOR_BYTE_LEN + 1] = {0,};
	...
	if (ts_bdata && ts_bdata->limit_name)
		strlcpy(order_name, ts_bdata->limit_name,
				sizeof(order_name));
	else
		strlcpy(order_name, default_order_name,
				sizeof(order_name));

	/*read content*/
	for (i = 0; i < 2; i++) {
		ret =
			request_firmware(&firmware, order_name,
				     dev);
		if (ret < 0) {
			board_delay_ms(1);
		} else {
	...
	/*malloc memory*/
	cont_buf = (void *)vmalloc(file_len + 1);
	if (cont_buf == NULL) {
		board_print_error("malloc buf error!");
	}

	*file_content = (void *)vmalloc(file_len + 1);
	if ((cont_buf == NULL) || (*file_content == NULL)) {
	...
	crc_calc =
		decrypt_generate_crc32(crc32_table, (u8 *) cont_buf,
