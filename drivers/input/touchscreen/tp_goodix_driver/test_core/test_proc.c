...
extern s32 test_proc_func(IN_OUT PST_TEST_PROC_DATA p_test_proc_data)
{
	...
	/*(1)callback function execute before all test items start
	//   1)you can notify all tests starting
	//       2)you can prepare some thing for all tests
	*/
	...
	/*(3)notify total result, by callback function
	//   1)in arm we need use usart send result
	//       2)in pc  we need show result in ui,or wo need send result by socket
	*/
