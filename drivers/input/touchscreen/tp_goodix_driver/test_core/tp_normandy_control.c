...
{
	return normandy_write_cfg_arr(p_dev, p_dev->cfg,
					p_dev->cfg_len);
}
				...
				IN u16 cfg_len)
{
	...
	for (i = 0; i < retry_cnt; i++) {
		buf_arr[0] = 0x00;
		ret =
			chip_reg_read(p_dev, p_dev->real_cmd_addr, buf_arr,
				1);
	...
	for (i = 0; i < retry_cnt; i++) {
		...
		buf_arr[2] = 0x100 - buf_arr[0] - buf_arr[1];
		ret =
			chip_reg_write(p_dev, p_dev->real_cmd_addr, buf_arr,
				3);
		if (1 == ret) {
			board_delay_ms(50);
			/*wait ready status*/
	...
	for (i = 0; i < retry_cnt; i++) {
		ret =
			chip_reg_write(p_dev, p_dev->cfg_start_addr, config,
				cfg_len);
		if (1 == ret) {
			break;
		}
					...
					IN u16 cfg_len)
{
	...
	//we assume that the cfg is valid
	//so we do not check it
	//parse normandy cfg protocol
	//Head:4 bytes    |byte0:version|byte1:config refresh|byte2:package total num|byte3:check sum|
	//Package1                |byte0:package id|byte1:package len|byte2~byte n content|byte n+1 check sum|
	//Package2                |byte0:package id|byte1:package len|byte2~byte n content|byte n+1 check sum|
	*/
				...
				IN u8 b_force)
{
	...
	if (b_ena) {	/*enter doze*/
		...
		if (p_dev->doze_ref_cnt == 0 || b_force) {
			...
			for (i = 0; i < retry_cnt; i++) {
				ret =
					board_write_chip_reg(p_dev, 0x30f0,
							&temp_data, 1);
				if (1 == ret) {
					break;
				}
