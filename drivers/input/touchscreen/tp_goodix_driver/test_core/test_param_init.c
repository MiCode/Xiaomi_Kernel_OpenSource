			...
			PST_TEST_PARAM p_test_param)
{
	...
	for (i = 0; i < item_num; i++) {
		...
		case TP_RAWDATA_TEST_ITEMS_SET_ID:
			{
				ret =
					rawdata_test_before_func((p_test_proc_data->pp_test_item_arr)[i], p_test_param);
			} break;
		...
		case TP_SHORT_TEST_ITEM_ID:
			{
				ret =
					short_test_before_func((p_test_proc_data->pp_test_item_arr)[i], p_test_param);
			}
			break;
		...
		case TP_VERSION_TEST_ITEM_ID:
			{
				ret =
					version_test_before_func((p_test_proc_data->pp_test_item_arr)[i], p_test_param);
			}
			break;
		...
		case TP_CHIP_CFG_PROC_ITEM_ID:
			{
				ret =
					send_cfg_test_before_func((p_test_proc_data->pp_test_item_arr)[i], p_test_param);
			}
			break;
		...
		case TP_RECOVER_CFG_ITEM_ID:
			{
				ret =
					cfg_recover_test_before_func((p_test_proc_data->pp_test_item_arr)[i]);
			}
			break;
		...
		case TP_FLASH_TEST_ITEM_ID:
			{
				ret =
					flash_test_before_func((p_test_proc_data->pp_test_item_arr)[i], p_test_param);
			}
			break;
	...
	for (i = 0; i < item_num; i++) {
		if ((p_test_proc_data->pp_test_item_arr)[i] != NULL) {
			...
			case TP_RAWDATA_TEST_ITEMS_SET_ID:
				{
					ret =
						release_rawdata_test_item((p_test_proc_data->pp_test_item_arr)[i]);
				}
				break;
			...
			case TP_SHORT_TEST_ITEM_ID:
				{
					ret =
						release_short_test_item((p_test_proc_data->pp_test_item_arr)[i]);
				}
				break;
			...
			case TP_VERSION_TEST_ITEM_ID:
				{
					ret =
						version_test_finished_func((p_test_proc_data->pp_test_item_arr)[i]);
				}
				break;
			...
			case TP_CHIP_CFG_PROC_ITEM_ID:
				{
					ret =
						release_cfg_test_item((p_test_proc_data->pp_test_item_arr)[i]);
				}
				break;
			...
			case TP_FLASH_TEST_ITEM_ID:
				{
					ret =
						release_flash_test_item((p_test_proc_data->pp_test_item_arr)[i]);
				}
				break;
			default:
				{
				...
				void *p_drv_dev)
{
	...
	ret =
		alloc_mem_in_heap((void **)&p_test_proc_data->p_dev,
				sizeof(ST_TP_DEV));
	if (0 == ret) {
				...
				pp_test_proc_data)
{
	...
	if (NULL != p_test_proc_data->p_dev) {
		if (p_test_proc_data->p_dev->chip_type != 0xffff) {
			if (p_test_proc_data->p_dev->chip_type ==
				TP_PHOENIX) {
				ret =
					chip_reg_write(p_test_proc_data->
							p_dev,
							p_test_proc_data->
							p_dev->cmd_set.
							hid_ena_cmd.addr,
	...
	if (NULL != p_test_proc_data->pp_test_item_arr) {
		for (i = 0; i < p_test_proc_data->test_item_num; i++) {
			ret =
				free_mem_in_heap(p_test_proc_data->
						pp_test_item_arr[i]);
...
extern int get_tp_rawdata(void *p_drv_dev, char *buf, int *buf_size)
{
	...
	ret =
		chip_mode_select((p_test_proc_data->
