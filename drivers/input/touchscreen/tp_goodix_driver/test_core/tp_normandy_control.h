...
extern s32 normandy_force_update_cfg(PST_TP_DEV p_dev, u8 *config,
					u16 cfg_len);

extern s32 normandy_write_cfg(PST_TP_DEV p_dev);
extern s32 normandy_read_cfg(PST_TP_DEV p_dev);

extern s32 normandy_write_cfg_arr(PST_TP_DEV p_dev, u8 *config,
					u16 cfg_len);
extern s32 normandy_read_cfg_arr(PST_TP_DEV p_dev, u8 *config,
					u16 cfg_len);

extern void normandy_cfg_update_chksum(u8 *config, u16 cfg_len);

extern s32 normandy_cmp_cfg(PST_TP_DEV p_dev, u8 *cfg1, u8 *cfg2,
					u16 cfg_len);

extern s32 normandy_write_custom_info(PST_TP_DEV p_dev,
					PST_CUSTOM_INFO_BYTE custom_info,
					u8 len);
