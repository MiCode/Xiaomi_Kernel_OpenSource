	...
	default:
		{
			for (i = 0; i < retry_cnt; i++) {
				ret =
					board_write_chip_reg(p_dev, addr,
							p_buf,
							buf_len);
				if (1 == ret) {
					break;
			...
			OUT u8 *p_buf, IN u16 buf_len)
{
	...
	case TP_OSLO:
		{
			ret =
				normandy_reg_read(p_dev, addr, p_buf,
						buf_len);
		}
		break;
	default:
		{
			for (i = 0; i < retry_cnt; i++) {
				ret =
					board_read_chip_reg(p_dev, addr,
							p_buf, buf_len);
				if (1 == ret) {
					break;
				}
				...
				IN u8 *p_buf, IN u16 buf_len)
{
	...
	if (1 == ret) {
		ret =
			alloc_mem_in_heap((void **)&p_buf_read_tmp,
					buf_len);
		if (0 == ret) {
			return ret;
		}
		...
		ret =
			chip_reg_read(p_dev, addr, p_buf_read_tmp, buf_len);
		if (1 == ret) {
			for (i = 0; i < buf_len; i++) {
...
extern s32 get_data_flag(IN PST_TP_DEV p_dev)
{
	...
	for (i = 0; i < retry_cnt; i++) {
		ret =
			chip_reg_read(p_dev, p_dev->syncflag_addr,
				&data_temp, 1);
		if ((1 == ret)
...
extern s32 clr_data_flag(IN PST_TP_DEV p_dev)
{
	...
	for (i = 0; i < retry_cnt; i++) {
		ret =
			chip_reg_write(p_dev, p_dev->syncflag_addr,
				&buf_tmp, 1);
		if (1 == ret) {
			break;
		}
				...
				IN u16 cfg_len)
{
	...
	case TP_OSLO:
		{
			ret =
				normandy_force_update_cfg(p_dev, config,
						cfg_len);
		}
		break;
	default:
			...
			IN u16 cfg_len)
{
	...
	case TP_OSLO:
		{
			ret =
				normandy_cmp_cfg(p_dev, cfg1, cfg2,
						cfg_len);
		}
		break;
	default:
			...
			IN u8 cmd_byte)
{
	...
	case TP_ALTO:
		{
			ret =
				chip_reg_write(p_dev, cmd_addr, &cmd_byte,
					1);
		}
		break;
	...
	default:
		{
			...
			ret =
				chip_reg_write(p_dev, cmd_addr, buf_tmp, 3);
			...
			IN u16 cfg_len)
{
	...
	case TP_OSLO:
		{
			ret =
				normandy_read_cfg_arr(p_dev, config,
						cfg_len);
		}
		break;
	default:
