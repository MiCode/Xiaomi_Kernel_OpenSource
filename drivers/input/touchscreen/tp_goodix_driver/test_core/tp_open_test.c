		...
		for (id = 0; id < node_num; id++) {
			...
			if (get_chn_need_check(p_test_item, id)) {
				data_tmp =
					get_chn_rawdata(p_test_item, id);
				para_tmp =
					get_chn_lower_limit(p_test_item,
							id);
				if (data_tmp < para_tmp) {
...
extern s32 rawdata_accord_test_func(IN_OUT ptr32 p_data)
{
	...
	if (0xFF != index) {
		...
		for (id = 0; id < node_num; id++) {
			if (get_chn_need_check(p_test_item, id)) {
				data_tmp =
					get_chn_accord(p_test_item, id);
				/*/upper limit*/
				para_tmp =
					get_chn_accord_limit(p_test_item,
							 id);
				if (data_tmp > para_tmp) {
	...
	if (0xFF != index) {
		...
		for (id = 0; id < key_num; id++) {
			if (get_key_need_check(p_test_item, id)) {
				...
				para_tmp =
					get_key_upper_limit(p_test_item,
							id);
				if (data_tmp > para_tmp) {
					/*mark this channel beyond upper limit count*/
					p_raw_test_res->
	...
	if (0xFF != index) {
		...
		for (id = 0; id < key_num; id++) {
			if (get_key_need_check(p_test_item, id)) {
				data_tmp = get_keydata(p_test_item, id);
				para_tmp =
					get_key_lower_limit(p_test_item,
							id);
				if (data_tmp < para_tmp) {
					/*mark this channel beyond lower limit count*/
					p_raw_test_res->
...
extern s32 rawdata_uniform_test_func(IN_OUT ptr32 p_data)
{
	...
	if (0xFF != index) {
		...
		if (data_tmp < para_tmp) {
			...
			p_raw_test_res->item_res_header.
				test_item_err_code.ptr_err_code_set[index +
								1] =
	...
	if (0xFF != index) {
		...
		for (id = 0; id < node_num; id++) {
			...
			if (get_chn_need_check(p_test_item, id)) {
				/*upper limit*/
				para_tmp =
					get_chn_offset_limit(p_test_item,
							id);
				if (data_tmp > para_tmp) {
...
extern s32 diffdata_test_sets_func(IN_OUT ptr32 p_data)
{
	...
	ret =
		chip_mode_select(p_test_item->p_tp_dev,
				CHIP_DIFF_DATA_MODE);
	if (0 == ret) {
	...
	while (1) {
		...
		ret =
			board_get_diffdata(p_test_item->p_tp_dev,
		...
		if ((cur_frame_total_num <= max_frame_total_num)
			...
			p_cur_data->cur_frame_id % add_frame_num)) {
			ret =
				analyze_difftest_res(p_cur_data->
						cur_frame_id,
						p_test_item);
			if (ret == 0) {
				break;
...
//      for (i = 0; i<3; i += 2)
//      {
//              if (pos_arr[i] != -1 && pos_arr[i + 1] != -1)
//              {
//                      tmp_data = get_chn_rawdata(p_test_item, (u16)pos_arr[i]);
//                      adj_data = get_chn_rawdata(p_test_item, (u16)pos_arr[i + 1]);
//                      tmp_diff = (tmp_data - adj_data) >0?(tmp_data-adj_data):(adj_data-tmp_data);
//                      tmp_data = (s32)(1000*tmp_diff/ cur_data);
//                      if (tmp_data > ret)
//                      {
//                              ret = tmp_data;
//                      }
...
extern s32 get_keydata(IN PST_TEST_ITEM p_test_item, IN u8 key_id)
{
	...
	if (key_id < p_dev->key_num) {
		tmp_id =
			p_dev->sc_sen_num * p_dev->sc_drv_num +
