...
extern s32 tp_chip_cfg_proc(IN_OUT ptr32 p_data)
{
	...
	if (p_param->option == CHIP_CFG_SEND) {
		/*write cfg*/
		ret &=
			force_update_cfg(p_dev, p_param->config,
					p_param->cfg_len);
		if (0 == ret) {
			p_test_res->item_res_header.test_item_err_code.
				ptr_err_code_set[0] =
...
/*
	//check cfg
	ret &=
	    cmp_cfg(p_dev, p_test_res->p_read_cfg, p_param->config,
		    p_param->cfg_len);
	if (0 == ret) {
		p_test_res->item_res_header.test_item_err_code.
		    ptr_err_code_set[0] = CFG_CHECK_TEST_CHECK_FAIL;
		p_test_res->item_res_header.test_result = TEST_NG;
		p_test_res->item_res_header.test_status = TEST_ABORTED;
		board_print_error
		    ("[send config test]cfg is not equal with chip config!\n");
		goto SEN_CFG_END;
	}
*/
...
extern s32 tp_send_spec_cfg_to_chip(IN_OUT ptr32 p_data)
{
	...
	ret &=
		force_update_cfg(p_dev, p_param->config, p_param->cfg_len);
	if (0 == ret) {
		p_test_res->item_res_header.test_item_err_code.
	...
	/*check cfg*/
	ret &=
		cmp_cfg(p_dev, p_test_res->read_config, p_param->config,
			p_param->cfg_len);
	if (0 == ret) {
		p_test_res->item_res_header.test_item_err_code.
...
extern s32 tp_flash_test(IN_OUT ptr32 p_data)
{
	...
	if ((p_param->option & FLASH_TEST_SEND_CFG_OPT) ==
		...
		/*write cfg*/
		ret &=
			force_update_cfg(p_dev, p_param->config_arr,
					p_param->cfg_len);
		if (0 == ret) {
			p_test_res->item_res_header.test_item_err_code.
				ptr_err_code_set[0] =
...
/*
	//check cfg
	ret &=
		cmp_cfg(p_dev, p_test_res->config_arr, p_param->config_arr,
			p_param->cfg_len);
	if (0 == ret) {
		p_test_res->item_res_header.test_item_err_code.
			ptr_err_code_set[0] = FLASH_TEST_CMP_ERROR;
		p_test_res->item_res_header.test_result = TEST_NG;
		ret = 1;
		board_print_info
			("[flash test]cfg is not equal with chip config!\n");
	}
*/
	p_test_res->item_res_header.test_status = TEST_FINISH;
