...
static short my_diffdata[TX_NUM_MAX][RX_NUM_MAX] = {{0}};
...
unsigned char FT5X46_TestItem_PanelDiffer_Uniformity_Test(bool *bTestResult)
{
	...
	if ( g_stCfg_FT5X46_BasicThreshold.PanelDiffer_Uniformity_CheckTx )
	{
		...
		Save_Test_Data(TxLinearity, 0, test_data.screen_param.tx_num, test_data.screen_param.rx_num, 1);
	}
	...
	if ( g_stCfg_FT5X46_BasicThreshold.PanelDiffer_Uniformity_CheckRx )
	{
		...
		Save_Test_Data(RxLinearity, 0, test_data.screen_param.tx_num, test_data.screen_param.rx_num, 2);
	}
...
unsigned char FT5X46_TestItem_WeakShortTest( bool *bTestResult )
{
	...
	if (i >= 5) {
		FTS_TEST_SAVE_INFO("\nStartScan Failed for several times.!\n");
		btmpresult = false;
		goto TEST_ERR;
	}
...
/************************************************************************
* Name: ReadDiffData(Same function name as FT_MultipleTest)
* Brief:  read Raw Data
* Input: Freq(No longer used, reserved), LineNum, ByteNum
* Output: pRevBuffer
* Return: Comm Code. Code = 0x00 is OK, else fail.
***********************************************************************/
unsigned char ReadDiffData(unsigned char Freq, unsigned char LineNum, int ByteNum, short *pRevBuffer)
{
	unsigned char ReCode = ERROR_CODE_COMM_ERROR;
	char I2C_wBuffer[3];
	int i, iReadNum;
	...
	if (ReCode == ERROR_CODE_OK) {
		sys_delay(10);
		ReCode = fts_i2c_read_write(I2C_wBuffer, 1,m_ucdiffData, tx_num*rx_num*2);
		if (ReCode != ERROR_CODE_OK) {
			FTS_TEST_SAVE_INFO("\nread rawdata fts_i2c_read_write Failed!1 ");
			goto READ_ERR;
		}
	...
	if (ReCode == ERROR_CODE_OK) {
		for (i = 0; i < (ByteNum >> 1); i++) {
			pRevBuffer[i] = ((short)(m_ucdiffData[i << 1] << 8) + m_ucdiffData[(i << 1) + 1]);
		}
	}
...
static unsigned char GetRawData(void)
{
	...
	for (iRow = 0; iRow < test_data.screen_param.tx_num; iRow++) {
		for (iCol = 0; iCol < test_data.screen_param.rx_num; iCol++) {
			...
			// my_diffdata[iRow][iCol] = (short)m_iTempRawData[iRow * test_data.screen_param.rx_num + iCol];
		}
	}
	printk("---------4-------\n");
	return ReCode;
...
static unsigned char WeakShort_GetAdcData( int AllAdcDataLen, int *pRevBuffer  )
{
	...
	ReCode = fts_i2c_read_write(pDataSend, 1, pDataSend + 1, iReadDataLen);
	if (ReCode == ERROR_CODE_OK) {
...
static struct proc_dir_entry *tp_data_dump_proc = NULL;

 static unsigned char Getdiff(void){
	 unsigned char ReCode = ERROR_CODE_OK;
	 int iRow = 0;
	 int iCol = 0;
	 u8 orgvalue = 0;
	 ...
	 ReCode = read_reg(0x06, &orgvalue); // read orginal value
	 FTS_INFO("0x06 orgvalue = %d", orgvalue);
	 ReCode = write_reg(0x06, 0x01);// switch to read diff
	 if (ReCode != ERROR_CODE_OK) {
		 printk("\nWriteReg error. \n");
		 return ReCode;
	 }
	 ...
	 memset(m_iTempDiffData, 0, sizeof(m_iTempDiffData));
	 memset(my_diffdata, 0, sizeof(my_diffdata));
	 ReCode = ReadDiffData( 1, 0xAA, tx_num*rx_num*2, m_iTempDiffData );
	 for (iRow = 0; iRow < tx_num; iRow++) {
		 for (iCol = 0; iCol < rx_num; iCol++) {
			my_diffdata[iRow][iCol] = m_iTempDiffData[iRow * rx_num + iCol];
		 }
	 }
	printk("---------4-------\n");

	ReCode = write_reg(0x06,orgvalue);
	if (ReCode != ERROR_CODE_OK) {
		printk("\nrecovery 0x06 reg value failed. \n");
		return ReCode;
	}
	msleep(100);
	return ReCode;
 ...
 static unsigned char GetRaw(void)
 {
	 unsigned char ReCode = ERROR_CODE_OK;
	 int iRow = 0;
	 int iCol = 0;
	 ...
	 memset(m_RawData, 0, sizeof(m_RawData));
	 memset(m_iTempRawData, 0, sizeof(m_iTempRawData));
	 ReCode = ReadRawData( 1, 0xAA, tx_num*rx_num* 2, m_iTempRawData );
	 for (iRow = 0; iRow < tx_num; iRow++) {
		 for (iCol = 0; iCol < rx_num; iCol++) {
			 m_RawData[iRow][iCol] = m_iTempRawData[iRow * rx_num + iCol];
		 }
	 }
	 printk("---------3-------\n");
	 return ReCode;
...
static int tp_data_dump_proc_show(struct seq_file *file, void* data)
{
	...
	read_reg(0x02,&tx_num);
	read_reg(0x03,&rx_num);
	...
	if (ReCode == ERROR_CODE_OK) {
		//----------------------------------------------------------Show DiffData
		seq_printf(file,"Tx:%2d\n", tx_num);
		seq_printf(file,"Rx:%2d\n", rx_num);
		for (iRow = 0; iRow < tx_num; iRow++) {
			for (iCol = 0; iCol < rx_num; iCol++) {
				seq_printf(file,"%-5d", my_diffdata[iRow][iCol]);
			}
			seq_printf(file,"\n");
		}
	}else {
		printk("cat proc tp_data_dum perror. error: %d\n", ReCode);
		goto TEST_ERR;
	}
	...
	if (ReCode == ERROR_CODE_OK) {
		...
		for (iRow = 0; iRow < tx_num; iRow++) {
			for (iCol = 0; iCol < rx_num; iCol++) {
				seq_printf(file,"%-6d", m_RawData[iRow][iCol]);
			}
			seq_printf(file,"\n");
		}
	}else {
