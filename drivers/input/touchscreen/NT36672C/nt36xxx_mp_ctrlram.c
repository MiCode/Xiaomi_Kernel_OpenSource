 ...
 *
 * $Revision: 79907 $
 * $Date: 2021-04-01 16:44:17 +0800 (周四, 01 4月 2021) $
 *
...
static int32_t nvt_lockdown_info_open(struct inode *inode, struct file *file)
{
	NVT_LOG("-----%s-------\n", __func__);
	return seq_open(file,&nvt_lockdown_info_ops);
}

static const struct file_operations nvt_lockdown_info_fops = {
	.owner = THIS_MODULE,
	.open = nvt_lockdown_info_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};
...
{
//add codes for wingtech product test
	wt_proc_touchscreen_dir = proc_mkdir(WT_PROC_TOUCHSCREEN_FOLDER , NULL);
	if (wt_proc_touchscreen_dir == NULL)  {
		printk(KERN_ERR "%s: wt_proc_touchpanel_dir file create failed!\n", __func__);
	}else{
		wt_proc_ctp_openshort_test_file = proc_create(WT_PROC_CTP_OPENSHORT_TEST_FILE,
			(S_IWUSR | S_IRUGO), wt_proc_touchscreen_dir, &nvt_selftest_fops);

		wt_proc_ctp_lockdown_info_file = proc_create(WT_PROC_CTP_LOCKDOWN_INFO_FILE,
			(S_IWUSR | S_IRUGO), wt_proc_touchscreen_dir, &nvt_lockdown_info_fops);

		if((wt_proc_ctp_openshort_test_file ==NULL)&&(wt_proc_ctp_lockdown_info_file == NULL)){
			printk(KERN_ERR "%s: file create failed!\n", __func__);
			remove_proc_entry(WT_PROC_TOUCHSCREEN_FOLDER, NULL);
		}else{
			printk("%s:  proc_create at lest one success",__func__);
		}
	}
