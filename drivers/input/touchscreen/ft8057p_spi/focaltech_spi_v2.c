...
const struct mtk_chip_config spi_ctrdata1 = {
        .sample_sel = 0,
	.cs_setuptime = 1920, //9.6兆赫兹的周期 = 1 / 9.6兆赫兹 = 0.1041666667纳秒 \\\ 200纳秒 / 0.1041666667纳秒 ≈ 1920
	.cs_holdtime = 1920,
	.cs_idletime = 10,
	.tick_delay = 0,
};
...
#ifdef CONFIG_SPI_MT65XX 
    memcpy(&ts_data->spi_ctrl, &spi_ctrdata1, sizeof(struct mtk_chip_config));
...
static void fts_ts_shutdown(struct spi_device *spi)
{
    ...
    FTS_INFO("Shutdown fts_ts driver ....");
    if (ts_data)
    {
        fts_ts_remove_entry(ts_data);
        spi_set_drvdata(spi, NULL);
...
static int __init fts_ts_spi_init(void)
{
    ...
    if (strcmp("dsi_panel_c3y_35_03_0b_hd_video",videolfb_tag->lcmname) == 0) {
           FTS_ERROR("ft8057p");
    } else {
          FTS_ERROR("Unkown Touch");
        return -1;
               }
 } 
    FTS_FUNC_ENTER();
    FTS_INFO("TP info: [Vendor]BOE [IC]Ft8057p");
