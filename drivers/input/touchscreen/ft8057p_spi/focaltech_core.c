...
int lct_fts_tp_gesture_callback(bool flag)
{
    ...
    if (ts_data->suspended) {
        FTS_ERROR("The gesture mode will be %s the next time you wakes up.", flag?"enabled":"disabled");
        return -1;
    }
   ...
   if (flag) {
     ...
     FTS_INFO("The gesture mode  %s .", flag?"enabled":"disabled");
    return 0;
}
...
int fts_proximity_readdata(struct fts_ts_data *ts_data)
{
    ...
    u8 buf_addr = 0;
    u8 buf_value = 0;
    //unsigned int proximity_level = 0;
...
static int fts_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *v)
{
