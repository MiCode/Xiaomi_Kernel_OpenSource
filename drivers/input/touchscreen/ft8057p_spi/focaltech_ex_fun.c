...
extern int fts_set_proximity_switch(uint8_t state);
/*****************************************************************************
...
int lct_fts_get_tpfwver(const char *cmd)
{
	...
	if ((fwver == 0xFF) || (fwver == 0x00)){
		...
		num_read_chars = snprintf(buf, 64, "[Vendor]BOE,[FW]0x%02x,[IC]ft8057p\n", fwver);
...
int lct_fts_tp_proximity_switch_callback(bool flag)
{
	int rc = 0;
        if (flag) {
                rc = fts_set_proximity_switch(1);
                FTS_INFO("fts enable proximity mode,rc = %d\n",rc);
        } else {
                rc = fts_set_proximity_switch(0);
                FTS_INFO("fts disable proximity mode,rc = %d\n",rc);
        }
   return 0;
}
...
int lct_create_procfs(struct fts_ts_data *ts_data)
{
	...
	ret = init_lct_tp_info("[Vendor]BOE,[FW]unknow,[IC]ft8057p\n", NULL);
	if(ret) {
	...
	ret = init_lct_tp_selftest(lct_fts_tp_selftest_callback);
	if (ret < 0) {
		FTS_ERROR("init_lct_tp_selftest Failed!");
		goto err_init_lct_tp_selftest_fail;
	} else {
		FTS_INFO("init_lct_tp_selftest Succeeded!");
	}
...
ret = init_lct_tp_proximity_switch(lct_fts_tp_proximity_switch_callback);
	if (ret < 0) {
		FTS_ERROR("init_lct_tp_proximity_switch Failed!\n");
		goto err_init_lct_tp_proximity_switch_failed;
	} else {
		FTS_ERROR("init_lct_tp_proximity_switch Succeeded!\n");
	}
