...
#define TEST_CODE_VERSION			"tp_test_v1.0.1"
/* test switch */
...
#define GTX8_TEST_FILE_NAME						"J10_test_limits.csv"
...
static s32 gtx8_save_header(struct gtx8_ts_test *ts_test, struct file *fp)
{
	...
	if (ts_test->test_result[GTP_NOISE_TEST]) {
		if (GTP_TEST_PASS == ts_test->test_result[GTP_NOISE_TEST])
			bytes += (int) sprintf((char*)&data[bytes],
					"<Item name=\"Rawdata Diffdata Jitter Test\" result=\"OK\"/>\n");
		else
			bytes += (int) sprintf((char*)&data[bytes],
					"<Item name=\"Rawdata Diffdata Jitter Test\" result=\"NG\"/>\n");
	}
#endif
		...
		u8 *title, int len)
{
	...
	bytes += (s32) sprintf((char *)&data[bytes], "</DataContent>\n");
	bytes += (s32) sprintf((char *)&data[bytes], "</%s>\n",title);
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
		ts_err("rawdata write fail.\n");
...
static int gtx8_save_rawdiff(struct gtx8_ts_test *ts_test, struct file *fp)
{
	...
	ret = gtx8_save_sampdata(ts_test, fp, &ts_test->rawdata.data[0],
...
int parse_csvfile(struct device *dev, char *target_name, int32_t  *data, int rows, int columns)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (ret < 0) {
			ts_err("limits file [%s] not available,error:%d, try_times:%d", GTX8_TEST_FILE_NAME, ret, i + 1);
			msleep(1000);
		} else {
...
static void gtx8_rawdata_test_spec(struct gtx8_ts_test *ts_test)
{
	...
	u8 frame_num = 16;
	u16 data_len = 0;
	u8* p_beyond_max_limit_cnt = NULL;
