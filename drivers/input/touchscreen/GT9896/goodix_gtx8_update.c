...
/* COMMON PART - START */
#define TS_DEFAULT_FIRMWARE			"goodix_gt9896_fw_J10.bin"
...
static struct completion fw_update_module_init_comp;
/**
...
int goodix_do_fw_update(int mode)
{
	...
	if (!fwu_ctrl->initialized) {
		ts_err("waiting for fw mode init complete!");
		if (!wait_for_completion_timeout(&fw_update_module_init_comp, 25 * HZ)) {
				...
				struct goodix_ext_module *module)
{
	...
	if (!core_data || !atomic_read(&core_data->initialized) ||
		!ts_bdata || !core_data->ts_dev) {
	...
	complete_all(&fw_update_module_init_comp);
err_out:
...
static int __init goodix_fwu_module_init(void)
{
	...
	init_completion(&fw_update_module_init_comp);
