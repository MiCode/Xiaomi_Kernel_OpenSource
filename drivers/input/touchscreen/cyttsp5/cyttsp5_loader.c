...
#if (defined(CYTTSP5_PLATFORM_TTCONFIG_UPGRADE) \
	|| defined(CYTTSP5_MANUAL_TTCONFIG_UPGRADE))
...
#ifdef CYTTSP5_MANUAL_TTCONFIG_UPGRADE
		...
		struct device *dev)
{
	...
	|| defined(CYTTSP5_PLATFORM_TTCONFIG_UPGRADE)
				 ...
				 struct cyttsp5_hex_image *row_image)
{
	...
	u8 *data = NULL;
	u8 offset = 0;
	data = (u8 *)kzalloc(length * sizeof(u8), GFP_KERNEL);
	if (data == NULL)
		return -ENOMEM;
...
static int cyttsp5_load_app_(struct device *dev, const u8 *fw, int fw_size)
{
	...
	while (p < last_row) {
		/* Get row */
		parade_debug(dev, DEBUG_LEVEL_2, "%s: read row=%d\n",
			__func__, ++row_count);
...
#endif /* CYTTSP5_PLATFORM_TTCONFIG_UPGRADE */
...
#endif /* CYTTSP5_MANUAL_TTCONFIG_UPGRADE */
...
bool cyttsp5_loader_is_init(void)
{
	return true;
}
EXPORT_SYMBOL_GPL(cyttsp5_loader_is_init);
