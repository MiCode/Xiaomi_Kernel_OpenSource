...
static int fts_test_get_ini_size(char *config_name)
{
	...
	"%s%s", FTS_INI_FILE_PATH, config_name);
	...
	for (i = 0; i < ini->section_num; i++) {
		...
		for (j = 0; j < section->keyword_num; j++) {
			...
			if (key_len == strlen(keyword->name)) {
				if (0 ==
				    fts_strncmp(keyword->name, key_name,
						key_len)) {
...
int get_keyword_value(char *section, char *name, int *value)
{
	...
	if (ret > 0) {
		/* search successfully,
		so change value, otherwise keep default */
		*value = fts_atoi(str);
	}
...
static int get_test_threshold_mc_sc(void)
{
	...
	get_detail_threshold("Tx_Linearity_Max_Tx", true,
			thr->tx_linearity_max, node_num);
	get_detail_threshold("Rx_Linearity_Max_Tx", true,
			thr->rx_linearity_max, node_num);
	...
	get_detail_threshold("Panel_Differ_Min_Tx", true,
			thr->panel_differ_min, node_num);
	get_detail_threshold("Panel_Differ_Max_Tx", true,
			thr->panel_differ_max, node_num);
