...
void print_buffer(int *buffer, int length, int line_num)
{
	...
	for (i = 0; i < length; i = i + tmpline) {
		...
		for (j = 0; j < tmpline; j++) {
			cnt +=
			    snprintf(tmpbuf + cnt, tmplen - cnt, "%5d ",
				     buffer[i + j]);
...
int fts_test_read(u8 addr, u8 *readbuf, int readlen)
{
	...
	/* FTS_TEST_DBG("packet num:%d,
	remainder:%d", packet_num, packet_remainder); */
...
int wait_state_update(u8 retval)
{
	...
	while (times++ < FACTORY_TEST_RETRY) {
		...
		if ((ret >= 0) && (retval == state))
			...
			FTS_TEST_DBG("reg%x=%x,retry:%d",
				     FACTORY_REG_PARAM_UPDATE_STATE, state,
				     times);
	}
...
static int read_rawdata(u8 off_addr,
			u8 off_val, u8 rawdata_addr, int byte_num, int *data)
{
	int ret = 0;
...
int chip_clb(void)
{
	...
	while (times++ < FACTORY_TEST_RETRY) {
		...
		if ((0 == ret) && (0x02 == val)) {
			...
			FTS_TEST_DBG("reg%x=%x,retry:%d", FACTORY_REG_CLB, val,
				     times);
	}
...
int get_cb_incell(u16 saddr, int byte_num, int *cb_buf)
{
	...
	FTS_TEST_INFO("cb packet:%d,remainder:%d", packet_num,
		      packet_remainder);
...
bool get_fw_wp(u8 wp_ch_sel, enum wp_type water_proof_type)
{
	...
	case WATER_PROOF_OFF_TX:
		 ...
		 */
		fw_wp_state = ((0x0 == (wp_ch_sel & 0x03))
			       || (0x02 == (wp_ch_sel & 0x03)));
		break;
	case WATER_PROOF_OFF_RX:
		fw_wp_state = ((0x01 == (wp_ch_sel & 0x03))
			       || (0x02 == (wp_ch_sel & 0x03)));
		break;
	default:
		break;
...
bool compare_mc_sc(bool tx_check, bool rx_check, int *data, int *min, int *max)
{
	...
	if (tx_check) {
		for (i = tdata->sc_node.rx_num; i < tdata->sc_node.node_num;
		     i++) {
			if (0 == tdata->node_valid_sc[i])
				continue;
			...
			if ((data[i] < min[i]) || (data[i] > max[i])) {
				FTS_TEST_SAVE_INFO
				    ("test fail,tx%d=%5d,range=(%5d,%5d)\n",
				     i - tdata->sc_node.rx_num + 1, data[i],
				     min[i], max[i]);
				result = false;
			}
		}
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
	...
	snprintf(filepath, FILE_NAME_LENGTH,
	"%s%s", FTS_ITO_RESULT_PATH, file_name);
	if (NULL == pfile) {
		vts_name = getname_kernel(filepath);
		pfile = file_open_name(vts_name, O_TRUNC | O_CREAT | O_RDWR, 0);
	}
	if (IS_ERR(pfile)) {
		FTS_TEST_ERROR("error occured while opening file %s.",
			       filepath);
		return -EIO;
	}
...
static int fts_test_save_data_csv(struct fts_test *tdata)
{
#if CSV_SUPPORT
	int i = 0;
	int j = 0;
		...
		if (info->mc_sc) {
			...
			for (j = 0; j < info->datalen; j++) {
				for (k = 0; k < tdata->sc_node.node_num; k++) {
					csv_length +=
					    snprintf(csv_buffer + csv_length,
						     CSV_BUFFER_LEN -
						     csv_length, "%d, ",
						     info->data[offset + k]);
	...
	ret = fts_test_save_test_data(FTS_CSV_FILE_NAME, csv_buffer, csv_length);
...
static int get_key_num(int *key_num_en, int max_key_num)
{
	...
	if (*key_num_en > max_key_num) {
		FTS_TEST_ERROR("get key num, fw:%d > max:%d", *key_num_en,
			       max_key_num);
		return -EIO;
	}
...
static int get_channel_num(struct fts_test *tdata)
{
	...
	if (IC_HW_SC == tdata->func->hwtype) {
		...
		ret =
		    get_tx_rx_num(FACTORY_REG_KEY_NUM_SC, &rx_num, KEY_NUM_MAX);
		if (ret < 0) {
	...
	if (IC_HW_MC_SC == tdata->func->hwtype) {
		if (tdata->v3_pattern) {
			ret =
			    get_tx_rx_num(FACTORY_REG_CHX_NUM_NOMAP, &tx_num,
					  TX_NUM_MAX);
			if (ret < 0) {
			...
			ret =
			    get_tx_rx_num(FACTORY_REG_CHY_NUM_NOMAP, &rx_num,
					  TX_NUM_MAX);
			if (ret < 0) {
