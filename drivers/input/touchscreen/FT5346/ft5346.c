...
static ssize_t audiomode_proc_write(struct file *filp, const char __user *userbuf, size_t count, loff_t *ppos);
static const struct file_operations audiomode_procs_fops =
{
    .write = audiomode_proc_write,
    .read = NULL,
    .owner = THIS_MODULE,
};
#endif
...
static void fts_ctpm_read_lockdown(struct i2c_client *client, struct ft5x06_ts_data *data)
{
	...
	dev_info(&client->dev, "TP_Maker = 0x%x\n", TP_Maker);
	dev_info(&client->dev, "LCD_Maker = 0x%x\n", LCD_Maker);
	dev_info(&client->dev, "Panel_Ink = 0x%x\n", Panel_Ink);
	dev_info(&client->dev, "Project_Id = 0x%x\n", Project_Id);
...
{
#include "ft_app_ic_truly_ebbg.txt"
};
...
*/
	...
	if(TP_Maker== TP_OUFEI && LCD_Maker == TP_EBBG)
    {
        pbt_buf = CTPM_FW1;
        fw_len = sizeof(CTPM_FW1);
        CTP_DEBUG("TP_OUFEI+LCD_EBBG");
    }

	else if(TP_Maker== TP_TRULY && LCD_Maker == TP_EBBG)
	{
	      pbt_buf = CTPM_FW2;
	      fw_len = sizeof(CTPM_FW2);
	      CTP_DEBUG("TP_TRULY+LCD_EBBG");
	}

	else
	{
		  CTP_ERROR("unkown tp modules \n");
		  return -1;
	}


    CTP_DEBUG("update firmware size:%d", fw_len);
    if ( (sizeof(CTPM_FW1)< 8 ) ||(sizeof(CTPM_FW1)>FT_MAX_SIZE*1024 ) || (sizeof(CTPM_FW2)< 8 ) ||
		(sizeof(CTPM_FW2)>FT_MAX_SIZE*1024 ))
    {
...
*/
        ...
        CTP_INFO("[FTS] uc_tp_fm_ver = %d.\n", uc_tp_fm_ver);
        CTP_INFO("[FTS] uc_host_fm_ver = %d.\n", uc_host_fm_ver);
...
static int hardwareinfo_set(struct ft5x06_ts_data *data, u8 value_name, u8 color)
{
	...
	if (lockdown_info[0] == VENDOR_O_FILM)
	...
	else if (lockdown_info[0] == TP_TRULY)
		snprintf(vendor_for_id, HARDWARE_MAX_ITEM_LONGTH, "TRULY");
		...
		snprintf(vendor_for_id, HARDWARE_MAX_ITEM_LONGTH, "DS");
	else if (data->fw_vendor_id == VENDOR_TRULY)
		snprintf(vendor_for_id, HARDWARE_MAX_ITEM_LONGTH, "TRULY");
	else
...
static ssize_t ctp_open_proc_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	*ppos += count;
    ...
    if(TP_Maker== TP_OUFEI&& LCD_Maker == TP_EBBG)
    {
        sprintf(cfgname, "%s", "ft5346_oufei_ebbg_c3g.ini");
        CTP_INFO("cfgname is: %s\n", cfgname);
    }
    else if(TP_Maker== TP_TRULY && LCD_Maker == TP_EBBG)
    {
         sprintf(cfgname, "%s", "ft5346_truly_ebbg_c3g.ini");
        CTP_INFO("cfgname is: %s\n", cfgname);
    }
   else
    {
...
static void create_ctp_proc(void)
{
   ...
   struct proc_dir_entry *audiomode_proc = NULL;
   if( ctp_device_proc == NULL)
   {
	    ctp_device_proc = proc_mkdir(CTP_PARENT_PROC_NAME, NULL);
	    if(ctp_device_proc == NULL)
    ...
    audiomode_proc = proc_create("audio_mode", 0777, ctp_device_proc, &audiomode_procs_fops);
    if (audiomode_proc == NULL)
    {
        CTP_ERROR("ft5x06: create audiomode_proc fail\n");
    }
}


static ssize_t audiomode_proc_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
...
static ssize_t ft5x0x_debug_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (copy_to_user(user_buf, debug_read_buf, num_read_chars)) {
