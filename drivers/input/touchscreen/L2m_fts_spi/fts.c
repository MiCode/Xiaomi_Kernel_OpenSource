				  ...
				  const char *buf, size_t count)
{
	...
	if (temp == 1) {
		...
		info->board->fod_lx = 5870;
		info->board->fod_ly = 23460;
		info->board->fod_x_size = 2660;
		info->board->fod_y_size = 2660;
...
static void fts_update_enter_idle_time(void)
{
	...
	if (fts_info->gamemode_enable) {
		...
		logError(1, "%s %s not in gamemode, set idle time to 1s\n", tag, __func__);
		set_cmd[3] = 0x06;
	}
