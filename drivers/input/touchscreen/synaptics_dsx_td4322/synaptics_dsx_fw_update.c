...
static void fwu_parse_image_header_05_06(void)
{
	...
	if (fwu->img.contains_bootloader || header->options_tddi
		/*(fwu->img.bl_version == BL_V5 && fwu->img.contains_bootloader) ||
...
static int fwu_parse_image_info(void)
{
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev, "bl version %d\n", fwu->bl_version);
...
static int fwu_write_f34_v5v6_command(unsigned char cmd)
{
	...
	case CMD_READ_CONFIG:
		...
		break;
	case CMD_ERASE_ALL:
		if (fwu->erase_all | fwu->dp_crc_failed)
...
static int fwu_synaptics_read_crc_err(void)
{
	...
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
				"%s: Failed to check crc err\n",
				__func__);
		return retval;
	}

	if (status.status_code == 0x0B) {
		fwu->dp_crc_failed = true;
		dev_info(&fwu->rmi4_data->i2c_client->dev,
				"%s: In flash prog mode, status = 0x%02x\n",
				__func__, status.status_code);
	} else {
		fwu->dp_crc_failed = false;
	}
...
static enum flash_area fwu_go_nogo(void)
{
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev,
			"%s: Image firmware ID = 0x%x\n",
			__func__, image_fw_id);
/*not update only when the firmware id is not equal*/
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev,
		"%s:Same firmware ID, start to compare config id\n",
		__func__);
...
static int fwu_erase_all(void)
{
	...
	if (fwu->flash_properties.has_disp_config &&
			fwu->img.contains_disp_config && fwu->erase_all) {
...
static int fwu_start_reflash(void)
{
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev, "%s: Start of reflash process\n", __func__);

	if (fwu->image == NULL) {
		if (strlen(rmi4_data->firmware_path) != 0) {
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
					rmi4_data->firmware_path, strlen(rmi4_data->firmware_path) + 1,
					strlen(rmi4_data->firmware_path) + 1);
		} else {
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
...
*/
	...
	if ((input != NORMAL) && (input != FORCE) && (input != UPDATE_ALL)) {
		retval = -EINVAL;
		goto exit;
	}
...
static ssize_t synaptics_rmi4_fwu_invoke(struct synaptics_rmi4_data *rmi4_data,
				enum exp_invoke_fn command, char *buf, void *data)
{
	ssize_t ret_size = 0;

	dev_info(&rmi4_data->i2c_client->dev, "%s command %d\n", __func__, command);
	switch (command) {
	case RMI_UPDATE_FIRMWARE:
		ret_size = fwu_sysfs_do_reflash_store(&rmi4_data->i2c_client->dev, NULL, buf, 1);
		break;
	default:
		dev_err(&rmi4_data->i2c_client->dev, "Invalid command");
	}
...
static struct synaptics_rmi4_exp_fn fwu_module = {
	...
	.invoke = synaptics_rmi4_fwu_invoke,
};
