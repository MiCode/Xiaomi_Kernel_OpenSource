...
/****************************************************************
	...
	mobile: 13244776877
-----------------------------------------------------------------*/
#define PLUG_CAL_T37_VERSION 0x0108
/*----------------------------------------------------------------
0.108
1 T65 burst check disable after middle time end
0.107
1 add T65 recal when enable bit change
0.106
...
#define T37_STEP_2_CHECK_REF_2			(1<<10)
#define T37_STEP_2_CHECK_SINGLE_TOUCH		(1<<11)
...
#define T37_FLAG_SUPPRESSED			(1<<17)
...
struct point_trace{
	...
	unsigned long time_touch_st;
	unsigned long time_touch_end;
...
#define T37_SKIP_LIMIT_COMPARE		(1<<2)
	...
	int grad_accumulator_low;
	int grad_accumulator_high;
	int ref_signal_ratio_shift;
...
#define NUM_GRAD_LEVEL_LIST 6
	...
	int grad_block_low_limit;
	int grad_block_high_limit;
	int grad_block_high_limit_list[T37_PAD3_GRAD_BLOCKS];
...
long step_interval(struct plugin_cal *p, unsigned long step)
{
	...
	if (test_flag(T37_STEP_0_IN, &step))
		...
		if (test_flag(T37_STEP_2_ENTER, &step))
			interval = cfg->interval_slow;
...
static void proc_reset_msg(struct plugin_cal *p)
{
	...
	clear_grad_record(obs, GRAD_TRACE_NUMBER, true);
...
int get_t8_state(unsigned long step, unsigned long flag, unsigned long pl_flag, int step_t8, int num_t8_config)
{
	...
	if (test_flag(T37_STEP_0_IN, &step)) {
		state = T8_NORMAL;
	} else if (test_flag(T37_STEP_1_IN|T37_STEP_2_IN, &step)) {
...
int get_t42_state(unsigned long step, unsigned long flag, unsigned long pl_flag)
{
	int state = T42_NORMAL;
	...
	for (i = area->x0; i <= area->x1; i++) {
		...
		for (j = area->y0; j < area->y1; j++) {
			...
			if (i < center.x0 && j < center.y0) {
				if (dst_buf[pad3 + T37_PAD3_GRAD_MAX + 0] < dst_buf[pos])
...
static int gradient_percent(s16 grad_val, s16 grad_base, struct stat_grad_config *grad_level_list, int num_grad_level_list)
{
	...
	if (grad_val >= grad_base + grad_level_list[0].thld) {
...
static int mxt_surface_aquire_and_compare(struct plugin_cal *p, int sf, int compare, s16 diff_threshold_t9, s16 diff_threshold_t15, int no_wait, unsigned long flag)
{
	...
	memcpy(&surface, &dcfg->m[MX_AA], sizeof(struct rect));
	memcpy(&area, &dcfg->m[MX_AA], sizeof(struct rect));
	x_size = surface.x1 - surface.x0 + 1;
	y_size = surface.y1 - surface.y0 + 1;
...
static int mxt_surface_aquire_and_compare_auto(struct plugin_cal *p, int sf, int compare, s16 diff_threshold_t9, s16 diff_threshold_t15, int no_wait, unsigned long flag)
{
 ...
 #if defined(TEST_TEST_T65)
		if (sf == T37_DELTA0) {
			if (sf_info->x >= x_size) {
				buf = obs->buf[sf];
				pos = 5 * y_size;

				memset(buf + pos, 0, (area.y1 - area.y0 + 1)*sizeof(s16));
...
static void plugin_cal_t37_hook_t6(struct plugin_cal *p, u8 status)
{
	...
	if (status & (MXT_T6_STATUS_RESET|MXT_T6_STATUS_CAL)) {
		...
		if (test_flag(T37_STEP_0_IN, &obs->step)) {
			dev_dbg2(dev, "T37 hook T6 end\n");
			set_and_clr_flag(T37_STEP_1_ENTER, T37_STEP_0_IN, &obs->step);
		}
	}

	dev_info2(dev, "mxt t37 step=0x%lx(0x%lx) flag=0x%lx\n",
			obs->step, obs->step_cmd, obs->flag);
}
...
static void plugin_cal_t37_hook_t42(struct plugin_cal *p, u8 status)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct device *dev = dcfg->dev;
	struct t37_observer *obs = p->obs;
	...
	if (status & 0x1) {
		dev_info2(dev, "T37 hook T42 suppressed\n");

		set_flag(T37_FLAG_SUPPRESSED, &obs->flag);
	} else {
		clear_flag(T37_FLAG_SUPPRESSED, &obs->flag);
	}
}
...
static int mxt_proc_step_2_msg(struct plugin_cal *p, unsigned long pl_flag)
{
	...
	if (test_flag(T37_STEP_2_CHECK_REF_1, &obs->step)) {
		...
		if (sf_info->x < x_size) {
			...
			if (0)
				ret = mxt_surface_aquire_and_compare(p, sf, T37_BUFFER_INVALID, 0, 0, 0, flag);
			else
				...
				if (wait) {
					for (j = 0; j < GRAD_STATE_LIST; j++) {
						...
						if ((!state_wait || test_flag(state_wait, &state)) &&
							...
							if (punish > cfg->num_grad_level_array) {
								punish -= cfg->num_grad_level_array;
								punish >>= 1;
							} else {
								punish = 0;
							}
	...
	if (test_flag(T37_STEP_2_CHECK_SINGLE_TOUCH, &obs->step)) {
		...
		for (i = 0; i < cfg->step2_check_point_count; i++) {
			...
			if ((i == 0) && test_flag(T37_STATE_TOUCH_RELEASED, &state) &&
					test_flag(T37_STATE_TOUCH_MOVING, &state)) {
	...
	if (ret == 0) {
		do {
			...
			if (sf == sf_delta) {
				...
				#endif
				if (!result[LOW]) {
					if (!obs->touch_id_list && !test_flag(T37_FLAG_SUPPRESSED, &obs->flag)) {
						sf_cnt->bad = 0;
							...
							if (accumulator <= obs->accumulator_ref) {
								...
								if (accumulator < cfg->grad_accumulator_low && accumulator0 < obs->accumulator_ref) {
									...
									if (test_flag(T37_FLAG_SURFACE_GRADIENT_SMOOTH, &obs->flag)) {
										...
										if (accumulator > cfg->grad_accumulator_high) {
											ref_diff = accumulator - obs->accumulator_ref_min;
				...
				obs->accumulator_ref = accumulator;
				if (!obs->accumulator_ref_min ||  obs->accumulator_ref_min > accumulator) {
...
static int mxt_process_obj_recovery(struct plugin_cal *p, unsigned long pl_flag)
{
	...
	state = get_t42_state(obs->step, obs->flag, pl_flag);
	p->set_t42_cfg(p->dev, state, 0);
	...
	if (!test_flag(WK_KEEP_T65_ALWAYS_ON, &cfg->grad_acc_flag)) {
		if ((state == T65_NORMAL) != test_flag(PL_STATUS_FLAG_T65_SET, &pl_flag)) {
			p->set_t6_cal(p->dev);
		}
	}
#endif
...
static long plugin_cal_t37_post_process_messages(struct plugin_cal *p, unsigned long pl_flag)
{
	...
	if (obs->step_cmd)
	dev_info2(dev, "mxt t37 touch= 0x%lx step=0x%lx (0x%lx) flag=0x%lx pl_flag=0x%lx\n",
			obs->touch_id_list, obs->step, obs->step_cmd, obs->flag, pl_flag);
	...
	if (!next_interval)
		next_interval = MAX_SCHEDULE_TIMEOUT;
...
static int init_t37_object(struct plugin_cal *p)
{
	...
	cfg->interval_delta_middle_check = HZ * 10;
	cfg->interval_delta_check_end = HZ * 30;
	cfg->interval_delta_check_end2 = HZ * 45;
	cfg->interval_grad_check = HZ * 30;
	cfg->interval_distance_check = HZ * 2;
	cfg->interval_step_0 = HZ / 10;
	cfg->interval_step_1 = HZ / 10;
	cfg->interval_step_2 = HZ / 6;
	cfg->interval_step_3 = HZ / 10;
	...
	cfg->distance_touch_move[MOVEMENT_SHORT] = 10;
	...
	cfg->grad_acc_flag |= WK_KEEP_T65_ALWAYS_ON|WK_KEEP_T80_ALWAYS_ON|BUGS_CHECK_ZERO_BURST|BUGS_CHECK_SOFT_T8|BUGS_CHECK_REFERENCE_IRON;
...
struct plugin_cal mxt_plugin_cal_t37 = {
	...
	.hook_t42 = plugin_cal_t37_hook_t42,
