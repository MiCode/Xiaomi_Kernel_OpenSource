...
int FW_VERSION_ID;
static u8 GTP_FW_SENSOR_ID;

#if  WT_ADD_CTP_INFO
extern int gtp_hardwareinfo_set(void);
#endif
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *path)
{
	...
	if (path) {
		...
		ret = gtp_i2c_read_dbl_check(client, GTP_REG_SENSOR_ID, &GTP_FW_SENSOR_ID, 1);
		if (SUCCESS == ret) {
			if (GTP_FW_SENSOR_ID >= 0x09) {
				GTP_ERROR("Invalid sensor_id(0x%02X), No Config Sent!", GTP_FW_SENSOR_ID);

			}
		}

		if (GTP_FW_SENSOR_ID == 0x2) {
			GTP_INFO("Update GT915L FW, Sensor ID : %d", GTP_FW_SENSOR_ID);
		} else {
			GTP_INFO("Force Upgrade Error !");
