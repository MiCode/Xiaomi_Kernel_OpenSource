...
static const char *fts_get_config(struct fts_ts_info *info)
{
	...
	logError(1, "%s Choose config 0: %s", tag,
		 pdata->config_array[0].fts_cfg_name);
	pdata->current_index = 0;
	return pdata->config_array[0].fts_cfg_name;
}
...
const char *fts_get_limit(struct fts_ts_info *info)
{
	...
	logError(1, "%s Choose limit file 0: %s", tag,
		 pdata->config_array[0].fts_limit_name);
	pdata->current_index = 0;
	return pdata->config_array[0].fts_limit_name;
}
