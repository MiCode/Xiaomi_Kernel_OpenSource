...
static struct function_descriptor fd_11;
...
unsigned short force[] = {0,(0x22<<1),I2C_CLIENT_END,I2C_CLIENT_END}; 
...
static int tpd_power(struct i2c_client *client, int on)
{
	...
	for(i = 0 ; i < 4; i++)
	{
		ret = tpd_i2c_write_byte_data(client, (fd_11.controlBase + 6 + i), tpd_res[i]); /* set touchscreen x/y resolution*/
		if (ret < 0)
        	{
            		TPD_DMESG("Error touch can not set resolution. \n");
			return ret;
        	}
	}	
	}
...
{
	TPD_DMESG("S3203 I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
...
{
	TPD_DMESG("S3203 touch panel driver init\n");
	i2c_register_board_info(0, &i2c_tpd, 1);
	if(tpd_driver_add(&tpd_device_driver) < 0)
		TPD_DMESG("Error Add S3203 driver failed\n");
	return 0;
}

static void __exit tpd_driver_exit(void)
{
	TPD_DMESG("S3203 touch panel driver exit\n");
...
MODULE_DESCRIPTION("Mediatek S3203 Driver");
