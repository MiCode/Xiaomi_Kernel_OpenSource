...
int fts_proximity_readdata(struct fts_ts_data *ts_data)
{
    ...
    if (regvalue == 0x01) {
        ...
        fts_write_reg(FTS_REG_FACE_HOST_STATUS, 0x01);
    } else if (regvalue == 0x00) {
        /* far away */
        proximity_status = PS_FAR_AWAY;
        fts_write_reg(FTS_REG_FACE_HOST_STATUS, 0x00);
    }

    FTS_INFO("fts_proximity_data.detect is %d,proximity_status = %d", fts_proximity_data.detect,proximity_status);
