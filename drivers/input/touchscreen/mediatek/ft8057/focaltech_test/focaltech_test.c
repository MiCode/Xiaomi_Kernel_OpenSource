...
static int fts_ito_test_show(struct seq_file *file, void *data)
{
	...
	/*C3T code for HQ-219137 by chenzimo at 2022/8/31 start*/
	struct fts_ts_data *ts_data = fts_data;
	/*C3T code for HQ-219137 by chenzimo at 2022/8/31 end*/
	int i = 0;
	int ret = 0;
...
int fts_test_init(struct fts_ts_data *ts_data)
{
    ...
    if (ret < 0) {
		...
		if (!fts_ito_test_proc){
			...
			remove_proc_entry("self_test", fts_android_touch_proc);
	...
	size_t count, loff_t *pos)
{
	...
	char tmp[6] = {0};
	char fwname[128] = {0};
	char buf[FILE_NAME_LENGTH] = FTS_INI_FILE_NAME;//"focaltech_mp_fw_tianma.ini";
	struct fts_ts_data *ts_data = fts_data;
...
*/
	...
	/*C3T code for HQ-231143 by chenzimo at 2022/9/15 start*/
	if (memcmp(tmp, "spi", 3) == 0) {
	/*C3T code for HQ-231143 by chenzimo at 2022/9/15 end*/
...
int fts_tp_selftest_proc_init(void)
{
	...
	if (fts_tp_selftest == NULL) {
		remove_proc_entry(FTS_PROC_TP_SELFTEST, NULL);
