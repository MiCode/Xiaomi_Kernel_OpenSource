...
static int fts_rw_iic_drv_myread(struct i2c_client *client, u8 *buf, int length)
{
	...
	ret = FT8707_I2C_Read(client, NULL, 0, buf, length);

	if (ret < 0)
		TOUCH_ERR("%s:IIC Read failed\n", __func__);
	return ret;
}
...
static int fts_rw_iic_drv_mywrite(struct i2c_client *client, u8 *buf, int length)
{
	...
	ret = FT8707_I2C_Write(client, buf, length);
	if (ret < 0)
		TOUCH_ERR("%s:IIC Write failed\n", __func__);
...
static int fts_rw_iic_drv_RDWR(struct i2c_client *client, unsigned long arg)
{
	...
	if (!access_ok
	    (VERIFY_READ, (struct fts_rw_i2c_queue *)arg, sizeof(struct fts_rw_i2c_queue)))
		return -EFAULT;

	if (copy_from_user(&i2c_rw_queue,
			   (struct fts_rw_i2c_queue *)arg, sizeof(struct fts_rw_i2c_queue)))
		return -EFAULT;
	...
	i2c_rw_msg = (struct fts_rw_i2c *)
	    kmalloc(i2c_rw_queue.queuenum * sizeof(struct fts_rw_i2c), GFP_KERNEL);
	if (!i2c_rw_msg)
		return -ENOMEM;
	...
	for (i = 0; i < i2c_rw_queue.queuenum; i++) {
		if (i2c_rw_msg[i].flag) {
			...
			if (ret >= 0)
				ret =
				    copy_to_user(data_ptrs[i], i2c_rw_msg[i].buf,
						 i2c_rw_msg[i].length);
		} else {
...
static void fts_rw_iic_drv_setup_cdev(struct fts_rw_i2c_dev *dev, int index)
{
	...
	if (err)
		TOUCH_ERR("Error %d adding LED%d", err, index);
}
...
static int fts_rw_iic_drv_myinitdev(struct i2c_client *client)
{
	...
	if (err < 0) {
		TOUCH_ERR("%s:ft_rw_iic_drv failed  error code=%d---\n", __func__, err);
		return err;
	}
	...
	if (!fts_rw_i2c_dev_tt) {
		...
		TOUCH_ERR("%s:ft_rw_iic_drv failed\n", __func__);
		return err;
	}
	...
	if (IS_ERR(fts_class)) {
		TOUCH_ERR("%s:failed in creating class.\n", __func__);
		return -1;
	}
	/*create device node */
	device_create(fts_class, NULL, MKDEV(fts_rw_iic_drv_major, 0), NULL, FTS_RW_IIC_DRV);
...
{
	TOUCH_LOG("[FTS]----ft_rw_iic_drv init ---\n");
