    ...
    struct device_node *np)
{
    ...
    if (ret) {
        cts_warn("Parse state filepath failed %d", ret);
    ...
    if (ret) {
        cts_warn("Parse poll interval failed %d", ret);
    }
...
static int start_earjack_detect(struct cts_earjack_detect_data *ed_data)
{
    ...
    cts_info("Start detect check file: '%s'",
...
static int get_earjack_state(struct cts_earjack_detect_data *ed_data)
{
...
#endif
    if (read_size < 0) {
        cts_err("Read state file '%s' failed %d",
    ...
    if (ret) {
        cts_warn("Close file '%s' failed %d",
    ...
    cts_dbg("State: %s", ed_data->state ? "ATTACHED" : "DETACHED");
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (ret) {
        cts_err("%s earjack detect failed",
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (ret) {
        cts_err("%s earjack detect failed %d",
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    return scnprintf(buf, PAGE_SIZE,
        "Earjack state: %s\n",
        ed_data->state ? "ATTACHED" : "DETACHED");
}
    ...
    if (ret) {
        ...
        if (ed_data->state != prev_state) {
            cts_info("State changed: %s -> %s",
                prev_state ? "ATTACHED" : "DETATCHED",
                ed_data->state ? "ATTACHED" : "DETATCHED");
            ...
            if (ret) {
                cts_err("Set dev earjack attached to %s failed %d",
                    ed_data->state ? "ATTACHED" : "DETATCHED", ret);
...
int cts_deinit_earjack_detect(struct chipone_ts_data *cts_data)
{
    ...
    if(ed_data->earjack_state_filepath) {
        cts_info("Kfree earjack state filepath");
        kfree(ed_data->earjack_state_filepath);
