...
#define LCT_ADD_TP_VERSION   //add by lizhiye
//#define PTC_NEED_TUNE //add by Morven
...
retry_selftest:
	...
	{
#if defined(PTC_NEED_TUNE)
		case MXT_T25_MSG_STATUS_SIGLIMIT:
			if(data->t25_msg[1] == 0x61 && retry > 0)
			{
				mxt_plugin_store(dev, 0, 	"msc ptc tune 3", sizeof("msc ptc tune 3"));
				do {
					msleep(100);
					ret = mxt_plugin_check_tune_status(&data->plug);
					if (ret != -EBUSY)
						break;
				}while(--wait);
			...
			dev_dbg(dev, "mxt_t25_selftest_show self test ptc tune fail\n");
			offset += scnprintf(buf, PAGE_SIZE, "FAILED\n");
			break;
#endif
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	if (ret) {
		...
		dev_info(dev,
			"[%u] status:%02X x:%u y:%u area:%02X amp:%02X vec:%02X (redefined)\n",
			id,
			status,
			x, y,
			(data->t100_aux_area) ? message[data->t100_aux_area] : 0,
...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	config_crc_ver = data->config_crc;
	dev_err(dev, "mxt_check_reg_init, config_crc_ver=%d\n", config_crc_ver);
				...
				const char *buf, size_t count, int alternative)
{
	...
	*file_name = file_name_tmp;
	...
	if (buf[count - 1] == '\n')
		...
		(*file_name)[count] = '\0';
	printk("susan [mxt]mxt_update_cfg_store atlternative is %d\n", alternative); //susan
	if (alternative == 1) {
	...
	printk("susan mxt_update_cfg_store file_name is %s\n", *file_name);  //susan
...
static DEVICE_ATTR(t19, S_IWUSR | S_IRUSR /*| S_IWGRP | S_IWOTH */, mxt_t19_gpio_show,
			mxt_t19_gpio_store);
static DEVICE_ATTR(t25, 0644/*S_IWUSR | S_IRUSR | S_IWGRP | S_IWOTH*/, mxt_t25_selftest_show,
...
#endif
#if defined(CONFIG_MXT_MISC_WORKAROUND)
	static DEVICE_ATTR(misc, 0644/*S_IWUSR | S_IRUSR*/, mxt_plugin_misc_show, 
...
static int ctp_proc_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	...
	printk("tpd, Enter ctp_proc_read\n");

	page = kmalloc(PAGE_SIZE, GFP_KERNEL);	
	if (!page) 
	{		
		printk("tpd Enter ctp_proc_read, !page\n");
		kfree(page);		
		return -ENOMEM;	
	}
	...
	ptr += sprintf(ptr, "[Vendor]o-film, [fw]%u.%u.%02x.%06x, [ic]%s\n", data->info->version>>4, data->info->version & 0xf, data->info->build, data->config_crc, "mxt336t");
...
static int mxt_probe(struct i2c_client *client,	const struct i2c_device_id *id)	//__devinit mxt_probe //sunwf modify
{
	struct mxt_data *data;
	int error;
