...
extern int32_t	priximity_event;
...
info_retry:
	...
	case TP_VENDOR_TRULY:
		sprintf(tp_info_buf, "[Vendor]truly,[FW]0x%02x,[IC]nt36525b\n", ts->fw_ver);
...

{
...
#if NVT_TOUCH_ESD_PROTECT
	NVT_LOG("nvt_ts_0x11_cmd: cancel delayed work sync\n");
	cancel_delayed_work_sync(&nvt_esd_check_work);
	nvt_esd_check_enable(false);
#endif /* #if NVT_TOUCH_ESD_PROTECT */

	mutex_lock(&ts->lock);
	NVT_LOG("nvt_ts_0x11_cmd start\n");
	//---write command to enter "deep sleep mode"---
	...
	msleep(50);
	NVT_LOG("nvt_ts_0x11_cmd end\n");
	return 0;
}

EXPORT_SYMBOL(nvt_ts_0x11_cmd);
...
static int32_t __init nvt_driver_init(void)
{
	...
	if (IS_ERR_OR_NULL(mtkfb_lcm_name)){
		...
		if (strcmp(mtkfb_lcm_name,"dsi_panel_c3s_43_02_0a_fhdp_video") == 0) {
			touch_vendor_id = TP_VENDOR_TRULY;
			NVT_LOG("TP info: [Vendor]TRULY [IC]nt36525b\n");
