...
#define SPLIT_LINE_STR \
	...
	for (r = 0; r < cts_dev->fwdata.rows; r++) {
		for (c = 0; c < cts_dev->fwdata.cols; c++) {
			...
			if ((min != NULL
			     && data[offset] < min[per_node ? offset : 0])
			    || (max != NULL
				&& data[offset] > max[per_node ? offset : 0])) {
				if (failed_cnt == 0) {
					cts_info(SPLIT_LINE_STR);
...
disable_get_tsdata:
	...
	if (driver_validate_data && data_valid) {
		...
		if (dump_test_data_to_console || dump_test_data_to_file) {
			cts_dump_tsdata(cts_dev, "Noise", noise, dump_test_data_to_console);
			cts_dump_tsdata(cts_dev, "Rawdata MAX", max_rawdata, dump_test_data_to_console);
			cts_dump_tsdata(cts_dev, "Rawdata MIN", min_rawdata, dump_test_data_to_console);
		}

		ret = validate_tsdata(cts_dev, "Noise test", noise,
...
int cts_test_short(struct cts_device *cts_dev, struct cts_test_param *param)
{
	...
	ret = cts_polling_test_data(cts_dev, (u8 *) test_result, RAWDATA_BUFFER_SIZE(cts_dev));
	if (ret) {
		cts_err("Read test result failed %d", ret);
		goto recovery_display_state;
	}
...
#define SPLIT_LINE_STR \
	...
	for (r = 0; r < cts_dev->fwdata.rows; r++) {
		for (c = 0; c < cts_dev->fwdata.cols; c++) {
			...
			if ((min != NULL
			     && cap[offset] < min[per_node ? offset : 0])
			    || (max != NULL
				&& cap[offset] > max[per_node ? offset : 0])) {
				if (failed_cnt == 0) {
					cts_info(SPLIT_LINE_STR);
					cts_info("%s failed nodes:", desc);
				}
