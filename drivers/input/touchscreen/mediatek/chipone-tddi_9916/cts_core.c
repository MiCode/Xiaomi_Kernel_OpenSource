...
static unsigned int prox_cur_status = 2;	//1: near; 2: far; default far;
static u8 prox_active = 0;					//1: enable; 0: disable; default 0;
static int open_near = 0 ;
static int tp_away = 0;
int cts_set_proximity_switch(unsigned int proximity_switch);
int cts_get_prox_status(void);
//extern int ps_report_select;
...
int cts_set_proximity_switch(unsigned int proximity_switch)
{
	...
	cts_info("%s() enbale: %d, prox_active = %d, prox_cur_status = %d", __func__,
		proximity_switch, prox_active, prox_cur_status);

	prox_active = proximity_switch ? 1 : 0;
	if (prox_active == 1) {
		tp_away = 0;
	...
	ret = cts_tcs_spi_write(&chipone_ts_data->cts_dev, TP_STD_CMD_PARA_PROXI_EN_RW,
		&prox_active, sizeof(prox_active));
	if (ret != 0) {
		cts_err("Send %s to device failed!", proximity_switch ? "CMD_EN_PROX" : "CMD_DIS_PROX");
		return ret;
	}
		return 0;
}
...
int cts_get_diffdata(const struct cts_device *cts_dev, void *buf)
{
	...
	cache_buf = kzalloc((cts_dev->fwdata.rows + 2) * (cts_dev->fwdata.cols + 2) * 2, GFP_KERNEL);
	if (cache_buf == NULL) {
...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
	...
	tp_info_version = fwdata->version&0x00ff;
	sprintf(tp_info_buf, "[Vendor]tianma,[FW]0x%02x,[IC]icnl9916\n", tp_info_version);
	update_lct_tp_info(tp_info_buf, NULL);

#ifdef CONFIG_CTS_CHECK_TOUCH_VENDOR
	switch(cts_vendor_info_val){
	case CTS_VENDOR_HOLITECH:
		sprintf(tp_info_buf, "[Vendor]Holitech,[FW]%04x,[IC]icnl9916\n", (fwdata->version)&0xff);
		update_lct_tp_info(tp_info_buf, NULL);
		break;

	case CTS_VENDOR_CTC:
		sprintf(tp_info_buf, "[Vendor]Ctc,[FW]%04x,[IC]icnl9916\n", (fwdata->version)&0xff);
		update_lct_tp_info(tp_info_buf, NULL);
		break;
	default:
		sprintf(tp_info_buf, "[Vendor]unknow,[FW]unknow,[IC]unknow\n", fwdata->version);
		update_lct_tp_info(tp_info_buf, NULL);
		break;
#endif
...
int cts_irq_handler(struct cts_device *cts_dev)
{
    ...
    if (unlikely(cts_dev->rtdata.suspended)) {
		...
		if(open_near == 1) {
			cts_dbg("4.Get ps status data = %x from TP,open_near status:%d-%s",touch_info->vkey_state,
				open_near, open_near == 1 ? "near":"far");

			if (touch_info->vkey_state & 0x80) {
				prox_cur_status = 1;
				open_near = 1 ;
				irq_number += 1;
				if (irq_number == 4) {
					irq_number = 0;
					//if (ps_report_select == 1)
					ps_send_touch_event(0);
				}
			} else if (((touch_info->vkey_state & 0x80) == 0) && open_near == 1) {
				prox_cur_status = 2;
				open_near = 0 ;
				//if (ps_report_select == 1)
			...
			cts_dbg("5.TP in promixity near status while suspend mode,status:%d-%s",open_near,
				open_near == 1 ? "near":"far");
			cts_dbg("6.%s() near open_near = %d, prox_cur_status = %d,", __func__, open_near, prox_cur_status);
...
#ifdef CONFIG_CTS_TP_PROXIMITY
		if (prox_active == 1) {
			cts_dbg("1.Get ps status data = %x from TP,open_near status:%d-%s",touch_info->vkey_state,
				open_near, open_near == 1 ? "near":"far");
				if (touch_info->vkey_state & 0x80) {
				prox_cur_status = 1;
				open_near = 1 ;
			...
			} else if ((((touch_info->vkey_state & 0x80) == 0) && open_near == 1) || tp_away == 0) {
				prox_cur_status = 2;
				open_near = 0 ;
                		tp_away = 1;
				irq_number = 0;
			...
			cts_dbg("2.TP in promixity near status while normal mode,status:%d-%s",open_near,
				open_near == 1 ? "near":"far");
			cts_dbg("3.%s() near open_near = %d, prox_cur_status = %d", __func__, open_near, prox_cur_status);
        	}
#endif
...
#ifdef CONFIG_CTS_TP_PROXIMITY
	if (prox_active == 1) {
		cts_set_proximity_switch(prox_active);
		if (!cts_dev->rtdata.suspended) {
			cts_dbg("suspend is false, return");
			return 0;
		}
	}
#endif
...
#ifdef CONFIG_CTS_TP_PROXIMITY
	if (prox_active == 1) {
		cts_dbg("suspend is true, set prox again");
		cts_set_proximity_switch(prox_active);
        }
#endif
