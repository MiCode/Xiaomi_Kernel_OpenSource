	...
	u32 hwid, u16 fwid, u16 curr_firmware_ver)
{
	...
	if (hwid == CTS_DEV_HWID_ICNL9911C) {
		...
		if (cts_vendor_info_val == CTS_VENDOR_HOLITECH) {
			if (is_filesystem_mounted(CFG_CTS_FIRMWARE_FILEPATH_HOLITECH)) {
				//filepath:CFG_CTS_FIRMWARE_FILEPATH => filp_open()
				//filepath:CFG_CTS_FIRMWARE_FILENAME => request_firmware()
				firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
					cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : CFG_CTS_FIRMWARE_FILEPATH_HOLITECH,
#else /* CFG_CTS_FW_UPDATE_FILE_LOAD */
					CFG_CTS_FIRMWARE_FILEPATH_HOLITECH,
#endif /* CFG_CTS_FW_UPDATE_FILE_LOAD */
					firmware_builtin ? FIRMWARE_VERSION(firmware_builtin) : curr_firmware_ver);
				}
		} else if (cts_vendor_info_val == CTS_VENDOR_CTC) {
			if (is_filesystem_mounted(CFG_CTS_FIRMWARE_FILEPATH_CTC)) {
				//filepath:CFG_CTS_FIRMWARE_FILEPATH => filp_open()
				//filepath:CFG_CTS_FIRMWARE_FILENAME => request_firmware()
				firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
					cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : CFG_CTS_FIRMWARE_FILEPATH_CTC,
#else /* CFG_CTS_FW_UPDATE_FILE_LOAD */
					CFG_CTS_FIRMWARE_FILEPATH_CTC,
#endif /* CFG_CTS_FW_UPDATE_FILE_LOAD */
					firmware_builtin ? FIRMWARE_VERSION(firmware_builtin) : curr_firmware_ver);
			}
		}
			...
			const struct cts_firmware *firmware, bool to_flash)
{
	...
	if (!to_flash || !cts_dev->rtdata.has_flash) {
		...
		ret = cts_sram_writesb_check_crc_retry(cts_dev, 0, firmware_info.firmware_sect,
			firmware_info.firmware_sect_size, firmware_info.firmware_sect_crc, 3);
		if (ret)
			cts_err("Write firmware section to sram failed %d", ret);
		...
		if (cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9911S ||
			...
			cts_sram_writesb_boot_crc_retry(cts_dev, firmware_info.firmware_sect_size,
