...
static int replace_3gamma(u8 *initcode, u8 *gamma, u16 gamma_len)
{
	...
	if ((initcode[gamma_analog[0]] == gamma[gamma_pos]) &&
		...
		memcpy(initcode + gamma_analog[0] + 4, gamma + gamma_pos + 4,
		       gamma_analog[5]);
	...
	if ((initcode[gamma_digital1[0]] == gamma[gamma_pos]) &&
		...
		memcpy(initcode + gamma_digital1[0] + 4, gamma + gamma_pos + 4,
		       gamma_digital1[5]);
	...
	if ((initcode[gamma_digital2[0]] == gamma[gamma_pos]) &&
		...
		memcpy(initcode + gamma_digital2[0] + 4, gamma + gamma_pos + 4,
		       gamma_digital2[5]);
...
static int fts_ft8006m_upgrade(u8 *buf, u32 len)
{
	...
	off = upgrade_func_ft8006m.appoff + FTS_APPINFO_OFF +
	      FTS_APPINFO_APPLEN_OFF;
	app_1_len = (((u32)buf[off] << 8) + buf[off + 1]);
	off = upgrade_func_ft8006m.appoff + FTS_APPINFO_OFF +
	      FTS_APPINFO_APPLEN2_OFF;
