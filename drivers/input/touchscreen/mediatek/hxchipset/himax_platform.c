...
static int himax_common_probe_spi(struct spi_device *spi)
{
...
#if 0	//edit for stoping spi crash cause pinctrl"state_spi_all"
	...
	if (ret < 0) {
		E("init_lct_tp_info Failed!\n");
		goto err_init_lct_tp_info_failed;
	} else {
		I("init_lct_tp_info Succeeded!\n");
	}
#endif
...
static int himax_common_local_init(void)
{
	...
	if (IS_ERR_OR_NULL(mtkfb_lcm_name)){
		I("mtkfb_lcm_name ERROR!\n");
		return -ENOMEM;
	} else {
		if (strcmp(mtkfb_lcm_name,"hx83102d_vdo_hdp_boe_xinli_drv") == 0) {
			I("TP info: [Vendor]XINLI [IC]hx83102d\n");
		} else {
			I("Unknow Touch\n");
			return -ENXIO;
		}
