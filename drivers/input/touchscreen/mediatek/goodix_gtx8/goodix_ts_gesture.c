...
#define GSX_SINGLE_TYPE				0x02
#define GSX_DOUBLE_TYPE				0x01
		...
		struct goodix_ext_module *module)
{
	...
	//input_dev->event = gsx_gesture_switch;
	...
	struct goodix_ext_module *module)
{
	...
	} else if (gsx_type == 'L') {	/* single tap */
		ts_debug("Gesture match success, aod event");
		input_report_key(core_data->input_dev, KEY_GOTO, 1);
		ts_debug("start input key");
		input_sync(core_data->input_dev);
		input_report_key(core_data->input_dev, KEY_GOTO, 0);
		ts_debug("match aod end");
		input_sync(core_data->input_dev);
		goto gesture_ist_exit;
	} else {
