		...
		if (test_error_code & _GT_SHORT) {
			...
			for (index = 0; index < shortresult[0]; index++) {
				...
				if (shortresult[2 + index * 4] & 0x80) {
					...
					bytes += (s32) sprintf((char *)&data[bytes], "Drv%d is short·", shortresult[2 + index * 4] & 0x7F);
					...
					if (shortresult[2 + index * 4] == (sys.max_driver_num + 1)) {
						...
						bytes += (s32) sprintf((char *)&data[bytes], "GND\\VDD is short·");

					}

					else {

						bytes += (s32) sprintf((char *)&data[bytes], "Sen%d is short·", shortresult[2 + index * 4] & 0x7F);
