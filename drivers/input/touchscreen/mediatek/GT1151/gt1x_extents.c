...
static int gt1x_gesture_data_show(struct seq_file *file, void *data)
{
	seq_printf(file, "%d\n", gesture_enabled);
...
static int gt1x_gesture_data_open (struct inode* inode, struct file* file)
{
	return single_open(file, gt1x_gesture_data_show, inode->i_private);
}
