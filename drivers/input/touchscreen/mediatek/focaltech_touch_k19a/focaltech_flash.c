...
struct upgrade_setting_nf upgrade_setting_list[] = {
	...
	{0x87, 0x20, 0, (88 * 1024), (128 * 1024), 0xA5, 0x01, 8,  0, 1, 0, 1},
	{0x87, 0x22, 0, (88 * 1024), (128 * 1024), 0xA5, 0x01, 8,  0, 1, 0, 1},
	{0x82, 0x01, 0, (96 * 1024), (128 * 1024), 0xA5, 0x01, 8,  0, 1, 0, 0},
};
...
static int fts_fw_resume(bool need_reset, enum FW_TYPE fw_type)
{
	...
	if ((fw_type == FW_GESTURE) ||
		((fw_type == FW_AUTO) && upg->ts_data->gesture_mode && upg->ts_data->suspended)) {
...
int fts_fw_recovery(void)
{
	...
	ret = fts_fw_resume(false, FW_AUTO);
	if (ret < 0) {
...
{
	return fts_fw_resume(true, FW_GESTURE);
}

int fts_enter_normal_fw(void)
{
	return fts_fw_resume(true, FW_NORMAL);
}
...
static void fts_fwupg_work(struct work_struct *work)
{
	...
	if (ret < 0) {
		...
		sprintf(tp_version_info, "[Vendor]Tianma [TP-IC]:FT8722, [FW]0x%x\n", fw_version);
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
	...
	INIT_WORK(&ts_data->fwres_work, fts_fwres_work);
...
{
	seq_printf(m, "[Vendor]Tianma, [TP-IC]: FT8722,[FW]0x%x\n", fw_version);
	return 0;
}
