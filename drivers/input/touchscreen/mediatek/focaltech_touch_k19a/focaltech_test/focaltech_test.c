...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
	...
	snprintf(filepath, FILE_NAME_LENGTH, "%s%s", FTS_ITO_RESULT_PATH, file_name);
...
static int fts_ito_test_show(struct seq_file *file, void *data)
{
	...
	if ((fts_ftest->test_item[FTS_ENTER_FACTORY_MODE].testresult) == 1)
	...
	//Rawdata result
	if ((fts_ftest->test_item[FTS_RAWDATA_TEST].testresult == 1)
		 && (fts_ftest->test_item[FTS_CB_TEST].testresult == 1))
		test_result_bmp[RAWDATA] = 'P';
	else
		test_result_bmp[RAWDATA] = 'F';

	//open test result
	if (fts_ftest->test_item[FTS_OPEN_TEST].testresult == 1)
		test_result_bmp[OPEN_DATA] = 'P';
	else
		test_result_bmp[OPEN_DATA] = 'F';

	//short test result
	if (fts_ftest->test_item[FTS_SHORT_CIRCUIT_TEST].testresult == 1)
...
#ifdef ITO_TEST_NORMALIZED_NODE
		fts_android_touch_proc = proc_mkdir("android_touch", NULL);
		fts_ito_test_proc = proc_create("self_test", 0664,
...
static struct proc_dir_entry	*fts_tp_selftest;
#define	TEST_OPEN	1
#define	TEST_SHORT	2
...
extern	int	fts_wait_tp_to_valid(void);
static ssize_t fts_tp_selftest_write(struct file *file, const char __user *buffer,
	size_t count, loff_t *pos)
{
	int ret = 0;
	char tmp[6] = {0};
	char fwname[FILE_NAME_LENGTH] = "focaltech_mp_fw_tianma.ini";
	struct fts_ts_data *ts_data = fts_data;
	struct input_dev *input_dev;
	my_test_result = 0;
...
int fts_tp_selftest_proc_init(void)
{
	int32_t ret = 0;
	FTS_TEST_FUNC_ENTER();
	fts_tp_selftest = proc_create(FTS_PROC_TP_SELFTEST, 0444, NULL, &fts_tp_selftest_ops);
	if (fts_tp_selftest == NULL) {
		FTS_TEST_ERROR("create proc/tp_selftest Failed!\n");
		ret = -1;
		return ret;
	} else {
			FTS_INFO("create /proc/tp_selftest Succeeded!\n");
			return 0;
	}
}

void fts_tp_selftest_proc_exit(void)
{
	if (fts_tp_selftest != NULL) {
		remove_proc_entry("tp_selftest", NULL);
		fts_tp_selftest = NULL;
