...
static int fts_test_save_data_csv(struct fts_test *tdata)
{
	...
	return -EIO;//ITO_TEST_NORMALIZED_NODE
	goto csv_save_err;
    }
...
static int fts_ito_test_show(struct seq_file *file, void *data)
{
	...
	//noise test result
	if (fts_ftest->test_item[FTS_NOISE_TEST].testresult == 1)
	...
	size_t count, loff_t *pos)
{
	int ret = 0;
	char tmp[6] = {0};
	char fwname[FILE_NAME_LENGTH] = "focaltech_mp_fw_huaxing.ini";
	struct fts_ts_data *ts_data = fts_data;
	struct input_dev *input_dev;
	my_test_result = 0;
...
int fts_tp_selftest_proc(void)
{
	...
	fts_tp_selftest = proc_create(FTS_PROC_TP_SELFTEST, mode, NULL, &fts_tp_selftest_ops);
	if (fts_tp_selftest == NULL) {
		FTS_TEST_INFO("fts, create_proc_entry fts_tp_selftest failed\n");
		ret = -1;
	}
	return ret;
}
