...
int gt1x_update_prepare(char *filename)
{
	...
	if (filename == NULL) {
		...
		snprintf(buf, sizeof(buf), "%s%s.img", GT1151_DEFAULT_FW,
		...
		update_info.fw_data = (u8 *)devm_kzalloc(
			&gt1x_i2c_client->dev, fw_entry->size, GFP_KERNEL);
...
int gt1x_check_firmware(void)
{
	...
	GTP_INFO("Update type: %s",
		 update_info.update_type == UPDATE_TYPE_HEADER ? "Header"
							       : "File");
	...
	while (loaded_length != patch_size) {
		if (loaded_length == 0 ||
			...
			ret = gt1x_i2c_write_with_readback(
				_bRW_MISCTL__PATCH_AREA_EN_ + bank - 4, &tmp,
				1);
			if (ret) {
				GTP_ERROR("enable patch area access fail!");
				return ret;
