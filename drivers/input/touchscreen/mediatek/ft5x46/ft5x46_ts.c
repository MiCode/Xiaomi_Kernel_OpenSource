...
#define FT5626_FIRMWARE_VERSION		0x6040
...
#define I2C_SWITCH_DELAY_MS	200
...
/*upgrade config of FT626*/
#define FT5626_UPGRADE_AA_DELAY 		2
#define FT5626_UPGRADE_55_DELAY 		2
#define FT5626_UPGRADE_ID_1		0x58
#define FT5626_UPGRADE_ID_2		0x2C
#define FT5626_UPGRADE_READID_DELAY 		20

#define FT5316_CHIP_ID		0x0A
#define FT5X36_CHIP_ID		0x14
#define FT5X46_CHIP_ID		0x54
#define FT5626_CHIP_ID		0x58
...
static void ft5x46_i2c_switch_delayed_work(struct work_struct *work)
{
	struct delayed_work *delayed_work = to_delayed_work(work);
	struct ft5x46_data *ft5x46 = container_of(delayed_work, struct ft5x46_data, i2c_switch_delayed_work);
	int error = 0;

	mutex_lock(&ft5x46->mutex);
	error = ft5x46_hid_to_std_i2c(ft5x46);
	if (!error)
		ft5x46->using_std_i2c = true;
	mutex_unlock(&ft5x46->mutex);
	ft5x46_enable_irq(ft5x46);
	if (error)
		dev_err(ft5x46->dev, "Unable to switch to Std I2C\n");
}
...
static u8 ft5x46_get_factory_id(struct ft5x46_data *ft5x46)
{
	...
	for (i = 0, error = -1; i < sizeof(reset_delay) && error; i++) {
		...
		if (val1 == FT5316_UPGRADE_ID_1 && val2 == FT5316_UPGRADE_ID_2) {
			...
			break;
		} else if (val1 == FT5626_UPGRADE_ID_1 && val2 == FT5626_UPGRADE_ID_2) {
			ft5x46->chip_id = FT5626_CHIP_ID;
			break;
		}
	}
		...
		struct ft5x46_firmware_data *firmware, bool *upgraded)
{
	...
	/* Update touch IC hardware info */
	update_hardware_info(TYPE_TOUCH, 3);	/* Focaltech */

	/* Update TP maker hardware info, currently we only have lens */
	if (vid == 0x01)
		update_hardware_info(TYPE_TP_MAKER, 0x01);	/* biel */
	else if (vid == 0x02)
		update_hardware_info(TYPE_TP_MAKER, 0x07);	/* laibao */
	...
	if (id == firmware->data[FT5626_FIRMWARE_VERSION])
		return 0;
	dev_info(ft5x46->dev, "upgrade firmware to %02x\n",
		firmware->data[FT5626_FIRMWARE_VERSION]);
...
static int ft5x46_read_touchdata(struct ft5x46_data *ft5x46)
{
	...
	if (!ft5x46->using_std_i2c) {
		dev_err(ft5x46->dev, "Not using STD I2C, just ignore the messages!\n");
		return -ENODEV;
	}
...
static void ft5x46_report_key(struct ft5x46_data *ft5x46, int x_pos, bool down)
{
	...
	case 450:
		...
		break;
	default:
		input_report_key(ft5x46->input, KEY_GUARD, (int)down);
		break;
	}
}
...
int ft5x46_suspend(struct ft5x46_data *ft5x46)
{
	...
	cancel_delayed_work_sync(&ft5x46->i2c_switch_delayed_work);
	...
	/* Ignore all touch events until switch to STD I2C */
	ft5x46->using_std_i2c = false;
...
int ft5x46_resume(struct ft5x46_data *ft5x46)
{
	...
	schedule_delayed_work(&ft5x46->noise_filter_delayed_work,
				NOISE_FILTER_DELAY);
	schedule_delayed_work(&ft5x46->i2c_switch_delayed_work,
				msecs_to_jiffies(I2C_SWITCH_DELAY_MS));
				...
				struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		__stringify(EV_KEY) ":" __stringify(KEY_GUARD) ":30:2108:10:10:"
		__stringify(EV_KEY) ":" __stringify(KEY_MENU)  ":90:2108:10:10:"
		__stringify(EV_KEY) ":" __stringify(KEY_GUARD) ":180:2108:10:10:"
		__stringify(EV_KEY) ":" __stringify(KEY_HOME)  ":270:2108:10:10:"
		__stringify(EV_KEY) ":" __stringify(KEY_GUARD) ":360:2108:10:10:"
		__stringify(EV_KEY) ":" __stringify(KEY_BACK)  ":450:2108:10:10:"
		__stringify(EV_KEY) ":" __stringify(KEY_GUARD) ":510:2108:10:10"
		"\n");
}
...
static int ft5x46_power_init(struct ft5x46_data *data, bool on)
{
	...
	data->vdd = regulator_get(data->dev, "vtouch");
	if (IS_ERR(data->vdd)) {
		rc = PTR_ERR(data->vdd);
		dev_err(data->dev,
	...
	rc = regulator_set_voltage(data->vdd, 3300000, 3300000);

	data->vddio = regulator_get(data->dev, "vtouchio");
	...
	rc = regulator_set_voltage(data->vddio, 1800000, 1800000);
...
static int ft5x46_initialize_pinctrl(struct ft5x46_data *ft5x46)
{
	...
	ft5x46->pins_default = pinctrl_lookup_state(ft5x46->ts_pinctrl, "default");
	if (IS_ERR_OR_NULL(ft5x46->pins_default)) {
		pr_err("Can not get ts default pinstate\n");
		ret = PTR_ERR(ft5x46->pins_default);
		ft5x46->ts_pinctrl = NULL;
		return ret;
	}

	ft5x46->eint_as_int = pinctrl_lookup_state(ft5x46->ts_pinctrl, "state_eint_as_int");
	if (IS_ERR_OR_NULL(ft5x46->eint_as_int)) {
		pr_err("Can not get ts state_eint_as_int pinstate\n");
		ret = PTR_ERR(ft5x46->eint_as_int);
		ft5x46->ts_pinctrl = NULL;
		return ret;
	}

	ft5x46->eint_output0 = pinctrl_lookup_state(ft5x46->ts_pinctrl, "state_eint_output0");
	if (IS_ERR_OR_NULL(ft5x46->eint_output0)) {
		pr_err("Can not get ts state_eint_output0 pinstate\n");
		ret = PTR_ERR(ft5x46->eint_output0);
		ft5x46->ts_pinctrl = NULL;
		return ret;
	}

	ft5x46->eint_output1 = pinctrl_lookup_state(ft5x46->ts_pinctrl, "state_eint_output1");
	if (IS_ERR_OR_NULL(ft5x46->eint_output1)) {
		pr_err("Can not get ts state_eint_output1 pinstate\n");
		ret = PTR_ERR(ft5x46->eint_output1);
		ft5x46->ts_pinctrl = NULL;
		return ret;
	}

	ft5x46->rst_output0 = pinctrl_lookup_state(ft5x46->ts_pinctrl, "state_rst_output0");
	if (IS_ERR_OR_NULL(ft5x46->rst_output0)) {
		pr_err("Can not get ts state_rst_output0 pinstate\n");
		ret = PTR_ERR(ft5x46->rst_output0);
		ft5x46->ts_pinctrl = NULL;
		return ret;
	}

	ft5x46->rst_output1 = pinctrl_lookup_state(ft5x46->ts_pinctrl, "state_rst_output1");
	if (IS_ERR_OR_NULL(ft5x46->rst_output1)) {
		pr_err("Can not get ts state_rst_output1 pinstate\n");
		ret = PTR_ERR(ft5x46->rst_output1);
		ft5x46->ts_pinctrl = NULL;
		return ret;
	}
...
static int ft5x46_pinctrl_select(struct ft5x46_data *ft5x46, bool on)
{
	...
	if (on) {
		ret = pinctrl_select_state(ft5x46->ts_pinctrl, ft5x46->eint_as_int);
		if (ret)
			return ret;
		ret = pinctrl_select_state(ft5x46->ts_pinctrl, ft5x46->rst_output1);
		if (ret)
			return ret;
	} else {
		ret = pinctrl_select_state(ft5x46->ts_pinctrl, ft5x46->eint_output0);
		if (ret)
			return ret;
		ret = pinctrl_select_state(ft5x46->ts_pinctrl, ft5x46->rst_output0);
		if (ret)
			return ret;
	}
...
static void ft5x46_configure_sleep(struct ft5x46_data *ft5x46, bool enable)
{
	int ret = 0;
...
{
	struct ft5x46_ts_platform_data *pdata = ft_data->dev->platform_data;

	ft5x46_disable_irq(ft_data);

	ft5x46_get_lockdown_info(ft_data);

	ft5x46_load_firmware(ft_data, pdata->firmware, NULL);

	msleep(300);

	ft_data->power_supply_notifier.notifier_call = ft5x46_power_supply_event;
	power_supply_reg_notifier(&ft_data->power_supply_notifier);

	ft5x46_enable_irq(ft_data);
}
				...
				const struct ft5x46_bus_ops *bops)
{
	...
	set_bit(KEY_GUARD, ft5x46->input->keybit);
	...
	ft5x46->vkeys_dir = kobject_create_and_add("board_properties", NULL);
	if (!ft5x46->vkeys_dir) {
		error = -ENOMEM;
	...
	error = sysfs_create_file(ft5x46->vkeys_dir, &ft5x46->vkeys_attr.attr);
	if (error) {
	...
	ft5x46_configure_sleep(ft5x46, true);

	INIT_DELAYED_WORK(&ft5x46->noise_filter_delayed_work,
				ft5x46_noise_filter_delayed_work);

	INIT_DELAYED_WORK(&ft5x46->i2c_switch_delayed_work,
				ft5x46_i2c_switch_delayed_work);

	INIT_WORK(&ft5x46->fw_work, firmware_update_work);
	schedule_work(&ft5x46->fw_work);
