...
//#define USE_SYNAPTICS_FW_UPGRADE
#if defined(USE_SYNAPTICS_FW_UPGRADE)
#include "SynaImage.h"
#endif

//#define USE_SYNAPTICS_RED_ON_MTK

#define USE_SYNAPTICS_F54
#if defined(USE_SYNAPTICS_F54)
#include "RefCode.h"
...
// 2014.01.27 DMA is required to trasnfer large i2c data(size > 8) in MT6582
#define TOUCH_I2C_USE_DMA
...
#if defined(USE_SYNAPTICS_RED_ON_MTK)
...
int synaptics_ts_read(struct i2c_client *client, u8 reg, int num, u8 *buf)
{
	...
	if (i2c_transfer(client->adapter, msgs, 2) < 0) {
		...
		if (printk_ratelimit()){
			SYNAPTICS_ERR_MSG("transfer error\n");
			printk("[jhee: synaptics_ts_read] transfer error\n");
		}
		return -EIO;
	} else
		return 0;
...
static void synaptics_ts_work_func(struct work_struct *work)
{
	...
	if (ts->ic_init) {
		...
		if (ret < 0) {
			SYNAPTICS_ERR_MSG("FINGER_STATE_REG read fail===========\n\n num_of_finger= %d , \n sizeof(ts_reg_data)= %d ,(Maxnum-num)*NUM_OF_EACH_FINGER_DATA_REG=%d  \n NUM_OF_EACH_FINGER_DATA_REG=%d\n\n",ts->pdata->num_of_finger,sizeof(ts_reg_data),((MAX_NUM_OF_FINGER - ts->pdata->num_of_finger) * NUM_OF_EACH_FINGER_DATA_REG),NUM_OF_EACH_FINGER_DATA_REG);
			goto exit_work;
		}
...
int touch_power_control(int on)
{
	...
	if(on)
		hwPowerOn(MT6323_POWER_LDO_VGP1, VOL_3000, "TP"); //jhee
	else
		hwPowerDown(MT6323_POWER_LDO_VGP1, "TP"); //jhee
...
static struct synaptics_ts_platform_data l10_ts_data = {
	...
	.irqflags = CUST_EINT_TOUCH_PANEL_TYPE, //Falling edge
	...
	.num_of_button = 3,
	.button = {KEY_BACK,KEY_MENU,KEY_HOMEPAGE},
#endif
...
#ifdef TOUCH_I2C_USE_DMA
	I2CDMABuf_va = (u8 *)dma_alloc_coherent(NULL, 4096, (dma_addr_t*)&I2CDMABuf_pa, GFP_KERNEL);
	if(!I2CDMABuf_va) {
...
#ifdef TOUCH_I2C_USE_DMA
	if(I2CDMABuf_va) {
		dma_free_coherent(NULL, 4096, I2CDMABuf_va, (dma_addr_t)I2CDMABuf_pa);
		I2CDMABuf_va = NULL;
		I2CDMABuf_pa = 0;
	}
#endif // TOUCH_I2C_USE_DMA
