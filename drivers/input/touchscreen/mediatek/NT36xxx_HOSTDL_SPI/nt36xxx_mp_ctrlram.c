...
{
#ifdef	CONFIG_NVT_ITO_SELFTEST
	NVT_LOG("show ITO test result!\n");

	/*SPI test result*/
	if (TestResult_SPI == 0) {
		ITO_result[1] = 'P';
		printk("[%s]: ITO SPI test succeed!\n", __func__);
	} else {
		ITO_result[1] = 'F';
		printk("[%s]: ITO SPI test fail!\n", __func__);
	}

	/* FW_Rawdata  test result*/
	if (TestResult_FW_Rawdata == 0) {
		ITO_result[4] = 'P';
		printk("[%s]: ITO FW_Rawdata test succeed!\n", __func__);
	} else {
		ITO_result[4] = 'F';
		printk("[%s]: ITO FW_Rawdata test fail!\n", __func__);
	}

	/* open test result*/
	if (TestResult_Open == 0) {
		ITO_result[7] = 'P';
		printk("[%s]: ITO Open test succeed!\n", __func__);
	} else {
		ITO_result[7] = 'F';
	...
	if (TestResult_Short == 0) {
		ITO_result[10] = 'P';
		printk("[%s]: ITO Short test succeed!\n", __func__);
	} else {
		ITO_result[10] = 'F';
		printk("[%s]: ITO Short test fail!\n", __func__);
	}

	/* Noise test result*/
	if (TestResult_Noise == 0) {
		ITO_result[13] = 'P';
		printk("[%s]: ITO Noise test succeed!\n", __func__);
	} else {
		ITO_result[13] = 'F';
...
/*******************************************************
Description:
	Novatek touchscreen /proc/tp_selftest open function.
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	//-----spi test----
	if (nvt_read_pid() != 0) {
		TestResult_SPI = -1; // -1:ERROR
	} else {
		TestResult_SPI = 0; // 0:PASS
	}
...
{
	NVT_proc_selftest_entry = proc_create("tp_selftest", 0664, NULL, &nvt_selftest_fops);
