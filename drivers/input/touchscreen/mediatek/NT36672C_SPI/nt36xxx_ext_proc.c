...
int32_t nvt_check_proximity(uint8_t input_id, uint8_t *data)
{
	...
	int ps_touch_event;
	if ((input_id == DATA_PROTOCOL) && (func_type == NVT_FUNCPAGE_PROXIMITY)) {
		ret = proximity_state;
		if (proximity_level == NVT_PROXIMITY_LEVEL1 || proximity_level == NVT_PROXIMITY_LEVEL2
			...
			ps_touch_event = proximity_level;
...
return:
	Executive outcomes. 0---succeed.
*******************************************************/
static ssize_t nvt_psensor_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char temp_buff[64] = {0};
	uint32_t pdata;
	unsigned long ps_touch_event = 1;
...
{
	seq_printf(m, "0x46,0x36,0x32,0x01,0x4D,0x13,0x31,0x00\n");
	return 0;
}
...
int proc_node_init(void)
{
	...
	if (proc_touchscreen_dir == NULL)  {
		...
		proc_lockdown_info_file = proc_create(PROC_CTP_LOCKDOWN_INFO_FILE,
...
int32_t nvt_extra_proc_init(void)
{
	...
	NVT_proc_psensor_entry = proc_create(NVT_PSENSOR, 0444, NULL,&nvt_psensor_fops);
	...
	NVT_proc_tp_lockdown_info_entry = proc_create(NVT_TP_LOCKDOWN_INFO, 0444, NULL, &nvt_tp_lockdown_info_fops);
	...
	NVT_proc_tp_selftest_entry = proc_create(NVT_TP_SELFTEST, 0777, NULL, &aftersale_test_ops);
