 ...
 ***********************************************************************/
static int fts_ft5422u_upgrade(u8 *buf, u32 len)
{
	int ret = 0;
	u32 start_addr = 0;
	u8 cmd[4] = {0};
	...
	if (memcmp((char *)buf, "APPIDX", 6) != 0) {
		FTS_ERROR(
			"fw buffer signature is not match IC(5422U), no upgrade");
		return -EINVAL;
	}
