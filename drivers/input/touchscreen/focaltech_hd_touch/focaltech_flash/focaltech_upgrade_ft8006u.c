 ...
 *****************************************************************************/
u8 pb_file_ft8006u[] = {
#include "../include/pramboot/FT8006U_Pramboot_V0.8_20180907.i"
};
...
static int cal_lcdinitcode_ecc(u8 *buf, u16 *ecc_val)
{
	...
	/*         FTS_INFO("actual mipi bank number = %x", bank_num); */
		...
		if (i >= sizeof(bank_mapping) / sizeof(u8)) {
			FTS_INFO(
				"actual mipi bank(%d) not find in bank mapping, need jump",
				bank_num);
		} else {
...
static int fts_ft8006u_upgrade_mode(enum FW_FLASH_MODE mode, u8 *buf, u32 len)
{
	int ret = 0;
	bool flag = false;
	u32 start_addr = 0;
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
static int fts_ft8006u_upgrade(u8 *buf, u32 len)
{
	int ret = 0;
	u8 *tmpbuf = NULL;
	u32 app_len = 0;
	...
	ret = fts_ft8006u_upgrade_mode(FLASH_MODE_APP, tmpbuf, app_len);
	if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
static int fts_ft8006u_lic_upgrade(u8 *buf, u32 len)
{
	int ret = 0;
	u8 *tmpbuf = NULL;
	u32 lic_len = 0;
...
/* remalloc memory for initcode, need change content of initcode */
	...
	ret = fts_ft8006u_upgrade_mode(FLASH_MODE_LIC, tmpbuf, lic_len);
	if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
static int fts_ft8006u_param_upgrade(u8 *buf, u32 len)
{
	int ret = 0;
	u8 *tmpbuf = NULL;
	u32 param_length = 0;
	...
	ret = fts_ft8006u_upgrade_mode(FLASH_MODE_PARAM, tmpbuf, param_length);
	if (ret < 0) {
		FTS_INFO("fw upgrade fail,reset to normal boot");
		if (fts_fwupg_reset_in_boot() < 0)
			FTS_ERROR("reset to normal boot fail");
