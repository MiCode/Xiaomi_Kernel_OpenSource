...
static size_t fwrite(const void* buf, size_t size, size_t count, struct file* fp)
{
    ...
    pos = fp->f_pos;
    len = kernel_write(fp, buf, size * count, &pos);
	fp->f_pos = pos;
...
#define GTX8_OPENSHORT_RESULT_PATH	"/sdcard/gtx8_openshort_result.txt"
...
#if defined(CONFIG_WT_QGKI)	
	fp = filp_open(GTX8_OPENSHORT_RESULT_PATH, O_CREAT|O_TRUNC|O_WRONLY ,0666);
#endif	
	if (IS_ERR(fp)) {
...
int parse_csvfile(struct goodix_ts_device *ts_dev, char *target_name, int32_t  *data, int rows, int columns)
{
	...
	buf = (char *)vmalloc(PAGE_SIZE * (firmware->size + 1));
	if(NULL == buf) {
...
int gtx8_get_rawdata(void *tsdev,struct ts_rawdata_info *info)
{
	...
	gts_test = vmalloc(PAGE_SIZE * sizeof(struct gtx8_ts_test));
	if (!gts_test) {
