...
#define GTX8_TEST_FILE_NAME			"gtx8_limits.csv"
#define GTX8_RESULT_SAVE_PATH			"/data/vendor/fac_sources/"
...
static int gtx8_cache_origconfig(struct gtx8_ts_test *ts_test)
{
	...
	if (core_data->ts_dev->hw_ops->read_config) {
		ret = core_data->ts_dev->hw_ops->read_config(core_data->ts_dev,
				&ts_test->orig_config.data[0], GOODIX_CFG_MAX_SIZE);
		if (ret < 0) {
...
static int gtx8_poll_read(struct gtx8_ts_test *ts_test,
		unsigned int addr, unsigned char *data, unsigned int len)
{
	struct goodix_ts_core *cd = ts_test->ts;
	const struct goodix_ts_hw_ops *hw_ops = cd->ts_dev->hw_ops;
	int ret;
	int retry;
	unsigned char val;

	val = 0;
	hw_ops->write_trans(cd->ts_dev, cd->ts_dev->reg.coor, &val, 1);

	retry = 20;
	while (retry--) {
		usleep_range(5000, 5100);
		ret = hw_ops->read_trans(cd->ts_dev, cd->ts_dev->reg.coor, &val, 1);
		if (ret < 0) {
			ts_err("read failed");
			return ret;
		}
	...
	if (retry < 0) {
		ts_err("rawdata is not ready, status:%x", val);
		return -EINVAL;
	}

	return hw_ops->read_trans(cd->ts_dev, addr, data, len);
}
...
static int gtx8_cache_rawdata(struct gtx8_ts_test *ts_test)
{
	...
	for (j = 0; j < GTX8_RETRY_NUM_3; j++) {
		/* read rawdata */
		ret = gtx8_poll_read(ts_test, rawdata_addr,
...
static int gtx8_cache_self_rawdata(struct gtx8_ts_test *ts_test)
{
	...
	for (j = 0; j < GTX8_RETRY_NUM_3; j++) {
		/* read selfrawdata */
		ret = gtx8_poll_read(ts_test, self_rawdata_addr,
...
static int gtx8_cache_noisedata(struct gtx8_ts_test *ts_test)
{
...
#ifdef NOISE_DATA_TEST
	/* read noise data */
	ret1 = gtx8_poll_read(ts_test, noisedata_addr,
	...
	ret1 = gtx8_poll_read(ts_test, self_noisedata_addr,
...
int goodix_get_lockdown_info(struct goodix_ts_core *core_data)
{
	...
	printk("lockdown_info:%0x", core_data->lockdown_info);
	return 0;
}

#define TOUCH_OS_LOCKDOWN	"tp_lockdown_info"
/* proc node */
			...
			size_t count, loff_t *pos)
{
	...
	snprintf(temp_buf, 100, "[MODULE]SAMSUNG,[IC]GT%s,[FW]%02x\n", pid, fw_ver->vid[3]);
	return simple_read_from_buffer(buf, count, pos, temp_buf, strlen(temp_buf));
}
...
static int gtp_i2c_test(void)
{
	...
	struct goodix_ts_device *ts_device = core_data->ts_dev;
				...
				size_t count, loff_t *pos)
{
	...
	if (!strncmp("short", tmp, 5) || !strncmp("open", tmp, 4)) {
		retval = gtx8_get_rawdata(dev, info);
	} else if (!strncmp("i2c", tmp, 3))
		retval = gtp_i2c_test();

	cd->result_type = retval;
...
#define PROC_FTS_LOCKDOWN_INFO_FILE   "lockdown_info"
static int ctp_lockdown_proc_read(struct seq_file *s, void *v)
{
    unsigned int g_tp_color = 0x00003200;

    ts_info("%s:Get tp color is:0x%02x.\n", __func__, g_tp_color);
     seq_printf(s, "%08x\n", g_tp_color);//black
        return  0;
}
...
static int lockdown_info_open(struct inode *inode, struct file *file)
{
    ...
    return single_open(file, ctp_lockdown_proc_read, NULL);
}
				...
				size_t count, loff_t *ppos)
{
	...
	ts_info("sleep_enable:%d", cd->sleep_enable);
	return count;
				...
				size_t count, loff_t *ppos)
{
	...
	ts_info("aftersale test in!");
	if (copy_from_user(temp_buf, buf, count)) {
		ts_err("read proc input error");
		return count;
	}

	ts_info("temp_buf = %s ", temp_buf);

	if(!strncmp("short", temp_buf, 5) || !strncmp("open", temp_buf, 4)){
		aftersale_selftest = 1;
	} else if (!strncmp("i2c", temp_buf, 3)) {
			...
			size_t count, loff_t *pos)
{
	...
	struct device *dev =  &cd->pdev->dev;
	struct goodix_ts_core *core_data = dev_get_drvdata(dev);
	struct goodix_ts_device *device = core_data->ts_dev;
	struct ts_rawdata_info *info;
	char temp[32] = "0P-1P-2P-3P-5P";
	int retval = 0;
	u8 arry[10] = {0};
	...
	if (aftersale_selftest == 1) {
		gtx8_get_rawdata(dev, info);
		if (strncmp(temp, info->result, 14) == 0) {
			retval = SELF_TEST_OK;
		} else {
			retval = SELF_TEST_NG;
		}
	} else if (aftersale_selftest == 2) {
		retval = device->hw_ops->read(device, 0x4022,arry,8);
		if (!retval)
			retval = SELF_TEST_OK;
		else
			retval = SELF_TEST_NG;
...
/*proc init part*/
struct proc_dir_entry *proc_ctp_lockdown_info = NULL;
int proc_node_init(struct goodix_ts_core *core_data)
{
	int r = 0;
	ts_info("procfs init start!");

	goodix_touch_proc_dir = proc_mkdir(TOUCH_PROC_DIR, NULL);

	proc_create_data("tp_data_dump", 0644, NULL, &goodix_rawdata_info_ops, core_data);
	proc_ctp_lockdown_info = proc_create(PROC_FTS_LOCKDOWN_INFO_FILE, 0644,goodix_touch_proc_dir, &fts_proc_lockdown_info_ops);
	proc_create_data(TOUCH_OS_LOCKDOWN, 0644, NULL, &goodix_lockdown_info_ops, core_data);
	proc_create_data("tp_info", 0644, NULL, &goodix_fw_version_info_ops, core_data);
	proc_create_data(PROC_CTP_OPENSHORT_TEST_FILE , 0644, goodix_touch_proc_dir, &gtp_selftest_ops, core_data);
	proc_create_data("tp_sleep", 0777, NULL, &gtp_sleep_ops, core_data);
	proc_create_data("tp_selftest", 0666, NULL, &aftersale_test_ops, core_data);
