...
static int gtp_set_cur_value(int gtp_mode, int gtp_value)
{
	...
	if(gtp_mode == Touch_Aod_Enable && goodix_core_data && gtp_value >= 0) {
		goodix_core_data->tid->aod_status = gtp_value;
		goodix_core_data->gesture_enabled = goodix_core_data->double_wakeup || gtp_value || goodix_core_data->tid->fod_status;
		ts_info("set aod status:%d, gesture_enabled:%d", gtp_value, goodix_core_data->gesture_enabled);
		return 0;
	}
	if(gtp_mode == Touch_Fod_Enable && goodix_core_data && gtp_value >= 0) {
		goodix_core_data->tid->fod_status = gtp_value;
		goodix_core_data->fod_status = gtp_value;
		goodix_core_data->gesture_enabled = goodix_core_data->double_wakeup || gtp_value || goodix_core_data->tid->aod_status;
		ts_info("set fod status:%d, gesture_enabled:%d", gtp_value, goodix_core_data->gesture_enabled);
		return 0;
	}
