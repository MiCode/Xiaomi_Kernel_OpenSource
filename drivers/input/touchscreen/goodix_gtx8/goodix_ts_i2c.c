	...
	struct goodix_ts_board_data *board_data)
{
	...
	if (r < 0) {
		ts_err("invalid avdd_gpio in dt: %d", r);
		return -EINVAL;
	}
	ts_info("get avdd_gpio[%d] from dt", r);
	board_data->avdd_gpio = r;

	r = of_get_named_gpio(node, "goodix,dvdd-gpio", 0);
	if (r < 0) {
		ts_err("invalid dvdd-gpio in dt: %d", r);
		return -EINVAL;
	}
	ts_info("get dvdd-gpio[%d] from dt", r);
	board_data->dvdd_gpio = r;

	/*memset(board_data->avdd_name, 0, sizeof(board_data->avdd_name));
	r = of_property_read_string(node, "goodix,avdd-name", &name_tmp);
	printk("taocheng:%s name_tmp is %s", __func__, name_tmp);
	if (!r) {
		...
		struct goodix_ts_version *version)
{
	...
	ts_info("PID:%s,SensorID:%d, VID:%*ph,fw:%02x", version->pid,
		version->sensor_id, (int)sizeof(version->vid), version->vid,version->vid[3]);
	sprintf(Ctp_name,"[MODULE]SAMSUNG,[IC]GT%s,FW:%02x",version->pid,version->vid[3]);
	ts_info("goodix_read_fw_ver Ctp_name is : %s\n",Ctp_name);
exit:
		...
		struct goodix_ts_config *config)
{
	...
	ts_info("[MODULE]Samsung, [IC]GT9896, [Cfg_ver]0x%02x\n", config->data[0]);
		...
		u8 *pre_buf, u32 pre_buf_len)
{
	...
	/*read done*/
	goodix_i2c_write_trans(dev, dev->reg.coor, &flag, 1);
		...
		struct goodix_ts_event *ts_event)
{
	...
	if (dev->ic_type == IC_TYPE_YELLOWSTONE &&
		...
		if (++error_cnt >= 2) {
			error_cnt = 0;
			pre_buf[0] = 0;
			goodix_i2c_write_trans(dev, dev->reg.coor, pre_buf, 1);
		}
		return -EINVAL;
	}
