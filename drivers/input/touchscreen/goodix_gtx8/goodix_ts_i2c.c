...
#define TS_DT_COMPATIBLE "goodix,gt9883"
...
#define TS_REG_REQUEST		0x60d1
#define TS_REG_VERSION		0x452c
	...
	if (enable) {
		...
		if (dev->doze_mode_set_count == 1) {
			...
			for (i = 0;i < TS_DOZE_DISABLE_RETRY_TIMES;i++) {
				...
				goodix_i2c_write_trans(dev, 0x4100, buf, 1);
			}
					      ...
					      u8 *config_data, u32 config_len)
{
	...
	// ts_info("goodix_ts_device->reg.command:%d", cmd_reg);
		...
		u8 *coor_buf, u32 coor_buf_len, int touch_num)
{
	...
	for (i = 0; i < touch_num - 1; i++) {
		...
		/*swap*/
		if (need_swap) {
			memmove(temp_buf, small, BYTES_PER_COORD);
			memcpy(small,
...
static int goodix_esd_check(struct goodix_ts_device *dev)
{
	...
	if (dev->board_data->tp_test_flag == TP_TEST_START) {
		ts_err("tp testing!");
		return 0;
	}
...
{
	if(!strcmp(str,"dsi_s6e8fco_samsung_hdp_video_display")) {
		is_ft_lcm = 1;
		printk("Func:%s is_ft 1:%d", __func__, is_ft_lcm);
		ts_info("enter lcm_deterct");
	}
	return 0;
}

__setup("msm_drm.dsi_display0=",is_lcm_detect);
