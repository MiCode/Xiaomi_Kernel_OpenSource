...
int fts_get_lockdown_info(u8 *data)
{
	...
	log_debug("%s lockDownInfo started...\n", tag);
	ret = fts_disableInterrupt();
	ret = fts_writeFwCmd(cmd, 1);
	if (ret < OK) {
		log_error("%s lockDownInfo: ERROR %08X\n", tag, ret);
		ret |= fts_enableInterrupt();
		return ret;
	}


	if (FTS_LOCKDOWN_SIZE <= 4)
		num_event = 1;
	else if (FTS_LOCKDOWN_SIZE % 4 == 0)
		num_event = FTS_LOCKDOWN_SIZE / 4;
	else
		num_event = (FTS_LOCKDOWN_SIZE) / 4 + 1;

	log_debug("%s lockDownInfo: num_event = %d \n", tag, num_event);
	for (i = 0; i < num_event; i++) {
		ret = pollForEvent(event_to_search, 3, &readEvent[0], GENERAL_TIMEOUT);
		if (ret < OK) {
			log_error("%s lockDownInfo: ERROR %02X\n", tag, ret);
			ret |= fts_enableInterrupt();
			return ret;
		}
	...
	ret |= fts_enableInterrupt();
	log_debug("%s lockDownInfo: Finished!\n", tag);
	return OK;
}
