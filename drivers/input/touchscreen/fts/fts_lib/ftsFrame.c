...
int getOffsetFrame(u16 address, u16 *offset)
{
	...
	if (readCmdU16(cmd, address, data, OFFSET_LENGTH, DUMMY_FRAMEBUFFER) < 0) {
		log_error("%s getOffsetFrame: ERROR %02X\n", tag, ERROR_I2C_R);
		return ERROR_I2C_R;
	} else {
		u8ToU16(data, offset);
...
int getChannelsLength(void)
{
	...
	if (data == NULL) {
		log_error("%s getChannelsLength: ERROR %02X\n", tag, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
	...
	log_debug("%s Force_len = %d   Sense_Len = %d \n", tag, force_len, sense_len);
	kfree(data);
	return OK;
}
...
int getFrameData(u16 address, int size, short **frame)
{
	...
	if (data == NULL) {
		log_error("%s getFrameData: ERROR %02X\n", tag, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
...
int getMSFrame(u16 type, MutualSenseFrame *frame, int keep_first_row)
{
	...
	if (getSenseLen() == 0 || getForceLen() == 0) {
		...
		if (ret < OK) {
			log_error("%s getMSFrame: ERROR %02X\n", tag, ERROR_CH_LEN);
...
int requestFrame(u16 type)
{
	...
	while (retry < FRAME_DATA_READ_RETRY) {
		...
		if (ret < OK) {
			log_error("%s requestFrame:  ERROR %02X\n", tag, ERROR_I2C_W);
			return ERROR_I2C_W;
		}
		...
		if (ret < OK) {
			log_debug("%s Event did not Found at %d attemp!\n", tag, retry + 1);
			retry += 1;
		} else {
			retry = 0;
			break;
	...
	if (answer == type)
		...
		log_error("%s The event found has a different type of Frame data ERROR %02X \n", tag, ERROR_DIFF_COMP_TYPE);
...
int readFrameDataHeader(u16 type, DataHeader *header)
{
	...
	if (readCmdU16(FTS_CMD_FRAMEBUFFER_R, offset, data, FRAME_DATA_HEADER, DUMMY_FRAMEBUFFER) < 0) {
		log_error("%s  readFrameDataHeader: ERROR %02X \n", tag, ERROR_I2C_R);
		return ERROR_I2C_R;
	}
	...
	if (data[0] != FRAME_HEADER_SIGNATURE) {
		log_error("%s readFrameDataHeader: ERROR %02X The Header Signature was wrong!  %02X != %02X \n", tag, ERROR_WRONG_COMP_SIGN, data[0], HEADER_SIGNATURE);
	...
	log_debug("%s Type of Frame data OK! \n", tag);
	header->type = type;
...
int getMSFrame2(u16 type, MutualSenseFrame *frame)
{
	...
	if (!(type == MS_TOUCH_ACTIVE || type == MS_TOUCH_LOW_POWER || type == MS_TOUCH_ULTRA_LOW_POWER || type == MS_KEY)) {
		log_error("%s getMSFrame: Choose a MS type of frame data ERROR %02X\n", tag, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
...
int getSSFrame2(u16 type, SelfSenseFrame *frame)
{
	...
	if (!(type == SS_TOUCH || type == SS_KEY || type == SS_HOVER || type == SS_PROXIMITY)) {
		log_error("%s getSSFrame: Choose a SS type of frame data ERROR %02X\n", tag, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
	...
	if (ret < 0) {
		log_error("%s getSSFrame: ERROR %02X\n", tag, ERROR_REQU_COMP_DATA);
		return (ret | ERROR_REQU_COMP_DATA);
	}
	...
	if (temp == NULL) {
		log_error("%s getSSFrame: temp ERROR %02X\n", tag, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
	...
	if (frame->force_data == NULL) {
		log_error("%s getSSFrame: frame->force_data ERROR %02X\n", tag, ERROR_ALLOC);
		kfree(temp);
		return ERROR_ALLOC;
	}
	...
	if (frame->sense_data == NULL) {
		log_error("%s getSSFrame: frame->sense_data ERROR %02X\n", tag, ERROR_ALLOC);
		kfree(temp);
