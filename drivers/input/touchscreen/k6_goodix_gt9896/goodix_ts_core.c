...
int goodix_ts_stage2_init(struct goodix_ts_core *core_data)
{
	...
	snprintf(temp_buf, 64, "[MODULE]SUNSANG,[IC]GT%s,VID:%02x %02x %02x %02x SENSOR_ID:%d\n",
		ts_dev->chip_version.pid, ts_dev->chip_version.vid[0], ts_dev->chip_version.vid[1], ts_dev->chip_version.vid[2],
		ts_dev->chip_version.vid[3], ts_dev->chip_version.sensor_id);
