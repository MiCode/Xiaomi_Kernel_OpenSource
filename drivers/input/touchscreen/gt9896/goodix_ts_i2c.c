	...
	struct goodix_ts_board_data *board_data)
{
	...
	if (!r) {
		ts_info("read touch gamemode parameter config1:[%d, %d, %d, %d, %d]",
	...
	if (!r) {
		ts_info("read touch gamemode parameter config2:[%d, %d, %d, %d, %d]",
	...
	if (!r) {
		ts_info("read touch gamemode parameter config3:[%d, %d, %d, %d, %d]",
		...
		struct goodix_ts_event *ts_event)
{
	...
	if (core_data->palmsensor_on) {
		r = dev->hw_ops->read_trans(dev, dev->reg.proximity,
						&temp_data, 1);
		if (r < 0) {
			ts_err("get palm event failed, ret=%d, temp_data=0x%x",
				r, temp_data);
		} else {
			core_data->palmsensor_val = (temp_data & GOODIX_LRAGETOUCH_EVENT);
