...
#define TEST_CODE_VERSION			"tp_test_v1.0.7"
/* test switch */
...
#define FORMAT_PATH(path, mdir, name, suffix) do {\
	...
	sprintf((char *)path, "%s%s%s", mdir, name, suffix);\
} while (0)

static int g_save_result;
...
static u8 gt9885_drv_map[] = {45, 47, 49, 48, 46, 50, 53, 52, 51, 54, 56, 55, 59, 58, 57, 60, 61, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255};
static u8 gt9885_sen_map[] = {32, 28, 34, 30, 26, 35, 24, 31, 33, 11, 27, 29, 25, 13, 12, 23, 8, 10, 9, 6, 7, 5, 4, 1, 0, 75, 74, 40, 73, 72, 39, 38, 37, 36, 255, 255};
...
static u8 gt9896_sen_map[] = {34, 32, 35, 33, 30, 31, 29, 28, 27, 25, 26, 19, 24, 14, 17, 18, 15, 16, 6, 13, 3, 12, 0, 1, 5, 75, 4, 2, 44, 74, 41, 42, 43, 38, 40, 36, 39, 37, 66, 65};
...
struct ts_test_diffdata {
	s16 data[MAX_SEN_NUM * MAX_DRV_NUM];
	u32 size;
};
...
struct gtx8_ts_test {
	...
	struct goodix_ts_cmd selfdata_cmd;
#endif
	struct ts_test_params *test_params;
	struct ts_test_rawdata rawdata;
	struct ts_test_rawdata *rawdata_arr;
	struct ts_test_rawdata *accord_arr;
	struct ts_test_rawdata noisedata;
	struct ts_test_diffdata diffdata;
...
static void gtx8_data_cal(u16 *data, size_t data_size, u16 *stat_result);

/*******************************save csv**********************************************/

#ifdef SAVE_IN_CSV
static size_t fwrite(const void *buf, size_t size, size_t count, struct file *fp)
{
	mm_segment_t fs;
	loff_t pos;
	int len;
	...
	set_fs(KERNEL_DS);
	pos = fp->f_pos;
	len = vfs_write(fp, buf, size*count, &pos);
	fp->f_pos = pos;
	set_fs(fs);
	return len;
}

/* save test config in csv file */
static s32 gtx8_save_test_config(struct gtx8_ts_test *ts_test, struct file *fp)
{
	s32 ret = 0;
	u16 i = 0;
	s32 bytes = 0;
	...
	bytes += (int) sprintf((char *)&data[bytes], "<OrderConfig>\n");
	for (i = 0; i < ts_test->test_config->length; i++) {
		bytes += (s32) sprintf((char *)&data[bytes], "0x%02x,", ts_test->test_config->data[i]);
		if ((i+1)%ts_test->test_params->drv_num == 0) {
			ret = fwrite(data, bytes, 1, fp);
			if (ret < 0) {
				ts_err("test config write fail.\n");
				goto save_end;
			}
			bytes = 0;
		}
...
/* save header info in csv file */
static s32 gtx8_save_header(struct gtx8_ts_test *ts_test, struct file *fp)
{
	s32 ret = 0;
	u16 i = 0;
	s32 bytes = 0;
	...
	bytes += (s32) sprintf((char *)&data[bytes], "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n");
	bytes += (int) sprintf((char *)&data[bytes], "<TESTLOG>\n");
	bytes += (int) sprintf((char *)&data[bytes], "<Header>\n");
	/* save test result */
	for (i = 0; i < MAX_TEST_ITEMS; i++) {
		if (ts_test->test_result[i] == SYS_SOFTWARE_REASON
				|| ts_test->test_result[i] == GTP_PANEL_REASON) {
			result = 1;
			break;
		}
	}
	if (result) {
		bytes += (int) sprintf((char *)&data[bytes], "<Result>NG</Result>\n");
	} else {
		bytes += (int) sprintf((char *)&data[bytes], "<Result>OK</Result>\n");
	}
	bytes += (int) sprintf((char *)&data[bytes], "<DeviceType>GT%s</DeviceType>\n",
			core_data->ts_dev->chip_version.pid);
	bytes += (int) sprintf((char *)&data[bytes], "<SensorId>%d</SensorId>\n",
			core_data->ts_dev->chip_version.sensor_id);
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
		ts_err("header write fail.\n");
		goto save_end;
	}
	bytes = 0;
	/* save test config */
	ret = gtx8_save_test_config(ts_test, fp);
	if (ret < 0)
		goto save_end;
	...
	/* item list */
	bytes += (int) sprintf((char *)&data[bytes], "<ItemList>\n");
...
static s32 gtx8_save_tail(struct gtx8_ts_test *ts_test, struct file *fp)
{
	s32 ret = 0;
	s32 bytes = 0;
	char *data;
...
/* save beyond compare */
static s32 gtx8_save_cnt_data(struct gtx8_ts_test *ts_test, struct file *fp,
		u8 *src_data, int len, u8 *title)
{
	s32 ret = 0;
	u16 i = 0;
	s32 bytes = 0;
	...
	bytes += (int) sprintf((char *)&data[bytes], "<%s>\n", title);
	for (i = 0; i < len; i++) {
		bytes += (s32) sprintf((char *)&data[bytes], "%d,", src_data[i]);
		if ((i+1)%ts_test->test_params->drv_num == 0) {
			bytes += (s32) sprintf((char *)&data[bytes], "\n");
			ret = fwrite(data, bytes, 1, fp);
			if (ret < 0) {
				ts_err("beyond cnt write fail.\n");
				goto save_end;
			}
			bytes = 0;
		}
	}
	if (i%ts_test->test_params->drv_num != 0)
		bytes += (s32) sprintf((char *)&data[bytes], "\n");
	bytes += (s32) sprintf((char *)&data[bytes], "</%s>\n", title);
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
		ts_err("beyond cnt  write fail.\n");
		goto save_end;
...
/* save data limits in csv file */
static s32 gtx8_save_limits_data(struct gtx8_ts_test *ts_test, struct file *fp,
		int *src_data, int len, u8 *title)
{
	s32 ret = 0;
	u16 i = 0;
	s32 bytes = 0;
...
static s32 gtx8_save_limits(struct gtx8_ts_test *ts_test, struct file *fp)
{
	s32 ret = 0;
	s32 bytes = 0;
	int i = 0;
	...
	bytes += (s32) sprintf((char *)&data[bytes], "<TestItems>\n");

#ifdef SHORT_TEST
	/* save short result */
	bytes += (int) sprintf((char *)&data[bytes], "<Item name=\"Short Test\">\n");
	bytes += (int) sprintf((char *)&data[bytes], "<ShortNum>%d</ShortNum>\n",
	...
	for (i=0;i<ts_test->short_res->short_num;i++) {
		...
		if (chn1 == CHN_VDD) {
			bytes += (int) sprintf((char *)&data[bytes], "<ShortMess Chn1=\"VDD\" ");
		} else if (chn1 == CHN_GND) {
			bytes += (int) sprintf((char *)&data[bytes], "<ShortMess Chn1=\"GND\" ");
		} else if (chn1 & DRV_CHANNEL_FLAG) {
			bytes += (int) sprintf((char *)&data[bytes],
					"<ShortMess Chn1=\"Tx%d\" ", chn1&0x7f);
		} else {
			bytes += (int) sprintf((char *)&data[bytes],
					"<ShortMess Chn1=\"Rx%d\" ", chn1&0x7f);
		}
		if (chn2 == CHN_VDD) {
			bytes += (int) sprintf((char *)&data[bytes],
	...
	bytes += (s32) sprintf((char *)&data[bytes], "</Item>\n");
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
	...
	/* rawdata limit */
	bytes += (int) sprintf((char *)&data[bytes], "<Item name=\"Rawdata Test Sets\">\n");
	bytes += (int) sprintf((char *)&data[bytes], "<TotalFrameCnt>%d</TotalFrameCnt>\n",
			frame_num);
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
		ts_err("limit write fail.\n");
	...
	bytes = 0;
	/* save max limit */
	gtx8_save_limits_data(ts_test, fp, &ts_test->test_params->max_limits[0],
			ts_test->rawdata.size, "MaxRawLimit");
	gtx8_save_cnt_data(ts_test, fp, &ts_test->open_res->beyond_max_limit_cnt[0],
			ts_test->rawdata.size, "BeyondRawdataUpperLimitCnt");
	gtx8_save_limits_data(ts_test, fp, &ts_test->test_params->min_limits[0],
			ts_test->rawdata.size, "MinRawLimit");
	gtx8_save_cnt_data(ts_test, fp, &ts_test->open_res->beyond_min_limit_cnt[0],
			ts_test->rawdata.size, "BeyondRawdataLowerLimitCnt");
	gtx8_save_limits_data(ts_test, fp, &ts_test->test_params->deviation_limits[0],
			ts_test->rawdata.size, "MaxAccordLimit");
	gtx8_save_cnt_data(ts_test, fp, &ts_test->open_res->beyond_accord_limit_cnt[0],
			ts_test->rawdata.size, "BeyondAccordLimitCnt");

	bytes += (s32) sprintf((char *)&data[bytes], "</Item>\n");
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
		ts_err("Item write fail.\n");
		goto save_end;
	}
	bytes = 0;
...
#ifdef NOISE_DATA_TEST
	/* save noise limit */
	bytes += (int) sprintf((char *)&data[bytes], "<Item name=\"Diffdata Test Sets\">\n");
	bytes += (int) sprintf((char *)&data[bytes], "<TotalFrameCnt>1</TotalFrameCnt>\n");
	bytes += (int) sprintf((char *)&data[bytes], "<MaxJitterLimit>%d</MaxJitterLimit>\n",
			ts_test->test_params->noise_threshold);
	bytes += (s32) sprintf((char *)&data[bytes], "</Item>\n");
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
		ts_err("Noise limit write fail.\n");
		goto save_end;
	}
	bytes = 0;
#endif
#ifdef SELF_RAWDATA_TEST
	/* self rawdata limit */
	bytes += (int) sprintf((char *)&data[bytes], "<Item name=\"Self Rawdata Test Sets\">\n");
	bytes += (int) sprintf((char *)&data[bytes], "<TotalFrameCnt>1</TotalFrameCnt>\n");
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
		ts_err("self raw limit write fail.\n");
		goto save_end;
	}
	bytes = 0;
	gtx8_save_limits_data(ts_test, fp, &ts_test->test_params->self_max_limits[0],
			ts_test->self_rawdata.size, "MaxRawLimit");
	gtx8_save_limits_data(ts_test, fp, &ts_test->test_params->self_min_limits[0],
			ts_test->self_rawdata.size, "MinRawLimit");
	bytes += (s32) sprintf((char *)&data[bytes], "</Item>\n");
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
		ts_err("self Item write fail.\n");
		goto save_end;
	}
	bytes = 0;
#endif
#ifdef SELFNOISE_DATA_TEST
	/* self diffdata limit */
	bytes += (int) sprintf((char *)&data[bytes], "<Item name=\"Self Diffdata Test Sets\">\n");
	bytes += (int) sprintf((char *)&data[bytes], "<TotalFrameCnt>1</TotalFrameCnt>\n");
	bytes += (int) sprintf((char *)&data[bytes], "<MaxJitterLimit>%d</MaxJitterLimit>\n",
...
/* save data content */
static s32 gtx8_save_datacont(struct gtx8_ts_test *ts_test, struct file *fp, u16 *src_data,
		u8 *title, int len, u8 frame_num)
{
	s32 ret = 0;
	u16 i = 0;
	s32 bytes = 0;
	...
	gtx8_data_cal(src_data, len, stat_result);
	bytes += (s32) sprintf((char *)&data[bytes],
			"<%s No.=\"%d\" DataCount=\"%d\" Maximum=\"%d\" Minimum=\"%d\" Average=\"%d\">\n",
			title, frame_num, len, stat_result[1], stat_result[2], stat_result[0]);
...
static s32 gtx8_save_samprawdata(struct gtx8_ts_test *ts_test, struct file *fp, u16 *src_data,
		u8 *title, int len)
{
	s32 ret = 0;
	u16 i = 0;
	...
	for (i = 0; i < frame_num; i++) {
		ret = gtx8_save_datacont(ts_test, fp, ts_test->rawdata_arr[i].data,
				"DataContent", ts_test->rawdata_arr[i].size, i);
		if (ret < 0)
			goto save_end;
		/* save accord */
		ret = gtx8_save_datacont(ts_test, fp, ts_test->accord_arr[i].data,
				"RawAccord", ts_test->accord_arr[i].size, i);
		if (ret < 0)
			goto save_end;
	}
	bytes += (s32) sprintf((char *)&data[bytes], "</%s>\n", title);
...
static s32 gtx8_save_sampdata(struct gtx8_ts_test *ts_test, struct file *fp, u16 *src_data,
		u8 *title, int len)
{
	s32 ret = 0;
	s32 bytes = 0;
	...
	ret = gtx8_save_datacont(ts_test, fp, src_data, "DataContent", len, 0);
	if (ret < 0)
		goto save_end;
...
/* save rawdiff data in csv file */
static int gtx8_save_rawdiff(struct gtx8_ts_test *ts_test, struct file *fp)
{
	int ret = 0;
	s32 bytes = 0;
	char *data;
	...
	bytes += (s32) sprintf((char *)&data[bytes], "<DataRecord>\n");
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
	...
	ret = gtx8_save_samprawdata(ts_test, fp, &ts_test->rawdata.data[0],
			"RawDataRecord", ts_test->rawdata.size);
	if (ret < 0)
		goto save_end;
#ifdef NOISE_DATA_TEST
	ret = gtx8_save_sampdata(ts_test, fp, &ts_test->noisedata.data[0],
			"DiffDataRecord", ts_test->noisedata.size);
	if (ret < 0)
		goto save_end;
#endif
#ifdef SELF_RAWDATA_TEST
	ret = gtx8_save_sampdata(ts_test, fp, &ts_test->self_rawdata.data[0],
			"selfDataRecord", ts_test->self_rawdata.size);
	if (ret < 0)
		goto save_end;
#endif
#ifdef SELFNOISE_DATA_TEST
	ret = gtx8_save_sampdata(ts_test, fp, &ts_test->self_noisedata.data[0],
...
/* save result and rawdiff data in csv file */
static int gtx8_save_result_data(struct gtx8_ts_test *ts_test)
{
	int ret = 0;
	...
	/* format result file */
	FORMAT_PATH(save_path, GTX8_RESULT_SAVE_PATH, "Test_Data", ".csv");

	fp = filp_open(save_path, O_CREAT|O_TRUNC|O_WRONLY, 0666);
	if (IS_ERR(fp)) {
		ts_err("open file:%s fail,fp:%ld", save_path, PTR_ERR(fp));
		return -1;
	}
	/* save header */
	ret = gtx8_save_header(ts_test, fp);
	if (ret < 0)
		goto save_end;

	/* save limits */
	ret = gtx8_save_limits(ts_test, fp);
	if (ret < 0)
		goto save_end;

	/* save data */
	ret = gtx8_save_rawdiff(ts_test, fp);
	if (ret < 0)
		goto save_end;
	/* save tail */
	ret = gtx8_save_tail(ts_test, fp);
	if (ret < 0)
		goto save_end;

	ts_info("the test result saves in %s\n", save_path);
save_end:
	filp_close(fp, NULL);
	return ret;
}
 ...
 */
int parse_csvfile(struct goodix_ts_device *ts_dev, char *target_name, int32_t  *data, int rows, int columns)
{
	int ret = 0;
	int i = 0;
	int32_t read_ret = 0;
	...
	sprintf(limit_file, "%s_test_sensor_%d.csv", GTX8_TEST_FILE_NAME, ts_dev->chip_version.sensor_id);
	ts_info("%s, file name is %s, target is %s.\n", __func__, limit_file, target_name);
	for (i = 0; i < 3; i++) {
		ret = request_firmware(&firmware, limit_file, dev);
		if (ret < 0) {
			ts_err("limits file [%s] not available,error:%d, try_times:%d", limit_file, ret, i + 1);
			msleep(1000);
		} else {
			ts_info("limits file  [%s] is ready, try_times:%d", limit_file, i + 1);
			break;
		}
	}
	if (i >= 3) {
...
*******************************************************************************/
static int gtx8_get_cfg_value(struct goodix_ts_core *core_data, u8 *config, u8 *buf, u8 len, u8 sub_bag_num, u8 offset)
{
	u8 *sub_bag_ptr = NULL;
	u8 i = 0;
	u8 chksum_len = 1;
...
static int gtx8_init_testlimits(struct gtx8_ts_test *ts_test)
{
	...
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_UNIFIED_LIMIT, data_buf, 1, 3);
	if (ret < 0) {
...
#ifdef NOISE_DATA_TEST
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_NOISE_LIMIT, data_buf, 1, 1);
	if (ret < 0) {
...
#endif
#ifdef SELFNOISE_DATA_TEST
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_SELFNOISE_LIMIT, data_buf, 1, 1);
	if (ret < 0) {
	...
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_SELF_UNIFIED_LIMIT, data_buf, 1, 2);
	if (ret < 0) {
	...
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_SPECIAL_SELFRAW_MAX, test_params->self_max_limits,
	...
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_SPECIAL_SELFRAW_MIN, test_params->self_min_limits,
			1, test_params->drv_num + test_params->sen_num);
	if (ret < 0) {
	...
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_SHORT_THRESHOLD, data_buf, 1, 7);
	if (ret < 0) {
	...
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_SPECIAL_RAW_MAX, test_params->max_limits,
	...
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_SPECIAL_RAW_MIN, test_params->min_limits,
			test_params->sen_num, test_params->drv_num);
	if (ret < 0) {
	...
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_SPECIAL_RAW_DELTA, test_params->deviation_limits,
			test_params->sen_num, test_params->drv_num);
	if (ret < 0) {
	...
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_TEST_CONFIG, tmp_config,
...
#if ((defined NOISE_DATA_TEST) || (defined SELFNOISE_DATA_TEST))
	ret = parse_csvfile(core_data->ts_dev, CSV_TP_NOISE_CONFIG, tmp_config,
			1, GOODIX_CFG_MAX_SIZE);
	if (ret < 0) {
		...
		if (core_data->ts_dev->ic_type == IC_TYPE_YELLOWSTONE) {
			chksum = ts_test->noise_config->data[0] + ts_test->noise_config->data[1] +
...
#ifdef SELF_RAWDATA_TEST
	if (core_data->ts_dev->ic_type == IC_TYPE_YELLOWSTONE)
		goodix_cmd_init(core_data->ts_dev, &ts_test->selfdata_cmd,
			GTX8_CMD_SELFDATA, 0, core_data->ts_dev->reg.command);
#endif
...
/* gtx8_dump_data_prepare
 *
 * preparation before dump data
 */
static int gtx8_dump_data_prepare(struct gtx8_ts_test *ts_test)
{
	int ret = 0;
	struct goodix_ts_core *core_data;
	core_data = (struct goodix_ts_core *)ts_test->ts;
...
static void gtx8_analyse_diffdata(struct gtx8_ts_test *ts_test)
{
#ifdef NOISE_DATA_TEST
	int i = 0;
	/*u32 find_bad_node = 0;*/
	...
	for (i = 0; i < ts_test->noisedata.size; i++) {
		noise_value = (s16)be16_to_cpu(ts_test->noisedata.data[i]);
		ts_test->diffdata.data[i] = noise_value;
	}

	rxtx_revert(&ts_test->diffdata.data[0], ts_test->test_params->drv_num, ts_test->test_params->sen_num);
#endif
	return;
}
		...
		int frame_cnt)
{
	...
	ts_test->accord_arr[frame_cnt].size = sc_data_num;
	...
	u8 *p_beyond_min_cnt, u8 *p_beyond_accord_cnt, u8 frame_cnt)
{
	...
	for (i = 0; i < rawdata->size; i++) {
		...
		if (p_beyond_min_cnt[i]*10/frame_cnt >= high_ratio) {
			ts_err("min_cnt:%d,rawdata[%d][%d]:%u < min_limit:%u, NG\n", p_beyond_min_cnt[i],
				(u32)div_s64(i, test_params->drv_num), i % test_params->drv_num,
	...
	for (i = 0; i < test_params->drv_num * test_params->sen_num; i++) {
		...
		if (p_beyond_accord_cnt[i]*10 >= high_ratio) {
			ts_err("beyond accord cnt:%d,rawdata[%d][%d]:%u, deviation_limit:%u, NG\n",
				p_beyond_accord_cnt[i],
				(u32)div_s64(i, test_params->drv_num), i % test_params->drv_num,
...
static void gtx8_rawdata_test_spec(struct gtx8_ts_test *ts_test)
{
	...
	u8 frame_num = 16;
	u16 data_len = 0;
	u8* p_beyond_max_limit_cnt = ts_test->open_res->beyond_max_limit_cnt;
	u8* p_beyond_min_limit_cnt = ts_test->open_res->beyond_min_limit_cnt;
	u8* p_beyond_accord_limit_cnt = ts_test->open_res->beyond_accord_limit_cnt;
	...
	for (i = 0; i < TOTAL_FRAME_NUM; i++) {
		...
		/* bak rawdata */
		memcpy(ts_test->rawdata_arr[i].data, ts_test->rawdata.data,
				ts_test->rawdata.size*sizeof(u16));
		ts_test->rawdata_arr[i].size = ts_test->rawdata.size;

		gtx8_rawcapacitance_test_spec(ts_test, p_beyond_max_limit_cnt,
				p_beyond_min_limit_cnt);
		ret = gtx8_accord_test_spec(ts_test, p_beyond_accord_limit_cnt, i);
		if (ret) {
			ts_test->test_result[GTP_DELTA_TEST] = GTP_PANEL_REASON;
...
static int gtx8_rawcapacitance_test(struct gtx8_ts_test *ts_test,
		struct ts_test_params *test_params)
{
	int i = 0;
	int ret = NO_ERR;
	struct ts_test_rawdata *rawdata = ts_test->rawdata;
	u8* p_beyond_max_limit_cnt = ts_test->open_res->beyond_max_limit_cnt;
	u8* p_beyond_min_limit_cnt = ts_test->open_res->beyond_min_limit_cnt

	memset(p_beyond_max_limit_cnt, 0, rawdata->size);
	memset(p_beyond_min_limit_cnt, 0, rawdata->size);
	for (i = 0; i < rawdata->size; i++) {
		if (rawdata->data[i] > test_params->max_limits[i]) {
			p_beyond_max_limit_cnt[i]++;
...
static int gtx8_deltacapacitance_test(struct gtx8_ts_test *ts_test,
		struct ts_test_params *test_params)
{
	int i = 0;
	int ret = NO_ERR;
	...
	memset(p_beyond_accord_limit_cnt, 0, rawdata->size);
	cols = test_params->drv_num;
	sc_data_num = test_params->drv_num * test_params->sen_num;
	if (cols <= 0) {
		ts_err("%s: parmas invalid\n", __func__);
	...
	ts_test->accord_arr[0].size = sc_data_num;
		...
		if (rawdata_val) {
			...
			if (max_val > test_params->deviation_limits[i]) {
				...
				ts_err("deviation[%d][%d]:%u > delta_limit:%u, NG\n",
					...
					i % test_params->drv_num, max_val,
					test_params->deviation_limits[i]);
				ret = RESULT_ERR;
			}
			/* bak accord */
			ts_test->accord_arr[0].data[i] = max_val;
		} else {
			ts_err("Find rawdata=0 when calculate deltacapacitance:[%d][%d]\n",
				(u32)div_s64(i, cols), i % cols);
			ret = RESULT_ERR;
			ts_test->accord_arr[0].data[i] = 0;
		}
	}
...
static void gtx8_rawdata_test(struct gtx8_ts_test *ts_test)
{
	...
	if (ret < 0) {
		...
		/* bak rawdata */
		memcpy(ts_test->rawdata_arr[0].data, ts_test->rawdata.data,
				ts_test->rawdata.size*sizeof(u16));
		ts_test->rawdata_arr[0].size = ts_test->rawdata.size;

		ret = gtx8_rawcapacitance_test(ts_test, ts_test->test_params);
		if (!ret) {
			ts_test->test_result[GTP_CAP_TEST] = GTP_TEST_PASS;
		...
		ret = gtx8_deltacapacitance_test(ts_test, ts_test->test_params);
		if (!ret)  {
			ts_test->test_result[GTP_DELTA_TEST] = GTP_TEST_PASS;
...
static void gtx8_capacitance_test(struct gtx8_ts_test *ts_test)
{
	...
	if (core_data->ts_dev->ic_type == IC_TYPE_YELLOWSTONE) {
		...
		/* enter selfraw data test mode */
		core_data->ts_dev->hw_ops->write_trans(core_data->ts_dev,
				ts_test->selfdata_cmd.cmd_reg, ts_test->selfdata_cmd.cmds,
				ts_test->selfdata_cmd.length);
		msleep(50);
	}
...
int alloc_mem_for_test(struct gtx8_ts_test *gts_test)
{
	int ret = 0;
	gts_test->test_config = NULL;
	gts_test->orig_config = NULL;
...
int gtx8_get_rawdata(void *tsdev, struct ts_rawdata_info *info)
{
	...
	ts_info("tp test code version:%s\n", TEST_CODE_VERSION);
	...
	if (!gts_test) {
		ts_err("%s: Failed to alloc mem size:%ld\n", __func__,
			sizeof(struct gtx8_ts_test));
		return -ENOMEM;
	}
	ts_info("%s: alloc mem for struct gtx8_ts_test size:%ld, addr:%p\n",
		__func__, sizeof(struct gtx8_ts_test), gts_test);
	gts_test->ts = core_data;
	ret = alloc_mem_for_test(gts_test);
	if (ret) {
		goto exit_finish;
	}
...
int gtx8_dump_data(void *tsdev, char *buf, int *buf_size)
{
	int ret = 0;
	int offset = 0;
	int r = 0, i = 0, j = 0;
	...
	ret = core_data->ts_dev->hw_ops->send_cmd(core_data->ts_dev, &gts_test->rawdata_cmd);
	msleep(50);
	ret = gtx8_cache_rawdata(gts_test);

	ret = gtx8_noisetest_prepare(gts_test);
	ret = gtx8_cache_noisedata(gts_test);
	gtx8_analyse_diffdata(gts_test);

	core_data->ts_dev->hw_ops->write_trans(core_data->ts_dev,
			gts_test->normal_cmd.cmd_reg, gts_test->normal_cmd.cmds,
			gts_test->normal_cmd.length);
	cmd[0] = 0x00;
	core_data->ts_dev->hw_ops->write_trans(core_data->ts_dev, core_data->ts_dev->reg.coor, cmd, 1);
	...
	/*end print diff data */
	r = snprintf(&buf[offset], 20, "Diffdata:\n");
	offset += r;
	ts_info("[%s]r = %d\n", __func__, r);
	for (i = 0; i < gts_test->test_params->sen_num; i++) {
		for (j = 0; j < gts_test->test_params->drv_num; j++) {
			data_tmp = (s16) gts_test->diffdata.data[i * gts_test->test_params->drv_num + j];
			r = snprintf(&buf[offset], 10, "%4d,", data_tmp);
			offset += r;
		}
		r = snprintf(&buf[offset], 2, "\n");
...
static void gtx8_data_statistics(u16 *data, size_t data_size, char *result, size_t res_size)
{
	...
	gtx8_data_cal(data, data_size, stat_value);
...
static int gtx8_short_test_prepare(struct gtx8_ts_test *ts_test)
{
	...
	core_data = (struct goodix_ts_core *)ts_test->ts;
	goodix_cmd_init(core_data->ts_dev, &short_code_cmd,
			0x0b, 0, core_data->ts_dev->reg.command);
...
static void gtx8_save_short_res(struct gtx8_ts_test *ts_test, u16 chn1, u16 chn2,
		int r)
{
	int i = 0;
	u8 repeat_cnt = 0;
	...
	for (i = 0; i < short_res->short_num; i++) {
		repeat_cnt = 0;
		if (short_res->short_msg[4*i+0] == chn1)
			repeat_cnt++;
		if (short_res->short_msg[4*i+0] == chn2)
			repeat_cnt++;
...
static int gtx8_check_resistance_to_gnd(struct gtx8_ts_test *ts_test,
				u16 adc_signal, u32 pos)
{
	...
	if (r < r_th) {
		...
		gtx8_save_short_res(ts_test, pin_num,
			...
			struct short_record *r_data, u16 self_capdata, u8 flag)
{
	...
	if (core_data->ts_dev->ic_type == IC_TYPE_YELLOWSTONE) {
		if (r_data->group1 != r_data->group2) {	/* different Group */
			ret = core_data->ts_dev->hw_ops->read_trans(core_data->ts_dev,
					YS_IC_MASK_TYPE_REG, &tmp_data, 1);  /*  */
			if (ret < 0) {
	...
	if (short_flag & 0x08) {
		...
		* resistance between channel and GND */
		...
		for (i = 0; i < size; i += 2) {
			...
			ret = gtx8_check_resistance_to_gnd(ts_test,
		...
		for (j = short_die_num + 1; j < MAX_DRV_NUM; j++) {
			...
			if (adc_signal > test_params->short_threshold) {
				...
				if (short_r < r_threshold) {
					...
					gtx8_save_short_res(ts_test, master_pin_num, slave_pin_num, short_r);
...
static void gtx8_shortcircut_test(struct gtx8_ts_test *ts_test)
{
	...
	for (i = 0; i < 110; i++) {
		...
		ret = core_data->ts_dev->hw_ops->read_trans(core_data->ts_dev,
				SHORT_TESTEND_REG, data, 1); /* SHORT_TESTEND_REG   0x8400 */
		if (ret) {
...
static u8 get_chn_group_ys(u8 chn)
{
	...
	else if ((chn >= 20) && (chn < 24)) /* (Setting3) C(bit00) */
