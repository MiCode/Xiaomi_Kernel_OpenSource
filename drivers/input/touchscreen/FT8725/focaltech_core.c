...
/* N19 code for HQ-354795 by liaoxianguo at 2024/1/24 start */
int fts_gestures_status = 0;
/* N19 code for HQ-354795 by liaoxianguo at 2024/1/24 end */
...
int fts_proximity_readdata(struct fts_ts_data *ts_data)
{
    ...
    if ((regvalue > 0x00) && (regvalue < 0x04)) {
        /* close. need lcd off */
        buf_value = 1;
        ...
        proximity_status = PS_FAR_AWAY;
        ret = -1;
        FTS_ERROR("Error Value. fts_proximity_data.detect is %d,Reg.0xB5 regvalue = %d", fts_proximity_data.detect,regvalue);
    }
    ...
    if (regvalue != last_proximity_value) {
        FTS_INFO("[PROXIMITY] p-sensor state:%s", proximity_status ? "AWAY" : "NEAR");
        FTS_INFO("last proximity value is %d", last_proximity_value) ;
        FTS_INFO("Current proximity value is %d", regvalue) ;
        last_proximity_value = regvalue;
        fts_proximity_data.detect = proximity_status ? PS_FAR_AWAY : PS_NEAR;
		fts_write_reg(FTS_REG_IDE_PARA_STATUS, buf_value);
  	}
  	return ret;
}
...
#endif
#if FTS_PSENSOR_ENABLE
	if (fts_proximity_readdata(fts_data) > 0 && ts_data->suspended == true)
        return IRQ_HANDLED;//Don't report points  when TP is suspended and detects proximity.
#endif
...
/* N19 code for HQ-354491 by p-luozhibin1 at 2023.1.22 start */
static int fts_charger_flag = 0;

int fts_charger_mode_set(void)
{
    u8 m_val = 0;
    int ret = 0;
...
{
    if(fts_charger_mode_set() < 0)
        FTS_ERROR("fts_charger_mode_work err\n");
}
                                ...
                                unsigned long event, void *data)
{
	...
	FTS_INFO("fts_detect_charger_notifier_callback start\n");

	fts_charger_flag = evdata->usb_touch_enable;
	ret = queue_work(ts_data->fts_charger_detect_workqueue, &ts_data->fts_charger_detect_work);
	FTS_INFO("fts_detect_charger_notifier_callback end\n");
	return 0;
}
/* N19 code for HQ-354491 by p-luozhibin1 at 2023.1.22 end */
/* N19 code for HQ-357608 by liaoxianguo at 2023/12/29 start */
...
static int fts_xiaomi_panel_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (ts) {
		...
		} else if (event == XIAOMI_PANEL_EVENT_BLANK) {
...
static void get_lockdown_info(struct work_struct *work)
{
    ...
    struct fts_ts_data *ts_data = fts_data;
    FTS_INFO("lkdown_readed = %d", ts_data->lkdown_readed);
    lockdown_ptr = qcom_smem_get(QCOM_SMEM_HOST_ANY, SMEM_SW_DISPLAY_LOCKDOWN_TABLE, &item_size);
    if (!IS_ERR(lockdown_ptr) && item_size > 0) {
        memcpy(ts_data->lockdown_info, lockdown_ptr, item_size);
        FTS_INFO("Lockdown:0x%02x,0x%02x\n",
        ts_data->lockdown_info[0], ts_data->lockdown_info[1]);
        ts_data->lkdown_readed = true;
        FTS_INFO("READ LOCKDOWN!!!");
    } else {
...
/* N19 code for HQ-354491 by p-luozhibin1 at 2023.1.22 start */
    ...
    if(!ts_data->fts_charger_detect_workqueue) {
        FTS_ERROR("create charger workqueue fail");
        goto err_create_fts_charger_detect_workqueue;
    }
    if(ts_data->fts_charger_detect_workqueue) {
	...
	if (!ts_data->fts_headset_workqueue) {
        ...
        goto err_create_fts_headset_workqueue;
    }
    if (ts_data->fts_headset_workqueue) {
...
#endif
/* N19 code for HQ-351947 by jiyongfei at 2023/3/4 start */
    ts_data->lkdown_readed = false;
    ts_data->fts_lockdown_wq = alloc_workqueue("fts_lockdown_wq", WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
    if (!ts_data->fts_lockdown_wq) {
        FTS_ERROR("fts_lockdown_wq create workqueue failed\n");
        ret = -ENOMEM;
    } else {
        FTS_INFO("fts_lockdown_wq create workqueue successful!\n");
    }
    INIT_DELAYED_WORK(&ts_data->fts_lockdown_work, get_lockdown_info);
    // please make sure boot update start after display reset(RESX) sequence
    queue_delayed_work(ts_data->fts_lockdown_wq, &ts_data->fts_lockdown_work, msecs_to_jiffies(5000));
/* N19 code for HQ-351947 by jiyongfei at 2023/3/4 end */
...
err_create_fts_headset_workqueue:
    if (ts_data->fts_headset_workqueue)
        destroy_workqueue(ts_data->fts_headset_workqueue);
err_create_fts_charger_detect_workqueue:
    if (ts_data->fts_charger_detect_workqueue)
        destroy_workqueue(ts_data->fts_charger_detect_workqueue);
    FTS_FUNC_EXIT();
    return ret;
}
...
/* N19 code for HQ-351947 by jiyongfei at 2023/3/4 start */
    if (ts_data->fts_lockdown_wq) {
        cancel_delayed_work_sync(&ts_data->fts_lockdown_work);
        destroy_workqueue(ts_data->fts_lockdown_wq);
        ts_data->fts_lockdown_wq = NULL;
    }
/* N19 code for HQ-351947 by jiyongfei at 2023/3/4 end */
    FTS_FUNC_EXIT();
    return 0;
