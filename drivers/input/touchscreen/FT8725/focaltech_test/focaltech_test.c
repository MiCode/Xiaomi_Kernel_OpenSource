...
static u8 ito_test_status = 0;
static struct proc_dir_entry *fts_ito_test_proc = NULL;
static struct proc_dir_entry *fts_android_touch_proc = NULL;
...
static struct lct_tp_data_dump *lct_tp_data_dump_p;
#endif
...
void print_buffer(int *buffer, int length, int line_num)
{
    ...
    tmplen = tmpline * 6 + MAX_SHOW;
    tmpbuf = kzalloc(tmplen, GFP_KERNEL);

    for (i = 0; i < length; i = i + tmpline) {
        cnt = 0;
        for (j = 0; j < tmpline; j++) {
            cnt += snprintf(tmpbuf + cnt, tmplen - cnt, "%d,", buffer[i + j]);
...
void show_buffer(int *buffer, int length, int line_num)
{
    ...
    int line_size = 0;
    int cnt = 0;
    char line_buf[MAX_SHOW] = { 0 };

    if (buffer && length) {
        line_size = line_num ? line_num : length;
        do {
            cnt += snprintf(line_buf + cnt, MAX_SHOW - cnt, "%d,", buffer[i++]);
            if (((i % line_size) == 0) || (i == length) || (cnt >= MAX_SHOW)) {
                FTS_TEST_DBG("%s", line_buf);
                cnt = 0;
                memset(line_buf, 0, MAX_SHOW);
            }
        } while (i < length);
    }
}
...
static int fts_test_main_init(void)
{
    ...
    tdata->result = TEST_RESULT_IN_PROGRESS;
    FTS_TEST_DBG("FTS_INI_FILE_PATH:%s", FTS_INI_FILE_PATH);
#ifdef LINUX_VERSION_CODE
    FTS_TEST_DBG("Linux Version:%d", LINUX_VERSION_CODE);
#endif
    ...
    tdata->csv_item_cnt = 0;
    tdata->csv_item_scb = 0;
    tdata->csv_item_sraw = 0;
...
{
    char test_result_bmp[5] = {0};
    //SPI_COMMUNICATION result
    if ((fts_ftest->test_item[FTS_ENTER_FACTORY_MODE].testresult) == RESULT_PASS)
        test_result_bmp[SPI_COMMUNICATION] = 'P';
    else {
        seq_printf(file, "0F-1F-2F-3F-4F\n");
        return 0;
    }
    //Rawdata result
    if ((fts_ftest->test_item[FTS_RAWDATA_TEST].testresult == RESULT_PASS)
         && (fts_ftest->test_item[FTS_CB_TEST].testresult == RESULT_PASS))
        test_result_bmp[RAWDATA] = 'P';
    else
        test_result_bmp[RAWDATA] = 'F';
    //open test result
    if (fts_ftest->test_item[FTS_OPEN_TEST].testresult == RESULT_PASS)
        test_result_bmp[OPEN_DATA] = 'P';
    else
        test_result_bmp[OPEN_DATA] = 'F';
    //short test result
    if (fts_ftest->test_item[FTS_SHORT_CIRCUIT_TEST].testresult == RESULT_PASS)
        test_result_bmp[SHORT_DATA] = 'P';
    else
        test_result_bmp[SHORT_DATA] = 'F';
    //noise test result
    if (fts_ftest->test_item[FTS_NOISE_TEST].testresult == RESULT_PASS)
        test_result_bmp[NOISE_DATA] = 'P';
    else
        test_result_bmp[NOISE_DATA] = 'F';
/* N19 code for HQ-355082 by liaoxianguo at 2024/1/15 start */
    seq_printf(file, "%s\n", fts_ftest->csv_file_buf);
    seq_printf(file, "0%c-1%c-2%c-3%c-4%c\n",
        test_result_bmp[SPI_COMMUNICATION],
        test_result_bmp[RAWDATA],
        test_result_bmp[OPEN_DATA],
        test_result_bmp[SHORT_DATA],
        test_result_bmp[NOISE_DATA]);
/* N19 code for HQ-355082 by liaoxianguo at 2024/1/15 end */
    return 0;
}
static int fts_ito_test_open(struct inode *inode, struct file *file)
{
    char fwname[128] = {0};
    struct fts_ts_data *ts_data = fts_ftest->ts_data;
    struct input_dev *input_dev = ts_data->input_dev;
    int i = 0;
    int ret = 0;
    memset(fwname, 0, sizeof(fwname));
    sprintf(fwname, "focaltech_mp_fw_huaxin.ini");
    FTS_TEST_DBG("fwname:%s.", fwname);
    for (i = FTS_ENTER_FACTORY_MODE ; i <= FTS_SHORT_CIRCUIT_TEST;  i++)
        fts_ftest->test_item[i].testresult = RESULT_NULL;
...
const struct proc_ops fts_ito_test_ops = {
    .proc_open = fts_ito_test_open,
    .proc_read = seq_read,
    .proc_lseek = seq_lseek,
    .proc_release = single_release,
};
//-------------------/proc/android_touch/self_test end-----------------
/* N19 code for HQ-354836 by liaoxianguo at 2023/12/21 end */
...
static int fts_csv_show(struct seq_file *s, void *v)
{
    ...
    int csv_size = (int)strlen(tdata->csv_file_buf);
...
static int lct_tp_get_diffdata(int *data, int byte_num)
{
	...
	/* read diffdata */
	ret = read_rawdata(tdata, 0x01, 0xAD, FACTORY_REG_RAWDATA_ADDR, byte_num, data);
	if (ret < 0) {
...
static int tp_data_dump_proc_show(struct seq_file *file, void *data)
{
	...
	for (row = 0; row < tx_num; row++) {
		for (col = 0; col < rx_num; col++) {
			seq_printf(file, "%5d", rawdata[row * rx_num + col]);
		}
		seq_printf(file, "\n");
	}
	seq_printf(file, "\nDIFF DATA\n");
	for (row = 0; row < tx_num; row++) {
		for (col = 0; col < rx_num; col++) {
			seq_printf(file, "%5d", diffdata[row * rx_num + col]);
		}
		seq_printf(file, "\n");
	}
...
static int tp_data_dump_proc_open(struct inode *inode, struct file *file)
{
	...
	ret = fts_enter_test_environment(1);
	if (ret < 0) {
		FTS_TEST_ERROR("enter test environment fail");
...
int fts_tp_data_dump_proc_init(void)
{
	int ret = 0;
...
void fts_tp_data_dump_proc_exit(void)
{
	if (IS_ERR_OR_NULL(lct_tp_data_dump_p))
		return;
	if (!IS_ERR_OR_NULL(lct_tp_data_dump_p->tp_data_dump_proc)) {
...
/* N19 code for HQ-354836 by liaoxianguo at 2023/12/21 start */
    ...
    fts_ito_test_proc = proc_create(FTS_ITO_TEST, 0664,
                        fts_android_touch_proc, &fts_ito_test_ops);
