...
/* N19 code for HQ-351885 by p-luozhibin1 at 2023.1.17 start */
#define FTS_USB_VERTICAL 1
#define FTS_USB_LEFT     2
#define FTS_USB_RIGHT    3
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '2' || buf[0] == '3') {
		...
		if (buf[0] == '3') /*USB PORTS RIGHT*/ {
			ret = fts_ex_mode_switch(MODE_EDGE, FTS_USB_RIGHT);
			if (ret >= 0) {
				ts_data->edge_mode = ENABLE;
				edge_mode_data = FTS_USB_RIGHT;
			}
		} else if (buf[0] == '2') /*USB PORTS LEFT*/ {
			ret = fts_ex_mode_switch(MODE_EDGE, FTS_USB_LEFT);
			if (ret >= 0) {
				ts_data->edge_mode = ENABLE;
				edge_mode_data = FTS_USB_LEFT;
			}
		}
	} else if (buf[0] == '1') {
		FTS_DEBUG("exit edge mode");
		ret = fts_ex_mode_switch(MODE_EDGE, FTS_USB_VERTICAL);
		if (ret >= 0) {
			ts_data->edge_mode = DISABLE;
			edge_mode_data = FTS_USB_VERTICAL;
		}
	}
