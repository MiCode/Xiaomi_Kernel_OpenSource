...
/* N19 code for HQ-369514 by liaoxianguo at 2024/2/4 start */
#define FTS_IRQ                                 "fts_irq"
/* N19 code for HQ-369514 by liaoxianguo at 2024/2/4 end */
...
/* N19 code for HQ-351947 by liuyupei at 2024/1/12 start */
struct proc_dir_entry *fts_proc_tp_lockdown_info_entry = NULL;
/* N19 code for HQ-351947 by liuyupei at 2024/1/12 end */
/* N19 code for HQ-369514 by liaoxianguo at 2024/2/4 start */
...
int fts_factory_test(void)
{
    ...
    if (ret < 0) {
        ...
        strcpy(fwname, "focaltech_mp_fw_huaxin.ini");
        FTS_INFO("fwname:%s.\n", fwname);
...
{
	if(fts_data->lkdown_readed) {
		seq_printf(m, "%02x%02x%02x%02x%02x%02x%02x%02x\n",
			fts_data->lockdown_info[0], fts_data->lockdown_info[1], fts_data->lockdown_info[2], fts_data->lockdown_info[3],
			fts_data->lockdown_info[4], fts_data->lockdown_info[5], fts_data->lockdown_info[6], fts_data->lockdown_info[7]);
	} else {
		FTS_ERROR("lockdown info is not ready yet !!!");
		seq_printf(m, "s\n","1234567890000000");
	}
	return 0;
}
...
/* N19 code for HQ-351947 by liuyupei at 2024/1/12 start */
	fts_proc_tp_lockdown_info_entry = proc_create(PROC_CTP_LOCKDOWN_INFO_FILE, (S_IWUSR | S_IRUGO), NULL, &fts_proc_tp_lockdown_info_fops);
	if (fts_proc_tp_lockdown_info_entry == NULL) {
		...
		printk ("%s:proc_create fts_proc_tp_info_entry success\n",__func__);
	}
/* N19 code for HQ-351947 by liuyupei at 2024/1/12 end */
/* N19 code for HQ-369514 by liaoxianguo at 2024/2/4 start */
	fts_proc_irq_entry = proc_create(FTS_IRQ, (S_IWUSR | S_IRUGO), NULL,&fts_irq_fops);
	if (fts_proc_irq_entry == NULL) {
		printk (KERN_ERR "%s: fts_proc_irq_entry create failed!\n", __func__);
		remove_proc_entry(FTS_IRQ, NULL);
	} else {
		printk ("%s:proc_create fts_proc_irq_entry success\n",__func__);
	}
/* N19 code for HQ-369514 by liaoxianguo at 2024/2/4 end */
/* N19 code for HQ-351846 by jiyongfei at 2023/3/4 start */
...
/* N19 code for HQ-351947 by liuyupei at 2024/1/12 start */
	if (fts_proc_tp_lockdown_info_entry != NULL) {
		remove_proc_entry(PROC_CTP_LOCKDOWN_INFO_FILE, NULL);
		fts_proc_tp_lockdown_info_entry = NULL;
