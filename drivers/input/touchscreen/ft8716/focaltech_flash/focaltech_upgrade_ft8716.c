...
#if 0
	/*check fw ver in tp & .i   file to decided to upgrade or not*/
  for (i = 0; i < 2; i++) {
	i_ret = fts_i2c_read_reg (client, 0xa6,  &tp_fw_ver);
	if (i_ret < 0) {
		printk (" read tp fw ver err , i=%d\n", i);
	...
	if (i >= 2)
	 ...
	 i_file_fw_ver = fts_ctpm_get_app_i_file_ver ();

	 if (i_file_fw_ver < tp_fw_ver) {
		return -EIO;
		}
