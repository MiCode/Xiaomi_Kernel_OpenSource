...
int FT8716_get_test_data_sharp(char *pTestData);
	...
	for (iItemCount = 0; iItemCount < g_TestItemNum_sharp; iItemCount++) {
		...
		if (Code_FT8716_ENTER_FACTORY_MODE == g_stTestItem_sharp[ucDevice][iItemCount].ItemCode
			...
			if (ERROR_CODE_OK != ReCode || (!bTempResult)) {
				bTestResult = false;
				g_stTestItem_sharp[ucDevice][iItemCount].TestResult = RESULT_NG;
				break;
			} else
...
unsigned char FT8716_TestItem_EnterFactoryMode(void)
{
	...
	if (ReCode == ERROR_CODE_OK) {
		...
		ReCode = ReadReg_sharp(0xFC, &keyFit);
		SetKeyBitVal(keyFit);
	}
	return ReCode;
}
...
unsigned char FT8716_TestItem_RawDataTest(bool *bTestResult)
{
	...
	if (IsKeyAutoFit()) {
		for (iCol = 0; iCol < g_stSCapConfEx_sharp.KeyNum; iCol++)
			FTS_TEST_DBG("%5d, ",  m_RawData[g_stSCapConfEx_sharp.ChannelXNum][iCol]);
	} else {
...
unsigned char FT8716_TestItem_CbTest(bool *bTestResult)
{
	...
	int ReadKeyLen = g_stSCapConfEx_sharp.KeyNumTotal;
	...
	ReCode = ReadReg_sharp(0x0B, &ucBits);
	if (ERROR_CODE_OK != ReCode) {
		btmpresult = false;
		FTS_TEST_ERROR("\r\n//=========  Read Reg Failed!");
	}

	ReadKeyLen = g_stSCapConfEx_sharp.KeyNumTotal;
	if (ucBits != 0) {
		ReadKeyLen = g_stSCapConfEx_sharp.KeyNumTotal * 2;
	}



	ReCode = GetTxRxCB(0, (short)(g_stSCapConfEx_sharp.ChannelXNum * g_stSCapConfEx_sharp.ChannelYNum + ReadKeyLen), m_ucTempData);
	if (ERROR_CODE_OK != ReCode) {
		btmpresult = false;
	...
	if (IsKeyAutoFit()) {
		for (iCol = 0; iCol < g_stSCapConfEx_sharp.KeyNum; iCol++) {
			FTS_TEST_DBG("%3d, ",  m_CBData[g_stSCapConfEx_sharp.ChannelXNum][iCol]);
...
static unsigned char ChipClb(unsigned char *pClbResult)
{
	...
	ReCode = WriteReg_sharp(REG_CLB, 4);

	if (ReCode == ERROR_CODE_OK) {
		while (TimeOutTimes--) {
			SysDelay_sharp(100);
			ReCode = WriteReg_sharp(DEVIDE_MODE_ADDR, 0x04<<4);
			ReCode = ReadReg_sharp(0x04, &RegData);
			if (ReCode == ERROR_CODE_OK) {
				if (RegData == 0x02) {
					*pClbResult = 1;
					break;
...
/************************************************************************
* Name: FT8716_get_test_data_sharp
...
static unsigned char ReadRawData(unsigned char Freq, unsigned char LineNum, int ByteNum, int *pRevBuffer)
{
	...
	if (ReCode == ERROR_CODE_OK) {
		focal_msleep_sharp(10);
		ReCode = Comm_Base_IIC_IO_sharp(I2C_wBuffer, 1, pReadData, BytesNumInTestMode1);
	}
	...
	for (i = 1; i < iReadNum; i++) {
		...
		if (i == iReadNum-1) {
			focal_msleep_sharp(10);
			ReCode = Comm_Base_IIC_IO_sharp(NULL, 0, pReadData+BYTES_PER_TIME*i, ByteNum-BYTES_PER_TIME*i);
		} else {
			focal_msleep_sharp(10);
			ReCode = Comm_Base_IIC_IO_sharp(NULL, 0, pReadData+BYTES_PER_TIME*i, BYTES_PER_TIME);
		}
...
static unsigned char GetPanelCols(unsigned char *pPanelCols)
{
	return ReadReg_sharp(REG_RX_NUM, pPanelCols);
}
...
static unsigned char GetChannelNum(void)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (ReCode == ERROR_CODE_OK) {
			if (0 < rBuffer[0] && rBuffer[0] < 80) {
				g_stSCapConfEx_sharp.ChannelYNum = rBuffer[0];
	...
	for (i = 0; i < 3; i++) {
		...
		ReCode = ReadReg_sharp(FT8716_LEFT_KEY_REG, &regData);
		if (ReCode == ERROR_CODE_OK) {
			if (((regData >> 0) & 0x01)) {
				g_stSCapConfEx_sharp.bLeftKey1 = true;
				++g_stSCapConfEx_sharp.KeyNum;
			}
			if (((regData >> 1) & 0x01)) {
				g_stSCapConfEx_sharp.bLeftKey2 = true;
				++g_stSCapConfEx_sharp.KeyNum;
			}
			if (((regData >> 2) & 0x01)) {
				g_stSCapConfEx_sharp.bLeftKey3 = true;
				++g_stSCapConfEx_sharp.KeyNum;
			}
		} else {
		...
		ReCode = ReadReg_sharp(FT8716_RIGHT_KEY_REG, &regData);
		if (ReCode == ERROR_CODE_OK) {
			if (((regData >> 0) & 0x01)) {
				g_stSCapConfEx_sharp.bRightKey1 = true;
				++g_stSCapConfEx_sharp.KeyNum;
			}
			if (((regData >> 1) & 0x01)) {
				g_stSCapConfEx_sharp.bRightKey2 = true;
				++g_stSCapConfEx_sharp.KeyNum;
			}
			if (((regData >> 2) & 0x01)) {
				g_stSCapConfEx_sharp.bRightKey3 = true;
				++g_stSCapConfEx_sharp.KeyNum;
			}
			break;
		} else {
...
static unsigned char GetRawData(void)
{
	...
	ReCode = ReadRawData(0, 0xAD, g_stSCapConfEx_sharp.ChannelXNum * g_stSCapConfEx_sharp.ChannelYNum * 2, m_iTempRawData);
	if (ERROR_CODE_OK != ReCode) {
	...
	for (iCol = 0; iCol < g_stSCapConfEx_sharp.KeyNumTotal; ++iCol) {
		m_RawData[g_stSCapConfEx_sharp.ChannelXNum][iCol] = m_iTempRawData[iCol];
	}
