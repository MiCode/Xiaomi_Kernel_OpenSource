...
FTS_I2C_READ_FUNCTION fts_i2c_read_test_sharp;
FTS_I2C_WRITE_FUNCTION fts_i2c_write_test_sharp;

char *g_testparamstring_sharp = NULL;
...
int init_i2c_read_func_sharp(FTS_I2C_READ_FUNCTION fpI2C_Read)
{
	...
	fts_i2c_read_test_sharp = fpI2C_Read;
...
int init_i2c_write_func_sharp(FTS_I2C_WRITE_FUNCTION fpI2C_Write)
{
	FTS_TEST_FUNC_ENTER();

	fts_i2c_write_test_sharp = fpI2C_Write;
	if (NULL == fts_i2c_write_test_sharp) {
		 FTS_TEST_ERROR("[focal] fts_i2c_read_test_sharp == NULL ");
	}
...
int set_param_data_sharp(char *TestParamData)
{
	...
	FTS_TEST_FUNC_ENTER();
	g_testparamstring_sharp = TestParamData;
	ret = ini_get_key_data_sharp(g_testparamstring_sharp);
	if (ret < 0) {
		 FTS_TEST_ERROR("ini_get_key_data_sharp error.");
		 return ret;
	}



	OnInit_InterfaceCfg_sharp(g_testparamstring_sharp);

	/*Get IC Name*/
	fts_ic_table_get_ic_name_from_ic_code(g_ScreenSetParam_sharp.iSelectedIC, g_strIcName_sharp);



	if (g_stTestFuncs.OnInit_TestItem) {
		 g_stTestFuncs.OnInit_TestItem(g_testparamstring_sharp);

	}

	if (g_stTestFuncs.OnInit_BasicThreshold) {
		 g_stTestFuncs.OnInit_BasicThreshold(g_testparamstring_sharp);
...
boolean start_test_tp_sharp(void)
{
	...
	FTS_TEST_FUNC_ENTER();
	FTS_TEST_DBG("IC_%s Test",  g_strIcName_sharp);
...
***********************************************************************/
int get_test_data_sharp(char *pTestData)
{
	int iLen = 0;
	FTS_TEST_DBG("[focal] %s start ",  __func__);


	if (g_stTestFuncs.get_test_data_sharp) {
