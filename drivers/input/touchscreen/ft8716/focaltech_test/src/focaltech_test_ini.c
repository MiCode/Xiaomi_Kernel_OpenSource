...
ST_INI_FILE_DATA *g_st_ini_file_data_sharp = NULL;
static int g_used_key_num;
...
static int fts_strncmp(const char *cs, const char *ct, size_t count)
{
...
int ini_get_key_sharp(char *filedata, char *section, char *key, char *value)
{
	...
	for (i = 0; i < g_used_key_num; i++) {
		 ...
		 if (strlen(key) == g_st_ini_file_data_sharp[i].iKeyNameLen) {
			if (fts_strncmp(key, g_st_ini_file_data_sharp[i].pKeyName,  g_st_ini_file_data_sharp[i].iKeyNameLen) == 0)

			{
				memcpy(value, g_st_ini_file_data_sharp[i].pKeyValue, g_st_ini_file_data_sharp[i].iKeyValueLen);
				ret = 0;
				break;
			}
		 }
...
int init_key_data_sharp(void)
{
	...
	g_st_ini_file_data_sharp = NULL;
	if (NULL == g_st_ini_file_data_sharp)
		 g_st_ini_file_data_sharp = fts_malloc(sizeof(ST_INI_FILE_DATA)*MAX_KEY_NUM);
	if (NULL == g_st_ini_file_data_sharp) {
		 FTS_TEST_ERROR("fts_malloc failed in function.");
		 return -EPERM;
	}
	for (i = 0; i < MAX_KEY_NUM; i++) {
		 memset(g_st_ini_file_data_sharp[i].pSectionName, 0, MAX_KEY_NAME_LEN);
		 memset(g_st_ini_file_data_sharp[i].pKeyName, 0, MAX_KEY_NAME_LEN);
		 memset(g_st_ini_file_data_sharp[i].pKeyValue, 0, MAX_KEY_VALUE_LEN);
		 g_st_ini_file_data_sharp[i].iSectionNameLen = 0;
		 g_st_ini_file_data_sharp[i].iKeyNameLen = 0;
		 g_st_ini_file_data_sharp[i].iKeyValueLen = 0;
	}
...
{
	if (NULL != g_st_ini_file_data_sharp)
		 fts_free(g_st_ini_file_data_sharp);
...
int ini_get_key_data_sharp(char *filedata)
{
	...
	while (1) /*find section */ {
		 ...
		 n = strlen(ini_str_trim_l_sharp(ini_str_trim_r_sharp(buf1)));
		 ...
		 strcpy(g_st_ini_file_data_sharp[g_used_key_num].pSectionName, tmpSectionName);
