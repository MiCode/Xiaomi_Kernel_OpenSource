...
/* COMMON PART - START */
#define TS_DEFAULT_FIRMWARE			"goodix_gt9896_fw_k16.bin"
		...
		const struct firmware_info *fw_info)
{
	...
	if (fw_ver.valid) {
		...
		if (ret == 0) {
			...
			ts_info("Warning: fw version is higher the IC's");
		}
...
static int __init goodix_fwu_module_init(void)
{
	...
	if (tp_load_ok == false) {
		ts_info("tp load error,will return in goodix_fwu_module_init\n");
		return -EINVAL;
	}
