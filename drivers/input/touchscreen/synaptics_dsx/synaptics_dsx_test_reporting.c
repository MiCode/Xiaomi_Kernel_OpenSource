...
static unsigned char *td43xx_noise_delta;
...
static int td43xx_ee_short_normalize_data(unsigned short *image)
{
	int retval = 0;
	int i, j;
	int tx_num = f54->tx_assigned;
	...
	unsigned short *left_median;
	unsigned short *right_median;
	unsigned short *left_column_buf;
	unsigned short *right_column_buf;
	unsigned int temp;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	...
	report_data_16 = (unsigned short *)image;
	for (i = 0; i < rx_num; i++) {
	...
	report_data_16 = (unsigned short *)image + right_size * rx_num;
	for (i = 0; i < rx_num; i++) {
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if (i < right_size) {
				...
				temp = temp * 100 / right_median[i];

			} else {
				temp = (unsigned int) *(image + i * rx_num + j);
				temp = temp * 100 / left_median[i];
			}
			pr_info("&&& %d &&&\n", temp);
			if (temp < 95)
				*(image + i * rx_num + j) = 1;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	unsigned short *td43xx_rt95_part_one;
	unsigned short *td43xx_rt95_part_two;
	...
	test_sysfs_read_report_td43xx(dev, attr, "95", count,
				false, true);

	secure_memcpy((unsigned char *)td43xx_rt95_part_one, td43xx_rt95_report_size,
		f54->report_data,
		td43xx_rt95_report_size,
		td43xx_rt95_report_size);

	secure_memcpy((unsigned char *)td43xx_rt95_part_two, td43xx_rt95_report_size,
		f54->report_data + td43xx_rt95_report_size,
		td43xx_rt95_report_size,
		td43xx_rt95_report_size);

	for (i = 0; i < tx_num * rx_num; i++)
		*(td43xx_rt95_part_one + i) =
				(*(td43xx_rt95_part_one + i) > 100) ? 1 : 0;


	pr_info("==========PART_TWO===========\n");
	for (i = 0; i < tx_num * rx_num; i++)
		pr_info("0x%04x \n", *(td43xx_rt95_part_two + i));
	td43xx_ee_short_normalize_data(td43xx_rt95_part_two);

	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			*(td43xx_ee_short_data + i*tx_num + j) =
			(unsigned char)(*(td43xx_rt95_part_one + i * tx_num + j) ||
				*(td43xx_rt95_part_two + i * tx_num + j));
			}
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	int times = 5;
	unsigned short *report_data_16;
	unsigned short *td43xx_noise_max;
	unsigned short *td43xx_noise_min;
	unsigned char *td43xx_noise_data;
	...
	td43xx_noise_delta = kzalloc(noise_report_size, GFP_KERNEL);
	if (!td43xx_noise_delta) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	for (repeat = 0 ; repeat < times; repeat++) {
		test_sysfs_read_report_td43xx(dev, attr, "94", count,
					false, false);
		...
		report_data_16 = (unsigned short *)td43xx_noise_data;
		for (i = 0; i < tx_num; i++) {
			for (j = 0; j < rx_num; j++) {
				*(td43xx_noise_max + i*tx_num + j) =
					max_t(unsigned short,
						*(td43xx_noise_max + i*tx_num + j),
						*report_data_16);
				*(td43xx_noise_min + i*tx_num + j) =
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if (*(td43xx_noise_delta + i*tx_num + j) > noise_upper_limit)
				result &= 0;
		}
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	test_sysfs_read_report_td43xx(dev, attr, "92", count,
				true, true);
...
static void test_f55_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	/* td43xx start */
	retval = synaptics_rmi4_reg_read(rmi4_data,
			f55->control_base_addr + AMP_SENSOR_CHAR_OFFSET,
			buf,
			sizeof(buf));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read f55 AMP characteristics\n",
				__func__);
		return;
	}

	f54->swap_sensor_side = buf[0] & MASK_1BIT;
	f54->left_mux_size = buf[1] & MASK_4BIT;
	f54->right_mux_size = (buf[1] >> 4) & MASK_4BIT;
	f54->tx_assigned = f54->left_mux_size + f54->right_mux_size;
	/* td43xx end */
