...
#define SYNAPTICS_RMI4_LOCKDOWN_INFO_SIZE 8
#define SYNAPTICS_RMI4_LOCKDOWN_REG_OFFSET 11
...
struct synaptics_rmi4_data {
	...
	unsigned char lockdown_info[SYNAPTICS_RMI4_LOCKDOWN_INFO_SIZE];
	int irq;
	int sensor_max_x;
	int sensor_max_y;
...
void synaptics_rmi4_incell_new_function(struct synaptics_rmi4_exp_fn *exp_fn_module,
		bool insert);

int synaptics_fw_updater_incell(unsigned char *fw_data);

int register_glove_mode_notifier(struct notifier_block *nb);

int unregister_glove_mode_notifier(struct notifier_block *nb);
