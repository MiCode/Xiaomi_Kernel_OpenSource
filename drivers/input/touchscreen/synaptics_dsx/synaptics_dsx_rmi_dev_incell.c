...
DECLARE_COMPLETION(rmidev_incell_remove_complete);
		...
		bool enable)
{
	...
	if (enable) {
		...
		retval = request_threaded_irq(rmi4_data->irq, NULL,
				...
				PLATFORM_INCELL_DRIVER_NAME, rmi4_data);
		if (retval < 0) {
...
exit:
	complete(&rmidev_incell_remove_complete);

	return;
}

static struct synaptics_rmi4_exp_fn rmidev_module_incell = {
	.fn_type = RMI_DEV,
...
static int __init rmidev_incell_module_init(void)
{
	if (get_hw_version_major() < 5)
		return 0;

	synaptics_rmi4_incell_new_function(&rmidev_module_incell, true);

	return 0;
}

static void __exit rmidev_incell_module_exit(void)
{
	synaptics_rmi4_incell_new_function(&rmidev_module_incell, false);

	wait_for_completion(&rmidev_incell_remove_complete);

	return;
}

module_init(rmidev_incell_module_init);
module_exit(rmidev_incell_module_exit);
