...
DECLARE_COMPLETION(fwu_remove_complete_incell);
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
		...
		strlcpy(fwu->image_name, bdata->fw_name, 256);
		dev_info(rmi4_data->pdev->dev.parent,
...
int synaptics_fw_updater_incell(unsigned char *fw_data)
{
	int retval;
...
EXPORT_SYMBOL(synaptics_fw_updater_incell);

#ifdef DO_STARTUP_FW_UPDATE
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	synaptics_fw_updater_incell(NULL);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_fw_updater_incell(fwu->ext_data_source);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to do reflash\n",
				__func__);
...
exit:
	complete(&fwu_remove_complete_incell);
...
static int __init rmi4_incell_fw_update_module_init(void)
{
	synaptics_rmi4_incell_new_function(&fwu_module, true);

	return 0;
}

static void __exit rmi4_incell_fw_update_module_exit(void)
{
	synaptics_rmi4_incell_new_function(&fwu_module, false);

	wait_for_completion(&fwu_remove_complete_incell);

	return;
}

module_init(rmi4_incell_fw_update_module_init);
module_exit(rmi4_incell_fw_update_module_exit);
