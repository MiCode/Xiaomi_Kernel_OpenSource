...
#ifdef TYPE_B_PROTOCOL
	input_mt_init_slots(rmi4_data->input_dev, bdata->max_finger_num);
#endif

	for (ii = 0; ii < bdata->cap_button_map->nbuttons; ii++) {
		input_set_capability(rmi4_data->input_dev,
				EV_KEY, bdata->cap_button_map->map[ii]);
	}
...
static void synaptics_rmi4_switch_mode_work(struct work_struct *work)
{
	...
	if (value == RMI4_INPUT_EVENT_SENSITIVE_MODE_OFF ||
		...
		data->enable_wakeup_gesture = value - RMI4_INPUT_EVENT_WAKUP_MODE_OFF;
	}
		...
		unsigned long event, void *data)
{
	...
	if (evdata && evdata->data &&
		...
		if (*blank == FB_BLANK_UNBLANK) {
			pr_info("##### UNBLANK SCREEN #####\n");
			synaptics_rmi4_input_enable(rmi4_data->input_dev);
		} else if (*blank == FB_BLANK_POWERDOWN) {
			pr_info("##### BLANK SCREEN #####\n");
			synaptics_rmi4_input_disable(rmi4_data->input_dev);
		}
	}
...
static int synaptics_rmi4_input_disable(struct input_dev *in_dev)
{
	...
	if (rmi4_data->enable_wakeup_gesture &&
		...
		dev_info(rmi4_data->pdev->dev.parent, "wakeup gesture enabled!\n");
		synaptics_rmi4_wakeup_gesture(rmi4_data, true);
		goto exit;
	}
	...
	/* Toggle RST gpio to reset IC to pass ESD test */
	if (bdata->reset_gpio >= 0) {
		gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
...
static int synaptics_rmi4_input_enable(struct input_dev *in_dev)
{
	...
	if (rmi4_data->enable_wakeup_gesture) {
		dev_info(rmi4_data->pdev->dev.parent, "wakeup gesture disabled!\n");
		synaptics_rmi4_wakeup_gesture(rmi4_data, false);
		goto exit;
	}
