...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
	...
	if (ts->pen_support) {
		...
		NVT_LOG("%s:RawData_PenTipX_DiffMax:\n", __func__);
		...
		csv_pen_noise_offset += ts->y_gang_num * ts->x_num * 7 + ts->y_gang_num * 2;
		NVT_LOG("%s:RawData_PenRingX_DiffMin:\n", __func__);
		...
		csv_pen_noise_offset += ts->y_gang_num * ts->x_num * 7 + ts->y_gang_num * 2;
		NVT_LOG("%s:RawData_PenRingY_DiffMax:\n", __func__);
		...
		csv_pen_noise_offset += ts->y_num * ts->x_gang_num * 7 + ts->y_num * 2;
		NVT_LOG("%s:RawData_PenRingY_DiffMin:\n", __func__);
