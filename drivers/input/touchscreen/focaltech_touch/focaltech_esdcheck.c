...
int idc_esdcheck_lcderror(struct fts_ts_data *ts_data)
{
	...
	MI_TOUCH_LOGN(1, "[ESD]Check LCD ESD");
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "[ESD]: Read ESD_SATURATE(0xED) failed ret=%d!", ret);
		return -EIO;
	}
	...
	if (val == 0xAA) {
		 ...
		 * 3. recover TP state
		 */
		MI_TOUCH_LOGI(1, "LCD ESD, Execute LCD reset!");
		lcd_need_reset = 1;
		tp_need_recovery = 1;
	}
...
static bool get_chip_id(struct fts_ts_data *ts_data)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "[ESD]: Read Reg 0xA3 failed ret = %d!!", ret);
	...
	if (i >= 3) {
		MI_TOUCH_LOGE(1, "[ESD]: Read Chip id 3 times failed, need execute TP reset!!");
		return 1;
	}
...
static bool get_flow_cnt(struct fts_ts_data *ts_data)
{
	...
	ret = fts_i2c_read(client, &reg_addr, 1, &reg_value, 1);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "[ESD]: Read Reg 0x91 failed ret = %d!!", ret);
		fts_esdcheck_data.i2c_nack_cnt++;
	} else {
	...
	if (fts_esdcheck_data.flow_work_hold_cnt >= 5) {
		MI_TOUCH_LOGN(1, "[ESD]: Flow Work Cnt(reg0x91) keep a value for 5 times, need execute TP reset!!");
		return 1;
	}
...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
	...
	if (fts_esdcheck_data.intr == 1) {
		MI_TOUCH_LOGN(1, "[ESD]: In interrupt state, not check esd, return immediately!!");
		return 0;
	}

	/* 2. check power state, if suspend, no need check esd */
	if (fts_esdcheck_data.suspend == 1) {
		MI_TOUCH_LOGN(1, "[ESD]: In suspend, not check esd, return immediately!!");
	...
	if (fts_esdcheck_data.proc_debug == 1) {
		MI_TOUCH_LOGI(1, "[ESD]: In apk or adb command mode, not check esd, return immediately!!");
		return 0;
	}
	...
	if (ret < 0) {
		...
		MI_TOUCH_LOGN(1, "[ESD]: not in work mode, no check esd, return immediately!!");
		return 0;
	}
	...
	MI_TOUCH_LOGN(1, "[ESD]: NoACK=%d, Error Data=%d, Hardware Reset=%d", fts_esdcheck_data.i2c_nack_cnt,
...
int fts_esdcheck_switch(bool enable)
{
	...
	if (fts_esdcheck_data.mode == ENABLE) {
		if (enable) {
			MI_TOUCH_LOGN(1, "[ESD]: ESD check start!!");
			fts_esdcheck_data.flow_work_hold_cnt = 0;
			fts_esdcheck_data.flow_work_cnt_last = 0;
			queue_delayed_work(ts_data->ts_workqueue, &ts_data->esdcheck_work,
						msecs_to_jiffies(ESDCHECK_WAIT_TIME));
		} else {
			MI_TOUCH_LOGN(1, "[ESD]: ESD check stop!!");
			cancel_delayed_work(&ts_data->esdcheck_work);
		}
	} else {
		MI_TOUCH_LOGN(1, "[ESD]: ESD should disable!!");
		cancel_delayed_work(&ts_data->esdcheck_work);
	}
...
static ssize_t fts_esdcheck_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		MI_TOUCH_LOGN(1, "enable esdcheck");
		fts_esdcheck_data.mode = ENABLE;
		fts_esdcheck_switch(ENABLE);
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		MI_TOUCH_LOGN(1, "disable esdcheck");
		fts_esdcheck_data.mode = DISABLE;
		fts_esdcheck_switch(DISABLE);
	}
...
int fts_create_esd_sysfs(struct i2c_client *client)
{
	...
	if (ret != 0) {
		MI_TOUCH_LOGE(1, "fts_create_esd_sysfs(sysfs) create failed!");
...
int fts_esdcheck_init(struct fts_ts_data *ts_data)
{
	...
	if (ts_data->ts_workqueue) {
		...
		MI_TOUCH_LOGE(1, "fts workqueue is NULL, can't run esd check function");
		return -EINVAL;
	}
