...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if ((count == 0) || (count > PROC_WRITE_BUF_SIZE)) {
		MI_TOUCH_LOGE(1, "apk proc wirte count(%d) fail", (int)count);
		return -EINVAL;
	}

	if (copy_from_user(&writebuf, buff, count)) {
		MI_TOUCH_LOGE(1, "[APK]: copy from user error!!");
		return -EFAULT;
	}
	...
	case PROC_SET_TEST_FLAG:
		MI_TOUCH_LOGI(1, "[APK]: PROC_SET_TEST_FLAG = %x!!", writebuf[1]);
#if FTS_ESDCHECK_EN
		if (writebuf[1] == 0) {
	...
	case PROC_READ_REGISTER:
		...
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "[APK]: write iic error!!");
		}
		break;
	...
	case PROC_SET_SLAVE_ADDR:
		...
		MI_TOUCH_LOGI(1, "Original i2c addr 0x%x", client->addr << 1);
		if (writebuf[1] != client->addr) {
			client->addr = writebuf[1];
			MI_TOUCH_LOGI(1, "Change i2c addr 0x%x to 0x%x", client->addr << 1, writebuf[1] << 1);
		}
#endif
		break;
	...
	case PROC_HW_RESET:
		...
		if (strncmp(tmp, "focal_driver", 12) == 0) {
			MI_TOUCH_LOGI(1, "APK execute HW Reset");
			fts_reset_proc(1);
		}
		break;
...
static ssize_t fts_debug_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
	...
	if ((count == 0) || (count > PROC_READ_BUF_SIZE)) {
		MI_TOUCH_LOGE(1, "apk proc read count(%d) fail", (int)count);
		return -EINVAL;
	}
#if FTS_ESDCHECK_EN
...
#endif
			MI_TOUCH_LOGE(1, "[APK]: read iic error!!");
			return ret;
		}
		num_read_chars = 1;
		break;
	...
	if (copy_to_user(buff, buf, num_read_chars)) {
		MI_TOUCH_LOGE(1, "[APK]: copy to user error!!");
		return -EFAULT;
	}
...
static int fts_debug_write(struct file *filp, const char __user *buff, unsigned long len, void *data)
{
	...
	switch (ts_data->proc_opmode) {
	case PROC_SET_TEST_FLAG:
		MI_TOUCH_LOGN(1, "[APK]: PROC_SET_TEST_FLAG = %x!!", writebuf[1]);
#if FTS_ESDCHECK_EN
		if (writebuf[1] == 0) {
			fts_esdcheck_switch(ENABLE);
		} else {
...
#if (FTS_CHIP_TYPE == _FT8201)
		ret = client->addr;
		MI_TOUCH_LOGN(1, "Original i2c addr 0x%x ", ret << 1);
		if (writebuf[1] != client->addr) {
			client->addr = writebuf[1];
			MI_TOUCH_LOGN(1, "Change i2c addr 0x%x to 0x%x", ret << 1, writebuf[1] << 1);
		}
#endif
		break;
	...
	case PROC_HW_RESET:
		...
		tmp[buflen - 1] = '\0';
		if (strncmp(tmp, "focal_driver", 12) == 0) {
			MI_TOUCH_LOGI(1, "Begin HW Reset");
			fts_reset_proc(1);
		}
		break;
...
int fts_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
	...
	if (NULL == ts_data->proc) {
		MI_TOUCH_LOGE(1, "create proc entry fail");
		return -ENOMEM;
	} else {
		MI_TOUCH_LOGI(1, "Create proc entry success!");
