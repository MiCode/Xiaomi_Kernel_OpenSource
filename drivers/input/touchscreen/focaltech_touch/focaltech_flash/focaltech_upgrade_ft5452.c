...
static int fts_ft5452_upgrade(struct i2c_client *client, u8 *buf, u32 len)
{
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "data len cmd write fail");
		goto fw_reset;
	}
	...
	MI_TOUCH_LOGN(1, "ecc calc startaddr:0x%04x, len:%d", start_addr, len);
	...
	for (i = 0; i < FTS_RETRIES_ECC_CAL; i++) {
		...
		MI_TOUCH_LOGN(1, "[UPGRADE]: reg_val[0]=%02x reg_val[0]=%02x!!", reg_val[0], reg_val[1]);
...
fw_reset:
	MI_TOUCH_LOGI(1, "upgrade fail, reset to normal boot");
	ret = fts_fwupg_reset_in_boot(client);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "reset to normal boot fail");
	}
