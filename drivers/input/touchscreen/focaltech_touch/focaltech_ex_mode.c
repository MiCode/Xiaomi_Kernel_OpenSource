...
static ssize_t fts_touch_glove_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		if (!g_fts_mode_flag.fts_glove_mode_flag) {
			MI_TOUCH_LOGI(1, "[Mode]enter glove mode");
		...
		if (g_fts_mode_flag.fts_glove_mode_flag) {
			MI_TOUCH_LOGI(1, "[Mode]exit glove mode");
	...
	MI_TOUCH_LOGI(1, "[Mode]glove mode status:  %d", g_fts_mode_flag.fts_glove_mode_flag);
	return count;
}
...
int fts_enter_glove_mode(struct i2c_client *client, int mode)
{
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "[Mode]fts_enter_glove_mode write value fail");
	}
...
static ssize_t fts_touch_cover_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		if (!g_fts_mode_flag.fts_cover_mode_flag) {
			MI_TOUCH_LOGI(1, "[Mode]enter cover mode");
		...
		if (g_fts_mode_flag.fts_cover_mode_flag) {
			MI_TOUCH_LOGI(1, "[Mode]exit cover mode");
	...
	MI_TOUCH_LOGI(1, "[Mode]cover mode status:  %d", g_fts_mode_flag.fts_cover_mode_flag);
	return count;
}
...
int fts_enter_cover_mode(struct i2c_client *client, int mode)
{
	...
	ret = fts_i2c_write_reg(client, buf_addr[0], buf_value[0]);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "[Mode] fts_enter_cover_mode write value fail");
	}
...
int fts_charger_mode_set(struct i2c_client *client, bool on)
{
	...
	if (on && !g_fts_mode_flag.fts_charger_mode_flag) {
		MI_TOUCH_LOGI(1, "[Mode]enter charger mode");
	...
	if (!on && g_fts_mode_flag.fts_charger_mode_flag) {
		MI_TOUCH_LOGI(1, "[Mode]exit charger mode");
...
static ssize_t fts_touch_charger_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	MI_TOUCH_LOGI(1, "[Mode]charger mode status: %d", g_fts_mode_flag.fts_charger_mode_flag);
	return count;
}
...
static int fts_enter_charger_mode(struct i2c_client *client, int mode)
{
	...
	if (fts_data->is_tp_testing) {
		MI_TOUCH_LOGE(1, "tp is doing test, skip set charger mode");
		return -EINVAL;
	}
	if (mode)
		buf_value[0] = 0x01;
	...
	ret = fts_i2c_write_reg(client, buf_addr[0], buf_value[0]);
	if (ret < 0) {
		MI_TOUCH_LOGN(1, "[Mode]fts_enter_charger_mode write value fail");
	}
...
int fts_ex_mode_init(struct i2c_client *client)
{
	...
	if (0 != err) {
		MI_TOUCH_LOGE(1, "[Mode]create sysfs failed.");
		sysfs_remove_group(&client->dev.kobj, &fts_touch_mode_group);
		return -EIO;
	} else {
		MI_TOUCH_LOGN(1, "[Mode]create sysfs succeeded");
	}
...
int fts_ex_mode_recovery(struct i2c_client *client)
{
	...
	if (g_fts_mode_flag.fts_charger_mode_flag) {
		MI_TOUCH_LOGI(1, "[Mode]restore charger mode");
		ret = fts_enter_charger_mode(client, true);
	}
#endif
