...
static void fts_test_save_data_csv(char *file_name, struct fts_test *tdata)
{
#if CSV_SUPPORT
	int i = 0;
	int j = 0;
...
static void fts_test_save_result_txt(char *file_name, struct fts_test *tdata)
{
#if TXT_SUPPORT
					    ...
					    bool mc_sc, bool key, bool result)
{
	...
	strncpy(info->name, name, TEST_ITEM_NAME_MAX - 1);
	info->code = code;
	info->mc_sc = mc_sc;
...
{
	struct fts_test *tdata = fts_ftest;
	char file_result_name[128];
	char file_data_name[128];
	int current_time;

	memset(file_result_name, 0, sizeof(file_result_name));
	memset(file_data_name, 0, sizeof(file_data_name));

	current_time = jiffies;
	sprintf(file_result_name, "%d%s", current_time, FTS_TXT_FILE_NAME);
	sprintf(file_data_name, "%d%s", current_time, FTS_CSV_FILE_NAME);

	FTS_TEST_INFO("test result name in txt:%s", file_result_name);
	FTS_TEST_INFO("test data name in txt:%s", file_data_name);

	FTS_TEST_FUNC_ENTER();
	fts_test_save_data_csv(file_data_name, tdata);
	fts_test_save_result_txt(file_result_name, tdata);
