...
int fts_test_i2c_read(u8 *writebuf, int writelen, u8 *readbuf, int readlen)
{
	int ret = 0;
	if (NULL == fts_data) {
		MI_TOUCH_LOGE(1, "fts_data is null, no test");
		return -EINVAL;
	}
...
int enter_work_mode(void)
{
	...
	if (i >= ENTER_WORK_FACTORY_RETRIES) {
		MI_TOUCH_LOGE(1, "Enter work mode fail");
		return -EIO;
	}
...
int enter_factory_mode(void)
{
	...
	if (i >= ENTER_WORK_FACTORY_RETRIES) {
		MI_TOUCH_LOGE(1, "Enter factory mode fail");
		return -EIO;
	}
...
int read_mass_data(u8 addr, int byte_num, int *buf)
{
	...
	/* read rawdata buffer */
	MI_TOUCH_LOGI(1, "mass data len:%d", byte_num);
	...
	MI_TOUCH_LOGI(1, "packet num:%d, remainder:%d", packet_num, packet_remainder);
...
int chip_clb_incell(void)
{
	...
	while (times++ < FACTORY_TEST_RETRY) {
		...
		if ((0 == ret) && (0x02 == val)) {
			...
			MI_TOUCH_LOGN(1, "reg%x=%x,retry:%d", FACTORY_REG_CLB, val, times);
	}
...
int start_scan_incell(void)
{
	...
	while (times++ < FACTORY_TEST_RETRY) {
		...
		if ((0 == ret) && (0 == (val >> 7))) {
			...
			MI_TOUCH_LOGN(1, "reg%x=%x,retry:%d", DEVIDE_MODE_ADDR, val, times);
	}
...
int get_cb_incell(u16 saddr, int byte_num, int *cb_buf)
{
	...
	MI_TOUCH_LOGI(1, "cb packet:%d,remainder:%d", packet_num, packet_remainder);
...
u32 fts_ic_table_get_ic_code_from_ic_name(char *ic_name)
{
	...
	MI_TOUCH_LOGE(1, "no IC type match");
	return 0;
}
...
int init_test_funcs(u32 ic_type)
{
	...
	MI_TOUCH_LOGI(1, "ic_type:%x, test functions len:%x", ic_type, funcs_len);
	...
	if (i >= funcs_len) {
		MI_TOUCH_LOGE(1, "no ic serial function match");
		return -ENODATA;
	}
...
void merge_all_testdata(void)
{
	...
	MI_TOUCH_LOGN(1, "lenStoreMsgArea=%d, lenStoreDataArea = %d", test_data.len_store_msg_area, test_data.len_store_data_area);
}
...
int allocate_init_testdata_memory(void)
{
	...
	return 0;
ERR:
	MI_TOUCH_LOGE(1, "malloc memory failed in function.");
	return -ENOMEM;
}
...
int get_tx_rx_num(u8 tx_rx_reg, u8 *ch_num, u8 ch_num_max)
{
	...
	if (i >= 3) {
		MI_TOUCH_LOGE(1, "get channel num fail");
		return -EIO;
	}
...
int get_channel_num(void)
{
	...
	ret = enter_factory_mode();
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "enter factory mode fail, can't get tx/rx num");
		return ret;
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "get tx_num fail");
		return ret;
	}

	ret = get_tx_rx_num(FACTORY_REG_CHY_NUM, &rx_num, RX_NUM_MAX);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "get rx_num fail");
		return ret;
	}
	...
	MI_TOUCH_LOGI(1, "TxNum=%d, RxNum=%d, MaxTxNum=%d, MaxRxNum=%d",
...
static int fts_test_main_init(void)
{
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "allocate memory for test data fail");
		return ret;
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "test init basicinfo fail");
		return ret;
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "Failed to malloc memory for detaithreshold");
		return ret;
	}
	...
	if (NULL == test_data.buffer) {
		MI_TOUCH_LOGE(1, "test_data.buffer malloc fail");
		return -ENOMEM;
	}
...
static bool fts_test_start(void)
{
	...
	MI_TOUCH_LOGN(1, "Enter");
	MI_TOUCH_LOGN(1, "IC_%s Test",  test_data.ini_ic_name);

	if (test_data.func && test_data.func->start_test) {
		testresult = test_data.func->start_test();
	} else {
		MI_TOUCH_LOGE(1, "test func/start_test func is null");
		testresult = false;
	}
...
static int fts_test_entry(char *ini_file_name)
{
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "fts_test_main_init() error.");
		goto TEST_ERR;
	}
...
static ssize_t fts_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ts_data->suspended) {
		MI_TOUCH_LOGI(1, "In suspend, no test, return now");
	...
	fwname[count - 1] = '\0';
	MI_TOUCH_LOGN(1, "fwname:%s.", fwname);
...
static void *data_start(struct seq_file *m, loff_t *pos)
{
	...
	if (!test_data.store_all_data) {
		MI_TOUCH_LOGE(1, "test data is error!");
		return NULL;
	}
...
static void *result_start(struct seq_file *m, loff_t *pos)
{
	...
	if (!test_data.testresult) {
		MI_TOUCH_LOGE(1, "test result is error!");
		return NULL;
	}
...
ssize_t fts_tp_selftest_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	if (!test_data.func) {
		MI_TOUCH_LOGE(1, "can't find test data func");
		retval = -SELFTEST_INVALID;
		fts_test_result = SELFTEST_INVALID;
		goto out;
	}
...
static int32_t c_show(struct seq_file *m, void *v)
{
	...
	if (retval) {
		MI_TOUCH_LOGE(1, "write data auto cal fail");
		retval = -EFAULT;
		goto out;
	}
/*********************GET RAWDATA*********************/
