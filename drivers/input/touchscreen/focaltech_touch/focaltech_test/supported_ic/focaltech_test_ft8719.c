...
static int ft8719_lcdnoise_test(bool *test_result)
{
	...
	MI_TOUCH_LOGN(1, " lcd_noise_test_frame:%d\n", frame_num);
...
static int ft8719_data_dump(int *rawdata, int *differ_data)
{
	...
	MI_TOUCH_LOGI(1, "tx_num:%d,rx_num:%d", test_data.screen_param.tx_num, test_data.screen_param.rx_num);
	...
	if (ret) {
		MI_TOUCH_LOGE(1, "write data select fail");
		ret = -EFAULT;
		goto out;
	}
	...
	ret = write_reg(FACTORY_REG_LINE_ADDR, 0xAD);
	if (ret) {
		MI_TOUCH_LOGE(1, "write REG LINE fail");
		ret = -EFAULT;
		goto out;
	}

	ret = read_mass_data(FACTORY_REG_RAWDATA_ADDR, test_data.screen_param.tx_num * test_data.screen_param.rx_num * 2, rawdata);
	if (ret) {
	...
	ret = read_mass_data(FACTORY_REG_RAWDATA_ADDR, test_data.screen_param.tx_num * test_data.screen_param.rx_num * 2, differ_data);
	if (ret) {
		FTS_TEST_SAVE_INFO("Failed to Get RawData of channel.\n");
		ret = -EFAULT;
		goto out;
