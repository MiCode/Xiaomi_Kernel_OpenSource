/************************************************************************
* Copyright (C) 2012-2017, Focaltech Systems (R)<A3><AC>All Rights Reserved.
...
static unsigned char AdcBuf[TX_NUM_MAX*RX_NUM_MAX] = {0};
...
unsigned char FT5X46_TestItem_WeakShortTest( bool* bTestResult )
{
	...
	for (i = 0; i < iChannelNum; i++)
	{
		...
		if ((iMin_CG > fGShortResistance[i]) || (iDsen - iDoffset < 0))
		{
			iCount++;
			if (i+1 <= iMaxTx)
				FTS_TEST_DBG("Tx%02d: %02d (k<A6><B8>),	", i+1, fGShortResistance[i]);
			else
				FTS_TEST_DBG("Rx%02d: %02d (k<A6><B8>),	", i+1 - iMaxTx, fGShortResistance[i]);
			if (iCount % 10 == 0)
				FTS_TEST_DBG("\n");
		}
	...
	for ( i = 0; i < iChannelNum; i++)
	{
		...
		if ( fMShortResistance[i] <= 0  || fMShortResistance[i] < iMin_CC )
		{
			iCount++;
			if (i+1 <= iMaxTx)
				FTS_TEST_DBG("Tx%02d: %02d(k<A6><B8>),	", i+1, fMShortResistance[i]);
			else
				FTS_TEST_DBG("Rx%02d: %02d(k<A6><B8>),	", i+1 - iMaxTx, fMShortResistance[i]);
...
static unsigned char WeakShort_GetAdcData( int AllAdcDataLen, int *pRevBuffer  )
{
	...
	for (i = 0; i < 100*5; i++)
	{
		...
		FTS_TEST_DBG("Time = %d, Reg 0x07 = %d", i+1, Data);
		if (ReCode == ERROR_CODE_OK)
		{
			if (Data == 0)
			{
	...
	ReCode = Comm_Base_IIC_IO(&TxBuf, 1, AdcBuf, iReadDataLen);
	if (ReCode == ERROR_CODE_OK)
	{
		FTS_TEST_DBG("\n Adc Data:\n");
		for (i = 0; i < iReadDataLen/2; i++)
		{
			...
			pRevBuffer[i] = (AdcBuf[2*i]<<8) + AdcBuf[ 2*i + 1];
