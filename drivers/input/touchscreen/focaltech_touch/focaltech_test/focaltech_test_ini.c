...
int print_key_data(void)
{
	...
	MI_TOUCH_LOGN(1, "test_data.ini_keyword_num = %d",  test_data.ini_keyword_num);
	for (i = 0; i < MAX_KEYWORD_NUM; i++) {

		MI_TOUCH_LOGN(1, "section_name_%d:%s, key_name_%d:%s\n,key_value_%d:%s",
...
int ini_get_key_data(char *filedata)
{
	...
	ret = init_key_data();
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "init key data failed");
		return -EPERM;
	}
	...
	while (1) { /*find section */
		...
		if (n >= MAX_CFG_BUF_LINE_LEN) {
			MI_TOUCH_LOGE(1, "Error Length:%d", n);
			goto cfg_scts_end;
		}
		dataoff += n;
		...
		if (n > 2 && ((buf1[0] == CFG_SSL && buf1[n - 1] != CFG_SSR))) {
			MI_TOUCH_LOGE(1, "Bad Section:%s", buf1);
			goto cfg_scts_end;
		}
		...
		if (buf1[0] == CFG_SSL) {
			...
			if (MAX_KEYWORD_NAME_LEN < test_data.ini_data[test_data.ini_keyword_num].section_name_len) {
				...
				MI_TOUCH_LOGE(1, "MAX_KEYWORD_NAME_LEN: CFG_ERR_OUT_OF_LEN");
				goto cfg_scts_end;
			}
			buf1[n - 1] = 0x00;
		...
		if (MAX_KEYWORD_VALUE_LEN < test_data.ini_data[test_data.ini_keyword_num].key_value_len) {
			ret = CFG_ERR_OUT_OF_LEN;
			MI_TOUCH_LOGE(1, "MAX_KEYWORD_VALUE_LEN: CFG_ERR_OUT_OF_LEN");
			goto cfg_scts_end;
		}
		...
		if (MAX_KEYWORD_NUM < test_data.ini_keyword_num) {
			...
			MI_TOUCH_LOGE(1, "MAX_KEYWORD_NUM: CFG_ERR_TOO_MANY_KEY_NUM");
			goto cfg_scts_end;
		}
	}
...
int GetPrivateProfileString(char *section, char *ItemName, char *defaultvalue, char *returnValue, char *IniFile)
{
	...
	if (NULL == returnValue) {
		MI_TOUCH_LOGN(1, "return Value==NULL");
		return 0;
	}
...
int fts_test_get_ini_size(char *config_name)
{
	...
	MI_TOUCH_LOGN(1, "Enter");
	if (config_name == NULL) {
		MI_TOUCH_LOGE(1, "config name is null.");
		return 0;
	}
	dev = fts_get_dev();
	if (dev == NULL) {
		MI_TOUCH_LOGE(1, "get dev error.");
		return 0;
	}
	...
	if (ret == 0) {
		...
		MI_TOUCH_LOGE(1, "request ini file error.");
		return 0;
	}
...
int fts_test_read_ini_data(char *config_name, char *config_buf)
{
	...
	ret = request_firmware(&fw, config_name, dev);
	if (ret == 0) {
		MI_TOUCH_LOGI(1, "start copy ini file.");
...
int init_interface(char *ini)
{
	...
	if (0 == ic_type) {
		MI_TOUCH_LOGE(1, "get ic code fail");
		return -EINVAL;
	}
	test_data.screen_param.selected_ic = ic_type;
	/*Get IC Name*/
	snprintf(test_data.ini_ic_name, PAGE_SIZE, "%s", str);
	MI_TOUCH_LOGI(1, "IC Name:%s IC Code:0x%02x.", test_data.ini_ic_name, test_data.screen_param.selected_ic);
...
int set_param_data(char *test_param)
{
	...
	if (NULL == test_data.ini_data) {
		MI_TOUCH_LOGE(1, "mallock memory for ini_data fail");
		goto SET_PARAM_ERR;
	}

	ret = ini_get_key_data(test_param);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "ini_get_key_data error.");
		return ret;
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "IC_Type in ini is not supported");
		return ret;
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "no ic series match");
		return ret;
	}

	if (NULL == test_data.func) {
		MI_TOUCH_LOGE(1, "test function is NULL");
		return -ENODATA;
	}
...
int fts_test_get_testparam_from_ini(char *config_name)
{
	...
	MI_TOUCH_LOGN(1, "ini_size = %d ", inisize);
	if (inisize <= 0) {
		MI_TOUCH_LOGE(1, "Get firmware size failed");
		return -EIO;
	}

	ini_file_data = fts_malloc(inisize + 1);
	if (NULL == ini_file_data) {
		MI_TOUCH_LOGE(1, "fts_malloc failed!");
		return -ENOMEM;
	}
	...
	if (ret) {
		MI_TOUCH_LOGE(1, " - fts_test_read_ini_data failed");
		goto GET_INI_DATA_ERR;
	} else {
		MI_TOUCH_LOGN(1, "fts_test_read_ini_data successful");
	}

	ret = set_param_data(ini_file_data);
	if (ret) {
		MI_TOUCH_LOGE(1, "set param data fail");
		goto GET_INI_DATA_ERR;
	}
