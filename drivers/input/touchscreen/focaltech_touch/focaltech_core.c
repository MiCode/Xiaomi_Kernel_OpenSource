...
static void fts_update_touchmode_data(int mode)
{
	...
	case Touch_Game_Mode:
		...
		if (temp_value == 1) {
			ret = fts_write_reg(FTS_REG_MONITOR_MODE, 0);
			if (ret < 0)
				FTS_ERROR("disable monitor mode error, ret=%d\n", ret);
			fts_data->gamemode_enabled = true;
		} else {
			/*restore touch parameters */
			ret = fts_write_reg(FTS_REG_MONITOR_MODE, 1);
			if (ret < 0)
				FTS_ERROR("restore monitor mode error, ret=%d\n", ret);
			ret = fts_write_reg(FTS_REG_SENSIVITY, (u8)xiaomi_touch_interfaces.touch_mode[Touch_UP_THRESHOLD][GET_DEF_VALUE]);
			if (ret < 0)
				FTS_ERROR("restore sensitivity error, ret=%d\n", ret);
			ret = fts_write_reg(FTS_REG_THDIFF, (u8)xiaomi_touch_interfaces.touch_mode[Touch_Tolerance][GET_DEF_VALUE]);
			if (ret < 0)
				FTS_ERROR("restore touch smooth error, ret=%d\n", ret);
			ret = fts_write_reg(FTS_REG_EDGE_FILTER_ORIENTATION, (u8)xiaomi_touch_interfaces.touch_mode[Touch_Panel_Orientation][GET_DEF_VALUE]);
			if (ret < 0)
				FTS_ERROR("restore orientation error, ret=%d\n", ret);
			ret = fts_write_reg(FTS_REG_EDGE_FILTER_LEVEL, (u8)xiaomi_touch_interfaces.touch_mode[Touch_Edge_Filter][GET_DEF_VALUE]);
			if (ret < 0)
				FTS_ERROR("restore orientation error, ret=%d\n", ret);
	...
	case Touch_Panel_Orientation:
			if (temp_value == 0 || temp_value == 2) {
				ret = fts_write_reg(FTS_REG_EDGE_FILTER_ORIENTATION, 0);
				if (ret < 0)
					FTS_ERROR("write orientation error, ret=%d\n", ret);
			}
			if (temp_value == 1) {
				ret = fts_write_reg(FTS_REG_EDGE_FILTER_ORIENTATION, 1);
				if (ret < 0)
					FTS_ERROR("write orientation error, ret=%d\n", ret);
			}
			if (temp_value == 3) {
				ret = fts_write_reg(FTS_REG_EDGE_FILTER_ORIENTATION, 2);
				if (ret < 0)
					FTS_ERROR("write orientation error, ret=%d\n", ret);
			}
		break;
	case Touch_Edge_Filter:
			if (fts_data->gamemode_enabled) {
				 ...
				 **/
				ret = fts_write_reg(FTS_REG_EDGE_FILTER_LEVEL, temp_value + 1);
				if (ret < 0)
...
static int fts_set_cur_value(int mode, int value)
{
	...
	if(mode == Touch_Aod_Enable && fts_data && value >=0) {
		fts_data->tid->aod_status = value;
		return 0;
	}

	if(mode == Touch_Fod_Enable && fts_data && value >=0) {
		mutex_lock(&fts_data->fod_mutex);
		fts_data->tid->fod_status = value;
		if(value) {
		...
		mutex_unlock(&fts_data->fod_mutex);
		return 0;
	}
