...
#include "./firmware/ist3038_fw_Version12.h"
#endif
...
int ist30xx_load_firmware(struct ist30xx_data *data, int fw_index)
{
	...
	tsp_err("Cannot load firmware err=%d\n", err);
	if (unlikely(err)) {
...
int ist30xx_set_tsp_fw(struct ist30xx_data *data)
{
	int i;
	struct ist30xx_fw *fw = &data->fw;
	struct ist30xx_config_info *info = data->pdata->config_array;
	...
	if (ist30xx_load_firmware(data, i) != 0) {
