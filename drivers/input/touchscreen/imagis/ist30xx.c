...
static struct timespec t_current;
...
void print_tkey_event(struct ist30xx_data *data, int id)
{
	...
	if (press) {
		if (data->tkey_pressed[idx] == false) {
			tsp_notc("k %s%d \n", TOUCH_DOWN_MESSAGE, id);
			data->tkey_pressed[idx] = true;
		}
	} else {
		if (data->tkey_pressed[idx] == true) {
			tsp_notc("k %s%d \n", TOUCH_UP_MESSAGE, id);
...
static irqreturn_t ist30xx_irq_thread(int irq, void *ptr)
{
	...
	if (unlikely((ms >= data->event_ms) && (ms - data->event_ms < data->noise_ms)))
		goto irq_end;
#endif
				 ...
				 unsigned long event, void *data)
{
	...
	int *blank;
	struct ist30xx_data *ist30xx_data =
		container_of(self, struct ist30xx_data, fb_notif);

	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
			ist30xx_data && ist30xx_data->client) {
		blank = evdata->data;
		if (*blank == FB_BLANK_UNBLANK)
			ist30xx_resume(&ist30xx_data->client->dev);
		else if (*blank == FB_BLANK_POWERDOWN)
			ist30xx_suspend(&ist30xx_data->client->dev);
	}
...
static int ist30xx_parse_dt(struct device *dev, struct ist30xx_platform_data *pdata)
{
	...
	for_each_child_of_node(np, temp) {
		...
		pr_info("### pdata->config_array[0]->cmcs_name = %s ####\n", info->cmcs_name);
		pr_info("### pdata->config_array[0]->fw-name = %s ####\n", info->fw_name);
		info++;
	}
...
static int ist30xx_pinctrl_select(struct ist30xx_data *data,
						bool on)
{
				   ...
				   const struct i2c_device_id * id)
{
    ...
    if (is_tp_driver_loaded) {
		printk(KERN_ERR "ist30xx_probe other driver has been loaded\n");
		return -ENODEV;
	}
    ...
    error = ist30xx_pinctrl_init(data);
	if (!error && data->ts_pinctrl) {
		error = ist30xx_pinctrl_select(data, true);
		if (error < 0)
			goto free_pinctrl;
	}
	...
	if (gpio_is_valid(pdata->irq_gpio)) {
		...
		if (unlikely(ret < 0)) {
			tsp_err("ist30xx irq gpio request failed!\n");
			goto err_alloc_input_dev;
		}
		ret = gpio_direction_input(pdata->irq_gpio);
		if (unlikely(ret < 0)) {
			tsp_err("ist30xx set direction for irq gpio failed!\n");
			goto free_irq_gpio;
		}
	}
