...
static int td4320_tcm_disp_notifier_callback(struct notifier_block *nb,
		unsigned long value, void *v)
{
	...
	LOG_ERR(tcm_hcd->pdev->dev.parent, "%s: value:0x%x, *data:0x%x in\n", __func__, value, *data);
	if (tcm_hcd && v) {
		if (value == MTK_DISP_EVENT_BLANK) {
			//resume: touch power on is after display to avoid display disturb
			if (*data == MTK_DISP_BLANK_UNBLANK) {
				LOG_ERR(tcm_hcd->pdev->dev.parent, "%s: resume in\n", __func__);
				retval = syna_tcm_resume(&tcm_hcd->pdev->dev);
				LOG_ERR(tcm_hcd->pdev->dev.parent, "%s: resume out\n", __func__);
			} else if (*data == MTK_DISP_BLANK_POWERDOWN) {
					LOG_ERR(tcm_hcd->pdev->dev.parent, "%s: suspend in\n", __func__);
					retval = syna_tcm_suspend(&tcm_hcd->pdev->dev);
					LOG_ERR(tcm_hcd->pdev->dev.parent, "%s: suspend out\n", __func__);
			}
		} else if (value == MTK_DISP_EARLY_EVENT_BLANK) {
			//suspend: touch power off is before display to avoid touch report event
			//after screen is off
				if (*data == MTK_DISP_BLANK_POWERDOWN) {
					LOG_ERR(tcm_hcd->pdev->dev.parent, "%s: ealy suspend in\n", __func__);
					retval = syna_tcm_early_suspend(&tcm_hcd->pdev->dev);
					LOG_ERR(tcm_hcd->pdev->dev.parent, "%s: ealy suspend out\n", __func__);
			}
		} else {
			LOG_ERR(tcm_hcd->pdev->dev.parent, "%s: touch IC can not suspend or resume\n", __func__);
				return -1;
		}
	}
	return 0;
...
static int syna_tcm_probe(struct platform_device *pdev)
{
	...
	touch_module_init(NULL);
	device_module_init(NULL);
	reflash_module_init(NULL);
	recovery_module_init(NULL);
	diag_module_init(NULL);
	testing_module_init(NULL);
	tcm_hcd->pdev = pdev;
	tcm_hcd->hw_if = hw_if;
 ...
 #if IS_ENABLED(CONFIG_DRM_MEDIATEK)
	tcm_hcd->disp_notifier.notifier_call = td4320_tcm_disp_notifier_callback;
	if (mtk_disp_notifier_register("Touch", &tcm_hcd->disp_notifier))
		LOG_ERR(tcm_hcd->pdev->dev.parent, "Failed to register disp notifier\n");
	else
...
static int syna_tcm_remove(struct platform_device *pdev)
{
	...
	if (disp_notify_reg_flag) {
		if (mtk_disp_notifier_unregister(&tcm_hcd->disp_notifier))
			LOG_ERR(tcm_hcd->pdev->dev.parent, "error occurred when unregister disp_notifier\n");
		else
			disp_notify_reg_flag = false;
	}
#endif
