...
static void wt_fts_test_save_data_csv(struct seq_file *m, struct fts_test *tdata)
{
#if CSV_SUPPORT
    int i = 0;
    int j = 0;
...
static int wt_fts_test_main_exit(struct seq_file *m)
{
    struct fts_test *tdata = fts_ftest;
    ...
    if (tdata->func && tdata->func->save_data_private)
        ...
        wt_fts_test_save_data_csv(m,tdata);
    fts_test_save_result_txt(tdata);
...
int wt_fts_test_entry(struct seq_file *m, char *ini_file_name)
{
    int ret = 0;
...
static int tp_data_dump_proc_open (struct inode *inode, struct file *file)
{
	...
	ret = fts_enter_test_environment(0);//normal fw
	if (ret < 0) {
		FTS_ERROR("enter test environment fail");
		goto err_disable_clb_fail;
	}
