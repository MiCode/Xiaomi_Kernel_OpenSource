...
extern int fts_proc_node_init(void);
extern void fts_proc_node_deinit(void);
...
void fts_charger_mode_work(struct work_struct *work)
{
	if (fts_charger_flag == XIAOMI_USB_ENABLE) {
		fts_write_reg(0x8B,0x01);
		FTS_INFO("fts in charger mode\n");
	} else if (fts_charger_flag == XIAOMI_USB_DISABLE){
		fts_write_reg(0x8B,0x00);
		FTS_INFO("fts out charger mode\n");
	}
}
                                ...
                                unsigned long event, void *data)
{
	...
	fts_charger_flag = evdata->usb_touch_enable;
	ret = queue_work(fts_data->fts_charger_detect_workqueue, &fts_data->fts_charger_detect_work);
	FTS_INFO("nvt_detect_charger_notifier_callback end\n");
	return 0;
}

void fts_headset_mode_work(struct work_struct *work)
{
	if (fts_headset_flag == XIAOMI_HEADSET_ENABLE) {
		fts_write_reg(0xC8,0x01);
		FTS_INFO("fts in headset mode\n");
	} else if (fts_headset_flag == XIAOMI_HEADSET_DISABLE){
		fts_write_reg(0xC8,0x00);
		FTS_INFO("fts out headset mode\n");
	}
}

static int fts_headset_notifier_callback(struct notifier_block *self,
                                unsigned long event, void *data)
{
	int ret;
	struct xiaomi_headset_notify_data  *evdata = data;
	FTS_INFO("fts_headset_notifier_callback start\n");
	fts_headset_flag = evdata->headset_touch_enable;
	ret = queue_work(fts_data->fts_headset_workqueue, &fts_data->fts_headset_work);
	FTS_INFO("fts_headset_notifier_callback end\n");
	return 0;
}
...
int fts_reset_proc(int hdelayms)
{
    ...
    /*Because the kernel msleep function time will drift,1 ms may chanfe to 20 ms,
     so mdelay (2) is adopted. If the actual mesurement shows that it will not drift
     for a long time, it can also be left unchanged,which will mainly affect the fw
     download time msleep (1)*/
    mdelay(2);
...
*/
            ...
            if ((ts_data->log_level >= 2) ||
                ...
                FTS_DEBUG("[B]P%d(%d, %d)[tm:%d] DOWN!",
                          events[i].id, events[i].x, events[i].y,events[i].area);
            }
        } else {
            input_mt_slot(input_dev, events[i].id);
            input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, false);
        ...
        for (i = 0; i < max_touch_num; i++) {
           ...
           /*events[i].x = ((touch_buf[FTS_TOUCH_OFF_E_XH + base] & 0x0F) << 8) \
			...
			events[i].x = ((touch_buf[FTS_TOUCH_OFF_E_XH + base] & 0x0F) << 11) \
					     ...
					     + ((touch_buf[FTS_TOUCH_OFF_PRE + base] & 0x70) >> 4);
			events[i].y = ((touch_buf[FTS_TOUCH_OFF_ID_YH + base] & 0x0F) << 11) \
					     + ((touch_buf[FTS_TOUCH_OFF_YL + base] & 0xFF) << 3 )\
					     + (touch_buf[FTS_TOUCH_OFF_PRE + base] & 0x07);
			events[i].area = touch_buf[FTS_TOUCH_OFF_AREA + base];
			events[i].x = ((events[i].x)*10)>>3;
			events[i].y = ((events[i].y)*10)>>3;
...
int enter_palm_mode(struct fts_ts_data *data)
{
	...
	FTS_INFO("get packet palm off event.\n");
...
#if IS_ENABLED(CONFIG_XIAOMI_PANEL_NOTIFIER)
static int fts_xiaomi_panel_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	struct xiaomi_panel_notify_data *evdata = data;

	struct fts_ts_data *ts =
		container_of(self, struct fts_ts_data, xiaomi_panel_notif);

	FTS_ERROR("%s: event = %lu\n", __func__, event);
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    ts_data->fts_charger_detect_workqueue = create_singlethread_workqueue("fts_charger_detect");
	if (!ts_data->fts_charger_detect_workqueue) {
        FTS_ERROR("create fts_charger_detect workqueue fail");
    }

    if (ts_data->fts_charger_detect_workqueue) {
	INIT_WORK(&ts_data->fts_charger_detect_work, fts_charger_mode_work);
    }

    ts_data->fts_charger_detect_notif.notifier_call = fts_detect_charger_notifier_callback;
	ret = xiaomi_usb_touch_notifier_register_client(&ts_data->fts_charger_detect_notif);
	if(ret) {
		FTS_ERROR("register charger_detect_notifier failed. ret=%d\n", ret);
		goto err_register_charger_nofif_failed;
	}

    ts_data->fts_headset_workqueue = create_singlethread_workqueue("fts_headset_wq");
	if (!ts_data->fts_headset_workqueue) {
        FTS_ERROR("create fts_headset_workqueue fail");
    }

    if (ts_data->fts_headset_workqueue) {
	INIT_WORK(&ts_data->fts_headset_work, fts_headset_mode_work);
    }

    ts_data->fts_headset_notif.notifier_call = fts_headset_notifier_callback;
	ret = xiaomi_headset_touch_notifier_register_client(&ts_data->fts_headset_notif);
	if(ret) {
		FTS_ERROR("register headset_notifier failed. ret=%d\n", ret);
		goto err_register_headset_nofif_failed;
	}
    ...
    ts_data->pm_suspend = false;
		...
		if (ret) {
			FTS_ERROR("register xiaomi_panel_notif failed. ret=%d\n", ret);
		} else {
			FTS_ERROR("register xiaomi_panel_notif successfully. ret=%d\n", ret);
		}
#elif defined(CONFIG_FB)
...
err_register_charger_nofif_failed:
	if(xiaomi_usb_touch_notifier_unregister_client(&ts_data->fts_charger_detect_notif))
		FTS_ERROR("Error occurred while unregistering fts_charger_detect_notifier.\n");
err_register_headset_nofif_failed:
	if(xiaomi_headset_touch_notifier_unregister_client(&ts_data->fts_headset_notif))
		FTS_ERROR("Error occurred while unregistering fts_headset_notifier.\n");
err_irq_req:
#if FTS_POWER_SOURCE_CUST_EN
err_power_init:
...
static int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
...
#if IS_ENABLED(CONFIG_XIAOMI_PANEL_NOTIFIER)
		if (xiaomi_panel_notifier_unregister_client(&ts_data->xiaomi_panel_notif))
			FTS_ERROR("Error occurred while unregistering xiaomi_panel_notif.\n");
#elif defined(CONFIG_FB)
	...
	device_init_wakeup(&ts_data->input_dev->dev, 0);
    FTS_FUNC_EXIT();

    return 0;
}

void fts_enable_irq_wake(bool enable)
{
	struct irq_desc *desc;
    struct fts_ts_data *ts_data = fts_data;

	if (enable) {
		if (!ts_data->irq_enable_wake) {
			enable_irq_wake(ts_data->irq);
			ts_data->irq_enable_wake = true;
		}
	} else {
		if (ts_data->irq_enable_wake) {
			disable_irq_wake(ts_data->irq);
			ts_data->irq_enable_wake = false;
		}
	}

	desc = irq_to_desc(ts_data->irq);
	FTS_INFO("enable=%d, desc->depth=%d\n", enable, desc->depth);
}
...
static int fts_set_cur_value(int mode, int value)
{
	...
	if (mode < Touch_Mode_NUM && mode >= 0) {
        ...
        xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE] = value;
        FTS_INFO("fts_set_cur_value mode = %d, value = %d\n", mode, xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE]);
		if (mode == 0) {
            FTS_INFO("fts_set_cur_value mode = %d, value = %d\n", mode, xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE]);
			if (value == 0) {
		...
		if (mode == 2) {
			...
			FTS_INFO("fts_set_cur_value mode = %d , value = %d\n", mode, xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE]);
			//fts_write_reg(0X8C, xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE]);
			switch (xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE]) {
			case 0:
				fts_write_reg(0X8C, 0x00); /*Panel orientation:Vertical screen*/
				FTS_INFO("%s,write value = %d\n", __func__, 0);
			break;
			case 1:
				fts_write_reg(0X8C, 0x01);/*Panel orientation:usb right screen*/
				FTS_INFO("%s,write value = %d\n", __func__, 1);
			break;
			case 3:
				fts_write_reg(0X8C, 0x02);/*Panel orientation:usb left screen*/
				FTS_INFO("%s,write value = %d\n", __func__, 2);
			break;
			default:
				FTS_INFO("%s,unknow value\n", __func__);
...
static int fts_get_mode_value(int mode, int value_type)
{
	...
	if (mode < Touch_Mode_NUM && mode >= 0) {
        ...
        FTS_INFO("fts_get_mode_value mode = %d , value = %d\n", mode, xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE]);
	} else
		FTS_INFO("%s ,don't support\n", __func__);
	return value;
}
...
static int fts_reset_mode(int mode)
{
	if (mode < Touch_Mode_NUM && mode >= 0) {
		...
		if (mode == 2)
			...
			FTS_INFO("%s,unknown value, mode = %d\n", __func__, mode);
	} else
