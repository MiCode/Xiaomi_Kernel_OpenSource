...
static struct proc_dir_entry *NVT_proc_selftest_result= NULL;
static int8_t nvt_mp_test_result_printed = 0;
static int8_t nvt_selftest_result = 0;
...
{
	NVT_LOG("++\n");
	if((TestResult_Short == 0) && (TestResult_Open == 0) && (TestResult_FWMutual == 0) &&
		(TestResult_FW_Rawdata == 0) && (TestResult_Noise == 0)){
		nvt_selftest_result = 0;
		NVT_LOG("selftest passs\n");
	}else{
		nvt_selftest_result = 1;
		NVT_LOG("selftest fail\n");
	}
...
int32_t parse_mp_setting_ain_array(char **ptr, const char *item_string, uint8_t *ain_array, uint32_t IC_CFG_SIZE)
{
	...
	*ptr = tmp;
	...
	nvt_print_result_log_in_one_line(ain_array, IC_CFG_SIZE);
	printk("\n");
	// go forward
	*ptr = *ptr + offset;
...
int32_t parse_mp_criteria_item_array(char **ptr, const char *item_string, int32_t *item_array)
{
	...
	*ptr = tmp;
	...
	for (i = 0; i < Y_Channel; i++) {
		...
		nvt_print_data_log_in_one_line(item_array + i * X_Channel, X_Channel);
		printk("\n");
		// go forward
		*ptr = *ptr + offset;
	}
	...
	nvt_print_data_log_in_one_line(item_array + Y_Channel * X_Channel, Key_Channel);
	printk("\n");
...
static int32_t nvt_load_mp_setting_criteria_from_csv(const char *filename)
{
	...
	if (ts->carrier_system) {
		ptr = fbufp;
		if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_Short_Diff_P:", PS_Config_Lmt_Short_Diff_P) < 0) {
			NVT_ERR("Cannot get PS_Config_Lmt_Short_Diff_P array value!\n");
			retval = -1;
			goto exit_free;
		}

		ptr = fbufp;
		if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_Short_Diff_N:", PS_Config_Lmt_Short_Diff_N) < 0) {
			NVT_ERR("Cannot get PS_Config_Lmt_Short_Diff_N array value!\n");
			retval = -1;
			goto exit_free;
		}

		ptr = fbufp;
		if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_Short_Base_P:", PS_Config_Lmt_Short_Base_P) < 0) {
			NVT_ERR("Cannot get PS_Config_Lmt_Short_Base_P array value!\n");
			retval = -1;
			goto exit_free;
		}

		ptr = fbufp;
		if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_Short_Base_N:", PS_Config_Lmt_Short_Base_N) < 0) {
			NVT_ERR("Cannot get PS_Config_Lmt_Short_Base_N array value!\n");
			retval = -1;
			goto exit_free;
		}
	} else {
	...
	if (ts->carrier_system) {
		ptr = fbufp;
		if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_FW_CC_I_P:", PS_Config_Lmt_FW_CC_I_P) < 0) {
			NVT_ERR("Cannot get PS_Config_Lmt_FW_CC_I_P array value!\n");
			retval = -1;
			goto exit_free;
		}

		ptr = fbufp;
		if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_FW_CC_I_N:", PS_Config_Lmt_FW_CC_I_N) < 0) {
			NVT_ERR("Cannot get PS_Config_Lmt_FW_CC_I_N array value!\n");
			retval = -1;
			goto exit_free;
		}

		ptr = fbufp;
		if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_FW_CC_Q_P:", PS_Config_Lmt_FW_CC_Q_P) < 0) {
			NVT_ERR("Cannot get PS_Config_Lmt_FW_CC_Q_P array value!\n");
			retval = -1;
			goto exit_free;
		}

		ptr = fbufp;
		if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_FW_CC_Q_N:", PS_Config_Lmt_FW_CC_Q_N) < 0) {
			NVT_ERR("Cannot get PS_Config_Lmt_FW_CC_Q_N array value!\n");
			retval = -1;
			goto exit_free;
		}
	} else {
...
static int nvt_selftest_result_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", nvt_selftest_result);
	return 0;
}
static int32_t nvt_selftest_result_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_selftest_result_show, NULL);
}
static const struct file_operations nvt_selftest_result_fops ={
	.owner = THIS_MODULE,
	.open = nvt_selftest_result_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};
...
int32_t nvt_mp_proc_init(void)
{
	...
	NVT_proc_selftest_result = proc_create("nvt_selftest_result", 0444, NULL, &nvt_selftest_result_fops);
	if (NVT_proc_selftest_result == NULL) {
		NVT_ERR("create /proc/nvt_selftest_result Failed!\n");
		return -1;
	}
