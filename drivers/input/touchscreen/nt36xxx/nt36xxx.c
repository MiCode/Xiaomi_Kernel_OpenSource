...
static void nvt_parse_dt(struct device *dev)
{
	...
	ts->reset_gpio = of_get_named_gpio_flags(np, "novatek,reset-gpio", 0, &ts->reset_gpio);
	...
	retval = of_property_read_string(np, "novatek,fw-name", &name);
	if (retval == -EINVAL)
		ts->fw_name = NULL;
	else if (retval < 0)
		return;
...
static int nvt_get_reg(struct nvt_ts_data *ts, bool get)
{
	...
	if ((ts->vddio_reg_name != NULL) && (*ts->vddio_reg_name != 0)) {
		ts->vddio_reg = regulator_get(ts->input_dev->dev.parent,
				ts->vddio_reg_name);
	...
	if ((ts->lab_reg_name != NULL) && (*ts->lab_reg_name != 0)) {
		ts->lab_reg = regulator_get(ts->input_dev->dev.parent,
				ts->lab_reg_name);
	...
	if ((ts->ibb_reg_name != NULL) && (*ts->ibb_reg_name != 0)) {
		ts->ibb_reg = regulator_get(ts->input_dev->dev.parent,
				ts->ibb_reg_name);
