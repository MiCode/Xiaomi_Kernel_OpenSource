...
unsigned char FT8716_TestItem_OpenTest(struct i2c_client *client)
{
	...
	m_ucTempData = (char *)kzalloc(TX_NUM_MAX * RX_NUM_MAX*2, GFP_DMA);
	if (!m_ucTempData) {
		FTS_TEST_ERR("\r\n//=========  Alloc memory Failed!");
		return -ENOMEM;
	}
	...
	ReCode = WriteReg(0x21, chValue1);
	FTS_TEST_PRINT("0x21 WriteReg:%d\n", chValue1);
	SysDelay(50);
	if (ERROR_CODE_OK != ReCode) {
		FTS_TEST_ERR("\r\n//=========  Write Reg Failed!");
		goto TEST_ERR;
...
static unsigned char WeakShort_GetAdcData(int AllAdcDataLen, int *pRevBuffer)
{
	...
	FTS_TEST_DBG("");
	ReadBuffer = (char *)kzalloc(PAGE_SIZE * 2, GFP_DMA);
	if (!ReadBuffer) {
		FTS_TEST_ERR("ReadBuffer alloc memory fail\n");
		return -ENOMEM;
	}
	...
	usReturnNum = BYTES_PER_TIME;
	if (ReCode == ERROR_CODE_OK)
		ReCode = Comm_Base_IIC_IO(wBuffer, 1, ReadBuffer, usReturnNum);
	for (i = 1; i < iReadNum; i++) {
		if (ReCode != ERROR_CODE_OK) {
		...
		if (i == iReadNum-1) {
			...
			ReCode = Comm_Base_IIC_IO(NULL, 0, ReadBuffer+BYTES_PER_TIME*i, usReturnNum);
		} else {
			usReturnNum = BYTES_PER_TIME;
			ReCode = Comm_Base_IIC_IO(NULL, 0, ReadBuffer+BYTES_PER_TIME*i, usReturnNum);
		}
	...
	for (index = 0; index < AllAdcDataLen/2; ++index)
		pRevBuffer[index] = (ReadBuffer[index * 2] << 8) + ReadBuffer[index * 2 + 1];
