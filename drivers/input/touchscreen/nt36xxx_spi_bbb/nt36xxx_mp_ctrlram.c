 ...
 *
 * $Revision: 41918 $
 * $Date: 2019-03-12 15:17:45 +0800 (週二, 12 三月 2019) $
 *
...
#define SHORT_TEST_CSV_FILE "/data/vendor/touchpad/NVT_ShortTest.csv"
#define OPEN_TEST_CSV_FILE "/data/vendor/touchpad/NVT_OpenTest.csv"
#define FW_RAWDATA_CSV_FILE "/data/vendor/touchpad/NVT_FWMutualTest.csv"
...
/*******************************************************
Description:
	Novatek touchscreen print self-test result on log function.

return:
	n.a.
*******************************************************/
static void print_selftest_result_on_log(int32_t TestResult, uint8_t RecordResult[], int32_t rawdata[], uint8_t x_len, uint8_t y_len)
{
	int32_t i = 0;
...
/*******************************************************
Description:
	Novatek touchscreen /proc/tp_selftest callback function.
...
int lct_nvt_tp_selftest_callback(unsigned char cmd)
{
	...
	switch (cmd) {
	case TP_SELFTEST_CMD_FFFF_MMI:
		NVT_LOG("touch selftest command: mmi\n");
		break;
	case TP_SELFTEST_CMD_AAAAAA_I2C:
		NVT_LOG("touch selftest command: i2c\n");
		NVT_LOG("--\n");
		return 2;//PASS
	case TP_SELFTEST_CMD_AAAAAA_OPEN:
		NVT_LOG("touch selftest command: open\n");
		break;
	case TP_SELFTEST_CMD_AAAAAA_SHORT:
		NVT_LOG("touch selftest command: short\n");
		break;
	default:
		NVT_LOG("touch selftest command: unknow\n");
		NVT_LOG("--\n");
		return 0;//Unknow
	}
	...
	//Open Test Result
	print_selftest_result_on_log(TestResult_Open, RecordResult_Open, RawData_Open, X_Channel, Y_Channel);

	//FW Rawdata Test Result
	if ((TestResult_FW_Rawdata == 0) || (TestResult_FW_Rawdata == 1)) {
		 print_selftest_result_on_log(TestResult_FWMutual, RecordResult_FWMutual, RawData_FWMutual, X_Channel, Y_Channel);
	} else { // TestResult_FW_Rawdata is -1
		if (TestResult_FWMutual == -1) { //FW Mutual
			print_selftest_result_on_log(TestResult_FWMutual, RecordResult_FWMutual, RawData_FWMutual, X_Channel, Y_Channel);
		}
		if (TestResult_FW_CC == -1) { //FW CC
			print_selftest_result_on_log(TestResult_FW_CC, RecordResult_FW_CC, RawData_FW_CC, X_Channel, Y_Channel);
		}
	}

	//Noise Test Result
	if ((TestResult_Noise == 0) || (TestResult_Noise == 1)) {
		print_selftest_result_on_log(TestResult_FW_DiffMax, RecordResult_FW_DiffMax, RawData_Diff_Max, X_Channel, Y_Channel);
	} else { // TestResult_Noise is -1
		if (TestResult_FW_DiffMax == -1) { //FW Diff Max
			print_selftest_result_on_log(TestResult_FW_DiffMax, RecordResult_FW_DiffMax, RawData_Diff_Max, X_Channel, Y_Channel);
		}
		if (TestResult_FW_DiffMin == -1) { //FW Diff Min
			print_selftest_result_on_log(TestResult_FW_DiffMin, RecordResult_FW_DiffMin, RawData_Diff_Min, X_Channel, Y_Channel);
		}
	}
