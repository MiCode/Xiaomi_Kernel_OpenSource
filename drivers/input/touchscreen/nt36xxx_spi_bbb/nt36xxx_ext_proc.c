 ...
 *
 * $Revision: 43077 $
 * $Date: 2019-04-09 19:13:23 +0800 (週二, 09 四月 2019) $
 *
...
#define NVT_AAAAAA_CONFIG_INFO "nvt_aaaaaa_config_info"
#define GGG_TP_DATA_DUMP "tp_data_dump"
...
static struct proc_dir_entry *NVT_proc_aaaaaa_config_info_entry;
static struct proc_dir_entry *GGG_proc_tp_data_dump_entry;

// aaaaaa Config Info.
static uint8_t nvt_aaaaaa_conf_info_fw_ver = 0;
static uint8_t nvt_aaaaaa_conf_info_fae_id = 0;
static uint64_t nvt_aaaaaa_conf_info_reservation = 0;
...
static int nvt_aaaaaa_config_info_show(struct seq_file *m, void *v)
{
	seq_printf(m, "FW version/Config version, Debug version: 0x%02X\n", nvt_aaaaaa_conf_info_fw_ver);
	seq_printf(m, "FAE ID: 0x%02X\n", nvt_aaaaaa_conf_info_fae_id);
	seq_printf(m, "Reservation byte: 0x%012llX\n", nvt_aaaaaa_conf_info_reservation);

	return 0;
}

static int32_t nvt_aaaaaa_config_info_open(struct inode *inode, struct file *file)
{
	uint8_t buf[16] = {0};
	...
	nvt_aaaaaa_conf_info_fw_ver = buf[1];
	nvt_aaaaaa_conf_info_fae_id = buf[2];
	nvt_aaaaaa_conf_info_reservation = (((uint64_t)buf[3] << 40) | ((uint64_t)buf[4] << 32) | ((uint64_t)buf[5] << 24) | ((uint64_t)buf[6] << 16) | ((uint64_t)buf[7] << 8) | (uint64_t)buf[8]);

	mutex_unlock(&ts->lock);

	NVT_LOG("--\n");

	return single_open(file, nvt_aaaaaa_config_info_show, NULL);
}

static const struct file_operations nvt_aaaaaa_config_info_fops = {
	.owner = THIS_MODULE,
	.open = nvt_aaaaaa_config_info_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
int32_t nvt_extra_proc_init(void)
{
	...
	NVT_proc_aaaaaa_config_info_entry = proc_create(NVT_AAAAAA_CONFIG_INFO, 0444, NULL, &nvt_aaaaaa_config_info_fops);
	if (NVT_proc_aaaaaa_config_info_entry == NULL) {
		NVT_ERR("create proc/%s Failed!\n", NVT_AAAAAA_CONFIG_INFO);
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/%s Succeeded!\n", NVT_AAAAAA_CONFIG_INFO);
	}

	GGG_proc_tp_data_dump_entry = proc_create(GGG_TP_DATA_DUMP, 0444, NULL, &lct_tp_data_dump_fops);
	if (GGG_proc_tp_data_dump_entry == NULL) {
		NVT_ERR("create proc/%s Failed!\n", GGG_TP_DATA_DUMP);
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/%s Succeeded!\n", GGG_TP_DATA_DUMP);
...
void nvt_extra_proc_deinit(void)
{
	...
	if (NVT_proc_aaaaaa_config_info_entry != NULL) {
		remove_proc_entry(NVT_AAAAAA_CONFIG_INFO, NULL);
		NVT_proc_aaaaaa_config_info_entry = NULL;
		NVT_LOG("Removed /proc/%s\n", NVT_AAAAAA_CONFIG_INFO);
	}

	if (GGG_proc_tp_data_dump_entry != NULL) {
		remove_proc_entry(GGG_TP_DATA_DUMP, NULL);
		GGG_proc_tp_data_dump_entry = NULL;
		NVT_LOG("Removed /proc/%s\n", GGG_TP_DATA_DUMP);
	}
}
#endif
