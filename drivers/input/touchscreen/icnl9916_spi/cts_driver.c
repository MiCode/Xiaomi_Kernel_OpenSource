...
extern void cts_ts_earphone_event_callback(void);
#endif
        ...
        unsigned long event, void *data)
{
    ...
    cts_info("mtk drm notifier callback");
...
{
    cts_info("Init MTK DRM notifier");
...
#ifdef CFG_CTS_DRM_NOTIFIER
    {
        ...
        if (active_panel) {
            ret =drm_register_client(&cts_data->pdata->fb_notifier);
            if (ret)
...
#else
    return mtk_disp_notifier_register("chipone_ts_notifier",&cts_data->pdata->fb_notifier);
#endif
}
...
static int cts_deinit_pm_fb_notifier(struct chipone_ts_data *cts_data)
{
...
#ifdef CFG_CTS_DRM_NOTIFIER
    {
        ...
        if (active_panel) {
            ret = drm_unregister_client(&cts_data->pdata->fb_notifier);
            if (ret)
...
#ifdef CONFIG_LCT_EARJACK_DETECT
        g_touchscreen_earphone_pulgin.event_callback = cts_ts_earphone_event_callback;
#endif
...
#endif
{
    ...
    if (cts_data) {
        ...
        cts_warn("Chipone i2c driver shutdown while NULL chipone_ts_data");
    }
...
int lct_cts_tp_proximity_switch_callback(bool flag)
{       
        struct cts_device *cts_dev = &g_cts_data->cts_dev;
	int rc = 0;
        if (flag) {
                rc = cts_tcs_set_proximity_mode(cts_dev,1);
                cts_info("cts enable proximity mode,rc = %d\n",rc);
        } else {
                rc = cts_tcs_set_proximity_mode(cts_dev,0);
                cts_info("cts disable proximity mode,rc = %d\n",rc);
        }
   return 0;
}
...
static int __init cts_driver_init(void)
{
    ...
    videolfb_tag = (struct tag_videolfb *)of_get_property(lcm_name,
        ...
        if (!videolfb_tag) {
             cts_info("Invalid lcm name\n");
             return -1;
       }
    cts_info("chipone_ts_spi_init read lcm name : %s", videolfb_tag->lcmname);
    if (strcmp("dsi_panel_c3y_42_0f_0c_hd_video",videolfb_tag->lcmname) == 0) {
           cts_info("icnl9916");
    } else {
          cts_info("Unkown Touch");
        return -1;
               }
 }  
