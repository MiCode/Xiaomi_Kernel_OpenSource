...
/* add tp_test by zhangchaofan start, 2018-11-30*/
...
/* add tp_test by zhangchaofan end, 2018-11-30*/
...
static int lct_fts_tp_selftest(void)
{
	...
	if ((TestResult_Maininit == 0)&& (TestResult_Testparams == 0) && (TestResult_Start == 0)&& (TestResult_Workmode ==0) && (TestResult_Mainexit == 0)){
		...
		FTS_TEST_INFO("Maininit=%d, Testparams=%d, Start=%d, Workmode=%d, Mainexit=%d, Fail!\n",
			TestResult_Maininit, TestResult_Testparams, TestResult_Start, TestResult_Workmode, TestResult_Mainexit);	//add  debug for tp by zhangchaofan 2018-11-30
		FTS_TEST_INFO("Fail\n");
		return 1;
	}
...
int fts_test_init(struct i2c_client *client)
{
	...
	/* add fts tp selftest by zhangchaofan end, 2018-11-30*/
