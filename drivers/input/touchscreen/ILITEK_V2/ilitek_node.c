...
static ssize_t ilitek_proc_get_delta_data_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	for (y = 0; y < row; y++) {
		...
		for (x = 0; x < col; x++) {
			...
			ipio_err(KERN_CONT "%5d", delta[shift]);
		}
		len += snprintf(g_user_buf + len, PAGE_SIZE - len, "\n");
		ipio_err(KERN_CONT "\n");
	}
...
static ssize_t ilitek_proc_fw_get_raw_data_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	for (y = 0; y < row; y++) {
		...
		for (x = 0; x < col; x++) {
			...
			ipio_err(KERN_CONT "%5d", rawdata[shift]);
		}
		len += snprintf(g_user_buf + len, PAGE_SIZE - len, "\n");
		ipio_err(KERN_CONT "\n");
	}
...
int get_tp_recore_data(bool mcu)
{
	...
	if (!ice)
		ilitek_ice_mode_ctrl(DISABLE, mcu);
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	if (strncmp(cmd, "hwreset", strlen(cmd)) == 0) {
		...
		ilitek_tddi_ic_get_ddi_reg_onepage(data[1], data[2], &ddi_data, OFF);
		ipio_info("ddi_data = %x\n", ddi_data);
	} else if (strncmp(cmd, "setddiregdata", strlen(cmd)) == 0) {
		ipio_info("Set ddi reg one page: page = %x, reg = %x, data = %x\n", data[1], data[2], data[3]);
		ilitek_tddi_ic_set_ddi_reg_onepage(data[1], data[2], data[3], OFF);
		...
		ret = ilitek_tddi_fw_dump_flash_data(data[1], data[2], false, OFF);
		...
		idev->fw_update_stat = FW_STAT_INIT;
		ret = ilitek_fw_dump_iram_data(data[1], data[2], true, OFF);
		idev->fw_update_stat = (ret < 0) ? FW_UPDATE_FAIL : FW_UPDATE_PASS;
		ipio_info("idev->fw_update_stat = %d\n", idev->fw_update_stat);
		...
		for (i = 0; i < 14; i++) {
			idev->edge_palm_para[i * 2 + 2] = (data[i + 1] & 0xFF00) >> 8;
			idev->edge_palm_para[i * 2 + 3] = data[i + 1] & 0xFF;
		}
		idev->edge_palm_para[30] = ilitek_calc_packet_checksum(idev->edge_palm_para
		...
		idev->knuckle = !idev->knuckle;
		if (idev->knuckle == true) {
			ilitek_config_knuckle_roi_ctrl(CMD_ENABLE);
		} else {
			ilitek_config_knuckle_roi_ctrl(CMD_DISABLE);
		}
		ipio_info("knuckle = %s\n", idev->knuckle ? "ENABLE" : "DISABLE");
#endif
	} else {
...
static long ilitek_node_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ILITEK_IOCTL_TP_DUMP_FLASH:
		...
		ret = ilitek_tddi_fw_dump_flash_data(0, 0, true, OFF);
		if (ret < 0) {
	...
	case ILITEK_IOCTL_DDI_WRITE:
		...
		ilitek_tddi_ic_set_ddi_reg_onepage(szBuf[0], szBuf[1], szBuf[2], OFF);
		break;
	case ILITEK_IOCTL_DDI_READ:
		...
		ilitek_tddi_ic_get_ddi_reg_onepage(szBuf[0], szBuf[1], &szBuf[2], OFF);
