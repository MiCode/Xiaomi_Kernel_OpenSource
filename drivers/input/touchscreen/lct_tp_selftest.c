...
static ssize_t tp_selftest_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	if( (strncmp(tmp_data, TP_SELF_TEST_MMI_CMD, strlen(TP_SELF_TEST_MMI_CMD)) == 0)
			&& (!is_in_self_test)) { //mmi

		TP_LOGW("MMI TP self-test ...\n");
		lct_tp_selftest_cmd = TP_SELFTEST_CMD_MMI;

	} else if ((strncmp(tmp_data, TP_SELF_TEST_AAABBB_I2C_CMD, strlen(TP_SELF_TEST_AAABBB_I2C_CMD)) == 0)
			&& (!is_in_self_test)) { //i2c

		TP_LOGW(" TP i2c self-test ...\n");
		lct_tp_selftest_cmd = TP_SELFTEST_CMD_AAABBB_I2C;


	} else if ((strncmp(tmp_data, TP_SELF_TEST_AAABBB_OPEN_CMD, strlen(TP_SELF_TEST_AAABBB_OPEN_CMD)) == 0)
			&& (!is_in_self_test)) { //open

		TP_LOGW(" TP open self-test ...\n");
		lct_tp_selftest_cmd = TP_SELFTEST_CMD_AAABBB_OPEN;

	} else if ((strncmp(tmp_data, TP_SELF_TEST_AAABBB_SHORT_CMD, strlen(TP_SELF_TEST_AAABBB_SHORT_CMD)) == 0)
			&& (!is_in_self_test)) { //short

		TP_LOGW(" TP short self-test ...\n");
		lct_tp_selftest_cmd = TP_SELFTEST_CMD_AAABBB_SHORT;
