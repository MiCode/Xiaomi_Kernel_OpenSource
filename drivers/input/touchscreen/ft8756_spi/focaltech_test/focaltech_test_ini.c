...
static int ini_parse_keyword(struct ini_data *ini, char *line_buffer)
{
	...
	if (offset > MAX_KEYWORD_NUM) {
		FTS_TEST_ERROR("keyword num(%d)>max(%d),please check MAX_KEYWORD_NUM", ini->keyword_num_total, MAX_KEYWORD_NUM);
		return -ENODATA;
	}
	...
	for (i = 0; i < tx_num + 1; i++) {
		...
		for (j = 0; j < divider_pos; j++) {
			if (',' == str[j]) {
				thr_pos = i * rx_num + k;
				if (thr_pos >= node_num) {
					FTS_TEST_ERROR("key:%s %d,dthr_num(%d>=%d) fail", key_name, i, thr_pos, node_num);
					break;
				}
