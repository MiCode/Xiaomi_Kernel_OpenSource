...
/* N17 code for HQ-329479 by huangshiquan at 2023/09/16 start */
                if (ret < 0) {
                    FTS_ERROR("fw upgrade fail, enter retry!");
                } else {
    ...
    if (FTS_GET_MODULE_NUM > 1) {
	    ...
	    /* N17 code for HQ-310974 by xionglei6 at 2023/08/14 start */
            if (tp_vendor) {
                ...
                } else if (strcmp(tp_vendor, FTS_MODULE2_NAME) == 0) {
                    info = &module_list[FTS_TianMa_PANLE_INDEX];
                    upg->module_id = info->id;
                    FTS_INFO("focaltech tp, vendor is %s !!!", tp_vendor);
                } else {
...
static int fts_get_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
    ...
    if (upg->ts_data->lockdown_info[7] == 0x01) {
        snprintf(fwname, FILE_NAME_LENGTH, "%s%s_mask0x%02x.bin", \
             ...
             upg->module_info->vendor_name, upg->ts_data->lockdown_info[7]);
    } else {
