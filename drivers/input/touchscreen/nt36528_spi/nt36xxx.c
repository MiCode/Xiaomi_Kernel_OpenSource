...
bool tp_sensor_probe_switch = false;
...
static int lct_nvt_tp_proximity_switch_callback(bool flag);
...
extern void set_gesture_lcd_reset_gpio_keep_high_c(bool en);
...
int lct_nvt_tp_proximity_switch_callback(bool flag)
{
        int rc = 0;
        if (flag) {
                rc = nvt_set_proximity_switch(1);
                NVT_LOG("nvt enable proximity mode rc = %d\n",rc);
        } else {
                rc = nvt_set_proximity_switch(0);
                NVT_LOG("nvt disable proximity mode rc = %d\n",rc);
        }
        return 0;
}
#if WAKEUP_GESTURE
...
info_retry:
	...
	sprintf(tp_info_buf, "[Vendor]xinli,[FW]0x%02x,[IC]nt36528\n", ts->fw_ver);
...
int32_t nvt_check_proximity(uint8_t input_id, uint8_t *data)
{
	...
	if ((input_id == DATA_PROTOCOL) && (func_type == FUNCPAGE_PROXIMITY)) {
		...
		if (proximity_state == PROXIMITY_ON) {
			tp_ps_data = PROX_NEAR;
			rc = mtk_ipi_send(&scp_ipidev, IPI_OUT_TP_PS, 0, &tp_ps_data, 1, 0);
			NVT_LOG("get proximity on near event, rc =%d level=%d.\n", rc, proximity_level);
		} else if (proximity_state == PROXIMITY_OFF) {
			tp_ps_data = PROX_FAR;
			rc = mtk_ipi_send(&scp_ipidev, IPI_OUT_TP_PS, 0, &tp_ps_data, 1, 0);
			NVT_LOG("get proximity on far event, rc =%d level=%d.\n", rc, proximity_level);
		} else {
			// should never go here
...
static int lct_tp_set_screen_angle_callback(int angle)
{
	...
	for (i = 0; i < retry; i++) {
		...
		tmp[0] = EVENT_MAP_HOST_CMD;
		tmp[1] = 0xFF;
		CTP_SPI_READ(ts->client, tmp, 2);
		if (tmp[1] == 0x00)
			break;
	}
	if (unlikely(i >= retry)) {
		NVT_ERR("send cmd failed, tmp[1] = 0x%02X\n", tmp[1]);
		ret = -1;
	} else {
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	int32_t retry = 0;
#endif
	tp_sensor_probe_switch = false;

#if IS_ENABLED(NVT_DRM_PANEL_NOTIFY) || IS_ENABLED(NVT_QCOM_PANEL_EVENT_NOTIFY)
	dp = client->dev.of_node;

	NVT_LOG("check dt start\n");
...
#endif
	ret = init_lct_tp_proximity_switch(lct_nvt_tp_proximity_switch_callback);
	if (ret < 0) {
		NVT_ERR("init_lct_tp_proximity_switch Failed!\n");
		goto err_init_lct_tp_proximity_switch_failed;
	} else {
		NVT_LOG("init_lct_tp_proximity_switch Succeeded!\n");
	}
...
#endif
	ret = init_lct_tp_selftest(lct_nvt_tp_selftest_callback);
	if (ret < 0) {
		NVT_ERR("init_lct_tp_selftest Failed!");
		goto err_init_lct_tp_selftest_fail;
	} else {
		NVT_LOG("init_lct_tp_selftest Succeeded!");
	}
#if LCT_TP_GRIP_AREA_EN
...
#endif
	ts->sensor_tp_probe_status = true;
        NVT_LOG("sensor_tp_probe_status_end = %d\n",ts->sensor_tp_probe_status);
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	msleep(50);
	if(proximity_event) {
        nvt_set_proximity_power_saving_switch(0);
...
int lct_nvt_tp_gesture_callback(bool flag)
{
	...
	if (flag) {
		...
		if(tp_lcmname == 1) {
			NVT_LOG("tp_lcmname_gesture = %d\n",tp_lcmname);
			set_lcd_reset_gpio_keep_high(true);
		} else if (tp_lcmname == 3) {
			NVT_LOG("tp_lcmname_gesture = %d\n",tp_lcmname);
			set_gesture_lcd_reset_gpio_keep_high_c(true);
		} else {
			NVT_LOG("tp_lcmname_gesture = %d\n",tp_lcmname);
			return 0;
		}
		...
		if(tp_lcmname == 1) {
			...
			set_lcd_reset_gpio_keep_high(false);
                        ...
                        NVT_LOG("tp_lcmname_gesture = %d\n",tp_lcmname);
                        set_gesture_lcd_reset_gpio_keep_high_c(false);
                } else {
                        NVT_LOG("tp_lcmname_gesture = %d\n",tp_lcmname);
                        return 0;
                }
		...
		unsigned long event, void *data)
{
    ...
    if (event == MTK_DISP_EARLY_EVENT_BLANK) {
        if (*blank == MTK_DISP_BLANK_POWERDOWN) {
			NVT_LOG("event=%lu, *blank=%d\n", event, *blank);
			NVT_LOG("proximity_event = %d\n", proximity_event);
			if(!proximity_event){
				nvt_ts_suspend(&ts->client->dev);
			} else {
				mutex_lock(&ts->lock);
				nvt_set_proximity_power_saving_switch(1);
				mutex_unlock(&ts->lock);
				bTouchIsAwake = 0;
#if MT_PROTOCOL_B
				for (i = 0; i < ts->max_touch_num; i++) {
...
static int32_t __init nvt_driver_init(void)
{
	...
	if (lcm_name) {
		...
		NVT_LOG("nvt_driver_init read lcm name : %s", videolfb_tag->lcmname);
		if (strcmp("dsi_panel_c3v_43_02_0a_hd_vdo",videolfb_tag->lcmname) == 0) {
			tp_lcmname = 1;
			NVT_LOG("nt36528-1");
		}
                else if(strcmp("dsi_panel_c3v_43_02_0c_hd_vdo",videolfb_tag->lcmname) == 0){
			tp_lcmname = 3;
                        NVT_LOG("nt36528-3");
                }
                else {
				NVT_ERR("Unkown Touch");
				return -1;
		}
	}
