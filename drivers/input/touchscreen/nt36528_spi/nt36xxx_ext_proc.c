...
#define NVT_PROXIMITY_POWER_SAVING_SWITCH "nvt_proximity_power_saving_switch"
...
extern uint32_t tp_lcmname;
extern void set_prox_lcd_reset_gpio_keep_high_c(bool en);
...
static struct proc_dir_entry *NVT_proc_proximity_power_saving_switch_entry;
...
int32_t nvt_set_proximity_switch(uint8_t proximity_switch)
{
	...
	for (i = 0; i < retry; i++) {
		...
		if (proximity_switch == 0) {
			// proximity disable
			if(tp_lcmname == 1) {
				NVT_LOG("tp_lcmname = %d\n",tp_lcmname);
				set_prox_lcd_reset_gpio_keep_high(false);
			} else if (tp_lcmname == 3) {
				NVT_LOG("tp_lcmname = %d\n",tp_lcmname);
				set_prox_lcd_reset_gpio_keep_high_c(false);
			} else {
				NVT_LOG("tp_lcmname = %d\n",tp_lcmname);
				return 0;
			}
			...
			if(tp_lcmname == 1) {
				set_prox_lcd_reset_gpio_keep_high(true);
                                ...
                                NVT_LOG("tp_lcmname = %d\n",tp_lcmname);
                                set_prox_lcd_reset_gpio_keep_high_c(true);
                        } else {
                                NVT_LOG("tp_lcmname = %d\n",tp_lcmname);
                                return 0;
                        }
...
int32_t nvt_set_proximity_power_saving_switch(uint8_t proximity_power_saving_switch)
{
	uint8_t buf[8] = {0};
	int32_t ret = 0;
	int32_t i = 0;
	...
	NVT_LOG("++\n");
	NVT_LOG("set proximity power saving switch: %d\n", proximity_power_saving_switch);
	...
	for (i = 0; i < retry; i++) {
		buf[0] = EVENT_MAP_HOST_CMD;
		if (proximity_power_saving_switch == 0) {
			// proximity power saving mode disable
			buf[1] = 0x89;
			buf[2] = 0x02;
		} else if (proximity_power_saving_switch == 1) {
			// proximity power saving mode enable
			buf[1] = 0x89;
			buf[2] = 0x01;
		} else {
			NVT_ERR("Invalid value! proximity_power_saving_switch = %d\n", proximity_power_saving_switch);
			ret = -EINVAL;
			goto out;
		}
...
static ssize_t nvt_proximity_power_saving_switch_proc_write(struct file *filp,const char __user *buf, size_t count, loff_t *f_pos)
{
	int32_t ret;
	int32_t tmp;
	uint8_t proximity_power_saving_switch;
	char *tmp_buf;
	...
	proximity_power_saving_switch = (uint8_t)tmp;
	NVT_LOG("proximity_power_saving_switch = %d\n", proximity_power_saving_switch);
	...
	nvt_set_proximity_power_saving_switch(proximity_power_saving_switch);
...
#ifdef HAVE_PROC_OPS
static const struct proc_ops nvt_proximity_power_saving_switch_fops = {
        .proc_write = nvt_proximity_power_saving_switch_proc_write,
};
#else
static const struct file_operations nvt_proximity_power_saving_switch_fops = {
        .owner = THIS_MODULE,
        .write = nvt_proximity_power_saving_switch_proc_write,
};
#endif
...
int32_t nvt_extra_proc_init(void)
{
	...
	NVT_proc_proximity_power_saving_switch_entry = proc_create(NVT_PROXIMITY_POWER_SAVING_SWITCH, 0666, NULL, &nvt_proximity_power_saving_switch_fops);
	if (NVT_proc_proximity_power_saving_switch_entry == NULL) {
		NVT_ERR("create proc/%s Failed!\n", NVT_PROXIMITY_POWER_SAVING_SWITCH);
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/%s Succeeded!\n", NVT_PROXIMITY_POWER_SAVING_SWITCH);
	}
...
void nvt_extra_proc_deinit(void)
{
	...
	if (NVT_proc_proximity_power_saving_switch_entry != NULL) {
		remove_proc_entry(NVT_PROXIMITY_POWER_SAVING_SWITCH, NULL);
		NVT_proc_proximity_power_saving_switch_entry = NULL;
		NVT_LOG("Removed /proc/%s\n", NVT_PROXIMITY_POWER_SAVING_SWITCH);
	}
}
#endif
