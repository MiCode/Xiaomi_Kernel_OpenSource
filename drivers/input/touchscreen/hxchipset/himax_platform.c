				   ...
				   struct himax_i2c_platform_data *pdata)
{
	...
	pdata->tddi_reset = of_get_named_gpio(dt, "himax,rst-tddi", 0);
	if (!gpio_is_valid(pdata->tddi_reset)) {
		I(" DT:tddi_reset value is not valid\n");
	}
...
int i2c_himax_write(struct i2c_client *client, uint8_t command, uint8_t *data, uint8_t length, uint8_t toRetry)
{
	...
	if (retry == toRetry) {
		E("%s: i2c_write_block retry over %d, ret:%d\n",
...
{
	int retval;
	pdata->vddio = regulator_get(&client->dev,
								   "vddio");
	if (IS_ERR(pdata->vddio)) {
		E("%s: Failed to get regulator vddio\n",
		  __func__);
		retval = PTR_ERR(pdata->vddio);
		return retval;
	}
	pdata->lab = regulator_get(&client->dev,
								   "lab");
	if (IS_ERR(pdata->lab)) {
		E("%s: Failed to get regulator lab\n",
		  __func__);
		retval = PTR_ERR(pdata->lab);
		regulator_put(pdata->lab);
		return retval;
	}
	pdata->ibb = regulator_get(&client->dev,
								   "ibb");
	if (IS_ERR(pdata->ibb)) {
		E("%s: Failed to get regulator ibb\n",
		  __func__);
		retval = PTR_ERR(pdata->ibb);
		regulator_put(pdata->ibb);
		return retval;
	}
...
static int himax_power_on(struct himax_i2c_platform_data *pdata, bool on)
{
	...
	if (on) {
		retval = regulator_enable(pdata->vddio);
		if (retval) {
			E("%s: Failed to enable regulator vddio\n",
			  __func__);
			return retval;
		}
		msleep(100);
		retval = regulator_enable(pdata->lab);
		if (retval) {
			E("%s: Failed to enable regulator lab\n",
			  __func__);
			regulator_disable(pdata->lab);
			return retval;
		}
		retval = regulator_enable(pdata->ibb);
		if (retval) {
			E("%s: Failed to enable regulator ibb\n",
			  __func__);
			regulator_disable(pdata->ibb);
			return retval;
		}
...
static int himax_power(int enable)
{
	struct himax_i2c_platform_data *pdata;

	if (!private_ts || !private_ts->pdata)
		return -ENOMEM;

	pdata = private_ts->pdata;

	return himax_power_on(pdata, enable);
}
...
/*
	if (gpio_is_valid(pdata->tddi_reset)) {
		error = gpio_request(pdata->tddi_reset, "tddi_reset_gpio");
		if (error) {
			E("unable to request tddi gpio [%d]\n",
			  pdata->tddi_reset);
			goto err_regulator_on;
		}

		error = gpio_direction_output(pdata->tddi_reset, 0);
		if (error) {
			E("unable to set direction for gpio [%d]\n",
			  pdata->tddi_reset);
			goto err_gpio_reset_req;
...
/*
	if (gpio_is_valid(pdata->tddi_reset)) {
		error = gpio_direction_output(pdata->tddi_reset, 1);
		if (error) {
			E("unable to set direction for tddi gpio [%d]\n",
			  pdata->tddi_reset);
			goto err_gpio_irq_req;
		}
	}
						 ...
						 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == DRM_EARLY_EVENT_BLANK && ts &&
			ts->client) {
		blank = evdata->data;
	...
	} else if (evdata && evdata->data && event == DRM_EVENT_BLANK && ts &&
			ts->client) {
		blank = evdata->data;
