...
extern char self_test_temp_buf[512];
									...
									size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (private_ts->selftest_type == 2) {
			val = himax_chip_self_test_short(private_ts->client);
		} else if (private_ts->selftest_type == 0xff) {
			val = himax_chip_self_test(private_ts->client);
			val = private_ts->selftest_type;
		} else {
			val = himax_chip_self_test_open(private_ts->client);
		}
		...
		if (val == 0x02) {
			...
			ret += snprintf(temp_buf + ret, len - ret, "%s", &self_test_temp_buf[0]);
		} else if (val == 0x01) {
			ret += snprintf(temp_buf + ret, len - ret, "Self_Test Fail\n");
			ret += snprintf(temp_buf + ret, len - ret, "%s", &self_test_temp_buf[0]);
		} else if (val == private_ts->selftest_type) {
			ret += snprintf(temp_buf + ret, len - ret, "Self_Test Finish\n");
			ret += snprintf(temp_buf + ret, len - ret, "%s", &self_test_temp_buf[0]);
			
		} else {
			ret += snprintf(temp_buf + ret, len - ret, "Self_Test invalid\n");
			ret += snprintf(temp_buf + ret, len - ret, "%s", &self_test_temp_buf[0]);
		}
										...
										size_t len, loff_t *pos)
{
	...
	if ((buf[0] == 'o') && (buf[1] == 'p') && (buf[2] == 'e') && (buf[3] == 'n')) {
		private_ts->selftest_type = 0;
	} else if ((buf[0] == 's') && (buf[1] == 'h') && (buf[2] == 'o') && (buf[3] == 'r') && (buf[4] == 't')) {
		private_ts->selftest_type = 2;
	} else if ((buf[0] == 'a') && (buf[1] == 'l') && (buf[2] == 'l')) {
		private_ts->selftest_type = 0xff;
	} else {
		/* Default is Open Test */
		private_ts->selftest_type = 0;
	}
	return len;
}
									 ...
									 size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		for (j = 0; j < 4; j++) {
			ret += snprintf(temp_buf + ret, len - ret, "g_guest_str%d:\n", j);
			for (i = 0; i < 128; i++) {
				if (i % 16 == 0 && i > 0)
					ret += snprintf(temp_buf + ret, len - ret, "\n");
				ret += snprintf(temp_buf + ret, len - ret, "0x%02X\t", g_guest_str[j][i]);

			}
			ret += snprintf(temp_buf + ret, len - ret, "\n");
			I("str[%d] %s\n", j, g_guest_str[j]);
		}
		ret += snprintf(temp_buf + ret, len - ret, "g_guest_str8:\n");
		for (i = 0; i < 128; i++) {
			if (i % 16 == 0 && i > 0)
				ret += snprintf(temp_buf + ret, len - ret, "\n");
			ret += snprintf(temp_buf + ret, len - ret, "0x%02X\t", g_guest_str[8][i]);
		...
		ret += snprintf(temp_buf + ret, len - ret, "g_guest_str9:\n");
		for (i = 0; i < 128; i++) {
			if (i % 16 == 0 && i > 0)
				ret += snprintf(temp_buf + ret, len - ret, "\n");
			ret += snprintf(temp_buf + ret, len - ret, "0x%02X\t", g_guest_str[9][i]);
		}
		ret += snprintf(temp_buf + ret, len - ret, "\n");
...
static ssize_t himax_lock_down_read(struct file *file, char *buf,
									size_t len, loff_t *pos)
{
	ssize_t ret = 0;
	char *temp_buf;
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		ret += snprintf(temp_buf + ret, len - ret, "lock down info = %2.2X%2.2X%2.2X%2.2X%2.2X%2.2X%2.2X%2.2X \n"
...
static struct file_operations himax_proc_lock_down_ops = {
	.owner = THIS_MODULE,
	.read = himax_lock_down_read,
};
#endif
...
#ifdef HX_TP_PROC_SELF_TEST
	himax_proc_self_test_file = proc_create(HIMAX_PROC_SELF_TEST_FILE, (S_IWUSR|S_IRUGO),
...
#ifdef HX_TP_PROC_LOCK_DOWN
	himax_proc_lock_down_file = proc_create(HIMAX_PROC_LOCK_DOWN_FILE, (S_IRUGO),
											himax_touch_proc_dir, &himax_proc_lock_down_ops);
	if (himax_proc_lock_down_file == NULL) {
		E(" %s: proc lockdown file create failed!\n", __func__);
		goto fail_12;
	}
#endif
...
#endif
#ifdef HX_TP_PROC_LOCK_DOWN
	remove_proc_entry(HIMAX_PROC_LOCK_DOWN_FILE, himax_touch_proc_dir);
fail_12:
#endif
#ifdef HX_TP_PROC_SELF_TEST
