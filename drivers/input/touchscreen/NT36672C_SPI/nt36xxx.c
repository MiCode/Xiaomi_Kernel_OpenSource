...
/*N19 code for HQ-354407 by liaoxianguo at 2023/12/15 start*/
#define DISP_ID_PIN                (375+34)
enum TOUCH_SELECT {
    TOUCH_SELECT_FOCAL = 0,
    TOUCH_SELECT_NOVATEK = 1,
};
/*N19 code for HQ-354407 by liaoxianguo at 2023/12/15 end*/
...
{
	nvt_get_fw_info();
	sprintf(tp_version_info, "[Vendor]Tianma [TP-IC]:NT36672C [FW]0x%x\n", tp_fw_version);
...
static void nvt_esd_check_func(struct work_struct *work)
{
	...
	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && esd_check) {
		mutex_lock(&ts->lock);
/* N19 code for HQ-376265 by liaoxianguo at 2024/3/20 start */
		nvt_read_fw_history_all();
/* N19 code for HQ-376265 by liaoxianguo at 2024/3/20 end */
...
int32_t nvt_check_proximity(uint8_t input_id, uint8_t *data)
{
	...
	if ((input_id == DATA_PROTOCOL) && (func_type == FUNCPAGE_PROXIMITY)) {
		if (proximity_state == PROXIMITY_ON) {
			proximity_status = proximity_level;
...
/* N19 code for HQ-369694 by liaoxianguo at 2024/1/31 start */
#if NVT_TOUCH_ESD_PROTECT
		/* update interrupt timer */
		irq_timer = jiffies;
#endif
/* N19 code for HQ-369694 by liaoxianguo at 2024/1/31 end */
		if (proximity_status != g_priximity_data) {
			NVT_LOG("proximity status is %d, get proximity event.\n",proximity_status);
...
static void get_lockdown_info(struct work_struct *work)
{
	...
	if (!IS_ERR(lockdown_ptr) && item_size > 0) {
		memcpy(ts->lockdown_info, lockdown_ptr, item_size);
...
/* N19 code for HQ-351663 by p-luozhibin1 at 2023.1.15 end */
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		...
		if (((point_data[position] & 0x07) == 0x01) ||
			...
			if ((input_x > (TOUCH_MAX_WIDTH - 1)) ||
			    (input_y > (TOUCH_MAX_HEIGHT - 1)))
				continue;
...
/* N19 code for HQ-351663 by p-luozhibin1 at 2023.1.15 end */
	...
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0,
			     TOUCH_MAX_WIDTH - 1, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0,
			     TOUCH_MAX_HEIGHT - 1, 0, 0);
#if MT_PROTOCOL_B
...
/* N19 code for HQ-370728 by liaoxianguo at 2024/2/20 end */
#if IS_ENABLED(CONFIG_XIAOMI_PANEL_NOTIFIER)
	ts->xiaomi_panel_notif.notifier_call =
		nvt_xiaomi_panel_notifier_callback;
...
/*N19 code for HQ-354407 by liaoxianguo at 2023/12/15 start*/
    ...
    NVT_LOG("flag = %d\n", flag);
    if (flag == TOUCH_SELECT_NOVATEK) {
        NVT_LOG("TP is novatek\n");
    } else {
        NVT_LOG("TP is not novatek\n");
        return 0;
    }
/*N19 code for HQ-354407 by liaoxianguo at 2023/12/15 end*/
