...
*/
static ssize_t fts_fw_ver_config_id_show(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	int error;
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (ret < OK) {
		...
		logError(1,
			 "%s fw_version = %04X, config_version = %04X, size = %d bytes\n",
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (numberParameters >= 1) {
		...
		case 0x00:
			...
			if (systemInfo.u8_fwCfgAfeVer !=
			    systemInfo.u8_fwCxAfeVer) {
				res = ERROR_OP_NOT_ALLOW;
				logError(0,
...
static void fts_event_handler(struct work_struct *work)
{
	...
	for (count = 0; count < FIFO_DEPTH; count++) {
		error =
		    fts_writeReadU8UX(regAdd, 0, 0, data, FIFO_EVENT_SIZE,
				      DUMMY_FIFO);
...
int fts_fw_update(struct fts_ts_info *info, const char *fw_name, int force)
{
	...
	if ((init_type == NO_INIT)) {
		if (systemInfo.u8_fwCfgAfeVer != systemInfo.u8_fwCxAfeVer) {
			init_type = SPECIAL_FULL_PANEL_INIT;
			logError(0,
				 "%s %s: Different cfg afe and cx afe version: %02X != %02X... Execute FULL Panel Init! \n",
			...
			init_type = SPECIAL_PANEL_INIT;
			logError(0,
				 "%s %s: Different Panel Ver: %02X != %02X... Execute Panel Init! \n",
				 tag, __func__, systemInfo.u8_fwCfgAfeVer,
				 systemInfo.u8_panelCfgAfeVer);
		} else {
			init_type = NO_INIT;
...
static void fts_switch_mode_work(struct work_struct *work)
{
	...
	static const char *fts_gesture_on = "01 10";
	char *gesture_result;
	int size = 6 * 2 + 1;
	char ch[16] = { 0x0, };
...
static int fts_probe(struct spi_device *client)
{
	...
	error = fts_fw_update(info, NULL, 1);
	if (error < OK) {
		logError(1,
