...
static void fts_set_grip_rect(int *buf)
{
	...
	x_end = *(buf + 4);
	y_end = *(buf + 5);
	logError(1, "%s grip_type:%d, grip_pos:%d,startpos[%d, %d],endpos:[%d, %d]\n", tag,
...
static int fts_set_mode_long_value(int mode, int len, int *buf)
{
	...
	if (len == 0) {
		...
		logError(1, "%s %s too many parameters!", tag, __func__);
		return -EINVAL;
	}
	...
	for (i = 0; i < len; i++) {
		...
		logError(0,"%s %s buf i: %d, value: %d \n" , tag, __func__, i, buf[i]);
	}
...
static int parse_gamemode_dt(struct device *dev, struct fts_hw_platform_data *bdata)
{
	...
	if (of_find_property(np, "fts,touch-edgezone-filter-hor", &byte_len)) {
		...
		logError(1, "%s %s parameters:  %d,  %d, %d, %d, %d, %d, %d, %d \n", tag, __func__,
