...
int production_test_ms_cx(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	if ((todo->MutualKeyCx1 | todo->MutualKeyCx2 | todo->
	     MutualKeyCxTotal) == 1) {
		ret =
...
int production_test_ss_raw(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	if (todo->SelfForceRaw == 1 || todo->SelfForceRawGap == 1) {
		...
		if (todo->SelfForceRaw == 1) {
			...
			if (ret != OK) {
				logError(1,
					 "%s production_test_data: checkLimitsMinMax SS RAW (PROXIMITY) FORCE failed... ERROR COUNT = %d \n",
					 tag, ret);
				logError(0,
		...
		if (todo->SelfForceRawGap == 1) {
			...
			if (ret != OK) {
				logError(1,
					 "%s production_test_data: checkLimitsGap SS RAW (PROXIMITY) FORCE GAP failed... ERROR = %08X \n",
					 tag, ret);
				logError(0,
					 "%s SS RAW (PROXIMITY) FORCE GAP TEST:.................FAIL \n\n",
					 tag);
				count_fail += 1;
				print_frame_short("SS Raw force frame =",
						  array1dTo2d_short(ssRawFrame.
	...
	if (todo->SelfSenseRaw == 1 || todo->SelfSenseRawGap == 1) {
		...
		if (todo->SelfSenseRaw == 1) {
			...
			if (ret != OK) {
				logError(1,
					 "%s production_test_data: checkLimitsMinMax SS RAW (PROXIMITY) SENSE failed... ERROR COUNT = %d \n",
					 tag, ret);
				logError(0,
					 "%s SS RAW (PROXIMITY) SENSE MIN MAX TEST:.................FAIL \n",
					 tag);
				count_fail += 1;
		...
		if (todo->SelfSenseRawGap == 1) {
			...
			if (ret != OK) {
				logError(1,
					 "%s production_test_data: checkLimitsGap SS RAW (PROXIMITY) SENSE GAP failed... ERROR = %08X \n",
					 tag, ret);
				logError(0,
					 "%s SS RAW (PROXIMITY) SENSE GAP TEST:.................FAIL \n",
					 tag);
				count_fail += 1;
				print_frame_short("SS Raw sense frame =",
						  array1dTo2d_short(ssRawFrame.
			     ...
			     TestToDo *todo)
{
	...
	if (todo->SelfForceCxTotal == 1 || todo->SelfForceCxTotalAdj == 1) {
		...
		if (todo->SelfForceCxTotalAdj == 1) {
			...
			ret = parseProductionTestLimits(path_limits, &limit_file, SS_TOTAL_CX_FORCE_ADJV_MAP_MAX, &thresholds_max, &trows, &tcolumns);
			if (ret < 0
			    || (trows != totCompData.header.force_node - 1
				|| tcolumns != 1)) {
				logError(1,
