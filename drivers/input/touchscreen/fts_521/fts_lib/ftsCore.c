...
int readSysInfo(int request)
{
	...
	u8ToU16(&data[index], &systemInfo.u16_svn_ver);
	index += 2;

	logError(1, "%s FW ID = %04X \n", tag, systemInfo.u16_svn_ver);
	...
	systemInfo.u8_fwCxAfeVer = data[index++];
	systemInfo.u8_panelCfgAfeVer = data[index++];
	logError(1, "%s AFE VER CFG = %02X- AFE VER CX = %02X- AFE VER PANEL = %02X \n", tag, systemInfo.u8_fwCfgAfeVer, systemInfo.u8_fwCxAfeVer, systemInfo.u8_panelCfgAfeVer);
	...
	logError(1, "%s FW CRC=  ", tag);
	for (i = 0; i < 4; i++) {
		systemInfo.u8_fwcrc[i] = data[index++];
		logError(1, "%02X ", systemInfo.u8_fwcrc[i]);
	}
	logError(1, "\n");

	logError(1, "%s CFG CRC=  ", tag);
	for (i = 0; i < 4; i++) {
		systemInfo.u8_fwcfg[i] = data[index++];
		logError(1, "%02X ", systemInfo.u8_fwcfg[i]);
	}
	logError(1, "\n");
	index += 16;
...
int fts_crc_check()
{
	...
	u8 error_to_search[6] = {
		...
		EVT_TYPE_ERROR_CRC_CX, EVT_TYPE_ERROR_CRC_CX_HEAD, EVT_TYPE_ERROR_CRC_CX_SUB,
		EVT_TYPE_ERROR_CRC_CX_SUB_HEAD
	};

	res = fts_writeReadU8UX(FTS_CMD_HW_REG_R, ADDR_SIZE_HW_REG, ADDR_CRC, &val, 1, DUMMY_HW_REG);
	if (res < OK) {
...
int readLockDownInfo(u8 *lockData, u8 lock_id, int size)
{
	...
	datatemp = printHex("Lockdown Code = ", lockData, size);
	if (datatemp != NULL) {
