...
/*for some projects, we do not use mtk framebuffer*/
	...
	tpd_mode_keypad_tolerance = TPD_RES_X * TPD_RES_X / 3040;
	...
	if (g_tpd_drv == NULL) {
		if (tpd_driver_list[0].tpd_device_name != NULL) {
			...
			pr_debug("Generic touch panel driver\n");
		} else {
			pr_debug("no touch driver is loaded!!\n");
			return 0;
		}
	}
...
#endif /* CONFIG_MTK_S3320 */
		pr_info("Cap touch panel driver\n");
	}
	...
	if (input_register_device(tpd->dev))
		pr_info("input_register_device failed.(tpd)\n");
	else
		tpd_register_flag = 1;
