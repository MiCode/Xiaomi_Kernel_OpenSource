...
struct mutex lct_tp_selftest_rw_mutex;
///////////////////////////////////////////////////////////////////////////
...
static ssize_t ctp_selftest_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	if((strncmp(tmp_data,"mmi",3) == 0) && (is_in_self_test == 0)) {
	...
	} else if((strncmp(tmp_data,"i2c",3) == 0) && (is_in_self_test == 0)) {
		printk("ctp_selftest_proc_write start \n");
		mutex_lock(&lct_tp_selftest_rw_mutex);
		strcpy(ft_ctp_selftest_status,"Testing");
		is_in_self_test = 1;
		retry_count = CTP_FELF_TEST_RETROY_COUNT;
		queue_work(ctp_selftest_workqueue, &ctp_selftest_work);
	} else if((strncmp(tmp_data,"open",4) == 0) && (is_in_self_test == 0)) {
		printk("ctp_selftest_proc_write start \n");
		mutex_lock(&lct_tp_selftest_rw_mutex);
		strcpy(ft_ctp_selftest_status,"Testing");
		is_in_self_test = 1;
		retry_count = CTP_FELF_TEST_RETROY_COUNT;
		queue_work(ctp_selftest_workqueue, &ctp_selftest_work);
	} else if((strncmp(tmp_data,"short",5) == 0) && (is_in_self_test == 0)) {
		printk("ctp_selftest_proc_write start \n");
		mutex_lock(&lct_tp_selftest_rw_mutex);
		strcpy(ft_ctp_selftest_status,"Testing");
		is_in_self_test = 1;
