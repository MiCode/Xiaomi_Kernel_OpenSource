...
int32_t nvt_check_proximity(uint8_t input_id, uint8_t *data)
{
	...
	uint8_t proximity_state = data[3];
  	unsigned int proximity_level = data[4];
	if ((input_id == DATA_PROTOCOL) && (func_type == FUNCPAGE_PROXIMITY)) {
		...
		if (proximity_state == PROXIMITY_ON) {
			...
			NVT_LOG("get proximity on near event, level=%d.\n",proximity_level);
			...
			NVT_LOG("get proximity on far event,level=%d.\n",proximity_level);
		} else {
	...
	bTouchIsAwake = 1;
 	...
 	if(proximity_event) {
		if (disable_irq_wake(ts->client->irq)) {
			NVT_ERR("disable_irq_wake(irq:%d) fail", ts->client->irq);
        }
...
static int32_t __init nvt_driver_init(void)
{
	...
	NVT_LOG("start\n");
       ...
       if (lcm_name) {
		...
		if (strcmp("dsi_panel_c3y_43_02_0a_hd_video",videolfb_tag->lcmname) == 0) {
			NVT_LOG("nt36528");
		}
		else {
