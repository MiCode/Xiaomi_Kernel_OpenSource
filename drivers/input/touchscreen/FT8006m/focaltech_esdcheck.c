...
int idc_esdcheck_lcderror(void)
{
	...
	if ((tp_need_recovery == 1) && (lcd_need_reset == 0)) {
		...
		ft8006m_tp_state_recovery(ft8006m_i2c_client);
	}

	ret = ft8006m_i2c_read_reg(ft8006m_i2c_client, FTS_REG_ESD_SATURATE, &val);
	if (ret < 0) {
...
static bool get_chip_id(void)
{
	...
	for (i = 0; i < 3; i++) {
		...
		err = ft8006m_i2c_read(ft8006m_i2c_client, &reg_addr, 1, &reg_value, 1);

		if (err < 0) {
			FTS_ERROR("[ESD]: Read Reg 0xA3 failed ret = %d!!", err);
			fts_esdcheck_data.i2c_nack_cnt++;
		} else {
			if ((reg_value == ft8006m_chip_types.chip_idh) || (reg_value == 0xEF)) /* Upgrade sometimes can't detect */
				break;
			else
...
/*****************************************************************************
*  Name: ft8006m_esdcheck_set_intr
...
/*****************************************************************************
*  Name: ft8006m_esdcheck_get_status(void)
...
/*****************************************************************************
*  Name: ft8006m_esdcheck_proc_busy
