...
static void fts_psensor_enable(struct fts_ts_data *data, int enable)
{
	...
	ft8006m_i2c_read_reg(data->client, FTS_REG_PSENSOR_ENABLE, &state);
	if (enable)
		state |= FTS_PSENSOR_ENABLE_MASK;
	else
		state &= ~FTS_PSENSOR_ENABLE_MASK;

	ret = ft8006m_i2c_write_reg(data->client, FTS_REG_PSENSOR_ENABLE, state);
	if (ret < 0)
...
static int fts_read_tp_psensor_data(struct fts_ts_data *data)
{
	...
	ft8006m_i2c_read_reg(data->client,
