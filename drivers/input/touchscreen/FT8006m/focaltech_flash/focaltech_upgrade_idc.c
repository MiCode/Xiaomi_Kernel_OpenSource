...
/************************************************************************
* Name: ft8006m_ctpm_upgrade_idc_init
* Brief:
* Input:
* Output:
* Return:
...
int ft8006m_ctpm_upgrade_idc_init(struct i2c_client *client)
{
	...
	reg_val_id[0] = 0x00;
	i_ret = ft8006m_i2c_read(client, auc_i2c_write_buf, 1, reg_val_id, 1);
	if (i_ret < 0) {
		return -EIO;
	}
	...
	ft8006m_i2c_write(client, auc_i2c_write_buf, 3);
...
/************************************************************************
* Name: ft8006m_ctpm_start_pramboot
* Brief:
* Input:
* Output:
* Return:
...
/************************************************************************
* Name: ft8006m_ctpm_start_fw_upgrade
* Brief:
* Input:
* Output:
* Return:
...
int ft8006m_ctpm_start_fw_upgrade(struct i2c_client *client)
{
	...
	msleep(10);
	i_ret = ft8006m_i2c_write_reg(client, FTS_RST_CMD_REG1, FTS_UPGRADE_55);
	msleep(200);
...
/************************************************************************
* Name: ft8006m_ctpm_check_run_state
* Brief:
* Input:
* Output:
* Return:
***********************************************************************/
bool ft8006m_ctpm_check_run_state(struct i2c_client *client, int rstate)
{
	int i = 0;
...
/************************************************************************
* Name: ft8006m_ctpm_pramboot_ecc
* Brief:
* Input:
* Output:
* Return:
...
int ft8006m_ctpm_pramboot_ecc(struct i2c_client *client)
{
	...
	if (reg_val[0] != ft8006m_upgrade_ecc) { /*pramboot checksum != host checksum, upgrade fail*/
...
int ft8006m_ctpm_upgrade_ecc(struct i2c_client *client, u32 startaddr, u32 length)
{
	...
	i_ret = ft8006m_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 1);
	if (reg_val[0] != ft8006m_upgrade_ecc) /*if check sum fail, upgrade fail*/ {
		FTS_ERROR("[UPGRADE]: ecc error! FW=%02x ft8006m_upgrade_ecc=%02x!!", reg_val[0], ft8006m_upgrade_ecc);
		return -EIO;
	}
...
int ft8006m_ctpm_write_pramboot_for_idc(struct i2c_client *client, u32 length, u8 *readbuf)
{
	...
	for (j = 0; j < packet_number; j++) {
		...
		for (i = 0; i < temp; i++) {
			...
			ft8006m_upgrade_ecc ^= packet_buf[6 + i];
		}
...
/************************************************************************
* Name: ft8006m_ctpm_write_app_for_idc
* Brief:
* Input:
* Output:
* Return:
...
int ft8006m_ctpm_write_app_for_idc(struct i2c_client *client, u32 length, u8 *readbuf)
{
	...
	for (j = 0; j < packet_number; j++) {
		...
		ft8006m_i2c_write(client, packet_buf, (writelenght + 6));
		...
		for (i = 0; i < 30; i++) {
			...
			ft8006m_ctpm_upgrade_delay(1000);
		}
	}
	msleep(50);
...
* Return:
*****************************************************************************/
u16 ft8006m_crc_calc(u8 *pbt_buf, u32 addr, u16 length)
{
	u16 cFcs = 0;
	u16 i, j;
...
* Return:
*****************************************************************************/
bool ft8006m_ecc_check(u8 *pbt_buf, u32 star_addr, u32 len, u16 ecc_addr)
{
	u16 ecc1;
	u16 ecc2;
	u16 cal_ecc;
...
/*****************************************************************************
* Name: ft8006m_check_app_bin_valid_idc
* Brief:
* Input:
* Output:
* Return:
