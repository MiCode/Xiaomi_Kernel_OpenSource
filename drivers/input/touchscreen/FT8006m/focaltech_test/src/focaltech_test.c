...
extern struct i2c_client* ft8006m_i2c_client;
extern int ft8006m_i2c_read(struct i2c_client *client, char *writebuf, int writelen, char *readbuf, int readlen);
extern int ft8006m_i2c_write(struct i2c_client *client, char *writebuf, int writelen);
#endif
static int fts_test_i2c_read(unsigned char *writebuf, int writelen, unsigned char *readbuf, int readlen)
{
    int iret = -1;
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    if (NULL != printdata) Ft8006m_fts_free(printdata);
...
static ssize_t ctp_open_proc_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	*ppos += count;
    ...
    ptr = kzalloc(count, GFP_KERNEL);
        ...
        ret=sprintf(ptr, "result=%d\n", result);

		if (copy_to_user(buf, ptr, ret+1)) {

        	FTS_TEST_ERROR("%s: fail to copy default config\n", __func__);
        	ret = -EFAULT;
