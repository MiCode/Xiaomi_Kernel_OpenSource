...
struct fts_mode_flag g_ft8006m_mode_flag;
...
*****************************************************************************/
int  ft8006m_enter_glove_mode(struct i2c_client *client, int mode);
int  ft8006m_glove_init(struct i2c_client *client);
int  ft8006m_glove_exit(struct i2c_client *client);

int  ft8006m_enter_cover_mode(struct i2c_client *client, int mode);
int  ft8006m_cover_init(struct i2c_client *client);
int  ft8006m_cover_exit(struct i2c_client *client);

int  ft8006m_enter_charger_mode(struct i2c_client *client, int mode);
int  ft8006m_charger_init(struct i2c_client *client);
int  ft8006m_charger_exit(struct i2c_client *client);
...
{
	return snprintf(buf, PAGE_SIZE, "Glove: %s\n", g_ft8006m_mode_flag.fts_glove_mode_flag ? "On" : "Off");
}
...
static ssize_t fts_touch_glove_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		if (!g_ft8006m_mode_flag.fts_glove_mode_flag) {
			...
			if (ret >= 0) {
				g_ft8006m_mode_flag.fts_glove_mode_flag = true;
			}
		}
		...
		if (g_ft8006m_mode_flag.fts_glove_mode_flag) {
			...
			if (ret >= 0) {
				g_ft8006m_mode_flag.fts_glove_mode_flag = false;
			}
		}
	}
...
int ft8006m_enter_glove_mode(struct i2c_client *client, int mode)
{
	...
	if (ret < 0) {
		FTS_ERROR("[Mode]ft8006m_enter_glove_mode write value fail");
	}
...
{
	return snprintf(buf, PAGE_SIZE, "Cover: %s\n", g_ft8006m_mode_flag.fts_cover_mode_flag ? "On" : "Off");
}
...
static ssize_t fts_touch_cover_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		if (!g_ft8006m_mode_flag.fts_cover_mode_flag) {
			...
			if (ret >= 0) {
				g_ft8006m_mode_flag.fts_cover_mode_flag = true;
			}
		}
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		if (g_ft8006m_mode_flag.fts_cover_mode_flag) {
			...
			if (ret >= 0) {
				g_ft8006m_mode_flag.fts_cover_mode_flag = false;
			}
		}
	}
...
int  ft8006m_enter_cover_mode(struct i2c_client *client, int mode)
{
	...
	ret = ft8006m_i2c_write_reg(client, buf_addr[0], buf_value[0]);
	if (ret < 0) {
		FTS_ERROR("[Mode] ft8006m_enter_cover_mode write value fail \n");
	}
...
{
	return snprintf(buf, PAGE_SIZE, "Charger: %s\n", g_ft8006m_mode_flag.fts_charger_mode_flag ? "On" : "Off");
}
...
static ssize_t fts_touch_charger_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		if (!g_ft8006m_mode_flag.fts_charger_mode_flag) {
			...
			if (ret >= 0) {
				g_ft8006m_mode_flag.fts_charger_mode_flag = true;
			}
		}
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		if (g_ft8006m_mode_flag.fts_charger_mode_flag) {
			...
			if (ret >= 0) {
				g_ft8006m_mode_flag.fts_charger_mode_flag = false;
			}
		}
	}
...
/************************************************************************
* Name: ft8006m_enter_charger_mode
...
#if FTS_GLOVE_EN
	if (g_ft8006m_mode_flag.fts_glove_mode_flag)
		ret = ft8006m_enter_glove_mode(client, true);
#endif

#if FTS_COVER_EN
	if (g_ft8006m_mode_flag.fts_cover_mode_flag)
		ret = ft8006m_enter_cover_mode(client, true);
#endif

#if FTS_CHARGER_EN
	if (g_ft8006m_mode_flag.fts_charger_mode_flag)
		ret = ft8006m_enter_charger_mode(client, true);
#endif
