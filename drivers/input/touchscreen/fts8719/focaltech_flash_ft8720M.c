...
extern u8 fw_file2[];
extern u8 fw_file3[];
extern struct upgrade_module module_list[];
...
int fts_upgrade_bin_ft8720M(char *fw_name, bool force)
{
    int ret = 0;
    u32 fw_file_len = 0;
    u8 *fw_file_buf = NULL;
...
int fts_enter_test_environment_ft8720M(bool test_state)
{
    int ret = 0;
    int i = 0;
    u8 detach_flag = 0;
...
static int fts_fw_resume(bool need_reset)
{
    int ret = 0;
    struct fts_upgrade *upg = fwupgrade;
...
int fts_fw_recovery_ft8720M(void)
{
    int ret = 0;
    u8 boot_state = 0;
    u8 chip_id = 0;
...
int fts_fwupg_init_ft8720M(struct fts_ts_data *ts_data)
{
    int i = 0;
