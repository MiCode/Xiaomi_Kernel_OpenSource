...
static struct proc_dir_entry *fts_proc_aftersales_test_entry;
static struct proc_dir_entry *fts_proc_test_data_entry;
...
extern int ft8719_open_test(struct fts_test *tdata, bool *test_result);
extern int ft8719_short_test(struct fts_test *tdata, bool *test_result);
extern int ft8720_open_test(struct fts_test *tdata, bool *test_result);
extern int ft8720_short_test(struct fts_test *tdata, bool *test_result);
extern int get_fts_ic_type(void);
...
static int copy_testdata(struct fts_proc_testdata *dst, struct fts_test_data *src, size_t sdata_len);
static int clear_testdata(struct fts_proc_testdata *td);
#endif
...
#if FTS_PROC_GET_TESTDATA_EN
	struct fts_ts_data *ts_data = tdata->ts_data;
	struct fts_proc_testdata *proc_testdata = ts_data->proc_test_data;
#endif
	if (!name || !data) {
...
#if FTS_PROC_GET_TESTDATA_EN
	if (copy_testdata(proc_testdata, td, datalen * sizeof(int)) < 0)
		FTS_TEST_ERROR("%s[%d]: copy failed", __func__, __LINE__);
#endif
	td->item_count++;
}
...
static int fts_test_main_init(void)
{
	...
	struct fts_test *tdata = fts_ftest;
#if FTS_PROC_GET_TESTDATA_EN
	struct fts_proc_testdata *proc_testdata = tdata->ts_data->proc_test_data;
#endif
	FTS_TEST_FUNC_ENTER();
	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (pos = 0; pos <= proc_testdata->item_count; pos++) {
		ret += snprintf(buf + ret, PAGE_SIZE, "%s %s\n", proc_testdata->info[pos].name,
			proc_testdata->info[pos].result ? "SUCCESSFUL" : "FAILED");
	}
	return ret;
#endif
	return -EPERM;
...
#if FTS_PROC_GET_TESTDATA_EN
/*auto find empty pos in dst*/
static int copy_testdata(struct fts_proc_testdata *dst, struct fts_test_data *src, size_t sdata_len)
{
	int *pos = &dst->item_count;
	struct item_info *info = dst->info;

	while (info[*pos].name[0] != '\0' || info[*pos].datalen != 0) {			/*already been used find next pos*/
		++*pos;
	}
	if (*pos >= PROC_PRINT_ITEM_COUNT_MAX) {
		FTS_TEST_ERROR("Item count is overflow, can't copy.");
		return -EINVAL;
	}

	if (sdata_len > PROC_ITEM_LENGTH * sizeof(int)) {
		FTS_TEST_ERROR("src data is large than dst buff can contained, can't copy.");
		return -EINVAL;
	}
	memcpy(info[*pos].data, src->info[src->item_count].data, sdata_len);
	strncpy(info[*pos].name, src->info[src->item_count].name, TEST_ITEM_NAME_MAX);
	info[*pos].code = src->info[src->item_count].code;
	info[*pos].datalen = src->info[src->item_count].datalen;
	info[*pos].result = src->info[src->item_count].result;
	info[*pos].mc_sc = src->info[src->item_count].mc_sc;
	info[*pos].key_support = src->info[src->item_count].key_support;
	return 0;
}
...
{
	int i;
	struct item_info *info = td->info;

	td->item_count = 0;

	for (i = 0; i < PROC_PRINT_ITEM_COUNT_MAX; i++) {
		info[i].code = 0;
		info[i].datalen = 0;
		info[i].result = 0;
		info[i].mc_sc = 0;
		info[i].key_support = 0;
		memset(info[i].name, 0, TEST_ITEM_NAME_MAX);
		memset(info[i].data, 0, PROC_ITEM_LENGTH * sizeof(int));
	}
...
{
	struct fts_proc_testdata *proc_testdata = fts_data->proc_test_data;

	if (*pos > proc_testdata->item_count) {
		*pos = 0;
		return NULL;
	}
	return &proc_testdata->info[*pos];
}
...
{
	int i, j;
	int tx_channel = fts_ftest->node.tx_num;
	int rx_channel = fts_ftest->node.rx_num;
	struct item_info *info = v;
	int *p_data = info->data;

	seq_printf(m, "=============%s==============\n", info->name);
	for (i = 0; i < tx_channel; i++) {
		for (j = 0; j < rx_channel; j++) {
			seq_printf(m, "%5d", p_data[i * rx_channel + j]);
		}
		seq_puts(m, "\n");
	}
	seq_puts(m, "\n\n");
...
static const struct seq_operations t_fops = {
	.start = t_start,
	.next = t_next,
	.stop = t_stop,
	.show = t_show,
...
static ssize_t fts_selftest_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	if (*pos != 0)
		return 0;
	cnt = snprintf(tmp, sizeof(fts_ftest->result_type), "%d\n", fts_ftest->result_type);
...
int fts_short_test(void)
{
	...
	if (ret < 0) {
		...
		/*Read parse configuration file*/
		if (get_fts_ic_type() == 1) {
			FTS_TEST_SAVE_INFO("ini_file_name:%s\n", DEFAULT_INI_FILE_NAME);
			ret = fts_test_get_testparams(DEFAULT_INI_FILE_NAME);
		} else {
			FTS_TEST_SAVE_INFO("ini_file_name:FT8720M_J22_TestINI.ini\n");
			ret = fts_test_get_testparams("FT8720M_J22_TestINI.ini");
		}
		if (ret < 0) {
			FTS_TEST_ERROR("get testparam fail");
			goto test_err;
...
test_err:
	...
	if (temp_result) {
		FTS_INFO("short test successful");
		ret = temp_result;
	}
	return ret;
}

int fts_open_test(void)
{
	int ret;
	bool temp_result = false;
	struct fts_test *tdata = fts_ftest;
...
int fts_spi_test(void)
{
	int ret;
	u8 chip_id;

	ret = fts_read_reg(FTS_REG_CHIP_ID, &chip_id);
	if (ret < 0 || chip_id != fts_data->ic_info.ids.chip_idh) {
		FTS_ERROR("spi may error");
	} else {
		ret = 1;
	}
	return ret;
...
static ssize_t fts_selftest_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	switch (retval) {
	case 1:
	fts_ftest->result_type = FTS_RESULT_PASS;
	break;
	case -EINVAL:
	fts_ftest->result_type = FTS_RESULT_INVALID;
	break;
	default:
	fts_ftest->result_type = FTS_RESULT_FAIL;
	break;
	}
...
static const struct file_operations fts_aftersales_test_ops = {
...
{
	int i, ret;
	/*malloc buf to contain test data*/
	struct fts_proc_testdata *proc_td;

	proc_td = kzalloc(sizeof(*proc_td), GFP_KERNEL);
	if (!proc_td) {
		FTS_TEST_ERROR("%s[%d]: alloc mem failed!", __func__, __LINE__);
		return -ENOMEM;
	} else {
		FTS_TEST_INFO("%s[%d]: alloc mem successful", __func__, __LINE__);
	}

	for (i = 0; i < PROC_PRINT_ITEM_COUNT_MAX; i++) {
		proc_td->info[i].data = (int *)kzalloc(PROC_ITEM_LENGTH * sizeof(int), GFP_KERNEL);
		if (!proc_td->info[i].data) {
			FTS_TEST_ERROR("%s[%d]: alloc test buf[%d] failed!", __func__, __LINE__, i);
			ret = -ENOMEM;
			goto err_mem;
		}
	}
	FTS_TEST_INFO("%s[%d]: alloc test buf successful", __func__, __LINE__);

	/*creat proc interface to read this test data*/

	fts_proc_test_data_entry = proc_create("fts_test_data", 0644, NULL, &fts_test_data_fops);
	if (!fts_proc_test_data_entry) {
		FTS_TEST_ERROR("create proc/fts_test_data failed!");
		ret = -EINVAL;
		goto err_mem;
	} else {
		FTS_TEST_INFO("create proc/fts_test_data successful");
	}

	ts_data->proc_test_data = proc_td;

	fts_proc_aftersales_test_entry = proc_create(FTS_XIAOMI_AFTERSALES_TEST, 0644, NULL, &fts_aftersales_test_ops);
	if (fts_proc_aftersales_test_entry == NULL) {
		FTS_ERROR("create /proc/%s Failed!\n", FTS_XIAOMI_AFTERSALES_TEST);
		return -1;
	} else {
...
err_mem:
	for (i = 0; i < PROC_PRINT_ITEM_COUNT_MAX; i++)
		kfree_safe(proc_td->info[i].data);
	kfree_safe(proc_td);
	return ret;
}
...
{
	int i;
	struct fts_proc_testdata *proc_td = ts_data->proc_test_data;

	if (fts_proc_test_data_entry) {
		remove_proc_entry("fts_test_data", NULL);
		fts_proc_test_data_entry = NULL;
		FTS_TEST_INFO("remove /proc/fts_test_data");
	}
