...
*****************************************************************************/
enum { RWREG_OP_READ = 0, RWREG_OP_WRITE = 1,
};
...
static ssize_t fts_debug_write(struct file *filp, const char __user * buff, size_t count, loff_t * ppos)
{
	...
	ret = buflen;
proc_write_err:if ((buflen > PROC_BUF_SIZE) && writebuf) {
		kfree(writebuf);
		writebuf = NULL;
	}
	return ret;
...
static ssize_t fts_debug_read(struct file *filp, char __user * buff, size_t count, loff_t * ppos)
{
	...
	ret = num_read_chars;
proc_read_err:if ((num_read_chars > 0) && copy_to_user(buff, readbuf, num_read_chars)) {
...
static const struct proc_ops fts_proc_fops = {.proc_read = fts_debug_read,.proc_write = fts_debug_write,
};

#else /*  */
static const struct file_operations fts_proc_fops = {.owner = THIS_MODULE,.read = fts_debug_read,.write =
	    fts_debug_write,
};

#endif /*  */
#else /*  */
static int fts_debug_write(struct file *filp, const char __user * buff, unsigned long count, void *data)
{
	u8 *writebuf = NULL;
	u8 tmpbuf[PROC_BUF_SIZE] = {
		0
...
static int fts_debug_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	ret = num_read_chars;
proc_read_err:if (!memcpy(page, readbuf, num_read_chars)) {
		FTS_ERROR("copy to user error");
		ret = -EFAULT;
	}
	if ((buflen > PROC_BUF_SIZE) && readbuf) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops fts_procta_fops = {.proc_open = fts_ta_open,.proc_release =
	    fts_ta_release,.proc_read = fts_ta_read,
};

#else /*  */
static const struct file_operations fts_procta_fops = {.open = fts_ta_open,.release = fts_ta_release,.read =
	    fts_ta_read,
};

#endif /*  */
#endif /*  */
static ssize_t fts_fw_version_read(struct file *filp, char __user * buf, size_t count, loff_t * pos)
{
	int ret = 0, cnt = 0;
	char tmp[PROC_BUF_SIZE];
	struct fts_ts_data *ts_data = fts_data;
...
static const struct file_operations tp_fw_version_fops = {.read = fts_fw_version_read,
};

static ssize_t fts_lockdown_info_read(struct file *filp, char __user * buf, size_t count, loff_t * pos)
{
	int cnt = 0, ret = 0;
	char tmp[PROC_BUF_SIZE];
	struct fts_ts_data *ts_data = fts_data;
	if (*pos != 0)
		return 0;
	cnt =
	    snprintf(tmp, PROC_BUF_SIZE, "0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x\n",
		     ts_data->lockdown_info[0], ts_data->lockdown_info[1], ts_data->lockdown_info[2],
		     ts_data->lockdown_info[3], ts_data->lockdown_info[4], ts_data->lockdown_info[5],
...
static const struct file_operations tp_lockdown_info_fops = {.read = fts_lockdown_info_read,
};
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
	ts_data->proc_ta.proc_entry = proc_create_data("fts_ta", 0777, NULL, &fts_procta_fops, ts_data);
	if (!ts_data->proc_ta.proc_entry) {
		FTS_ERROR("create proc_ta entry fail");
		return -ENOMEM;
	}
#endif /*  */
	proc->tp_lockdown_info_proc = proc_create("tp_lockdown_info", 0444, NULL, &tp_lockdown_info_fops);
...
static ssize_t fts_driverinfo_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (ts_data->bus_type == BUS_TYPE_I2C) {
		...
		count +=
		    snprintf(buf + count, PAGE_SIZE, "BUS:%s,mode:%d,max_freq:%d\n", "SPI", ts_data->spi->mode,
...
/* fts_pen node */
static ssize_t fts_pen_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	struct fts_ts_data *ts_data = dev_get_drvdata(dev);
	struct input_dev *input_dev = ts_data->input_dev;
...
static ssize_t fts_pen_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int value = 0;
	struct fts_ts_data *ts_data = dev_get_drvdata(dev);
	struct input_dev *input_dev = ts_data->input_dev;
...
static ssize_t fts_tamode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&input_dev->mutex);
	count +=
	    snprintf(buf + count, PAGE_SIZE, "touch analysis:%s\n",
...
static struct attribute *fts_attributes[] =
    { &dev_attr_fts_fw_version.attr, &dev_attr_fts_rw_reg.attr, &dev_attr_fts_dump_reg.attr,
	&dev_attr_fts_upgrade_bin.attr, &dev_attr_fts_force_upgrade.attr, &dev_attr_fts_driver_info.attr,
	    &dev_attr_fts_hw_reset.attr,
	&dev_attr_fts_irq.attr, &dev_attr_fts_boot_mode.attr, &dev_attr_fts_touch_point.attr,
	    &dev_attr_fts_log_level.attr, &dev_attr_fts_pen.attr,
	&dev_attr_fts_touch_size.attr, &dev_attr_fts_ta_mode.attr, &dev_attr_fts_lockdown_info.attr, NULL
};
