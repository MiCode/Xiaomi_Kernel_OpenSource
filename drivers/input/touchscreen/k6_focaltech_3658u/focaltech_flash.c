...
static int fts_fwupg_get_boot_state(struct fts_upgrade *upg, enum FW_STATUS *fw_sts)
{
	...
	if (upg->ts_data->ic_info.is_incell || (upg->func->upgspec_version >= UPGRADE_SPEC_V_1_0))
...
static int fts_pram_ecc_cal_algo(struct fts_upgrade *upg, u32 start_addr, u32 ecc_length)
{
	...
	for (i = 0; i < FTS_ECC_FINISH_TIMEOUT; i++) {
		...
		if (upg->func->new_return_value_from_ic || (upg->func->upgspec_version >= UPGRADE_SPEC_V_1_0)) {
...
static bool fts_fwupg_check_flash_status(u16 flash_status, int retries, int retries_delay)
{
	...
	FTS_ERROR("flash status fail,ok:%04x,read:%04x,retries:%d", flash_status, read_status, i);
	return false;
}
...
int fts_fwupg_ecc_cal(u32 saddr, u32 len)
{
	...
	for (i = 0; i < packet_num; i++) {
		...
		bflag =
		    fts_fwupg_check_flash_status(FTS_CMD_FLASH_STATUS_ECC_OK, FTS_RETRIES_ECC_CAL,
...
int fts_upgrade_bin(char *fw_name, bool force)
{
	...
	ret = 0;
err_bin:fts_esdcheck_switch(upg->ts_data, ENABLE);
	fts_irq_enable();
