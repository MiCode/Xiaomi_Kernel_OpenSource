...
struct test_funcs *test_func_list[] = { &test_func_ft5652,
};
...
read_massdata_err:fts_free(data);
	return ret;
}
...
static int read_rawdata(struct fts_test *tdata, u8 off_addr, u8 off_val, u8 rawdata_addr, int byte_num, int *data)
{
	int ret = 0;
...
void fts_test_save_fail_result(struct fts_test *tdata, char *prefix, char *suffix, char *buf, int len)
{
	char file_name[128];
...
static void fts_test_save_data_csv(struct fts_test *tdata)
{
	...
	for (i = 0; i < td->item_count; i++) {
		...
		for (j = 1; j <= data_count; j++) {
			...
			line2_length +=
			    snprintf(line2_buffer + line2_length, CSV_LINE2_BUFFER_LEN - line2_length,
				     "%s, %d, %d, %d, %d, %d, ", info->name, info->code, tx, rx, start_line, index);
			start_line += tx;
			csv_item_count++;
		}
	}
	csv_length +=
	    snprintf(csv_buffer + csv_length, CSV_BUFFER_LEN - csv_length, "TestItem Num, %d, ", csv_item_count);
	if (line2_length > 0) {
		...
		if (info->mc_sc) {
			...
			for (j = 0; j < info->datalen;) {
				for (k = 0; k < tdata->sc_node.node_num; k++) {
					csv_length +=
					    snprintf(csv_buffer + csv_length, CSV_BUFFER_LEN - csv_length, "%d, ",
						     info->data[offset + k]);
...
#if defined(TEST_SAVE_FAIL_RESULT) && TEST_SAVE_FAIL_RESULT
	fts_test_save_fail_result(tdata, "testdata_fail", ".csv", csv_buffer, csv_length);

#endif /*  */
csv_save_err:if (line2_buffer) {
		vfree(line2_buffer);
		line2_buffer = NULL;
	}
	if (csv_buffer) {
...
#if defined(TEST_SAVE_FAIL_RESULT) && TEST_SAVE_FAIL_RESULT
	fts_test_save_fail_result(tdata, "testresult_fail", ".txt", tdata->testresult, tdata->testresult_len);
...
static int get_channel_num(struct fts_test *tdata)
{
	...
	FTS_TEST_INFO("node_num:%d, tx:%d, rx:%d, key:%d", tdata->node.node_num, tdata->node.tx_num,
...
static int fts_test_main_exit(void)
{
	...
	FTS_TEST_INFO("data len: %d, result len: %d", tdata->csv_data_len, tdata->testresult_len);
...
static struct attribute *fts_test_attributes[] = { &dev_attr_fts_test.attr, NULL
};

static struct attribute_group fts_test_attribute_group = {.attrs = fts_test_attributes
};
...
static const struct seq_operations data_fops = {.start = data_start,.next = data_next,.stop =
	    data_stop,.show = data_show,
};
...
static const struct file_operations tp_test_data_fops = {.owner = THIS_MODULE,.open = tp_test_data_open,.read =
...
static const struct seq_operations result_fops = {.start = result_start,.next = result_next,.stop =
	    result_stop,.show = result_show,
};
...
static const struct file_operations tp_test_result_fops = {.owner = THIS_MODULE,.open =
	    tp_test_result_open,.read = seq_read,.llseek = seq_lseek,.release = seq_release,
};
...
static int fts_i2c_test(void)
{
	...
	do {
		...
		if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
			...
			ret = SELFTEST_PASS;
			FTS_TEST_INFO("i2c test successm, Device id: 0x%02x%02x", chip_id[0], chip_id[1]);
			break;
		}
	} while (cnt < 10);
	return ret;
...
ssize_t tp_selftest_write(struct file * file, const char __user * buf, size_t count, loff_t * pos)
{
	...
	if (!strncmp(tmp, "short", 5) && tdata->func->short_test)
		...
		enter_work_mode();
		tp_selftest_result = fts_i2c_test();
	}
	ret = tp_selftest_result;
	fts_test_main_exit();
	fts_free_test_memory();
	enter_work_mode();
test_err:fts_irq_enable();
	mutex_unlock(&input_dev->mutex);
out:	if (ret >= 0)
		ret = count;
	return ret;
...
static const struct file_operations tp_selftest_fops = {.open = tp_selftest_open,.read =
	    tp_selftest_read,.write = tp_selftest_write,.release = tp_selftest_release,
};
