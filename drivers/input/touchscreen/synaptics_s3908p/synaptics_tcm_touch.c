 ...
 */
static int touch_free_objects(struct syna_tcm_hcd *tcm_hcd)
{
...
static void touch_report(void)
{
	...
	area = le2_to_uint(touch_data->gesture_data.area);
	LOGI(tcm_hcd->pdev->dev.parent,
			"Gesture Event:%02x, Gesture Data:x:%4d, y:%4d, area:%4d\n",
			touch_data->gesture_id, x, y, area);
	...
	if ((touch_data->gesture_id == GESTURE_DOUBLE_TAP ||
		...
		LOGE(tcm_hcd->pdev->dev.parent, "Double TAP Detected\n");
