...
static int testing_self_test(void);
					...
					struct kobj_attribute *attributes, char *buf)
{
	...
	retval = testing_hcd->collect_reports(REPORT_DELTA, 0x01);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
			"Failed to read RID18 data\n");
		goto exit;
	}

	LOGI(tcm_hcd->pdev->dev.parent, "collect RID18 done\n");

	retval = testing_print_report_data(REPORT_DELTA, buf);
					...
					struct kobj_attribute *attributes, char *buf)
{
	...
	retval = testing_hcd->collect_reports(REPORT_RID161, 0x01);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
			"Failed to read RID18 data\n");
		goto exit;
	...
	LOGI(tcm_hcd->pdev->dev.parent, "collect RID161 done\n");

	retval = testing_print_report_data(REPORT_RID161, buf);
