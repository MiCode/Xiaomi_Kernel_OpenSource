...
//	gf_debug(INFO_LOG, "cluster_num = %d \n", cluster_num);
...
#ifdef CONFIG_PRODUCT_DAUMIER_FP
	buck = regulator_get(NULL, "mt6373_vfp");
	gf_debug(ERR_LOG, "%s, get daumier vfp!!\n", __func__);
#else
	buck = regulator_get(NULL, "mt6368_vfp");
	gf_debug(ERR_LOG, "%s, get other vfp!!\n", __func__);
#endif
...
#else
	gf_debug(ERR_LOG, "%s, fail to set daumier vfp, start set 3.3V\n", __func__);
	...
	if ((rx_test[0] != 0x13) || (rx_test[1] != 0x3)) {
		gf_debug(ERR_LOG, "%s, fail to get chip id 0x0313, other.\n", __func__);
#endif
