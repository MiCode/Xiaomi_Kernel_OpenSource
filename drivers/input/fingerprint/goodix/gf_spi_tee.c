...
static int gf_probe(struct spi_device *spi)
{
	...
	if (buck1v8 == NULL) {
		gf_debug(INFO_LOG, "regulator_get vufs fail");
		goto err_freqbuff;
	}
	gf_debug(ERR_LOG, "%s, try to set regulator\n", __func__);
	...
	if (status_1v8 < 0) {
		gf_debug(ERR_LOG, "%s, regulator_enable vfs fail!!\n", __func__);
		goto err_freqbuff;
	}
	...
	if ((rx_test[0] != 0x13) || (rx_test[1] != 0x3)) {
		gf_debug(ERR_LOG, "%s, fail to get chip id 0x313.\n", __func__);
