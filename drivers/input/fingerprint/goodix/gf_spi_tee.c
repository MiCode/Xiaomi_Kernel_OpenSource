...
static int gf_probe(struct spi_device *spi)
{
	...
	buck = regulator_get(NULL,"mt6373_vcn33_2");
	if (buck == NULL) {
		gf_debug(INFO_LOG, "regulator_get corot vfp fail");
		goto err_freqbuff;
	}
	gf_debug(ERR_LOG, "%s, corot try to get mt6373_vcn33_2 success\n", __func__);

	gf_debug(ERR_LOG, "%s, try to set corot\n", __func__);
	...
	if ((rx_test[0] != 0x12) || (rx_test[1] != 0x31)) {
		gf_debug(ERR_LOG, "%s, fail to get chip id 0x1231, corot.\n", __func__);
