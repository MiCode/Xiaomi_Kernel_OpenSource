...
#ifdef CONFIG_PRODUCT_PLATO_FP
struct regulator *buck1v8;
#endif
/* for netlink use */
static int pid = 0;
...
//	gf_debug(INFO_LOG, "cluster_num = %d \n", cluster_num);
	...
	if (buck == NULL) {
		gf_debug(INFO_LOG, "regulator_get plato vfp fail");
		goto err_freqbuff;
	}
	buck1v8 = regulator_get(NULL, "mt6363_vufs18");
	if (buck1v8 == NULL) {
		gf_debug(INFO_LOG, "regulator_get plato vufs fail");
		goto err_freqbuff;
	}
#else
	buck = regulator_get(NULL, "mt6373_vfp");
	if (buck == NULL) {
		gf_debug(INFO_LOG, "regulator_get daumier vfp fail");
		goto err_freqbuff;
	}
#endif
#if defined(CONFIG_GOODIX_G6_SENSOR) || defined(CONFIG_PRODUCT_DAUMIER_FP)
	gf_debug(ERR_LOG, "%s, try to set daumier vfp.\n", __func__);
	status = regulator_set_voltage(buck, 3000000, 3000000);
#else
	gf_debug(ERR_LOG, "%s, try to set plato\n", __func__);
	status = regulator_set_voltage(buck, 3300000, 3300000);
	status_1v8 = regulator_set_voltage(buck1v8, 1800000, 1800000);
#endif
	if (status < 0) {
...
#ifdef CONFIG_PRODUCT_PLATO_FP
	if (status_1v8 < 0) {
		gf_debug(INFO_LOG, "regulator_set  1v8 fail");
		goto err_freqbuff;
	}
	/*status_1v8 = regulator_set_load(buck1v8, 200000);
	if (status_1v8 < 0) {
		gf_debug(INFO_LOG, "regulator_set_load 1v8 failed");
		goto err_freqbuff;
	}*/
	status_1v8 = regulator_enable(buck1v8);
	if (status_1v8 < 0) {
		gf_debug(ERR_LOG, "%s, regulator_enable plato vfs fail!!\n", __func__);
		goto err_freqbuff;
	}
	status_1v8 = regulator_get_voltage(buck1v8);
	gf_debug(ERR_LOG, "%s, regulator_value %d!!\n", __func__,status_1v8);
#endif
	...
	if ((rx_test[0] != 0x12) || (rx_test[1] != 0x12)) {
		gf_debug(ERR_LOG, "%s, fail to get chip id 0x1212, daumier.\n", __func__);
#else
	if ((rx_test[0] != 0x13) || (rx_test[1] != 0x3)) {
		gf_debug(ERR_LOG, "%s, fail to get chip id 0x0313, plato.\n", __func__);
#endif
		...
		regulator_put(buck);
		regulator_disable(buck1v8);
		// regulator_set_load(buck1v8, 0);
		regulator_put(buck1v8);
#endif
		gf_spi_clk_enable(gf_dev, 0);
		kfree(gf_dev->spi_buffer);
		mutex_destroy(&gf_dev->buf_lock);
