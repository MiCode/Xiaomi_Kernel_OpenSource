...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	if (IS_ERR(gf_dev->pins_reset_low)) {
   	...
   	gf_dev->pins_spi_cs_up = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fpsensor_goodix_spi_cs_up");
	if (IS_ERR(gf_dev->pins_spi_cs_up)) {
		ret = PTR_ERR(gf_dev->pins_spi_cs_up);
		gf_debug(ERR_LOG, "%s can't find fingerprint pins_spi_cs_up\n", __func__);
		return ret;
	}

   	gf_dev->pins_spi_cs_low = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fpsensor_goodix_spi_cs_low");
...
static int gf_probe(struct platform_device  *pdev)
{
	...
	pr_err("%s %d now reset sensor pull high\n",__func__, __LINE__);
	pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_reset_high);
	pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_spi_cs_up);
