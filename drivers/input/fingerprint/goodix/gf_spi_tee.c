...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	if (node) {
		irq_gpio = of_get_named_gpio(node, "gf,gpio_irq", 0);
		pr_debug( "fp::irq_gpio:%d\n",irq_gpio);
		if (!gpio_is_valid(irq_gpio)) {
			pr_debug("IRQ GPIO is invalid.\n");
			//return -EPERM;
		}
		virq = gpio_to_irq(irq_gpio);
	...
	gf_dev->pins_avdd_high = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "vdd_high");
	if (IS_ERR(gf_dev->pins_avdd_high)) {
		ret = PTR_ERR(gf_dev->pins_avdd_high);
		gf_debug(ERR_LOG, "%s can't find fingerprint pinctrl vdd_high\n", __func__);
		return ret;
	}
	gf_dev->pins_avdd_low = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "vdd_low");
	if (IS_ERR(gf_dev->pins_avdd_low)) {
		ret = PTR_ERR(gf_dev->pins_avdd_low);
		gf_debug(ERR_LOG, "%s can't find fingerprint pinctrl vdd_low\n", __func__);
		return ret;
	}
...
{
	/* TODO: LDO configure */
	gf_debug(DEBUG_LOG,"%s,device_available =%d\n", __func__,gf_dev->device_available);
	if(gf_dev->device_available == 1) {
		pr_err("gf_pwr have already powered on\n");
	} else {
		pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_avdd_high);
		gf_debug(DEBUG_LOG, "%s, power on now!\n", __func__);
...
void gf_hw_power_disable(struct gf_device *gf_dev)
{
	FUNC_ENTRY();
	if(gf_dev->device_available == 0) {
		pr_err("has already powered off\n");
	} else {
		pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_avdd_low);
		gf_debug(DEBUG_LOG,"%s, power off now!\n", __func__);
...
static void gf_irq_gpio_cfg(struct gf_device *gf_dev)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,goodix-fp");
	if (node) {
		int irq_gpio = of_get_named_gpio(node, "gf,gpio_irq", 0);
		pr_debug( "fp::irq_gpio:%d\n",irq_gpio);
		if (!gpio_is_valid(irq_gpio)) {
			pr_debug("IRQ GPIO is invalid.\n");
			//return -EPERM;
		}
		gf_dev->irq_num = gpio_to_irq(irq_gpio);
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
...
#else
		if (!(mtk_disp_notifier_register("gf_disp_notifier", &gf_dev->notifier)))
		   printk("mtk_disp_notifier_register fail.\n");
		else
		   printk("mtk_disp_notifier_register success.\n");
#endif
#endif
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		if ((GF_KEY_HOME_DOUBLE_CLICK == gf_key.key) && (gf_key.value >= 0) && (gf_key.value < 3)) {
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "-8", 2)) {
		...
		/* register screen on/off callback */
		gf_dev->notifier.notifier_call = gf_fb_notifier_callback;
		mtk_disp_notifier_register("gf_disp_notifier", &gf_dev->notifier);
#endif
