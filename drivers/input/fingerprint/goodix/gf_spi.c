					...
					unsigned long val, void *data)
{
	...
	if (val != XIAOMI_PANEL_EVENT_BLANK)
		return 0;
	pr_debug
	    ("[info] %s go to22222 the goodix_fb_state_chg_callback value = %d\n",
	     __func__, (int)val);
	...
	if (val == XIAOMI_PANEL_EVENT_BLANK && gf_dev) {
		...
		case XIAOMI_PANEL_BLANK_POWERDOWN:
            pr_debug("[XMFP] blank = DRM_BLANK_POWERDOWN");
			if (gf_dev->device_available == 1) {
                pr_debug("[XMFP] gf_dev->fb_black = 1");
				gf_dev->fb_black = 1;
		...
		case XIAOMI_PANEL_BLANK_UNBLANK:
            pr_debug("[XMFP] blank = DRM_BLANK_UNBLANK");
			if (gf_dev->device_available == 1) {
                pr_debug("[XMFP] gf_dev->fb_black = 0");
				gf_dev->fb_black = 0;
#if defined(GF_NETLINK_ENABLE)
...
static void unblank_work(struct work_struct *work)
{
    ...
    g_notify_data.blank = XIAOMI_FP_IRQ_EVENT;
    xiaomi_fp_notifier_call_chain(XIAOMI_FP_EVENT_BLANK, &g_notify_data);
}
#endif
...
#endif
{
	...
	xiaomi_panel_notifier_register_client(&gf_dev->notifier);
#endif
	//gf_dev->irq = gf_irq_num(gf_dev);
