...
/*N19 code for HQ-380927 by zhujingyi at 20240407 start*/
#define GF_INPUT_NAME		"uinput-goodix"	/*"goodix_fp" */
/*N19 code for HQ-380927 by zhujingyi at 20240407 end*/
...
static int gf_open(struct inode *inode, struct file *filp)
{
	...
	printk("Try to enable fp_vdd_vreg\n");
					...
					unsigned long val, void *data)
{
	...
	pr_debug("%s start\n", __func__);
	if (val != XIAOMI_PANEL_EVENT_BLANK)
		return 0;
	pr_debug
	    ("[info] %s go to the goodix_fb_state_chg_callback value = %d\n",
	     __func__, (int)val);
	gf_dev = container_of(nb, struct gf_dev, notifier);
	if (val == XIAOMI_PANEL_EVENT_BLANK && gf_dev) {
		if (evdata == NULL) {
		...
		switch (blank) {
		case XIAOMI_PANEL_BLANK_POWERDOWN:
			if (gf_dev->device_available == 1) {
				pr_debug("%s fingerprint received lcd off notify\n", __func__);
				gf_dev->fb_black = 1;
			...
			break;
		case XIAOMI_PANEL_BLANK_UNBLANK:
			if (gf_dev->device_available == 1) {
				pr_debug("%s fingerprint received lcd on notify\n", __func__);
				gf_dev->fb_black = 0;
#if defined(GF_NETLINK_ENABLE)
...
#endif
{
...
#if 0
	INIT_WORK(&gf_dev->work, unblank_work);
#endif
	...
	if (xiaomi_panel_notifier_register_client(&gf_dev->notifier))
		pr_debug("xiaomi_panel_notifier_register_client fail.\n");
	else
		pr_debug("xiaomi_panel_notifier_register_client success.\n");
#endif
...
static int __init gf_init(void)
{
	...
	netlink_init();
#endif
	pr_debug("goodix fingerprint init status = 0x%x\n", status);
	return 0;
}
