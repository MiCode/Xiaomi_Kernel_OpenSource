...
static int gf_probe(struct spi_device *spi)
{
	...
	gf_debug(INFO_LOG, " cannon && cannong Setting gf device configuration==========\n");
	...
	if (1) {
		if (((rx_test[0] != 0x07) || (rx_test[3] != 0x25)) && ((rx_test[0] != 0x10) || (rx_test[3] != 0x25))) {
			...
			if (regulator_is_enabled(buck)) {
				...
				regulator_put(buck);
				buck = NULL;
			        gf_debug(ERR_LOG, "[gf][goodix_test] power down doing\n");
			}
			kfree(gf_dev->spi_buffer);
			mutex_destroy(&gf_dev->buf_lock);
			mutex_destroy(&gf_dev->release_lock);
