...
#include <linux/arch_topology.h>  //gyw
#include <linux/pm_wakeup.h>      //gyw

//#include "cpu_ctrl.h" //gyw

#include "teei_fp.h"  //gyw
#include "tee_client_api.h"   //gyw
...
//#include <mt-plat/sync_write.h>   //gyw
...
//	gf_debug(INFO_LOG, "cluster_num = %d \n", cluster_num);
...
#ifdef CONFIG_PRODUCT_RUBENS_FP
	buck = regulator_get(NULL, "mt6368_vfp");
	gf_debug(ERR_LOG, "%s, get rubens vfp!!\n", __func__);
#elif defined(CONFIG_PRODUCT_REMBRANDT_FP)
	buck = regulator_get(NULL, "mt6368_vfp");
	gf_debug(ERR_LOG, "%s, get rembrandt  vfp!!\n", __func__);
#else
	buck = regulator_get(NULL, "mt6373_vfp");
	gf_debug(ERR_LOG, "%s, get matisse vfp!!\n", __func__);
#endif
	...
	status = regulator_set_load(buck, 200000);
	if (status < 0) {
...
/*
    ...
    status = request_threaded_irq(gf_dev->irq, NULL, gf_irq,
                      ...
                      IRQF_ONESHOT, "goodix_fp_irq",
                      gf_dev);
    pr_err("%s %d request_threaded_irq %d!", __func__, __LINE__, status);
    //gf_enable_irq(gf_dev);
*/
	...
	if (1) {
		uint16_t chip_id = (uint16_t)rx_test[3];
		chip_id = (chip_id<<8) + (uint16_t)rx_test[0];
		switch (chip_id) {
			case 0x2506:
			case 0x2507:
			case 0x2508:
			case 0x2510:
				...
				gf_debug(INFO_LOG, "%s %d goodix fingerprint sensor detected\n",
				       __func__, __LINE__);
				break;
			default:
				goodix_fp_exist = false;
				gf_debug(ERR_LOG, "%s %d goodix Fingerprint sensor NOT detected!!\n",
				       __func__, __LINE__);
				//goto err_readid;
    ...
    fp_wakesrc = wakeup_source_register(gf_dev->device, "fp_wakesrc");
