...
static int enable_regulator_3V3(struct device *dev)
{
	...
	mutex_lock(&regulator_ocp_lock);
	p_3v3_vreg = devm_regulator_get(dev, "fp_vdd_vreg");
...
static int gf_open(struct inode *inode, struct file *filp)
{
	...
	if (status == 0) {
		...
		if (gf_dev->screen_state_wq) {
			queue_delayed_work(gf_dev->screen_state_wq, &gf_dev->screen_state_dw, 5 * HZ);
	...
	if (active_panel) {
		...
		if (!cookie) {
			cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
					...
					goodix_screen_state_for_fingerprint_callback, (void*)gf_dev);
			if (IS_ERR(cookie))
