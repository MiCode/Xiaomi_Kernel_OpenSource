...
static int silfp_parse_dts(struct silfp_data* fp_dev)
{
    ...
    fp_dev->pin.spi_default = pinctrl_lookup_state(fp_dev->pin.pinctrl, "spi_mode");
    ...
    fp_dev->avdd_ldo = regulator_get(&fp_dev->spi1->dev, "vfp");
...
static int silfp_set_spi(struct silfp_data *fp_dev, bool enable)
{
    ...
    spi_master_get_devdata(fp_dev->spi1->master);
...
static int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info)
{
    ...
    if(dev_info == NULL) {
        LOG_MSG_DEBUG(ERR_LOG, "[%s] dev_info is NULL\n", __func__);
        return -ENOENT;
    } else {
