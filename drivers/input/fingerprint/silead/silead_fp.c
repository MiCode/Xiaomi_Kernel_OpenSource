...
#include "../../../gpu/drm/mediatek/mediatek_v2/mi_notifier_odm.h"
...
#define FP_VENDOR 0x0666
#define FP_PRODUCT 0x0888
                             ...
                             unsigned long event, void *data)
{
    ...
    if (evdata && evdata->data && event == SIL_EVENT_BLANK && fp_dev && g_fp_dev) {
        ...
        case SIL_EVENT_POWERDOWN:
            printk("[%s] LCD OFF\n", __func__);
            g_fp_dev->wait_finger_down = true;
            fp_dev->scr_off = 1;
            g_fp_dev->scr_off = 1;
            silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_OFF);
            break;
...
#ifdef SUPPORT_SPEED_SCREEN_UNBLANK
    if ((g_fp_dev->wait_finger_down == true) && (g_fp_dev->scr_off == 1)) {
		pr_info("[XMFP]: %s enter fingerdown & scr_off then schedule_work\n", __func__);
		g_fp_dev->wait_finger_down = false;
...
static int silfp_input_init(struct silfp_data *fp_dev)
{
    ...
    fp_dev->input->id.vendor  = FP_VENDOR;
    fp_dev->input->id.product = FP_PRODUCT;
...
#else
    drm_unregister_client(&fp_dev->notif);
#endif /* CONFIG_HAS_EARLYSUSPEND */
