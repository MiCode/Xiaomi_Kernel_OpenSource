...
static struct regulator *fp_vreg = NULL;
static int disable_regulator_3V0(struct device *dev);
static int enable_regulator_3V0(struct device *dev);
...
static int disable_regulator_3V0(struct device *dev)
{
	...
	if (fp_vreg == NULL) {
		dev_err(dev, "fp_vdd_vreg is NULL!\n");
		rc = -1;
		goto disable_regulator_err;
	}

	if (regulator_is_enabled(fp_vreg)) {
		rc = regulator_disable(fp_vreg);
		if (rc) {
			goto disable_regulator_err;
		}
	}
	regulator_put(fp_vreg);
	fp_vreg = NULL;
	dev_err(dev, "disable fp_vdd_vreg voltage success!\n");
	mutex_unlock(&regulator_ocp_lock);
	return rc;

disable_regulator_err:
	dev_err(dev, "disable fp_vdd_vreg voltage failed.\n");
	mutex_unlock(&regulator_ocp_lock);
	return rc;
}
...
static int enable_regulator_3V0(struct device *dev)
{
	...
	mutex_lock(&regulator_ocp_lock);
	if (fp_vreg == NULL) {
		dev_err(dev, "Try to enable fp_vdd_vreg.\n");
		fp_vreg = regulator_get(dev, "fp_vdd_vreg");
	...
	if (regulator_is_enabled(fp_vreg)) {
		dev_err(dev, "fp_vdd_vreg is already enabled.\n");
	} else {
		rc = regulator_set_load(fp_vreg, 200000);

		if (rc) {
			dev_err(dev, "fp_vdd_vreg set load faild.\n");
			goto enable_regulator_err;
		}

		rc = regulator_enable(fp_vreg);
	...
	mutex_unlock(&regulator_ocp_lock);
	dev_err(dev, "fp_vdd_vreg is enabled success!\n");
	return rc;

enable_regulator_err:
	regulator_put(fp_vreg);
	fp_vreg = NULL;
get_regulator_err:
	mutex_unlock(&regulator_ocp_lock);
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_ENABLE_POWER:
		...
		if (gf_dev->device_available == 1) {
			...
			enable_regulator_3V0(&gf_dev->spi->dev);
		}
	...
	case GF_IOC_DISABLE_POWER:
		...
		if (gf_dev->device_available == 0) {
			...
			disable_regulator_3V0(&gf_dev->spi->dev);
		}
...
static void goodix_fb_state_chg_callback(enum panel_event_notifier_tag notifier_tag,
			struct panel_event_notification *notification, void *client_data)
{
	...
	if (gf_dev && notifier_tag == PANEL_EVENT_NOTIFICATION_PRIMARY) {
...
static int gf_open(struct inode *inode, struct file *filp)
{
	...
	if (status == 0) {
		...
		rc = enable_regulator_3V0(&gf_dev->spi->dev);
	...
	if (prim_panel) {
		...
		if (!cookie) {
			cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
					...
					goodix_fb_state_chg_callback, (void*)gf_dev);
			if (IS_ERR(cookie))
