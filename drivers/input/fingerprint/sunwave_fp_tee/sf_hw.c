...
static struct pinctrl *sf_pinctrl;
...
static int sf_ctl_device_reset(void)
{
	...
	err = pinctrl_select_state(sf_pinctrl,
			sf_pin_states[SF_PIN_STATE_RST_HIGH]);
	msleep(1);
	err = pinctrl_select_state(sf_pinctrl,
			sf_pin_states[SF_PIN_STATE_RST_LOW]);
	msleep(10);
	err = pinctrl_select_state(sf_pinctrl,
			sf_pin_states[SF_PIN_STATE_RST_HIGH]);
#endif
...
static int sf_ctl_device_power_by_gpio(bool on)
{
	...
	if (on) {
		err = pinctrl_select_state(sf_pinctrl,
				sf_pin_states[SF_PIN_STATE_PWR_ON]);
		msleep(10);
	} else {
		err = pinctrl_select_state(sf_pinctrl,
				sf_pin_states[SF_PIN_STATE_PWR_OFF]);
	}
...
static int sf_ctl_device_init_gpio_pins(struct sf_ctl_device *ctl_dev)
{
	...
	sf_ctl_dev = ctl_dev;
	xprintk(SF_LOG_LEVEL, "%s(..) enter.\n", __func__);
	ctl_dev->pdev->dev.of_node = of_find_compatible_node(NULL, NULL,
				     COMPATIBLE_SW_FP);
	...
	xprintk(KERN_INFO, "irq number is %d.\n", ctl_dev->irq_num);
	{
		...
		/* init spi,sunch as cs clck miso mosi mode,
		...
		for (i = SF_PIN_STATE_INT_SET + 1; i < SF_PIN_STATE_MAX; ++i) {
			...
			if (err) {
				xprintk(KERN_ERR,
				"%s() pinctrl_select_state(%s) failed.\n",
				__func__,
				sf_pinctrl_state_names[i]);
				break;
			}
...
#else
	ctl_dev->rst_num = of_get_named_gpio(ctl_dev->pdev->dev.of_node,
					     COMPATIBLE_RESET_GPIO, 0);
	ctl_dev->irq_pin = of_get_named_gpio(ctl_dev->pdev->dev.of_node,
					     COMPATIBLE_IRQ_GPIO, 0);
	xprintk(KERN_INFO, "reset pin number = %d\n", ctl_dev->rst_num);
	xprintk(KERN_INFO, "irq pin number = %d\n", ctl_dev->irq_pin);
#if (SF_POWER_MODE_SEL == PWR_MODE_GPIO)
	ctl_dev->pwr_num = of_get_named_gpio(ctl_dev->pdev->dev.of_node,
					     COMPATIBLE_PWR_GPIO, 0);
	...
	if (regulator_count_voltages(ctl_dev->vdd_reg) > 0) {
		...
		if (err) {
			xprintk(KERN_ERR,
				"Regulator set_vtg failed vdd err = %d\n",
				err);
			return err;
		}
	}
