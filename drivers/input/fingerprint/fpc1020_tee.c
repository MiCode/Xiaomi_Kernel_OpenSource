...
static int fpc_register_powermanger(struct fpc1020_data *gs);
static int fpc_unregister_powermanger(struct fpc1020_data *gs);
...
static int fpc1020_probe(struct platform_device *pdev)
{
	...
	rc = hw_reset(fpc1020);
	fpc_register_powermanger(fpc1020);
	dev_info(dev, "%s: ok\n", __func__);
exit:
	return rc;
}
static int fpc1020_remove(struct platform_device *pdev)
{
	...
	fpc_unregister_powermanger(fpc1020);
...
static int fpc_suspend(struct fpc1020_data *pdev)
{
	struct fpc1020_data *fpc1020 = pdev;
	disable_irq(gpio_to_irq(fpc1020->irq_gpio));
	return 0;
}
static int fpc_resume(struct fpc1020_data *pdev)
{
	struct fpc1020_data *fpc1020 = pdev;
	enable_irq(gpio_to_irq(fpc1020->irq_gpio));
	return 0;
}
static int fpc_fb_notifier_callback(struct notifier_block *noti, unsigned long event, void *data)
{
	struct fb_event *ev_data = data;
	struct fpc1020_data *fpcds = container_of(noti, struct fpc1020_data, notifier);
	int *blank;
	if (ev_data && ev_data->data && event == FB_EVENT_BLANK && fpcds) {
		blank = ev_data->data;
...
#endif
static int fpc_register_powermanger(struct fpc1020_data *fpcds)
{
#if   defined(CONFIG_FB)
	fpcds->notifier.notifier_call = fpc_fb_notifier_callback;
	fb_register_client(&fpcds->notifier);
#endif
	return 0;
}
static int fpc_unregister_powermanger(struct fpc1020_data *fpcds)
{
#if   defined(CONFIG_FB)
	fb_unregister_client(&fpcds->notifier);
#endif
	return 0;
}
