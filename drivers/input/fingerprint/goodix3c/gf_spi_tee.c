...
*/
//boost cpu
static int gf_fingerprint_freq_set(void)
{
	int i, cluster_num;

	gf_debug(DEBUG_LOG, "gf_fingerprint_freq_set\n");
...
static int gf_fingerprint_freq_release(void)
{
	int i,cluster_num;

	gf_debug(DEBUG_LOG, "gf_fingerprint_freq_release\n");
	cluster_num = arch_get_nr_clusters();
	if(cluster_num > BSP_CERVINO_CLUSTER_NUMBERS)
		cluster_num = BSP_CERVINO_CLUSTER_NUMBERS;
...
{
	gf_debug(DEBUG_LOG, "notification_work gf unblank start\n");
	gf_fingerprint_freq_set();
	mdss_prim_panel_fb_unblank(FP_UNLOCK_REJECTION_TIMEOUT);
	gf_fingerprint_freq_release();
	gf_debug(DEBUG_LOG, "notification_work gf unblank end\n");
}
...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	gf_dev->pins_irq = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "default");
...
static irqreturn_t gf_irq(int irq, void *handle)
{
	...
	gf_debug(INFO_LOG, "%s \n", __func__);
	__pm_wakeup_event(gf_dev->ttw_wl, msecs_to_jiffies(3000));

	gf_netlink_send(gf_dev, GF_NETLINK_IRQ);
	gf_dev->sig_count++;

		gf_debug(INFO_LOG, "%s fastScreenOn wait_finger_down = %d,fb_black = %d \n",
				__func__, gf_dev->wait_finger_down, gf_dev->fb_black);
	if ((gf_dev->fb_black == true ) && (gf_dev->wait_finger_down == true)) {
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case GF_IOC_INIT:
		...
		gf_debug(INFO_LOG, "[%s] : register_gf_fb_notifier_callback\n", __func__);
...
static int gf_probe(struct spi_device *spi)
{
	...
	//Set GPIO25-26 to SPI_MI and SPI_CS 
	...
	*/
	...
	gf_dev->ttw_wl = wakeup_source_register("goodix_ttw_wl");
