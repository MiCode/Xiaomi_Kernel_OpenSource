...
static const char * const pctl_names[] = {
	...
	"fpsensor_fpc_spi_cs_up",
	"fpsensor_fpc_spi_cs_low",
};
...
static int fpc_read_id(struct fpc_data *fpc,struct spi_device *spidev)
 {
    ...
    if((rx[1] == 0x18 && rx[2] == 0x11) || (rx[1] == 0x18 && rx[2] == 0x21))
    {
     	fpc1542_fp_exist = 1;
...
static int mtk6797_probe(struct spi_device *spidev)
{
    ...
    select_pin_ctl(fpc, "fpsensor_fpc_spi_cs_up");
    ...
    {
		select_pin_ctl(fpc, "fpsensor_fpc_rst_low");
		select_pin_ctl(fpc, "fpsensor_fpc_spi_cs_low");
