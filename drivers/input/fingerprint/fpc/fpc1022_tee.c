...
struct fpc1022_data {
	...
	//struct pinctrl_state *fpc_hw_power_disable;
	//struct pinctrl_state *fpc_hw_power_enable;
			    ...
			    size_t count)
{
	...
	//pr_err("%s hw_reset_set enter disabel pwr \n", __func__);
	if (!strncmp(buf, "reset", strlen("reset"))) {

		pr_err("%s hw_reset_set enter reset\n", __func__);
                power_reset();
...
/*static int check_hwid(struct spi_device *spi)
{
	...
	do {
		...
		if (((FPC1022_CHIP_MASK_SENSOR_TYPE & hardware_id) ==
		    FPC1022_CHIP) || ((FPC1021_CHIP_MASK_SENSOR_TYPE & hardware_id) ==
		    FPC1021_CHIP)) {
...
/*N17 code for HQ-291632 by zhujingyi at 2023/04/24 start */
    ...
    if (spi_fingerprint == NULL) {
        pr_notice("%s spi device is NULL,cannot do spi transfer\n", __func__);
        goto err_fpc1022_malloc;
    } else {
        fpc1022->spi = spi_fingerprint;
        buck = regulator_get(&fpc1022->spi->dev, "fp_vdd_vreg");

        if (buck == NULL) {
            dev_err(dev, "failed to get fp_vdd_vreg!!\n");
