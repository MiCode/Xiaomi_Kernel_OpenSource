...
//boost cpu
struct ppm_limit_data fpc_fingerprint_freq_to_set[BSP_CERVINO_CLUSTER_NUMBERS];
struct ppm_limit_data fpc_fingerprint_freq_to_release[BSP_CERVINO_CLUSTER_NUMBERS];
...
static int fpc_fingerprint_freq_set(void)
{
  ...
  for (i = 0; i < BSP_CERVINO_CLUSTER_NUMBERS; i++) {
    fpc_fingerprint_freq_to_set[i].min = 2001000;
    fpc_fingerprint_freq_to_set[i].max = -1;
  }

  if(cluster_num > 0){
    update_userlimit_cpu_freq(CPU_KIR_FINGERPRINT, cluster_num, fpc_fingerprint_freq_to_set);
    return 0;
  }
...
static int fpc_fingerprint_freq_release(void)
{
  ...
  for (i = 0; i < BSP_CERVINO_CLUSTER_NUMBERS; i++) {
    fpc_fingerprint_freq_to_release[i].min = -1;
    fpc_fingerprint_freq_to_release[i].max = -1;
  }

  if(cluster_num > 0){
    update_userlimit_cpu_freq(CPU_KIR_FINGERPRINT, cluster_num, fpc_fingerprint_freq_to_release);
    return 0;
  }
...
static irqreturn_t fpc1022_irq_handler(int irq, void *handle)
{
         ...
         ** since this is interrupt context (other thread...) */
         ...
         if (fpc1022->wakeup_enabled) {
                   __pm_wakeup_event(fpc1022->ttw_wl,
...
{
  struct fpc1022_data *fpc1022 = container_of(handler, struct fpc1022_data,
      early_suspend);
  printk("%s enter\n", __func__);
...
static int fpc1022_platform_probe(struct platform_device *pldev)
{
	...
	*/
         fpc1022->st_rst_h = pinctrl_lookup_state(fpc1022->pinctrl, "reset_high");
	...
	/* regu_buck = regulator_get(NULL, "irtx_ldo"); */
	/* if (regu_buck ==NULL) { */
	/* 	pr_err("regulator_get fail"); */
	/* 	goto err_create_sysfs; */
	/* } */
	/* ret = regulator_set_voltage(regu_buck, 2800000, 2800000); */
	/* if (ret < 0) { */
	/* 	pr_err("regulator_set_voltage fail"); */
	/*     goto err_create_sysfs; */
	/* } */
	/* ret = regulator_enable(regu_buck); */
	/* if (ret < 0) { */
	/* 	pr_err("%s, regulator_enable fail!!\n" , __func__); */
	/* 	goto err_create_sysfs; */
	/* } */
...
static int check_hwid(struct spi_device *spi)
{
         ...
         do {
		   ...
		   pr_err("fpc hardware_id[0]= 0x%x id[1]=0x%x\n", tmp_buf[0], tmp_buf[1]);
                   ...
                   if((FPC1022_CHIP_MASK_SENSOR_TYPE & hardware_id) == FPC1022_CHIP)
                   {
...
__maybe_unused static struct spi_driver spi_driver = {
         .driver = {
                   .name       = "btp",
                   .owner     = THIS_MODULE,
...
static int __init fpc1022_init(void)
{
    ...
    if (goodix_fp_exist) {
        pr_err("%s goodix sensor has been detected, so exit FPC sensor detect.\n",__func__);
        return -EINVAL;
    }
