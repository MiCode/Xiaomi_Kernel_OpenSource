...
#include "mi-ufs-fault-injection.h"
#include "mi_ufs_bsg.h"
#include "mi-ufshcd-crypto.h"
#include "mi_ufshpb.h"
...
#define QUERY_REQ_TIMEOUT 5500 /* 1.5 seconds */
...
#define MI_UFS_GEAR4_ENABLE false   /* default not enable G4 */
#define MI_UFS_FORCE_GEAR5 false   /* default not enable G4 */
#define MI_UFS_GEAR4_MAX_TIME 4 /* 100 millisecs */
#define MI_UFS_GEAR4_THRESHOLD 0 /* GEAR4 THRESHOLD */
#define MI_UFS_GEAR5_THRESHOLD 70 /* GEAR5_THRESHOLD */
#define MI_UIC_THRESHOLD 10
...
static void ufshcd_device_reset(struct ufs_hba *hba)
{
	...
	if (!err) {
		...
		if (ufshcd_is_wb_allowed(hba)) {
			...
			hba->dev_info.wb_flush_during_h8 = false;
		}
	}
	if (err != -EOPNOTSUPP)
...
static bool is_gear4;
static unsigned int gear4_count;
static unsigned int ufs_curr_capacity;
					       ...
					       bool scale_up)
{
	...
	list_for_each_entry(clki, head, list) {
		if (!IS_ERR_OR_NULL(clki->clk)) {
			if (scale_up && clki->max_freq) {
				if (clki->curr_freq == clki->max_freq && gear4_count < hba->gear4_time)
					continue;
				return true;
			} else if (!scale_up && clki->min_freq) {
				if (clki->curr_freq == clki->min_freq)
...
static int ufshcd_scale_gear(struct ufs_hba *hba, bool scale_up)
{
	...
	if (scale_up) {
		memcpy(&new_pwr_info, &hba->max_pwr_info.info,
		       sizeof(struct ufs_pa_layer_attr));

		/* set pwr info to gear4, if needed */
		if (!hba->force_gear5 && hba->gear4_enable && is_gear4) {
			new_pwr_info.gear_tx = UFS_HS_G4;
			new_pwr_info.gear_rx = UFS_HS_G4;
		}
				...
				unsigned long *freq, u32 flags)
{
	...
	*freq = (unsigned long) clk_round_rate(clki->clk, *freq);
	...
	/* Decide gear4 enable or not*/
	if (scale_up) {
		if (hba->pwr_info.gear_tx == UFS_HS_G5 && hba->pwr_info.gear_rx == UFS_HS_G5) {
			gear4_count = 0;
			is_gear4 = false;
		} else if (hba->gear4_enable && ufs_curr_capacity > hba->gear4_threshold &&
				ufs_curr_capacity < hba->gear5_threshold && gear4_count < hba->gear4_time) {
			gear4_count ++;
			is_gear4 = true;
		} else if (hba->pwr_info.gear_tx == UFS_HS_G4 && ufs_curr_capacity > hba->gear5_threshold)
                {
			gear4_count = 4;
			is_gear4 = false;
		} else {
	...
	/* performance mode force gear5*/
	if (hba->force_gear5 || !strcmp(hba->devfreq->governor->name, "performance")) {
		scale_up = true;
		gear4_count = hba->gear4_time;
	}
		...
		struct devfreq_dev_status *stat)
{
	...
	ufs_curr_capacity = stat->busy_time * 100 / stat->total_time;
...
void ufshcd_wb_toggle_flush_during_h8(struct ufs_hba *hba, bool set)
{
	...
	hba->dev_info.wb_flush_during_h8 = set ? true:false;
...
static int ufshcd_probe_hba(struct ufs_hba *hba, bool init_dev_params)
{
	...
	hba->gear4_enable = MI_UFS_GEAR4_ENABLE;
	hba->force_gear5 = MI_UFS_FORCE_GEAR5;
	hba->gear4_time = MI_UFS_GEAR4_MAX_TIME;
	hba->gear4_threshold = MI_UFS_GEAR4_THRESHOLD;
	hba->gear5_threshold = MI_UFS_GEAR5_THRESHOLD;
...
{
	//dev_err(hba->dev, " keep vcc/vccq always on + npm \n", __func__);
