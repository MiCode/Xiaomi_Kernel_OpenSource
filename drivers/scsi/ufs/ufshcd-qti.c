...
static int ufshcd_tag_req_type(struct request *rq)
{
	...
	if (!rq)
		...
		rq_type = (rq->cmd_flags & REQ_HIPRI) ?
			TS_URGENT_READ : TS_READ;
	else if (rq->cmd_flags & REQ_HIPRI)
...
static inline void ufshcd_copy_sense_data(struct ufshcd_lrb *lrbp)
{
	...
	if (lrbp->sense_buffer &&
		...
		memcpy(lrbp->sense_buffer,
			...
			min_t(int, len_to_copy, UFS_SENSE_SIZE));
	}
}
 ...
 */
static int ufshcd_prepare_req_desc_hdr(struct ufshcd_lrb *lrbp,
					u32 *upiu_flags,
...
static int ufshcd_comp_devman_upiu(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	ret = ufshcd_prepare_req_desc_hdr(lrbp, &upiu_flags,
			DMA_NONE);
...
static int ufshcd_comp_scsi_upiu(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	if (likely(lrbp->cmd)) {
		ret = ufshcd_prepare_req_desc_hdr(lrbp,
...
EXPORT_SYMBOL(ufshcd_dme_set_attr);
...
EXPORT_SYMBOL(ufshcd_dme_get_attr);
...
EXPORT_SYMBOL(ufshcd_config_pwr_mode);
...
static int ufshcd_reset_and_restore(struct ufs_hba *hba)
{
	 ...
	 */
	WARN_ON(err && ufshcd_is_embedded_dev(hba));
...
static int ufs_get_device_desc(struct ufs_hba *hba)
{
	...
	dev_info->i_product_name = desc_buf[DEVICE_DESC_PARAM_PRDCT_NAME];

	model_index = desc_buf[DEVICE_DESC_PARAM_PRDCT_NAME];


	err = ufshcd_read_string_desc(hba, model_index, &dev_info->model,
				      true/*ASCII*/);
	if (err < 0) {
...
EXPORT_SYMBOL(ufshcd_dump_regs);

MODULE_DESCRIPTION("QTI UFS host controller driver Core");
