...
static u8 ufstw_get_query_idx(struct ufstw_lu *tw)
{
	...
	if (tw->ufsf->tw_dev_info.tw_buf_type == TW_SINGLE_SHARE_BUFFER_TYPE)
		idx = 0;
	else
		idx = (u8)tw->lun;
...
static inline void ufstw_cancel_lu_jobs(struct ufstw_lu *tw)
{
	...
	INFO_MSG("cancel_delayed_work_sync(tw_flush_work) ufstw_lu%d = %d",
		  tw->lun, ret);
...
{
	INFO_MSG("tw_dev [0x4d] wTurboWriteVersion Driver = %.4x, Device = %.4x",
...
INFO_MSG("tw_dev [0x53] bTurboWriteBufferNoUserSpaceReductionEn %u",
		 tw_dev_info->tw_buf_no_reduct);
	INFO_MSG("tw_dev [0x54] bTurboWriteBufferType %u",
...
void ufstw_get_geo_info(struct ufstw_dev_info *tw_dev_info, u8 *geo_buf)
{
	...
	INFO_MSG("tw_geo [0x4F:0x52] dTurboWriteBufferMaxNAllocUnits %u",
		 LI_EN_32(&geo_buf[GEOMETRY_DESC_TW_MAX_SIZE]));
	INFO_MSG("tw_geo [0x53] bDeviceMaxTurboWriteLUs %u",
		 tw_dev_info->tw_number_lu);
	INFO_MSG("tw_geo [0x54] bTurboWriteBufferCapAdjFac %u",
		 geo_buf[GEOMETRY_DESC_TW_CAP_ADJ_FAC]);
	INFO_MSG("tw_geo [0x55] bSupportedTurboWriteBufferUserSpaceReductionTypes %u",
		 geo_buf[GEOMETRY_DESC_TW_SUPPORT_USER_REDUCTION_TYPES]);
	INFO_MSG("tw_geo [0x56] bSupportedTurboWriteBufferTypes %u",
...
int ufstw_get_lu_info(struct ufsf_feature *ufsf, int lun, u8 *lu_buf)
{
	...
	if ((lu_desc.tw_lu_buf_size) ||
		((ufsf->tw_dev_info.tw_buf_type == TW_SINGLE_SHARE_BUFFER_TYPE)
		&& (lun == 2))) {
		ufsf->tw_lup[lun] =
		...
		tw->ufsf = ufsf;
		tw->lun = lun;
		INIT_INFO("tw_lu LUN(%d) [0x29:0x2C] dLUNumTurboWriteBufferAllocUnits %u",
...
static void ufstw_lu_init(struct ufsf_feature *ufsf, int lun)
{
    ...
    INIT_INFO("TW tw->tw_enable = 0x%x",tw->tw_enable);
    INIT_INFO("TW tw->tw_flush_enable = 0x%x",tw->tw_flush_enable);
    INIT_INFO("TW tw->tw_flush_during_hibern_enter = 0x%x",tw->tw_flush_during_hibern_enter);
    INIT_INFO("TW tw->tw_flush_status = 0x%x",tw->tw_flush_status);
    INIT_INFO("TW tw->tw_available_buffer_size = 0x%x",tw->tw_available_buffer_size);
    INIT_INFO("TW tw->tw_lifetime_est = 0x%x",tw->tw_lifetime_est);
...
define_sysfs_attr_r_function(current_tw_buffer_size, QUERY_ATTR_IDN_TW_CURR_BUFF_SIZE)
