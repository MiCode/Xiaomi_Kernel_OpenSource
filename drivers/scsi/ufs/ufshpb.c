				   ...
				   int transfer_len)
{
	...
	do {
		...
		/*
		 * rgn_idx < hpb->srgns_per_lu && srgn_idx < hpb->srgns_per_rgn
		 */
		if (rgn_idx >= hpb->rgns_per_lu || srgn_idx >= hpb->srgns_per_rgn)
			return 0;
...
static ssize_t ufshpb_sysfs_region_stat_show(struct ufshpb_lu *hpb, char *buf)
{
	...
	ret = snprintf(buf, sizeof(buf), "Total %d pinned %d active %d inactive %d\n",
