...
static struct ufs_dev_info dev_info;
...
static inline bool ufshpb_is_support_chunk(int transfer_len)
{
	...
	if (dev_info.w_manufacturer_id == UFS_VENDOR_MICRON){
		return transfer_len <= HPB_32_CHUNK_LEN; //Micron current device don't support larger than 32K tunck size
	} else {
				     ...
				     struct scsi_cmnd *cmd)
{
	...
	if (dev_info.w_manufacturer_id == UFS_VENDOR_MICRON){
		return -EINVAL;// Micron current device don't supprot write buffer feature
	}
...
static inline int ufshpb_may_field_valid(struct ufshcd_lrb *lrbp,
					 struct ufshpb_rsp_field *rsp_field)
{
	int max_inactive_num = 0;

	if (dev_info.w_manufacturer_id == UFS_VENDOR_MICRON)
		max_inactive_num = MC_MAX_INACTIVE_NUM;
	else
		max_inactive_num = SS_MAX_INACTIVE_NUM;
...
static inline int ufshpb_version_check(struct ufshpb_dev_info *hpb_dev_info)
{
	...
	dev_info = ufsf->hba->dev_info;

	switch (dev_info.w_manufacturer_id){
		case UFS_VENDOR_SAMSUNG:
			hpb_version = UFSHPB_SAMSUNG_VER;
			break;
		case UFS_VENDOR_MICRON:
			hpb_version = UFSHPB_MICRON_VER;
			break;
		default:
			hpb_version = UFSHPB_VER;
	}

	if (hpb_dev_info->hpb_ver != hpb_version) {
