...
static int max_cmd_loop_cnt;
static int cmd_hist_cnt;
			    ...
			    struct seq_file *m)
{
	...
	ufs_mtk_dbg_print_err_hist(buff, size, m,
			      UFS_EVT_PERF_WARN, "performance_warning");
}
				 ...
				 u32 intr, u64 lba, u8 opcode, u8 group_id)
{
	...
	if (cmd_hist[cmd_hist_ptr].duration >= 1000000000) {
		/*
		* op code[31:24]
		* length(4KB)[23:16]
		* duration(ms)[15:0]
		*/
		ufshcd_update_evt_hist(ufshba, UFS_EVT_PERF_WARN,
		    (u32) ((opcode << 24) |
		    (((transfer_len >> 12) & 0xFF) << 16) |
		    (cmd_hist[cmd_hist_ptr].duration / 1000000)));
	}
					...
					struct seq_file *m, int ptr)
{
	...
	SPREAD_PRINTF(buff, size, m,
		"CLK:no=%d:%6ld,%3d-c(%d),%6llu.%lu,%5d,%2d,%13s\n",
		max_cmd_loop_cnt,
		cmd_hist[ptr].seq_no,
		ptr,
		cmd_hist[ptr].cpu,
					...
					struct seq_file *m, int ptr)
{
	...
	dur = ns_to_timespec64(cmd_hist[ptr].time);
	SPREAD_PRINTF(buff, size, m,
		"RESET:no=%d:%6ld,%3d-c(%d),%6llu.%lu,%5d,%2d,%13s\n",
		max_cmd_loop_cnt,
		cmd_hist[ptr].seq_no,
		ptr,
		cmd_hist[ptr].cpu,
				   ...
				   struct seq_file *m, int ptr)
{
	...
	dur = ns_to_timespec64(cmd_hist[ptr].time);
	SPREAD_PRINTF(buff, size, m,
		"UIC:no=%d:%6ld,%3d-u(%d),%6llu.%lu,%5d,%2d,0x%2x,arg1=0x%X,arg2=0x%X,arg3=0x%X,\t%llu\n",
		max_cmd_loop_cnt,
		cmd_hist[ptr].seq_no,
		ptr,
		cmd_hist[ptr].cpu,
				   ...
				   struct seq_file *m, int ptr)
{
	...
	SPREAD_PRINTF(buff, size, m,
		"UTP:no=%d:%6ld,%3d-r(%d),%6llu.%lu,%5d,%2d,0x%2x,t=%2d,db:0x%8x,is:0x%8x,crypt:%d,%d,lba=%10llu,len=%6d,\t%llu\n",
		max_cmd_loop_cnt,
		cmd_hist[ptr].seq_no,
		ptr,
		cmd_hist[ptr].cpu,
				   ...
				   struct seq_file *m, int ptr)
{
	...
	if (cmd_hist[ptr].cmd.utp.lba == 0xFFFFFFFFFFFFFFFF)
		cmd_hist[ptr].cmd.utp.lba = 0;
	SPREAD_PRINTF(buff, size, m,
		"TM:no=%d:%6ld,%3d-r(%d),%6llu.%lu,%5d,%2d,0x%2x,lun=%d,tag=%d,task_tag=%d\n",
		max_cmd_loop_cnt,
		cmd_hist[ptr].seq_no,
		ptr,
		cmd_hist[ptr].cpu,
