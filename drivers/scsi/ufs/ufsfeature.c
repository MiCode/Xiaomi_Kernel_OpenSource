		    ...
		    enum flag_idn idn, u8 index, bool *flag_res)
{
	...
	if (hba->dev_info.w_manufacturer_id == UFS_VENDOR_SAMSUNG ||
		hba->dev_info.w_manufacturer_id == UFS_VENDOR_MICRON)
...
static int check_if_bug_on()
{
	...
	if (!ret && (!strncmp(hwlevel, "P2", strlen("P2"))
		|| !strncmp(hwlevel, "P2.1", strlen("P2.1"))
		|| !strncmp(hwlevel, "P2.2", strlen("P2.2"))
...
static int ufsf_read_unit_desc(struct ufsf_feature *ufsf, int lun, u8 selector)
{
	...
	if (ufsf->hpb_dev_info.hpb_device) {
        ...
        if (ufsf->hba->dev_info.w_manufacturer_id == UFS_VENDOR_SAMSUNG
         || ufsf->hba->dev_info.w_manufacturer_id == UFS_VENDOR_MICRON)
		    check_if_bug_on();
	}
#endif
