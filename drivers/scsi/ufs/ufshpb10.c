...
u32 ufshpb_debug_mask = HPB_LOG_ERR | HPB_LOG_INFO;
//u32 ufshpb_debug_mask = HPB_LOG_ERR | HPB_LOG_INFO | HPB_LOG_DEBUG | HPB_LOG_HEX;

#define HPB_READ_LARGE_CHUNK_SUPPORT
#if defined(HPB_READ_LARGE_CHUNK_SUPPORT)
#define HPB_READ_LARGE_CHUNK_DIRTY_CHECK_SUPPORT
#endif
#define HPB_READ_LARGE_CHUNK_MAX_BLOCK_COUNT (255) //TRANSFER LENGTH: 8bit
...
static int ufshpb_create_sysfs(struct ufs_hba *hba, struct ufshpb10_lu *hpb);
static void ufshpb_error_handler(struct work_struct *work);
static void ufshpb_evict_region(struct ufshpb10_lu *hpb,
						struct ufshpb10_region *cb);
static void ufshpb_purge_active_block(struct ufshpb10_lu *hpb);
static int ufshpb_set_map_req(struct ufshpb10_lu *hpb,
		int region, int subregion, struct ufshpb_map_ctx *mctx,
		struct ufshpb_rsp_info *rsp_info);
static int ufshpb_generate_map_cmd_req(struct ufshpb10_lu *hpb,
										unsigned int region,
										unsigned int subregion,
										enum HPB_BUFFER_MODE mode);
static void ufshpb_map_loading_trigger(struct ufshpb10_lu *hpb,
		bool only_pinned, bool do_work_handler);

static inline void ufshpb_purge_active_page(struct ufshpb10_lu *hpb,
		struct ufshpb10_subregion *cp, int state);

static inline void ufshpb_get_bit_offset(
		struct ufshpb10_lu *hpb, int subregion_offset,
...
static bool ufshpb_ppn_dirty_check(struct ufshpb10_lu *hpb,
		struct ufshpb10_subregion *cp, int subregion_offset)
{
	bool is_dirty;
...
#if defined(HPB_READ_LARGE_CHUNK_DIRTY_CHECK_SUPPORT)
static bool ufshpb_subregion_dirty_check(struct ufshpb10_lu *hpb,
		struct ufshpb10_subregion *cp, int subregion_offset, int *reqBlkCnt)
{
	unsigned int bit_dword, bit_offset;
	unsigned int tmp;
	int checkCnt;
	...
	ufshpb_get_bit_offset(hpb, subregion_offset,
			&bit_dword, &bit_offset);
	while (true) {
		checkCnt = BITS_PER_DWORD - bit_offset;
		if (cp->mctx->ppn_dirty[bit_dword]) {
			tmp = cp->mctx->ppn_dirty[bit_dword] << bit_offset;
			if (BITS_PER_DWORD - *reqBlkCnt > 0)
				tmp = tmp >> (BITS_PER_DWORD - *reqBlkCnt);
			if (tmp)
				return true;
		}
		*reqBlkCnt -= checkCnt;
...
static void ufshpb_ppn_prep(struct ufshpb10_lu *hpb,
		struct ufshcd_lrb *lrbp, hpb_t ppn,
		unsigned int sector_len)
{
	unsigned char cmd[16] = { 0 };

	if (hpb->hba->ufshpb_quirk & UFSHPB_QUIRK_USE_READ_16_FOR_ENCRYPTION)
		cmd[0] = READ_16;
	else
		cmd[0] = UFSHPB_READ;
	...
	cmd[14] = (u8)(sector_len >> sects_per_blk_shift);	//Transfer length
	if (hpb->hba->ufshpb_quirk & UFSHPB_QUIRK_USE_READ_16_FOR_ENCRYPTION)
		cmd[15] = 0x01; 					//Control
	else
		cmd[15] = 0x00; 					//Control
	...
	/* HPB_DRIVER_HEXDUMP("[HPB] HPB READ ", 16, 1, cmd, sizeof(cmd), 1); */
}

/* called with hpb_lock (irq) */
static inline void ufshpb_set_dirty_bits(struct ufshpb10_lu *hpb,
		struct ufshpb10_region *cb, struct ufshpb10_subregion *cp,
...
static void ufshpb_set_dirty(struct ufshpb10_lu *hpb,
...
static inline void ufshpb_get_pos_from_lpn(struct ufshpb10_lu *hpb,
...
static hpb_t ufshpb_get_ppn(struct ufshpb_map_ctx *mctx, int pos)
{
	hpb_t *ppn_table;
	int index, offset;

	index = pos / HPB10_ENTREIS_PER_OS_PAGE;
	offset = pos % HPB10_ENTREIS_PER_OS_PAGE;
...
void ufshpb10_prep_fn(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	 ...
	 *	goto normal_read;
	 */
	if (ufshpb_is_read_lrbp(lrbp) || ufshpb_is_write_discard_lrbp(lrbp)) {
		lpn = rq_pos / SECTORS_PER_BLOCK;
		ufshpb_get_pos_from_lpn(
...
#if defined(HPB_READ_LARGE_CHUNK_SUPPORT)
	if (((rq_sectors & (SECTORS_PER_BLOCK - 1)) != 0) ||
		 rq_sectors >
		 (HPB_READ_LARGE_CHUNK_MAX_BLOCK_COUNT << sects_per_blk_shift)) {
#else
...
#if defined(HPB_READ_LARGE_CHUNK_DIRTY_CHECK_SUPPORT)
	if (rq_sectors > SECTORS_PER_BLOCK) {
		struct ufshpb10_region *reg;
		struct ufshpb10_subregion *subReg;
		int regNum = region;
		int subRegNum = subregion;
		int subRegOffset = subregion_offset;
		int reqBlkCnt = rq_sectors >> sects_per_blk_shift;

		/* HPB_DRIVER_D("%s [LARG CHNK] CHECK START reqBlkCnt:%d\n",
				__func__, reqBlkCnt); */
		reg = cb;
		subReg = cp;
		while (1) {
			spin_lock_bh(&hpb->hpb_lock);
			if (ufshpb_subregion_dirty_check(
					hpb, subReg, subRegOffset, &reqBlkCnt)) {
				/* HPB_DRIVER_D("%s [LARG CHNK] NORMAL_READ DIRTY - SubRegion(%d) \n",
						__func__, subRegNum); */
				spin_unlock_bh(&hpb->hpb_lock);
			...
			if (reqBlkCnt <= 0) {
				/* HPB_DRIVER_D("%s [LARG CHNK] PASS - reqBlkCnt(%d) \n",
						 __func__, reqBlkCnt); */
				break;
			}
			if (subRegNum == cb->subregion_count-1) {
				regNum++;
				subRegNum = 0;
				if (regNum >= hpb->regions_per_lu) {
					//Something wrong, just return
					HPB_DRIVER_D("%s [LARG CHNK] NORMAL_READ OUT OF REGION \
							- Region(%d) SubRegion(%d) \n",
							__func__, regNum, subRegNum);
					atomic64_inc(&hpb->lc_err_miss);
					return;
				}
				reg = hpb->region_tbl + regNum;
			} else {
				subRegNum++;
			}
			subReg = reg->subregion_tbl + subRegNum;
			if (reg->region_state == HPBREGION_INACTIVE ||
				subReg->subregion_state != HPBSUBREGION_CLEAN) {
				HPB_DRIVER_D("%s [LARG CHNK] NORMAL_READ STATE(NUM:STATE) \
						- Region(%d:%d) SubRegion(%d:%d) \n",
						__func__, regNum, reg->region_state,
						subRegNum, subReg->subregion_state);
	...
	//HPB_DRIVER_D("XXX ufs ppn %016llx, lba %u\n", (unsigned long long)ppn, lpn);

	ufshpb_ppn_prep(hpb, lrbp, ppn, rq_sectors);
...
static int ufshpb_clean_dirty_bitmap(
		struct ufshpb10_lu *hpb, struct ufshpb10_subregion *cp)
{
	struct ufshpb10_region *cb;
		...
		struct ufshpb10_lu *hpb, struct ufshpb10_subregion *cp)
{
	...
	/* if mctx is null, active block had been evicted out */
	if (cb->region_state == HPBREGION_INACTIVE || !cp->mctx) {
		HPB_DRIVER_E("%d - %d evicted\n", cp->region, cp->subregion);
		return;
	}
	cp->subregion_state = HPBSUBREGION_DIRTY;
}

static void ufshpb_map_compl_process(struct ufshpb10_lu *hpb,
		struct ufshpb_map_req *map_req)
{

	map_req->RSP_end = ktime_to_ns(ktime_get());

	HPB_DRIVER_D("UFSHPB COMPL READ BUFFER %d - %d\n",
			map_req->region, map_req->subregion);
	HPB_DRIVER_D("start=%llu, issue=%llu, endio=%llu, endio=%llu\n",
			map_req->RSP_tasklet_enter - map_req->RSP_start,
			map_req->RSP_issue - map_req->RSP_tasklet_enter,
 ...
 */
static struct ufshpb_rsp_info *ufshpb_get_req_info(struct ufshpb10_lu *hpb)
{
	struct ufshpb_rsp_info *rsp_info =
		list_first_entry_or_null(&hpb->lh_rsp_info_free,
				...
				list_rsp_info);
	if (!rsp_info) {
		HPB_DRIVER_E("there is no rsp_info");
		return NULL;
	}
...
{
	struct ufshpb_map_req *map_req = req->end_io_data;
	struct ufs_hba *hba;
	struct ufshpb10_lu *hpb;
	...
	HPB_DRIVER_E("error number %d ( %d - %d )\n",
		error, map_req->region, map_req->subregion);
	scsi_normalize_sense(map_req->sense,
				SCSI_SENSE_BUFFERSIZE, &sshdr);
	HPB_DRIVER_E("code %x sense_key %x asc %x ascq %x\n",
				sshdr.response_code,
				sshdr.sense_key, sshdr.asc, sshdr.ascq);
	HPB_DRIVER_E("byte4 %x byte5 %x byte6 %x additional_len %x\n",
	...
	if (sshdr.sense_key == ILLEGAL_REQUEST) {
		...
		if (sshdr.asc == 0x00 && sshdr.ascq == 0x16) {
			...
			schedule_delayed_work(&hpb->ufshpb_map_req_retry_work,
...
static int ufshpb_execute_req_dev_ctx(struct ufshpb10_lu *hpb,
			      ...
			      int buf_length)
{
	...
	if (ret < 0)
		HPB_DRIVER_E("failed with err %d\n", ret);
	return ret;
}
		...
		int region, int subregion, int subregion_mem_size)
{
	...
	HPB_DRIVER_HEXDUMP("[HPB] READ BUFFER ", 16, 1, cmd, 10, 1);
}

static int ufshpb_add_bio_page(struct ufshpb10_lu *hpb,
...
static int ufshpb_map_req_issue(
	struct ufshpb10_lu *hpb, struct ufshpb_map_req *map_req)
{
	...
	if (IS_ERR(req)) {
		int rv = PTR_ERR(req);
		HPB_DRIVER_E("%s: blk_get_request errno %d, \
					 retry #%d, READ BUFFER %d:%d\n",
					 __func__, rv, map_req->retry_cnt,
					 map_req->region, map_req->subregion);
		...
		schedule_delayed_work(&hpb->ufshpb_map_req_retry_work, msecs_to_jiffies(10));
		...
		struct ufshpb_rsp_info *rsp_info)
{
	...
	map_req = list_first_entry_or_null(&hpb->lh_map_req_free,
					    ...
					    list_map_req);
	if (!map_req) {
		HPB_DRIVER_E("There is no map_req\n");
		spin_unlock(&hpb->hpb_lock);
		return -ENOMEM;
	}
	...
	map_req->RSP_tasklet_enter = rsp_info->RSP_tasklet_enter;

	INIT_LIST_HEAD(&map_req->list_map_req);
	spin_lock(&hpb->hpb_lock);
	list_add_tail(&map_req->list_map_req, &hpb->lh_map_req_retry);
	spin_unlock(&hpb->hpb_lock);
	schedule_delayed_work(&hpb->ufshpb_map_req_retry_work, 0);

	return 0;
}

static struct ufshpb_map_ctx *ufshpb_get_map_ctx(struct ufshpb10_lu *hpb)
{
	struct ufshpb_map_ctx *mctx;
...
static inline void ufshpb_add_lru_info(struct victim10_select_info *lru_info,
				       struct ufshpb10_region *cb)
{
...
static inline int ufshpb_add_region(struct ufshpb10_lu *hpb,
					struct ufshpb10_region *cb)
{
	...
	HPB_DRIVER_D("E->active region: %d", cb->region);
...
out:
	if (err)
		HPB_DRIVER_E("get mctx failed. err %d subregion %d free_table %d\n",
			err, subregion, hpb->debug_free_table);
	return err;
}

static inline void ufshpb_put_map_ctx(
		struct ufshpb10_lu *hpb, struct ufshpb_map_ctx *mctx)
{
...
static inline void ufshpb_cleanup_lru_info(
	struct victim10_select_info *lru_info,
	struct ufshpb10_region *cb)
{
...
static inline void ufshpb_evict_region(struct ufshpb10_lu *hpb,
				     struct ufshpb10_region *cb)
{
	struct victim10_select_info *lru_info;
	struct ufshpb10_subregion *cp;
	...
	if (cb->region_state == HPBREGION_INACTIVE) {
		HPB_DRIVER_E("Region:%d was already inactivated.\n",
				cb->region);
		return;
	}
	lru_info = &hpb->lru_info;

	HPB_DRIVER_D("C->EVICT region: %d\n", cb->region);
...
static void ufshpb_hit_lru_info(struct victim10_select_info *lru_info,
				       struct ufshpb10_region *cb)
{
...
static struct ufshpb10_region *ufshpb_victim_lru_info(
				struct victim10_select_info *lru_info)
{
	/* struct ufshpb10_region *cb; */
	struct ufshpb10_region *victim_cb = NULL;
	/* u32 hit_count = 0xffffffff; */

	switch (lru_info->selection_type) {
	case LRU:
		victim_cb = list_first_entry(&lru_info->lru,
				struct ufshpb10_region, list_region);
		break;
#if 0
	case LFU:
...
static int ufshpb_evict_load_region(struct ufshpb10_lu *hpb,
				struct ufshpb_rsp_info *rsp_info)
{
	struct ufshpb10_region *cb;
	struct ufshpb10_region *victim_cb;
	struct victim10_select_info *lru_info;
	int region, ret, iter;

	lru_info = &hpb->lru_info;
	HPB_DRIVER_D("active_cnt :%lld\n",
			(long long)atomic64_read(&lru_info->active_count));
	...
	for (iter = 0; iter < rsp_info->inactive_cnt; iter++) {
		...
		if (cb->is_pinned) {
			 ...
			 */
			HPB_DRIVER_E("UFSHPB pinned active-block drop-out error\n");
			return -ENOMEM;
		}
	...
	for (iter = 0; iter < rsp_info->active_cnt; iter++) {
		...
		if(!cb->is_pinned){
			...
			spin_lock(&hpb->hpb_lock);
			// If not inactive, the memory is already prepared.
			if (cb->region_state != HPBREGION_INACTIVE) {
				spin_unlock(&hpb->hpb_lock);
				continue;
			}
			...
			if (atomic64_read(&lru_info->active_count) ==
				...
				if (!victim_cb) {
					HPB_DRIVER_E("UFSHPB victim_cb is NULL\n");
					goto unlock_error;
				}

				HPB_DRIVER_D("max lru case. victim : %d\n",
						victim_cb->region);
		...
		if (ret) {
			HPB_DRIVER_E("UFSHPB memory allocation failed\n");
			goto unlock_error;
		}
		spin_unlock(&hpb->hpb_lock);
	}
...
static inline struct ufshpb10_rsp_field *ufshpb_get_hpb_rsp(
		struct ufshcd_lrb *lrbp)
{
	return (struct ufshpb10_rsp_field *)&lrbp->ucd_rsp_ptr->sr.sense_data_len;
}
										...
										enum HPB_BUFFER_MODE mode)
{
	...
	if (region >= hpb->regions_per_lu)
		return -ERANGE;
	if (mode == R_BUFFER) {
		if (subregion >= hpb->region_tbl[region].subregion_count)
			return -ERANGE;
	}
	...
	} else if (mode == W_BUFFER) {
		rsp_info->active_cnt = 0;
		rsp_info->inactive_cnt = 1;
		rsp_info->inactive_list.region[0] = region;
	}
...
static void ufshpb_rsp_map_cmd_req(struct ufshpb10_lu *hpb,
		struct ufshpb_rsp_info *rsp_info)
{
	struct ufshpb10_region *cb;
	struct ufshpb10_subregion *cp;
	...
	if (ret) {
		HPB_DRIVER_E("region evict/load failed. ret %d\n", ret);
	...
	for (iter = 0; iter < rsp_info->active_cnt; iter++) {
		...
		if (region >= hpb->regions_per_lu ||
			...
			HPB_DRIVER_E("ufshpb issue-map %d - %d range error\n",
				region, subregion);
			goto wakeup_ee_worker;
		}
		...
		ret = ufshpb_set_map_req(hpb, region, subregion,
						cp->mctx, rsp_info);
		if (ret) {
			HPB_DRIVER_E("ufshpb_set_map_req error %d\n", ret);
			goto wakeup_ee_worker;
		}
	}
	return;
...
{
	struct ufshpb10_lu *hpb;
	struct ufshpb10_rsp_field *rsp_field;
	struct ufshpb_rsp_info *rsp_info;
	...
	if (rsp_field->lun >= UFS_UPIU_MAX_GENERAL_LUN) {
		HPB_DRIVER_E("lun is not general = %d", rsp_field->lun);
		return;
	}
	hpb = hba->ufshpb_lup[rsp_field->lun];
	if (!hpb) {
		HPB_DRIVER_E("%s:%d UFS-LU%d is not UFSHPB LU\n", __func__,
			__LINE__, rsp_field->lun);
		return;
	}
	if (hpb->force_map_req_disable)
		return;
	HPB_DRIVER_D("HPB-Info Noti: %d LUN: %d Seg-Len %d, Req_type = %d\n",
		rsp_field->hpb_type, rsp_field->lun,
		data_seg_len, rsp_field->hpb_type);
	if (!hpb->lu_hpb_enable) {
		HPB_DRIVER_E("UFSHPB(%s) LU(%d) not HPB-LU\n",
				__func__, rsp_field->lun);
		return;
	}

	//To verify the values within RESPONSE UPIU.
	HPB_DRIVER_HEXDUMP("[HPB] RESP UPIU ", 16, 1,
		...
		for (num = 0; num < rsp_field->active_region_cnt; num++) {
			...
			rsp_info->active_list.subregion[num] =
				get_unaligned_be16(rsp_field->hpb_active_field
								+ blk_idx + 2);
			HPB_DRIVER_D("active num: %d, #block: %d, page#: %d\n",
				num + 1,
				rsp_info->active_list.region[num],
				rsp_info->active_list.subregion[num]);
		}
	...
	case HPB_RSP_HPB_RESET:
		for (num = 0 ; num < UFS_UPIU_MAX_GENERAL_LUN ; num++)
		{
			hpb = hba->ufshpb_lup[num];
			if (!hpb || !hpb->lu_hpb_enable)
				continue;
			...
			rsp_info->type = HPB_RSP_HPB_RESET;
			rsp_info->RSP_start = ktime_to_ns(ktime_get());
	...
	default:
		HPB_DRIVER_E("hpb_type is not available : %d\n",
					rsp_field->hpb_type);
		break;
	}
}
		...
		u8 desc_id, u8 desc_index, u8 *desc_buf, u32 size)
{
	...
	if (err) {
		HPB_DRIVER_E("%s:%d reading Device Desc failed. err = %d\n",
			__func__, __LINE__, err);
	}
	return err;
}

static int ufshpb_read_device_desc(
...
static inline void ufshpb_add_subregion_to_req_list(struct ufshpb10_lu *hpb,
		struct ufshpb10_subregion *cp)
{
...
static int ufshpb_execute_req(struct ufshpb10_lu *hpb, unsigned char *cmd,
		struct ufshpb10_subregion *cp)
{
	struct ufs_hba *hba = hpb->hba;
	struct scsi_device *sdp;
...
static int ufshpb_issue_map_req_from_list(struct ufshpb10_lu *hpb)
{
	struct ufshpb10_subregion *cp, *next_cp;
	int ret;
	...
	list_for_each_entry_safe(cp, next_cp, &req_list, list_subregion) {
		...
		HPB_DRIVER_D("issue map_request: %d - %d/%d\n",
				cp->region, cp->subregion, subregion_mem_size);

		ret = ufshpb_execute_req(hpb, cmd, cp);
		if (ret < 0) {
			HPB_DRIVER_E("region %d sub %d failed with err %d",
...
static void ufshpb_pinned_work_handler(struct work_struct *work)
{
	...
	struct ufshpb10_lu *hpb =
		container_of(dwork, struct ufshpb10_lu, ufshpb_pinned_work);
	int ret;

	HPB_DRIVER_D("worker start\n");
	...
	if (!list_empty(&hpb->lh_subregion_req)) {
		 ...
		 */
		if (ret)
			HPB_DRIVER_E("failed map-issue. ret %d\n", ret);
		pm_runtime_mark_last_busy(HPB_DEV(hpb));
		pm_runtime_put_noidle(HPB_DEV(hpb));
	}

	HPB_DRIVER_D("worker end\n");
}

static void ufshpb_map_req_retry_work_handler(struct work_struct *work)
{
	struct ufshpb10_lu *hpb;
	struct delayed_work *dwork = to_delayed_work(work);
	struct ufshpb_map_req *map_req;
	...
	hpb = container_of(dwork, struct ufshpb10_lu, ufshpb_map_req_retry_work);
	HPB_DRIVER_D("retry worker start");
	...
	while (1) {
		...
		if (!map_req) {
			HPB_DRIVER_D("There is no map_req");
			break;
		}
		list_del(&map_req->list_map_req);
		...
		ret = ufshpb_map_req_issue(hpb, map_req);
		if (ret) {
			HPB_DRIVER_E("ufshpb_set_map_req error %d", ret);
			goto wakeup_ee_worker;
		}
	}
	HPB_DRIVER_D("worker end");
...
{
	struct ufshpb10_lu *hpb = (struct ufshpb10_lu *)private;
	struct ufshpb_rsp_info *rsp_info;
	unsigned long flags;
	...
	while (1) {
		...
		case HPB_RSP_HPB_RESET:
			...
			ufshpb_map_loading_trigger(hpb, true, false);
			break;
...
static int ufshpb_init_pinned_active_block(struct ufshpb10_lu *hpb,
		struct ufshpb10_region *cb)
{
	struct ufshpb10_subregion *cp;
	int subregion, j;
...
static inline bool ufshpb_is_pinned(
...
static inline void ufshpb_init_jobs(struct ufshpb10_lu *hpb)
{
	INIT_DELAYED_WORK(&hpb->ufshpb_pinned_work, ufshpb_pinned_work_handler);
	INIT_DELAYED_WORK(&hpb->ufshpb_map_req_retry_work, ufshpb_map_req_retry_work_handler);
	tasklet_init(&hpb->ufshpb_tasklet, ufshpb_tasklet_fn,
							(unsigned long)hpb);
}

static inline void ufshpb_cancel_jobs(struct ufshpb10_lu *hpb)
{
	cancel_delayed_work_sync(&hpb->ufshpb_pinned_work);
	cancel_delayed_work_sync(&hpb->ufshpb_map_req_retry_work);
	tasklet_kill(&hpb->ufshpb_tasklet);
}

static void ufshpb_init_subregion_tbl(struct ufshpb10_lu *hpb,
		struct ufshpb10_region *cb, bool last_region)
{
	int subregion;

	for (subregion = 0 ; subregion < cb->subregion_count ; subregion++) {
		struct ufshpb10_subregion *cp = cb->subregion_tbl + subregion;
...
static inline int ufshpb_alloc_subregion_tbl(struct ufshpb10_lu *hpb,
		struct ufshpb10_region *cb, int subregion_count)
{
	cb->subregion_tbl = kzalloc(
			sizeof(struct ufshpb10_subregion) * subregion_count,
			GFP_KERNEL);
	if (!cb->subregion_tbl)
		return -ENOMEM;

	cb->subregion_count = subregion_count;
	/* HPB_DRIVER_D("region %d subregion_count %d active_page_table bytes %lu\n",
		cb->region, subregion_count,
		sizeof(struct ufshpb10_subregion *) * hpb->subregions_per_region); */

	return 0;
}

static int ufshpb_table_mempool_init(struct ufshpb10_lu *hpb,
		...
		int entry_count, int entry_byte)
{
	...
	for (i = 0 ; i < num_regions * subregions_per_region ; i++) {
		...
		/* HPB_DRIVER_D("[%d] mctx->m_page %p get_order %d\n", i,
	...
	/* HPB_DRIVER_D("number of mctx %d %d %d. debug_free_table %d\n",
...
static int ufshpb_req_mempool_init(struct ufs_hba *hba,
				struct ufshpb10_lu *hpb, int queue_depth)
{
	...
	if (!queue_depth) {
		...
		HPB_DRIVER_E("lu_queue_depth is 0. we use device's queue info.\n");
		HPB_DRIVER_E("hba->nutrs = %d\n", hba->nutrs);
	}
	...
	hpb->map_req = kvzalloc(queue_depth * sizeof(struct ufshpb_map_req),
								GFP_KERNEL);
	if (!hpb->map_req)
		goto release_mem;
...
static void ufshpb_init_lu_constant(struct ufshpb10_lu *hpb,
		...
		struct ufshpb_func_desc *func_desc)
{
	...
	/*	From descriptors	*/
	region_unit_size = SECTOR * (1ULL << func_desc->hpb_region_size);
	region_mem_size = region_unit_size / BLOCK * HPB_ENTRY_SIZE;

	hpb->subregion_unit_size = SECTOR * (1ULL << func_desc->hpb_subregion_size);
	...
	hpb->last_subregion_mem_size =
		last_subregion_unit_size / BLOCK * HPB_ENTRY_SIZE;
	...
	/* HPB_DRIVER_D("%s ppn_dirties_per_subregion:%d, %d \n",
			__func__, hpb->ppn_dirties_per_subregion, BITS_PER_PPN_DIRTY); */
#endif
	/*
	 * 1. regions_per_lu
	...
	HPB_DRIVER_I("===== From Device Descriptor! =====\n");
	HPB_DRIVER_I("hpb_region_size = %d, hpb_subregion_size = %d\n",
			func_desc->hpb_region_size,
			func_desc->hpb_subregion_size);
	HPB_DRIVER_I("=====   Constant Values(LU)   =====\n");
	HPB_DRIVER_I("region_unit_size = %llu, region_mem_size %llu\n",
			region_unit_size, region_mem_size);
	HPB_DRIVER_I("subregion_unit_size = %llu, subregion_mem_size %d\n",
			hpb->subregion_unit_size, hpb->subregion_mem_size);
	HPB_DRIVER_I("last_subregion_mem_size = %d\n", hpb->last_subregion_mem_size);
	HPB_DRIVER_I("lu_num_blks = %llu, reg_per_lu = %d, subreg_per_lu = %d\n",
			hpb->lu_num_blocks, hpb->regions_per_lu,
			hpb->subregions_per_lu);
	HPB_DRIVER_I("subregions_per_region = %d\n",
			hpb->subregions_per_region);
	HPB_DRIVER_I("entries_per_region %u shift %u mask 0x%X\n",
			entries_per_region, hpb->entries_per_region_shift,
			hpb->entries_per_region_mask);
	HPB_DRIVER_I("entries_per_subregion %u shift %u mask 0x%X\n",
			hpb->entries_per_subregion,
			hpb->entries_per_subregion_shift,
			hpb->entries_per_subregion_mask);
	HPB_DRIVER_I("mpages_per_subregion : %d\n",
			hpb->mpages_per_subregion);
	HPB_DRIVER_I("===================================\n");
}

static int ufshpb_lu_hpb_init(struct ufs_hba *hba, struct ufshpb10_lu *hpb,
		struct ufshpb_func_desc *func_desc,
		...
		bool *do_work_lun)
{
	...
	*do_work_lun = false;
	...
	hpb->region_tbl = kzalloc(sizeof(struct ufshpb10_region) *
				hpb->regions_per_lu, GFP_KERNEL);
	if (!hpb->region_tbl)
		return -ENOMEM;

	HPB_DRIVER_D("active_block_table bytes: %lu\n",
		(sizeof(struct ufshpb10_region) * hpb->regions_per_lu));
	...
	if (ret) {
		HPB_DRIVER_E("ppn table mempool init fail!\n");
		goto release_mempool;
	}

	ret = ufshpb_req_mempool_init(hba, hpb, lu_desc->lu_queue_depth);
	if (ret) {
		HPB_DRIVER_E("rsp_info_mempool init fail!\n");
		goto release_mempool;
	}
	...
	HPB_DRIVER_D("total_subregion_count: %d\n", total_subregion_count);
	for (region = 0, subregion_count = 0,
		...
		subregion_count = min(total_subregion_count,
				hpb->subregions_per_region);
		/* HPB_DRIVER_D("total: %d subregion_count: %d\n",
				total_subregion_count, subregion_count); */

		ret = ufshpb_alloc_subregion_tbl(hpb, cb, subregion_count);
		if (ret)
			goto release_region_cp;
		ufshpb_init_subregion_tbl(hpb, cb, region == hpb->regions_per_lu - 1);

		if (ufshpb_is_pinned(lu_desc, region)) {
			HPB_DRIVER_D("region: %d PINNED %d ~ %d\n",
				region, lu_desc->hpb_pinned_region_startidx,
			...
			*do_work_lun = true;
			...
			/* HPB_DRIVER_D("region: %d inactive\n", cb->region); */
			cb->is_pinned = false;
			cb->region_state = HPBREGION_INACTIVE;
		}
	}
	...
	if (total_subregion_count != 0) {
		HPB_DRIVER_E("error total_subregion_count: %d\n",
			total_subregion_count);
		goto release_region_cp;
	}
		...
		struct ufshpb_lu_desc *lu_desc, int lun)
{
	...
	if (ret) {
		HPB_DRIVER_E("read unit desc failed. ret %d\n", ret);
		return ret;
	}
	...
	if (lu_desc->lu_hpb_enable) {
		HPB_DRIVER_D("LUN(%d) [0A] bLogicalBlockSize %d\n",
				lun, lu_desc->lu_logblk_size);
		HPB_DRIVER_D("LUN(%d) [0B] qLogicalBlockCount %llu\n",
				lun, lu_desc->lu_logblk_cnt);
		HPB_DRIVER_D("LUN(%d) [03] bLuEnable %d\n",
				lun, logical_buf[UNIT_DESC_PARAM_LU_ENABLE]);
		HPB_DRIVER_D("LUN(%d) [06] bLuQueueDepth %d\n",
				lun, lu_desc->lu_queue_depth);
		HPB_DRIVER_D("LUN(%d) [23:24] wLUMaxActiveHPBRegions %d\n",
				lun, lu_desc->lu_max_active_hpb_regions);
		HPB_DRIVER_D("LUN(%d) [25:26] wHPBPinnedRegionStartIdx %d\n",
				lun, lu_desc->hpb_pinned_region_startidx);
		HPB_DRIVER_D("LUN(%d) [27:28] wNumHPBPinnedRegions %d\n",
	...
	if (lu_desc->lu_hpb_enable)
		HPB_DRIVER_I("UFSHPB: Enable, LU: %d, MAX_REGION: %d, PIN: %d - %d\n",
			lun,
			lu_desc->lu_max_active_hpb_regions,
			lu_desc->hpb_pinned_region_startidx,
			lu_desc->lu_num_hpb_pinned_regions);
...
static void ufshpb_quirk_setup(struct ufs_hba *hba, u16 dev_version)
{
	if (dev_version >= 0x0101) {
		hba->ufshpb_quirk |= UFSHPB_QUIRK_USE_READ_16_FOR_ENCRYPTION;
		HPB_DRIVER_I("UFSHPB: QUIRK set USE_READ_16_FOR_ENCRYPTION\n");
	}
	if (dev_version >= 0x0102) {
		hba->ufshpb_quirk |= UFSHPB_QUIRK_ALWAYS_DEVICE_CONTROL_MODE;
		HPB_DRIVER_I("UFSHPB: QUIRK set ALWAYS_DEVICE_CONTROL_MODE\n");
	}
}
		...
		struct ufshpb_func_desc *desc)
{
	...
	if (desc_buf[DEVICE_DESC_PARAM_FEAT_SUP] &
		...
		HPB_DRIVER_I("UFSHPB: FeaturesSupport = support\n");
	} else {
		HPB_DRIVER_I("UFSHPB: FeaturesSupport = not support\n");
		return -ENODEV;
	}

	desc->lu_cnt = desc_buf[DEVICE_DESC_PARAM_NUM_LU];
	HPB_DRIVER_D("device lu count %d\n", desc->lu_cnt);

	desc->hpb_ver =
		(u16)SHIFT_BYTE_1(desc_buf[DEVICE_DESC_PARAM_HPB_VER]) |
		(u16)SHIFT_BYTE_0(desc_buf[DEVICE_DESC_PARAM_HPB_VER + 1]);

	HPB_DRIVER_I("UFSHPB: Dev Ver=%x.%x, DD Ver=%x.%x.%x\n",
			GET_BYTE_1(desc->hpb_ver),
			GET_BYTE_0(desc->hpb_ver),
			GET_BYTE_2(UFSHPB_DD_VER),
			GET_BYTE_1(UFSHPB_DD_VER),
			GET_BYTE_0(UFSHPB_DD_VER));

	ufshpb_quirk_setup(hba, desc->hpb_ver);

	if (hba->ufshpb_quirk & UFSHPB_QUIRK_ALWAYS_DEVICE_CONTROL_MODE)
	...
	if (desc->hpb_control_mode == HOST_CTRL_MODE) {
		HPB_DRIVER_E("UFSHPB: Driver does support Host Control Mode");
		return -ENODEV;
	}
	HPB_DRIVER_I("HPB Control Mode = %s",
		...
		struct ufshpb_func_desc *desc)
{
	...
	HPB_DRIVER_D("[48] bHPBRegionSize %u\n", desc->hpb_region_size);
	HPB_DRIVER_D("[49] bHPBNumberLU %u\n", desc->hpb_number_lu);
	HPB_DRIVER_D("[4A] bHPBSubRegionSize %u\n", desc->hpb_subregion_size);
	HPB_DRIVER_D("[4B:4C] wDeviceMaxActiveHPBRegions %u\n",
			desc->hpb_device_max_active_regions);

	if (desc->hpb_number_lu == 0) {
		HPB_DRIVER_E("UFSHPB: HPB is not supported\n");
		return -ENODEV;
	}
	/* for activation */
...
static int ufshpb_init(struct ufs_hba *hba)
{
	...
	if (ret == 0) {
		...
		ufshcd_query_flag_retry(hba,
				...
				&fHPBReset);
		if (fHPBReset)
			HPB_DRIVER_E("fHPBReset still set\n");
	}
	...
	for (lun = 0 ; lun < UFS_UPIU_MAX_GENERAL_LUN ; lun++) {
		...
		hba->ufshpb_lup[lun] = kzalloc(sizeof(struct ufshpb10_lu),
								GFP_KERNEL);
	...
	if (do_work) {
		for (lun = 0; lun < UFS_UPIU_MAX_GENERAL_LUN; lun++) {
			struct ufshpb10_lu *hpb = hba->ufshpb_lup[lun];

			if (hpb)
				schedule_delayed_work(&hpb->ufshpb_pinned_work, 0);
		}
	}
...
out_free_mem:
	ufshpb10_release(hba, HPB_NOT_SUPPORTED);
out_state:
...
static void ufshpb_purge_active_block(struct ufshpb10_lu *hpb)
{
	...
	for (region = 0 ; region < hpb->regions_per_lu ; region++) {
		...
		if (cb->is_pinned){
			...
			HPB_DRIVER_E("Unsupported state of region\n");
			continue;
		}

		HPB_DRIVER_D("region %d state %d dft %d\n",
				region, state,
				hpb->debug_free_table);
		for (subregion = 0 ; subregion < cb->subregion_count;
							subregion++) {
...
static void ufshpb_retrieve_rsp_info(struct ufshpb10_lu *hpb)
{
	struct ufshpb_rsp_info *rsp_info;
	unsigned long flags;
...
static void ufshpb_probe(struct ufs_hba *hba)
{
	...
	for (lu = 0 ; lu < UFS_UPIU_MAX_GENERAL_LUN ; lu++) {
		...
		if (hpb && hpb->lu_hpb_enable) {
			...
			ufshpb_purge_active_block(hpb);
			HPB_DRIVER_I("UFSHPB lun %d reset\n", lu);
...
static void ufshpb_destroy_subregion_tbl(struct ufshpb10_lu *hpb,
		struct ufshpb10_region *cb)
{
	int subregion;
...
static void ufshpb_destroy_region_tbl(struct ufshpb10_lu *hpb)
{
	int region;
...
void ufshpb10_release(struct ufs_hba *hba, int state)
{
	...
	if (alloc_mctx != 0)
		HPB_DRIVER_E("warning: alloc_mctx %d", alloc_mctx);
...
void ufshpb10_init_handler(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct ufs_hba *hba =
		container_of(dwork, struct ufs_hba, ufshpb_init_work);
	...
	if (hba->ufshpb_state == HPB_NEED_INIT) {
		...
		if (hba->ufshpb_state == HPB_NOT_SUPPORTED)
			HPB_DRIVER_E("UFSHPB: run without HPB - err=%d\n", err);
...
void ufshcd10_init_hpb(struct ufs_hba *hba)
{
	...
	INIT_DELAYED_WORK(&hba->ufshpb_init_work, ufshpb10_init_handler);
}
...
static void ufshpb_error_handler(struct work_struct *work)
{
	...
	HPB_DRIVER_E("UFSHPB driver runs without UFSHPB\n");
	HPB_DRIVER_E("UFSHPB will be removed from the kernel\n");

	ufshpb10_release(hba, HPB_FAILED);
}

static void ufshpb_stat_init(struct ufshpb10_lu *hpb)
{
	...
	atomic64_set(&hpb->lc_err_miss, 0);
#endif
		...
		const char *buf, size_t count)
{
	...
	if (kstrtoull(buf, 0, &value)) {
		HPB_DRIVER_E( "kstrtoul error\n");
		return -EINVAL;
	}

	if (value >= hpb->regions_per_lu) {
		HPB_DRIVER_E("value %llu >= regions_per_lu %d error\n",
	...
	HPB_DRIVER_I("get_info_from_region[%d]=", region);
	HPB_DRIVER_I("region %u state %s", region,
			   ((cb->region_state == HPBREGION_INACTIVE) ? "INACTIVE" :
				((cb->region_state == HPBREGION_ACTIVE) ? "ACTIVE" : "INVALID"))
			   );
	for (subregion = 0; subregion < cb->subregion_count; subregion++) {
		cp = cb->subregion_tbl + subregion;
		HPB_DRIVER_I("subregion %u state %s", subregion,
				   ((cp->subregion_state == HPBSUBREGION_UNUSED) ? "UNUSED" :
					((cp->subregion_state == HPBSUBREGION_DIRTY) ? "DIRTY" :
					 ((cp->subregion_state == HPBSUBREGION_CLEAN) ? "CLEAN" :
					  (cp->subregion_state == HPBSUBREGION_ISSUED) ?
					  "ISSUED" : "INVALID")))
				  );
	}
...
static ssize_t ufshpb_sysfs_info_from_lba_store(struct ufshpb10_lu *hpb,
		const char *buf, size_t count)
{
	hpb_t ppn;
	unsigned long long value = 0;
	...
	if (value > hpb->lu_num_blocks * SECTORS_PER_BLOCK) {
		HPB_DRIVER_E("value %llu > lu_num_blocks %llu error\n",
	...
	HPB_DRIVER_I("get_info_from_lba[%llu]=", value);
	HPB_DRIVER_I("sector %llu region %d state %s subregion %d state %s",
			   value, region,
			   ((cb->region_state == HPBREGION_INACTIVE) ? "INACTIVE" :
				((cb->region_state == HPBREGION_ACTIVE) ? "ACTIVE" : "INVALID")),
			   subregion,
	...
	HPB_DRIVER_I("sector %llu lpn %u ppn %llx dirty %d",
			value, lpn, ppn, dirty);
	return count;
}

static ssize_t ufshpb_sysfs_map_req_show(struct ufshpb10_lu *hpb, char *buf)
{
...
static ssize_t ufshpb_sysfs_add_evict_show(struct ufshpb10_lu *hpb, char *buf)
{
...
static ssize_t ufshpb_sysfs_hit_show(struct ufshpb10_lu *hpb, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "hit_count= %lld\n",
				(long long)atomic64_read(&hpb->hit));
}

static ssize_t ufshpb_sysfs_miss_show(struct ufshpb10_lu *hpb, char *buf)
{
	long long region_miss, subregion_miss, entry_dirty_miss,
	...
	count = snprintf(buf, PAGE_SIZE,
		"HPB_READ miss_count[ALL REGION SUBREGION ENTRY_DIRTY]= %lld %lld %lld %lld\n",
...
#if defined(HPB_READ_LARGE_CHUNK_DIRTY_CHECK_SUPPORT)
	{
		long long lc_dirty_miss, lc_state_miss, lc_err_miss;
		int ret = 0;

		lc_dirty_miss = atomic64_read(&hpb->lc_entry_dirty_miss);
		lc_state_miss = atomic64_read(&hpb->lc_reg_subreg_miss);
		lc_err_miss = atomic64_read(&hpb->lc_err_miss);
		ret = snprintf(buf + count, PAGE_SIZE - count,
					   "LARG_CHNK miss_count[ENTRY_DIRTY REG_SUBREG ERR]= %lld %lld %lld\n",
					   lc_dirty_miss, lc_state_miss, lc_err_miss);
		count += ret;
	}
#endif
	return count;
...
static ssize_t ufshpb_sysfs_version_show(struct ufshpb10_lu *hpb, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
			"HPBversion[HPB DD]= %x.%x %x.%x.%x\n",
			GET_BYTE_1(hpb->hpb_ver),
			GET_BYTE_0(hpb->hpb_ver),
			GET_BYTE_2(UFSHPB_DD_VER),
			GET_BYTE_1(UFSHPB_DD_VER),
			GET_BYTE_0(UFSHPB_DD_VER));
}

static ssize_t ufshpb_sysfs_active_count_show(
	struct ufshpb10_lu *hpb, char *buf)
{
	struct ufshpb10_region *cb;
	int region;
	int pinned_cnt = 0;
	...
	for (region = 0 ; region < hpb->regions_per_lu ; region++) {
		...
		if (cb->is_pinned && cb->region_state == HPBREGION_ACTIVE)
			pinned_cnt++;
	}
	return snprintf(buf, PAGE_SIZE,
		...
		const char *buf, size_t count)
{
	...
	if (value == 1)
		...
		ufshpb_map_loading_trigger(hpb, false, true);

	return count;
}

static ssize_t ufshpb_sysfs_hpb_disable_show(
	struct ufshpb10_lu *hpb, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		"[0] BOTH_enabled= %d\
		...
		const char *buf, size_t count)
{
	...
	if (value < 0 || value > 3) {
		HPB_DRIVER_E("Error, Only [0-3] is valid:\
			 \n[0] BOTH_enable\
...
static ssize_t debug_activate_store(
	struct ufshpb10_lu *hpb, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u:%u", &region, &subregion) != 2)
		return -EINVAL;

	rv = ufshpb_generate_map_cmd_req(hpb, region, subregion, R_BUFFER);
	if (rv < 0)
		return rv;
	...
	struct ufshpb10_lu *hpb, const char *buf, size_t count)
{
	...
	if (rv < 0)
		return rv;
	if (region >= READ_ONCE(hpb->regions_per_lu))
		return -ERANGE;

	rv = ufshpb_generate_map_cmd_req(hpb, region, 0, W_BUFFER);
	if (rv < 0)
		return rv;
		...
		const char *buf, size_t count)
{
	...
	for (retries = 1; retries <= 20; retries++) {
		...
		if (!doorbel) {
			...
			pm_runtime_put_noidle(HPB_DEV(hpb));
			if(!ret) {
				HPB_DRIVER_I("Query fHPBReset is successfully sent\n");
				break;
			}
		}
		HPB_DRIVER_I("fHPBReset failed and will retry[%d] after some time, DOORBELL: 0x%x\n",
				 retries, doorbel);
		msleep(200);
	}
	if (ret == 0) {
		...
		ufshcd_query_flag_retry(hpb->hba,
		 ...
		 pm_runtime_put_noidle(HPB_DEV(hpb));
		if (fHPBReset)
			HPB_DRIVER_E("fHPBReset is still in progress at device, but keep going\n");
	} else {
		HPB_DRIVER_E("Fail to set fHPBReset flag\n");
		goto out;
	}
	for (lun = 0 ; lun < UFS_UPIU_MAX_GENERAL_LUN ; lun++)
	{
		hpb_lu = hpb->hba->ufshpb_lup[lun];
		if (!hpb_lu || !hpb_lu->lu_hpb_enable)
			continue;

		ufshpb_stat_init(hpb_lu);

		spin_lock_irqsave(&hpb_lu->rsp_list_lock, flags);
		rsp_info = ufshpb_get_req_info(hpb_lu);
		...
		spin_unlock_irqrestore(&hpb_lu->rsp_list_lock, flags);
		HPB_DRIVER_I("Host HPB reset start LU%d - fHPBReset\n", lun);
		tasklet_schedule(&hpb_lu->ufshpb_tasklet);
	}
out:
	return count;
}

static ssize_t ufshpb_sysfs_debug_log_show(
	struct ufshpb10_lu *hpb, char *buf)
{
	int value = ufshpb_debug_mask;

	if (value == (HPB_LOG_OFF)) {
		value = HPB_LOG_LEVEL_OFF;
	} else if (value == (HPB_LOG_ERR)) {
		value = HPB_LOG_LEVEL_ERR;
	} else if (value == (HPB_LOG_ERR | HPB_LOG_INFO)) {
		value = HPB_LOG_LEVEL_INFO;
	} else if (value == (HPB_LOG_ERR | HPB_LOG_INFO | HPB_LOG_DEBUG)) {
		value = HPB_LOG_LEVEL_DEBUG;
	} else if (value == (HPB_LOG_ERR | HPB_LOG_INFO | HPB_LOG_DEBUG | HPB_LOG_HEX)) {
		value = HPB_LOG_LEVEL_HEX;
	}
		...
		const char *buf, size_t count)
{
	...
	if (value < HPB_LOG_LEVEL_OFF || value > HPB_LOG_LEVEL_HEX) {
		HPB_DRIVER_E("Error, Only [0-4] is valid:\
			 \n[0] : LOG_LEVEL_OFF\
			 \n[1] : LOG_LEVEL_ERR\
			 \n[2] : LOG_LEVEL_INFO\
			 \n[3] : LOG_LEVEL_DEBUG\
	...
	if (value == HPB_LOG_LEVEL_OFF) {
		ufshpb_debug_mask = HPB_LOG_OFF;
	} else if (value == HPB_LOG_LEVEL_ERR) {
		ufshpb_debug_mask = HPB_LOG_ERR;
	} else if (value == HPB_LOG_LEVEL_INFO) {
		ufshpb_debug_mask = HPB_LOG_ERR | HPB_LOG_INFO;
	} else if (value == HPB_LOG_LEVEL_DEBUG) {
		ufshpb_debug_mask = HPB_LOG_ERR | HPB_LOG_INFO | HPB_LOG_DEBUG;
	} else if (value == HPB_LOG_LEVEL_HEX) {
		ufshpb_debug_mask = HPB_LOG_ERR | HPB_LOG_INFO | HPB_LOG_DEBUG | HPB_LOG_HEX;
	}
...
static struct ufshpb10_sysfs_entry ufshpb_sysfs_entries[] = {
	__ATTR(hpb_disable, 0644,
			ufshpb_sysfs_hpb_disable_show,
			ufshpb_sysfs_hpb_disable_store),
	...
	__ATTR(active_count, 0444, ufshpb_sysfs_active_count_show, NULL),
	...
	__ATTR(get_info_from_region, 0200, NULL,
		   ufshpb_sysfs_info_from_region_store),
	__ATTR(get_info_from_lba, 0200, NULL, ufshpb_sysfs_info_from_lba_store),
	__ATTR(debug_activate,   0200, NULL, debug_activate_store),
	__ATTR(debug_inactivate, 0200, NULL, debug_inactivate_store),
	__ATTR(hpb_reset, 0200, NULL, ufshpb_sysfs_hpb_reset_store),
	__ATTR(debug_log, 0644,
			ufshpb_sysfs_debug_log_show,
			ufshpb_sysfs_debug_log_store),
	__ATTR_NULL
};
...
{
	struct ufshpb10_sysfs_entry *entry;
	struct ufshpb10_lu *hpb;
	ssize_t error;

	entry = container_of(attr,
			struct ufshpb10_sysfs_entry, attr);
	hpb = container_of(kobj, struct ufshpb10_lu, kobj);
		...
		const char *page, size_t length)
{
	...
	entry = container_of(attr, struct ufshpb10_sysfs_entry, attr);
	hpb = container_of(kobj, struct ufshpb10_lu, kobj);
