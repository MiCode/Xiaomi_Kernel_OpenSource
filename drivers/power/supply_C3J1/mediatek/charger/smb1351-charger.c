...
struct smb1351_charger {
	...
	struct delayed_work     check_init_boot_work;
...
static void smb1351_rerun_aicl(struct smb1351_charger *chip)
{
	...
	msleep(200);
	if (chip->chg_type != CHARGING_HOST) {
...
static void smb1351_chg_hvdcp_det_work(struct work_struct *work)
{
	...
	if ((hvdcp_status & BIT(2)) && (hvdcp_status & BIT(7))) {
...
static int smb1351_set_usbchg_current(struct charger_device *chg_dev, u32 uA)
{
	...
	if (prev_current < current_ma && thermal_icl_ua < 0 ) {
...
static int smb1351_set_fastchg_current(struct charger_device *chg_dev, u32 uA)
{
	...
	if (is_pre_chg) {
		...
		pr_debug("prechg setting 0x%02x\n", i);
...
//		pr_err("Couldn't start SW conn therm rc=%d\n", rc);
	if(!first_boot_flag)
		schedule_delayed_work(&chip->check_init_boot_work, msecs_to_jiffies(18000));
...
static int smb1351_set_float_voltage(struct charger_device *chg_dev, u32 uV)
{
	...
	u8 temp;
	int rc = 0;
	int float_max, float_cv;

	if (cm->enable_sw_jeita && cm->sw_jeita.cv != 0)
		float_max = cm->sw_jeita.cv;	
	else
		float_max = cm->data.battery_cv;

	if (cycle_count < 100) {
		float_cv = float_max;
	} else if (cycle_count < 200) {
		float_cv = float_max - 10000;
	} else if (cycle_count < 300) {
		float_cv = float_max - 20000;
	} else {
		float_cv = float_max - 30000;
	}
	if (uV > float_max) {
		uV = float_cv;
	}

	pr_err("set float voltage %d cycle_count : %d battery_cv = :%d", uV, cycle_count);
...
static void smb1351_check_init_boot_work(struct work_struct *work)
{
	struct smb1351_charger *chip = container_of(work,
		struct smb1351_charger, check_init_boot_work.work);
...
/*set QC2.0 9V*/
	...
	INIT_DELAYED_WORK(&chip->check_init_boot_work, smb1351_check_init_boot_work);
	smb1351_set_suspend_to_iic(chip->chg_dev, false);
...
static int smb1351_charger_remove(struct i2c_client *client)
{
	...
	cancel_delayed_work_sync(&chip->check_init_boot_work);
...
static void smb1351_charger_shutdown(struct i2c_client *client)
{
	...
	if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT ||
		boot_mode == LOW_POWER_OFF_CHARGING_BOOT) {	
			pr_notice("%s: plug out Charger/USB\n", __func__);
