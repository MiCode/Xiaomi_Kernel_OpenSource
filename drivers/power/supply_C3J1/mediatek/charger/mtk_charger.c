...
static int thermal_mitigation_dcp[THERMAL_MAX] = {2000000, 1800000, 1800000, 1600000, 1600000, 1400000, 1400000, 1200000, 1200000, 1000000, 1000000, 800000, 800000,600000, 600000, 500000};

static int thermal_mitigation_qc2[THERMAL_MAX] = {2400000, 2200000, 2000000, 1800000, 1600000, 1400000, 1400000, 1200000, 1200000, 1000000, 1000000, 800000, 800000, 600000, 600000, 500000};

static int thermal_mitigation_qc3[THERMAL_MAX] = {3000000, 2800000, 2800000, 2600000, 2600000, 2400000, 2400000, 2200000, 2000000, 1800000, 1600000, 1400000, 1200000, 1000000, 800000, 600000};

static int thermal_mitigation_dcp_cn[THERMAL_MAX] =  {2000000, 1900000, 1800000, 1700000, 1600000, 1500000, 1400000, 1300000, 1200000, 1100000, 1000000, 900000, 800000, 700000, 600000, 500000};

static int thermal_mitigation_qc2_cn[THERMAL_MAX] =  {2000000, 1900000, 1800000, 1700000, 1600000, 1500000, 1400000, 1300000, 1200000, 1100000, 1000000, 900000, 800000, 700000, 600000, 500000};

static int thermal_mitigation_qc3_cn[THERMAL_MAX] = {3000000, 2900000, 2800000, 2600000, 2500000, 2400000, 2200000, 2100000, 2000000, 1800000, 1600000, 1400000, 1200000, 1000000, 800000, 600000};
#endif
...
//		struct switch_charging_alg_data *swchgalg = info->algorithm_data;
		...
		if (info->data.parallel_vbus) {
			...
			//pdata->thermal_charging_current_limit = input_current;
		}
...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	if (sw_jeita->sm != TEMP_T2_TO_T3) {
		if (sw_jeita->sm == TEMP_ABOVE_T4)
			...
			sw_jeita->cc = info->data.jeita_temp_t1p5_to_t2_cc;
			sw_jeita->cv = info->data.jeita_temp_t1p5_to_t2_cv;
		} else if (sw_jeita->sm == TEMP_T1_TO_T1P5) {
			sw_jeita->cc = info->data.jeita_temp_t1_to_t1p5_cc;
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "jeita_temp_t1p5_to_t2_cc", &val) >= 0) {
		info->data.jeita_temp_t1p5_to_t2_cc = val;
		chr_err("use default JEITA_TEMP_T1P5_TO_T2_CC:%d\n",
			info->data.jeita_temp_t1p5_to_t2_cc);
	}

	if (of_property_read_u32(np, "jeita_temp_t1_to_t1p5_cc", &val) >= 0) {
		info->data.jeita_temp_t1_to_t1p5_cc = val;
		chr_err("use default JEITA_TEMP_T1_TO_T1P5_CC:%d\n",
			info->data.jeita_temp_t1_to_t1p5_cc);
	}
