...
static void mt6375_get_dpdm_voltage(struct mt6375_chg_data *ddata)
{
	int dp_voltage = 0, dm_voltage = 0, ret = 0;

	ret = iio_read_channel_processed(&ddata->iio_adcs[ADC_CHAN_USBDP], &dp_voltage);
	if (ret < 0)
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to read DP voltage\n");

	ret = iio_read_channel_processed(&ddata->iio_adcs[ADC_CHAN_USBDM], &dm_voltage);
	if (ret < 0)
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to read DM voltage\n");
...
static void mt6375_detect_hvchg_work(struct work_struct *work)
{
	...
	ret = iio_read_channel_processed(&ddata->iio_adcs[ADC_CHAN_CHGVINDIV5], &vbus);
	if (ret) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to get VBUS\n");
		vbus = 5000000;
	}
	vbus /= 1000;
	dev_info(ddata->dev, "[XMCHG][BBC_MT6375] detect HVCHG, vbus = %d\n", vbus);
...
static int mt6375_fcc_vote_callback(struct votable *votable, void *data, int value, const char *client)
{
	struct mt6375_chg_data *ddata = data;
	int ret = 0;

	ret = mt6375_chg_field_set(ddata, F_CC, value > MT6375_MAX_FCC ? MT6375_MAX_FCC : value);
	if (ret)
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to set FCC\n");

	return ret;
}

static int mt6375_icl_vote_callback(struct votable *votable, void *data, int value, const char *client)
{
	struct mt6375_chg_data *ddata = data;
	int ret = 0;

	ret = mt6375_chg_field_set(ddata, F_IAICR, value > MT6375_MAX_ICL ? MT6375_MAX_ICL : value);
	if (ret)
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to set ICL\n");

	return ret;
}

static int mt6375_fv_vote_callback(struct votable *votable, void *data, int value, const char *client)
{
	struct mt6375_chg_data *ddata = data;
	int ret = 0;

	ret = mt6375_chg_set_cv(ddata, value);
	if (ret)
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to set FV\n");

	return ret;
}

static int mt6375_iterm_vote_callback(struct votable *votable, void *data, int value, const char *client)
{
	struct mt6375_chg_data *ddata = data;
	int ret = 0;

	ret = mt6375_chg_field_set(ddata, F_IEOC, value);
	if (ret)
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to set ITERM\n");

	return ret;
}

static int mt6375_vinmin_vote_callback(struct votable *votable, void *data, int value, const char *client)
{
	struct mt6375_chg_data *ddata = data;
	int ret = 0;

	ret = mt6375_chg_field_set(ddata, F_VMIVR, value);
	if (ret)
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to set VINMIN\n");

	return ret;
}

static int mt6375_enable_vote_callback(struct votable *votable, void *data, int enable, const char *client)
{
	struct mt6375_chg_data *ddata = data;
	int ret = 0;

	ret = mt6375_chg_enable_charging(data, enable);
	if (ret)
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to set CHG_ENABLE\n");

	return ret;
}

static int mt6375_bbc_create_votable(struct mt6375_chg_data *ddata)
{
	int rc = 0;

	ddata->fcc_votable = create_votable("BBC_FCC", VOTE_MIN, mt6375_fcc_vote_callback, ddata);
	if (IS_ERR(ddata->fcc_votable)) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to create voter BBC_FCC\n");
		return -1;
	}

	ddata->icl_votable = create_votable("BBC_ICL", VOTE_MIN, mt6375_icl_vote_callback, ddata);
	if (IS_ERR(ddata->icl_votable)) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to create voter BBC_ICL\n");
		return -1;
	}

	ddata->fv_votable = create_votable("BBC_FV", VOTE_MIN, mt6375_fv_vote_callback, ddata);
	if (IS_ERR(ddata->fv_votable)) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to create voter BBC_FV\n");
		return -1;
	}

	ddata->iterm_votable = create_votable("BBC_ITERM", VOTE_MIN, mt6375_iterm_vote_callback, ddata);
	if (IS_ERR(ddata->iterm_votable)) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to create voter BBC_ITERM\n");
		return -1;
	}

	ddata->vinmin_votable = create_votable("BBC_VINMIN", VOTE_MIN, mt6375_vinmin_vote_callback, ddata);
	if (IS_ERR(ddata->vinmin_votable)) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to create voter BBC_VINMIN\n");
		return -1;
	}

	ddata->enable_votable = create_votable("BBC_ENABLE", VOTE_SET_ANY, mt6375_enable_vote_callback, ddata);
	if (IS_ERR(ddata->enable_votable)) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] failed to create voter BBC_ENABLE\n");
		return -1;
	}
...
static enum power_supply_property mt6375_bbc_properties[] = {
...
static int mt6375_bbc_get_property(struct power_supply *psy, enum power_supply_property prop, union power_supply_propval *val)
{
	struct mt6375_chg_data *ddata = power_supply_get_drvdata(psy);
	int ret = 0;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		ret = mt6375_get_adc(ddata->chgdev, ADC_CHANNEL_VBUS, &val->intval, &val->intval);
		if (ret)
			val->intval = 0;
		val->intval /= 1000;
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		ret = mt6375_get_adc(ddata->chgdev, ADC_CHANNEL_IBUS, &val->intval, &val->intval);
		if (ret)
			val->intval = 0;
		val->intval /= 1000;
		break;
	default:
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] not support property %d\n", prop);
		return -ENODATA;
	}
...
static const struct power_supply_desc mt6375_bbc_psy_desc = {
	.name = "bbc",
	.type = POWER_SUPPLY_TYPE_UNKNOWN,
	.properties = mt6375_bbc_properties,
	.num_properties = ARRAY_SIZE(mt6375_bbc_properties),
	.get_property = mt6375_bbc_get_property,
};

static int mt6375_bbc_init_psy(struct mt6375_chg_data *ddata)
{
	struct power_supply_config mt6375_bbc_psy_cfg = {};

	mt6375_bbc_psy_cfg.drv_data = ddata;
	ddata->bbc_psy = power_supply_register(NULL, &mt6375_bbc_psy_desc, &mt6375_bbc_psy_cfg);
	if (IS_ERR(ddata->bbc_psy)) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] invalid BBC_PSY\n");
		return -1;
	}
...
static int mt6375_ops_charge_enable(struct xmc_device *dev, bool enable)
{
	struct mt6375_chg_data *ddata = (struct mt6375_chg_data *)xmc_ops_get_data(dev);
	int ret = 0;

	ret = mt6375_chg_enable_charging(ddata, enable);
	if (ret)
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] ops failed to set CHG_ENABLE\n");

	return ret;
}

static int mt6375_ops_get_charge_enable(struct xmc_device *dev, bool *enable)
{
	struct mt6375_chg_data *ddata = (struct mt6375_chg_data *)xmc_ops_get_data(dev);

	return mt6375_chg_is_enabled(ddata, enable);
}

static int mt6375_ops_powerpath_enable(struct xmc_device *dev, bool enable)
{
	struct mt6375_chg_data *ddata = (struct mt6375_chg_data *)xmc_ops_get_data(dev);

	return mt6375_chg_field_set(ddata, F_BUCK_EN, enable);
}

static int mt6375_ops_get_powerpath_enable(struct xmc_device *dev, bool *enable)
{
	struct mt6375_chg_data *ddata = (struct mt6375_chg_data *)xmc_ops_get_data(dev);
	int ret = 0;
	u32 val = 0;
	...
	ret = mt6375_chg_field_get(ddata, F_BUCK_EN, &val);
	if (ret < 0) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] ops failed to get powerpath_enable\n");
		return ret;
	}
...
static int mt6375_ops_get_vbus(struct xmc_device *dev, int *value)
{
	...
	ret = mt6375_get_adc(ddata->chgdev, ADC_CHANNEL_VBUS, &vbus, &vbus);
	if (ret < 0) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] ops failed to get IBUS\n");
		return ret;
	}
...
static int mt6375_ops_get_ibus(struct xmc_device *dev, int *value)
{
	struct mt6375_chg_data *ddata = (struct mt6375_chg_data *)xmc_ops_get_data(dev);
	int ibus = 0, ret = 0;

	ret = mt6375_get_adc(ddata->chgdev, ADC_CHANNEL_IBUS, &ibus, &ibus);
	if (ret < 0) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] ops failed to get VBUS\n");
		return ret;
	}
...
static int mt6375_ops_get_ts(struct xmc_device *dev, int *value)
{
	...
	ret = mt6375_get_adc(ddata->chgdev, ADC_CHANNEL_TS, &ts, &ts);
	if (ret < 0) {
		dev_err(ddata->dev, "[XMCHG][BBC_MT6375] ops failed to get TS\n");
		return ret;
	}
...
static int mt6375_chg_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "[XMCHG][BBC_MT6375] MT6375 probe start\n");
	dev_info(dev, "%s\n", __func__);
	...
	ret = mt6375_bbc_create_votable(ddata);
	if (ret < 0) {
		dev_err(dev, "[XMCHG][BBC_MT6375] failed to init VOTER\n");
		goto out_chgdev;
	}

	ddata->bbc_dev = xmc_device_register("bbc", &mt6375_ops, ddata);
	if (!ddata->bbc_dev) {
		dev_err(dev, "[XMCHG][BBC_MT6375] failed to register BBC_DEV\n");
		goto out_chgdev;
	}

	ret = mt6375_bbc_init_psy(ddata);
	if (ret < 0) {
		dev_err(dev, "[XMCHG][BBC_MT6375] failed to init BBC_PSY\n");
		goto out_chgdev;
	}

	dev_info(dev, "[XMCHG][BBC_MT6375] MT6375 probe success\n");
