...
static void xmc_tune_fixed_pdo(struct charge_chip *chip)
{
	...
	if (chip->monitor_count != 3 && chip->monitor_count != 4)
		return;
...
static bool xmc_gauge_smooth_uisoc(struct charge_chip *chip)
{
	...
	ktime_t tmp_time = 0;
	int time_per_point, system_soc = 0, ibat = 0;
	...
	if (chip->battery.rsoc > 90 || chip->battery.rsoc < 8) {
		time_per_point = 4;
	} else {
	...
	if (chip->battery.uisoc != system_soc && time_now.tv_sec - last_change_time.tv_sec >= time_per_point) {
		...
		if (chip->battery.uisoc < system_soc) {
			...
			if (val.intval != POWER_SUPPLY_STATUS_DISCHARGING && chip->battery.uisoc - system_soc < 3 && chip->battery.vbat > 3600)
				return false;
			else
				chip->battery.uisoc--;
		}
...
static void xmc_gauge_monitor(struct charge_chip *chip)
{
	...
	if (uisoc_change || (chip->battery.tbat / 5) != last_tbat_step) {
...
static void xmc_check_ffc_charge(struct charge_chip *chip)
{
	...
	if (chip->ffc_enable)  {
		...
		chip->iterm_effective = chip->iterm - 100; /* Ensure software terminate is before wardware terminate */
	}
...
{
	static int full_count = 0, recharge_count = 0;

	if (chip->charge_full) {
		full_count = 0;
		if (chip->battery.vbat <= chip->fv_effective - 150)
			recharge_count++;
		else
			recharge_count = 0;

		if (chip->battery.uisoc < 100 || recharge_count >= 4) {
			recharge_count = 0;
...
static void xmc_main_monitor_func(struct work_struct *work)
{
	...
	xmc_info("[XMC_MONITOR] [UISOC VBAT IBAT TBAT] = [%d %d %d %d], [CAN EN INPUT VBUS IBUS] = [%d %d %d %d %d], [FULL FG_FULL STATE RECHARGE] = [%d %d %d %d], [BC12 QC PD AUTH] = [%d %d %d %d], [TL SIC STEP JEITA] = [%d %d %d %d], [FFC NIGHT_CHG FV_SHIFT] = [%d %d %d]\n",
		chip->battery.uisoc, chip->battery.vbat, chip->battery.ibat, chip->battery.tbat,
		chip->can_charge, chip->bbc.charge_enable, chip->bbc.input_enable, chip->bbc.vbus, chip->bbc.ibus,
		chip->charge_full, chip->battery.gauge_full, chip->bbc.state, chip->recharge,
