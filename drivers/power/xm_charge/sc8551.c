...
/*	date			author			comment
 *	2021-05-25		chenyichun@xiaomi.com	create driver for SC8551/BQ25970
 */
...
static int sc8551_check_work_mode(struct sc8551 *chip, int driver_data)
{
	...
	chip->mode = (data & SC8551_WORK_MODE_MASK) >> SC8551_WORK_MODE_SHIFT;
	sc_info("work_mode = %d, driver_data = %d\n", chip->mode, driver_data);
...
static int sc8551_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chip->irq_handle_wakelock = wakeup_source_register(chip->dev, "sc8551_irq_handle_wakelock");
	...
	ret = sc8551_register_charger(chip, id->driver_data);
	if (ret) {
		sc_err("%s failed to register charger\n", chip->log_tag);
		return ret;
	}

	ret = sc8551_power_supply_init(chip, dev, id->driver_data);
	if (ret) {
...
bool sc8551_init(void)
{
	if (i2c_add_driver(&sc8551_driver))
		return false;
	else
		return true;
}
