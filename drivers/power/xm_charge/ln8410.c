...
static unsigned int ln8410_reg_list[] = {
...
//	ret = ln8410_set_batovp_alarm(chip, chip->bat_ovp_alarmreshold);
	...
	ret = ln8410_set_busocp(chip, 3750);
...
static void ln8410_dump_register(struct ln8410_device *chip)
{
	...
	int i = 0, reg_num = ARRAY_SIZE(ln8410_reg_list);

	for (i = 0; i < reg_num; i++) {
		if (i == 0)
			printk(KERN_CONT "[XMCHG] %s REG: ", chip->log_tag);
		printk(KERN_CONT "0x%02x ", ln8410_reg_list[i]);
		if (i == reg_num - 1)
			printk(KERN_CONT "\n");
	}

	for (i = 0; i < reg_num; i++)
		regmap_read(chip->regmap, ln8410_reg_list[i], &data[i]);
