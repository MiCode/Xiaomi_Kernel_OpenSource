...
* 		This xmc_qcm.c is the fast charge manager for QC3/QC3+.
*		For 2-serial battery project, we don't enable this module.
* ------------------------------ Revision History: --------------------------------
...
static int qcm_remove(struct platform_device *pdev)
{
	...
	cancel_work(&chip->psy_change_work);
...
bool xmc_qcm_init(struct charge_chip *chip)
{
	if (platform_driver_register(&qcm_driver))
		return false;
	else
		return true;
}
