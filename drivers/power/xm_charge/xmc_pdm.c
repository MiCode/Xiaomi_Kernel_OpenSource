...
#else
static int pdm_vbus_low_gap = 750;
module_param_named(pdm_vbus_low_gap, pdm_vbus_low_gap, int, 0600);

static int pdm_vbus_high_gap = 1000;
module_param_named(pdm_vbus_high_gap, pdm_vbus_high_gap, int, 0600);
#endif

static int pdm_ibus_gap = 350;
...
static void pdm_update_status(struct pdm_chip *pdm)
{
	...
	xmc_info("[XMC_PDM] BUS = [%d %d %d %d], CP = [%d %d %d %d], BAT = [%d %d %d], STEP = [%d %d %d %d %d], CC_CV = [%d %d %d %d]\n",
		pdm->master_cp_vbus, pdm->master_cp_ibus, pdm->slave_cp_ibus, pdm->total_ibus,
		pdm->master_cp_enable, pdm->slave_cp_enable, pdm->master_cp_mode, pdm->slave_cp_mode,
...
static int pdm_tune_pdo(struct pdm_chip *pdm)
{
	...
	if (pdm->apdo_max_ibus <= 3000)
		ibus_limit = min(ibus_limit, pdm->apdo_max_ibus - 50);
...
static int pdm_check_condition(struct pdm_chip *pdm)
{
	...
	else if (g_chip->usb_typec.cmd_input_suspend || g_chip->usb_typec.burn_detect || g_chip->battery.connector_remove || !g_chip->can_charge)
		return PDM_STATUS_HOLD;
	else if (pdm->thermal_limit_fcc > 0 && pdm->thermal_limit_fcc < MIN_PDM_FCC)
		return PDM_STATUS_HOLD;
