...
static int configure_icl_check(struct smbchg_chip *chip, u8 val)
{
...
static int smbchg_otg_regulator_enable(struct regulator_dev *rdev)
{
	...
	configure_icl_check(chip, ICL_BUF_SS_DONE_VAL);
...
static int smbchg_otg_regulator_disable(struct regulator_dev *rdev)
{
	...
	smbchg_icl_loop_disable_check(chip);
	configure_icl_check(chip, ICL_BUF_SYSON_LDO_VAL);
