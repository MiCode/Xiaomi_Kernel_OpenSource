	...
	struct chg_limit_setting *setting)
{
	...
	if (info->real_type == XMUSB350_TYPE_FLOAT) {
			...
			if (!info->pd_verify_done) {
				 for(wait_count = 0; wait_count < 5; wait_count++) {
					msleep(100);
					if (info->pd_verify_done)
						break;
				}
	...
	if (info->chr_type == POWER_SUPPLY_TYPE_USB &&
		...
		if (!info->usb350_dev && info->pd_type != MTK_PD_CONNECT_PE_READY_SNK_PD30 
		...
		chr_err("HVCHG set input current 1600mA charging\n");
		...
		chr_err("C to C set input current 1000mA charging\n");
		pdata->input_current_limit = 1000000;
		pdata->charging_current_limit = 1000000;
		is_basic = true;
		if (!info->usb350_dev)
