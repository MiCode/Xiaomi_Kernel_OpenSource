...
void smart_batt_set_diff_fv(int val)
{
	...
	chr_err("%s diff_fv_val=%d\n", __func__, pinfo->smart_batt_reduceXMv);
}
...
stop_charging:
	...
	chr_err("[MTK_CHG] tmp:%d (jeita:%d sm:%d cv:%d en:%d) (sm:%d) en:%d c:%d s:%d ov:%d sc:%d %d %d saf_cmd:%d bat_mon:%d %d\n",
...
static void kpoc_power_off_check(struct mtk_charger *info)
{
	...
	if (boot_mode == 8 || boot_mode == 9) {
		...
		if (vbus >= 0 && vbus < 2500 && !mtk_is_charger_on(info) && !info->pd_reset && (time_now.tv_sec > 5)) {
			chr_err("Unplug Charger/USB in KPOC mode, vbus=%d, vcount=%d, shutdown\n", vbus, vcount);
			if (vcount > 6) {
			/*while (1) {*/
	...
	while (1) {
		...
		while (is_module_init_done == false) {
			if (charger_init_algo(info) == true) {
				...
				if (info->charger_unlimited) {		//mtbf can enable
					info->enable_sw_safety_timer = false;
		...
		chr_err("[MTK_CHG] Vbat=%d pmic_vbats=%d vbus:%d ibus:%d I=%d T=%d uisoc:%d type:%s>%s pd:%d swchg_ibat:%d cv:%d\n",
		...
		if (get_charger_type(info) == POWER_SUPPLY_TYPE_USB_CDP && get_usb_type(info) == POWER_SUPPLY_USB_TYPE_DCP && (!info->pd_type))
			info->real_type = XMUSB350_TYPE_FLOAT; //can move
