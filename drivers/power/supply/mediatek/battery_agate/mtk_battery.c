...
static int fcc = 12000000;
static int thermal_input_current = 6000000;
#endif
...
static enum power_supply_property battery_props[] = {
	...
	POWER_SUPPLY_PROP_LOW_POWER_CHARGING,
#ifdef CONFIG_FUEL_GAUGE_BQ27Z561
...
static bool is_psy_voter_available(struct battery_data *data)
{
	if (!mtk_batt_create_votable) {
		pr_err("mtk_battery mtk_batt_create_votable\n");
		return false;
	}

	if (mtk_batt_destroy_votable) {
		pr_err("mtk_battery mtk_batt_destroy_votable\n");
		return false;
	}
	...
	if (!data->fcc_votable) {
		...
		if (!data->fcc_votable) {
			pr_err("mtk_battery Couldn't find FCC voltable\n");
			return false;
		}
	}
		...
		union power_supply_propval *val)
{
	...
	if (!is_psy_voter_available(data))
		return;
	...
	if (pre_night_chg_flag != data->night_chg_flag) {
		if (data->night_chg_flag && data->BAT_CAPACITY >= 80) {
			...
			vote(data->fcc_votable, FCC_BATT_NIGHT_VOTER, true, 0);
	...
	if (pre_night_chg_flag && !data->night_chg_flag) {
		...
		vote(data->fcc_votable, FCC_BATT_NIGHT_VOTER, true, CURRENT_MAX);
		...
		vote(data->fcc_votable, FCC_BATT_NIGHT_VOTER, true, NORMAL_CHG_CURRENT_MAX);
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		val->intval = data->BAT_STATUS;
		if (battery_main.battery_67w)
			input_suspend = charger_manager_is_input_suspend();
		else
			input_suspend = mtk_get_prop_input_suspend();
	...
	case POWER_SUPPLY_PROP_NIGHT_CHARGING:
		...
		break;
	case POWER_SUPPLY_PROP_LOW_POWER_CHARGING:
		val->intval = data->battery_67w;
		break;
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		power_supply_get_property(data->ti_bms_psy,
	...
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		if (battery_main.battery_67w)
			...
			val->intval = mtk_get_prop_input_suspend();
		break;
	...
	case POWER_SUPPLY_PROP_FAST_CHARGE_CURRENT:
		if (data->fcc_votable)
			val->intval = get_effective_result_locked(data->fcc_votable);
		else
			val->intval = fcc;
		break;
	...
	const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		if (battery_main.battery_67w)
			...
			mtk_set_prop_input_suspend(val->intval);
		break;
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
...
static int __init battery_probe(struct platform_device *dev)
{
	...
	battery_main.battery_67w = of_property_read_bool(np, "battery-67w");
	battery_main.ti_bms_psy = power_supply_get_by_name("bms");
#endif
