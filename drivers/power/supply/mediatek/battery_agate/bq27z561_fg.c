...
#include "../charger_agate/mtk_charger_init.h"
#include "../charger_agate/mtk_charger_intf.h"
...
#define FCC_FG_BAT_VERIFY_VOTER		"FCC_FG_BAT_VERIFY_VOTER"
...
#define BATTERY_DIGEST_LEN_120W 20
...
#define BQ27Z561_DEFUALT_TERM_120W		-121
#define BQ27Z561_DEFUALT_FFC_TERM		-680
#define BQ27Z561_DEFUALT_FFC_TERM_120W		-972
#define BQ27Z561_DEFUALT_RECHARGE_VOL		4380
#define BQ27Z561_DEFUALT_RECHARGE_VOL_120W	8760
...
#define BQ_PACK_MAXIUM_VOLTAGE_FOR_PMIC_120W		8980
#define BQ_MAXIUM_VOLTAGE_FOR_CELL_120W			8960
#define BQ_PACK_MAXIUM_VOLTAGE_FOR_PMIC_SAFETY_120W	8954
...
static bool is_psy_voter_available(struct bq_fg_chip *bq)
{
	if (!mtk_batt_create_votable) {
		bq_dbg(PR_REGISTER, "bq27z561_fg mtk_batt_create_votable\n");
		return false;
	}

	if (mtk_batt_destroy_votable) {
		bq_dbg(PR_REGISTER, "bq27z561_fg mtk_batt_destroy_votable\n");
		return false;
	}

	if (!bq->fcc_votable) {
		bq->fcc_votable = find_votable("FCC_MAIN");
		if (!bq->fcc_votable) {
			bq_dbg(PR_REGISTER, "bq27z561_fg Couldn't find FCC voltable\n");
			return false;
		}
	}

	if (!bq->fcc_main_votable) {
		bq->fcc_main_votable = find_votable("FCC_MAIN");
		if (!bq->fcc_main_votable) {
			bq_dbg(PR_REGISTER, "bq27z561_fg Couldn't find FCC_MAIN voltable\n");
			return false;
		}
	}
...
static int fg_sha256_auth(struct bq_fg_chip *bq, u8 *rand_num, int length)
{
	...
	*/
	...
	if (bq->charge_120w_enable) {
		ret = fg_read_block(bq, bq->regs[BQ_FG_REG_MAC_DATA], bq->digest_120w, length);
	} else {
...
#define RECHARGE_VOL_BASE_120W	8400
...
static int fg_get_manufacture_data(struct bq_fg_chip *bq)
{
	...
	if (strncmp(t_buf, "MI", 2) != 0) {
		...
		if (bq->charge_120w_enable) {
			manu_info[TERMINATION].data = BQ27Z561_DEFUALT_TERM_120W;
			manu_info[FFC_TERMINATION].data = BQ27Z561_DEFUALT_FFC_TERM_120W;
			manu_info[RECHARGE_VOL].data = BQ27Z561_DEFUALT_RECHARGE_VOL_120W;
		} else {
	...
	if (bq->charge_120w_enable)
		manu_info[RECHARGE_VOL].base = RECHARGE_VOL_BASE_120W;
	else
		manu_info[RECHARGE_VOL].base = RECHARGE_VOL_BASE;
...
static int fg_read_system_soc(struct bq_fg_chip *bq)
{
	...
	soc = bq_battery_soc_smooth_tracking_sencond(bq, raw_soc, bat_soc, soc);
...
#define SHUTDOWN_DELAY_VOL_120W	6600
#define SHUTDOWN_VOL		3400
#define SHUTDOWN_VOL_120W	6800
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (bq->shutdown_delay_enable) {
			if (val->intval == 0) {
				...
				if (status != POWER_SUPPLY_STATUS_CHARGING) {
					if (bq->charge_120w_enable) {
						if (vbat_mv > SHUTDOWN_VOL_120W) {
							bq->shutdown_delay = false;
							val->intval = 1;
						} else if (vbat_mv > SHUTDOWN_DELAY_VOL_120W && vbat_mv <= SHUTDOWN_VOL_120W) {
						...
						if (vbat_mv > SHUTDOWN_VOL) {
							bq->shutdown_delay = false;
							val->intval = 1;
						} else if (vbat_mv > SHUTDOWN_DELAY_VOL && vbat_mv <= SHUTDOWN_VOL) {
							bq->shutdown_delay = true;
							val->intval = 1;
						} else {
							bq->shutdown_delay = false;
					...
					bq->shutdown_delay = false;
					if (bq->charge_120w_enable) {
						if (vbat_mv > SHUTDOWN_DELAY_VOL_120W)
							val->intval = 1;
					} else {
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		if (bq->charge_120w_enable && (val->intval == BQ_MAXIUM_VOLTAGE_FOR_CELL_120W)) {
			if (bq->batt_volt > BQ_PACK_MAXIUM_VOLTAGE_FOR_PMIC_SAFETY_120W) {
				ov_count++;
				if (ov_count > 2) {
					ov_count = 0;
					bq->cell_ov_check++;
			...
			val->intval = BQ_PACK_MAXIUM_VOLTAGE_FOR_PMIC_120W - bq->cell_ov_check * 10;
			if ((bq->batt_soc == 100) && (val->intval == BQ_PACK_MAXIUM_VOLTAGE_FOR_PMIC_120W))
				val->intval = BQ_MAXIUM_VOLTAGE_FOR_CELL_120W;
		} else if (!bq->charge_120w_enable && (val->intval == BQ_MAXIUM_VOLTAGE_FOR_CELL)) {
			       ...
			       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		...
		vote(bq->fcc_votable, FCC_FG_BAT_VERIFY_VOTER, true,
				!bq->verify_digest_success ? 2000000 : CURRENT_MAX);
		bq_dbg(PR_OEM, "verify_digest_success=%d\n", bq->verify_digest_success);
		//charger_manager_set_current_limit(!bq->verify_digest_success ? 2000000 : CURRENT_MAX, BAT_VERIFY_FCC);
		break;
	case POWER_SUPPLY_PROP_CHIP_OK:
		...
		char *buf)
{
	...
	if (bq->charge_120w_enable) {
		for (i = 0; i < BATTERY_DIGEST_LEN_120W; i++) {
			memset(digest_buf, 0, sizeof(digest_buf));
			snprintf(digest_buf, sizeof(digest_buf) - 1, "%02x", bq->digest_120w[i]);
			strlcat(buf, digest_buf, BATTERY_DIGEST_LEN_120W * 2 + 1);
		}
	} else {
		...
		size_t count)
{
	...
	if (bq->charge_120w_enable) {
		fg_sha256_auth(bq, random, BATTERY_DIGEST_LEN_120W);
	} else {
...
static ssize_t fg_attr_show_Cell_1_RaScale(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bq_fg_chip *bq = i2c_get_clientdata(client);
...
static DEVICE_ATTR(Cell_1_RaScale, S_IRUGO, fg_attr_show_Cell_1_RaScale, NULL);
...
static void fg_update_status(struct bq_fg_chip *bq)
{
	...
	bq_dbg(PR_OEM, "SOC:%d,Volt:%d,Cur:%d,Temp:%d,RM:%d,FC:%d,FAST:%d,ST:%d",
			bq->batt_soc, bq->batt_volt, bq->batt_curr, bq->batt_temp,
			bq->batt_rm, bq->batt_fcc, bq->fast_mode, bq->batt_st);
...
#define	CHG_DONE_LIMIT		-200
#define	CHG_DONE_LIMIT_120W	-121
#define	CHG_DONE_COUNT_MAX	3
#define	BATT_WARM_CV		4100
#define	BATT_WARM_CV_120W	8200
#define	BATT_WARM_CV_HY	10
#define	BATT_WARM_CHARGE_FULL_SOC	66

static int fg_update_charge_full(struct bq_fg_chip *bq)
{
	int rc;
	bool pmic_chg_done = false;
	static int chg_done_count = 0;
	...
	if (battery_get_vbus() <= 0) {
		bq_dbg(PR_OEM, "vbus does not exist, exit\n");
		return 0;
	}

	if (!bq->chg1_dev)
		bq->chg1_dev = get_charger_by_name("primary_chg");
	if (!bq->battery_67w && !bq->chg5_dev)
		bq->chg5_dev = get_charger_by_name("quinary_chg");
	...
	if (bq->charge_120w_enable)
		charger_dev_is_charging_done(bq->chg5_dev, &pmic_chg_done);
	else
		charger_dev_is_charging_done(bq->chg1_dev, &pmic_chg_done);

	rc = power_supply_get_property(bq->batt_psy,
		POWER_SUPPLY_PROP_HEALTH, &prop);
	bq->health = prop.intval;

	bq_dbg(PR_OEM, "raw:%d,done:%d,pmic_done:%d,full:%d,health:%d\n",
			bq->raw_soc, bq->charge_done, pmic_chg_done, bq->charge_full, bq->health);

	if (bq->charge_120w_enable && bq->batt_temp > BATT_WARM_THRESHOLD && bq->batt_curr >= CHG_DONE_LIMIT_120W &&
			bq->batt_curr <= 0 && bq->batt_volt >= BATT_WARM_CV_120W - BATT_WARM_CV_HY) {
		chg_done_count++;
		bq_dbg(PR_OEM, "batt warm chg_done_count:%d\n", chg_done_count);
		if (chg_done_count > CHG_DONE_COUNT_MAX) {
			battery_main.CHG_FULL_STATUS = true;
			bq_dbg(PR_OEM, "batt warm Setting charge_done\n");
		}
	} else if (!bq->charge_120w_enable && bq->batt_temp > BATT_WARM_THRESHOLD && bq->batt_curr >= CHG_DONE_LIMIT &&
			bq->batt_curr <= 0 && bq->batt_volt >= BATT_WARM_CV - BATT_WARM_CV_HY) {
		chg_done_count++;
		bq_dbg(PR_OEM, "batt warm chg_done_count:%d\n", chg_done_count);
		if (chg_done_count > CHG_DONE_COUNT_MAX) {
			battery_main.CHG_FULL_STATUS = true;
	...
	} else if (bq->batt_temp > BATT_WARM_THRESHOLD && battery_main.CHG_FULL_STATUS &&
			bq->batt_soc <= BATT_WARM_CHARGE_FULL_SOC) {
	...
	if (bq->charge_120w_enable && pmic_chg_done && !battery_main.CHG_FULL_STATUS) {
		if (bq->batt_soc >= BQ_DEFUALT_FULL_SOC) {
			bq_dbg(PR_OEM, "Setting charge_full to true\n");
			bq->charge_full = true;
			bq->cell_ov_check = 0;

			charger_dev_enable(bq->chg5_dev, 0);
			charger_dev_do_event(bq->chg5_dev, EVENT_EOC, 0);
			//charger_dev_set_charging_current(bq->chg5_dev, 0);
			vote(bq->fcc_main_votable, FCCMAIN_FG_CHARGE_FULL_VOTER, true, 0);
		} else {
			bq_dbg(PR_OEM, "charging is done raw soc:%d\n", bq->raw_soc);
		}
	} else if (!bq->charge_120w_enable && (bq->charge_done || pmic_chg_done) && !battery_main.CHG_FULL_STATUS) {
		if (bq->batt_soc >= BQ_DEFUALT_FULL_SOC) {
			bq_dbg(PR_OEM, "Setting charge_full to true\n");
			bq->charge_full = true;
			bq->cell_ov_check = 0;

			charger_dev_enable(bq->chg1_dev, 0);
			charger_dev_do_event(bq->chg1_dev, EVENT_EOC, 0);
			//charger_dev_set_charging_current(bq->chg5_dev, 0);
			vote(bq->fcc_main_votable, FCCMAIN_FG_CHARGE_FULL_VOTER, true, 0);
		} else {
			bq_dbg(PR_OEM, "charging is done raw soc:%d\n", bq->raw_soc);
	...
	bq_dbg(PR_OEM, "battery_main.CHG_FULL_STATUS:%d\n", battery_main.CHG_FULL_STATUS);
	if ((bq->raw_soc <= BQ_RECHARGE_SOC) && !bq->charge_done && !pmic_chg_done && battery_main.CHG_FULL_STATUS &&
			bq->health != POWER_SUPPLY_HEALTH_WARM && bq->batt_temp <= BATT_WARM_THRESHOLD) {
		bq_dbg(PR_OEM, "Setting recharg to true\n");
		battery_main.FORCE_RECHARGE = true;
		if (bq->battery_67w) {
			charger_dev_do_event(bq->chg1_dev, EVENT_RECHARGE, 0);
			charger_dev_enable(bq->chg1_dev, 1);
		} else {
			charger_dev_do_event(bq->chg5_dev, EVENT_RECHARGE, 0);
			charger_dev_enable(bq->chg5_dev, 1);
		}
		//charger_dev_set_charging_current(bq->chg5_dev, 3000000);
		vote(bq->fcc_main_votable, FCCMAIN_FG_CHARGE_FULL_VOTER, true, 3000000);
	}
	...
	int raw_soc, int batt_soc, int soc)
{
	...
	if (bq->charge_120w_enable && (raw_soc > 9500)) {
		if (raw_soc == 10000 && bq->last_soc < 99) {
			...
			soc = 100;
	} else if (!bq->charge_120w_enable && (raw_soc > 9700)) {
		if (raw_soc == 10000 && bq->last_soc < 99) {
			unit_time = 20000;
			calc_delta_time(changed_time, &change_delta);
			if (delta_time < 0) {
	...
	if(bq->last_soc != soc){
		if(abs(soc - bq->last_soc) > 1){
			...
			bq_dbg(PR_OEM,"avoid jump soc = %d last = %d soc_change = %d state = %d ,delta_time = %d\n",
			...
			if(status == POWER_SUPPLY_STATUS_CHARGING){
				if(soc > bq->last_soc){
					...
					bq_dbg(PR_OEM,"Do not smooth waiting real soc increase here\n");
					soc = bq->last_soc;
				}
				...
				if(soc < bq->last_soc){
					...
					bq_dbg(PR_OEM,"Do not smooth waiting real soc decrease here\n");
					soc = bq->last_soc;
				}
			}
		}else{
...
static void fg_monitor_workfunc(struct work_struct *work)
{
	...
	if (!bq->old_hw) {
		...
		fg_update_status(bq);
		charger_dev_is_charging_done(bq->chg1_dev, &pmic_chg_done);
		if ((bq->batt_soc < BQ_DEFUALT_FULL_SOC) && pmic_chg_done) {
			charger_dev_enable(bq->chg1_dev, 0);
			msleep(100);
			charger_dev_enable(bq->chg1_dev, 1);
		}
		...
		if ((bq->batt_soc == BQ_DEFUALT_FULL_SOC) ||
			((bq->batt_temp > BATT_WARM_THRESHOLD) && (bq->batt_volt >= BATT_WARM_CV - BATT_WARM_CV_HY)))
...
static int bq_parse_dt(struct bq_fg_chip *bq)
{
	...
	bq->battery_67w = of_property_read_bool(node, "bq,battery-67w");
	bq_dbg(PR_OEM, "battery_67w:[%d]\n", bq->battery_67w);
	...
	bq->charge_120w_enable = of_property_read_bool(node, "charge-120w-enable");

	return 0;
}

static bool before_P0;
static const char *HwVersion[] = {
	"3.0.0",
	"3.10.0",
	"3.20.0",
};

static int __init early_parse_hwversion(char *p)
{
	char* hwid_ptr = NULL;
	int i = 0;

	pr_info("get hwversion from cmdline : %s\n",p);

	if (p) {
		for (i = 0; i < 3; i++) {
			hwid_ptr = strstr(p, HwVersion[i]);
			if (hwid_ptr != NULL) {
				before_P0 = true;
				pr_info("get hwversion from cmdline flag : %d\n", before_P0);
				return 0;
			}
		}
		before_P0 = false;
...
__setup("androidboot.hwversion=", early_parse_hwversion);
				...
				const struct i2c_device_id *id)
{
	...
	pr_info("bq->chip:%d,model_name:%s\n",id->driver_data, (char *)id->name);
	if ((id->driver_data == BQ27Z561_I2C0 && before_P0 == false) ||
		(id->driver_data == BQ27Z561_I2C9 && before_P0 == true)) {
...
static const struct i2c_device_id bq_fg_id[] = {
	{ "bq27z561", BQ27Z561 },
	{ "bq28z610", BQ28Z610 },
	{ "bq27z561_i2c0", BQ27Z561_I2C0 },
	{ "bq27z561_i2c9", BQ27Z561_I2C9 },
	{},
};
