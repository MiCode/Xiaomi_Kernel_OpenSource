...
extern int charge_enabled_flag;
	...
	struct charger_consumer *consumer, int idx, int charging_current)
{
	...
	if (info != NULL) {
		...
		chr_err("ZLF:%s: dev:%s idx:%d en:%d\n", __func__,
...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	if (info->battery_temp >= info->data.temp_t4_thres) {
		...
		chr_err("[SW_JEITA] Battery Temperature between %d and %d !!\n",
				...
				info->data.temp_t0_thres);
		sw_jeita->sm = TEMP_neg_TO_T0;
	} else {
		chr_err("[SW_JEITA] Battery below low Temperature(%d) !!\n",
			info->data.temp_t0_thres);
		sw_jeita->sm = TEMP_BELOW_neg;
		sw_jeita->charging = false;
	}
	...
	if (sw_jeita->sm != TEMP_T2_TO_T3) {
		if (sw_jeita->sm == TEMP_ABOVE_T4) {
			...
			if(sw_jeita->pre_sm != sw_jeita->sm){
				ret = charger_manager_set_charging_current_limit(
						power_supply_consumer, 0, 0);
				if(ret < 0){
					chr_err("charger set TEMP_ABOVE_T4 charging current limit fail!\n");
					sw_jeita->sm = sw_jeita->pre_sm;
				}
			}
			...
			if(sw_jeita->pre_sm != sw_jeita->sm){
				ret = charger_manager_set_charging_current_limit(
						power_supply_consumer, 0, 1000000);
				if(ret < 0){
					chr_err("charger set TEMP_T3_TO_T4 charging current limit fail!\n");
					sw_jeita->sm = sw_jeita->pre_sm;
				}
			}
			...
			if(sw_jeita->pre_sm != sw_jeita->sm){
				ret = charger_manager_set_charging_current_limit(
						power_supply_consumer, 0, 2000000);
				if(ret < 0){
					chr_err("charger set TEMP_T2_TO_T3 charging current limit fail!\n");
					sw_jeita->sm = sw_jeita->pre_sm;
				}
			}
			...
			if(sw_jeita->pre_sm != sw_jeita->sm){
				ret = charger_manager_set_charging_current_limit(
						power_supply_consumer, 0, 1500000);
				if(ret < 0){
					chr_err("charger set TEMP_T1_TO_T2 charging current limit fail!\n");
					sw_jeita->sm = sw_jeita->pre_sm;
				}
			}
			...
			if(sw_jeita->pre_sm != sw_jeita->sm){
				ret = charger_manager_set_charging_current_limit(
						power_supply_consumer, 0, 900000);
				if(ret < 0){
					chr_err("charger set TEMP_T0_TO_T1 charging current limit fail!\n");
					sw_jeita->sm = sw_jeita->pre_sm;
				}
			}
		} else if (sw_jeita->sm == TEMP_neg_TO_T0) {
			sw_jeita->cv = info->data.jeita_temp_t0_to_t1_cv;
			if(sw_jeita->pre_sm != sw_jeita->sm){
				ret = charger_manager_set_charging_current_limit(
						power_supply_consumer, 0, 500000);
				if(ret < 0){
					chr_err("charger set TEMP_neg_TO_T0 charging current limit fail!\n");
					sw_jeita->sm = sw_jeita->pre_sm;
				}
			}
		} else if (sw_jeita->sm == TEMP_BELOW_neg) {
		...
		if(sw_jeita->pre_sm != sw_jeita->sm){
			ret = charger_manager_set_charging_current_limit(
					power_supply_consumer, 0, 2050000);
			if(ret < 0){
				chr_err("charger set TEMP_T2_TO_T3 charging current limit fail!\n");
				sw_jeita->sm = sw_jeita->pre_sm;
			}
...
static bool mtk_is_charger_on(struct charger_manager *info)
{
	...
	if (chr_type == CHARGER_UNKNOWN) {
		if (info->chr_type != CHARGER_UNKNOWN){
			...
			if ( charge_enabled_flag == 0){
				charge_enabled_flag = 1;
			}
		}
	} else {
		if (info->chr_type == CHARGER_UNKNOWN)
...
static void charger_check_status(struct charger_manager *info)
{
	...
	if (info->enable_sw_jeita == true) {
		...
		pr_err("ZLF:enable_sw_jeita=true.\n");
...
int Ibus_tmp = 0;
extern int charger_dev_dump_ibus(struct charger_device *charger_dev);
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		chg_current = charger_dev_dump_ibus(info->chg1_dev);
		Ibus_tmp = chg_current;
		chr_err("ZLF:Vbat=%d,Ibat=%d,Ibus=%d,Vbus=%d,T=%d,Soc=%d:%d,CT:%d:%d hv:%d pd:%d:%d\n",
