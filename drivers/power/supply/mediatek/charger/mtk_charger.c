	...
	if (info != NULL) {
		...
		for (i = 0; i < 10; i++) {
			...
			if (charger_identify_psy) {
				ret = power_supply_get_property(charger_identify_psy,
						POWER_SUPPLY_PROP_QC35_VID, &val);
				if (!ret)
					break;
			}
		}

		if ((i == 10) && (info->xmusb_vid != 0x584d3335))
			info->data.bc12_charger = MAIN_CHARGER;
		chr_err("%s: bc12_charger = %d, vid from xmusb350 = %02x %02x %02x %02x\n", __func__,
			info->data.bc12_charger, val.arrayval[0], val.arrayval[1], val.arrayval[2], val.arrayval[3]);
#endif
