...
static int ln8000_init_device(struct ln8000_info *info)
{
        ...
        ln8000_write_reg(info, LN8000_REG_THRESHOLD_CTRL, 0x06);
        ...
        if (info->dev_role == LN_ROLE_MASTER) {
                ...
                ln8000_update_reg(info, LN8000_REG_BC_OP_1, 0x7, 0x5);
                // OVPFETDR_HIGH_IMP=0
                ln8000_update_reg(info, LN8000_REG_PRODUCT_ID, 0x1 << 5, 0x0 << 5);
                ln8000_write_reg(info, LN8000_REG_LION_CTRL, 0x00);
        } else if (info->dev_role == LN_ROLE_SLAVE) {
                ln8000_write_reg(info, LN8000_REG_LION_CTRL, 0xAA);
                // dual_function_en=1, dual_cfg=1, dual_lockout_en=1
                ln8000_update_reg(info, LN8000_REG_BC_OP_1, 0x7, 0x7);
                // OVPFETDR_HIGH_IMP=1
                ln8000_update_reg(info, LN8000_REG_PRODUCT_ID, 0x1 << 5, 0x1 << 5);
                ln8000_write_reg(info, LN8000_REG_LION_CTRL, 0x00);
        }
#endif        
...
static int ln8000_init_irq(struct ln8000_info *info)
{
	 ...
	 ln_info("irq : %d\n",  info->irq);
        ...
        if (info->irq) {
                ret = devm_request_threaded_irq(info->dev, info->irq,
        ...
        mask = LN8000_MASK_ADC_DONE_INT | LN8000_MASK_TIMER_INT | LN8000_MASK_FAULT_INT | LN8000_MASK_REV_CURR_INT;
...
static int ln8000_parse_dt(struct ln8000_info *info)
{
	...
	ret = of_get_named_gpio(np, "ln8000_charger,interrupt_gpios", 0);
	if (ret < 0) {
		ln_info("no intr_gpio info\n");
		return ret;
	}
	info->irq_gpio = ret;
       ln_info("info->irq_gpio=%d\n", info->irq_gpio);
...
static int ln8000_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
        ...
        hardwareinfo_set_prop(HARDWARE_SUB_CHARGER_MASTER_IC_INFO, "LN8000_CHARGER_MASTER");
        return 0;
...
static int ln8000_suspend(struct device *dev)
{
        ...
        if (device_may_wakeup(dev) && info->irq)
                enable_irq_wake(info->irq);
