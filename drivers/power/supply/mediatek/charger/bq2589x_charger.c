...
static int bq2589x_plug_in(struct charger_device *chg_dev)
{
	...
	u8 reg_val = 0;
	int vbus_stat = 0;
  	union power_supply_propval pd_active = {-1,};
	static struct power_supply * usb_psy = NULL;
	usb_psy = power_supply_get_by_name("usb");
	if (usb_psy) {
		power_supply_get_property(usb_psy, POWER_SUPPLY_PROP_PD_ACTIVE, &pd_active);
	}

	ret = bq2589x_read_byte(bq, BQ2589X_REG_0B, &reg_val);
	if (ret)
		pr_err("%s: read vbus_stat err：%d\n", __func__, ret);

	vbus_stat = (reg_val & BQ2589X_VBUS_STAT_MASK);
	vbus_stat >>= BQ2589X_VBUS_STAT_SHIFT;
	pr_err("%s: get pd_active:%d,Auto bc12 get vbus_stat:%d\n", __func__, pd_active.intval, vbus_stat);

	if (vbus_stat == BQ2589X_VBUS_TYPE_NONE || (vbus_stat == BQ2589X_VBUS_TYPE_UNKNOWN && pd_active.intval == 0)) {
		__bq2589x_switch_dp_dm(bq, BQ2589X_DP_DM_CONNECT_CHG);
		bq2589x_force_dpdm(bq);
	}
