...
int get_quick_charge_type(struct mt_charger *mtk_chg)
{
	...
	if (pval.intval == QC35_HVDCP_3_PLUS_27 || pval.intval == QC35_HVDCP_30_27)
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			pr_info("%s USB Plug in, pol = %d, state = %d, rp_level = %d\n", __func__,
					noti->typec_state.polarity, noti->typec_state.new_state, noti->typec_state.rp_level);
			cti->usb_plug = 1;
