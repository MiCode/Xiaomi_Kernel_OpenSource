...
static int charge_current_limit_tmp = 2000;
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		val->intval = charge_current_limit_tmp;
		break;
	default:
		return -EINVAL;
	}
...
static struct charger_consumer *power_supply_consumer;
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		if(val->intval > 2000)
			charge_current_limit_tmp = 2000;
		else if(val->intval < 0)
			charge_current_limit_tmp = 0;
		else
			charge_current_limit_tmp = val->intval;
		charger_manager_set_charging_current_limit(
				power_supply_consumer, 0, charge_current_limit_tmp * 1000);
		pr_err("ZLF:set_charging_current=%d.\n", charge_current_limit_tmp);
		break;
...
static int mt_charger_probe(struct platform_device *pdev)
{
	...
	power_supply_consumer = charger_manager_get_by_name(&pdev->dev, "charger");
