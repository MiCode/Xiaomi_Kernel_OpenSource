...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (mtk_is_TA_support_pd_pps(info)) {
		...
		pdata->charging_current_limit =
				info->data.charging_host_charger_current;
		chr_err("ZLF:CHARGING_HOST:pdata->input_current_limit=%d, pdata->charging_current_limit=%d.\n",
	...
	if (info->chr_type == STANDARD_HOST
		...
		ret = charger_dev_is_powerpath_enabled(info->chg1_dev, &is_en);
		if (info->usb_state == USB_SUSPEND) {
			pr_debug("USB SUSPEND, PC into suspend.\n");
			if (is_en == true)
			...
			pr_debug("PC nosuspendand charging\n");
			if (is_en == false)
