...
#define CON2_ENABLE_AICR_MASK       1
#define CON2_ENABLE_AICR_SHIFT      4
...
static void do_set_input_current_work(struct work_struct *work)
{
	...
	pre_current = g_chg_info->set_input_curr;
	...
	if (g_chg_info->set_input_curr < 1500000) {
		SYV690_set_aicr(chg_dev, g_chg_info->set_input_curr);
		goto out;
	}

	if (g_chg_info->set_input_curr > 1500000) {
		SYV690_set_aicr(chg_dev, 1500000);
	} else {
		SYV690_set_aicr(chg_dev, g_chg_info->set_input_curr);
	}
	...
	vbus_volt = battery_get_vbus();
	if (vbus_volt < vindpm) {
		SYV690_set_aicr(chg_dev, 1200000);
		pr_info("%s %d vbus(%d) too low, return\n", __func__, __LINE__, vbus_volt);
		goto out;
	} else if (g_chg_info->set_input_curr <= 1500000) {
		SYV690_set_aicr(chg_dev, g_chg_info->set_input_curr);
		goto out;
	}

	if (g_chg_info->set_input_curr > 2000000) {
		SYV690_set_aicr(chg_dev, 2000000);
	} else {
		SYV690_set_aicr(chg_dev, g_chg_info->set_input_curr);
	}
	...
	if (vbus_volt < vindpm) {
		...
		pr_info("%s %d vbus(%d) too low, return\n", __func__, __LINE__, vbus_volt);
		goto out;
	} else if (g_chg_info->set_input_curr <= 2000000) {
		SYV690_set_aicr(chg_dev, g_chg_info->set_input_curr);
		goto out;
	}

	if (g_chg_info->set_input_curr > 2650000) {
		SYV690_set_aicr(chg_dev, 2650000);
	} else {
		SYV690_set_aicr(chg_dev, g_chg_info->set_input_curr);
	}
	...
	if (vbus_volt < vindpm) {
		...
		pr_info("%s %d vbus(%d) too low, return\n", __func__, __LINE__, vbus_volt);
		goto out;
	} else if (g_chg_info->set_input_curr <= 2650000) {
		SYV690_set_aicr(chg_dev, g_chg_info->set_input_curr);
		goto out;
	}
...
out:
	SYV690_get_aicr(chg_dev, &pre_reg);
	pr_err("%s curr:%d\n", __func__, pre_reg);
}
...
static  int SYV690_enable_aicr(struct SYV690_device *chgInfo, bool en);
static int SYV690_hw_init(struct SYV690_device *bq)
{
	...
	if (bq->chip_id == SYV690_ID)
		SYV690_enable_aicr(bq, false);
...
{
	int ret = 0;
	SYV690_update_bits(chgInfo,SYV690_REG_CON2, (en ? 1 : 0), CON2_ENABLE_AICR_MASK, CON2_ENABLE_AICR_SHIFT);
	return ret;
}
...
static int SYV690_parse_dt(struct device_node *np,   struct SYV690_device *chgInfo)
{
	...
	if (ret < 0) {
		...
		ret = gpio_request(chgInfo->usb_switch_cb1_gpio, "GPIO42");
	}
	...
	if (ret < 0) {
		...
		ret = gpio_request(chgInfo->usb_switch_cb2_gpio, "GPIO43");
	}
	...
	if (ret < 0) {
		...
		ret = gpio_request(chgInfo->otg_en_gpio, "GPIO79");
	}
...
static void do_hvdcp_det_work(struct work_struct *work)
{
	...
	if (g_chg_info->charger) {
		...
		if ((propval.intval == PPS_CHARGER) && (delta >= CHG_TYPE_REPORT_DELAY))
			return;
	}
	...
	if (pre_type != HVDCP_CHARGER && (SYV690_check_charger_type(g_chg_info) == SYV690_CHG_TYPE_HVDCP)) {
		...
		if (g_chg_info->chip_id != SC89890H_ID) {
			SYV690_handle_hvdcp(g_chg_info, false);
			SYV690_enable_force_dpdm(g_chg_info,true);
		}
		if(delta < CHG_TYPE_REPORT_DELAY)	
			schedule_delayed_work(&g_chg_info->hvdcp_det_work, msecs_to_jiffies(HVDCP_DETECT_DELAY*30));
	} else if (g_chg_info->chg_type == HVDCP_CHARGER || g_chg_info->chg_type == STANDARD_HOST) {
		if(delta < CHG_TYPE_REPORT_DELAY) {
			SYV690_psy_chg_type_changed(g_chg_info);	
			schedule_delayed_work(&g_chg_info->hvdcp_det_work, msecs_to_jiffies(HVDCP_DETECT_DELAY*30));
		}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (chg_info->chip_id == SYV690_ID)
		hardwareinfo_set_prop(HARDWARE_CHARGER_IC_INFO, "SYV690_CHARGER");
	else if (chg_info->chip_id == SC89890H_ID)
		hardwareinfo_set_prop(HARDWARE_CHARGER_IC_INFO, "SC89890H_CHARGER");
