...
#define pr_fmt(fmt)	"[sc8551_2] %s: " fmt, __func__
...
/************************************************************************/
static int __sc8551_3_read_byte(struct sc8551 *sc, u8 reg, u8 *data)
{
	s32 ret;
...
static int __sc8551_3_write_byte(struct sc8551 *sc, int reg, u8 val)
{
	s32 ret;
...
static int sc8551_3_read_byte(struct sc8551 *sc, u8 reg, u8 *data)
{
	int ret;
	...
	ret = __sc8551_3_read_byte(sc, reg, data);
	mutex_unlock(&sc->i2c_rw_lock);

	return ret;
}

static int sc8551_3_write_byte(struct sc8551 *sc, u8 reg, u8 data)
{
	int ret;
	...
	mutex_lock(&sc->i2c_rw_lock);
	ret = __sc8551_3_write_byte(sc, reg, data);
	mutex_unlock(&sc->i2c_rw_lock);

	return ret;
}

static int sc8551_3_update_bits(struct sc8551*sc, u8 reg,
				    u8 mask, u8 data)
{
	int ret;
	u8 tmp;
	...
	mutex_lock(&sc->i2c_rw_lock);
	ret = __sc8551_3_read_byte(sc, reg, &tmp);
	if (ret) {
	...
	ret = __sc8551_3_write_byte(sc, reg, tmp);
	if (ret)
		sc_err("Failed: reg=%02X, ret=%d\n", reg, ret);
...
static int sc8551_3_enable_charge(struct sc8551 *sc, bool enable)
{
	int ret;	
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_0C,
				SC8551_CHG_EN_MASK, val);

	return ret;
}
EXPORT_SYMBOL_GPL(sc8551_3_enable_charge);

static int sc8551_3_check_charge_enabled(struct sc8551 *sc, bool *enabled)
{
	int ret;
	u8 val;

	ret = sc8551_3_read_byte(sc, SC8551_REG_0C, &val);
	sc_info(">>>reg [0x0c] = 0x%02x\n", val);
	if (!ret)
		*enabled = !!(val & SC8551_CHG_EN_MASK);
	return ret;
}
EXPORT_SYMBOL_GPL(sc8551_3_check_charge_enabled);

static int sc8551_3_enable_wdt(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_0B,
				SC8551_WATCHDOG_DIS_MASK, val);
	return ret;
}
EXPORT_SYMBOL_GPL(sc8551_3_enable_wdt);

static int sc8551_3_set_wdt(struct sc8551 *sc, int ms)
{
	int ret;
	u8 val;
...
EXPORT_SYMBOL_GPL(sc8551_3_set_wdt);


static int sc8551_3_enable_batovp(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_00,
				SC8551_BAT_OVP_DIS_MASK, val);
	return ret;
}
EXPORT_SYMBOL_GPL(sc8551_3_enable_batovp);

static int sc8551_3_set_batovp_th(struct sc8551 *sc, int threshold)
{
	int ret;
	u8 val;
...
EXPORT_SYMBOL_GPL(sc8551_3_set_batovp_th);

static int sc8551_3_enable_batovp_alarm(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_01,
				SC8551_BAT_OVP_ALM_DIS_MASK, val);
	return ret;
}


static int sc8551_3_set_batovp_alarm_th(struct sc8551 *sc, int threshold)
{
	int ret;
	u8 val;
...
static int sc8551_3_enable_batocp(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_02,
				SC8551_BAT_OCP_DIS_MASK, val);
	return ret;
}


static int sc8551_3_set_batocp_th(struct sc8551 *sc, int threshold)
{
	int ret;
	u8 val;
...
static int sc8551_3_enable_batocp_alarm(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_03,
				SC8551_BAT_OCP_ALM_DIS_MASK, val);
	return ret;
}


static int sc8551_3_set_batocp_alarm_th(struct sc8551 *sc, int threshold)
{
	int ret;
	u8 val;
...
static int sc8551_3_set_busovp_th(struct sc8551 *sc, int threshold)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_06,
				SC8551_BUS_OVP_MASK, val);
	return ret;
}

static int sc8551_3_enable_busovp_alarm(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_07,
				SC8551_BUS_OVP_ALM_DIS_MASK, val);
	return ret;
}


static int sc8551_3_set_busovp_alarm_th(struct sc8551 *sc, int threshold)
{
	int ret;
	u8 val;
...
static int sc8551_3_enable_busocp(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_08,
				SC8551_BUS_OCP_DIS_MASK, val);
	return ret;
}


static int sc8551_3_set_busocp_th(struct sc8551 *sc, int threshold)
{
	int ret;
	u8 val;
...
static int sc8551_3_enable_busocp_alarm(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_09,
				SC8551_BUS_OCP_ALM_DIS_MASK, val);
	return ret;
}


static int sc8551_3_set_busocp_alarm_th(struct sc8551 *sc, int threshold)
{
	int ret;
	u8 val;
...
static int sc8551_3_enable_batucp_alarm(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_04,
				SC8551_BAT_UCP_ALM_DIS_MASK, val);
	return ret;
}


static int sc8551_3_set_batucp_alarm_th(struct sc8551 *sc, int threshold)
{
	int ret;
	u8 val;
...
static int sc8551_3_set_acovp_th(struct sc8551 *sc, int threshold)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_05,
...
static int sc8551_3_set_vdrop_th(struct sc8551 *sc, int threshold)
{
	int ret;
	u8 val;
...
static int sc8551_3_set_vdrop_deglitch(struct sc8551 *sc, int us)
{
	int ret;
	u8 val;
...
static int sc8551_3_enable_bat_therm(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
...
static int sc8551_set_bat_therm_th(struct sc8551 *sc, u8 threshold)
{
	...
	ret = sc8551_3_write_byte(sc, SC8551_REG_29, threshold);
	return ret;
}


static int sc8551_3_enable_bus_therm(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
 ...
 * the input threshold is the raw value that would write to register directly.
 */
static int sc8551_3_set_bus_therm_th(struct sc8551 *sc, u8 threshold)
{
	int ret;

	ret = sc8551_3_write_byte(sc, SC8551_REG_28, threshold);
	return ret;
}
 ...
 */
static int sc8551_3_set_die_therm_th(struct sc8551 *sc, u8 threshold)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_2A,
				SC8551_TDIE_ALM_MASK, val);
	return ret;
}

static int sc8551_3_enable_adc(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_14,
				SC8551_ADC_EN_MASK, val);
	return ret;
}


static int sc8551_3_set_adc_scanrate(struct sc8551 *sc, bool oneshot)
{
	int ret;
	u8 val;
...
static int sc8551_3_get_adc_data(struct sc8551 *sc, int channel,  int *result)
{
	int ret;
	u8 val_l, val_h;
	u16 val;
	...
	ret = sc8551_3_read_byte(sc, ADC_REG_BASE + (channel << 1), &val_h);
	ret = sc8551_3_read_byte(sc, ADC_REG_BASE + (channel << 1) + 1, &val_l);
...
//EXPORT_SYMBOL_GPL(sc8551_3_get_adc_data);

static int sc8551_3_set_adc_scan(struct sc8551 *sc, int channel, bool enable)
{
	int ret;
	u8 reg;
	u8 mask;
	...
	ret = sc8551_3_update_bits(sc, reg, mask, val);

	return ret;
}

static int sc8551_3_set_alarm_int_mask(struct sc8551 *sc, u8 mask)
{
	int ret;
	u8 val;

	ret = sc8551_3_read_byte(sc, SC8551_REG_0F, &val);
	if (ret)
		return ret;

	val |= mask;

	ret = sc8551_3_write_byte(sc, SC8551_REG_0F, val);

	return ret;
}


static int sc8551_3_clear_alarm_int_mask(struct sc8551 *sc, u8 mask)
{
	int ret;
	u8 val;
...
EXPORT_SYMBOL_GPL(sc8551_3_clear_alarm_int_mask);

static int sc8551_3_set_fault_int_mask(struct sc8551 *sc, u8 mask)
{
	int ret;
	u8 val;

	ret = sc8551_3_read_byte(sc, SC8551_REG_12, &val);
	if (ret)
		return ret;

	val |= mask;

	ret = sc8551_3_write_byte(sc, SC8551_REG_12, val);

	return ret;
}
EXPORT_SYMBOL_GPL(sc8551_3_set_fault_int_mask);

static int sc8551_3_clear_fault_int_mask(struct sc8551 *sc, u8 mask)
{
	int ret;
	u8 val;
...
EXPORT_SYMBOL_GPL(sc8551_3_clear_fault_int_mask);

static int sc8551_3_set_sense_resistor(struct sc8551 *sc, int r_mohm)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_2B,
				SC8551_SET_IBAT_SNS_RES_MASK,
				val);
	return ret;
}

static int sc8551_3_enable_regulation(struct sc8551 *sc, bool enable)
{
	int ret;
	u8 val;
...
static int sc8551_3_set_ss_timeout(struct sc8551 *sc, int timeout)
{
	int ret;
	u8 val;
...
static int sc8551_3_set_ibat_reg_th(struct sc8551 *sc, int th_ma)
{
	int ret;
	u8 val;
	...
	ret = sc8551_3_update_bits(sc, SC8551_REG_2C,
...
static int sc8551_3_set_vbat_reg_th(struct sc8551 *sc, int th_mv)
{
	int ret;
	u8 val;
...
static int sc8551_3_get_work_mode(struct sc8551 *sc, int *mode)
{
	int ret;
	u8 val;
...
static int sc8551_3_check_vbus_error_status(struct sc8551 *sc)
{
	int ret;
	u8 data;
	
	ret = sc8551_3_read_byte(sc, SC8551_REG_0A, &data);
	if(ret == 0){
...
static int sc8551_3_detect_device(struct sc8551 *sc)
{
	int ret;
	u8 data;

	ret = sc8551_3_read_byte(sc, SC8551_REG_13, &data);
	if (ret == 0) {
...
static int sc8551_3_parse_dt(struct sc8551 *sc, struct device *dev)
{
	int ret;
...
static int sc8551_3_init_protection(struct sc8551 *sc)
{
	int ret;

	ret = sc8551_3_enable_batovp(sc, !sc->cfg->bat_ovp_disable);
	sc_info("%s bat ovp %s\n",
		sc->cfg->bat_ovp_disable ? "disable" : "enable",
		!ret ? "successfullly" : "failed");

	ret = sc8551_3_enable_batocp(sc, !sc->cfg->bat_ocp_disable);
	sc_info("%s bat ocp %s\n",
		sc->cfg->bat_ocp_disable ? "disable" : "enable",
		!ret ? "successfullly" : "failed");

	ret = sc8551_3_enable_batovp_alarm(sc, !sc->cfg->bat_ovp_alm_disable);
	sc_info("%s bat ovp alarm %s\n",
		sc->cfg->bat_ovp_alm_disable ? "disable" : "enable",
		!ret ? "successfullly" : "failed");

	ret = sc8551_3_enable_batocp_alarm(sc, !sc->cfg->bat_ocp_alm_disable);
	sc_info("%s bat ocp alarm %s\n",
		sc->cfg->bat_ocp_alm_disable ? "disable" : "enable",
		!ret ? "successfullly" : "failed");

	ret = sc8551_3_enable_batucp_alarm(sc, !sc->cfg->bat_ucp_alm_disable);
	sc_info("%s bat ocp alarm %s\n",
		sc->cfg->bat_ucp_alm_disable ? "disable" : "enable",
		!ret ? "successfullly" : "failed");

	ret = sc8551_3_enable_busovp_alarm(sc, !sc->cfg->bus_ovp_alm_disable);
	sc_info("%s bus ovp alarm %s\n",
		sc->cfg->bus_ovp_alm_disable ? "disable" : "enable",
		!ret ? "successfullly" : "failed");

	ret = sc8551_3_enable_busocp(sc, !sc->cfg->bus_ocp_disable);
	sc_info("%s bus ocp %s\n",
		sc->cfg->bus_ocp_disable ? "disable" : "enable",
		!ret ? "successfullly" : "failed");

	ret = sc8551_3_enable_busocp_alarm(sc, !sc->cfg->bus_ocp_alm_disable);
	sc_info("%s bus ocp alarm %s\n",
		sc->cfg->bus_ocp_alm_disable ? "disable" : "enable",
		!ret ? "successfullly" : "failed");

	ret = sc8551_3_enable_bat_therm(sc, !sc->cfg->bat_therm_disable);
	sc_info("%s bat therm %s\n",
		sc->cfg->bat_therm_disable ? "disable" : "enable",
		!ret ? "successfullly" : "failed");

	ret = sc8551_3_enable_bus_therm(sc, !sc->cfg->bus_therm_disable);
	sc_info("%s bus therm %s\n",
		sc->cfg->bus_therm_disable ? "disable" : "enable",
		!ret ? "successfullly" : "failed");

	ret = sc8551_3_set_batovp_th(sc, sc->cfg->bat_ovp_th);
	sc_info("set bat ovp th %d %s\n", sc->cfg->bat_ovp_th,
		!ret ? "successfully" : "failed");

	ret = sc8551_3_set_batovp_alarm_th(sc, sc->cfg->bat_ovp_alm_th);
	sc_info("set bat ovp alarm threshold %d %s\n", sc->cfg->bat_ovp_alm_th,
		!ret ? "successfully" : "failed");

	ret = sc8551_3_set_batocp_th(sc, sc->cfg->bat_ocp_th);
	sc_info("set bat ocp threshold %d %s\n", sc->cfg->bat_ocp_th,
		!ret ? "successfully" : "failed");

	ret = sc8551_3_set_batocp_alarm_th(sc, sc->cfg->bat_ocp_alm_th);
	sc_info("set bat ocp alarm threshold %d %s\n", sc->cfg->bat_ocp_alm_th,
		!ret ? "successfully" : "failed");

	ret = sc8551_3_set_busovp_th(sc, sc->cfg->bus_ovp_th);
	sc_info("set bus ovp threshold %d %s\n", sc->cfg->bus_ovp_th,
		!ret ? "successfully" : "failed");

	ret = sc8551_3_set_busovp_alarm_th(sc, sc->cfg->bus_ovp_alm_th);
	sc_info("set bus ovp alarm threshold %d %s\n", sc->cfg->bus_ovp_alm_th,
		!ret ? "successfully" : "failed");

	ret = sc8551_3_set_busocp_th(sc, sc->cfg->bus_ocp_th);
	sc_info("set bus ocp threshold %d %s\n", sc->cfg->bus_ocp_th,
		!ret ? "successfully" : "failed");

	ret = sc8551_3_set_busocp_alarm_th(sc, sc->cfg->bus_ocp_alm_th);
	sc_info("set bus ocp alarm th %d %s\n", sc->cfg->bus_ocp_alm_th,
		!ret ? "successfully" : "failed");

	ret = sc8551_3_set_batucp_alarm_th(sc, sc->cfg->bat_ucp_alm_th);
	...
	sc_info("set die therm threshold %d %s\n", sc->cfg->bat_therm_th,
		!ret ? "successfully" : "failed");
	ret = sc8551_3_set_bus_therm_th(sc, sc->cfg->bus_therm_th);
	sc_info("set bus therm threshold %d %s\n", sc->cfg->bus_therm_th,
		!ret ? "successfully" : "failed");
	ret = sc8551_3_set_die_therm_th(sc, sc->cfg->die_therm_th);
	sc_info("set die therm threshold %d %s\n", sc->cfg->die_therm_th,
		!ret ? "successfully" : "failed");

	ret = sc8551_3_set_acovp_th(sc, sc->cfg->ac_ovp_th);
...
static int sc8551_3_init_adc(struct sc8551 *sc)
{

	sc8551_3_set_adc_scanrate(sc, false);
	sc8551_3_set_adc_scan(sc, ADC_IBUS, true);
	sc8551_3_set_adc_scan(sc, ADC_VBUS, true);
	sc8551_3_set_adc_scan(sc, ADC_VOUT, true);
	sc8551_3_set_adc_scan(sc, ADC_VBAT, true);
	sc8551_3_set_adc_scan(sc, ADC_IBAT, true);
	sc8551_3_set_adc_scan(sc, ADC_TBUS, true);
	sc8551_3_set_adc_scan(sc, ADC_TBAT, true);
	sc8551_3_set_adc_scan(sc, ADC_TDIE, true);
	sc8551_3_set_adc_scan(sc, ADC_VAC, true);

	sc8551_3_enable_adc(sc, true);

	return 0;
}

static int sc8551_3_init_int_src(struct sc8551 *sc)
{
	int ret;
	 ...
	 */
	ret = sc8551_3_set_alarm_int_mask(sc, ADC_DONE
		/*			| BAT_UCP_ALARM */
					| BAT_OVP_ALARM);
	if (ret) {
...
static int sc8551_3_init_regulation(struct sc8551 *sc)
{
	sc8551_3_set_ibat_reg_th(sc, 300);
	sc8551_3_set_vbat_reg_th(sc, 100);

	sc8551_3_set_vdrop_deglitch(sc, 5000);
	sc8551_3_set_vdrop_th(sc, 400);

	sc8551_3_enable_regulation(sc, false);
	
	sc8551_3_write_byte(sc, SC8551_REG_2E, 0x08);
	sc8551_3_update_bits(sc, SC8551_REG_34, 0x01, 0x01);
	//add 20210825
	//sc8551_3_update_bits(sc, SC8551_REG_30, 0x80, 0x80);
	
	return 0;
}

static int sc8551_3_init_device(struct sc8551 *sc)
{
	sc8551_3_update_bits(sc, SC8551_REG_0B, 0x80, 0x80);
	sc8551_3_enable_wdt(sc, false);

	sc8551_3_set_ss_timeout(sc, 100000);
	sc8551_3_set_sense_resistor(sc, sc->cfg->sense_r_mohm);

	sc8551_3_init_protection(sc);
	sc8551_3_init_adc(sc);
	sc8551_3_init_int_src(sc);

	sc8551_3_init_regulation(sc);

	return 0;
}


static int sc8551_3_set_present(struct sc8551 *sc, bool present)
{
	sc->usb_present = present;

	if (present)
		sc8551_3_init_device(sc);
	return 0;
}

static ssize_t sc8551_show_registers3(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	for (addr = 0x0; addr <= 0x31; addr++) {
		ret = sc8551_3_read_byte(sc, addr, &val);
		if (ret == 0) {
...
static ssize_t sc8551_store_register3(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret == 2 && reg <= 0x31)
		sc8551_3_write_byte(sc, (unsigned char)reg, (unsigned char)val);

	return count;
}


static DEVICE_ATTR(registers3, 0660, sc8551_show_registers3, sc8551_store_register3);

static void sc8551_3_create_device_node(struct device *dev)
{
	device_create_file(dev, &dev_attr_registers3);
}


static enum power_supply_property sc8551_3_charger_props[] = {
...
static void sc8551_3_check_alarm_status(struct sc8551 *sc);
static void sc8551_3_check_fault_status(struct sc8551 *sc);

static int sc8551_3_charger_get_property(struct power_supply *psy,
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		sc8551_3_check_charge_enabled(sc, &sc->charge_enabled);
	...
	case POWER_SUPPLY_PROP_SC_BATTERY_PRESENT:
		ret = sc8551_3_read_byte(sc, SC8551_REG_0D, &reg_val);
		if (!ret)
	...
	case POWER_SUPPLY_PROP_SC_BATTERY_VOLTAGE:
		ret = sc8551_3_get_adc_data(sc, ADC_VBAT, &result);
		if (!ret)
			sc->vbat_volt = result;
	...
	case POWER_SUPPLY_PROP_SC_BATTERY_CURRENT:
		ret = sc8551_3_get_adc_data(sc, ADC_IBUS, &result);
		if (!ret)
			sc->ibat_curr = result;
	...
	case POWER_SUPPLY_PROP_SC_BATTERY_TEMPERATURE:
		ret = sc8551_3_get_adc_data(sc, ADC_TBAT, &result);
		if (!ret)
			sc->bat_temp = result;
	...
	case POWER_SUPPLY_PROP_SC_BUS_VOLTAGE:
		ret = sc8551_3_get_adc_data(sc, ADC_VBUS, &result);
		if (!ret)
			sc->vbus_volt = result;
	...
	case POWER_SUPPLY_PROP_SC_BUS_TEMPERATURE:
		ret = sc8551_3_get_adc_data(sc, ADC_TBUS, &result);
		if (!ret)
			sc->bus_temp = result;
	...
	case POWER_SUPPLY_PROP_SC_DIE_TEMPERATURE:
		ret = sc8551_3_get_adc_data(sc, ADC_TDIE, &result);
		if (!ret)
			sc->die_temp = result;
	...
	case POWER_SUPPLY_PROP_SC_ALARM_STATUS:
		...
		sc8551_3_check_alarm_status(sc);
	...
	case POWER_SUPPLY_PROP_SC_FAULT_STATUS:
		sc8551_3_check_fault_status(sc);
	...
	case POWER_SUPPLY_PROP_SC_VBUS_ERROR_STATUS:
		sc8551_3_check_vbus_error_status(sc);
...
static int sc8551_3_charger_set_property(struct power_supply *psy,
				       ...
				       const union power_supply_propval *val)
{
	...
	switch (prop) {
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		sc8551_3_enable_charge(sc, val->intval);
		sc8551_3_check_charge_enabled(sc, &sc->charge_enabled);
		sc_info("POWER_SUPPLY_PROP_CHARGING_ENABLED: %s\n",
				val->intval ? "enable" : "disable");
		break;
	case POWER_SUPPLY_PROP_PRESENT:
		sc8551_3_set_present(sc, !!val->intval);
		break;
	default:
		return -EINVAL;
	}
...
static int sc8551_3_charger_is_writeable(struct power_supply *psy,
				       enum power_supply_property prop)
{
	int ret;
...
static int sc8551_3_psy_register(struct sc8551 *sc)
{
	int ret;

	sc->psy_cfg.drv_data = sc;
	sc->psy_cfg.of_node = sc->dev->of_node;
	sc->psy_desc.name = "sc8551_2-standalone";
	sc->psy_desc.type = POWER_SUPPLY_TYPE_MAINS;
	sc->psy_desc.properties = sc8551_3_charger_props;
	sc->psy_desc.num_properties = ARRAY_SIZE(sc8551_3_charger_props);
	sc->psy_desc.get_property = sc8551_3_charger_get_property;
	sc->psy_desc.set_property = sc8551_3_charger_set_property;
	sc->psy_desc.property_is_writeable = sc8551_3_charger_is_writeable;
...
static irqreturn_t sc8551_3_charger_interrupt(int irq, void *dev_id);
static int sc8551_3_init_irq(struct sc8551 *sc)
{
	int ret;
	...
	if (sc->mode == SC8551_ROLE_MASTER) {
		ret = devm_request_threaded_irq(sc->dev, sc->irq,
			NULL, sc8551_3_charger_interrupt,
...
static void sc8551_3_check_alarm_status(struct sc8551 *sc)
{
	...
	ret = sc8551_3_read_byte(sc, SC8551_REG_0E, &flag);
	if (!ret && flag)
		sc_dbg("INT_FLAG =0x%02X\n", flag);

	ret = sc8551_3_read_byte(sc, SC8551_REG_0D, &stat);
	...
	ret = sc8551_3_read_byte(sc, SC8551_REG_08, &stat);
	if (!ret && (stat & 0x50))
		sc_err("Reg[05]BUS_UCPOVP = 0x%02X\n", stat);

	ret = sc8551_3_read_byte(sc, SC8551_REG_0A, &stat);
	if (!ret && (stat & 0x02))
...
static void sc8551_3_check_fault_status(struct sc8551 *sc)
{
	...
	ret = sc8551_3_read_byte(sc, SC8551_REG_10, &stat);
	if (!ret && stat)
		sc_err("FAULT_STAT = 0x%02X\n", stat);

	ret = sc8551_3_read_byte(sc, SC8551_REG_11, &flag);
	if (!ret && flag)
...
/*static int sc8551_check_reg_status(struct sc8551 *sc)
{
	...
	ret = sc8551_3_read_byte(sc, SC8551_REG_2C, &val);
	if (!ret) {
...
static void determine_3_initial_status(struct sc8551 *sc)
{
	if (sc->client->irq)
		sc8551_3_charger_interrupt(sc->client->irq, sc);
}


static struct of_device_id sc8551_3_charger_match_table[] = {
	{
		.compatible = "sc,sc8551_2-standalone",
		.data = &sc8551_3_mode_data[SC8551_STDALONE],
	},
	{},
};

static int sc8551_3_charger_probe(struct i2c_client *client,
					const struct i2c_device_id *id)
{
	...
	if (ret) {
        ...
        if (sc->part_no != SC8551_DEVICE_ID) {
             sc_err("detect sc8551_3 device fail, sc->part_no = %d!\n", sc->part_no);
             return -ENODEV;
	}

	i2c_set_clientdata(client, sc);
	sc8551_3_create_device_node(&(client->dev));

	match = of_match_node(sc8551_3_charger_match_table, node);
	if (match == NULL) {
		sc_err("device tree match not found!\n");
		return -ENODEV;
	}

	sc8551_3_get_work_mode(sc, &sc->mode);
	...
	ret = sc8551_3_parse_dt(sc, &client->dev);
	if (ret)
		return -EIO;

	ret = sc8551_3_init_device(sc);
	if (ret) {
		sc_err("Failed to init device\n");
		return ret;
	}

	ret = sc8551_3_psy_register(sc);
	if (ret)
		goto err_1;

	ret = sc8551_3_init_irq(sc);
	if (ret)
		return ret;

	determine_3_initial_status(sc);

	sc_info("sc8551 probe successfully, Part Num:%d\n!",
				sc->part_no);

	hardwareinfo_set_prop(HARDWARE_SUB_CHARGER_SLAVE_IC_INFO, "SC8551_CHARGER_SLAVE");
...
static inline bool is_device_3_suspended(struct sc8551 *sc)
{
	return !sc->resume_completed;
}

static int sc8551_3_suspend(struct device *dev)
{
	...
	if (chgType == CHARGER_UNKNOWN)
		sc8551_3_update_bits(sc, SC8551_REG_14, SC8551_ADC_EN_MASK, regVal);
...
static int sc8551_3_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct sc8551 *sc = i2c_get_clientdata(client);
	enum charger_type chgType = CHARGER_UNKNOWN;
...
static int sc8551_3_charger_remove(struct i2c_client *client)
{
	struct sc8551 *sc = i2c_get_clientdata(client);


	sc8551_3_enable_adc(sc, false);
...
static void sc8551_3_charger_shutdown(struct i2c_client *client)
{
	struct sc8551 *sc = i2c_get_clientdata(client);
...
static const struct dev_pm_ops sc8551_3_pm_ops = {
	.resume		= sc8551_3_resume,
//	.suspend_noirq = sc8551_3_suspend_noirq,
	.suspend	= sc8551_3_suspend,
};

static const struct i2c_device_id sc8551_3_charger_id[] = {
	{"sc8551_2-standalone", SC8551_ROLE_STDALONE},
	{},
};
	
static struct i2c_driver sc8551_3_charger_driver = {
	.driver		= {
		.name	= "sc8551_2-charger",
		.owner	= THIS_MODULE,
		.of_match_table = sc8551_3_charger_match_table,
		.pm	= &sc8551_3_pm_ops,
	},
	.id_table	= sc8551_3_charger_id,

	.probe		= sc8551_3_charger_probe,
	.remove		= sc8551_3_charger_remove,
	.shutdown	= sc8551_3_charger_shutdown,
};

module_i2c_driver(sc8551_3_charger_driver);
