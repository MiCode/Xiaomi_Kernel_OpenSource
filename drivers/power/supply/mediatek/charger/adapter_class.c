		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ((adapter_dev->adapter_id == 0) && (adapter_dev->adapter_svid == 0) && (adapter_dev->verify_process == 0)) {
		pr_err("adapter maybe removed, force set usbpd verifyed :0\n");
		val = 0;
	}
...
static int __init adapter_class_init(void)
{
	...
	if (IS_ERR(adapter_class)) {
		pr_notice("Unable to create Charging_Adapter class; errno = %ld\n",
