...
int charge_enabled_flag = 1;
extern struct charger_manager *chg_info;
int charger_dev_enable(struct charger_device *chg_dev, bool en)
{
	bool is_en = true;
	int ret = 0;

	en = (en && charge_enabled_flag);
	pr_err("ZLF:charger_dev_enable:en = %d,charge_enable_flag = %d\n",en,charge_enabled_flag);

	ret = charger_dev_is_powerpath_enabled(chg_dev, &is_en);
	if(en) {
		if(is_en == false) {
			pr_err("ZLF:%s: power path is disabled, now enable power path, is_en = %d\n", __func__, is_en);
			charger_dev_enable_powerpath(chg_info->chg1_dev, true);
			charger_manager_notifier(chg_info, CHARGER_NOTIFY_START_CHARGING);
		} else {
			pr_err("ZLF:is_en true.\n");
		}
	} else {
		if(is_en == true) {
			pr_err("ZLF:%s: power path is enabled, now disable power path, is_en = %d\n", __func__, is_en);
			charger_dev_enable_powerpath(chg_info->chg1_dev, false);
			charger_manager_notifier(chg_info, CHARGER_NOTIFY_STOP_CHARGING);
		} else {
			pr_err("ZLF:is_en false.\n");
		}
	}
...
int charger_dev_dump_ibus(struct charger_device *charger_dev)
{
	if (charger_dev != NULL && charger_dev->ops != NULL && charger_dev->ops->dump_registers)
		return charger_dev->ops->dump_ibus(charger_dev);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_dump_ibus);
