...
#include "../charger_ares/mtk_charger_init.h"
#include "../charger_ares/mtk_charger_intf.h"
...
struct bq_fg_chip {
	...
	u8 regs[NUM_REGS];
	char *model_name;
	char vendor_name[BATTERY_DIGEST_LEN];
...
static int fg_get_manufacture_data(struct bq_fg_chip *bq)
{
	...
	if (t_buf[2] == '4') {
		...
		if (ret < 0)
			bq_dbg(PR_OEM, "failed to get FG_MSC_DEVICE_NAME\n");

		if (!strncmp(data, "NFG1000A", 8)) {
			bq_dbg(PR_OEM, "NFG1000A\n");
			strcpy(bq->vendor_name, "NFG1000A");
		} else if (!strncmp(data, "NFG1000B", 8)) {
			bq_dbg(PR_OEM, "NFG1000B\n");
			strcpy(bq->vendor_name, "NFG1000B");
		} else {
			bq_dbg(PR_OEM, "UNKNOWN\n");
			strcpy(bq->vendor_name, "UNKNOWN");
		}
	} else if (t_buf[2] == 'C') {
		bq_dbg(PR_OEM, "bq27z561\n");
		strcpy(bq->vendor_name, "bq27z561");
	} else {
		bq_dbg(PR_OEM, "UNKNOWN\n");
		strcpy(bq->vendor_name, "UNKNOWN");
	}
...
static void fg_update_status(struct bq_fg_chip *bq)
{
	...
	bq_dbg(PR_OEM, "SOC:%d,Volt:%d,Cur:%d,Temp:%d,RM:%d,FC:%d,FAST:%d,ST:%d,NAME:%s",
			bq->batt_soc, bq->batt_volt, bq->batt_curr, bq->batt_temp,
			bq->batt_rm, bq->batt_fcc, bq->fast_mode, bq->batt_st, bq->vendor_name);
	...
	int raw_soc, int batt_soc, int soc)
{
	...
	if (rc < 0) {
		bq_dbg(PR_OEM, "Failed to get charge status\n");
		return batt_soc;
	}
	status = pval.intval;
	rc = power_supply_get_property(bq->usb_psy, POWER_SUPPLY_PROP_REAL_TYPE, &pval);
	if (rc < 0) {
		bq_dbg(PR_OEM, "Failed to get charger type\n");
		return batt_soc;
	}
	...
	if(bq->last_soc <= 0)
		bq->last_soc = soc;
	if (((status == POWER_SUPPLY_STATUS_CHARGING) && (soc < bq->last_soc) && (charger_type != POWER_SUPPLY_TYPE_USB)) ||
			((status == POWER_SUPPLY_STATUS_DISCHARGING) && (soc > bq->last_soc))) {
			soc = bq->last_soc;
	}else if(bq->last_soc != soc){
