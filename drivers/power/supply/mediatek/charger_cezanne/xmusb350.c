...
struct xmusb350_charger {
	...
	enum thermal_status_levels	otg_mode_thermal_status;
...
static void smblib_conn_therm_work(struct work_struct *work)
{
	...
	if (chip->connector_temp >= CONNECTOR_THERM_TOO_HIG) {
		...
		if ((thermal_status == TEMP_ABOVE_RANGE)  &&
			...
			xm35_err("chg->connector_temp:%d is still warm\n", chip->connector_temp);
		} else {
...
static int smblib_set_otg_conn_therm_regulation(struct xmusb350_charger *chip,
						bool enable)
{
	xm35_dbg("%s enable: %d\n", __func__, enable);
	...
	if (enable) {
		chip->entry_time = ktime_get();
		schedule_delayed_work(&chip->otg_mode_conn_therm_work,
				msecs_to_jiffies(THERM_REG_RECHECK_DELAY_1S));

	} else {
		if (chip->thermal_status != TEMP_ABOVE_RANGE)
			cancel_delayed_work(&chip->otg_mode_conn_therm_work);
	}
...
static void smblib_otg_mode_conn_therm_work(struct work_struct *work)
{
	...
	u64 elapsed_us;
	static bool otg_mode_conn_enable = true;
	...
	if (chip->connector_temp >=  CONNECTOR_THERM_TOO_HIG) {
		xm35_err("chg->otg_mode_connector_temp:%d is too high\n", chip->connector_temp);
		otg_mode_thermal_status = TEMP_ABOVE_RANGE;
		wdog_timeout = THERM_REG_RECHECK_DELAY_1S;
	} else if (chip->connector_temp >=  CONNECTOR_THERM_HIG) {
		if ((otg_mode_thermal_status == TEMP_ABOVE_RANGE)  &&
			(chip->connector_temp > CONNECTOR_THERM_TOO_HIG - 100)) {
			xm35_err("chg->otg_mode_connector_temp:%d is still warm\n", chip->connector_temp);
		} else {
	...
	if (otg_mode_thermal_status != chip->otg_mode_thermal_status) {
		chip->otg_mode_thermal_status = otg_mode_thermal_status;
		if (otg_mode_thermal_status == TEMP_ABOVE_RANGE) {
			xm35_err("otg mode connect temp is too hot, disable OTG\n");
			otg_mode_conn_enable = false;
			charger_manager_enable_otg(chip->chg_consumer, MAIN_CHARGER, false);
		} else {
			xm35_err("connect temp normal recovery enable OTG\n");
			otg_mode_conn_enable = true;
			charger_manager_enable_otg(chip->chg_consumer, MAIN_CHARGER, true);
			chip->usb_switch = USBSW_USB;
			xmusb350_set_usbsw_state(chip);
		}
	}

	xm35_err("%s: conn_temp:%d, otg_therm_status:%d otg_en:%d wdog_timeout:%d.\n",
			chip->connector_temp, otg_mode_thermal_status, otg_mode_conn_enable, wdog_timeout);
	...
	schedule_delayed_work(&chip->otg_mode_conn_therm_work, msecs_to_jiffies(wdog_timeout));
...
void xmusb350_hard_reset(struct xmusb350_charger *chip)
{
	...
	msleep(10);// 10us, at least
	gpio_set_value(chip->xmusb350_rst_gpio, 0);// OUT: 0
	msleep(60);// 5.3ms, at least
...
static int xmusb350_enable_chg_type_det(struct charger_device *chg_dev, bool en)
{
	struct xmusb350_charger *chip = dev_get_drvdata(&chg_dev->dev);
	union power_supply_propval pval = {0,};
	int max_cdp_block = 200, ret = 0, i = 0, typec_mode = POWER_SUPPLY_TYPEC_NONE, boot_mode = NORMAL_BOOT;
	...
	if (en && (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || boot_mode == LOW_POWER_OFF_CHARGING_BOOT))
		xmusb350_hard_reset(chip);
...
static int xmusb350_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	smblib_set_otg_conn_therm_regulation(chip, en);
...
static void xmusb350_hard_reset_work(struct work_struct *work)
{
	...
	xm35_err("chip_version = %04x\n", chip_version);
}

#define CHIP_UPDATE_DELAY	15000
			...
			const struct i2c_device_id *id)
{
	...
	schedule_delayed_work(&chip->chip_update_work, msecs_to_jiffies(CHIP_UPDATE_DELAY));
	schedule_delayed_work(&chip->chip_reset_xmusb350, msecs_to_jiffies(CHIP_RESET_DELAY));
	...
	INIT_DELAYED_WORK(&chip->otg_mode_conn_therm_work, smblib_otg_mode_conn_therm_work);
...
static int xmusb350_charger_remove(struct i2c_client *client)
{
	...
	cancel_delayed_work_sync(&chip->otg_mode_conn_therm_work);
...
static int xmusb350_suspend(struct device *dev)
{
	...
	smblib_set_otg_conn_therm_regulation(chip, false);
