...
extern int batt_id;
static const char *batt_type_Default	= "Unknown Battery";
static const char *batt_type_NVT    = "NVT_68K";
static const char *batt_type_SWD    = "SWD_330K";
...
void battery_update_psd(struct battery_data *bat_data)
{
	...
	bat_data->BAT_batt_temp = force_get_tbat(false);
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_batt_type:
		if (batt_id == 1)
			val->strval = batt_type_NVT;
		else if (batt_id == 3)
			val->strval = batt_type_SWD;
		else
			val->strval = batt_type_Default;
		break;
	case POWER_SUPPLY_PROP_batt_full_design:
		val->intval = 3000000;
		break;
	...
	case POWER_SUPPLY_PROP_BatteryAverageCurrent:
		...
		break;
	case POWER_SUPPLY_PROP_Ibus:
		val->intval = Ibus_tmp / 1000;
		break;
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		val->intval = charge_enabled_flag;
		break;
	...
	const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		if(val->intval == 0) {
			charge_enabled_flag = 0;
			pr_err("ZLF:charge disabled:set_prop:charge_enabled_flag=%d.\n", charge_enabled_flag);
		} else {
			charge_enabled_flag = 1;
			pr_err("ZLF:charge enabled:set_prop:charge_enabled_flag=%d.\n", charge_enabled_flag);
		}
		break;
	default:
		return -EINVAL;
...
void battery_update(struct battery_data *bat_data)
{
	...
	if (batt_full_flag == 1 && battery_get_bat_temperature() < 45 &&
			upmu_get_rgs_chrdet() != 0 && bat_data->BAT_CAPACITY == 100){
		bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_FULL;
		bm_err("battery_update set FULL! ui:%d chr:%d %d\n",bat_data->BAT_CAPACITY, upmu_get_rgs_chrdet(), bat_data->BAT_STATUS);
	}else if(batt_full_flag == 0 && battery_get_bat_temperature() < 45 &&
			upmu_get_rgs_chrdet() != 0 && bat_data->BAT_CAPACITY == 100){
		bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_CHARGING;
		bm_err("battery_update set charging! ui:%d chr:%d %d\n",bat_data->BAT_CAPACITY, upmu_get_rgs_chrdet(), bat_data->BAT_STATUS);
	}
...
int force_get_tbat_internal(bool update)
{
	...
	if (update == true || pre_bat_temperature_val == -1) {
		...
		if (bat_temperature_volt != 0) {
			...
			bm_err("[force_get_tbat] nowT %d, preT %d \n", bat_temperature_val, pre_bat_temperature_val);
		...
		bat_temperature_val = pre_bat_temperature_val;
		  bm_err("[force_get_tbat] use pre_bat_temperature_val  %d \n", pre_bat_temperature_val);
	}
