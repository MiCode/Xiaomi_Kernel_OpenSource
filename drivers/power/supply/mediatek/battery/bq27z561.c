...
#include "../charger/mtk_charger_init.h"
...
static int fg_sha256_auth(struct bq_fg_chip *bq, u8 *rand_num, int length)
{
	...
	*/
	...
	/*4. Write the checksum (complement sum of (1), (2), and (3)) to address 0x60.*/
...
static int fg_read_rsoc(struct bq_fg_chip *bq)
{
	...
	bq_dbg(PR_OEM, "fg_read_rsoc, RSOC= %d\n", soc);
	if (ret < 0) {
...
extern int charger_get_vbat(void);
extern int charger_get_ichg(void);
...
static int fg_read_battID(struct bq_fg_chip *bq)
{
	...
	for(retry = 0 ; retry < I2C_RETRY_MAX; retry++){
		ret = fg_mac_read_block(bq, FG_MAC_CMD_CHEM_ID, t_buf, 4);
		if (ret < 0) {
...
static void fg_update_status(struct bq_fg_chip *bq)
{
	...
	bq_dbg(PR_OEM, "SOC:%d,Volt:%d,Cur:%d,Temp:%d,True_RM:%d,True_FCC:%d,FAST:%d,ST:%d,BATT_ID:%d",
			bq->batt_soc, bq->batt_volt, bq->batt_curr, bq->batt_temp,
			bq->batt_rm, bq->batt_fcc, bq->fast_mode, bq->batt_st,bq->chip_id);

	
	if ((bq->batt_soc <= 1) && (bq->batt_curr >= 0) && bq->batt_soc > last_soc) {
		bq->batt_soc = last_soc;
	}
...
static int fg_update_charge_full(struct bq_fg_chip *bq)
{
	...
	if (bq->batt_temp > BATT_WARM_THRESHOLD && bq->batt_curr >= CHG_DONE_LIMIT &&
	...
	if ((bq->charge_done || pmic_chg_done) && !battery_main.CHG_FULL_STATUS) {
		if (bq->batt_soc >= BQ_DEFUALT_FULL_SOC) {
			bq_dbg(PR_OEM, "Setting charge_full to true\n");
			bq->charge_full = true;
			bq->cell_ov_check = 0;
#if 0
			charger_dev_set_charging_current(bq->chg1_dev, 0);
	...
	if ((bq->raw_soc <= BQ_RECHARGE_SOC) && !bq->charge_done && !pmic_chg_done && battery_main.CHG_FULL_STATUS &&
		...
		charger_dev_set_charging_current(bq->chg1_dev, 3000000);
...
//	int optimiz_delta = 0;
	...
	bq_dbg(PR_DEBUG, "%s:bq->optimiz_soc:%d last_batt_soc:%d raw_soc:%d bq->ffc_smooth:%d batt_soc:%d",__func__, bq->optimiz_soc, last_batt_soc, raw_soc, bq->ffc_smooth, batt_soc);
	if (bq->optimiz_soc > 0) {
		bq->ffc_smooth = true;
		last_batt_soc = bq->optimiz_soc;
		system_soc = bq->optimiz_soc;
		last_change_time = ktime_get_boottime();
		bq->optimiz_soc = 0;
	}
	...
	if (!bq->usb_psy || !bq->batt_psy) {
		...
		if (!bq->usb_psy) {
			bq_dbg(PR_DEBUG, "%s:bq->usb_psy NULL!",__func__);
			return batt_soc;
		}
		bq->batt_psy = power_supply_get_by_name("battery");
		if (!bq->batt_psy) {
			bq_dbg(PR_DEBUG, "%s:bq->batt_psy NULL!",__func__);
			return batt_soc;
		}
	}
	...
	if (bq->ffc_smooth) {
		rc = power_supply_get_property(bq->batt_psy,
				POWER_SUPPLY_PROP_STATUS, &pval);
		if (rc < 0) {
			bq_dbg(PR_OEM, "%s:failed get batt staus\n",__func__);
			return -EINVAL;
		}
		status = pval.intval;
		if (batt_soc == system_soc) {
			bq->ffc_smooth = false;
			bq_dbg(PR_DEBUG, "%s:return soc %d directly!", __func__, batt_soc);
			return batt_soc;
		}
	...
	last_optimiz_time = ktime_get_boottime();
#endif
	...
	if (last_batt_soc < batt_soc && batt_ma < 0)
		...
		if(last_batt_soc - batt_soc >= 1) {
			if(batt_soc == 100 && change_delta > 180000){
				last_batt_soc--;
				last_change_time = ktime_get_boottime();
				bq->update_now = false;
			} else if(batt_soc >= 30 && batt_soc < 100 && change_delta > 30000) {
				last_batt_soc--;
				last_change_time = ktime_get_boottime();
				bq->update_now = false;
	...
	if(old_batt_ma == 0 && batt_ma == 0 && bq->batt_volt > 4350){
				...
				const struct i2c_device_id *id)
{
	...
	if ((id->driver_data == BQ27Z561_I2C0 && before_P11 == false) ||
		(id->driver_data == BQ27Z561_I2C9 && before_P11 == true)) {
	...
	*/
	...
	hardwareinfo_set_prop(HARDWARE_BMS_GAUGE_ID, "BQ27Z561");
	hardwareinfo_set_prop(HARDWARE_BATTERY_ID, battery_name[status]);
	return 0;
}
