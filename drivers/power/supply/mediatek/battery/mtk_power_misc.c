...
static int shutdown_event_handler(struct shutdown_controller *sdd)
{
	...
	if (sdd->shutdown_status.is_under_shutdown_voltage) {
		...
		bm_err("low tracking bound is %d and %d \n",
				BAT_VOLTAGE_LOW_BOUND, LOW_TMP_BAT_VOLTAGE_LOW_BOUND);
			...
			if (down_to_low_bat == 0) {
				if (IS_ENABLED(
					...
					if (tmp >= LOW_TEMP_THRESHOLD) {
						down_to_low_bat = 1;
						bm_err("normal tmp, battery voltage is low \n");
						notify_fg_shutdown();
					} else if (sdd->avgvbat <=
						LOW_TMP_BAT_VOLTAGE_LOW_BOUND) {
						down_to_low_bat = 1;
						bm_err("cold tmp, battery voltage is low \n");
						notify_fg_shutdown();
					} else
