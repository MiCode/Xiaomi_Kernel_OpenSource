	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_REAL_TYPE:
		if (mtk_chg->cti->pd_active != POWER_SUPPLY_PD_INACTIVE) {
			...
			switch (pval.intval) {
			case QC35_SDP:
				val->intval = POWER_SUPPLY_TYPE_USB;// SDP -> SDP
				break;
			case QC35_CDP:
				val->intval = POWER_SUPPLY_TYPE_USB_CDP;// CDP -> CDP
				break;
			case QC35_DCP:
			case QC35_OCP:
				val->intval = POWER_SUPPLY_TYPE_USB_DCP;// DCP -> DCP
				break;
			case QC35_HVDCP:
			case QC35_HVDCP_20:
			...
			case QC35_HVDCP_30_27:
				val->intval = POWER_SUPPLY_TYPE_USB_HVDCP_3;// SDP -> SDP
				break;
			case QC35_HVDCP_3_PLUS_18:
			case QC35_HVDCP_3_PLUS_27:
				val->intval = POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS;// SDP -> SDP
				break;
			case QC35_FLOAT:// float -> float
				val->intval = POWER_SUPPLY_TYPE_USB_FLOAT;// FLOAT -> FLOAT
				break;
			case QC35_NA:// NA -> unknown
			case QC35_UNKNOW:
				val->intval = POWER_SUPPLY_TYPE_UNKNOWN;
				break;
			default:
...
#if 0
	mtk_chg->bms_psy = power_supply_get_by_name("bms");
	if (!mtk_chg->bms_psy) {
		pr_err("%s: get bms power supply failed\n", __func__);
		return -EINVAL;
	}
#endif
