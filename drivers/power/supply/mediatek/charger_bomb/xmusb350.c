...
/*
	...
	if (ret)
		xm35_err("%s: soft reset failed!\n", __func__);
	else
		xm35_dbg("%s: soft reset success!\n", __func__);
...
static int xmusb350_intb_enable(struct xmusb350_charger *chip, bool control)
{
	...
	ret = xmusb350_write_byte(chip, XMUSB350_REG_INTB_EN, (u8)control);

	if (ret)
		xm35_err("%s: Int enable/disable[%d] failed!\n", __func__, control);
	else
		xm35_dbg("%s: Int enable/disable[%d] success!\n", __func__, control);
...
static int xmusb350_sleep_enable(struct xmusb350_charger *chip, bool control)
{
	...
	ret = xmusb350_write_byte(chip, XMUSB350_REG_SLEEP_EN, (u8)control);

	if (ret)
		xm35_err("%s: sleep enable/disable[%d] failed!\n", __func__, control);
	else
		xm35_dbg("%s: sleep enable/disable[%d] success!\n", __func__, control);
...
void xmusb350_hard_reset(struct xmusb350_charger *chip)
{
	...
	mdelay(10);// 10us, at least
	gpio_set_value(chip->xmusb350_rst_gpio, 0);// OUT: 0
	mdelay(60);// 5.3ms, at least
	// after reset, 10s will sleep
}

#define HVDCP_RETRY_TIMER			3
#define HVDCP_DET_RETRY_DELAY		10000	/* 10 sec */
	...
	if (ret) {
		...
		if (type == QC35_HVDCP) {
			if (chip->hvdcp_retry_timer++ >= HVDCP_RETRY_TIMER) {
				...
				chip->chg_type = STANDARD_CHARGER;// DCP -> DCP
...
static void xmusb350_charger_type_det_work(struct work_struct *work)
{
	...
	enum usbsw_state usbsw;
	/*
	 * if otg plug in happened during charger type det work
	 * return
	 */
	if (chip->otg_enable)
		return;
	...
	if (ret) {
		...
		//workaround, change the charging type between ocp and float
	...
	if (type == QC35_HVDCP) {
		...
		if (type == QC35_HVDCP_30) {
			...
			if (ret) {
				...
				if (ret) {
					...
					msleep(300);
					/*
					 * if otg plug in happened during qc3 class-a and class-b detection
					 * return
					 */
					if (chip->otg_enable)
						return;
		...
		if (type != chip->qc35_chg_type) {
			...
			case QC35_SDP:
				chip->chg_type = STANDARD_HOST;// SDP -> SDP
				break;
			case QC35_CDP:
				chip->chg_type = CHARGING_HOST;// CDP -> CDP
				break;
			case QC35_OCP:// ocp -> float
			case QC35_DCP:
				chip->chg_type = STANDARD_CHARGER;// DCP -> DCP
				chip->mt_chg->usb_desc.type = POWER_SUPPLY_TYPE_USB_DCP;
				break;
			...
			case QC35_HVDCP_20:
				...
				chip->chg_type = STANDARD_CHARGER;// SDP -> SDP
				break;
			case QC35_HVDCP_30:
			...
			case QC35_HVDCP_3_PLUS_27:
				...
				break;
			case QC35_FLOAT:// float -> float
				chip->chg_type = NONSTANDARD_CHARGER;
				break;
			case QC35_NA:// NA -> float
			case QC35_UNKNOW:// unknown -> float
...
static int xmusb350_psy_chg_type_changed(struct xmusb350_charger *chip, bool force_update)
{
	...
	/* if otg enabled, do not set charger type unkown to mtk_det_chg driver */
	if (!chip->otg_enable) {
...
static int xmusb350_enable_chg_type_det(struct charger_device *chg_dev, bool en)
{
	...
	if (!en) {
		chip->chg_type = CHARGER_UNKNOWN;
		//xmusb350_psy_chg_type_changed(chip, true);
		//xmusb350_set_usbsw_state(chip, USBSW_USB);
		goto out;
	}
	...
	//chip->irq_data_process_enable = true;
	...
	case QC35_DCP:
		...
		break;
	case QC35_HVDCP:
		/* enable hvdcp put into interrupt */
		/*msleep(500);
		ret = xmusb350_hvdcp_enable(chip, ENABLE);
		if (ret)
			xm35_err("%s: hvdcp enable failed!\n", __func__);*/
...
static irqreturn_t xmusb350_interrupt(int irq, void *dev_id)
{
	...
	/*
	 * if connect to a SDP Type-C pc port which support pd
	 * should go on bc12 and go on usb enumeration process
	 * if other bc12 types and support pd, return right now
	 */
	if ((chip->mt_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_PD)
		&& (type != QC35_SDP)) {
...
int xmusb350_i2c_master_send(const struct i2c_client *client, const char *buf, int count)
{
	...
	//xm35_info("%s: count is %d.\n", __func__, count);
	//xm35_info("%s: data is %02x %02x %02x %02x.\n", __func__, buf[0], buf[1], buf[2], buf[3]);
...
int xmusb350_i2c_get_chip_id(const struct i2c_client *client, const char *buf)
{
	...
	//xm35_info("%s: xmusb350_i2c_get_chip_id.\n", __func__);
...
static int xmusb350_i2c_sequence_send(const struct i2c_client *client, const char *buf, int count)
{
	...
	//xm35_info("%s: count is %d.\n", __func__, count);
	//xm35_info("%s: addr is %02x, data is %02x.\n", __func__, msg.addr, msg.buf[0]);
...
struct device_attribute *attr, char *buf)
{
	...
	gpio_set_value(chip->xmusb350_rst_gpio, 1);// OUT: 1
	mdelay(15);// 10us, at least
	gpio_set_value(chip->xmusb350_rst_gpio, 0);// OUT: 0
	mdelay(1);// 5.3ms, at least
	// after reset, 10s will sleep

	// step 2: sequence
	xmusb350_i2c_sequence_send(chip->client, i2c_buf, 1);
	mdelay(10);// 10ms, at least
	// step 3: enable cmd send
...
#if 0
static int check_firmware_format(struct device *dev,
...
#if 0
static int StringToHex(const u8 *str, int str_len, u8 *out, size_t *outlen)
{
	const u8 *p = str;
	char high = 0, low = 0;
	int tmplen = str_len, cnt = 0;
...
#define RETRY_ENTER_ISP_TIMES			3
static int retry_enter_isp_count = 0;
static int load_fw(struct device *dev, const char *fn, bool force)
{
	...
	u8 *file_data;
	//u8 *fw_after;
	//size_t fw_after_length = 0;
	char flash_addr = 0;

	if (in_interrupt()) {
		file_data = kmalloc(FIRMWARE_FILE_LENGTH, GFP_ATOMIC);
		//fw_after = kmalloc(FIRMWARE_FILE_LENGTH / 2, GFP_ATOMIC);
	} else {
		file_data = kmalloc(FIRMWARE_FILE_LENGTH, GFP_KERNEL);
		//fw_after = kmalloc(FIRMWARE_FILE_LENGTH / 2, GFP_KERNEL);
	}
	memset(file_data, 0, FIRMWARE_FILE_LENGTH);
	//memset(fw_after, 0, FIRMWARE_FILE_LENGTH / 2);
	...
	rc = check_firmware_format(dev, fw);
	if (rc) {
		rc = ERROR_CHECK_FIREWARE_FORMAT;
		goto release_firmware;
	}
#endif
...
#if 0
	StringToHex(fw->data, fw->size, fw_after, &fw_after_length);
// debug
	for (i = 0; i < fw_after_length; i++) {
		dev_dbg(dev, "fw_after[%d] = %02x\n", i, *(fw_after + i));
	}
// debug
#endif
...
RETRY_ENTER_ISP:
	...
	// these codes can print all flash datas before update
		...
		// no need end cmd, hard reset is all right.
		//xmusb350_i2c_master_send(chip->client, end_cmd, 3);
		//rc = xmusb350_i2c_master_send(chip->client, end_cmd, 3);
		//if (rc != 3) {
		//	dev_err(chip->dev, "%s: end_cmd is failed, rc=%d\n", __func__, rc);
		//	goto update_failed;
		//}
		rc = UPDATE_SUCCESS;
	} else {
		dev_err(chip->dev, "%s: retry_enter_isp_count = %d.\n", __func__, retry_enter_isp_count);
		if (retry_enter_isp_count > RETRY_ENTER_ISP_TIMES) {
			dev_err(chip->dev, "%s: chip id is not right, and end update.\n", __func__);
			rc = ERROR_GET_CHIPID_FAILED;
			goto update_failed;
		} else {
			retry_enter_isp_count++;
			goto RETRY_ENTER_ISP;
		}
	}
...
static void xmusb350_hard_reset_work(struct work_struct *work)
{
	...
	/* Get an interrupt when restarting */
	xmusb350_hard_reset(chip);

	/* version can not get, maybe there is a bad firmware. so, need update */
	/* version is not right, and need update. */
	...
	if (chip->bms_psy) {
		...
		if (!rc) {
			if (val.intval > 10) {
				...
				// check if there is a good firmware 0x584d3335
				rc = xmusb350_get_vendor_id(chip, buf);
			...
			const struct i2c_device_id *id)
{
	...
	//mutex_init(&chip->i2c_rw_lock);// xiaomi
	...
	*/
	...
	// maybe change to get fw and sw version and log it
	rc = xmusb350_get_vendor_id(chip, buf);
	if (rc != 2) {
...
//sysfs_remove_groups(&chip->dev->kobj, &(&xmusb350_attr_group));
err_charger_device:
