...
extern void set_fake_plug_out(bool value);
...
int charger_manager_get_prop_system_brightness(void)
{
	int screen_brightness = SCREEN_OFF;

	screen_brightness = mt_leds_brightness_get();
	if (screen_brightness < SCREEN_OFF)
		screen_brightness = SCREEN_OFF;
	else if (screen_brightness > SCREEN_FULL)
		screen_brightness = SCREEN_FULL;

	return screen_brightness;
}

int charger_manager_get_prop_system_temp_scene(void)
{
	int thermal_scene = NORMAL_CHG_SCENE;
	int screen_brightness = SCREEN_OFF;

	thermal_scene = get_thermal_charge_ctrl_scene();
	screen_brightness = charger_manager_get_prop_system_brightness();

	if (thermal_scene < NORMAL_CHG_SCENE || thermal_scene > OTHER_CHG_SCENE)
...
int charger_manager_set_current_limit(int data, int type)
{
	...
	union power_supply_propval val = {0,};
	int i = 0, batt_cycle_cnt = 0, rc = 0;

	if (pinfo == NULL || pinfo->battery_psy == NULL || pinfo->bms_psy == NULL)
		return false;
	...
	if (pinfo->data.enable_cv_aging) {
		rc = power_supply_get_property(pinfo->bms_psy,
				POWER_SUPPLY_PROP_CYCLE_COUNT, &val);
		if (rc < 0) {
			pr_info("Couldn't get batt cycle count, rc:%d, use default cycle cnt.\n", rc);
			batt_cycle_cnt = 0;
		} else
			batt_cycle_cnt = val.intval;

		if (batt_cycle_cnt > STEP_CYCLECNT_THRESHOLD)
			effective_fcc = pinfo->data.current_max_aging;
	}
...
int chg_set_fastcharge_mode(bool enable)
{
	...
	/*if night_charging do not set fastcharge flag*/
	...
	if (enable) {
		...
		pval.intval = 4480000;//pinfo->data.ffc_cv;
...
void charger_manager_set_prop_system_temp_level(int temp_level)
{
	...
	pr_info("%s, thermal[scene:%d level:%d icl:%d fcc:%d], usb_type:%d\n",
		...
		if (pd_authentication || charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS) {
			...
			if ((info->battery_temp > info->data.temp_t3_thres || info->battery_temp <= info->data.temp_t2_thres) && fastcharge_mode) {
				...
				if (ret < 0) {
					...
					!fastcharge_mode) {
				/* battery_temp < 47 || battery_temp > 17 recover*/
...
{
	static int batt_vol, step_fcc;
	int rc = 0;
	int batt_cycle_cnt = 0;
	int step_a_cv = STEP_A, step_b_cv = STEP_B, step_c_cv = STEP_C;
	int current_step = TAPER_STEP_MA;
	union power_supply_propval val = {0,};
	...
	rc = power_supply_get_property(info->bms_psy,
			POWER_SUPPLY_PROP_CYCLE_COUNT, &val);
	if (rc < 0) {
		pr_info("Couldn't get batt cycle count:%d\n", rc);
		return rc;
	}
	batt_cycle_cnt = val.intval;
	...
	rc = power_supply_get_property(info->battery_psy,
			POWER_SUPPLY_PROP_FAST_CHARGE_CURRENT, &val);
	if (rc < 0) {
		pr_info("Couldn't get batt fcc:%d\n", rc);
		return rc;
	}
	step_fcc = val.intval;

	pr_info("step_chg: vbat=%d, batt_cyclecnt=%d, fcc=%d.\n",
			batt_vol, batt_cycle_cnt, step_fcc);

	/* select step cv acc.to batt aging state */
	if (info->data.enable_cv_aging && batt_cycle_cnt > STEP_CYCLECNT_THRESHOLD) {
		step_a_cv = info->data.step_a_aging;
		step_b_cv = info->data.step_b_aging;
		step_c_cv = info->data.step_c_aging;
		current_step = info->data.current_step_aging;
	} else {
		step_a_cv = info->data.step_a;
		step_b_cv = info->data.step_b;
		step_c_cv = info->data.step_c;
	}

	if (info->data.enable_cv_step) {//cv_step
		if (batt_vol > (step_a_cv - TAPER_CV_HY) && batt_vol <= (step_b_cv - TAPER_CV_HY)
				&& step_fcc >= info->data.current_a) {
			step_fcc -= current_step;
		...
		} else if (batt_vol > (step_b_cv - TAPER_CV_HY) && batt_vol <= (step_c_cv - TAPER_CV_HY)
				&& step_fcc >= info->data.current_b) {
			step_fcc -= current_step;
		...
		} else if (batt_vol > (step_c_cv - TAPER_CV_HY)
				&& step_fcc >= info->data.current_c) {
			step_fcc -= current_step;
		...
		if (batt_vol > step_a_cv && batt_vol <= step_b_cv && info->step_flag != STEP_A_TR) {
		...
		} else if (batt_vol > step_b_cv && batt_vol <= step_c_cv && info->step_flag != STEP_B_TR) {
		...
		} else if (batt_vol > step_c_cv && info->step_flag != STEP_C_TR) {
			charger_manager_set_current_limit(info->data.current_c, STEPCHG_FCC);
			info->step_flag = STEP_C_TR;
			pr_info("step_chg STEP_C_TR vbatt = %d , set fcc %d\n", batt_vol, info->data.current_c);
		}
	}
	...
	case STEP_NORMAL:
		...
		break;
	case STEP_A_TR:
		if (batt_vol < step_a_cv - info->data.step_hy_down_a) {
		...
		break;
	case STEP_B_TR:
		if (batt_vol < step_b_cv - info->data.step_hy_down_b) {
			info->step_flag = STEP_A_TR;
			charger_manager_set_current_limit(info->data.current_a, STEPCHG_FCC);
		...
		break;
	case STEP_C_TR:
		if (batt_vol < step_c_cv - info->data.step_hy_down_c) {
			info->step_flag = STEP_B_TR;
			charger_manager_set_current_limit(info->data.current_b, STEPCHG_FCC);
				...
				struct device *dev)
{
	...
	if (info->data.enable_vote) {
		...
		info->data.enable_cv_aging = of_property_read_bool(np, "enable_cv_aging");
		...
		if (info->data.enable_cv_aging) {
			if (of_property_read_u32(np, "step_a_aging", &val) >= 0)
				info->data.step_a_aging = val;
			else {
				chr_err("use default STEP_A_AGING:%d\n", STEP_A);
				info->data.step_a_aging = STEP_A;
			}

			if (of_property_read_u32(np, "step_b_aging", &val) >= 0)
				info->data.step_b_aging = val;
			else {
				chr_err("use default STEP_B_AGING:%d\n", STEP_B);
				info->data.step_b_aging = STEP_B;
			}

			if (of_property_read_u32(np, "step_c_aging", &val) >= 0)
				info->data.step_c_aging = val;
			else {
				chr_err("use default STEP_C_AGING:%d\n", STEP_C);
				info->data.step_c_aging = STEP_C;
			}

			if (of_property_read_u32(np, "current_step_aging", &val) >= 0)
				info->data.current_step_aging = val;
			else {
				chr_err("use default CURRENT_STEP_AGING:%d\n", TAPER_STEP_MA);
				info->data.current_step_aging = TAPER_STEP_MA;
			}

			if (of_property_read_u32(np, "current_max_aging", &val) >= 0)
				info->data.current_max_aging = val;
			else {
				chr_err("use default CURRENT_MAX_AGING:%d\n", CURRENT_MAX);
				info->data.current_max_aging = CURRENT_MAX;
			}
		}
	}
...
static int screen_brightness_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", charger_manager_get_prop_system_brightness());
	return 0;
}

static int screen_brightness_open(struct inode *inode, struct file *file)
{
	return single_open(file, screen_brightness_proc_show, NULL);
}

static const struct file_operations screen_brightness_proc_fops = {
	.open = screen_brightness_open,
	.read = seq_read,
	.llseek	= seq_lseek,
};
...
void charger_debug_init(void)
{
	...
	proc_create("screen_brightness", 0440,
		charger_dir, &screen_brightness_proc_fops);
}
