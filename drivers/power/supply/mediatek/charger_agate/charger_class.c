...
int charger_dev_get_hiz_mode(struct charger_device *charger_dev,
				       int *status)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
				       charger_dev->ops->get_hiz_mode)
		return charger_dev->ops->get_hiz_mode(charger_dev, status);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_hiz_mode);

int charger_dev_set_hiz_mode(struct charger_device *charger_dev,
				       bool en)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
				       charger_dev->ops->set_hiz_mode)
		return charger_dev->ops->set_hiz_mode(charger_dev, en);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_hiz_mode);

int charger_dev_set_recharge_vbat(struct charger_device *charger_dev,
				       int vol)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
				       charger_dev->ops->set_recharge_vbat)
		return charger_dev->ops->set_recharge_vbat(charger_dev, vol);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_recharge_vbat);
