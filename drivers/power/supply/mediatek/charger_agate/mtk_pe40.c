...
extern struct charger_manager *votable_info;
...
static bool is_psy_voter_available(struct charger_manager *info)
{
	if (!mtk_batt_create_votable) {
		pr_err("mtk_pe40 mtk_batt_create_votable\n");
		return false;
	}

	if (mtk_batt_destroy_votable) {
		pr_err("mtk_pe40 mtk_batt_destroy_votable\n");
		return false;
	}
	...
	if (!info->fcc_main_votable) {
		...
		if (!info->fcc_main_votable) {
			pr_err("mtk_pe40 Couldn't find FCC_MAIN voltable\n");
			return false;
		}
	}
	...
	if (!info->fv_votable) {
		...
		if (!info->fv_votable) {
			pr_err("mtk_pe40 Couldn't find FV voltable\n");
			return false;
		}
	}
	...
	if (!info->usb_icl_votable) {
		...
		if (!info->usb_icl_votable) {
			pr_err("mtk_pe40 Couldn't find USB_ICL voltable\n");
			return false;
		}
	}
...
int pe40_pd_1st_request(int adapter_mv, int adapter_ma, int ma)
{
	...
	if (!is_psy_voter_available(votable_info))
		return -EAGAIN;
	...
	if (oldmA > ma) {
		...
		vote(votable_info->usb_icl_votable, USBIN_PE40_1ST_REQUEST_VOTER,
				true, ma * 1000);
	}
...
int pe40_pd_request(int *adapter_vbus, int *adapter_ibus, int ma)
{
	...
	if (oldmA > ma) {
		//charger_set_input_current(ma * 1000);
		vote(votable_info->usb_icl_votable, USBIN_PE40_PD_REQUEST_VOTER,
				true, ma * 1000);
	}
...
int pe40_set_current(void)
{
	...
	vote(votable_info->usb_icl_votable, USBIN_PE40_SETCURRENT_VOTER,
				true, pe4->data.input_current_limit);
	//charger_set_charging_current(pe4->data.charging_current_limit);
	vote(votable_info->fcc_main_votable, FCCMAIN_PE40_SETCURRENT_VOTER, true,
				pe4->data.charging_current_limit);
...
int pe40_set_cv(void)
{
	...
	vote(votable_info->fv_votable, FV_PE40_SET_CV_VOTER, true,
						pe4->data.battery_cv);
