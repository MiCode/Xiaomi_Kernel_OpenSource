...
static int ops_xmusb350_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	if (en) {
		...
		chip->usb_switch = USBSW_XMUSB350;
		if(product_name == PISSARRO && chip->thermal_status != TEMP_ABOVE_RANGE)
...
static void xmusb_parse_cmdline(void)
{
	...
	xm35_info("pissarro = %d, pissarropro = %d, pissarroinpro = %d\n", pissarro ? 1 : 0, pissarropro ? 1 : 0, pissarroinpro ? 1 : 0);
			...
			const struct i2c_device_id *id)
{
	...
	wakeup_source_init(&chip->irq_handle_wakelock, "xmusb350_irq_handle_wakelock");
	wakeup_source_init(&chip->check_hvdcp3_wakelock, "xmusb350_check_hvdcp3_wakelock");
	wakeup_source_init(&chip->check_usb_ready_wakelock, "xmusb350_check_usb_ready_wakelock");
	wakeup_source_init(&chip->update_fw_wakelock, "update_fw_wakelock");
	wakeup_source_init(&chip->vbus_wave_wa_wakelock, "vbus_wave_wa_wakelock");
	wakeup_source_init(&chip->slow_pd_wa_wakelock, "slow_pd_wa_wakelock");
...
static int xmusb350_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct xmusb350_charger *chip = i2c_get_clientdata(client);
	if(product_name == PISSARRO && chip->vbus_disable)
		schedule_delayed_work(&chip->conn_therm_work,
					msecs_to_jiffies(THERM_REG_RECHECK_DELAY_200MS));
