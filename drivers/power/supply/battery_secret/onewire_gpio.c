...
#define ow_err(_fmt, ...) \
	{ \
		if(!charger_ipc_log_context){   \
			...
			ipc_log_string(charger_ipc_log_context, "onewire: %s %d "_fmt, __func__, __LINE__, ##__VA_ARGS__); \
		}\
	}
#define ow_log(_fmt, ...) \
	{ \
...
#define GPIO_ENABLE			(0x1 << 12)  // add hao
#define GPIO_OUTPUT				(0x1 << 9)
#define GPIO_INPUT				(0x0 << 9)
#define GPIO_PULL_UP			0x3
#define OUTPUT_HIGH				0x2 //(0x1 << 1)
#define OUTPUT_LOW				0x0 //0x1

#define ONE_WIRE_CONFIG_OUT		writel_relaxed(GPIO_ENABLE | DRV_STRENGTH_16MA | GPIO_OUTPUT | GPIO_PULL_UP, g_onewire_data->gpio_cfg_reg)// OUT
#define ONE_WIRE_CONFIG_IN		writel_relaxed(GPIO_ENABLE | DRV_STRENGTH_16MA | GPIO_INPUT | GPIO_PULL_UP, g_onewire_data->gpio_cfg_reg)// IN
...
static int onewire_gpio_detected = false;
...
void Software_Reset(void)
{
	...
	if (step > 2 || IS_ERR_OR_NULL(g_onewire_data)) {
		return;
	}
	ONE_WIRE_CONFIG_OUT;
	ONE_WIRE_OUT_LOW;
...
unsigned char ow_reset(void)
{
	...
	if (IS_ERR_OR_NULL(g_onewire_data)) {
		return -ENODEV;
	}
...
static int onewire_gpio_probe(struct platform_device *pdev)
{
	...
	raw_spin_lock_init(&onewire_data->lock);
	...
	ow_log("onewire_gpio_level_addr is %x; onewire_gpio_cfg_addr is %x", (uint64_t)(onewire_data->onewire_gpio_level_addr), (uint64_t)(onewire_data->onewire_gpio_cfg_addr));
	ow_log("onewire_data->gpio_cfg_reg is %x; onewire_data->gpio_in_out_reg is %x", (uint64_t)(onewire_data->gpio_cfg_reg), (uint64_t)(onewire_data->gpio_in_out_reg));
...
static int __init onewire_gpio_init(void)
{
	...
	ow_err(" probe successfully, onewire_gpio_detected = %d .\n", onewire_gpio_detected);
	return retval;
