		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BQ_CHARGE_DONE:
		val->intval = info->bq_charge_done;
		break;
	case POWER_SUPPLY_PROP_HV_CHARGE_ENABLED:
		val->intval = info->hv_charge_enable;
		break;
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TI_SET_BUS_PROTECTION_FOR_QC3:
		...
		break;
	case POWER_SUPPLY_PROP_BQ_CHARGE_DONE:
		info->bq_charge_done = val->intval;
		break;
	case POWER_SUPPLY_PROP_HV_CHARGE_ENABLED:
		info->hv_charge_enable = val->intval;
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
...
static int ln8000_set_bq_chg_done(struct charger_device *chg_dev, int enable)
{
	struct ln8000_info *info = charger_get_data(chg_dev);

	info->bq_charge_done = enable;
	return 0;
}

static int ln8000_get_bq_chg_done(struct charger_device *chg_dev, int *bq_chg_done)
{
	struct ln8000_info *info = charger_get_data(chg_dev);

	*bq_chg_done = info->bq_charge_done;
	return 0;
}

static int ln8000_set_hv_charge_enable(struct charger_device *chg_dev, int enable)
{
	struct ln8000_info *info = charger_get_data(chg_dev);

	info->hv_charge_enable = enable;
	return 0;
}

static int ln8000_get_hv_charge_enable(struct charger_device *chg_dev, int *hv_chg_enable)
{
	struct ln8000_info *info = charger_get_data(chg_dev);

	*hv_chg_enable = info->hv_charge_enable;
	return 0;
}

static int ln8000_set_present(struct charger_device *chg_dev, bool present)
{
	struct ln8000_info *info = charger_get_data(chg_dev);
	info->usb_present = present;
...
static int ln8000_get_present(struct charger_device *chg_dev, bool *present)
{
	struct ln8000_info *info = charger_get_data(chg_dev);

	*present = info->usb_present;

	return 0;
}

static int ln8000_set_bus_protection(struct charger_device *chg_dev, int hvdcp3_type)
{
	struct ln8000_info *info = charger_get_data(chg_dev);
...
static const struct charger_ops ln8000_chg_ops = {
	...
	.set_bus_protection = ln8000_set_bus_protection,
	.set_present = ln8000_set_present,
	.get_present = ln8000_get_present,
	.set_bq_chg_done = ln8000_set_bq_chg_done,
	.get_bq_chg_done = ln8000_get_bq_chg_done,
	.set_hv_charge_enable = ln8000_set_hv_charge_enable,
	.get_hv_charge_enable = ln8000_get_hv_charge_enable,
	//.set_chg_mode = sc8551_set_charge_mode,
	//.get_chg_mode = sc8551_get_charge_mode,

	.get_battery_present = ln8000_get_battery_present,
	.get_vbus_present = ln8000_get_vbus_present,

	.get_vbat_adc = ln8000_get_vbat_volt,
	.get_ibat_adc = ln8000_get_ibat_curr,
	.get_vbus_adc = ln8000_get_vbus_volt,
	.get_ibus_adc = ln8000_get_ibus_curr,
	.get_battery_temp = ln8000_get_battery_temp,
	//.get_bus_temp = bq2597x_get_bus_temp,
	//.get_die_temp = bq2597x_get_die_temp,

	.get_alarm_status = ln8000_get_alarm_status,
	.get_fault_status = ln8000_get_fault_status,
...
static int ln8000_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	info->chg_dev = charger_device_register(chg_name, info->dev,
			info, &ln8000_chg_ops, &info->chg_props);
	if (IS_ERR_OR_NULL(info->chg_dev)) {
		ln_err("ln8000 charger device register fail\n");
