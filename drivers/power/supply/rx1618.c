	...
	switch (privite_cmd) {
		...
		case 0x09:
			if (g_id_done_flag == 0)
			{
				...
				rx1618_write(chip, PRIVATE_ID_CMD, REG_RX_SENT_CMD);
				rx1618_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
				g_id_done_flag = 1;
		...
		case 0x0A:
			if (g_sar_done_flag == 0)
			{
				...
				rx1618_write(chip, PRIVATE_USB_TYPE_CMD, REG_RX_SENT_CMD);
				rx1618_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
				g_sar_done_flag = 1;
		...
		case 0x12:
			if (g_fast_chg_flag == 0)
			{
				...
				rx1618_write(chip, PRIVATE_TX_HW_ID_CMD, REG_RX_SENT_CMD);
				rx1618_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
		...
		case 0x0B:
			if (g_usb_type_flag == 0)
			{
				...
				rx1618_write(chip, PRIVATE_FAST_CHG_CMD, REG_RX_SENT_CMD);
				rx1618_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
				g_usb_type_flag = 1;
		...
		case 0x0D:
			...
			if (tx_cmd==0x12)
			{
				rx1618_write(chip, 0x38, REG_RX_SENT_DATA1);
			...
			{
				rx1618_write(chip, 0x28, REG_RX_SENT_DATA1);
			...
			rx1618_write(chip, PRIVATE_PRODUCT_TEST_CMD, REG_RX_SENT_CMD);
			rx1618_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
			break;
