...
//add ipc log end
//add cycle count
static unsigned int cv_step_8mv_config[] = {
	4448,
	4440,
	4432,
	4424,
...
static unsigned int cv_step_16mv_config[] = {
	4448,
	4432,
	4432,
	4416,
};
#define SC89890_NUM 1
#define SYV6970D_NUM 2
...
static int nopmi_chg_jeita_get_bms_cycle_count(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	union power_supply_propval prop = {0, };
	int ret = 0;
	int cycle_count = 0;
	...
	cycle_count = prop.intval;
	pr_info("sw_jeita:get_bms_cycle_count is %d\n", cycle_count);

	return cycle_count;
}

static int nopmi_chg_jeita_get_bbc_manufacturer(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	union power_supply_propval prop = {0, };
	int ret = 0;
	int part_num = 0;
	...
	ret = power_supply_get_property(nopmi_chg_jeita->bbc_psy,
				POWER_SUPPLY_PROP_MANUFACTURER, &prop);
	if (ret < 0) {
		pr_err("couldn't read temperature property, ret=%d\n", ret);
		return -EINVAL;
	}
	if(strcmp(prop.strval, "Silergy") == 0)
		part_num = SYV6970D_NUM;
	else if(strcmp(prop.strval, "SOUTHCHIP") == 0)
		part_num = SC89890_NUM;

	pr_info("sw_jeita:get_bbc_manufacturer is %s, part_num = %d\n", prop.strval, part_num);
...
static void nopmi_chg_handle_jeita_current(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
   ...
   // union power_supply_propval pval = {0, };
	...
	//add cycle_count
	if (sw_jeita->sm != TEMP_T3_TO_T4 && sw_jeita->sm != TEMP_ABOVE_T4) {
		part_num = nopmi_chg_jeita_get_bbc_manufacturer(nopmi_chg_jeita);
		cycle_count = nopmi_chg_jeita_get_bms_cycle_count(nopmi_chg_jeita);
		if(cycle_count < 100){
			sw_jeita->cv = cv_step_8mv_config[0];
		}else if(cycle_count < 200){
			if(part_num == SYV6970D_NUM)
				sw_jeita->cv = cv_step_16mv_config[1];
			else
				sw_jeita->cv = cv_step_8mv_config[1];
		}else if(cycle_count < 300){
			sw_jeita->cv = cv_step_8mv_config[2];
		}else{
			if(part_num == SYV6970D_NUM)
				sw_jeita->cv = cv_step_16mv_config[3];
			else
				sw_jeita->cv = cv_step_8mv_config[3];
		}
		pr_info("sw_jeita: set cv to %d, cycle_count:%d\n", sw_jeita->cv, cycle_count);
	}
	...
	if(NOPMI_CHARGER_IC_MAXIM != nopmi_get_charger_ic_type()){
		...
		pr_info("[SW_JEITA]preState:%d newState:%d temp:%d cv:%d,%d jeita_current_limit:%d, term_curr:%d,%d, batt_id:%d\n",
				sw_jeita->pre_sm, sw_jeita->sm, nopmi_chg_jeita->battery_temp,
				sw_jeita->cv, chg1_cv, jeita_current_limit, sw_jeita->term_curr,
				term_curr_pre, nopmi_chg_jeita->battery_id);
	}else{
