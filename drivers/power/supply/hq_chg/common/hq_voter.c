...
int vote(struct votable *votable, const char *client_str, bool enabled, int val)
{
	...
	if ((votable->votes[client_id].enabled == enabled) &&
		...
		hq_debug("%s: %s,%d same vote %s of val=%d\n",
				votable->name,
	...
	if (!votable->voted_on || (effective_result != votable->effective_result)) {
		...
		hq_info("%s: effective vote %s, val = %d, override_result = %d\n", votable->name, get_client_str(votable, votable->effective_client_id),
			votable->effective_result, votable->override_result);
	}
		  ...
		  bool enabled, int val)
{
	...
	lock_votable(votable);
	hq_info("%s: current overrivote enabled:%d val:%d voted by vote %s\n", votable->name, enabled, val, override_client);
	if (votable->force_active) {
	...
	if (enabled) {
		...
		hq_info("%s: current vote is now %d voted by vote_override %s\n",
				...
				void *data)
{
	...
	if (debug_root == NULL) {
		...
		if (!debug_root) {
			hq_err("Couldn't create debug dir\n");
			return ERR_PTR(-ENOMEM);
		}
	}

	if (votable_type >= NUM_VOTABLE_TYPES) {
		hq_err("Invalid votable_type specified for voter\n");
		return ERR_PTR(-EINVAL);
	}
	...
	if (!votable->root) {
		hq_err("Couldn't create debug dir %s\n", name);
		kfree(votable->name);
		kfree(votable);
		return ERR_PTR(-ENOMEM);
	}
