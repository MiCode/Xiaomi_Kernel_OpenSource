 ...
 */
#ifndef __LINUX_XIAOMI_ADAPTER_CLASS_H__
#define __LINUX_XIAOMI_ADAPTER_CLASS_H__
...
#define XM_ADAPTER_CAP_MAX_NR 10

struct adapter_power_cap {
	uint8_t selected_cap_idx;
	uint8_t nr;
	uint8_t pdp;
	uint8_t pwr_limit[XM_ADAPTER_CAP_MAX_NR];
	int max_mv[XM_ADAPTER_CAP_MAX_NR];
	int min_mv[XM_ADAPTER_CAP_MAX_NR];
	int ma[XM_ADAPTER_CAP_MAX_NR];
	int maxwatt[XM_ADAPTER_CAP_MAX_NR];
	int minwatt[XM_ADAPTER_CAP_MAX_NR];
	uint8_t type[XM_ADAPTER_CAP_MAX_NR];
	int info[XM_ADAPTER_CAP_MAX_NR];
};
...
struct xm_adapter_ops {
	...
	int (*get_usbpd_verifed)(struct adapter_device *dev, bool *verifed);
};
...
extern struct adapter_device *adapter_device_register(
	const char *name, struct device *parent, void *devdata, const struct xm_adapter_ops *ops,
...
extern int adapter_dev_get_cap(struct adapter_device *adapter_dev,
	enum xm_adapter_cap_type type, struct adapter_power_cap *cap);
...
#endif /*__LINUX_XIAOMI_ADAPTER_CLASS_H__*/
