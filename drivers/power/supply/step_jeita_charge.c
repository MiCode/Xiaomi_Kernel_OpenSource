...
static int handle_ffc_charge(struct mtk_charger *info)
{
	...
	if ((!info->recharge && info->entry_soc <= info->ffc_high_soc && is_between(info->ffc_low_tbat, info->ffc_high_tbat - 1, info->temp_now) &&
	...
	if (is_between(info->ffc_medium_tbat, info->ffc_high_tbat, info->temp_now))
		iterm_ffc = min(info->iterm_ffc_warm, 300);
	else
		iterm_ffc = min(info->iterm_ffc, 300);
...
static void check_full_recharge(struct mtk_charger *info)
{
	...
	if (info->charge_full) {
		...
		if ((info->soc == 100 && (info->vbat_now >= fv_effective - threshold_mv) && (((-info->current_now <= iterm) && info->fg_full) || ((-info->current_now <= iterm_effective + 100) && info->bbc_charge_done)))
                    || ((info->vbat_now >= fv_effective - 30) && (-info->current_now <= iterm) && (info->temp_now >= 480)))
			full_count++;
		else
			full_count = 0;
...
if (!info->cp_master) {
	...
	chr_err("[CHARGE_LOOP] TYPE = [%d %d %d %d], BMS = [%d %d %d %d], FULL = [%d %d %d %d %d], thermal_level=%d, FFC = %d, sw_cv=%d, gauge_authentic=%d\n",
		info->pd_type, info->pd_adapter->verifed, info->real_type, info->qc3_type, info->soc, info->vbat_now, info->current_now, info->temp_now,
		info->bbc_charge_enable, info->charge_full, info->fg_full, info->bbc_charge_done, info->recharge, info->thermal_level, info->ffc_enable, info->sw_cv, info->gauge_authentic);
