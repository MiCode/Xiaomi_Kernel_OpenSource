...
#define FG_OPTIMIZ_FULL_TIME	64
...
static int fg_update_charge_full(struct bq_fg_chip *bq)
{
	...
	if (bq->charge_done && !bq->charge_full) {
		if (bq->raw_soc >= BQ_REPORT_FULL_SOC && bq->health == POWER_SUPPLY_HEALTH_GOOD) {
	...
	if ((bq->raw_soc <= BQ_RECHARGE_SOC) && bq->charge_done && bq->health == POWER_SUPPLY_HEALTH_GOOD) {
		prop.intval = true;
...
{
	static int last_batt_soc = -1, ffc_smooth, system_soc, cold_smooth;
	static int last_status;
	int change_delta = 0, rc;
	int optimiz_delta = 0, status;
	static struct timeval last_change_time;
	static struct timeval last_optimiz_time;
	int unit_time = 0;
	...
	if (ffc_smooth) {
		...
		if (status != last_status) {
			if (last_status == POWER_SUPPLY_STATUS_CHARGING
				...
				do_gettimeofday(&last_change_time);
			last_status = status;
		}
	}

	if (bq->fast_mode && raw_soc >= FG_REPORT_FULL_SOC && raw_soc != FG_RAW_SOC_FULL) {
		if (last_optimiz_time.tv_sec == 0)
			do_gettimeofday(&last_optimiz_time);
		...
		if (raw_soc > last_raw_soc && soc_changed) {
			...
			do_gettimeofday(&last_optimiz_time);
			bq_dbg(PR_DEBUG, "optimiz_soc:%d, last_optimiz_time.tv_sec:%d\n",
					optimiz_soc, last_optimiz_time.tv_sec);
			if (optimiz_soc > 100)
				optimiz_soc = 100;
			ffc_smooth = true;
		}
	...
	if (batt_temp > BATT_COOL_THRESHOLD && !cold_smooth && batt_soc != 0) {
		if (ffc_smooth && (status == POWER_SUPPLY_STATUS_DISCHARGING ||
	...
	bq_dbg(PR_DEBUG, "batt_ma_avg:%d, batt_ma:%d, cold_smooth:%d"
			"unit_time:%d, delta_time:%d, change_delta:%d"
			"bq->update_now:%d, last_change_time:%d, last_optimiz_time:%d",
			batt_ma_avg, batt_ma, cold_smooth, unit_time, delta_time,
			change_delta, bq->update_now, last_change_time.tv_sec,
			last_optimiz_time.tv_sec);
	...
	bq_dbg(PR_DEBUG, "batt_soc:%d,last_batt_soc:%d,system_soc:%d,raw_soc:%d"
			"batt_ma:%d,soc_changed:%d,bq->fast_mode:%d",
			batt_soc, last_batt_soc, system_soc, raw_soc,
			batt_ma, soc_changed, bq->fast_mode);
