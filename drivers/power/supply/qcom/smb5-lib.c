...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	pr_info("%s icl%d connector_type%d real_charger_type %d typec_legacy %d",
		__FUNCTION__,icl_ua,chg->connector_type,chg->real_charger_type,chg->typec_legacy);
				...
				union power_supply_propval *val)
{
	...
	/*2019/1/04 modified by yangshengtao for HTH-43063 warm and overheat report charging status as customer require ,begin*/
	...
	/*2019/1/04 modified by yangshengtao for HTH-43063 warm and overheat report charging status as customer require ,end*/
				...
				union power_supply_propval *val)
{
	...
	pr_info("%s stat %d",__FUNCTION__,stat);
	return 0;
}
			       ...
			       union power_supply_propval *val)
{
	...
	if (get_client_vote_locked(chg->usb_icl_votable, USER_VOTER) == 0) {
		smblib_err(chg, "force return false\n");
		val->intval = false;
		return rc;
	}
...
irqreturn_t usbin_uv_irq_handler(int irq, void *data)
{
	...
	smblib_dbg(chg, PR_OEM, "chg->qc2_unsupported_voltage=%d,chg->qc2_unsupported=%d,apsd->pst=%d\n",
		chg->qc2_unsupported_voltage,chg->qc2_unsupported,apsd->pst);
...
static void typec_src_insertion(struct smb_charger *chg)
{
	...
	pr_info("%s pr_swap_in_progress %x \n",__FUNCTION__,chg->pr_swap_in_progress);
	...
	pr_info("%s typec_legacy %x *chg->pd_disabled %x\n",
			__FUNCTION__,chg->typec_legacy,*chg->pd_disabled);
	pr_info("%s typec_legacy %x chg->early_usb_attach %x\n",
			__FUNCTION__,chg->typec_legacy,chg->early_usb_attach);
	pr_info("%s typec_legacy %x !chg->pd_not_supported %x\n",
			__FUNCTION__,chg->typec_legacy,!chg->pd_not_supported);
	pr_info("%s typec_legacy %x ok_to_pd %x\n",
			__FUNCTION__,chg->typec_legacy,chg->ok_to_pd);
...
irqreturn_t typec_attach_detach_irq_handler(int irq, void *data)
{
	...
	pr_info(" sink_src_mode %x \n",__FUNCTION__,chg->sink_src_mode);
