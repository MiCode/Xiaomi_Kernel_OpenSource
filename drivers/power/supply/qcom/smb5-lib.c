					...
					u8 allowed_voltage)
{
	...
	smblib_err(chg, "sucess write 0x%02x to USBIN_ADAPTER_ALLOW_CFG rc=%d\n",
			allowed_voltage, rc);
...
#ifdef CONFIG_BATT_VERIFY_BY_DS28E16
	if (!strcmp(psy->desc->name, "maxim-ds28e16")) {
		if (!chg->batt_verify_psy)
			chg->batt_verify_psy = psy;
		if (ev == PSY_EVENT_PROP_CHANGED) {
			rc = power_supply_get_property(chg->batt_verify_psy,
					POWER_SUPPLY_PROP_MAXIM_BATT_VERIFY, &pval);
			if (rc < 0) {
				pr_err("Couldn't get batt verify status rc=%d\n", rc);
				chg->batt_verify_psy = 0;
			}
		}
			chg->batt_verified = pval.intval;
			pr_info("batt_verified[%d]\n", chg->batt_verified);
	}
#endif
...
int smblib_get_quick_charge_type(struct smb_charger *chg)
{
	int i = 0, rc;
	union power_supply_propval pval = {0, };
	static bool last_real_charger_type_is_qc2;
	if (!chg) {
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP)
		last_real_charger_type_is_qc2 = true;
	else if (!chg->recheck_charger && last_real_charger_type_is_qc2)
		last_real_charger_type_is_qc2 = false;

	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_DCP &&
		last_real_charger_type_is_qc2 && chg->recheck_charger)
