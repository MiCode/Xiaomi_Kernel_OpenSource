...
static int __debug_mask = PR_MISC | PR_PARALLEL | PR_OTG | PR_OEM;
		...
		union power_supply_propval *val)
{
	...
	/*2019/01/04 modified by yangshengtao for HTH-43080 remove this incorrect logical(sync with qcom) ,begin*/
#if 0
		if (!rc)
			val->intval *= (-1);
#endif
	/*2019/01/04 modified by yangshengtao for HTH-43080 remove this incorrect logical(sync with qcom) ,end*/
		break;
...
static struct smb_irq_info smb5_irqs[] = {
	...
	[TYPEC_OR_RID_DETECTION_CHANGE_IRQ] = {
		...
		/*disable wakeup of of typec_or_rid_detection_change_irq_handler */
		/*.wake 		  = true,*/
	},
