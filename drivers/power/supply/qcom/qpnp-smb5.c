...
static int __debug_mask = PR_INTERRUPT | PR_MISC | PR_PARALLEL | PR_OTG | PR_OEM | PR_WLS;
module_param_named(
...
static int smb5_parse_dt(struct smb5 *chip)
{
	...
	rc = smblib_get_iio_channel(chg, "conn_therm",
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VBUS_DISABLE:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		vote(chg->chg_disable_votable, CHG_STATE_VOTER,
				!val->intval, 0);
		break;
	case POWER_SUPPLY_PROP_TYPE_RECHECK:
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_MAIN_FCC_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_FORCE_MAIN_FCC:
		pr_err("ILIM POWER_SUPPLY_PROP_FORCE_MAIN_FCC %d\n", val->intval);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_QNOVO:
		...
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		val->intval = get_effective_result_locked(chg->fcc_votable);
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
	...
	case POWER_SUPPLY_PROP_FCC_STEPPER_ENABLE:
		...
		break;
	case POWER_SUPPLY_PROP_STEP_INDEX:
		val->intval = qcom_step_chg_get_step_index();
		break;
...
static int smb5_configure_iterm_thresholds_adc(struct smb5 *chip)
{
	...
	if (chip->dt.term_current_thresh_hi_ma < -100000 ||
			chip->dt.term_current_thresh_hi_ma > 10000 ||
			chip->dt.term_current_thresh_lo_ma < -100000 ||
...
static int smb5_determine_initial_status(struct smb5 *chip)
{
	...
	if (chg->early_usb_attach)
		schedule_delayed_work(&chg->early_attach,
			msecs_to_jiffies(EARLY_ATTACH_DELAY_MS));
