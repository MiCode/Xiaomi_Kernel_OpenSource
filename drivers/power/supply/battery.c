...
#define pr_fmt(fmt) "MTK-BATT: %s: " fmt, __func__
...
#include "mediatek/charger_agate/mtk_charger_init.h"
...
int mtk_batt_create_votable;
int mtk_batt_destroy_votable;
...
static bool is_cp_available(struct pl_data *chip)
{
	if (!chip->cp_master_psy) {
		if (votable_info->charge_120w_enable) {
			chip->cp_master_psy =
				power_supply_get_by_name("bq25980-master");
		} else {
			chip->cp_master_psy =
...
static bool is_chg5_available(struct pl_data *chip)
{
	if (votable_info->charge_120w_enable) {
		if (!chip->chg5_dev)
			chip->chg5_dev = get_charger_by_name("quinary_chg");
		return !!chip->chg5_dev;
	} else {
		return true;
	}
}

static bool is_chg1_available(struct pl_data *chip)
{
	if (!chip->chg1_dev)
		chip->chg1_dev = get_charger_by_name("primary_chg");

	return !!chip->chg1_dev;
}
...
{
	int rc, fcc, target_icl = chip->chg_param->hvdcp3_max_icl_ua;
				...
				int *slave_icl_ua, int *total_settled_icl_ua)
{
	...
	if (!is_chg5_available(chip))
		return -EINVAL;

	if (!is_chg1_available(chip))
		return -EINVAL;
	...
	if (!get_effective_result_locked(chip->pl_disable_votable)) {
		...
		if (votable_info->charge_120w_enable) {
			rc = charger_dev_get_mivr(chip->chg5_dev, &pval.intval);
		} else {
			rc = charger_dev_get_mivr(chip->chg1_dev, &pval.intval);
		}
		if (rc < 0) {
...
static void split_settled(struct pl_data *chip)
{
	...
	if (slave_ua > chip->pl_settled_ua) {
		...
		if (votable_info->charge_120w_enable) {
			rc = charger_dev_set_input_current(chip->chg5_dev, main_ua);
		} else {
			rc = charger_dev_set_input_current(chip->chg1_dev, main_ua);
		}
		if (rc < 0) {
...
static void pl_taper_work(struct work_struct *work)
{
	...
	chip->taper_entry_fv = get_effective_result_locked(chip->fv_votable);
	...
	while (true) {
		 ...
		 */
		if (get_effective_result_locked(chip->fv_votable) >
						chip->taper_entry_fv) {
			pl_dbg(chip, PR_PARALLEL, "Float voltage increased. Exiting taper\n");
			goto done;
		} else {
			chip->taper_entry_fv =
					get_effective_result_locked(chip->fv_votable);
		}
			...
			int fcc_main_ua, const char *client)
{
	...
	//			  &pval);
	if (votable_info->charge_120w_enable) {
		rc = charger_dev_set_charging_current(chip->chg5_dev, fcc_main_ua);
	} else {
		rc = charger_dev_set_charging_current(chip->chg1_dev, fcc_main_ua);
	}
	if (rc < 0) {
			...
			int total_fcc_ua, const char *client)
{
	...
	if (!chip->cp_slave_psy) {
		if (votable_info->charge_120w_enable) {
			chip->cp_slave_psy =
				power_supply_get_by_name("bq25980-slave");
		} else {
			chip->cp_slave_psy =
...
static int dynamic_recharge_vbat(struct pl_data *chip, int fv_uv)
{
	...
	if (votable_info->charge_120w_enable)
		rc = charger_dev_set_recharge_vbat(chip->chg5_dev, recharge_vbat);
	if (rc < 0) {
			...
			int fv_uv, const char *client)
{
	...
	if (votable_info->charge_120w_enable) {
		rc = charger_dev_set_constant_voltage(chip->chg5_dev, fv_uv);
	} else {
		rc = charger_dev_set_constant_voltage(chip->chg1_dev, fv_uv);
	}
	if (rc < 0) {
			...
			int icl_ua, const char *client)
{
	...
	//		&pval);
	if (votable_info->charge_120w_enable) {
		rc = charger_dev_set_input_current(chip->chg5_dev, icl_ua);
	} else {
		rc = charger_dev_set_input_current(chip->chg1_dev, icl_ua);
	}
	if (rc < 0) {
		pr_err("Couldn't set input current rc=%d\n", rc);
		return rc;
	}
	...
	if (!pl_disable) {
		...
		if (chip->fcc_stepper_enable) {
			get_fcc_stepper_params(chip, master_fcc_ua,
					slave_fcc_ua);
			if (chip->step_fcc)
				schedule_delayed_work(&chip->fcc_stepper_work, 0);
		} else {
			/*
			 * If there is an increase in slave share
			 * (Also handles parallel enable case)
...
static void handle_settled_icl_change(struct pl_data *chip)
{
	...
	//		       &pval);
	if (votable_info->charge_120w_enable) {
		rc = charger_dev_get_input_current(chip->chg5_dev, &pval.intval);
	} else {
		rc = charger_dev_get_input_current(chip->chg1_dev, &pval.intval);
	}
	if (rc < 0) {
		pr_err("Couldn't get aicl settled value rc=%d\n", rc);
		return;
...
int mtk_batt_init(struct charger_param *chg_param)
{
	struct pl_data *chip;
	int rc = 0;
	...
	if (votable_info->charge_120w_enable) {
		chip->chg5_dev = get_charger_by_name("quinary_chg");
		if (!chip->chg5_dev) {
			pr_err("%s: get quinary charger device failed\n", __func__);
			return -ENODEV;
		}
	}
