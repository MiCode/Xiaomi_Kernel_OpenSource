...
int charger_dev_cp_get_chip_vendor(struct charger_device *chg_dev, int *chip_id)
{
	if (chg_dev != NULL && chg_dev->ops != NULL && chg_dev->ops->cp_get_chip_vendor)
		return chg_dev->ops->cp_get_chip_vendor(chg_dev, chip_id);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_cp_get_chip_vendor);
...
int charger_dev_cp_get_mode(struct charger_device *charger_dev, int *value)
{
	if (charger_dev != NULL && charger_dev->ops != NULL && charger_dev->ops->cp_get_mode)
		return charger_dev->ops->cp_get_mode(charger_dev, value);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_cp_get_mode);
