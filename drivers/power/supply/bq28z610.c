...
static void fg_update_status(struct bq_fg_chip *bq)
{
	...
	if (!battery_get_psy(bq)) {
		...
		fg_info("%s [FG_STATUS] [UISOC RSOC RAWSOC TEMP_SOC SOH] = [%d %d %d %d %d], [VBAT CELL0 CELL1 IBAT TBAT FC FAST_MODE] = [%d %d %d %d %d %d %d], [RM FCC qmax0 qmax1]=[%d %d]\n", bq->log_tag,
			bq->ui_soc, bq->rsoc, bq->raw_soc, temp_soc, bq->soh, bq->vbat, bq->cell_voltage[0], bq->cell_voltage[1], bq->ibat, bq->tbat, bq->batt_fc, bq->fast_chg, bq->rm, bq->fcc);
...
static int fg_check_device(struct bq_fg_chip *bq)
{
	...
	if (!strncmp(data, "MI", 2)) {
		...
		if (bq->chip_ok) {
			i = data[4];
			fg_info("%s: adapting_power num:%d, %s.\n", __func__, i, data[4]);
			if ((i >= 0) && (i <= 19)) {
