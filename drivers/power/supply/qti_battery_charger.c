				...
				size_t len)
{
	...
	case BC_XM_STATUS_GET:
		...
		if (len == sizeof(*chg_dfx_data)) {
			xm_handle_adsp_dfx_report(chg_dfx_data->type);
			ack_set = true;
			break;
		}
...
static CLASS_ATTR_RO(ui_slave_soh);
