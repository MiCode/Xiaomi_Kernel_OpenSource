...
#define ADAPTER_XIAOMI_PD_80W     0xf
					...
					struct class_attribute *attr, char *buf)
{
	...
	else if(pst->prop[WLS_TX_ADAPTER] == ADAPTER_XIAOMI_PD_80W)
	      return scnprintf(buf, PAGE_SIZE, "%u", 80);
	else if(pst->prop[WLS_TX_ADAPTER] == ADAPTER_XIAOMI_PD_100W)
	      return scnprintf(buf, PAGE_SIZE, "%u", 100);
	else if((pst->prop[WLS_TX_ADAPTER] == ADAPTER_XIAOMI_PD_30W)
				|| (pst->prop[WLS_TX_ADAPTER] == ADAPTER_VOICE_BOX_30W))
	      return scnprintf(buf, PAGE_SIZE, "%u", 30);
	else if((pst->prop[WLS_TX_ADAPTER] == ADAPTER_XIAOMI_QC3_20W)
				|| (pst->prop[WLS_TX_ADAPTER] == ADAPTER_XIAOMI_PD_20W)
				|| (pst->prop[WLS_TX_ADAPTER] == ADAPTER_XIAOMI_CAR_20W))
