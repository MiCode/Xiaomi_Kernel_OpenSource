				...
				int len)
{
	...
	if (!rc) {
		...
		if (!rc) {
			...
			/* should return 0 when bcdev->read_capacity_timeout is true */
...
/* according to blank state to enable lpd check*/
				...
				size_t len)
{
	...
	case BC_XM_STATUS_GET:
	    ...
	    // lingshi jiejuefangan ,houxu youhua 
		pst = &bcdev->psy_list[PSY_TYPE_BATTERY];
		break;
	default:
		break;
...
static bool check_batt_capacity_whether_glink_timeout(struct power_supply *psy)
{
	...
	if (rc < 0) {
		pr_err("Failed to read BATT_VOLT_NOW, rc=%d\n", rc);
		/*
		 * 8650 projects now are all 1S battery, use 3.7V as default when glink read fail
		 * if new project which use 2S battery, should modify it to 3.7V * 2
		 */
		vbat_mv = 3700;
	} else {
		vbat_mv = pst->prop[BATT_VOLT_NOW] / 1000;
		pr_err("BATT_VOLT_NOW %d\n", vbat_mv);
	}
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		/* record last capacity when it above 0% */
		if (pval->intval > 0)
				     ...
				     const char *buf, size_t count)
{
	...
	pr_info("UI SOH buf: %s, count: %d ", buf, count);
