...
static void xm_charger_debug_info_print_work(struct work_struct *work)
{
	...
	int vbus_vol_uv = 0, ibus_ua = 0, icl = 0;
	int interval = NOT_CHARGING_PERIOD_S;
	union power_supply_propval val = {0, };
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_XM];
	struct psy_state *bat_pst = &bcdev->psy_list[PSY_TYPE_BATTERY];
	...
	if ((usb_present == 1) || (wls_present == 1)) {
		...
		rc = read_property_id(bcdev, bat_pst, BATT_CONSTANT_CURRENT);
		rc = usb_psy_get_prop(usb_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &val);
		if (!rc)
			icl = val.intval;
		else
			icl = 0;
		pr_err("XM_CHG_INFO, Bus[v:%d i:%d] Vote[FCC:%d ICL:%d]\n",
				vbus_vol_uv/1000, ibus_ua/1000, bat_pst->prop[BATT_CONSTANT_CURRENT]/1000, icl/1000);
	}

	rc = read_property_id(bcdev, bat_pst, BATT_CAPACITY);
	rc = read_property_id(bcdev, pst, XM_PROP_FG1_FCC);
	rc = read_property_id(bcdev, pst, XM_PROP_FG1_RM);
	rc = read_property_id(bcdev, pst, XM_PROP_FG1_RSOC);
	rc = read_property_id(bcdev, bat_pst, BATT_VOLT_NOW);
	rc = read_property_id(bcdev, bat_pst, BATT_CURR_NOW);
	rc = read_property_id(bcdev, pst, XM_PROP_FG1_CYCLE);
	rc = read_property_id(bcdev, pst, XM_PROP_FG1_SOH);
	pr_err("XM_FG_INFO, Soc[s:%d raw:%d r:%d] Bat[v:%d i:%d] Fg[cy:%d rm:%d fc:%d soh:%d]\n",
			bat_pst->prop[BATT_CAPACITY]/100, pst->prop[XM_PROP_FG1_RM] / (pst->prop[XM_PROP_FG1_FCC]/100), pst->prop[XM_PROP_FG1_RSOC],
			bat_pst->prop[BATT_VOLT_NOW]/1000, bat_pst->prop[BATT_CURR_NOW],
			pst->prop[XM_PROP_FG1_CYCLE], pst->prop[XM_PROP_FG1_RM]/1000, pst->prop[XM_PROP_FG1_FCC]/1000, pst->prop[XM_PROP_FG1_SOH]);
