...
static const char * const power_supply_usb_type_text[] = {
	...
	"Wipower", "USB_HVDCP_3", "USB_HVDCP_3P5", "Charge_Pump",
	"BMS", "Parallel", "Main","USB_HVDCP","UFP","DFP","USB_FLOAT",
};
...
static ssize_t cc_logic_name_show(struct class *c,
				struct class_attribute *attr, char *buf)
{
	char *str = NULL;
	int rc;
	...
	rc = nopmi_chg_get_iio_channel(g_nopmi_chg, NOPMI_CC, CC_CHIP_ID, &(chg->cc_chip_id));
	if (rc < 0) {
		pr_err("Couldn't get iio cc_chip_id\n");
		return -ENODATA;
	}

	if (chg->cc_chip_id == 0)
		str = "Unknown";
	else if (chg->cc_chip_id == 1)
		str = "WUSB3801";
	else
		str = "AW35616";
...
static CLASS_ATTR_RO(cc_logic_name);
		...
		struct class_attribute *attr, char *buf)
{
	...
	int val = 0;
	int rc = 0;
	rc = nopmi_chg_get_iio_channel(chg, NOPMI_MAIN,
				MAIN_CHARGE_SET_SHIP_MODE, &val);
	 if (rc < 0) {
                pr_err("Couldn't get iio [%s], ret = %d\n",
                                main_chg_ext_iio_chan_name[MAIN_CHARGE_SET_SHIP_MODE], rc);
                return -ENODATA;
        }
		...
		struct class_attribute *attr,const char *buf, size_t len)
{
	...
	rc = nopmi_chg_set_iio_channel(chg, NOPMI_MAIN, MAIN_CHARGE_SET_SHIP_MODE, val);
	return len;
}
		...
		struct class_attribute *attr,const char *buf, size_t len)
{
	...
	rc = kstrtoint(buf, 10, &g_shipmode_en);
	if(rc){
		pr_err("%s kstrtoint fail\n", __func__);
		return -EINVAL;
	}
...
static ssize_t battery_type_show(struct class *c,
                                struct class_attribute *attr, char *buf)
{
	struct nopmi_chg *chg = container_of(c, struct nopmi_chg,
						battery_class);
	...
	if (rc < 0) {
		pr_err("Cannot get the FG_RESISTANCE_ID\n");
		return -ENODATA;
	}

	if(batt_id == 330000)
		return scnprintf(buf, CP_SIZE, "%s\n", "SUNWODA-330k-8000mAh");
	else if(batt_id == 220000)
		return scnprintf(buf, CP_SIZE, "%s\n", "SUNWODA_SAFE-220k-8000mAh");
	else
		return scnprintf(buf, CP_SIZE, "%s\n", "UNKNOWN");
}
				  ...
				  struct class_attribute *attr, char *buf)
{
	...
	val = nopmi_chg_get_iio_channel(chg, NOPMI_BMS, FG_RAW_SOC, &pval.intval);
	if (val) {
				...
				struct class_attribute *attr, char *buf)
{
	...
	rc = nopmi_chg_get_iio_channel(g_nopmi_chg, NOPMI_CC, CC_TYPEC_CC_ORIENTATION, &(chg->cc_orientation));
	rc1 = nopmi_chg_get_iio_channel(g_nopmi_chg, NOPMI_CC, CC_TYPEC_MODE, &(chg->typec_mode));
	if (rc < 0 || rc1 < 0) {
		pr_err("Couldn't get iio cc_orientation or typec_mode\n");
		return -ENODATA;
	}
				...
				struct class_attribute *attr, char *buf)
{
	...
	rc = nopmi_chg_get_iio_channel(g_nopmi_chg, NOPMI_CC, CC_TYPEC_MODE, &(chg->typec_mode));
	if (rc < 0) {
		pr_err("Couldn't get iio typec_mode\n");
		return -ENODATA;
	}
	val = chg->typec_mode;
...
static ssize_t charge_vbus_volt_show(struct class *c,
				struct class_attribute *attr, char *buf)
{
	int val = 0;
	int rc;
	...
	rc = nopmi_chg_get_iio_channel(g_nopmi_chg,
			NOPMI_MAIN, MAIN_CHARGE_VBUS_VOL, &val);
	if (rc < 0) {
		pr_err("[%s] Couldn't get iio vbus_vol, ret = %d\n",rc);
		return -ENODATA;
	}

	return scnprintf(buf, sizeof(int), "%d", val);
}
static CLASS_ATTR_RO(charge_vbus_volt);
...
static struct attribute *battery_class_attrs[] = {
	...
	&class_attr_charge_vbus_volt.attr,
...
int get_usb_typec_mode(void)
{
	...
	if (IS_ERR_OR_NULL(g_nopmi_chg))
		return -ENODATA;

	rc = nopmi_chg_get_iio_channel(g_nopmi_chg, NOPMI_CC, CC_TYPEC_MODE, &typec_mode);
	if (rc < 0 ) {
		pr_err("Couldn't get iio typec_mode\n");
		return -ENODATA;
	}
				...
				const union power_supply_propval *val)
{
	...
	if (nopmi_chg->system_temp_level == 16 || nopmi_chg->system_temp_level == 17) {
		if (nopmi_chg->real_type == QTI_POWER_SUPPLY_TYPE_USB_HVDCP ) {
...
static int nopmi_get_batt_health(struct nopmi_chg *nopmi_chg)
{
	...
	else if(pval.intval >= 56 && pval.intval < 58)
	{
		nopmi_chg->batt_health = POWER_SUPPLY_HEALTH_HOT;
	}
	else if(pval.intval >= 48 && pval.intval < 56)
	{
		nopmi_chg->batt_health = POWER_SUPPLY_HEALTH_WARM;
	}
	else if(pval.intval >= 15 && pval.intval < 48)
	{
		nopmi_chg->batt_health = POWER_SUPPLY_HEALTH_GOOD;
	}
		...
		const union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_ONLINE:
			...
			if(nopmi_chg->usb_online && !nopmi_chg->is_nopmi_awake) {
				nopmi_chg->is_nopmi_awake = 1;
				pm_stay_awake(nopmi_chg->dev);
				start_nopmi_chg_workfunc();
			} else if (!nopmi_chg->usb_online && nopmi_chg->is_nopmi_awake) {
				nopmi_chg->is_nopmi_awake = 0;
...
static void nopmi_chg_workfunc(struct work_struct *work)
{
	...
	if (nopmi_chg_is_usb_present(chg->usb_psy)) {
		...
		if (!psy){
			...
			val = nopmi_get_quick_charge_type(psy);
			rc = power_supply_get_property(chg->main_psy, POWER_SUPPLY_PROP_ONLINE, &pval);
                  	if(rc < 0){
                        	pr_err("%s set usb online fail\n", __func__);
                        }
		}
		start_nopmi_chg_jeita_workfunc();

		if (last_quick_charge_type != val || last_online != pval.intval) {
			chg->update_cont = 15;
			...
			pr_err("%s nopmi_chg->usb_online[%d]:nopmi_chg->usb_online[%d]\n", __func__, pval.intval, last_online);
		...
		int *val2, long mask)
{
	...
	*val1 = 0;
	...
	case PSY_IIO_USB_REAL_TYPE:
		...
		break;
	case PSY_IIO_TYPEC_MODE:
		rc = nopmi_chg_get_iio_channel(chip, NOPMI_CC, CC_TYPEC_MODE, &(chip->typec_mode));
		if (rc < 0) {
			pr_err("Couldn't get iio typec_mode\n");
			return -ENODATA;
		}
		*val1 = chip->typec_mode;
		break;
	case PSY_IIO_TYPEC_CC_ORIENTATION:
		rc = nopmi_chg_get_iio_channel(chip, NOPMI_CC, CC_TYPEC_CC_ORIENTATION, &(chip->cc_orientation));
		if (rc < 0) {
			pr_err("Couldn't get iio cc_orientation\n");
			return -ENODATA;
		}
...
static int reboot_ship_mode(struct notifier_block *nb, unsigned long code, void *unused)
{
	...
	if (g_shipmode_en == 1) {
		rc = nopmi_chg_set_iio_channel(g_nopmi_chg, NOPMI_MAIN, MAIN_CHARGE_SET_SHIP_MODE, g_shipmode_en);
		if (rc < 0)
