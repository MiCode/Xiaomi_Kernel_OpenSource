...
#define ONE_WIRE_CONFIG_OUT		writel_relaxed(0x10000000, g_onewire_data->gpio_dir_set_addr)// OUT
#define ONE_WIRE_CONFIG_IN		writel_relaxed(0x10000000, g_onewire_data->gpio_dir_clr_addr)// IN
#define ONE_WIRE_OUT_HIGH		writel_relaxed(0x10000000, g_onewire_data->gpio_dout_set_addr)// OUT: 1
#define ONE_WIRE_OUT_LOW		writel_relaxed(0x10000000, g_onewire_data->gpio_dout_clr_addr)// OUT: 0
...
struct onewire_gpio_data {
	...
	void *gpio_dir_addr;// [rw]0：in  1：out
	void *gpio_dir_set_addr;// [w]0:keep   1:set
	void *gpio_dir_clr_addr;// [w]0:keep   1:clr
	void *gpio_dout_addr;// [rw]0:L 1:H
	void *gpio_dout_set_addr;// [w]0:keep   1:set
	void *gpio_dout_clr_addr;// [w]0:keep   1:clr
	void *gpio_din_addr;// [R]
...
unsigned char ow_reset(void)
{
	...
	ONE_WIRE_CONFIG_IN;
	Delay_us(7);
	presence = (unsigned char)((readl_relaxed(g_onewire_data->gpio_din_addr) & 0x10000000) >> 28); // Read
	Delay_us(50);
