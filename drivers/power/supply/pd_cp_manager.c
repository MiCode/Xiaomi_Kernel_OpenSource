...
static int thermal_limit_fcc_slow_decline = 1000;
module_param_named(thermal_limit_fcc_slow_decline, thermal_limit_fcc_slow_decline, int, 0600);
...
static bool pdm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
	...
	for (i = 0; i < pdpm->cap.nr; i++) {
		pdm_info("dump PDO vbus=%d, ibus=%d, type=%d\n", pdpm->cap.max_mv[i], pdpm->cap.ma[i], pdpm->cap.type[i]);
...
static void pdm_update_status(struct usbpd_pm *pdpm)
{
	...
	pdm_info("mt6375 vbus= %d, master_vbus=%d, master_ibus=%d, master_vbatt=%d, slave_ibus=%d, total_ibus=%d, master_enable=%d, slave_enable=%d\n",
...
static void pdm_bypass_check(struct usbpd_pm *pdpm)
{
	...
	ktime_t tmp_time = 0;
	static int thermal_limit_fcc_last = 0;
	int diff_thermal_limit_fcc = 0;
	int apdo_max_watt_temp = pdpm->apdo_max_watt/1000000;
	if(thermal_limit_fcc_last == 0)
		thermal_limit_fcc_last = pdpm->thermal_limit_fcc;
	 diff_thermal_limit_fcc = thermal_limit_fcc_last - pdpm->thermal_limit_fcc;
	if(pdpm->thermal_limit_fcc <= bypass_enter_fcc_set && thermal_limit_fcc_last > bypass_enter_fcc_set)
	{
		thermal_limit_fcc_slow_decline = 500;
	}
	pdm_info("%s thermal_limit_fcc=%d thermal_limit_fcc_last=%d diff_thermal_limit_fcc=%d\n", __func__, pdpm->thermal_limit_fcc, thermal_limit_fcc_last, diff_thermal_limit_fcc);
	if (pdpm->cp_work_mode == SC8561_FORWARD_4_1_CHARGER_MODE)
			...
			res = 1;
	if (pdpm->bypass_support && pdpm->apdo_max_watt > MAX_WATT_33W && pdpm->soc < 90 
		&& ((apdo_max_watt_temp != 67 && pdpm->soc >=25) || (apdo_max_watt_temp == 67))) {
		if(diff_thermal_limit_fcc > thermal_limit_fcc_slow_decline)
		{
			pdpm->thermal_limit_fcc = thermal_limit_fcc_last - thermal_limit_fcc_slow_decline;
			pdm_info("%s thermal_limit_fcc=%d target_fcc=%d\n", __func__, pdpm->thermal_limit_fcc, pdpm->target_fcc);
		}
		if (pdpm->bypass_enable) {
	...
	thermal_limit_fcc_last = pdpm->thermal_limit_fcc;
	pdm_info("%s thermal_limit_fcc_last=%d\n",  __func__, thermal_limit_fcc_last);
	if (pdpm->switch_mode) {
...
static int pdm_tune_pdo(struct usbpd_pm *pdpm)
{
	...
	pdm_err("%s vbus_limit=%d ibus_limit=%d apdo_max_ibus_temp=%d", __func__, vbus_limit, ibus_limit, apdo_max_ibus_temp);
