...
static int fake_work_mode = LN8000_MASTER;
...
static int ops_ln8000_enable_bypass(struct charger_device *chg_dev, bool enable)
{
	struct ln8000_info *info = charger_get_data(chg_dev);
	int ret = 0;

	ret = ln8000_enable_bypass(info, enable);
	if (ret)
...
static const struct charger_ops ln8000_chg_ops = {
	...
	.enable_bypass = ops_ln8000_enable_bypass,
...
static int ln8000_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev,"LN8000 probe start\n");
...
MODULE_DESCRIPTION("LN8000 Charger Driver");
MODULE_LICENSE("GPL v2");
MODULE_AUTHOR("weijun");
