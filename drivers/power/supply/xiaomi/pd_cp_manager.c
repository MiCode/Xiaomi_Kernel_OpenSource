...
static int vbus_low_gap_bypass = 120;
module_param_named(vbus_low_gap_bypass, vbus_low_gap_bypass, int, 0600);

static int vbus_high_gap_bypass = 160;
...
static bool pdm_handle_sm(struct usbpd_pm *pdpm)
{
	...
	case PD_PM_STATE_INIT_VBUS:
		...
		if (pdpm->bbc_vbus <= pdpm->vbat * (pdpm->bypass_enable ? 1 : 2) + pdpm->vbus_low_gap) {
	...
	case PD_PM_STATE_ENABLE_CP:
		...
		if (pdpm->enable_cp_count == 1 || pdpm->switch_mode) {
	...
	case PD_PM_STATE_EXIT:
		...
		if (pdpm->typec_burn || product_name == PISSARRO || pdpm->sm_status == PDM_SM_EXIT)
			adapter_set_cap_xm(DEFAULT_PDO_VBUS_1S, DEFAULT_PDO_IBUS_1S);
		else
			adapter_set_cap_xm(DEFAULT_PDO_VBUS_2S, DEFAULT_PDO_IBUS_2S);
...
static const struct platform_device_id pdm_id[] = {
	{ "pd_cp_manager_a", PISSARRO },
	{ "pd_cp_manager_b", PISSARROPRO },
	{},
};
MODULE_DEVICE_TABLE(platform, pdm_id);

static const struct of_device_id pdm_of_match[] = {
	{ .compatible = "pd_cp_manager_a", .data = &pdm_id[0], },
	{ .compatible = "pd_cp_manager_b", .data = &pdm_id[1], },
	{},
};
