					...
					const char *buf, size_t count)
{
	...
	if(val != 0) {
		if(!ffc_enable)
			...
			rc = vote(g_xm_charger->fv_votable, SMART_BATTERY_FV, true, g_xm_charger->dt.chg_voltage_max - val*1000);
	} else {
		if(!ffc_enable)
			rc = vote(g_xm_charger->fv_votable, SMART_BATTERY_FV, false, g_xm_charger->dt.non_ffc_cv - val*1000);
		else
			rc = vote(g_xm_charger->fv_votable, SMART_BATTERY_FV, false, g_xm_charger->dt.chg_voltage_max - val*1000);
	}

	pr_err("the voter result: %d\n",get_effective_result(g_xm_charger->fv_votable));
