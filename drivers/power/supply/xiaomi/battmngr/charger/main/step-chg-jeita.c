...
static int get_step_chg_jeita_setting_from_profile(struct step_chg_info *chip)
{
	...
	if (rc < 0) {
		stepchg_err("%s: Read xm,step-chg-ranges failed from battery profile, rc=%d\n",
				__func__, rc);
	...
	if (!rc) {
		...
		stepchg_err("%s: jeita-fcc-hys: rise_hys=%u, fall_hys=%u\n", __func__,
...
static int handle_step_chg_config(struct step_chg_info *chip)
{
	...
	stepchg_err("%s: VBAT_NOW = %d Step-FCC = %duA taper-fcc: %d\n", __func__,
		pval.intval, get_client_vote(chip->fcc_votable, STEP_CHG_VOTER),
		chip->taper_fcc);
...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	stepchg_err("temp:%d, new jeita index:%d, new jeita FCC:%d, new jeita FV:%d\n",
					pval.intval, chip->jeita_fcc_index, fcc_ua, fv_uv);
	...
	if (g_xm_charger->pd_verifed) {
		...
		if ((batt_temp >= BATT_WARM_THRESHOLD || batt_temp <= BATT_COOL_THRESHOLD)
			...
			xm_charger_set_fastcharge_mode(g_xm_charger, false);
				...
				&& !fastcharge_mode) {
			xm_charger_set_fastcharge_mode(g_xm_charger, true);
		}
	}
...
static int step_chg_register_notifier(struct step_chg_info *chip)
{
	...
	if (rc < 0) {
		stepchg_err("%s: Couldn't register psy notifier rc = %d\n", __func__, rc);
		return rc;
	}
...
int xm_step_chg_init(struct xm_charger *charger, bool step_chg_enable, bool sw_jeita_enable)
{
	int rc;
	...
	chip->step_chg_ws = wakeup_source_register(charger->dev, "xm-step-chg");
	if (!chip->step_chg_ws)
		return -EINVAL;
