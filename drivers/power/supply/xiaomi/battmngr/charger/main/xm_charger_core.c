...
static void  update_bc12_active(struct xm_charger *charger, int bc12_active)
{
	charger->bc12_active = bc12_active;
	if (bc12_active) {
		vote(charger->awake_votable, BC12_AWAKE_VOTER, true, 0);
	} else {
		vote(charger->awake_votable, BC12_AWAKE_VOTER, false, 0);
	}
...
static void  update_pd_active(struct xm_charger *charger, int pd_active)
{
	...
	if (pd_active) {
		vote(charger->awake_votable, PD_AWAKE_VOTER, true, 0);
	} else {
		charger->pd_verified = 0;
		g_battmngr_noti->pd_msg.pd_verified = 0;
		vote(charger->awake_votable, PD_AWAKE_VOTER, false, 0);
	}
...
static void update_usb_type(struct xm_charger *chg)
{
	...
	if(chg->real_type != usb_psy_desc.type || chg->pd_active == QTI_POWER_SUPPLY_PD_PPS_ACTIVE) {
		cancel_delayed_work(&(g_xm_charger->chg_feature->xm_prop_change_work));
		chg->chg_feature->update_cont = 15;
		schedule_delayed_work(&(g_xm_charger->chg_feature->xm_prop_change_work), msecs_to_jiffies(1000));
	}
...
int charger_process_event_mainchg(struct battmngr_notify *noti_data)
{
	...
	case BATTMNGR_MSG_MAINCHG_TYPE:
		update_bc12_active(g_xm_charger, noti_data->mainchg_msg.chg_plugin);
		...
		if (g_xm_charger->bc12_type != POWER_SUPPLY_USB_TYPE_SDP &&
			...
			charger_request_dpdm(g_xm_charger, g_xm_charger->bc12_active);
...
static int xm_charger_parse_dt(struct xm_charger *charger)
{
	...
	rc = of_property_read_u32(node,
				"xm,non_ffc_icl", &charger->dt.non_ffc_icl);
	if (rc < 0)
		charger->dt.non_ffc_cc = 3000000;
