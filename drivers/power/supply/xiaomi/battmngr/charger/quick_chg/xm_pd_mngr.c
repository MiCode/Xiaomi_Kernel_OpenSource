...
static int usbpd_pm_enable_sw(struct usbpd_pm *pdpm, bool en)
{
	...
	if(!en) {
		vote(pdpm->usb_icl_votable, MAIN_ICL_MIN_VOTER,
				true, MAIN_ICL_MIN);
...
static int usbpd_pm_check_sw_enabled(struct usbpd_pm *pdpm)
{
	...
	val = !(get_client_vote(pdpm->usb_icl_votable, MAIN_ICL_MIN_VOTER)
				== MAIN_ICL_MIN);
...
static void usbpd_fc2_exit_work(struct work_struct *work)
{
	...
	usbpd_select_pdo(pdpm, XM_PD_APDO, 10000, 2000);
...
static int usbpd_pm_probe(struct platform_device *pdev)
{
	...
	if (!g_tcpc_rt1711h || !g_battmngr) {
		pr_err("tcpc_rt1711h or g_battmngr not ready, defer\n");
		ret = -EPROBE_DEFER;
		msleep(100);
