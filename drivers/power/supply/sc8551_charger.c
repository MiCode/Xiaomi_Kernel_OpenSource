...
static int sc8551_enable_charge(struct sc8551 *sc, bool enable)
{
	...
	/*for(i=0; i<0x31; i++)
	{
		sc8551_read_byte(sc, i, &data);
		sc_err("reg[%02x] = 0x%02x\n", i, data);
	}*/
...
static int sc8551_get_adc_data(struct sc8551 *sc, int channel,  int *result)
{
	...
	if(sc->is_sc8551)
	{
		...
		else if(channel == ADC_VBAT)	val = val * SC8551_VBAT_ADC_LSB * 1.006;
		else if(channel == ADC_IBAT)	val *= SC8551_IBAT_ADC_LSB;
...
static int sc8551_init_regulation(struct sc8551 *sc)
{
	...
	if(sc->is_sc8551)
	{
		...
		sc8551_update_bits(sc, SC8551_REG_34, 0x01, 1);
	}
	return 0;
}
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SC_BUS_VOLTAGE:
		...
		break;
	case POWER_SUPPLY_PROP_SC_VAC_VOLTAGE:
		ret = sc8551_get_adc_data(sc, ADC_VAC, &result);
		if (!ret)
			sc->vac_volt = result;

		val->intval = sc->vac_volt;
		break;
	...
	case POWER_SUPPLY_PROP_SC_REG_STATUS:
		sc8551_check_reg_status(sc);
		val->intval = (sc->vbat_reg << VBAT_REG_STATUS_SHIFT) |
				(sc->ibat_reg << IBAT_REG_STATUS_SHIFT);
