...
#define BUS_OVP_FOR_QC35					11000
#define BUS_OVP_ALARM_FOR_QC35				9500
...
static int bq2597x_set_batovp_th(struct bq2597x *bq, int threshold)
{
	...
	if (bq->chip_vendor == SC8551)
		ovp_base = SC8551_BAT_OVP_BASE;
	else
		ovp_base = BQ2597X_BAT_OVP_BASE;

	if (threshold < ovp_base)
		threshold = ovp_base;

	val = (threshold - ovp_base) / BQ2597X_BAT_OVP_LSB;
...
static int sc8551_init_adc(struct bq2597x *bq)
{
	...
	ret = bq2597x_read_byte(bq, SC8551_REG_34, &val);
	if (ret)
		return ret;

	val |= 0x01;
	/* improve adc accuracy */
	ret = bq2597x_write_byte(bq, SC8551_REG_34, val);
...
static int bq2597x_detect_device(struct bq2597x *bq)
{
	...
	/* SC8551 reg[2F] bit7 - bit4 is 0x50 */
	ret = bq2597x_read_byte(bq, SC8551_REG_2F, &data);
	if (ret == 0 && (data & SC8551_CHIP_VENDOR_MASK) == 0x50)
...
static int bq2597x_set_bus_protection(struct bq2597x *bq, int hvdcp3_type)
{
	...
	if (hvdcp3_type == HVDCP3_CLASSA_18W) {
		...
		bq2597x_set_busovp_th(bq, BUS_OVP_FOR_QC35);
		bq2597x_set_busovp_alarm_th(bq, BUS_OVP_ALARM_FOR_QC35);
		bq2597x_set_busocp_th(bq, BUS_OCP_FOR_QC35_CLASS_A_P);
		bq2597x_set_busocp_alarm_th(bq, BUS_OCP_ALARM_FOR_QC35_CLASS_A_P);
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_HV_CHARGE_ENABLED:
		val->intval = bq->hv_charge_enable;
		break;
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BQ_CHARGE_DONE:
		...
		break;
	case POWER_SUPPLY_PROP_HV_CHARGE_ENABLED:
		bq->hv_charge_enable = val->intval;
		break;
	case POWER_SUPPLY_PROP_TI_CHARGE_MODE:
		if (bq->chip_vendor == SC8551)
