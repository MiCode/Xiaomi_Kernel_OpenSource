...
static int sc8551_init_irq(struct sc8551 *chip)
{
	...
	if (ret) {
		sc_err("%s ZLF:failed to request gpio\n", chip->log_tag);
		chip->irq = -1;
		return 0;
	}
...
static int sc8551_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "sc8551_probe start.\n");
	...
	if (IS_ERR_VALUE((unsigned long)ret)) {
		...
		ret = i2c_smbus_read_byte_data(client, SC8551_PART_INFO_REG);
		if (IS_ERR_VALUE((unsigned long)ret)) {
			if (!strcmp(client->name, "sc8551_i2c0")) {
				client->addr = 0x66;
			} else if (!strcmp(client->name, "sc8551_i2c10")) {
				client->addr = 0x66;
			}
			ret = i2c_smbus_read_byte_data(client, SC8551_PART_INFO_REG);
			if (IS_ERR_VALUE((unsigned long)ret)) {
...
static const struct i2c_device_id sc8551_i2c_ids[] = {
	{ "sc8551_i2c10", SC8551_STANDALONE },
...
static const struct of_device_id sc8551_of_match[] = {
	{ .compatible = "sc8551_i2c10", .data = (void *)SC8551_STANDALONE},
