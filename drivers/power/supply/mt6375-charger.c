...
static void mt6375_rerun_bc12_work(struct work_struct *work)
{
	...
	if (ddata->psy_desc.type == POWER_SUPPLY_TYPE_USB && ddata->psy_usb_type == POWER_SUPPLY_USB_TYPE_DCP) {
...
static void mt6375_detect_hvchg_work(struct work_struct *work)
{
	...
	if (vbus < HVDCP_VBUS_LIMIT) {
		dev_err(ddata->dev, "%s: HVCHG detect fail.\n", __func__);
				   ...
				   const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_VOLTAGE_LIMIT:
		...
		mutex_lock(&ddata->pe_lock);
		if (vbus > HVDCP_VBUS_LIMIT)
			ret = mt6375_chg_field_set(ddata, F_VMIVR, 8600);
		else
