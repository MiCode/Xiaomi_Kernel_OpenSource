...
static const struct mt6375_chg_dtprop mt6375_chg_dtprops[] = {
	...
	MT6375_CHG_DTPROP(nr_port, F_MAX, DTPROP_U32, true),
};
...
static int mt6375_chg_get_pdata(struct device *dev)
{
	...
	if (np) {
		...
		if (of_property_read_u32(np, "bc12_sel", &val) < 0) {
...
static int mt6375_chg_init_regulator(struct mt6375_chg_data *ddata)
{
	...
	if (IS_ERR(ddata->rdev))
		return PTR_ERR(ddata->rdev);
	ddata->regulator = devm_regulator_get(ddata->dev, "usb-otg-vbus");
	return IS_ERR(ddata->regulator) ? PTR_ERR(ddata->regulator) : 0;
}
