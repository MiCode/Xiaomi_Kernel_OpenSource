...
do {											\
	if (bq->mode == BQ25970_ROLE_MASTER)						\
		...
		printk(KERN_ERR "[bq2597x-STANDALONE-2]:%s:" fmt, __func__, ##__VA_ARGS__);\
} while (0);
...
do {											\
	if (bq->mode == BQ25970_ROLE_MASTER)						\
		...
		printk(KERN_INFO "[bq2597x-STANDALONE-2]:%s:" fmt, __func__, ##__VA_ARGS__);\
} while (0);
...
do {											\
	if (bq->mode == BQ25970_ROLE_MASTER)						\
		...
		printk(KERN_DEBUG "[bq2597x-STANDALONE-2]:%s:" fmt, __func__, ##__VA_ARGS__);\
} while (0);
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TI_BYPASS_MODE_ENABLED:
		...
		break;
	case POWER_SUPPLY_PROP_SEC_CP_INIT_DEVICE:
		if (val->intval)
			bq2597x_init_device(bq);
		break;	
	default:
					...
					const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		...
		ret = i2c_smbus_read_byte_data(client, BQ2597X_REG_13);
		if (ret < 0) {
			bq_err("%s failed to communicate with chip\n", __func__)
			return -ENODEV;
		}
