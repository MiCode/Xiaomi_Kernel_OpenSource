...
{
	unsigned int vbus_adc_lsb, vbus_adc_msb;
	u16 vbus_adc;
	int ret;
	...
	if (vbus_adc_msb & BQ25980_ADC_POLARITY_BIT)
		return ((vbus_adc ^ 0xffff) + 1) * bq->chip_info->adc_vbus_volt_step;

	return vbus_adc * bq->chip_info->adc_vbus_volt_step / 10000;
}

static int bq25980_get_adc_vbat(struct bq25980_device *bq)
{
	unsigned int vbat_adc_lsb, vbat_adc_msb;
	u16 vbat_adc;
	int ret;
	...
	if (vbat_adc_msb & BQ25980_ADC_POLARITY_BIT)
		return ((vbat_adc ^ 0xffff) + 1) * bq->chip_info->adc_vbat_volt_step;

	return vbat_adc * bq->chip_info->adc_vbat_volt_step / 10000;
}
...
static int ops_bq25980_get_vbus(struct charger_device *chg_dev, u32 *vbus_mv)
{
	struct bq25980_device *bq = charger_get_data(chg_dev);

	*vbus_mv = bq25980_get_adc_vbus(bq);

	return 0;
}

static int ops_bq25980_get_vbat(struct charger_device *chg_dev, u32 *vbat_mv)
{
	struct bq25980_device *bq = charger_get_data(chg_dev);

	*vbat_mv = bq25980_get_adc_vbat(bq);
...
static const u32 wdt_timer_table[] = {
	0, 1, 5, 30
};
static const u8 wdt_timer_config_table[] = {
	0x00, 0x01, 0x02, 0x03
};

static int ops_bq25980_enable_wdt(struct charger_device *chg_dev, bool enable, u32 timer)
{
	u8 data = 0;
	int i = 0;
	u32 value = 0;
	...
	for (i = 0; i < ARRAY_SIZE(wdt_timer_table); i++) {
		if (timer <= wdt_timer_table[i])
			break;
	}
	data =  wdt_timer_config_table[i];

	regmap_update_bits(bq->regmap, BQ25980_CHRGR_CTRL_3, BQ25980_WATCHDOG_DIS, enable? 0: BQ25980_WATCHDOG_DIS);
	regmap_update_bits(bq->regmap, BQ25980_CHRGR_CTRL_3, BQ25980_WATCHDOG_MASK, data<<3);

	regmap_read(bq->regmap, BQ25980_CHRGR_CTRL_3, &value);
	bq_err("%s BQ25980_CHRGR_CTRL_3: 0x%0x\n", __func__, value);

	return 0;
}

static int ops_bq25980_enable_adc(struct charger_device *chg_dev, bool enable)
{
	struct bq25980_device *bq = charger_get_data(chg_dev);
	int ret = 0;

	ret = bq25980_enable_adc(bq, enable);
	if (ret)
...
static const struct charger_ops bq25980_chg_ops = {
	...
	.get_vbus_adc = ops_bq25980_get_vbus,
	.get_vbat_adc = ops_bq25980_get_vbat,
	.get_tchg_adc = ops_bq25980_get_tchg,
	.set_ac_ovp = ops_bq25980_set_ac_ovp,
	.enable_wdt = ops_bq25980_enable_wdt,
	.enable_adc = ops_bq25980_enable_adc,
};
...
static int bq25980_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = bq25980_get_adc_vbus(bq);
		break;
	case POWER_SUPPLY_PROP_CP_VBAT:
		val->intval = bq25980_get_adc_vbat(bq);
		break;
	case POWER_SUPPLY_PROP_CP_IBAT:
		val->intval = bq25980_get_adc_ibat(bq);
		break;
	default:
		return -EINVAL;
	}
							...
							int driver_data)
{
	...
	case BQ25980_SLAVE:
		...
		break;
	case BQ25980_STANDALONE1:
		bq->psy_desc.name = "cp_master";
		break;
	case BQ25980_STANDALONE2:
		bq->psy_desc.name = "cp_slave";
		break;
	case BQ25980_STANDALONE3:
		bq->psy_desc.name = "cp_third";
		break;
	case BQ25960_MASTER:
		bq->psy_desc.name = "cp_master";
		break;
...
static int bq25980_parse_dt(struct bq25980_device *bq)
{
	...
	pinctrl_get_select(bq->dev, "interrupt");
...
static int bq25980_parse_dt_id(struct bq25980_device *bq, int driver_data)
{
	...
	case BQ25980_STANDALONE1:
		if(bq->chip_vendor == TI25980){
			...
			strcpy(bq->log_tag, "[XMCHG_BQ25980_ALONE1]");
	...
	case BQ25980_STANDALONE2:
		if(bq->chip_vendor == TI25980){
			bq->device_id = BQ25980;
			bq->mode = BQ_STANDALONE;
			strcpy(bq->log_tag, "[XMCHG_BQ25980_ALONE2]");
		}else if(bq->chip_vendor == SOUTH8571){
			bq->device_id = SC8571;
			bq->mode = BQ_STANDALONE;
			strcpy(bq->log_tag, "[XMCHG_SC8571_ALONE]");
	...
	case BQ25980_STANDALONE3:
		if(bq->chip_vendor == TI25980){
			bq->device_id = BQ25980;
			bq->mode = BQ_STANDALONE;
			strcpy(bq->log_tag, "[XMCHG_BQ25980_ALONE3]");
		}else if(bq->chip_vendor == SOUTH8571){
			bq->device_id = SC8571;
			bq->mode = BQ_STANDALONE;
			strcpy(bq->log_tag, "[XMCHG_SC8571_ALONE]");
...
static int bq25980_register_charger(struct bq25980_device *bq, int driver_data)
{
	switch (driver_data) {
	case BQ25980_STANDALONE1:
		bq->chg_dev = charger_device_register("cp_master", bq->dev, bq, &bq25980_chg_ops, &bq25980_standalone_chg_props);
		break;
	case BQ25980_STANDALONE2:
		bq->chg_dev = charger_device_register("cp_slave", bq->dev, bq, &bq25980_chg_ops, &bq25980_standalone_chg_props);
		break;
	case BQ25980_STANDALONE3:
		bq->chg_dev = charger_device_register("cp_third", bq->dev, bq, &bq25980_chg_ops, &bq25980_standalone_chg_props);
		break;
	case BQ25980_SLAVE:
...
static void bq25980_parse_cmdline(void)
{
	...
	bq_info("product_name = %d, ruby = %d, rubypro = %d, rubyplus = %d\n", product_name, ruby ? 1 : 0, rubypro ? 1 : 0, rubyplus ? 1 : 0);
}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (product_name == RUBYPLUS) {
		...
		bq_info("ruby and rubypro no need to probe BQ25980\n");
		return -ENODEV;
	}
	...
	if (product_name == RUBYPLUS) {
		if (client->addr == 0x65 || client->addr == 0x67 || client->addr == 0x66) {
...
static const struct i2c_device_id bq25980_i2c_ids[] = {
	{ "bq25980_standalone1", BQ25980_STANDALONE1 },
	{ "bq25980_standalone2", BQ25980_STANDALONE2 },
	{ "bq25980_standalone3", BQ25980_STANDALONE3 },
...
static const struct of_device_id bq25980_of_match[] = {
	{ .compatible = "bq25980_standalone1", .data = (void *)BQ25980_STANDALONE1},
	{ .compatible = "bq25980_standalone2", .data = (void *)BQ25980_STANDALONE2},
	{ .compatible = "bq25980_standalone3", .data = (void *)BQ25980_STANDALONE3},
