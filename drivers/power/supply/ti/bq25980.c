...
static struct reg_default cpchipx_reg_init_val[] = {
	{BQ25980_BATOVP,	0x69}, /* CP sense battery package, so set BATOVP to 9100mV */
	{BQ25980_BATOVP_ALM,	0x64}, /* CP sense battery package, so set BATOVP to 9000mV */
	{BQ25980_BATOCP,	0xFF}, /* disable */
	{BQ25980_BATOCP_ALM,	0xFF}, /* disable */
...
static void bq25980_irq_handler(struct work_struct *work)
{
	...
	if(bq->chip_vendor == MICPCHIPX){
		ret = regmap_read(bq->regmap, CPCHIPX_INTERNAL_1, &data);
		if(ret < 0){
			bq_err("%s bq25980_interrupt regmap read err and return:%d\n", bq->log_tag,ret);
			return;
		}
		flag = !!(data & CPCHIPX_POR_FLAG);
...
static const struct regmap_config cpchipx_regmap_config = {
	.reg_bits = 8,
	.val_bits = 8,
...
static const struct bq25980_chip_info bq25980_chip_info_tbl[] = {
	...
	[CPCHIPX] = {
		.model_id = CPCHIPX,
		.regmap_config = &cpchipx_regmap_config,
		.reg_init_values = cpchipx_reg_init_val,
		...
		.adc_curr_step = CPCHIPX_ADC_CURR_STEP_IBUS_uA,
		.adc_vbat_volt_step = CPCHIPX_ADC_VOLT_STEP_VBAT_deciuV,
		.adc_vbus_volt_step = CPCHIPX_ADC_VOLT_STEP_VBUS_deciuV,
		.adc_vbus_volt_offset = BQ25980_ADC_VOLT_OFFSET_VBUS,
		.adc_tchg_step = BQ25980_ADC_TCHG_STEP,
...
static int bq25980_parse_dt_id(struct bq25980_device *bq, int driver_data)
{
	...
	case BQ25980_STANDALONE:
		...
		}else if(bq->chip_vendor == MICPCHIPX){
			bq->device_id = CPCHIPX;
			bq->mode = BQ_STANDALONE;
			strcpy(bq->log_tag, "[XMCHG_CPCHIPX_ALONE]");
		}
		break;	
	case BQ25980_SLAVE:
		if(bq->chip_vendor == TI25980){
			...
			bq->device_id = CPCHIPX;
			bq->mode = BQ_SLAVE;
			strcpy(bq->log_tag, "[XMCHG_CPCHIPX_SLAVE]");
		}
		break;
	case BQ25980_MASTER:
		if(bq->chip_vendor == TI25980){
			...
			bq->device_id = CPCHIPX;
			bq->mode = BQ_MASTER;
			strcpy(bq->log_tag, "[XMCHG_CPCHIPX_MASTER]");
		}
		break;
	default:
...
static int bq25980_detect_device(struct bq25980_device *bq)
{
	...
	if (ret == 0) {
		...
		if (data == CPCHIPX_DEVICE_ID)
			bq->chip_vendor = MICPCHIPX;
