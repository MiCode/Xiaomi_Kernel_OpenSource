...
retry:
	...
	if (ret < 0) {
		if (!retry) {
			...
			cv = (product_name == PISSARRO) ? 4480 : 8960;
			if (bq->i2c_error_count < 10)
				bq->i2c_error_count++;
		}
	} else {
...
retry:
	...
	if (ret < 0) {
		if (!retry) {
			...
			cc = (product_name == PISSARRO) ? 12400 : 11000;
			if (bq->i2c_error_count < 10)
				bq->i2c_error_count++;
		}
	} else {
...
static void fg_update_status(struct bq_fg_chip *bq)
{
	...
	fg_info("%s [FG_STATUS] [UISOC RSOC RAWSOC TEMP_SOC SOH] = [%d %d %d %d %d], [VBAT CELL0 CELL1 IBAT TBAT FC] = [%d %d %d %d %d %d]\n", bq->log_tag,
		bq->ui_soc, bq->rsoc, bq->raw_soc, temp_soc, bq->soh, bq->vbat, bq->cell_voltage[0], bq->cell_voltage[1], bq->ibat, bq->tbat,bq->batt_fc);
...
static bool fg_get_psy(struct bq_fg_chip *bq)
{
	...
	if (!bq->usb_psy) {
		fg_err("%s failed to get usb_psy", bq->log_tag);
		return false;
	}
...
static void pdm_parse_cmdline(void)
{
	...
	fg_info("pissarro = %d, pissarropro = %d, pissarroinpro = %d\n", pissarro ? 1 : 0, pissarropro ? 1 : 0, pissarroinpro ? 1 : 0);
