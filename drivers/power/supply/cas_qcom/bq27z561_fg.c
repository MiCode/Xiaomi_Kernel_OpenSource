...
{
	int batt_soc, curr, temp, raw_soc, soc;
...
{
	static int last_batt_soc = -1, cold_smooth, last_raw_soc, system_soc;
	static int last_status;
	int change_delta = 0, rc, status;
#ifndef HW_OPTIMIZ_REPORT_FULL
	int optimiz_delta = 0
	...
	if (bq->fast_mode && raw_soc >= FG_REPORT_FULL_SOC && raw_soc != FG_RAW_SOC_FULL) {
		...
		soc_changed = min(1, delta_time);
		bq_dbg(PR_DEBUG, "delta_time:%ld, optimiz_delta:%ld last_optimiz_time%ld\n",
					delta_time, optimiz_delta, last_optimiz_time);
		bq_dbg(PR_DEBUG, "raw_soc:%d, last_raw_soc:%d soc_changed%d\n",
					raw_soc, last_raw_soc, soc_changed);
#endif
...
#ifndef HW_OPTIMIZ_REPORT_FULL
			last_optimiz_time = ktime_get();
			bq_dbg(PR_DEBUG, "increse optimiz_soc:%d, last_optimiz_time%ld\n",
	...
	bq_dbg(PR_DEBUG, "batt_ma_avg:%d, batt_ma:%d, cold_smooth:%d",
			batt_ma_avg, batt_ma, cold_smooth);
	...
	bq_dbg(PR_DEBUG, "raw_soc:%d batt_soc:%d,last_batt_soc:%d, bq->fast_mode:%d",
			raw_soc, batt_soc, last_batt_soc, bq->fast_mode);
