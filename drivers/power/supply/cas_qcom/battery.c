			...
			int total_fcc_ua, const char *client)
{
	...
	pl_dbg(chip, PR_PARALLEL, "cp_fcc_ua:%d, total_fcc_ua:%d, main_fcc_ua:%d, cp_reason:%d\n",
	...
	if (cp_fcc_ua > 0) {
		...
		if (chip->cp_disable_votable) {
			 ...
			 * falls below threshold. 250mA * 4 = 1000mA
			 */
			vote(chip->cp_disable_votable, FCC_VOTER,
			     (total_fcc_ua <= fcc_thr_ua * 4), 0);
		}
	}
	...
	if (total_fcc_ua <= passthrough_limit && (total_fcc_ua > fcc_thr_ua * 4)
			&& !chip->passthrough_dis) {
		/*
		...
		if (chip->passthrough_mode_disable_votable) {
			vote(chip->passthrough_mode_disable_votable,
					FCC_VOTER, false, 0);
			pl_dbg(chip, PR_PARALLEL, "FCC < %d, Enable Passthrough\n", passthrough_limit);
		}
	} else {
		if (chip->passthrough_mode_disable_votable)
			vote(chip->passthrough_mode_disable_votable,
