...
int smblib_set_input_suspend(struct smb_charger *chg, bool suspend)
{
	int rc = 0;
	union power_supply_propval pval = {0,};
...
static bool smblib_can_cp_enable(struct smb_charger *chg)
{
	...
	if (is_cp_available(chg)) {
		if (chg->dc_temp_level <= MAX_THERMAL_LEVEL_FOR_CP && chg->rx_ready_for_cp)
			return true;
		else
			return false;
	} else {
...
int smblib_get_prop_input_suspend(struct smb_charger *chg,
				  union power_supply_propval *val)
{
	val->intval = ((get_client_vote(chg->chg_disable_votable, INPUT_SUSPEND_VOTER) == true) &&
		(get_client_vote(chg->cp_disable_votable, INPUT_SUSPEND_VOTER) == true));
				  ...
				  const union power_supply_propval *val)
{
	...
	smblib_set_input_suspend(chg, val->intval);
		...
		if (chg->pd_active == POWER_SUPPLY_PD_PPS_ACTIVE) {
			if (chg->thermal_taper) {
				if (chg->system_temp_level > SCREEN_ON_LEVEL)
					chg->current_thermal_fcc = chg->thermal_fcc_pps_cp[chg->system_temp_level];
				else {
					chg->current_thermal_fcc = get_effective_result(chg->fcc_votable);
				}
				chg->target_thermal_fcc = chg->thermal_fcc_pps_cp[chg->system_temp_level];
				printk("chg->target_thermal_fcc:%d, chg->system_temp_level:%d, chg->thermal_fcc_pps_cp[chg->system_temp_level]:%d\n",
						chg->target_thermal_fcc, chg->system_temp_level, chg->thermal_fcc_pps_cp[chg->system_temp_level]);
				if (chg->current_thermal_fcc < chg->target_thermal_fcc) {
					...
					printk("++ chg->current_thermal_fcc:%d, chg->thermal_taper_step:%d\n", chg->current_thermal_fcc, chg->thermal_taper_step);
					...
					printk("== chg->current_thermal_fcc:%d, chg->target_thermal_fcc:%d\n", chg->current_thermal_fcc, chg->target_thermal_fcc);
				}
			} else
				thermal_fcc_ua = chg->thermal_fcc_pps_cp[chg->system_temp_level];
		} else {
			thermal_icl_ua =
...
static void smblib_report_soc_decimal_work(struct work_struct *work)
{
	...
	if (quick_charge_type >= QUICK_CHARGE_TURBE)
		power_supply_changed(chg->bms_psy);
}
	...
	if (chg->pd_active) {
		...
		if (chg->pd_active == POWER_SUPPLY_PD_PPS_ACTIVE
			...
			if (prop_val.intval == POWER_SUPPLY_HEALTH_GOOD &&
					!(get_client_vote(chg->chg_disable_votable, INPUT_SUSPEND_VOTER))
