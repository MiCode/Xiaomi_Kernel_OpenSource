...
static int sc8561_set_sync(struct sc8561_device *chip, int driver_data)
{
	int ret = 0;

	if (driver_data == SC8561_MASTER)
		ret = regmap_write(chip->regmap, SC8561_REG_0C, 0x5B);	/*575Hz*/
	else if (driver_data == SC8561_SLAVE)
		ret = regmap_write(chip->regmap, SC8561_REG_0C, 0x63);	/*600Hz*/
...
//	ret = sc8561_set_wdt(chip, 0);
	...
	ret = sc8561_set_sync(chip, driver_data);
...
static int sc8561_ops_charge_enable(struct charger_device *chg_dev, bool enable)
{
...
static int sc8561_ops_get_charge_enable(struct charger_device *chg_dev, bool *enabled)
{
	struct sc8561_device *chip = charger_get_data(chg_dev);
	int ret = 0;
...
static int sc8561_ops_get_ibus(struct charger_device *chg_dev, u32 *value)
{
	struct sc8561_device *chip = charger_get_data(chg_dev);
	int ret = 0;

	ret = sc8561_get_adc(chip, ADC_IBUS, value);
	if (ret) {
...
static int sc8561_ops_get_vbus(struct charger_device *chg_dev, u32 *value)
{
	struct sc8561_device *chip = charger_get_data(chg_dev);
	int ret = 0;

	ret = sc8561_get_adc(chip, ADC_VBUS, value);
	if (ret) {
		xmc_err("failed to get ADC_VBUS\n");
		*value = 0;
	}
...
static int sc8561_ops_adc_enable(struct charger_device *chg_dev, bool enable)
{
	struct sc8561_device *chip = charger_get_data(chg_dev);
	int ret = 0;
...
static int sc8561_ops_set_div_mode(struct charger_device *chg_dev, int mode)
{
	struct sc8561_device *chip = charger_get_data(chg_dev);
	int ret = 0;
	...
	case 4:
		...
		break;
	case 6:
		ret = sc8561_set_div_mode(chip, SC8561_REVERSE_1_1_CONVERTER_MODE);
		break;
	default:
...
/*
static int sc8561_ops_get_div_mode(struct sc8561_device *chip, enum xmc_cp_div_mode *mode)
{
	int value = 0, ret = 0;
...
static int sc8561_ops_device_init(struct sc8561_device *chip, enum xmc_cp_div_mode mode)
{
	int ret = 0;
...
*/
static const struct charger_ops sc8561_ops = {
	.enable = sc8561_ops_charge_enable,
	.is_enabled = sc8561_ops_get_charge_enable,
	.get_ibus_adc = sc8561_ops_get_ibus,
	.get_vbus_adc = sc8561_ops_get_vbus,
	.enable_adc = sc8561_ops_adc_enable,
	.cp_set_mode = sc8561_ops_set_div_mode,
/*
...
static int sc8561_register_charger(struct sc8561_device *bq, int driver_data)
{
	switch (driver_data) {
	case SC8561_SLAVE:
		bq->chg_dev = charger_device_register("cp_slave", bq->dev, bq, &sc8561_ops, &sc8561_slave_chg_props);
		strcpy(bq->log_tag, "[XMCHG_SC8561_SLAVE]");
		sc8561_enable_parallel_func(bq, false);
		sc8561_enable_config_func(bq, false);
		break;
	case SC8561_MASTER:
		bq->chg_dev = charger_device_register("cp_master", bq->dev, bq, &sc8561_ops, &sc8561_master_chg_props);
...
static int sc8561_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CP_VBAT:
		ret = sc8561_get_adc(chip, ADC_VBAT, &data);
		if (ret)
			val->intval = 0;
		else
			val->intval = data;
	...
	case POWER_SUPPLY_PROP_CP_IBAT:
		ret = sc8561_get_adc(chip, ADC_IBAT, &data);
		if (ret)
			val->intval = 0;
		else
			val->intval = data;
...
static void sc8561_parse_cmdline(void)
{
	...
	xmc_err("product_name = %d, ruby = %d, rubypro = %d, rubyplus = %d\n", product_name, ruby ? 1 : 0, rubypro ? 1 : 0, rubyplus ? 1 : 0);
}
...
static int sc8561_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sc8561_parse_cmdline();

	if (product_name == RUBYPRO) {
		xmc_err("SC8561 probe start\n");
	} else {
		xmc_err("ruby and rubyplus no need to probe SC8561\n");
		return -ENODEV;
	}

	/*For reslove address conflicts, need set a false address in dts, manually change the address to correct when probe*/
	if (id->driver_data == SC8561_MASTER)
		client->addr = 0x64;
	else if (id->driver_data == SC8561_SLAVE)
		client->addr = 0x65;
...
MODULE_AUTHOR("liuliang <liuliang10@xiaomi.com>");
