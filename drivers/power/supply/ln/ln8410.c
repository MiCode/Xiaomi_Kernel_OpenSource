...
/*
static int ln8410_ops_charge_enable(struct ln8410_device *chip, bool enable)
{
	int ret = 0;
...
static int ln8410_ops_get_charge_enable(struct ln8410_device *chip, bool *enabled)
{
	int ret = 0;
...
static int ln8410_ops_get_vbus(struct ln8410_device *chip, int *value)
{
	u32 vbus = 0;
	int ret = 0;
...
static int ln8410_ops_get_ibus(struct ln8410_device *chip, int *value)
{
	u32 ibus = 0;
	int ret = 0;
...
static int ln8410_ops_set_div_mode(struct ln8410_device *chip, enum xmc_cp_div_mode mode)
{
	int ret = 0;
...
static int ln8410_ops_get_div_mode(struct ln8410_device *chip, enum xmc_cp_div_mode *mode)
{
	int value = 0, ret = 0;
...
static int ln8410_ops_adc_enable(struct ln8410_device *chip, bool enable)
{
	int ret = 0;
...
static int ln8410_ops_device_init(struct ln8410_device *chip, enum xmc_cp_div_mode mode)
{
	int ret = 0;
...
static int ln8410_register_charger(struct ln8410_device *bq, int driver_data)
{
	switch (driver_data) {
	case LN8410_SLAVE:
		bq->chg_dev = charger_device_register("cp_slave", bq->dev, bq, &ln8410_ops, &ln8410_slave_chg_props);
		strcpy(bq->log_tag, "[XMCHG_Ln8410_SLAVE]");
		ln8410_enable_parallel_func(bq, false);
		ln8410_enable_config_func(bq, false);
		break;
	case LN8410_MASTER:
		bq->chg_dev = charger_device_register("cp_master", bq->dev, bq, &ln8410_ops, &ln8410_master_chg_props);
...
static void ln8410_parse_cmdline(void)
{
...
static int ln8410_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ln8410_parse_cmdline();

	if (product_name == RUBYPRO) {
		xmc_err("LN8410 probe start\n");
	} else {
		xmc_err("ruby and rubyplus no need to probe LN8410\n");
		return -ENODEV;
	}
