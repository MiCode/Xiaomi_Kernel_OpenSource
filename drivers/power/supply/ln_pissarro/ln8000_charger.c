...
static void ln8000_parse_cmdline(void)
{
	...
	//dev_info(&client->dev,"pissarro = %d, pissarropro = %d, pissarroinpro = %d\n", pissarro ? 1 : 0, pissarropro ? 1 : 0, pissarroinpro ? 1 : 0);
...
static int ln8000_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ln8000_parse_cmdline();

	if (product_name == PISSARRO) {
		dev_info(&client->dev,"LN8000 probe start\n");
	} else {
		dev_info(&client->dev,"PISSARROPRO no need to probe ln8000\n");
		return -ENODEV;
	}
