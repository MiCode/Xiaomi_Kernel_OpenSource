...
bool adapter_dev_get_hardreset_state(struct adapter_device *adapter_dev)
{
	if (adapter_dev != NULL && adapter_dev->ops != NULL && adapter_dev->ops->get_pd_hardreset_state)
		return adapter_dev->ops->get_pd_hardreset_state(adapter_dev);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(adapter_dev_get_hardreset_state);
...
{
	struct adapter_device *adapter_dev = to_adapter_device(dev);
	if (adapter_dev != NULL && adapter_dev->ops != NULL && adapter_dev->ops->get_pd_current_state)
		adapter_dev->ops->get_pd_current_state(adapter_dev);
