...
/***** sysfs enable *************************************************/
static void akm_sysfs_update_active_status(struct akm_compass_data *akm)
{
	uint32_t en;
	mutex_lock(&akm->val_mutex);
	en = akm->enable_flag;
...
static ssize_t akm_sysfs_enable_show(struct akm_compass_data *akm, char *buf)
{
	int flag;
...
static ssize_t akm_sysfs_enable_store(struct akm_compass_data *akm,
...
device_initcall_sync(akm_compass_init);
