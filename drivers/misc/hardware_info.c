...
bool camerainfo_centre(int cmd, unsigned long arg)
{
        ...
        if(!strcmp(hardwareinfo_name[cmd],"NULL" )) {
                pr_info("copy from user");
                if(copy_from_user(hardwareinfo_name[cmd], data, HARDWARE_MAX_ITEM_LONGTH)) {
                        pr_err("%d camera data copy_from_user fail",cmd);
                        ret =  -EINVAL;
                }
        } else {
                pr_info("copy to user");
                if (copy_to_user(data, hardwareinfo_name[cmd], HARDWARE_MAX_ITEM_LONGTH)) {
                        pr_err("%d camera data copy_from_user fail",cmd);
                        ret =  -EINVAL;
                }
        }
...
static long hardwareinfo_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
        ...
        case HARDWARE_FRONT_CAM_GET:
                ret = camerainfo_centre(HARDWARE_FRONT_CAM,arg);
                goto set_ok;
        case HARDWARE_BACK_CAM_GET:
                ret = camerainfo_centre(HARDWARE_BACK_CAM,arg);
                goto set_ok;
        case HARDWARE_BACK_SUBCAM_GET:
                ret = camerainfo_centre(HARDWARE_BACK_SUBCAM,arg);
                goto set_ok;
        case HARDWARE_BACK_WIDE_CAM_GET:
                ret = camerainfo_centre(HARDWARE_BACK_WIDE_CAM,arg);
                goto set_ok;
        case HARDWARE_BACK_MACRO_CAM_GET:
                ret = camerainfo_centre(HARDWARE_BACK_MACRO_CAM,arg);
                goto set_ok;
	...
	case HARDWARE_CHARGER_IC_INFO_GET:
        ...
        case HARDWARE_BACK_CAM_MOUDULE_ID_GET:
                ret = camerainfo_centre(HARDWARE_BACK_CAM_MOUDULE_ID,arg);
                goto set_ok;
        case HARDWARE_FRONT_CAM_MODULE_ID_GET:
                ret = camerainfo_centre(HARDWARE_FRONT_CAM_MOUDULE_ID,arg);
                goto set_ok;
        case HARDWARE_BACK_WIDE_CAM_MOUDULE_ID_GET:
                ret = camerainfo_centre(HARDWARE_BACK_WIDE_CAM_MOUDULE_ID,arg);
                goto set_ok;
        case HARDWARE_BACK_MACRO_CAM_MOUDULE_ID_GET:
                ret = camerainfo_centre(HARDWARE_BACK_MACRO_CAM_MOUDULE_ID,arg);
                goto set_ok;
        case HARDWARE_BACK_SUBCAM_MODULEID_GET:
                ret = camerainfo_centre(HARDWARE_BACK_SUBCAM_MODULEID,arg);
                goto set_ok;
