...
/* To do: move atomic variables into ufs context */
...
{
	struct mpbe_mictx *mictx;
	struct mpbe_throughput_rw *tprw;
	unsigned long flags;
	...
	list_for_each_entry_rcu(mictx, &btag->ctx.mictx.list, list) {
		struct mpbe_mictx_data *data = mictx->data;
			     ...
			     __u32 total_len, __u32 top_len)
{
	...
	if (top_len && btag->vops->earaio_enabled)
		mpbe_earaio_update_pwd(write, top_len);
}
					 ...
					 __u64 t_begin, __u64 t_cur)
{
	...
	if (idx >= btag->ctx.count)
		return;
	MPBE_DBG("enter t_begin=%d,t_end=%d\n",t_begin,t_cur);
	rcu_read_lock();
	list_for_each_entry_rcu(mictx, &btag->ctx.mictx.list, list) {
		struct mpbe_mictx_data *data = mictx->data;
...
void mpbe_mictx_update(struct mpbe *btag, __u32 idx,
			   __u32 q_depth, __u64 sum_of_inflight_start, int cmd_rsp)
{
	...
	if (q_depth && btag->vops->earaio_enabled)
		mpbe_earaio_check_pwd();
}

static void mpbe_mictx_reset(struct mpbe_mictx_data *data,
				 __u64 window_begin)
{
	...
	data->weighted_qd = 0;
	memset(&data->tp, 0, sizeof(struct mpbe_throughput));
	memset(&data->req, 0, sizeof(struct mpbe_req));
}

static struct mpbe_mictx *mpbe_mictx_find(struct mpbe *btag,
						  __s8 id)
{
	struct mpbe_mictx *mictx;
...
void mpbe_mictx_check_window(struct mpbe_mictx_id mictx_id)
{
	...
	btag = mpbe_find_by_type(mictx_id.storage);
	if (!btag)
		return;

	rcu_read_lock();
	mictx = mpbe_mictx_find(btag, mictx_id.id);
	if (!mictx) {
		rcu_read_unlock();
		return;
	}
	...
	for (idx = 0; idx < btag->ctx.count; idx++) {
		...
		spin_lock_irqsave(&data[idx].lock, flags);
		/* To do: check if this is necessary, or mpbe_mictx_reset()
		 *        only in mpbe_mictx_get_data() is enough?
		 */
		if ((t_cur - data[idx].window_begin) > MICTX_RESET_NS)
			mpbe_mictx_reset(data + idx, t_cur);
		spin_unlock_irqrestore(&data[idx].lock, flags);
	}
	rcu_read_unlock();
}

static __u32 mpbe_eval_tp_speed(__u32 bytes, __u64 duration)
{
	__u32 speed_kbs = 0;
...
int mpbe_mictx_get_data(struct mpbe_mictx_id mictx_id,
			    struct mpbe_mictx_iostat_struct *iostat)
{
	...
	*/
	...
	memset(iostat, 0, sizeof(struct mpbe_mictx_iostat_struct));
	for (i = 0; i < btag->ctx.count; i++) {
		struct mpbe_mictx_data tmp_data, *data = mictx->data;
		__u64 tp_dur;

		spin_lock_irqsave(&data[i].lock, flags);
		memcpy(&tmp_data, &data[i], sizeof(struct mpbe_mictx_data));
		mpbe_mictx_reset(data + i, time_cur);
		...
		*/
		...
		iostat->tp_req_r += mpbe_eval_tp_speed(
			tmp_data.tp.r.size, tmp_data.tp.r.usage);
		iostat->tp_req_w += mpbe_eval_tp_speed(
		...
		iostat->tp_all_r += mpbe_eval_tp_speed(
			tmp_data.tp.r.size, tp_dur);
		iostat->tp_all_w += mpbe_eval_tp_speed(
	...
	*/
	...
	/* To do: protect multi-threads call mpbe_mictx_get_data */
...
EXPORT_SYMBOL_GPL(mpbe_mictx_get_data);

static int mpbe_mictx_alloc(enum mpbe_storage_type type)
{
	struct mpbe *btag;
	struct mpbe_mictx *mictx;
	struct mpbe_mictx_data *data;
	unsigned long flags;
	__u64 time = sched_clock();
	int i;

	btag = mpbe_find_by_type(type);
	if (!btag)
		return -1;

	mictx = kzalloc(sizeof(struct mpbe_mictx), GFP_NOFS);
	if (!mictx)
		return -1;

	data = kcalloc(btag->ctx.count, sizeof(struct mpbe_mictx_data),
		       GFP_NOFS);
	if (!data) {
		kfree(mictx);
		return -1;
...
static void mpbe_mictx_free(struct mpbe_mictx_id *mictx_id)
{
	struct mpbe *btag;
	struct mpbe_mictx *mictx;
	unsigned long flags;
...
{
	if (enable)
		mictx_id->id = mpbe_mictx_alloc(mictx_id->storage);
	else
		mpbe_mictx_free(mictx_id);
}
EXPORT_SYMBOL_GPL(mpbe_mictx_enable);
