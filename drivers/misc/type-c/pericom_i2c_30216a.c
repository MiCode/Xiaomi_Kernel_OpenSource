...
static irqreturn_t pericom_30216a_irq_handler(int irq, void *dev_id)
{
	...
	if ((reg[2] == 0x2) || (reg[3] == 0x00) || (reg[3] == 0x80)) {
		...
		if ((reg[2] == 0x2) && (reg[3] == 0x00))
			plug_flag = false;
		curr_mode = DRP_MODE << PERICOM_ROLE_OFFSET; /*enter drp mode*/
	} else if (reg[3] == 0x04) {
		...
		if ((reg[3] == 0x05) || (reg[3] == 0x06)) { /* Attached port status:device*/
			if (plug_flag == false) {
				...
				if (reg[3] & 0x08)
					curr_mode = DEVICE_MODE << PERICOM_ROLE_OFFSET; /*enter device mode*/
				else
					curr_mode = DRP_MODE << PERICOM_ROLE_OFFSET; /*enter drp mode*/
			}
		}
...
err_fs:
	free_irq(unsigned int irq, void *dev_id)(pericom_data->irq, pericom_data);
