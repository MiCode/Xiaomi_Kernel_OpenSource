		...
		const struct i2c_device_id *dev_id)
{
	...
	/*1. Set current mode detect  register */
	ti_tusb320_i2c_read(tusb320_data, TI_CURRENT_MODE_REG, &buf, 1);
	buf = (buf & ~TI_CURRENT_MODE_MASK)|(TI_CURRENT_MODE_VAL);
	ret = ti_tusb320_i2c_write(tusb320_data, TI_CURRENT_MODE_REG, &buf, 1);
	if (ret < 0)
		dev_err(&tusb320_data->i2c_client->dev, "set TI_CURRENT_MODE_REG fail\n");
