...
int wl2866d_camera_power_up(int out_iotype, int out_delay)
{
	...
	case OUT_DVDD1:
		//front imx596 DVDD
		mutex_lock(&wl2866d_mutex);
		ret = wl2866d_i2c_write(camera_chip, wl2866d_on_config[OUT_DVDD1].reg, 0x54);//bit0
		if (ret < 0) {
			mutex_unlock(&wl2866d_mutex);
	...
	case OUT_DVDD2:
 		//ultra ov8856 DVDD
		mutex_lock(&wl2866d_mutex);
		ret = wl2866d_i2c_write(camera_chip, wl2866d_on_config[OUT_DVDD2].reg, 0x64);//bit1
		if (ret < 0) {
			mutex_unlock(&wl2866d_mutex);
	...
	case OUT_AVDD1:
		...
		mutex_lock(&wl2866d_mutex);
		ret = wl2866d_i2c_write(camera_chip, wl2866d_on_config[OUT_AVDD1].reg, 0x30);//bit2
		if (ret < 0) {
			mutex_unlock(&wl2866d_mutex);
	...
	case OUT_AVDD2:
		//macro gc02m1 IOVDD
		mutex_lock(&wl2866d_mutex);
		ret = wl2866d_i2c_write(camera_chip, wl2866d_on_config[OUT_AVDD2].reg, 0x30);//bit3
		if (ret < 0) {
			mutex_unlock(&wl2866d_mutex);
			pr_err("wl2866d set avdd1 failed\n");
			break;
