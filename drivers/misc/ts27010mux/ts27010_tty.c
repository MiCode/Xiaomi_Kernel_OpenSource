...
static struct ts27010_tty_channel_data {
	atomic_t ref_count;
	struct tty_struct *tty;
};

static struct ts27010_tty_data {
			  ...
			  int data_idx, int len)
{
	...
	direct_ptr = ts27010_ringbuf_direct_ptr(rbuf, data_idx, len);
	if ((len <= 0) || (direct_ptr == NULL)) {
		pr_err("ts27010_tty_send_rbuf len=%d direct_ptr=%d\n", len, direct_ptr);
		return 0;
	}

	disc = tty_ldisc_ref(tty);
	if (disc == NULL) { /*  !TTY_LDISC */
		pr_err("ts27010_tty_send_rbuf disc is NULL\n");
		return 0;
	}
	disc->ops->receive_buf(tty, direct_ptr, NULL, len);
