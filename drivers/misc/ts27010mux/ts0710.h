...
struct ts0710_pkt_frame {
	struct list_head list;
	u8 data[0];
};

struct ts0710_pkt_queue {
	struct ts0710_con *con;
	spinlock_t lock;
	struct mutex mutex;
	struct list_head pkt_frame_list;
	struct task_struct *pkt_send_task;
	wait_queue_head_t pkt_send_wq;
	wait_queue_head_t pkt_send_finish_wq;
	int pkt_send_exit;
	struct completion pkt_send_exit_notifier;
	int suspend_count;
};
