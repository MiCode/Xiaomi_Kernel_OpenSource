...
static int sar_testing_mode_probe(struct platform_device *pdev)
{
	...
	for ( i=0; i<ant_data->count; i++) {
		...
		ret = devm_request_threaded_irq(dev, ant_data->data[i].irq, NULL,
			sar_testing_threaded_irq_handler, IRQF_ONESHOT | IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "sar_testing_mode", ant_data);
		if (ret < 0) {
...
MODULE_DESCRIPTION("A simple driver for GPIO testing mode for sar controller");
MODULE_LICENSE("GPL");
