...
static atomic_t mimdtrigger_type = ATOMIC_INIT(-1);
...
static ssize_t mimdtrigger_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	err = kstrtoint(buf, 10, &trigger_type);
	if (err != 0)
		return err;

	atomic_set(&mimdtrigger_type, trigger_type);
...
static int __init perf_helper_init(void)
{
	...
	if (mimd_kobj)
		sysfs_create_group(mimd_kobj, &mimd_attr_group);
