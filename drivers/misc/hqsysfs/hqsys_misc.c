...
ssize_t hq_emmcinfo(char *buf)
{
	...
	Size_buf >>= 1; /*witch to KB */


	count = snprintf(buf, strlen(buf), "%dGB", round_kbytes_to_readable_mbytes((unsigned int)Size_buf)/1024);
	...
	case MISC_RAM_SIZE:
			{
				...
				if (round_kbytes_to_readable_mbytes(K(i.totalram)) >= 1024) {
					count = snprintf(buf, strlen(buf), "%dGB", round_kbytes_to_readable_mbytes(K(i.totalram))/1024);
				} else {
					count = snprintf(buf, strlen(buf), "%dMB", round_kbytes_to_readable_mbytes(K(i.totalram)));
				}
	...
	case MISC_OTP_SN:
#ifdef CONFIG_MTK_EMMC_SUPPORT_OTP
			{
				...
				if (0 == result) {
					...
					for (i = 0; i < SN_LEN; i++) {
						if (!isalnum(temp[i])) {
							count = snprintf(buf, strlen(buf), "Not Valid SN\n");
							goto r_error;
						}
					}
					#endif
					count = snprintf(buf, strlen(buf), "%s", temp);
				} else {
					count = snprintf(buf, strlen(buf), "Read SN in OTP error %d\n", result);
				}
			}

#else
			count = snprintf(buf, strlen(buf), "SN in OTP not enabled\n");
#endif
			break;
		default:
			count = snprintf(buf, strlen(buf), "Not support");
			break;
	}
