...
struct pcba_info pcba[] = {
	...
	{PCBA_V1_CN, "PCBA_V1_CN"},
	{PCBA_V1_SA, "PCBA_V1_SA"},
	{PCBA_V1_GLOBAL, "PCBA_V1_GLOBAL"},
	{PCBA_V1_IN, "PCBA_V1_IN"},
	{PCBA_V2_CN, "PCBA_V2_CN"},
	{PCBA_V2_GLOBAL, "PCBA_V2_GLOBAL"},
	{PCBA_V2_IN, "PCBA_V2_IN"},
	{PCBA_E7_CN, "PCBA_E7_CN"},
	{PCBA_E7_IN, "PCBA_E7_IN"},
	{PCBA_E7_GLOBAL, "PCBA_E7_GLOBAL"}
...
static ssize_t huaqin_show(struct kobject *kobj, struct attribute *a, char *buf)
{
	...
	if (NULL == hw) {
		return snprintf(buf, strlen(buf), "Data error\n");
	}

	if (HWID_VER == hw->hw_id) {
		count = snprintf(buf, strlen(buf), "%s\n", HQ_SYS_FS_VER);
		...
		while (attr) {
			...
			if (curent_hw->hw_exist && (NULL != curent_hw->hw_device_name)) {
				count += snprintf(buf+count, strlen(buf+count), "%s: %s\n" , curent_hw->attr.name, curent_hw->hw_device_name);
			}
		}
		...
		if (0 == hw->hw_exist) {
			...
			count = snprintf(buf, strlen(buf), "Installed with no device Name\n");
		} else{
			count = snprintf(buf, strlen(buf), "%s\n" , hw->hw_device_name);
		}
	}
