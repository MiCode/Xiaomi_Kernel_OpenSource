...
int Set_ResetStatCounter_Proc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, ": ATE_AGENT iwpriv Set_ResetStatCounter_Proc\n");
...
int SetATEDa(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 6) {
		DBGLOG(RFTEST, ERROR, ": ATE_AGENT iwpriv SetATEDa Sa:%02x:%02x:%02x:%02x:%02x:%02x\n",
...
int SetATESa(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x", &addr[0], &addr[1], &addr[2], &addr[3], &addr[4], &addr[5]);
	if (rv == 6) {
		DBGLOG(RFTEST, ERROR, ": ATE_AGENT iwpriv SetATESa Sa:%02x:%02x:%02x:%02x:%02x:%02x\n",
		       addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]);
...
int Set_TxBfProfileTag_InValid(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTag_InValid prInBuf = %s, ucInValid = %d\n", prInBuf,
		       ucInValid);
...
int Set_TxBfProfileTag_PfmuIdx(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTag_PfmuIdx prInBuf = %s, ucProfileIdx = %d\n", prInBuf,
		       ucProfileIdx);
...
int Set_TxBfProfileTag_BfType(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTag_BfType prInBuf = %s, ucBFType = %d\n",
			prInBuf, ucBFType);
...
int Set_TxBfProfileTag_DBW(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTag_DBW prInBuf = %s, ucBW = %d\n", prInBuf, ucBW);
...
int Set_TxBfProfileTag_SuMu(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTag_SuMu prInBuf = %s, ucSuMu = %d\n", prInBuf, ucSuMu);
...
int Set_TxBfProfileTag_SNR(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 4) {
		DBGLOG(RFTEST, ERROR, ": ATE_AGENT iwpriv Set_TxBfProfileTag_SNR aucInput:%d:%d:%d:%d\n",
...
int Set_TxBfProfileTag_SmartAnt(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTag_SmartAnt prInBuf = %s, ucSmartAnt = %d\n", prInBuf,
		       ucSmartAnt);
...
int Set_TxBfProfileTag_SeIdx(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "TxBfProfileTag_SeIdx prInBuf = %s, ucSeIdx = %d\n", prInBuf, ucSeIdx);
...
int Set_TxBfProfileTag_RmsdThrd(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTag_RmsdThrd prInBuf = %s, ucRmsdThrd = %d\n", prInBuf,
		       ucRmsdThrd);
...
int Set_TxBfProfileTag_TimeOut(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTag_TimeOut prInBuf = %s, ucTimeOut = %d\n", prInBuf,
		       ucTimeOut);
...
int Set_TxBfProfileTag_DesiredBW(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTag_DesiredBW prInBuf = %s, ucDesiredBW = %d\n", prInBuf,
		       ucDesiredBW);
...
int Set_TxBfProfileTag_DesiredNc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTag_DesiredNc prInBuf = %s, ucDesiredNc = %d\n", prInBuf,
		       ucDesiredNc);
...
int Set_TxBfProfileTag_DesiredNr(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTag_DesiredNr prInBuf = %s, ucDesiredNr = %d\n", prInBuf,
		       ucDesiredNr);
...
int Set_TxBfProfileTagWrite(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfileTagWrite prInBuf = %s, profileIdx = %d\n", prInBuf,
		       profileIdx);
...
int Set_TxBfProfilePnRead(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	rv = kstrtoint(prInBuf, 0, &profileIdx);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_TxBfProfilePnRead prInBuf = %s, profileIdx = %d\n",
			prInBuf, profileIdx);
...
int Set_TxBfProfilePnWrite(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%u:%hu:%hu:%hu:%hu:%hu:%hu:%hu:%hu:%hu:%hu:%hu:%hu:%hu",
	...
	DBGLOG(RFTEST, ERROR,
	       "prInBuf=%s,Idx=%u,u2bw=%d, au2XSTS: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
	       prInBuf, u4ProfileIdx, u2bw, au2XSTS[0], au2XSTS[1], au2XSTS[2], au2XSTS[3], au2XSTS[4], au2XSTS[5],
	       au2XSTS[6], au2XSTS[7], au2XSTS[8], au2XSTS[9], au2XSTS[10], au2XSTS[11]);

	return TxBfProfilePnWrite(prNetDev, u4ProfileIdx, u2bw, au2XSTS);
}
...
int Set_TxBfPfmuMemAlloc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 2) {
		DBGLOG(RFTEST, ERROR, "TxBfPfmuMemAlloc ucSuMuMode = %d, ucWlanIdx = %d", ucSuMuMode, ucWlanIdx);
...
int Set_TxBfPfmuMemRelease(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "TxBfPfmuMemRelease ucWlanId = %d", ucWlanId);
...
int Set_StaRecBfUpdate(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 23) {
		/* DBGLOG(RFTEST, ERROR,
		 * "Set_StaRecBfUpdate prInBuf = %s, u4WlanId = %x, u4BssId = %x, u4Aid = %x,
...
int Set_MUGetInitMCS(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		...
		DBGLOG(RFTEST, ERROR, "Set_MUGetInitMCS prInBuf = %s, u4groupIdx = %x", prInBuf, u4groupIdx);
...
int Set_MUGetLQ(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	/* { */
	/* DBGLOG(RFTEST, ERROR, "Set_MUGetLQ prInBuf = %s, u4Type = %x", prInBuf, u4Type); */
...
int Set_MUSetSNROffset(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_MUSetSNROffset prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
int Set_MUSetZeroNss(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_MUSetZeroNss prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
int Set_MUSetSpeedUpLQ(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	rv = kstrtouint(prInBuf, 0, &u4Val);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_MUSetSpeedUpLQ prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
int Set_MUGetQD(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	if (sscanf(prInBuf, "%x:%x", &u4SubcarrierIndex, &u4Length) == 2) {
		DBGLOG(RFTEST, ERROR, "Set_MUGetQD prInBuf = %s, u4SubcarrierIndex = %x, u4Length = %x", prInBuf,
...
int Set_MUSetEnable(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	rv = kstrtouint(prInBuf, 0, &u4Val);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "Set_MUSetEnable prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
int WriteEfuse(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "ATE_AGENT iwpriv WriteEfuse, prInBuf: %s\n", prInBuf);
	DBGLOG(INIT, ERROR, "ATE_AGENT iwpriv WriteEfuse :%02x:%02x\n", addr[0], addr[1]);
