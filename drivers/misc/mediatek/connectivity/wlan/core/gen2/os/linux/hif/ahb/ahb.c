...
static VOID HifAhbLoopbkAuto(struct timer_list *timer);
#else
...
VOID glSetHifInfo(GLUE_INFO_T *GlueInfo, ULONG ulCookie)
{
	...
	{
#if KERNEL_VERSION(4, 15, 0) <= CFG80211_VERSION_CODE
		timer_setup(&HifInfo->HifTmrLoopbkFn, HifAhbLoopbkAuto, 0);
#else
...
kalDevPortRead(IN P_GLUE_INFO_T GlueInfo, IN UINT_16 Port, IN UINT_32 Size, OUT PUINT_8 Buf, IN UINT_32 MaxBufSize)
{
	...
	if ((HifInfo->fgDmaEnable == TRUE) && (HifInfo->DmaOps != NULL)
		...
		if (HifInfo->fgDmaUsleepEnable && GlueInfo->fgIsFwDlDone) {
...
{
#if KERNEL_VERSION(4, 15, 0) <= CFG80211_VERSION_CODE
	GL_HIF_INFO_T *HifInfo = from_timer(HifInfo, timer, HifTmrLoopbkFn);
#else
