	...
	*pPkt = ucDialogToken;
	...
	if (prAdapter->rWifiVar.fgSupportQoS)
	{
		u4IeLen = mqmGenerateWmmParamIEByParam(\
							prAdapter, prBssInfo, pPkt, OP_MODE_INFRASTRUCTURE);
		LR_TDLS_FME_FIELD_FILL(u4IeLen);
	}
...
/*----------------------------------------------------------------------------*/
/*! \brief  This routine is called to display TDLS related informations.
*
* \param[in] prGlueInfo Pointer to the Adapter structure
* \param[in] prInBuf A pointer to the command string buffer
* \param[in] u4InBufLen The length of the buffer
...
/*----------------------------------------------------------------------------*/
/*! \brief  This routine is called to display key related informations.
*
* \param[in] prGlueInfo Pointer to the Adapter structure
* \param[in] prInBuf A pointer to the command string buffer
* \param[in] u4InBufLen The length of the buffer
	...
	)
{
	...
	if (ucActionId == TDLS_FRM_ACTION_TEARDOWN)
	{
		DBGLOG(TDLS, WARN,
			...
			__FUNCTION__, *(pPkt+13+4))); /* reason code */
