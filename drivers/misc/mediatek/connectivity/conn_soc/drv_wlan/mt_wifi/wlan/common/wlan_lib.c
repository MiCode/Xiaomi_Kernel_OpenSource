...
#endif
    wlanCfgSet(prAdapter, "TxErrReport","Y", 0);
...
#define STR2BYTE(s) (((((PUINT_8)s)[0]-'0')*10)+(((PUINT_8)s)[1]-'0'))
	...
	P_CMD_TX_PARAM_T prTxParam = &prRegInfo->rTxParam; //TxParam
    P_CMD_PS_INTERVAL_PARAM_T prPsIntervalParam = &prRegInfo->rPsIntervalParam; //PsIntervalParam
    ...
    //TxParam
    wlanCfgApplyTxParam(prAdapter, prTxParam);

    //PsIntervalParam
    wlanCfgApplyPsIntervalParam(prAdapter, prPsIntervalParam);
    ...
    if(wlanCfgGet(prAdapter, "TxErrReport", aucValue, "",0) == WLAN_STATUS_SUCCESS) {
        printk(KERN_ERR "CFG_FILE: TxErr Key, Vaule=%s\n", aucValue);	
        prAdapter->fgIsRoamingTxErrReport = (*aucValue == 'Y') ? 1:0;
        printk(KERN_ERR "CFG_FILE: TxErr Key, prAdapter->fgIsRoamingTxErrReport=%d\n", prAdapter->fgIsRoamingTxErrReport);
    }
...
wlanCfgApplyTxParam(
    IN P_ADAPTER_T prAdapter,
    IN P_CMD_TX_PARAM_T prTxParam
    )
{
	//TxParam
	prTxParam->fgForceTxParamEnable = wlanCfgGetUint32(prAdapter, "TX_PARAM_EN", 0);
	prTxParam->u2TxOP = wlanCfgGetUint32(prAdapter, "TXOP", 65535);
	prTxParam->u2CwMax = wlanCfgGetUint32(prAdapter, "CW_MAX", 65535);
	prTxParam->u2CwMin = wlanCfgGetUint32(prAdapter, "CW_MIN", 65535);
	prTxParam->u2Aifsn = wlanCfgGetUint32(prAdapter, "AIFSN", 65535);
	prTxParam->u2AmpduNum = wlanCfgGetUint32(prAdapter, "AMPDU_NUM", 65535);
	printk("//TxParamTx param Enable=%d, TxOP=%d, CwMax=%d, CwMin=%d, Aifsn=%d, AmpduNum=%d\n", 
		prTxParam->fgForceTxParamEnable,
		prTxParam->u2TxOP, 
		prTxParam->u2CwMax,
	    prTxParam->u2CwMin,
	    prTxParam->u2Aifsn,
	    prTxParam->u2AmpduNum);
}

VOID
wlanCfgApplyPsIntervalParam(
    IN P_ADAPTER_T prAdapter,
    IN P_CMD_PS_INTERVAL_PARAM_T prPsIntervalParam
    )
{
    //PsIntervalParam
	prPsIntervalParam->fgPsIntvEnable = wlanCfgGetUint32(prAdapter, "PsIntvEn", 0);
	prPsIntervalParam->u2PsModeSwIntv = wlanCfgGetUint32(prAdapter, "PsInt", 0xFFFF);
	prPsIntervalParam->u2ActModeSwIntv = wlanCfgGetUint32(prAdapter, "ActInt", 0xFFFF);
	prPsIntervalParam->u2TxThrSwToAct = wlanCfgGetUint32(prAdapter, "TxThrToAct", 0xFFFF);
	prPsIntervalParam->u2TxThrSwToPs = wlanCfgGetUint32(prAdapter, "TxThrToPs", 0xFFFF);
	prPsIntervalParam->u2RxThrSwToAct = wlanCfgGetUint32(prAdapter, "RxThrToAct", 0xFFFF);
	prPsIntervalParam->u2RxThrSwToPs = wlanCfgGetUint32(prAdapter, "RxThrToPs", 0xFFFF);
	prPsIntervalParam->u4ConsecuteRxThrSwToAct = wlanCfgGetUint32(prAdapter, "ContRxThrToAct", 0xFFFF);
	printk("//PsIntervalParam Enable=%d, PsSwIntv=%d, ActSwIntv=%d, TxToAct=%d, TxToPs=%d, RxToAct=%d, RxToPs=%d\n", 
	prPsIntervalParam->fgPsIntvEnable,
	prPsIntervalParam->u2PsModeSwIntv,
	prPsIntervalParam->u2ActModeSwIntv,
	prPsIntervalParam->u2TxThrSwToAct,
	prPsIntervalParam->u2TxThrSwToPs,
	prPsIntervalParam->u2RxThrSwToAct,
	prPsIntervalParam->u2RxThrSwToPs);
}
