    ...
    )
{
    ...
    if(prScanParam->eNetTypeIndex == NETWORK_TYPE_P2P_INDEX) {
	    ...
	    if(prScanParam->eNetTypeIndex == NETWORK_TYPE_AIS_INDEX) {
	        printk(KERN_ERR "SCN_FAST: ScanReqV1 DwellTime=%dms\n", CFG_FAST_SCAN_DWELL_TIME);
    ...
    )
{
	...
	if(prScanParam->ucChannelListNum > 32 && prScanParam->eScanChannel == SCAN_CHANNEL_SPECIFIED) {
	    ...
	    if (prScanParam->eScanChannel == SCAN_CHANNEL_SPECIFIED) {
			...
			rCmdScanReq.ucChannelListNum = 0;
	        for(i = 0 ; i < 32 ; i++) {
	            rCmdScanReq.arChannelList[i].ucBand = 0;
	            rCmdScanReq.arChannelList[i].ucChannelNum = 0;
	        }
		}
    ...
    )
{
    ...
    if (prScanParam->eScanChannel == SCAN_CHANNEL_SPECIFIED) {
		...
		rCmdScanReq.ucChannelListNum = 0;
        for(i = 0 ; i < MAXIMUM_OPERATION_CHANNEL_LIST; i++) {
            rCmdScanReq.arChannelList[i].ucBand = 0;
            rCmdScanReq.arChannelList[i].ucChannelNum = 0;
        }
    }
