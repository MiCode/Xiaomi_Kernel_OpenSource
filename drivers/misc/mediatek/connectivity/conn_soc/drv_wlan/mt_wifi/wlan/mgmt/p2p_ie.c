        ...
        for (i = 0; i < u4AttriTableSize; i++) {
            ...
            if (arAppendAttriTable[i].pfnAppendAttri) {
                ...
                if (u4OverallAttriLen > P2P_MAXIMUM_ATTRIBUTE_LEN) {
                    ...
                    pucBuffer = (PUINT_8)((ULONG)prIeWFD + (WFD_IE_OUI_HDR + P2P_MAXIMUM_ATTRIBUTE_LEN));

                    prIeWFD = (P_IE_WFD_T)((ULONG)prIeWFD + (WFD_IE_OUI_HDR + P2P_MAXIMUM_ATTRIBUTE_LEN));
    ...
    )
{
    ...
    p2pFuncComposeNoaAttribute(prAdapter, prMsduInfo/*prMsduInfo->ucBssIndex, prIeP2P->aucP2PAttributes, &u4AttributeLen*/);
