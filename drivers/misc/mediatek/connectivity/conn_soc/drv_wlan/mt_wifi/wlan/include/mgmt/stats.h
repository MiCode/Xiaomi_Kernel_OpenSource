...
/* for TX path log */
#define STATS_LOG_FUNC_TX					BIT(0)
#define STATS_LOG_FUNC_VAL1_MODE			BIT(6)
#define STATS_LOG_FUNC_VAL2_MODE			BIT(7)

typedef struct _STATS_TX_LOG_T {
	UINT_8		ucMode; /* reference to STATS_LOG_FUNC_TX */
	UINT_8		ucFuncId; /* reference to STATS_TX_LOG_FUNC */
	UINT_16		u2FuncPt; /* EX: reference to STATS_TX_LOG_POINT_SEND_MNG_PACKETS */
	UINT_32		u4Val1;
	UINT_32		u4Val2;
} STATS_TX_LOG_T;

#if (CFG_SUPPORT_STATISTICS == 1)
#define STATS_LOG_TX_REQ(__Adapter__)										\
do {																		\
	STATS_CMD_CORE_T __rCmd;												\
	statsInfoTxLogRequest(__Adapter__, &__rCmd, 0, NULL);					\
} while(0)
#else
