...
** [BORA00002222] MT6630 unified MAC RXM
** change the sequence to retrive SSN by group4.
**
** 08 06 2013 terry.wu
** [BORA00002207] [MT6630 Wi-Fi] TXM & MQM Implementation
		...
		if (prStaRec->fgIsTxAllowed) {
			...
			while (!QUEUE_IS_EMPTY(prCurrQueue)) {
				...
				if ((u4CurStaForwardFrameCount >= u4MaxForwardFrameCountLimit) ||
                    ...
                    /* Avaliable Resource is not enough */
...
* \param[in] ucMaxNum The maximum amount of availiable resource
*
* \return (none)
*/
/*----------------------------------------------------------------------------*/
	...
	do {
        ...
        if (prCurrSwRfb->fgReorderBuffer && !fgIsBMC && fgIsHTran) {
            ...
            */
			...
			/* Check Class Error */
            {
				if (fgIsHTran && (secCheckClassError
                   ...
                   DBGLOG(QM, WARN, ("Mark NULL the Packet for class error or fgIsHTran (%d) is false\n", fgIsHTran));
                   prCurrSwRfb->eDst = RX_PKT_DESTINATION_NULL;
					QUEUE_INSERT_TAIL(prReturnedQue, (P_QUE_ENTRY_T) prCurrSwRfb);
               }
            }
    ...
    IN ENUM_FRAME_TYPE_IN_CMD_Q_T eFrameType, IN UINT_16 u2FrameLength)
{
    ...
    if(eFrameType == FRAME_TYPE_MMPDU) {
        ...
        if(u2FreeResource < ucReqResource) {
            eFrameAction = FRAME_ACTION_QUEUE_PKT;
			DBGLOG(QM, INFO, ("Queue MGMT (MSDU[0x%p] "
                "Req/Rsv/Free[%u/%u/%u])\n", prMsduInfo, 
...
VOID qmDumpQueueStatus(IN P_ADAPTER_T prAdapter)
{
    ...
    for(i = TC0_INDEX; i < TC_NUM; i++) {
        DBGLOG(SW4, INFO, ("TC%u ResCount: Max[%02u/%03u] Free[%02u/%03u] "
            ...
            prTxCtrl->rTc.au2PreUsedPageCount[i]));
...
#if QM_ADAPTIVE_TC_RESOURCE_CTRL
    for(i = TC0_INDEX; i < TC_NUM; i++) {
        DBGLOG(SW4, INFO, ("TC%u AvgQLen[%04u] minRsv[%02u] CurTcRes[%02u] "
    ...
    DBGLOG(SW4, INFO,("QueLenMovingAvg[%u] Time2AdjResource[%u] "
			 ...
			 IN P_RX_BA_ENTRY_T prRxBaEntry, IN ENUM_BA_ENTRY_STATUS_T eStatus)
{
    ...
    case BA_ENTRY_STATUS_ACTIVE:
		...
		if (!MQM_CHECK_FLAG(prAdapter->u4FlagBitmap, MQM_FLAG_IDLE_RX_BA_TIMER_STARTED)) {
			cnmTimerInitTimer(prAdapter, &prAdapter->rMqmIdleRxBaDetectionTimer, (PFN_MGMT_TIMEOUT_FUNC) mqmTimeoutCheckIdleRxBa, (ULONG) NULL);	/* No parameter */
        ...
        */
		...
		if (fgIsNewEntryAdded) {	/* Only when a new BA entry is indeed added will the host be notified */
			...
			if (!prSwRfbEventToHost) {
                ...
                /* Note: DELBA will not be sent since cnmMgtPktAlloc() may fail again. However, it does not
                *  matter because upon receipt of AMPDUs without a RX BA agreement, MQM will send DELBA frames
                */
	...
	if (arpOpCode == ARP_PRO_RSP) {
		...
		if (prAdapter->prAisBssInfo &&
			...
			if (EQUAL_MAC_ADDR(&(pucData[ETH_TYPE_LEN_OFFSET + 10]), /* source hardware address */
					prAdapter->prAisBssInfo->prStaRecOfAP->aucMacAddr)) {
				kalMemCopy(apIp, &(pucData[ETH_TYPE_LEN_OFFSET + 16]), sizeof(apIp)); /* source ip address */
