...
P_BSS_DESC_T scanAddToBssDesc(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if(!prSwRfb->prRxStatusGroup3){
		DBGLOG(SCN, WARN, ("prRxStatusGroup3 NULL, group vld=%d\n", prSwRfb->ucGroupVLD));
		return NULL;
	}
		...
		if (prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED &&
			...
			if (rsnCheckSecurityModeChanged(prAdapter, prAisBssInfo, prBssDesc)
				...
				if (!prConnSettings->fgSecModeChangeStartTimer) {
					cnmTimerStartTimer(prAdapter, &prAdapter->rWifiVar.rAisFsmInfo.rSecModeChangeTimer,
							SEC_TO_MSEC(3));
	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
		...
		if (!prCandidateBssDesc) {
			...
			if (fgIsFindBestRSSI) {
				...
				DBGLOG(SCN, TRACE, ("Candidate [" MACSTR "]: RCPI=%d, RSSI=%d, joinFailCnt=%d, Primary [" MACSTR
					"]: RCPI=%d, RSSI=%d, joinFailCnt=%d\n", 
							MAC2STR(prCandidateBssDesc->aucBSSID),
							prCandidateBssDesc->ucRCPI, u4CandAdjRssi, prCandidateBssDesc->ucJoinFailureCount,
							MAC2STR(prPrimaryBssDesc->aucBSSID),
							prPrimaryBssDesc->ucRCPI, u4PrimAdjRssi, prPrimaryBssDesc->ucJoinFailureCount));
