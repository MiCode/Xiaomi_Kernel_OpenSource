			     ...
			     bool pairwise, const u8 *mac_addr, struct key_params *params)
{
	...
	rStatus = kalIoctl(prGlueInfo,
			   wlanoidSetAddP2PKey,
			   &rKey, rKey.u4Length, FALSE, FALSE, TRUE, TRUE, &u4BufLen);
...
int
mtk_p2p_cfg80211_scan(struct wiphy *wiphy,
...
/* P_IE_SSID_T prSsidIE = (P_IE_SSID_T)NULL; */
	...
	do {
		...
		if (settings->beacon.head_len != 0) {
			...
			pucBuffer =
			    (PUINT_8) ((UINT_32) pucBuffer + (UINT_32) settings->beacon.head_len);
		} else {
				   ...
				   unsigned int duration, u64 *cookie)
{
	...
	do {
		if ((wiphy == NULL) || (dev == NULL) || (chan == NULL) || (cookie == NULL)) {
			break;
		}
...
int
mtk_p2p_cfg80211_mgmt_tx(struct wiphy *wiphy, struct net_device *dev,
			 ...
			 u64 *cookie)
{
	...
	do {
		if ((wiphy == NULL) ||
		    (buf == NULL) || (len == 0) || (dev == NULL) || (cookie == NULL)) {
			break;
		}
...
int mtk_p2p_cfg80211_testmode_cmd(IN struct wiphy *wiphy, IN void *data, IN int len)
{
	...
	if (prParams) {
		...
		case 3:	/* Hotspot Client Management */
			if (mtk_p2p_cfg80211_testmode_hotspot_block_cmd(wiphy, data, len))
				fgIsValid = TRUE;
			break;
		default:
			fgIsValid = TRUE;
