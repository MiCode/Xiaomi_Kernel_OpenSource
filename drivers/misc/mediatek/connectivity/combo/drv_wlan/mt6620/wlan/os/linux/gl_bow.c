...
static ssize_t
mt6620_ampc_read(IN struct file *filp, IN char __user *buf, IN size_t size, IN OUT loff_t *ppos);

static ssize_t
mt6620_ampc_write(IN struct file *filp,
...
/* g_u4CurrentSysTime = (OS_SYSTIME)kalGetTimeTick(); */
	...
	g_arBowRevPalPacketTime[i]++; g_u4PrevSysTime = g_u4CurrentSysTime;
#endif
...
/*----------------------------------------------------------------------------*/
BOOLEAN
kalInitBowDevice(IN P_GLUE_INFO_T prGlueInfo, IN const char *prDevName)
{
...
BOOLEAN kalUninitBowDevice(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	if (prGlueInfo->rBowInfo.fgIsNetRegistered == TRUE) {
		...
		prGlueInfo->rBowInfo.prDevHandler = NULL; return TRUE;
	} else {
		return FALSE;
	}
}
