		...
		case SCAN_CHANNEL_SPECIFIED:
			{
				...
				for (u4Idx = 0; u4Idx < prScanReqInfo->ucNumChannelList; u4Idx++) {
					prScanReq->arChnlInfoList[u4Idx].ucChannelNum =
					    prDomainInfo->ucChannelNum;
					prScanReq->arChnlInfoList[u4Idx].eBand =
					    prDomainInfo->eBand;
					prDomainInfo++;
				}
		...
		case SCAN_CHANNEL_P2P_SOCIAL:
			{
				...
				/* For compatible. */
				if (EQUAL_SSID
				    (aucP2pSsid, P2P_WILDCARD_SSID_LEN, prScanReq->aucSSID,
				     prScanReq->ucSSIDLength)) {
		    ...
		    IN UINT_32 u4BodyLen, IN UINT_32 u4DtimPeriod, IN UINT_32 u4BcnInterval)
{
	...
	do {
		...
		if (pucBcnHdr) {
			...
			prBcnMsduInfo->ucMacHeaderLength = (WLAN_MAC_MGMT_HEADER_LEN + (TIMESTAMP_FIELD_LEN + BEACON_INTERVAL_FIELD_LEN + CAP_INFO_FIELD_LEN));
		...
		case OP_MODE_ACCESS_POINT:
			 ...
			 */
			{
				...
				while (!LINK_IS_EMPTY(prStaRecOfClientList)) {
					...
					p2pFuncDisconnect(prAdapter, prCurrStaRec, TRUE,
							  u2ReasonCode);
		   ...
		   IN P_MSDU_INFO_T prMgmtTxMsdu, IN UINT_64 u8Cookie)
{
	...
	do {
		...
		prStaRec =
		    cnmGetStaRecByAddress(prAdapter, NETWORK_TYPE_P2P_INDEX, prWlanHdr->aucAddr1);
		...
		if (prStaRec != NULL) {
			DBGLOG(P2P, TRACE,
			       ("Mgmt with station record: " MACSTR " .\n",
			    ...
			    IN P_STA_RECORD_T prStaRec, IN P_SW_RFB_T prAssocRspSwRfb)
{
	...
	COPY_SSID(prP2pBssInfo->aucSSID,
		  ...
		  prP2pConnSettings->aucSSID, prP2pConnSettings->ucSSIDLen);
		...
		*pu2StatusCode = STATUS_CODE_REQ_DECLINED;
		...
		if (prWfdCfgSettings->ucWfdEnable) {
			if (prWfdCfgSettings->u4WfdPolicy & BIT(6)) {
				...
				if ((prWfdCfgSettings->u4WfdPolicy & BIT(5))
				    && (prAttriWfdDevInfo != NULL)) {
					/* Rejected with WFD IE. */
					break;
				}

				if ((prWfdCfgSettings->u4WfdPolicy & BIT(0))
				    && (prAttriWfdDevInfo == NULL)) {
				...
				if (prAttriWfdDevInfo != NULL) {
					...
					WLAN_GET_FIELD_BE16(&prAttriWfdDevInfo->u2WfdDevInfo,
							    &u2WfdDevInfo);
					DBGLOG(P2P, INFO,
					       ("RX Assoc Req WFD Info:0x%x.\n", u2WfdDevInfo));

					if ((prWfdCfgSettings->u4WfdPolicy & BIT(1))
					    && ((u2WfdDevInfo & 0x3) == 0x0)) {
						/* Rejected because of SOURCE. */
						break;
					}

					if ((prWfdCfgSettings->u4WfdPolicy & BIT(2))
					    && ((u2WfdDevInfo & 0x3) == 0x1)) {
						/* Rejected because of Primary Sink. */
						break;
					}

					if ((prWfdCfgSettings->u4WfdPolicy & BIT(3))
					    && ((u2WfdDevInfo & 0x3) == 0x2)) {
						/* Rejected because of Secondary Sink. */
						break;
					}

					if ((prWfdCfgSettings->u4WfdPolicy & BIT(4))
					    && ((u2WfdDevInfo & 0x3) == 0x3)) {
					...
					if (prWfdCfgSettings->u4WfdFlag & WFD_FLAGS_DEV_INFO_VALID) {
						...
						if ((prWfdCfgSettings->u2WfdDevInfo & BITS(0, 1)) ==
						    0x3) {
							...
							p2pFsmRunEventWfdSettingUpdate(prAdapter,
										       NULL);
			...
			case ELEM_ID_SSID:	/* 0 *//* V *//* Done */
				{
					...
					/* SSID IE set in beacon from supplicant will not
					   always be the true since hidden SSID case */
					/*
					COPY_SSID(prP2pBssInfo->aucSSID,
						  prP2pBssInfo->ucSSIDLen,
			...
			case ELEM_ID_DS_PARAM_SET:	/* 3 *//* V *//* Done */
				{
					...
					ASSERT(prP2pConnSettings->ucOperatingChnl ==
...
VOID p2pFuncStoreAssocRspIEBuffer(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		kalMemCopy(prJoinInfo->aucIEBuf, prAssocRspFrame->aucInfoElem,
