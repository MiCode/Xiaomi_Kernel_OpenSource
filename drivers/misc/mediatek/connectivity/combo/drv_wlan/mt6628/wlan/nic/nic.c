...
WLAN_STATUS nicAllocateAdapterMemory(IN P_ADAPTER_T prAdapter)
{
	...
	do {
		...
		LOCAL_NIC_ALLOCATE_MEMORY(prAdapter->pucMgtBufCached,
					  ...
					  VIR_MEM_TYPE, "COMMON MGMT MEMORY POOL");
...
VOID nicReleaseAdapterMemory(IN P_ADAPTER_T prAdapter)
{
	...
	if (prAdapter->pucMgtBufCached) {
		kalMemFree((PVOID) prAdapter->pucMgtBufCached,
			   VIR_MEM_TYPE, prAdapter->u4MgtBufCachedSize);
		prAdapter->pucMgtBufCached = (PUINT_8) NULL;
	}
#if CFG_DBG_MGT_BUF
...
/* prAdapter->rWlanInfo.ePowerSaveMode.ucPsProfile = (UINT_8)ePwrMode; */
	...
	return wlanSendSetQueryCmd(prAdapter,
				   ...
				   (PUINT_8) & (prAdapter->rWlanInfo.
						arPowerSaveMode[eNetTypeIndex]), NULL,
...
VOID nicFreeScanResultIE(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Idx)
{
	...
	pucMovePivot =
	    (PUINT_8) ((UINT_32) (prAdapter->rWlanInfo.apucScanResultIEs[u4Idx]) + u4FreeSize);
	...
	for (i = 0; i < prAdapter->rWlanInfo.u4ScanResultNum; i++) {
		if (i != u4Idx) {
			if (prAdapter->rWlanInfo.apucScanResultIEs[i] >= pucMovePivot) {
				prAdapter->rWlanInfo.apucScanResultIEs[i] =
				    (PUINT_8) ((UINT_32) (prAdapter->rWlanInfo.apucScanResultIEs[i])
					       - u4FreeSize);
			}
		}
	}
		     ...
		     IN P_EVENT_LINK_QUALITY prEventLinkQuality)
{
	...
	switch (eNetTypeIdx) {
	case NETWORK_TYPE_AIS_INDEX:
		if (prAdapter->rWifiVar.arBssInfo[eNetTypeIdx].eConnectionState ==
			...
			if (prAdapter->fgIsLinkRateValid == FALSE
			    ...
			    CFG_LINK_QUALITY_VALID_PERIOD) {
				nicUpdateLinkSpeed(prAdapter, eNetTypeIdx,
						   prEventLinkQuality->u2LinkSpeed);
			}
		}
		break;
