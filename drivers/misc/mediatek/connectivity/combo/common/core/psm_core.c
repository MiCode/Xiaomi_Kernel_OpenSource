...
do { if (gPsmDbgLevel >= STP_PSM_LOG_LOUD)	\
		osal_dbg_print(PFX_PSM "%s: "  fmt, __func__ , ##arg);	\
} while (0)
#define STP_PSM_DBG_FUNC(fmt, arg...)	\
do { if (gPsmDbgLevel >= STP_PSM_LOG_DBG)	\
		osal_dbg_print(PFX_PSM "%s: "  fmt, __func__ , ##arg);	\
} while (0)
#define STP_PSM_INFO_FUNC(fmt, arg...)	\
do { if (gPsmDbgLevel >= STP_PSM_LOG_INFO)	\
		osal_print(PFX_PSM "[I]%s: "  fmt, __func__ , ##arg);	\
} while (0)
#define STP_PSM_WARN_FUNC(fmt, arg...)	\
do { if (gPsmDbgLevel >= STP_PSM_LOG_WARN)	\
		osal_print(PFX_PSM "[W]%s: "  fmt, __func__ , ##arg);	\
} while (0)
#define STP_PSM_ERR_FUNC(fmt, arg...)	\
do { if (gPsmDbgLevel >= STP_PSM_LOG_ERR)	\
		osal_print(PFX_PSM "[E]%s(%d):ERROR! "   fmt, __func__ , __LINE__, ##arg);	\
} while (0)
	...
	if (pOpQ == &stp_psm->rActiveOpQ) {
		...
		while (RB_COUNT(pOpQ) > 2) {
			...
			if ((prev_opId == STP_OPID_PSM_SLEEP
                ...
                RB_PUT(pFreeOpQ, pOp);
			}else if ((prev_opId == STP_OPID_PSM_WAKEUP && prev_prev_opId == STP_OPID_PSM_HOST_AWAKE)||
                (prev_opId == STP_OPID_PSM_HOST_AWAKE && prev_prev_opId == STP_OPID_PSM_WAKEUP))
			{
				STP_PSM_WARN_FUNC("prev_opId(%d), prev_prev_opId(%d)\n", prev_opId, prev_prev_opId);
				RB_GET(pOpQ, pOp);
				RB_PUT(pFreeOpQ, pOp);
			}
			else
        	{
        		STP_PSM_ERR_FUNC("prev_opId(%d), prev_prev_opId(%d), this should never happen!!!\n", prev_opId, prev_prev_opId);
				break;
        	}
        }
    }
...
INT32 _stp_psm_put_act_op(MTKSTP_PSM_T *stp_psm, P_OSAL_OP pOp)
{
	...
	if (bCleanup) {
		...
		if (bRet == 0) {
			STP_PSM_WARN_FUNC
			    ("+++++++++++ Put op active free fail, maybe disable/enable psm\n");
		}
	}
...
static inline INT32 _stp_psm_notify_wmt_sleep_wq(MTKSTP_PSM_T *stp_psm)
{
	...
	if (stp_psm == NULL) {
		...
		if (osal_test_bit
		    (STP_PSM_WMT_EVENT_DISABLE_MONITOR_TX_HIGH_DENSITY, &stp_psm->flag)) {
			return 0;
		}
#if PSM_USE_COUNT_PACKAGE

		if (osal_test_bit
		    (STP_PSM_WMT_EVENT_DISABLE_MONITOR_RX_HIGH_DENSITY, &stp_psm->flag)) {
			return 0;
		}
#endif
		if (osal_test_bit(STP_PSM_WMT_EVENT_DISABLE_MONITOR, &stp_psm->flag)) {
...
static inline INT32 _stp_psm_wait_wmt_event_wq(MTKSTP_PSM_T *stp_psm)
{
	...
	if (stp_psm == NULL) {
		...
		osal_wait_for_event_timeout(&stp_psm->wait_wmt_q,
					    _stp_psm_wait_wmt_event, (PVOID)stp_psm);
...
/* osal_unlock_unsleepable_lock(&stp_psm->flagSpinlock); */
				...
				_stp_psm_set_state(stp_psm, INACT);
				STP_PSM_INFO_FUNC
				    ("[WARNING]PSM state rollback due too wakeup fail\n");
			}
