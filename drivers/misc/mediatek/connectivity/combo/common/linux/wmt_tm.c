...
#define wmt_tm_dprint(fmt, args...)   \
do {                                    \
    if (wmt_tm_debug_log) {                \
	pr_warn(ANDROID_LOG_DEBUG, "Power/WMT_Thermal", fmt, ##args); \
    }                                   \
} while (0)

#define wmt_tm_print(fmt, args...)   \
do {                                    \
    pr_warn(ANDROID_LOG_DEBUG, "Power/WMT_Thermal", fmt, ##args); \
} while (0)
...
do {                                    \
    pr_warn(ANDROID_LOG_INFO, "Power/WMT_Thermal", fmt, ##args); \
} while (0)
...
/*New Wifi throttling Algo+*/
ssize_t wmt_wifi_algo_write(struct file *filp, const char __user *buf, unsigned long len,
			    void *data)
{
	char desc[MAX_LEN] = { 0 };
...
static int wmt_tm_proc_register(void)
{
	...
	if (!wmt_tm_proc_dir) {
		...
		entry =
		    create_proc_entry("wmt_tm", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP,
				      wmt_tm_proc_dir);
		if (entry) {
		...
		entry =
		    create_proc_entry("wmt_val", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP,
				      wmt_tm_proc_dir);
		if (entry) {
		...
		entry =
		    create_proc_entry("wfd_stat", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP,
				      wmt_tm_proc_dir);
		if (entry) {
