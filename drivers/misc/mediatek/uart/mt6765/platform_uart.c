...
/* pinctrl-names from dtsi.GPIO operations:*/
/* rx set, rx clear, tx set, tx clear */
...
#endif
/* !defined(CONFIG_FPGA_EARLY_PORTING) && !defined(CONFIG_MTK_CLKMGR) */
...
#if defined(CONFIG_MTK_CLKMGR) && !defined(CONFIG_FPGA_EARLY_PORTING)
	 .set_bit = PDN_FOR_UART1,
	 .clr_bit = PDN_FOR_UART1,
	 .pll_id = PDN_FOR_UART1,
#endif	/* defined(CONFIG_MTK_CLKMGR) || defined(CONFIG_FPGA_EARLY_PORTING) */
...
#if defined(CONFIG_MTK_CLKMGR) && !defined(CONFIG_FPGA_EARLY_PORTING)
	 .set_bit = PDN_FOR_UART2,
	 .clr_bit = PDN_FOR_UART2,
	 .pll_id = PDN_FOR_UART2,
#endif	/* defined(CONFIG_MTK_CLKMGR) || defined(CONFIG_FPGA_EARLY_PORTING) */
	 .sysrq = FALSE, .hw_flow = TRUE, .vff = TRUE,
	 },
};
...
void set_uart_default_settings(int idx)
{
	...
	pr_debug("[UART%d]physreg=0x%lx, reg=0x%lx, irq=%d, irqflags=0x%lx\n",
		idx,
...
static inline void dump_reg(struct mtk_uart *uart, const char *caller)
{
	...
	MSG(CFG, "%s: PNT(%02X); DLH(%02X), DLL(%02X), IER(%02X)\n",
	    caller, usamplepnt, udlh, udll, ier);
#endif
}
...
/*---------------------------------------------------------------------------*/
static int
mtk_uart_tty_insert_flip_string(struct mtk_uart *uart,
	const unsigned char *chars, size_t size)
{
	...
	if (dma->mode == UART_RX_VFIFO_DMA || dma->mode == UART_TX_VFIFO_DMA) {
		...
		while (UART_READ32(VFF_FLUSH(base))) {
			...
			if (polling_cnt > 10000) {
				pr_debug("polling VFF_FLUSH fail VFF_DEBUG_STATUS=0x%x\n",
		...
		while (UART_READ32(VFF_EN(base))) {
			polling_cnt++;
			if (polling_cnt > 10000) {
				pr_debug("polling VFF_EN fail VFF_DEBUG_STATUS=0x%x\n",
					UART_READ32(VFF_DEBUG_STATUS(base)));
				break;
			}
		}
...
/*---------------------------------------------------------------------------*/
static void
	mtk_uart_cal_baud(struct mtk_uart *uart, int baudrate, int highspeed)
{
	unsigned long base = uart->base;
...
void mtk_uart_power_up(struct mtk_uart *uart)
{
	...
	if (uart->poweron_count > 0) {
		...
		if (clk_en_ret) {
			pr_notice("[UART%d][CCF]enable failed.ret:%d, clk_main:%p\n",
			...
			if ((uart != console_port)
				...
				if (clk_en_ret) {
					pr_notice("[UART%d][CCF]enable failed.ret:%d, clk_dma:%p\n",
						uart->nport, clk_en_ret,
						clk_uart_dma);
				} else {
...
void mtk_uart_power_down(struct mtk_uart *uart)
{
	...
	if (uart->poweron_count == 0) {
		...
		uart->nport, uart->nport, setting->clk_uart_main);
...
{				/*assume UART_EFR_EN is on */
	...
	mb(); /* memory barrier enable intrs*/
}
...
{				/*assume UART_EFR_EN is on */
	...
	mb(); /* memory barrier disable intrs*/
}
...
void mtk_uart_rx_pre_handler(struct mtk_uart *uart, int intrs)
{
	...
	if (intrs == UART_IIR_CTI) {
		 ...
		 * Without the function call, the
		 * RX data timeout interrupt will be
...
static void
save_rx_raw_data(struct mtk_uart *uart,
	const unsigned char *chars, size_t size)
{
	unsigned long flags;
	int i;
...
void switch_uart_gpio(int uartport, int gpioopid)
{
	...
	if (IS_ERR(pins_uart)) {
		pr_notice("[UART%d][PinC]%s (%d, %s) fail!!pctrl:%p, err:%ld\n",
