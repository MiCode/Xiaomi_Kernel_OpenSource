...
#if defined(MTK_BCM_BT_LPM)
struct bcm_bt_lpm {
	int bt_host_wake_gpio;
	int bt_host_wake_irq;
	int bt_dev_wake_gpio;
	int bt_host_irq_enabled;
	int bt_dev_wake_enabled;

	struct hrtimer wake_peer_timer;    /* control wake peer stop */
	ktime_t wake_peer_delay;
} bt_lpm_pdata;

struct mtk_uart *bcm_bt_port = &mtk_uarts[1];        /* ttyMT1 for bcm bt uart channal*/
#endif
...
#ifdef BCM_BT_LPM
void (*uart_wake_peer)(void) = NULL;
struct mtk_uart *bcm_bt_port = &mtk_uarts[1];

void mtk_uart_set_wake_peer(int port, void (*wake_peer)(void))
{
	printk(KERN_ERR "mtk_uart_set_wake_peer port:%d\n", port);
	uart_wake_peer = wake_peer;
}
EXPORT_SYMBOL(mtk_uart_set_wake_peer);
#endif
...
#if defined(MTK_BCM_BT_LPM)
/*---------------------------------------------------------------------------*/
static irqreturn_t bluetooth_hostwake_irq_handler(int irq, void *dev_id)
{
	int host_wake;

	host_wake = gpio_get_value(bt_lpm_pdata.bt_host_wake_gpio);

	printk(KERN_ERR "%s: lpm %s\n", __func__, host_wake ? "off" : "on");

	irq_set_irq_type(irq, host_wake ? IRQF_TRIGGER_FALLING : IRQF_TRIGGER_RISING);
	printk(KERN_ERR "bt host wake type:%d\n", host_wake);

	return IRQ_HANDLED;
}

/*---------------------------------------------------------------------------*/
static enum hrtimer_restart bt_wake_peer_timeout(struct hrtimer *timer)
{
	printk(KERN_ERR "%s +++\n", __func__);

	/* pull down dev_wake pin, need syncliazed?  */
	if (bt_lpm_pdata.bt_dev_wake_enabled) {
		printk(KERN_ERR "bt_dev_wake_enabled:%d\n", bt_lpm_pdata.bt_dev_wake_enabled);
		gpio_set_value(bt_lpm_pdata.bt_dev_wake_gpio, 0);
		bt_lpm_pdata.bt_dev_wake_enabled = 0;
	}
...
static int  mtk_uart_init_bt_lpm(void)
{
	...
	bt_lpm_pdata.bt_host_wake_gpio = -1;
	bt_lpm_pdata.bt_host_wake_irq = -1;
	bt_lpm_pdata.bt_dev_wake_gpio = -1;
	bt_lpm_pdata.bt_host_irq_enabled = 0;
	bt_lpm_pdata.bt_dev_wake_enabled = 0;

	hrtimer_init(&bt_lpm_pdata.wake_peer_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	/* wake peer timer set 1 sec to check need disable dev_wake pin or not */
	bt_lpm_pdata.wake_peer_delay = ktime_set(1, 0);
	bt_lpm_pdata.wake_peer_timer.function = bt_wake_peer_timeout;


	/* get host_wake irq */
	np = of_find_compatible_node(NULL, NULL, bt_node);
	if (!np) {
		return -ENODEV;
	}

	bt_lpm_pdata.bt_host_wake_gpio = of_get_named_gpio(np, "bcm_bt_host_wake", 0);
	if (bt_lpm_pdata.bt_host_wake_gpio >= 0) {
		printk(KERN_ERR "%s: bt_host_wake:%d.\n", __FUNCTION__,
			bt_lpm_pdata.bt_host_wake_gpio);
		bt_lpm_pdata.bt_host_wake_irq = gpio_to_irq(bt_lpm_pdata.bt_host_wake_gpio);
		printk(KERN_ERR "brcm_bt_host_wake_irq:%d.\n",
			bt_lpm_pdata.bt_host_wake_irq);
	} else {
		printk(KERN_ERR "get bt_host_wake gpio error:%d.\n",
			bt_lpm_pdata.bt_host_wake_gpio);
	}

	ret = gpio_direction_input(bt_lpm_pdata.bt_host_wake_gpio);
	if (ret < 0) {
		printk(KERN_ERR "%s: Unable to set direction for gpio %d\n", __func__,
					bt_lpm_pdata.bt_host_wake_gpio);
		gpio_free(bt_lpm_pdata.bt_host_wake_gpio);
		return ret;
	}
	gpio_set_value(bt_lpm_pdata.bt_host_wake_gpio, 0);
	ret = request_threaded_irq(bt_lpm_pdata.bt_host_wake_irq, NULL,
			bluetooth_hostwake_irq_handler,
			IRQF_TRIGGER_HIGH | IRQF_ONESHOT,
			"bluetooth host wake", NULL);

	if (ret  < 0) {
		printk(KERN_ERR "Couldn't acquire BT_HOST_WAKE IRQ");
		gpio_free(bt_lpm_pdata.bt_host_wake_irq);
	}

	/* get device wake gpio */
	bt_lpm_pdata.bt_dev_wake_gpio = of_get_named_gpio(np, "bcm_bt_dev_wake", 0);
	printk(KERN_ERR "get bt_dev_wake gpio:%d.\n",
		bt_lpm_pdata.bt_dev_wake_gpio);
	if (bt_lpm_pdata.bt_dev_wake_gpio < 0) {
		printk(KERN_ERR "get bt_dev_wake gpio error:%d.\n",
			bt_lpm_pdata.bt_dev_wake_gpio);
	}

	ret = gpio_request(bt_lpm_pdata.bt_dev_wake_gpio, "bt_dev_wake");
	if (ret) {
		printk(KERN_ERR "unable to request dev wake gpio %d (%d)\n",
				bt_lpm_pdata.bt_dev_wake_gpio, ret);
		return ret;
	}

	ret =  gpio_direction_output(bt_lpm_pdata.bt_dev_wake_gpio, 0);
	if (ret < 0) {
		printk(KERN_ERR "%s: Unable to set direction for gpio %d\n", __func__,
					bt_lpm_pdata.bt_dev_wake_gpio);
		gpio_free(bt_lpm_pdata.bt_dev_wake_gpio);
...
/*---------------------------------------------------------------------------*/
/* perform wake peer activities on the port */
static void mtk_uart_wake_peer(struct uart_port *port)
{
#ifdef BCM_BT_LPM
	struct mtk_uart *uart = (struct mtk_uart *)port;
	/*  TODO: set device wake pin */
	if (uart != bcm_bt_port) {
		return;
	}

	if (uart_wake_peer != NULL) {
		uart_wake_peer();
	}
#endif
#ifdef MTK_BCM_BT_LPM
	struct mtk_uart *uart = (struct mtk_uart *)port;

	if (uart == bcm_bt_port) {
		mtk_uart_bt_wake_peer_activity();
	}
#endif
}
...
static int mtk_uart_suspend(struct platform_device *pdev, pm_message_t state)
{
	int ret = 0;
	struct mtk_uart *uart = platform_get_drvdata(pdev);
	printk(KERN_ERR "mtk_uart_suspend state=%d\n", state.event);
...
#if defined(MTK_BCM_BT_LPM)
	if (uart == bcm_bt_port) {
		printk(KERN_ERR "mtk_uart_suspend irq_enable:%d\n", bt_lpm_pdata.bt_host_irq_enabled);
		if (bt_lpm_pdata.bt_host_irq_enabled == 0) {
			enable_irq(bt_lpm_pdata.bt_host_wake_irq);
			ret = irq_set_irq_wake(bt_lpm_pdata.bt_host_wake_irq, 1);
			if (ret < 0) {
				printk(KERN_ERR "irq_set_irq_wake enable ret:%d\n", ret);
			}
			bt_lpm_pdata.bt_host_irq_enabled = 1;
		}
	}
#endif
...
static int mtk_uart_resume(struct platform_device *pdev)
{
	int ret = 0;
	struct mtk_uart *uart = platform_get_drvdata(pdev);
	printk(KERN_ERR "mtk_uart_resume\n");
...
#if defined(MTK_BCM_BT_LPM)
	if (uart == bcm_bt_port) {
		printk(KERN_ERR "mtk_uart_resume irq_enable:%d\n", bt_lpm_pdata.bt_host_irq_enabled);
		if (bt_lpm_pdata.bt_host_irq_enabled) {
			disable_irq(bt_lpm_pdata.bt_host_wake_irq);
			ret = irq_set_irq_wake(bt_lpm_pdata.bt_host_wake_irq, 0);
			if (ret < 0) {
				printk(KERN_ERR "irq_set_irq_wake disable ret:%d\n", ret);
			}
			bt_lpm_pdata.bt_host_irq_enabled = 0;
		}
	}
