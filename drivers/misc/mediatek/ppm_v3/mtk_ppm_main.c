...
int mt_ppm_main(void)
{
	...
	if (!ppm_main_info.client_info[PPM_CLIENT_DVFS].limit_cb ||
		...
		ppm_info("dvfs/hps clients not yet registered!\n");
		goto end;
	}
...
static void ppm_add_policy(void)
{
	ppm_cpi_init();
	ppm_dlpt_policy_init();
	ppm_forcelimit_policy_init();
	ppm_hard_userlimit_policy_init();
	ppm_ptpod_policy_init();
	ppm_pwrthro_policy_init();
	ppm_sysboost_policy_init();
	ppm_thermal_policy_init();
	ppm_userlimit_policy_init();
	ppm_ut_policy_init();
...
static void ppm_policy_exit(void)
{
	ppm_cpi_exit();
	ppm_dlpt_policy_exit();
	ppm_forcelimit_policy_exit();
	ppm_hard_userlimit_policy_exit();
	ppm_ptpod_policy_exit();
	ppm_pwrthro_policy_exit();
	ppm_sysboost_policy_exit();
	ppm_thermal_policy_exit();
	ppm_userlimit_policy_exit();
	ppm_ut_policy_exit();

	ppm_lcmoff_policy_exit();
}
...
static int __init ppm_main_init(void)
{
	...
	struct device_node *map = NULL;
	struct device_node *c = NULL;
