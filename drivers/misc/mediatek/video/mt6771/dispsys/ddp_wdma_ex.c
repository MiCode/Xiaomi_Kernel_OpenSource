...
unsigned int wdma_index(enum DISP_MODULE_ENUM module)
{
	...
	default:
		/* invalid module */
		DDPPR_ERR("[DDP] error: invalid wdma module=%d\n", module);
		ASSERT(0);
	}
	return idx;
}
...
int wdma_reset(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	if (!handle) {
		while ((DISP_REG_GET(offset + DISP_REG_WDMA_FLOW_CTRL_DBG) &
			...
			udelay(10);
			if (delay_cnt > 2000) {
				DDPPR_ERR("wdma%d reset timeout!\n", idx);
				break;
			}
		}
	} else {
...
void wdma_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("en=%d,src(%dx%d),clip=(%d,%d,%dx%d),pitch=(W=%d,UV=%d),addr=(0x%08x,0x%08x,0x%08x),fmt=%s\n",
	...
	DDPDUMP("state=%s,in_req=%d(prev sent data),in_ack=%d(ask data to prev),exec=%d,in_pix=(L:%d,P:%d)\n",
		...
		(DISP_REG_GET(DISP_REG_WDMA_EXEC_DBG + idx_offst) >> 16) &
		0xffff,
		DISP_REG_GET(DISP_REG_WDMA_INPUT_CNT_DBG + idx_offst) & 0xffff);
...
void wdma_dump_reg(enum DISP_MODULE_ENUM module)
{
	if (disp_helper_get_option(DISP_OPT_REG_PARSER_RAW_DUMP)) {
		...
		DDPDUMP("0x020:0x%08x 0x028:0x%08x 0x%08x 0x038:0x%08x 0x078:0x%08x\n",
			       ...
			       unsigned int is_primary_flag, void *cmdq)
{
	...
	preultra_low_UV = (preultra_low_us + fifo_off_drs_enter) *
			consume_rate;
	...
	ultra_high = (ultra_high_us + fifo_off_drs_enter) *
			consume_rate * bytes_per_sec;
	ultra_high_UV = (ultra_high_us + fifo_off_drs_enter) *
			consume_rate;
	...
	regval = 0;
	temp = (long long)fifo_pseudo_size - preultra_low;
	temp = (temp > 0) ? temp : 1;
	...
	regval = 0;
	temp = (long long)fifo_pseudo_size - preultra_high;
	temp = (temp > 0) ? temp : 1;
	regval |= REG_FLD_VAL(BUF_CON_FLD_PRE_ULTRA_HIGH, temp);
	temp = (long long)fifo_pseudo_size - ultra_high;
	temp = (temp > 0) ? temp : 1;
	...
	regval = 0;
	temp = (long long)fifo_pseudo_size - preultra_low_UV;
	temp = DIV_ROUND_UP(temp, 4);
	temp = (temp > 0) ? temp : 1;
	regval |= REG_FLD_VAL(BUF_CON_FLD_PRE_ULTRA_LOW, temp);
	temp = (long long)fifo_pseudo_size - ultra_low_UV;
	temp = DIV_ROUND_UP(temp, 4);
	temp = (temp > 0) ? temp : 1;
	regval |= REG_FLD_VAL(BUF_CON_FLD_ULTRA_LOW, temp);
	...
	regval = 0;
	temp = (long long)fifo_pseudo_size - preultra_high_UV;
	temp = DIV_ROUND_UP(temp, 4);
	temp = (temp > 0) ? temp : 1;
	regval |= REG_FLD_VAL(BUF_CON_FLD_PRE_ULTRA_HIGH, temp);
	temp = (long long)fifo_pseudo_size - ultra_high_UV;
	temp = DIV_ROUND_UP(temp, 4);
	temp = (temp > 0) ? temp : 1;
	regval |= REG_FLD_VAL(BUF_CON_FLD_ULTRA_HIGH, temp);
...
static int wdma_check_input_param(struct WDMA_CONFIG_STRUCT *config)
{
	...
	if (config->dstAddress == 0 || config->srcWidth == 0 ||
		config->srcHeight == 0) {
		DDPPR_ERR("wdma parameter invalidate: addr=0x%lx, w=%d, h=%d\n",
