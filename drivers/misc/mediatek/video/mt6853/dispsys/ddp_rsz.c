...
static int rsz_check_params(struct RSZ_CONFIG_STRUCT *rsz_config)
{
	if ((rsz_config->frm_in_w != rsz_config->frm_out_w ||
		...
		DISPERR("%s:need rsz but input width(%u) > limit(%u)\n",
		      ...
		      struct disp_ddp_path_config *pconfig, void *qhandle)
{
	...
	DISP_REG_SET(qhandle, rsz_base + DISP_REG_RSZ_INPUT_IMAGE,
		     in_h << 16 | in_w);
	DISP_REG_SET(qhandle, rsz_base + DISP_REG_RSZ_OUTPUT_IMAGE,
		     out_h << 16 | out_w);
	DDPDBG("%s:%s:(%ux%u)->(%ux%u)\n",
	       __func__, ddp_get_module_name(module), in_w, in_h, out_w, out_h);

	DISP_REG_SET(qhandle, rsz_base + DISP_REG_RSZ_HORIZONTAL_COEFF_STEP,
		     rsz_config->tw[tile_idx].step);
	DISP_REG_SET(qhandle, rsz_base + DISP_REG_RSZ_VERTICAL_COEFF_STEP,
		     rsz_config->th.step);
	DISP_REG_SET(qhandle,
		     rsz_base + DISP_REG_RSZ_LUMA_HORIZONTAL_INTEGER_OFFSET,
		     rsz_config->tw[tile_idx].int_offset);
	DISP_REG_SET(qhandle,
		     rsz_base + DISP_REG_RSZ_LUMA_HORIZONTAL_SUBPIXEL_OFFSET,
		     rsz_config->tw[tile_idx].sub_offset);
	DISP_REG_SET(qhandle,
		     rsz_base + DISP_REG_RSZ_LUMA_VERTICAL_INTEGER_OFFSET,
		     rsz_config->th.int_offset);
	DISP_REG_SET(qhandle,
		     rsz_base + DISP_REG_RSZ_LUMA_VERTICAL_SUBPIXEL_OFFSET,
...
static int rsz_start(enum DISP_MODULE_ENUM module, void *qhandle)
{
	...
	if (disp_helper_get_option(DISP_OPT_SHADOW_REGISTER)) {
		...
		DISP_REG_SET(qhandle, rsz_base + DISP_REG_RSZ_SHADOW_CTRL,
			     reg_val);
	}

	DISP_REG_SET_FIELD(qhandle, FLD_RSZ_EN,
			   rsz_base + DISP_REG_RSZ_ENABLE, 0x1);
	DISP_REG_SET_FIELD(qhandle, FLD_RSZ_READ_WRK_REG,
			   rsz_base + DISP_REG_RSZ_SHADOW_CTRL, 0x1);
	DISP_REG_SET_FIELD(qhandle, FLD_RSZ_BYPASS_SHADOW,
			   rsz_base + DISP_REG_RSZ_SHADOW_CTRL, 0x1);

	DISP_REG_SET(qhandle, rsz_base + DISP_REG_RSZ_DEBUG_SEL, 0x3);
...
static int rsz_stop(enum DISP_MODULE_ENUM module, void *qhandle)
{
	...
	DISP_REG_SET_FIELD(qhandle, FLD_RSZ_EN,
			   rsz_base + DISP_REG_RSZ_ENABLE, 0x0);
...
void rsz_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	int_flag = DISP_REG_GET(rsz_base + DISP_REG_RSZ_INT_FLAG);
	in_size = DISP_REG_GET(rsz_base + DISP_REG_RSZ_INPUT_IMAGE);
	out_size = DISP_REG_GET(rsz_base + DISP_REG_RSZ_OUTPUT_IMAGE);
	in_pos = DISP_REG_GET(rsz_base + DISP_REG_RSZ_DEBUG);
	shadow = DISP_REG_GET(rsz_base + DISP_REG_RSZ_SHADOW_CTRL);
	...
	DDPDUMP("in(%ux%u),out(%ux%u),h_step:%d,v_step:%d\n",
		...
		DISP_REG_GET(rsz_base + DISP_REG_RSZ_HORIZONTAL_COEFF_STEP),
		DISP_REG_GET(rsz_base + DISP_REG_RSZ_VERTICAL_COEFF_STEP));
...
void rsz_dump_reg(enum DISP_MODULE_ENUM module)
{
	...
	for (i = 0; i < 3; i++) {
		DDPDUMP("0x%03X: 0x%08x 0x%08x 0x%08x 0x%08x\n",
			i * 0x10, DISP_REG_GET(rsz_base + i * 0x10),
			DISP_REG_GET(rsz_base + i * 0x10 + 0x4),
			DISP_REG_GET(rsz_base + i * 0x10 + 0x8),
			DISP_REG_GET(rsz_base + i * 0x10 + 0xC));
	}
...
static int rsz_bypass(enum DISP_MODULE_ENUM module, int bypass)
{
	...
	if (bypass) {
		...
		DISP_REG_SET_FIELD(NULL, FLD_RSZ_HORIZONTAL_EN,
				   rsz_base + DISP_REG_RSZ_CONTROL_1, 0x0);
