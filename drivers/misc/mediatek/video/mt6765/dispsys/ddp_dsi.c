	...
	struct cmdqRecStruct *cmdq, struct LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		* 1011:0.46v
		*/
		MIPITX_OUTREGBIT(DSI_PHY_REG[i] + MIPITX_VOLTAGE_SEL, FLD_RG_DSI_HSTX_LDO_REF_SEL, 0x08);//0.38v
...
//222.4.20 longcheer zgq add for display mipi reg start
...
long  lcm_mipi_reg_write(char *buf, unsigned long  count)
{
		...
		if(count > 11)
	{
		...
		lcm_adb_cmd.count = packet_count;//packet_count;
		lcm_adb_cmd.cmd = lcm_mipi_read_write.lcm_setting_table.cmd;
		for(i=0;i<lcm_mipi_read_write.lcm_setting_table.count;i++)//lcm_mipi_read_write.lcm_setting_table.count
		{
...
//2022.4.20 longcheer zgq add for display mipi reg end
