...
int mtkfb_set_cabc_mode(unsigned int mode)
{
	MTKFB_FUNC();
	DISPDBG("mtkfb_set_cabc_mode:%d Start\n",
		mode);
	primary_display_set_cabc(mode);
	DISPDBG("mtkfb_set_cabc_mode End\n");
	return 0;
}
EXPORT_SYMBOL(mtkfb_set_cabc_mode);
...
struct fb_lcd_lotus_para lcd_lotus_para = {0};
...
static int __init mtkfb_get_white_point(char *p)
{
	...
	pr_info("%s:  (p=%s)\n", __func__,  p);
	pr_info("white_point = %s\n",wpoint);

	lcd_lotus_para.white_point_x = (wpoint[0]-'0') * 100
		+ (wpoint[1]-'0') * 10 + (wpoint[2]-'0');

	lcd_lotus_para.white_point_y = (wpoint[3]-'0') * 100
...
static ssize_t mtkfb_change_dispparam(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t len)
{
	return 0;
}

static ssize_t mtkfb_get_dispparam(struct device *dev,
...
static ssize_t mtkfb_setcabc_mode(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t len)
{
	if(buf[0] == '0'){
		lcd_lotus_para.cabc_mode = FALSE;
		primary_display_set_cabc(0);
	}else{
		lcd_lotus_para.cabc_mode = TRUE;
		primary_display_set_cabc(1);
	}
	return len;
}

static ssize_t mtkfb_getcabc_mode(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "cabe mode:%d\n",lcd_lotus_para.cabc_mode);
}

static ssize_t mtkfb_setce_mode(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t len)
{
	if(buf[0] == '0'){
		lcd_lotus_para.ce_mode = FALSE;
		primary_display_set_ce(0);
	}else{
		lcd_lotus_para.ce_mode = TRUE;
		primary_display_set_ce(1);
	}
	return len;
}

static ssize_t mtkfb_getce_mode(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "ce mode:%d\n",lcd_lotus_para.ce_mode);
}

static ssize_t mtkfb_set_wpoint(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t len)
{
	return 0;
}

static ssize_t mtkfb_get_wpoint(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%3d%3d\n",
		lcd_lotus_para.white_point_x, lcd_lotus_para.white_point_y);
}

static ssize_t mtkfb_set_rpoint(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t len)
{
	return 0;
}
static ssize_t mtkfb_get_rpoint(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%6d\n",
		lcd_lotus_para.white_point_r);
}

static ssize_t mtkfb_set_gpoint(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t len)
{
	return 0;
}
static ssize_t mtkfb_get_gpoint(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%6d\n",
		lcd_lotus_para.white_point_g);
}

static ssize_t mtkfb_set_bpoint(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t len)
{
	return 0;
}
static ssize_t mtkfb_get_bpoint(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%6d\n",
		lcd_lotus_para.white_point_b);
}

static DEVICE_ATTR(mtkfb_dispparam, S_IRUGO | S_IWUSR,
	mtkfb_get_dispparam, mtkfb_change_dispparam);

static DEVICE_ATTR(mtkfb_set_cabc, S_IRUGO | S_IWUSR,
	mtkfb_getcabc_mode, mtkfb_setcabc_mode);

static DEVICE_ATTR(mtkfb_set_ce, S_IRUGO | S_IWUSR,
	mtkfb_getce_mode, mtkfb_setce_mode);

static DEVICE_ATTR(mtkfb_dispwpoint, S_IRUGO | S_IWUSR,
	mtkfb_get_wpoint, mtkfb_set_wpoint);

static DEVICE_ATTR(mtkfb_disprpoint, S_IRUGO | S_IWUSR,
	mtkfb_get_rpoint, mtkfb_set_rpoint);

static DEVICE_ATTR(mtkfb_dispgpoint, S_IRUGO | S_IWUSR,
	mtkfb_get_gpoint, mtkfb_set_gpoint);

static DEVICE_ATTR(mtkfb_dispbpoint, S_IRUGO | S_IWUSR,
	mtkfb_get_bpoint, mtkfb_set_bpoint);

static struct attribute *mtkfb_attrs[] = {
	&dev_attr_mtkfb_dispparam.attr,
	&dev_attr_mtkfb_set_cabc.attr,
	&dev_attr_mtkfb_set_ce.attr,
...
{
	if(strncmp(mtkfb_lcm_name, "ili7807d_fhdp_dsi_vdo_ebbg", 26) ==0){
		lcd_lotus_para.white_point_r = EBBG_ILI_WPOINTR;
		lcd_lotus_para.white_point_g = EBBG_ILI_WPOINTG;
		lcd_lotus_para.white_point_b = EBBG_ILI_WPOINTB;
		return 0;
	}else if(strncmp(mtkfb_lcm_name, "otm1911a_fhdp_dsi_vdo_cost", 26) ==0){
		lcd_lotus_para.white_point_r = COST_FOCAL_WPOINTR;
		lcd_lotus_para.white_point_g = COST_FOCAL_WPOINTG;
		lcd_lotus_para.white_point_b = COST_FOCAL_WPOINTB;
		return 0;
	}else{
		return -1;
	}
...
static int mtkfb_register_sysfs(struct mtkfb_device *fbdev)
{
	...
	rc = mtkfb_set_rgb_point_init();
	if (rc){
		pr_err("rgb set failed,rc=%d\n",rc);
	}

	rc = sysfs_create_group(&fbdev->dev->kobj, &mtkfb_attr_group);
	if (rc)
...
{
	sysfs_remove_group(&fbdev->dev->kobj, &mtkfb_attr_group);
}
#else
static int mtkfb_register_sysfs(struct mtkfb_device *fbdev)
{
