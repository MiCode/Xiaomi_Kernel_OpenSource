...
int primary_display_suspend(void)
{
	...
	if (primary_display_get_power_mode_nolock() == FB_SUSPEND) {
		DISPINFO("[POWER]lcm suspend power [begin]\n");
...
int _set_cabc_by_cmdq(unsigned int enable)
{
                ...
                if (primary_display_is_video_mode()) {
                                ...
                                /*Async flush by cmdq*/
                                _cmdq_flush_config_handle_mira(cmdq_handle_cabc, 0);
                                DISPMSG("[BL]_set_cabc_by_cmdq ret=%d\n", ret);
                } else {
                                cmdqRecReset(cmdq_handle_cabc);
...
int _set_ce_by_cmdq(unsigned int enable)
{
                int ret = 0;
                struct cmdqRecStruct *cmdq_handle_ce = NULL;

                ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP, &cmdq_handle_ce);
                DISPDBG("primary ce, handle=%p\n", cmdq_handle_ce);
                if (ret) {
                                DISPWARN("fail to create primary cmdq handle for ce \n");
                                return -1;
                }

                if (primary_display_is_video_mode()) {
                                cmdqRecReset(cmdq_handle_ce);
                                _cmdq_insert_wait_frame_done_token_mira(cmdq_handle_ce);
                                disp_lcm_set_ce (pgc->plcm, cmdq_handle_ce, enable);
                                /*Async flush by cmdq*/
                                _cmdq_flush_config_handle_mira(cmdq_handle_ce, 0);
                                DISPMSG("[BL]_set_ce_by_cmdq ret=%d\n", ret);
                } else {
                                cmdqRecReset(cmdq_handle_ce);
                                cmdqRecWait(cmdq_handle_ce, CMDQ_SYNC_TOKEN_CABC_EOF);
                                _cmdq_handle_clear_dirty(cmdq_handle_ce);
                                _cmdq_insert_wait_frame_done_token_mira(cmdq_handle_ce);
                                disp_lcm_set_ce(pgc->plcm, cmdq_handle_ce, enable);
                                cmdqRecSetEventToken(cmdq_handle_ce,
                                                CMDQ_SYNC_TOKEN_CONFIG_DIRTY);
                                cmdqRecSetEventToken(cmdq_handle_ce,
                                                CMDQ_SYNC_TOKEN_CABC_EOF);
                                DISPMSG("[BL]_set_ce_by_cmdq ret=%d\n", ret);
                }
                cmdqRecDestroy(cmdq_handle_ce);
                cmdq_handle_ce = NULL;
...
int primary_display_set_ce(unsigned int enable)
{
         int ret = 0;
         static unsigned int last_status;
         ...
         _primary_path_lock(__func__);
         if (pgc->state == DISP_SLEPT) {
                   DISPWARN("Sleep State set CE invald\n");
         } else {
                   primary_display_idlemgr_kick(__func__, 0);
                   if (primary_display_cmdq_enabled()) {
                            _set_ce_by_cmdq(enable);
                            atomic_set(&delayed_trigger_kick, 1);
                   } else {
                            DISPWARN("CAMQ disbaled, not support set CE\n");
                   }
                   last_status = enable;
         }
         _primary_path_unlock(__func__);
