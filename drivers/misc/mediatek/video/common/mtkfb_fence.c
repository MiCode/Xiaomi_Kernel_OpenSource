...
#define MTKFB_FENCE_ERR(fmt, arg...)	\
	pr_debug("DISP/fence error(%d):"fmt, __LINE__, ##arg)
					...
					unsigned int session_id)
{
	...
	for (i = 0; i < ARRAY_SIZE(_disp_fence_context); i++) {
		if (_disp_fence_context[i].session_id == 0xffffffff) {
			DISPMSG(
	"no session info for session_id:0x%08x,insert %p to array index:%d\n",
		...
		dprec_logger_event_init(&session_info->event_prepare, name,
			...
			sprintf(name,
			"%s%d_frame_cfg", disp_session_mode_spy(session_id),
				DISP_SESSION_DEV(session_id));
	...
	for (j = 0; j < (sizeof(session_info->session_layer_info) /
		...
		if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY)
			...
			prefix, DISP_SESSION_DEV(session_id), j);
				...
				unsigned int layer_id, unsigned int idx)
{
	...
	if (mva != 0x0) {
		...
		MTKFB_FENCE_LOG(
		"query buf mva: layer=%d, idx=%d, mva=0x%08x\n", layer_id, idx,
				(unsigned int)(buf->mva));
	} else {
		/* FIXME: non-ion buffer need cache sync here? */
		DISPERR("cannot find session(0x%x) buf, layer=%d, idx=%d ",
			...
			unsigned int layer_id, unsigned int idx)
{
	...
	if (va == 0x0) {
		/* FIXME: non-ion buffer need cache sync here? */
		MTKFB_FENCE_ERR
		    ("cannot find this buf, layer=%d, idx=%d, fence_idx=%d",
				     ...
				     unsigned int idx, unsigned int ticket)
{
	...
	if (layer_id >= DISP_SESSION_TIMELINE_COUNT) {
		DISPERR("mtkfb_update_buf_state return MVA=0x0");
		DISPERR("mtkfb_query_buf_mva layer_id %d !!!!!!(Warning)\n",
			layer_id);
		return mva;
	}
		...
		int query_type)
{
	...
	mutex_lock(&layer_info->sync_lock);
	list_for_each_entry(buf, &layer_info->buf_list, list) {
		DISPDBG("mva updaten layer%d, idx=0x%x, mva=0x%08lx-%x py %lx\n"
				, layer_id,
		...
		unsigned int mva_offset, unsigned int seq)
{
	...
	list_for_each_entry(buf, &layer_info->buf_list, list) {
		if (buf->idx == idx) {
			...
			ret = true;
			mmprofile_log_ex(
		ddp_mmp_get_events()->primary_seq_insert, MMPROFILE_FLAG_PULSE,
...
static struct mtkfb_fence_buf_info *mtkfb_get_buf_info(void)
{
	...
	/* we must use another mutex for buffer list because
	 * it will be operated by ALL layer info.
	 */
	...
	if (!list_empty(&info_pool_head)) {
		...
		mtkfb_init_buf_info(info);
		MTKFB_FENCE_LOG(
			"create new mtkfb_fence_buf_info node %p\n", info);
		mutex_unlock(&fence_buffer_mutex);
		return info;
	}
 ...
 */
void mtkfb_release_fence(unsigned int session_id,
			...
			struct mtkfb_fence_buf_info *buf_info)
{
	...
	if (handle)
		...
		DISPPR_FENCE("can't import ion handle for fd:%d\n",
			buf->ion_fd);
#endif
...
struct mtkfb_fence_buf_info *disp_sync_prepare_buf(struct disp_buffer_info *buf)
{
	...
	snprintf(data.name, sizeof(data.name),
			"disp-S%x-L%d-%d", session_id, timeline_id,
		 data.value);
	ret = fence_create(layer_info->timeline, &data);
	if (ret != 0) {
		/* Does this really happened? */
		DISPPR_FENCE("%s%d,layer%d create Fence Object failed!\n",
				disp_session_mode_spy(session_id),
				DISP_SESSION_DEV(session_id),
				timeline_id);
	}
				     ...
				     unsigned long phy_addr)
{
	...
	disp_sync_get_cached_layer_info(session_id, timeline_id,
	...
	if (phy_addr) {
		...
		list_for_each_entry(buf, &layer_info->buf_list, list) {
			/*
			 * Because buf_list stores the
			 *lates fence info from prepare,
			 * so we should gurantee only release
...
{
	return __disp_sync_query_buf_info(session_id,
					timeline_id, idx, mva, size, 1);
}
...
int disp_sync_get_debug_info(char *stringbuf, int buf_len)
{
	...
	for (i = 0; i < ARRAY_SIZE(_disp_fence_context) ; i++) {
		...
		session_info = &(_disp_fence_context[i]);
		len += scnprintf(stringbuf + len,
			buf_len - len, "|Session id\t0x%08x\n", session_id);
		for (layer_id = 0; layer_id < DISP_SESSION_TIMELINE_COUNT;
								layer_id++) {
			layer_info =
				&(session_info->session_layer_info[layer_id]);
			len += scnprintf(stringbuf + len, buf_len - len,
	"|layerinfo %d\tfence_fd(%d)\tfence_idx(%d)\ttimeline_idx(%d)\n",
				      layer_id, layer_info->fence_fd,
					layer_info->fence_idx,
				      layer_info->timeline_idx);
		}
		len += scnprintf(stringbuf + len, buf_len - len,
