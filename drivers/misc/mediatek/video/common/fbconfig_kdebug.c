					...
					uint8_t *buffer, uint32_t num)
{
	...
	if ((dsi_id == PM_DSI0) || (dsi_id == PM_DSI_DUAL))
		ret = DSI_dcs_read_lcm_reg_v2(
			pm_get_dsi_handle(PM_DSI0), NULL, cmd, buffer, num);
	else if (dsi_id == PM_DSI1)
		ret = DSI_dcs_read_lcm_reg_v2(
			pm_get_dsi_handle(PM_DSI1), NULL, cmd, buffer, num);
			   ...
			   unsigned long arg)
{
	...
	case SET_DSI_ID:
	{
		...
		if (copy_from_user(&set_dsi_id,
				   (void __user *)arg, sizeof(set_dsi_id))) {
			pr_debug(
			  "[SET_DSI_ID]: copy_from_user failed! line:%d\n",
			  __LINE__);
			return -EFAULT;
		}
	...
	case LCM_TEST_DSI_CLK:
	{
		...
		return copy_to_user(
		  argp, &lcm_fb, sizeof(lcm_fb)) ? -EFAULT : 0;
	}
	case LCM_GET_ID:
	{
/* LCM_DRIVER*pLcm_drv=pm->pLcm_drv; */
...
#endif
		return copy_to_user(
		  argp, &lcm_id, sizeof(lcm_id)) ? -EFAULT : 0;
	}
	...
	case MIPI_SET_CC:
	{
		...
		if (get_user(enable, (uint32_t __user *) argp)) {
			pr_debug(
			  "[MIPI_SET_CC]: copy_from_user failed! line:%d\n",
				 __LINE__);
			return -EFAULT;
		}
	...
	case MIPI_SET_CLK:
	{
		...
		if (get_user(clk, (uint32_t __user *) argp)) {
			pr_debug(
			  "[MIPI_SET_CLK]: copy_from_user failed! line:%d\n",
				 __LINE__);
			return -EFAULT;
		}
		...
		if ((clk >= 5) && (clk <= 625)) {
			...
			pr_debug(
	"MIPI_SET_CLK=>dsi:%d is wrong, the value should be 25~625!line:%d\n",
			  clk, __LINE__);
			return -EFAULT;
		}
		return 0;
	...
	case MIPI_SET_SSC:
	{
		...
		if (copy_from_user(&dsi_ssc, (void __user *)argp,
				   sizeof(dsi_ssc))) {
			pr_debug(
			  "[MIPI_SET_SSC]: copy_from_user failed! line:%d\n",
				 __LINE__);
			return -EFAULT;
		}
		...
		if ((dsi_ssc >= 1) && (dsi_ssc <= 8)) {
			...
			pr_debug(
	"MIPI_SET_SSC=>dsi_ssc:%d is wrong, the value should be 1~8!line:%d\n",
				dsi_ssc, __LINE__);
			return -EFAULT;
		}
		return 0;
	...
	case LCM_GET_DSI_TIMING:
	{
		...
		if (copy_from_user(&timing, (void __user *)argp,
				   sizeof(timing))) {
			pr_debug(
			  "[MIPI_GET_TIMING]: copy_from_user failed! line:%d\n",
				 __LINE__);
			return -EFAULT;
		}
	...
	case MIPI_SET_TIMING:
	{
		...
		if (primary_display_is_sleepd())
			return -EFAULT;
		if (copy_from_user(&timing,
				   (void __user *)argp, sizeof(timing))) {
			pr_debug(
	...
	case LCM_GET_ESD:
	{
		...
		if (copy_from_user(&esd_para,
				   (void __user *)arg, sizeof(esd_para))) {
			pr_debug(
			  "[LCM_GET_ESD]: copy_from_user failed! line:%d\n",
				 __LINE__);
			return -EFAULT;
		}
		...
		ret =
			fbconfig_get_esd_check_test(dsi_id,
						    esd_para.addr, buffer,
						    esd_para.para_num);
		if (ret < 0) {
			kfree(buffer);
			return -EFAULT;
