...
#define MTKFB_FENCE_ERR(fmt, arg...)                                           \
	DISPERR("DISP/fence error(%d):" fmt, __LINE__, ##arg)
...
_get_session_sync_info(unsigned int session)
{
	...
	for (i = 0; i < ARRAY_SIZE(_disp_fence_context); i++) {
		if (session == _disp_fence_context[i].session_id) {
			s_info = &(_disp_fence_context[i]);
			goto done;
		}
	}
	...
	for (i = 0; i < ARRAY_SIZE(_disp_fence_context); i++) {
		...
		DISPMSG("not found session info for session_id:0x%08x,insert %p to array index:%d\n",
			session, &(_disp_fence_context[i]), i);
		_disp_fence_context[i].session_id = session;
		s_info = &(_disp_fence_context[i]);

		sprintf(name, "%s%d_prepare", disp_session_mode_spy(session),
		...
		sprintf(name, "%s%d_frame_cfg", disp_session_mode_spy(session),
			DISP_SESSION_DEV(session));
		dprec_logger_event_init(&s_info->event_frame_cfg, name,
					DPREC_LOGGER_LEVEL_DEFAULT,
					&s_info->event_prepare.mmp);

		sprintf(name, "%s%d_wait_fence", disp_session_mode_spy(session),
			DISP_SESSION_DEV(session));
		dprec_logger_event_init(&s_info->event_wait_fence, name,
					DPREC_LOGGER_LEVEL_DEFAULT,
					&s_info->event_prepare.mmp);
		sprintf(name, "%s%d_setinput", disp_session_mode_spy(session),
			DISP_SESSION_DEV(session));
		...
		sprintf(name, "%s%d_setoutput", disp_session_mode_spy(session),
			DISP_SESSION_DEV(session));
		...
		sprintf(name, "%s%d_trigger", disp_session_mode_spy(session),
			DISP_SESSION_DEV(session));
		...
		sprintf(name, "%s%d_findidx", disp_session_mode_spy(session),
			DISP_SESSION_DEV(session));
		dprec_logger_event_init(&s_info->event_findidx, name,
					DPREC_LOGGER_LEVEL_DEFAULT,
					&s_info->event_prepare.mmp);

		sprintf(name, "%s%d_release", disp_session_mode_spy(session),
			DISP_SESSION_DEV(session));
		...
		sprintf(name, "%s%d_waitvsync", disp_session_mode_spy(session),
			DISP_SESSION_DEV(session));
		...
		sprintf(name, "%s%d_err", disp_session_mode_spy(session),
			DISP_SESSION_DEV(session));
					 ...
					 unsigned long phy_addr)
{
	...
	session_info = _get_session_sync_info(session_id);
	if (session_info == 0)
		return 0; /* fix: MTK_FB_ION_SUPPORT not enable */
	layer_info = &(session_info->session_layer_info[layer_id]);
			 ...
			 int fence)
{
	...
	list_for_each_entry_safe(buf, n, &layer_info->buf_list, list) {
		...
		if (buf->va && ((DISP_SESSION_TYPE(session_id) >
				    ...
				    unsigned long mva)
{
	...
	if (src) {
		disp_sync_convert_input_to_fence_layer_info(
			   ...
			   struct mtkfb_fence_buf_info *buf_info)
{
	...
	if (handle)
		buf_info->size = mtkfb_ion_phys_mmu_addr(ion_client,
							 handle, &mva);
	else
...
struct mtkfb_fence_buf_info *disp_sync_prepare_buf(struct disp_buffer_info *buf)
{
	...
	DISPPR_FENCE("P+/%s%d/L%d/id%d/fd%d/mva0x%08lx/size0x%08x\n",
		     disp_session_mode_spy(session_id),
		     DISP_SESSION_DEV(session_id), timeline_id, buf_info->idx,
		...
		list_for_each_entry(buf, &layer_info->buf_list, list) {
			...
			if ((buf->mva + buf->mva_offset) == phy_addr) {
				if (phy_addr >= buf->mva &&
					...
					DISPERR("wrong addr:0x%lx, 0x%lx,0x%08x\n",
...
unsigned int disp_sync_query_buf_info(unsigned int session_id,
				      ...
				      unsigned int *size)
{
	return __disp_sync_query_buf_info(session_id, timeline_id, idx, mva,
					  size, 1);
}
