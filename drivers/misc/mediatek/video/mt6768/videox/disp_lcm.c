...
extern char lcd_whitepoint_info[32];                     /* 2021.03.16 add for read whitepoint baiyihan */
...
/* 2021.03.23 add for tianma&huaxing whitepoint dev_attr baiyihan start */
		...
		struct device_attribute *attr, char *buf)
{
	...
	if(islcmconnected == 1){
		if(strnstr(mtkfb_lcm_name,"36_02_0a",strlen(mtkfb_lcm_name))){
		...
		}else if(strnstr(mtkfb_lcm_name,"42_03_0b",strlen(mtkfb_lcm_name))){
			struct ddp_lcm_write_cmd_table write_table[1] = {
				{0x00, 1, {0x60} },
			};
			do_lcm_vdo_lp_write(write_table, 1);
			read_table.cmd[0] = 0xF4;
			read_table.cmd[1] = 0xF4;
			do_lcm_vdo_lp_read(&read_table);
			sprintf(buf,"val0=%d,val1=%d\n",read_table.data[0].byte1,read_table.data[1].byte1);
			}
		else
...
/* 2021.03.23 add for tianma&huaxing whitepoint dev_attr baiyihan end */
