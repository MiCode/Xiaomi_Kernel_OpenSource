...
/* defined struct for info parsed from FPSGo  */
typedef struct GED_KPI_FPSGO_INFO_TAG {
	int target_fps;
	int target_margin;
	int t_cpu;
} GED_KPI_FPSGO_INFO;
...
static void ged_kpi_work_cb(struct work_struct *psWork)
{
	...
	case GED_TIMESTAMP_TYPE_D:
		...
		if (!psHead) {
			...
			if (psHead) {
				memset(psHead, 0, sizeof(GED_KPI_HEAD));
	...
	case GED_TIMESTAMP_TYPE_2:
		...
		if (psHead) {
			...
			if (psKPI) {
...
#endif
				/* calculate gpu done interval
				 * between frame n and n-1
				 */
				...
				/* determine which GPU loading to use*/
#ifdef GED_ENABLE_FB_DVFS
...
GED_ERROR ged_kpi_system_init(void)
{
	...
	ghLogBuf = ged_log_buf_alloc(GED_KPI_MAX_FPS * 10,
		300 * GED_KPI_MAX_FPS * 10, GED_LOG_BUF_TYPE_RINGBUFFER,
		NULL, "KPI");
...
{
	GED_KPI_HEAD *psHead = (GED_KPI_HEAD *)pvoid;
	GED_KPI_HEAD *psRiskyBQ = (GED_KPI_HEAD *)pvParam;
