...
static int dma_buf_lock_handle_release(struct inode *inode, struct file *file)
{
	...
	pr_warn("dma_buf_lock_handle_release\n");
#endif
...
#if DMA_BUF_LOCK_DEBUG
	pr_warn("dma_buf_lock_kds_callback\n");
#endif
                                             ...
                                             struct poll_table_struct *wait)
{
	...
	resource = file->private_data;
#if DMA_BUF_LOCK_DEBUG
	pr_warn("dma_buf_lock_handle_poll\n");
#endif
...
#if DMA_BUF_LOCK_DEBUG
	pr_warn("dma_buf_lock_handle_poll : return %i\n", ret);
#endif
	return ret;
}
...
#if DMA_BUF_LOCK_DEBUG
		pr_warn("dma_buf_lock_dolock : dma_buf_fd %i dma_buf %X kds_resource %X\n", resource->list_of_dma_buf_fds[i],
...
#if DMA_BUF_LOCK_DEBUG
	pr_warn("dma_buf_lock_dolock : complete\n");
#endif
	mutex_lock(&dma_buf_lock_mutex);
	kref_put(&resource->refcount, dma_buf_lock_dounlock);
	mutex_unlock(&dma_buf_lock_mutex);
...
#if DMA_BUF_LOCK_DEBUG
	pr_warn("dma_buf_lock_init failed\n");
#endif
	return err;
}
...
#if DMA_BUF_LOCK_DEBUG
			pr_warn("DMA_BUF_LOCK_FUNC_LOCK_ASYNC - %i\n", request.count);
#endif
