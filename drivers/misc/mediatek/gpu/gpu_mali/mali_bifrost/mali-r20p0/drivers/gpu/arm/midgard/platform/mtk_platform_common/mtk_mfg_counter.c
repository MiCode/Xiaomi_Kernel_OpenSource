...
static uint32_t _read_u_rate_w_loading
(int u_rate_numerator, int u_rate_denominator)
{
	uint32_t num = 0;
	...
	num = mali_pmus[u_rate_numerator].value;

	return (!num || !active_cycle) ? 1 : _cal_urate(num, active_cycle);
}

static uint32_t _read_var_u_rate_w_loading
(int u_rate_numerator, int u_rate_denominator)
{
...
static struct {
	...
	{"SHADER_U_RATE", EXEC_INSTR_COUNT, GPU_ACTIVE, _read_u_rate},
	{"ALU_U_RATE", EXEC_INSTR_COUNT, EXEC_ACTIVE, _read_u_rate},
	{"TEX_U_RATE", TEX_FILT_NUM_OPERATIONS, EXEC_ACTIVE, _read_u_rate},
	{"LSC_U_RATE", L2_ANY_LOOKUP, EXEC_ACTIVE, _read_u_rate},
	{"VAR_U_RATE", 0, 0, _read_var_u_rate},
	{"SHADER_U_RATE_W_LOADING",
	EXEC_CORE_ACTIVE, 0, _read_u_rate_w_loading},
	{"ALU_U_RATE_W_LOADING", EXEC_INSTR_COUNT, 0, _read_u_rate_w_loading},
	{"TEX_U_RATE_W_LOADING",
	TEX_FILT_NUM_OPERATIONS, 0, _read_u_rate_w_loading},
	{"LSC_U_RATE_W_LOADING", L2_ANY_LOOKUP, 0, _read_u_rate_w_loading},
	{"VAR_U_RATE_W_LOADING", 0, 0, _read_var_u_rate_w_loading},
};
...
static void _mtk_mfg_init_counter(void)
{
	...
	if (!binited) {
		number_of_hardware_counters =
			cnt -
			empty_cnt +
			MFG_MTK_COUNTER_SIZE;
		mali_pmus = kcalloc(number_of_hardware_counters,
			sizeof(GPU_PMU), GFP_KERNEL);
		if (!mali_pmus)
			return;
	...
	if (ret != PMU_NG) {
		...
		for (i = 0; i < nr_hwc_blocks; i++) {
			...
			for (j = 0; j < MALI_COUNTERS_PER_BLOCK; j++) {
				...
				if (strstr(mali_pmus[cnt].name,
					"TEX_FILT_NUM_OPERATIONS") ||
				    strstr(mali_pmus[cnt].name,
				    "EXEC_INSTR_COUNT")) {
		...
		for (i = 0; i < MFG_MTK_COUNTER_SIZE; i++) {
			...
			if (mfg_mtk_counters[i].read) {
				numerator =
				perf_index_table[
				mfg_mtk_counters[i].u_rate_numerator];
				denominator =
				perf_index_table[
				mfg_mtk_counters[i].u_rate_denominator];
				mali_pmus[cnt].value =
				mfg_mtk_counters[i].read(
				numerator,
				denominator);
				if (mali_pmus[cnt].value == 0) {
					mali_pmus[cnt].overflow = 0;
...
static int mali_get_gpu_pmu_init(GPU_PMU *pmus, int pmu_size, int *ret_size)
{
	...
	if (pmus) {
		...
		for (i = 0; i < nr_hwc_blocks; i++) {
			...
			if (block_type == RESERVED_BLOCK)
				continue;
			//ignore same MM block
			if (mm_block == 1 && block_type == MMU_L2_BLOCK)
				continue;
		...
		for (i = 0; i < MFG_MTK_PERF_COUNTER_SIZE; i++) {
			...
			if (ret == -1)
				pr_info("[PMU] index fail:%s",
				mfg_mtk_perf_index[i].name);
			else
				perf_index_table[mfg_mtk_perf_index[i].index] =
				cnt;
		}
...
int gator_gpu_pmu_init(void)
{
	...
	if (!binited)
		...
		for (i = 0; i < nr_hwc_blocks; i++) {
			...
			name_offset =
				name_offset_table[block_type] *
				MALI_COUNTERS_PER_BLOCK;
			data_offset = i * MALI_COUNTERS_PER_BLOCK;
			for (j = 0; j < MALI_COUNTERS_PER_BLOCK; j++) {
				const char *name =
				hardware_counter_names[name_offset+j];
