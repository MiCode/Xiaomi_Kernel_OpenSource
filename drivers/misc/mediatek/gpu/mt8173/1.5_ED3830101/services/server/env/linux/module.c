...
struct device *psDev;
/* export for letting PTPOD check GPU's status*/
int is_gpu_ready(void);
...
#endif
{
	...
	PVR_DPF((PVR_DBG_MESSAGE,"PVRSRVDriverProbe (pDevice=%p), name %s", pDevice, pDevice->name));
...
int is_gpu_ready(void)
{
	return bCoreinitSucceeded == 1 ? 1 : 0;
}
EXPORT_SYMBOL(is_gpu_ready)
