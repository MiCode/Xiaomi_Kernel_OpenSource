				 ...
				 unsigned long *size, u64 upper_limit)
{
	...
	do {
		...
		feature->virt_addr =
			dma_alloc_attrs(ssmr_dev, alloc_size,
	...
	ret_map = memory_unmapping((unsigned long)phys_to_virt(dma_to_phys(
					   ssmr_dev, feature->phy_addr)),
				   alloc_size);
	if (ret_map < 0)
...
static int memory_region_online(struct SSMR_Feature *feature)
{
	...
	if (feature->is_unmapping) {
		...
		ret_map =
			memory_mapping((unsigned long)phys_to_virt(dma_to_phys(
					       ssmr_dev, feature->phy_addr)),
				       alloc_size);
		if (ret_map < 0)
			pr_info("[remapping fail]: virt:0x%lx, size:0x%lx",
				(unsigned long)phys_to_virt(dma_to_phys(
					ssmr_dev, feature->phy_addr)),
				alloc_size);
		else
			feature->is_unmapping = false;
