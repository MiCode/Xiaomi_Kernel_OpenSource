	...
	if (!strncmp(uname, SSMR_FEATURES_DT_UNAME,
		...
		for (; i < __MAX_NR_SSMR_FEATURES; i++) {
			if (!strncmp(_ssmr_feats[i].enable, "on", 2)) {
				get_feature_size_by_dt_prop(
					node, _ssmr_feats[i].dt_prop_name,
	...
	for (; i < __MAX_NR_SSMRSUBS; i++) {
		...
		for (; j < __MAX_NR_SSMR_FEATURES; j++) {
			if (_ssmr_feats[j].region == i
			    && !strncmp(_ssmr_feats[j].enable, "on", 2)) {
				max_feat_req_size =
					max(max_feat_req_size,
...
static int __init ssmr_preinit(struct reserved_mem *rmem)
{
	...
	if (total_target_size == 0) {
		...
		// saved_memory_ssmr_registration.align = (1 <<
		// SSMR_ALIGN_SHIFT);
...
static int __init ssmr_memory_init(struct reserved_mem *rmem)
{
	int ret;
	int cma_area_count = 0;
	u64 total_target_size = 0;
	phys_addr_t alignment, ssmr_align;
	...
	total_target_size = get_total_target_size();
	// pr_info("%s: total_target_size = %llu\n",__func__,
	// total_target_size);
	// end = rmem->base + rmem->size;
	end = rmem->base + total_target_size;

	/* cma alignment */
	ssmr_align = 1 << SSMR_ALIGN_SHIFT;
	alignment = PAGE_SIZE << max(MAX_ORDER - 1, pageblock_order);
	alignment = max(ssmr_align, alignment);

	start = round_down(rmem->base, alignment);
	...
	ret = cma_init_reserved_mem(start, end - start, 0, rmem->name, &cma);
	if (ret) {
	...
	rmem->size -= total_target_size;

	/* Putback remaining rmem to memblock.memory */
	if (rmem->size > 0) {
		memblock_free(rmem->base + total_target_size, rmem->size);
		memblock_add(rmem->base + total_target_size, rmem->size);
	}

	return 0;
}
RESERVEDMEM_OF_DECLARE(memory_ssmr_init, "mediatek,memory_ssmr",
		       ssmr_memory_init);
...
RESERVEDMEM_OF_DECLARE(tui_memory, "mediatek,memory-tui", dedicate_tui_memory);
#endif
...
static int memory_region_offline(struct SSMR_Region *region, phys_addr_t *pa,
				 unsigned long *size, u64 upper_limit)
{
	...
	if (region->cache_page)
		...
		alloc_pages =
			_ssmr_feats[region->cur_feat].req_size / PAGE_SIZE;
	...
	do {
		...
		page = cma_alloc(cma, alloc_pages, SSMR_CMA_ALIGN_PAGE_ORDER,
				 GFP_KERNEL);
	...
	if (ret_map < 0) {
		...
		aee_kernel_warning_api(
			...
			DB_OPT_DEFAULT | DB_OPT_DUMPSYS_ACTIVITY
				...
				| DB_OPT_PROCESS_COREDUMP | DB_OPT_PAGETYPE_INFO
...
static int memory_region_online(struct SSMR_Region *region)
{
	...
	if (region->is_unmapping) {
		...
		ret_map = memory_mapping(region_page_va, region->alloc_pages
								 << PAGE_SHIFT);
	...
	if (!region->is_unmapping && !region->use_cache_memory) {
		cma_release(cma, region->page, region->alloc_pages);
...
#if defined(CONFIG_TRUSTONIC_TRUSTED_UI) || defined(CONFIG_BLOWFISH_TUI_SUPPORT)
int _tui_region_offline(phys_addr_t *pa, unsigned long *size, u64 upper_limit)
{
	int retval = 0;
	...
	pr_info("%s: [reserve done]: pa 0x%lx, size 0x%lx\n", __func__,
		(unsigned long)page_to_phys(region->page),
...
static int _svp_wdt_kthread_func(void *data)
{
	...
	aee_kernel_warning_api(
		...
		DB_OPT_DEFAULT | DB_OPT_DUMPSYS_ACTIVITY
			...
			| DB_OPT_PID_MEMORY_INFO /* smaps and hprof */
			| DB_OPT_PROCESS_COREDUMP
			| DB_OPT_DUMPSYS_SURFACEFLINGER | DB_OPT_DUMPSYS_GFXINFO
			| DB_OPT_DUMPSYS_PROCSTATS,
		"SVP online fail.\nCRDISPATCH_KEY:SVP_SS1\n",
...
int svp_start_wdt(void)
{
	...
	if (_svp_online_task == NULL) {
		_svp_online_task = kthread_create(_svp_wdt_kthread_func, NULL,
						  "svp_online_kthread");
				  ...
				  u64 upper_limit, unsigned int feat)
{
	...
	trace_printk("ssvp offline start (%s)\n", _ssmr_feats[feat].feat_name);
	...
	trace_printk("ssvp offline end (%s)\n", _ssmr_feats[feat].feat_name);

	if (retval < 0) {
		region->state = SSMR_STATE_ON;
		retval = -EAGAIN;
		trace_printk("ssvp offline fail (%s)\n",
			     _ssmr_feats[feat].feat_name);
		goto out;
	}
	trace_printk("ssvp offline success (%s)\n",
		     _ssmr_feats[feat].feat_name);
	region->state = SSMR_STATE_OFF;
	pr_info("%s %d: [reserve done]: pa 0x%lx, size 0x%lx\n", __func__,
		__LINE__, (unsigned long)page_to_phys(region->page),
		region->alloc_pages << PAGE_SHIFT);

out:
	pr_info("%s %d: <<<<< request feat: %s, cur feat: %s, ", __func__,
		__LINE__,
		feat < __MAX_NR_SSMR_FEATURES ? _ssmr_feats[feat].feat_name
					      : "NULL",
		region->cur_feat < __MAX_NR_SSMR_FEATURES
			? _ssmr_feats[region->cur_feat].feat_name
			: "NULL");
...
static int memory_ssmr_show(struct seq_file *m, void *v)
{
	...
	if (cma) {
		...
		seq_printf(m, "cma info: [%pa-%pa] (0x%lx)\n", &cma_base,
			   &cma_end, cma_get_size(cma));

		seq_printf(m, "cma info: base %pa pfn [%lu-%lu] count %lu\n",
			   &cma_base, __phys_to_pfn(cma_base),
			   __phys_to_pfn(cma_end),
	...
	for (; i < __MAX_NR_SSMRSUBS; i++) {
		...
		seq_printf(m, "%s base:0x%lx, count %lu, ", _ssmregs[i].name,
		...
		seq_printf(m, "alloc_pages %lu, cur_feat %s, state %s.%s\n",
			   ...
			   _ssmregs[i].cur_feat < __MAX_NR_SSMR_FEATURES
				   ? _ssmr_feats[_ssmregs[i].cur_feat].feat_name
				   : "NULL",
	...
	i = ssmr_query_total_sec_heap_count();
	seq_printf(m, "total ssmr heap count: %d\n", i);
	for (j = 0; j < i; j++) {
		seq_printf(m, "ssmr heap info[%d]: heap_id=%d, heap_name=%s\n",
			   i, _ssmr_heap_info[j].heap_id,
			   _ssmr_heap_info[j].heap_name);
		heap_id = ssmr_query_heap_info(j, heap_name);
		seq_printf(m, "heap_id=%d, heap_name=%s\n", heap_id, heap_name);
	}
	return 0;
}
...
static int __init
memory_ssmr_init_region(char *name, u64 size, struct SSMR_Region *region,
			const struct file_operations *entry_fops)
{
	...
	if (is_pre_reserve_memory) {
		...
		page = cma_alloc(cma, region->count, SSMR_CMA_ALIGN_PAGE_ORDER,
				 GFP_KERNEL);
...
int __init memory_ssmr_debug_init(void)
{
 ...
 */
	...
	for (; i < __MAX_NR_SSMRSUBS; i++) {
		memory_ssmr_init_region(_ssmregs[i].name,
					_ssmregs[i].usable_size, &_ssmregs[i],
