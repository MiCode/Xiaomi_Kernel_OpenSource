...
int init_audio_ipi_dma(const uint8_t task)
{
	...
	dma->base_phy.addr =
		(uint8_t *)get_reserve_mem_phys(opendsp_id, mem_id);
	dma->base_vir.addr =
		(uint8_t *)get_reserve_mem_virt(opendsp_id, mem_id);
	dma->size = size;
...
int audio_ipi_dma_alloc(const uint8_t task,
	struct aud_ptr_t *phy_addr, const uint32_t size)
{
	uint32_t opendsp_id = audio_get_opendsp_id(task);
	...
	if (phy_addr == NULL || size == 0) {
		pr_info("arg err, %p, %u", phy_addr, size);
		return -EINVAL;
	}
	...
	phy_addr->addr_val = gen_pool_alloc(g_dma_pool[opendsp_id], size);
	...
	struct aud_ptr_t *phy_addr, const uint32_t size)
{
	...
	gen_pool_free(g_dma_pool[opendsp_id], phy_addr->addr_val, size);
			       ...
			       const uint32_t dsp_to_ap_size)
{
	...
	for (i = 0; i < NUM_AUDIO_IPI_DMA_PATH; i++) {
		...
		phy_value = gen_pool_alloc(g_dma_pool[opendsp_id], size[i]);
		if (phy_value == 0) {
			pr_notice("gen_pool_alloc(%u) fail, (%zu/%zu)",
				  size[i],
				  gen_pool_avail(g_dma_pool[opendsp_id]),
