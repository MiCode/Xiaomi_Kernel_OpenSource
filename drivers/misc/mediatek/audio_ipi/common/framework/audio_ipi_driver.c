	...
	uint8_t data_type)
{
	...
	if (data_type == AUDIO_IPI_DMA) {
		...
		if (hal_wb_buf_size != 0 && hal_wb_buf_addr != NULL) {
			...
			ipi_dbg(
				"hal_wb_buf_addr(%p), wb dram_buf(%p), hal_wb_buf_size(%d)"
	...
	if (data_type == AUDIO_IPI_DMA &&
		...
		if (wb_dram->data_size > hal_wb_buf_size) {
			...
			opendsp_id = audio_get_opendsp_id(ipi_msg.task_scene);
			retval = copy_to_user(
					 hal_wb_buf_addr,
...
parsing_exit:
	...
	if (dram_buf.addr_val != 0) {
		ipi_dbg(
			"task %d msg 0x%x, free wb buffer %p, hal_wb_buf_size 0x%x"
			, ipi_msg.task_scene, ipi_msg.msg_id,
			dram_buf, hal_wb_buf_size);
