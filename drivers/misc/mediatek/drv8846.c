				...
				struct pwm_setting *pwm)
{
	...
	pwm_no = 2;
		...
		if(pstate.enabled == 0) {
			...
			if(pstate.period == 54171){
				conf.pwm_no = pwm_no;
				conf.mode = PWM_MODE_OLD;
				conf.clk_div = CLK_DIV64;
				...
				conf.PWM_MODE_OLD_REGS.DATA_WIDTH = 21;
				conf.PWM_MODE_OLD_REGS.THRESH = 10;
			} else if(pstate.period == 625000) {
				conf.pwm_no = pwm_no;
				conf.mode = PWM_MODE_OLD;
			...
			pr_debug("mt_pwm_ clk_div %d, clk_src=%d \n",conf.clk_div, conf.clk_src);
...
static ssize_t drv8846_debug_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	c_ctrl->pdata.speed_period = 54171;

	if (99 == enable) {
		c_ctrl->pdata.dir = UP;
		c_ctrl->pdata.speed_duration = 604;
	} else {
		c_ctrl->pdata.dir = DOWN;
		c_ctrl->pdata.speed_duration = 606;
	}
	c_ctrl->state = STILL;
	drv8846_move(c_ctrl);
