...
static int _sample_execute(struct apusys_cmd_handle *hnd,
	struct apusys_device *dev)
{
	...
	if (hnd->num_cmdbufs != 1) {
		spl_drv_err("command num invalid(%u)\n", hnd->num_cmdbufs);
		return -EINVAL;
	}

	if (hnd->cmdbufs[0].size != sizeof(struct sample_request)) {
		spl_drv_err("command size invalid(%u)\n", hnd->cmdbufs[0].size);
		return -EINVAL;
	}

	spl_drv_dbg("multicore_total = %u\n", hnd->multicore_total);
	req = (struct sample_request *)hnd->cmdbufs[0].kva;
	...
	spl_drv_dbg("|====================================================|\n");
	spl_drv_dbg("| sample driver request(0x%llx) (use #%-2d device)   |\n",
