...
static int mt6886_rproc_init(struct mtk_apu *apu)
{
	return 0;
}

static int mt6886_rproc_exit(struct mtk_apu *apu)
{
	return 0;
}
...
static int mt6886_rproc_start(struct mtk_apu *apu)
{
	int ns = 1; /* Non Secure */
	int domain = 0;
...
static int mt6886_rproc_stop(struct mtk_apu *apu)
{
	struct device *dev = apu->dev;
...
static int mt6886_apu_power_init(struct mtk_apu *apu)
{
	struct device *dev = apu->dev;
...
static int mt6886_ipi_send_post(struct mtk_apu *apu)
{
	cancel_delayed_work_sync(&timeout_work);
...
static int mt6886_irq_affin_init(struct mtk_apu *apu)
{
	int i;
...
static int mt6886_irq_affin_set(struct mtk_apu *apu)
{
...
static int mt6886_irq_affin_unset(struct mtk_apu *apu)
{
	irq_set_affinity_hint(apu->mbox0_irq_number, &normal_cpus);
...
static int mt6886_apu_memmap_init(struct mtk_apu *apu)
{
...
static void mt6886_apu_memmap_remove(struct mtk_apu *apu)
{
}

static void mt6886_rv_cg_gating(struct mtk_apu *apu)
{
	iowrite32(0x0, apu->md32_sysctrl + MD32_CLK_EN);
}

static void mt6886_rv_cg_ungating(struct mtk_apu *apu)
{
	iowrite32(0x1, apu->md32_sysctrl + MD32_CLK_EN);
}

static void mt6886_rv_cachedump(struct mtk_apu *apu)
{
	int offset;
	unsigned long flags;
...
const struct mtk_apu_platdata mt6886_platdata = {
	...
	.ops		= {
		.init	= mt6886_rproc_init,
		.exit	= mt6886_rproc_exit,
		.start	= mt6886_rproc_start,
		.stop	= mt6886_rproc_stop,
		.apu_memmap_init = mt6886_apu_memmap_init,
		.apu_memmap_remove = mt6886_apu_memmap_remove,
		.cg_gating = mt6886_rv_cg_gating,
		.cg_ungating = mt6886_rv_cg_ungating,
		.rv_cachedump = mt6886_rv_cachedump,
		.power_init = mt6886_apu_power_init,
		.power_on = mt6886_apu_power_on,
		.power_off = mt6886_apu_power_off,
		.ipi_send_post = mt6886_ipi_send_post,
		.irq_affin_init = mt6886_irq_affin_init,
		.irq_affin_set = mt6886_irq_affin_set,
		.irq_affin_unset = mt6886_irq_affin_unset,
	},
};
