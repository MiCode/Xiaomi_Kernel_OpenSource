...
TRACE_EVENT(apupwr_pwr,
	...
	unsigned int vsram, unsigned int dsp1_freq, unsigned int dsp2_freq,
	unsigned int dsp_freq, unsigned int ipuif_freq,
	unsigned long long id),
	TP_ARGS(vvpu, vcore, vsram, dsp1_freq, dsp2_freq,
	dsp_freq, ipuif_freq, id),
	TP_STRUCT__entry(
	...
	TP_printk(
		"V_vpu=%d,V_core=%d,V_sram=%d,F_v0=%d,F_v1=%d,F_conn=%d,F_ipuif=%d,time_id=%llu",
		__entry->vvpu,
		__entry->vcore,
		__entry->vsram,
		__entry->dsp1_freq,
...
TRACE_EVENT(apupwr_rpc,
	...
	TP_printk(
		"spm_wakeup=0x%x,rpc_intf_rdy=0x%x,vcore_cg=0x%x,conn_cg=0x%x,v0_cg=0x%x,v1_cg=0x%x",
		__entry->spm_wakeup,
