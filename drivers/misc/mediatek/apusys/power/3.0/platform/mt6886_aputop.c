...
#if REGULATOR_FRAMEWORK
	if (vapu_reg_id) {
		pr_info("%s vapu_mode:%d vapu_enabled:%d, vapu_voltage:%d\n",
				__func__,
				regulator_get_mode(vapu_reg_id),
				regulator_is_enabled(vapu_reg_id),
				regulator_get_voltage(vapu_reg_id));
	}

	if (vsram_reg_id) {
		pr_info("%s vsram_mode:%d vsram_enabled:%d, vsram_voltage:%d\n",
				__func__,
				regulator_get_mode(vsram_reg_id),
				regulator_is_enabled(vsram_reg_id),
				regulator_get_voltage(vsram_reg_id));
	}

	if (vcore_reg_id) {
		pr_info("%s vcore_enabled:%d, vcore_voltage:%d\n",
				__func__,
				regulator_is_enabled(vcore_reg_id),
...
static int __apu_wake_rpc_rcx(struct device *dev)
{
	...
	if (log_lvl)
		dev_info(dev,
			"%s before wakeup RCX APU_RPC_INTF_PWR_RDY 0x%x = 0x%x\n",
			__func__,
...
static int mt6886_apu_top_on(struct device *dev)
{
	int ret = 0;
...
static int mt6886_apu_top_off(struct device *dev)
{
	int ret = 0, val = 0;
...
static int mt6886_apu_top_pb(struct platform_device *pdev)
{
	...
	if (!vapu_reg_id || !vsram_reg_id || !vcore_reg_id) {
		pr_info("%s regulator_get reg_id failed\n", __func__);
		return -ENOENT;
	}
#endif
	dump_volt_info();
	...
	mt6886_init_remote_data_sync(apupw.regs[apu_md32_mbox]);

	return ret;
}

static int mt6886_apu_top_rm(struct platform_device *pdev)
{
	int idx;
...
static int mt6886_apu_top_func(struct platform_device *pdev,
		enum aputop_func_id func_id, struct aputop_func_param *aputop)
{
	...
	case APUTOP_FUNC_OPP_LIMIT_HAL:
		mt6886_aputop_opp_limit(aputop, OPP_LIMIT_HAL);
		break;
	case APUTOP_FUNC_OPP_LIMIT_DBG:
		mt6886_aputop_opp_limit(aputop, OPP_LIMIT_DEBUG);
		break;
	case APUTOP_FUNC_DUMP_REG:
		aputop_dump_pwr_reg(&pdev->dev);
		break;
	case APUTOP_FUNC_DRV_CFG:
		mt6886_drv_cfg_remote_sync(aputop);
		break;
...
const struct apupwr_plat_data mt6886_plat_data = {
	.plat_name = "mt6886_apupwr",
	.plat_aputop_on = mt6886_apu_top_on,
	.plat_aputop_off = mt6886_apu_top_off,
	.plat_aputop_pb = mt6886_apu_top_pb,
	.plat_aputop_rm = mt6886_apu_top_rm,
#if IS_ENABLED(CONFIG_PM_SLEEP)
	.plat_aputop_suspend = mt6886_apu_top_suspend,
	.plat_aputop_resume = mt6886_apu_top_resume,
#endif
	.plat_aputop_func = mt6886_apu_top_func,
#if IS_ENABLED(CONFIG_DEBUG_FS)
	.plat_aputop_dbg_open = mt6886_apu_top_dbg_open,
	.plat_aputop_dbg_write = mt6886_apu_top_dbg_write,
#endif
	.plat_rpmsg_callback = mt6886_apu_top_rpmsg_cb,
	.bypass_pwr_on = 0,
	.bypass_pwr_off = 0,
};
