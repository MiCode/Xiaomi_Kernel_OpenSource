...
static int apusys_probe(struct platform_device *pdev)
{
	...
	ret = apusys_sched_init();
	if (ret)
		goto sched_init_fail;

	ret = apusys_mem_init(&pdev->dev);
	if (ret)
		goto mem_init_fail;

	ret = apusys_dbg_init(core_info->dbg_root);
	if (ret)
		goto dbg_init_fail;
...
static struct platform_device mdw_device = {
	.name = APUSYS_DEV_NAME,
	.id = 0,
	.dev = {
...
int apusys_mdw_init(struct apusys_core_info *info)
{
	...
	if (platform_driver_register(&mdw_driver)) {
		mdw_drv_err("failed to register apusys midware driver");
		return -ENODEV;
	}

	if (platform_device_register(&mdw_device)) {
...
{
	platform_driver_unregister(&mdw_driver);
	platform_device_unregister(&mdw_device);
}
