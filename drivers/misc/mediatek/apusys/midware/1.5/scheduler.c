...
int mdw_set_ctx(int type, int idx, uint8_t ctxid)
{
#ifdef APUSYS_OPTIONS_INF_REVISER
	return reviser_set_context(type, idx, ctxid);
#else
	return 0;
#endif
}
...
static int exec_cmd_func(void *isc, void *idev_info)
{
	...
	if (dev_info->dev->dev_type == APUSYS_DEVICE_MDLA ||
		...
		cmd_hnd.context_callback = mdw_set_ctx;
	} else {
		mdw_set_ctx(dev_info->dev->dev_type,
