...
struct mdw_rsc_mgr {
	unsigned long dev_sup_bmp[BITS_TO_LONGS(MDW_DEV_MAX)];
	unsigned long dev_avl_bmp[BITS_TO_LONGS(MDW_DEV_MAX)];
	unsigned long cmd_avl_bmp[BITS_TO_LONGS(MDW_DEV_MAX)];
...
{
	unsigned long bmp[BITS_TO_LONGS(MDW_DEV_MAX)];
	uint64_t b = 0;

	memset(&bmp, 0, sizeof(unsigned long) *
		BITS_TO_LONGS(MDW_DEV_MAX));

	bitmap_and(bmp, rsc_mgr.cmd_avl_bmp,
		rsc_mgr.dev_avl_bmp, MDW_DEV_MAX);
	bitmap_to_arr32((uint32_t *)&b, bmp, MDW_DEV_MAX);
...
void mdw_rsc_update_avl_bmp(int type)
{
	...
	bitmap_to_arr32((uint32_t *)&cb,
		rsc_mgr.cmd_avl_bmp, MDW_DEV_MAX);
...
static int mdw_rsc_delete_dev(struct mdw_dev_info *d)
{
	...
	if (d->type >= MDW_DEV_MAX || d->type < 0 ||
...
int mdw_rsc_init(void)
{
	...
	bitmap_zero(rsc_mgr.dev_sup_bmp, MDW_DEV_MAX);
