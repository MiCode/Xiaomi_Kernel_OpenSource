...
static int mdw_queue_deadline_task_start(struct mdw_ap_sc *sc, void *q)
{
	uint64_t load = 0;
	struct mdw_rsc_tab *tab;
	struct deadline_root *root;
...
static int mdw_queue_deadline_task_end(struct mdw_ap_sc *sc, void *q)
{
	uint64_t load = 0;
	struct mdw_rsc_tab *tab;
	struct deadline_root *root;
...
static struct mdw_ap_sc *mdw_queue_deadline_pop(void *q)
{
	struct rb_node *node;
	struct mdw_ap_sc *entry = NULL;
	struct deadline_root *root = (struct deadline_root *)q;

	mutex_lock(&root->lock);
	node = rb_first_cached(&root->root);
	if (node) {
		entry = rb_entry(node, struct mdw_ap_sc, node);
...
static int mdw_queue_deadline_insert(struct mdw_ap_sc *sc,
	void *q, int is_front)
{
	struct deadline_root *root = (struct deadline_root *)q;
	struct rb_node **link = &root->root.rb_root.rb_node;
	struct rb_node *parent = NULL;
	struct mdw_ap_sc *entry;
	bool leftmost = true;

	mutex_lock(&root->lock);
	while (*link) {
		parent = *link;
		entry = rb_entry(parent, struct mdw_ap_sc, node);
...
static int mdw_queue_deadline_delete(struct mdw_ap_sc *sc, void *q)
{
	struct deadline_root *root = (struct deadline_root *)q;
