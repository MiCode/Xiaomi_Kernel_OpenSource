...
static int trigger_vio_test(struct devapc_ctx *dctx, int round)
{
	...
	if (round <= 0) {
		pr_err("invalid round %d\n", round);
		return -EINVAL;
	}

	test_virt = ioremap(test_base, 4096);
	if (!test_virt) {
		pr_err("ioremap test base(0x%lx) virt failed\n",
			...
			unsigned int dbg0, unsigned int dbg1, unsigned int dbg2)
{
	...
	if (dctx->enable_ke) {
		pr_err("Violation Slave: %s %s%s %s%x %s%x, %s%x, %s%x\n",
...
static int shift_vio_dbg(int shift_bit)
{
	...
	if ((readl(DEVAPC_VIO_SHIFT_CON) & 0x3) == 0x3)
		...
		shift_done = -EFAULT;
		pr_err("Shift failed shift_bit: %d\n", shift_bit);
	}
