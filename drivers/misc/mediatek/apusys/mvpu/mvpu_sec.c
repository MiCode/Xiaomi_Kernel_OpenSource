...
{
	uint32_t ker_img_total_num = mvpu_algo_img[ker_img_offset/sizeof(uint32_t) +
		KER_NUM_OFFSET];
	...
	for (i = 0; i < ker_img_total_num; i++) {
		...
		shift = ker_img_offset/sizeof(uint32_t) + IMG_HEADER_SIZE +
		ker_size_offset/sizeof(uint32_t) + KER_INFO_SIZE*i;
...
void set_ker_iova(uint32_t ker_bin_offset, uint32_t ker_bin_num, uint32_t *ker_bin_each_iova)
{
	...
	for (i = 0; i < ker_bin_num; i++) {
		shift = ker_bin_offset/sizeof(uint32_t) +
			ker_size_offset/sizeof(uint32_t) + KER_BIN_INFO_SIZE*i;
...
void clear_hash(uint32_t session_id, uint32_t hash_id)
{
	...
	if (hash_pool[session_id]->hash_list[hash_id] != 0) {
		...
		dma_buf_unmap_attachment(hash_pool[session_id]->attach[hash_id],
			hash_pool[session_id]->sgt[hash_id], DMA_BIDIRECTIONAL);
					...
					uint32_t *mem_is_kernel)
{
	...
	hash_pool[session_id]->hash_dma_buf[hash_id] =
		...
		total_buf_size, O_RDWR | O_CLOEXEC, DMA_HEAP_VALID_HEAP_FLAGS);
	...
	hash_pool[session_id]->attach[hash_id] =
		dma_buf_attach(hash_pool[session_id]->hash_dma_buf[hash_id], mvpu_dev);
	...
	hash_pool[session_id]->sgt[hash_id] =
		dma_buf_map_attachment(hash_pool[session_id]->attach[hash_id], DMA_BIDIRECTIONAL);
