...
uint32_t get_ptn_size(uint32_t hash)
{
	...
	for (i = 0; i < ptn_total_num; i++) {
		...
		// check hash
		if ((ptn_hash & MVPU_BATCH_MASK) == (hash & MVPU_BATCH_MASK)) {
...
bool get_ptn_hash(uint32_t hash)
{
	...
	mdw_trace_end("[MVPU] %s end", __func__);
...
void get_ker_info(uint32_t hash, uint32_t *ker_bin_offset, uint32_t *ker_bin_num)
{
	...
	for (i = 0; i < ker_img_total_num; i++) {
		...
		// check hash
		if ((ker_hash & MVPU_BATCH_MASK) == (hash & MVPU_BATCH_MASK)) {
			if (mvpu_loglvl_sec >= APUSYS_MVPU_LOG_DBG)
						...
						uint32_t buf_cmd_next)
{
	...
	for (j = 0; j < rp_num; j++) {
		...
		for (i = 0; i < buf_num; i++) {
			...
			if (sec_chk_addr[i] == MVPU_CMD_BUFF_ADDR)
				continue;
...
uint32_t get_saved_session_id(void *session)
{
	...
	for (cnt = 0; cnt < MAX_SAVE_SESSION; cnt++) {
		if (saved_session[cnt] == 0xFFFFFFFFFFFFFFFF)
			continue;
...
void clear_session(void *session)
{
	...
	for (session_id = 0; session_id < MAX_SAVE_SESSION; session_id++) {
		if (saved_session[session_id] == 0xFFFFFFFFFFFFFFFF)
			continue;
...
void update_session_id(uint32_t session_id, void *session)
{
	...
	if (saved_session[session_id] == 0xFFFFFFFFFFFFFFFF && session != NULL) {
							...
							uint32_t *sec_buf_attr)
{
	...
	if (hash_pool[session_id]->hash_offset[hash_id] != NULL) {
		...
		for (cnt = 0; cnt < hash_pool[session_id]->buf_num[hash_id]; cnt++) {
			...
			case BUF_NORMAL:
			case BUF_RINGBUFFER:
				if (sec_chk_addr[cnt] != MVPU_CMD_BUFF_ADDR) {
					buf_kva = apusys_mem_query_kva_by_sess(
							session, sec_chk_addr[cnt]);
					if (buf_kva == NULL) {
						pr_info(
	"[MVPU][Sec] apusys_mem_query_kva_by_sess fail (session: 0x%llx, sec_chk_addr[%3d]: 0x%08x)\n",
					...
					uint32_t *primem_size)
{
	...
	pool_ptr_base = dma_buf_vmap(hash_pool[session_id]->hash_dma_buf[hash_id]);
							...
							bool protect_phase)
{
	...
	//get all IO buf addr
	for (i = 0; i < buf_num; i++) {
		if ((sec_chk_addr[i] >= VIRTUAL_APUSYS_TCM_BASE) &&
			(sec_chk_addr[i] < VIRTUAL_APUSYS_TCM_BASE_END))
			continue;
		...
		if (protect_phase == true) {
			...
			if ((sec_buf_attr[i] == BUF_IO) ||
				((sec_buf_attr[i] == BUF_NORMAL) &&
					(sec_chk_addr[i] != MVPU_CMD_BUFF_ADDR)) ||
				(sec_buf_attr[i] == BUF_KERNEL) ||
				(sec_buf_attr[i] == BUF_RINGBUFFER)) {
				buf_io_addr[buf_io_cnt] = sec_chk_addr[i];
				buf_io_cnt++;
			}
		}
						...
						bool protect_phase)
{
	...
	if (protect_phase == true) {
		...
		mpu_seg[buf_io_cnt + POOL_BASE_SFT] =
			hash_pool[session_id]->hash_base_iova[hash_id];
		...
		if (mvpu_loglvl_sec >= APUSYS_MVPU_LOG_DBG) {
			pr_info("[MVPU][SEC] [MPU] set algo region: 0x%08x - 0x%08x\n",
				mpu_seg[buf_io_cnt + IMG_BASE_SFT],
				mpu_seg[buf_io_cnt + IMG_END_SFT]);
...
bool mem_use_iova(uint32_t addr)
{
	if ((addr == MVPU_CMD_BUFF_ADDR) ||
		...
		((addr >= VIRTUAL_MVPU_TCM_BASE) &&  (addr < VIRTUAL_MVPU_TCM_BASE_END)) ||
		((addr & 0xFFFC0000) == 0))
		return false;
					...
					uint32_t chk_size)
{
	...
	case DESC_TYPE_EDMA:
		...
		if (desc_ptr == NULL) {
			pr_info(
			"[MVPU][Sec] apusys_mem_query_kva_by_sess fail (session: 0x%llx, chk_base: 0x%08x)\n",
			session, chk_base);
			ret = -ENOMEM;
			goto END;
		}
						...
						uint32_t knl_num)
{
	...
	uint32_t desc[8] = {7, 8, 10, 35, 36, 38, 0, 0};
	uint32_t sample = 1;
