...
static int mvpu_validation(void *hnd)
{
	...
	if ((batch_name_hash & MVPU_BATCH_MASK) !=
		...
		mdw_trace_end("[MVPU] mvpu_req integrity check, buf_num %d, rp_num %d end",
			buf_num, rp_num);

		// buf integrity check
		mdw_trace_begin("[MVPU] user buf integrity check, buf_num %d", buf_num);
		for (i = 0; i < buf_num; i++) {
			if (mvpu_loglvl_drv >= APUSYS_MVPU_LOG_DBG) {
		...
		mdw_trace_end("[MVPU] user buf integrity check, buf_num %d end", buf_num);
