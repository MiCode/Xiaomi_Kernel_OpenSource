...
unsigned int pmu_reg_read(u32 offset)
{
	...
	return ioread32(mdla_reg_control[0].apu_mdla_biu_top + offset);
}

static void pmu_reg_write(u32 value, u32 offset)
{
	//TODO: Need handle for Multi Core
	iowrite32(value, mdla_reg_control[0].apu_mdla_biu_top + offset);
}
...
void pmu_reset(void)
{
	...
	mdla_pmu_debug("mdla: %s, PMU_CFG_PMCR: 0x%x\n",
		__func__, pmu_reg_read(PMU_CFG_PMCR));
#endif
}
