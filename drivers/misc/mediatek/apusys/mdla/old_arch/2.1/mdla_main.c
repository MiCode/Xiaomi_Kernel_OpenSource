...
struct smp_deadline g_smp_deadline[PRIORITY_LEVEL];
...
static irqreturn_t mdla_irq0_handler(int irq, void *dev_id)
{
	...
	return mdla_scheduler_2_1(0);
}

static irqreturn_t mdla_irq1_handler(int irq, void *dev_id)
{
	//return mdla_interrupt(1);
	return mdla_scheduler_2_1(1);
}

static int mdla_smp_deadline_init(void)
{
	uint16_t i;

	for (i = 0; i < PRIORITY_LEVEL; i++) {
		spin_lock_init(&g_smp_deadline[i].lock);
		g_smp_deadline[i].deadline = 0;
	}
...
static int mdla_scheduler_init(struct device *dev)
{
	...
	for (i = 0; i < mdla_max_num_core; i++) {
		...
		sched->pro_ce = NULL;
		sched->enqueue_ce  = mdla_enqueue_ce_2_1;
		sched->dequeue_ce  = mdla_dequeue_ce_2_1;
		sched->process_ce  = mdla_process_ce_2_1;
		sched->issue_ce    = mdla_issue_ce_2_1;
		sched->complete_ce = mdla_complete_ce_2_1;
		sched->preempt_ce  = mdla_preempt_ce_2_1;
		sched->issue_dual_lowce = mdla_normal_dual_cmd_issue;
	...
	void *hnd, struct apusys_device *dev)//struct apusys_run_cmd *acd)
{
	...
	case APUSYS_CMD_EXECUTE:
	{
		...
		u16 priority = MDLA_LOW_PRIORITY;

		if (dev->dev_type == APUSYS_DEVICE_MDLA_RT)
			priority = MDLA_HIGH_PRIORITY;
		else
			priority = MDLA_LOW_PRIORITY;
