...
#include "mdla_mt6779_config.h"
#include "mdla_mt6779_hw_reg.h"
#include "mdla_mt6779_irq.h"
#include "mdla_mt6779_pmu.h"
...
static int mdla_plat_hw_e1_timeout_detect(int core_id)
{
	u32 ste_debug_if_1;

	ste_debug_if_1 = mdla_reg_read(core_id, 0x0EA8);
...
static unsigned long mdla_plat_get_wait_time(int core_id)
{
	unsigned long time;
	...
	if (mdla_dbg_read_u32(FS_CFG_TIMER_EN))
		...
		time = msecs_to_jiffies(
				mdla_dbg_read_u64(MDLA_POLLING_CMD_DONE_TIME));
	return time;
}
...
static int mdla_dts_map(struct platform_device *pdev)
{
	...
	if (mdla_mt6779_irq_request(dev, nr_core_ids))
		goto err_irq;
...
static void mdla_dts_unmap(struct platform_device *pdev)
{
	...
	mdla_mt6779_irq_release(&pdev->dev);
...
static int mdla_sw_multi_devices_init(void)
{
	...
	for (i = 0; i < nr_core_ids; i++) {
		...
		mdla_mt6779_pmu_init(&mdla_plat_devices[i]);
	}
...
static void mdla_sw_multi_devices_deinit(void)
{
	...
	for (i = 0; i < nr_core_ids; i++) {
		...
		mdla_mt6779_pmu_deinit(&mdla_plat_devices[i]);
...
static void mdla_mt6779_reset(int core, const char *str)
{
	unsigned long flags;
	struct mdla_dev *dev = mdla_get_device(core);

	if (unlikely(!dev)) {
		mdla_drv_debug("%s(): No mdla device (%d)\n", __func__, core);
		return;
	}
	...
	mdla_infracfg_ao_write(INFRA_TOPAXI_PROTECTEN_MCU_SET,
					VPU_CORE2_PROT_STEP1_0_MASK);

	while ((mdla_infracfg_ao_read(INFRA_TOPAXI_PROTECTEN_MCU_STA1) &
		VPU_CORE2_PROT_STEP1_0_ACK_MASK) !=
		VPU_CORE2_PROT_STEP1_0_ACK_MASK) {
	}

	// Reset
	mdla_apu_conn_set_b(APU_CONN_SW_RST, APU_CORE2_RSTB);
	mdla_apu_conn_clr_b(APU_CONN_SW_RST, APU_CORE2_RSTB);

	// Release Bus Prot
	mdla_infracfg_ao_write(INFRA_TOPAXI_PROTECTEN_MCU_CLR,
		VPU_CORE2_PROT_STEP1_0_MASK);
...
int mdla_mt6779_init(struct platform_device *pdev)
{
	...
	if (mdla_pwr_register(pdev, mdla_pwr_on_v1_0,
				mdla_pwr_off_v1_0, mdla_mt6779_reset))
		goto err_pwr;

	mdla_util_setup_decode_ops(mdla_decode_v1_0);
	...
	io->infra_cfg.read  = mdla_infracfg_ao_read;
	io->infra_cfg.write = mdla_infracfg_ao_write;
	io->infra_cfg.set_b = mdla_infracfg_ao_set_b;
	io->infra_cfg.clr_b = mdla_infracfg_ao_clr_b;
