...
#include "mdla_mt6873_hw_reg.h"
#include "mdla_mt6873_irq.h"
#include "mdla_mt6873_pmu.h"
#include "mdla_mt6873_config.h"
#include "mdla_mt6873_sched.h"
...
static int mdla_dts_map(struct platform_device *pdev)
{
	...
	if (mdla_mt6873_irq_request(dev, nr_core_ids))
		goto err_irq;
...
static void mdla_dts_unmap(struct platform_device *pdev)
{
	...
	mdla_mt6873_irq_release(&pdev->dev);
...
static int mdla_sw_multi_devices_init(void)
{
	...
	for (i = 0; i < nr_core_ids; i++) {
		...
		mdla_mt6873_pmu_init(&mdla_plat_devices[i]);
	}
...
static void mdla_sw_multi_devices_deinit(void)
{
	...
	for (i = 0; i < nr_core_ids; i++) {
		...
		mdla_mt6873_pmu_deinit(&mdla_plat_devices[i]);
...
static void mdla_mt6873_reset(int core, const char *str)
{
	unsigned long flags;
...
int mdla_mt6873_init(struct platform_device *pdev)
{
	...
	if (mdla_pwr_register(pdev, mdla_pwr_on_v1_x,
				mdla_pwr_off_v1_x, mdla_mt6873_reset))
		goto err_pwr;

	if (mdla_mt6873_sched_init())
		goto err_sched;
	...
	cmd_cb->get_irq_num         = mdla_mt6873_get_irq_num;
