...
static int mdla_dts_map(struct platform_device *pdev)
{
	...
	if (mdla_mt8195_irq_request(dev, nr_core_ids))
		goto err;
...
static void mdla_dts_unmap(struct platform_device *pdev)
{
	...
	mdla_mt8195_irq_release(&pdev->dev);
...
static int mdla_sw_multi_devices_init(void)
{
	...
	for (i = 0; i < nr_core_ids; i++) {
		...
		mdla_mt8195_pmu_init(&mdla_plat_devices[i]);
	}
...
static void mdla_sw_multi_devices_deinit(void)
{
	...
	for (i = 0; i < nr_core_ids; i++) {
		...
		mdla_mt8195_pmu_deinit(&mdla_plat_devices[i]);
...
static void mdla_mt8195_reset(int core, const char *str)
{
	unsigned long flags;
...
int mdla_mt8195_init(struct platform_device *pdev)
{
	...
	if (mdla_pwr_register(pdev, mdla_pwr_on_v2_0,
				mdla_pwr_off_v2_0, mdla_mt8195_reset))
		goto err_pwr;

	if (mdla_mt8195_sched_init())
		goto err_sched;
	...
	cmd_cb->get_irq_num         = mdla_mt8195_get_irq_num;
