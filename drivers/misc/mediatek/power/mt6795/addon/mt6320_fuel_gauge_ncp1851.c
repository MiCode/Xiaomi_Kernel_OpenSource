...
//remove it after feature option enabled
...
kal_int32 chip_diff_trim_value_en=0;
...
void fgauge_precharge_uninit(void)
{
    ...
    ret=pmic_config_interface(FGADC_CON0, 0x28, 0xFF, 0x0);
    ...
    if(g_ocv_lookup_done == 0)
    {
        profile_p = fgauge_get_profile(TEMPERATURE_T);
        if(profile_p != NULL)
    	    memset(profile_p, 0, sizeof(battery_profile_t2_bat1));
    
        profile_p_r_table = fgauge_get_profile_r_table(TEMPERATURE_T);
        if(profile_p_r_table != NULL)
    	    memset(profile_p_r_table, 0, sizeof(r_profile_t2_bat1));
    }
#else
    if(g_ocv_lookup_done == 0)
    {
...
MODULE_DESCRIPTION("MT6329 FGADC Device Driver NCP1851");
MODULE_LICENSE("GPL");
#endif
