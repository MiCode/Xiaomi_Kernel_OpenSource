 ...
 *
 * The 'fmt_sync' file can be opened many times as to create different
 * timelines.
 *
 * Fences can be created with FMT_SYNC_IOC_CREATE_FENCE ioctl with struct
 * fmt_sync_ioctl_create_fence as parameter.
 *
 * To increment the timeline counter, FMT_SYNC_IOC_INC ioctl should be used
 * with the increment as u32. This will update the last signaled value
 * from the timeline and signal any fence that has a seqno smaller or equal
 * to it.
 *
 * struct fmt_sync_ioctl_create_fence
 * @value:	the seqno to initialise the fence with
 * @name:	the name of the new sync point
 * @fence:	return the fd of the new sync_file with the created fence
 */
struct fmt_sync_create_fence_data {
	__u32	value;
	char	name[32];
...
/**
 * fmt_timeline_create() - creates a sync object
 * @name:	sync_timeline name
 *
 * Creates a new sync_timeline. Returns the sync_timeline object or NULL in
 * case of error.
 */
struct sync_timeline *fmt_timeline_create(const char *name)
{
...
static void timeline_fence_value_str(struct dma_fence *dma_fence,
				    char *str, int size)
{
	snprintf(str, size, "%lu", dma_fence->seqno);
}
...
/* opening fmt_sync create a new sync obj */
static int fmt_sync_open(struct inode *inode, struct file *file)
{
	struct sync_timeline *obj;
	...
	obj = fmt_timeline_create(task_comm);
	if (!obj)
		return -ENOMEM;
...
static long fmt_sync_ioctl_create_fence(struct sync_timeline *obj,
				       unsigned long arg)
{
	...
	struct fmt_sync_create_fence_data data;
...
static long fmt_sync_ioctl(struct file *file, unsigned int cmd,
			  unsigned long arg)
{
	struct sync_timeline *obj = file->private_data;
	...
	case FMT_SYNC_IOC_CREATE_FENCE:
		return fmt_sync_ioctl_create_fence(obj, arg);

	case FMT_SYNC_IOC_INC:
		return fmt_sync_ioctl_inc(obj, arg);
...
const struct file_operations fmt_sync_fops = {
	.open           = fmt_sync_open,
	.release        = fmt_sync_release,
	.unlocked_ioctl = fmt_sync_ioctl,
	.compat_ioctl	= fmt_sync_ioctl,
};
