...
int mc_clock_init(void)
{
	...
	if (IS_ERR(clk_ctx.mc_ce_core_src_clk)) {
		...
		mc_dev_notice("cannot get core src clock: %d", ret);
		goto error;
	}
	...
	if (ret) {
		...
		mc_dev_notice("cannot set core clock src rate: %d", ret);
		ret = -EIO;
		goto error;
	}
	...
	if (IS_ERR(clk_ctx.mc_ce_core_clk)) {
		...
		mc_dev_notice("cannot get core clock: %d", ret);
		goto error;
	}
	/* Get Interface clk */
	...
	if (IS_ERR(clk_ctx.mc_ce_iface_clk)) {
		...
		mc_dev_notice("cannot get iface clock: %d", ret);
		goto error;
	}
	/* Get AXI clk */
	...
	if (IS_ERR(clk_ctx.mc_ce_bus_clk)) {
		...
		mc_dev_notice("cannot get AXI bus clock: %d", ret);
		goto error;
	}
	return ret;
