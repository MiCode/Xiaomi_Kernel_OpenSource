...
struct cmdq_sec *gCmdq = NULL;
#if IS_ENABLED(CONFIG_MACH_MT6885)
struct cmdq_sec *gCmdq_m = NULL;
#endif
	...
	struct cmdq_sec_task *task, u32 thrd_idx)
{
	struct iwcCmdqMessage_t *iwc_msg =
		(struct iwcCmdqMessage_t *)context->iwc_msg;
	struct iwcCmdqMessageEx_t *iwc_msg_ex1 =
		(struct iwcCmdqMessageEx_t *)context->iwc_ex1;
	struct iwcCmdqMessageEx2_t *iwc_msg_ex2 =
		(struct iwcCmdqMessageEx2_t *)context->iwc_ex2;
...
static struct cmdq_sec_context *cmdq_sec_context_create(u32 tgid)
{
	struct cmdq_sec_context *context = NULL;

	context = kzalloc(sizeof(*context), GFP_ATOMIC);
	if (!context) {
		cmdq_err("[NOMEM] allocate sec_context failed, tgid[%u]",
			tgid);
		return NULL;
	} else {
		context->state = IWC_INIT;
		context->tgid = tgid;
	}

	cmdq_log("Alloc Sec_Context[0x%p] Success, tgid[%u]",
		context, tgid);
	...
	const u32 iwc_cmd, const u32 thrd_idx, void *data)
{
	...
	do {
		if (!cmdq->context) {
			cmdq_err("[Warn] cmdq->context is NULL");
		...
		if (cmdq->context->state == IWC_INIT)
			cmdq_sec_setup_tee_context(&cmdq->context->tee);
...
static int cmdq_sec_probe(struct platform_device *pdev)
{
	...
	if (!gCmdq)
		...
		cmdq_err("[%s] probe should ready done",
			__func__);
...
#ifdef CMDQ_LATE_INIT_SUPPORT
static s32 cmdq_sec_init_secure_path(void *data)
{
	s32 status = 0;
	struct cmdq_sec *cmdq = (struct cmdq_sec *)data;
	...
	while(1) {
		...
		if (!cmdq)
			continue;
		if (!cmdq->context) {
			cmdq_err(" allocate gContext again!");
			cmdq->context = cmdq_sec_context_create(current->tgid);

			continue; //check gContext again
		}
		...
		status = cmdq_sec_session_init(cmdq->context);

		mutex_unlock(&cmdq->exec_lock);

		if (status)
			cmdq_err("[Fail] late init secure path, status: %d",
				status);
		else {
			cmdq_msg("[Done] init secure path done! cmdq: %llx",
				cmdq);
			break;
		}
	}
...
static s32 __init cmdq_sec_late_init(void)
{
#ifdef CMDQ_LATE_INIT_SUPPORT
	struct task_struct *open_th =
		kthread_run(cmdq_sec_init_secure_path, gCmdq, "Context_WSM_init");

#if IS_ENABLED(CONFIG_MACH_MT6885)
	struct task_struct *open_th_m =
		kthread_run(cmdq_sec_init_secure_path, gCmdq_m, "Context_WSM_init_m");
	if (IS_ERR(open_th_m))
		cmdq_err("[%s] init kthread_run_m failed!\n", __func__);
#endif

	if (IS_ERR(open_th))
		cmdq_err("[%s] init kthread_run failed!\n", __func__);
#endif
