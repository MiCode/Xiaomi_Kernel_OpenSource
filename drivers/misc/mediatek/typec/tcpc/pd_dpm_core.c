...
static void dpm_build_sink_pdo_info(struct dpm_pdo_info_t *sink_pdo_info,
				uint32_t type, int request_v, int request_i)
{
	...
	sink_pdo_info->vmax = sink_pdo_info->vmin = request_v;
	sink_pdo_info->uw = request_i * request_v;
}
		...
		struct pd_port_power_caps *src_cap, uint8_t charging_policy)
{
	...
	dpm_build_sink_pdo_info(&sink_pdo_info, PDO_TYPE_APDO,
