...
int fusb303_fault_status_clear(struct tcpc_device *tcpc,
		uint8_t status)
{
	pr_info("%s: enter \n", __func__);
	return 0;
...
int fusb303_get_alert_status(struct tcpc_device *tcpc,
		uint32_t *alert)
{
	pr_info("%s: enter \n", __func__);
	return 0;
}
static int fusb303_get_power_status(struct tcpc_device *tcpc,
		uint16_t *pwr_status)
{
	pr_info("%s: enter \n", __func__);
	return 0;
}
int fusb303_get_fault_status(struct tcpc_device *tcpc,
		uint8_t *status)
{
	pr_info("%s: enter \n", __func__);
	return 0;
...
static int fusb303_set_low_rp_duty(struct tcpc_device *tcpc,
		bool low_rp)
{
	pr_info("%s: enter \n", __func__);
	return 0;
		...
		const struct i2c_device_id *id)
{
	...
	if (!chip) {
		dev_err(cdev, "%s: can't alloc fusb303_chip\n", __func__);
		return -ENOMEM;
	}
	chip->client = client;
	...
	if (ret != FUSB303_REV) {
		dev_err(cdev, "%s: FUSB303 not support\n", __func__);
		goto err1;
	}
	data = devm_kzalloc(cdev,
			sizeof(struct fusb303_data), GFP_KERNEL);
	if (!data) {
		dev_err(cdev, "%s: can't alloc fusb303_data\n", __func__);
		ret = -ENOMEM;
		goto err1;
	}
	chip->pdata = data;
	...
	chip->type = FUSB303_TYPE_INVALID;
	chip->state = FUSB_STATE_DISABLED;
	chip->bc_lvl = FUSB303_SNK_0MA;
	chip->ufp_power = 0;
	...
	if (!chip->cc_wq) {
		dev_err(cdev, "%s: unable to create workqueue fusb303-wq\n",
				__func__);
		goto err2;
	}
