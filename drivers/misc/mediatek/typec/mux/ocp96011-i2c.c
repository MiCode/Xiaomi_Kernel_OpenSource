...
//	int switch_control = 0;
	...
	case FSA_SWITCH_TO_AUDIO://config as Audio switch
		...
		if (fsa_priv->addr == 0x43)
			...
			headset_value = 1;
		blocking_notifier_call_chain(&fsa_priv->ocp96011_notifier, event, NULL);
		dev_info(fsa_priv->dev,"%s  headset plug in\n", __func__);
		break;
...
static int ocp96011_tcpc_init(struct ocp96011_priv *fsa_priv)
{
	...
	if (!tcpc_dev) {
		dev_err(fsa_priv->dev, "get tcpc device fail\n");
		return -ENODEV;
	}
	...
	ret = register_tcp_dev_notifier(tcpc_dev, &fsa_priv->tcpc_nb,
			TCP_NOTIFY_TYPE_USB);
	if (ret < 0) {
		dev_err(fsa_priv->dev, "register notifer fail\n");
		return -EINVAL;
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		dev_err(fsa_priv->dev, "failed to init tcpc\n");
		goto err_supply;
	}
#endif
	return 0;
