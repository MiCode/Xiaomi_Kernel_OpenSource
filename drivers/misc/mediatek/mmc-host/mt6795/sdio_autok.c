...
#define MAX_PAD_DELAY_IN_PS (6400)//(5000) /*wcl_RCcworst SS/LT/LV*/
#endif
...
#if (AUTOK_VCORE_SCAN_NUM == 1)
static unsigned int g_autok_vcore_sel[AUTOK_VCORE_SCAN_NUM] = {
    1150000};
#elif (AUTOK_VCORE_SCAN_NUM == 3)
static unsigned int g_autok_vcore_sel[AUTOK_VCORE_SCAN_NUM] = {
    #if 0
    1150000, 1212500, 1281250};
    #else
    1187500, 1237500, 1281250};
    #endif
#elif (AUTOK_VCORE_SCAN_NUM == 5)
static unsigned int g_autok_vcore_sel[AUTOK_VCORE_SCAN_NUM] = {
    1087500, 1150000, 1212500, 1281250, 1343750};
#endif
...
//extern void mt_cpufreq_volt_set_by_sdio(unsigned int volt);
...
//        pmic_read_interface(0x21E, &vcore_uv_offset, 0xFFFF, 0x0);
//    else if(vproc_vosel_ctrl == 1)
//        pmic_read_interface(0x220, &vcore_uv_offset, 0xFFFF, 0x0);
//#else   // PMIC MT6333
//    vcore_uv_offset = mt6333_get_reg_value(0x6C);
//#endif
    vcore_uv = vcorefs_sdio_get_vcore_nml();
...
ReScanCycle:
    ...
    while(k <= range_max) {
        ...
        if(bTryFindClkGenCycle == 1 && pad_delay_period_cycle == 0 && k >= minClkGenCycleScore) {
            ...
            for(x=0;x<=0/*k-minClkGenCycleScore*/ && pad_delay_period_cycle==0;x++) {
...
ReTuneCMDPad:
    ...
    memset(&CMDPadScan, 0, sizeof(AUTOK_RAWD_SCAN_T));
...
//#endif //ccyeh@xiaojian@ckgen = 3
        ...
        /* In case of Right boundary at CKGEN = 0 */
            ...
            /* Calculate the right boundary on baseCKGEN */
            if (baseCKGEN > 2)
                RBoundPad -= baseCKGEN * MIN_CLK_GEN_DELAY_IN_PS/MIN_PAD_DELAY_IN_PS;
            else
                RBoundPad -= baseCKGEN * DIV_CEIL_FUNC(MIN_CLK_GEN_DELAY_IN_PS, MIN_PAD_DELAY_IN_PS);
            
            AUTOK_PRINT("Right boundary found at initial timing(CKGEN=%d)(Rbound:%d vs. Margin.Max:%d)\r\n", 
                baseCKGEN, RBoundPad, DataPassWin/2);
        ...
        if (autok_rdata_scan[RealCKGEN].Reg1Cnt && 
            ...
            AUTOK_PRINT("Left boundary after shifting CKGEN(%d)\r\n", RealCKGEN);
        ...
        /* In case of right boundary by shifting CKGEN */
            ...
            AUTOK_PRINT("Right boundary after shifting CKGEN(%d)\r\n", RealCKGEN);

            /* 
             * If in the scenario below, try to select the middle point of 
             * pass window anyway, to avoid the possible fake right boundary
             * 1. Left boundary on baseCKGEN, and
             * 2. Right boundary on realCKGEN
             */
            if (autok_rdata_scan[RealCKGEN].BoundReg1_S && fLeftBoundary) {
                AUTOK_PRINT("To avoid possible fake right boundary\r\n");
                AUTOK_PRINT("Try to select middle point of pass window\r\n");

                autok_select_range(autok_rdata_scan[RealCKGEN].RawData, &RealPadDelay);
            }
            /* 
             ...
             */
            else if ((IntBound < 32) && ((RealPadDelay >= autok_rdata_scan[RealCKGEN].BoundReg1_S) ||
        ...
        if (autok_rdata_scan[RealCKGEN].RawData & (unsigned int)(1 << RealPadDelay)) {
            AUTOK_PRINT("[ERR] Invalid pad delay(%d): NOT within the pass window\r\n", RealPadDelay);
            AUTOK_ERR();
        }
...
static void autok_vcore_set(unsigned int vcore_uv) 
{
    ...
    #else
        #ifdef CONFIG_SDIOAUTOK_SUPPORT
        //mt_cpufreq_volt_set_by_sdio(mt65x2_vcore_tbl[vcore_sel]);
        ret = vcorefs_sdio_set_vcore_nml(vcore_uv);
        if(ret != 0){
            AUTOK_PRINT("[%d]Set vcore to %duV(0x%x) Failed\r\n", ret, mt65x2_vcore_tbl[vcore_sel], vcore_sel);   
            autok_abort_action();
        }
        #endif
...
static void autok_setup_envir(struct msdc_host *host)
{
    ...
    #elif (defined(MT6592LTE) || defined(MT6595WIFI))
        if (freq_mhz >= 200) {
            autok_vcore_scan_num = AUTOK_VCORE_SCAN_NUM;
            autok_rdat_fbound_th = 2;
        }
...
exit:
    ...
    // Do not restore voltage on 95 env
