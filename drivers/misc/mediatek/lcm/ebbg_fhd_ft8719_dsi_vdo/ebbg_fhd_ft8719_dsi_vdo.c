...
static void lcm_set_disp_param(unsigned int param)
{
	...
	ret = panel_disp_param_send_lock(PANEL_EBBG_FT8719, param, push_table);
	return;
}
...
static int lcm_get_lockdowninfo_for_tp(unsigned char *plockdowninfo)
{
	...
	read_tab.dtype = 0xF4;
	read_tab.payload = kmalloc(8 * sizeof(unsigned char), GFP_KERNEL);
	...
	for (i = 0; i < 8; i++) {
		...
		plockdowninfo[i] = read_tab.payload[i];
	}
...
static void lcm_init(void)
{
	...
	LCM_LOGI("ft8719----tps6132----lcm mode = vdo mode :%d----\n",
			 lcm_dsi_mode);
}
...
static unsigned int lcm_compare_id(void)
{
	...
	LCM_LOGI("%s,ft8719_id=0x%08x,version_id=0x%x\n",
		 __func__, id, version_id);

	if (id == LCM_ID_FT8719 && version_id == 0x81)
		return 1;
	else
		return 0;
...
#endif
struct LCM_DRIVER ebbg_fhd_ft8719_dsi_vdo_lcm_drv = {
	.name = "ebbg_fhd_ft8719_dsi_vdo_lcm_drv",
