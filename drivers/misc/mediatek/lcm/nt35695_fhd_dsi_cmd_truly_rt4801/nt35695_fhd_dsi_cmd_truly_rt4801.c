...
static void lcm_init_power(void)
{
	if (lcm_util.set_gpio_lcd_enp_bias)
		...
		pr_debug("set_gpio_lcd_enp_bias not defined...\n");
}
...
static void lcm_init(void)
{
	...
	if (lcm_dsi_mode == CMD_MODE) {
		...
		pr_debug("[LCM]nt35695----lcm mode = cmd mode :%d----\n",
			lcm_dsi_mode);
	} else {
		size = sizeof(init_setting2) /
			sizeof(struct LCM_setting_table);
		push_table(NULL, init_setting2, size, 1);
		pr_debug("[LCM]nt35695----lcm mode = vdo mode :%d----\n",
			lcm_dsi_mode);
	}
}
...
struct LCM_DRIVER nt35695_fhd_dsi_cmd_truly_rt4801_lcm_drv = {
