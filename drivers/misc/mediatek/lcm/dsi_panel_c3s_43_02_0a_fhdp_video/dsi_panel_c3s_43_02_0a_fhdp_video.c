...
static void lcm_suspend_power(void)
{
	if (lcd_reset_keep_high) {
		pr_info("[LCM]%s:tp_promixity_en_init_power\n",__func__);
		return;
	}
	pr_info("[LCM]%s\n",__func__);
...
extern nvt_ts_0x11_cmd(void);

static void lcm_suspend(void)
{
	if (lcd_reset_keep_high) {
		pr_info("[LCM]%s:tp_promixity_en_init_power\n",__func__);
		push_table(lcm_suspend_setting1,
			sizeof(lcm_suspend_setting1) / sizeof(struct LCM_setting_table),
			1);
		return;
	}
	pr_info("[LCM]%s\n",__func__);
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	pr_info("[LCM]%s,nt36525b backlight: level = %d ", __func__, level);
//	if((0 != level) && (level <= 14))
//		level = 14;
        level = level*73/100;
	...
	bl_level[0].para_list[1] = level & 0xFF;
        ...
        if (is_hbm_on == 1){
           bl_level[0].para_list[0] = 0x07;
           bl_level[0].para_list[1] = 0x32;
        }
...
struct LCM_DRIVER dsi_panel_c3s_43_02_0a_fhdp_video = {
	.name = "dsi_panel_c3s_43_02_0a_fhdp_video",
