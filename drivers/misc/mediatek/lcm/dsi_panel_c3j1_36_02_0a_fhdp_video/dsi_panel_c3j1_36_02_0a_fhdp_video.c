...
static unsigned int lcm_compare_id(void)
{
	...
	read_reg_v2(0x50, buffer, 2);
	id = buffer[0];	   /* we only need ID */

	read_reg_v2(0x51, buffer, 2);
	version_id = buffer[0];

	LCM_LOGI("%s,ft8719_fhdp_dsi_vdo_xinli_lcm, id=0x%x,version_id=0x%x\n", __func__, id, version_id);

	//if (id == 0x00 && version_id ==0x92)
	   return 1;
	//else
	   //return 0;
#endif
...
struct LCM_DRIVER dsi_panel_c3j1_36_02_0a_fhdp_video = {
	.name = "dsi_panel_c3j1_36_02_0a_fhdp_video",
