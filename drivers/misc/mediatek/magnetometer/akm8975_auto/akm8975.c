...
static int akm8975_init_flag =0; // 0<==>OK -1 <==> fail
...
static int akm8975_remove(void);
static int akm8975_local_init(void);
...
static struct sensor_init_info akm8975_init_info = {
		.name = "akm8975",
		.init = akm8975_local_init,
		.uninit = akm8975_remove,
...
static int AKECS_GetData(char *rbuf, int size)
{
	...
	if(loop_i >= AKM8975_RETRY_COUNT)
	{
		...
		if(0 ==factory_mode)
		{
		  return -1;//do not return in factory mode ,if return we can not get data
		}
	}
...
static int akm8975_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->hw = akm8975_get_cust_mag_hw();	
	...
	this_client = new_client;	
     ...
     printk(KERN_ERR "fwq AKM8975 akm8975_probe: befor init prob \n");
	/* Check connection */
	...
	if(err = akm8975_create_attr(&(akm8975_init_info.platform_diver_addr->driver)))
	{
...
static int akm8975_i2c_remove(struct i2c_client *client)
{
	...
	if(err = akm8975_delete_attr(&(akm8975_init_info.platform_diver_addr->driver)))
	{
...
static int akm8975_local_init(void) 
{
	struct mag_hw *hw = akm8975_get_cust_mag_hw();
...
{
	printk("akm8975_init \n");
	hwmsen_msensor_add(&akm8975_init_info);
	return 0;    
}
