...
static void fbt_do_jerk_locked(struct render_info *thr, struct fbt_jerk *jerk, int jerk_id)
{
	...
	case FPSGO_JERK_ONLY_CEILING_WAIT_ENQ:
	case FPSGO_JERK_ONLY_CEILING_WAIT_DEQ:
		fbt_do_jerk_boost(thr, temp_blc, 0);
		fbt_set_cap_margin_locked(0);
	...
	unsigned long long t_deq_len_ns, int target_fpks)
{
	...
	fpsgo_main_trace(
		"%s raw[%d]:%d raw[%d]:%d window_cnt:%d target_fpks:%d cnt:%d sum:%d avg:%d std_sqr:%d quota:%d mod:%d enq:%d enq_avg:%d",
...
SKIP:
	...
	for (i = 0; i < cluster_num; i++) {
		...
		if (clus_status[i]) {
			fpsgo_systrace_c_fbt_gm(-100, 0, i, "clus_iso");
			continue;
		}
