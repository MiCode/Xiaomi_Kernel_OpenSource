	...
	unsigned long long identifier)
{
	...
	case VSYNC_ALIGNED_TYPE:
		...
		FPSGO_COM_TRACE(
			"pid[%d] type[%d] enqueue_s:%llu frame_time:%llu buffer_id:%llu",
			pid, f_render->frame_type, enqueue_start_time,
			f_render->self_time, f_render->buffer_id);
		if (f_render->render_method == GLSURFACE) {
			xgf_ret = fpsgo_comp2xgf_qudeq_notify(pid,
					XGF_QUEUE_START, &slptime);
			if (xgf_ret != XGF_SLPTIME_OK)
				pr_debug(COMP_TAG"%s xgf_ret:%d",
						__func__, xgf_ret);
	...
	case NON_VSYNC_ALIGNED_TYPE:
		if (f_render->t_enqueue_start) {
			f_render->self_time = enqueue_start_time -
				f_render->t_enqueue_end -
		...
		f_render->t_enqueue_start = enqueue_start_time;
		FPSGO_COM_TRACE(
			"pid[%d] type[%d] enqueue_s:%llu frame_time:%llu",
			pid, f_render->frame_type,
			enqueue_start_time, f_render->self_time);
	...
	unsigned long long identifier)
{
	...
	case NON_VSYNC_ALIGNED_TYPE:
		...
		FPSGO_COM_TRACE(
			...
			dequeue_end_time, f_render->dequeue_length);
		xgf_ret =
			fpsgo_comp2xgf_qudeq_notify(pid, XGF_DEQUEUE_END, NULL);
		if (xgf_ret != XGF_NOTIFY_OK)
			pr_debug(COMP_TAG"%s xgf_ret:%d", __func__, xgf_ret);
		fpsgo_comp2fbt_deq_end(f_render, dequeue_end_time);
	...
	unsigned long long t_frame_start, unsigned long long id)
{
	...
	list_for_each_entry_safe(pos, next, &ui->render_list, ui_list) {
		...
		if (pos->frame_type != BY_PASS_TYPE) {
			if (pos->render_method != GLSURFACE) {
				for (i = 0; i < 2; i++) {
					if (pos->frame_id[i].complete == 1) {
						...
						FPSGO_COM_TRACE(
						"%s [%d] pid[%d] id[%llu] complete[%d] draw[%d]",
						__func__, i, pos->pid,
						pos->frame_id[i].id,
						pos->frame_id[i].complete,
						pos->frame_id[i].draw);
						break;
					}
				}
			...
			fpsgo_comp2fbt_frame_start(pos,
				t_frame_start, pos->sleep_time);
		}
	...
	int render, unsigned long long t_frame_done, unsigned long long id)
{
	...
	list_for_each_entry_safe(pos, next, &ui->render_list, ui_list) {
		...
		if (pos->frame_type != BY_PASS_TYPE) {
			...
			fpsgo_systrace_c_fbt_gm(-300, render,
				"%d_%d-render", pos->pid, pos->frame_type);
			FPSGO_COM_TRACE(
				"%s pid[%d] ui_pid[%d] type[%d] render[%d]",
				__func__, pos->pid, pos->ui_pid,
				pos->frame_type, render);
...
void fpsgo_ctrl2comp_vysnc_aligned_draw_start(int pid, unsigned long long id)
{
	...
	list_for_each_entry_safe(pos, next, &ui->render_list, ui_list) {
		fpsgo_thread_lock(&pos->thr_mlock);
		if (pos->frame_type != BY_PASS_TYPE &&
				pos->render_method != GLSURFACE) {
			for (i = 0; i < 2; i++) {
				if (pos->frame_id[i].id == id &&
					pos->frame_id[i].complete == 0) {
					pos->frame_id[i].draw++;
					FPSGO_COM_TRACE(
						"%s pid[%d] id[%llu] complete[%d] draw[%d]"
						, __func__, pos->pid,
						pos->frame_id[i].id,
						pos->frame_id[i].complete,
						pos->frame_id[i].draw);
	...
	if (render) {
		...
		for (i = 0; i < 2; i++) {
			if (f_render->frame_id[i].id == id &&
				...
				FPSGO_COM_TRACE(
					"%s [%d] pid[%d] id[%llu] complete[%d] draw[%d]",
					__func__, i, f_render->pid,
					f_render->frame_id[i].id,
					f_render->frame_id[i].complete,
					f_render->frame_id[i].draw);
				fpsgo_systrace_c_fbt_gm(-300,
						f_render->frame_id[i].complete,
						"%d_%d-complete", f_render->pid,
						f_render->frame_type);
				break;
			}
		}
...
out:
	fpsgo_comp2fstb_queue_time_update(pid, f_render->frame_type,
			...
			f_render->buffer_id, f_render->api);
	fpsgo_systrace_c_fbt_gm(-300, render,
			"%d_%d-render", pid, f_render->frame_type);
	FPSGO_COM_TRACE(
		"frame_done pid[%d] ui[%d] type[%d] frame_t:%llu render:%d method:%d",
	...
	int queue_SF, unsigned long long identifier, int create)
{
	...
	FPSGO_LOGI("bqid %d: bufid %llu, id %llu, queue_SF %d, create %d\n",
