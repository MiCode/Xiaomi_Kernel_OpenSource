...
static struct glg_context *glg_context_obj = NULL;
		...
		if(false==cxt->is_active_data)
		{
			...
			{ 
				...
				if(err)
				{
					...
					if(err)
					{
						GLG_ERR("enable_glance_gesture enable(%d) err 3 timers = %d\n", enable, err);
						return err;
					}
				}
			}
...
static ssize_t glg_show_enable_nodata(struct device* dev, 
...
/*----------------------------------------------------------------------------*/
static ssize_t glg_show_active(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	struct glg_context *cxt = NULL;
	cxt = glg_context_obj;
...
static ssize_t glg_store_delay(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	int len = 0;
...
static ssize_t glg_show_delay(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	int len = 0;
	GLG_LOG(" not support now\n");
...
static ssize_t glg_show_batch(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	int len = 0;
	GLG_LOG(" not support now\n");
...
static ssize_t glg_show_flush(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
    int len = 0;
	GLG_LOG(" not support now\n");
...
static ssize_t glg_show_devnum(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	char *devname = NULL;
...
//	cxt->glg_ctl.enable_nodata = ctl->enable_nodata;
	...
	err = sysfs_create_group(&glg_context_obj->mdev.this_device->kobj,
			&glg_attribute_group);
	if (err < 0)
	{
...
static int glg_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_group(&glg_context_obj->idev->dev.kobj,
				&glg_attribute_group);
	
	if((err = misc_deregister(&glg_context_obj->mdev)))
	{
...
{
	atomic_set(&(glg_context_obj->early_suspend), 0);
	if(!atomic_read(&glg_context_obj->wake) && resume_enable_status) //not wake up, disable in early suspend
	{
