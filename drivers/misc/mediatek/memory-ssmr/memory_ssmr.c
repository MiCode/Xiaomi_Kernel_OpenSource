...
struct reserved_mem *mpu_reserved;
...
static int __init debug_mpu_init(struct reserved_mem *rmem)
{
	mpu_reserved = rmem;
	pr_info("%s, name: %s, base: 0x%pa, size: 0x%pa\n",
	 __func__, mpu_reserved->name,
	&mpu_reserved->base, &mpu_reserved->size);

	return 0;
}

RESERVEDMEM_OF_DECLARE(debug_mpu_init, "mediatek,debug", debug_mpu_init);

static int __init debug_mpu_setup(void)
{
	struct emimpu_region_t md_region;

	mtk_emimpu_init_region(&md_region, 18);
	mtk_emimpu_set_addr(&md_region, mpu_reserved->base,
				mpu_reserved->base + mpu_reserved->size - 1);
