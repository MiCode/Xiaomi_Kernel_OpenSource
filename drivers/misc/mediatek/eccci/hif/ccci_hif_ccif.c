...
unsigned int ccb_wkcnt;
#define CCCIF_CH_LEN 200
unsigned int cccif_wkcnt[CCCIF_CH_LEN] = {0};
	...
	while (1) {
		...
		if (atomic_cmpxchg(&md_ctrl->wakeup_src, 1, 0) == 1) {
			CCCI_NOTICE_LOG(md_ctrl->md_id, TAG,
				...
				ccci_h->reserved);
			//xujia add for power analysis bugreport start
			if (ccci_h->channel < CCCIF_CH_LEN)
			cccif_wkcnt[ccci_h->channel]++;
                        //xujia add for power analysis bugreport end
		}
...
static void md_ccif_launch_work(struct md_ccif_ctrl *md_ctrl)
{
	...
	if (md_ctrl->channel_id & (1 << AP_MD_CCB_WAKEUP)) {
		...
		if (atomic_cmpxchg(&md_ctrl->wakeup_src, 1, 0) == 1) {
			...
			ccb_wkcnt++;//xujia add for power analysis bugreport
		}
