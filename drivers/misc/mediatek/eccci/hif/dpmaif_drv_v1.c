...
extern struct hif_dpmaif_ctrl *dpmaif_ctrl_v1;
...
static void drv1_dpmaif_dl_pit_only_update_enable_bit_done(unsigned char q_num)
{
...
static void drv1_dpmaif_check_dl_fifo_idle(void)
{
...
static int drv1_dpmaif_check_dl_awidx(void)
{
	...
	if (aidx == widx) {
		...
		drv1_dpmaif_dl_pit_init_done(0);

	}
	return 0;
}

static int drv1_dpmaif_dl_set_idle(bool set_en)
{
	int ret = 0;
	...
	if (set_en == true) {
		while (1) {
			drv1_dpmaif_dl_pit_en(0, false);
			drv1_dpmaif_dl_pit_only_update_enable_bit_done(0);
			while (drv1_dpmaif_dl_idle_check() != 0) {
				if (++count >= 1600000) {
					CCCI_MEM_LOG_TAG(0, TAG,
						"drv1_dpmaif_dl_idle poll\n");
					count = 0;
					ret = HW_REG_CHK_FAIL;
					break;
				}
			...
			if ((DPMA_READ_AO_DL(DPMAIF_AO_DL_PIT_STA3)&0x01)
				...
				drv1_dpmaif_check_dl_fifo_idle();
				break;
			}
			if (++count1 >= 1600000) {
				CCCI_ERROR_LOG(0, TAG,
		...
		ret = drv1_dpmaif_check_dl_awidx();
	} else {
		drv1_dpmaif_dl_pit_en(0, true);
		drv1_dpmaif_dl_pit_only_update_enable_bit_done(0);
	}
	return ret;
}
...
#if defined(_E1_SB_SW_WORKAROUND_)
	count = drv1_dpmaif_dl_set_idle(true);
	if (count < 0)
		return count;
	count = 0;
#endif
		...
		unsigned short pit_remain_cnt)
{
	...
	ret = drv1_dpmaif_dl_set_idle(true);
	if (ret < 0)
		return ret;
#endif
...
void drv1_dpmaif_mask_dl_interrupt(unsigned char q_num)
{
	...
	ui_que_done_mask |= (DPMAIF_DL_INT_BATCNT_LEN_ERR(q_num) |
		DPMAIF_DL_INT_PITCNT_LEN_ERR(q_num));
#endif
	drv1_dpmaif_set_dl_interrupt_mask(ui_que_done_mask);
...
void drv1_dpmaif_common_hw_init(void)
{
	...
	drv1_dpmaif_dl_set_wdma();
	drv1_dpmaif_md_hw_bus_remap();
...
int drv1_dpmaif_intr_hw_init(void)
{
	...
	DPMA_WRITE_AO_UL(NRL2_DPMAIF_AO_UL_APDL_L2TIMSR0,
			~(AP_DL_L2INTR_En_Msk));
	drv1_dpmaif_set_dl_interrupt_mask(~(AP_DL_L2INTR_En_Msk));
	/* 3. check mask sts*/
	count = 0;
...
int drv1_dpmaif_dl_restore(unsigned int mask)
{
	...
	drv1_dpmaif_set_dl_interrupt_mask((mask));
	/* 3. check mask sts*/
	while ((DPMA_READ_PD_MISC(DPMAIF_PD_AP_DL_L2TIMR0) &
		mask) != mask) {
		if (++count >= 1600000) {
...
#ifdef _HW_REORDER_SW_WORKAROUND_
static int drv1_dpmaif_set_dl_idle(bool set_en)
{
	int ret = 0;
	int count = 0, count1 = 0;
...
int drv1_dpmaif_dl_add_apit_num(unsigned short ap_entry_cnt)
{
	...
	count = drv1_dpmaif_set_dl_idle(true);
	if (count < 0)
		return count;
	count = 0;
