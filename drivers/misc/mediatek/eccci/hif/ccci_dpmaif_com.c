...
inline unsigned int ringbuf_get_next_idx(unsigned int total_cnt,
	unsigned int buf_idx, unsigned int cnt)
{
	buf_idx += cnt;

	if (buf_idx >= total_cnt)
		buf_idx -= total_cnt;

	return buf_idx;
}

inline unsigned int ringbuf_readable(unsigned int total_cnt,
...
inline unsigned int ringbuf_writeable(unsigned int total_cnt,
...
inline unsigned int ringbuf_releasable(unsigned int total_cnt,
...
static void dpmaif_dump_rxq_data(int qno)
{
	...
	if ((qno < 0) || (qno >= DPMAIF_RXQ_NUM))  //dump all rxq
		qno = -1;

	for (; i < DPMAIF_RXQ_NUM; i++) {
		if ((qno >= 0) && (i != qno))
			continue;
...
{
	rxq->pit_wr_idx = ops.drv_dl_get_wridx();

	return ringbuf_readable(rxq->pit_cnt, rxq->pit_rd_idx, rxq->pit_wr_idx);
}
	...
	unsigned int buffer_id, struct dpmaif_normal_pit_v2 *nml_pit_v2)
{
	...
	if (unlikely((skb->tail + nml_pit_v2->data_len) > skb->end)) {
		CCCI_ERROR_LOG(0, TAG,
			...
			__func__, rxq->pit_rd_idx, buffer_id, nml_pit_v2->data_len,
	...
	struct dpmaif_normal_pit_v2 *nml_pit_v2)
{
	...
	if (dpmaif_ctl->enable_pit_debug > 0)
		DPMAIF_DEBUG_ADD(DEBUG_TYPE_RX_DONE,
		DEBUG_VERION_V3,
		rxq->index, nml_pit_v2->data_len,
		rxq->pit_rd_idx, rxq->pit_wr_idx,
		(unsigned short)buffer_id,
		dpmaif_ctl->bat_skb->bat_wr_idx,
	...
	unsigned short pit_remain_cnt)
{
	...
	ret = ccci_drv_dl_add_pit_remain_cnt(pit_remain_cnt);
	if (ret < 0) {
		CCCI_ERROR_LOG(0, TAG,
	...
	unsigned short read_cnt)
{
	...
	struct dpmaif_normal_pit_v2 *nml_pit_v2;
	unsigned short pit_hw_update_cnt = 0;
	...
	for (rx_cnt = 0; rx_cnt < read_cnt; rx_cnt++) {
		...
		rxq->pit_rd_idx = ringbuf_get_next_idx(rxq->pit_cnt, rxq->pit_rd_idx, 1);
...
occur_err:
	CCCI_ERROR_LOG(0, TAG,
		...
		__func__, ret, rxq->pit_rd_idx, rx_cnt, read_cnt);
...
static int dpmaif_rxq_data_collect(struct dpmaif_rx_queue *rxq)
{
	...
	if (rd_cnt) {
		...
		if (real_cnt >= 0) {
			/* hw interrupt ack. */
			L2RISAR0 = (ccci_drv_get_dl_isr_event() & DPMAIF_DL_INT_QDONE_MSK);
			if (L2RISAR0) {
...
#ifdef DPMAIF_REDUCE_RX_FLUSH
	int pkg_count = 0, need_rx_flush;
#endif

	while (1) {
		if (ccci_dl_queue_len(qno) == 0
#ifdef ENABLE_DPMAIF_DEBUG_LOG
			&& ccci_get_debug_skb_cnt() == 0
#endif
			) {
#ifdef DPMAIF_REDUCE_RX_FLUSH
...
static void dpmaif_disable_irq(void)
{
	if (atomic_cmpxchg(&dpmaif_ctl->irq_enabled, 1, 0) == 1)
		disable_irq(dpmaif_ctl->irq_id);
}

static void dpmaif_enable_irq(void)
{
	if (atomic_cmpxchg(&dpmaif_ctl->irq_enabled, 0, 1) == 0)
		enable_irq(dpmaif_ctl->irq_id);
}
...
static int dpmaif_irq_init(void)
{
	int ret;

	/* request IRQ */
	ret = request_irq(dpmaif_ctl->irq_id, ops.drv_isr,
			dpmaif_ctl->irq_flags, "DPMAIF_AP", dpmaif_ctl);
	if (ret) {
		CCCI_ERROR_LOG(0, TAG,
			"[%s] error: request_irq(%d) fail -> %d\n",
			__func__, dpmaif_ctl->irq_id, ret);
		return ret;
	}

	if (g_dpmf_ver > 1) {  //version 2, 3
		ret = irq_set_irq_wake(dpmaif_ctl->irq_id, 1);
		if (ret)
			CCCI_ERROR_LOG(0, TAG,
	...
	atomic_set(&dpmaif_ctl->irq_enabled, 1);
...
#ifdef ENABLE_DPMAIF_DEBUG_LOG
		ccci_dpmaif_debug_late_init(&(dpmaif_ctl->rxq[0].rxq_wq));
#endif
	}
...
{
	txq->drb_rd_idx = (ops.drv_ul_get_rdidx(txq->index) / DPMAIF_UL_DRB_ENTRY_WORD);

	return ringbuf_releasable(txq->drb_cnt, txq->drb_rel_rd_idx, txq->drb_rd_idx);
}
	...
	for (idx = 0 ; idx < release_cnt; idx++) {
		...
		if (cur_drb->dtyp == DES_DTYP_PD) {
			...
			if (cur_drb_skb->skb == NULL) {
				CCCI_ERROR_LOG(0, TAG,
					"[%s] error: drb%d skb is NULL; txq pos: w/r/rel=(%u, %u, %u)\n",
					__func__, txq->index, txq->drb_wr_idx,
		...
		cur_idx = ringbuf_get_next_idx(txq->drb_cnt, cur_idx, 1);
	...
	unsigned int send_cnt)
{
	...
	/* 2. buffer check */
	remain_cnt = ringbuf_writeable(txq->drb_cnt,
	...
	struct sk_buff *skb, unsigned int prio_count, unsigned int send_cnt)
{
	...
	txq->drb_wr_idx = ringbuf_get_next_idx(txq->drb_cnt, txq->drb_wr_idx, send_cnt);
...
static void dpmaif_clear_traffic_data(void)
{
	...
	memset(dpmaif_ctl->rx_tfc_pkgs, 0, sizeof(dpmaif_ctl->rx_tfc_pkgs));
...
static int dpmaif_start(unsigned char hif_id)
{
	...
	irq_set_affinity_hint(dpmaif_ctl->irq_id, &imask);
...
static void dpmaif_total_spd_cb(u64 total_ul_speed, u64 total_dl_speed)
{
	if (dpmaif_ctl->enable_pit_debug > -1) {
		if ((total_ul_speed < UL_SPEED_THRESHOLD) &&
			...
			dpmaif_ctl->enable_pit_debug = 1;
		else
			dpmaif_ctl->enable_pit_debug = 0;
	}
...
static void dpmaif_init_cap(struct device *dev)
{
	...
	if (dpmaif_ctl->capability & DPMAIF_CAP_PIT_DEG)
		dpmaif_ctl->enable_pit_debug = 1;
	else
		dpmaif_ctl->enable_pit_debug = -1;
		...
		enum MODEM_DUMP_FLAG flag, void *buff, int length)
{
	...
	CCCI_NORMAL_LOG(0, TAG, "Dump AP DPMAIF IRQ status:\n");
	mt_irq_dump_status(dpmaif_ctl->irq_id);
#endif
...
static void dpmaif_traffic_monitor_func(struct timer_list *t)
{
	...
	if (3 < DPMAIF_TXQ_NUM) {
		...
		CCCI_NORMAL_LOG(0, TAG,
			...
			dpmaif_ctl->txq[0].drb_wr_idx,
			dpmaif_ctl->txq[0].drb_rd_idx,
			dpmaif_ctl->txq[0].drb_rel_rd_idx,
			dpmaif_ctl->txq[1].drb_wr_idx,
			dpmaif_ctl->txq[1].drb_rd_idx,
			dpmaif_ctl->txq[1].drb_rel_rd_idx,
			dpmaif_ctl->txq[2].drb_wr_idx,
			dpmaif_ctl->txq[2].drb_rd_idx,
			dpmaif_ctl->txq[2].drb_rel_rd_idx,
			dpmaif_ctl->txq[3].drb_wr_idx,
			dpmaif_ctl->txq[3].drb_rd_idx,
			dpmaif_ctl->txq[3].drb_rel_rd_idx,
	...
	for (i = 0; i < DPMAIF_RXQ_NUM; i++) {
		CCCI_NORMAL_LOG(0, TAG,
			...
			dpmaif_ctl->rxq[i].pit_wr_idx,
			dpmaif_ctl->rxq[i].pit_rd_idx);
	}
...
static int dpmaif_init_com(struct device *dev)
{
	...
	if (ccci_dl_pool_init(DPMAIF_RXQ_NUM))
		goto DPMAIF_INIT_FAIL;
