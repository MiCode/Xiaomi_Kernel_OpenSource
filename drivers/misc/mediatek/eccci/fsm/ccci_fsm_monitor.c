...
static char *fsm_monitor_name = {"ccci_monitor"};
...
static int dev_char_close(struct inode *inode, struct file *file)
{
	...
	if (ret)
		CCCI_ERROR_LOG(0, FSM, "force stop MD fail\n");
	return 0;
}
...
int fsm_monitor_init(struct ccci_fsm_monitor *monitor_ctl)
{
	...
	ret = alloc_chrdev_region(&monitor_ctl->dev_n, 0, 1, FSM_NAME);
	if (ret)
		CCCI_ERROR_LOG(0, FSM,
	...
	ret = ccci_register_dev_node(fsm_monitor_name,
