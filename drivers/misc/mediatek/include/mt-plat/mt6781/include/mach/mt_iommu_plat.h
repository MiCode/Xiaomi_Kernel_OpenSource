...
//#define MTK_APU_TFRP_SUPPORT no need for mt6781

#define MTK_IOMMU_SIZE_NOT_ALIGNMENT

#ifndef CONFIG_FPGA_EARLY_PORTING
//#define IOMMU_POWER_CLK_SUPPORT need turn on later for mt6781
...
const struct mtk_iova_domain_data mtk_domain_array[MTK_IOVA_DOMAIN_COUNT] = {
	...
	{ /* boundary(0~4GB) IOVA space for display(larb0,1) */
	 ...
	 .port_mask = {0x3f, 0xff, 0x0, 0x0, //0~3
			0x0, 0x0, 0x0, 0x0, //4~7
...
#endif
	{ /* boundary(4GB~8GB) IOVA space for video */
	  /* larb4,5,7,8 */
	 .boundary = 1,
	 .owner = -1,
	 ...
	 .port_mask = {0x0, 0x0, 0x0, 0x0, //0~3
			0x3fff, 0xff, 0x0, 0x7fff, //4~7
			0x7ffffff, 0x0, 0x0, 0x0, //8~11
			0x0, 0x0, 0x0, 0x0, //12~15
			0x0, 0x0, 0x0, 0x0, //16~19
			0x0, 0x0, 0x0, 0x0} //20~23
	},
	{ /* boundary(8GB~12GB) IOVA space for CAM &  MDP */
...
const struct mtk_iova_domain_data mtk_domain_array[MTK_IOVA_DOMAIN_COUNT] = {
	...
	{ //REE IOVA space
	 ...
	 .resv_type = IOVA_REGION_REMOVE,
	 .port_mask = {0x3f, 0xff, 0x1f, 0x0, //0~3
			0x3fff, 0xff, 0x0, 0x7fff, //4~7
...
struct mau_config_info mt6833_mau_info[MTK_IOMMU_M4U_COUNT] = {
	{
		.start = 0x40000000,
		.end = 0xffffffff,
