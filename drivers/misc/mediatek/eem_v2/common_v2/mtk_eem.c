...
#if IS_ENABLED(CONFIG_CPU_FORCE_TO_BIN2)
...
__func__, __LINE__, det->name, det->real_vboot, det->VBOOT);
			...
			if (det->real_vboot == det->VBOOT)
				...
				eem_error("det: %s, det->real_vboot != det->VBOOT\n", det->name);
...
static int __init eem_init(void)
{
	...
	mt_ppm_get_cluster_ptpod_fix_freq_idx_register(get_cluster_ptpod_fix_freq_idx);
	if (err) {
