 ...
 */
static ssize_t bus_tracer_watchpoint_filter_show(struct device_driver *driver,
		char *buf)
{
	return snprintf(buf, PAGE_SIZE, "Usage:\n"
		"echo $tracer_id $enabled $addr_h $addr $mask >\n"
		"/sys/bus/platform/drivers/bus_tracer/watchpoint_filter\n");
}

static ssize_t bus_tracer_watchpoint_filter_store(struct device_driver *driver,
		const char *buf, size_t count)
{
	struct bus_tracer_plt *plt = bus_tracer_drv.cur_plt;
...
static DRIVER_ATTR(watchpoint_filter, 0664, bus_tracer_watchpoint_filter_show,
		bus_tracer_watchpoint_filter_store);
 ...
 * setup bypass filter
 */
static ssize_t bus_tracer_bypass_filter_show(struct device_driver *driver,
		char *buf)
{
	return snprintf(buf, PAGE_SIZE, "Usage:\n"
			"echo $tracer_id $enabled $addr $mask >\n"
			"/sys/bus/platform/drivers/bus_tracer/bypass_filter\n");
}

static ssize_t bus_tracer_bypass_filter_store(struct device_driver *driver,
		const char *buf, size_t count)
{
	struct bus_tracer_plt *plt = bus_tracer_drv.cur_plt;
	char *p = (char *)buf, *arg;
...
static DRIVER_ATTR(bypass_filter, 0664, bus_tracer_bypass_filter_show,
		bus_tracer_bypass_filter_store);
 ...
 * setup id filter
 */
static ssize_t bus_tracer_id_filter_show(struct device_driver *driver,
		char *buf)
{
	return snprintf(buf, PAGE_SIZE, "Usage:\n"
		"echo $tracer_id $idf_id $enabled $ID >\n"
		"/sys/bus/platform/drivers/bus_tracer/id_filter\n");
}

static ssize_t bus_tracer_id_filter_store(struct device_driver *driver,
		const char *buf, size_t count)
{
	struct bus_tracer_plt *plt = bus_tracer_drv.cur_plt;
	char *p = (char *)buf, *arg;
...
static DRIVER_ATTR(id_filter, 0664, bus_tracer_id_filter_show,
		bus_tracer_id_filter_store);

/*
 * interface: /sys/bus/platform/drivers/bus_tracer/rw_filter
 * setup read/write filter
 */
static ssize_t bus_tracer_rw_filter_show(struct device_driver *driveri,
		char *buf)
{
	return snprintf(buf, PAGE_SIZE, "Usage:\n"
		"echo $tracer_id $read $write >\n"
		"/sys/bus/platform/drivers/bus_tracer/rw_filter\n");
}

static ssize_t bus_tracer_rw_filter_store(struct device_driver *driver,
		const char *buf, size_t count)
{
	struct bus_tracer_plt *plt = bus_tracer_drv.cur_plt;
	char *p = (char *)buf, *arg;
...
static DRIVER_ATTR(rw_filter, 0664, bus_tracer_rw_filter_show,
		bus_tracer_rw_filter_store);
#endif
/*
...
{
	/* FIXME: get the last status from ram console */
	snprintf(buf, PAGE_SIZE, "1\n");

	return strlen(buf);
}

static ssize_t last_status_store(struct device_driver *driver, const char *buf,
		size_t count)
{
	return count;
}
