...
#include "ov5670_flt_mipi_Sensor.h"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5670_FLT_MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				if(devinfo_add !=0)
			    {
				    ...
				    dev_ofilm->device_vendor = "Ofilm_flt";
...
UINT32 OV5670_FLT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	...
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	OV5670_FLT_MIPI_RAW_SensorInit	*/
