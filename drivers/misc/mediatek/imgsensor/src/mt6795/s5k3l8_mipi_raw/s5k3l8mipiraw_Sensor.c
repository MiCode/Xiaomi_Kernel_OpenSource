...
#define S5k3L8OTP_WRITE_ID         0xB0
static struct S5k3L8_MIPI_otp_struct current_otp;

inline kal_uint16 S5k3L8_read_cmos_sensor1(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	
	char puSendCmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };
	iReadRegI2C(puSendCmd , 2, (u8*)&get_byte, 1, S5k3L8OTP_WRITE_ID);
	return get_byte&0x00ff;
}

void S5k3L8_MIPI_read_otp_wb(struct S5k3L8_MIPI_otp_struct *otp)
{
   	...
   	FLG = S5k3L8_read_cmos_sensor1(0x0000); 
   
  	if(FLG==0)
		LOG_INF("No OTP Data or OTP data is invalid");
   	else
   	{
   		//golden_R = S5k3L8_read_cmos_sensor1(0x0005);
		//golden_Gr = S5k3L8_read_cmos_sensor1(0x0006);
 		//golden_Gb = S5k3L8_read_cmos_sensor1(0x0007);
		//golden_B = S5k3L8_read_cmos_sensor1(0x0008);
  		//LOG_INF("[S5k3L8] [S5k3L8_MIPI_read_otp_wb11] golden_R=0x%x, golden_Gr=0x%x, golden_Gb=0x%x, golden_B=0x%x\n", golden_R, golden_Gr, golden_Gb, golden_B);
		golden_R = 0x4b;
		golden_Gr = 0x8f;
 		golden_Gb = 0x8f;
		golden_B = 0x4e;
  		LOG_INF("[S5k3L8] [S5k3L8_MIPI_read_otp_wb] golden_R=0x%x, golden_Gr=0x%x, golden_Gb=0x%x, golden_B=0x%x\n", golden_R, golden_Gr, golden_Gb, golden_B);
  		current_R = S5k3L8_read_cmos_sensor1(0x0009);
  		current_Gr = S5k3L8_read_cmos_sensor1(0x000a);
  		current_Gb = S5k3L8_read_cmos_sensor1(0x000b);
  		current_B = S5k3L8_read_cmos_sensor1(0x000c); 
  		LOG_INF("[S5k3L8] [S5k3L8_MIPI_read_otp_wb] current_R=0x%x, current_Gr=0x%x, current_Gb=0x%x, current_B=0x%x\n", current_R, current_Gr, current_Gb, current_B);
   		...
   		LOG_INF("[S5k3L8] [S5k3L8_MIPI_read_otp_wb] r_ratio=0x%x, b_ratio=0x%x\n", otp->r_ratio, otp->b_ratio);      
   	}
}



void S5k3L8_MIPI_algorithm_otp_wb1(struct S5k3L8_MIPI_otp_struct *otp)
{
   ...
   LOG_INF("[S5k3L8] [S5k3L8_MIPI_algorithm_otp_wb1] R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", otp->R_Gain, otp->B_Gain, otp->G_Gain);    
}



void S5k3L8_MIPI_write_otp_wb(struct S5k3L8_MIPI_otp_struct *otp)
{
...
void S5k3L8_MIPI_update_wb_register_from_otp(void)
{
	...
	S5k3L8_MIPI_algorithm_otp_wb1(&current_otp);
...
static void sensor_init(void)
{
 ...
 S5k3L8_MIPI_update_wb_register_from_otp();
}	/*	sensor_init  */
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	LOG_INF("sensor_info->PDAF_Support = 1\n");
	#else 
	sensor_info->PDAF_Support = 0;
	LOG_INF("sensor_info->PDAF_Support = 0\n");
	#endif
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_INFO:
			LOG_INF("SENSOR_FEA O scenarioId:%d\n", *feature_data);
