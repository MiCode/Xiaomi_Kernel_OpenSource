...
#include "ov13853mipiraw_Sensor_mts.h"
...
//read pdaf data
int read_otp_pdaf_data_mts(kal_uint16 addr, BYTE* data, kal_uint32 size)
{
	int i = 0;
...
static bool OV13853_selective_read_eeprom_mts(kal_uint16 addr, BYTE* data)
{
...
int OV13853_selective_read_eeprom_mts_VCM_ID(kal_uint16 addr)
{
    int data = 0;
	char pu_send_cmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };
    if(addr > MAX_OFFSET)
...
static bool OV13853_read_eeprom_mts(kal_uint16 addr, BYTE* data, kal_uint32 size ){
	int i = 0;
	//int offset = addr;
	int offset = 0x79B;
	//for(i = 0; i < 1404; i++) {
	for(i = 0; i < 1372; i++) {
		if(!OV13853_selective_read_eeprom_mts(offset, &data[i])){
...
bool read_otp_pdaf_data_mts( kal_uint16 addr, BYTE* data, kal_uint32 size){
	
	LOG_INF("read_otp_pdaf_data_mts enter");
	if(!get_done || last_size != size || last_offset != addr) {
		//if(!_read_eeprom(addr, eeprom_data, size)){
		if(!OV13853_read_eeprom_mts(addr, data, size)){
			get_done = 0;
            last_size = 0;
            last_offset = 0;
			LOG_INF("read_otp_pdaf_data_mts fail");
			return false;
		}
	}
	//memcpy(data, eeprom_data, size);
	LOG_INF("read_otp_pdaf_data_mts end");
    return true;
}
