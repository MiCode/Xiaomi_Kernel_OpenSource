...
void do_async_setting(void *param)
{
	...
	setting_work->write_setting(setting_work->setting, setting_work->len);
	pr_info("do_async_setting : %d-%s - \n", setting_work->type, setting_work->name);
...
struct setting_work* create_and_queue_setting_work(
			...
			char *name,
			SETTING_TYPE type,
			unsigned short (*write_setting)(unsigned short *setting, unsigned int len),
			unsigned short *setting,
			unsigned int len)
{
	struct setting_work *setting_work = NULL;

	if (!name || !setting || !write_setting || !setting_workqueue) {
