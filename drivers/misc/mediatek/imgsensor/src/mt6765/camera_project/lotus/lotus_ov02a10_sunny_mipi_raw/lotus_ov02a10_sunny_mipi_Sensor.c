 ...
 * Filename:
 * ---------
 *     lotus_ov02a10_sunny_mipi_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "lotus_ov02a10_sunny_mipi_Sensor.h"

//#include <linux/hardware_info.h>	//For hardwareinfo

/****************************Modify Following Strings for Debug****************************/
#define PFX "lotus_ov02a10_sunny_camera_sensor"
#define LOG_1 LOG_INF("LOTUS_OV02A10_SUNNY,MIPI 1LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = LOTUS_OV02A10_SUNNY_SENSOR_ID,

	.checksum_value = 0x654e71b5, //0xf7375923, //checksum value for Camera Auto Test
...
#undef DEBUG_SENSOR
#ifdef DEBUG_SENSOR
#define lotus_ov02a10_sunnyMIPI_OP_CODE_INI		0x00		/* Initial value. */
#define lotus_ov02a10_sunnyMIPI_OP_CODE_REG		0x01		/* Register */
#define lotus_ov02a10_sunnyMIPI_OP_CODE_DLY		0x02		/* Delay */
#define lotus_ov02a10_sunnyMIPI_OP_CODE_END		0x03		/* End of initial setting. */
...
} lotus_ov02a10_sunnyMIPI_initial_set_struct;

lotus_ov02a10_sunnyMIPI_initial_set_struct lotus_ov02a10_sunnyMIPI_Init_Reg[1000];
...
u8 lotus_ov02a10_sunnyMIPI_Initialize_from_T_Flash(void)
{
	...
	fp = filp_open("/mnt/sdcard/lotus_ov02a10_sunny_sd", O_RDONLY , 0);
	if (IS_ERR(fp))
	{
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			lotus_ov02a10_sunnyMIPI_Init_Reg[i].op_code = lotus_ov02a10_sunnyMIPI_OP_CODE_REG;

			lotus_ov02a10_sunnyMIPI_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */

			lotus_ov02a10_sunnyMIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		...
		else 									/* DLY */
		{
			...
			curr_ptr += 4;
			lotus_ov02a10_sunnyMIPI_Init_Reg[i].op_code = lotus_ov02a10_sunnyMIPI_OP_CODE_DLY;

			lotus_ov02a10_sunnyMIPI_Init_Reg[i].init_reg = 0xFF;
			lotus_ov02a10_sunnyMIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
	...
	lotus_ov02a10_sunnyMIPI_Init_Reg[i].op_code = lotus_ov02a10_sunnyMIPI_OP_CODE_END;
	lotus_ov02a10_sunnyMIPI_Init_Reg[i].init_reg = 0xFF;
	lotus_ov02a10_sunnyMIPI_Init_Reg[i].init_val = 0xFF;
	i++;
	//for (j=0; j<i; j++)
	...
	for (j=0; j<i; j++)
	{
		if (lotus_ov02a10_sunnyMIPI_Init_Reg[j].op_code == lotus_ov02a10_sunnyMIPI_OP_CODE_END)	/* End of the setting. */
		{
			break ;
		}
		else if (lotus_ov02a10_sunnyMIPI_Init_Reg[j].op_code == lotus_ov02a10_sunnyMIPI_OP_CODE_DLY)
		{
			msleep(lotus_ov02a10_sunnyMIPI_Init_Reg[j].init_val);		/* Delay */
		}
		else if (lotus_ov02a10_sunnyMIPI_Init_Reg[j].op_code == lotus_ov02a10_sunnyMIPI_OP_CODE_REG)
		{

			write_cmos_sensor(lotus_ov02a10_sunnyMIPI_Init_Reg[j].init_reg, lotus_ov02a10_sunnyMIPI_Init_Reg[j].init_val);
			printk("%x = %x\n",lotus_ov02a10_sunnyMIPI_Init_Reg[j].init_reg,lotus_ov02a10_sunnyMIPI_Init_Reg[j].init_val);
		}
		else
		{
			printk("REG ERROR!\n");
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if(gain >= BASEGAIN && gain <= 15*BASEGAIN)
	{
		...
		if(iReg<=0x10)
		{
			...
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("LOTUS_OV02A10_SUNNYMIPI_SetGain = 16");
		}
		else if(iReg>= 0xa0)//gpw
		{
			...
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("LOTUS_OV02A10_SUNNYMIPI_SetGain = 160");
		}
		else
		{
			write_cmos_sensor(0xfd, 0x01);
			...
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("LOTUS_OV02A10_SUNNYMIPI_SetGain = %d",iReg);
		}
	}
	else
...
static void sensor_init(void)
{
	...
	if(fromsd == 1)//是否从SD读取//gepeiwei   120903
	{
		...
		lotus_ov02a10_sunnyMIPI_Initialize_from_T_Flash();//从SD卡读取的主要函数
...

	{
		...
		write_cmos_sensor(0x9d,0x06); // 0x16 by zyk for mipi test
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
                 ...
                 if(0x01 == vendor_id){
			         pr_info("ov02a10_sunny i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x\n",
                                                imgsensor.i2c_write_id,*sensor_id,vendor_id);
	                return ERROR_NONE;
                 }
			}
		     pr_info("ov02a10_sunny  check failed i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x\n",
                                                imgsensor.i2c_write_id,*sensor_id,vendor_id);
...
UINT32 LOTUS_OV02A10_SUNNY_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}    /*    LOTUS_OV02A10_SUNNYMIPISensorInit    */
