 ...
 * Filename:
 * ---------
 *	 lotus_s5k4h7yx_qtech_mipi_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "lotus_s5k4h7yx_qtech_mipiraw_Sensor.h"


#define PFX "lotus_s5k4h7yx_qtech_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = LOTUS_S5K4H7YX_QTECH_SENSOR_ID,
		...
		do {
			*sensor_id = return_sensor_id() + 1;
            if (*sensor_id == imgsensor_info.sensor_id) {
                  ...
                  if(0x06 == vendor_id){
                     pr_info("s5k4h7yx_qtech i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x\n",
                                                imgsensor.i2c_write_id,*sensor_id,vendor_id);
				    return ERROR_NONE;
                  }

             }
             pr_info("s5k4h7yx_qtech check failed i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x\n",
                                                imgsensor.i2c_write_id,*sensor_id,vendor_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id() + 1;
			LOG_INF("lotus_s5k4h7yx_qtech_mipiraw open sensor_id = %x\r\n", sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
...
UINT32 LOTUS_S5K4H7YX_QTECH_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
