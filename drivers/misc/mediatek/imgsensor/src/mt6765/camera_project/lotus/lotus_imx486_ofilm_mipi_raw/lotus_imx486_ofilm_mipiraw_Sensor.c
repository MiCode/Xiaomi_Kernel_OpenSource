 ...
 * Filename:
 * ---------
 *	 LOTUS_IMX486_OFILM_mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#define PFX "LOTUS_IMX486_OFILM_camera_sensor"
...
#include "lotus_imx486_ofilm_mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	/* LOTUS_IMX486_OFILM_MIPI_SENSOR_ID, sensor_id = 0x2680*/
	.sensor_id = LOTUS_IMX486_OFILM_SENSOR_ID,
...
static kal_uint16 lotus_imx486_ofilm_table_write_cmos_sensor(
...
#define LOTUS_IMX486_OFILM_MIPI_MaxGainIndex (193)
kal_uint16 LOTUS_IMX486_OFILM_MIPI_sensorGainMapping[LOTUS_IMX486_OFILM_MIPI_MaxGainIndex][2] = {
	{64, 0},
	{66, 16},
	{68, 30},
	{70, 44},
...
static void set_shutter(kal_uint64 shutter)
{
    ...
    unsigned int value_3004 = 0;
    unsigned int value_0202 = 0;
    unsigned int value_0203 = 0;

    unsigned int value_fll = 0;
    unsigned int value_llp = 0;

    LOG_INF("ENTER! shutter =%llu, framelength =%d, auto_extend=%d\n",
    ...
    if(shutter < 73658){
        if(cur_state){
            pr_info("set_shutter longexposue exit shutter =%llu\n",shutter);
            write_cmos_sensor(0x3004,0);
            cur_state = 0;
        }
    ...
    (shutter < imgsensor_info.min_shutter) ? imgsensor_info.min_shutter : shutter;
    	...
    	write_cmos_sensor(0x0104, 0x00);
    	LOG_INF("Exit! shutter =%llu, framelength =%d, auto_extend=%d\n",
    		shutter, imgsensor.frame_length, read_cmos_sensor(0x0350));
    }else{
          cur_state = 1;
          pr_info("set_shutter longexposue enter shutter =%llu\n",shutter);
        ...
        case 78266:
            value_3004 = 0x03;
            value_0202 = (cur_res == 1) ? 0x26 : 0x2C;
            value_0203 = (cur_res == 1) ? 0x39 : 0xF8;
            value_fll  = 0x22D;
            value_llp  = 0x6010;
            break;
        ...
        case 2504514:
            value_3004 = 0x07;
            value_0202 = (cur_res == 1) ? 0x4C : 0x54;
            value_0203 = (cur_res == 1) ? 0x71 : 0x4E;
            value_fll  = 0x5854;
            value_llp  = 0x6010;
            break;

        default: LOG_INF("ERROR! shutter =%llu not supported \n",shutter);
        }
        write_cmos_sensor(0x0104, 0x01);
        write_cmos_sensor(0x3004,value_3004);
        write_cmos_sensor(0x0202,value_0202);
        write_cmos_sensor(0x0203,value_0203);
        ...
        write_cmos_sensor(0x0340, value_fll & 0xFF);
        write_cmos_sensor(0x0341, value_fll >> 8);
        
        write_cmos_sensor(0x0342, value_llp & 0xFF);
        write_cmos_sensor(0x0343, value_llp >> 8);
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	return reg_gain;
#else
	for (i = 0; i < LOTUS_IMX486_OFILM_MIPI_MaxGainIndex; i++) {
		if (gain <= LOTUS_IMX486_OFILM_MIPI_sensorGainMapping[i][0])
			break;
	}
	if (gain != LOTUS_IMX486_OFILM_MIPI_sensorGainMapping[i][0])
		LOG_INF("Gain mapping don't correctly:%d %d\n", gain,
			LOTUS_IMX486_OFILM_MIPI_sensorGainMapping[i][0]);
	return LOTUS_IMX486_OFILM_MIPI_sensorGainMapping[i][1];
#endif
}
...
#if 0
	/*      preview_setting  */
static void lotus_imx486_ofilm_ImageQuality_Setting(void)
{
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_lotus_imx486_ofilm_[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x4979, 0x0d,
	0x3040, 0x03,
...
//From reg tool at 2018-1-6 16:54
	...
	lotus_imx486_ofilm_table_write_cmos_sensor(
		addr_data_pair_init_lotus_imx486_ofilm_,
		sizeof(addr_data_pair_init_lotus_imx486_ofilm_) /
		sizeof(kal_uint16));
#else
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_preview_lotus_imx486_ofilm_[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x08,
...
#if MULTI_WRITE
	lotus_imx486_ofilm_table_write_cmos_sensor(
		addr_data_pair_preview_lotus_imx486_ofilm_,
		sizeof(addr_data_pair_preview_lotus_imx486_ofilm_) /
		sizeof(kal_uint16));
#else
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_capture_15fps_lotus_imx486_ofilm_[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x10,
...
kal_uint16 addr_data_pair_capture_24fps_lotus_imx486_ofilm_[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x10,
...
kal_uint16 addr_data_pair_capture_30fps_lotus_imx486_ofilm_[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x10,
...
static void capture_setting(kal_uint16 curretfps, kal_uint8 pdaf_mode)
{
	...
	if (curretfps == 150) {
	lotus_imx486_ofilm_table_write_cmos_sensor(
		addr_data_pair_capture_15fps_lotus_imx486_ofilm_,
		sizeof(addr_data_pair_capture_15fps_lotus_imx486_ofilm_) /
		sizeof(kal_uint16));
	} else if (curretfps == 240) {
	lotus_imx486_ofilm_table_write_cmos_sensor(
		addr_data_pair_capture_24fps_lotus_imx486_ofilm_,
		sizeof(addr_data_pair_capture_24fps_lotus_imx486_ofilm_) /
		sizeof(kal_uint16));
	} else {
	lotus_imx486_ofilm_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_lotus_imx486_ofilm_,
		sizeof(addr_data_pair_capture_30fps_lotus_imx486_ofilm_) /
		sizeof(kal_uint16));
	}
#else
	if (curretfps == 150) {
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_hs_video_lotus_imx486_ofilm_[] = {
       0x0112, 0x0A,
       0x0113, 0x0A,
       0x0114, 0x03,
       0x0342, 0x08,
...
#if MULTI_WRITE
	lotus_imx486_ofilm_table_write_cmos_sensor(
		addr_data_pair_hs_video_lotus_imx486_ofilm_,
		sizeof(addr_data_pair_hs_video_lotus_imx486_ofilm_) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x0112, 0x0A);
	write_cmos_sensor(0x0113, 0x0A);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	s_DEVINFO_ccm->device_ic = "LOTUS_IMX486_OFILM_";
...
static kal_uint32 open(void)
{
	...
	LOG_INF("LOTUS_IMX486_OFILM_,MIPI 4LANE\n");
	/* sensor have two i2c address 0x6c 0x6d & 0x21 0x20,
	 * we should detect the module used i2c address
	 */
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		...
		#if 0
		read_lotus_imx486_ofilm_pdaf((kal_uint16) (*feature_data),
...
UINT32 LOTUS_IMX486_OFILM_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
