 ...
 * Filename:
 * ---------
 *	 lotus_s5k4h7yx_ofilm_mipi_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "lotus_s5k4h7yx_ofilm_mipiraw_Sensor.h"


#define PFX "lotus_s5k4h7yx_ofilm_camera_sensor"
#define LOG_INF(format, args...)	pr_debug(PFX "[%s] " format, __func__, ##args)


static DEFINE_SPINLOCK(imgsensor_drv_lock);
extern int get_s5k4h7yx_vendor_id(void);
extern int apply_s5k4h7yx_otp(void);
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = LOTUS_S5K4H7YX_OFILM_SENSOR_ID,
...
#if 0
static unsigned char S5K4H7_read_cmos_sensor(u32 addr)
{
	kal_uint16 get_byte = 0;
...
static void preview_setting(void)
{
	...
	write_cmos_sensor_8(0X3400, 0X00);// lsc enable
#ifdef LEGACY_MODE
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor_8(0X0202, 0X02);
	write_cmos_sensor_8(0X0203, 0X08);
	write_cmos_sensor_8(0X3400, 0X00);//enable lsc
#ifdef LEGACY_MODE
	write_cmos_sensor_8(0x0100, 0x01);
#endif
}
		...
		do {
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				  ...
				  vendor_id = get_s5k4h7yx_vendor_id();
                  if(0x07 == vendor_id){
                     pr_info("s5k4h7yx_ofilm i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x\n",
            ...
            pr_err("s5k4h7yx_ofilm check id failed i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x\n",
                                        imgsensor.i2c_write_id,*sensor_id,vendor_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			LOG_INF("lotus_s5k4h7yx_ofilm_mipiraw open sensor_id = %x\r\n", sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
    ...
    if((apply_s5k4h7yx_otp()))
    {
        LOG_INF("lotus_s5k4h7yx_ofilm otp update failed!");
    }
...
UINT32 LOTUS_S5K4H7YX_OFILM_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
