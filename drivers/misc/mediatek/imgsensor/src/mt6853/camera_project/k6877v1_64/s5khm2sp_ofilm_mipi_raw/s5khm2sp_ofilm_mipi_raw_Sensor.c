 ...
 * Filename:
 * ---------
 *	 S5KHM2SP_OFILMmipiraw_sensor.c
 *
 * Project:
 * --------
 *	 ALPS MT6795
...
#include "s5khm2sp_ofilm_mipi_raw_Sensor.h"
...
#define LOG_TAG "s5khm2sp_ofilm_"
#define S5KHM2SP_OFILM_LOG_INF(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
#define S5KHM2SP_OFILM_LOG_DBG(format, args...) pr_debug(LOG_TAG "[%s] " format, __func__, ##args)
#define VENDOR_ID 0x07

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KHM2SP_OFILM_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[11] = {
	...
	{ 12000, 9000,    0,   374,  12000, 6756,  4000, 2252,    0,    0,  4000, 2252,    0,    0,  4000, 2252}, // video
	...
	{ 12000, 9000,    0,   374,  12000, 6756,  4000, 2252,    0,    0,  4000, 2252,    0,    0,  4000, 2252}, // custom3 as 12M @ 60fps
	{ 12000, 9000,    0,     0,  12000, 9000, 12000, 9000,    0,    0, 12000, 9000,    0,    0, 12000, 9000}, // custom4 full size capture
	{ 12000, 9000, 4500,  3374,   3000, 2252,  3000, 2252,    0,    0,  3000, 2252,    0,    0,  3000, 2252}, // custom5 in sensor zoom4x
...
static kal_uint32 return_sensor_id(void)
{
	...
	S5KHM2SP_OFILM_LOG_DBG("[%s] sensor_id: 0x%x", __func__, sensor_id);
...
{
	S5KHM2SP_OFILM_LOG_DBG("dummyline = %d, dummypixels = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	S5KHM2SP_OFILM_LOG_DBG("framerate = %d, min framelength =%d, should enable? \n", framerate, min_framelength_en);
...
static void set_shutter(kal_uint32 shutter)
{
	...
	S5KHM2SP_OFILM_LOG_DBG("shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length);
}
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length, kal_bool auto_extend_en)
{
	...
	S5KHM2SP_OFILM_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length);
}
...
static void set_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
    ...
    S5KHM2SP_OFILM_LOG_DBG("write awb gain r:g:b %d:%d:%d \n", r_Gain, g_Gain, b_Gain);
}
#endif
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	S5KHM2SP_OFILM_LOG_DBG("set_gain %d \n", gain);
	if (gain < imgsensor_info.min_gain || gain > imgsensor_info.max_gain) {
		S5KHM2SP_OFILM_LOG_DBG("Error gain setting");
	...
	spin_unlock(&imgsensor_drv_lock);
	S5KHM2SP_OFILM_LOG_DBG("gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
...
{
	S5KHM2SP_OFILM_LOG_DBG("le:0x%x, se:0x%x, gain:0x%x\n", le, se, gain);
...
static kal_uint32 get_sensor_temperature(void)
{
	...
	S5KHM2SP_OFILM_LOG_DBG("temp_c(%d), read_reg(%d)\n", temperature_convert, temperature);
	return temperature_convert;
}

static kal_uint32 streaming_control(kal_bool enable)
{
	S5KHM2SP_OFILM_LOG_DBG("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
...
#if MULTI_WRITE
#if 0 /*for debug*/
	S5KHM2SP_OFILM_LOG_DBG("[s5khm2sp_ofilm_-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d start", len);
	for (i = 0; i < len/2; i++)
		S5KHM2SP_OFILM_LOG_DBG("[s5khm2sp_ofilm_-debug]readback addr 0x%04x, 0x%02x,\n",
			para[2*i], read_cmos_sensor(para[2*i]) & 0xFF);
	S5KHM2SP_OFILM_LOG_DBG("[s5khm2sp_ofilm_-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d end", len);
#endif
#endif
	return 0;
}
...
/*static void read_cali_data_from_eeprom(void)
{
	...
	if (ret != 0) {
		S5KHM2SP_OFILM_LOG_DBG("iReadRegI2C error");
		return;
	}
...
{
	if (Is_Read_Otp_Data == 1) {
		S5KHM2SP_OFILM_LOG_DBG("write pdc to sensor E\n");
...
{
	if (Is_Read_Otp_Data == 1) {
		S5KHM2SP_OFILM_LOG_DBG("write xtalk to sensor E\n");
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x0000, 0x0F1E);
    ...
    S5KHM2SP_OFILM_LOG_DBG("write sensor init setting\n");
    // Global
	table_write_cmos_sensor(s5khm2sp_ofilm__init_setting,
			sizeof(s5khm2sp_ofilm__init_setting) / sizeof(kal_uint16));
}	/*	sensor_init  */

static void preview_setting(void)
{
	S5KHM2SP_OFILM_LOG_DBG("write sensor previews setting\n");

	table_write_cmos_sensor(s5khm2sp_ofilm__preview_setting,
		sizeof(s5khm2sp_ofilm__preview_setting) / sizeof(kal_uint16));
}	/*	preview_setting  */

static void capture_setting(kal_uint16 currefps)
{
	S5KHM2SP_OFILM_LOG_DBG("E\n");
...
static void normal_video_setting(void)
{
	...
	table_write_cmos_sensor(s5khm2sp_ofilm__normal_video_setting,
		sizeof(s5khm2sp_ofilm__normal_video_setting) / sizeof(kal_uint16));
}
...
static void hs_video_setting(void)
{
	...
	table_write_cmos_sensor(s5khm2sp_ofilm__hs_video_setting,
		sizeof(s5khm2sp_ofilm__hs_video_setting) / sizeof(kal_uint16));
}
...
static void custom2_setting(void)
{
	...
	table_write_cmos_sensor(s5khm2sp_ofilm__custom2_setting,
			sizeof(s5khm2sp_ofilm__custom2_setting) / sizeof(kal_uint16));
}
...
static void custom3_setting(void)
{
	...
	table_write_cmos_sensor(s5khm2sp_ofilm__custom3_setting,
			sizeof(s5khm2sp_ofilm__custom3_setting) / sizeof(kal_uint16));
}
...
static void custom4_setting(void)
{
	...
	table_write_cmos_sensor(s5khm2sp_ofilm__custom4_setting,
			sizeof(s5khm2sp_ofilm__custom4_setting) / sizeof(kal_uint16));
}
...
static void custom5_setting(void)
{
	...
	table_write_cmos_sensor(s5khm2sp_ofilm__custom5_setting,
			sizeof(s5khm2sp_ofilm__custom5_setting) / sizeof(kal_uint16));
}
...
static void custom6_setting(void)
{
	S5KHM2SP_OFILM_LOG_DBG("E\n");
	//custom3_setting();
	table_write_cmos_sensor(s5khm2sp_ofilm__custom6_setting,
			sizeof(s5khm2sp_ofilm__custom6_setting) / sizeof(kal_uint16));
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	S5KHM2SP_OFILM_LOG_DBG("S5KHM2SP_OFILM get_imgsensor_id in\n");
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				S5KHM2SP_OFILM_LOG_DBG("probe success, i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				S5KHM2SP_OFILM_LOG_DBG("K16 not support s5khm2sp");
				*sensor_id = 0xFFFFFFFF;
				return ERROR_SENSOR_CONNECT_FAIL;
			} else {
				S5KHM2SP_OFILM_LOG_DBG("Read sensor id fail,i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			}
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	S5KHM2SP_OFILM_LOG_DBG("PLATFORM:MT6893, MIPI 3LANE\n");
	//S5KHM2SP_OFILM_LOG_DBG("preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n");
	//sensor have two i2c address 0x6c 0x6d & 0x21 0x20, we should detect the module used i2c address
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				S5KHM2SP_OFILM_LOG_DBG("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			S5KHM2SP_OFILM_LOG_DBG("Read sensor id fail,i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
...
{
	S5KHM2SP_OFILM_LOG_DBG("loading preview_setting\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2SP_OFILM_LOG_DBG("loading capture_setting\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2SP_OFILM_LOG_DBG("loading normal_video_setting\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2SP_OFILM_LOG_DBG("loading hs_video_setting\n");
...
static kal_uint32 custom1(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2SP_OFILM_LOG_DBG("loading custom1_setting\n");
...
static kal_uint32 custom2(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2SP_OFILM_LOG_DBG("loading custom2_setting\n");
...
static kal_uint32 custom3(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2SP_OFILM_LOG_DBG("loading custom3_setting\n");
...
static kal_uint32 custom4(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2SP_OFILM_LOG_DBG("loading custom4_setting\n");
...
static kal_uint32 custom5(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2SP_OFILM_LOG_DBG("loading custom5_setting\n");
...
static kal_uint32 custom6(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2SP_OFILM_LOG_DBG("loading custom6_setting\n");
...
static kal_uint32 get_info(enum MSDK_SCENARIO_ID_ENUM scenario_id,
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2SP_OFILM_LOG_DBG("scenario_id = %d\n", scenario_id);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case MSDK_SCENARIO_ID_CUSTOM6:
		...
		break;
	default:
		S5KHM2SP_OFILM_LOG_DBG("Error ScenarioId setting");
		preview(image_window, sensor_config_data);
...
{
	S5KHM2SP_OFILM_LOG_DBG("framerate = %d\n ", framerate);
...
{
	S5KHM2SP_OFILM_LOG_DBG("enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	S5KHM2SP_OFILM_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		...
		if (imgsensor.frame_length > imgsensor.shutter) {
			...
			/*No need to set*/
			S5KHM2SP_OFILM_LOG_DBG("frame_length %d < shutter %d", imgsensor.frame_length, imgsensor.shutter);
		}
		break;
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
		if (framerate == 0)
	...
	default:  //coding with  preview scenario by default
		...
		S5KHM2SP_OFILM_LOG_DBG("error scenario_id = %d, we use preview scenario \n", scenario_id);
		break;
	}
	return ERROR_NONE;
}
...
{
	S5KHM2SP_OFILM_LOG_DBG("enable: %d\n", enable);
		...
		UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	S5KHM2SP_OFILM_LOG_DBG("feature_id = %d\n", feature_id);
	switch (feature_id) {
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		S5KHM2SP_OFILM_LOG_DBG("current fps :%d\n", (UINT32)*feature_data);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.current_fps = *feature_data;
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_SET_HDR:
		S5KHM2SP_OFILM_LOG_DBG("ihdr enable :%d\n", (BOOL)*feature_data);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.ihdr_en = (BOOL)*feature_data;
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_GET_CROP_INFO:
		S5KHM2SP_OFILM_LOG_DBG("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n", (UINT32)*feature_data);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		S5KHM2SP_OFILM_LOG_DBG("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n", (UINT16)*feature_data, (UINT16)*(feature_data + 1), (UINT16)*(feature_data + 2));
	...
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		S5KHM2SP_OFILM_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		S5KHM2SP_OFILM_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(*feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		S5KHM2SP_OFILM_LOG_DBG("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
	...
	case SENSOR_FEATURE_GET_VC_INFO:
		S5KHM2SP_OFILM_LOG_DBG("SENSOR_FEATURE_GET_VC_INFO %d\n",
			(UINT16) *feature_data);
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		S5KHM2SP_OFILM_LOG_DBG("odin GET_PDAF_DATA EEPROM\n");
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
		case MSDK_SCENARIO_ID_CUSTOM1:
			S5KHM2SP_OFILM_LOG_DBG("get pd info\n");
	...
	case SENSOR_FEATURE_SET_PDAF:
		...
		S5KHM2SP_OFILM_LOG_DBG("[odin] pdaf mode : %d \n", imgsensor.pdaf_mode);
		break;
...
UINT32 S5KHM2SP_OFILM_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    S5KHM2SP_OFILM_LOG_DBG("S5KHM2SP_OFILM_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	S5KHM2SP_OFILM_MIPI_RAW_SensorInit	*/
