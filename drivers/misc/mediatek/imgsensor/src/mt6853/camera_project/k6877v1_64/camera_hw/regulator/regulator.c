...
struct REGULATOR_CTRL regulator_control_wl2868c[REGULATOR_TYPE_MAX_NUM] = {
	{"vcamaw"},
	{"vcamdw"},
	{"vcamiow"},
};
	...
	struct IMGSENSOR_HW_DEVICE_COMMON *pcommon)
{
	...
	for (idx = IMGSENSOR_SENSOR_IDX_MIN_NUM;
		...
		for (type = 0;
			...
			pr_err("%s :camera_power=%d",__func__,camera_power);
		    if(camera_power==2)
		    {
				ret = snprintf(str_regulator_name,
					sizeof(str_regulator_name),
					"cam%d_%s",
					idx,
					regulator_control_wl2868c[type].pregulator_type);
		    }
			if (ret < 0)
				return ret;
