...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC02M1B_QTECH_DEPTH_SENSOR_ID,
...
unsigned int
gc02m1b_read_otp_info(struct i2c_client *client,
...
static void gc02m1b_read_otp(void)
{
	int i = 0;
			...
			if (vendor_id == 0x06) {
				*sensor_id = return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
					LOG_INF("QT i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
					gc02m1b_read_otp();
					return ERROR_NONE;
				}
			}
