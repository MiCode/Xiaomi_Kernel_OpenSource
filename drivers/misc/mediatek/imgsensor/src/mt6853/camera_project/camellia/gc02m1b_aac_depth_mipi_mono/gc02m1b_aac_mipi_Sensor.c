...
#include "gc02m1b_aac_mipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc02m1b_aac_camera_sensor"
#define LOG_1 LOG_INF("GC02M1B_AAC, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC02M1B_AAC_DEPTH_SENSOR_ID,
...
kal_uint16 addr_data_pair_init_gc02m1b_aac[] = {
	/*system*/
	0xfc, 0x01,
	0xf4, 0x41,
	0xf5, 0xc0,
...
kal_uint16 addr_data_pair_preview_gc02m1b_aac[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_capture_gc02m1b_aac[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_normal_video_gc02m1b_aac[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_hs_video_gc02m1b_aac[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_slim_video_gc02m1b_aac[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_custom1_gc02m1b_aac[] = {
	0x41, 0x04,
	0x42, 0xf4,
	0x07, 0x00,
	0x08, 0x18,
...
static void sensor_init(void)
{
	...
	gc02m1b_table_write_cmos_sensor(
	    addr_data_pair_init_gc02m1b_aac,
		sizeof(addr_data_pair_init_gc02m1b_aac) /
		sizeof(kal_uint16));
}
...
static void preview_setting(void)
{
	LOG_INF("E\n");
	gc02m1b_table_write_cmos_sensor(
		addr_data_pair_preview_gc02m1b_aac,
		sizeof(addr_data_pair_preview_gc02m1b_aac) /
		sizeof(kal_uint16));
}
...
static void capture_setting(void)
{
	LOG_INF("E\n");
	gc02m1b_table_write_cmos_sensor(
		addr_data_pair_capture_gc02m1b_aac,
		sizeof(addr_data_pair_capture_gc02m1b_aac) /
		sizeof(kal_uint16));
}
...
static void normal_video_setting(void)
{
	LOG_INF("E\n");
	gc02m1b_table_write_cmos_sensor(
	    addr_data_pair_normal_video_gc02m1b_aac,
		sizeof(addr_data_pair_normal_video_gc02m1b_aac) /
		sizeof(kal_uint16));
}
...
static void hs_video_setting(void)
{
	LOG_INF("E\n");
	gc02m1b_table_write_cmos_sensor(
		addr_data_pair_hs_video_gc02m1b_aac,
		sizeof(addr_data_pair_hs_video_gc02m1b_aac) /
		sizeof(kal_uint16));
}
...
static void slim_video_setting(void)
{
	LOG_INF("E\n");
	gc02m1b_table_write_cmos_sensor(
		addr_data_pair_slim_video_gc02m1b_aac,
		sizeof(addr_data_pair_slim_video_gc02m1b_aac) /
		sizeof(kal_uint16));
}
...
static void custom1_setting(void)
{
	LOG_INF("E\n");
	gc02m1b_table_write_cmos_sensor(
		addr_data_pair_custom1_gc02m1b_aac,
		sizeof(addr_data_pair_custom1_gc02m1b_aac) /
		sizeof(kal_uint16));
}
...
static kal_uint8 gc02m1b_get_vendor_id(void)
{
	kal_uint8 vendor_info;
	int vendor_id_addr = 0x80;
	...
	write_cmos_sensor(0x17, vendor_id_addr);
	write_cmos_sensor(0xf3, 0x34);
	vendor_info = read_cmos_sensor(0x19);
	write_cmos_sensor(0xf7, 0x00);
	write_cmos_sensor(0xfe, 0x00);
	return vendor_info;
}

static char otp_data[10] = { 0 };

unsigned int
gc02m1b_aac_read_otp_info(struct i2c_client *client,
		 unsigned int addr, unsigned char *data, unsigned int size)
{
	int i = 0;
	unsigned int actual_size = 0;

	if (addr >= sizeof(otp_data))
		return 0;
	else if (addr + size >= sizeof(otp_data))
		actual_size = sizeof(otp_data) - addr;
	else
		actual_size = size;
	LOG_INF("gc02m1b_aac_otp addr %d,size %d", addr, size);
	for (i = 0; i < actual_size; i++) {
		data[i] = otp_data[addr + i];
		LOG_INF("gc02m1b_aac_otp_info %x,data %x", data[i],
				otp_data[addr + i]);
	}
	return  actual_size;
}

static void gc02m1b_aac_read_otp(void)
{
	int i = 0;
	int otp_addr[] = { 0x80, 0x18, 0x20, 0x28, 0x30, 0x38, 0x40, 0x48, 0x50, 0x58};
	kal_uint8 otp_info[10];
	...
	write_cmos_sensor(0xf3, 0x30);
	write_cmos_sensor(0xfe, 0x02);
	for (i = 0; i < sizeof(otp_data); i++) {
		write_cmos_sensor(0x17, otp_addr[i]);
		write_cmos_sensor(0xf3, 0x34);
	...
	if (otp_info[0] == 0x10) {
		for (i = 0; i < sizeof(otp_data); i++) {
			...
			LOG_INF("gc02m1b_aac_otp_info[%d] = %x\n", i, otp_data[i]);
		}
	} else {
		LOG_INF(" QT_vendor_ID Error\n");
	}
	write_cmos_sensor(0xf7, 0x00);
	write_cmos_sensor(0xfe, 0x00);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	vendor_id = gc02m1b_get_vendor_id();

	if (vendor_id != 0x10) {
		LOG_INF("get vendor id=0x%x is not aac module\n", vendor_id);
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO:
		...
		case MSDK_SCENARIO_ID_CUSTOM1:
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) =
		(imgsensor_info.custom1.framelength << 16) + imgsensor_info.custom1.linelength;
			break;
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		default:
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) =
...
UINT32 GC02M1B_AAC_MIPI_MONO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
