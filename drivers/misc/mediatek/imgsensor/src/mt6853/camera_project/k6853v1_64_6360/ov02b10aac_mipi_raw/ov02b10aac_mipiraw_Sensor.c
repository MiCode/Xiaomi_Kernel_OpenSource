...
#include "ov02b10aac_mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV02B10AAC_SENSOR_ID,
...
unsigned int
ov02b10aac_read_otp_info(struct i2c_client *client,
        ...
        do {
            if (vendor_id == 0x10) {
                *sensor_id = return_sensor_id() + 1;
                ...
                LOG_INF("hi259 aac vendor_id not match :%x", vendor_id);
            }
            retry--;
        } while (retry > 0);
        i++;
...
UINT32 OV02B10AAC_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
