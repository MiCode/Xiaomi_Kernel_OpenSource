...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_hi259[] = {
0x03, 0x00,
0x01, 0x01,
0x01, 0x03,
0x01, 0x01,
...
static void sensor_init(void)
{
	...
	hi259_table_write_cmos_sensor(
		addr_data_pair_init_hi259,
		sizeof(addr_data_pair_init_hi259) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x03, 0x00);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_preview_hi259[] = {
...
#if MULTI_WRITE
	hi259_table_write_cmos_sensor(
		addr_data_pair_preview_hi259,
		sizeof(addr_data_pair_preview_hi259) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x03, 0x00);
	write_cmos_sensor(0x07, 0x05);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_capture_30fps_hi259[] = {
...
kal_uint16 addr_data_pair_capture_15fps_hi259[] = {
0x03, 0x00,
0x01, 0x01,
};
#endif
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300) {
	hi259_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_hi259,
		sizeof(addr_data_pair_capture_30fps_hi259) /
		sizeof(kal_uint16));

	} else {
	hi259_table_write_cmos_sensor(
		addr_data_pair_capture_15fps_hi259,
		sizeof(addr_data_pair_capture_15fps_hi259) /
		sizeof(kal_uint16));
	}
#else
  if( currefps == 300) {
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_video_hi259[] = {
0x03, 0x00,
0x01, 0x01,
};
#endif
...
#if MULTI_WRITE
	hi259_table_write_cmos_sensor(
		addr_data_pair_video_hi259,
		sizeof(addr_data_pair_video_hi259) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x03, 0x00);
	write_cmos_sensor(0x01, 0x01);
...
static kal_uint16 read_cmos_sensor_hi259(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
...
static int hi259_vendor_id_read(int addr)
{
    ...
    pr_info("hynix_hi259_II  read vendor id , form addr 0x%x is: 0x%x\n",addr,flag);
	return flag;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if(flag != hi259_vendor_id){
        pr_info("hynix_hi259_II  match vendor id fail, reead vendor id is: 0x%x,expect vendor id is 0x%x \n", flag,hi259_vendor_id);
