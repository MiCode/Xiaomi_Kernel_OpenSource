...
#if defined(CONFIG_IMGSENSOR_MAIN) || defined(CONFIG_IMGSENSOR_SUB) ||         \
	defined(CONFIG_IMGSENSOR_MAIN2) || defined(CONFIG_IMGSENSOR_SUB2)
...
#define IMGSENSOR_STRINGIZE(stringizedName) IMGSENSOR_TOSTRING(stringizedName)
	...
	case IMGSENSOR_SENSOR_IDX_MAIN:
		psensor_name_config =
		    IMGSENSOR_STRINGIZE(CONFIG_IMGSENSOR_MAIN);
		break;
	...
	case IMGSENSOR_SENSOR_IDX_MAIN2:
		psensor_name_config =
		    IMGSENSOR_STRINGIZE(CONFIG_IMGSENSOR_MAIN2);
		break;
	case IMGSENSOR_SENSOR_IDX_SUB2:
		psensor_name_config =
		    IMGSENSOR_STRINGIZE(CONFIG_IMGSENSOR_SUB2);
		break;
	default:
		break;
	}

	if (strlen(psensor_name_config) > 2 &&
	    !strstr(psensor_name_config, curr_sensor_name))
		return IMGSENSOR_RETURN_ERROR;

#endif
	PK_DBG("sensor_idx %d, power %d curr_sensor_name %s\n", sensor_idx,
	       pwr_status, curr_sensor_name);
