 ...
 * Filename:
 * ---------
 *	 s5k4h7_ofilm_main_iii_mipi_raw.c
 *
 * Project:
 * --------
 *	 ALPS
...
//#include "s5k4h7otp.h"
#include "s5k4h7_ofilm_main_iii_mipi_raw.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K4H7_OFILM_MAIN_III_SENSOR_ID,  //0x487B
	...
	//zaki copied from capture for short video
	.custom1 = {
		.pclk = 271730000,
		.linelength = 3580,
		.framelength = 2530,
...
{{ 3264, 2448,   0,   0, 3264, 2448, 1632, 1224, 0000, 0000, 1632, 1224, 0, 0, 1632, 1224},	/* Preview */
 ...
 { 3264, 2448,   0,   0, 3264, 2448, 3264, 2448, 0000, 0000, 3264, 2448, 0, 0, 3264, 2448}, // custom1 (for shortvideo; copy normal video)
}; 
...
static void sensor_init(void)
{
	...
	//write_cmos_sensor_8(0x0000, 0x12); setfile_ver
	//write_cmos_sensor_8(0x0000, 0x48); sensor_id
	//write_cmos_sensor_8(0x0A02, 0x15); otp_userpage_start
	...
	write_cmos_sensor_8(0x3400, 0x01); // zhurunlong:not touch! 00sarah off //LSC enable 1123 df
	write_cmos_sensor_8(0x0B00, 0x00); // zhurunlong:not touch! 01sarah off//LSC enable 1123 df
	write_cmos_sensor_8(0x3931, 0x02); // zhurunlong:not touch! mipi test failed
	write_cmos_sensor_8(0x392F, 0x01); // zhurunlong:not touch! mipi test failed
	write_cmos_sensor_8(0x3930, 0x80); // zhurunlong:not touch! mipi test failed
	write_cmos_sensor_8(0x3932, 0x88); // zhurunlong:not touch! mipi test failed
}	/*	sensor_init  */
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300) {
		...
		write_cmos_sensor_8(0x0341, 0x52);
		write_cmos_sensor_8(0x0342, 0x0D);
		write_cmos_sensor_8(0x0343, 0xFC);
		write_cmos_sensor_8(0x0200, 0x0D);
		write_cmos_sensor_8(0x0201, 0x6C);
...
static kal_uint16 read_cmos_sensor_s5k4h7(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pu_send_cmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
...
static void s5k4h7_fusion_id_read(void)
{
	int i;
	for (i=0; i<8; i++) {
		fusion_id_main[i] = read_cmos_sensor_s5k4h7(0x10+i);
		//pr_info("%s %d fusion_id_front[%d]=0x%2x\n",__func__, __LINE__, i, fusion_id_main[i]);
	}
}
static void s5k4h7_sn_read(void)
{
	int i;
	for (i=0; i<14; i++) {
		sn_main[i] = read_cmos_sensor_s5k4h7(0x1F92+i);
...
UINT32 S5K4H7_OFILM_MAIN_III_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
