...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = SAMS_S5K4H7_III_SENSOR_ID,  //0x487B+1
...
static UINT8 get_sensor_module_id(void)
{
    ...
    for (i=0; i<15; i++) {
        ...
        //pr_err("%s %d s5k4h7_iii module_id[%d]=0x%2x\n",__func__, __LINE__, i, calmoduleversion[i]);
	}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if (module_id != 0x1) {
					pr_err("Read sunny module id failed, module id = 0x%x\n", module_id);
...
UINT32 SAMS_S5K4H7_III_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
