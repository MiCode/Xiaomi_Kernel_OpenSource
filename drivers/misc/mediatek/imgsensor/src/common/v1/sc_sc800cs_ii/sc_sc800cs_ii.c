...
#include "sc_sc800cs_ii.h"

#define PFX "sc_sc800cs_ii_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = SC_SC800CS_II_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] ={
	...
	{ 3264, 2448, 0, 0, 3264, 2448, 3264, 2448, 0, 0, 3264, 2448, 0, 0, 3264, 2448}, /*custom1 for shortvideo*/
};
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	write_cmos_sensor(0x3e09, SC800CS_AGC_Param[gain_index][1]);
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x3650,0x51);
	write_cmos_sensor(0x3651,0x9f);
	write_cmos_sensor(0x0100,0x01);
	write_cmos_sensor(0x302d,0x00);
	
	write_cmos_sensor(0x3650,0x31);//not touch zaki add for mipi failed-teotfail 20220620
	write_cmos_sensor(0x481b,0x28);//not touch zaki add for mipi failed-teotfail 20220620
}
...
static kal_uint16 read_cmos_sensor_sc800cs(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pu_send_cmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
...
static void sc800cs_fusion_id_read(void)
{
	int i;
	for (i=0; i<8; i++) {
		fusion_id_main[i] = read_cmos_sensor_sc800cs(0x10+i);
		//pr_err("%s %d wpc fusion_id_front[%d]=0x%2x\n",__func__, __LINE__, i, fusion_id_front[i]);
	}
}
static void sc800cs_sn_read(void)
{
	int i;
	for (i=0; i<14; i++) {
		sn_main[i] = read_cmos_sensor_sc800cs(0x1F92+i);
...
static void get_sensor_module_info(void)
{
    ...
    for (i = 0; i < 15; i++) {
        calmoduleversion[i] = read_cmos_sensor_sc800cs(0x01+i);
        //pr_err("%s %d sc800cs module_id[%d]=0x%2x\n",__func__, __LINE__, i, calmoduleversion[i]);
    }
