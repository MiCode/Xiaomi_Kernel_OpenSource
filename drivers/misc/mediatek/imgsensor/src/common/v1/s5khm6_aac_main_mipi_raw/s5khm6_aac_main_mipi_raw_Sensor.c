...
#define PFX "s5khm6_aac_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = S5KHM6_AAC_MAIN_SENSOR_ID, // 0x1AD6
    ...
    do {
      ...
      *sensor_id += 1;
      if (*sensor_id == imgsensor_info.sensor_id) {
        LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, S5khm6_Modelid=0x%x\n", imgsensor.i2c_write_id,
                *sensor_id, S5khm6_Modelid);
        return ERROR_NONE;
      }
      LOG_INF("Read sensor id fail, sensor id: 0x%x, i2c: 0x%x, S5khm6_Modelid=0x%x\n", *sensor_id,
              imgsensor.i2c_write_id, S5khm6_Modelid);
      retry--;
    } while (retry > 0);
    i++;
    retry = 2;
...
UINT32
S5KHM6_AAC_MAIN_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc) {
