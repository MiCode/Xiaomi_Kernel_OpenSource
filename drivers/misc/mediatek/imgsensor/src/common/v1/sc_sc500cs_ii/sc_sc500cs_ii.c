...
#include "sc_sc500cs_ii.h"

#define PFX "sc_sc500cs_ii_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = SC_SC500CS_II_SENSOR_ID,
...
static void sensor_init(void)
{
	...
	write_cmos_sensor8(0x36ea,0x39);
	write_cmos_sensor8(0x36eb,0x0c);
	write_cmos_sensor8(0x36ec,0x0b);
	write_cmos_sensor8(0x36ed,0x36);
	...
	write_cmos_sensor8(0x301f,0x31);
	...
	write_cmos_sensor8(0x4837,0x20);
...
static void sc500cs_fusion_id_read(void)
{
	int i;
	for (i=0; i<8; i++) {
		fusion_id_front[i] = read_cmos_sensor_sc500cs(0x10+i);
		//pr_err("%s %d wpc fusion_id_front[%d]=0x%2x\n",__func__, __LINE__, i, fusion_id_main[i]);
	}
}
static void sc500cs_sn_id_read(void)
{
	int i;
	for (i=0; i<14; i++) {
		sn_front[i] = read_cmos_sensor_sc500cs(0x1F92+i);
...
UINT32 SC_SC500CS_II_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
