...
* bf30a2 marco
******************************************************************/
#define BF30A2_DRIVER_VERSION	"V2.0"
...
static struct bf30a2 *bf30a2_malloc_init(struct i2c_client *client)
{
	...
	pr_info("%s enter , client_addr = 0x%02x\n", __func__,
		bf30a2->i2c_client->addr);

	return bf30a2;
}
#if 1
void bf30a2_Init(struct bf30a2 *bf30a2)
{
    /*SYS*/
...
#endif
int bf30a2_GetSensorID(struct bf30a2 *bf30a2)
{
	int retry = 5;
	int len;
	...
	do {
		...
		qvga_dev_err(bf30a2->dev, "drv-%s: Read MSB Sensor ID sucess = 0x%02x\n", __func__, reg_data);
		if (reg_data == BF30A2_SENSOR_ID) {
			qvga_dev_err(bf30a2->dev, "drv-%s: Read Sensor ID sucess = 0x%02x\n", __func__, reg_data);
			driver_flag = 1;
			return 0;
		} else {
			qvga_dev_err(bf30a2->dev, "rv-%s: Read Sensor ID Fail = 0x%02x\n", __func__, reg_data);
			driver_flag = 0;
		}
		mdelay(10);
		retry--;
...
static void bf30a2_vcam_control(struct bf30a2 *bf30a2, bool flag)
{
	...
	if (IS_ERR(vcama)) {
		qvga_dev_err(bf30a2->dev, "%s get regulator vcama failed\n", __func__);
		regulator_put(vcama);
		return;
	}
	...
	if (IS_ERR(vcamio)) {
		qvga_dev_err(bf30a2->dev, "%s get regulator vcamio failed\n", __func__);
	...
	if (flag) {
        ...
        vcamd = regulator_get(bf30a2->dev,"vcamd");
        if (IS_ERR(vcamd)) {
                qvga_dev_err(bf30a2->dev, "%s get regulator vcamd failed\n", __func__);
                regulator_put(vcamd);
                return;
        }
...
static void bf30a2_hw_on_reset1(struct bf30a2 *bf30a2)
{
	qvga_dev_info(bf30a2->dev, "%s enter\n", __func__);

	if (gpio_is_valid(bf30a2->reset_gpio1)) {
		gpio_set_value_cansleep(bf30a2->reset_gpio1, 1);
	}
}
...
static void bf30a2_hw_off_reset1(struct bf30a2 *bf30a2)
{
	qvga_dev_info(bf30a2->dev, "%s enter\n", __func__);

	if (gpio_is_valid(bf30a2->reset_gpio1)) {
		gpio_set_value_cansleep(bf30a2->reset_gpio1, 0);
	}
}

static void bf30a2_hw_on(struct bf30a2 *bf30a2)
{
	bf30a2_hw_on_reset1(bf30a2);
			       ...
			       size_t len)
{
	...
	unsigned char reg_data = 0x00;
	int length;
	//struct bf30a2 *bf30a2 = dev_get_drvdata(dev);
...
{
	qvga_dev_info(bf30a2->dev, "%s enter, dev_i2c%d@0x%02X\n", __func__,
	...
	if (bf30a2->reset_gpio < 0) {
		...
		qvga_dev_info(bf30a2->dev, "%s: reset gpio provided ok\n",
			 __func__);
	}

	bf30a2->reset_gpio1 = of_get_named_gpio(np, "reset-gpio1", 0);
	if (bf30a2->reset_gpio1 < 0) {
		qvga_dev_err(bf30a2->dev,
			   "%s: no reset gpio1 provided, hardware reset unavailable\n",
			__func__);
		bf30a2->reset_gpio1 = -1;
	} else {
		qvga_dev_info(bf30a2->dev, "%s: reset gpio1 provided ok\n",
			 __func__);
	}
}
			     ...
			     const struct i2c_device_id *id)
{
	...
	struct pinctrl_state *bf30a2_mclk_on;
	struct pinctrl_state *bf30a2_mclk_off;
	...
	if (gpio_is_valid(bf30a2->reset_gpio1)) {
		ret = devm_gpio_request_one(&client->dev,
					    bf30a2->reset_gpio1,
					    GPIOF_OUT_INIT_LOW, "bf30a2_rst1");//byd
		if (ret) {
			qvga_dev_err(&client->dev,
	...
	bf30a2_pinctrl = devm_pinctrl_get(&client->dev);
	if (IS_ERR_OR_NULL(bf30a2_pinctrl)) {
		qvga_dev_err(&client->dev, "%s: bf30a2_pinctrl not defined\n", __func__);
	} else {
		set_state = pinctrl_lookup_state(bf30a2_pinctrl, BF30A2_MCLK_ON);
		if (IS_ERR_OR_NULL(set_state)) {
			qvga_dev_err(&client->dev, "%s: bf30a2_pinctrl lookup failed for mclk on\n", __func__);
		} else {
			bf30a2_mclk_on = set_state;
		}
		set_state = pinctrl_lookup_state(bf30a2_pinctrl, BF30A2_MCLK_OFF);
		if (IS_ERR_OR_NULL(set_state)) {
			qvga_dev_err(&client->dev, "%s: bf30a2_pinctrl lookup failed for mclk off\n", __func__);
		} else {
			bf30a2_mclk_off = set_state;
		}
		ret = pinctrl_select_state(bf30a2_pinctrl, bf30a2_mclk_off);
		if (ret < 0) {
			qvga_dev_err(&client->dev, "%s: bf30a2_pinctrl select failed for mclk off\n", __func__);
		}
	}

	//power on camera
	bf30a2_vcam_control(bf30a2, true);
	mdelay(1);
	ret = pinctrl_select_state(bf30a2_pinctrl, bf30a2_mclk_on);
	if (ret < 0) {
		qvga_dev_err(&client->dev, "%s: bf30a2_pinctrl select failed for mclk on\n", __func__);
	}
	mdelay(5);
	bf30a2_hw_off_reset1(bf30a2);
	bf30a2_hw_on_reset1(bf30a2);
	bf30a2_hw_on_reset(bf30a2);
...
//	bf30a2->hwen_flag = 1;
	...
	if (ret < 0) {
		qvga_dev_err(&client->dev,
			   "%s: bf30a2read_sensorid failed ret=%d\n", __func__,
			   ret);
		goto exit_i2c_check_id_failed;
	}

        //power off camera
	bf30a2_vcam_control(bf30a2, false);
	bf30a2_hw_off_reset1(bf30a2);
//	bf30a2_Init(bf30a2);
...
static int bf30a2_i2c_remove(struct i2c_client *client)
{
	...
	if (gpio_is_valid(bf30a2->reset_gpio1))
		devm_gpio_free(&client->dev, bf30a2->reset_gpio1);
