...
static void bf30a2_avdd_control(struct bf30a2 *bf30a2, bool flag)
{
	...
	if (IS_ERR(vcama)) {
		qvga_dev_err(bf30a2->dev, "%s get regulator AVDD failed\n", __func__);
		regulator_put(vcama);
		return;
	}
	if (flag) {
...
#endif
static void bf30a2_hw_reset_level(struct bf30a2 *bf30a2,bool level)
{
	qvga_dev_info(bf30a2->dev, "%s enter\n", __func__);
	if (gpio_is_valid(bf30a2->reset_gpio)) {
		if(level)
...
#if FRONT_RST
static void bf30a2_hw_front_reset_level(struct bf30a2 *bf30a2,bool level)
{
	qvga_dev_info(bf30a2->dev, "%s enter\n", __func__);
...
static void bf30a2_hw_on(struct bf30a2 *bf30a2)
{
	...
	struct pinctrl_state *bf30a2_mclk_on = NULL;
	struct pinctrl_state *set_state = NULL;
	int ret = -1;
	bf30a2_pinctrl = devm_pinctrl_get(bf30a2->dev);
	if (IS_ERR_OR_NULL(bf30a2_pinctrl)) {
		qvga_dev_err(bf30a2->dev, "%s: bf30a2_pinctrl not defined\n", __func__);
	} else {
		set_state = pinctrl_lookup_state(bf30a2_pinctrl, BF30A2_MCLK_ON);
		if (IS_ERR_OR_NULL(set_state)) {
			qvga_dev_err(bf30a2->dev, "%s: bf30a2_pinctrl lookup failed for mclk on\n", __func__);
		} else {
...
#if FRONT_RST
	if (gpio_is_valid(bf30a2->reset_gpio1))
		bf30a2_hw_front_reset_level(bf30a2,false);
#endif
	bf30a2_hw_reset_level(bf30a2,true);
	ret = pinctrl_select_state(bf30a2_pinctrl, bf30a2_mclk_on);
	if (ret < 0) {
		qvga_dev_err(bf30a2->dev, "%s: bf30a2_pinctrl select failed for mclk off\n", __func__);
	}
	bf30a2_avdd_control(bf30a2, true);
	mdelay(1);
	bf30a2_hw_reset_level(bf30a2,false);
	mdelay(1);
...
{
	struct pinctrl *bf30a2_pinctrl;
	struct pinctrl_state *bf30a2_mclk_off = NULL;
	struct pinctrl_state *set_state = NULL;
	int ret = -1;
	bf30a2_pinctrl = devm_pinctrl_get(bf30a2->dev);
	if (IS_ERR_OR_NULL(bf30a2_pinctrl)) {
		...
		if (IS_ERR_OR_NULL(set_state)) {
			qvga_dev_err(bf30a2->dev, "%s: bf30a2_pinctrl lookup failed for mclk off\n", __func__);
		} else {
...
#if FRONT_RST
	if (gpio_is_valid(bf30a2->reset_gpio1))
		bf30a2_hw_front_reset_level(bf30a2,true);
#endif
					...
					struct device_node *np)
{
	...
	if (bf30a2->reset_gpio1 < 0) {
		...
		qvga_dev_info(bf30a2->dev, "%s: reset gpio 1 provided ok\n",
			 __func__);
	}
#endif 
}
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(bf30a2->reset_gpio)) {
		ret = devm_gpio_request_one(&client->dev,
					    bf30a2->reset_gpio,
					    GPIOF_OUT_INIT_LOW, "bf30a2_rst");//ofilm
		if (ret) {
