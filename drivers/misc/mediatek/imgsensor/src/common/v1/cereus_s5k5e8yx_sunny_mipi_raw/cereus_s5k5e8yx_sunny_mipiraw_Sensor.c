...
#include "cereus_s5k5e8yx_sunny_mipiraw_Sensor.h"
...
#define PFX "cereus_s5k5e8yx_sunny_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = { 
	.sensor_id = CEREUS_S5K5E8YX_SUNNY_SENSOR_ID,
...
static struct stCAM_CAL_DATAINFO_STRUCT sensor_eeprom_data ={
	.sensorID= CEREUS_S5K5E8YX_SUNNY_SENSOR_ID,
	.deviceID = 0x02,
	.dataLength = 386,
	.sensorVendorid = 0x010AFFFF,
	.vendorByte = {1,8,9,10},
	.dataBuffer = NULL,
};
static struct stCAM_CAL_CHECKSUM_STRUCT s5k5e8Checksum[3] =
{
  	{MODULE_ITEM,0x0000,0x0000,0x0017,0x0018,0xff},
	{LSC_ITEM,0x0019,0x0019,0x0180,0x0181,0xff},
	{MAX_ITEM,0xFFFF,0xFFFF,0xFFFF,0xFFFF,0xff},  // this line must haved 
};
...
static kal_uint16 cereus_s5k5e8yx_sunny_table_write_cmos_sensor(
...
static void read_efuseID(struct stCAM_CAL_DATAINFO_STRUCT*pData){
    ...
    for (i = 0; i < 8; i++)
    {
        buf[i] = read_cmos_sensor((0x0a04+i));
    }
    write_cmos_sensor_8(0x0a00, 0x04);
...
static int read_otp_data(struct stCAM_CAL_DATAINFO_STRUCT*pData){
	int iCount = 0;
	int ret = 0;
	u8 streamFlag = 0;
	u8 pages = 0;
	...
	streamFlag = read_cmos_sensor(0x0100);
	if(streamFlag == 0x01){
		write_cmos_sensor_8(0x0100,0x00);
	}

	//read page 14,find AWB&module infomation
	write_cmos_sensor_8(0x0a00,0x04);
	write_cmos_sensor_8(0x0a02,0x0e);
	write_cmos_sensor_8(0x0a00,0x01);
	if(read_cmos_sensor(0x0a38) == 0x55){ // grop 3
		//cmp vendor id
		tmpBuf[0] = read_cmos_sensor(0x0a3a);
		Vendorid = tmpBuf[0];
		if (((pData->sensorVendorid)>>24) != Vendorid) {
			LOG_ERR("group 3 Vendorid check failed  0x%x != 0x%x\n", Vendorid, pData->sensorVendorid >> 24);
			return -1;
		}
		for(iCount = 0;((iCount < 11)&&(ret < pData->dataLength));iCount++,ret++){
			pData->dataBuffer[ret] = (kal_uint8)read_cmos_sensor(0x0a39+iCount);
			//LOG_INF("awb&module OTP[%d]= 0x%2x",ret,pData->dataBuffer[ret]);
		}
		//read page 15,read 15 bytes
		write_cmos_sensor_8(0x0a00,0x04);
		write_cmos_sensor_8(0x0a02,0x0f);
		write_cmos_sensor_8(0x0a00,0x01);
		for(iCount = 0;((iCount < 14)&&(ret < pData->dataLength));iCount++,ret++){
			pData->dataBuffer[ret] = (kal_uint8)read_cmos_sensor(0x0a04+iCount);
			//LOG_INF("awb&module OTP[%d]= 0x%2x",ret,pData->dataBuffer[ret]);
		}
	}else if(read_cmos_sensor(0x0a1e) == 0x55){ // grop 2
		//cmp vendor id
		tmpBuf[0] = read_cmos_sensor(0x0a20);
		Vendorid = tmpBuf[0];
		if (((pData->sensorVendorid)>>24) != Vendorid) {
			LOG_ERR("group 2 Vendorid check failed  0x%x != 0x%x\n", Vendorid, pData->sensorVendorid >> 24);
			return -1;
		}
		for(iCount = 0;((iCount < 25)&&(ret < pData->dataLength));iCount++,ret++){
			pData->dataBuffer[ret] = (kal_uint8)read_cmos_sensor(0x0a1f+iCount);
			//LOG_INF("awb&module OTP[%d]= 0x%2x",ret,pData->dataBuffer[ret]);
		}
	}else if(read_cmos_sensor(0x0a04) == 0x55){ // grop 1
		//cmp vendor id
		tmpBuf[0] = read_cmos_sensor(0x0a06);
		Vendorid = tmpBuf[0];
		if (((pData->sensorVendorid)>>24) != Vendorid) {
			LOG_ERR("group 1 Vendorid check failed  0x%x != 0x%x\n", Vendorid, pData->sensorVendorid >> 24);
			return -1;
		}
		for(iCount = 0;((iCount < 25)&&(ret < pData->dataLength));iCount++,ret++){
			pData->dataBuffer[ret] = (kal_uint8)read_cmos_sensor(0x0a05+iCount);
			//LOG_INF("awb&module OTP[%d]= 0x%2x",ret,pData->dataBuffer[ret]);
		}
	}else{
		LOG_ERR("we not find otp data\n");
		kfree(pData->dataBuffer);
		pData->dataBuffer = NULL;
		return -1;
	}

	//read page 4,read 16 bytes
	write_cmos_sensor_8(0x0a00,0x04);
	write_cmos_sensor_8(0x0a02,0x04);
	write_cmos_sensor_8(0x0a00,0x01);
	for(iCount = 0;((iCount < 16)&&(ret < pData->dataLength));iCount++,ret++){
		pData->dataBuffer[ret] = (kal_uint8)read_cmos_sensor(0x0a34+iCount);
		//LOG_INF("lsc OTP[%d]= 0x%2x",ret,pData->dataBuffer[ret]);
	}

	//read page 5~9,read 64 bytes erery page
	for(pages = 5 ;pages < 10 ;pages++){
		write_cmos_sensor_8(0x0a00,0x04);
		write_cmos_sensor_8(0x0a02,pages);
		write_cmos_sensor_8(0x0a00,0x01);
		for(iCount = 0;((iCount < 64)&&(ret < pData->dataLength));iCount++,ret++){
			pData->dataBuffer[ret] = (kal_uint8)read_cmos_sensor(0x0a04+iCount);
			//LOG_INF("lsc OTP[%d]= 0x%2x",ret,pData->dataBuffer[ret]);
		}
	}

	//read page 10,read 24 bytes erery page
	write_cmos_sensor_8(0x0a00,0x04);
	write_cmos_sensor_8(0x0a02,0x0a);
	write_cmos_sensor_8(0x0a00,0x01);
	for(iCount = 0;((iCount < 24)&&(ret < pData->dataLength));iCount++,ret++){
		pData->dataBuffer[ret] = (kal_uint8)read_cmos_sensor(0x0a04+iCount);
		//LOG_INF("lsc OTP[%d]= 0x%2x",ret,pData->dataBuffer[ret]);
	}

	//read page 15,read 1 bytes erery page
	write_cmos_sensor_8(0x0a00,0x04);
	write_cmos_sensor_8(0x0a02,0x0f);
	write_cmos_sensor_8(0x0a00,0x01);
	pData->dataBuffer[ret] = (kal_uint8)read_cmos_sensor(0xa12);;//lsc checksum
	//LOG_INF("lsc OTP[%d]= 0x%2x",ret,pData->dataBuffer[ret]);
	ret++;

	write_cmos_sensor_8(0x0100,streamFlag);
	write_cmos_sensor_8(0x0a00,0x04);
    write_cmos_sensor_8(0x0a00,0x00); // NVM disable 
	if(imgSensorCheckEepromData(pData,s5k5e8Checksum) != 0){
 ...
 *************************************************************************/
#if MULTI_WRITE
static kal_uint16 addr_data_pair_init_cereus_s5k5e8yx_sunny[] = {
	0x0100, 0x00,
	0x3906, 0x7E,
	0x3C01, 0x0F,
	0x3C14, 0x00,
...
static void sensor_init(void)
{
	...
	cereus_s5k5e8yx_sunny_table_write_cmos_sensor(
		addr_data_pair_init_cereus_s5k5e8yx_sunny,
		sizeof(addr_data_pair_init_cereus_s5k5e8yx_sunny) /
		sizeof(kal_uint16));
#else
...
#if MULTI_WRITE
static kal_uint16 addr_data_pair_preview_cereus_s5k5e8yx_sunny[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x18,
...
#if MULTI_WRITE
	cereus_s5k5e8yx_sunny_table_write_cmos_sensor(
		addr_data_pair_preview_cereus_s5k5e8yx_sunny,
		sizeof(addr_data_pair_preview_cereus_s5k5e8yx_sunny) /
		sizeof(kal_uint16));
#else
...
#if MULTI_WRITE
static kal_uint16 addr_data_pair_capture_15fps_cereus_s5k5e8yx_sunny[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x18,
...
static kal_uint16 addr_data_pair_capture_24fps_cereus_s5k5e8yx_sunny[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x18,
...
static kal_uint16 addr_data_pair_capture_30fps_cereus_s5k5e8yx_sunny[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x18,
...
{
	LOG_DBG("capture currefps:%d\n", currefps);
#if MULTI_WRITE
	if (currefps == 150) {
	cereus_s5k5e8yx_sunny_table_write_cmos_sensor(
		addr_data_pair_capture_15fps_cereus_s5k5e8yx_sunny,
		sizeof(addr_data_pair_capture_15fps_cereus_s5k5e8yx_sunny) /
		sizeof(kal_uint16));
	} else if (currefps == 240) {
	cereus_s5k5e8yx_sunny_table_write_cmos_sensor(
		addr_data_pair_capture_24fps_cereus_s5k5e8yx_sunny,
		sizeof(addr_data_pair_capture_24fps_cereus_s5k5e8yx_sunny) /
		sizeof(kal_uint16));
	} else {
	cereus_s5k5e8yx_sunny_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_cereus_s5k5e8yx_sunny,
		sizeof(addr_data_pair_capture_30fps_cereus_s5k5e8yx_sunny) /
		sizeof(kal_uint16));
	}
#else
	if (currefps == 150){
...
{
	LOG_DBG("normal_video currefps:%d\n", currefps);
	// +++++++++++++++++++++++++++//
#if MULTI_WRITE
	cereus_s5k5e8yx_sunny_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_cereus_s5k5e8yx_sunny,
...
#if MULTI_WRITE
static kal_uint16 addr_data_pair_hs_video_cereus_s5k5e8yx_sunny[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x18,
...
{
	LOG_DBG("E! hs_video_setting 120fps\n");
	//VGA 120fps
//$MV1[MCLK:24,Width:640,Height:480,Format:MIPI_RAW10,mipi_lane:2,
//mipi_datarate:836,pvi_pclk_inverwrite_cmos_sensor(0xe:0]
#if MULTI_WRITE
	cereus_s5k5e8yx_sunny_table_write_cmos_sensor(
		addr_data_pair_hs_video_cereus_s5k5e8yx_sunny,
		sizeof(addr_data_pair_hs_video_cereus_s5k5e8yx_sunny) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x0136,0x18);
	write_cmos_sensor(0x0137,0x00);
	...
	write_cmos_sensor(0x3308,0x09);
	write_cmos_sensor(0x3309,0xFF);
	write_cmos_sensor(0x330A,0x01);
	write_cmos_sensor(0x330B,0x01);
	write_cmos_sensor(0x330E,0x00);
	write_cmos_sensor(0x330F,0x00);
	write_cmos_sensor(0x3310,0x07);
	write_cmos_sensor(0x3311,0x7F);
...
#if MULTI_WRITE
static kal_uint16 addr_data_pair_slim_video_cereus_s5k5e8yx_sunny[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x18,
...
{
	LOG_DBG("slim_video_setting 30fps\n");
//$MV1[MCLK:24,Width:1280,Height:720,MIPI_RAW10,mipi_lane:2,
//mipi_datarate:836,pvi_pclk_inverwrite_cmos_sensor(0xe:0]);
#if MULTI_WRITE
cereus_s5k5e8yx_sunny_table_write_cmos_sensor(
	addr_data_pair_slim_video_cereus_s5k5e8yx_sunny,
	sizeof(addr_data_pair_slim_video_cereus_s5k5e8yx_sunny) /
	sizeof(kal_uint16));
#else
	write_cmos_sensor(0x0136,0x18);
	write_cmos_sensor(0x0137,0x00);
...
#define CEREUS_S5K5E8YX_SUNNY_I2CBUS    (4)
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	LOG_DBG("CEREUS_S5K5E8YX_SUNNY_I2CBUS = %d, I2C_bus = %d\n", CEREUS_S5K5E8YX_SUNNY_I2CBUS, I2C_bus);
	if (I2C_bus != CEREUS_S5K5E8YX_SUNNY_I2CBUS) {
		*sensor_id = 0xFFFFFFFF;
		LOG_ERR("CEREUS_S5K5E8YX_SUNNY_I2CBUS: %d, I2C_bus = %d, Check Error!\n", CEREUS_S5K5E8YX_SUNNY_I2CBUS, I2C_bus);
	...
	while(imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		size = read_otp_data(&sensor_eeprom_data);
		...
		do {
			*sensor_id = return_sensor_id()+2;
			...
			LOG_ERR("Read sensor id fail,i2c_write_id 0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LOG_DBG("R sensorid fail,write_id 0x%x id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while(retry > 0);
		i++;
	...
	if (imgsensor_info.sensor_id != sensor_id) {
		LOG_ERR("Open sensor id failed: 0x%x\n", sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
			...
			MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps ==
			...
			if (imgsensor.current_fps !=
				...
				LOG_ERR("%d fps not support,cap's: %d fps\n",
					framerate,
	...
	default:  //coding with  preview scenario by default
		...
		if (imgsensor.frame_length > imgsensor.shutter)
			set_dummy();
		LOG_ERR("error scenario_id = %d, preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
		LOG_DBG("imgsensor.pclk = %d,imgsensor.current_fps = %d\n",
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		LOG_DBG("SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
