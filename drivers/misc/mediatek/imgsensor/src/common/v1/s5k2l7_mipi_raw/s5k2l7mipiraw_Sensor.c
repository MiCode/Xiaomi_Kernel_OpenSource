...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{4032, 3024, 0, 378, 4032, 2328, 1344, 756, 0, 0, 1344, 756, 0, 0, 1344, 756},	/* hs_video */
	{4032, 3024, 0, 378, 4032, 2328, 1344, 756, 0, 0, 1336, 756, 0, 0, 1344, 756},	/* slim_video */
};
...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	...
	iReadRegI2CTiming(pu_send_cmd, 2, (u8 *) &get_byte, 1, imgsensor.i2c_write_id,
...
static BOOL is_module_v2(void)
{
	...
	if (module_id != 0) {
		if (module_id == MODULE_V2_ID) {
			LOG_INF("It is module v2");
			return TRUE;
		}

		LOG_INF("It is module v1");
		return FALSE;
	}
	...
	while (_imgsensor_info_m1_v2.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = _imgsensor_info_m1_v2.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
		do {
			module_id = read_cmos_sensor_twobyte(0x0002);
			LOG_INF("i2c write id: 0x%x, module id: 0x%x\n", imgsensor.i2c_write_id,
				module_id);
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_TYPE:
		*feature_para = pdaf_sensor_mode;
		if (pdaf_sensor_mode == 1)
			...
			sprintf(feature_para, "configure S5K2L7 as unknown mode");
	...
	case SENSOR_FEATURE_SET_PDAF_TYPE:
		if (strstr(&(*feature_para), "mode1")) {
			...
			LOG_INF("configure PDAF as unknown mode\n");
			proc_pdaf_sensor_mode = 1;
		}
		break;
