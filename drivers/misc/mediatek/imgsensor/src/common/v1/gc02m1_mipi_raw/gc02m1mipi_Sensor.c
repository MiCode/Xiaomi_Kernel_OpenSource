...
static struct gc02m1_ae_t update_ae = {
  .flag = 0,
  .ratio = 1024,
};
...
static kal_uint16 gc02m1_otp_init_addr_data[] = {
	0xfe, 0x00,
	0xfc, 0x01,
	0xf4, 0x41,
	0xf5, 0xc0,
...
static kal_uint16 gc02m1_otp_close_addr_data[] = {
	0xf7, 0x01,
	0xfe, 0x00,
};

static kal_uint8 gc02m1_otp_read_byte(kal_uint8 addr)
{
    write_cmos_sensor(0xfe, 0x02);
    write_cmos_sensor(0x17, addr & 0xff);
    write_cmos_sensor(0xf3, 0x34);
    return read_cmos_sensor(0x19);
}

static void gc02m1_otp_read_ae_info(void)
{ 
    kal_uint8 idx = 0,i = 0;
    kal_uint8 flag_update_ae = 0;
    kal_uint8 ae_group_offset = 0;
    kal_uint16 ae_checksum = 0;

    flag_update_ae = gc02m1_otp_read_byte(0x78);
    LOG_INF("ae_update: flag_update_ae = 0x%x\n", flag_update_ae);

    for (idx = 0; idx < 2; idx++) {
      switch (GC02M1_OTP_GET_2BIT_FLAG(flag_update_ae, 2 * idx)) {
        case GC02M1_OTP_FLAG_EMPTY: {
          LOG_INF("ae_update: AE calibration Group %d is empty!\n", idx + 1);
          update_ae.flag = update_ae.flag | GC02M1_OTP_FLAG_EMPTY;
          break;
        }
        case GC02M1_OTP_FLAG_VALID: {
          LOG_INF("ae_update: AE calibration Group %d is valid!\n", idx + 1);
          ae_group_offset = 7 * 8 * idx;

          for(i = 0; i < 6; i++){
            ae_checksum += gc02m1_otp_read_byte(0x80 + i * 8 + ae_group_offset );
          }
          LOG_INF("ae_update: ae_checksum = 0x%.2x\n", ae_checksum % 255 + 1);
      
          if((ae_checksum % 255 + 1) == gc02m1_otp_read_byte(0xb0 + ae_group_offset)){
            update_ae.ratio = gc02m1_otp_read_byte(0xa0 + ae_group_offset) << 8 | gc02m1_otp_read_byte(0xa8 + ae_group_offset);
            update_ae.flag = update_ae.flag | GC02M1_OTP_FLAG_VALID;
            LOG_INF("ae_update: ae_checksum ok, update_ae.ratio = 0x%.4x\n", update_ae.ratio);
          }else{
            update_ae.flag = update_ae.flag | GC02M1_OTP_FLAG_INVALID;
            LOG_INF("ae_update: ae_checksum fail, actual_checksum = 0x%.2x\n", gc02m1_otp_read_byte(0xb0 + ae_group_offset));
          }
          break;
        }
        case GC02M1_OTP_FLAG_INVALID:
        case GC02M1_OTP_FLAG_INVALID2: {
          LOG_INF("ae_update: AE calibration Group %d is invalid!\n", idx + 1);
          update_ae.flag = update_ae.flag | GC02M1_OTP_FLAG_INVALID;
          break;
        }
        default:
...
static kal_uint8 gc02m1_otp_function(void) {
	kal_uint8 flag_lens = 0;

	gc02m1_table_write_cmos_sensor(gc02m1_otp_init_addr_data,
		sizeof(gc02m1_otp_init_addr_data)/sizeof(kal_uint16));

	flag_lens = gc02m1_otp_read_byte(0x10);
	LOG_INF("ae_update: flag_lens = 0x%x\n", flag_lens);

	if(((flag_lens >> 6) & 0x01) == 0){
		write_cmos_sensor(0xfe, 0x00);
		write_cmos_sensor(0xb0, 0x86);
		LOG_INF("ae_update: normal lens global gain update\n");
	}else{
		gc02m1_otp_read_ae_info();
	}

	gc02m1_table_write_cmos_sensor(gc02m1_otp_close_addr_data,
		sizeof(gc02m1_otp_close_addr_data)/sizeof(kal_uint16));
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if((update_ae.flag & 0x01) == GC02M1_OTP_FLAG_VALID){
		reg_gain = reg_gain * (update_ae.ratio * GC02M1_MULTI_GAIN) / 102400;
		LOG_INF("ae_update: reg_gain = 0x%x\n", reg_gain);
	}
