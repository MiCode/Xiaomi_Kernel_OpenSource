...
#if FRONT_RST
static void sc080cs_hw_front_reset_level(struct sc080cs *sc080cs,bool level)
{
	qvga_dev_info(sc080cs->dev, "%s enter\n", __func__);
	if (gpio_is_valid(sc080cs->reset_gpio2)) {
		if(level)
			gpio_set_value_cansleep(sc080cs->reset_gpio2, 1);
		else
			gpio_set_value_cansleep(sc080cs->reset_gpio2, 0);
	}
}
#endif
...
#if FRONT_RST
	if (gpio_is_valid(sc080cs->reset_gpio2))
		sc080cs_hw_front_reset_level(sc080cs,false);
#endif
	udelay(10);
...
#if FRONT_RST
	if (gpio_is_valid(sc080cs->reset_gpio2))
		sc080cs_hw_front_reset_level(sc080cs,true);
#endif
}
...
#if FRONT_RST
	sc080cs->reset_gpio2 = of_get_named_gpio(np, "reset-gpio2", 0);
	if (sc080cs->reset_gpio2 < 0) {
		qvga_dev_err(sc080cs->dev,
			   "%s: no reset gpio2 provided, hardware reset unavailable\n",
			__func__);
		sc080cs->reset_gpio2 = -1;
	} else {
		qvga_dev_info(sc080cs->dev, "%s: reset gpio 2 provided ok\n",
			 __func__);
	}
#endif
}
