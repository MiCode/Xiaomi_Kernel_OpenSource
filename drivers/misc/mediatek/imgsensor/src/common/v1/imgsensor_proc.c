				...
				loff_t *data)
{
	...
	if (psensor_func)
		psensor_func->SensorFeatureControl(SENSOR_FEATURE_SET_PDAF_TYPE,
				regBuf, &u4CopyBufSize);
...
static ssize_t CAMERA_HW_Reg_Debug4(struct file *file,
	const char *buffer, size_t count,
				loff_t *data)
{
	char regBuf[64] = { '\0' };
...
enum IMGSENSOR_RETURN imgsensor_proc_init(void)
{
	...
	proc_create("driver/camsensor4", 0644, NULL, &fcamera_proc_fops4);
