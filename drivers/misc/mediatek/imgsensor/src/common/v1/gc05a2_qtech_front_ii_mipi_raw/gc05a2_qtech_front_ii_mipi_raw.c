...
#include "gc05a2_qtech_front_ii_mipi_raw.h"
...
static void gc05a2_fusion_id_read(void)
{
	int i;
	for (i=0; i<8; i++) {
		fusion_id_front[i] = read_cmos_sensor_gc05a2(0x0010+i);
		pr_info("zengx %s addr = 0x%4x fusion_id_front[%d]=0x%2x\n",__func__, 0x0010 + i, i, fusion_id_front[i]);
	}
}
static void gc05a2_sn_read(void)
{
	int i;
	for (i=0; i<14; i++) {
		sn_front[i] = read_cmos_sensor_gc05a2(0x1f92+i);
