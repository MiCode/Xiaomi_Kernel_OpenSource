...
static struct stCAM_CAL_DATAINFO_STRUCT sensor_eeprom_data ={
	...
	.deviceID = 1,
	.dataLength = 0x1BF6,
	.sensorVendorid = 0x071D0C0A,
	.vendorByte = {1,8,9,10},
	.dataBuffer = NULL,
};

static struct stCAM_CAL_CHECKSUM_STRUCT ov12a10Checksum[9] =
{
...
static kal_uint16 cereus_ov12a10_table_write_cmos_sensor(
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
	...
	LOG_DBG("set max framerate = %d, frame_length =%d\n", framerate, frame_length);
...
{
	LOG_DBG("streaming_control enable =%d\n", enable);
	if (enable)
...
{
	/* LOG_DBG("night_mode do nothing"); */
}

#if MULTI_WRITE
kal_uint16 addr_data_pair_init_cereus_ov12a10[] = {
	0x0103, 0x01,
	0x0302, 0x49,
	0x0303, 0x00,
	0x0304, 0x00,
...
{
#if MULTI_WRITE
	LOG_DBG("sensor_init init_setting MULTI_WRITE\n");
	cereus_ov12a10_table_write_cmos_sensor(
		addr_data_pair_init_cereus_ov12a10,
		sizeof(addr_data_pair_init_cereus_ov12a10) /
		sizeof(kal_uint16));
#else
	LOG_DBG("sensor_init init_setting\n");
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_preview_cereus_ov12a10[] = {
	0x3642, 0x10,
	0x3666, 0x08,
	0x366b, 0x20,
	0x366c, 0xa4,
...
{
	LOG_DBG("preview_setting RES_2048*1536_30fps\n");
#if MULTI_WRITE
	cereus_ov12a10_table_write_cmos_sensor(
		addr_data_pair_preview_cereus_ov12a10,
		sizeof(addr_data_pair_preview_cereus_ov12a10) /
		sizeof(kal_uint16));
#else
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_capture_15fps_cereus_ov12a10[] = {
	0x3642, 0x12,
	0x3666, 0x08,
	0x366b, 0x20,
	0x366c, 0xa4,
...
kal_uint16 addr_data_pair_capture_24fps_cereus_ov12a10[] = {
	0x3642, 0x12,
	0x3666, 0x08,
	0x366b, 0x20,
	0x366c, 0xa4,
...
kal_uint16 addr_data_pair_capture_30fps_cereus_ov12a10[] = {
	0x3642, 0x12,
	0x3666, 0x08,
	0x366b, 0x20,
	0x366c, 0xa4,
...
{
	LOG_DBG("capture 4096*3072 currefps = %d\n",
		currefps);
#if MULTI_WRITE
	if (currefps == 150) {
	cereus_ov12a10_table_write_cmos_sensor(
		addr_data_pair_capture_15fps_cereus_ov12a10,
		sizeof(addr_data_pair_capture_15fps_cereus_ov12a10) /
		sizeof(kal_uint16));
	} else if (currefps == 240) {
	cereus_ov12a10_table_write_cmos_sensor(
		addr_data_pair_capture_24fps_cereus_ov12a10,
		sizeof(addr_data_pair_capture_24fps_cereus_ov12a10) /
		sizeof(kal_uint16));
	} else {
	cereus_ov12a10_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_cereus_ov12a10,
		sizeof(addr_data_pair_capture_30fps_cereus_ov12a10) /
		sizeof(kal_uint16));
	}
#else
	if (currefps == 150) {
...
{
	LOG_DBG("normal_video_setting RES_4096*3072_30fps\n");
#if MULTI_WRITE
	cereus_ov12a10_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_cereus_ov12a10,
		sizeof(addr_data_pair_capture_30fps_cereus_ov12a10) /
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_hs_video_cereus_ov12a10[] = {
	0x3642, 0x08,
	0x3666, 0x08,
	0x366b, 0x20,
	0x366c, 0xa4,
...
#if MULTI_WRITE
	cereus_ov12a10_table_write_cmos_sensor(
		addr_data_pair_hs_video_cereus_ov12a10,
		sizeof(addr_data_pair_hs_video_cereus_ov12a10) /
		sizeof(kal_uint16));
#else
...
{
	LOG_DBG("slim_video_setting RES_2048*1536_30fps\n");
	preview_setting();
}

static void custom1_setting(void)
{
	LOG_DBG("CUSTOM1_setting 2048*1536_30fps\n");
#if MULTI_WRITE
	cereus_ov12a10_table_write_cmos_sensor(
		addr_data_pair_preview_cereus_ov12a10,
		sizeof(addr_data_pair_preview_cereus_ov12a10) /
...
{
	LOG_DBG("CUSTOM2_setting 4096*3072_30fps\n");
#if MULTI_WRITE
	cereus_ov12a10_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_cereus_ov12a10,
		sizeof(addr_data_pair_capture_30fps_cereus_ov12a10) /
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	LOG_DBG("CEREUS_OV12A10_OFILM_I2CBUS = %d, I2C_bus = %d\n", CEREUS_OV12A10_OFILM_I2CBUS, I2C_bus);
	if (I2C_bus != CEREUS_OV12A10_OFILM_I2CBUS) {
		*sensor_id = 0xFFFFFFFF;
		LOG_ERR("CEREUS_OV12A10_OFILM_I2CBUS: %d, I2C_bus = %d, Check Error!\n", CEREUS_OV12A10_OFILM_I2CBUS, I2C_bus);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		size = imgSensorReadEepromData(&sensor_eeprom_data,ov12a10Checksum);
		...
		do {
			*sensor_id = return_sensor_id();
			...
			LOG_ERR("ov12a read sensor id  fail [get_imgsensor_id] id: 0x%x\n",*sensor_id);
			retry--;
		} while(retry > 0);
...
static kal_uint32 open(void) //check out
{
	...
	spin_unlock(&imgsensor_drv_lock);
	do {
		sensor_id = return_sensor_id();//jack_yan read the id info
	if (sensor_id == imgsensor_info.sensor_id) {
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if (imgsensor.current_fps == imgsensor_info.cap2.max_framerate) {//PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
        imgsensor.pclk = imgsensor_info.cap2.pclk;
        imgsensor.line_length = imgsensor_info.cap2.linelength;
        imgsensor.frame_length = imgsensor_info.cap2.framelength;
        imgsensor.min_frame_length = imgsensor_info.cap2.framelength;
        //imgsensor.autoflicker_en = KAL_FALSE;
    }else if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {//PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
...
{
    LOG_DBG("set_video_mode framerate = %d\n ", framerate);
...
{
	LOG_DBG("set_auto_flicker_mode enable = %d, framerate = %d\n",
		enable, framerate);
			...
			MUINT32 framerate)
{
	...
	LOG_DBG("scenario_id = %d, framerate = %d, current_fps: %d\n",
		scenario_id, framerate, imgsensor.current_fps);
	LOG_DBG("set_framelength: %d\n", imgsensor.frame_length);
	...
	break;
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor_info.cap1.max_framerate ==
			...
			imgsensor.min_frame_length = imgsensor.frame_length;
			spin_unlock(&imgsensor_drv_lock);
		}	else if	(imgsensor_info.cap2.max_framerate ==
				imgsensor.current_fps) {
...
{
	LOG_DBG("set_test_pattern_mode enable: %d\n", enable);
