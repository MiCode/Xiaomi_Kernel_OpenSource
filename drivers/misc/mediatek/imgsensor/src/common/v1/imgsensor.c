...
void IMGSENSOR_PROFILE(struct timeval *ptv, char *tag)
{
	...
	do_gettimeofday(&tv);
	time_interval = (tv.tv_sec - ptv->tv_sec) * 1000000 +
...
struct IMGSENSOR_SENSOR *imgsensor_sensor_get_inst(
	...
	MUINT32 *pFeatureParaLen)
{
	...
	if (psensor_func &&
		...
		ret = psensor_func->SensorFeatureControl(FeatureId,
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	imgsensor_hw_power(&pgimgsensor->hw, psensor,
		psensor_inst->psensor_name, IMGSENSOR_HW_POWER_STATUS_ON);

	imgsensor_sensor_feature_control(psensor,
		SENSOR_FEATURE_CHECK_SENSOR_ID, (MUINT8 *)&sensorID, &retLen);
	...
	if (err != ERROR_NONE)
		PK_DBG("ERROR:%s(), No imgsensor alive\n", __func__);

	imgsensor_hw_power(&pgimgsensor->hw, psensor,
		psensor_inst->psensor_name, IMGSENSOR_HW_POWER_STATUS_OFF);
...
#define STRINGIZE(stringizedName) TOSTRING(stringizedName)
	...
	struct device_node *of_node = of_find_compatible_node(NULL,
									NULL, "mediatek,camera_hw");
	...
	if (get_search_list) {
		psensor_list_with_end = kmalloc(strlen(sensor_kconfig)-1,
									GFP_KERNEL);
	}
	if (psensor_list_with_end != NULL) {
		...
		memcpy(psensor_list_with_end, sensor_kconfig+1,
			strlen(sensor_kconfig)-2);
		*(psensor_list_with_end+strlen(sensor_kconfig)-2) = '\0';
		of_property_read_string(of_node, "enable-sensor",
			&pDTS_sensors);

		PK_DBG("psensor_list_with_end %s ,pDTS_sensors %s\n",
			psensor_list_with_end,
			pDTS_sensors == NULL ? "null" : pDTS_sensors);
		...
		while (driver_name != NULL) {
			for (j = 0; j < MAX_NUM_OF_SUPPORT_SENSOR; j++) {
				if (pSensorList[j].init == NULL)
					break;
				else if (!strcmp(driver_name,
							pSensorList[j].name)) {
					if (pDTS_sensors != NULL &&
						!strstr(pDTS_sensors, driver_name))
						continue;
 ...
 *\n %d %d %d %d\n %d %d %d %d\n %d %d %d %d\n %d %d %d %d\n",
 *get_search_list,
 *orderedSearchList[0], orderedSearchList[1],
 *orderedSearchList[2], orderedSearchList[3],
 *orderedSearchList[4], orderedSearchList[5],
 *orderedSearchList[6], orderedSearchList[7],
 *orderedSearchList[8], orderedSearchList[9],
 *orderedSearchList[10], orderedSearchList[11],
 *orderedSearchList[12], orderedSearchList[13],
 *orderedSearchList[14], orderedSearchList[15]);
 */
...
MUINT32 Get_Camera_Temperature(CAMERA_DUAL_CAMERA_SENSOR_ENUM senDevId,
...
static void cam_temperature_report_wq_routine(struct work_struct *data)
{
	...
	/* Main cam */
	ret = Get_Camera_Temperature(DUAL_CAMERA_MAIN_SENSOR,
		&valid[0], &temp[0]);
	...
	/* Sub cam */
	ret = Get_Camera_Temperature(DUAL_CAMERA_SUB_SENSOR,
		&valid[1], &temp[1]);
	PK_INFO("senDevId(%d), valid(%d), temperature(%d)\n",
	...
	/* Main2 cam */
	ret = Get_Camera_Temperature(DUAL_CAMERA_MAIN_2_SENSOR,
			&valid[2], &temp[2]);
	PK_INFO("senDevId(%d), valid(%d), temperature(%d)\n",
	...
	ret = Get_Camera_Temperature(DUAL_CAMERA_SUB_2_SENSOR,
			&valid[3], &temp[3]);
	PK_INFO("senDevId(%d), valid(%d), temperature(%d)\n",
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	psensorResolution = kzalloc(sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT),
		GFP_KERNEL);
	...
	imgsensor_sensor_get_info(psensor,
		MSDK_SCENARIO_ID_CAMERA_PREVIEW, pInfo, pConfig);
	imgsensor_sensor_get_info(psensor,
		MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG, pInfo1, pConfig1);
	imgsensor_sensor_get_info(psensor,
		MSDK_SCENARIO_ID_VIDEO_PREVIEW, pInfo2, pConfig2);
	imgsensor_sensor_get_info(psensor,
		MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO, pInfo3, pConfig3);
	imgsensor_sensor_get_info(psensor,
		MSDK_SCENARIO_ID_SLIM_VIDEO, pInfo4, pConfig4);
	...
	imgsensor_sensor_get_info(psensor,
		MSDK_SCENARIO_ID_CUSTOM1, pInfo, pConfig);
	imgsensor_sensor_get_info(psensor,
		MSDK_SCENARIO_ID_CUSTOM2, pInfo1, pConfig1);
	imgsensor_sensor_get_info(psensor,
		MSDK_SCENARIO_ID_CUSTOM3, pInfo2, pConfig2);
	imgsensor_sensor_get_info(psensor,
		MSDK_SCENARIO_ID_CUSTOM4, pInfo3, pConfig3);
	imgsensor_sensor_get_info(psensor,
		MSDK_SCENARIO_ID_CUSTOM5, pInfo4, pConfig4);
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
 ...
 /* return TRUE:play flashlight */
	...
	case SENSOR_FEATURE_GET_EV_AWB_REF:
	{
		...
		pAeAwbRef = kmalloc(sizeof(SENSOR_AE_AWB_REF_STRUCT),
						GFP_KERNEL);
		if (pAeAwbRef == NULL) {
			kfree(pFeaturePara);
			PK_PR_ERR(" ioctl allocate mem failed\n");
	...
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
	{
		...
		void *usr_ptr_Reg = (void *)(uintptr_t) (*(pFeaturePara_64
							+ 1));
		kal_uint32 *pReg = NULL;

		/* buffer size exam */
		if ((sizeof(kal_uint8) * u4RegLen) >
			FEATURE_CONTROL_MAX_DATA_SIZE) {
			kfree(pFeaturePara);
	...
	case SENSOR_FEATURE_GET_SHUTTER_GAIN_AWB_GAIN:
	{
		...
		pCurAEAWB = kmalloc(sizeof(SENSOR_AE_AWB_CUR_STRUCT),
						GFP_KERNEL);
		if (pCurAEAWB == NULL) {
			kfree(pFeaturePara);
			PK_PR_ERR(" ioctl allocate mem failed\n");
	...
	case SENSOR_FEATURE_GET_DELAY_INFO:
	{
		...
		pDelayInfo = kmalloc(sizeof(SENSOR_DELAY_INFO_STRUCT),
						GFP_KERNEL);
		if (pDelayInfo == NULL) {
			kfree(pFeaturePara);
			PK_PR_ERR(" ioctl allocate mem failed\n");
	...
	case SENSOR_FEATURE_GET_AE_FLASHLIGHT_INFO:
	{
		...
		pFlashInfo = kmalloc(sizeof(SENSOR_FLASHLIGHT_AE_INFO_STRUCT),
						GFP_KERNEL);
		if (pFlashInfo == NULL) {
			kfree(pFeaturePara);
			PK_PR_ERR(" ioctl allocate mem failed\n");
...
/*PK_DBG("[CAMERA SENSOR] CAOMPAT_KDIMGSENSORIOC_X_GETINFO E\n"); */
		...
		ret = filp->f_op->unlocked_ioctl(filp,
			KDIMGSENSORIOC_X_GET_CONFIG_INFO, (unsigned long)data);
...
/* PK_DBG("[CAMERA SENSOR] CAOMPAT_KDIMGSENSORIOC_X_FEATURECONCTROL\n"); */
		...
		err = compat_get_acdk_sensor_featurecontrol_struct(data32,
			data);
		if (err)
			return err;

		ret = filp->f_op->unlocked_ioctl(filp,
			KDIMGSENSORIOC_X_FEATURECONCTROL, (unsigned long)data);
		err = compat_put_acdk_sensor_featurecontrol_struct(data32,
			data);
	...
	case COMPAT_KDIMGSENSORIOC_X_CONTROL:
	{
		...
		if (err)
			return err;
		ret = filp->f_op->unlocked_ioctl(filp,
			KDIMGSENSORIOC_X_CONTROL, (unsigned long)data);
	...
	case COMPAT_KDIMGSENSORIOC_X_GETINFO2:
	{
		...
		if (err)
			return err;
		ret = filp->f_op->unlocked_ioctl(filp,
			KDIMGSENSORIOC_X_GETINFO2, (unsigned long)data);
	...
	unsigned long a_u4Param)
{
	...
	case KDIMGSENSORIOC_X_SET_MCLK_PLL:
		i4RetValue = imgsensor_clk_set(&pgimgsensor->clk,
			(ACDK_SENSOR_MCLK_STRUCT *)pBuff);
		break;
...
static inline int imgsensor_driver_register(void)
{
	...
	gimgsensor_device = device_create(gpimgsensor_class,
		NULL, dev_no, NULL, IMGSENSOR_DEV_NAME);
...
#ifdef CONFIG_MTK_SMI_EXT
	mmdvfs_register_mmclk_switch_cb(mmsys_clk_change_cb,
...
static int __init imgsensor_init(void)
{
	...
	INIT_DELAYED_WORK(&cam_temperature_wq,
