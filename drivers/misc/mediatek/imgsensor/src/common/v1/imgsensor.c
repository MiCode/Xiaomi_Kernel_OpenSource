	...
	for (i = 0; i < MAX_NUM_OF_SUPPORT_SENSOR; i++) {
        ...
        switch(psensor->inst.sensor_idx){
            ...
            case IMGSENSOR_SENSOR_IDX_MAIN:
                if((pSensorList[drv_idx].id != LOTUS_IMX486_OFILM_SENSOR_ID) &&\
                    (pSensorList[drv_idx].id != LOTUS_OV12A10_SUNNY_SENSOR_ID))
                    continue;
            break;

            case IMGSENSOR_SENSOR_IDX_SUB:
                if((pSensorList[drv_idx].id != LOTUS_S5K4H7YX_OFILM_SENSOR_ID) &&\
                    (pSensorList[drv_idx].id != LOTUS_S5K4H7YX_QTECH_SENSOR_ID))
                    continue;
            break;

            case IMGSENSOR_SENSOR_IDX_MAIN2:
                if((pSensorList[drv_idx].id != LOTUS_OV02A10_OFILM_SENSOR_ID) &&\
                    (pSensorList[drv_idx].id != LOTUS_OV02A10_SUNNY_SENSOR_ID))
                    continue;
            break;
			...
			if (psensor->pfunc) {
				...
				psensor_inst->status.arch =
					...
					if (!imgsensor_check_is_alive(psensor)) {
						switch (pSensorList[drv_idx].id) {
						case LOTUS_IMX486_OFILM_SENSOR_ID:
						hq_regiser_hw_info(HWID_MAIN_CAM + psensor->inst.sensor_idx,"ofilm_imx486_i");
						break;
					case LOTUS_OV12A10_SUNNY_SENSOR_ID:
						hq_regiser_hw_info(HWID_MAIN_CAM + psensor->inst.sensor_idx,"sunny_ov12a10_ii");
						break;
					case LOTUS_S5K4H7YX_OFILM_SENSOR_ID:
						hq_regiser_hw_info(HWID_MAIN_CAM + psensor->inst.sensor_idx,"ofilm_s5k4h7yx_i");
						break;
					case LOTUS_S5K4H7YX_QTECH_SENSOR_ID:
						hq_regiser_hw_info(HWID_MAIN_CAM + psensor->inst.sensor_idx,"qtech_s5k4h7yx_ii");
						break;
					case LOTUS_OV02A10_OFILM_SENSOR_ID:
						hq_regiser_hw_info(HWID_MAIN_CAM + psensor->inst.sensor_idx,"ofilm_ov02a10_i");
						break;
					case LOTUS_OV02A10_SUNNY_SENSOR_ID:
						hq_regiser_hw_info(HWID_MAIN_CAM + psensor->inst.sensor_idx,"sunny_ov02a10_ii");
						break;
					}
					pr_info(
					    "[%s]:[%d][%d][%s]\n",
