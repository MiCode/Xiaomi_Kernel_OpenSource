 ...
 * Filename:
 * ---------
 *     sc202cs_sunny2_mipi_raw_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "sc202cs_sunny2_mipi_raw_Sensor.h"
#define Lens_ID 0xB7
/****************************Modify Following Strings for Debug****************************/
#define PFX "sc202cs_sunny2_mipi_raw"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SC202CS_SUNNY2_DEPTH_SENSOR_ID,
...
{
	return ((read_cmos_sensor(0x3107) << 8) | read_cmos_sensor(0x3108)+4); //0xeb56
}
...
{
	LOG_INF("depth-sunny2 sensor_init");
...
*************************************************************************/
  ...
  static kal_int8 sc202cs_sunny2_read_general_info(int *addr, kal_uint16 addr_len, kal_uint8 *result)
  {
  	int i = 0;
    ...
    write_cmos_sensor(0x440b, 0x12);
  	...
  	for (i = 0; i < addr_len; i++) {
        ...
        LOG_INF("depth-sunny2 read result[%d] is %x", i, result[i]);
  	}
  	return 0;
  }

  unsigned int sc202cs_sunny2_read_otp(void)
  {
  	int ret = 0;
	kal_uint8 i;
	int otpdata_addr[] = {0x8008,0x8009, 0x800A,0x800B,0x8011, 0x8012, 0x8013, 0x8014,0x8015,0x8016, 0x8017, 0x8018, 0x8019,0x801A,0x801B,0x801D,0x801F,0x8000,0x8001,0x8002,0x8003,0x8004,0x8005,0x8006,0x8007};
	ret = sc202cs_sunny2_read_general_info(otpdata_addr, ARRAY_SIZE(otpdata_addr), otpData);
  	if (ret != 0) {
  		LOG_INF("depth-sunny2 read fuseid failed\n");
  		return -1;
  	}
     for(i = 0; i < ARRAY_SIZE(otpData); i++){
		LOG_INF("depth-sunny2 read otp data 0x%x", otpData[i]);
	}
  	return ret;
  }

  unsigned int sc202cs_sunny2_read_otp_info(struct i2c_client *client,
  		 unsigned int addr, unsigned char *data, unsigned int size)
  {
  	int num = ARRAY_SIZE(otpData);
	int index = 0;
	LOG_INF(" depth-sunny2 addr:0x%x ,size:%d",addr, size);
	for(;index<num ;index++){
		data[index] = otpData[index];
	}
  	return num;
  }
EXPORT_SYMBOL(sc202cs_sunny2_read_otp_info);
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	LOG_INF("depth-sunny2 getimgsensorid!!!\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            LOG_INF("depth-sunny2 return_sensor_id:0x%x , lens_id = 0x%x\n",*sensor_id,lens_id);
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("depth-sunny2 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                if(lens_id == Lens_ID){
                    ret = sc202cs_sunny2_read_otp();
                    return ERROR_NONE;
                }
                else{
                    LOG_INF("depth-sunny2 read lens_id fail");
                    *sensor_id = 0xFFFFFFFF;
                    return ERROR_SENSOR_CONNECT_FAIL;
                }
            }
            LOG_INF("depth-sunny2 Read sensor id fail, return_sensor_id: 0x%x\n",*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    /* N17 code for HQ-293330 by chenxiaoyong at 2023/05/8 start */
    sensor_info->FrameTimeDelayFrame = imgsensor_info.frame_time_delay_frame;
    /* N17 code for HQ-293330 by chenxiaoyong at 2023/05/8 end */
...
UINT32 SC202CS_SUNNY2_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
    ...
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SC202CS_sunny2_MIPI_RAW_SensorInit    */
