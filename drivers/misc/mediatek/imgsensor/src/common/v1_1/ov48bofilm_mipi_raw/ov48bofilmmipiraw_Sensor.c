...
#include "ov48bofilmmipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV48BOFILM_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
			...
			kal_uint16 frame_length)
{
	...
	write_cmos_sensor(0x3502, (shutter) & 0xFF);
	write_cmos_sensor(0x3501, (shutter >> 8) & 0xFF);
	printk("[OV48BI]Add for N3D! shutter =%d, framelength =%d\n",
		shutter, imgsensor.frame_length);
}
...
#if MULTI_WRITE
#if 0 /*for debug*/
	printk("[ov48bofilm-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d start", len);
	for (i = 0; i < len/2; i++)
		LOG_INF("[ov48bofilm-debug]readback addr 0x%04x, 0x%02x,\n",
			para[2*i], read_cmos_sensor(para[2*i]) & 0xFF);
	printk("[ov48bofilm-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d end", len);
#endif
#endif
	return 0;
}
...
static void sensor_init(void)
{
	...
	//mdelay(10);
	table_write_cmos_sensor(ov48bofilm_init_setting,
			sizeof(ov48bofilm_init_setting) / sizeof(kal_uint16));
}	/*	sensor_init  */
...
static void preview_setting(void)
{
	...
	table_write_cmos_sensor(ov48bofilm_preview_setting,
		sizeof(ov48bofilm_preview_setting) / sizeof(kal_uint16));
}	/*	preview_setting  */
...
static void capture_setting(kal_uint16 currefps)
{
	...
	table_write_cmos_sensor(ov48bofilm_capture_setting,
		sizeof(ov48bofilm_capture_setting) / sizeof(kal_uint16));
}	/*	preview_setting  */
...
static void hs_video_setting(void)
{
	...
	table_write_cmos_sensor(ov48bofilm_hs_video_setting,
		sizeof(ov48bofilm_hs_video_setting) / sizeof(kal_uint16));
}
...
static void custom1_setting(void)
{
	...
	table_write_cmos_sensor(ov48bofilm_custom1_setting,
			sizeof(ov48bofilm_custom1_setting) / sizeof(kal_uint16));
}
...
static void custom2_setting(void)
{
	...
	table_write_cmos_sensor(ov48bofilm_custom2_setting,
			sizeof(ov48bofilm_custom2_setting) / sizeof(kal_uint16));
}
...
static void custom3_setting(void)
{
	...
	table_write_cmos_sensor(ov48bofilm_custom3_setting,
			sizeof(ov48bofilm_custom3_setting) / sizeof(kal_uint16));
}
...
static void custom4_setting(void)
{
	...
	table_write_cmos_sensor(ov48bofilm_custom4_setting,
			sizeof(ov48bofilm_custom4_setting) / sizeof(kal_uint16));
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	LOG_INF("OV48BOFILM get_imgsensor_id in\n");
...
UINT32 OV48BOFILM_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    LOG_INF("OV48BOFILM_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	OV48BOFILM_MIPI_RAW_SensorInit	*/
