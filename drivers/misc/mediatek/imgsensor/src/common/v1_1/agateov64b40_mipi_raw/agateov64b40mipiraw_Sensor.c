...
#include "agateov64b40mipiraw_Sensor.h"
#include "agateov64b40_seamless_switch.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = AGATEOV64B40_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
#else
 { 9248, 6944, 2312, 1736,    4624, 3472, 4624, 3472,   0,     0, 4624, 3472,     0, 0, 4624, 3472}, // custom5 in sensor zoom2x
#endif
...
#ifdef IS_ZOOM_4X
	/* Custom5 mode setting 288(pxiel)*432 */
	{
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
		0x00, 0x2b, 2312, 1736, 0x00, 0x00, 0x0000, 0x0000,
		0x01, 0x2b,  288,  432, 0x00, 0x00, 0x0000, 0x0000
	},
#else
	/* Custom5 mode setting 288(pxiel)*432 */
	{
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
		0x00, 0x2b, 4624, 3472, 0x00, 0x00, 0x0000, 0x0000,
		//0x01, 0x2b,  576,  868, 0x00, 0x00, 0x0000, 0x0000
	},
#endif
...
static struct SEAMLESS_SYS_DELAY seamless_sys_delays[] = {
	{MSDK_SCENARIO_ID_CUSTOM1, MSDK_SCENARIO_ID_CUSTOM5, 1},
	{MSDK_SCENARIO_ID_CUSTOM5, MSDK_SCENARIO_ID_CUSTOM1, 1},
};
...
static kal_uint32 get_frame_length(kal_uint32 shutter)
{
		unsigned long flags;
...
#if MULTI_WRITE
#if 0 /*for debug*/
	LOG_INF("[agateov64b40-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d start", len);
	for (i = 0; i < len/2; i++)
		LOG_INF("[agateov64b40-debug]readback addr 0x%04x, 0x%02x,\n",
			para[2*i], read_cmos_sensor(para[2*i]) & 0xFF);
	LOG_INF("[agateov64b40-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d end", len);
#endif
#endif
	return 0;
}
...
#define PDC_EEPROM_OFFSET_SUNNY   0x12D2  // [12-731]
#define PDC_EEPROM_OFFSET_OFILM   0x12D0  // [12-731]
...
static void read_cali_data_from_eeprom(void)
{
	...
	kal_uint16 pdc_eeprom_offset      = PDC_EEPROM_OFFSET_SUNNY;
	...
	if (0x01 == vendor_id) {
		pdc_eeprom_offset = PDC_EEPROM_OFFSET_SUNNY;
	} else if (0x07 == vendor_id) {
		pdc_eeprom_offset = PDC_EEPROM_OFFSET_OFILM;
	}
...
static void sensor_init(void)
{
	...
	//mdelay(10);
	table_write_cmos_sensor(agateov64b40_init_setting,
			sizeof(agateov64b40_init_setting) / sizeof(kal_uint16));
}	/*	sensor_init  */
...
static void preview_setting(void)
{
	...
	table_write_cmos_sensor(agateov64b40_preview_setting,
		sizeof(agateov64b40_preview_setting) / sizeof(kal_uint16));
}	/*	preview_setting  */
...
static void normal_video_setting()
{
	...
	table_write_cmos_sensor(agateov64b40_normal_video_setting,
		sizeof(agateov64b40_normal_video_setting) / sizeof(kal_uint16));
}
...
static void hs_video_setting(void)
{
	...
	table_write_cmos_sensor(agateov64b40_hs_video_setting,
		sizeof(agateov64b40_hs_video_setting) / sizeof(kal_uint16));
}
...
static void custom2_setting(void)
{
	...
	table_write_cmos_sensor(agateov64b40_custom2_setting,
			sizeof(agateov64b40_custom2_setting) / sizeof(kal_uint16));
}
...
static void custom3_setting(void)
{
	...
	table_write_cmos_sensor(agateov64b40_custom3_setting,
			sizeof(agateov64b40_custom3_setting) / sizeof(kal_uint16));
}
...
static void custom4_setting(void)
{
	...
	table_write_cmos_sensor(agateov64b40_custom4_setting,
			sizeof(agateov64b40_custom4_setting) / sizeof(kal_uint16));
}
...
static void custom5_setting(void)
{
	...
	table_write_cmos_sensor(agateov64b40_custom5_setting,
			sizeof(agateov64b40_custom5_setting) / sizeof(kal_uint16));
}
...
static void custom6_setting(void)
{
	LOG_INF("E\n");
	preview_setting();
	//table_write_cmos_sensor(agateov64b40_custom6_setting,
			//sizeof(agateov64b40_custom6_setting) / sizeof(kal_uint16));
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	LOG_INF("AGATEOV64B40 get_imgsensor_id in\n");
...
{
	kal_uint32 delay       = 40;
	kal_uint32 current_fps = 300;
	kal_uint32 reg_gain;

	if (gain > 992)
		gain = 992;
	reg_gain = gain*4;
	shutter = get_frame_length(shutter);

	LOG_INF("reg_gain =%d, shutter =%d, frame_length =%d,\n",reg_gain, shutter, imgsensor.frame_length);

	switch (scenario_id) {
	case MSDK_SCENARIO_ID_CUSTOM1:
		LOG_INF("seamless switch to full size!\n");
		spin_lock(&imgsensor_drv_lock);
...
//		imgsensor.frame_length = imgsensor_info.custom1.framelength;
		...
		if (reg_gain != 0) {
			ov64b40sunny_seamless_preview[35] =
				(reg_gain >> 8) & 0xff;
			ov64b40sunny_seamless_preview[37] =
				reg_gain & 0xff;
		}
		if (shutter != 0) {
			ov64b40sunny_seamless_preview[31] =
				(shutter >> 8) & 0xff;
			ov64b40sunny_seamless_preview[33] =
				shutter & 0xff;
			ov64b40sunny_seamless_preview[109] =
				(imgsensor.frame_length >> 8) & 0xff;
			ov64b40sunny_seamless_preview[111] =
...
//		imgsensor.frame_length = imgsensor_info.custom5.framelength;
		...
		if (reg_gain != 0) {
			ov64b40sunny_seamless_custom5_2x[35] =
				(reg_gain >> 8) & 0xff;
			ov64b40sunny_seamless_custom5_2x[37] =
				reg_gain & 0xff;
		}
		if (shutter != 0) {
			ov64b40sunny_seamless_custom5_2x[31] =
				(shutter >> 8) & 0xff;
			ov64b40sunny_seamless_custom5_2x[33] =
				shutter & 0xff;
			ov64b40sunny_seamless_custom5_2x[109] =
				(imgsensor.frame_length >> 8) & 0xff;
			ov64b40sunny_seamless_custom5_2x[111] =
				imgsensor.frame_length & 0xff;
		}
...
#ifdef IS_ZOOM_4X
		LOG_INF("seamless switch to zoom 4x!\n");
		table_write_cmos_sensor(ov64b40sunny_seamless_custom5_4x,
			sizeof(ov64b40sunny_seamless_custom5_4x) / sizeof(kal_uint16));
#else
		LOG_INF("seamless switch to zoom 2x!\n");
		table_write_cmos_sensor(ov64b40sunny_seamless_custom5_2x,
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	mdelay(10); // workaround : fix i2c timeout
	switch (scenario_id) {
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
...
UINT32 AGATEOV64B40_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    LOG_INF("AGATEOV64B40_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	AGATEOV64B40_MIPI_RAW_SensorInit	*/
