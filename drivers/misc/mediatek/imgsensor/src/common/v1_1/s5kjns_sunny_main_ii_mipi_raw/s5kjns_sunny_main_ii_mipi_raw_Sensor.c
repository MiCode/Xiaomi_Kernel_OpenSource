...
#include "s5kjns_sunny_main_ii_mipi_raw_Sensor.h"
#define FPTPDAFSUPPORT
#define MULTI_WRITE 1
#define OTP_DATA_NUMBER 9
#define PFX "s5kjns_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJNS_SUNNY_MAIN_II_SENSOR_ID ,
	.checksum_value = 0xdb9c643,
	.pre = {
		.pclk = 560000000,
		.linelength = 5888,//5888,
		.framelength = 3168,//3168,
		.startx = 0,
		.starty = 0,
...
static kal_uint16 read_cmos_sensor_s5kjns(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pu_send_cmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2CTiming(pu_send_cmd, 2, (u8 *) &get_byte, 1,
				0xA2, imgsensor_info.i2c_speed);
	return get_byte;
}
static void s5kjns_fusion_id_read(void)
{
	int i;
	for (i=0; i<6; i++) {
...
static void set_shutter(kal_uint32 shutter)
{
	...
	static kal_uint32 long_shutter = 0;
	static kal_uint32 new_framelength = 0;
	...
	if (shutter < SHUTTER_1) {
		...
		//write_cmos_sensor(0x0E10, 0x0005);
		//write_cmos_sensor(0x0E12, 0x0020);
		//write_cmos_sensor(0x0E14, 0xEC0C);//30 shutter
		//write_cmos_sensor(0x0E16, 0x0020);
		//write_cmos_sensor(0x0704, 0x0500);//32 sec
		//write_cmos_sensor_byte(0x0100, 0x01); //stream on

		//SHUTTER = (HEX2DEC(4000_0342) * HEX2DEC(4000_0202) * (2^HEX2DEC(4000_0704))) / (560*10^6)
		//HEX2DEC(4000_0202) = SHUTTER * (560*10^6) / ((HEX2DEC(4000_0342) * (2^HEX2DEC(4000_0704))
		//HEX2DEC(4000_0342) = 0x1700 = 5888;
		//HEX2DEC(4000_0704) = 2^7 = 128;
		//SHUTTER = shutter / 94750;
		//HEX2DEC(4000_0202) = (SHUTTER * 560*10^6) / ((HEX2DEC(4000_0342) * (2^HEX2DEC(4000_0704))
		//		     = ((shutter / 94750) * 560*10^6) / (5888 * 128) 
		//		     = shutter/128
		...
		write_cmos_sensor(0x0702, 0x0700); //shifter for shutter
		write_cmos_sensor(0x0704, 0x0700); //shifter for shutter
		write_cmos_sensor_byte(0x0100, 0x01); //stream on

		LOG_INF("cxc  long_shutter =%d, new_framelength =%d\n", long_shutter,new_framelength);
	}
	//write_cmos_sensor(0X0202, shutter & 0xFFFF);
...
UINT32 S5KJNS_SUNNY_MAIN_II_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	LOG_INF("S5KJNS_SUNNY_MAIN_II_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc =  &sensor_func;
	return ERROR_NONE;
}	/*	S5KJNS_SUNNY_MAIN_II_MIPI_RAW_SensorInit	*/
