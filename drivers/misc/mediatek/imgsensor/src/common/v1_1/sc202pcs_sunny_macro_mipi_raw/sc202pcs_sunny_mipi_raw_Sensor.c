 ...
 * Filename:
 * ---------
 *     sc202pcs_sunny_mipi_raw_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "sc202pcs_sunny_mipi_raw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "sc202pcs_sunny_mipi_raw"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SC202PCS_SUNNY_MACRO_SENSOR_ID,
...
{
	return ((read_cmos_sensor(0x3107) << 8) | read_cmos_sensor(0x3108))+3; //0xeb52
}
...
/* N17 code for HQ-296502 by yinrong at 2023/05/24 start */
#if 1
/* N17 code for HQ-296502 by yinrong at 2023/05/24 end */
...
{
	LOG_INF("macro-sunny sensor_init");
...
*************************************************************************/
  ...
  static kal_int8 sc202pcs_sunny_read_general_info(int *addr, kal_uint16 addr_len, kal_uint8 *result)
  {
  	int i = 0;
    ...
    write_cmos_sensor(0x440b, 0x12);
  	...
  	for (i = 0; i < addr_len; i++) {
        ...
        LOG_INF("macro-sunny read result[%d] is %x", i, result[i]);
  	}
  	return 0;
  }

  unsigned int sc202pcs_sunny_read_otp(void)
  {
  	int ret = 0;
	kal_uint8 i;
	int otpdata_addr[] = {0x8009, 0x800A,0x800B,0x8011, 0x8012, 0x8013, 0x8016, 0x8017, 0x8018, 0x8019};
	ret = sc202pcs_sunny_read_general_info(otpdata_addr, ARRAY_SIZE(otpdata_addr), otpData);
  	if (ret != 0) {
  		LOG_INF("macro-sunny read fuseid failed\n");
  		return -1;
  	}
     for(i = 0; i < ARRAY_SIZE(otpData); i++){
		LOG_INF("macro-sunny read otp data 0x%x", otpData[i]);
	}
  	return ret;
  }

  unsigned int sc202pcs_sunny_read_otp_info(struct i2c_client *client,
  		 unsigned int addr, unsigned char *data, unsigned int size)
  {
  	int num = ARRAY_SIZE(otpData);
	int index = 0;
	LOG_INF(" macro-sunny addr:0x%x ,size:%d",addr, size);
	for(;index<num ;index++){
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
   ...
   // int ret = 0;
    /*ret  = sc202pcs_sunny_read_otp();
	if (ret != 0) {
		pr_err("macro-sunny Read sc202pcs sunny otp info failed");
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}*/
	pr_err("macro-sunny get imgsensorid!!!\n");
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            pr_err("macro-sunny return_sensor_id:0x%x\n",*sensor_id);
	...
	kal_uint8 i = 0;
	 ...
	 while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		 ...
		 do {
			 sensor_id = return_sensor_id();
			 if (sensor_id == imgsensor_info.sensor_id) {				 
				 LOG_INF("macro-sunny i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);   
				 break;
			 }	 
			 LOG_INF("macro-sunny Read sensor id fail, return_sensor_id: 0x%x\n",sensor_id);
			 retry--;
		 } while(retry > 0);
		 i++;
		 if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 SC202PCS_SUNNY_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
    ...
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SC202PCS_SUNNY_MIPI_RAW_SensorInit    */
