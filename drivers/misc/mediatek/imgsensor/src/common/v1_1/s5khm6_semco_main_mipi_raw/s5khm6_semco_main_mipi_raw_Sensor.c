...
#include "s5khm6_semco_main_mipi_raw_Sensor.h"
//N17 code for HQ-293325 by wangqiang start
#include "s5khm6_semco_setting.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KHM6_SEMCO_MAIN_SENSOR_ID,	//0x1AD6
...
/* N17 code for HQ-293325 by wuzhenyue at 2023/04/29 end */
/*************************************************************************
 * FUNCTION
 *	get_imgsensor_id
 *
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 3;
	/* N17 code for HQ-293325 by wuzhenyue at 2023/04/29 start */
...
//N17 code for HQ-293325 by wuzhenyue at 2023/05/16 start
			*sensor_id = read_cmos_sensor(0x0000);
			LOG_INF("s5khm6-semco sensor id = 0x%x, vendor id = 0x%x",*sensor_id ,vendor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("s5khm6-semco i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				if(vendor_id == VENDOR_ID){
					return ERROR_NONE;
				}
				else{
					LOG_INF("s5khm6-semco read vendor id fail");
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				}
			}
			LOG_INF("s5khm6-semco Read sensor id fail, sensor id: 0x%x, i2c: 0x%x\n", *sensor_id, imgsensor.i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
