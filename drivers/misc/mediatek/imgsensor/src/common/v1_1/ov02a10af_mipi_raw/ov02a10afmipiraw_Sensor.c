...
#include "ov02a10afmipiraw_Sensor.h"
...
static kal_uint16 read_eeprom_module_id(kal_uint32 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8 *) &get_byte, 1, 0xA8);//Ofilm
...
static kal_uint32 return_sensor_id(void)
{
	...
	printk("[%s][ov02a10af] sensor_id: 0x%x vcm_id: 0x%x", __func__, sensor_id, vcm_id);
			...
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_debug("i2c write id: 0x%x, sensor id: 0x%x module id: %s\n",
					imgsensor.i2c_write_id, *sensor_id, "OFILM");
