...
static void gc6133c_vcam_control(struct gc6133c *gc6133c, bool flag)
{
        ...
        if (flag) {
                ...
                ret_vaule = regulator_enable(vcamio);
        } else {
...
{
	gc6133c_hw_off_reset1(g_gc6133c);
	mdelay(5);
	// gc6133c_vcam_control(g_gc6133c, true);
	mdelay(1);
	gc6133c_vcama_control(g_gc6133c, true);
	mdelay(5);
	gc6133c_hw_on_reset1(g_gc6133c);
			     ...
			     const struct i2c_device_id *id)
{
	...
	struct pinctrl_state *gc6133c_mclk_on = NULL;
	struct pinctrl_state *gc6133c_mclk_off = NULL;
...
void gc6133c_yuv_init_robe(void)
{
	printk("robe_debug: %s run begin.\n", __func__);
	gc6133c_yuv_init();
	printk("robe_debug: %s run end.\n", __func__);
...
void gc6133c_yuv_exit_robe(void)
{
	printk("robe_debug: %s run begin.\n", __func__);
	gc6133c_yuv_exit();
	printk("robe_debug: %s run end.\n", __func__);
...
MODULE_SOFTDEP("pre: imgsensor_isp6s post: gc_gc6133c_i");
*/
