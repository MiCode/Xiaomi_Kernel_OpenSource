...
static imgsensor_info_struct imgsensor_info = { 
    .sensor_id = SC202CS_AAC_DEPTH_SENSOR_ID,
	...
	.min_gain_iso = 100,
	.gain_step = 4,
	.gain_type = 3,/*to be modify,no gain table for gc*/
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_MONO,    //+bug682590,zhanghengyuan.wt,ADD,2021/8/26,fix depth sc202cs output_dataformat
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	else if (reg_gain > SC202CS_SENSOR_GAIN_MAX)
		reg_gain = SC202CS_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 SC202CS_AGC_Param[SC202CS_SENSOR_GAIN_MAX_VALID_INDEX][2] = {
		{  1024,  0x00 },
		{  2048,  0x01 },
		{  4096,  0x03 },
		{  8192,  0x07 },
...
/* N17 code for HQ-299574 by wuzhenyue at 2023/06/15 start */
	for (gain_index = SC202CS_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 1; gain_index--)
/* N17 code for HQ-299574 by wuzhenyue at 2023/06/15 end */
		if (reg_gain >= SC202CS_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0x3e09, SC202CS_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * SC202CS_SENSOR_GAIN_BASE / SC202CS_AGC_Param[gain_index][0];
...
/* N17 code for HQ-293329 by wuzhenyue at 2023/04/28 start */
  static char otpData[30]={0};
/* N17 code for HQ-293329 by wuzhenyue at 2023/04/28 start */
     /* N17 code for HQ-293329 by wuzhenyue at 2023/04/27 start */
  static kal_int8 sc202cs_aac_read_general_info(int *addr, kal_uint16 addr_len, kal_uint8 *result)
     /* N17 code for HQ-293329 by wuzhenyue at 2023/04/27 end */
  {
    ...
    write_cmos_sensor(0x440b, 0x12);
  	...
  	for (i = 0; i < addr_len; i++) {
        ...
        LOG_INF("depth-aac read result[%d] is %x", i, result[i]);
  	}
  	return 0;
  }
   /* N17 code for HQ-293329 by wuzhenyue at 2023/04/28 start */
  unsigned int sc202cs_aac_read_otp(void)
  {
  	int ret = 0;
	kal_uint8 i;
/* N17 code for HQ-293329 by wuzhenyue at 2023/04/28 start */ 
	int otpdata_addr[] = {0x8008,0x8009, 0x800A,0x800B,0x8011, 0x8012, 0x8013, 0x8014,0x8015,0x8016, 0x8017, 0x8018, 0x8019,0x801A,0x801B,0x8000,0x8001,0x8002,0x8003,0x8004,0x8005,0x8006,0x8007};
/* N17 code for HQ-293329 by wuzhenyue at 2023/04/28 end */
    ret = sc202cs_aac_read_general_info(otpdata_addr, ARRAY_SIZE(otpdata_addr), otpData);
    /* N17 code for HQ-293329 by wuzhenyue at 2023/04/28 end */
  	if (ret != 0) {
  		LOG_INF("depth-aac read fuseid failed\n");
  		return -1;
  	}
     for(i = 0; i < ARRAY_SIZE(otpData); i++){
		LOG_INF("depth-aac read otp data 0x%x", otpData[i]);
	}
  	return ret;
  }

  unsigned int sc202cs_aac_read_otp_info(struct i2c_client *client,
  		 unsigned int addr, unsigned char *data, unsigned int size)
  {
  	int num = ARRAY_SIZE(otpData);
	int index = 0;
	LOG_INF(" depth-aac addr:0x%x ,size:%d",addr, size);
	for(;index<num ;index++){
   ...
   /* N17 code for HQ-293329 by wuzhenyue at 2023/04/27 start */
    EXPORT_SYMBOL(sc202cs_aac_read_otp_info);
   /* N17 code for HQ-293329 by wuzhenyue at 2023/04/27 end */
...
/* N17 code for HQ-293329 by wuzhenyue at 2023/05/22 start */
	...
	lens_id = read_cmos_sensor(0x800A);
	LOG_INF("depth-aac getimgsensorid!!!\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            LOG_INF("depth-aac return_sensor_id:0x%x , lens_id = 0x%x\n",*sensor_id,lens_id);
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("depth-aac i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                if(lens_id == Lens_ID){
                    ret = sc202cs_aac_read_otp();
                    return ERROR_NONE;
                }
                else{
                    LOG_INF("depth-aac read lens_id fail");
                    *sensor_id = 0xFFFFFFFF;
                    return ERROR_SENSOR_CONNECT_FAIL;
                }
            }
/* N17 code for HQ-293329 by wuzhenyue at 2023/05/22 end */
            LOG_INF("depth-aac Read sensor id fail, return_sensor_id: 0x%x\n",*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    //sensor_info->SensorStillCaptureFrameRate= imgsensor_info.cap.max_framerate/10; /* not use */0xCD
...
/* N17 code for HQ-293330 by chenxiaoyong at 2023/05/1 start */
    sensor_info->FrameTimeDelayFrame = imgsensor_info.frame_time_delay_frame;
/* N17 code for HQ-293330 by chenxiaoyong at 2023/05/1 end */
...
UINT32 SC202CS_AAC_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
    ...
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SC202CS_AAC_MIPI_RAW_SensorInit    */
