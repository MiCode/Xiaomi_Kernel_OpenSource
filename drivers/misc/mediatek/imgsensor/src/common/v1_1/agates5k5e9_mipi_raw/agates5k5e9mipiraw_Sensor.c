...
#define PFX "agates5k5e9_camera_sensor"
#define LOG_INF(format, args...) pr_debug(PFX "[%s] " format, __func__, ##args)
static DEFINE_SPINLOCK(imgsensor_drv_lock);

#include "agates5k5e9mipiraw_Sensor.h"
...
#define ENABLE_LONG_EXP 0

static bool bIsLongExposure = KAL_FALSE;
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = AGATES5K5E9_SENSOR_ID,	/*AGATES5K5E9_SENSOR_ID = 0x487B */
...
static void write_shutter(kal_uint32 shutter)
{
	...
	kal_uint32 fix_shutter = 0xFFFA;
	kal_uint32 lineLengthPixelClock = 0x0C28;
	kal_uint32 fineIntgTime = lineLengthPixelClock - 140;
	// In order to keep the framerate of hs video mode,
	// preventing to enlarge the frame length

	if (ENABLE_LONG_EXP && (shutter >= fix_shutter)) {  //1s at pclk=280mhz
		...
		shutter = (lineLengthPixelClock * shutter + fineIntgTime + 0xFF70) / 0xFFFC;
		if (shutter > 0xE282)
		{
			shutter = 0xE282;
		}
...
{
	table_write_cmos_sensor(agates5k5e9_init_setting,
		sizeof(agates5k5e9_init_setting) / sizeof(kal_uint16));
}				/*  agates5k5e9MIPI_Sensor_Init  */

static void preview_setting(void)
{
	LOG_INF("start \n");
	if (bIsLongExposure == KAL_FALSE) {
		table_write_cmos_sensor(agates5k5e9_preview_setting,
			sizeof(agates5k5e9_preview_setting) / sizeof(kal_uint16));
	} else {
		sensor_init();
		table_write_cmos_sensor(agates5k5e9_preview_setting,
			sizeof(agates5k5e9_preview_setting) / sizeof(kal_uint16));
...
}				/*  agates5k5e9MIPI_Capture_Setting  */

static void capture_setting(kal_uint16 currefps)
{
	LOG_INF("start \n");
	table_write_cmos_sensor(agates5k5e9_cap_setting,
			sizeof(agates5k5e9_cap_setting) / sizeof(kal_uint16));
}

static void normal_video_setting(kal_uint16 currefps)
{
	LOG_INF("E! currefps:%d\n", currefps);
	table_write_cmos_sensor(agates5k5e9_normal_video_setting,
		sizeof(agates5k5e9_normal_video_setting) / sizeof(kal_uint16));
}

static void hs_video_setting(void)
{
	LOG_INF("EVGA 120fps");
	table_write_cmos_sensor(agates5k5e9_hs_video_setting,
		sizeof(agates5k5e9_hs_video_setting) / sizeof(kal_uint16));
}

static void slim_video_setting(void)
{
	LOG_INF("E");
	table_write_cmos_sensor(agates5k5e9_slim_video_setting,
		sizeof(agates5k5e9_slim_video_setting) / sizeof(kal_uint16));
}
...
UINT32 AGATES5K5E9_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      agates5k5e9MIPI_RAW_SensorInit      */
