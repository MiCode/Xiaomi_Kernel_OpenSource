...
static kal_uint16 gc02m1b_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
			...
			kal_uint16 frame_length)
{
	...
	kal_uint16 cal_shutter = (shutter >> 2) << 2;
	kal_uint32 cal_frame_length = (imgsensor.frame_length >> 2) << 2;
	...
	if (imgsensor.autoflicker_en) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305)
			...
			// Extend frame length
			write_cmos_sensor(0xfe, 0x00);
			write_cmos_sensor(0x41, (cal_frame_length >> 8) & 0x1F);
	...
	printk("[GC02M1B]Add for N3D! shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length);
}
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < GC02M1B_SENSOR_GAIN_BASE)
		reg_gain = GC02M1B_SENSOR_GAIN_BASE;
	else if (reg_gain > GC02M1B_SENSOR_GAIN_MAX)
		reg_gain = GC02M1B_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xb6, GC02M1B_AGC_Param[gain_index][1]);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			LOG_INF("Read sensor id fail, write id: 0x%x, ret sensor_id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
