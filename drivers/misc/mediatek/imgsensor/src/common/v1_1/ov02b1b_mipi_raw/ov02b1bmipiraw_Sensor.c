...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02B1B_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	...
	.sensor_mode_num = 2,	  //support sensor mode num ,don't support Slow motion
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {//15fps
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			LOG_INF("Warning: current_fps  is not support, so use cap's setting: %d fps!\n",
...
{
    LOG_INF("OV02B1B_MIPI_RAW_SensorInit in\n");
