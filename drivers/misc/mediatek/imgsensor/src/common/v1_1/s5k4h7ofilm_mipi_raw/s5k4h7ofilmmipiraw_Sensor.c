...
#define PFX "s5k4h7ofilm_camera_sensor"
#define LOG_INF(format, args...) pr_debug(PFX "[%s] " format, __func__, ##args)
static DEFINE_SPINLOCK(imgsensor_drv_lock);

#include "s5k4h7ofilmmipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K4H7OFILM_SENSOR_ID,	/*S5K4H7OFILM_SENSOR_ID = 0x487B */
			...
			kal_uint16 frame_length)
{
	...
	write_cmos_sensor_8(0x0202, (shutter >> 8) & 0xFF);
	write_cmos_sensor_8(0x0203, shutter & 0xFF);
	printk("[S5K4H7I]Add for N3D! shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length);
}
...
{
	table_write_cmos_sensor(s5k4h7ofilm_init_setting,
		sizeof(s5k4h7ofilm_init_setting) / sizeof(kal_uint16));
}				/*  s5k4h7ofilmMIPI_Sensor_Init  */

static void preview_setting(void)
{
	LOG_INF("start \n");
	if (bIsLongExposure == KAL_FALSE) {
		table_write_cmos_sensor(s5k4h7ofilm_preview_setting,
			sizeof(s5k4h7ofilm_preview_setting) / sizeof(kal_uint16));
	} else {
		sensor_init();
		table_write_cmos_sensor(s5k4h7ofilm_preview_setting,
			sizeof(s5k4h7ofilm_preview_setting) / sizeof(kal_uint16));
...
}				/*  s5k4h7ofilmMIPI_Capture_Setting  */

static void capture_setting(kal_uint16 currefps)
{
	LOG_INF("start \n");
	table_write_cmos_sensor(s5k4h7ofilm_cap_setting,
			sizeof(s5k4h7ofilm_cap_setting) / sizeof(kal_uint16));
}

static void normal_video_setting(kal_uint16 currefps)
{
	LOG_INF("E! currefps:%d\n", currefps);
	table_write_cmos_sensor(s5k4h7ofilm_normal_video_setting,
		sizeof(s5k4h7ofilm_normal_video_setting) / sizeof(kal_uint16));
}

static void hs_video_setting(void)
{
	LOG_INF("EVGA 120fps");
	table_write_cmos_sensor(s5k4h7ofilm_hs_video_setting,
		sizeof(s5k4h7ofilm_hs_video_setting) / sizeof(kal_uint16));
}

static void slim_video_setting(void)
{
	LOG_INF("E");
	table_write_cmos_sensor(s5k4h7ofilm_slim_video_setting,
		sizeof(s5k4h7ofilm_slim_video_setting) / sizeof(kal_uint16));
}
...
UINT32 S5K4H7OFILM_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      s5k4h7ofilm_MIPI_RAW_SensorInit      */
