...
#include "s5khm2sd_main_sunny_mipi_raw.h"
...
#define LOG_TAG "s5khm2sd_sunny_"
#define S5KHM2SD_SUNNY_LOG_INF(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
#define S5KHM2SD_SUNNY_LOG_DBG(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KHM2SD_MAIN_SUNNY_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[11] = {
	...
	{ 12000, 9000,    0,     0,  12000, 9000,  4000, 3000,    0,    0,  4000, 3000,    0,    0,  4000, 3000}, // custom6 as 12M @ 24fps
};
...
static void set_shutter(kal_uint32 shutter)
{
	...
	imgsensor.current_ae_effective_frame = 2;
	S5KHM2SD_SUNNY_LOG_DBG("shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length);
}
#endif
...
#if MULTI_WRITE
#if 0 /*for debug*/
	S5KHM2SD_SUNNY_LOG_DBG("[s5khm2sd_sunny_-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d start", len);
	for (i = 0; i < len/2; i++)
		S5KHM2SD_SUNNY_LOG_DBG("[s5khm2sd_sunny_-debug]readback addr 0x%04x, 0x%02x,\n",
			para[2*i], read_cmos_sensor(para[2*i]) & 0xFF);
	S5KHM2SD_SUNNY_LOG_DBG("[s5khm2sd_sunny_-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d end", len);
#endif
#endif
	return 0;
}
...
static void sensor_init(void)
{
	...
	// Global
	table_write_cmos_sensor(s5khm2sd_sunny__init_setting,
			sizeof(s5khm2sd_sunny__init_setting) / sizeof(kal_uint16));
}	/*	sensor_init  */

static void preview_setting(void)
{
	S5KHM2SD_SUNNY_LOG_DBG("write sensor previews setting\n");
	table_write_cmos_sensor(s5khm2sd_sunny__preview_setting,
		sizeof(s5khm2sd_sunny__preview_setting) / sizeof(kal_uint16));
}	/*	preview_setting  */

static void capture_setting(kal_uint16 currefps)
{
	S5KHM2SD_SUNNY_LOG_DBG("capture setting E\n");
...
static void normal_video_setting(void)
{
	...
	table_write_cmos_sensor(s5khm2sd_sunny__normal_video_setting,
		sizeof(s5khm2sd_sunny__normal_video_setting) / sizeof(kal_uint16));
}
...
static void hs_video_setting(void)
{
	...
	table_write_cmos_sensor(s5khm2sd_sunny__hs_video_setting,
		sizeof(s5khm2sd_sunny__hs_video_setting) / sizeof(kal_uint16));
}
...
{
	S5KHM2SD_SUNNY_LOG_DBG("E\n");
	table_write_cmos_sensor(s5khm2sd_sunny__custom1_setting,
		sizeof(s5khm2sd_sunny__custom1_setting) / sizeof(kal_uint16));
}

static void custom2_setting(void)
{
	S5KHM2SD_SUNNY_LOG_DBG(" 120fps E\n");

	table_write_cmos_sensor(s5khm2sd_sunny__custom2_setting,
			sizeof(s5khm2sd_sunny__custom2_setting) / sizeof(kal_uint16));
}

static void custom3_setting(void)
{
	S5KHM2SD_SUNNY_LOG_DBG(" 60fps E\n");

	table_write_cmos_sensor(s5khm2sd_sunny__custom3_setting,
			sizeof(s5khm2sd_sunny__custom3_setting) / sizeof(kal_uint16));
}

static void custom4_setting(void)
{
	S5KHM2SD_SUNNY_LOG_DBG(" full size capture E\n");
	table_write_cmos_sensor(s5khm2sd_sunny__custom4_setting,
			sizeof(s5khm2sd_sunny__custom4_setting) / sizeof(kal_uint16));
}
...
static void custom5_setting(void)
{
	...
	table_write_cmos_sensor(s5khm2sd_sunny__custom5_setting,
			sizeof(s5khm2sd_sunny__custom5_setting) / sizeof(kal_uint16));
}
...
static void custom6_setting(void)
{
	S5KHM2SD_SUNNY_LOG_DBG("E\n");
	//custom3_setting();
	table_write_cmos_sensor(s5khm2sd_sunny__custom6_setting,
			sizeof(s5khm2sd_sunny__custom6_setting) / sizeof(kal_uint16));
}
...
static kal_uint32 custom6(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2SD_SUNNY_LOG_DBG("loading custom6_setting\n");
		...
		UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 2670000;
		break;
	...
	case SENSOR_FEATURE_GET_TEMPERATURE_VALUE:
		*feature_return_para_32 = 0;//get_sensor_temperature();
		*feature_para_len = 4;
		break;
...
UINT32 S5KHM2SD_MAIN_SUNNY_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
