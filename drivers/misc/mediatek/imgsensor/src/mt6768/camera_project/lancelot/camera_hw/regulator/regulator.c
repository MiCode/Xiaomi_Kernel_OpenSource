...
enum IMGSENSOR_RETURN imgsensor_oc_interrupt(enum IMGSENSOR_SENSOR_IDX
					     sensor_idx, bool enable)
{
	...
	if (enable) {
		...
		for (i = 0; i < REGULATOR_TYPE_MAX_NUM; i++) {
			...
			if (preg && regulator_is_enabled(preg)) {
				pmic_enable_interrupt(int_oc_type[i], 1,
						      OC_MODULE);
				regulator_put(preg);
		...
		for (i = 0; i < REGULATOR_TYPE_MAX_NUM; i++) {
			...
			preg = regulator_get(pdevice, str_regulator_name);
			if (preg) {
				pmic_enable_interrupt(int_oc_type[i], 0,
						      OC_MODULE);
				regulator_put(preg);
				pr_debug("[regulator] %s idx=%d %s enable=%d\n",
					 __func__,
	...
	for (j = IMGSENSOR_SENSOR_IDX_MIN_NUM;
		...
		for (i = 0; i < REGULATOR_TYPE_MAX_NUM; i++) {
			snprintf(str_regulator_name,
				 sizeof(str_regulator_name),
				 "cam%d_%s",
				 j, regulator_control[i].pregulator_type);
			preg->pregulator[j][i] =
...
static enum IMGSENSOR_RETURN regulator_release(void *pinstance)
{
	...
	for (idx = IMGSENSOR_SENSOR_IDX_MIN_NUM;
	     idx < IMGSENSOR_SENSOR_IDX_MAX_NUM; idx++) {
...
static enum IMGSENSOR_RETURN regulator_set(void *pinstance,
					   ...
					   pin_state)
{
	...
	pregulator =
	    preg->pregulator[sensor_idx][reg_type_offset + pin -
					 IMGSENSOR_HW_PIN_AVDD];

	enable_cnt =
	    &preg->enable_cnt[sensor_idx][reg_type_offset + pin -
					  IMGSENSOR_HW_PIN_AVDD];
...
enum IMGSENSOR_RETURN imgsensor_hw_regulator_open(struct IMGSENSOR_HW_DEVICE
						  **pdevice)
{
	*pdevice = &device;
	return IMGSENSOR_RETURN_SUCCESS;
