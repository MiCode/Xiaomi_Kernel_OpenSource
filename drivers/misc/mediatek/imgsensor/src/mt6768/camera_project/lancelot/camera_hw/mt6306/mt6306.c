...
static enum IMGSENSOR_RETURN mt6306_set(void *pinstance,
					...
					enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	if (pin == IMGSENSOR_HW_PIN_AVDD ||
		...
		pin_offset = (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN)
		    ? MT6306_PIN_CAM_PDN0 :
...
enum IMGSENSOR_RETURN imgsensor_hw_mt6306_open(struct IMGSENSOR_HW_DEVICE
					       **pdevice)
{
	*pdevice = &device;
	return IMGSENSOR_RETURN_SUCCESS;
