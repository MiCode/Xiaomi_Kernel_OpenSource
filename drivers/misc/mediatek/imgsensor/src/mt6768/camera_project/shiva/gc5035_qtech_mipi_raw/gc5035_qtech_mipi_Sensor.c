			...
			if (0x06 == vendor_id) {
				*sensor_id = return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
					gc5035_otp_identify();
					pr_debug("[gc5035_qtech_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x, vendor_id: 0x%x\n",
						imgsensor.i2c_write_id, *sensor_id, vendor_id);
					return ERROR_NONE;
				}
				pr_debug("[gc5035_qtech_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x, vendor_id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id, vendor_id);
			}
			retry--;
		} while (retry > 0);
