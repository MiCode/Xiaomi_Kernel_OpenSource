...
#include "ov8856_qtech_front_mipiraw_Sensor.h"
#include <linux/slab.h>

#define PFX "OV8856"

#define OV8856OTP
#define OV8856_OTP_INFO_SIZE    600
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV8856_QTECH_FRONT_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static void sensor_init(void)
{
   ...
   */
	...
	write_cmos_sensor(0x0300, 0x04);//add for 700Mbps mipi
	write_cmos_sensor(0x0302, 0xaf);
		...
		do {
				*sensor_id = return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
					LOG_INF("[ov8856_qtech_front]get sensor id:imgsensor.i2c_write_id: 0x%x, *sensor_id=0x%x\n",
		imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;

				}
	LOG_INF("[ov8856_qtech_front]Read sensor id fail,imgsensor.i2c_write_id: 0x%x, *sensor_id=0x%x\n",
		imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
int otp_ov8856[OV8856_OTP_INFO_SIZE];
...
struct otp_struct *ov8856_otp_ptr;

#define RG_Ratio_Typical   0x24C
#define BG_Ratio_Typical   0x2A6

unsigned int ov8856_qtech_front_read_otp_info(struct i2c_client *client, unsigned int addr, unsigned char *data, unsigned int size)
{
	int ii = 0;
	printk("ov8856_qtech_front_read_otp_info: param: addr=%d, data=%p, size=%d \n", addr, data, size);
	for (ii = 0; ii < size; ii++) {
		if (OV8856_OTP_INFO_SIZE <= (addr + ii)) {
			return ii;
		}
		data[ii] = otp_ov8856[addr + ii];
	}
	return size;
}
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	addr = 0;
	printk("ov8856_qtech_mipiraw_sensor: check opt awb goup1 flag=0x%x\n", otp_flag);
	if (otp_flag == 0x55) {
		addr = 0x7011;
		AWB_Group = 1;
		printk("ov8856_qtech_mipiraw_sensor: check opt awb use goup1 \n");
	} else {
		if (otp_flag == 0x00) {
			printk("ov8856_qtech_mipiraw_sensor: check opt awb use group1 is empty\n");
		} else if (otp_flag == 0xff) {
			printk("ov8856_qtech_mipiraw_sensor: check opt awb use group1 is Invalid\n");
		}
		otp_flag = 0;
		otp_flag = read_cmos_sensor(0x701D);
		printk("ov8856_qtech_mipiraw_sensor: check opt awb goup2 flag=0x%x\n", otp_flag);
		if (otp_flag == 0x55) {
			addr = 0x701E;
			AWB_Group = 2;
			printk("ov8856_qtech_mipiraw_sensor: check opt awb use goup2 \n");
		} else if (otp_flag == 0x00) {
			printk("ov8856_qtech_mipiraw_sensor: check opt awb use group2 is empty\n");
		} else if (otp_flag == 0xff) {
			printk("ov8856_qtech_mipiraw_sensor: check opt awb use group2 is Invalid\n");
		}
	}
	...
	if (addr != 0) {
		...
		(*otp_ptr).lens_id = read_cmos_sensor(addr + 4);
		(*otp_ptr).production_year = read_cmos_sensor(addr + 1);
		(*otp_ptr).production_month = read_cmos_sensor(addr + 2);
		(*otp_ptr).production_day = read_cmos_sensor(addr + 3);
		temp = read_cmos_sensor(addr + 10);
		(*otp_ptr).rg_ratio = (read_cmos_sensor(addr + 8)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (read_cmos_sensor(addr + 9)<<2) + ((temp>>4) & 0x03);
	...
	printk("ov8856_qtech_mipiraw_sensor: check opt lsc goup1 flag=0x%x\n", otp_flag);
	addr = 0;
	if (otp_flag == 0x55) {
		addr = 0x702B;
		LSC_Group = 1;
		printk("ov8856_qtech_mipiraw_sensor: check opt lsc use goup1 \n");
	} else {
		if (otp_flag == 0x00) {
			printk("ov8856_qtech_mipiraw_sensor: check opt lsc use group1 is empty\n");
		} else if (otp_flag == 0xff) {
			printk("ov8856_qtech_mipiraw_sensor: check opt lsc use group1 is Invalid\n");
		}
		otp_flag = read_cmos_sensor(0x711c);
		if (otp_flag == 0x55) {
			addr = 0x711D;
			LSC_Group = 2;
			printk("ov8856_qtech_mipiraw_sensor: check opt lsc use goup2 \n");
		} else if (otp_flag == 0x00) {
			printk("ov8856_qtech_mipiraw_sensor: check opt lsc use group2 is empty\n");
		} else if (otp_flag == 0xff) {
			printk("ov8856_qtech_mipiraw_sensor: check opt lsc use group2 is Invalid\n");
		}
	}
	...
	for (i = 0x7000; i <= 0x700f; i++) {
		otp_ov8856[j] = read_cmos_sensor(i);
		j++;
	}

	/*second save awb otp value */
	if (AWB_Group == 1) {
		temp_start_addr = 0x7010;
		temp_end_addr = 0x701C;
	} else if (AWB_Group == 2) {
		temp_start_addr = 0x701D;
		temp_end_addr = 0x7029;
	}
	for (i = temp_start_addr; i <= temp_end_addr; i++) {
		otp_ov8856[j] = read_cmos_sensor(i);
		j++;
	}

	/*third save lsc otp value*/
	if (LSC_Group == 1) {
		temp_start_addr = 0x702A;
		temp_end_addr = 0x711B;
	} else if (LSC_Group == 2) {
		temp_start_addr = 0x711C;
		temp_end_addr = 0x720D;
	}
	for (i = temp_start_addr; i <= temp_end_addr; i++) {
		otp_ov8856[j] = read_cmos_sensor(i);
		j++;
...
static kal_uint32 open(void)
{
	...
	mdelay(10);
		...
		if (NULL == ov8856_otp_ptr) {
			ov8856_otp_ptr = (struct otp_struct *)kzalloc(sizeof(struct otp_struct), GFP_KERNEL);
			if (ov8856_otp_ptr) {
...
UINT32 OV8856_QTECH_FRONT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
