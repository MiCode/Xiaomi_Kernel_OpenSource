#define PFX "ZIRCONIMX355SUNNY_camera_sensor"
...
#include "zirconimx355sunny_ana_gain_table.h"
#include "zirconimx355sunnymipiraw_Sensor.h"
...
#define zirconimx355sunny_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
#define zirconimx355sunny_burst_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8_burst(__VA_ARGS__)
#define LOG_TAG "[zirconimx355sunny]"
#define IMX355_LOG_INF(format, args...) pr_debug(LOG_TAG "[%s] " format, __func__, ##args)
#define IMX355_LOG_DBG(format, args...) pr_debug(LOG_TAG "[%s] " format, __func__, ##args)
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = ZIRCONIMX355SUNNY_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[] = {
	...
	{3280, 2464,   8,    8,    3264, 2448, 3264, 2448, 0,   0,   3264, 2448,  0,  0, 3264, 2448}, /* hs_video */
	...
	{3280, 2464,   8,    8,    3264, 2448, 3264, 2448, 0,   0,   3264, 2448,  0,  0, 3264, 2448}, /* custom4 */
	{3280, 2464,   8,    8,    3264, 2448, 3264, 2448, 0,   0,   3264, 2448,  0,  0, 3264, 2448}, /* custom5 */
	{3280, 2464,   8,    8,    3264, 2448, 3264, 2448, 0,   0,   3264, 2448,  0,  0, 3264, 2448}, /* custom6 */
	{3280, 2464,   8,    8,    3264, 2448, 3264, 2448, 0,   0,   3264, 2448,  0,  0, 3264, 2448}, /* custom7 */
	{3280, 2464,   8,    8,    3264, 2448, 3264, 2448, 0,   0,   3264, 2448,  0,  0, 3264, 2448}, /* custom8 */
	{3280, 2464,   8,    8,    3264, 2448, 3264, 2448, 0,   0,   3264, 2448,  0,  0, 3264, 2448}, /* custom9 */
	{3280, 2464,   8,    8,    3264, 2448, 3264, 2448, 0,   0,   3264, 2448,  0,  0, 3264, 2448}, /* custom10 */
};
...
static void zirconimx355sunny_get_pdaf_reg_setting(struct subdrv_ctx *ctx, MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void zirconimx355sunny_set_pdaf_reg_setting(struct subdrv_ctx *ctx,
...
static void zirconimx355sunny_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
static kal_uint32 streaming_control(struct subdrv_ctx *ctx, kal_bool enable)
{
	IMX355_LOG_INF("streaming_enable(0=Sw Standby,1=streaming): %d\n",
		enable);
	if (enable) {
		kal_uint8 sensorVersion = 0;
		sensorVersion = read_cmos_sensor_8(ctx, 0x0018);
		IMX355_LOG_INF("sensor version[cut0.9-0x00,cut1.0-0x01,MP-1xh]:%x", sensorVersion);
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	zirconimx355sunny_table_write_cmos_sensor(ctx, zirconimx355sunny_init_setting,
		sizeof(zirconimx355sunny_init_setting)/sizeof(kal_uint16));
...
{
	IMX355_LOG_INF("%s +\n", __func__);
	zirconimx355sunny_table_write_cmos_sensor(ctx, zirconimx355sunny_preview_setting,
		sizeof(zirconimx355sunny_preview_setting)/sizeof(kal_uint16));
...
static void normal_video_setting(struct subdrv_ctx *ctx, kal_uint16 currefps)
{
	...
	zirconimx355sunny_table_write_cmos_sensor(ctx, zirconimx355sunny_normal_video_setting,
		sizeof(zirconimx355sunny_normal_video_setting)/sizeof(kal_uint16));
...
{
	IMX355_LOG_INF("%s +\n", __func__);
	zirconimx355sunny_table_write_cmos_sensor(ctx, zirconimx355sunny_custom2_setting,
		sizeof(zirconimx355sunny_custom2_setting)/sizeof(kal_uint16));
	IMX355_LOG_INF("%s -\n", __func__);
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	sensor_id = ((read_cmos_sensor_8(ctx, 0x0016) << 8) | read_cmos_sensor_8(ctx, 0x0017));

	IMX355_LOG_INF("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(zirconimx355sunny_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)zirconimx355sunny_ana_gain_table,
			sizeof(zirconimx355sunny_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		IMX355_LOG_INF("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		zirconimx355sunny_get_pdaf_reg_setting(ctx,
	...
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		IMX355_LOG_INF("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		zirconimx355sunny_set_pdaf_reg_setting(ctx,
				(*feature_para_len) / sizeof(UINT32),
				feature_data_16);
		break;
...
break;
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		zirconimx355sunny_set_lsc_reg_setting(ctx, index, feature_data_16,
...
const struct subdrv_entry zirconimx355sunny_mipi_raw_entry = {
	.name       = "zirconimx355sunny_mipi_raw",
	.id         = ZIRCONIMX355SUNNY_SENSOR_ID,
	.pw_seq     = pw_seq,
