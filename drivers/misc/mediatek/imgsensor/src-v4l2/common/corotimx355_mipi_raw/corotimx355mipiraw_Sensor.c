#define PFX "COROTIMX355_camera_sensor"
...
#include "corotimx355_ana_gain_table.h"
#include "corotimx355mipiraw_Sensor.h"
...
#define corotimx355_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
#define corotimx355_burst_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8_burst(__VA_ARGS__)
#define LOG_TAG "[corotimx355]"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = COROTIMX355_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[] = {
	...
	{3280, 2464,   0,  312,    3280, 1840, 3280, 1840,  0,  0,   1640, 920,   0,  0, 1640, 920 }, /* custom1 */
	{3280, 2464, 200,  152,    2880, 2160, 2880, 2160, 0,   0,   2880, 2160,  0,  0, 2880, 2160}, /* custom2 */
	{3280, 2464,   0,    0,    3280, 2464, 3280, 2464, 0,   0,   3280, 2464,  0,  0, 3280, 2464}, /* custom3 */
	{3280, 2464,   0,    0,    3280, 2464, 3280, 2464, 0,   0,   3280, 2464,  0,  0, 3280, 2464}, /* custom4 */
...
static void corotimx355_get_pdaf_reg_setting(struct subdrv_ctx *ctx, MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void corotimx355_set_pdaf_reg_setting(struct subdrv_ctx *ctx,
...
static void disable_sensor_bpc(struct subdrv_ctx *ctx)
{
	kal_uint8 itemp;
	itemp = read_cmos_sensor_8(ctx, 0x0B06);
	IMX355_LOG_INF("0x0B06 default value = %x\n", itemp);
	write_cmos_sensor_8(ctx, 0x0B06, itemp&0xFE);
}
...
{
	u64 frame_time = frame_length * linetime_in_ns;
	u64 frame_time_250ms = 250000000; // ns
	...
	if (shutter_cnt == 1) {
		...
		if (frame_time <  frame_time_250ms && frame_length > ctx->frame_length)
			ctx->frame_length = frame_length;
		...
		IMX355_LOG_DBG("frame_time =%lldns, shutter =%d, framelength =%d\n",
			frame_time, shutters[0], ctx->frame_length);
	}
}
...
static void corotimx355_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	corotimx355_table_write_cmos_sensor(ctx, corotimx355_init_setting,
		sizeof(corotimx355_init_setting)/sizeof(kal_uint16));
...
{
	IMX355_LOG_INF("%s +\n", __func__);
	corotimx355_table_write_cmos_sensor(ctx, corotimx355_preview_setting,
		sizeof(corotimx355_preview_setting)/sizeof(kal_uint16));
...
static void normal_video_setting(struct subdrv_ctx *ctx, kal_uint16 currefps)
{
	...
	corotimx355_table_write_cmos_sensor(ctx, corotimx355_normal_video_setting,
		sizeof(corotimx355_normal_video_setting)/sizeof(kal_uint16));
...
{
	IMX355_LOG_INF("%s +\n", __func__);
	corotimx355_table_write_cmos_sensor(ctx, corotimx355_custom1_setting,
		sizeof(corotimx355_custom1_setting)/sizeof(kal_uint16));
	IMX355_LOG_INF("%s -\n", __func__);
...
{
	IMX355_LOG_INF("%s +\n", __func__);
	corotimx355_table_write_cmos_sensor(ctx, corotimx355_custom2_setting,
		sizeof(corotimx355_custom2_setting)/sizeof(kal_uint16));
	IMX355_LOG_INF("%s -\n", __func__);
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	linetime_in_ns = (u64)ctx->line_length * 1000000 +
			(ctx->pclk / 1000 - 1);
	do_div(linetime_in_ns, ctx->pclk / 1000);
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(corotimx355_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)corotimx355_ana_gain_table,
			sizeof(corotimx355_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		IMX355_LOG_INF("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		corotimx355_get_pdaf_reg_setting(ctx,
	...
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		IMX355_LOG_INF("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		corotimx355_set_pdaf_reg_setting(ctx,
				(*feature_para_len) / sizeof(UINT32),
				feature_data_16);
		break;
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		 *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 281500;
		 break;
...
break;
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		corotimx355_set_lsc_reg_setting(ctx, index, feature_data_16,
	...
	struct mtk_csi_param *csi_param)
{
	...
	IMX355_LOG_INF("csi_param->dphy_trail:%d,\n", csi_param->dphy_trail);
	return 0;
}
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	{HW_ID_AVDD1,  2700000, 2},
	{HW_ID_MCLK,  24,      0},
	{HW_ID_MCLK_DRIVING_CURRENT, 4, 2},
	{HW_ID_RST,   1,       1},
};

const struct subdrv_entry corotimx355_mipi_raw_entry = {
	.name       = "corotimx355_mipi_raw",
	.id         = COROTIMX355_SENSOR_ID,
	.pw_seq     = pw_seq,
