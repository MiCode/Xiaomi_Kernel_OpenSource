#define PFX "COROTIMX800_camera_sensor"
...
#include "corotimx800_ana_gain_table.h"
#include "corotimx800mipiraw_Sensor.h"
...
#define corotimx800_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
#define corotimx800_burst_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8_burst(__VA_ARGS__)
#define LOG_TAG "[corotimx800]"
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.min_gain_iso     = 50,
	.min_dig_gain     = 1  * BASEGAIN, //1024*1x
	.max_dig_gain     = 16 * BASEGAIN, //16x
	.dig_gain_step    = 4,
	.exp_step         = 2,
	.gain_step        = 1,
	.gain_type        = 0,
...
#if ENABLE_PDAF
static struct SENSOR_VC_INFO2_STRUCT SENSOR_VC_INFO[] = {
	{/* preview/capture/slim_video/custom1/custom6 */
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 4096, 3072},
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 4096, 768},
		},
		1
	},
	{/* normal video /custom7*/
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
		{
			...
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 4096, 576},
		},
		1
	},
	{/* custom3 */
	...
	{/* hs_video/custom2 */
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 2048, 1152},
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 2048, 288},
		},
		1
	},
	{/* cust4,fullsize */
	...
	{/* cust5, in sensor zoom */
		...
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 4096, 3072},
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 2048, 1536},
		},
		1
	},
	{/* cust8, 2x bokeh */
	...
	{/* cust9 */
		...
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 4096, 2304},
			{VC_STAGGER_ME, 0x00, 0x30, 4096, 2304},
			{VC_PDAF_STATS_NE_PIX_1, 0x01, 0x2b, 4096, 576},
			{VC_PDAF_STATS_ME_PIX_1, 0x01, 0x30, 4096, 576},
		},
		1
	},
	{/* cust10, 8k 30fps */
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 7680, 4320},
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 3840, 2160},
		},
		1
	},
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info[] = {
	{
		...
		.i4Crop = {
			...
			{0, 0}, {0, 384}, {1024, 768}, {0, 384}, {256,912},{0, 0}
		},
...
static void corotimx800_get_pdaf_reg_setting(struct subdrv_ctx *ctx, MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void corotimx800_set_pdaf_reg_setting(struct subdrv_ctx *ctx,
...
static kal_uint16 corotimx800_QSC_setting[EEPROM_QSC_SIZE * 2];
static kal_uint16 corotimx800_LRC_setting[432 * 2];
...
static void write_sensor_QSC(struct subdrv_ctx *ctx)
{
	...
	corotimx800_burst_write_cmos_sensor(ctx, corotimx800_QSC_setting,
		sizeof(corotimx800_QSC_setting) / sizeof(kal_uint16));
...
static void write_sensor_LRC(struct subdrv_ctx *ctx)
{
	...
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_LRC_setting,
		sizeof(corotimx800_LRC_setting) / sizeof(kal_uint16));
...
static void read_sensor_Cali(struct subdrv_ctx *ctx)
{
	...
	for (idx = 0; idx < EEPROM_QSC_SIZE; idx++) {
		corotimx800_QSC_setting[2 * idx]     = sensor_qsc + idx;
		corotimx800_QSC_setting[2 * idx + 1] = read_cmos_eeprom_8(ctx, eeprom_qsc + idx);
	}

	for (idx = 0; idx < 216; idx++) {
		corotimx800_LRC_setting[2 * idx]               = sensor_lrc_0 + idx;
		corotimx800_LRC_setting[2 * idx + 1]           = read_cmos_eeprom_8(ctx, eeprom_lrc_0 + idx);
		corotimx800_LRC_setting[2 * idx + 192 * 2]     = sensor_lrc_1 + idx;
		corotimx800_LRC_setting[2 * idx + 1 + 192 * 2] = read_cmos_eeprom_8(ctx, eeprom_lrc_1 + idx);
	}
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter, bool hold_en)
{
	...
	if (ctx->fast_mode_on) {
		IMX800_LOG_INF("skip : fast_mode_on(%d)", ctx->fast_mode_on);
		return;
	}
				...
				kal_uint32 frame_length)
{
	...
	if (shutter_cnt == 1) {
		...
		IMX800_LOG_INF("frame_time =%lldns, shutter =%d, framelength =%d\n",
...
static kal_uint16 again2reg(struct subdrv_ctx *ctx, const kal_uint32 gain)
{
...
static u16 dgain2reg(struct subdrv_ctx *ctx, u32 dgain)
{
	...
	u8 dec = (u8) ((dgain % BASE_DGAIN) >> 2); // decimal parts
	u16 ret = ((u16)integ << 8) | dec;

	IMX800_LOG_INF("dgain(%d) dgain_reg = 0x%x\n", dgain, ret);
...
{
	kal_uint16 ana_reg_gain;
	kal_uint32 cur_ana_gain;
	kal_uint32 max_ana_gain;
	kal_uint32 min_ana_gain;

	kal_uint16 dig_reg_gain;
	kal_uint32 cur_dig_gain;
	kal_uint32 max_dig_gain;
	kal_uint32 min_dig_gain;

	max_ana_gain = imgsensor_info.max_gain;//setuphere for mode use
	min_ana_gain = imgsensor_info.min_gain;//setuphere for mode use

	max_dig_gain = imgsensor_info.max_dig_gain;//setuphere for mode use
	min_dig_gain = imgsensor_info.min_dig_gain;//setuphere for mode use

	if (ctx->sensor_mode == IMGSENSOR_MODE_CUSTOM4 ||//16x for full mode
			ctx->sensor_mode == IMGSENSOR_MODE_CUSTOM5) {
		/* 8K6K */
		max_ana_gain = 16 * BASEGAIN;
	}

	gain = max(gain, min_ana_gain);

	if (gain > max_ana_gain) {
		cur_ana_gain = max_ana_gain;
		cur_dig_gain = gain * BASEGAIN / max_ana_gain;
		if (cur_dig_gain > max_dig_gain) {
			cur_dig_gain = max_dig_gain;
		} else if (cur_dig_gain < min_dig_gain) {
			cur_dig_gain = min_dig_gain;
		}
	} else {
		cur_ana_gain = gain;
		cur_dig_gain = min_dig_gain;
	...
	ana_reg_gain = again2reg(ctx, cur_ana_gain);
	ctx->gain = ana_reg_gain;
	IMX800_LOG_INF("cur_ana_gain = %d, ana_reg_gain = 0x%x, max_ana_gain:%d\n ",
		cur_ana_gain, ana_reg_gain, max_ana_gain);

	dig_reg_gain = dgain2reg(ctx, cur_dig_gain);
	IMX800_LOG_INF("cur_dig_gain = %d, dig_reg_gain = 0x%x, max_dig_gain:%d\n ",
		cur_dig_gain, dig_reg_gain, max_dig_gain);

	write_cmos_sensor_8(ctx, 0x0204, (ana_reg_gain>>8) & 0xFF);
	write_cmos_sensor_8(ctx, 0x0205, ana_reg_gain & 0xFF);
	write_cmos_sensor_8(ctx, 0x020E, (dig_reg_gain>>8) & 0xFF);
	write_cmos_sensor_8(ctx, 0x020F, dig_reg_gain & 0xFF);
...
static void set_multi_dig_gain(struct subdrv_ctx *ctx, u32 *gains, u16 exp_cnt)
{
	u32 max_dig_gain = imgsensor_info.max_dig_gain;
	u32 min_dig_gain = imgsensor_info.min_dig_gain;
	int i = 0;
	u16 reg_dig_lg = 0;
	u16 reg_dig_mg = 0;
	u16 reg_dig_sg = 0;
	...
	for (i = 0; i < exp_cnt; i++) {
		...
		gains[i] = max(gains[i], min_dig_gain);
		gains[i] = min(gains[i], max_dig_gain);
	}
	...
	switch (exp_cnt) {
	case 1:
		reg_dig_lg = dgain2reg(ctx, gains[0]);
		break;
	case 2:
		reg_dig_lg = dgain2reg(ctx, gains[0]);
		reg_dig_sg = dgain2reg(ctx, gains[1]);
		break;
	case 3:
		reg_dig_lg = dgain2reg(ctx, gains[0]);
		reg_dig_mg = dgain2reg(ctx, gains[1]);
		reg_dig_sg = dgain2reg(ctx, gains[2]);
		break;
	default:
		break;
	}
	...
	if (reg_dig_lg) {
		write_cmos_sensor_8(ctx, 0x020E, (reg_dig_lg>>8) & 0xFF);
		write_cmos_sensor_8(ctx, 0x020F, reg_dig_lg & 0xFF);
	}

	if (reg_dig_mg) {
		write_cmos_sensor_8(ctx, 0x3166, (reg_dig_mg>>8) & 0xFF);
		write_cmos_sensor_8(ctx, 0x3167, reg_dig_mg & 0xFF);
	}

	if (reg_dig_sg) {
		write_cmos_sensor_8(ctx, 0x0218, (reg_dig_sg>>8) & 0xFF);
		write_cmos_sensor_8(ctx, 0x0219, reg_dig_sg & 0xFF);
	}
	...
	IMX800_LOG_INF("ae_ctrl_gph_en(%d) dgain reg[lg/mg/sg]: 0x%x 0x%x 0x%x\n",
		ctx->ae_ctrl_gph_en, reg_dig_lg, reg_dig_mg, reg_dig_sg);
}
		...
		kal_uint32 lg, kal_uint32 mg, kal_uint32 sg, kal_bool gph)
{
	...
	if (ctx->fast_mode_on) {
		IMX800_LOG_DBG("skip : fast_mode_on(%d)", ctx->fast_mode_on);
		return;
	}

	reg_lg = again2reg(ctx, lg);
	reg_mg = mg ? again2reg(ctx, mg) : 0;
	reg_sg = again2reg(ctx, sg);
...
static kal_uint16 corotimx800_feedback_awbgain[] = {
	0x0B8E, 0x00,
	0x0B8F, 0x00,
	0x0B90, 0x00,
	0x0B91, 0x00,
...
static kal_uint32 corotimx800_awb_gain(struct subdrv_ctx *ctx,
		struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	...
	corotimx800_feedback_awbgain[1]  = (gr_gain_int & 0xFF00) >> 8;
	corotimx800_feedback_awbgain[3]  = gr_gain_int & 0xFF;
	corotimx800_feedback_awbgain[5]  = (r_gain_int & 0xFF00) >> 8;
	corotimx800_feedback_awbgain[7]  = r_gain_int & 0xFF;
	corotimx800_feedback_awbgain[9]  = (b_gain_int & 0xFF00) >> 8;
	corotimx800_feedback_awbgain[11] = b_gain_int & 0xFF;
	corotimx800_feedback_awbgain[13] = (gb_gain_int & 0xFF00) >> 8;
	corotimx800_feedback_awbgain[15] = gb_gain_int & 0xFF;

	corotimx800_table_write_cmos_sensor(ctx, corotimx800_feedback_awbgain,
		sizeof(corotimx800_feedback_awbgain)/sizeof(kal_uint16));

	return ERROR_NONE;
}

static void corotimx800_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_init_setting,
		sizeof(corotimx800_init_setting)/sizeof(kal_uint16));

	corotimx800_table_write_cmos_sensor(ctx, corotimx800_Image_quality_setting,
                sizeof(corotimx800_Image_quality_setting)/sizeof(kal_uint16));
...
{
	IMX800_LOG_INF("%s +\n", __func__);
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_preview_setting,
		sizeof(corotimx800_preview_setting)/sizeof(kal_uint16));
...
static void normal_video_setting(struct subdrv_ctx *ctx, kal_uint16 currefps)
{
	...
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_normal_video_setting,
		sizeof(corotimx800_normal_video_setting)/sizeof(kal_uint16));
...
static void hs_video_setting(struct subdrv_ctx *ctx)
{
	...
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_hs_video_setting,
		sizeof(corotimx800_hs_video_setting)/sizeof(kal_uint16));
...
static void custom1_setting(struct subdrv_ctx *ctx)
{
	...
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_custom1_setting,
		sizeof(corotimx800_custom1_setting)/sizeof(kal_uint16));
...
static void custom2_setting(struct subdrv_ctx *ctx)
{
	...
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_custom2_setting,
		sizeof(corotimx800_custom2_setting)/sizeof(kal_uint16));
...
static void custom3_setting(struct subdrv_ctx *ctx)
{
	...
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_custom3_setting,
		sizeof(corotimx800_custom3_setting)/sizeof(kal_uint16));
...
static void custom4_setting(struct subdrv_ctx *ctx)
{
	...
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_custom4_setting,
		sizeof(corotimx800_custom4_setting)/sizeof(kal_uint16));
...
static void custom5_setting(struct subdrv_ctx *ctx)
{
	...
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_custom5_setting,
		sizeof(corotimx800_custom5_setting)/sizeof(kal_uint16));
...
{
	IMX800_LOG_INF("+\n");
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_custom8_setting,
		sizeof(corotimx800_custom8_setting)/sizeof(kal_uint16));
	IMX800_LOG_INF("-\n");
}
...
{
	IMX800_LOG_INF("+\n");
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_custom9_setting,
		sizeof(corotimx800_custom9_setting)/sizeof(kal_uint16));
	IMX800_LOG_INF("-\n");
}
...
{
	IMX800_LOG_INF("+\n");
	corotimx800_table_write_cmos_sensor(ctx, corotimx800_custom10_setting,
		sizeof(corotimx800_custom10_setting)/sizeof(kal_uint16));
	IMX800_LOG_INF("-\n");
}
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (last_sensor_awb.ABS_GAIN_GR != 0 &&
		...
		corotimx800_awb_gain(ctx, &last_sensor_awb);
	}
	return ERROR_NONE;
}	/* custom4 */
		...
		enum SENSOR_SCENARIO_ID_ENUM scenario_id, uint32_t *ae_ctrl)
{
	...
	case SENSOR_SCENARIO_ID_NORMAL_PREVIEW:
		...
		break;
	case SENSOR_SCENARIO_ID_CUSTOM1:
		IMX800_LOG_INF("seamless switch to custom1 mode!\n");
		ctx->sensor_mode = scenario_id;
		ctx->autoflicker_en = KAL_FALSE;
		ctx->pclk = imgsensor_info.custom1.pclk;
		ctx->line_length = imgsensor_info.custom1.linelength;
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(corotimx800_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)corotimx800_ana_gain_table,
			sizeof(corotimx800_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_DIG_GAIN_RANGE_BY_SCENARIO:
		switch (*feature_data) {
		default:
			*(feature_data + 1) = imgsensor_info.min_dig_gain;
			*(feature_data + 2) = imgsensor_info.max_dig_gain;
			break;
		}
		break;
	case SENSOR_FEATURE_GET_DIG_GAIN_STEP:
		*(feature_data) = imgsensor_info.dig_gain_step;
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		IMX800_LOG_INF("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		corotimx800_get_pdaf_reg_setting(ctx,
				(*feature_para_len) / sizeof(UINT32),
				feature_data_16);
		break;
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		 *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 72333;
		 break;
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		IMX800_LOG_INF("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		corotimx800_set_pdaf_reg_setting(ctx,
				(*feature_para_len) / sizeof(UINT32),
				feature_data_16);
		break;
...
break;
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		corotimx800_set_lsc_reg_setting(ctx, index, feature_data_16,
	...
	case SENSOR_FEATURE_SEAMLESS_SWITCH:
		...
		if (pSetSensorAWB != NULL) {
			corotimx800_awb_gain(ctx, pSetSensorAWB);
	...
	case SENSOR_FEATURE_GET_CUST_PIXEL_RATE:
			...
			case SENSOR_SCENARIO_ID_NORMAL_VIDEO:
			case SENSOR_SCENARIO_ID_CUSTOM7:
				*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 700000000;
				break;
			case SENSOR_SCENARIO_ID_CUSTOM3:
				*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 700000000;
				break;
...
const struct subdrv_entry corotimx800_mipi_raw_entry = {
	.name       = "corotimx800_mipi_raw",
	.id         = COROTIMX800_SENSOR_ID,
	.pw_seq     = pw_seq,
