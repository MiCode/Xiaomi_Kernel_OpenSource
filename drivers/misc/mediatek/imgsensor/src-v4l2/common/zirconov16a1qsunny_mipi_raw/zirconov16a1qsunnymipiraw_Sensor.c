 ...
 * Filename:
 * ---------
 *	 ZIRCONOV16A1QQSUNNYmipi_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
 ...
 *============================================================================
 ****************************************************************************/
#define PFX "ZIRCONOV16A1QSUNNY_camera_sensor"
...
#include "zirconov16a1qsunny_ana_gain_table.h"
#include "zirconov16a1qsunnymipiraw_Sensor.h"
...
#define zirconov16a1qsunny_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
#define LOG_TAG "[zirconov16a1qsunny]"
#define OV16A1Q_LOG_INF(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
#define OV16A1Q_LOG_DBG(format, args...) pr_debug(LOG_TAG "[%s] " format, __func__, ##args)
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
 ...
 { 4656, 3496, 0, 0,   4656, 3496, 2328, 1748,  204, 334, 1920, 1080, 0, 0, 1920, 1080},// custom1
};
...
{
	OV16A1Q_LOG_DBG("dummyline = %d, dummypixels = %d \n", ctx->dummy_line, ctx->dummy_pixel);
...
static void set_mirror_flip(struct subdrv_ctx *ctx, kal_uint8 image_mirror)
{
	...
	OV16A1Q_LOG_DBG("image_mirror = %d\n", image_mirror);
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	OV16A1Q_LOG_DBG("shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	OV16A1Q_LOG_DBG("Framelength: set=%d/input=%d/min=%d, auto_extend=%d\n",
				     ...
				     kal_bool auto_extend_en)
{
	...
	write_cmos_sensor_8(ctx, 0x3208, 0x11);
	write_cmos_sensor_8(ctx, 0x3208, 0xa1);
	OV16A1Q_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	if (gain < min_gain || gain > max_gain) {
		OV16A1Q_LOG_INF("Error max gain setting: %d Should between %d & %d\n",
	...
	ctx->gain = reg_gain;
	OV16A1Q_LOG_DBG("gain = %d, reg_gain = 0x%x, max_gain:0x%x\n ",
...
static kal_uint32 zirconov16a1qsunny_awb_gain(struct subdrv_ctx *ctx,
		struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	return ERROR_NONE;
}

static void zirconov16a1qsunny_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
static kal_uint32 streaming_control(struct subdrv_ctx *ctx, kal_bool enable)
{
	...
	OV16A1Q_LOG_INF("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	if (enable) {
...
{
	OV16A1Q_LOG_INF("+\n");
	zirconov16a1qsunny_table_write_cmos_sensor(ctx, zirconov16a1qsunny_init_setting,
		sizeof(zirconov16a1qsunny_init_setting)/sizeof(kal_uint16));
...
{
	OV16A1Q_LOG_INF("+\n");
	zirconov16a1qsunny_table_write_cmos_sensor(ctx, zirconov16a1qsunny_preview_setting,
		sizeof(zirconov16a1qsunny_preview_setting) / sizeof(kal_uint16));
	OV16A1Q_LOG_INF("-\n");
} /* preview_setting */
...
{
	OV16A1Q_LOG_INF("+\n");
	zirconov16a1qsunny_table_write_cmos_sensor(ctx, zirconov16a1qsunny_normal_video_setting,
		sizeof(zirconov16a1qsunny_normal_video_setting) / sizeof(kal_uint16));
	OV16A1Q_LOG_INF("-\n");
}
...
{
	OV16A1Q_LOG_INF("+\n");
	zirconov16a1qsunny_table_write_cmos_sensor(ctx, zirconov16a1qsunny_hs_video_setting,
		sizeof(zirconov16a1qsunny_hs_video_setting)/sizeof(kal_uint16));
	OV16A1Q_LOG_INF("-\n");
}
...
{
	OV16A1Q_LOG_INF("+\n");
	zirconov16a1qsunny_table_write_cmos_sensor(ctx, zirconov16a1qsunny_custom1_setting,
		sizeof(zirconov16a1qsunny_custom1_setting)/sizeof(kal_uint16));
	OV16A1Q_LOG_INF("-\n");
}
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	OV16A1Q_LOG_INF("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		do {
			*sensor_id = return_sensor_id(ctx);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				OV16A1Q_LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			OV16A1Q_LOG_INF("Read sensor id fail, id: 0x%x\n",
				ctx->i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM1:
		...
		break;
	default:
		OV16A1Q_LOG_DBG("Error ScenarioId setting");
		preview(ctx, image_window, sensor_config_data);
		...
		kal_bool enable, UINT16 framerate)
{
	if (enable) /*enable auto flicker*/ {
		...
		OV16A1Q_LOG_DBG("enable! fps = %d", framerate);
	} else {
		 /*Cancel Auto flick*/
		ctx->autoflicker_en = KAL_FALSE;
	}
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	OV16A1Q_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	default:  /*coding with  preview scenario by default*/
		...
		if (ctx->frame_length > ctx->shutter)
			set_dummy(ctx);
		OV16A1Q_LOG_DBG("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
...
{
	OV16A1Q_LOG_DBG("enable: %d\n", enable);

#if TESTPATTERN
	if (enable) {
		write_cmos_sensor_8(ctx, 0x5081, 0x04);/*100% Color bar*/
	} else {
...
static kal_uint32 get_sensor_temperature(struct subdrv_ctx *ctx)
{
	...
	*****************************************/
	...
	OV16A1Q_LOG_DBG("temp_c(%d), read_reg(%d)\n", temperature_convert, temperature);
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(zirconov16a1qsunny_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)zirconov16a1qsunny_ana_gain_table,
			sizeof(zirconov16a1qsunny_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		OV16A1Q_LOG_DBG("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		OV16A1Q_LOG_DBG("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		OV16A1Q_LOG_DBG("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		OV16A1Q_LOG_DBG("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		OV16A1Q_LOG_DBG("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		OV16A1Q_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(ctx, KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		OV16A1Q_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(ctx, *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		OV16A1Q_LOG_DBG("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
...
break;
	...
	case SENSOR_FEATURE_SET_AWB_GAIN:
		...
		zirconov16a1qsunny_awb_gain(ctx,
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		zirconov16a1qsunny_set_lsc_reg_setting(ctx, index, feature_data_16,
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		 *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 300000;
		 break;
	default:
		break;
	}
...
const struct subdrv_entry zirconov16a1qsunny_mipi_raw_entry = {
	.name = "zirconov16a1qsunny_mipi_raw",
	.id = ZIRCONOV16A1QSUNNY_SENSOR_ID,
	.pw_seq = pw_seq,
