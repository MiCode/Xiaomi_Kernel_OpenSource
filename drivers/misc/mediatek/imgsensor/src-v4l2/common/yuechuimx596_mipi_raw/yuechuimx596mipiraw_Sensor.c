 ...
 *============================================================================
 ****************************************************************************/
#define PFX "YUECHUIMX596_camera_sensor"
...
#include "yuechuimx596_ana_gain_table.h"
#include "yuechuimx596mipiraw_Sensor.h"
//#include "yuechuimx596_seamless_switch.h"
#include "yuechuimx596_eeprom.h"
...
//#define yuechuimx596_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u16(__VA_ARGS__)
#define yuechuimx596_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
#define LOG_TAG "[yuechuimx596]"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = YUECHUIMX596_SENSOR_ID,
...
static kal_uint32 yuechuimx596_awb_gain(struct subdrv_ctx *ctx,
		struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	return ERROR_NONE;
}

static void yuechuimx596_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
{
	IMX596_LOG_INF("+\n");
	yuechuimx596_table_write_cmos_sensor(ctx, yuechuimx596_init_setting,
		sizeof(yuechuimx596_init_setting)/sizeof(kal_uint16));
...
{
	IMX596_LOG_INF("+\n");
	yuechuimx596_table_write_cmos_sensor(ctx, yuechuimx596_preview_setting,
		sizeof(yuechuimx596_preview_setting) / sizeof(kal_uint16));
	IMX596_LOG_INF("-\n");
} /* preview_setting */
...
{
	IMX596_LOG_INF("+\n");
	yuechuimx596_table_write_cmos_sensor(ctx, yuechuimx596_normal_video_setting,
		sizeof(yuechuimx596_normal_video_setting)/sizeof(kal_uint16));
	IMX596_LOG_INF("-\n");
}
...
{
	IMX596_LOG_INF("+\n");
	yuechuimx596_table_write_cmos_sensor(ctx, yuechuimx596_hs_video_setting,
		sizeof(yuechuimx596_hs_video_setting)/sizeof(kal_uint16));
	IMX596_LOG_INF("-\n");
}
...
{
	IMX596_LOG_INF("+\n");
	yuechuimx596_table_write_cmos_sensor(ctx, yuechuimx596_custom1_setting,
		sizeof(yuechuimx596_custom1_setting)/sizeof(kal_uint16));
	IMX596_LOG_INF("-\n");
}
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	IMX596_LOG_INF("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		do {
			*sensor_id = return_sensor_id(ctx);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				IMX596_LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n",
...
static kal_uint32 get_sensor_temperature(struct subdrv_ctx *ctx)
{
	...
	IMX596_LOG_INF("temp_c(%d), read_reg(%d)\n", temperature_convert, temperature);
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO:
		...
		IMX596_LOG_INF("SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO get:%d\n",*(feature_data + 1));
	break;
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(yuechuimx596_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)yuechuimx596_ana_gain_table,
			sizeof(yuechuimx596_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		IMX596_LOG_INF("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		IMX596_LOG_INF("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		IMX596_LOG_INF("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		IMX596_LOG_INF("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
...
break;
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		yuechuimx596_set_lsc_reg_setting(ctx, index, feature_data_16,
...
const struct subdrv_entry yuechuimx596_mipi_raw_entry = {
	.name = "yuechuimx596_mipi_raw",
	.id = YUECHUIMX596_SENSOR_ID,
	.pw_seq = pw_seq,
