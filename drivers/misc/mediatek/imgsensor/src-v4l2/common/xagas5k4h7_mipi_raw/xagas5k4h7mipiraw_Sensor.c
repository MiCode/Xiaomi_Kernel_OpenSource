...
#define S5K4H7_LOG_INF(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
#define S5K4H7_LOG_DBG(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
...
{
	S5K4H7_LOG_INF("dummyline = %d, dummypixels = %d \n", ctx->dummy_line, ctx->dummy_pixel);

	write_cmos_sensor(ctx, GROUP_HOLD_ADDR, 0x01);
	...
	write_cmos_sensor(ctx, GROUP_HOLD_ADDR, 0x00);
}	/*	set_dummy  */
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	write_cmos_sensor(ctx, GROUP_HOLD_ADDR, 0x00);
	S5K4H7_LOG_INF("realtime_fps =%d\n", realtime_fps);
	S5K4H7_LOG_INF("shutter =%d, framelength =%d\n",
				     ...
				     kal_bool auto_extend_en)
{
	...
	write_cmos_sensor(ctx, GROUP_HOLD_ADDR, 0x00);
	S5K4H7_LOG_INF("set shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	ctx->gain = reg_gain;
	S5K4H7_LOG_INF("gain = %d, reg_gain = 0x%x, max_gain:0x%x\n ",
		...
		struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
    ...
    S5K4H7_LOG_INF("write awb gain r:g:b %d:%d:%d \n", r_Gain, g_Gain, b_Gain);
		...
		kal_bool enable, UINT16 framerate)
{
	if (enable) /*enable auto flicker*/ {
		...
		S5K4H7_LOG_INF("enable! fps = %d", framerate);
	} else {
		 /*Cancel Auto flick*/
		ctx->autoflicker_en = KAL_FALSE;
	}
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	S5K4H7_LOG_INF("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
