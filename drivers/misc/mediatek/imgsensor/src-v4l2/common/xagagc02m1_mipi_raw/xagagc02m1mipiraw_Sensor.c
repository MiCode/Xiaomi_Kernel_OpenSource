 ...
 *============================================================================
 ****************************************************************************/
#define PFX "XAGAGC02M1_camera_sensor"
...
#include "xagagc02m1_ana_gain_table.h"
#include "xagagc02m1mipiraw_Sensor.h"
...
#define xagagc02m1_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8_u8(__VA_ARGS__)
#define LOG_TAG "[xagagc02m1]"
#define GC02M1_LOG_INF(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
#define GC02M1_LOG_DBG(format, args...) pr_debug(LOG_TAG "[%s] " format, __func__, ##args)
...
#define GC02M1SUNNY_SENSOR_GAIN_BASE             0x400
#define GC02M1SUNNY_SENSOR_GAIN_MAX              (12 * GC02M1SUNNY_SENSOR_GAIN_BASE)
#define GC02M1SUNNY_SENSOR_GAIN_MAX_VALID_INDEX  16
#define GC02M1SUNNY_SENSOR_GAIN_MAP_SIZE         16
#define GC02M1SUNNY_SENSOR_DGAIN_BASE            0x400



static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = XAGAGC02M1_SENSOR_ID,
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[] = {
	{
		...
		0x00, 0x2b, 1600, 1200, 0x00, 0x00, 0x0000, 0x0000,
	},
};
...
{
	GC02M1_LOG_DBG("dummyline = %d, dummypixels = %d \n", ctx->dummy_line, ctx->dummy_pixel);
	write_cmos_sensor_8(ctx, 0xfe, 0x00);
	write_cmos_sensor_8(ctx, 0x41, (ctx->frame_length >> 8) & 0x3f);
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	// Update Shutter
	write_cmos_sensor_8(ctx, 0xfe, 0x00);
	write_cmos_sensor_8(ctx, 0x03, (shutter >> 8) & 0x3f);
	write_cmos_sensor_8(ctx, 0x04, shutter  & 0xff);
	write_cmos_sensor_8(ctx, 0x41, (ctx->frame_length >> 8) & 0x3f);
	write_cmos_sensor_8(ctx, 0x42, ctx->frame_length & 0xff);
	GC02M1_LOG_DBG("shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	GC02M1_LOG_DBG("Framelength: set=%d/input=%d/min=%d\n",
				     ...
				     kal_bool auto_extend_en)
{
	...
	write_cmos_sensor_8(ctx, 0x41, (ctx->frame_length >> 8) & 0x3f);
	write_cmos_sensor_8(ctx, 0x42, ctx->frame_length & 0xff);
	GC02M1_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static kal_uint16 gain2reg(struct subdrv_ctx *ctx, const kal_uint16 gain)
{
	...
	if (reg_gain < GC02M1SUNNY_SENSOR_GAIN_BASE)
		reg_gain = GC02M1SUNNY_SENSOR_GAIN_BASE;
	else if (reg_gain > GC02M1SUNNY_SENSOR_GAIN_MAX)
		reg_gain = GC02M1SUNNY_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	kal_uint16 GC02M1SUNNY_AGC_Param[GC02M1SUNNY_SENSOR_GAIN_MAX_VALID_INDEX][2] = {
		{  1024,  0 },
		{  1536,  1 },
		{  2035,  2 },
		{  2519,  3 },
	...
	for (gain_index = GC02M1SUNNY_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (reg_gain >= GC02M1SUNNY_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor_8(ctx, 0xfe, 0x00);
	write_cmos_sensor_8(ctx, 0xb6, GC02M1SUNNY_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * GC02M1SUNNY_SENSOR_DGAIN_BASE / GC02M1SUNNY_AGC_Param[gain_index][0];
	write_cmos_sensor_8(ctx, 0xb1, (temp_gain >> 8) & 0x1f);
	write_cmos_sensor_8(ctx, 0xb2, temp_gain & 0xff);
	GC02M1_LOG_DBG("GC02M1SUNNY_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d\n",
		GC02M1SUNNY_AGC_Param[gain_index][1], temp_gain, reg_gain);

	return reg_gain;

} /* set_gain */

static kal_uint32 xagagc02m1_awb_gain(struct subdrv_ctx *ctx,
		struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
    ...
    GC02M1_LOG_DBG("write awb gain r:g:b %d:%d:%d \n", r_Gain, g_Gain, b_Gain);

	return ERROR_NONE;
}


static void xagagc02m1_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
{
	GC02M1_LOG_INF("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	write_cmos_sensor_8(ctx, 0xfe, 0x00);
	if (enable)
		write_cmos_sensor_8(ctx, 0x3e, 0x90);
	else
		write_cmos_sensor_8(ctx, 0x3e, 0x00);
	write_cmos_sensor_8(ctx, 0xfe, 0x00);
	return ERROR_NONE;
}
...
static void sensor_init(struct subdrv_ctx *ctx)
{
    ...
    // Global
	xagagc02m1_table_write_cmos_sensor(ctx, xagagc02m1_init_setting,
		sizeof(xagagc02m1_init_setting) / sizeof(kal_uint16));
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	GC02M1_LOG_INF("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		do {
			*sensor_id = return_sensor_id(ctx);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				GC02M1_LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			GC02M1_LOG_INF("Read sensor id fail, id: 0x%x\n",
				ctx->i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM6:
		...
		break;
	default:
		GC02M1_LOG_DBG("Error ScenarioId setting");
		preview(ctx, image_window, sensor_config_data);
		...
		kal_bool enable, UINT16 framerate)
{
	if (enable) /*enable auto flicker*/ {
		...
		GC02M1_LOG_DBG("enable! fps = %d", framerate);
	} else {
		 /*Cancel Auto flick*/
		ctx->autoflicker_en = KAL_FALSE;
	}
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	GC02M1_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	default:  /*coding with  preview scenario by default*/
		...
		if (ctx->frame_length > ctx->shutter)
			set_dummy(ctx);
		GC02M1_LOG_DBG("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO:
		...
		GC02M1_LOG_DBG("SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO get:%d\n", *(feature_data + 1));
		break;
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(xagagc02m1_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)xagagc02m1_ana_gain_table,
			sizeof(xagagc02m1_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		GC02M1_LOG_DBG("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		GC02M1_LOG_DBG("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		GC02M1_LOG_DBG("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		GC02M1_LOG_DBG("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		GC02M1_LOG_DBG("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		GC02M1_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(ctx, KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		GC02M1_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(ctx, *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		GC02M1_LOG_DBG("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		xagagc02m1_set_lsc_reg_setting(ctx, index, feature_data_16,
...
const struct subdrv_entry xagagc02m1_mipi_raw_entry = {
	.name = "xagagc02m1_mipi_raw",
	.id = XAGAGC02M1_SENSOR_ID,
	.pw_seq = pw_seq,
