...
#define xagaov02b10_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8_u8(__VA_ARGS__)
#define LOG_TAG "[xagaov02b10]"
#define OV02B10_LOG_INF(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
#define OV02B10_LOG_DBG(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
...
{
	OV02B10_LOG_DBG("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	OV02B10_LOG_DBG("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		do {
			*sensor_id = return_sensor_id(ctx);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				OV02B10_LOG_DBG("i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			OV02B10_LOG_DBG("Read sensor id fail, id: 0x%x\n",
				ctx->i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
