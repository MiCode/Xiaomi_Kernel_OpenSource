 ...
 *============================================================================
 ****************************************************************************/
#define PFX "XAGAS5KHM2_camera_sensor"
...
#include "xagas5khm2_ana_gain_table.h"
#include "xagas5khm2mipiraw_Sensor.h"
#include "xagas5khm2_seamless_switch.h"
...
#define xagas5khm2_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u16(__VA_ARGS__)
#define LOG_TAG "[xagas5khm2]"
#define S5KHM2_LOG_INF(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
#define S5KHM2_LOG_DBG(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[3] = {
	...
	// Full crop 12M mode setting 664(pxiel)*500
	{
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
		0x00, 0x2b, 4000, 3000, 0x00, 0x00, 0x0000, 0x0000,
		0x01, 0x30, 832,  500,  0x00, 0x00, 0x0000, 0x0000
	},
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info[3] = {
	...
	},
	{
		...
		.i4PosL = {
			{16, 4 }, {16, 5 }, {28, 6 }, {28, 7 },
			{22, 16}, {22, 17}, {10, 18}, {10, 19},
		},
		.i4PosR = {
			{17, 4 }, {17, 5 }, {29, 6 }, {29, 7 },
			{23, 16}, {23, 17}, {11, 18}, {11, 19},
		}
	},
};

static void xagas5khm2_get_pdaf_reg_setting(struct subdrv_ctx *ctx, MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
	...
	for (i = 0; i < regNum; i++) {
		...
		S5KHM2_LOG_DBG("%x %x", regDa[idx], regDa[idx+1]);
	}
}
static void xagas5khm2_set_pdaf_reg_setting(struct subdrv_ctx *ctx,
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	S5KHM2_LOG_DBG("Framelength: set=%d/input=%d/min=%d, auto_extend=%d\n",
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	if (gain < min_gain || gain > max_gain) {
		S5KHM2_LOG_DBG("Error max gain setting: %d Should between %d & %d\n",
...
static kal_uint32 xagas5khm2_awb_gain(struct subdrv_ctx *ctx,
...
static void xagas5khm2_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
{
	S5KHM2_LOG_DBG("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
...
#if 0
static kal_uint16 s5khm2_burst_write_cmos_sensor(
                                 kal_uint16 *para, kal_uint32 len)
{
	...
	while (len > IDX) {
		...
		if ((tosend >= I2C_BUFFER_LEN) || len == IDX ||
			addr != para[IDX]) {
...
static kal_uint16 s5khm2_fast_write_cmos_sensor(
                                 kal_uint16 *para, kal_uint32 len)
{
	...
	kal_uint32 table_pos_end = 0;
	kal_uint32 table_len     = 0;
	kal_uint32 burst_pos_sta = 0;
	kal_uint32 burst_pos_end = 0;
	kal_uint32 burst_len     = 0;

	S5KHM2_LOG_DBG("len = %d +\n", len);

	while (len > IDX) {
		addr = para[IDX];

		if (addr == 0x6F12) {
			if (burst_pos_sta == burst_pos_end) {
				burst_pos_sta = IDX;
			}
			burst_pos_end = IDX + 2;
		} else {
			if ((burst_pos_end - burst_pos_sta) > burst_thr) {
				table_pos_end = burst_pos_sta;

				table_len = table_pos_end - table_pos_sta;
				burst_len = burst_pos_end - burst_pos_sta;
			} else {
				table_pos_end = IDX + 2;
			}

			burst_pos_end = burst_pos_sta;
		}
		...
		if (table_len > 0) {
			...
			table_write_cmos_sensor(para + table_pos_sta, table_len);
			S5KHM2_LOG_DBG("table_pos_sta = %d, table_len = %d\n", table_pos_sta, table_len);
			table_pos_sta = table_pos_end = IDX;
			table_len = 0;
		}

		if (burst_len > 0) {
			/* Find the sequence that requires Burst Mode */
			s5khm2_burst_write_cmos_sensor(para + burst_pos_sta, burst_len);
			S5KHM2_LOG_DBG("burst_pos_sta = %d, burst_len = %d\n", burst_pos_sta, burst_len);
			table_pos_sta = table_pos_end = IDX;
			burst_len = 0;
		}
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	write_cmos_sensor(ctx, 0x6028, 0x4000);
	write_cmos_sensor(ctx, 0x0000, 0x0004);
	...
	// 2nd Tnp
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_2nd_tnp_setting,
		sizeof(xagas5khm2_2nd_tnp_setting) / sizeof(kal_uint16));
	...
	mdelay(3);
	// 1st Tnp
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_1st_tnp_setting,
		sizeof(xagas5khm2_1st_tnp_setting) / sizeof(kal_uint16));
	write_cmos_sensor(ctx, 0x0A02, 0xFFFF);

	// Global
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_init_setting,
		sizeof(xagas5khm2_init_setting) / sizeof(kal_uint16));
...
static void preview_setting(struct subdrv_ctx *ctx)
{
	...
	if (enable_seamless) {
		// Fast Mode Change
		S5KHM2_LOG_DBG("enable seamless switch setting\n");
		xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_fmc_setting,
			sizeof(xagas5khm2_fmc_setting) / sizeof(kal_uint16));

		xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_fastmode_preview,
			sizeof(xagas5khm2_fastmode_preview) / sizeof(kal_uint16));
	} else {
		xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_preview_setting,
			sizeof(xagas5khm2_preview_setting) / sizeof(kal_uint16));
	}
	S5KHM2_LOG_DBG("-\n");
} /* preview_setting */
...
static void capture_setting(struct subdrv_ctx *ctx, kal_uint16 currefps)
{
	...
	if (enable_seamless) {
		// Fast Mode Change
		xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_fmc_108M_setting,
			sizeof(xagas5khm2_fmc_108M_setting) / sizeof(kal_uint16));
...
static void normal_video_setting(struct subdrv_ctx *ctx, kal_uint16 currefps)
{
	...
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_switch_normal,
		sizeof(xagas5khm2_switch_normal) / sizeof(kal_uint16));
#endif
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_normal_video_setting,
		sizeof(xagas5khm2_normal_video_setting)/sizeof(kal_uint16));
	S5KHM2_LOG_DBG("-\n");
}
...
static void hs_video_setting(struct subdrv_ctx *ctx)
{
	...
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_switch_normal,
		sizeof(xagas5khm2_switch_normal) / sizeof(kal_uint16));
#endif
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_hs_video_setting,
		sizeof(xagas5khm2_hs_video_setting)/sizeof(kal_uint16));
	S5KHM2_LOG_DBG("-\n");
}
...
#endif
	/*************MIPI output setting************/
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_custom2_setting,
		sizeof(xagas5khm2_custom2_setting)/sizeof(kal_uint16));
	S5KHM2_LOG_DBG("+\n");
}
...
static void custom3_setting(struct subdrv_ctx *ctx)
{
	...
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_switch_normal,
		sizeof(xagas5khm2_switch_normal) / sizeof(kal_uint16));
#endif
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_custom3_setting,
		sizeof(xagas5khm2_custom3_setting)/sizeof(kal_uint16));
	S5KHM2_LOG_DBG("-\n");
}
...
static void custom4_setting(struct subdrv_ctx *ctx)
{
	...
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_switch_normal,
		sizeof(xagas5khm2_switch_normal) / sizeof(kal_uint16));
#endif
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_custom4_setting,
		sizeof(xagas5khm2_custom4_setting)/sizeof(kal_uint16));
	S5KHM2_LOG_DBG("-\n");
}
...
static void custom5_setting(struct subdrv_ctx *ctx)
{
	...
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_switch_normal,
		sizeof(xagas5khm2_switch_normal) / sizeof(kal_uint16));
#endif
	xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_custom5_setting,
		sizeof(xagas5khm2_custom5_setting)/sizeof(kal_uint16));
	S5KHM2_LOG_DBG("-\n");
}
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	S5KHM2_LOG_DBG("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		do {
			*sensor_id = return_sensor_id(ctx);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				S5KHM2_LOG_DBG("i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			S5KHM2_LOG_DBG("Read sensor id fail, id: 0x%x\n",
				ctx->i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM7:
		...
		break;
	default:
		S5KHM2_LOG_DBG("Error ScenarioId setting");
		preview(ctx, image_window, sensor_config_data);
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	default:  /*coding with  preview scenario by default*/
		...
		if (ctx->frame_length > ctx->shutter)
			set_dummy(ctx);
		S5KHM2_LOG_DBG("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
		...
		enum SENSOR_SCENARIO_ID_ENUM scenario_id, uint32_t *ae_ctrl)
{
	...
	switch (scenario_id) {
	case SENSOR_SCENARIO_ID_NORMAL_PREVIEW:
		S5KHM2_LOG_DBG("seamless switch to preview!\n");
		...
		xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_switch_fastmode_init,
			sizeof(xagas5khm2_switch_fastmode_init) / sizeof(kal_uint16));
		...
		xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_switch_fastmode_preview,
			sizeof(xagas5khm2_switch_fastmode_preview) / sizeof(kal_uint16));

		write_cmos_sensor(ctx, GROUP_HOLD_ADDR, 0x0001);
		//mdelay(40);
		break;
	case SENSOR_SCENARIO_ID_CUSTOM5:
		S5KHM2_LOG_DBG("seamless switch to 3x mode!\n");
		ctx->sensor_mode = scenario_id;
		ctx->autoflicker_en = KAL_FALSE;
		ctx->pclk = imgsensor_info.custom5.pclk;
		ctx->line_length = imgsensor_info.custom5.linelength;
		...
		xagas5khm2_table_write_cmos_sensor(ctx, xagas5khm2_switch_fastmode_3x,
			sizeof(xagas5khm2_switch_fastmode_3x) / sizeof(kal_uint16));
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO:
		...
		S5KHM2_LOG_DBG("SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO get:%d\n",*(feature_data + 1));
	break;
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(xagas5khm2_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)xagas5khm2_ana_gain_table,
			sizeof(xagas5khm2_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		S5KHM2_LOG_DBG("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		S5KHM2_LOG_DBG("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		S5KHM2_LOG_DBG("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		S5KHM2_LOG_DBG("SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%d\n",
			(UINT16) *feature_data);
		PDAFinfo =
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		S5KHM2_LOG_DBG("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		xagas5khm2_get_pdaf_reg_setting(ctx,
				(*feature_para_len) / sizeof(UINT32),
				feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		S5KHM2_LOG_DBG("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		xagas5khm2_set_pdaf_reg_setting(ctx,
				(*feature_para_len) / sizeof(UINT32),
				feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF:
		S5KHM2_LOG_DBG("PDAF mode :%d\n", *feature_data_16);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		S5KHM2_LOG_DBG("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		S5KHM2_LOG_DBG("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		S5KHM2_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(ctx, KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		S5KHM2_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(ctx, *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		S5KHM2_LOG_DBG("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
...
break;
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		xagas5khm2_set_lsc_reg_setting(ctx, index, feature_data_16,
...
const struct subdrv_entry xagas5khm2_mipi_raw_entry = {
	.name = "xagas5khm2_mipi_raw",
	.id = XAGAS5KHM2_SENSOR_ID,
	.pw_seq = pw_seq,
