...
#define S5KGW1_LOG_INF(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
#define S5KGW1_LOG_DBG(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[9] = {
	...
	{ 9280, 6944,    0,     0,  9280, 6944,  4640, 3472,    0,    0,  4640, 3472,    0,    0,  4640, 3472}, // video
	...
	{ 9280, 6944,    0,     0,  9280, 6944,  2320, 1736,    0,    0,  2320, 1736,    0,    0,  2320, 1736}, // custom2 as 120fps
	{ 9280, 6944,    0,     0,  9280, 6944,  4640, 3472,    0,    0,  4640, 3472,    0,    0,  4640, 3472}, // custom3 as 16M @ 60fps
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	write_cmos_sensor(ctx, GROUP_HOLD_ADDR, 0x01);
	write_cmos_sensor(ctx, FRAME_LENGTH_LINES_ADDR, ctx->frame_length & 0xFFFF);
	write_cmos_sensor(ctx, COARSE_INTEGRATION_TIME_ADDR, shutter & 0xFFFF);
	write_cmos_sensor(ctx, GROUP_HOLD_ADDR, 0x00);
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	if (gain < min_gain || gain > max_gain) {
		S5KGW1_LOG_DBG("Error max gain setting: %d Should between %d & %d\n",
...
{
	MUINT32 gr_Gain = (pSetSensorAWB->ABS_GAIN_GR << 8) >> 9;
	MUINT32 r_Gain = (pSetSensorAWB->ABS_GAIN_R << 8) >> 9;
	MUINT32 b_Gain = (pSetSensorAWB->ABS_GAIN_B << 8) >> 9;
...
{
	S5KGW1_LOG_DBG("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	write_cmos_sensor(ctx, 0x6028, 0x4000);
	write_cmos_sensor(ctx, 0x6214, 0xF9F0);
	write_cmos_sensor(ctx, 0x6218, 0xF9F0);
	if (enable) {
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	write_cmos_sensor(ctx, 0x0000, 0x0971);
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	S5KGW1_LOG_DBG("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		do {
			*sensor_id = return_sensor_id(ctx);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				S5KGW1_LOG_DBG("i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			S5KGW1_LOG_DBG("Read sensor id fail, id: 0x%x\n",
				ctx->i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
