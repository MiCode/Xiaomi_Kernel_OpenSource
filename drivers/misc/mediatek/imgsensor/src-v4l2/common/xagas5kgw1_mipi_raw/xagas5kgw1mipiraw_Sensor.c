 ...
 *============================================================================
 ****************************************************************************/
#define PFX "XAGAS5KGW1_camera_sensor"
...
#include "xagas5kgw1_ana_gain_table.h"
#include "xagas5kgw1mipiraw_Sensor.h"
...
#define xagas5kgw1_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u16(__VA_ARGS__)
#define LOG_TAG "[xagas5kgw1]"
#define S5KGW1_LOG_INF(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
#define S5KGW1_LOG_DBG(format, args...) pr_debug(LOG_TAG "[%s] " format, __func__, ##args)
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.custom1 = { /* 4640x3472 stereo 24fps*/
		.pclk = 1440000000,
		.linelength  = 10080,
		.framelength = 5950,
		.startx = 0,
	...
	.custom4 = { /*9280x6944 full size 15fps*/
		.pclk = 1440000000,
		.linelength  = 10696,
		.framelength = 8972,
		.startx = 0,
	...
	.max_gain     = 32 * BASEGAIN,//32x
	.min_gain_iso = 50,
	.exp_step     = 2,
	.gain_step    = 1,
	.gain_type    = 0,

	.max_frame_length           = 0x420FFF,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[11] = {
	...
	{ 9280, 6944,    0,     0,  9280, 6944,  4640, 3472,    0,    0,  4640, 3472,    0,  430,  4640, 2612}, // video
	{ 9280, 6944,  800,  1312,  7680, 4320,  1920, 1080,    0,    0,  1920, 1080,    0,    0,  1920, 1080}, // high speed video 240fps
	{ 9280, 6944,    0,     0,  9280, 6944,  4640, 3472,    0,    0,  4640, 3472,    0,    0,  4640, 3472}, // slim video
	{ 9280, 6944,    0,     0,  9280, 6944,  4640, 3472,    0,    0,  4640, 3472,    0,    0,  4640, 3472}, // custom1 stereo 24fps
	{ 9280, 6944,  800,  1312,  7680, 4320,  1920, 1080,    0,    0,  1920, 1080,    0,    0,  1920, 1080}, // custom2 as 120fps
	{ 9280, 6944,    0,     0,  9280, 6944,  4640, 3472,    0,    0,  4640, 3472,    0,  430,  4640, 2612}, // custom3 as 16M @ 60fps
	{ 9280, 6944,    0,     0,  9280, 6944,  9280, 6944,    0,    0,  9280, 6944,    0,    0,  9280, 6944}, // custom4 full size capture
	{ 9280, 6944,    0,     0,  9280, 6944,  4640, 3472,    0,    0,  4640, 3472,    0,  430,  4640, 2612}, // custom5
	{ 9280, 6944,    0,     0,  9280, 6944,  4640, 3472,    0,    0,  4640, 3472,    0,  430,  4640, 2612}, // custom6
};
#if ENABLE_PDAF
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[3] = {
	// Preview mode setting
	{
		...
		0x01, 0x2B, 0x0248, 0x0D90, 0x03, 0x00, 0x0000, 0x0000
	},
	// Video mode setting
	{
		0x02, 0x0A,   0x00,   0x08, 0x40, 0x00,
		0x00, 0x2B, 0x1220, 0x0A34, 0x01, 0x00, 0x0000, 0x0000,
		0x01, 0x2B, 0x0248, 0x0A20, 0x03, 0x00, 0x0000, 0x0000
	},
	// Custom1 for stereo
	{
		0x02, 0x0A,   0x00,   0x08, 0x40, 0x00,
...
static void xagas5kgw1_get_pdaf_reg_setting(struct subdrv_ctx *ctx, MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
	...
	for (i = 0; i < regNum; i++) {
		...
		S5KGW1_LOG_DBG("%x %x", regDa[idx], regDa[idx+1]);
	}
}
static void xagas5kgw1_set_pdaf_reg_setting(struct subdrv_ctx *ctx,
...
{
	S5KGW1_LOG_DBG("dummyline = %d, dummypixels = %d\n", ctx->dummy_line, ctx->dummy_pixel);
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	write_cmos_sensor(ctx, GROUP_HOLD_ADDR, 0x0001);
	S5KGW1_LOG_DBG("shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	S5KGW1_LOG_DBG("Framelength: set=%d/input=%d/min=%d, auto_extend=%d\n",
				     ...
				     kal_bool auto_extend_en)
{
	...
	write_cmos_sensor(ctx, COARSE_INTEGRATION_TIME_SHIFT_ADDR, (Rshift << 8) & 0xFFFF);
	write_cmos_sensor(ctx, GROUP_HOLD_ADDR, 0x0001);
	S5KGW1_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	if (gain < min_gain || gain > max_gain) {
		S5KGW1_LOG_INF("Error max gain setting: %d Should between %d & %d\n",
	...
	ctx->gain = reg_gain;
	S5KGW1_LOG_DBG("gain = %d, reg_gain = 0x%x, max_gain:0x%x\n ",
...
static kal_uint32 xagas5kgw1_awb_gain(struct subdrv_ctx *ctx,
		struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	MUINT32 gr_Gain = pSetSensorAWB->ABS_GAIN_GR;
	MUINT32 r_Gain = pSetSensorAWB->ABS_GAIN_R;
	MUINT32 b_Gain = pSetSensorAWB->ABS_GAIN_B;

	write_cmos_sensor(ctx, 0x6028, 0x4000);
	write_cmos_sensor(ctx, AWB_R_GAIN_ADDR, r_Gain >> 1);
	write_cmos_sensor(ctx, AWB_G_GAIN_ADDR, gr_Gain >> 1);
	write_cmos_sensor(ctx, AWB_B_GAIN_ADDR, b_Gain >> 1);

	S5KGW1_LOG_DBG("write awb gain gr:r:b %d:%d:%d \n", gr_Gain, r_Gain, b_Gain);

	return ERROR_NONE;
}


static void xagas5kgw1_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
{
	S5KGW1_LOG_INF("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	write_cmos_sensor(ctx, 0x6028, 0x4000);
	if (enable) {
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	write_cmos_sensor(ctx, 0x6028, 0x4000);
	write_cmos_sensor(ctx, 0x0000, 0x0008);
	write_cmos_sensor(ctx, 0x0000, 0xF971);
	write_cmos_sensor(ctx, 0x6010, 0x0001);
	mdelay(35); //p35 delay 35ms
	write_cmos_sensor(ctx, 0x6214, 0xF9F0);
	write_cmos_sensor(ctx, 0x6218, 0xF150);
	write_cmos_sensor(ctx, 0x0A02, 0x7E00);

	xagas5kgw1_table_write_cmos_sensor(ctx, xagas5kgw1_init_setting,
		sizeof(xagas5kgw1_init_setting) / sizeof(kal_uint16));
...
{
	S5KGW1_LOG_INF("+\n");
	xagas5kgw1_table_write_cmos_sensor(ctx, xagas5kgw1_preview_setting,
		sizeof(xagas5kgw1_preview_setting) / sizeof(kal_uint16));
	S5KGW1_LOG_INF("-\n");
} /* preview_setting */
...
{
	S5KGW1_LOG_INF("+\n");
	xagas5kgw1_table_write_cmos_sensor(ctx, xagas5kgw1_normal_video_setting,
		sizeof(xagas5kgw1_normal_video_setting)/sizeof(kal_uint16));
	S5KGW1_LOG_INF("-\n");
}
...
{
	S5KGW1_LOG_INF("+\n");
	xagas5kgw1_table_write_cmos_sensor(ctx, xagas5kgw1_hs_video_setting,
		sizeof(xagas5kgw1_hs_video_setting)/sizeof(kal_uint16));
	S5KGW1_LOG_INF("-\n");
}
...
{
	S5KGW1_LOG_INF("+\n");
	xagas5kgw1_table_write_cmos_sensor(ctx, xagas5kgw1_custom1_setting,
		sizeof(xagas5kgw1_custom1_setting)/sizeof(kal_uint16));
	S5KGW1_LOG_INF("-\n");
}
...
{
	S5KGW1_LOG_INF("+\n");
	xagas5kgw1_table_write_cmos_sensor(ctx, xagas5kgw1_custom2_setting,
		sizeof(xagas5kgw1_custom2_setting)/sizeof(kal_uint16));
	S5KGW1_LOG_INF("+\n");
}
...
{
	S5KGW1_LOG_INF("+\n");
	xagas5kgw1_table_write_cmos_sensor(ctx, xagas5kgw1_custom3_setting,
		sizeof(xagas5kgw1_custom3_setting)/sizeof(kal_uint16));
	S5KGW1_LOG_INF("-\n");
}
...
{
	S5KGW1_LOG_INF("+\n");
	xagas5kgw1_table_write_cmos_sensor(ctx, xagas5kgw1_custom4_setting,
		sizeof(xagas5kgw1_custom4_setting)/sizeof(kal_uint16));
	S5KGW1_LOG_INF("-\n");
}
...
{
	S5KGW1_LOG_INF("+\n");
	xagas5kgw1_table_write_cmos_sensor(ctx, xagas5kgw1_custom5_setting,
		sizeof(xagas5kgw1_custom5_setting)/sizeof(kal_uint16));
	S5KGW1_LOG_INF("-\n");
}
...
{
	S5KGW1_LOG_INF("+\n");
	xagas5kgw1_table_write_cmos_sensor(ctx, xagas5kgw1_custom6_setting,
		sizeof(xagas5kgw1_custom6_setting)/sizeof(kal_uint16));
	S5KGW1_LOG_INF("-\n");
}
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	S5KGW1_LOG_INF("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		do {
			*sensor_id = return_sensor_id(ctx);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				S5KGW1_LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			S5KGW1_LOG_INF("Read sensor id fail, id: 0x%x\n",
				ctx->i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM6:
		...
		break;
	default:
		S5KGW1_LOG_DBG("Error ScenarioId setting");
		preview(ctx, image_window, sensor_config_data);
		...
		kal_bool enable, UINT16 framerate)
{
	if (enable) /*enable auto flicker*/ {
		...
		S5KGW1_LOG_DBG("enable! fps = %d", framerate);
	} else {
		 /*Cancel Auto flick*/
		ctx->autoflicker_en = KAL_FALSE;
	}
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	S5KGW1_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	default:  /*coding with  preview scenario by default*/
		...
		if (ctx->frame_length > ctx->shutter)
			set_dummy(ctx);
		S5KGW1_LOG_DBG("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
...
static kal_uint32 get_sensor_temperature(struct subdrv_ctx *ctx)
{
	...
	temp = read_cmos_sensor(ctx, 0x013a);
	write_cmos_sensor(ctx, 0x013a, temp | 0x01);
	...
	S5KGW1_LOG_DBG("temp_c(%d), read_reg(%d)\n", temperature_convert, temperature);
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(xagas5kgw1_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)xagas5kgw1_ana_gain_table,
			sizeof(xagas5kgw1_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		S5KGW1_LOG_DBG("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		S5KGW1_LOG_DBG("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		S5KGW1_LOG_DBG("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		S5KGW1_LOG_DBG("SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%d\n",
			(UINT16) *feature_data);
		PDAFinfo =
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		S5KGW1_LOG_DBG("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		xagas5kgw1_get_pdaf_reg_setting(ctx,
				(*feature_para_len) / sizeof(UINT32),
				feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		S5KGW1_LOG_DBG("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		xagas5kgw1_set_pdaf_reg_setting(ctx,
				(*feature_para_len) / sizeof(UINT32),
				feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF:
		S5KGW1_LOG_DBG("PDAF mode :%d\n", *feature_data_16);
		ctx->pdaf_mode = *feature_data_16;
		break;
#endif
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		S5KGW1_LOG_DBG("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		S5KGW1_LOG_DBG("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		S5KGW1_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(ctx, KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		S5KGW1_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(ctx, *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		S5KGW1_LOG_DBG("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
...
break;
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		xagas5kgw1_set_lsc_reg_setting(ctx, index, feature_data_16,
...
const struct subdrv_entry xagas5kgw1_mipi_raw_entry = {
	.name = "xagas5kgw1_mipi_raw",
	.id = XAGAS5KGW1_SENSOR_ID,
	.pw_seq = pw_seq,
