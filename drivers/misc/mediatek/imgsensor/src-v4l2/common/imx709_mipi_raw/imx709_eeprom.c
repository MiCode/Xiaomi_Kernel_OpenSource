...
#define IMX709_I2C_SPEED       100
#define IMX709_MAX_OFFSET      0x8000
	...
	kal_uint16 addr, BYTE *data)
{
	...
	if (adaptor_i2c_rd_u8(ctx->i2c_client,
			IMX709_EEPROM_READ_ID >> 1, addr, data) < 0) {
		return false;
	}
	return true;
}

static bool read_imx709_eeprom(struct subdrv_ctx *ctx,
...
static struct EEPROM_PDAF_INFO *get_eeprom_pdaf_info(struct subdrv_ctx *ctx)
{
	...
	if (pinfo == NULL) {
		read_imx709_eeprom(ctx, 0x1, read_data, 4);
		if (((read_data[3] << 24) |
			 (read_data[2] << 16) |
			 (read_data[1] << 8) |
...
unsigned int read_imx709_LRC(struct subdrv_ctx *ctx, BYTE *data)
{
	...
	LOG_INF("read imx709 LRC, addr = %d, size = %u\n",
		pinfo->LRC_addr, pinfo->LRC_size);

	mutex_lock(&gimx709_eeprom_mutex);
	if ((readed_size == 0) &&
		read_imx709_eeprom(ctx, pinfo->LRC_addr,
				   IMX709_LRC_data, pinfo->LRC_size)) {
		readed_size = pinfo->LRC_size;
	}
	mutex_unlock(&gimx709_eeprom_mutex);

	memcpy(data, IMX709_LRC_data, pinfo->LRC_size);
	return readed_size;
}

unsigned int read_imx709_DCC(struct subdrv_ctx *ctx, BYTE *data)
{
	static BYTE IMX709_DCC_data[DCC_SIZE] = { 0 };
	static unsigned int readed_size;
	struct EEPROM_PDAF_INFO *pinfo = get_eeprom_pdaf_info(ctx);

	LOG_INF("read imx709 DCC, addr = %d, size = %u\n",
		pinfo->DCC_addr, pinfo->DCC_size);

	mutex_lock(&gimx709_eeprom_mutex);
	if ((readed_size == 0) &&
		read_imx709_eeprom(ctx, pinfo->DCC_addr,
				   IMX709_DCC_data, pinfo->DCC_size)) {
		readed_size = pinfo->DCC_size;
	}
	mutex_unlock(&gimx709_eeprom_mutex);

	memcpy(data, IMX709_DCC_data, pinfo->DCC_size);
	return readed_size;
}
#endif
