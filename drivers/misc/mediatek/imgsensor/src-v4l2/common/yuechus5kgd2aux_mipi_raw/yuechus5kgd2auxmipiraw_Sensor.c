 ...
 *============================================================================
 ****************************************************************************/
#define PFX "YUECHUS5KGD2AUX_camera_sensor"
...
#include "yuechus5kgd2aux_ana_gain_table.h"
#include "yuechus5kgd2auxmipiraw_Sensor.h"
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	...
	{ 6560, 4928,  16, 608, 6528, 3712, 1632,  928,    0,    0, 1632,  928, 0, 0, 1632,  928}, // hs_video as custom2 
	...
	{ 6560, 4928,  16, 608, 6528, 3712, 1632,  928,    0,    0, 1632,  928, 0, 0, 1632,  928}, // custom2 
	{ 6560, 4928,   0,   0, 6560, 4928, 3280, 2464,    0,    0, 3280, 2464, 0, 0, 3280, 2464}, // custom3 as Preview
	{ 6560, 4928, 720, 550, 5120, 3848, 2560, 1924,    0,    2, 2560, 1920, 0, 0, 2560, 1920}, // custom4
	{ 6560, 4928, 256, 192, 6048, 4544, 3024, 2272,    0,    0, 3024, 2272, 0, 0, 3024, 2272}, // custom5
};

static void set_dummy(struct subdrv_ctx *ctx)
{
	S5KGD2AUX_LOG_DBG("dummyline = %d, dummypixels = %d \n", ctx->dummy_line, ctx->dummy_pixel);
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	S5KGD2AUX_LOG_DBG("realtime_fps =%d\n", realtime_fps);
	S5KGD2AUX_LOG_DBG("shutter =%d, framelength =%d\n",
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	S5KGD2AUX_LOG_INF("Framelength: set=%d/input=%d/min=%d, auto_extend=%d\n",
				     ...
				     kal_bool auto_extend_en)
{
	...
	write_cmos_sensor_8(ctx, 0x0202, (shutter >> 8) & 0xFF);
	write_cmos_sensor_8(ctx, 0x0203, shutter & 0xFF);
	S5KGD2AUX_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	if (gain < min_gain || gain > max_gain) {
		S5KGD2AUX_LOG_INF("Error max gain setting: %d Should between %d & %d\n",
	...
	S5KGD2AUX_LOG_DBG("ctx->sensor_mode: %d",ctx->sensor_mode);
	S5KGD2AUX_LOG_DBG("gain = %d, ctx->gain = 0x%x ,reg_gain = 0x%x, max_gain:0x%x\n ",
...
static kal_uint32 yuechus5kgd2aux_awb_gain(struct subdrv_ctx *ctx,
...
static void yuechus5kgd2aux_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
static kal_uint32 streaming_control(struct subdrv_ctx *ctx, kal_bool enable)
{
	...
	S5KGD2AUX_LOG_INF("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	if (enable) {
...
static kal_uint16 yuechus5kgd2aux_fast_write_cmos_sensor(struct subdrv_ctx *ctx,
	...
	while (len > IDX) {
		...
		if (need_write) {
			if (table_len > 0) {
				...
				yuechus5kgd2aux_table_write_cmos_sensor(ctx, para + table_pos_sta, table_len);
				S5KGD2AUX_LOG_INF("table_pos_sta = %d, table_len = %d\n", table_pos_sta, table_len);
				table_len = 0;
			}
			if (burst_len > 0) {
				/* Find the sequence that requires Burst Mode */
				yuechus5kgd2aux_burst_write_cmos_sensor(ctx, para + burst_pos_sta, burst_len);
				S5KGD2AUX_LOG_INF("burst_pos_sta = %d, burst_len = %d\n", burst_pos_sta, burst_len);
				burst_len = 0;
			}
			need_write = 0;
		}
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	yuechus5kgd2aux_fast_write_cmos_sensor(ctx, yuechus5kgd2aux_init_setting,
		sizeof(yuechus5kgd2aux_init_setting) / sizeof(kal_uint16));
...
static void preview_setting(struct subdrv_ctx *ctx)
{
	...
	yuechus5kgd2aux_table_write_cmos_sensor(ctx, yuechus5kgd2aux_preview_setting,
			sizeof(yuechus5kgd2aux_preview_setting) / sizeof(kal_uint16));
...
{
	S5KGD2AUX_LOG_INF("+\n");
	yuechus5kgd2aux_table_write_cmos_sensor(ctx, yuechus5kgd2aux_normal_video_setting,
		sizeof(yuechus5kgd2aux_normal_video_setting)/sizeof(kal_uint16));
	S5KGD2AUX_LOG_INF("-\n");
}
...
{
	S5KGD2AUX_LOG_INF("+\n");
	yuechus5kgd2aux_table_write_cmos_sensor(ctx, yuechus5kgd2aux_custom2_setting,
			sizeof(yuechus5kgd2aux_custom2_setting)/sizeof(kal_uint16));
	S5KGD2AUX_LOG_INF("-\n");
}
...
{
	S5KGD2AUX_LOG_INF("+\n");
	yuechus5kgd2aux_table_write_cmos_sensor(ctx, yuechus5kgd2aux_custom1_setting,
		sizeof(yuechus5kgd2aux_custom1_setting)/sizeof(kal_uint16));
	S5KGD2AUX_LOG_INF("-\n");
...
{
	S5KGD2AUX_LOG_INF("+\n");
	yuechus5kgd2aux_table_write_cmos_sensor(ctx, yuechus5kgd2aux_custom4_setting,
		sizeof(yuechus5kgd2aux_custom4_setting) / sizeof(kal_uint16));
	S5KGD2AUX_LOG_INF("-\n");
}
...
{
	S5KGD2AUX_LOG_INF("+\n");
	yuechus5kgd2aux_table_write_cmos_sensor(ctx, yuechus5kgd2aux_custom5_setting,
		sizeof(yuechus5kgd2aux_custom5_setting) / sizeof(kal_uint16));
	S5KGD2AUX_LOG_INF("-\n");
}
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	S5KGD2AUX_LOG_INF("[%s] sensor_id: 0x%x", __func__, sensor_id);
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = (return_sensor_id(ctx)+1);

			if (*sensor_id == imgsensor_info.sensor_id) {
				S5KGD2AUX_LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			S5KGD2AUX_LOG_INF("Read sensor id fail, id: 0x%x\n",
				ctx->i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
...
static int get_info(struct subdrv_ctx *ctx,
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KGD2AUX_LOG_INF("scenario_id = %d\n", scenario_id);
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM5:
		...
		break;
	default:
		S5KGD2AUX_LOG_INF("Error ScenarioId setting");
		preview(ctx, image_window, sensor_config_data);
		...
		kal_bool enable, UINT16 framerate)
{
	if (enable) /*enable auto flicker*/ {
		...
		S5KGD2AUX_LOG_DBG("enable! fps = %d", framerate);
	} else {
		 /*Cancel Auto flick*/
		ctx->autoflicker_en = KAL_FALSE;
	}
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	S5KGD2AUX_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	default:  /*coding with  preview scenario by default*/
		...
		if (ctx->frame_length > ctx->shutter)
			set_dummy(ctx);
		S5KGD2AUX_LOG_INF("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
...
static kal_uint32 get_sensor_temperature(struct subdrv_ctx *ctx)
{
	...
	S5KGD2AUX_LOG_INF("temp_c(%d), read_reg(%d)\n", temperature_convert, temperature);
...
static int vsync_notify(struct subdrv_ctx *ctx, unsigned int sof_cnt)
{
	...
	S5KGD2AUX_LOG_INF("sensormode(%d) sof_cnt(%d) sensor_output_cnt(%d)\n",
		...
		UINT32 *feature_para_len)
{
	...
	//S5KGD2AUX_LOG_INF("feature_id = %d\n", feature_id);
	...
	case SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO:
		...
		S5KGD2AUX_LOG_INF("SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO get:%d\n",*(feature_data + 1));
	break;
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(yuechus5kgd2aux_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)yuechus5kgd2aux_ana_gain_table,
			sizeof(yuechus5kgd2aux_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		S5KGD2AUX_LOG_INF("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		S5KGD2AUX_LOG_INF("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		S5KGD2AUX_LOG_INF("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		S5KGD2AUX_LOG_INF("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		S5KGD2AUX_LOG_INF("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		S5KGD2AUX_LOG_INF("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(ctx, KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		S5KGD2AUX_LOG_INF("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(ctx, *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		S5KGD2AUX_LOG_INF("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
...
break;
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		yuechus5kgd2aux_set_lsc_reg_setting(ctx, index, feature_data_16,
...
const struct subdrv_entry yuechus5kgd2aux_mipi_raw_entry = {
	.name = "yuechus5kgd2aux_mipi_raw",
	.id = YUECHUS5KGD2AUX_SENSOR_ID,
	.pw_seq = pw_seq,
