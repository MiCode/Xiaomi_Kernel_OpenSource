 ...
 *============================================================================
 ****************************************************************************/
#define PFX "COROTOV50D_camera_sensor"
...
#include "corotov50d_ana_gain_table.h"
#include "corotov50d_Sensor_setting.h"
#include "corotov50dmipiraw_Sensor.h"
...
#define corotov50d_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
#define corotov50d_burst_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8_burst(__VA_ARGS__)
#define LOG_TAG "[corotov50d]"
...
static void write_pdc_data_to_sensor(struct subdrv_ctx *ctx)
{
	...
	corotov50d_burst_write_cmos_sensor(ctx,pdc_cali_setting,
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = COROTOV50D_SENSOR_ID,
	...
	.custom2 = { //bokeh, LEICA Vivid
		.pclk = 100000000,
		.linelength = 425,
		.framelength = 9800, //7840 * 30 / 24
		.startx = 0,
		.starty = 0,
		.grabwindow_width = 4096,
		.grabwindow_height = 3072,
	...
	.max_gain     = 62 * BASEGAIN,	/* only preview setting suppot 62x, other settings supoort 15.5x */
	.min_gain_iso = 50,
	.exp_step     = 2,
	.gain_step    = 4,
	.gain_type    = 1,
...
static void COROTOV50D_get_pdaf_reg_setting(struct subdrv_ctx *ctx, MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void COROTOV50D_set_pdaf_reg_setting(struct subdrv_ctx *ctx,
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	OV50D_LOG_INF("ctx->sensor_mode: %d, gain = %d, ctx->gain = 0x%x ,reg_gain = 0x%x, max_gain:0x%x, r3646_value = 0x%x\n ",
...
static kal_uint32 corotov50d_awb_gain(struct subdrv_ctx *ctx,
...
static void corotov50d_set_lsc_reg_setting(struct subdrv_ctx *ctx,
 ...
 *
 * DESCRIPTION
 *	This function is to set mirror or flip status of sensor
 *
 * PARAMETERS
 *	image_mirror: IMAGE_NORMAL IMAGE_V_MIRROR IMAGE_H_MIRROR IMAGE_HV_MIRROR
 *
 * RETURNS
 *	None
 *
...
static void set_mirror_flip(struct subdrv_ctx *ctx, kal_uint8 image_mirror)
{
	kal_uint8 itemp,filp;

	OV50D_LOG_INF("image_mirror = %d\n", image_mirror);

	itemp = read_cmos_sensor_8(ctx, 0x3821);
    filp = read_cmos_sensor_8(ctx, 0x3820);

	//0x3821 bit[2] mirror 0x 3820 bit[2] Vfilp
	OV50D_LOG_INF("itemp 0x3821= %x, 0x3820= 0X%x\n", itemp,read_cmos_sensor_8(ctx, 0x3820));

	switch (image_mirror) {

	case IMAGE_NORMAL:
	write_cmos_sensor_8(ctx, 0x3821, itemp);
	write_cmos_sensor_8(ctx, 0x3820, filp);
	break;

	case IMAGE_V_MIRROR:
	write_cmos_sensor_8(ctx, 0x3820, filp|0x04);
	break;

	case IMAGE_H_MIRROR:
	write_cmos_sensor_8(ctx, 0x3821, itemp|0x04);
	break;
	...
	OV50D_LOG_INF("itemp 0x3821= 0x%x, 0x3820= 0x%x end\n", read_cmos_sensor_8(ctx, 0x3821),read_cmos_sensor_8(ctx, 0x3820));
}
...
static kal_uint32 streaming_control(struct subdrv_ctx *ctx, kal_bool enable)
{
	...
	set_mirror_flip(ctx, IMAGE_H_MIRROR);
	if (enable) {
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	corotov50d_table_write_cmos_sensor(ctx, corotov50d_init_setting,
		sizeof(corotov50d_init_setting) / sizeof(kal_uint16));
		//mDELAY(5);
...
static void preview_setting(struct subdrv_ctx *ctx)
{
	OV50D_LOG_INF("+\n");
	//mDELAY(10);
	corotov50d_table_write_cmos_sensor(ctx, corotov50d_preview_setting,
			sizeof(corotov50d_preview_setting) / sizeof(kal_uint16));
	mDELAY(5);
	OV50D_LOG_INF("-\n");
} /* preview_setting */
...
static void normal_video_setting(struct subdrv_ctx *ctx, kal_uint16 currefps)
{
	OV50D_LOG_INF("+\n");
	//preview_setting(ctx);
	 corotov50d_table_write_cmos_sensor(ctx, corotov50d_normal_video_setting,
	 	sizeof(corotov50d_normal_video_setting)/sizeof(kal_uint16));
	OV50D_LOG_INF("-\n");
}
...
{
	OV50D_LOG_INF("+\n");
	corotov50d_table_write_cmos_sensor(ctx, corotov50d_custom1_setting,
	 		sizeof(corotov50d_custom1_setting) / sizeof(kal_uint16));
	OV50D_LOG_INF("-\n");
}
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	OV50D_LOG_INF("[%s] 0x300B=0x%x,0x300C=0x%Xsensor_id: 0x%x", __func__,
		read_cmos_sensor_8(ctx, 0x300B),read_cmos_sensor_8(ctx, 0x300C), sensor_id);
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM4:
		...
		break;
	default:
		OV50D_LOG_INF("Error ScenarioId setting");
		preview(ctx, image_window, sensor_config_data);
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	OV50D_LOG_INF("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	default:  /*coding with  preview scenario by default*/
		...
		if (ctx->frame_length > ctx->shutter)
			set_dummy(ctx);
		OV50D_LOG_INF("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
...
{
	OV50D_LOG_INF("enable: %d\n", enable);

	if (enable) {
		write_cmos_sensor_8(ctx, 0x50c1, 0x10);
	} else {
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO:
		...
		OV50D_LOG_INF("SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO get:%d\n",*(feature_data + 1));
	break;
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(corotov50d_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)corotov50d_ana_gain_table,
			sizeof(corotov50d_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		OV50D_LOG_INF("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		OV50D_LOG_INF("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		OV50D_LOG_INF("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	(struct SENSOR_WINSIZE_INFO_STRUCT *)(uintptr_t)(*(feature_data+1));
		...
		case SENSOR_FEATURE_GET_PDAF_INFO:
			...
			case SENSOR_SCENARIO_ID_HIGHSPEED_VIDEO:
			case SENSOR_SCENARIO_ID_CUSTOM2:
			case SENSOR_SCENARIO_ID_CUSTOM3:       //4096*3072
				memcpy((void *)PDAFinfo, (void *)&imgsensor_pd_info[0], sizeof(struct SET_PD_BLOCK_INFO_T));
				break;
			case SENSOR_SCENARIO_ID_NORMAL_VIDEO:  //4096*2304
			case SENSOR_SCENARIO_ID_CUSTOM1:
		...
		case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
			OV50D_LOG_DBG("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
				(*feature_para_len));
			COROTOV50D_get_pdaf_reg_setting(ctx,
		...
		case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
			OV50D_LOG_DBG("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d",
				(*feature_para_len));
			COROTOV50D_set_pdaf_reg_setting(ctx,
					(*feature_para_len) / sizeof(UINT32),
					feature_data_16);
			break;
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		OV50D_LOG_INF("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		OV50D_LOG_INF("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		OV50D_LOG_INF("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(ctx, KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		OV50D_LOG_INF("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(ctx, *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		OV50D_LOG_INF("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
...
break;
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		corotov50d_set_lsc_reg_setting(ctx, index, feature_data_16,
...
const struct subdrv_entry corotov50d_mipi_raw_entry = {
	.name = "corotov50d_mipi_raw",
	.id = COROTOV50D_SENSOR_ID,
	.pw_seq = pw_seq,
