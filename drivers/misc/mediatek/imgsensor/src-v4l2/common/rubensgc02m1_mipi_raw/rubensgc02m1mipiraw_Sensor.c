 ...
 *============================================================================
 ****************************************************************************/
#define PFX "RUBENSGC02M1_camera_sensor"
...
#include "rubensgc02m1_ana_gain_table.h"
#include "rubensgc02m1mipiraw_Sensor.h"
...
#define rubensgc02m1_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8_u8(__VA_ARGS__)
#define LOG_TAG "[rubensgc02m1]"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = RUBENSGC02M1_SENSOR_ID,
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	GC02M1_LOG_INF("Framelength: set=%d/input=%d/min=%d\n",
...
static kal_uint32 rubensgc02m1_awb_gain(struct subdrv_ctx *ctx,
...
static void rubensgc02m1_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
static void sensor_init(struct subdrv_ctx *ctx)
{
    ...
    // Global
	rubensgc02m1_table_write_cmos_sensor(ctx, rubensgc02m1_init_setting,
		sizeof(rubensgc02m1_init_setting) / sizeof(kal_uint16));
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM6:
		...
		break;
	default:
		GC02M1_LOG_INF("Error ScenarioId setting");
		preview(ctx, image_window, sensor_config_data);
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	default:  /*coding with  preview scenario by default*/
		...
		if (ctx->frame_length > ctx->shutter)
			set_dummy(ctx);
		GC02M1_LOG_INF("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(rubensgc02m1_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)rubensgc02m1_ana_gain_table,
			sizeof(rubensgc02m1_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		GC02M1_LOG_INF("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		GC02M1_LOG_INF("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		GC02M1_LOG_INF("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		GC02M1_LOG_INF("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		GC02M1_LOG_INF("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		GC02M1_LOG_INF("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(ctx, KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		GC02M1_LOG_INF("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(ctx, *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		GC02M1_LOG_INF("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		rubensgc02m1_set_lsc_reg_setting(ctx, index, feature_data_16,
...
const struct subdrv_entry rubensgc02m1_mipi_raw_entry = {
	.name = "rubensgc02m1_mipi_raw",
	.id = RUBENSGC02M1_SENSOR_ID,
	.pw_seq = pw_seq,
