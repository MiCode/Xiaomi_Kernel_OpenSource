 ...
 *============================================================================
 ****************************************************************************/
#define PFX "DAUMIEROV32B40_camera_sensor"
...
#include "daumierov32b40_ana_gain_table.h"
#include "daumierov32b40mipiraw_Sensor.h"
//#include "daumierov32b40_seamless_switch.h"
#include "daumierov32b40_eeprom.h"
...
//#define daumierov32b40_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u16(__VA_ARGS__)
#define daumierov32b40_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
#define daumierov32b40_burst_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8_burst(__VA_ARGS__)
#define LOG_TAG "[daumierov32b40]"
#define OV32B40_LOG_INF(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
#define OV32B40_LOG_DBG(format, args...) pr_debug(LOG_TAG "[%s] " format, __func__, ##args)
...
#define GROUP_HOLD_ADDR 0x3208



static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = DAUMIEROV32B40_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[7] = {
	...
	{ 6528, 4896,    0,     0,  3264, 2448,  1632, 1224,    0,    152,  1632, 920,    0,    0,  1632, 920}, // high speed video 120ps
	{ 6528, 4896,    0,     0,  6528, 4896,  3264, 2448,    0,    0,  3264, 2448,    0,    0,  3264, 2448}, // slim video same as preview
	{ 6528, 4896,    0,     0,  6528, 4896,  3264, 2448,    0,    304,  3264, 1840,    0,    0,  3264, 1840}, // custom1 stere0 as Preview
	{ 6528, 4896,    0,     0,  6528, 4896,  3264, 2448,    408,    0,  2448, 2448,    0,    0,  2448, 2448}, // custom2 1:1
};
...
{
	OV32B40_LOG_DBG("dummyline = %d, dummypixels = %d \n", ctx->dummy_line, ctx->dummy_pixel);
...
static void set_mirror_flip(struct subdrv_ctx *ctx, kal_uint8 image_mirror)
{
	...
	OV32B40_LOG_DBG("image_mirror = %d\n", image_mirror);
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305)
			...
			write_cmos_sensor_8(ctx, 0x380e, ctx->frame_length >> 8);
			write_cmos_sensor_8(ctx, 0x380f, ctx->frame_length & 0xFF);
			write_cmos_sensor_8(ctx, GROUP_HOLD_ADDR, 0x10);
			write_cmos_sensor_8(ctx, GROUP_HOLD_ADDR, 0xa0);
		}
	}
	...
	write_cmos_sensor_8(ctx, GROUP_HOLD_ADDR, 0x11);
	write_cmos_sensor_8(ctx, GROUP_HOLD_ADDR, 0xa1);
	OV32B40_LOG_DBG("shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	OV32B40_LOG_INF("Framelength: set=%d/input=%d/min=%d, auto_extend=%d\n",
				     ...
				     kal_bool auto_extend_en)
{
	...
	write_cmos_sensor_8(ctx, GROUP_HOLD_ADDR, 0x11);
	write_cmos_sensor_8(ctx, GROUP_HOLD_ADDR, 0xa1);
	OV32B40_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static kal_uint32 set_gain(struct subdrv_ctx *ctx, kal_uint32 gain)
{
	...
	if (gain < min_gain || gain > max_gain) {
		OV32B40_LOG_INF("Error max gain setting: %d Should between %d & %d\n",
	...
	ctx->gain = reg_gain;
	OV32B40_LOG_DBG("gain = %d, reg_gain = 0x%x, max_gain:0x%x\n ",
...
static kal_uint32 daumierov32b40_awb_gain(struct subdrv_ctx *ctx,
		struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	return ERROR_NONE;
}

static void daumierov32b40_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
{
	OV32B40_LOG_INF("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	if (enable)
...
static kal_uint16 daumierov32b40_fast_write_cmos_sensor(struct subdrv_ctx *ctx,
                                 kal_uint16 *para, kal_uint32 len)
{
	kal_uint32 burst_thr     = 100;
	kal_uint32 IDX           = 2;
	kal_uint16 addr          = 0;
	kal_uint32 table_pos_sta = 2;
	kal_uint32 table_len     = 0;
	kal_uint32 burst_pos_sta = 2;
	kal_uint32 burst_len     = 0;
	kal_uint32 need_write    = 0;
	OV32B40_LOG_INF("len = %d +\n", len);
	...
	while (len > IDX) {
		if (len == (IDX + 2)){
			...
			addr = para[IDX];
			if (((addr + 1) == para[IDX + 2]) || ((addr - 1) == para[IDX - 2])) {
				if (burst_len == 0) {
					burst_pos_sta = IDX;
				}
				burst_len += 2;
		...
		if (need_write) {
			if (table_len > 0) {
				...
				daumierov32b40_table_write_cmos_sensor(ctx, para + table_pos_sta, table_len);
				OV32B40_LOG_INF("table_pos_sta = %d, table_len = %d\n", table_pos_sta, table_len);
				table_len = 0;
			}

			if (burst_len > 0) {
				/* Find the sequence that requires Burst Mode */
				daumierov32b40_burst_write_cmos_sensor(ctx, para + burst_pos_sta, burst_len);
				OV32B40_LOG_INF("burst_pos_sta = %d, burst_len = %d\n", burst_pos_sta, burst_len);
				burst_len = 0;
			}
			need_write = 0;
		}
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	daumierov32b40_fast_write_cmos_sensor(ctx, daumierov32b40_init_setting,
		sizeof(daumierov32b40_init_setting) / sizeof(kal_uint16));
...
{
	OV32B40_LOG_DBG("+\n");
	daumierov32b40_table_write_cmos_sensor(ctx, daumierov32b40_preview_setting,
		sizeof(daumierov32b40_preview_setting) / sizeof(kal_uint16));
	OV32B40_LOG_DBG("-\n");
} /* preview_setting */
...
{
	OV32B40_LOG_DBG("+\n");
	daumierov32b40_table_write_cmos_sensor(ctx, daumierov32b40_normal_video_setting,
		sizeof(daumierov32b40_normal_video_setting)/sizeof(kal_uint16));
	OV32B40_LOG_DBG("-\n");
}
...
{
	OV32B40_LOG_DBG("+\n");
	daumierov32b40_table_write_cmos_sensor(ctx, daumierov32b40_hs_video_setting,
		sizeof(daumierov32b40_hs_video_setting)/sizeof(kal_uint16));
	OV32B40_LOG_DBG("-\n");
}
...
{
	OV32B40_LOG_DBG("+\n");
	daumierov32b40_table_write_cmos_sensor(ctx, daumierov32b40_custom1_setting,
		sizeof(daumierov32b40_custom1_setting)/sizeof(kal_uint16));
	OV32B40_LOG_DBG("-\n");
}
...
{
	OV32B40_LOG_DBG("+\n");
	daumierov32b40_table_write_cmos_sensor(ctx, daumierov32b40_custom2_setting,
		sizeof(daumierov32b40_custom2_setting)/sizeof(kal_uint16));
	OV32B40_LOG_DBG("-\n");
}
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	OV32B40_LOG_INF("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		do {
			*sensor_id = return_sensor_id(ctx);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				OV32B40_LOG_DBG("i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			OV32B40_LOG_INF("Read sensor id fail, id: 0x%x\n",
				ctx->i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM2:
		...
		break;
	default:
		OV32B40_LOG_INF("Error ScenarioId setting");
		preview(ctx, image_window, sensor_config_data);
		...
		kal_bool enable, UINT16 framerate)
{
	if (enable) /*enable auto flicker*/ {
		...
		OV32B40_LOG_DBG("enable! fps = %d", framerate);
	} else {
		 /*Cancel Auto flick*/
		ctx->autoflicker_en = KAL_FALSE;
	}
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	OV32B40_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	default:  /*coding with  preview scenario by default*/
		...
		if (ctx->frame_length > ctx->shutter)
			set_dummy(ctx);
		OV32B40_LOG_INF("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
...
static kal_uint32 get_sensor_temperature(struct subdrv_ctx *ctx)
{
	...
	OV32B40_LOG_DBG("temp_c(%d), read_reg(%d)\n", temperature_convert, temperature);
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO:
		...
		OV32B40_LOG_DBG("SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO get:%d\n",*(feature_data + 1));
	break;
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(daumierov32b40_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)daumierov32b40_ana_gain_table,
			sizeof(daumierov32b40_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		OV32B40_LOG_DBG("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		OV32B40_LOG_DBG("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		OV32B40_LOG_DBG("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		OV32B40_LOG_DBG("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		OV32B40_LOG_INF("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		OV32B40_LOG_INF("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(ctx, KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		OV32B40_LOG_INF("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(ctx, *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		OV32B40_LOG_INF("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
...
break;
	...
	case SENSOR_FEATURE_GET_VC_INFO:
		...
		case SENSOR_SCENARIO_ID_CUSTOM15:
		default:
			OV32B40_LOG_INF("error: get wrong vc_INFO id = %d",
			*feature_data_32);
			break;
		}
	break;
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		daumierov32b40_set_lsc_reg_setting(ctx, index, feature_data_16,
...
static struct mtk_mbus_frame_desc_entry frame_desc_cust2[] = { //type1 need be checked
	{
		.bus.csi2 = {
			.channel = 0,
			.data_type = 0x2b,
...
const struct subdrv_entry daumierov32b40_mipi_raw_entry = {
	.name = "daumierov32b40_mipi_raw",
	.id = DAUMIEROV32B40_SENSOR_ID,
	.pw_seq = pw_seq,
