 ...
 *============================================================================
 ****************************************************************************/
#define PFX "XAGAOV16A1_camera_sensor"
...
#include "xagaov16a1_ana_gain_table.h"
#include "xagaov16a1mipiraw_Sensor.h"
//#include "xagaov16a1_seamless_switch.h"
...
//#define xagaov16a1_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u16(__VA_ARGS__)
#define xagaov16a1_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
#define LOG_TAG "[xagaov16a1]"
#define OV16A1_LOG_INF(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
#define OV16A1_LOG_DBG(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = XAGAOV16A1_SENSOR_ID,
...
{
	OV16A1_LOG_DBG("dummyline = %d, dummypixels = %d \n", ctx->dummy_line, ctx->dummy_pixel);
	write_cmos_sensor_8(ctx, 0x380e, (ctx->frame_length >> 8) & 0x7F);
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	write_cmos_sensor_8(ctx, 0x3500, (shutter >> 16) & 0x7F);
	write_cmos_sensor_8(ctx, 0x3501, (shutter >>  8) & 0xFF);
	write_cmos_sensor_8(ctx, 0x3502,  shutter  & 0xFE);
	write_cmos_sensor_8(ctx, 0x3208, 0x11);
	write_cmos_sensor_8(ctx, 0x3208, 0xa1);
	OV16A1_LOG_DBG("shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	OV16A1_LOG_DBG("Framelength: set=%d/input=%d/min=%d, auto_extend=%d\n",
				     ...
				     kal_bool auto_extend_en)
{
	...
	write_cmos_sensor_8(ctx, 0x3208, 0x11);
	write_cmos_sensor_8(ctx, 0x3208, 0xa1);
	OV16A1_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	if (gain < min_gain || gain > max_gain) {
		OV16A1_LOG_DBG("Error max gain setting: %d Should between %d & %d\n",
	...
	ctx->gain = reg_gain;
	OV16A1_LOG_DBG("gain = %d, reg_gain = 0x%x, max_gain:0x%x\n ",
		gain, reg_gain, max_gain);

	write_cmos_sensor_8(ctx, 0x3508, (reg_gain>>8) & 0x7F);
	write_cmos_sensor_8(ctx, 0x3509,  reg_gain & 0xFF);

	return gain;
} /* set_gain */

static kal_uint32 xagaov16a1_awb_gain(struct subdrv_ctx *ctx,
		struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	return ERROR_NONE;
}

static void xagaov16a1_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
{
	OV16A1_LOG_DBG("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	if (enable)
...
{
	OV16A1_LOG_DBG("+\n");
	xagaov16a1_table_write_cmos_sensor(ctx, xagaov16a1_init_setting,
		sizeof(xagaov16a1_init_setting)/sizeof(kal_uint16));
...
{
	OV16A1_LOG_DBG("+\n");
	xagaov16a1_table_write_cmos_sensor(ctx, xagaov16a1_preview_setting,
		sizeof(xagaov16a1_preview_setting) / sizeof(kal_uint16));
	OV16A1_LOG_DBG("-\n");
} /* preview_setting */
...
{
	OV16A1_LOG_DBG("+\n");
	xagaov16a1_table_write_cmos_sensor(ctx, xagaov16a1_hs_video_setting,
		sizeof(xagaov16a1_hs_video_setting)/sizeof(kal_uint16));
	OV16A1_LOG_DBG("-\n");
}
...
{
	OV16A1_LOG_DBG("+\n");
	xagaov16a1_table_write_cmos_sensor(ctx, xagaov16a1_custom1_setting,
		sizeof(xagaov16a1_custom1_setting)/sizeof(kal_uint16));
	OV16A1_LOG_DBG("-\n");
}
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	OV16A1_LOG_DBG("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		do {
			*sensor_id = return_sensor_id(ctx);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				OV16A1_LOG_DBG("i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			OV16A1_LOG_DBG("Read sensor id fail, id: 0x%x\n",
				ctx->i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM1:
		...
		break;
	default:
		OV16A1_LOG_DBG("Error ScenarioId setting");
		preview(ctx, image_window, sensor_config_data);
		...
		kal_bool enable, UINT16 framerate)
{
	if (enable) /*enable auto flicker*/ {
		...
		OV16A1_LOG_DBG("enable! fps = %d", framerate);
	} else {
		 /*Cancel Auto flick*/
		ctx->autoflicker_en = KAL_FALSE;
	}
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	OV16A1_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	default:  /*coding with  preview scenario by default*/
		...
		if (ctx->frame_length > ctx->shutter)
			set_dummy(ctx);
		OV16A1_LOG_DBG("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
...
static kal_uint32 get_sensor_temperature(struct subdrv_ctx *ctx)
{
	...
	*****************************************/
	...
	OV16A1_LOG_DBG("temp_c(%d), read_reg(%d)\n", temperature_convert, temperature);
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(xagaov16a1_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)xagaov16a1_ana_gain_table,
			sizeof(xagaov16a1_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		OV16A1_LOG_DBG("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		OV16A1_LOG_DBG("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		OV16A1_LOG_DBG("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		OV16A1_LOG_DBG("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		OV16A1_LOG_DBG("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		OV16A1_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(ctx, KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		OV16A1_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(ctx, *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		OV16A1_LOG_DBG("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
...
break;
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		xagaov16a1_set_lsc_reg_setting(ctx, index, feature_data_16,
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	{HW_ID_AVDD,  2900000, 2},
	{HW_ID_MCLK,  24,      2},
	{HW_ID_MCLK_DRIVING_CURRENT, 4, 1},
	{HW_ID_RST,   1,       5},
};

const struct subdrv_entry xagaov16a1_mipi_raw_entry = {
	.name = "xagaov16a1_mipi_raw",
	.id = XAGAOV16A1_SENSOR_ID,
	.pw_seq = pw_seq,
