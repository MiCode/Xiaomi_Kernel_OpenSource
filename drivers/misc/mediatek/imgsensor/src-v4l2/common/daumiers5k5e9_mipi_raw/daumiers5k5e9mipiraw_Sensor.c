 ...
 *============================================================================
 ****************************************************************************/
#define PFX "DAUMIERS5K5E9_camera_sensor"
...
#include "daumiers5k5e9_ana_gain_table.h"
#include "daumiers5k5e9mipiraw_Sensor.h"
...
#define daumiers5k5e9_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
#define LOG_TAG "[daumiers5k5e9]"
#define S5K5E9_LOG_INF(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
#define S5K5E9_LOG_DBG(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
...
#define S5K5E9SUNNY_SENSOR_GAIN_BASE             0x400
#define S5K5E9SUNNY_SENSOR_GAIN_MAX              (16 * S5K5E9SUNNY_SENSOR_GAIN_BASE)
#define S5K5E9SUNNY_SENSOR_GAIN_MAX_VALID_INDEX  16
#define S5K5E9SUNNY_SENSOR_GAIN_MAP_SIZE         16
#define S5K5E9SUNNY_SENSOR_DGAIN_BASE            0x400
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[] = {
	{
		...
		0x00, 0x2b, 2592, 1944, 0x00, 0x00, 0x0000, 0x0000,
	},
	{
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
		0x00, 0x2b, 2592, 1464, 0x00, 0x00, 0x0000, 0x0000,
	},
};
...
{
	S5K5E9_LOG_DBG("dummyline = %d, dummypixels = %d \n", ctx->dummy_line, ctx->dummy_pixel);
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	write_cmos_sensor_8(ctx, 0x0340, ctx->frame_length >> 8);
	write_cmos_sensor_8(ctx, 0x0341, ctx->frame_length & 0xFF);
	S5K5E9_LOG_DBG("shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
}	/*	write_shutter  */
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	S5K5E9_LOG_INF("Framelength: set=%d/input=%d/min=%d\n",
				     ...
				     kal_bool auto_extend_en)
{
	...
	write_cmos_sensor_8(ctx, 0x0342, ctx->line_length >> 8);
	write_cmos_sensor_8(ctx, 0x0343, ctx->line_length & 0xFF);
	S5K5E9_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	if (gain < imgsensor_info.min_gain || gain > imgsensor_info.max_gain) {
		S5K5E9_LOG_DBG("Error gain setting");
		if (gain < S5K5E9SUNNY_SENSOR_GAIN_BASE)
		gain = S5K5E9SUNNY_SENSOR_GAIN_BASE;
	else if (gain > S5K5E9SUNNY_SENSOR_GAIN_MAX)
		gain = S5K5E9SUNNY_SENSOR_GAIN_MAX;
	}
	...
	S5K5E9_LOG_DBG("gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);

	return reg_gain;

} /* set_gain */

static kal_uint32 daumiers5k5e9_awb_gain(struct subdrv_ctx *ctx,
		struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
    ...
    S5K5E9_LOG_DBG("write awb gain r:g:b %d:%d:%d \n", r_Gain, g_Gain, b_Gain);

	return ERROR_NONE;
}


static void daumiers5k5e9_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
{
	S5K5E9_LOG_INF("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	if (enable)
...
{
	S5K5E9_LOG_INF("+\n");
	write_cmos_sensor_8(ctx, 0x0000, 0X05);
	write_cmos_sensor_8(ctx, 0x0000, 0X55);
	write_cmos_sensor_8(ctx, 0x0A02, 0X3F);
	// Global
	daumiers5k5e9_table_write_cmos_sensor(ctx, daumiers5k5e9_init_setting,
		sizeof(daumiers5k5e9_init_setting) / sizeof(kal_uint16));
	S5K5E9_LOG_INF("-\n");
}	/*	  sensor_init  */
...
{
	S5K5E9_LOG_INF("+\n");
	daumiers5k5e9_table_write_cmos_sensor(ctx, daumiers5k5e9_preview_setting,
		sizeof(daumiers5k5e9_preview_setting) / sizeof(kal_uint16));
	S5K5E9_LOG_INF("-\n");
} /* preview_setting */
...
{
	S5K5E9_LOG_INF("+\n");
	daumiers5k5e9_table_write_cmos_sensor(ctx, daumiers5k5e9_normal_video_setting,
		sizeof(daumiers5k5e9_normal_video_setting) / sizeof(kal_uint16));
	S5K5E9_LOG_INF("-\n");
}
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	S5K5E9_LOG_INF("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		do {
			*sensor_id = return_sensor_id(ctx);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				S5K5E9_LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			S5K5E9_LOG_INF("Read sensor id fail, id: 0x%x\n",
				ctx->i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM6:
		...
		break;
	default:
		S5K5E9_LOG_INF("Error ScenarioId setting");
		preview(ctx, image_window, sensor_config_data);
		...
		kal_bool enable, UINT16 framerate)
{
	if (enable) /*enable auto flicker*/ {
		...
		S5K5E9_LOG_DBG("enable! fps = %d", framerate);
	} else {
		 /*Cancel Auto flick*/
		ctx->autoflicker_en = KAL_FALSE;
	}
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	S5K5E9_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	default:  /*coding with  preview scenario by default*/
		...
		if (ctx->frame_length > ctx->shutter)
			set_dummy(ctx);
		S5K5E9_LOG_INF("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO:
		...
		S5K5E9_LOG_INF("SENSOR_FEATURE_GET_OUTPUT_FORMAT_BY_SCENARIO get:%d\n",*(feature_data + 1));
	break;
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(daumiers5k5e9_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)daumiers5k5e9_ana_gain_table,
			sizeof(daumiers5k5e9_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		S5K5E9_LOG_INF("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		S5K5E9_LOG_INF("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		S5K5E9_LOG_INF("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		S5K5E9_LOG_INF("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		S5K5E9_LOG_INF("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		S5K5E9_LOG_INF("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(ctx, KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		S5K5E9_LOG_INF("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(ctx, *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		S5K5E9_LOG_INF("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		daumiers5k5e9_set_lsc_reg_setting(ctx, index, feature_data_16,
...
const struct subdrv_entry daumiers5k5e9_mipi_raw_entry = {
	.name = "daumiers5k5e9_mipi_raw",
	.id = DAUMIERS5K5E9_SENSOR_ID,
	.pw_seq = pw_seq,
