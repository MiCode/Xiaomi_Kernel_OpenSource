...
#define imx398_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
...
static void set_dummy(struct subdrv_ctx *ctx)
{
	...
	/*
	 * you can set dummy by ctx->dummy_line and ctx->dummy_pixel,
	 * or you can set dummy by ctx->frame_length
	 * and ctx->line_length
	 */
...
{
	if (write_cmos_sensor(ctx, 0x0A02, 0x1F) < 0)
		return 0xFFFFFFFF;
	write_cmos_sensor(ctx, 0x0A00, 0x01);
	mdelay(10);
	return (
	    (read_cmos_sensor(ctx, 0x0A29) << 4) | (read_cmos_sensor(ctx, 0x0A2A) >> 4));
}
...
static kal_uint16 gain2reg(struct subdrv_ctx *ctx, const kal_uint32 gain)
{
	...
	for (iI = 0; iI < IMX398MIPI_MaxGainIndex; iI++) {
		if (gain <= IMX398MIPI_sensorGainMapping[iI][0] * 16)
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	imx398_table_write_cmos_sensor(ctx, addr_data_pair_init_imx398,
...
static void preview_setting(struct subdrv_ctx *ctx)
{
	...
	imx398_table_write_cmos_sensor(ctx, addr_data_pair_preview_imx398,
...
static void preview_setting_HDR_ES2(struct subdrv_ctx *ctx)
{
	pr_debug("Enter\n");

	imx398_table_write_cmos_sensor(ctx, addr_data_pair_preview_imx398_hdr,
...
static void capture_setting(struct subdrv_ctx *ctx, kal_uint16 currefps)
{
	...
	imx398_table_write_cmos_sensor(ctx, addr_data_pair_capture_imx398,
...
static void capture_setting_pdaf(struct subdrv_ctx *ctx, kal_uint16 currefps)
{
	pr_debug("E! currefps:%d\n", currefps);
	if (currefps == 150) {
		imx398_table_write_cmos_sensor(ctx,
...
{
	imx398_table_write_cmos_sensor(ctx, addr_data_pair_video_imx398,
...
{
	imx398_table_write_cmos_sensor(ctx, addr_data_pair_hs_video_imx398,
...
{
	imx398_table_write_cmos_sensor(ctx, addr_data_pair_slim_video_imx398,
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	ctx->autoflicker_en = KAL_FALSE;
	if (ctx->ihdr_mode == 2)
		preview_setting_HDR_ES2(ctx);	/*HDR + PDAF */
	else
		preview_setting(ctx);	/*PDAF only */
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (ctx->pdaf_mode == 1)
		capture_setting_pdaf(ctx, ctx->current_fps);	/*PDAF only */
	else
...
static kal_uint32 imx398_awb_gain(struct subdrv_ctx *ctx,
		...
		UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_VIDEO_MODE:
		set_video_mode(ctx, *feature_data_16);
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		...
		read_imx398_DCC(ctx, (kal_uint16) (*feature_data),
	...
	case SENSOR_FEATURE_SET_AWB_GAIN:
		imx398_awb_gain(ctx, pSetSensorAWB);
		break;
		/*END OF HDR CMD */
		/*PDAF CMD */
