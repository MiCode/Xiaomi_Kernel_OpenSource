...
#define OV50D_LOG_DBG(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
...
static int vsync_notify(struct subdrv_ctx *ctx, unsigned int sof_cnt)
{
	...
	OV50D_LOG_DBG("sensormode(%d) sof_cnt(%d) sensor_output_cnt(%d)\n",
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[] = {
	{/* preview/capture/slim_video/hs_video */
...
{
	OV50D_LOG_DBG("dummyline = %d, dummypixels = %d \n", ctx->dummy_line, ctx->dummy_pixel);
	group_hold(ctx, true);
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	OV50D_LOG_DBG("realtime_fps =%d\n", realtime_fps);
	OV50D_LOG_DBG("shutter =%d, framelength =%d\n",
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	OV50D_LOG_DBG("Framelength: set=%d/input=%d/min=%d\n",
				     ...
				     kal_bool auto_extend_en)
{
	...
	OV50D_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
}	/* set_shutter_frame_length */
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	if ( reg_gain >= 8 * 256){
		write_cmos_sensor_8(ctx, 0x3646, 0xb5);
		OV50D_LOG_INF("write 0xb5 in 0x3646");
	}else {
		write_cmos_sensor_8(ctx, 0x3646, 0xb7);
		OV50D_LOG_INF("write 0xb7 in 0x3646");
	}

	OV50D_LOG_DBG("ctx->sensor_mode: %d",ctx->sensor_mode);
	OV50D_LOG_DBG("gain = %d, ctx->gain = 0x%x ,reg_gain = 0x%x, max_gain:0x%x\n ",
