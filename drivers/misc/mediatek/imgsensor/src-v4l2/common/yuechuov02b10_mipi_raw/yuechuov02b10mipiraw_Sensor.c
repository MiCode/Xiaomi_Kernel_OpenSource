 ...
 *============================================================================
 ****************************************************************************/
#define PFX "YUECHUOV02B10_camera_sensor"
...
#include "yuechuov02b10_ana_gain_table.h"
#include "yuechuov02b10mipiraw_Sensor.h"
...
#define yuechuov02b10_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8_u8(__VA_ARGS__)
#define LOG_TAG "[yuechuov02b10]"
#define OV02B10_LOG_INF(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
#define OV02B10_LOG_DBG(format, args...) pr_debug(LOG_TAG "[%s] " format, __func__, ##args)
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = YUECHUOV02B10_SENSOR_ID,
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	OV02B10_LOG_DBG("Framelength: set=%d/input=%d/min=%d\n",
...
static kal_uint16 set_gain(struct subdrv_ctx *ctx, kal_uint16 gain)
{
	...
	if (gain < imgsensor_info.min_gain || gain > imgsensor_info.max_gain) {
		OV02B10_LOG_DBG("Error gain setting");
...
static kal_uint32 yuechuov02b10_awb_gain(struct subdrv_ctx *ctx,
		struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
    ...
    OV02B10_LOG_DBG("write awb gain r:g:b %d:%d:%d \n", r_Gain, g_Gain, b_Gain);

	return ERROR_NONE;
}


static void yuechuov02b10_set_lsc_reg_setting(struct subdrv_ctx *ctx,
...
{
	OV02B10_LOG_INF("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
...
static void sensor_init(struct subdrv_ctx *ctx)
{
    ...
    mdelay(5);
	yuechuov02b10_table_write_cmos_sensor(ctx, yuechuov02b10_init_setting,
		sizeof(yuechuov02b10_init_setting) / sizeof(kal_uint16));
#endif
	OV02B10_LOG_INF("-\n");
}	/*	  sensor_init  */
...
static void preview_setting(struct subdrv_ctx *ctx)
{
	...
	write_cmos_sensor_8(ctx, 0xfc, 0x01);
    mdelay(5);
	yuechuov02b10_table_write_cmos_sensor(ctx, yuechuov02b10_preview_setting,
		sizeof(yuechuov02b10_preview_setting) / sizeof(kal_uint16));
	OV02B10_LOG_INF("-\n");
} /* preview_setting */
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	sensor_id = ((read_cmos_sensor_8(ctx, 0x02) << 8) | read_cmos_sensor_8(ctx, 0x03));

	OV02B10_LOG_INF("[%s] sensor_id: 0x%x", __func__, sensor_id);
		...
		do {
			*sensor_id = return_sensor_id(ctx);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				OV02B10_LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			OV02B10_LOG_INF("Read sensor id fail, id: 0x%x\n",
				ctx->i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		...
		kal_bool enable, UINT16 framerate)
{
	if (enable) /*enable auto flicker*/ {
		...
		OV02B10_LOG_DBG("enable! fps = %d", framerate);
	} else {
		 /*Cancel Auto flick*/
		ctx->autoflicker_en = KAL_FALSE;
	}
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	OV02B10_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	default:  /*coding with  preview scenario by default*/
		...
		if (ctx->frame_length > ctx->shutter)
			set_dummy(ctx);
		OV02B10_LOG_DBG("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
...
{
	OV02B10_LOG_DBG("enable: %d\n", enable);

	if (enable) {
		write_cmos_sensor(ctx, 0xfd, 0x03);
		write_cmos_sensor(ctx, 0x63, 0x01);
	} else {
		write_cmos_sensor(ctx, 0xfd, 0x03);
		write_cmos_sensor(ctx, 0x63, 0x00);
	}
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(yuechuov02b10_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)yuechuov02b10_ana_gain_table,
			sizeof(yuechuov02b10_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		OV02B10_LOG_DBG("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		OV02B10_LOG_DBG("current fps :%d\n", (UINT32)*feature_data_32);
		ctx->current_fps = *feature_data_32;
		break;
	case SENSOR_FEATURE_SET_HDR:
		OV02B10_LOG_DBG("ihdr enable :%d\n", (BOOL)*feature_data_32);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		OV02B10_LOG_DBG("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		OV02B10_LOG_DBG("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data+1));
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		OV02B10_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(ctx, KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		OV02B10_LOG_DBG("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(ctx, *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		OV02B10_LOG_DBG("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		...
		yuechuov02b10_set_lsc_reg_setting(ctx, index, feature_data_16,
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	//{HW_ID_DVDD,  1800000, 1}, //VIO Power
	{HW_ID_AVDD,  2800000, 5},
	{HW_ID_MCLK,  24,      1},
	{HW_ID_MCLK_DRIVING_CURRENT, 6, 4},
	{HW_ID_RST,   1,       9},
};

const struct subdrv_entry yuechuov02b10_mipi_raw_entry = {
	.name = "yuechuov02b10_mipi_raw",
	.id = YUECHUOV02B10_SENSOR_ID,
	.pw_seq = pw_seq,
