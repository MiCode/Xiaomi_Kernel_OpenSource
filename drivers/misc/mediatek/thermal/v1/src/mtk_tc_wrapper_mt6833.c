...
int get_immediate_tslvts4_2_wrap(void)
{
	...
	curr_temp = tscpu_ts_lvts_temp[L_TS_LVTS4_2];
	tscpu_dprintk("%s curr_temp=%d\n", __func__, curr_temp);
	return curr_temp;
}
#endif
...
int tscpu_get_curr_temp(void)
{
	...
	MAX(
		...
		(MAX(
			tscpu_ts_lvts_temp[L_TS_LVTS2_0],
			tscpu_ts_lvts_temp[L_TS_LVTS2_1])
		)
	);
	tscpu_curr_gpu_temp = MAX(tscpu_ts_lvts_temp[L_TS_LVTS3_0],
								tscpu_ts_lvts_temp[L_TS_LVTS3_1]);
#endif /* CFG_THERM_LVTS */
#else
...
int get_io_reg_base(void)
{
	...
	tscpu_printk("[THERM_CTRL] phy_base 0x%p\n", thermal_phy_base);
	...
	tscpu_printk("[THERM_CTRL] auxadc_ts_phy_base 0x%p\n", auxadc_ts_phy_base);
	...
	tscpu_printk("[THERM_CTRL] apmixed_phy_base= 0x%p\n", apmixed_phy_base);
