...
static void mtktspmic_read_efuse(struct regmap *pmic_map)
{
	...
	g_o_vts_4 = thermal_pmic_get_register_value(pmic_map, PMIC_AUXADC_EFUSE_4RSV0_ADDR,
...
int mtktspmic_get_from_dts(struct platform_device *pdev)
{
	...
	if (ret) {
		if (ret != -EPROBE_DEFER)
			mtktspmic_dprintk("AUXADC_CHIP_TEMP get fail, ret=%d\n", ret);
		return ret;
	}
	...
	if (ret) {
		if (ret != -EPROBE_DEFER)
			mtktspmic_dprintk("AUXADC_VCORE_TEMP get fail, ret=%d\n", ret);
		return ret;
	}
	...
	if (ret) {
		if (ret != -EPROBE_DEFER)
			mtktspmic_dprintk("AUXADC_VPROC_TEMP get fail, ret=%d\n", ret);
		return ret;
	}
	...
	if (ret) {
		if (ret != -EPROBE_DEFER)
			mtktspmic_dprintk("AUXADC_VGPU_TEMP get fail, ret=%d\n", ret);
		return ret;
	}
	chan_tsx_temp = devm_iio_channel_get(&pdev->dev, "pmic_tsx_temp");
	ret = PTR_ERR_OR_ZERO(chan_tsx_temp);
	if (ret) {
		if (ret != -EPROBE_DEFER)
			mtktspmic_dprintk("AUXADC_TSX_TEMP get fail, ret=%d\n", ret);
		return ret;
	}
	chan_dcxo_temp = devm_iio_channel_get(&pdev->dev, "pmic_dcxo_temp");
	ret = PTR_ERR_OR_ZERO(chan_dcxo_temp);
	if (ret) {
		if (ret != -EPROBE_DEFER)
			mtktspmic_dprintk("AUXADC_DCXO_TEMP get fail, ret=%d\n", ret);
		return ret;
	}
	return 0;
}
