...
static int tsallts_get_temp(struct thermal_zone_device *thermal, unsigned long *t)
{
	...
	if (curr_temp != 0) {/* not the first temp read after resume from suspension */
		if ((curr_temp > 150000) || (curr_temp < -20000)) {	/* invalid range */
			...
			if ((curr_temp - last_abb_read_temp > 20000) || (last_abb_read_temp - curr_temp > 20000)) {
				tsallts_printk("ts1 temp float hugely temp=%d, lasttemp=%d\n",
	...
	*t = (unsigned long)curr_temp;
	...
	curr_temp = get_immediate_ts1_wrap();
	tsallts_dprintk("tsallts_get_temp ts1 =%d\n", curr_temp);
...
static int tsallts_bind(struct thermal_zone_device *thermal, struct thermal_cooling_device *cdev)
{
	...
	if (mtk_thermal_zone_bind_cooling_device(thermal, table_val, cdev)) {
		tsallts_dprintk("[tsallts_bind_ts1] error binding cooling dev\n");
		return -EINVAL;
	}

	tsallts_dprintk("[tsallts_bind_ts1] binding OK, %d\n", table_val);
	return 0;
}
...
static int tsallts_unbind(struct thermal_zone_device *thermal, struct thermal_cooling_device *cdev)
{
	...
	tsallts_dprintk("[tsallts_unbind_ts1]\n");
	if (!strcmp(cdev->type, g_bind0)) {
		table_val = 0;
	...
	if (thermal_zone_unbind_cooling_device(thermal, table_val, cdev)) {
		tsallts_dprintk("[tsallts_unbind_ts1] error unbinding cooling dev\n");
		return -EINVAL;
	}

	tsallts_dprintk("[tsallts_unbind_ts1] unbinding OK\n");
	return 0;
}
...
{
	seq_printf(m, "[ tsallts_read_ts1] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,\n",
			     ...
			     loff_t *data)
{
	...
	if (sscanf
		...
		if (NULL == thz_dev) {
			thz_dev = mtk_thermal_zone_device_register("mtkts1", num_trip, NULL,
...
static int __init tsallts_init(void)
{
	...
	tsallts_dprintk("[tsallts_init] ts1\n");
	...
	if (!tsallts_dir) {
		...
		entry =
		    proc_create("tzts1", S_IRUGO | S_IWUSR | S_IWGRP, tsallts_dir, &tsallts_fops);
		if (entry)
...
{
	tsallts_dprintk("[tsallts_exit] ts1\n");
