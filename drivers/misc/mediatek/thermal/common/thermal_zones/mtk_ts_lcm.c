...
#define MTKTS_LCM_TEMP_CRIT 60000	/* 60.000 degree Celsius */

#define mtkts_lcm_dprintk(fmt, args...)   \
do {                                    \
	if (mtkts_lcm_debug_log) {                \
		pr_notice("[Thermal/TZ/LCM]" fmt, ##args); \
	}                                   \
} while (0)


#define mtkts_lcm_printk(fmt, args...) \
pr_debug("[Thermal/TZ/LCM]" fmt, ##args)

#if IS_ENABLED(CONFIG_MEDIATEK_MT6577_AUXADC)
struct iio_channel *thermistor_ch4;
#endif
/* #define INPUT_PARAM_FROM_USER_LCM */
...
struct LCM_TEMPERATURE {
	__s32 LCM_Temp;
	__s32 TemperatureR;
};

static int g_RAP_pull_up_R = LCM_RAP_PULL_UP_R;
static int g_TAP_over_critical_low = LCM_TAP_OVER_CRITICAL_LOW;
static int g_RAP_pull_up_voltage = LCM_RAP_PULL_UP_VOLTAGE;
static int g_RAP_ntc_table = LCM_RAP_NTC_TABLE;
static int g_RAP_ADC_channel = LCM_RAP_ADC_CHANNEL;
static int g_LCM_TemperatureR;
/* LCM_TEMPERATURE LCM_Temperature_Table[] = {0}; */

static struct LCM_TEMPERATURE *LCM_Temperature_Table;
static int ntc_tbl_size;

/* AP_NTC_BL197 */
static struct LCM_TEMPERATURE LCM_Temperature_Table1[] = {
	{-40, 74354},		/* FIX_ME */
	{-35, 74354},		/* FIX_ME */
	{-30, 74354},		/* FIX_ME */
	{-25, 74354},		/* FIX_ME */
...
/* AP_NTC_TSM_1 */
static struct LCM_TEMPERATURE LCM_Temperature_Table2[] = {
	{-40, 70603},		/* FIX_ME */
	{-35, 70603},		/* FIX_ME */
	{-30, 70603},		/* FIX_ME */
	{-25, 70603},		/* FIX_ME */
...
/* AP_NTC_10_SEN_1 */
static struct LCM_TEMPERATURE LCM_Temperature_Table3[] = {
	{-40, 74354},		/* FIX_ME */
	{-35, 74354},		/* FIX_ME */
	{-30, 74354},		/* FIX_ME */
	{-25, 74354},		/* FIX_ME */
...
static struct LCM_TEMPERATURE LCM_Temperature_Table4[] = {
	{-40, 188500},
	{-35, 144290},
	{-30, 111330},
	{-25, 86560},
...
/* AP_NTC_47 */
static struct LCM_TEMPERATURE LCM_Temperature_Table5[] = {
	{-40, 483954},		/* FIX_ME */
	{-35, 483954},		/* FIX_ME */
	{-30, 483954},		/* FIX_ME */
	{-25, 483954},		/* FIX_ME */
...
/* NTCG104EF104F(100K) */
static struct LCM_TEMPERATURE LCM_Temperature_Table6[] = {
	{-40, 4251000},
	{-35, 3005000},
	{-30, 2149000},
	{-25, 1554000},
...
static struct LCM_TEMPERATURE LCM_Temperature_Table7[] = {
	{-40, 4397119},
	{-35, 3088599},
	{-30, 2197225},
	{-25, 1581881},
...
static __s32 mtkts_lcm_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
	__s32 TAP_Value = -200, TMP1 = 0, TMP2 = 0;
#ifdef APPLY_PRECISE_LCM_TEMP
	TAP_Value = TAP_Value * 1000;
#endif
	asize = (ntc_tbl_size / sizeof(struct LCM_TEMPERATURE));
	...
	if (Res >= LCM_Temperature_Table[0].TemperatureR) {
		...
		TMP1 = LCM_Temperature_Table[0].LCM_Temp;
		...
		for (i = 0; i < asize; i++) {
			if (Res >= LCM_Temperature_Table[i].TemperatureR) {
				...
				TMP2 = LCM_Temperature_Table[i].LCM_Temp;
			...
			TMP1 = LCM_Temperature_Table[i].LCM_Temp;
...
/* convert ADC_LCM_temp_volt to register */
/*Volt to Temp formula same with 6589*/
static __s32 mtk_ts_lcm_volt_to_temp(__u32 dwVolt)
{
	__s32 TRes;
	__u64 dwVCriAP = 0;
	__s32 LCM_TMP = -100;
...
#endif
	/* ------------------------------------------------------------------ */
	g_LCM_TemperatureR = TRes;

	/* convert register to temperature */
	LCM_TMP = mtkts_lcm_thermistor_conver_temp(TRes);
...
int mtkts_lcm_get_hw_temp(void)
{
	...
	if (t_ret2 < 0)
		pr_notice("[Thermal/TZ/LCM]wakeup_ta_algo out of memory\n");

	lcm_cur_temp = t_ret;

	if (t_ret > 40000)	/* abnormal high temp */
		mtkts_lcm_printk("T_LCM=%d\n", t_ret);

	mtkts_lcm_dprintk("[%s] T_LCM, %d\n", __func__, t_ret);
	return t_ret;
}

static int mtkts_lcm_get_temp(struct thermal_zone_device *thermal, int *t)
{
	*t = mtkts_lcm_get_hw_temp();

	/* if ((int) *t > 52000) */
	/* mtkts_lcm_dprintk("T=%d\n", (int) *t); */
...
struct thermal_zone_device *thermal, struct thermal_cooling_device *cdev)
{
	...
	mtkts_lcm_dprintk("[%s] binding OK, %d\n", __func__, table_val);
	return 0;
}

static int mtkts_lcm_unbind(struct thermal_zone_device *thermal,
			    struct thermal_cooling_device *cdev)
{
	...
	mtkts_lcm_dprintk("[%s] unbinding OK\n", __func__);
	return 0;
}


static int mtkts_lcm_change_mode(
struct thermal_zone_device *thermal, enum thermal_device_mode mode)
{
	kernelmode = mode;
	return 0;
}
static int mtkts_lcm_get_trip_type(
...
static int mtkts_lcm_get_trip_temp(
...
static int mtkts_lcm_get_crit_temp(
struct thermal_zone_device *thermal, int *temperature)
{
	*temperature = MTKTS_LCM_TEMP_CRIT;
	return 0;
}

/* bind callback functions to thermalzone */
static struct thermal_zone_device_ops mtkts_LCM_dev_ops = {
	.bind = mtkts_lcm_bind,
	.unbind = mtkts_lcm_unbind,
	.get_temp = mtkts_lcm_get_temp,
	.change_mode = mtkts_lcm_change_mode,
	.get_trip_type = mtkts_lcm_get_trip_type,
	.get_trip_temp = mtkts_lcm_get_trip_temp,
	.get_crit_temp = mtkts_lcm_get_crit_temp,
};



static int mtkts_lcm_read(struct seq_file *m, void *v)
{
...
static int mtkts_lcm_register_thermal(void);
static void mtkts_lcm_unregister_thermal(void);

static ssize_t mtkts_lcm_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	int len = 0, i;

	struct mtktslcm_data {
		int trip[10];
		int t_type[10];
	...
	struct mtktslcm_data *ptr_mtktslcm_data = kmalloc(
					sizeof(*ptr_mtktslcm_data), GFP_KERNEL);

	if (ptr_mtktslcm_data == NULL)
		return -ENOMEM;


	len = (count < (sizeof(ptr_mtktslcm_data->desc) - 1)) ?
				count : (sizeof(ptr_mtktslcm_data->desc) - 1);

	if (copy_from_user(ptr_mtktslcm_data->desc, buffer, len)) {
		kfree(ptr_mtktslcm_data);
		return 0;
	}

	ptr_mtktslcm_data->desc[len] = '\0';

	if (sscanf
	    (ptr_mtktslcm_data->desc,
	     "%d %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d",
		&num_trip,
		&ptr_mtktslcm_data->trip[0], &ptr_mtktslcm_data->t_type[0],
		ptr_mtktslcm_data->bind0,
		&ptr_mtktslcm_data->trip[1], &ptr_mtktslcm_data->t_type[1],
		ptr_mtktslcm_data->bind1,
		&ptr_mtktslcm_data->trip[2], &ptr_mtktslcm_data->t_type[2],
		ptr_mtktslcm_data->bind2,
		&ptr_mtktslcm_data->trip[3], &ptr_mtktslcm_data->t_type[3],
		ptr_mtktslcm_data->bind3,
		&ptr_mtktslcm_data->trip[4], &ptr_mtktslcm_data->t_type[4],
		ptr_mtktslcm_data->bind4,
		&ptr_mtktslcm_data->trip[5], &ptr_mtktslcm_data->t_type[5],
		ptr_mtktslcm_data->bind5,
		&ptr_mtktslcm_data->trip[6], &ptr_mtktslcm_data->t_type[6],
		ptr_mtktslcm_data->bind6,
		&ptr_mtktslcm_data->trip[7], &ptr_mtktslcm_data->t_type[7],
		ptr_mtktslcm_data->bind7,
		&ptr_mtktslcm_data->trip[8], &ptr_mtktslcm_data->t_type[8],
		ptr_mtktslcm_data->bind8,
		&ptr_mtktslcm_data->trip[9], &ptr_mtktslcm_data->t_type[9],
		ptr_mtktslcm_data->bind9,
		&ptr_mtktslcm_data->time_msec) == 32) {

		down(&sem_mutex);
		mtkts_lcm_dprintk(
			"[%s] mtkts_lcm_unregister_thermal\n", __func__);

		mtkts_lcm_unregister_thermal();

		if (num_trip < 0 || num_trip > 10) {
			#if IS_ENABLED(CONFIG_MTK_AEE_FEATURE)
			aee_kernel_warning_api(__FILE__, __LINE__,
					DB_OPT_DEFAULT, "mtkts_lcm_write",
					"Bad argument");
			#endif
			mtkts_lcm_dprintk("[%s] bad argument\n", __func__);
			kfree(ptr_mtktslcm_data);
			up(&sem_mutex);
			return -EINVAL;
		}
		...
		for (i = 0; i < num_trip; i++)
			g_THERMAL_TRIP[i] = ptr_mtktslcm_data->t_type[i];
		...
		for (i = 0; i < 20; i++) {
			g_bind0[i] = ptr_mtktslcm_data->bind0[i];
			g_bind1[i] = ptr_mtktslcm_data->bind1[i];
			g_bind2[i] = ptr_mtktslcm_data->bind2[i];
			g_bind3[i] = ptr_mtktslcm_data->bind3[i];
			g_bind4[i] = ptr_mtktslcm_data->bind4[i];
			g_bind5[i] = ptr_mtktslcm_data->bind5[i];
			g_bind6[i] = ptr_mtktslcm_data->bind6[i];
			g_bind7[i] = ptr_mtktslcm_data->bind7[i];
			g_bind8[i] = ptr_mtktslcm_data->bind8[i];
			g_bind9[i] = ptr_mtktslcm_data->bind9[i];
		}
		...
		for (i = 0; i < num_trip; i++)
			trip_temp[i] = ptr_mtktslcm_data->trip[i];

		interval = ptr_mtktslcm_data->time_msec / 1000;
		...
		mtkts_lcm_dprintk("trip_9_temp=%d,time_ms=%d\n",
			trip_temp[9], interval * 1000);

		mtkts_lcm_dprintk(
			"[%s] mtkts_lcm_register_thermal\n", __func__);

		mtkts_lcm_register_thermal();
		up(&sem_mutex);
		kfree(ptr_mtktslcm_data);
		/* AP_write_flag=1; */
		return count;
...
void mtkts_lcm_prepare_table(int table_num)
{
	...
	pr_notice("[Thermal/TZ/LCM] %s table_num=%d\n", __func__, table_num);

}

static int mtkts_lcm_param_read(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", g_RAP_pull_up_R);
...
static ssize_t mtkts_lcm_param_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	int len = 0;
	struct mtktslcm_param_data {
		char desc[512];
	...
	struct mtktslcm_param_data *ptr_mtktslcm_parm_data;

	ptr_mtktslcm_parm_data = kmalloc(
				sizeof(*ptr_mtktslcm_parm_data), GFP_KERNEL);

	if (ptr_mtktslcm_parm_data == NULL)
		return -ENOMEM;
	 ...
	 */
	ptr_mtktslcm_parm_data->adc_channel = 11;

	len = (count < (sizeof(ptr_mtktslcm_parm_data->desc) - 1)) ?
			count : (sizeof(ptr_mtktslcm_parm_data->desc) - 1);

	if (copy_from_user(ptr_mtktslcm_parm_data->desc, buffer, len)) {
		kfree(ptr_mtktslcm_parm_data);
		return 0;
	}

	ptr_mtktslcm_parm_data->desc[len] = '\0';

	mtkts_lcm_dprintk("[%s]\n", __func__);

	if (sscanf
	    (ptr_mtktslcm_parm_data->desc, "%9s %d %9s %d %15s %d %9s %d %d",
		ptr_mtktslcm_parm_data->pull_R, &ptr_mtktslcm_parm_data->valR,
		ptr_mtktslcm_parm_data->pull_V, &ptr_mtktslcm_parm_data->valV,
		ptr_mtktslcm_parm_data->overcrilow,
		&ptr_mtktslcm_parm_data->over_cri_low,
		ptr_mtktslcm_parm_data->NTC_TABLE,
		&ptr_mtktslcm_parm_data->ntc_table,
		&ptr_mtktslcm_parm_data->adc_channel) >= 8) {

		if (!strcmp(ptr_mtktslcm_parm_data->pull_R, "PUP_R")) {
			g_RAP_pull_up_R = ptr_mtktslcm_parm_data->valR;
			mtkts_lcm_dprintk("g_RAP_pull_up_R=%d\n",
							g_RAP_pull_up_R);
		} else {
			mtkts_lcm_printk(
		...
		if (!strcmp(ptr_mtktslcm_parm_data->pull_V, "PUP_VOLT")) {
			g_RAP_pull_up_voltage = ptr_mtktslcm_parm_data->valV;
			mtkts_lcm_dprintk("g_Rat_pull_up_voltage=%d\n",
							g_RAP_pull_up_voltage);
		} else {
			mtkts_lcm_printk(
		...
		if (!strcmp(ptr_mtktslcm_parm_data->overcrilow,
			"OVER_CRITICAL_L")) {
			g_TAP_over_critical_low =
					ptr_mtktslcm_parm_data->over_cri_low;
			mtkts_lcm_dprintk("g_TAP_over_critical_low=%d\n",
						g_TAP_over_critical_low);
		} else {
			mtkts_lcm_printk(
		...
		if (!strcmp(ptr_mtktslcm_parm_data->NTC_TABLE, "NTC_TABLE")) {
			g_RAP_ntc_table = ptr_mtktslcm_parm_data->ntc_table;
			mtkts_lcm_dprintk("g_RAP_ntc_table=%d\n",
							g_RAP_ntc_table);
		} else {
			mtkts_lcm_printk(
		 ...
		 * choose "adc_channel=11" to check if there is any param input
		 */
		if ((ptr_mtktslcm_parm_data->adc_channel >= 2)
		&& (ptr_mtktslcm_parm_data->adc_channel <= 11))
			 ...
			 */
			if (ptr_mtktslcm_parm_data->adc_channel != 11)
				g_RAP_ADC_channel =
					ptr_mtktslcm_parm_data->adc_channel;
			else
				g_RAP_ADC_channel = AUX_IN4_NTC;
		}
		mtkts_lcm_dprintk("adc_channel=%d\n",
					ptr_mtktslcm_parm_data->adc_channel);
		mtkts_lcm_dprintk("g_RAP_ADC_channel=%d\n",
						g_RAP_ADC_channel);
	...
	mtkts_lcm_printk("[%s] bad argument\n", __func__);
	kfree(ptr_mtktslcm_parm_data);
	return -EINVAL;
}

/* int  mtkts_LCM_register_cooler(void)
 * {
 *  cooling devices
 *  cl_dev_sysrst = mtk_thermal_cooling_device_register(
 *  "mtktsAPtery-sysrst", NULL,
 *  &mtkts_LCM_cooling_sysrst_ops);
 *  return 0;
 * }
 */
...
static int mtkts_lcm_register_thermal(void)
{
	...
	thz_dev = mtk_thermal_zone_device_register("backlight_therm", num_trip, NULL,
						&mtkts_LCM_dev_ops, 0, 0, 0,
						interval * 1000);

	return 0;
}

/* void mtkts_LCM_unregister_cooler(void) */
/* { */
	/* if (cl_dev_sysrst) { */
...
static int mtkts_lcm_open(struct inode *inode, struct file *file)
{
	return single_open(file, mtkts_lcm_read, NULL);
}

static const struct proc_ops mtkts_LCM_fops = {
	.proc_open = mtkts_lcm_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtkts_lcm_write,
	.proc_release = single_release,
};


static int mtkts_lcm_param_open(struct inode *inode, struct file *file)
{
	return single_open(file, mtkts_lcm_param_read, NULL);
}

static const struct proc_ops mtkts_LCM_param_fops = {
	.proc_open = mtkts_lcm_param_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtkts_lcm_param_write,
	.proc_release = single_release,
};
...
static int mtkts_lcm_probe(struct platform_device *pdev)
{
	...
	struct proc_dir_entry *mtkts_LCM_dir = NULL;
	...
	if (ret) {
		mtkts_lcm_printk("[%s] fail to get auxadc iio ch4: %d\n",
			__func__, ret);
		return ret;
	}
	...
	mtkts_LCM_dir = mtk_thermal_get_proc_drv_therm_dir_entry();
	if (!mtkts_LCM_dir) {
		mtkts_lcm_dprintk(
			"[%s]: mkdir /proc/driver/thermal failed\n",
				__func__);
	} else {
		entry = proc_create("tzlcm", 0664, mtkts_LCM_dir,
							&mtkts_LCM_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

	entry = proc_create("tzlcm_param", 0664, mtkts_LCM_dir,
				&mtkts_LCM_param_fops);
		if (entry)
			proc_set_user(entry, uid, gid);
...
int mtkts_lcm_init(void)
{
	...
	mtkts_LCM_dir = mtk_thermal_get_proc_drv_therm_dir_entry();
	if (!mtkts_LCM_dir) {
		mtkts_lcm_dprintk("[%s]: mkdir /proc/driver/thermal failed\n",
								__func__);
	} else {
		entry = proc_create("tzlcm", 0664, mtkts_LCM_dir,
							&mtkts_LCM_fops);
...
void mtkts_lcm_exit(void)
{
	mtkts_lcm_dprintk("[%s]\n", __func__);
	mtkts_lcm_unregister_thermal();
	/* mtkts_LCM_unregister_cooler(); */
}
