...
struct thermal_zone_device *thermal, struct thermal_cooling_device *cdev)
{
	...
	if (mtk_thermal_zone_bind_cooling_device(thermal, table_val, cdev)) {
		mtktsdram_dprintk(
			"[mtktsdram_bind] error binding cooling dev\n");
		return -EINVAL;
	}
...
static int mtktsdram_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m,
		"[mtktsdram_read] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d,\n",
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(ptr_mtktsdram_data->desc,
		...
		mtktsdram_dprintk(
			"[mtktsdram_write] mtktsdram_unregister_thermal\n");
		...
		mtktsdram_dprintk(
			"[mtktsdram_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,",
			g_THERMAL_TRIP[0], g_THERMAL_TRIP[1],
			g_THERMAL_TRIP[2]);
		...
		mtktsdram_dprintk(
			"[mtktsdram_write] cooldev0=%s,cooldev1=%s,cooldev2=%s,cooldev3=%s,cooldev4=%s,",
			g_bind0, g_bind1, g_bind2, g_bind3, g_bind4);
		...
		mtktsdram_dprintk(
			"[mtktsdram_write] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,",
		...
		mtktsdram_dprintk(
			"[mtktsdram_write] mtktsdram_register_thermal\n");
