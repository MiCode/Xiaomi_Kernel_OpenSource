...
char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	...
	if (data == NULL) {
		mtk_cooler_mddtxpwr_dprintk(
				"[_mtk_cl_mddtxpwr_proc_read] null data\n");
	} else {
		int i = 0;
...
struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	if (data == NULL) {
		mtk_cooler_mddtxpwr_dprintk(
					"[_mtk_cl_mddtxpwr_proc_write] null data\n");
...
#endif
	mtk_cooler_mddtxpwr_dprintk(
				"[_mtk_cl_mddtxpwr_proc_write] bad argument\n");
	return -EINVAL;
}
...
static int __init mtk_cooler_mddtxpwr_init(void)
{
	...
	/* create a proc file */
	{
		...
		mtk_cooler_mddtxpwr_dprintk(
					"[mtk_cooler_mddtxpwr_init] proc file created: %x\n",
					entry->data);
	}
