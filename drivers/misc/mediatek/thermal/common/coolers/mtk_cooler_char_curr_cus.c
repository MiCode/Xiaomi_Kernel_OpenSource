...
static const struct proc_ops _cl_char_curr_fops = {
	.proc_open	= _cl_char_curr_open,
	.proc_read	= seq_read,
...
(struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	if (pinfo == NULL) {
		...
		if (psy == NULL) {
			mtk_cooler_char_curr_dprintk("[%s]psy is not rdy\n", __func__);
			return -1;
		}

		pinfo = (struct mtk_charger *)power_supply_get_drvdata(psy);
		if (pinfo == NULL) {
			mtk_cooler_char_curr_dprintk("[%s]mtk_gauge is not rdy\n", __func__);
			return -1;
		}
	}
	...
	if (state >= max_char_curr_index || state < 0) {
		...
		if (chr_input_curr_limit != 0) {
			charger_dev_set_input_current(pinfo->chg1_dev, chr_input_curr_limit);
			...
			if (g_thm_en_charging == 0) {
				g_thm_en_charging = 1;
				charger_dev_enable_powerpath(pinfo->chg1_dev, g_thm_en_charging);
...
static int mtkcooler_char_curr_late_init(void)
{
	int ret = 0;
...
int mtk_cooler_char_curr_init(void)
{
	int err = 0;
...
void mtk_cooler_char_curr_exit(void)
{
	mtk_cooler_char_curr_unregister_ltf();
}
