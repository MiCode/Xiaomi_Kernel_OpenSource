...
int amutt_param_read(struct seq_file *m, void *v)
{
	seq_printf(m, "[up]\t%3d(sec)\t%2d\n[low]\t%3d(sec)\t%2d\nrst=%2d\ninterval=%d\nmax_step=%d\n", up_duration, up_step, \
								low_duration, low_step, low_rst_max, polling_interval, amutt.max_level);

	mtk_cooler_amutt_dprintk_always("[%s] [up]%d %d, [low]%d %d, rst=%d, interval=%d, max_step=%d\n", __func__, up_duration, \
...
ssize_t amutt_asparam_write(struct file *filp, const char __user *buf, size_t len, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d %d", &tmp_step, &active_ms, &suspend_ms) == 3) {
	    ...
	    if (tmp_step > 0 && tmp_step < COOLER_STEPS)
	    {
            ...
            else if (active_ms >= 100 && active_ms <= 25500 && suspend_ms >= 100 && suspend_ms <= 25500)
                amutt_param[tmp_step] = ((suspend_ms/100) << 16) | ((active_ms/100) << 8) | 1;

            mtk_cooler_amutt_dprintk_always("[%s] %s %u %u %u %x\n", __func__, desc, tmp_step, active_ms, suspend_ms, amutt_param[tmp_step]);
	    }
