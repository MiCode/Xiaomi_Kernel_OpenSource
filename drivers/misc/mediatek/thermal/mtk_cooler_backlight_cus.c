...
#define MAX_BACKLIGHT_BRIGHTNESS 2047
...
extern int mt_leds_brightness_thermal_set(char *name, int brightness);
...
(struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	//setMaxBrightness("lcd-backlight", (state * 100 / 2047), enable);
	//disp_aal_notify_backlight_changed(state);
	ret = mt_leds_brightness_thermal_set("lcd-backlight", state);
	if (ret > 0) {
