...
static struct mutex teei_cpus_mutex;
static int g_teei_chan_stat;
static void *teei_cpu_mutex_owner;
...
int teei_set_switch_pri(unsigned long policy)
{
	...
	if (policy == TEEI_RT_POLICY) {
		IMSG_PRINTK("TEEI: Set teei_switch_thread with RT\n");
		sched_setscheduler_nocheck(teei_switch_task,
						SCHED_FIFO, &param);
		return 0;
	} else if (policy == TEEI_NORMAL_POLICY) {
		IMSG_PRINTK("TEEI: Set teei_switch_thread with NORMAL\n");
...
int teei_set_chan_mode(int new_mode)
{
	if ((new_mode == TEEI_SINGAL_CHANNEL_MODE)
		|| (new_mode == TEEI_MUTIL_CHANNEL_MODE)) {

		g_teei_chan_stat = new_mode;
		return 0;
	}
...
int teei_get_chan_mode(void)
{
	return g_teei_chan_stat;
}

void teei_cpus_read_lock(int teei_chan_mode)
{
	if (current != teei_cpu_write_owner) {
		if (teei_chan_mode == TEEI_SINGAL_CHANNEL_MODE) {
			if (current != teei_cpu_mutex_owner) {
				IMSG_PRINTK("TEEI: <singal chan> lock teei_cpus_mutex!\n");
				mutex_lock(&teei_cpus_mutex);
				IMSG_PRINTK("TEEI: <singal chan> GOT teei_cpus_mutex!\n");
			}
		}
		cpus_read_lock();
	}
...
void teei_cpus_read_unlock(int teei_chan_mode)
{
	if (current != teei_cpu_write_owner) {
		if (teei_chan_mode == TEEI_SINGAL_CHANNEL_MODE) {
			if (current != teei_cpu_mutex_owner) {
				IMSG_PRINTK("TEEI: Unlock the teei_cpus_mutex!\n");
				mutex_unlock(&teei_cpus_mutex);
			}
		}
...
void teei_cpus_mutex_lock(void)
{
	mutex_lock(&teei_cpus_mutex);
	teei_cpu_mutex_owner = current;
}

void teei_cpus_mutex_unlock(void)
{
	teei_cpu_mutex_owner = NULL;
	mutex_unlock(&teei_cpus_mutex);
}
...
{
	cpus_write_lock();
	IMSG_PRINTK("TEEI: cpus_write_lock\n");
	teei_cpu_write_owner = current;
}

void teei_cpus_write_unlock(void)
{
	IMSG_PRINTK("TEEI: cpus_write_unlock\n");
...
static int load_tee(void)
{
	...
	IMSG_PRINTK("load TEE BLOCK\n");
	down(&boot_sema);
	IMSG_PRINTK("load TEE END\n");

	return retVal;
}


int set_sch_load_img_cmd(void)
{
	int retVal = 0;
...
void secondary_boot_stage1(void *info)
{
	...
	smc_type = teei_secure_call(N_INIT_T_BOOT_STAGE1, cd->vfs_phy_addr,
					cd->tlog_phy_addr, 0);
...
EXPORT_SYMBOL_GPL(handle_move_core);
...
static int init_teei_framework(void)
{
	...
	register_ut_smc_handler(&ut_smc_nb);
