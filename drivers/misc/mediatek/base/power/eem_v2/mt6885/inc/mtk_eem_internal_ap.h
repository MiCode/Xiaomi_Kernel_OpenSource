...
struct eem_det {
	...
	/* dvfs */
	unsigned int
		num_freq_tbl; /* could be got @ the same time with freq_tbl[] */
	unsigned int max_freq_khz;       /* maximum frequency used to calculate
					  * percentage
					  */
	unsigned char freq_tbl[NR_FREQ]; /* percentage to maximum freq */

	unsigned int volt_tbl_orig[NR_FREQ];  /* orig volt table for restoreing
					       * to dvfs
					       */
	...
	unsigned char set_volt_to_upower; /* only when init2, eem need to set
					   * volt to upower
					   */
};
