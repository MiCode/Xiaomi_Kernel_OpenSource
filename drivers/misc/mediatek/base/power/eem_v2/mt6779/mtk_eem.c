...
static unsigned int gpu_bin, cpu_level4;

static struct pi_efuse_index pi_efuse_idx[] = {
	{EEM_CTRL_CCI, 1, 0, 2, 16, 0, 16, 0, 0, 0},
	{EEM_CTRL_L, 4, 0, 5, 16, 0, 16, 0, 0, 0},
	{EEM_CTRL_B, 6, 0, 5, 0, 0, 0, 0, 0, 0},
...
static int get_devinfo(void)
{
	...
	/* CCI */
	aee_rr_rec_ptp_devinfo_1((unsigned int) pi_efuse_idx[0].orig_mdes_bdes);
	aee_rr_rec_ptp_devinfo_2((unsigned int) pi_efuse_idx[0].orig_mtdes);

	/* Little: MDES and BDES */
	aee_rr_rec_ptp_devinfo_3((unsigned int) pi_efuse_idx[1].orig_mdes_bdes);

	/* Big and little: MTDES */
	aee_rr_rec_ptp_devinfo_4((unsigned int) pi_efuse_idx[1].orig_mtdes);

	/* Big: MDES and BDES */
	aee_rr_rec_ptp_devinfo_5((unsigned int) pi_efuse_idx[2].orig_mdes_bdes);
	...
	for (i = 1; i < NR_HW_RES_FOR_BANK; i++) {
		if (val[i] == 0) {
			ret = 1;
			eem_checkEfuse = 0;
			eem_error("No EEM EFUSE available, turn off EEM\n");
			eem_error("EEM (val[%d] !!\n", i);
			for_each_det(det)
				det->disabled = 1;
			break;
...
{
		eem_error("cannot find %s (thermal not ready!)\n", __func__);
	return 0;
}

void __attribute__((weak)) get_thermal_slope_intercept(struct TS_PTPOD *ts_info,
...
void base_ops_set_phase(struct eem_det *det, enum eem_phase phase)
{
	...
	if (det->loo_role == LOW_BANK) {
		...
		eem_write(EEM_FREQPCT74,
			  ...
			  ((det->freq_tbl[12])
				& 0xff));
	} else if (det->loo_role == HIGH_BANK) {
		eem_write(EEM_FREQPCT30,
			  ((det->freq_tbl[3]
				<< 24) & 0xff000000)	|
			  ((det->freq_tbl[2]
...
static void get_volt_table_in_thread(struct eem_det *det)
{
	...
	if (det->init2_done == 0) {
		...
		if (det->loo_role == HIGH_BANK) {
			...
			memcpy(det->volt_tbl_init2, det->volt_tbl,
				sizeof(det->volt_tbl_init2)/2);
			final_init02_flag |= BIT(det->ctrl_id);
	...
	for (i = 0; i < ndet->num_freq_tbl; i++) {
		...
		case EEM_CTRL_B:
			if ((cpu_level4 == 1) && (i == 0))
				ndet->volt_tbl_pmic[i] =
				(ndet->volt_tbl_orig[i] + ndet->volt_offset +
				ndet->volt_aging[i]);
			else
				ndet->volt_tbl_pmic[i] = min(
				(unsigned int)(clamp(
		...
		case EEM_CTRL_GPU:
			if (i == 0)
				ndet->volt_tbl_pmic[i] = ndet->ops->volt_2_pmic(
				ndet, gpu_opp0_t_volt[gpu_bin]);
			else
				ndet->volt_tbl_pmic[i] = min(
				(unsigned int)(clamp(
				ndet->ops->eem_2_pmic(ndet,
		...
		eem_error("low_temp_offset:%d, rm_dvtfix:%d\n",
			low_temp_offset, rm_dvtfix_offset);
#endif
#if ENABLE_LOO
	...
	if (ndet->set_volt_to_upower == 0) {
		if ((ndet->ctrl_id == EEM_CTRL_L) ||
			...
			((final_init02_flag & EEM_GPU_INIT02_FLAG) ==
			EEM_GPU_INIT02_FLAG)))
			init2chk = 1;
...
static int eem_init1stress_thread_handler(void *data)
{
	...
	start = ktime_get();
	node = of_find_compatible_node(NULL, NULL, EEM_DT_NODE);
	ret = of_property_read_u32(node, "eem-ct", &ct_flag);
	if (ret)
		eem_error("cant find eem-ct node\n");
	while (1) {
		if (!ct_flag) {
			err_flag = 1;
			eem_error("EEM corner tighten close by DT\n");
			break;
		}
		eem_debug("wait vpu time %d WAIT_TIME = %d\n",
			curtime, WAIT_TIME);
...
static void eem_init_det(struct eem_det *det, struct eem_devinfo *devinfo)
{
	...
	case EEM_DET_B:
		...
		if ((strstr(CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES,
			...
			if (cpu_level4 == 1)
				det->volt_aging[0] = AGING_VAL_CPU_B_TURBO;
		}
#endif
		break;
	...
	case EEM_DET_GPU:
		...
		if ((devinfo->FT_PGM == 1) || (devinfo->FT_PGM == 2))
			det->DVTFIXED = 3;
		break;
#else
	case EEM_DET_GPU:
...
#if ENABLE_LOO
	if (det->loo_role == LOW_BANK) {
		det->volt_tbl[8] = (temp30 & 0xff);
		det->volt_tbl[9] = (temp30 >> 8)	& 0xff;
		det->volt_tbl[10] = (temp30 >> 16) & 0xff;
		det->volt_tbl[11] = (temp30 >> 24) & 0xff;

		det->volt_tbl[12] = (temp74 & 0xff);
		det->volt_tbl[13] = (temp74 >> 8) & 0xff;
		det->volt_tbl[14] = (temp74 >> 16) & 0xff;
		det->volt_tbl[15] = (temp74 >> 24) & 0xff;
	} else if (det->loo_role == HIGH_BANK) {
		det->volt_tbl[0] = (temp30 & 0xff);
		det->volt_tbl[1] = (temp30 >> 8)	& 0xff;
		det->volt_tbl[2] = (temp30 >> 16) & 0xff;
		det->volt_tbl[3] = (temp30 >> 24) & 0xff;

		det->volt_tbl[4] = (temp74 & 0xff);
		det->volt_tbl[5] = (temp74 >> 8)	& 0xff;
		det->volt_tbl[6] = (temp74 >> 16) & 0xff;
		det->volt_tbl[7] = (temp74 >> 24) & 0xff;

		/* Work around for 2-line high bank */
#ifdef FIX_ME
		if (det->ctrl_id == EEM_CTRL_B_HI) {
			det->volt_tbl[6] = det->volt_tbl[5];
			det->volt_tbl[7] = det->volt_tbl[5];
		} else if (det->ctrl_id == EEM_CTRL_L_HI)
			det->volt_tbl[7] = det->volt_tbl[6];
#endif
	} else {
		det->volt_tbl[0] = (temp30 & 0xff);
...
static void eem_dconfig_set_det(struct eem_det *det, struct device_node *node)
{
...
#ifdef FIX_ME
	eem_debug("[DCONFIG] det_id:%d, feature modified by DT(0x%x)\n",
			det_id, doe_initmon);
	eem_debug("[DCONFIG] doe_offset:%x, doe_clamp:%x\n",
	...
	if (!rc1) {
		if ((((doe_initmon >= 0x0) && (doe_initmon <= 0x3)) ||
			...
			det->features = doe_initmon;
			eem_debug("[DCONFIG] feature modified by DT(0x%x)\n",
				doe_initmon);

			if ((det->features & 0x1) == 0)
...
#if ENABLE_LOO
		num_bank_freq = det->num_freq_tbl / 2;
		start_oft = aging_val[0];
		end_oft = aging_val[1];
		offset = start_oft - end_oft;
		for (i = 0; i < num_bank_freq; i++) {
			if (i == 0)
				det->volt_aging[i] = start_oft;
			else
				det->volt_aging[i] = start_oft -
				((offset * i) / num_bank_freq);
			eem_error("det->volt_aging[%d]: %d\n", i,
					det->volt_aging[i]);
		}

		start_oft = aging_val[1];
		end_oft = aging_val[2];
		offset = start_oft - end_oft;
		for (i = num_bank_freq; i < det->num_freq_tbl; i++) {
			if (i == 0)
				det->volt_aging[i] = start_oft;
			else
				det->volt_aging[i] = start_oft -
...
static int eem_dump_proc_show(struct seq_file *m, void *v)
{
	...
	for (p = &pi_efuse_idx[0]; p->mdes_bdes_index != 0; p++) {
		...
		/*
		 * No need to print out the MTDES efuse of the big CPUs
		 * since big CPUs and and little CPUs have the same
		 * PTP efuse address.
		 */
		if (p->ctrl_id == EEM_CTRL_B)
			continue;
