...
#define C4_PLL_DIVIDER_CFG     (MCUCFG_BASE + 0xa2a4)
#define C5_PLL_DIVIDER_CFG     (MCUCFG_BASE + 0xa2a8)
#define C6_PLL_DIVIDER_CFG     (MCUCFG_BASE + 0xa2ac)
#define C7_PLL_DIVIDER_CFG     (MCUCFG_BASE + 0xa2b0)
...
static void switch_clock_source_v2(unsigned char clock_switch_mask, int source)
{
	...
	if (clock_switch_mask & (1 << 1)) { /* BL (CA77) */
		switch_pll(source, C4_PLL_DIVIDER_CFG);
		switch_pll(source, C5_PLL_DIVIDER_CFG);
		switch_pll(source, C6_PLL_DIVIDER_CFG);
	}

	if (clock_switch_mask & (1 << 2)) { /* B (CA77) */
		switch_pll(source, C7_PLL_DIVIDER_CFG);
	}
...
static int mssv_clock_source_proc_show(struct seq_file *m, void *v)
{
	...
	value = udi_reg_read(C4_PLL_DIVIDER_CFG);
	seq_printf(m, "C4 reg [%x] = 0x%x\n", C4_PLL_DIVIDER_CFG, value);
	value = udi_reg_read(C5_PLL_DIVIDER_CFG);
	seq_printf(m, "C5 reg [%x] = 0x%x\n", C5_PLL_DIVIDER_CFG, value);
	value = udi_reg_read(C6_PLL_DIVIDER_CFG);
	seq_printf(m, "C6 reg [%x] = 0x%x\n", C6_PLL_DIVIDER_CFG, value);
	value = udi_reg_read(C7_PLL_DIVIDER_CFG);
	seq_printf(m, "C7 reg [%x] = 0x%x\n", C7_PLL_DIVIDER_CFG, value);

//	seq_printf(m, "4L freq(armpll) = %d khz.\n", mt_get_abist_freq(22));
//	seq_printf(m, "C4 freq(armpll) = %d khz.\n", mt_get_abist_freq(20));
//	seq_printf(m, "C5 freq(armpll) = %d khz.\n", mt_get_abist_freq(20));
//	seq_printf(m, "C6 freq(armpll) = %d khz.\n", mt_get_abist_freq(20));
//	seq_printf(m, "C7 freq(armpll) = %d khz.\n", mt_get_abist_freq(20));
