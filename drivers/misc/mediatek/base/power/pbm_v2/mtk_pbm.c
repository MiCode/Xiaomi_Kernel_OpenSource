...
{
	u32 *share_mem;
	u32 mem_2g = 0, mem_3g = 0;
	u32 mem_4g_upL1 = 0, mem_4g_upL2 = 0, mem_tdd = 0;
	int section;
	...
	for (section = 1; section <= SECTION_NUM; section++) {
		mem_2g |=
		md1_section_level_2g[section] << section_level[section];
		mem_3g |=
		md1_section_level_3g[section] << section_level[section];
		mem_4g_upL1 |=
		md1_section_level_4g_upL1[section] << section_level[section];
		mem_4g_upL2 |=
		md1_section_level_4g_upL2[section] << section_level[section];
		mem_tdd |=
		md1_section_level_tdd[section] << section_level[section];
	}
...
static int get_md1_scenario(void)
{
	...
	/* get scenario index when working & max power */
	/* (bit4 and bit5 no use) */
...
static int get_md1_2g_dbm_power(u32 *share_mem)
{
	...
	if (share_mem[DBM_2G_TABLE] == bef_share_mem) {
		pbm_debug
("MD1 2G dBm, no TX power, reg: 0x%x(0x%x) return 0, pa: %d, rf: %d\n",
share_mem[DBM_2G_TABLE], bef_share_mem, pa_power, rf_power);
	...
	for (section = 1; section <= SECTION_NUM; section++) {
		if (
((share_mem[DBM_2G_TABLE] >> section_level[section]) & SECTION_VALUE) !=
((bef_share_mem >> section_level[section]) & SECTION_VALUE)) {
			...
			pbm_debug
("MD1 2G dBm update, reg: 0x%x, bef_reg: 0x%x, pa: %d, rf: %d, section: %d\n",
...
static int get_md1_3g_dbm_power(u32 *share_mem)
{
	...
	if (share_mem[DBM_3G_TABLE] == bef_share_mem) {
		pbm_debug
("MD1 3G dBm, no TX power, reg: 0x%x(0x%x) return 0, pa: %d, rf: %d\n",
share_mem[DBM_3G_TABLE], bef_share_mem, pa_power, rf_power);
	...
	for (section = 1; section <= SECTION_NUM; section++) {
		if (
((share_mem[DBM_3G_TABLE] >> section_level[section]) & SECTION_VALUE) !=
((bef_share_mem >> section_level[section]) & SECTION_VALUE)) {
			/* get PA power */
			...
			pbm_debug
("MD1 3G dBm update, reg: 0x%x, bef_reg: 0x%x, pa: %d, rf: %d, section: %d\n",
...
static int get_md1_4g_upL1_dbm_power(u32 *share_mem)
{
	...
	if (share_mem[DBM_4G_TABLE] == bef_share_mem) {
		pbm_debug
("MD1 4G dBm, no TX power, reg: 0x%x(0x%x) return 0, pa: %d, rf: %d\n",
share_mem[DBM_4G_TABLE], bef_share_mem, pa_power, rf_power);
	...
	for (section = 1; section <= SECTION_NUM; section++) {
		if (
((share_mem[DBM_4G_TABLE] >> section_level[section]) & SECTION_VALUE) !=
((bef_share_mem >> section_level[section]) & SECTION_VALUE)) {
			/* get PA power */
			...
			pbm_debug
("MD1 4G dBm update, reg: 0x%x, bef_reg: 0x%x, pa: %d, rf: %d, section: %d\n",
...
share_mem[DBM_4G_1_TABLE], bef_share_mem, pa_power, rf_power);
	...
	for (section = 1; section <= SECTION_NUM; section++) {
		if (
((share_mem[DBM_4G_1_TABLE] >> section_level[section]) & SECTION_VALUE) !=
((bef_share_mem >> section_level[section]) & SECTION_VALUE)) {
			/* get PA power */
...
static int get_md3_dBm_power(void)
{
	...
	if (share_mem == NULL) {
		pbm_debug
("MD3 share_mem is NULL, , use max pa and rf power (%d + %d)\n",
md3_pa_pwr_c2k[1], PW_MD3_RF_C2K);
	...
	for (i = 0; i < SHARE_MEM_BLOCK_NUM; i++) {
		usedBytes +=
sprintf(log_buffer + usedBytes, "0x%x ", share_mem[i]);
	...
	if (share_mem[DBM_C2K_1_TABLE] == bef_share_mem) {
		pbm_debug
("MD3 dBm, no TX power, reg: 0x%x(0x%x) return 0, pa: %d, rf: %d\n",
share_mem[DBM_C2K_1_TABLE], bef_share_mem, pa_power, rf_power);
	...
	for (section = 1; section <= SECTION_NUM; section++) {
		if (
((share_mem[DBM_C2K_1_TABLE] >> section_level[section]) & SECTION_VALUE) !=
((bef_share_mem >> section_level[section]) & SECTION_VALUE)) {
			/* get PA power */
			...
			pbm_debug
("MD3 dBm update, reg: 0x%x, bef_reg: 0x%x, pa: %d, rf: %d, section: %d\n",
...
static void test_md_dbm_power(void)
{
	...
	for (i = 1; i <= SECTION_NUM; i++) {
		for (j = 1; j <= SECTION_VALUE; j++) {
			...
			y = (section[DBM_2G_TABLE] >>
			section_level[i]) & SECTION_VALUE;
...
{
	int _dlpt = 0, leakage = 0, md1 = 0, md3 = 0;
	int dlpt = 0, cpu = 0, gpu = 0, flash = 0;
	int tocpu = 0, togpu = 0;
	int multiple = 0;
	...
	if (mt_pbm_debug) {
		pbm_debug
("(C/G)=%d,%d => (D/L/M1/M3/F/C/G)=%d,%d,%d,%d,%d,%d,%d (Multi:%d),%d\n",
cpu, gpu, dlpt, leakage, md1, md3, flash,
tocpu, togpu, multiple, cpu_lower_bound);
	} else {
...
static void mtk_power_budget_manager(enum pbm_kicker kicker,
struct mrp *mrpmgr)
{
 ...
 */
static ssize_t mt_pbm_debug_proc_write(struct file *file,
const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (kstrtoint(desc, 10, &debug) == 0) {
		if (debug == 0)
			...
			pbm_warn("Should be 0/1 [0: disable, 1: enable]\n");
	} else
		pbm_warn("Should be 0/1 [0: disable, 1: enable]\n");
...
static ssize_t mt_pbm_debug_log_reduc_proc_write(struct file *file,
const char __user *buffer, size_t count, loff_t *data)
{
	char desc[32];
	int len = 0;
