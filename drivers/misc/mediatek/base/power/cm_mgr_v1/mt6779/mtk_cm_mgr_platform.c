...
{
	mtk_pm_qos_add_request(&ddr_opp_req_by_cpu_opp, PM_QOS_DDR_OPP,
			MTK_PM_QOS_DDR_OPP_DEFAULT_VALUE);
}
#endif /* USE_CPU_TO_DRAM_MAP */
...
{
	unsigned int cur_vcore_opp = 0;

	cur_vcore_opp = mtk_dvfsrc_query_opp_info(MTK_DVFSRC_CURR_VCORE_OPP);
	if (cur_vcore_opp > CM_MGR_VCORE_OPP_COUNT)
		cur_vcore_opp = 0;

	if (cm_mgr_get_bw() > cm_mgr_vcore_opp_to_bw[cur_vcore_opp])
		return 1;
	else
		return 0;
}
