...
void mtk_drcc_trig(unsigned int code,
		unsigned int hwgatepct,
		unsigned int vreffilt,
		unsigned int autocalibdelay)
{
	mt_secure_call_drcc(MTK_SIP_KERNEL_DRCC_TRIG,
		code, hwgatepct, vreffilt, autocalibdelay);
	mtk_drcc_log2RamConsole();
}
	...
	for (drcc_n = 0; drcc_n < drccNum; drcc_n++) {
		...
		if (drcc_n < drcc_l_num)
			value = mt_secure_call_drcc(
				...
				0,
				0);
		else if (drcc_n >= drcc_l_num)
			value = (mt_secure_call_drcc(
				MTK_SIP_KERNEL_DRCC_READ,
				DRCC_BASEADDR + ((u64)0xFFE4),
				0,
				0,
				0) >> drcc_n) & 0x01;
...
static ssize_t drcc_trig_proc_write(struct file *file,
	const char __user *buffer, size_t count, loff_t *pos)
{
	int code = 0, hwgatepct = 0, vreffilt = 0, autocalibdelay = 0;
	char *buf = (char *) __get_free_page(GFP_USER);
	...
	if (sscanf(buf, "%u %u %u %u",
		...
		&vreffilt,
		&autocalibdelay) != 4) {
		drcc_debug("bad argument!! Should input 4 arguments.\n");
		goto out;
	}

	mtk_drcc_trig(code, hwgatepct, vreffilt, autocalibdelay);
