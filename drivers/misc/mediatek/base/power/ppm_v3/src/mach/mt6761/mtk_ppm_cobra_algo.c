...
static int get_delta_pwr(unsigned int core, unsigned int opp)
{
	...
	cur_pwr = cobra_tbl.ptbl[core-1][opp].power_idx;

	if (opp == COBRA_OPP_NUM - 1)
		prev_pwr = (core > 1)
		? cobra_tbl.ptbl[core-2][opp].power_idx : 0;
	else
		prev_pwr = cobra_tbl.ptbl[core-1][opp + 1].power_idx;
...
void ppm_cobra_update_limit(void *user_req)
{
	...
	int max_thro_core[NR_PPM_CLUSTERS] = {0};
#endif
	int i;
...
prepare_next_round:
			...
			ppm_dbg(COBRA, "[+](delta/Cl/Pwr)=(%d,%d,%d), opp=%d\n",
					delta_power, ChoosenCl,
					ChoosenPwr, opp[PPM_CLUSTER_LL]);
		}
	} else {
		...
		while (delta_power < 0) {
			...
			if (opp[LxLL] >= 0
				...
				&& !ex_core_flag[LxLL]) {
				ChoosenCl = 1;
				ChoosenPwr = get_delta_pwr(ACT_CORE(LL),
					opp[LxLL]);

				if (opp[PPM_CLUSTER_LL]
...
void ppm_cobra_init(void)
{
	...
	for (i = 0; i < TOTAL_CORE_NUM; i++) {
		for (j = 0; j < DVFS_OPP_NUM; j++) {
			p->ptbl[i][j].power_idx =
				20*(i+1)*(DVFS_OPP_NUM-j);
			p->ptbl[i][j].perf_idx =
				40*(i+1)*(DVFS_OPP_NUM-j);
		}
	}
#endif
...
static void ppm_cobra_lookup_by_limit(struct seq_file *m)
{
	...
	for_each_ppm_clusters(i) {
		core = (p->limit[i].core > get_cluster_max_cpu_core(i))
			? get_cluster_max_cpu_core(i) : p->limit[i].core;
		opp = (p->limit[i].opp > get_cluster_min_cpufreq_idx(i))
			? get_cluster_min_cpufreq_idx(i) : p->limit[i].opp;

		if (core)
			budget +=
			cobra_tbl.ptbl[4*i+core-1][opp].power_idx;
