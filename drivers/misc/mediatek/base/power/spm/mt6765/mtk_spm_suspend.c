...
unsigned int wakecnt;
u32 r13flg, spm_dbgflg;
/*spm_info[6][2]
...
static void summary_wakereason(struct wake_status *wakesta)
{
	...
	if (wakesta->r12 == WAKE_SRC_R12_EINT_EVENT_B) {
		...
		if ((spm_dbgflg&0xf) != 0xf) {
			spm_info[5][1]++;
			spm_info[4][0] = spm_info[3][0];
			spm_info[4][1] = spm_info[3][1];
			spm_info[3][0] = spm_info[2][0];
			spm_info[3][1] = spm_info[2][1];
			spm_info[2][0] = spm_info[1][0];
			spm_info[2][1] = spm_info[1][1];
			spm_info[1][0] = spm_info[0][0];
			spm_info[1][1] = spm_info[0][1];
			spm_info[0][0] = r13flg;
...
static unsigned int spm_output_wake_reason(struct wake_status *wakesta)
{
	...
	summary_wakereason(wakesta);//xujia add for power analysis bugreport
	return wr;
}
