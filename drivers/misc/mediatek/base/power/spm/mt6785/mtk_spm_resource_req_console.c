...
#define IS_MTK_CONSOLE_SPM_RES_REQ(x) ({\
	SMC_CALL(GET_PWR_CTRL_ARGS,\
		SPM_PWR_CTRL_IDLE_DRAM, PW_REG_SPM_##x##_REQ, 0) |\
	SMC_CALL(GET_PWR_CTRL_ARGS,\
		SPM_PWR_CTRL_IDLE_SYSPLL, PW_REG_SPM_##x##_REQ, 0) |\
	SMC_CALL(GET_PWR_CTRL_ARGS,\
		SPM_PWR_CTRL_IDLE_BUS26M, PW_REG_SPM_##x##_REQ, 0) |\
	SMC_CALL(GET_PWR_CTRL_ARGS,\
		SPM_PWR_CTRL_SUSPEND, PW_REG_SPM_##x##_REQ, 0);\
})

#define SUSPEND_PWR		(&pwrctrl_suspend)
...
static int __init spm_resource_console_init(void)
{
	...
	struct spm_resource_console_req
		...
		{"resource-requested-ddren", MTK_SPM_RES_EX_DRAM_S1},
		{"resource-requested-apsrc", MTK_SPM_RES_EX_DRAM_S0},
