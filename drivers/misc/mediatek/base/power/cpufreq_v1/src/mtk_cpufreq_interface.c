...
static int cpufreq_sum_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "Temp:%d,%d,%u,%u\n",
		(mtkts_bts_get_hw_temp()/1000),
		(tscpu_get_curr_max_ts_temp()/1000),
...
{
	int temp_tz1 = 0, temp_tz2 = 0, temp_tz3 = 0,
			temp_tz4 = 0, temp_tz5 = 0;
	int ret = 0;
	struct thermal_zone_device *mtktsbattery, *mtktspmic;
	struct thermal_zone_device *mtktsbtsmdpa, *mtktswmt, *mtktscharger;
	...
	seq_printf(m, "mtktz:%d,%d,%d,%d,%d\n",
			temp_tz1/1000, temp_tz2/1000, temp_tz3/1000,
			temp_tz4/1000, temp_tz5/1000);
	return 0;
}
...
int cpufreq_procfs_init(void)
{
	...
	const struct pentry cpu_entries[] = {
		...
		PROC_ENTRY(cpufreq_sum),//xujia add for power analysis bugreport
		PROC_ENTRY(cpufreq_tz_related),//xujia add for power analysis bugreport
	};
