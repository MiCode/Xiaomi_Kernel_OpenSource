...
void dump_pmic_6315(int pmic_num, const char *scenario,
...
static const char * const pmic_6315_name[]
	/* 6315-1     6315-2      6315-3 */
= {NULL, "6_vbuck1", "7_vbuck1", "3_vbuck1"};
	...
	const unsigned int *pmic_gs, unsigned int pmic_gs_len)
{
	...
	char *p;
	struct regulator *vBuck1;
	struct regmap *regmap;

	if (pmic_num < 1 || pmic_num > 3)
		return;
	vBuck1 = regulator_get_optional(NULL, pmic_6315_name[pmic_num]);
	if (IS_ERR(vBuck1)) {
		pr_notice("%s regulator get failed\n", __func__);
		goto ERROR;
	}
	regmap = regulator_get_regmap(vBuck1);
	if (IS_ERR(regmap)) {
	...
	if (slp_chk_golden_diff_mode) {
		...
		"Scenario - 6315-%d - Addr      - Value    - Mask       - Golden     - Wrong Bit\n",
				pmic_num);
		...
		for (i = 0; i < pmic_gs_len; i += 3) {
			...
			if (val1 != val2) {
				dump_cnt++;
				p += snprintf(p, sizeof(buf) - (p - buf),
			"%s - 6315-%d - 0x%08x - 0x%08x- 0x%08x - 0x%08x -",
