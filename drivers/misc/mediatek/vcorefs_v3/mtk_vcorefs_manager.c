...
void __iomem *vcore_dvfsrc_base;
...
static int __init mtk_vcorefs_init(void)
{
	...
	if (!node) {
		vcorefs_crit("[DVFSRC] find node failed\n");
		goto dvfsrc_exit;
	}
	/* get PMIC regmap */
	pmic_node = of_parse_phandle(node, "pmic", 0);
	if (!pmic_node) {
		vcorefs_crit("[DVFSRC] fail to find pmic node\n");
		goto dvfsrc_exit;
	}

	pmic_pdev = of_find_device_by_node(pmic_node);
	if (!pmic_pdev) {
		vcorefs_crit("[DVFSRC] fail to find pmic device or some project no pmic config\n");
		goto dvfsrc_exit;
	}

	chip = dev_get_drvdata(&(pmic_pdev->dev));
	if (!chip) {
		vcorefs_crit("[DVFSRC] fail to find pmic drv data\n");
		goto dvfsrc_exit;
	}
	...
	if (IS_ERR_VALUE(pmic_regmap)) {
		pmic_regmap = NULL;
		vcorefs_crit("[DVFSRC] get pmic regmap fail\n");
		goto dvfsrc_exit;
	}

	vcore_dvfsrc_base = of_iomap(node, 0);
	if (!vcore_dvfsrc_base) {
		vcorefs_crit("[DVFSRC] base failed\n");
		goto dvfsrc_exit;
	}

dvfsrc_exit:
	vcorefs_crit("mtk_vcorefs_init: vcore_dvfsrc_base = %p\n",
		vcore_dvfsrc_base);
	return 0;
}

static void __exit mtk_vcorefs_exit(void)
{
}

module_init(mtk_vcorefs_init);
module_exit(mtk_vcorefs_exit);
