...
/*
 * buddyinfo
 * Node 0, zone   Normal    486    297    143     59     30
 *                           16      7      0      2      1     54
 * Node 0, zone  HighMem     74     18      7     65    161
 *                           67     23     10      0      1     21
 */
...
static void mlog_meminfo(void)
{
	...
	mlock = P2K(global_zone_page_state(NR_MLOCK));
...
#ifdef PROFILE_MLOG_OVERHEAD
	MLOG_PRINTK("[%s] %llu ns\n", __func__, local_clock() - t1);
#endif
}
EXPORT_SYMBOL(mlog);
