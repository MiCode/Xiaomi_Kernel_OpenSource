...
unsigned int jpeg_drv_enc_set_dst_buff(struct device *dev, int dstFd, unsigned int stall_size,
		 unsigned int init_offset, unsigned int offset_mask)
{
	...
	dma_buf = jpg_dmabuf_get(dstFd);
	if (dma_buf == NULL) {
		JPEG_MSG("import handle fail line %d\n", __LINE__);
		return 0;
	}
	if (jpg_dmabuf_get_iova(dma_buf, &obuf_iova, dev, &o_attach, &o_sgt) != 0)
		return 0;
	dst_addr = (dma_addr_t)obuf_iova;
	jpg_dmabuf_free_iova(dma_buf,
		o_attach,
		o_sgt);
	jpg_dmabuf_put(dma_buf);
...
unsigned int jpeg_drv_enc_set_src_buf(struct device *dev, unsigned int yuv_format,
	unsigned int img_stride, unsigned int mem_stride, unsigned int mem_height,
	int srcFd, int srcFd2)
{
	...
	struct dma_buf *dma_buf = NULL;
	struct dma_buf *dma_buf2 = NULL;
	struct dma_buf_attachment *i_attach, *i_attach2;
	struct sg_table *i_sgt, *i_sgt2;
	u64 ibuf_iova, ibuf_iova2;
	...
	if (yuv_format == YUYV || yuv_format == YVYU) {
		dma_buf = jpg_dmabuf_get(srcFd);
		if (dma_buf == NULL) {
			JPEG_MSG("import handle fail line %d\n", __LINE__);
			return 0;
		}
		if (jpg_dmabuf_get_iova(dma_buf, &ibuf_iova, dev, &i_attach, &i_sgt) != 0)
			return 0;
		srcAddr = (dma_addr_t)ibuf_iova;
		jpg_dmabuf_free_iova(dma_buf,
			i_attach,
			i_sgt);
		jpg_dmabuf_put(dma_buf);
		...
		dma_buf2 = jpg_dmabuf_get(srcFd2);
		if (dma_buf2 == NULL) {
			JPEG_MSG("import handle fail line %d\n", __LINE__);
			return 0;
		}
		if (jpg_dmabuf_get_iova(dma_buf2, &ibuf_iova2, dev, &i_attach2, &i_sgt2) != 0)
			return 0;
		srcAddr_C = (dma_addr_t)ibuf_iova2;
		jpg_dmabuf_free_iova(dma_buf2,
					i_attach2,
					i_sgt2);
		jpg_dmabuf_put(dma_buf2);
		JPEG_MSG("srcAddr 0x%p srcAddr_C 0x%p line %d\n", srcAddr, srcAddr_C, __LINE__);
	}
