...
dma_addr_t mu3d_hal_gpd_virt_to_phys(void *vaddr, USB_DIR dir, DEV_UINT32 num)
{
	...
	if (dir == USB_RX)
		ptr = rx_gpd_map[num].p_desc_dma +
		(dma_addr_t) (vaddr - rx_gpd_map[num].p_desc);
	else
		ptr = tx_gpd_map[num].p_desc_dma +
		(dma_addr_t) (vaddr - tx_gpd_map[num].p_desc);
...
void init_bd_list(USB_DIR dir, int num, PBD ptr, dma_addr_t io_ptr, DEV_UINT32 size)
{
	if (dir == USB_RX) {
		...
		os_printk(K_DEBUG, "Rx_bd_List[%d].pStart=%p, pNext=%p, pEnd=%p\n",
			  ...
			  Rx_bd_List[num].pNext, Rx_bd_List[num].pEnd);

		os_printk(K_DEBUG, "rx_bd_map[%d] vir=%p dma=%08llx\n", num,
			  rx_bd_map[num].p_desc,
			  (unsigned long long)rx_bd_map[num].p_desc_dma);

		os_printk(K_DEBUG, "vir=%p dma=%08llx\n",
			  ptr, (unsigned long long)io_ptr);
	} else {
		Tx_bd_List[num].pStart = ptr;
		Tx_bd_List[num].pEnd = (PBD) ((DEV_UINT8 *) (ptr + size) +
					      AT_BD_EXT_LEN * size);
		tx_bd_map[num].p_desc = (void *)ptr;
		tx_bd_map[num].p_desc_dma = io_ptr;
		ptr++;
		Tx_bd_List[num].pNext = (PBD) ((DEV_UINT8 *) ptr +
					       AT_BD_EXT_LEN);

		os_printk(K_DEBUG,
			  "Tx_bd_List[%d].pStart=%p, pNext=%p, pEnd=%p\n",
			  num, Tx_bd_List[num].pStart, Tx_bd_List[num].pNext,
			  Tx_bd_List[num].pEnd);

		os_printk(K_DEBUG,
			  "tx_bd_map[%d] vir=%p dma=%08llx\n", num,
			  tx_bd_map[num].p_desc,
			  (unsigned long long)tx_bd_map[num].p_desc_dma);
...
void init_gpd_list(USB_DIR dir, int num, PGPD ptr, dma_addr_t io_ptr, DEV_UINT32 size)
{
	if (dir == USB_RX) {
		...
		Rx_gpd_List[num].pEnd = (PGPD) ((DEV_UINT8 *) (ptr + size) +
						AT_GPD_EXT_LEN * size);
		rx_gpd_map[num].p_desc = (void *)ptr;
		rx_gpd_map[num].p_desc_dma = io_ptr;
		ptr++;
		Rx_gpd_List[num].pNext = (PGPD) ((DEV_UINT8 *) ptr +
						 AT_GPD_EXT_LEN);

		qmu_printk(K_INFO,
			   "Rx_gpd_List[%d].pStart=%p, pNext=%p, pEnd=%p\n",
			   num, Rx_gpd_List[num].pStart,
			   Rx_gpd_List[num].pNext,
			   Rx_gpd_List[num].pEnd);

		qmu_printk(K_INFO,
			   "rx_gpd_map[%d] vir=%p dma=%08llx\n", num,
			   rx_gpd_map[num].p_desc,
			   (unsigned long long)rx_gpd_map[num].p_desc_dma);

		qmu_printk(K_INFO, "vir=%p, dma=%08llx\n",
			   ptr, (unsigned long long)io_ptr);
	} else {
		Tx_gpd_List[num].pStart = ptr;
		Tx_gpd_List[num].pEnd = (PGPD) ((DEV_UINT8 *) (ptr + size) +
						AT_GPD_EXT_LEN * size);
		tx_gpd_map[num].p_desc = (void *)ptr;
		tx_gpd_map[num].p_desc_dma = io_ptr;
		ptr++;
		Tx_gpd_List[num].pNext = (PGPD) ((DEV_UINT8 *) ptr +
						 AT_GPD_EXT_LEN);

		qmu_printk(K_INFO,
			   "Tx_gpd_List[%d].pStart=%p, pNext=%p, pEnd=%p\n",
			   num, Tx_gpd_List[num].pStart,
			   Tx_gpd_List[num].pNext,
			   Tx_gpd_List[num].pEnd);

		qmu_printk(K_INFO, "tx_gpd_map[%d] vir=%p dma=%08llx\n", num,
			   tx_gpd_map[num].p_desc,
			   (unsigned long long)tx_gpd_map[num].p_desc_dma);
...
void _ex_mu3d_hal_alloc_qmu_mem(struct device *dev)
{
	...
	for (i = 1; i <= MAX_QMU_EP; i++) {
		...
		ptr = (TGPD *) dma_alloc_coherent(dev, size,
		...
		qmu_printk(K_INFO,
			   "ALLOC RX GPD End [%d] Virtual Mem=%p, DMA addr=%08llx\n",
		...
		qmu_printk(K_INFO, "RQSAR[%d]=%08llx\n", i,
			   (unsigned long long)
			   mu3d_hal_gpd_virt_to_phys(Rx_gpd_end[i],
						     USB_RX, i));
		...
		qmu_printk(K_INFO,
			   "ALLOC TXGPD End %d VirtualMem=%p DMAaddr=%08llx\n",
		...
		qmu_printk(K_INFO, "TQSAR[%d]=%08llx\n", i,
			   (unsigned long long)
			   mu3d_hal_gpd_virt_to_phys(Tx_gpd_end[i],
						     USB_TX, i));
	}
}
...
void mu3d_hal_alloc_qmu_mem(void)
{
	...
	for (i = 1; i <= MAX_QMU_EP; i++) {
		...
		os_printk(K_DEBUG,
			  "ALLOC RXGPD End %d VirtualMem=%p DMAaddr=%08llx\n",
			  i, Rx_gpd_end[i], (unsigned long long)io_ptr);
...
void _ex_mu3d_hal_init_qmu(void)
{
	...
	for (i = 1; i <= MAX_QMU_EP; i++) {
		qmu_printk(K_INFO,
			   "==EP[%d]==Start addr RXQ=0x%08lx, TXQ=0x%08lx\n",
			   i,
			   (uintptr_t)
			   mu3d_hal_gpd_virt_to_phys(Rx_gpd_head[i],
						     USB_RX, i),
			   (uintptr_t)
			   mu3d_hal_gpd_virt_to_phys(Tx_gpd_head[i],
						     USB_TX, i));
		QCR |= QMU_RX_EN(i);
		QCR |= QMU_TX_EN(i);
	...
	os_writel(U3D_QIESR1, TXQ_EMPTY_IESR | TXQ_CSERR_IESR
		  | TXQ_LENERR_IESR |
		  RXQ_EMPTY_IESR | RXQ_CSERR_IESR |
		  RXQ_LENERR_IESR | RXQ_ZLPERR_IESR);
...
void mu3d_hal_init_qmu(void)
{
	...
	os_writel(U3D_QIESR1,
		  TXQ_EMPTY_IESR | TXQ_CSERR_IESR | TXQ_LENERR_IESR |
		  RXQ_EMPTY_IESR |
 ...
 * arg3: data length, arg4: ep number,
 * arg5: with bd or not, arg6: write hwo bit or not,
 * arg7: write ioc bit or not
 */
TGPD *_ex_mu3d_hal_prepare_tx_gpd(TGPD *gpd, dma_addr_t pBuf,
				  DEV_UINT32 data_len,
				  DEV_UINT8 ep_num, DEV_UINT8 _is_bdp,
				  DEV_UINT8 isHWO,
				  DEV_UINT8 ioc, DEV_UINT8 bps, DEV_UINT8 zlp)
{
	/*
	 * qmu_printk(K_DEBUG,
	 * "TX %s gpd=%p epnum=%d len=%d zlp=%d
	 * size(TGPD)=%lld pBuf=%llx\n",
	 * __func__, gpd, ep_num, data_len, zlp,
	 * (u64) sizeof(TGPD),
	 * (long long unsigned int) pBuf);
	 */
...
#else
	TGPD_SET_DATA_TXHI(gpd, (unsigned long) (pBuf));
#endif
#endif
	...
	TGPD_SET_NEXT(gpd,
		      (unsigned long)
		      mu3d_hal_gpd_virt_to_phys(Tx_gpd_end[ep_num],
						USB_TX, ep_num));
#if defined(CONFIG_USB_MU3D_DRV_36BIT)
#ifdef CONFIG_ARM64
	TGPD_SET_NEXT_TXHI(gpd,
	(unsigned long) (mu3d_hal_gpd_virt_to_phys(Tx_gpd_end[ep_num],
							USB_TX,
							ep_num) >> 32));
#else
	TGPD_SET_NEXT_TXHI(gpd,
...
TGPD *mu3d_hal_prepare_tx_gpd(TGPD *gpd, dma_addr_t pBuf,
			      DEV_UINT32 data_len,
			      DEV_UINT8 ep_num, DEV_UINT8 _is_bdp,
			      DEV_UINT8 isHWO,
			      DEV_UINT8 ioc, DEV_UINT8 bps,
			      DEV_UINT8 zlp)
{
	DEV_UINT32 offset;
	DEV_INT32 i;
	...
	os_printk(K_INFO,
		  "%s gpd=%p, epnum=%d, len=%d, _is_bdp=%d\n",
		  __func__,
	...
	if (!_is_bdp) {
		...
		bd_num = (!(length % offset)) ?
		  (length / offset) : ((length / offset) + 1);

		os_printk(K_INFO, "bd_num=%d\n", bd_num);

		/*If the size of BD buffer is bigger than the
		 *length of actual transfer, use the actual length
		 */
		if (offset > length)
			offset = length;
		...
		for (i = 0; i < bd_num; i++) {
			...
			if (i == (bd_num - 1)) {	/*The last BD */
			  ...
			  /*Set "BD Checksum" */
				...
				/*There is no data left to
				 *be transferred by GPD
				 */
				/* data_len=length; */
				data_len = 0;
			...
			/*"Data Buffer Length" = the MAX BD transfer size */
				/*Store the data pointer to "Data Buffer" */
				TBD_SET_DATA(bd, (unsigned long)pBuffer);
				...
				TBD_SET_NEXT(bd,
					     (unsigned long)
					     bd_virt_to_phys(bd_next, USB_TX,
								ep_num));
				TBD_SET_CHKSUM(bd, CHECKSUM_LENGTH);
			  /*Set BD Checksum */
		...
		TGPD_SET_DATA(gpd, (unsigned long
				   )bd_virt_to_phys(bd_head, USB_TX, ep_num));
	...
	TGPD_SET_NEXT(gpd,
		      (unsigned long)
		      mu3d_hal_gpd_virt_to_phys(
			Tx_gpd_end[ep_num], USB_TX, ep_num));
 ...
 * @args - arg1: gpd address, arg2: data buffer
 * address, arg3: data length,
 * arg4: ep number, arg5: with bd or not,
 * arg6: write hwo bit or not,  arg7: write ioc bit or not
 */
TGPD *_ex_mu3d_hal_prepare_rx_gpd(TGPD *gpd, dma_addr_t pBuf,
				  DEV_UINT32 data_len,
				  DEV_UINT8 ep_num, DEV_UINT8 _is_bdp,
				  DEV_UINT8 isHWO,
				  DEV_UINT8 ioc, DEV_UINT8 bps,
...
{
	/*
	 * qmu_printk(K_DEBUG, "[RX] %s gpd=%p,
	 * epnum=%d, len=%d, pBuf=%llx\n",
	 * __func__, gpd, ep_num,
	 * data_len, (long long unsigned int)pBuf);
	 */
	/*Set actual data point to "DATA Buffer" */
	TGPD_SET_DATA(gpd, (unsigned long)pBuf);
#if defined(CONFIG_USB_MU3D_DRV_36BIT)
...
#else
	TGPD_SET_DATA_RXHI(gpd, (unsigned long) (pBuf));
#endif
#endif
	/*Clear "BDP(Buffer Descriptor Present)" flag */
	TGPD_CLR_FORMAT_BDP(gpd);
	/*
	 * Set "Allow Data Buffer Length" = 0
	 * (If data length >
	 *  GPD buffer length, use BDs),
	 *  data_len (If data length <
	 * GPD buffer length, only use GPD)
	 */
	...
	TGPD_SET_NEXT(gpd,
		      (unsigned long)mu3d_hal_gpd_virt_to_phys(
			Rx_gpd_end[ep_num], USB_RX, ep_num));

#if defined(CONFIG_USB_MU3D_DRV_36BIT)
#ifdef CONFIG_ARM64
		TGPD_SET_NEXT_RXHI(gpd,
		(unsigned long) (mu3d_hal_gpd_virt_to_phys(Rx_gpd_end[ep_num],
							USB_RX, ep_num) >> 32));
#else
		TGPD_SET_NEXT_RXHI(gpd,
		(unsigned long) (mu3d_hal_gpd_virt_to_phys(Rx_gpd_end[ep_num],
	...
	/* os_printk(K_DEBUG,"Rx gpd info
	 *{ HWO %d, Next_GPD %x ,DataBufferLength %d,
	 */
			      ...
			      DEV_UINT8 ioc, DEV_UINT8 bps, DEV_UINT32 cMaxPacketSize)
{
	...
	if (!_is_bdp) {
		...
		for (i = 0; i < bd_num; i++) {
			...
			if (i == (bd_num - 1)) {
			  ...
			  /*Set "Transferred Data Length" = 0 */
				...
				/*The last one's data buffer lengnth
				 * must be precise, or the GPD will never
				 * done unless ZLP or short packet.
				 */
				/*"Allow Data Buffer Length" =
				 *the rest of data length*
				 */
				length =
				    (!(length % cMaxPacketSize)) ?
							(length) : ((length /
							cMaxPacketSize) +
							1) * cMaxPacketSize;
			...
			/*WARNING:UNNECESSARY_ELSE: else is not
		...
		/*Set the BD pointer into "BD Pointer" at GPD */
		TGPD_SET_DATA(gpd, (unsigned long)bd_virt_to_phys(bd_head,
								  USB_RX,
								  ep_num));
 ...
 */
void _ex_mu3d_hal_insert_transfer_gpd(DEV_INT32 ep_num, USB_DIR dir,
				      dma_addr_t buf,
				      DEV_UINT32 count,
				      ...
				      DEV_UINT32 maxp)
{
	...
	if (dir == USB_TX) {
		...
		_ex_mu3d_hal_prepare_tx_gpd(gpd, buf, count,
					    ep_num, IS_BDP,
					    isHWO, ioc, bps, zlp);
	} else if (dir == USB_RX) {
		gpd = Rx_gpd_end[ep_num];
		_ex_mu3d_hal_prepare_rx_gpd(gpd, buf, count,
					    ep_num, IS_BDP,
					    isHWO, ioc, bps, maxp);
	}
}

void mu3d_hal_insert_transfer_gpd(DEV_INT32 ep_num, USB_DIR dir,
				  dma_addr_t buf,
				  DEV_UINT32 count,
				  DEV_UINT8 isHWO,
				  DEV_UINT8 ioc,
				  ...
				  DEV_UINT32 maxp)
{
	...
	if (dir == USB_TX) {
		...
		mu3d_hal_prepare_tx_gpd(gpd, buf,
					count, ep_num,
					IS_BDP, isHWO,
					ioc, bps, zlp);
	} else if (dir == USB_RX) {
		...
		mu3d_hal_prepare_rx_gpd(gpd, buf, count,
					ep_num, IS_BDP, isHWO,
					ioc, bps, maxp);
	}
}
 ...
 * mu3d_hal_init_qmu ->mu3d_hal_start_qmu ->
 *nmu3d_hal_insert_transfer_gpd -> mu3d_hal_resume_qmu)
 * @args - arg1: ep number, arg2: dir
 */
void mu3d_hal_start_qmu(DEV_INT32 Q_num, USB_DIR dir)
{
	...
	if (dir == USB_TX) {
		...
		os_writel(U3D_QCR1, QCR | QMU_TX_ZLP(Q_num));
#endif
		os_writel(U3D_QEMIESR, os_readl(U3D_QEMIESR) |
			  QMU_TX_EMPTY(Q_num));
		os_writel(U3D_TQERRIESR0, QMU_TX_LEN_ERR(Q_num) |
			  QMU_TX_CS_ERR(Q_num));
		...
		os_writel(U3D_QEMIESR, os_readl(U3D_QEMIESR) |
			  QMU_RX_EMPTY(Q_num));
		os_writel(U3D_RQERRIESR0, QMU_RX_LEN_ERR(Q_num) |
			  QMU_RX_CS_ERR(Q_num));
		os_writel(U3D_RQERRIESR1, QMU_RX_EP_ERR(Q_num)
			  | QMU_RX_ZLP_ERR(Q_num));
...
/**
 * mu3d_hal_stop_qmu - stop qmu function
 *(after qmu stop, fifo should be flushed)
 * @args - arg1: ep number, arg2: dir
 */
	...
	if (dir == USB_TX) {
		if (!(os_readl(USB_QMU_TQCSR(q_num)) & (QMU_Q_ACTIVE))) {
			...
			if (wait_for_value_us(USB_QMU_TQCSR(q_num),
				...
				qmu_printk(K_DEBUG,
					   "Tx%d stop Now! CSR=0x%x\n",
					   q_num,
					   os_readl(USB_QMU_TQCSR(q_num)));
			else {
				qmu_printk(K_CRIT,
					   "Tx%d UNSTOPABLE!! CSR=0x%x\n",
					   q_num,
					   os_readl(USB_QMU_TQCSR(q_num)));
				WARN_ON(1);
			}
		...
		os_writel(U3D_QCR0,
			  os_readl(U3D_QCR0)&~(QMU_TX_CS_EN(q_num)));
		os_writel(U3D_QEMIESR,
			  os_readl(U3D_QEMIESR)&~(QMU_TX_EMPTY(q_num)));
		os_writel(U3D_TQERRIESR0,
			  os_readl(U3D_TQERRIESR0)
				  &~(QMU_TX_LEN_ERR(q_num))
			  &~(QMU_TX_CS_ERR(q_num)));
		...
		if (!(os_readl(USB_QMU_RQCSR(q_num)) & QMU_Q_ACTIVE)) {
			...
			if (wait_for_value_us(USB_QMU_RQCSR(q_num),
				QMU_Q_ACTIVE, 0, 10, 100) == RET_SUCCESS)
				qmu_printk(K_DEBUG,
					   "Rx%d stop Now! CSR=0x%x\n",
					   q_num,
					   os_readl(USB_QMU_RQCSR(q_num)));
			else {
				qmu_printk(K_CRIT,
					   "Rx%d UNSTOPABLE!! CSR=0x%x\n",
					   q_num,
					   os_readl(USB_QMU_RQCSR(q_num)));
				WARN_ON(1);
			}
...
void mu3d_hal_send_stall(DEV_INT32 q_num, USB_DIR dir)
{
	if (dir == USB_TX) {
		USB_WriteCsr32(U3D_TX1CSR0, q_num,
			       USB_ReadCsr32(U3D_TX1CSR0, q_num) |
			       TX_SENDSTALL);
...
void _ex_mu3d_hal_flush_qmu(DEV_INT32 Q_num, USB_DIR dir)
{
	...
	if (dir == USB_TX) {
		...
		Tx_gpd_end[Q_num] =
		  Tx_gpd_last[Q_num] = Tx_gpd_head[Q_num];
		...
		os_writel(USB_QMU_TQSAR(Q_num),
			  mu3d_hal_gpd_virt_to_phys(
			    Tx_gpd_last[Q_num], USB_TX, Q_num));
		qmu_printk(K_DEBUG,
			   "USB_QMU_TQSAR %x\n",
...
void mu3d_hal_flush_qmu(DEV_INT32 Q_num, USB_DIR dir)
{
	...
	if (dir == USB_TX) {
		...
		gpd_current =
		  (TGPD *) (uintptr_t) (os_readl(USB_QMU_TQCPR(Q_num)));

		/*If gpd_current = 0, it means QMU
		 *has not yet to execute GPD in QMU.
		 */
		if (!gpd_current) {
