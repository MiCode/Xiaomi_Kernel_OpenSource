...
static struct vcorefs_profile vcorefs_ctrl = {
	.plat_init_opp	= 0,
	.init_done	= 0,
#if IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT6759) || IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT6758) \
	|| IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT6771)
	.autok_finish   = 1,
#else
	.autok_finish   = 0,
#endif
...
{
#if !IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT6759)
	struct vcorefs_profile *pwrctrl = &vcorefs_ctrl;
...
EXPORT_SYMBOL(spm_msdc_dvfs_setting);
...
void vcorefs_drv_init(int plat_init_opp)
{
	...
	ccci_set_svcorefs_request_dvfs_cb(&vcorefs_request_dvfs_opp);
#endif
				...
				const char *buf, size_t count)
{
	...
	if (!strcmp(cmd, "feature_en")) {
		...
		mutex_unlock(&vcorefs_mutex);
#if IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT6775)
...
int init_vcorefs_sysfs(void)
{
	...
	vcorefs_kobj = kobject_create_and_add("vcorefs", kernel_kobj);
	if (!vcorefs_kobj)
		return -ENOMEM;

	r = sysfs_create_group(vcorefs_kobj, &vcorefs_attr_group);
	if (r) {
		vcorefs_crit("[DVFSRC] FAILED TO CREATE VCOREFS (%d)\n", r);
		kobject_put(vcorefs_kobj);
	}

	return r;
}

MODULE_DESCRIPTION("VCOREFS V3");
