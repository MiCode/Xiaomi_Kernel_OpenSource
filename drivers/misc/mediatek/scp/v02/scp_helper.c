...
#define SCP_READY_TIMEOUT (3 * HZ) /* 3 seconds*/
...
int reset_scp(int reset)
{
	...
	if (scp_enable[SCP_A_ID]) {
		...
		pr_err("[SCP] %s: ERROR: scp_enable = %d.\n", __func__,
		       scp_enable[SCP_A_ID]);

	pr_notice("[SCP] %s: done\n", __func__);
	return 0;
}
...
void scp_sys_reset_ws(struct work_struct *ws)
{
	...
	pr_debug("[SCP] Core0: RSTN,STA,PC= %x/%x/%x\n",
			readl(R_CORE0_SW_RSTN_CLR), readl(R_CORE0_STATUS),
			readl(R_CORE0_MON_PC));

	pr_debug("[SCP] Core1: RSTN,STA,PC= %x/%x/%x\n",
			readl(R_CORE1_SW_RSTN_CLR), readl(R_CORE1_STATUS),
			readl(R_CORE1_MON_PC));

	pr_debug("[SCP] scp_region_info = %px\n", scp_region_info);

	pr_debug("[SCP]    ldr_start/sz    = %x -%x\n",
			scp_region_info->ap_loader_start,
			scp_region_info->ap_loader_size);
	pr_debug("[SCP]    fw_start/sz     = %x -%x\n",
			scp_region_info->ap_firmware_start,
			scp_region_info->ap_firmware_size);

	pr_debug("[SCP] dvfs setting, Freq= %x\n", readl(CURRENT_FREQ_REG));
...
static int scp_device_probe(struct platform_device *pdev)
{
	...
	if (!scp_enable[SCP_A_ID]) {
		pr_err("[SCP] ERROR: read dts fail, force scp ON.\n");
		scp_enable[SCP_A_ID] = 1;
	}
