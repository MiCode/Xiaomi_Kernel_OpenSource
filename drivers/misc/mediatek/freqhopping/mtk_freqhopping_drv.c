...
static int mt_freqhopping_ioctl(struct file *file,
...
static int mt_fh_disable_usrdef(struct freqhopping_ioctl *fh_ctl)
{
	...
	if (fh_ctl->pll_id < FH_PLL_COUNT) {
		g_p_fh_hal_drv->mt_fh_lock(&flags);
		memset(&g_fh_drv_usr_def[pll_id], 0,
			sizeof(g_fh_drv_usr_def[pll_id]));
#ifdef CONFIG_MTK_TINYSYS_SSPM_SUPPORT
...
static int freqhopping_userdefine_proc_read(struct seq_file *m, void *v)
{
	...
	seq_puts(m, "===============================================\r\n");
	seq_puts(m, "     freq ==  delta t ==  delta f ==  up bond == low bond ==      dds ==\r\n");

	for (i = 0; i < g_drv_pll_count; ++i) {
		seq_printf(m,
			"%10d  0x%08x  0x%08x  0x%08x  0x%08x  0x%08x\r\n",
			0, g_fh_drv_usr_def[i].dt,
...
static ssize_t freqhopping_userdefine_proc_write(struct file *file,
						 const char *buffer,
...
static int freqhopping_status_proc_read(struct seq_file *m, void *v)
{
	...
	seq_puts(m, "===============================================\r\n");
	seq_puts(m, "id == fh_status == pll_status == setting_id");
	seq_puts(m, " == curr_freq == user_defined ==\r\n");
#ifdef CONFIG_MTK_TINYSYS_SSPM_SUPPORT
	for (i = 0; i < g_drv_pll_count; ++i) {
...
static ssize_t freqhopping_dramc_proc_write(struct file *file,
					    const char *buffer,
					    size_t count, loff_t *data)
{
	return (ssize_t) (g_p_fh_hal_drv->proc.dramc_write(file,
		buffer, count, data));
}
...
static ssize_t freqhopping_dvfs_proc_write(struct file *file,
					   const char *buffer,
					   size_t count, loff_t *data)
{
	return (ssize_t) (g_p_fh_hal_drv->proc.dvfs_write(file,
		buffer, count, data));
}
...
static int freqhopping_userdefine_proc_open(struct inode *inode,
	struct file *file)
{
...
static int freqhopping_debug_proc_init(void)
{
	...
	prDebugEntry = proc_create("freqhopping_debug", 0664,
	fh_proc_dir, &freqhopping_debug_fops);
	if (prDebugEntry) {
		/* prDebugEntry->read_proc  = freqhopping_debug_proc_read; */
		/* prDebugEntry->write_proc = freqhopping_debug_proc_write; */
		FH_MSG("[%s]: create freqhopping_debug", __func__);
	} else {
		FH_MSG("[%s]: failed create freqhopping_debug", __func__);
		return 1;
	}


	prDramcEntry = proc_create("dvfs", 0664, fh_proc_dir, &dvfs_fops);
	if (prDramcEntry) {
		FH_MSG("[%s]: create dvfs", __func__);
	} else {
		FH_MSG("[%s]: failed create dvfs", __func__);
		return 1;
	}


	prDumpregEntry = proc_create("dumpregs", 0664, fh_proc_dir,
		&dumpregs_fops);
	if (prDumpregEntry) {

		FH_MSG("[%s]: create dumpregs", __func__);
	} else {
		FH_MSG("[%s]: failed dumpregs", __func__);
		return 1;
	}


	/* /proc/freqhopping/status */
	prStatusEntry = proc_create("status", 0664, fh_proc_dir,
	&status_fops);
	if (prStatusEntry) {
		FH_MSG("[%s]: create status", __func__);
	} else {
		FH_MSG("[%s]: failed status", __func__);
		return 1;
	}

	/* /proc/freqhopping/userdefine */
	prUserdefEntry = proc_create("userdef", 0664, fh_proc_dir,
	&userdef_fops);
	if (prUserdefEntry) {
		FH_MSG("[%s]: create userdef", __func__);
	} else {
		FH_MSG("[%s]: failed create userdef", __func__);
		return 1;
	}
...
int mt_dfs_mempll(unsigned int target_dds)
{
	if ((!g_p_fh_hal_drv) || (!g_p_fh_hal_drv->mt_dfs_mempll)) {
		FH_MSG("[%s]: ", __func__);
		FH_MSG("g_p_fh_hal_drv or obj_mt_dfs_mempll is uninitialized");
		return 1;
	}
...
int mt_dfs_general_pll(unsigned int pll_id, unsigned int target_dds)
{
	if ((!g_p_fh_hal_drv) || (!g_p_fh_hal_drv->mt_dfs_general_pll)) {
		FH_MSG("[%s]: obj_mt_dfs_general_pll is uninitialized."
		, __func__);
		return 1;
	}
...
void mt_freqhopping_init(void)
{
	...
	/* TODO_HAL: wait for clkmgr to invoke this function */
