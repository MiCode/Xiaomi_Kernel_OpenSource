...
static ssize_t st54spi_sync(struct st54spi_data *st54spi, struct spi_message *message)
{
...
static inline ssize_t st54spi_sync_write(struct st54spi_data *st54spi, size_t len)
{
...
static inline ssize_t st54spi_sync_read(struct st54spi_data *st54spi, size_t len)
{
	struct spi_transfer t = {
...
static ssize_t st54spi_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
...
static ssize_t st54spi_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	struct st54spi_data *st54spi;
	ssize_t status = 0;
	unsigned long missing;
...
static int st54spi_message(struct st54spi_data *st54spi, struct spi_ioc_transfer *u_xfers,
...
static long st54spi_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int err = 0;
	int retval = 0;
	struct st54spi_data *st54spi;
...
#ifdef CONFIG_COMPAT
static long st54spi_compat_ioc_message(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct spi_ioc_transfer __user *u_ioc;
	int retval = 0;
	struct st54spi_data *st54spi;
...
static long st54spi_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	if (_IOC_TYPE(cmd) == SPI_IOC_MAGIC &&
		_IOC_NR(cmd) == _IOC_NR(SPI_IOC_MESSAGE(0)) &&
		_IOC_DIR(cmd) == _IOC_WRITE)
