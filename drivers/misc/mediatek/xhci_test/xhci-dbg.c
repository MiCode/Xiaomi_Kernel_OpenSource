...
void mtktest_xhci_debug_trb(struct xhci_hcd *xhci, union xhci_trb *trb)
{
	...
	case TRB_TYPE(TRB_LINK):
		...
		xhci_dbg(xhci, "Next ring segment DMA address = 0x%llx(0x%llx)\n", address, address);
	...
	case TRB_TYPE(TRB_TRANSFER):
		 ...
		 */
		xhci_dbg(xhci, "DMA address or buffer contents= %llu(0x%llx)\n", address, address);
		break;
	case TRB_TYPE(TRB_COMPLETION):
		address = trb->event_cmd.cmd_trb;
		xhci_dbg(xhci, "Command TRB pointer = %llu(0x%llx)\n", address, address);
