...
static int musb_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(base)) {
		DBG(0, "%s get base fail\n", __func__);
		return PTR_ERR(base);
	}
	DBG(0, "%s base(mac): physical=0x%1x, virtual=0x%1x\n", __func__,
			(unsigned long)(iomem->start), (unsigned long)base);

	iomem = platform_get_resource(pdev, IORESOURCE_MEM, 1);
	pbase = devm_ioremap(dev, iomem->start, resource_size(iomem));
	if (IS_ERR(pbase)) {
		DBG(0, "%s get pbase fail\n", __func__);
		return PTR_ERR(pbase);
	}
	DBG(0, "%s pbase(phy): physical=0x%1x, virtual=0x%1x\n", __func__,
			(unsigned long)(iomem->start), (unsigned long)pbase);

	irq = platform_get_irq(pdev, 0);
	if (irq <= 0) {
		DBG(0, "%s get irq fail\n", __func__);
		return -ENODEV;
	}
	DBG(0, "%s irq: %d\n", __func__, irq);
