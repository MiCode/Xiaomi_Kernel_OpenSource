...
unsigned long dcm_mcusys_par_wrap_complex2_base;

#define DCM_NODE "mediatek,mt6886-dcm"
...
void dcm_dump_regs(void)
{
	...
	REG_DUMP(MCUSYS_PAR_WRAP_COMPLEX2_STALL_DCM_CONF);
	...
	REG_DUMP(VDNR_DCM_TOP_VLP_PAR_BUS_u_VLP_PAR_BUS_CTRL_3);
}
...
struct DCM_BASE dcm_base_array[] = {
	...
	DCM_BASE_INFO(dcm_mcusys_par_wrap_complex2_base),
};
...
static void disabled_by_dts(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6886-dcm");
	if (node) {
		...
		dcm_pr_notice("[DCM] mediatek,mt6886-dcm not found in DTS!\n");
	}
}

static int __init mt6886_dcm_init(void)
{
	int ret = 0;
...
static void __exit mt6886_dcm_exit(void)
{
}
MODULE_SOFTDEP("pre:mtk_dcm.ko");
module_init(mt6886_dcm_init);
module_exit(mt6886_dcm_exit);
