...
void mt_getmembw_registerCB(getmembw_func pCB)
{
	if (pCB == NULL) {
		...
		LastWordAllCount = 0;
		EMI_ERR("[get_mem_bw] register CB is a null function\n");
	}	else {
		EMI_DBG("[get_mem_bw] register CB successful\n");
	}
...
unsigned long long get_mem_bw(void)
{
	...
	/* EMI_DBG("[get_mem_bw]emi_dcm_disable = %d\n", emi_dcm_disable); */
	current_time_ns = sched_clock();
	time_period_ns = current_time_ns - last_time_ns;
	/* EMI_DBG("[get_mem_bw]last_time=%llu, current_time=%llu,
	...
	if (time_period_ns >= 0xFFFFFFFF) { /* uint32_t overflow */
		...
		EMI_DBG("time_period_ns overflow lst\n");
		/* EMI_DBG("[get_mem_bw] time_period_ns overflow 1st\n"); */

		if (time_period_ns >= 0xFFFFFFFF) { /* uint32_t overflow */
			do_div(time_period_ns, 1000);
			do_div(throughput, 1000);
			EMI_DBG("time_period_ns overflow 2nd\n");
		}
	}

	do_div(throughput, time_period_ns);
	/* EMI_DBG("[get_mem_bw]Total MEMORY THROUGHPUT =%llu(MB/s),
	...
	/*EMI_DBG("[get_mem_bw]loop count = %d,
	...
	/*EMI_DBG("[get_mem_bw]throughput = %llx\n", throughput);*/
...
{
	/*EMI_DBG("[get_mem_bw]mem_bw_suspend_callback\n");*/
	LastWordAllCount = 0;
	BM_Pause();
	return 0;
}
...
{
	/* EMI_DBG("[get_mem_bw]mem_bw_resume_callback\n"); */
	BM_Continue();
	return 0;
}
...
static int __init mon_kernel_init(void)
{
	...
	/* disable_infra_dcm(); */
	emi_dcm_disable = BM_GetEmiDcm();
	/* EMI_DBG("[MT_MEM_BW]emi_dcm_disable = %d\n", emi_dcm_disable); */
	BM_SetEmiDcm(0xff);	/* disable EMI dcm */
	...
	if (ret) {
		EMI_ERR("fail to register mem_bw device @ %s()\n", __func__);
		goto out;
	}

	ret = platform_driver_register(&mt_mem_bw_pdrv);
	if (ret) {
		EMI_ERR("fail to register mem_bw driver @ %s()\n", __func__);
