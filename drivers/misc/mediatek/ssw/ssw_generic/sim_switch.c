...
static int set_sim_gpio(unsigned int mode)
{
	...
	SSW_DBG("Current sim mode(%d), SIM1_MODE(0x%x), SIM2_MODE(0x%x), SIM1_PULL(0x%x), SIM2_PULL(0x%x)\n", \
		mode, sim_switch_readl(reg_base, GPIO_SIM1_MODE), sim_switch_readl(reg_base, GPIO_SIM2_MODE), \
		sim_switch_readl(reg_base, GPIO_SIM1_PULL), sim_switch_readl(reg_base, GPIO_SIM2_PULL));
