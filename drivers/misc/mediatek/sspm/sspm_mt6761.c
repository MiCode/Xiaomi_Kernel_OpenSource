...
#include "sspm_ipi_define_mt6761.h"
...
static int __init mt6761_sspm_module_init(void)
{
	...
	pr_info("[SSPM] mt6761-sspm_module_init.\n");

	/* static initialise */
	sspm_ready = 0;

	mt6761_sspm_workqueue = create_workqueue("mt6761-SSPM_WQ");

	if (!mt6761_sspm_workqueue) {
...
static int __init mt6761_sspm_probe(struct platform_device *pdev)
{
	struct resource *res;
	...
	recv_pintable = mt6761_recv_pintable;
	pin_name = mt6761_pin_name;

	sspm_reserve_mblock = mt6761_sspm_reserve_mblock;

	pr_info("[SSPM] mt6761-sspm_probe Done.\n");

	mt6761_sspm_module_init();

	return 0;
}

#ifdef CONFIG_PM
static int mt6761_sspm_suspend(struct device *dev)
{
	sspm_timesync_suspend();
	return 0;
}

static int mt6761_sspm_resume(struct device *dev)
{
	sspm_timesync_resume();
	return 0;
}

static const struct dev_pm_ops mt6761_sspm_dev_pm_ops = {
	.suspend = mt6761_sspm_suspend,
	.resume  = mt6761_sspm_resume,
};
#endif

static const struct of_device_id mt6761_sspm_of_match[] = {
	{ .compatible = "mediatek,mt6761-sspm", },
	{},
};

static const struct platform_device_id mt6761_sspm_id_table[] = {
	{ "mt6761-sspm", 0},
	{ },
};

static struct platform_driver mtk_mt6761_sspm_driver __refdata = {
	.probe  = mt6761_sspm_probe,
	.remove = NULL,
	.shutdown = NULL,
	.suspend = NULL,
	.resume = NULL,
	.driver = {
		.name = "mt6761-sspm",
		.owner = THIS_MODULE,
		.of_match_table = mt6761_sspm_of_match,
#ifdef CONFIG_PM
		.pm = &mt6761_sspm_dev_pm_ops,
#endif
	},
	.id_table = mt6761_sspm_id_table,
};

/*
 * driver initialization entry point
 */
static int __init mt6761_sspm_init(void)
{
	return platform_driver_register(&mtk_mt6761_sspm_driver);
}

static void __exit mt6761_sspm_exit(void)
{
	pr_info("[SSPM] mt6761-sspm Exit.\n");
}

MODULE_DESCRIPTION("MEDIATEK Module SSPM driver");
MODULE_LICENSE("GPL v2");

subsys_initcall(mt6761_sspm_init);
module_exit(mt6761_sspm_exit);
