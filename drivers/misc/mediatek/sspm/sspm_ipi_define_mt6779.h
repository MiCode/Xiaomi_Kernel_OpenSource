 ...
 */
struct _pin_send mt6779_send_pintable[] = {
...
#define MT6779_TOTAL_SEND_PIN	\
		(sizeof(mt6779_send_pintable)/sizeof(struct _pin_send))


/* act, mbox, slot, size, shared, retdata, lock, share_grp, unused */
struct _pin_recv mt6779_recv_pintable[] = {
...
#define MT6779_TOTAL_RECV_PIN	\
		(sizeof(mt6779_recv_pintable)/sizeof(struct _pin_recv))

/* info for all mbox: start, end, used_slot, mode, unused */
struct _mbox_info mt6779_mbox_table[IPI_MBOX_TOTAL] = {
...
char *mt6779_pin_name[IPI_ID_TOTAL] = {
	"PLATFORM",
	"CPU_DVFS",
	"QOS",
	"TST1",
