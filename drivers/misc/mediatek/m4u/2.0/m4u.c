...
struct sg_table *m4u_create_sgtable(unsigned long va, unsigned int size)
{
	...
	if (va < PAGE_OFFSET) {	/* from user space */
		if (va >= VMALLOC_START && va <= VMALLOC_END) {	/* vmalloc */
			M4ULOG_MID("user space vmalloc, va = 0x%lx, nents=%d\n",
				 va, table->nents);
			for_each_sg(table->sgl, sg, table->nents, i) {
			    ...
			    struct sg_table *table)
{
	...
	if (va < PAGE_OFFSET) {	/* from user space */
		if (va >= VMALLOC_START && va <= VMALLOC_END) /* vmalloc */
			ret = __m4u_cache_sync_kernel(
			    (void *)va, size, sync_type);
		else
...
long m4u_dma_op(struct m4u_client_t *client, unsigned int port,
		unsigned long va, unsigned int size, unsigned int mva,
...
int m4u_sec_init(void)
{
	...
	M4UMSG("call %s in normal driver\n", __func__);
