		  ...
		  unsigned int *pMva)
{
	...
	if (va && ((flags & M4U_FLAGS_SG_READY) == 0)) {
		...
		if (IS_ERR_OR_NULL(sg_table)) {
			M4UMSG("%s, cannot create sg: larb=%d,module=%s,va=0x%lx,sg=0x%p,size=%d,prot=0x%x,flags=0x%x\n",
			       ...
			       m4u_get_port_name(port), va, sg_table, size,
			       prot, flags);
			ret = -EFAULT;
			goto err;
		}
	...
	*pMva = mva;
	...
	M4ULOG_MID(
		...
		m4u_get_port_name(port), va, sg_table, size, prot, flags, mva);
...
#define M4U_DRV_UUID                                                           \
	{                                                                      \
		{                                                              \
			0x90, 0x73, 0xF0, 0x3A, 0x96, 0x18, 0x38, 0x3B, 0xB1,  \
...
static int m4u_open_trustlet(uint32_t deviceId)
{
	...
	ret = TEEC_OpenSession(&m4u_tci_context, &m4u_tci_session, &destination,
...
#endif
	ret = TEEC_InvokeCommand(m4u_session, m4u_msg->cmd, &m4u_operation,
				 NULL);
...
static int __m4u_sec_init(int reinit)
{
	...
	M4UMSG("%s call m4u_exec_cmd CMD_M4UTL_INIT, nonsec_pt_pa: 0x%lx (0x%llx)\n",
	       __func__, pt_pa_nonsec, m4u_tci_msg->init_param.nonsec_pt_pa);
...
#define M4U_TL_UUID                                                            \
	{                                                                      \
		{                                                              \
			0x98, 0xfb, 0x95, 0xbc, 0xb4, 0xbf, 0x42, 0xd2, 0x64,  \
...
static int m4u_open_trustlet(uint32_t deviceId)
{
	...
	mcRet = mc_open_session(&m4u_tci_session, &m4u_tl_uuid,
				(uint8_t *)m4u_tci_msg,
...
/* Open MobiCore device */
	...
	mcRet = mc_open_session(&m4u_dci_session, &m4u_drv_uuid,
				(uint8_t *)m4u_dci_msg,
				(uint32_t)sizeof(struct m4u_msg));
	if (mcRet != MC_DRV_OK) {
		M4UMSG("tz_m4u: mc_open_session returned: %d\n", mcRet);
