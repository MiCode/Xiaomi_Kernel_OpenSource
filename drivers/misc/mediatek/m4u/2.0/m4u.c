		...
		if ((vma->vm_flags) & VM_PFNMAP) {
			...
			if (-1 == ret) {
				...
				if (vma_temp)
					...
					__func__, vma_temp->vm_start, vma_temp->vm_end,
					vma_temp->vm_flags);
			}
		}
		if (ret) {
		  ...
		  unsigned int *pMva)
{
	...
	*pMva = mva;
	...
	if (flags & M4U_FLAGS_SEQ_ACCESS)
		pMvaInfo->seq_id = m4u_insert_seq_range(port, mva,
							mva + size - 1);
		     ...
		     struct sg_table *sg_table)
{
	...
	if (port_info->flags & M4U_FLAGS_SG_READY)
		...
		port_info->va = 0;
	ret = m4u_alloc_mva(ion_m4u_client, port_info->emoduleid,
			    port_info->va,
			    sg_table,
		    ...
		    unsigned int mva)
{
	...
	M4ULOG_MID("%s: larb=%d,module=%s,mva=0x%x, size=%d\n", __func__,
...
{
	unmap_kernel_range((unsigned long)cache_map_vm_struct->addr,
			   PAGE_SIZE);
}
...
long m4u_dma_op(struct m4u_client_t *client, M4U_PORT_ID port,
		unsigned long va, unsigned int size, unsigned int mva,
...
static int m4u_open_trustlet(uint32_t deviceId)
{
	...
	ret = TEEC_OpenSession(&m4u_tci_context, &m4u_tci_session,
			       &destination,
...
static int m4u_exec_tci(struct TEEC_Session *m4u_session,
...
#endif
	ret = TEEC_InvokeCommand(m4u_session, m4u_msg->cmd,
				 &m4u_operation, NULL);
...
static int __m4u_sec_init(int reinit)
{
	...
	m4u_tci_msg->init_param.sec_pt_pa =
		0;	/* m4u_alloc_sec_pt_for_debug(); */
...
int m4u_config_port_array_tee(unsigned char
			      *port_array)	/* native */
{
	int ret;
...
static int compat_get_m4u_module_struct(COMPAT_M4U_MOUDLE_STRUCT
					__user *data32,
...
static int compat_put_m4u_module_struct(COMPAT_M4U_MOUDLE_STRUCT
					__user *data32,
					struct M4U_MOUDLE __user *data)
{
	compat_uint_t u;
...
static int compat_get_m4u_cache_struct(COMPAT_M4U_CACHE_STRUCT
				       __user *data32,
...
static int m4u_probe(struct platform_device *pdev)
{
	...
	if (pdev->id == 0) {
...
#ifdef M4U_TEE_SERVICE_ENABLE
		{
			...
			mva = m4u_do_mva_alloc(0, M4U_NONSEC_MVA_START - 0x100000,
					       pMvaInfo);
