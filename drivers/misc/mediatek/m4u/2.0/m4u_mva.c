...
void m4u_mvaGraph_dump(void)
{
	...
	M4ULOG_HIGH(
		"[M4U_2.0] mva allocation info dump:====================>\n");
	...
	M4ULOG_HIGH("\n");
	M4ULOG_HIGH(
		"[M4U_2.0] mva alloc summary: (unit: blocks)========================>\n");
	M4ULOG_HIGH("free: %d , alloc: %d, total: %d\n",
		nr_free, nr_alloc, nr_free + nr_alloc);
	M4ULOG_HIGH(
		"[M4U_2.0] free region fragments in 2^x blocks unit:===============\n");
	M4ULOG_HIGH(
...
unsigned int m4u_do_mva_alloc(unsigned long va, unsigned int size, void *priv)
{
	...
	if (s > MVA_MAX_BLOCK_NR) {
		spin_unlock_irqrestore(&gMvaGraph_lock, irq_flags);
		M4UMSG("%s: err no available MVA for %d blocks!\n",
		  __func__, nr);
#ifdef M4U_PROFILE
							...
							void *priv)
{
	...
	M4ULOG_MID("%s: mva:0x%x, startIdx=%d, size = %d, nr= %d\n",
	...
	M4ULOG_MID(
		"region_start: %d, region_end= %d, region= %d, next_region_start= %d, search start: %d\n",
		region_start, region_end,
		MVA_GET_NR(region_start), next_region_start, s);
	...
	if (s == 0) {
		...
		region_end = region_start + MVA_GET_NR(region_start) - 1;
		M4UMSG("start:%d, end:%d, startIdx:%d, endIdx:%d\n",
