...
int m4u_invalid_prog_dist_by_id(int port)
{
	...
	/*
	 * m4u_err("%s cannot found proj dist set for port %d.\n",
	 *	__func__, port);
	 */
...
irqreturn_t MTK_M4U_isr(int irq, void *dev_id)
{
...

	{
		...
		if (IntrSrc & F_INT_TRANSLATION_FAULT(m4u_slave_id)) {
			...
			m4u_err("fault: port=%s, mva=0x%x, pa=0x%x, layer=%d, wr=%d, 0x%x\n",
				m4u_get_port_name(m4u_port), fault_mva,
				fault_pa, layer, write, regval);
