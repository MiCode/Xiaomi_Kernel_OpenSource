...
#define M4U_DRV_UUID {{0x90, 0x73, 0xF0,\
		0x3A, 0x96, 0x18, 0x38, 0x3B,\
		0xB1, 0x85, 0x6E, 0xB3, 0xF9,\
		0x90, 0xBA, 0xBD} }
...
#if !defined(CONFIG_MTK_TEE_GP_SUPPORT)
#define M4U_TL_UUID {{0x98, 0xfb, 0x95,\
		0xbc, 0xb4, 0xbf, 0x42, 0xd2,\
...
static void m4u_teei_dci_init(void)
{
	 ...
	 * Therefore, we need to check with m4u driver owner to see
	 * if we need to move
	 * these code to other place. Otherwise, it will block
	 * m4u initialization
	 */
	while (!is_teei_ready())
		msleep(1000);
	...
	if (ret != TEEC_SUCCESS) {
		M4UMSG(
			"tz_load_drv failed, ready?(%d), ret%x\n",
				is_teei_ready(), ret);
		return;
	}

	while ((ret = tz_create_share_buffer(DRM_M4U_DRV_DRIVER_ID,
				SZ_4K)) != 0)
		msleep(1000);

	/* The share buffer is created in teei tz driver,
	 *we only need to get it here
	 */
	m4u_dci_msg =
		(struct m4u_msg *) tz_get_share_buffer(DRM_M4U_DRV_DRIVER_ID);
...
static int m4u_open_trustlet(uint32_t deviceId)
{
	...
	mcRet = mc_malloc_wsm(deviceId, 0,
		sizeof(struct m4u_msg), (uint8_t **) &m4u_tci_msg, 0);
	if (mcRet != MC_DRV_OK) {
...
static int m4u_exec_cmd(
	struct mc_session_handle *m4u_session, struct m4u_msg *m4u_msg)
{
	enum mc_result ret;
...
int m4u_sec_init(void)
{
	...
	if (mcRet != MC_DRV_OK) {
		M4UMSG(
			"tz_m4u: error mc_open_device returned: %d,deviceid=%d\n",
			mcRet, deviceId);
	...
	if (mcRet != MC_DRV_OK) {
		M4UMSG(
			"tz_m4u: mc_open_session returned: %d,uuid(%d)\n",
