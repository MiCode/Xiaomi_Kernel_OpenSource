	...
	for (way = 0; way < way_nr; way++) {
		for (set = 0; set < set_nr; set++) {
			...
			M4ULOG_HIGH(
				...
				!!(tlb.tag & F_PFH_TAG_AUTO_PFH), valid, set,
				way, tlb.desc);
	...
	for (way = 0; way < way_nr; way++) {
		for (set = 0; set < set_nr; set++) {
			...
			pfh_buf[pfh_id].page_size =
				pfh_buf[pfh_id].layer ? MMU_SMALL_PAGE_SIZE
						      : MMU_SECTION_SIZE;
	...
	for (i = 0; i < gM4UTagCount[m4u_index]; i++) {
		...
		if (regval & (F_MAIN_TLB_VALID_BIT)) {
			...
			if (layer) { /* pte */
				...
				if (large)
					tag_e = tag_s + MMU_SUPERSECTION_SIZE -
						1;
				else
				  ...
				  unsigned int MVAEnd)
{
	...
	/* /> check Main TLB part */
	result = m4u_confirm_main_range_invalidated(m4u_index, 0, MVAStart,
						    MVAEnd);
	if (result < 0)
		return -1;
...
static int _m4u_config_port(int port, int virt, int sec, int dis, int dir)
{
	...
	if (m4u_index == 0) {
		...
		if (!!(mmu_en) != virt) {
			M4ULOG_HIGH(
				"%s error, port=%s, Virtuality=%d, mmu_en=%x (%x, %x)\n",
				__func__, m4u_get_port_name(port), virt, mmu_en,
