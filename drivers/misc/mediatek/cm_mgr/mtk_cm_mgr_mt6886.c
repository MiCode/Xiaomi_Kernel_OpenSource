...
/* #include "mtk_cm_mgr_events_mt6886.h" */
...
int cm_mgr_get_latency_awareness_model_indexes_mt6886(unsigned int *buf)
{
	int i = 0;
...
u32 cm_mgr_get_perfs_mt6886(int num)
{
	if (num < 0 || num >= cm_mgr_get_num_perf())
		return 0;
	return cm_mgr_perfs[num];
}
EXPORT_SYMBOL_GPL(cm_mgr_get_perfs_mt6886);
...
static int cm_mgr_check_dram_type(void)
{
	...
	if (ddr_type == 0)
		cm_mgr_idx = CM_MGR_LP5; /* default LP5 if type unknown */
...
void cm_mgr_perf_platform_set_status_mt6886(int enable)
{
	...
	if (enable) {
		...
		if (!cm_thresh)
			icc_set_bw(cm_mgr_get_bw_path(), 0,
				cm_mgr_get_perfs_mt6886(cm_mgr_dram_opp));
...
EXPORT_SYMBOL_GPL(cm_mgr_perf_platform_set_status_mt6886);
...
void cm_mgr_perf_set_status_mt6886(int enable)
{
	...
	cm_mgr_perf_platform_set_status_mt6886(enable);
}
EXPORT_SYMBOL_GPL(cm_mgr_perf_set_status_mt6886);

void cm_mgr_perf_set_force_status_mt6886(int enable)
{
...
EXPORT_SYMBOL_GPL(cm_mgr_perf_set_force_status_mt6886);

void check_cm_mgr_status_mt6886(unsigned int cluster, unsigned int freq,
		unsigned int idx)
{
...
EXPORT_SYMBOL_GPL(check_cm_mgr_status_mt6886);
...
static int platform_cm_mgr_probe(struct platform_device *pdev)
{
	...
	local_hk.cm_mgr_get_perfs =
		cm_mgr_get_perfs_mt6886;
	local_hk.cm_mgr_perf_set_force_status =
		cm_mgr_perf_set_force_status_mt6886;
	local_hk.check_cm_mgr_status =
		check_cm_mgr_status_mt6886;
	local_hk.cm_mgr_perf_platform_set_status =
		cm_mgr_perf_platform_set_status_mt6886;
	local_hk.cm_mgr_perf_set_status =
		cm_mgr_perf_set_status_mt6886;
	local_hk.cm_mgr_get_latency_awareness_model_indexes =
		cm_mgr_get_latency_awareness_model_indexes_mt6886;
...
static const struct of_device_id platform_cm_mgr_of_match[] = {
	{ .compatible = "mediatek,mt6886-cm_mgr", },
	{},
};
...
MODULE_AUTHOR("Alex-KC Yuan<alex-kc.yuan@mediatek.com>");
MODULE_LICENSE("GPL");
