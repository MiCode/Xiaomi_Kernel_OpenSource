// SPDX-License-Identifier: GPL-2.0
/*
 * Copyright (c) 2020 MediaTek Inc.
 */

#include <linux/arm-smccc.h>
#include <linux/clk.h>
#include <linux/clk-provider.h>
#include <linux/delay.h>
#include <linux/fs.h>
#include <linux/init.h>
#include <linux/input.h>
#include <linux/io.h>
#include <linux/jiffies.h>
#include <linux/kernel.h>
#include <linux/ktime.h>
#include <linux/mfd/syscon.h>
#include <linux/miscdevice.h>
#include <linux/module.h>
#include <linux/platform_device.h>
#include <linux/pm_wakeup.h>
#include <linux/printk.h>
#include <linux/proc_fs.h>
#include <linux/regmap.h>
#include <linux/regulator/consumer.h>
#include <linux/sched.h>
#include <linux/seq_file.h>
#include <linux/slab.h>
#include <linux/soc/mediatek/mtk_sip_svc.h> /* for SMC ID table */
#include <linux/spinlock.h>
#include <linux/string.h>
#include <linux/suspend.h>
#include <linux/uaccess.h>

#if IS_ENABLED(CONFIG_MFD_MT6397)
#include <linux/mfd/mt6397/core.h>
#endif /* IS_ENABLED(CONFIG_MFD_MT6397) */

#if IS_ENABLED(CONFIG_OF)
#include <linux/of.h>
#include <linux/of_address.h>
#include <linux/of_platform.h>
#endif /* IS_ENABLED(CONFIG_OF) */

#include "vcp_ipi.h"
#include "vcp_helper.h"
#include "vcp_excep.h"
#include "vcp_dvfs.h"
#include "vcp.h"

#ifdef pr_fmt
#undef pr_fmt
#endif
#define pr_fmt(fmt)			"[vcp_dvfs]: " fmt

#define DRV_Reg32(addr)			readl(addr)
#define DRV_WriteReg32(addr, val)	writel(val, addr)
#define DRV_SetReg32(addr, val) DRV_WriteReg32(addr, DRV_Reg32(addr) | (val))
#define DRV_ClrReg32(addr, val) DRV_WriteReg32(addr, DRV_Reg32(addr) & ~(val))

#if IS_ENABLED(CONFIG_MFD_MT6397)
#define pmic_main_chip			mt6397_chip
#endif

#define PMIC_PHANDLE_NAME		"pmic"
#define GPIO_PHANDLE_NAME		"gpio-base"
#define VCP_CLK_CTRL_PHANDLE_NAME	"vcp_clk_ctrl"
#define TOPCK_PHANDLE_NAME		"topckgen"
#define APMIX_PHANDLE_NAME		"apmixedsys"

unsigned int vcp_ipi_ackdata0, vcp_ipi_ackdata1;
struct ipi_tx_data_t {
	unsigned int arg1;
	unsigned int arg2;
};

/* -1:VCP DVFS OFF, 1:VCP DVFS ON */
static int vcp_dvfs_flag = 1;

/*
 * 0: VCP Sleep: OFF,
 * 1: VCP Sleep: ON,
 * 2: VCP Sleep: sleep without wakeup,
 * 3: VCP Sleep: force to sleep
 */
static int vcp_resrc_current_req = -1;

static struct mt_vcp_pll_t mt_vcp_pll;
static struct wakeup_source *vcp_suspend_lock;
static int g_vcp_dvfs_init_flag = -1;

static struct vcp_dvfs_hw dvfs;

static struct regulator *dvfsrc_vvcp_power;
static struct regulator *reg_vcore;
static struct regulator *reg_vsram;

const char *ulposc_ver[MAX_ULPOSC_VERSION] __initconst = {
	[ULPOSC_VER_1] = "v1",
};

const char *clk_dbg_ver[MAX_CLK_DBG_VERSION] __initconst = {
	[CLK_DBG_VER_1] = "v1",
};

const char *vcp_clk_ver[MAX_VCP_CLK_VERSION] __initconst = {
	[VCP_CLK_VER_1] = "v1",
};

const char *vcp_dvfs_hw_chip_ver[MAX_VCP_DVFS_CHIP_HW] __initconst = {
	[MT6853] = "mediatek,mt6853",
	[MT6873] = "mediatek,mt6873",
	[MT6893] = "mediatek,mt6893",
};

struct ulposc_cali_regs cali_regs[MAX_ULPOSC_VERSION] __initdata = {
	[ULPOSC_VER_1] = {
		REG_DEFINE(con0, 0x2C0, REG_MAX_MASK, 0)
		REG_DEFINE(cali, 0x2C0, 0x7F, 0)
		REG_DEFINE(con1, 0x2C4, REG_MAX_MASK, 0)
		REG_DEFINE(con2, 0x2C8, REG_MAX_MASK, 0)
	},
};

struct clk_cali_regs clk_dbg_reg[MAX_CLK_DBG_VERSION] __initdata = {
	[CLK_DBG_VER_1] = {
		REG_DEFINE(clk_misc_cfg0, 0x140, REG_MAX_MASK, 0)
		REG_DEFINE_WITH_INIT(meter_div, 0x140, 0xFF, 24, 0, 0)

		REG_DEFINE(clk_dbg_cfg, 0x17C, 0x3, 0)
		REG_DEFINE_WITH_INIT(fmeter_ck_sel, 0x17C, 0x3, 0, 0, 0)
		REG_DEFINE_WITH_INIT(abist_clk, 0x17C, 0x3F, 16, 36, 0)

		REG_DEFINE(clk26cali_0, 0x220, REG_MAX_MASK, 0)
		REG_DEFINE_WITH_INIT(fmeter_en, 0x220, 0x1, 12, 1, 0)
		REG_DEFINE_WITH_INIT(trigger_cal, 0x220, 0x1, 4, 1, 0)

		REG_DEFINE(clk26cali_1, 0x224, REG_MAX_MASK, 0)
		REG_DEFINE(cal_cnt, 0x224, 0xFFFF, 0)
		REG_DEFINE_WITH_INIT(load_cnt, 0x224, 0x3FF, 16, 0x1FF, 0)
	},
};

struct vcp_clk_hw vcp_clk_hw_regs[MAX_VCP_CLK_VERSION] = {
	[VCP_CLK_VER_1] = {
		REG_DEFINE(clk_high_en, 0x4, 0x1, 1)
		REG_DEFINE(ulposc2_en, 0x6C, 0x1, 5)
		REG_DEFINE(ulposc2_cg, 0x5C, 0x1, 1)
		REG_DEFINE(sel_clk, 0x0, 0xF, 8)
	}
};

struct vcp_pmic_regs vcp_pmic_hw_regs[MAX_VCP_DVFS_CHIP_HW] = {
	[MT6853] = {
		REG_DEFINE_WITH_INIT(sshub_op_mode, 0x1520, 0x1, 11, 0, 1)
		REG_DEFINE_WITH_INIT(sshub_op_en, 0x1514, 0x1, 11, 1, 1)
		REG_DEFINE_WITH_INIT(sshub_op_cfg, 0x151a, 0x1, 11, 1, 1)
	},
	[MT6873] = {
		REG_DEFINE_WITH_INIT(sshub_op_mode, 0x15a0, 0x1, 11, 0, 1)
		REG_DEFINE_WITH_INIT(sshub_op_en, 0x1594, 0x1, 11, 1, 1)
		REG_DEFINE_WITH_INIT(sshub_op_cfg, 0x159a, 0x1, 11, 1, 1)
		REG_DEFINE_WITH_INIT(sshub_buck_en, 0x15aa, 0x1, 0, 1, 0)
		REG_DEFINE_WITH_INIT(sshub_ldo_en, 0x1f28, 0x1, 0, 0, 0)
		REG_DEFINE_WITH_INIT(pmrc_en, 0x1ac, 0x1, 2, 0, 1)
	},
	[MT6893] = {
		REG_DEFINE_WITH_INIT(sshub_op_mode, 0x15a0, 0x1, 11, 0, 1)
		REG_DEFINE_WITH_INIT(sshub_op_en, 0x1594, 0x1, 11, 1, 1)
		REG_DEFINE_WITH_INIT(sshub_op_cfg, 0x159a, 0x1, 11, 1, 1)
		REG_DEFINE_WITH_INIT(sshub_buck_en, 0x15aa, 0x1, 0, 1, 0)
		REG_DEFINE_WITH_INIT(sshub_ldo_en, 0x1f28, 0x1, 0, 0, 0)
		REG_DEFINE_WITH_INIT(pmrc_en, 0x1ac, 0x1, 2, 0, 1)
	},
};

static void slp_ipi_init(void)
{
	int ret;

	ret = mtk_ipi_register(&vcp_ipidev, IPI_OUT_C_SLEEP_0,
		NULL, NULL, &vcp_ipi_ackdata0);
	if (ret) {
		pr_notice("vcp0 slp ipi register failed\n");
		WARN_ON(1);
	}

	if (dvfs.core_nums == 2) {
		ret = mtk_ipi_register(&vcp_ipidev, IPI_OUT_C_SLEEP_1,
			NULL, NULL, &vcp_ipi_ackdata1);
		if (ret)
			pr_notice("vcp1 slp ipi register failed\n");
	}
	if (!ret)
		dvfs.sleep_init_done = true;
}

static int vcp_get_vcore_table(unsigned int gear)
{
	struct arm_smccc_res res;
/*
	arm_smccc_smc(MTK_SIP_VCP_DVFS_CONTROL, VCORE_ACQUIRE,
		gear, 0, 0, 0, 0, 0, &res);
*/
	if (!(res.a0))
		return res.a1;

	pr_notice("[%s]: should not end here\n", __func__);
	return -1;
}

int vcp_resource_req(unsigned int req_type)
{
	struct arm_smccc_res res;

	if (req_type < 0 || req_type >= VCP_REQ_MAX)
		return 0;
/*
	arm_smccc_smc(MTK_SIP_VCP_DVFS_CONTROL, RESOURCE_REQ,
		req_type, 0, 0, 0, 0, 0, &res);
*/
	if (!res.a0)
		vcp_resrc_current_req = req_type;
	else
		pr_notice("[%s]: resource request failed, req: %u\n",
			__func__, req_type);

	return res.a0;
}

static int vcp_reg_update(struct regmap *regmap, struct reg_info *reg, u32 val)
{
	u32 mask;
	int ret = 0;

	if (!reg->msk) {
		pr_notice("[%s]: reg not support\n", __func__);
		return -EVCP_REG_NOT_SUPPORTED;
	}
	mask = reg->msk << reg->bit;
	val = (val & reg->msk) << reg->bit;

	if (reg->setclr) {
		ret = regmap_write(regmap, reg->ofs + 4, mask);
		ret = regmap_write(regmap, reg->ofs + 2, val);
	} else {
		ret = regmap_update_bits(regmap, reg->ofs, mask, val);
	}

	return ret;
}

static int vcp_reg_read(struct regmap *regmap, struct reg_info *reg, u32 *val)
{
	int ret = 0;

	if (!reg->msk) {
		pr_notice("[%s]: reg not support\n", __func__);
		return -EVCP_REG_NOT_SUPPORTED;
	}

	ret = regmap_read(regmap, reg->ofs, val);
	if (!ret)
		*val = (*val >> reg->bit) & reg->msk;
	return ret;
}

static inline int vcp_reg_init(struct regmap *regmap, struct reg_info *reg)
{
	return vcp_reg_update(regmap, reg, reg->init_config);
}

static int vcp_get_freq_idx(unsigned int clk_opp)
{
	int i;

	for (i = 0; i < dvfs.vcp_opp_nums; i++)
		if (clk_opp == dvfs.opp[i].freq)
			break;

	if (i == dvfs.vcp_opp_nums) {
		pr_notice("no available opp, freq: %u\n", clk_opp);
		return -EINVAL;
	}

	return i;
}

static int vcp_update_pmic_vow_lp_mode(bool on)
{
	int ret = 0;

	if (on)
		ret = vcp_reg_update(dvfs.pmic_regmap,
			&dvfs.pmic_regs->_sshub_op_cfg, 1);
	else
		ret = vcp_reg_update(dvfs.pmic_regmap,
			&dvfs.pmic_regs->_sshub_op_cfg, 0);

	return ret;
}

static int vcp_set_pmic_vcore(unsigned int cur_freq)
{
	int ret = 0;
#if !IS_ENABLED(CONFIG_FPGA_EARLY_PORTING)
	int idx = 0;
	unsigned int ret_vc = 0, ret_vs = 0;

	idx = vcp_get_freq_idx(cur_freq);
	if (idx >= 0 && idx < dvfs.vcp_opp_nums) {
		ret_vc = regulator_set_voltage(reg_vcore,
				dvfs.opp[idx].tuned_vcore,
				dvfs.opp[dvfs.vcp_opp_nums - 1].vcore + 100000);

		ret_vs = regulator_set_voltage(reg_vsram,
				dvfs.opp[idx].vsram,
				dvfs.opp[dvfs.vcp_opp_nums - 1].vsram + 100000);
	} else {
		ret = -EVCP_DVFS_OPP_OUT_OF_BOUND;
		pr_notice("[%s]: cur_freq=%d is not supported\n",
			__func__, cur_freq);
		WARN_ON(1);
	}

	if (ret_vc != 0 || ret_vs != 0) {
		ret = -EVCP_DVFS_PMIC_REGULATOR_FAILED;
		pr_notice("[%s]: ERROR: vcp vcore/vsram setting error, (%d, %d)\n",
				__func__, ret_vc, ret_vs);
		WARN_ON(1);
	}

	/* vcore > 0.6v cannot hold pmic/vcore in lp mode */
	if (idx < dvfs.vow_lp_en_gear)
		/* enable VOW low power mode */
		vcp_update_pmic_vow_lp_mode(true);
	else
		/* disable VOW low power mode */
		vcp_update_pmic_vow_lp_mode(false);
#endif /* CONFIG_FPGA_EARLY_PORTING */

	return ret;
}

uint32_t vcp_get_freq(void)
{
	uint32_t i;
	uint32_t sum = 0;
	uint32_t return_freq = 0;

	/*
	 * calculate vcp frequence
	 */
	for (i = 0; i < NUM_FEATURE_ID; i++) {
		if (feature_table[i].enable == 1)
			sum += feature_table[i].freq;
	}
	/*
	 * calculate vcp sensor frequence
	 */
	for (i = 0; i < NUM_SENSOR_TYPE; i++) {
		if (sensor_type_table[i].enable == 1)
			sum += sensor_type_table[i].freq;
	}

	for (i = 0; i < dvfs.vcp_opp_nums; i++) {
		if (sum <= dvfs.opp[i].freq) {
			return_freq = dvfs.opp[i].freq;
			break;
		}
	}

	if (i == dvfs.vcp_opp_nums) {
		return_freq = dvfs.opp[dvfs.vcp_opp_nums - 1].freq;
		pr_notice("warning: request freq %d > max opp %d\n",
				sum, return_freq);
	}

	return return_freq;
}

static void vcp_vcore_request(unsigned int clk_opp)
{
	int idx;
	int ret = 0;

	pr_debug("[%s]: opp(%d)\n", __func__, clk_opp);

	/* VCP vcore request to PMIC */
	if (dvfs.pmic_sshub_en)
		ret = vcp_set_pmic_vcore(clk_opp);

	idx = vcp_get_freq_idx(clk_opp);
	if (idx < 0) {
		pr_notice("[%s]: invalid clk_opp %d\n", __func__, clk_opp);
		WARN_ON(1);
		return;
	}

	/* VCP vcore request to DVFSRC
	 * min & max set to requested Vcore value
	 * DVFSRC SW will find corresponding idx to process
	 * if opp[idx].dvfsrc_opp == 0xff, means that
	 * opp[idx] is not supported by DVFSRC
	 */
	if (dvfs.opp[idx].dvfsrc_opp != 0xff)
		ret = regulator_set_voltage(dvfsrc_vvcp_power,
			dvfs.opp[idx].vcore,
			dvfs.opp[idx].vcore);
	if (ret) {
		pr_notice("[%s]: dvfsrc vcore update error, opp: %d\n",
			__func__, idx);
		WARN_ON(1);
	}

	/* VCP vcore request to SPM */
	DRV_WriteReg32(VCP_VCP2SPM_VOL_LV, dvfs.opp[idx].spm_opp);
}

void vcp_init_vcore_request(void)
{
	if (vcp_dvfs_flag != 1)
		vcp_vcore_request(dvfs.opp[0].freq);
}

/* vcp_request_freq
 * return :-1 means the vcp request freq. error
 * return :0  means the request freq. finished
 */
int vcp_request_freq(void)
{
	int value = 0;
	int timeout = 250;
	int ret = 0;
	unsigned long spin_flags;
	int is_increasing_freq = 0;
	int opp_idx;

	if (vcp_dvfs_flag != 1) {
		pr_debug("[%s]: warning: VCP DVFS is OFF\n", __func__);
		return 0;
	}

	/* because we are waiting for vcp to update register:vcp_current_freq
	 * use wake lock to prevent AP from entering suspend state
	 */
	__pm_stay_awake(vcp_suspend_lock);

	if (vcp_current_freq != vcp_expected_freq) {

		vcp_awake_lock((void *)VCP_A_ID);

		/* do DVS before DFS if increasing frequency */
		if (vcp_current_freq < vcp_expected_freq) {
			vcp_vcore_request(vcp_expected_freq);
			is_increasing_freq = 1;
		}

		/* Request SPM not to turn off mainpll/26M/infra */
		/* because VCP may park in it during DFS process */
		vcp_resource_req(VCP_REQ_26M |
				VCP_REQ_INFRA |
				VCP_REQ_SYSPLL);

		/*  turn on PLL if necessary */
		vcp_pll_ctrl_set(PLL_ENABLE, vcp_expected_freq);

		do {
			ret = mtk_ipi_send(&vcp_ipidev,
				IPI_OUT_DVFS_SET_FREQ_0,
				IPI_SEND_WAIT, &value,
				PIN_OUT_SIZE_DVFS_SET_FREQ_0, 0);
			if (ret != IPI_ACTION_DONE)
				pr_notice("VCP send IPI fail - %d\n", ret);

			mdelay(2);
			timeout -= 1; /*try 50 times, total about 100ms*/
			if (timeout <= 0) {
				pr_notice("set freq fail, current(%d) != expect(%d)\n",
					vcp_current_freq, vcp_expected_freq);
				__pm_relax(vcp_suspend_lock);
				WARN_ON(1);
				return -EVCP_DVFS_IPI_FAILED;
			}

			/* read vcp_current_freq again */
			spin_lock_irqsave(&vcp_awake_spinlock, spin_flags);
			vcp_current_freq = readl(CURRENT_FREQ_REG);
			spin_unlock_irqrestore(&vcp_awake_spinlock, spin_flags);

		} while (vcp_current_freq != vcp_expected_freq);

		/* turn off PLL if necessary */
		vcp_pll_ctrl_set(PLL_DISABLE, vcp_expected_freq);

		/* do DVS after DFS if decreasing frequency */
		if (is_increasing_freq == 0)
			vcp_vcore_request(vcp_expected_freq);

		vcp_awake_unlock((void *)VCP_A_ID);

		opp_idx = vcp_get_freq_idx(vcp_current_freq);
		if (dvfs.opp[opp_idx].resource_req)
			vcp_resource_req(dvfs.opp[opp_idx].resource_req);
		else
			vcp_resource_req(VCP_REQ_RELEASE);
	}

	__pm_relax(vcp_suspend_lock);
	pr_debug("[VCP] succeed to set freq, expect=%d, cur=%d\n",
			vcp_expected_freq, vcp_current_freq);
	return 0;
}

void wait_vcp_dvfs_init_done(void)
{
	int count = 0;

	while (g_vcp_dvfs_init_flag != 1) {
		mdelay(1);
		count++;
		if (count > 3000) {
			pr_notice("VCP dvfs driver init fail\n");
			WARN_ON(1);
		}
	}
}

static int set_vcp_clk_mux(unsigned int  pll_ctrl_flag)
{
	int ret = 0;

	if (pll_ctrl_flag == PLL_ENABLE) {
		if (!dvfs.pre_mux_en) {
			ret = clk_prepare_enable(mt_vcp_pll.clk_mux);
			if (ret) {
				pr_notice("[%s]: clk_prepare_enable failed\n",
					__func__);
				WARN_ON(1);
				return -1;
			}
			dvfs.pre_mux_en = true;
		}
	} else if (pll_ctrl_flag == PLL_DISABLE) {
		clk_disable_unprepare(mt_vcp_pll.clk_mux);
		dvfs.pre_mux_en = false;
	}

	return 0;
}

static int __vcp_pll_sel_26M(unsigned int pll_ctrl_flag, unsigned int pll_sel)
{
	int ret = 0;

	if (pll_sel != CLK_26M)
		return -EINVAL;

	ret = set_vcp_clk_mux(pll_ctrl_flag);
	if (ret)
		return ret;

	if (pll_ctrl_flag == PLL_ENABLE) {
		ret = clk_set_parent(mt_vcp_pll.clk_mux, mt_vcp_pll.clk_pll[0]);
		if (ret) {
			pr_notice("[%s]: clk_set_parent() failed for 26M\n", __func__);
			WARN_ON(1);
		}
	}

	return ret;
}

int vcp_pll_ctrl_set(unsigned int pll_ctrl_flag, unsigned int pll_sel)
{
	int idx;
	int mux_idx = 0;
	int ret = 0;

	pr_debug("[%s]: (%d, %d)\n", __func__, pll_ctrl_flag, pll_sel);

	if (pll_sel == CLK_26M)
		return __vcp_pll_sel_26M(pll_ctrl_flag, pll_sel);

	idx = vcp_get_freq_idx(pll_sel);
	if (idx < 0) {
		pr_notice("invalid idx %d\n", idx);
		WARN_ON(1);
		return -EINVAL;
	}

	mux_idx = dvfs.opp[idx].clk_mux;

	if (mux_idx < 0) {
		pr_notice("invalid mux_idx %d\n", mux_idx);
		WARN_ON(1);
		return -EINVAL;
	}

	if (pll_ctrl_flag == PLL_ENABLE) {
		ret = set_vcp_clk_mux(pll_ctrl_flag);
		if (ret)
			return ret;
		if (idx >= 0 && idx < dvfs.vcp_opp_nums
				&& idx < mt_vcp_pll.pll_num)
			ret = clk_set_parent(mt_vcp_pll.clk_mux,
					mt_vcp_pll.clk_pll[mux_idx]);
		else {
			pr_notice("[%s]: not support opp freq %d\n",
				__func__, pll_sel);
			WARN_ON(1);
		}

		if (ret) {
			pr_notice("[%s]: clk_set_parent() failed, opp=%d\n",
				__func__, pll_sel);
			WARN_ON(1);
		}
	} else if ((pll_ctrl_flag == PLL_DISABLE) &&
			(dvfs.opp[idx].resource_req == 0)) {
		set_vcp_clk_mux(pll_ctrl_flag);
	}
	return ret;
}

void mt_vcp_dvfs_state_dump(void)
{
	unsigned int vcp_state;

	vcp_state = readl(VCP_A_SLEEP_DEBUG_REG);
	pr_info("vcp status: %s\n",
		((vcp_state & IN_DEBUG_IDLE) == IN_DEBUG_IDLE) ? "idle mode"
		: ((vcp_state & ENTERING_SLEEP) == ENTERING_SLEEP) ?
			"enter sleep"
		: ((vcp_state & IN_SLEEP) == IN_SLEEP) ?
			"sleep mode"
		: ((vcp_state & ENTERING_ACTIVE) == ENTERING_ACTIVE) ?
			"enter active"
		: ((vcp_state & IN_ACTIVE) == IN_ACTIVE) ?
			"active mode" : "none of state");
}

#if IS_ENABLED(CONFIG_PROC_FS)
/*
 * PROC
 */

/****************************
 * show VCP state
 *****************************/
static int mt_vcp_dvfs_state_proc_show(struct seq_file *m, void *v)
{
	unsigned int vcp_state;

	vcp_state = readl(VCP_A_SLEEP_DEBUG_REG);
	seq_printf(m, "vcp status: %s\n",
		((vcp_state & IN_DEBUG_IDLE) == IN_DEBUG_IDLE) ? "idle mode"
		: ((vcp_state & ENTERING_SLEEP) == ENTERING_SLEEP) ?
			"enter sleep"
		: ((vcp_state & IN_SLEEP) == IN_SLEEP) ?
			"sleep mode"
		: ((vcp_state & ENTERING_ACTIVE) == ENTERING_ACTIVE) ?
			"enter active"
		: ((vcp_state & IN_ACTIVE) == IN_ACTIVE) ?
			"active mode" : "none of state");
	seq_printf(m, "current debug vcp core: %d\n",
		dvfs.cur_dbg_core);
	return 0;
}

/****************************
 * show VCP count
 *****************************/
static int mt_vcp_dvfs_sleep_cnt_proc_show(struct seq_file *m, void *v)
{
	struct ipi_tx_data_t ipi_data;
	unsigned int *vcp_ack_data = NULL;
	int ret;

	if (!dvfs.sleep_init_done)
		slp_ipi_init();

	ipi_data.arg1 = VCP_SLEEP_GET_COUNT;
	if (dvfs.cur_dbg_core == VCP_CORE_0) {
		ret = mtk_ipi_send_compl(&vcp_ipidev, IPI_OUT_C_SLEEP_0,
			IPI_SEND_WAIT, &ipi_data, PIN_OUT_C_SIZE_SLEEP_0, 500);
		vcp_ack_data = &vcp_ipi_ackdata0;
	} else if (dvfs.cur_dbg_core == VCP_CORE_1) {
		ret = mtk_ipi_send_compl(&vcp_ipidev, IPI_OUT_C_SLEEP_1,
			IPI_SEND_WAIT, &ipi_data, PIN_OUT_C_SIZE_SLEEP_1, 500);
		vcp_ack_data = &vcp_ipi_ackdata1;
	} else {
		pr_notice("[%s]: invalid vcp core num: %d\n",
			__func__, dvfs.cur_dbg_core);
		return -EVCP_DVFS_DBG_INVALID_CMD;
	}
	if (ret != IPI_ACTION_DONE) {
		pr_notice("[%s] ipi send failed with error: %d\n",
			__func__, ret);
		return -EVCP_DVFS_IPI_FAILED;
	}
	seq_printf(m, "vcp core%d sleep count: %d\n",
		dvfs.cur_dbg_core, *vcp_ack_data);
	pr_notice("[%s]: vcp core%d sleep count :%d\n",
		__func__, dvfs.cur_dbg_core, *vcp_ack_data);


	return 0;
}

/**********************************
 * write vcp dvfs sleep
 ***********************************/
static ssize_t mt_vcp_dvfs_sleep_cnt_proc_write(
					struct file *file,
					const char __user *buffer,
					size_t count,
					loff_t *data)
{
	char desc[64], cmd[32];
	unsigned int len = 0;
	unsigned int ipi_cmd = -1;
	unsigned int ipi_cmd_size = -1;
	int ret = 0;
	int n = 0;
	struct ipi_tx_data_t ipi_data;

	if (count <= 0)
		return 0;

	len = (count < (sizeof(desc) - 1)) ? count : (sizeof(desc) - 1);
	if (copy_from_user(desc, buffer, len))
		return 0;

	desc[len] = '\0';

	if (!dvfs.sleep_init_done)
		slp_ipi_init();

	n = sscanf(desc, "%31s", cmd);
	if (n != 1) {
		pr_notice("[%s]: invalid command", __func__);
		return -EVCP_DVFS_DBG_INVALID_CMD;
	}

	if (!strcmp(cmd, "reset")) {
		ipi_data.arg1 = VCP_SLEEP_RESET;
		if (dvfs.cur_dbg_core == VCP_CORE_0) {
			ipi_cmd = IPI_OUT_C_SLEEP_0;
			ipi_cmd_size = PIN_OUT_C_SIZE_SLEEP_0;
		} else if (dvfs.cur_dbg_core == VCP_CORE_1) {
			ipi_cmd = IPI_OUT_C_SLEEP_1;
			ipi_cmd_size = PIN_OUT_C_SIZE_SLEEP_1;
		} else {
			pr_notice("[%s]: invalid core index: %d\n",
				__func__, dvfs.cur_dbg_core);
			return -EVCP_DVFS_DBG_INVALID_CMD;
		}
		ret = mtk_ipi_send(&vcp_ipidev, ipi_cmd, IPI_SEND_WAIT,
			&ipi_data, ipi_cmd_size, 500);
		if (ret != VCP_IPI_DONE) {
			pr_info("[%s]: VCP send IPI failed - %d\n",
				__func__, ret);
			return -EVCP_DVFS_IPI_FAILED;
		}
	} else {
		pr_notice("[%s]: invalid command: %s\n", cmd);
		return -EVCP_DVFS_DBG_INVALID_CMD;
	}

	return count;
}

/****************************
 * show vcp dvfs sleep
 *****************************/
static int mt_vcp_dvfs_sleep_proc_show(struct seq_file *m, void *v)
{
	struct ipi_tx_data_t ipi_data;
	unsigned int *vcp_ack_data = NULL;
	int ret = 0;

	if (!dvfs.sleep_init_done)
		slp_ipi_init();

	ipi_data.arg1 = VCP_SLEEP_GET_DBG_FLAG;

	if (dvfs.cur_dbg_core == VCP_CORE_0) {
		ret = mtk_ipi_send_compl(&vcp_ipidev, IPI_OUT_C_SLEEP_0,
			IPI_SEND_WAIT, &ipi_data, PIN_OUT_C_SIZE_SLEEP_0, 500);
		vcp_ack_data = &vcp_ipi_ackdata0;
	} else if (dvfs.cur_dbg_core == VCP_CORE_1) {
		ret = mtk_ipi_send_compl(&vcp_ipidev, IPI_OUT_C_SLEEP_1,
			IPI_SEND_WAIT, &ipi_data, PIN_OUT_C_SIZE_SLEEP_1, 500);
		vcp_ack_data = &vcp_ipi_ackdata1;
	} else {
		pr_notice("[%s]: invalid vcp core index: %d\n",
			__func__, dvfs.cur_dbg_core);
		return -EVCP_DVFS_DBG_INVALID_CMD;
	}
	if (ret != IPI_ACTION_DONE) {
		pr_notice("[%s] ipi send failed with error: %d\n",
			__func__, ret);
	} else {
		if (*vcp_ack_data >= VCP_SLEEP_OFF &&
				*vcp_ack_data <= VCP_SLEEP_NO_CONDITION)
			seq_printf(m, "vcp sleep flag: %d\n",
				*vcp_ack_data);
		else
			seq_printf(m, "invalid sleep flag: %d\n",
				*vcp_ack_data);
	}

	return 0;
}

/**********************************
 * write vcp dvfs sleep
 ***********************************/
static ssize_t mt_vcp_dvfs_sleep_proc_write(
					struct file *file,
					const char __user *buffer,
					size_t count,
					loff_t *data)
{
	char desc[64], cmd[32];
	unsigned int len = 0;
	unsigned int ipi_cmd = -1;
	unsigned int ipi_cmd_size = -1;
	int ret = 0;
	int n = 0;
	int slp_cmd = -1;
	struct ipi_tx_data_t ipi_data;

	if (count <= 0)
		return 0;

	len = (count < (sizeof(desc) - 1)) ? count : (sizeof(desc) - 1);
	if (copy_from_user(desc, buffer, len))
		return 0;

	desc[len] = '\0';

	if (!dvfs.sleep_init_done)
		slp_ipi_init();

	n = sscanf(desc, "%31s %d", cmd, &slp_cmd);
	if (n != 2) {
		pr_notice("[%s]: invalid command", __func__);
		return -EVCP_DVFS_DBG_INVALID_CMD;
	}
	if (!strcmp(cmd, "sleep")) {
		if (slp_cmd < VCP_SLEEP_OFF
				|| slp_cmd > VCP_SLEEP_NO_CONDITION) {
			pr_info("[%s]: invalid slp cmd: %d\n",
				__func__, slp_cmd);
			return -EVCP_DVFS_DBG_INVALID_CMD;
		}
		ipi_data.arg1 = slp_cmd;
		if (dvfs.cur_dbg_core == VCP_CORE_0) {
			ipi_cmd = IPI_OUT_C_SLEEP_0;
			ipi_cmd_size = PIN_OUT_C_SIZE_SLEEP_0;
		} else if (dvfs.cur_dbg_core == VCP_CORE_1) {
			ipi_cmd = IPI_OUT_C_SLEEP_1;
			ipi_cmd_size = PIN_OUT_C_SIZE_SLEEP_1;
		} else {
			pr_notice("[%s]: invalid debug core: %d\n",
				__func__, dvfs.cur_dbg_core);
		}
		ret = mtk_ipi_send(&vcp_ipidev, ipi_cmd, IPI_SEND_WAIT,
			&ipi_data, ipi_cmd_size, 500);
		if (ret != VCP_IPI_DONE) {
			pr_info("%s: VCP send IPI fail - %d\n",
				__func__, ret);
			return -EVCP_DVFS_IPI_FAILED;
		}
	} else if (!strcmp(cmd, "dbg_core")) {
		if (slp_cmd < VCP_MAX_CORE_NUM)
			dvfs.cur_dbg_core = slp_cmd;
	} else {
		pr_notice("[%s]: invalid command: %s\n", cmd);
		return -EVCP_DVFS_DBG_INVALID_CMD;
	}

	return count;
}

/****************************
 * show vcp dvfs ctrl
 *****************************/
static int mt_vcp_dvfs_ctrl_proc_show(struct seq_file *m, void *v)
{
	unsigned long spin_flags;
	int i;

	spin_lock_irqsave(&vcp_awake_spinlock, spin_flags);
	vcp_current_freq = readl(CURRENT_FREQ_REG);
	vcp_expected_freq = readl(EXPECTED_FREQ_REG);
	spin_unlock_irqrestore(&vcp_awake_spinlock, spin_flags);
	seq_printf(m, "VCP DVFS: %s\n", (vcp_dvfs_flag == 1)?"ON":"OFF");
	seq_printf(m, "VCP frequency: cur=%dMHz, expect=%dMHz\n",
				vcp_current_freq, vcp_expected_freq);

	for (i = 0; i < NUM_FEATURE_ID; i++)
		seq_printf(m, "feature=%d, freq=%d, enable=%d\n",
			feature_table[i].feature, feature_table[i].freq,
			feature_table[i].enable);

	for (i = 0; i < NUM_SENSOR_TYPE; i++)
		seq_printf(m, "sensor id=%d, freq=%d, enable=%d\n",
			sensor_type_table[i].feature, sensor_type_table[i].freq,
			sensor_type_table[i].enable);

	return 0;
}

/**********************************
 * write vcp dvfs ctrl
 ***********************************/
static ssize_t mt_vcp_dvfs_ctrl_proc_write(
					struct file *file,
					const char __user *buffer,
					size_t count,
					loff_t *data)
{
	char desc[64], cmd[32];
	unsigned int len = 0;
	int dvfs_opp;
	int freq;
	int n;

	if (count <= 0)
		return 0;

	len = (count < (sizeof(desc) - 1)) ? count : (sizeof(desc) - 1);
	if (copy_from_user(desc, buffer, len))
		return 0;

	desc[len] = '\0';

	n = sscanf(desc, "%31s %d", cmd, &dvfs_opp);
	if (n == 1 || n == 2) {
		if (!strcmp(cmd, "on")) {
			vcp_dvfs_flag = 1;
			pr_info("VCP DVFS: ON\n");
		} else if (!strcmp(cmd, "off")) {
			vcp_dvfs_flag = -1;
			pr_info("VCP DVFS: OFF\n");
		} else if (!strcmp(cmd, "opp")) {
			if (dvfs_opp == -1) {
				pr_info("remove the opp setting of command\n");
				feature_table[VCORE_TEST_FEATURE_ID].freq = 0;
				vcp_deregister_feature(
						VCORE_TEST_FEATURE_ID);
			} else if (dvfs_opp >= 0 &&
					dvfs_opp < dvfs.vcp_opp_nums) {
				uint32_t i;
				uint32_t sum = 0, added_freq = 0;

				pr_info("manually set opp = %d\n", dvfs_opp);

				/*
				 * calculate vcp frequence
				 */
				for (i = 0; i < NUM_FEATURE_ID; i++) {
					if (i != VCORE_TEST_FEATURE_ID &&
						feature_table[i].enable == 1)
						sum += feature_table[i].freq;
				}

				/*
				 * calculate vcp sensor frequence
				 */
				for (i = 0; i < NUM_SENSOR_TYPE; i++) {
					if (sensor_type_table[i].enable == 1)
						sum +=
						sensor_type_table[i].freq;
				}

				for (i = 0; i < dvfs.vcp_opp_nums; i++) {
					freq = dvfs.opp[i].freq;

					if (dvfs_opp == i && sum < freq) {
						added_freq = freq - sum;
						break;
					}
				}

				for (i = 0; i < NUM_FEATURE_ID; i++)
					if (VCORE_TEST_FEATURE_ID ==
						feature_table[i].feature) {
						feature_table[i].freq =
							added_freq;
						pr_notice("[%s]: test freq: %d\n",
							__func__,
							feature_table[i].freq);
						break;
					}

				pr_debug("request freq: %d + %d = %d (MHz)\n",
						sum,
						added_freq,
						sum + added_freq);

				vcp_register_feature(
						VCORE_TEST_FEATURE_ID);
			} else {
				pr_info("invalid opp value %d\n", dvfs_opp);
			}
		} else {
			pr_info("invalid command %s\n", cmd);
		}
	} else {
		pr_info("invalid length %d\n", n);
	}

	return count;
}

#define PROC_FOPS_RW(name) \
static int mt_ ## name ## _proc_open(\
					struct inode *inode, \
					struct file *file) \
{ \
	return single_open(file, \
					mt_ ## name ## _proc_show, \
					PDE_DATA(inode)); \
} \
static const struct proc_ops \
	mt_ ## name ## _proc_fops = {\
	.proc_open		= mt_ ## name ## _proc_open, \
	.proc_read		= seq_read, \
	.proc_lseek		= seq_lseek, \
	.proc_release	= single_release, \
	.proc_write		= mt_ ## name ## _proc_write, \
}

#define PROC_FOPS_RO(name) \
static int mt_ ## name ## _proc_open(\
				struct inode *inode,\
				struct file *file)\
{\
	return single_open(file, \
						mt_ ## name ## _proc_show, \
						PDE_DATA(inode)); \
} \
static const struct proc_ops mt_ ## name ## _proc_fops = {\
	.proc_open		= mt_ ## name ## _proc_open,\
	.proc_read		= seq_read,\
	.proc_lseek		= seq_lseek,\
	.proc_release	= single_release,\
}

#define PROC_ENTRY(name)	{__stringify(name), &mt_ ## name ## _proc_fops}

PROC_FOPS_RO(vcp_dvfs_state);
PROC_FOPS_RW(vcp_dvfs_sleep_cnt);
PROC_FOPS_RW(vcp_dvfs_sleep);
PROC_FOPS_RW(vcp_dvfs_ctrl);

static int mt_vcp_dvfs_create_procfs(void)
{
	struct proc_dir_entry *dir = NULL;
	int i, ret = 0;

	struct pentry {
		const char *name;
		const struct proc_ops *fops;
	};

	const struct pentry entries[] = {
		PROC_ENTRY(vcp_dvfs_state),
		PROC_ENTRY(vcp_dvfs_sleep_cnt),
		PROC_ENTRY(vcp_dvfs_sleep),
		PROC_ENTRY(vcp_dvfs_ctrl)
	};

	dir = proc_mkdir("vcp_dvfs", NULL);
	if (!dir) {
		pr_notice("fail to create /proc/vcp_dvfs @ %s()\n", __func__);
		return -ENOMEM;
	}

	for (i = 0; i < ARRAY_SIZE(entries); i++) {
		if (!proc_create(entries[i].name,
						0664,
						dir,
						entries[i].fops)) {
			pr_notice("ERROR: %s: create /proc/vcp_dvfs/%s failed\n",
						__func__, entries[i].name);
			ret = -ENOMEM;
		}
	}

	return ret;
}
#endif /* CONFIG_PROC_FS */

static const struct of_device_id vcpdvfs_of_ids[] = {
	{.compatible = "mediatek,vcp_dvfs",},
	{}
};

static void __init mt_pmic_sshub_init(void)
{
#if !IS_ENABLED(CONFIG_FPGA_EARLY_PORTING)
	int max_vcore = dvfs.opp[dvfs.vcp_opp_nums - 1].tuned_vcore + 100000;
	int max_vsram = dvfs.opp[dvfs.vcp_opp_nums - 1].vsram + 100000;

	if (dvfs.pmic_sshub_en) {
		/* set VCP VCORE voltage */
		if (regulator_set_voltage(reg_vcore, dvfs.opp[0].tuned_vcore,
				max_vcore) != 0)
			pr_notice("Set wrong vcore voltage\n");

		/* set VCP VSRAM voltage */
		if (regulator_set_voltage(reg_vsram, dvfs.opp[0].vsram,
				max_vsram) != 0)
			pr_notice("Set wrong vsram voltage\n");
	}

	vcp_reg_init(dvfs.pmic_regmap, &dvfs.pmic_regs->_sshub_op_mode);
	vcp_reg_init(dvfs.pmic_regmap, &dvfs.pmic_regs->_sshub_op_en);
	vcp_reg_init(dvfs.pmic_regmap, &dvfs.pmic_regs->_sshub_op_cfg);


	if (dvfs.pmic_sshub_en) {
		/* BUCK_VCORE_SSHUB_EN: ON */
		/* LDO_VSRAM_OTHERS_SSHUB_EN: ON */
		/* PMRC mode: OFF */
		vcp_reg_init(dvfs.pmic_regmap,
			&dvfs.pmic_regs->_sshub_buck_en);
		vcp_reg_init(dvfs.pmic_regmap,
			&dvfs.pmic_regs->_sshub_ldo_en);
		vcp_reg_init(dvfs.pmic_regmap,
			&dvfs.pmic_regs->_pmrc_en);

		if (regulator_enable(reg_vcore) != 0)
			pr_notice("Enable vcore failed!!!\n");
		if (regulator_enable(reg_vsram) != 0)
			pr_notice("Enable vsram failed!!!\n");
	}
#endif
}

void sync_ulposc_cali_data_to_vcp(void)
{
	unsigned int sel_clk = 0;
	unsigned int ipi_data[2];
	unsigned short *p = (unsigned short *)&ipi_data[1];
	int i, ret;

	if (!dvfs.ulposc_hw.do_ulposc_cali) {
		pr_notice("[%s]: ulposc2 calibration is not done by AP\n",
			__func__);
		return;
	}

	if (dvfs.ulposc_hw.cali_failed) {
		pr_notice("[%s]: ulposc2 calibration failed\n", __func__);
		return;
	}

	if (!dvfs.sleep_init_done)
		slp_ipi_init();

	ipi_data[0] = VCP_SYNC_ULPOSC_CALI;

	for (i = 0; i < dvfs.ulposc_hw.cali_nums; i++) {
		*p = dvfs.ulposc_hw.cali_freq[i];
		*(p + 1) = dvfs.ulposc_hw.cali_val[i];

		pr_notice("[%s]: ipi to vcp: freq=%d, cali_val=0x%x\n",
			__func__,
			dvfs.ulposc_hw.cali_freq[i],
			dvfs.ulposc_hw.cali_val[i]);

		ret = mtk_ipi_send_compl(&vcp_ipidev,
					IPI_OUT_C_SLEEP_0,
					IPI_SEND_WAIT,
					&ipi_data[0],
					PIN_OUT_C_SIZE_SLEEP_0,
					500);
		if (ret != IPI_ACTION_DONE) {
			pr_notice("[%s]: ipi send ulposc cali val(%d, 0x%x) fail\n",
				__func__,
				dvfs.ulposc_hw.cali_freq[i],
				dvfs.ulposc_hw.cali_val[i]);
			WARN_ON(1);
		}
	}

	vcp_reg_read(dvfs.clk_hw->vcp_clk_regmap,
		&dvfs.clk_hw->_sel_clk, &sel_clk);
	if ((sel_clk & (VCP_ULPOSC_SEL_CORE | VCP_ULPOSC_SEL_PERI)) == 0) {
		pr_notice("[%s]:ERROR vcp is not switched to ULPOSC, CLK_SW_SEL=0x%x\n",
			__func__, sel_clk);
		WARN_ON(1);
	}
}

static inline bool __init is_ulposc_cali_pass(unsigned int cur,
		unsigned int target)
{
	if (cur > (target * (1000 - CALI_MIS_RATE) / 1000)
			&& cur < (target * (1000 + CALI_MIS_RATE) / 1000))
		return 1;

	/* calibrated failed here */
	pr_notice("[%s]: cur: %dMHz, target: %dMHz calibrate failed\n",
		__func__,
		(cur * 26) / CALI_DIV_VAL,
		(target * 26) / CALI_DIV_VAL);
	return 0;
}

static unsigned int __init get_ulposc_clk_by_fmeter(void)
{
	unsigned int i = 0, result = 0;
	unsigned int misc_org, dbg_org, cali0_org, cali1_org;
	unsigned int measure_done = 0;
	int is_fmeter_timeout = 0;

	/* backup regsiters */
	vcp_reg_read(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_clk_misc_cfg0, &misc_org);
	vcp_reg_read(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_clk_dbg_cfg, &dbg_org);
	vcp_reg_read(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_clk26cali_0, &cali0_org);
	vcp_reg_read(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_clk26cali_1, &cali1_org);

	/* set load cnt */
	vcp_reg_update(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_load_cnt,
		dvfs.ulposc_hw.clkdbg_regs->_load_cnt.init_config);

	/* select meter clock input CLK_DBG_CFG[1:0] = 0x0 */
	vcp_reg_update(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_fmeter_ck_sel,
		dvfs.ulposc_hw.clkdbg_regs->_fmeter_ck_sel.init_config);

	/* select clock source CLK_DBG_CFG[21:16] */
	vcp_reg_update(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_abist_clk,
		dvfs.ulposc_hw.clkdbg_regs->_abist_clk.init_config);

	/* select meter div CLK_MISC_CFG_0[31:24] = 0x3 for div 1 */
	vcp_reg_update(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_meter_div,
		dvfs.ulposc_hw.clkdbg_regs->_meter_div.init_config);

	/* enable fmeter */
	vcp_reg_update(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_fmeter_en,
		dvfs.ulposc_hw.clkdbg_regs->_fmeter_en.init_config);

	/* trigger fmeter to start measure */
	vcp_reg_update(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_trigger_cal,
		dvfs.ulposc_hw.clkdbg_regs->_trigger_cal.init_config);

	vcp_reg_read(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_trigger_cal, &measure_done);

	/* wait for frequency measurement done */
	while (measure_done) {
		i++;
		udelay(10);
		if (i > 40000) {
			is_fmeter_timeout = 1;
			break;
		}
		vcp_reg_read(dvfs.ulposc_hw.topck_regmap,
			&dvfs.ulposc_hw.clkdbg_regs->_trigger_cal,
			&measure_done);
	}

	/* disable fmeter */
	vcp_reg_update(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_fmeter_en,
		!dvfs.ulposc_hw.clkdbg_regs->_fmeter_en.init_config);

	/* get fmeter result */
	if (!is_fmeter_timeout) {
		vcp_reg_read(dvfs.ulposc_hw.topck_regmap,
			&dvfs.ulposc_hw.clkdbg_regs->_cal_cnt,
			&result);
	} else {
		result = 0;
	}

	/* restore freq meter registers */
	vcp_reg_update(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_clk_misc_cfg0, misc_org);
	vcp_reg_update(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_clk_dbg_cfg, dbg_org);
	vcp_reg_update(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_clk26cali_0, cali0_org);
	vcp_reg_update(dvfs.ulposc_hw.topck_regmap,
		&dvfs.ulposc_hw.clkdbg_regs->_clk26cali_1, cali1_org);

	return result;
}

static void __init set_ulposc_cali_value(unsigned int cali_val)
{
	int ret = 0;

	ret = vcp_reg_update(dvfs.ulposc_hw.apmixed_regmap,
		&dvfs.ulposc_hw.ulposc_regs->_cali,
		cali_val);

	udelay(50);
}

static int __init ulposc_cali_process(unsigned int cali_idx,
		unsigned short *cali_res)
{
	unsigned int target_val = 0, current_val = 0;
	unsigned int min = CAL_MIN_VAL, max = CAL_MAX_VAL, mid;
	unsigned int diff_by_min = 0, diff_by_max = 0xffff;

	target_val = dvfs.ulposc_hw.cali_freq[cali_idx] * CALI_DIV_VAL / 26;

	do {
		mid = (min + max) / 2;
		if (mid == min) {
			pr_debug("mid(%u) == min(%u)\n", mid, min);
			break;
		}

		set_ulposc_cali_value(mid);
		current_val = get_ulposc_clk_by_fmeter();

		if (current_val > target_val)
			max = mid;
		else
			min = mid;
	} while (min <= max);

	set_ulposc_cali_value(min);
	current_val = get_ulposc_clk_by_fmeter();
	diff_by_min = (current_val > target_val) ?
		(current_val - target_val):(target_val - current_val);

	set_ulposc_cali_value(max);
	current_val = get_ulposc_clk_by_fmeter();
	diff_by_max = (current_val > target_val) ?
		(current_val - target_val):(target_val - current_val);

	*cali_res = (diff_by_min < diff_by_max) ? min : max;

	set_ulposc_cali_value(*cali_res);
	current_val = get_ulposc_clk_by_fmeter();
	if (!is_ulposc_cali_pass(current_val, target_val)) {
		pr_notice("[%s]: calibration failed for: %dMHz\n",
			__func__,
			dvfs.ulposc_hw.cali_freq[cali_idx]);
		*cali_res = 0;
		return -EVCP_DVFS_CALI_FAILED;
	}

	pr_notice("[%s]: target: %uMhz, calibrated = %uMHz\n",
		__func__,
		(target_val * 26) / CALI_DIV_VAL,
		(current_val * 26) / CALI_DIV_VAL);

	return 0;
}

static void turn_onoff_ulposc2(enum ulposc_onoff_enum on)
{
	if (on) {
		/* turn on ulposc */
		vcp_reg_update(dvfs.clk_hw->vcp_clk_regmap,
			&dvfs.clk_hw->_clk_high_en, on);
		vcp_reg_update(dvfs.clk_hw->vcp_clk_regmap,
			&dvfs.clk_hw->_ulposc2_en, !on);

		/* wait settle time */
		udelay(150);

		vcp_reg_update(dvfs.clk_hw->vcp_clk_regmap,
			&dvfs.clk_hw->_ulposc2_cg, on);
	} else {
		vcp_reg_update(dvfs.clk_hw->vcp_clk_regmap,
			&dvfs.clk_hw->_ulposc2_cg, on);
		udelay(50);
		vcp_reg_update(dvfs.clk_hw->vcp_clk_regmap,
			&dvfs.clk_hw->_ulposc2_en, !on);
	}
	udelay(50);
}

static int __init mt_vcp_dvfs_do_ulposc_cali_process(void)
{
	int ret = 0;
	unsigned int i;

	if (!dvfs.ulposc_hw.do_ulposc_cali) {
		pr_notice("[%s]: ulposc2 calibration is not done by AP\n",
			__func__);
		return 0;
	}

	for (i = 0; i < dvfs.ulposc_hw.cali_nums; i++) {
		turn_onoff_ulposc2(ULPOSC_OFF);

		ret += vcp_reg_update(dvfs.ulposc_hw.apmixed_regmap,
			&dvfs.ulposc_hw.ulposc_regs->_con0,
			dvfs.ulposc_hw.cali_configs[i].con0_val);
		ret += vcp_reg_update(dvfs.ulposc_hw.apmixed_regmap,
			&dvfs.ulposc_hw.ulposc_regs->_con1,
			dvfs.ulposc_hw.cali_configs[i].con1_val);
		ret += vcp_reg_update(dvfs.ulposc_hw.apmixed_regmap,
			&dvfs.ulposc_hw.ulposc_regs->_con2,
			dvfs.ulposc_hw.cali_configs[i].con2_val);
		if (ret) {
			pr_notice("[%s]: config ulposc register failed\n",
				__func__);
			return ret;
		}

		turn_onoff_ulposc2(ULPOSC_ON);

		ret = ulposc_cali_process(i, &dvfs.ulposc_hw.cali_val[i]);
		if (ret) {
			pr_notice("[%s]: cali %uMHz ulposc failed\n",
				__func__, dvfs.ulposc_hw.cali_freq[i]);
			dvfs.ulposc_hw.cali_failed = true;
			return -EVCP_DVFS_CALI_FAILED;
		}
	}

	turn_onoff_ulposc2(ULPOSC_OFF);

	pr_notice("[%s]: ulposc calibration all done\n", __func__);

	return ret;
}

static int __init mt_vcp_dts_gpio_check(struct platform_device *pdev)
{
	struct device_node *node = pdev->dev.of_node;
	struct reg_info vreq_gpio;
	u32 vreq_mode = 0;
	u32 val = 0;
	int ret = 0;

	ret = of_property_read_u32(node, "gpio-vreq-mode", &vreq_mode);
	if (ret)
		goto GET_GPIO_INFO_FAILED;

	ret = of_property_count_u32_elems(node, "gpio-vreq");
	if ((ret / GPIO_ELEM_CNT) <= 0 || (ret % GPIO_ELEM_CNT) != 0) {
		pr_notice("[%s]: get gpio info failed, count: %d\n",
			__func__, ret);
		return ret;
	}

	ret = of_property_read_u32_index(node, "gpio-vreq", 0, &vreq_gpio.ofs);
	if (ret)
		goto GET_GPIO_INFO_FAILED;

	ret = of_property_read_u32_index(node, "gpio-vreq", 1, &vreq_gpio.msk);
	if (ret)
		goto GET_GPIO_INFO_FAILED;

	ret = of_property_read_u32_index(node, "gpio-vreq", 2, &vreq_gpio.bit);
	if (ret)
		goto GET_GPIO_INFO_FAILED;

	ret = vcp_reg_read(dvfs.gpio_regmap, &vreq_gpio, &val);

	if (val == vreq_mode) {
		pr_notice("v_req muxpin setting is correct\n");
	} else {
		pr_notice("WRONG v_req muxpin setting, func mode: %d\n",
			vreq_mode);
		return -EVCP_DVFS_GPIO_CONFIG_FAILED;
		WARN_ON(1);
	}

	return ret;
GET_GPIO_INFO_FAILED:
	pr_notice("[%s]: get gpio info failed with err: %d\n",
		__func__, ret);
	return ret;
}

static int __init mt_vcp_dts_get_cali_hw_setting(struct device_node *node,
		struct ulposc_cali_hw *cali_hw)
{
	unsigned int i;
	int ret = 0;

	/* find hw calibration configuration data */
	/* update clk_dbg or ulposc_cali data if there is minor change by hw */
	ret = of_property_count_u32_elems(node, "ulposc-cali-config");
	if ((ret / CALI_CONFIG_ELEM_CNT) <= 0
		|| (ret % CALI_CONFIG_ELEM_CNT) != 0) {
		pr_notice("[%s]: cali config count does not equal to cali nums\n",
			__func__);
		return ret;
	}

	cali_hw->cali_configs = kcalloc(cali_hw->cali_nums,
				sizeof(struct ulposc_cali_config),
				GFP_KERNEL);
	if (!(cali_hw->cali_configs))
		return -ENOMEM;

	for (i = 0; i < cali_hw->cali_nums; i++) {
		ret = of_property_read_u32_index(node, "ulposc-cali-config",
			(i * CALI_CONFIG_ELEM_CNT),
			&(cali_hw->cali_configs[i].con0_val));
		if (ret) {
			pr_notice("[%s]: get con0 setting failed\n", __func__);
			goto CALI_DATA_INIT_FAILED;
		}

		ret = of_property_read_u32_index(node, "ulposc-cali-config",
			(i * CALI_CONFIG_ELEM_CNT + 1),
			&(cali_hw->cali_configs[i].con1_val));
		if (ret) {
			pr_notice("[%s]: get con1 setting failed\n", __func__);
			goto CALI_DATA_INIT_FAILED;
		}

		ret = of_property_read_u32_index(node, "ulposc-cali-config",
			(i * CALI_CONFIG_ELEM_CNT + 2),
			&(cali_hw->cali_configs[i].con2_val));
		if (ret) {
			pr_notice("[%s]: get con2 setting failed\n", __func__);
			goto CALI_DATA_INIT_FAILED;
		}
	}

	return 0;
CALI_DATA_INIT_FAILED:
	kfree(cali_hw->cali_configs);
	return ret;
}

static int __init mt_vcp_dts_get_cali_target(struct device_node *node,
		struct ulposc_cali_hw *cali_hw)
{
	int ret = 0;
	unsigned int i;
	unsigned int tmp;

	/* find number of ulposc need to do calibration */
	ret = of_property_read_u32(node, "ulposc-cali-num",
		&cali_hw->cali_nums);
	if (ret) {
		pr_notice("[%s]: find ulposc calibration numbers failed\n",
			__func__);
		return ret;
	}

	ret = of_property_count_u32_elems(node, "ulposc-cali-target");
	if (ret != cali_hw->cali_nums) {
		pr_notice("[%s]: target nums does not equals to ulposc-cali-num\n",
			__func__);
		return ret;
	}

	cali_hw->cali_val = kcalloc(cali_hw->cali_nums, sizeof(unsigned short),
				GFP_KERNEL);
	if (!cali_hw->cali_val)
		return -ENOMEM;

	cali_hw->cali_freq = kcalloc(cali_hw->cali_nums,
				sizeof(unsigned short),
				GFP_KERNEL);
	if (!cali_hw->cali_freq) {
		ret = -ENOMEM;
		goto CALI_TARGET_ALLOC_FAILED;
	}

	for (i = 0; i < cali_hw->cali_nums; i++) {
		ret = of_property_read_u32_index(node, "ulposc-cali-target",
			i, &tmp);
		if (ret) {
			pr_notice("[%s]: find cali target failed, idx: %d\n",
				__func__, i);
			goto FIND_TARGET_FAILED;
		}
		cali_hw->cali_freq[i] = (unsigned short) tmp;
	}

	return ret;

FIND_TARGET_FAILED:
	kfree(cali_hw->cali_freq);
CALI_TARGET_ALLOC_FAILED:
	kfree(cali_hw->cali_val);
	return ret;
}

static int __init mt_vcp_dts_get_cali_hw_regs(struct device_node *node,
		struct ulposc_cali_hw *cali_hw)
{
	const char *str = NULL;
	int ret = 0;
	int i;

	/* find ulposc register hw version */
	ret = of_property_read_string(node, "ulposc-cali-ver", &str);
	if (ret) {
		pr_notice("[%s]: find ulposc-cali-ver failed with err: %d\n",
			__func__, ret);
		return ret;
	}

	for (i = 0; i < MAX_ULPOSC_VERSION; i++)
		if (!strcmp(ulposc_ver[i], str))
			cali_hw->ulposc_regs = &cali_regs[i];

	if (!cali_hw->ulposc_regs) {
		pr_notice("[%s]: no ulposc cali reg found\n");
		return -EVCP_DVFS_NO_CALI_HW_FOUND;
	}

	/* find clk dbg register hw version */
	ret = of_property_read_string(node, "clk-dbg-ver", &str);
	if (ret) {
		pr_notice("[%s]: find clk-dbg-ver failed with err: %d\n",
			__func__, ret);
		return ret;
	}

	for (i = 0; i < MAX_CLK_DBG_VERSION; i++)
		if (!strcmp(clk_dbg_ver[i], str))
			cali_hw->clkdbg_regs = &clk_dbg_reg[i];
	if (!cali_hw->clkdbg_regs) {
		pr_notice("[%s]: no clkfbg regs found\n",
			__func__);
		return -EVCP_DVFS_NO_CALI_HW_FOUND;
	}

	return ret;
}

static int __init mt_vcp_dts_init_vcp_clk_hw(struct device_node *node)
{
	const char *str = NULL;
	unsigned int i;
	int ret = 0;

	ret = of_property_read_string(node, "vcp-clk-hw-ver", &str);
	if (ret) {
		pr_notice("[%s]: find vcp-clk-hw-ver failed with err: %d\n",
			__func__, ret);
		return ret;
	}

	for (i = 0; i < MAX_VCP_CLK_VERSION; i++) {
		if (!strcmp(vcp_clk_ver[i], str)) {
			dvfs.clk_hw = &(vcp_clk_hw_regs[i]);
			return 0;
		}
	}

	pr_notice("[%s]: no vcp clk hw reg found\n", __func__);

	return -EVCP_DVFS_NO_CALI_HW_FOUND;
}

static int __init mt_vcp_dts_init_cali_regmap(struct device_node *node,
		struct ulposc_cali_hw *cali_hw)
{
	/* init regmap for calibration process */
	cali_hw->topck_regmap = syscon_regmap_lookup_by_phandle(node,
							TOPCK_PHANDLE_NAME);
	if (IS_ERR(cali_hw->topck_regmap)) {
		pr_notice("topck regmap init failed: %d\n",
			PTR_ERR(cali_hw->topck_regmap));
		return PTR_ERR(cali_hw->topck_regmap);
	}
	cali_hw->apmixed_regmap = syscon_regmap_lookup_by_phandle(node,
							APMIX_PHANDLE_NAME);
	if (IS_ERR(cali_hw->apmixed_regmap)) {
		pr_notice("apmixedsys regmap init failed: %d\n",
			PTR_ERR(cali_hw->apmixed_regmap));
		return PTR_ERR(cali_hw->apmixed_regmap);
	}

	return 0;
}

static int __init mt_vcp_dts_ulposc_cali_init(struct device_node *node,
		struct ulposc_cali_hw *cali_hw)
{
	int ret = 0;

	if (!cali_hw) {
		pr_notice("[%s]: ulposc calibration hw is NULL\n",
			__func__);
		WARN_ON(1);
		return -EVCP_DVFS_NO_CALI_HW_FOUND;
	}

	/* check if current platform need to do calibration */
	dvfs.ulposc_hw.do_ulposc_cali = of_property_read_bool(node,
							"do-ulposc-cali");
	if (!dvfs.ulposc_hw.do_ulposc_cali) {
		pr_notice("[%s]: skip ulposc calibration process\n", __func__);
		return 0;
	}

	ret = mt_vcp_dts_init_cali_regmap(node, cali_hw);
	if (ret)
		return ret;

	ret = mt_vcp_dts_init_vcp_clk_hw(node);
	if (ret)
		return ret;

	ret = mt_vcp_dts_get_cali_hw_regs(node, cali_hw);
	if (ret)
		return ret;

	dvfs.clk_hw->vcp_clk_regmap = syscon_regmap_lookup_by_phandle(node,
						VCP_CLK_CTRL_PHANDLE_NAME);
	if (!dvfs.clk_hw->vcp_clk_regmap) {
		pr_notice("[%s]: get vcp clk regmap failed\n", __func__);
		return ret;
	}

	ret = mt_vcp_dts_get_cali_target(node, cali_hw);
	if (ret)
		return ret;

	ret = mt_vcp_dts_get_cali_hw_setting(node, cali_hw);
	if (ret)
		return ret;

	return ret;
}

static int __init mt_vcp_dts_clk_init(struct platform_device *pdev)
{
	char buf[15];
	int ret = 0;
	int i;

	mt_vcp_pll.clk_mux = devm_clk_get(&pdev->dev, "clk_mux");
	if (IS_ERR(mt_vcp_pll.clk_mux)) {
		dev_notice(&pdev->dev, "cannot get clock mux\n");
		WARN_ON(1);
		return PTR_ERR(mt_vcp_pll.clk_mux);
	}

	/* vcp_sel has most 8 member of clk source */
	mt_vcp_pll.pll_num = 8;
	for (i = 0; i < 8; i++) {
		ret = snprintf(buf, 15, "clk_pll_%d", i);
		if (ret < 0 || ret >= 15) {
			pr_notice("[%s]: clk name buf len: %d\n",
				__func__, ret);
			return ret;
		}

		mt_vcp_pll.clk_pll[i] = devm_clk_get(&pdev->dev, buf);
		if (IS_ERR(mt_vcp_pll.clk_pll[i])) {
			dev_notice(&pdev->dev,
					"cannot get %dst clock parent\n",
					i);
			mt_vcp_pll.pll_num = i;
			break;
		}
	}

	return 0;
}

static int __init mt_vcp_dts_init_dvfs_data(struct device_node *node,
		struct dvfs_opp **opp)
{
	int ret = 0;
	int i;

	if (*opp) {
		pr_notice("[%s]: opp is initialized\n", __func__);
		return -EVCP_DVFS_DATA_RE_INIT;
	}

	/* get vcp dvfs opp count */
	ret = of_property_count_u32_elems(node, "dvfs-opp");
	if ((ret / OPP_ELEM_CNT) <= 0 || (ret % OPP_ELEM_CNT) != 0) {
		pr_notice("[%s]: get dvfs opp count failed, count: %d\n",
			__func__, ret);
		return ret;
	}
	dvfs.vcp_opp_nums = ret / 7;

	*opp = kcalloc(dvfs.vcp_opp_nums, sizeof(struct dvfs_opp), GFP_KERNEL);
	if (!(*opp))
		return -ENOMEM;

	/* get each dvfs opp data from dts node */
	for (i = 0; i < dvfs.vcp_opp_nums; i++) {
		ret = of_property_read_u32_index(node, "dvfs-opp",
				i * OPP_ELEM_CNT,
				&(*opp)[i].vcore);
		if (ret) {
			pr_notice("Cannot get property vcore(%d)\n", ret);
			goto OPP_INIT_FAILED;
		}

		ret = of_property_read_u32_index(node, "dvfs-opp",
				(i * OPP_ELEM_CNT) + 1,
				&(*opp)[i].vsram);
		if (ret) {
			pr_notice("Cannot get property vsram(%d)\n", ret);
			goto OPP_INIT_FAILED;
		}

		ret = of_property_read_u32_index(node, "dvfs-opp",
				(i * OPP_ELEM_CNT) + 2,
				&(*opp)[i].dvfsrc_opp);
		if (ret) {
			pr_notice("Cannot get property dvfsrc opp(%d)\n", ret);
			goto OPP_INIT_FAILED;
		}
		if ((*opp)[i].dvfsrc_opp != 0xff) {
			ret = vcp_get_vcore_table((*opp)[i].dvfsrc_opp);
			if (ret > 0) {
				(*opp)[i].tuned_vcore = ret;
			} else {
				/* As default value, if atf is not available */
				(*opp)[i].tuned_vcore = (*opp)[i].vcore;
			}
		} else {
			(*opp)[i].tuned_vcore = (*opp)[i].vcore;
		}

		ret = of_property_read_u32_index(node, "dvfs-opp",
				(i * OPP_ELEM_CNT) + 3,
				&(*opp)[i].spm_opp);
		if (ret) {
			pr_notice("Cannot get property spm opp(%d)\n", ret);
			goto OPP_INIT_FAILED;
		}

		ret = of_property_read_u32_index(node, "dvfs-opp",
				(i * OPP_ELEM_CNT) + 4,
				&(*opp)[i].freq);

		if (ret) {
			pr_notice("Cannot get property freq(%d)\n", ret);
			goto OPP_INIT_FAILED;
		}

		ret = of_property_read_u32_index(node, "dvfs-opp",
				(i * OPP_ELEM_CNT) + 5,
				&(*opp)[i].clk_mux);
		if (ret) {
			pr_notice("Cannot get property clk mux(%d)\n", ret);
			goto OPP_INIT_FAILED;
		}

		ret = of_property_read_u32_index(node, "dvfs-opp",
				(i * OPP_ELEM_CNT) + 6,
				&(*opp)[i].resource_req);
		if (ret) {
			pr_notice("Cannot get property opp resource(%d)\n", ret);
			goto OPP_INIT_FAILED;
		}
	}
	return ret;

OPP_INIT_FAILED:
	kfree(*opp);
	return ret;
}

static int __init mt_vcp_dts_init_pmic_data(void)
{
	unsigned int i;

	for (i = 0; i < MAX_VCP_DVFS_CHIP_HW; i++) {
		if (of_machine_is_compatible(vcp_dvfs_hw_chip_ver[i])) {
			dvfs.pmic_regs = &vcp_pmic_hw_regs[i];
			return 0;
		}
	}

	/* init pmic data failed here */
	pr_notice("[%s]: no vcp pmic regs found\n", __func__);
	return -EVCP_DVFS_NO_PMIC_REG_FOUND;
}

static int __init mt_vcp_dts_regmap_init(struct platform_device *pdev,
		struct device_node *node)
{
	struct platform_device *pmic_pdev;
	struct device_node *pmic_node;
	struct pmic_main_chip *chip;
	struct regmap *regmap;

	/* get GPIO regmap */
	regmap = syscon_regmap_lookup_by_phandle(node, GPIO_PHANDLE_NAME);
	if (IS_ERR(regmap)) {
		dev_notice(&pdev->dev,
			"Get gpio regmap fail: %ld\n",
			PTR_ERR(regmap));
		goto REGMAP_FIND_FAILED;
	}

	dvfs.gpio_regmap = regmap;

	/* get PMIC regmap */
	pmic_node = of_parse_phandle(node, PMIC_PHANDLE_NAME, 0);
	if (!pmic_node) {
		dev_notice(&pdev->dev, "fail to find pmic node\n");
		goto REGMAP_FIND_FAILED;
	}

	pmic_pdev = of_find_device_by_node(pmic_node);
	if (!pmic_pdev) {
		dev_notice(&pdev->dev, "fail to find pmic device\n");
		goto REGMAP_FIND_FAILED;
	}

	chip = dev_get_drvdata(&(pmic_pdev->dev));
	if (!chip) {
		dev_notice(&pdev->dev, "fail to find pmic drv data\n");
		goto REGMAP_FIND_FAILED;
	}

	regmap = chip->regmap;
	if (IS_ERR_VALUE(regmap)) {
		dvfs.pmic_regmap = NULL;
		dev_notice(&pdev->dev, "get pmic regmap fail\n");
		goto REGMAP_FIND_FAILED;
	}

	dvfs.pmic_regmap = regmap;

	return 0;
REGMAP_FIND_FAILED:
	WARN_ON(1);
	return -EVCP_DVFS_REGMAP_INIT_FAILED;
}

static int __init mt_vcp_dts_init(struct platform_device *pdev)
{
	struct device_node *node;
	int ret = 0;

	/* find device tree node of vcp_dvfs */
	node = pdev->dev.of_node;
	if (!node) {
		dev_notice(&pdev->dev, "fail to find VCPDVFS node\n");
		return -ENODEV;
	}

	ret = of_property_read_u32(node, "vow-lp-en-gear",
		&dvfs.vow_lp_en_gear);
	if (ret) {
		pr_notice("[%s]: no vow-lp-enable-gear property, set gear to -1\n",
			__func__);
		dvfs.vow_lp_en_gear = -1;
	}

	ret = of_property_read_u32(node, "vcp-cores",
		&dvfs.core_nums);
	if (ret || dvfs.core_nums > VCP_MAX_CORE_NUM) {
		pr_notice("[%s]: find invalid core numbers, set to 1\n",
			__func__);
		dvfs.core_nums = 1;
	}

	dvfs.pmic_sshub_en = of_property_read_bool(node, "pmic-sshub-support");

	ret = mt_vcp_dts_regmap_init(pdev, node);
	if (ret) {
		pr_notice("[%s]: vcp regmap init failed with err: %d\n",
			__func__, ret);
		goto DTS_FAILED;
	}

	ret = mt_vcp_dts_init_pmic_data();
	if (ret)
		goto DTS_FAILED;

	/* init dvfs data */
	ret = mt_vcp_dts_init_dvfs_data(node, &dvfs.opp);
	if (ret) {
		pr_notice("[%s]: vcp dvfs opp data init failed with err: %d\n",
			__func__, ret);
		goto DTS_FAILED;
	}

	/* init clock mux/pll */
	ret = mt_vcp_dts_clk_init(pdev);
	if (ret) {
		pr_notice("[%s]: init vcp clk failed with err: %d\n",
			__func__, ret);
		goto DTS_FAILED_FREE_RES;
	}

	/* init ulposc cali dts data */
	ret = mt_vcp_dts_ulposc_cali_init(node, &dvfs.ulposc_hw);
	if (ret) {
		pr_notice("[%s]: init vcp ulposc cali data with err: %d\n",
			__func__, ret);
		goto DTS_FAILED_FREE_RES;
	}

	/* get dvfsrc regulator */
	dvfsrc_vvcp_power = regulator_get(&pdev->dev, "dvfsrc-vvcp");
	if (IS_ERR(dvfsrc_vvcp_power) || !dvfsrc_vvcp_power) {
		pr_notice("regulator dvfsrc-vvcp is not available\n");
		ret = PTR_ERR(dvfsrc_vvcp_power);
		goto PASS;
	}

	/* get Vcore/Vsram Regulator */
	reg_vcore = devm_regulator_get_optional(&pdev->dev, "sshub-vcore");
	if (IS_ERR(reg_vcore) || !reg_vcore) {
		pr_notice("regulator vcore sshub supply is not available\n");
		ret = PTR_ERR(reg_vcore);
		goto PASS;
	}

	reg_vsram = devm_regulator_get_optional(&pdev->dev, "sshub-vsram");
	if (IS_ERR(reg_vsram) || !reg_vsram) {
		pr_notice("regulator vsram sshub supply is not available\n");
		ret = PTR_ERR(reg_vsram);
		goto PASS;
	}
PASS:
	return 0;

DTS_FAILED_FREE_RES:
	kfree(dvfs.opp);
DTS_FAILED:
	return ret;
}

static int __init mt_vcp_dvfs_pdrv_probe(struct platform_device *pdev)
{
	int ret = 0;

	ret = mt_vcp_dts_init(pdev);
	if (ret) {
		pr_notice("[%s]: dts init failed with err: %d\n",
			__func__, ret);
		goto DTS_INIT_FAILED;
	}

	/* check gpio setting */
	ret = mt_vcp_dts_gpio_check(pdev);
	if (ret)
		goto GPIO_CHECK_FAILED;

	/* init sshub */
	mt_pmic_sshub_init();

	/* do ulposc calibration */
	mt_vcp_dvfs_do_ulposc_cali_process();
	kfree(dvfs.ulposc_hw.cali_configs);

	g_vcp_dvfs_init_flag = 1;
	pr_notice("[%s]: vcp_dvfs probe done\n", __func__);

	return 0;

GPIO_CHECK_FAILED:
	kfree(dvfs.opp);
	kfree(dvfs.ulposc_hw.cali_configs);
DTS_INIT_FAILED:
	WARN_ON(1);

	return -EVCP_DVFS_INIT_FAILED;
}

/***************************************
 * this function should never be called
 ****************************************/
static int mt_vcp_dvfs_pdrv_remove(struct platform_device *pdev)
{
	kfree(dvfs.opp);
	kfree(dvfs.ulposc_hw.cali_val);
	kfree(dvfs.ulposc_hw.cali_freq);
	kfree(dvfs.ulposc_hw.cali_configs);

	return 0;
}

#if IS_ENABLED(CONFIG_PM)
static int mt_vcp_dump_sleep_count(void)
{
	int ret = 0;
	unsigned int ipi_data = VCP_SLEEP_GET_COUNT;

	if (!dvfs.sleep_init_done)
		slp_ipi_init();

	ret = mtk_ipi_send_compl(&vcp_ipidev, IPI_OUT_C_SLEEP_0,
		IPI_SEND_WAIT, &ipi_data, PIN_OUT_C_SIZE_SLEEP_0, 500);
	if (ret != IPI_ACTION_DONE)
		pr_notice("[VCP] [%s:%d] - vcp ipi failed, ret = %d\n",
			__func__, __LINE__, ret);
	else
		pr_notice("[VCP] [%s:%d] - vcp_sleep_cnt_0 = %d\n",
			__func__, __LINE__, vcp_ipi_ackdata0);

	if (dvfs.core_nums != 2)
		return 0;

	ret = mtk_ipi_send_compl(&vcp_ipidev, IPI_OUT_C_SLEEP_1,
		IPI_SEND_WAIT, &ipi_data, PIN_OUT_C_SIZE_SLEEP_1, 500);
	if (ret != IPI_ACTION_DONE)
		pr_notice("[VCP] [%s:%d] - vcp ipi failed, ret = %d\n",
			__func__, __LINE__, ret);
	else
		pr_notice("[VCP] [%s:%d] - vcp_sleep_cnt_1 = %d\n",
			__func__, __LINE__, vcp_ipi_ackdata1);

	return 0;
}

static int vcp_pm_event(struct notifier_block *notifier,
		unsigned long pm_event, void *unused)
{
	switch (pm_event) {
	case PM_HIBERNATION_PREPARE:
		return NOTIFY_DONE;
	case PM_RESTORE_PREPARE:
		return NOTIFY_DONE;
	case PM_POST_HIBERNATION:
		return NOTIFY_DONE;
	case PM_SUSPEND_PREPARE:
	case PM_POST_SUSPEND:
		mt_vcp_dvfs_state_dump();
		mt_vcp_dump_sleep_count();
		return NOTIFY_DONE;
	}
	return NOTIFY_OK;
}

static struct notifier_block vcp_pm_notifier_func = {
	.notifier_call = vcp_pm_event,
};
#endif /* IS_ENABLED(CONFIG_PM) */

struct platform_device mt_vcp_dvfs_pdev = {
	.name = "mt-vcpdvfs",
	.id = -1,
};

static struct platform_driver mt_vcp_dvfs_pdrv __refdata = {
	.probe = mt_vcp_dvfs_pdrv_probe,
	.remove = mt_vcp_dvfs_pdrv_remove,
	.driver = {
		.name = "vcpdvfs",
		.owner = THIS_MODULE,
		.of_match_table = vcpdvfs_of_ids,
	},
};

/**********************************
 * mediatek vcp dvfs initialization
 ***********************************/
int __init vcp_dvfs_init(void)
{
	int ret = 0;

	pr_debug("%s\n", __func__);

#if IS_ENABLED(CONFIG_PROC_FS)
	/* init proc */
	if (mt_vcp_dvfs_create_procfs()) {
		pr_notice("mt_vcp_dvfs_create_procfs fail..\n");
		goto fail;
	}
#endif /* CONFIG_PROC_FS */

	/* register platform device/driver */
	ret = platform_device_register(&mt_vcp_dvfs_pdev);
	if (ret) {
		pr_notice("fail to register vcp dvfs device @ %s()\n", __func__);
		goto fail;
	}

	ret = platform_driver_register(&mt_vcp_dvfs_pdrv);
	if (ret) {
		pr_notice("fail to register vcp dvfs driver @ %s()\n", __func__);
		platform_device_unregister(&mt_vcp_dvfs_pdev);
		goto fail;
	}

	vcp_suspend_lock = wakeup_source_register(NULL, "vcp wakelock");

#if IS_ENABLED(CONFIG_PM)
	ret = register_pm_notifier(&vcp_pm_notifier_func);
	if (ret) {
		pr_notice("[%s]: failed to register PM notifier.\n", __func__);
		return ret;
	}
#endif /* IS_ENABLED(CONFIG_PM) */

	pr_notice("[%s]: vcp_dvfs init done\n", __func__);
	return 0;
fail:
	WARN_ON(1);
	return -EVCP_DVFS_INIT_FAILED;
}

void __exit vcp_dvfs_exit(void)
{
	platform_driver_unregister(&mt_vcp_dvfs_pdrv);
	platform_device_unregister(&mt_vcp_dvfs_pdev);
}

