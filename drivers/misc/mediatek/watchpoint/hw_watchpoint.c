...
#ifdef CONFIG_SMP
extern int dbgregs_hotplug_callback(unsigned int this_cpu);
#endif
...
EXPORT_SYMBOL(reset_watchpoint);
...
int add_hw_watchpoint(struct wp_event *wp_event)
{
	...
	for (i = 0; i < MAX_NR_WATCH_POINT; i++) {
		...
		if (wp_tracer.wp_events[i].virt == (wp_event->virt & ~3)) {
			pr_debug("This address have been watched in cpu%d's watchpoint\n",
				 i);
...
int wp_probe(struct platform_device *pdev)
{
	...
	wp_tracer.debug_regs =
		kmalloc_array(wp_tracer.nr_dbg, sizeof(void *), GFP_KERNEL);
	...
	ret = cpuhp_setup_state_nocalls(CPUHP_AP_ONLINE_DYN,
		"watchpoint: online", dbgregs_hotplug_callback, NULL);
	WARN_ON(ret < 0);
...
static int __init hw_watchpoint_init(void)
{
	...
	if (err) {
		pr_debug("[MTK WP] watchpoint registration failed\n");
		return err;
	}
#ifdef CONFIG_ARM64
