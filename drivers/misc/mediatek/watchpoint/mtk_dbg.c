...
{
	printk_deferred("[MTK WP] cpu %d, EDSCR 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGEDSCR);
	printk_deferred("[MTK WP] cpu %d, DBGDSCRext 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGDSCRext);
	printk_deferred("[MTK WP] cpu %d, DBGBVR0 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBVR0);
	printk_deferred("[MTK WP] cpu %d, DBGBVR1 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBVR1);
	printk_deferred("[MTK WP] cpu %d, DBGBVR2 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBVR2);
	printk_deferred("[MTK WP] cpu %d, DBGBVR3 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBVR3);
	printk_deferred("[MTK WP] cpu %d, DBGBVR4 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBVR4);
	printk_deferred("[MTK WP] cpu %d, DBGBVR5 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBVR5);

	printk_deferred("[MTK WP] cpu %d, DBGBCR0 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBCR0);
	printk_deferred("[MTK WP] cpu %d, DBGBCR1 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBCR1);
	printk_deferred("[MTK WP] cpu %d, DBGBCR2 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBCR2);
	printk_deferred("[MTK WP] cpu %d, DBGBCR3 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBCR3);
	printk_deferred("[MTK WP] cpu %d, DBGBCR4 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBCR4);
	printk_deferred("[MTK WP] cpu %d, DBGBCR5 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGBCR5);

	printk_deferred("[MTK WP] cpu %d, DBGWVR0 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGWVR0);
	printk_deferred("[MTK WP] cpu %d, DBGWVR1 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGWVR1);
	printk_deferred("[MTK WP] cpu %d, DBGWVR2 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGWVR2);
	printk_deferred("[MTK WP] cpu %d, DBGWVR3 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGWVR3);

	printk_deferred("[MTK WP] cpu %d, DBGWCR0 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGWCR0);
	printk_deferred("[MTK WP] cpu %d, DBGWCR1 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGWCR1);
	printk_deferred("[MTK WP] cpu %d, DBGWCR2 0x%lx\n",
		 cpuid, dbgregs[cpuid].DBGWCR2);
	printk_deferred("[MTK WP] cpu %d, DBGWCR3 0x%lx\n",
...
unsigned long *mt_save_dbg_regs(unsigned long *p, unsigned int cpuid)
{
	...
	cs_cpu_write(wp_context->debug_regs[cpuid], OSLAR_EL1, ~UNLOCK_KEY);
#ifdef DBG_REG_DUMP
	printk_deferred("[MTK WP] %s\n", __func__);
#endif
	if (*p == ~0x0) {
		printk_deferred("[MTK WP]restore pointer is NULL\n");
		return 0;
	}
...
#ifdef CONFIG_SMP
int dbgregs_hotplug_callback(unsigned int this_cpu)
{
	unsigned int args = 0;
	struct wp_trace_context_t *wp_context;
...
#ifdef DBG_REG_DUMP
	printk_deferred("[MTK WP] cpu %x do %s\n",
		 this_cpu, __func__);
#endif
...
#ifdef DBG_REG_DUMP
	printk_deferred("[MTK WP] cpu %x do %s, CPU%d's dbgdscr=0x%x\n",
