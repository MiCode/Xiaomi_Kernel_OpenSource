...
//	spin_lock(&dev->lock);
	status = mtk_rndis_msg_parser(rndis->params, (u8 *) req->buf);
	if (status < 0)
		pr_err("RNDIS command error %d, %d/%d\n",
			status, req->actual, req->length);

	buf = (struct rndis_init_msg_type *)req->buf;
...
rndis_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		if (w_value || w_index != rndis->ctrl_id)
			...
			/* return the result */
			buf = mtk_rndis_get_next_response(rndis->params, &n);
			if (buf) {
...
static int rndis_set_alt(struct usb_function *f,
		unsigned int intf, unsigned int alt)
{
	...
	if (intf == rndis->ctrl_id) {
		...
		if (rndis->port.in_ep->enabled) {
			...
			mtk_gether_disconnect(&rndis->port);
		}
		 ...
		 * either never activated or after mtk_rndis_uninit().
		 *
		...
		net = mtk_gether_connect(&rndis->port);
		if (IS_ERR(net))
			return PTR_ERR(net);

		mtk_rndis_set_param_dev(rndis->params, net,
...
static void rndis_open(struct gether *geth)
{
	...
	spin_lock_irqsave(&rndis_lock, flags);
	mtk_rndis_set_param_medium(rndis->params, RNDIS_MEDIUM_802_3,
				bitrate(cdev->gadget) / 100);
	mtk_rndis_signal_connect(rndis->params);
	spin_unlock_irqrestore(&rndis_lock, flags);
}
...
static void rndis_close(struct gether *geth)
{
	...
	F_RNDIS_DBG("\n");
	DBG(geth->func.config->cdev, "rndis close\n");

	spin_lock_irqsave(&rndis_lock, flags);
	mtk_rndis_set_param_medium(rndis->params, RNDIS_MEDIUM_802_3, 0);
	mtk_rndis_signal_disconnect(rndis->params);
	spin_unlock_irqrestore(&rndis_lock, flags);
}
...
rndis_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (rndis_opts) {
		if (!rndis_opts->bound) {
			mtk_gether_set_gadget(rndis_opts->net, cdev->gadget);
			status = mtk_gether_register_netdev(rndis_opts->net);
			if (status)
				goto fail;
	...
	mtk_rndis_set_param_medium(rndis->params, RNDIS_MEDIUM_802_3, 0);
	mtk_rndis_set_host_mac(rndis->params, rndis->ethaddr);
	rndis_set_max_pkt_xfer(rndis->params, rndis_ul_max_pkt_per_xfer);

	if (rndis->manufacturer && rndis->vendorID &&
			mtk_rndis_set_param_vendor(rndis->params, rndis->vendorID,
					       rndis->manufacturer)) {
		status = -EINVAL;
...
void mtk_rndis_borrow_net(struct usb_function_instance *f, struct net_device *net)
{
	struct f_rndis_opts *opts;

	opts = container_of(f, struct f_rndis_opts, func_inst);
	if (opts->bound)
		mtk_gether_cleanup(netdev_priv(opts->net));
	else
		free_netdev(opts->net);
	opts->borrowed_net = opts->bound = true;
	opts->net = net;
}
EXPORT_SYMBOL_GPL(mtk_rndis_borrow_net);
...
static struct usb_function_instance *rndis_alloc_inst(void)
{
	...
	opts->net = mtk_gether_setup_name_default("rndis");
	if (IS_ERR(opts->net)) {
...
static void rndis_free(struct usb_function *f)
{
	...
	mtk_rndis_deregister(rndis->params);
...
static struct usb_function *rndis_alloc(struct usb_function_instance *fi)
{
	...
	mtk_gether_get_host_addr_u8(opts->net, rndis->ethaddr);
	...
	rndis->port.unwrap = mtk_rndis_rm_hdr;
	rndis->port.ul_max_pkts_per_xfer = rndis_ul_max_pkt_per_xfer;
	rndis->port.dl_max_pkts_per_xfer = rndis_dl_max_pkt_per_xfer;

	rndis->port.func.name = "mtk_rndis";
	...
	params = mtk_rndis_register(rndis_response_available, rndis);
	if (IS_ERR(params)) {
		kfree(rndis);
...
DECLARE_USB_FUNCTION_INIT(mtk_rndis, rndis_alloc_inst, rndis_alloc);
MODULE_LICENSE("GPL");
