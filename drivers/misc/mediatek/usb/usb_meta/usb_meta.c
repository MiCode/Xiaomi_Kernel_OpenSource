			       ...
			       const char *buff, size_t size)
{
	...
	while (b) {
		...
		is_ffs = 0;
		strscpy(aliases, dev->ffs_aliases, sizeof(aliases));
		a = aliases;
...
{									\
	if (size >= sizeof(buffer))					\
		return -EINVAL;						\
	return strscpy(buffer, buf, sizeof(buffer));			\
}									\
static DEVICE_ATTR(field, 0444, field ## _show, field ## _store)
...
static const char TAG_NAME[] = "usbconfig=";
...
int meta_dt_get_mboot_params(void)
{
	...
	if (ptr) {
		...
		pr_notice("not fast mode [%c]", mode);
	} else
		pr_notice("cannot find \"%s=\" in cmdline", TAG_NAME);

	return 0;
}

static int dummy_bind(struct usb_composite_dev *cdev)
{
	return 0;
}
...
static int usb_meta_probe(struct platform_device *pdev)
{
	...
	INIT_LIST_HEAD(&dev->enabled_functions);
	INIT_WORK(&dev->work, android_work_meta);
	...
	// It cannot specify 'udc_name' in usb_composite_probe().
	// Thus, use dummy_usb_driver to get gadget_driver in composite.c, and
	// then specify 'udc_name' by usb_gadget_probe_driver() instead.
	err = usb_composite_probe(&dummy_usb_driver);
	if (err) {
