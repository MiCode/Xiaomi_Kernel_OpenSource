...
static int wk_bat_temp_dbg(int bat_temp_prev, int bat_temp)
{
	...
	if (bat_temp < 200 || abs(bat_temp_prev - bat_temp) > 100) {
		...
		wk_auxadc_reset();
		for (i = 0; i < 5; i++) {
			bat = auxadc_priv_read_channel(pmic_auxadc_dev,
						       AUXADC_BATADC);
#if (CONFIG_MTK_GAUGE_VERSION == 30)
...
static void wk_auxadc_dbg_init(void)
{
	...
	/* Clock related */
	adc_dbg_addr[i++] = MT6390_HK_TOP_CLK_CON0;
	adc_dbg_addr[i++] = MT6390_HK_TOP_CLK_CON1;
	/* RST related */
	adc_dbg_addr[i++] = MT6390_HK_TOP_RST_CON0;
	/* Others */
	adc_dbg_addr[i++] = MT6390_BATON_ANA_CON0;
}
...
static void mdrt_reg_dump(void)
{
	...
	pr_notice("AUXADC_ADC16 = 0x%x\n"
		  , upmu_get_reg_value(MT6390_AUXADC_ADC16));
	pr_notice("AUXADC_ADC17 = 0x%x\n"
		  , upmu_get_reg_value(MT6390_AUXADC_ADC17));
	pr_notice("AUXADC_ADC18 = 0x%x\n"
		  , upmu_get_reg_value(MT6390_AUXADC_ADC18));
	pr_notice("AUXADC_ADC36 = 0x%x\n"
		  , upmu_get_reg_value(MT6390_AUXADC_ADC36));
	pr_notice("AUXADC_MDRT_0 = 0x%x\n"
		  , upmu_get_reg_value(MT6390_AUXADC_MDRT_0));
	pr_notice("AUXADC_MDRT_1 = 0x%x\n"
		  , upmu_get_reg_value(MT6390_AUXADC_MDRT_1));
	pr_notice("AUXADC_MDRT_2 = 0x%x\n"
		  , upmu_get_reg_value(MT6390_AUXADC_MDRT_2));
	pr_notice("AUXADC_MDRT_3 = 0x%x\n"
		  , upmu_get_reg_value(MT6390_AUXADC_MDRT_3));
	pr_notice("AUXADC_MDRT_4 = 0x%x\n"
		  , upmu_get_reg_value(MT6390_AUXADC_MDRT_4));
	/*--AUXADC CLK--*/
	...
	struct device *dev, bool is_timeout, unsigned char ch_num)
{
	...
	dev_notice(dev, "(%d)Time out!STA0=0x%x,STA1=0x%x,STA2=0x%x\n",
		ch_num,
		upmu_get_reg_value(MT6390_AUXADC_STA0),
		upmu_get_reg_value(MT6390_AUXADC_STA1),
		upmu_get_reg_value(MT6390_AUXADC_STA2));
	dev_notice(dev, "RST: Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
		MT6390_STRUP_CON6,
		upmu_get_reg_value(MT6390_STRUP_CON6),
		MT6390_HK_TOP_RST_CON0,
		upmu_get_reg_value(MT6390_HK_TOP_RST_CON0));
	dev_notice(dev, "CLK: Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
		MT6390_HK_TOP_CLK_CON0,
		upmu_get_reg_value(MT6390_HK_TOP_CLK_CON0),
		MT6390_HK_TOP_CLK_CON1,
		upmu_get_reg_value(MT6390_HK_TOP_CLK_CON1));
}
