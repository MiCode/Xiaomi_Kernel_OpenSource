	...
	bool en)
{
	...
	for (i = 0; i < max_wait_times; i++) {
		...
		if (adc_vbus < 2500000) {		//vbus没电了
			dev_info(chg_data->dev,"%s: vbus is %d ,break \n", __func__, adc_vbus);
			break;
		}
		msleep(1000);
		ret = mt6370_pmu_reg_test_bit(chg_data->chip,
...
static int mt6370_dump_ibus(struct charger_device *chg_dev)
{
	int ibus = 0, ret = 0;
	struct mt6370_pmu_charger_data *chg_data = dev_get_drvdata(&chg_dev->dev);

	ret = mt6370_get_adc(chg_data, MT6370_ADC_IBUS, &ibus);
	dev_err(chg_data->dev,"ZLF:%s:ret = %d,ibus = %d ma",
			__func__, ret, ibus / 1000);
					...
					struct device_attribute *attr, char *buf)
{
	...
	ret = pmic_read_interface(0x14c , &reg_val, 0xFFFF, 0x0);
	if (ret != 0) {
			dev_notice(dev, "read pmic 0x14c fail\n");
			return ret;
	}
	dev_notice(dev, "reg_val = 0x%x \n", reg_val);
	reg_val = (reg_val >> 9) & 0x1;   //check pmic reg 0x14c bit 9 0:diasble 1:enable

	value = mt6370_pmu_reg_read(chg_data->chip,
					MT6370_PMU_REG_CHGPUMP);
	if (value < 0) {
		dev_notice(dev, "set reg chgpump fail\n");
		return ret;
	}
	dev_notice(dev, "value = 0x%x \n", value);
	value = (value >> 7) & 0x1;    //check charge ic reg 0x2a bit 7 0:enable 1:disable
					...
					const char *buf, size_t size)
{
	...
	if (0 == value){
		pmic_config_interface(0x14c,0x21, 0xFFFF, 0x0);
		...
		dev_notice(dev,
			"%s: disable kpdpwr reset \n", __func__);
	}else if (1 == value){
		pmic_config_interface(0x14c,0x221, 0xFFFF, 0x0);

		ret = mt6370_pmu_reg_write(chg_data->chip,
			MT6370_PMU_REG_CHGPUMP, 0x60);
		if (ret < 0) {
			dev_notice(dev, "set reg chgpump fail\n");
			return ret;
		}
		dev_notice(dev,
			"%s: enable kpdpwr reset \n", __func__);
	}
...
&& !defined(CONFIG_TCPC_CLASS)
	...
	ret = device_create_file(chg_data->dev, &dev_attr_kpdpwr_reset);
	if (ret < 0) {
		dev_notice(&pdev->dev, "create kpdpwr_reset attr fail\n");
		goto err_register_ls_dev;
	}
...
static int mt6370_pmu_charger_remove(struct platform_device *pdev)
{
	...
	if (chg_data) {
		...
		device_remove_file(chg_data->dev, &dev_attr_kpdpwr_reset);
