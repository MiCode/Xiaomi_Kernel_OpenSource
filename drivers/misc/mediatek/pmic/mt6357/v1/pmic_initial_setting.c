...
/* used for MT6739 E1/E2 PMIC clear interrupt */
...
int PMIC_POWER_HOLD(unsigned int hold)
{
	if (hold > 1) {
		pr_debug(
		    "[PMIC_KERNEL] %s hold = %d only 0 or 1\n", __func__,
		    hold);
		return -1;
	}
...
unsigned int PMIC_LP_CHIP_VER(void)
{
	...
	/* PMIC special flavor project */
	if (strncmp(CONFIG_BUILD_ARM_APPENDED_DTB_IMAGE_NAMES, "evb6739_lp",
		    10) == 0 ||
	    strncmp(CONFIG_BUILD_ARM_APPENDED_DTB_IMAGE_NAMES, "k39v1_lp", 8) ==
		0)
		return 2;
#endif
