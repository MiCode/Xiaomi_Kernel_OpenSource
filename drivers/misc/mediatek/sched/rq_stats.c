				...
				struct kobj_attribute *attr, char *buf)
{
	...
	for_each_possible_cpu(cpu) {
		sched_get_mcps(cpu, &hw_max_freq,
			&last_mcps, &inc_mcps, &inc_time);
	if (inc_time-lastmcps[cpu][3] != 0) {
		load_pct = (inc_mcps-lastmcps[cpu][2])*100;
		load_pct /= (inc_time-lastmcps[cpu][3])*hw_max_freq/1000000;
	} else
		load_pct = -1;
	...
	len += snprintf(buf+len, max_len-len, ",[%d,%u]",
					load_pct, last_mcps);
	}
	len += snprintf(buf+len, max_len-len, "\n");
	return len;
...
static struct attribute *rq_attrs[] = {
	&cpu_mcps_attr.attr,
	&cpu_mcps_sum_attr.attr,//xujia add for power analysis bugreport
