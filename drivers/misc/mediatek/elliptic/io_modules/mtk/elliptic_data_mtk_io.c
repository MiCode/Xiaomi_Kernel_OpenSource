	...
	int id, /*void *prdata,*/ void *data, unsigned int len)
{
    ...
    {
        pr_warn("[ELUS] Offset mismatch between SCP and AP kernel, current_ipi_counter: %u, target_ipi_message_count: %u!\n",
	...
	while( current_ipi_counter != target_ipi_message_count )
	{
		...
		{
			payload = header + 1;
			pr_info( "[ELUS] Got data via dram payload, counter: %u", current_ipi_counter );
		}
