...
struct elliptic_engine_data {
	union {
		uint8_t reserved[ELLIPTIC_MSG_BUF_SIZE];
		uint32_t u32[ELLIPTIC_MSG_BUF_SIZE >> 2];
	};
};

static struct elliptic_engine_data
	elliptic_engine_data_cache
		= { .reserved = {0} };
...
struct elliptic_engine_tag_info {
	char engine_tag[ELLIPTIC_TAG_INFO_MAX_SIZE];
};
...
static struct elliptic_shared_data_block shared_data_blocks[] = {
	...
	{ ELLIPTIC_OBJ_ID_TAG_INFO, ELLIPTIC_TAG_INFO_MAX_SIZE,
		&elliptic_engine_tag_cache },
	{ ELLIPTIC_OBJ_ID_ML_DATA,
		ELLIPTIC_ML_DATA_SIZE,
		&elliptic_engine_ml_data_cache },
	{ ELLIPTIC_OBJ_ID_ENGINE_DATA,
...
void elliptic_set_calibration_data(uint8_t *calib_data, size_t size)
{
	...
	if (calibration_obj == NULL) {
		pr_err("[ELUS] ell..set_calibration_data() calib=NULL (%zu)",
		       size);
		return;
	}
...
void elliptic_set_diagnostics_data(uint8_t *diag_data, size_t size)
{
	...
	if (size <= ELLIPTIC_DIAGNOSTICS_DATA_SIZE) {
		...
		if (diagnostics_obj == NULL) {
			pr_err("[ELUS] el..set_diagnostics_data() NULL (%zu)",
				size);
			return;
		}
...
int get_elliptic_calibration_data(uint8_t *caldata, uint32_t max_size)
{
	...
	if (max_size < ELLIPTIC_CALIBRATION_DATA_SIZE) {
		copied = max_size;
		pr_debug("[ELUS] size mismatch : %u vs %u",
					  ...
					  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ultrasound_tx_port_cache == ucontrol->value.integer.value[0]) {
		pr_debug("[ELUS] ultrasound_tx_port_set: ignoring duplicate request");
		return 0;
	}
	...
	pr_debug("[ELUS] ultrasound_tx_port: enable=%d ret=%d",
					  ...
					  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ultrasound_rx_port_cache == ucontrol->value.integer.value[0]) {
		pr_debug("[ELUS] ultrasound_rx_port_set: ignoring duplicate request");
		return 0;
	}
	...
	pr_debug("[ELUS] ultrasound_rx_port: enable=%d ret=%d",
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	case ELLIPTIC_CALIBRATION_STATE:
		...
		if(elliptic_system_configuration_cache.calibration_state == 1){
			elliptic_debug_io_open();
			pr_info("[ELUS] %s() elliptic_debug_io_open", __func__);
		}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (mc->shift >= ELLIPTIC_SYSTEM_CONFIGURATION_CUSTOM_SETTING_0 &&
		...
		pr_debug(
			"[ELUS] get ELLIPTIC_SYSTEM_CONFIGURATION_CUSTOM_SETTING_%02d",
			mc->shift -
	...
	case ELLIPTIC_SYSTEM_CONFIGURATION_INPUT_CHANNELS:
		...
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_LIB_ID:
		ucontrol->value.integer.value[0] =
			elliptic_system_configuration_cache.lib_id;
		break;
	default:
		pr_err("[ELUS] Invalid mixer control");
		return -EINVAL;
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (mc->shift >= ELLIPTIC_SYSTEM_CONFIGURATION_CUSTOM_SETTING_0 &&
		...
		const size_t csi =
			...
			ELLIPTIC_SYSTEM_CONFIGURATION_CUSTOM_SETTING_0;
		pr_debug(
			"[ELUS] ELLIPTIC_SYSTEM_CONFIGURATION_CUSTOM_SETTING_XX csi:%zu",
			csi);
		if (csi >= ARRAY_SIZE(elliptic_system_configuration_cache
					      .custom_settings))
			return -EINVAL;
		pr_debug("[ELUS] ucontrol->value.integer.value[0]:%ld",
			 ucontrol->value.integer.value[0]);
		...
		pr_debug(
			"[ELUS] calling elliptic_data_write(custom_setting) csi:%zu",
			csi);
		return elliptic_data_write(ELLIPTIC_ULTRASOUND_SET_PARAMS,
...
static const struct snd_kcontrol_new ultrasound_filter_mixer_controls[] = {
	...
	ELLIPTIC_SYSTEM_CONFIGURATION,
	ELLIPTIC_SYSTEM_CONFIGURATION_CUSTOM_SETTING_0,
	ELLIPTIC_SYSCONF_CUSTOM_SETTING_MAX_VALUE,
	0,
	elliptic_system_configuration_param_get,
	elliptic_system_configuration_param_put),
	...
	SOC_SINGLE_EXT("Ultrasound Library Id",
	ELLIPTIC_SYSTEM_CONFIGURATION,
	ELLIPTIC_SYSTEM_CONFIGURATION_LIB_ID,
	256,
	0,
	elliptic_system_configuration_param_get,
	elliptic_system_configuration_param_put),
...
unsigned int elliptic_add_platform_controls(void *component)
{
