...
static void dpidle_timer_wakeup_func(unsigned long data)
{
...

	{
		...
		if (__ratelimit(&ratelimit)) {
			os_printk(K_INFO,
				  "dpidle_timer<%p> alive, skip_cnt<%d>\n",
				  timer, skip_cnt);
			skip_cnt = 0;
		} else
			skip_cnt++;
...
bool u3_loop_back_test(void)
{
	...
	writel(readl(U3D_SSUSB_U3_CTRL_0P) &
	       ~(SSUSB_IP_PCIE_PDN | SSUSB_U3_PORT_PDN), U3D_SSUSB_U3_CTRL_0P);
...
void usb_phy_savecurrent(unsigned int clk_on)
{
 ...
 reg_done:
	...
	if (clk_on) {
		U3PhyWriteField32((phys_addr_t) (uintptr_t) U3D_USB30_PHYA_REG1,
				  RG_SSUSB_VUSB09_ON_OFST, RG_SSUSB_VUSB09_ON,
				  0);
...
void usb_phy_recover(unsigned int clk_on)
{
	...
	/* EFUSE related sequence */
	{
		...
		if (evalue) {
			os_printk(K_INFO,
				  "apply efuse setting, RG_SSUSB_IEXT_INTR_CTRL=0x%x\n",
				  evalue);
			U3PhyWriteField32((phys_addr_t) (uintptr_t)
					  U3D_USB30_PHYA_REG0, 10, (0x3f << 10),
					  evalue);
		} else
			os_printk(K_DEBUG, "!evalue\n");
