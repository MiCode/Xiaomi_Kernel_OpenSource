...
static u16 gs_pmic_read(u16 reg)
{
	...
	ret = regmap_read(gs_regmap, reg, &reg_val);
	return (u16)reg_val;
}
...
static void __iomem *_golden_io_phys_to_virt(unsigned int addr)
{
	...
	if (!_g.phy_base || _g.phy_base != base) {
		...
		_g.phy_base = base;
		_g.io_base =
			ioremap_wc(_g.phy_base, REMAP_SIZE_MASK+1);

		if (!_g.io_base)
			pr_info("warning: ioremap_nocache(0x%x, 0x%x)\n",
...
int mt_golden_setting_init(void)
{
...
EXPORT_SYMBOL(mt_golden_setting_init);
...
EXPORT_SYMBOL(_golden_read_reg);
		...
		for (i = 0; i < pmic_gs_len; i += 3) {
			...
			if (!_is_exist_in_phys_to_virt_table(base)) {
				...
				table[br.table_pos].va =
					ioremap_wc(base, REMAP_SIZE_MASK + 1);
