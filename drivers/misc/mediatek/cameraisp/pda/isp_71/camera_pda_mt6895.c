...
static int PDA_probe(struct platform_device *pdev)
{
	...
	// mraw node
	mraw_node = of_find_compatible_node(NULL, NULL, "mediatek,mt6895-camsys_mraw");
	if (!mraw_node) {
...
#ifdef PDA_TF_DUMP_71_3
	// PDA 0
	mtk_iommu_register_fault_callback(M4U_LARB25_PORT11,
			(mtk_iommu_fault_callback_t)pda_tfault_callback,
			NULL, false);
	mtk_iommu_register_fault_callback(M4U_LARB25_PORT12,
			(mtk_iommu_fault_callback_t)pda_tfault_callback,
			NULL, false);
	mtk_iommu_register_fault_callback(M4U_LARB25_PORT13,
			(mtk_iommu_fault_callback_t)pda_tfault_callback,
			NULL, false);
#endif
