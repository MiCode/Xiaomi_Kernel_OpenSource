...
unsigned int CQ_Recover(unsigned int IrqStatus,
	unsigned int ErrStatus, enum ISP_DEV_NODE_ENUM reg_module)
{
	if (ErrStatus & CQ_VS_ERR_ST) {
		unsigned int val;
		unsigned int  DmaEnStatus[_cam_max_];
		union FBC_CTRL_1 fbc_ctrl2[_cam_max_];

		val = ISP_RD32(CAM_REG_DCM_STATUS(reg_module));

		if ((val & 0x100) && (!(val & 0x80))) {
			/**/
			ISP_GetDmaPortsStatus(reg_module, DmaEnStatus);
			if (DmaEnStatus[_aao_]) {
				fbc_ctrl2[_aao_].Raw = ISP_RD32(
					CAM_REG_FBC_AAO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_aao_].Raw = 0;
			}

			if (DmaEnStatus[_afo_]) {
				fbc_ctrl2[_afo_].Raw = ISP_RD32(
					CAM_REG_FBC_AFO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_afo_].Raw = 0;
			}

			if (DmaEnStatus[_flko_]) {
				fbc_ctrl2[_flko_].Raw = ISP_RD32(
					CAM_REG_FBC_FLKO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_flko_].Raw = 0;
			}

			if (DmaEnStatus[_pdo_]) {
				fbc_ctrl2[_pdo_].Raw = ISP_RD32(
					CAM_REG_FBC_PDO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_pdo_].Raw = 0;
			}

			if (DmaEnStatus[_pso_]) {
				fbc_ctrl2[_pso_].Raw = ISP_RD32(
					CAM_REG_FBC_PSO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_pso_].Raw = 0;
			}

			if (DmaEnStatus[_imgo_]) {
				fbc_ctrl2[_imgo_].Raw = ISP_RD32(
					CAM_REG_FBC_IMGO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_imgo_].Raw = 0;
			}

			if (DmaEnStatus[_rrzo_]) {
				fbc_ctrl2[_rrzo_].Raw = ISP_RD32(
					CAM_REG_FBC_RRZO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_rrzo_].Raw = 0;
			}

			if (DmaEnStatus[_ufeo_]) {
				fbc_ctrl2[_ufeo_].Raw = ISP_RD32(
					CAM_REG_FBC_UFEO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_ufeo_].Raw = 0;
			}

			if (DmaEnStatus[_ufgo_]) {
				fbc_ctrl2[_ufgo_].Raw = ISP_RD32(
					CAM_REG_FBC_UFGO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_ufgo_].Raw = 0;
			}

			if (DmaEnStatus[_rsso_]) {
				fbc_ctrl2[_rsso_].Raw = ISP_RD32(
					CAM_REG_FBC_RSSO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_rsso_].Raw = 0;
			}

			if (DmaEnStatus[_lmvo_]) {
				fbc_ctrl2[_lmvo_].Raw = ISP_RD32(
					CAM_REG_FBC_LMVO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_lmvo_].Raw = 0;
			}

			if (DmaEnStatus[_lcso_]) {
				fbc_ctrl2[_lcso_].Raw = ISP_RD32(
					CAM_REG_FBC_LCSO_CTL2(reg_module));
			} else {
				fbc_ctrl2[_lcso_].Raw = 0;
			}
			/**/
			ISP_WR32(CAM_REG_CTL_SW_CTL(reg_module), 0x0);
			ISP_WR32(CAM_REG_CTL_SW_CTL(reg_module), 0x1);
			ISP_WR32(CAM_REG_TG_VF_CON(reg_module), 0x0);

			LOG_NOTICE(
				"start HW recover due to CQ over Vsync ...\n");
			LOG_NOTICE(
				"fbc:imgo:0x%x,rrzo:0x%x,ufeo:0x%x, ufgo:0x%x,rsso:0x%x,lmvo:0x%x,lcso:0x%x\n",
				fbc_ctrl2[_imgo_].Raw,
				fbc_ctrl2[_rrzo_].Raw,
				fbc_ctrl2[_ufeo_].Raw,
				fbc_ctrl2[_ufgo_].Raw,
				fbc_ctrl2[_rsso_].Raw,
				fbc_ctrl2[_lmvo_].Raw,
				fbc_ctrl2[_lcso_].Raw);
			LOG_NOTICE(
				"fbc:aao:0x%x,afo:0x%x,flko:0x%x,pdo:0x%x,pso:0x%x\n",
				fbc_ctrl2[_imgo_].Raw,
				fbc_ctrl2[_rrzo_].Raw,
				fbc_ctrl2[_ufeo_].Raw,
				fbc_ctrl2[_ufgo_].Raw,
			...
			/**/
			if (DmaEnStatus[_aao_]) {
				ISP_WR32(CAM_REG_FBC_AAO_CTL2(
					reg_module), fbc_ctrl2[_aao_].Raw);
			}

			if (DmaEnStatus[_afo_]) {
				ISP_WR32(CAM_REG_FBC_AFO_CTL2(
					reg_module), fbc_ctrl2[_afo_].Raw);
			}

			if (DmaEnStatus[_flko_]) {
				ISP_WR32(CAM_REG_FBC_FLKO_CTL2(
					reg_module), fbc_ctrl2[_flko_].Raw);
			}

			if (DmaEnStatus[_pdo_]) {
				ISP_WR32(CAM_REG_FBC_PDO_CTL2(
					reg_module), fbc_ctrl2[_pdo_].Raw);
			}

			if (DmaEnStatus[_pso_]) {
				ISP_WR32(CAM_REG_FBC_PSO_CTL2(
					reg_module), fbc_ctrl2[_pso_].Raw);
			}

			if (DmaEnStatus[_imgo_]) {
				ISP_WR32(CAM_REG_FBC_IMGO_CTL2(
					reg_module), fbc_ctrl2[_imgo_].Raw);
			}

			if (DmaEnStatus[_rrzo_]) {
				ISP_WR32(CAM_REG_FBC_RRZO_CTL2(
					reg_module), fbc_ctrl2[_rrzo_].Raw);
			}

			if (DmaEnStatus[_ufeo_]) {
				ISP_WR32(CAM_REG_FBC_UFEO_CTL2(
					reg_module), fbc_ctrl2[_ufeo_].Raw);
			}

			if (DmaEnStatus[_ufgo_]) {
				ISP_WR32(CAM_REG_FBC_UFGO_CTL2(
					reg_module), fbc_ctrl2[_ufgo_].Raw);
			}

			if (DmaEnStatus[_rsso_]) {
				ISP_WR32(CAM_REG_FBC_RSSO_CTL2(
					reg_module), fbc_ctrl2[_rsso_].Raw);
			}

			if (DmaEnStatus[_lmvo_]) {
				ISP_WR32(CAM_REG_FBC_LMVO_CTL2(
					reg_module), fbc_ctrl2[_lmvo_].Raw);
			}

			if (DmaEnStatus[_lcso_]) {
				ISP_WR32(CAM_REG_FBC_LCSO_CTL2(
					reg_module), fbc_ctrl2[_lcso_].Raw);
			}

			ISP_WR32(CAM_REG_TG_VF_CON(reg_module), 0x1);

			/**/
			IrqStatus = IrqStatus & ~(0x1001);
		} else {
			unsigned int val = ISP_RD32(
				CAM_REG_DCM_STATUS(reg_module));

			LOG_NOTICE("cqvs: cq over vsync dma_dcm:0x%x\n", val);
		}
	}
...
irqreturn_t ISP_Irq_CAM(enum ISP_IRQ_TYPE_ENUM irq_module)
{
	...
	#if Sylvia_WAM_CQ_ERR
	IrqStatus = CQ_Recover(IrqStatus, ErrStatus, (
		enum ISP_DEV_NODE_ENUM)reg_module);
	#endif
	if (IrqStatus & SOF_INT_ST) {
		unsigned int frmPeriod = ((ISP_RD32
