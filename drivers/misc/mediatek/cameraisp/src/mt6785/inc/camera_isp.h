...
struct ISP_IRQ_TIME_STRUCT {
	unsigned int tLastSig_sec; /* time stamp of the latest signal */
	unsigned int tLastSig_usec; /* time stamp of the latest signal */
	unsigned int tMark2WaitSig_sec; /* time period for user wait signal */
	unsigned int tMark2WaitSig_usec; /* time period for user wait signal */
	unsigned int tLastSig2GetSig_sec; /* time period for latest signal */
	unsigned int tLastSig2GetSig_usec; /* time period from latest signal */
...
struct ISP_WAIT_IRQ_ST {
	...
	unsigned int Status; /*ref. in isp_drv_stddef.h*/
...
enum _isp_dma_enum_ {
	...
	_pdi_,
	_cam_max_,
	_dmao_max_ = _bpci_, /* For user space, have the same number of dmao */
	_camsv_imgo_ = _imgo_,
	_camsv_max_,
};
...
struct ISP_RT_RING_BUF_INFO_STRUCT {
	...
	unsigned int total_count; /* total buffer number. */
	unsigned int empty_count; /* total empty buffer number */
...
enum ISP_CMD_ENUM {
	...
	ISP_CMD_GET_DMA_ERR,
	ISP_CMD_GET_INT_ERR,
	ISP_CMD_GET_DROP_FRAME, /* 1 for drop frmae, 2 for last working frame */
	ISP_CMD_WAKELOCK_CTRL,
	ISP_CMD_REGISTER_IRQ_USER_KEY, /* register user key for irq operation */
	ISP_CMD_MARK_IRQ_REQUEST, /* mark for a specific register before wait */
	ISP_CMD_GET_MARK2QUERY_TIME, /* query time between read and mark */
	...
	ISP_CMD_GET_SUPPORTED_ISP_CLOCKS, /* Get supported isp clocks */
