...
#define IRQ_LOG_KEEPER_ERR(irq,     ppb, logT, fmt, ...) do {\
		char *ptr;\
		char *pDes;\
		signed int avaLen;\
		unsigned int *ptr2 = &gSvLog[irq]._cnt[ppb][logT];\
...
/**/            /**/
	...
	/**/
					...
					if ((MTRUE ==
						...
						.active)) {
/* record wheather all enabled dma r already enqued, */
				      ...
				      unsigned long usec)
{
	...
	if (curr != _tmp)
		IRQ_LOG_KEEPER_ERR(irqT, m_CurrentPPB, _LOG_ERR,
...
/*get in the first     for     loop */
	...
	if (idx == -1)
		log_err("Could not find	match buffer tag(%d) pid/cid/p2dupCQidx(%lu/%lu/%d)",
...
static signed int ISP_ED_BufQue_CTRL_FUNC(struct ISP_ED_BUFQUE_STRUCT param)
{
	...
	case ISP_ED_BUFQUE_CTRL_WAIT_DEQUE:
		...
		spin_unlock(&(SpinLockEDBufQueList));
		if (idx == -1) {
			log_err("Can't find match buffer(pid/cid %lu/%lu)to deque!",
		...
		if (restTime == 0) {
			log_err("Wait Deque fail, idx(%d) pID(%lu),cID(%lu)",
	...
	case ISP_ED_BUFQUE_CTRL_WAIT_FRAME:
		...
		if (idx == -1) {
			spin_unlock(&(SpinLockEDBufQueList));
			log_err("could not find	match buffer pID/cID(%lu/%lu)",
				param.processID, param.callerID);
		...
		if (restTime == 0) {
			log_err("Dequeue Buffer	fail, rT(%d),idx(%d) pID(%lu),cID(%lu),p2SupportBNum(%d)\n",
...
static __tcmfunc irqreturn_t ISP_Irq_CAMSV(signed int Irq, void *DeviceId)
{
	...
	if (IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_INT_CAMSV] & IrqStatus_CAMSV)
		IRQ_LOG_KEEPER_ERR(_CAMSV_IRQ, m_CurrentPPB, _LOG_ERR,
