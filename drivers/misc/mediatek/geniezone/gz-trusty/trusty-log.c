...
static void trusty_dump_logs(struct trusty_log_state *s)
{
	...
	while ((put = log->put) != get) {
		/* Make sure that the read of put occurs before the read of
		 * log data
		 */
		rmb();
...
static bool trusty_supports_logging(struct device *device)
{
	...
	if (result == SM_ERR_UNDEFINED_SMC) {
		...
		pr_info("trusty std call (SMC_SC_SHARED_LOG_VERSION) failed: %d\n",
		       result);
		return false;
	}
...
static int trusty_log_probe(struct platform_device *pdev)
{
	...
	result = trusty_std_call32(s->trusty_dev,
				   ...
				   TRUSTY_LOG_SIZE);
	if (result < 0) {
		pr_info("trusty std call (SMC_SC_SHARED_LOG_ADD) failed: %d %pa\n",
		       result, &pa);
		goto error_std_call;
	}
