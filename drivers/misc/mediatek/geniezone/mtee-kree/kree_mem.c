				...
				uint64_t *ary)
{
	...
	KREE_DEBUG(
		"[%s] ====> share memory param encoding. numOfPA=%d, runLeng_arySize=%d\n",
				  ...
				  uint32_t cmd)
{
	...
	KREE_DEBUG("[%s] kree register share memory is calling.\n", __func__);
				 ...
				 KREE_SHAREDMEM_PARAM *param)
{
	...
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("KREE Register Shared Memory Error: 0x%x\n", ret);
		return ret;
	}
				   ...
				   KREE_SHAREDMEM_HANDLE shm_handle)
{
	...
	if (ret < 0) {
		KREE_ERR("KREE Unregister Shared Memory Error: 0x%x\n", ret);
		return ret;
	}
					...
					KREE_SHAREDMEM_HANDLE cm_handle)
{
	...
	if (ret < 0) {
		KREE_ERR("KREE Unregister Chunk Memory Error: 0x%x\n", ret);
		return ret;
	}
					...
					KREE_SHAREDMEM_PARAM *param)
{
	...
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("KREE Alloc Chunk Memory Error: 0x%x\n", ret);
		return ret;
	}
...
#ifdef DBG_KREE_MEM
	KREE_DEBUG("KREE Unreference Secure Memory: count = 0x%x\n", count);
#endif
					  ...
					  KREE_SHAREDMEM_HANDLE cm_handle)
{
	...
	ret = kree_release_chunkmem(session, cm_handle);
	if (ret < 0) {
		KREE_ERR("KREE Release Secure Multi-chunk Memory Error: 0x%x\n",
			ret);
		return ret;
	}
				      ...
				      KREE_ION_HANDLE IONHandle, uint32_t cmd)
{
	...
	if (p[1].value.a != 0) {
		KREE_DEBUG(
			"KREE ION Query Chunk Memory Test Fail: mem_handle=0x%x\n",
			mem_handle);
...
#ifdef DBG_KREE_MEM
	KREE_DEBUG("KREE Unreference Secure Chunk Memory: count = 0x%x\n",
		count);
#endif
				  ...
				  uint32_t size, void *buffer)
{
	...
	ret = KREE_TeeServiceCall(session, TZCMD_MEM_SECURECM_READ,
				  ...
				  p);
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("KREE Read Secure Chunk Memory Error: 0x%x\n", ret);
		return ret;
	}
				   ...
				   uint32_t size, void *buffer)
{
	...
	ret = KREE_TeeServiceCall(session, TZCMD_MEM_SECURECM_WRITE,
				  ...
				  p);
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("KREE Write Secure Chunk Memory Error: 0x%x\n", ret);
		return ret;
	}
					 ...
					 uint32_t *size)
{
	...
	ret = KREE_TeeServiceCall(session, TZCMD_MEM_SECURECM_RSIZE,
				  TZ_ParamTypes1(TZPT_VALUE_OUTPUT), p);
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("KREE Get Secure Chunk Release Size Error: 0x%x\n",
			ret);
		return ret;
	}
				      ...
				      unsigned long start_pa, uint32_t size)
{
	...
	if (ret != TZ_RESULT_SUCCESS) {
#ifdef DBG_KREE_MEM
		pr_debug("[kree] KREE Start Secure Chunk Memory Service Error: 0x%x\n",
			 ret);
#endif
		return ret;
	}
				     ...
				     unsigned long *cm_pa, uint32_t *size)
{
	...
	if (ret != TZ_RESULT_SUCCESS) {
#ifdef DBG_KREE_MEM
		pr_debug("[kree] KREE Stop Secure Chunk Memory Service Error: 0x%x\n",
			 ret);
#endif
		return ret;
	}
				   ...
				   unsigned long *cm_pa, uint32_t *size)
{
	...
	if (ret != TZ_RESULT_SUCCESS) {
#ifdef DBG_KREE_MEM
		pr_debug("[kree] KREE Query Secure Chunk Memory Error: 0x%x\n",
			ret);
#endif
		return ret;
	}
...
TZ_RESULT KREE_GetTEETotalSize(KREE_SESSION_HANDLE session, uint32_t *size)
{
	...
	ret = KREE_TeeServiceCall(session, TZCMD_MEM_TOTAL_SIZE,
				  TZ_ParamTypes1(TZPT_VALUE_OUTPUT), p);
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("KREE Get TEE Total Size Error: 0x%x\n", ret);
		return ret;
	}
