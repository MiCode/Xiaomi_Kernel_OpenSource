...
#include "ktd3136.h"
#include "lct_bkl_show.h"
//#include "lct_backlight_show.h"
...
static int ktd3137_bl_enable_channel(struct ktd3137_chip *chip)
{
	...
	if (pdata->channel == 0) {
		...
		ret = ktd3137_masked_write(chip->client, REG_PWM, 0x9B, 0x9B);
	}
...
static void ktd3137_pwm_mode_enable(struct ktd3137_chip *chip, bool en)
{
	...
	if (en) {
		...
		if (pdata->pwm_mode)
			pdata->pwm_mode = en;
		ktd3137_masked_write(chip->client, REG_PWM, 0x9B, 0x9B);
	}
...
static void ktd3137_backlight_init(struct ktd3137_chip *chip)
{
	...
	if (pdata->using_lsb) {
		ktd3137_masked_write(chip->client, REG_RATIO_LSB,
						0x07, pdata->default_brightness);
		ktd3137_masked_write(chip->client, REG_RATIO_MSB,
						0xff, pdata->default_brightness>>3);
	} else {
		ktd3137_masked_write(chip->client, REG_RATIO_LSB, 0x07,
			ktd3137_brightness_table_reg4[pdata->default_brightness]);
		ktd3137_masked_write(chip->client, REG_RATIO_MSB, 0xff,
			ktd3137_brightness_table_reg5[pdata->default_brightness]);
	}
	pdata->prev_bl_current = pdata->default_brightness;
...
int ktd_hbm_set(int hbm_mode)
{
	...
	case HBM_MODE_DEFAULT:
		//ktd3137_write_reg(bkl_chip->client, REG_MODE, 0x79);
        	ktd3137_write_reg(bkl_chip->client, 0x04, 0x07); 
        	ktd3137_write_reg(bkl_chip->client, 0x05, 0xDB);
		LOG_DBG("This is hbm mode 1\n");
		break;
	case HBM_MODE_LEVEL1:
		//ktd3137_write_reg(bkl_chip->client, REG_MODE, 0x81);
        	ktd3137_write_reg(bkl_chip->client, 0x04, 0x07); 
        	ktd3137_write_reg(bkl_chip->client, 0x05, 0xE9);
		LOG_DBG("This is hbm mode 2\n");
		break;
	case HBM_MODE_LEVEL2:
		//ktd3137_write_reg(bkl_chip->client, REG_MODE, 0x89);
            	ktd3137_write_reg(bkl_chip->client, 0x04, 0x07); 
        	ktd3137_write_reg(bkl_chip->client, 0x05, 0xFF);
	...
	case HBM_MODE_DEFAULT:
		ktd3137_write_reg(bkl_chip->client, 0x04, 0x07); 
        	ktd3137_write_reg(bkl_chip->client, 0x05, 0xD8);
		LOG_DBG("This is hbm mode 1\n");
		break;
	case HBM_MODE_LEVEL1:
		ktd3137_write_reg(bkl_chip->client, 0x04, 0x07); 
        	ktd3137_write_reg(bkl_chip->client, 0x05, 0xE3);
		LOG_DBG("This is hbm mode 2\n");
		break;
	case HBM_MODE_LEVEL2:
		ktd3137_write_reg(bkl_chip->client, 0x04, 0x07); 
		...
		break;
	//case HBM_MODE_LEVEL3:
	//	ktd3137_write_reg(bkl_chip->client, REG_MODE, 0xA9);
	//	LOG_DBG("This is hbm mode 3\n");
	//	break;	
	default:
		LOG_DBG("This isn't hbm mode\n");
...
int lm_hbm_set(int hbm_mode)
{
	...
	switch (hbm_mode) {
	case HBM_MODE_DEFAULT:
		i2c_sgm_write(bkl_chip->client,0x22, 0x07);
        	i2c_sgm_write(bkl_chip->client,0x23, 0xF9);
		pr_info("Turn off  hbm mode \n");
		break;
	case HBM_MODE_LEVEL1:
            	i2c_sgm_write(bkl_chip->client,0x22, 0x07);
        	i2c_sgm_write(bkl_chip->client,0x23, 0xFB);
		pr_info("This is hbm mode 1\n");
		break;
	case HBM_MODE_LEVEL2:
            	i2c_sgm_write(bkl_chip->client,0x22, 0x07);
                i2c_sgm_write(bkl_chip->client,0x23, 0xFF);
		pr_info("This is hbm mode 2\n");
		break;
	//case HBM_MODE_LEVEL3:
	//	i2c_sgm_write(bkl_chip->client,0x18, 0x1A);
	...
	case HBM_MODE_DEFAULT:
		i2c_sgm_write(bkl_chip->client,0x22, 0x07);
        	i2c_sgm_write(bkl_chip->client,0x23, 0xF8);
		pr_info("Turn off  hbm mode \n");
		break;
	case HBM_MODE_LEVEL1:
                i2c_sgm_write(bkl_chip->client,0x22, 0x07);
        	i2c_sgm_write(bkl_chip->client,0x23, 0xFA);
		pr_info("This is hbm mode 1\n");
		break;
	case HBM_MODE_LEVEL2:
                i2c_sgm_write(bkl_chip->client,0x22, 0x07);
		...
		break;
	//case HBM_MODE_LEVEL3:
	//	i2c_sgm_write(bkl_chip->client,0x18, 0x1B);
	//	pr_info("This is hbm mode 3\n");
	//	break;
	default:
		pr_info("This isn't hbm mode\n");
...
void ktd3137_brightness_set_workfunc(struct ktd3137_chip *chip, int brightness)
{
	...
	if (pdata->pwm_mode) {
		...
		printk("current brightness is --<%d>\n", brightness);
		if (pdata->using_lsb) {
			ktd3137_masked_write(chip->client, REG_RATIO_LSB,
							0x07, brightness);
			ktd3137_masked_write(chip->client, REG_RATIO_MSB,
	...
	//ktd3137_write_reg(chip->client, REG_CONTROL, 0x6e);
...
static ssize_t lm_bl_ctrl_reg_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct ktd3137_chip *chip = dev_get_drvdata(dev);
	uint8_t reg_val = 0;

	ktd3137_read_reg(chip->client, 0x18, &reg_val);
	return snprintf(buf, 1024, "0x%x\n", reg_val);
}

static ssize_t lm_bl_ctrl_reg_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	//struct ktd3137_chip *chip = dev_get_drvdata(dev);
	ssize_t ret;
	...
	i2c_sgm_write(bkl_chip->client,0x18, value);
...
//dump start  djy
static ssize_t ktd_i2c_reg_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct ktd3137_chip *chip = dev_get_drvdata(dev);
	ssize_t len = 0;
	...
	for (i = 0; i < 0x24; i++) {
		ktd3137_read_reg(chip->client, i, &reg_val);
...
static ssize_t ktd_i2c_reg_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
{
        struct ktd3137_chip *chip = dev_get_drvdata(dev);
...
//start
static ssize_t ktd3137_bl_current_reg_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	//struct ktd3137_chip *chip = dev_get_drvdata(dev);
	//uint8_t reg_val = 0;

	//ktd3137_read_reg(chip->client, HBM, &reg_val);
	return snprintf(buf, 1024, "0x%x\n", HBM);
}

static ssize_t ktd3137_bl_current_reg_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct ktd3137_chip *chip = dev_get_drvdata(dev);
	ssize_t ret;
	...
	ktd_hbm_set(HBM);
	lm_hbm_set(HBM);
	//ktd3137_write_reg(chip->client, HBM, value);

	return count;
}
//end
static DEVICE_ATTR(ktd_chip_id, 0444, ktd3137_bl_chip_id_show, NULL);
static DEVICE_ATTR(dump_reg, 0664, ktd_i2c_reg_show, ktd_i2c_reg_store);
...
static DEVICE_ATTR(lm_ctrl_reg, 0664, lm_bl_ctrl_reg_show,
				lm_bl_ctrl_reg_store);
...
static DEVICE_ATTR(c3j1_HBM, 0664, ktd3137_bl_current_reg_show,
				ktd3137_bl_current_reg_store);
...
int lct_ktd_bkl_show_callback(bool en)
{
	return 1;
}
...
static int ktd3137_probe(struct i2c_client *client,
	...
	pr_err("taocheng add:kt3137 probe start");
	client->addr = 0x36;
	LOG_DBG("probe start!\n");
	if (!pdata) {
	...
	if(strnstr(saved_command_line, "androidboot.read_bc_buf=0x7",
                strlen(saved_command_line))){
                printk("--------------[djy] 1 ---------------");
        ktd3137_write_reg(bkl_chip->client, REG_MODE, 0xB9);
        ktd3137_write_reg(bkl_chip->client, 0x08, 0x19);  
        ktd3137_write_reg(bkl_chip->client, 0x04, 0x07); 
        ktd3137_write_reg(bkl_chip->client, 0x05, 0xDB);
          
        i2c_sgm_write(bkl_chip->client,0x18, 0x18);
        i2c_sgm_write(bkl_chip->client,0x13, 0x33);
        i2c_sgm_write(bkl_chip->client,0x22, 0x07);
        i2c_sgm_write(bkl_chip->client,0x23, 0xF9);
                }else{
                        printk("--------------[djy] 2----------------");
        ktd3137_write_reg(bkl_chip->client, REG_MODE, 0xC1);
        ktd3137_write_reg(bkl_chip->client, 0x08, 0x19); 
        ktd3137_write_reg(bkl_chip->client, 0x04, 0x07); 
        ktd3137_write_reg(bkl_chip->client, 0x05, 0xD8);
          
        i2c_sgm_write(bkl_chip->client,0x18, 0x1C);
        i2c_sgm_write(bkl_chip->client,0x13, 0x33);
        i2c_sgm_write(bkl_chip->client,0x22, 0x07);
        i2c_sgm_write(bkl_chip->client,0x23, 0xF8);
          
                }     
	ret = init_lct_bkl_show(lct_ktd_bkl_show_callback);
	if (ret < 0) {
		pr_err("init_lct_bkl_show Failed!");
	} else {
		LOG_DBG("init_lct_bkl_show Succeeded!");
	}

	pr_err("taocheng add:kt3137 probe end");
exit0:
	return err;
}
