...
int bl_mapping_table1[2048] = {0, 405, 405, 424, 449, 474, 494, 513, 533, 550, 567, 581, 597, 611, 622, 635, 646, 658, 669, 679, 690, 700, 709,\
												...
												1010, 1014, 1018, 1022, 1026, 1030, 1034, 1038, 1042, 1050, 1054, 1058, 1062, 1066, 1068, 1072, 1076, 1078, 1082, 1086, 1090,\
												1094, 1098, 1102, 1106, 1110, 1114, 1118, 1122, 1126, 1130, 1134, 1138, 1142, 1146, 1150, 1154, 1158, 1162, 1166, 1170, 1174,\
												1178, 1182, 1185, 1188, 1191, 1194, 1197, 1200, 1203, 1206, 1209, 1212, 1215, 1217, 1220, 1223, 1225, 1228, 1231, 1234, 1237, 1240,\
...
int bl_mapping_table2[2048] = {0, 442, 442, 457, 467, 484, 500, 516, 533, 550, 567, 581, 597, 611, 622, 635, 646, 658, 669, 679, 690, 700, 709,\
												719, 727, 734, 744, 751, 759, 766, 773, 782, 790, 797, 803, 808, 815, 822, 828, 834, 841, 846, 852, 857, 864, 869, 874, 879, \
												884, 890, 896, 901, 907, 912, 917, 922, 927, 932, 937, 942, 947, 952, 957, 962, 966, 971,  976, 981, 986, 991, 996, 1002, 1006, \
												1010, 1014, 1018, 1022, 1026, 1030, 1034, 1038, 1042, 1050, 1054, 1058, 1062, 1066, 1068, 1072, 1076, 1078, 1082, 1086, 1090,\
												1094, 1098, 1102, 1106, 1110, 1114, 1118, 1122, 1126, 1130, 1134, 1138, 1142, 1146, 1150, 1154, 1158, 1162, 1166, 1170, 1174,\
...
int ti_hbm_set(enum backlight_hbm_mode hbm_mode)
{
	...
	pr_err("[bkl] %s enter\n", __func__);
	if (strnstr(saved_command_line, LCD_NAME_1, strlen(saved_command_line)) != NULL) { //一供
		pr_err("[bkl] %s hbm set for %s\n", __func__, LCD_NAME_1);
		switch (hbm_mode) {
		case HBM_MODE_DEFAULT:
			regmap_write(regmap, 0x18, 0x14);//21mA
			pr_err("This is hbm mode 0\n");
			break;
		case HBM_MODE_LEVEL1:
			regmap_write(regmap, 0x18, 0x16);//22.6mA
						       ...
						       int brightness)
{
	...
	if (cfg->max_brightness == MAX_BRIGHTNESS_11BIT) {
		...
		pr_info("%s: LM3697_backlight before_mapping_level: %d, after_mapping_level: %d\n", __func__, bl_level, brightness);
...
static int ti_lmu_backlight_init(struct ti_lmu_bl_chip *chip)
{
	...
	ret = regmap_write(regmap, 0x10, 0x03);
	pr_debug("[bkl] %s line %d: ret of regmap_write = %d\n", __func__, __LINE__, ret);

	ret = regmap_write(regmap, 0x13, 0x11);
	pr_debug("[bkl] %s line %d: ret of regmap_write = %d\n", __func__, __LINE__, ret);

	ret = regmap_write(regmap, 0x16, 0x00);
	pr_debug("[bkl] %s line %d: ret of regmap_write = %d\n", __func__, __LINE__, ret);

	ret = regmap_write(regmap, 0x19, 0x03);
	pr_debug("[bkl] %s line %d: ret of regmap_write = %d\n", __func__, __LINE__, ret);

	if (strnstr(saved_command_line, LCD_NAME_1, strlen(saved_command_line)) != NULL) { //一供
	    lcd_id = 1; /* 1 is the first lcd id*/
	    ret = regmap_write(regmap, 0x18, 0x1A); //25.8 ma
	} else if (strnstr(saved_command_line, LCD_NAME_2, strlen(saved_command_line)) != NULL) { //二供
	    lcd_id = 0; /* 0 is the second lcd id*/
	    ret = regmap_write(regmap, 0x18, 0x19); //25.0 ma
	}
	pr_debug("[bkl] %s line %d: ret of regmap_write = %d\n", __func__, __LINE__, ret);

	ret = regmap_write(regmap, 0x1A, 0x0E);
	pr_debug("[bkl] %s line %d: ret of regmap_write = %d\n", __func__, __LINE__, ret);

	ret = regmap_write(regmap, 0x1C, 0x0E); /* enable pwm */
	pr_debug("[bkl] %s line %d: ret of regmap_write = %d\n", __func__, __LINE__, ret);

	//ret = regmap_write(regmap, 0x22, 0x07);
	//pr_debug("[bkl] %s line %d: ret of regmap_write = %d\n", __func__, __LINE__, ret);

	//ret = regmap_write(regmap, 0x23, 0xFF);
	//pr_debug("[bkl] %s line %d: ret of regmap_write = %d\n", __func__, __LINE__, ret);

	ret = regmap_write(regmap, 0x24, 0x02);
	pr_debug("[bkl] %s line %d: ret of regmap_write = %d\n", __func__, __LINE__, ret);
