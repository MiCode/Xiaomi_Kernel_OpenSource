 ...
 ****************************************************************************************
 */
#define TP_WORK_NAME          "brightness_show"
#define TP_WORK_LOG_ENABLE
#define TP_WORK_TAG           "LCT_BKL_SHOW"
...
int init_lct_bkl_show(tp_work_cb_t callback)
{
	if (NULL == callback) {
...
EXPORT_SYMBOL(init_lct_bkl_show);

void uninit_lct_bkl_show(void)
{
...
EXPORT_SYMBOL(uninit_lct_bkl_show);

void set_lct_bkl_show_status(bool en)
{
	lct_tp_p->enable_tp_work_flag = en;
}
EXPORT_SYMBOL(set_lct_bkl_show_status);

bool get_lct_bkl_show_status(void)
{
	return lct_tp_p->enable_tp_work_flag;
}
EXPORT_SYMBOL(get_lct_bkl_show_status);
...
static int lct_creat_proc_tp_entry(void)
{
	...
	if (IS_ERR_OR_NULL(lct_tp_p->proc_entry_tp)) {
		TP_LOGE("add /proc/brightness_show error \n");
		return -1;
	}
	TP_LOGW("/proc/brightness_show is okay!\n");
