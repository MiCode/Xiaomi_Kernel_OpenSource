...
static int ocp8132_flash_test(void)  //FIXME: 0=152 ENM;  1=153 ENF
{
	//int pin = OCP8132_PINCTRL_PIN_HWEN;
	int pin_flash = OCP8132_PINCTRL_PIN_FLASH;
	PK_DBG("ocp8132_flash_test %d\n");

	/* flash mode */
	ocp8132_pinctrl_set(pin_flash, OCP8132_PINCTRL_PINSTATE_HIGH);
	msleep(400);
	ocp8132_pinctrl_set(pin_flash, OCP8132_PINCTRL_PINSTATE_LOW);
	msleep(3600);
	return 0;
}
					...
					const char *buf, size_t count)
{
	...
	if ((strcmp ("0", buf) == 0) || (strcmp ("0\x0a", buf) == 0)) {
		...
		ocp8132_disable();
		ocp8132_set_driver(0);
	} else if ((strcmp ("flash_test", buf) == 0) || (strcmp ("flash_test\x0a", buf) == 0)) {
		ocp8132_set_driver(1);
		ocp8132_flash_test();
	} else {
