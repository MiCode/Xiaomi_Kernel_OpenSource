...
static unsigned int flash_enable = 0;
/* define usage count */
		...
		enum led_brightness value)
{
	...
	if (LED_OFF == value) {
	...
	} else if((value > 0) && (value <= 255) && (flash_enable < 1)){
	    if(value > 100 && value < 132) {
            value = value - 100;
            if(value > 31){
                value = 31;
            }
            ...
            arg.level = 4; //torch current 125ma
        }
		//torch mode
		flash_is_use = 1;
        mt6370_set_driver(1);
        ...
        //mt6370_operate(MT6370_CHANNEL_CH2, MT6370_DISABLE);
        mt6370_set_level(arg.channel, arg.level);
        mt6370_set_level(MT6370_CHANNEL_CH2,0);//close channel 2
        mt6370_timeout_ms[arg.channel] = 0;
        //mt6370_timeout_ms[MT6370_CHANNEL_CH2] = 0;
		mt6370_operate(arg.channel, MT6370_ENABLE);
        //mt6370_operate(MT6370_CHANNEL_CH2, MT6370_ENABLE);
	}else {
        pr_err("invalid brightness value %d or flashlight enabled %d",value,flash_enable);
    }
