	...
	size_t count, loff_t *ppos)
{
	...
	h_l_period = DIV_ROUND_CLOSEST(IRTX_PWM_CLOCK, pwm_ir.carrier*3);
	...
	pr_info("%s() irtx config:%d, period:%d, count:%zu, total:%d\n",
		__func__, ret, h_l_period, count, total_time);
#ifdef IRTX_DEBUG
	pr_info("%s() irtx pwm buf size = %d\n", __func__, buf_size);
	for (i = 0; i < buf_size; i++) {
		if (i && i % 16 == 0)
			pr_err("\n");
		pr_err("[%d]0x%x, ", i, buf_ptr[i]);
	}
	pr_err("\n");
#endif
		...
		unsigned long arg)
{
	...
	case IRTX_IOC_SET_CARRIER_FREQ:
		ret = copy_from_user(&para, (void __user *)arg,
			sizeof(unsigned int));
		if (ret) {
			pr_notice("%s() IRTX_IOC_SET_CARRIER_FREQ fail\n",
				__func__);
			ret = -EFAULT;
		} else {
			pwm_ir.carrier = para;
			pr_err("irtx carrier freq = %d\n", pwm_ir.carrier);
		}
		break;
	default:
