...
static long ccu_ioctl(struct file *flip, unsigned int cmd, unsigned long arg)
{
	...
	case CCU_IOCTL_GET_IOVA:
	{
		...
		if (IS_ERR(buf)) {
			LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] dma_buf_get failed, ret=%lx fd=%d\n", buf, fd);
			mutex_unlock(&g_ccu_device->dev_mutex);
			return -EFAULT;
		}
		LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] dma_buf_get fd=%d va=%lx\n", fd, buf);
		...
		if (IS_ERR(ccu_iova[iova_buf_count].attach)) {
			LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] dma_buf_attach failed, attach=%lx dmabuf=%lx\n",
			 ccu_iova[iova_buf_count].attach, ccu_iova[iova_buf_count].dma_buf);
			goto err_attach;
		}
		LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] dma_buf_attach va=%lx dmabuf=%lx\n",
			ccu_iova[iova_buf_count].attach, ccu_iova[iova_buf_count].attach->dmabuf);
		...
		if (IS_ERR(ccu_iova[iova_buf_count].sgt)) {
			LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] map_attachment failed, attach=%lx sgt=%lx opt=%d\n",
			 ccu_iova[iova_buf_count].attach, ccu_iova[iova_buf_count].sgt,
			 DMA_BIDIRECTIONAL);
			goto err_map;
		}
		LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] dma_buf_map_attachment sgt=%lx\n",
			ccu_iova[iova_buf_count].sgt);
		...
		iova_buf_count++;
		LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] sg_dma_address, ret=%lx\n", dma_addr);
