...
static int ccu_i2c_probe_main2(struct i2c_client *client, const struct i2c_device_id *id);
...
static int i2c_query_dma_buffer_addr(struct ccu_device_s *g_ccu_device, enum CCU_I2C_CHANNEL i2c_controller_id,
...
static int ccu_i2c_probe_main2(struct i2c_client *client, const struct i2c_device_id *id)
{
	LOG_DBG_MUST("[ccu_i2c_probe] Attach I2C for HW trriger g_ccuI2cClientMain2 %p\n", client);
...
int ccu_i2c_controller_uninit_all(void)
{
	...
	LOG_INF_MUST("ccu_i2c_controller_uninit_all done.\n");
 ...
 struct ccu_i2c_buf_mva_ioarg *ioarg)
{
	...
	ret = i2c_query_dma_buffer_addr(g_ccu_device, ioarg->i2c_controller_id,
...
void ccu_i2c_dump_errr(void)
{
	...
	for (i = CCU_I2C_CHANNEL_MIN ; i < CCU_I2C_CHANNEL_MAX ; i++) {
		LOG_INF_MUST("CCU Dump I2C controller[%d] reg ==============================\n", i);
	...
	, uint32_t *mva, uint32_t *va_h, uint32_t *va_l, uint32_t *i2c_id)
{
	...
	if (g_ccu_device->i2c_dma_mva == 0)	{
		ret = ccu_allocate_mva(&g_ccu_device->i2c_dma_mva, g_ccu_device->i2c_dma_vaddr, &i2c_buffer_handle, CCU_I2C_DMA_BUF_SIZE);
		if (ret != 0) {
...
static int ccu_i2c_controller_en(enum CCU_I2C_CHANNEL i2c_controller_id, int enable)
{
	...
	LOG_INF_MUST("ccu_i2c_controller_en, id(%d):(%d)->(%d)\n",
		 i2c_controller_id, ccu_i2c_initialized[i2c_controller_id], enable);

	pClient = get_ccu_i2c_client(i2c_controller_id);
	LOG_DBG("ccu_i2c_controller_en, pClient: %p\n", pClient);
...
static int ccu_i2c_controller_uninit(enum CCU_I2C_CHANNEL i2c_controller_id)
{
	...
	LOG_DBG_MUST("ccu_i2c_controller_uninit done: id(%d)\n", i2c_controller_id);
