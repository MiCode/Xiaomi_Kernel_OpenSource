...
int ccu_ipc_send(struct ccu_msg *msg)
{
	...
	LOG_DBG("+, tg(%d), msg_id(%d)", msg->tg_info, msg->msg_id);
	...
	ccu_write_reg_bit(ccu_ipc.ccu_base, CTL_CCU_INT,
		INT_CTL_CCU, 1);
	...
	if (timeout) {
		...
		LOG_ERR("tg(%d), msg_id(%d), ackValue(%d), loopCount(%d)",
			msg->tg_info, msg->msg_id, ackValue, loopCount);
		return -1;
	}

	LOG_DBG("-, tg(%d), msg_id(%d), loopCount(%d)",
		msg->tg_info, msg->msg_id, loopCount);
	return 0;
}
	...
	uint32_t inDataSize, void *outDataPtr, uint32_t outDataSize)
{
	...
	struct ccu_msg msg = {msgId, inDataBufInfo.addr_ccu,
		outDataBufInfo.addr_ccu, tg_info};
	uint32_t ret;

	mutex_lock(&ccu_ipc_mutex);

	LOG_DBG("[%s]+: tg(%d), msgId(%d)\n",
		 __func__, tg_info, msgId);
