 ...
 */
int ccu_push_command_to_queue(
	struct ccu_user_s *user, struct ccu_cmd_s *cmd);
 ...
 */
int ccu_pop_command_from_queue(
	struct ccu_user_s *user, struct ccu_cmd_s **rcmd);
...
#define LOG_WRN_MUST(format, args...) \
...
#define ccu_error(format, args...) \
		do {\
			pr##_##err(CCU_TAG " error:"format, ##args);  \
			aee_kernel_exception("CCU", "[CCU] error:" \
			format, ##args);  \
		} while (0)
