...
struct CCU_REG_IO_STRUCT {
	struct CCU_REG_STRUCT *pData;	/* pointer to CCU_REG_STRUCT */
	unsigned int Count;	/* count */
};

struct CCU_IRQ_INFO_STRUCT {
	unsigned int
		Status[CCU_IRQ_TYPE_AMOUNT]
	...
	unsigned int
		MarkedTime_sec[CCU_IRQ_TYPE_AMOUNT][32][IRQ_USER_NUM_MAX];
	/* time for marking a specific interrupt */
	unsigned int
		MarkedTime_usec[CCU_IRQ_TYPE_AMOUNT][32][IRQ_USER_NUM_MAX];
	/* number of a specific signal that passed by */
	signed int
		PassedBySigCnt[CCU_IRQ_TYPE_AMOUNT][32][IRQ_USER_NUM_MAX];
	/* */
