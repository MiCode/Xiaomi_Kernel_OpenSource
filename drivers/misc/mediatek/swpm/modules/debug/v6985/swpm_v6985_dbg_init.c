...
static unsigned int swpm_pmsr_res_dbg_en;
static ssize_t swpm_pmsr_res_dbg_en_read(char *ToUser, size_t sz, void *priv)
{
	char *p = ToUser;
	...
	swpm_dbg_log("%d\n", swpm_pmsr_res_dbg_en);

	return p - ToUser;
}

static ssize_t swpm_pmsr_res_dbg_en_write(char *FromUser, size_t sz, void *priv)
{
	unsigned int enable = 0;
	...
	if (!kstrtouint(FromUser, 0, &enable)) {
		swpm_pmsr_res_dbg_en = !!enable;
		swpm_set_only_cmd(0, swpm_pmsr_res_dbg_en,
				  PMSR_RES_DBG_EN, PMSR_CMD_TYPE);
	}

	return sz;
}

static const struct mtk_swpm_sysfs_op swpm_pmsr_res_dbg_en_fops = {
	.fs_read = swpm_pmsr_res_dbg_en_read,
	.fs_write = swpm_pmsr_res_dbg_en_write,
};
...
static void swpm_v6985_dbg_fs_init(void)
{
	...
	mtk_swpm_sysfs_entry_func_node_add("swpm_pmsr_res_dbg_en"
			, 0644, &swpm_pmsr_res_dbg_en_fops, NULL, NULL);
#if SWPM_EXT_DBG
