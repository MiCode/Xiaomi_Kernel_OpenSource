...
#ifndef __SWPM_V6886_H__
#define __SWPM_V6886_H__

#include <swpm_gpu_v6886.h>
#include <swpm_isp_v6886.h>
#include <swpm_me_v6886.h>

#define SWPM_TEST (0)
#define SWPM_DEPRECATED (0)

#define MAX_RECORD_CNT				(64)
/* Todo: Check the ddrphy vcore power data numbers */
#define MAX_APHY_CORE_PWR			(12)
/* Todo: Check the ddrphy others power data numbers */
#define MAX_APHY_OTHERS_PWR			(16)

#define DEFAULT_AVG_WINDOW		(50)

/* VPROC3 + VPROC2 + VPROC1 + VDRAM + VGPU + VCORE */
/* Todo: decide CPUB is vproc2 or vproc3 */
#define DEFAULT_LOG_MASK			(0x3F)

#define POWER_CHAR_SIZE				(256)
#define POWER_INDEX_CHAR_SIZE			(4096)

/* ddrphy vcore domain voltage numbers for each state */
#define NR_CORE_VOLT				(5)
/* LP-Dram table number */
...
/* Todo: check the power rail v.s IP?? */
enum power_rail {
	VPROC2,
	VPROC1,
	VGPU,
...
/* Todo: Already fix but double check again */
enum ddr_freq {
	DDR_400,
	DDR_800,
	DDR_933,
...
/* Todo: why cannot define in pmu_idx enum ? */
enum dsu_pmu_idx {
	DSU_PMU_IDX_CYCLES,
...
extern int swpm_v6886_init(void);
extern void swpm_v6886_exit(void);
