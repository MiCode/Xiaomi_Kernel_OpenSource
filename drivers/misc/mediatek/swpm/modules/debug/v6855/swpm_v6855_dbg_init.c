...
static void swpm_v6855_dbg_fs_init(void)
{
...
static int __init swpm_v6855_dbg_early_initcall(void)
{
	return 0;
}
#ifndef MTK_SWPM_KERNEL_MODULE
subsys_initcall(swpm_v6855_dbg_early_initcall);
#endif

static int __init swpm_v6855_dbg_device_initcall(void)
{
	return 0;
}

static int __init swpm_v6855_dbg_late_initcall(void)
{
	/*
	...
	swpm_v6855_init();
	swpm_v6855_ext_init();
	swpm_v6855_dbg_fs_init();
	pr_notice("swpm init success\n");

	return 0;
}
#ifndef MTK_SWPM_KERNEL_MODULE
late_initcall_sync(swpm_v6855_dbg_late_initcall);
#endif

int __init swpm_v6855_dbg_init(void)
{
	int ret = 0;
#ifdef MTK_SWPM_KERNEL_MODULE
	ret = swpm_v6855_dbg_early_initcall();
#endif
	if (ret)
		goto swpm_v6855_dbg_init_fail;

	ret = swpm_v6855_dbg_device_initcall();

	if (ret)
		goto swpm_v6855_dbg_init_fail;

#ifdef MTK_SWPM_KERNEL_MODULE
	ret = swpm_v6855_dbg_late_initcall();
#endif
	...
	return 0;
swpm_v6855_dbg_init_fail:
	return -EAGAIN;
}

void __exit swpm_v6855_dbg_exit(void)
{
	swpm_v6855_exit();
	swpm_v6855_ext_exit();
}

module_init(swpm_v6855_dbg_init);
module_exit(swpm_v6855_dbg_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("v6855 software power model debug module");
