...
#define ACCDET_INIT1_ONCE 1
/* config analog, enable accdet eint, top interrupt...  */
...
#define EINT_PIN_MOISTURE_DETECTED -1
...
#if defined(CONFIG_MOISTURE_EXTERNAL_SUPPORT) ||                               \
	defined(CONFIG_MOISTURE_INTERNAL_SUPPORT)
...
static bool accdet_timeout_ns(u64 start_time_ns, u64 timeout_time_ns)
{
	...
	if (cur_time < start_time_ns) {
		pr_info("[accdet]start_time:%lld,cur_time:%lld\n",
			    start_time_ns, cur_time);
		start_time_ns = cur_time;
		timeout_time_ns = 400 * 1000; /* 400us */
		pr_info("[accdet]time: start:%lld,set:%lld\n",
	...
	if (timeout_time_ns <= elapse_time) { /* timeout */
		pr_info("[%s]ACCDET IRQ clear Timeout\n", __func__);
		return false;
	}
	return true;
}
...
static int accdet_set_debounce(int state, unsigned int debounce)
{
	...
	case accdet_auxadc: /* set auxadc debounce:0x42(2ms) */
		...
		break;
	default:
		pr_info("[accdet]error: no state:%d,deb:%d\n", state,
			    debounce);
		break;
	}

	/* pr_info("[%s]state:%d,debounce:%d,ret:%d\n", __func__, state,
	 * debounce, ret);
	 */
	return ret;
}
...
static int accdet_get_dts_data(void)
{
	...
	defined(CONFIG_MOISTURE_INTERNAL_SUPPORT)
		...
		pr_info("[%s]mid-Key = %d, voice = %d, up_key = %d, down_key = %d\n",
			__func__,
		...
		pr_info("[%s]mid-Key = %d, up_key = %d, down_key = %d\n",
			__func__,
			    headset_dts_data.three_key.mid_key,
		...
		pr_info("[%s]pwm_width=0x%x, pwm_thresh=0x%x, mic_mode=%d, mic_vol=%d\n",
			    __func__, accdet_cust_setting->pwm_width,
			    accdet_cust_setting->pwm_thresh,
			    headset_dts_data.accdet_mic_mode,
			    headset_dts_data.mic_bias_vol);
		pr_info("[%s]deb0=0x%x,deb1=0x%x,deb3=0x%x,deb4=0x%x\n",
		...
		pr_info("%s can't find compatible dts node\n", __func__);
		return -1;
	}
	return 0;
}
...
static void accdet_pmic_Read_Efuse_HPOffset(void)
{
	...
	if (s_4_key_efuse_flag) {
		...
		headset_dts_data.four_key.voice_key_four = tmp_8bit
							   << 2; /* DB */
		...
		headset_dts_data.four_key.down_key_four =
		    600; /* the max voltage */
		pr_info("[accdet]key thresh: mid=%d mv,voice=%d mv,up=%d mv,down=%d mv\n",
			    headset_dts_data.four_key.mid_key_four,
			    headset_dts_data.four_key.voice_key_four,
			    headset_dts_data.four_key.up_key_four,
			    headset_dts_data.four_key.down_key_four);
	...
	pr_info(" efusevalue = 0x%x, accdet_auxadc_offset = %d\n",
	...
	pr_info(
	    "[moisture_vdd_efuse]efuse=0x%x, moisture_vdd_offset=%d mv\n",
	...
	pr_info("[moisture_efuse]efuse=0x%x,moisture_offset=%d mv\n",
	...
	pr_info("[moisture_eint0]efuse=0x%x,moisture_eint0=0x%x\n",
		    efusevalue, moisture_eint0);

	efusevalue = pmic_Read_Efuse_HPOffset(85);
	moisture_eint1 = (int)(efusevalue & ACCDET_CALI_MASK0);
	pr_info("[moisture_eint1]efuse=0x%x,moisture_eint1=0x%x\n",
	...
	pr_info("[moisture_eint_efuse]moisture_eint_offset=%d ohm\n",
		    g_moisture_eint_offset);
	g_moisture_vm = (2800 + g_moisture_vdd_offset) *
			    (water_r + internal_r) /
			    ((water_r + internal_r) +
			     (8 * g_moisture_eint_offset) + 450000) +
			g_moisture_offset / 2;
	pr_info("[moisture_vm]moisture_vm=%d mv\n", g_moisture_vm);
#endif
#ifdef CONFIG_MOISTURE_EXTERNAL_SUPPORT
	g_moisture_vm =
	    (2800 + g_moisture_vdd_offset) * water_r / (water_r + external_r) +
	    g_moisture_offset / 2;
	pr_info("[moisture_vm]moisture_vm=%d mv\n", g_moisture_vm);
#endif
}
...
static int Accdet_PMIC_IMM_GetOneChannelValue(int key_check)
{
	...
	if (key_check) {
		if (vol_val > g_accdet_auxadc_offset)
			...
			vol_val = 0;
		pr_debug("[accdet] adc_offset=%d mv,MIC_Vol=%d mv\n",
...
{
	/* 0.24V ~  */
	/* pr_debug("[key_check] 3-keys enter:%d!!\n", b); */
...
static void send_accdet_status_event(int cable, int status)
{
	...
	case HEADSET_NO_MIC:
		input_report_switch(kpd_accdet_dev, SW_HEADPHONE_INSERT,
				    status);
		/* when plug 4-pole out, if both AB=3 AB=0 happen,3-pole plug in
		 * will be incorrectly reported, then 3-pole plug-out is
		...
		pr_debug("HEADPHONE(3-pole) %s\n",
			     status ? "PlugIn" : "PlugOut");
		break;
	case HEADSET_MIC:
		/* when plug 4-pole out, 3-pole plug out should also be reported
...
static void multi_key_detection(int current_status)
{
	...
	if (current_status == ACCDET_STATE_ABC_00) {
		...
		pr_debug("[Accdet]adc cali_voltage1 = %d mv\n",
			     cali_voltage);
	...
	if (((reg_val & ACCDET_EINT_IRQ_B2_B3) != ACCDET_EINT_IRQ_B2_B3) ||
		...
		pr_info("[accdet]plug out side effect,not report key=%d\n",
			     g_cur_key);
		g_cur_key = NO_KEY;
	}
#endif
...
static inline void check_cable_type(void)
{
	...
	pr_debug("[accdet]addr:[0x%x]=0x%x, cur_ABC=0x%x\n", ACCDET_CON14,
	...
	case MIC_BIAS:
		...
		if (current_status == ACCDET_STATE_ABC_00) {
			...
			if (s_eint_accdet_sync_flag == 1)
				...
				pr_info("[accdet]2.1 Headset has plugged out(000)\n");
			mutex_unlock(&accdet_eint_irq_sync_mutex);
			s_button_status = 1;
			if (s_button_status) {
				mutex_lock(&accdet_eint_irq_sync_mutex);
				if (s_eint_accdet_sync_flag == 1)
					...
					pr_info("[accdet]2.2 Headset has plugged out (000)\n");
				mutex_unlock(&accdet_eint_irq_sync_mutex);
				pr_debug(
			...
			if (s_eint_accdet_sync_flag == 1) {
				...
				pr_info(
				    "[accdet]2.Headset has plugged out(010)\n");
			}
			mutex_unlock(&accdet_eint_irq_sync_mutex);
		} else if (current_status == ACCDET_STATE_ABC_11) {
			pr_debug(
	...
	case HOOK_SWITCH:
		if (current_status == ACCDET_STATE_ABC_00) {
			...
			if (s_eint_accdet_sync_flag == 1)
				...
				/* s_accdet_status = HOOK_SWITCH; */
				pr_debug("[accdet]3.HOOK_SWITCH state not change!\n");
			else
				pr_info(
			...
			if (s_eint_accdet_sync_flag == 1) {
				...
				s_accdet_status = MIC_BIAS; /* 4 poles */
				s_cable_type = HEADSET_MIC;
				pr_debug("[accdet]3.switch to MIC_BIAS\n");
			} else {
				pr_info(
			...
			pr_debug("[accdet]3.do not send plug out event in hook switch\n");
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (s_eint_accdet_sync_flag == 1)
				s_accdet_status = PLUG_OUT;
			else
	...
	case STAND_BY:
		if (current_status == ACCDET_STATE_ABC_11)
			pr_info("[accdet]4.accdet no send plug out event in stand by!\n");
		else
			pr_info(
		...
		break;
	default:
		pr_info("[accdet]5.accdet current status error!\n");
		break;
	}

	pr_debug("[accdet]7.cable type:[%s], status switch:[%s]->[%s]\n",
...
static inline void headset_plug_out(void)
{
	...
	if (g_cur_key != 0) {
		...
		pr_info("[accdet]plug_out send key = %d release\n",
			    g_cur_key);
		g_cur_key = 0;
	}
...
{
	/* pr_debug("[accdet][enable_accdet]enter..\n"); */
	/* enable clock */
...
static inline void disable_accdet(void)
{
	...
	while (reg_val & ACCDET_IRQ_B0) {
		reg_val = pmic_pwrap_read(ACCDET_CON12);
		pr_debug("[%s]Clear interrupt on-going..\n",
			__func__);
		msleep(20);
	}
	irq_temp = reg_val;
	...
	pmic_pwrap_write(ACCDET_CON02,
		ACCDET_EINT0_PWM_EN | ACCDET_EINT0_PWM_IDLE);
#elif defined CONFIG_ACCDET_SUPPORT_EINT1
	pmic_pwrap_write(ACCDET_CON02,
		ACCDET_EINT1_PWM_EN | ACCDET_EINT1_PWM_IDLE);
#elif defined CONFIG_ACCDET_SUPPORT_BI_EINT
	pmic_pwrap_write(ACCDET_CON02,
		ACCDET_EINT0_PWM_EN | ACCDET_EINT1_PWM_EN |
...
static inline void clear_accdet_eint_interrupt(int eint_id)
{
	...
	if (eint_id == ACCDET_EINT0_IRQ_IN)
		...
		pmic_pwrap_write(ACCDET_CON12,
				 reg_val |
				     ACCDET_EINT_IRQ_CLR_B10_11); /* abnormal */
		pr_info(
		    "[%s]error:eint_id=0x%x\n", __func__,
		    eint_id);
	}
#endif
	pr_debug("[%s][0x%x]=0x%x\n", __func__, ACCDET_CON12,
		     pmic_pwrap_read(ACCDET_CON12));
#else
	pr_info("[%s]CONFIG_ACCDET_EINT no support the API\n",
		__func__);
#endif
}
...
#endif
		disable_accdet();
		pr_debug("[%s]more than 5s MICBIAS, disable micbias\n",
			__func__);
	}
}
...
static void accdet_eint_work_callback(struct work_struct *work)
{
	...
	if (g_cur_eint_state == EINT_PIN_PLUG_IN) { /* EINT_PIN_PLUG_IN */
		pr_debug("[accdet]DCC plug-in, g_cur_eint_state = %d\n",
			     g_cur_eint_state);
		mutex_lock(&accdet_eint_irq_sync_mutex);
...
#ifdef CONFIG_ACCDET_SUPPORT_EINT0
		enable_accdet(
		    ACCDET_EINT0_PWM_IDLE_B11); /* enable ACCDET EINT0 unit */
#elif defined CONFIG_ACCDET_SUPPORT_EINT1
		enable_accdet(
		    ACCDET_EINT1_PWM_IDLE_B12); /* enable ACCDET EINT1 unit */
		...
		/* Disable ACCDET */
		pr_debug("[accdet]DCC EINT:plug-out, cur_eint_state=%d\n",
			     g_cur_eint_state);
		mutex_lock(&accdet_eint_irq_sync_mutex);
...
#ifdef CONFIG_ACCDET_EINT_IRQ
/* pwrap_write(ACCDET_CON24,
 * pmic_pwrap_read(ACCDET_CON24)&(~0x1F));
 */
#endif
		reg_val = pmic_pwrap_read(ACCDET_CON02);
		pmic_pwrap_write(ACCDET_CON02,
		...
		pmic_pwrap_write(ACCDET_CON12, irq_temp);
		pr_debug("[accdet]plug-out,[0x%x]=0x%x\n", ACCDET_CON12,
			     pmic_pwrap_read(ACCDET_CON12));
	}
#endif
	...
	if (g_cur_eint_state == EINT_PIN_PLUG_IN) {
		pr_debug("[accdet]ACC plug-in, g_cur_eint_state = %d\n",
			     g_cur_eint_state);
		mutex_lock(&accdet_eint_irq_sync_mutex);
		s_eint_accdet_sync_flag = 1;
		mutex_unlock(&accdet_eint_irq_sync_mutex);
		...
		/* Disable ACCDET */
		pr_debug("[accdet]ACC plug-out, g_cur_eint_state = %d\n",
			     g_cur_eint_state);
		mutex_lock(&accdet_eint_irq_sync_mutex);
		s_eint_accdet_sync_flag = 0;
		mutex_unlock(&accdet_eint_irq_sync_mutex);
...
static int accdet_eint_func(int eint_id)
{
	...
	if (eint_id == ACCDET_EINT1_IRQ_IN) {
		...
		pr_info("[accdet][%s]eint_id is not ACCDET_EINT1_IRQ_IN!\n",
			__func__);
	}
#elif defined CONFIG_ACCDET_SUPPORT_BI_EINT
	if (eint_id == ACCDET_EINT0_IRQ_IN) {
	...
	if (g_cur_eint_state == EINT_PIN_PLUG_OUT) {
		...
		if ((g_cur_eint0_state | g_cur_eint1_state) ==
			...
			pr_info(
			    "[%s]wait state1:eint0=%d;eint1=%d\n",
			    __func__, g_cur_eint0_state, g_cur_eint1_state);
		}
	} else {
		pr_info("[%s]g_cur_eint_state err!\n", __func__);
	}
#endif
	/* pr_debug("[accdet_eint_func]end, cur_eint_state= %d\n",
	 * g_cur_eint_state);
	 */
	return ret;
}
...
#endif
	pr_debug("[accdet] report cable_type:%d\n", s_cable_type);
}
...
static irqreturn_t accdet_ap_eint_func(int irq, void *data)
{
	...
	if (g_cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		/* the headset was plugged out set the opposite polarity to what
		 * initialed
		 */
		if (g_accdet_eint_type == IRQ_TYPE_LEVEL_HIGH)
			irq_set_irq_type(g_accdet_irq, IRQ_TYPE_LEVEL_LOW);
	...
	pr_debug("[%s]end,g_cur_eint_state=%d\n",
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	if (IS_ERR(pins_default)) {
		...
		/* dev_info(&accdet_device->dev, "[accdet]Cannot find accdet
		 * pinctrl default!\n");
		 */
	}
	...
	if (node) {
		...
		ret =
		    of_property_read_u32(node, "debounce", &g_gpio_headset_deb);
		if (ret < 0) {
			pr_info("debounce time not found\n");
			return ret;
		}
		gpio_set_debounce(g_gpio_pin, g_gpio_headset_deb);
		...
		pr_info("[Accdet] gpiopin:%d debounce:%d accdet_irq:%d accdet_eint_type:%d\n",
			     g_gpio_pin, g_gpio_headset_deb, g_accdet_irq,
			     g_accdet_eint_type);
		ret = request_irq(g_accdet_irq, accdet_ap_eint_func,
				  IRQF_TRIGGER_NONE, "accdet-eint", NULL);
		if (ret != 0) {
			pr_info("[Accdet]EINT IRQ LINE NOT AVAILABLE\n");
		} else {
			pr_info("[Accdet]accdet set EINT finished, accdet_irq=%d, headsetdebounce=%d\n",
				     g_accdet_irq, g_gpio_headset_deb);
		}
	} else {
		pr_info("[Accdet]%s can't find compatible node\n",
			     __func__);
	}
	return 0;
}
...
static int accdet_irq_handler(void)
{
	...
	unsigned int reg_val = 0;
	u64 cur_time = 0;
#if defined(CONFIG_MOISTURE_INTERNAL_SUPPORT) ||                               \
	defined(CONFIG_MOISTURE_EXTERNAL_SUPPORT)
	...
	if (g_cur_eint_state == EINT_PIN_PLUG_OUT) {
		pr_debug(
		    "=========[ACCDET]Moisture Enable=============\n\r");
		/* Disable ACCDET to AUXADC */
		pmic_pwrap_write(AUDENC_ANA_CON10,
				 pmic_pwrap_read(AUDENC_ANA_CON10) &
				     0x1FFF); /*set bit[13-15] = 0*/
		pmic_pwrap_write(ACCDET_CON00, pmic_pwrap_read(ACCDET_CON00) &
						   0xFBFF); /*set bit[10] = 0*/
		pmic_pwrap_write(ACCDET_CON00, pmic_pwrap_read(ACCDET_CON00) |
						   0x800); /*set bit[11] = 1*/
		/* Enable moisture detection, set 219A bit[13] = 1*/
		pmic_pwrap_write(AUDENC_ANA_CON9,
				 pmic_pwrap_read(AUDENC_ANA_CON9) | 0x2000);
		/* select PAD_HP_EINT for moisture detection, set 219A bit[14] =
		 * 0
		 */
		pmic_pwrap_write(AUDENC_ANA_CON9,
				 pmic_pwrap_read(AUDENC_ANA_CON9) & 0xBFFF);
#ifdef CONFIG_MOISTURE_INTERNAL_SUPPORT
		/* select VTH for 2v, and 500k, use internal resitance set 219C
		 * bit[10][12] = 1
		 */
		pmic_pwrap_write(AUDENC_ANA_CON10,
				 pmic_pwrap_read(AUDENC_ANA_CON10) | 0x1400);
#endif
#ifdef CONFIG_MOISTURE_EXTERNAL_SUPPORT
		/* select VTH for 2v, and 500k, use external resitance set 219C
		 * bit[10] = 1, bit[11] = 0
		 */
		pmic_pwrap_write(AUDENC_ANA_CON10,
				 pmic_pwrap_read(AUDENC_ANA_CON10) & 0xF7FF);
		pmic_pwrap_write(AUDENC_ANA_CON10,
				 pmic_pwrap_read(AUDENC_ANA_CON10) | 0x0400);
#endif
		moisture = Accdet_PMIC_IMM_GetOneChannelValue(0);
		pr_info(
		    "[ACCDET]Moisture Read Auxadc] new moisture =  %d\n\r",
		    moisture);
		...
		pmic_pwrap_write(AUDENC_ANA_CON9,
				 pmic_pwrap_read(AUDENC_ANA_CON9) & 0xDFFF);
#ifdef CONFIG_MOISTURE_INTERNAL_SUPPORT
		/* revert select VTH for 2v, and 500k, set 219C bit[10][12] =
		 * 0
		 */
		pmic_pwrap_write(AUDENC_ANA_CON10,
				 pmic_pwrap_read(AUDENC_ANA_CON10) & 0xEBFF);
#endif
#ifdef CONFIG_MOISTURE_EXTERNAL_SUPPORT
		/* revert select VTH for 2v, and 500k, set 219C bit[10] = 0,
		 * bit[11] = 1
		 */
		pmic_pwrap_write(AUDENC_ANA_CON10,
				 pmic_pwrap_read(AUDENC_ANA_CON10) & 0xFBFF);
		pmic_pwrap_write(AUDENC_ANA_CON10,
				 pmic_pwrap_read(AUDENC_ANA_CON10) | 0x0800);
#endif
	} else if (g_cur_eint_state == EINT_PIN_MOISTURE_DETECTED) {
		pr_debug("ACCDET Moisture plug out detectecd\n");
		if (g_cur_eint_state ==
		    EINT_PIN_MOISTURE_DETECTED) { /*just for low level trigger*/
			if (g_accdet_eint_type == IRQ_TYPE_LEVEL_HIGH)
				pmic_pwrap_write(ACCDET_CON12,
						 reg_val |
	...
	if (moisture > g_moisture_vm) {
		pr_debug("ACCDET Moisture plug in detectecd\n");
		if (g_cur_eint_state ==
		    EINT_PIN_PLUG_OUT) { /*just for low level trigger*/
			if (g_accdet_eint_type == IRQ_TYPE_LEVEL_HIGH)
				pmic_pwrap_write(
				    ACCDET_CON12,
				    reg_val & (~ACCDET_EINT0_IRQ_POL_B14));
		...
		eint_type = ACCDET_EINT0_IRQ_IN;
		g_cur_eint_state = EINT_PIN_MOISTURE_DETECTED;
		clear_accdet_eint_interrupt(eint_type);
		while (((pmic_pwrap_read(ACCDET_CON12) & ACCDET_EINT0_IRQ_B2)
			&& (accdet_timeout_ns(cur_time, ACCDET_TIME_OUT))))
			;
		...
		pmic_pwrap_write(AUD_TOP_INT_STATUS0,
				 RG_INT_STATUS_ACCDET_EINT0_B6);
		pr_debug(
		    "[mositure clear_accdet_eint_interrupt][0x%x]=0x%x\n",
		...
		if ((reg_val & ACCDET_IRQ_B0) &&
			...
			while (
			    ((pmic_pwrap_read(ACCDET_CON12) & ACCDET_IRQ_B0) &&
			     (accdet_timeout_ns(cur_time, ACCDET_TIME_OUT))))
				;
			pmic_pwrap_write(ACCDET_CON12,
					 pmic_pwrap_read(ACCDET_CON12) &
			...
			/* interrupt issued */
			pmic_pwrap_write(AUD_TOP_INT_STATUS0,
					 RG_INT_STATUS_ACCDET_B5);
		} else if ((reg_val & ACCDET_EINT0_IRQ_B2) ==
			   ACCDET_EINT0_IRQ_B2) {
			if (g_cur_eint_state == EINT_PIN_PLUG_IN) {
				if (g_accdet_eint_type == IRQ_TYPE_LEVEL_HIGH)
					pmic_pwrap_write(
			...
			eint_type = ACCDET_EINT0_IRQ_IN;
			clear_accdet_eint_interrupt(eint_type);
			while (((pmic_pwrap_read(ACCDET_CON12) &
				 ACCDET_EINT0_IRQ_B2) &&
				(accdet_timeout_ns(cur_time, ACCDET_TIME_OUT))))
				;
			...
			pr_info("[%s]ACCDET IRQ and EINT0 IRQ don't be triggerred!!\n",
				__func__);
		}
#elif defined CONFIG_ACCDET_SUPPORT_EINT1
	if ((reg_val & ACCDET_IRQ_B0) &&
		...
		pr_info("[%s]ACCDET IRQ and EINT1 IRQ isn't triggerred!!\n",
			__func__);
	}
#elif defined CONFIG_ACCDET_SUPPORT_BI_EINT /* EINT0 & EINT1 */
	if ((reg_val & ACCDET_IRQ_B0) &&
		...
		pr_info("[%s]ACCDET IRQ and EINT IRQ don't be triggerred!!\n",
			__func__);
	}
#endif
#if defined(CONFIG_MOISTURE_INTERNAL_SUPPORT) ||                               \
...
static int accdet_irq_handler(void)
{
	...
	cur_time = accdet_get_current_time();
	pr_debug("[%s]CONFIG_ACCDET_EINT-->enter\n", __func__);
...
static int accdet_irq_handler(void)
{
	pr_info("[accdet]CONFIG_ACCDET_EINT or CONFIG_ACCDET_EINT_IRQ is not defined\n");
	return 0;
}
#endif
...
static inline void accdet_eint_high_level_support(void)
{
	...
	if (g_cur_eint_state == EINT_PIN_PLUG_OUT) {
		...
		if (headset_dts_data.eint_level_pol == IRQ_TYPE_LEVEL_HIGH) {
			...
			reg_val = pmic_pwrap_read(ACCDET_CON01);
			pmic_pwrap_write(ACCDET_CON01, reg_val &
				(~ACCDET_EINT0_SEQ_INIT_EN));
		} else {
			g_accdet_eint_type = IRQ_TYPE_LEVEL_LOW;
			/* default level_low */
			pwrap_write(ACCDET_CON12, reg_val &
				(~ACCDET_EINT0_IRQ_POL_B14));
		}
	 ...
	 * pmic_pwrap_read(ACCDET_CON02)|ACCDET_EINT0_PWM_EN);
	 */
#elif defined CONFIG_ACCDET_SUPPORT_EINT1
	if (g_cur_eint_state == EINT_PIN_PLUG_OUT) {
		reg_val = pmic_pwrap_read(ACCDET_CON12);
		if (headset_dts_data.eint_level_pol == IRQ_TYPE_LEVEL_HIGH) {
			...
			pwrap_write(ACCDET_CON12, reg_val |
				ACCDET_EINT1_IRQ_POL_B15);
			pr_info("[accdet1]high:[0x%x]=0x%x\n",
				ACCDET_CON12, reg_val);

			/* set pmic eint default value */
			reg_val = pmic_pwrap_read(ACCDET_CON01);
			pr_info("[accdet1]high:1.[0x%x]=0x%x\n",
				ACCDET_CON01, reg_val);
			/* set bit5 to enable default EINT init status */
			pmic_pwrap_write(ACCDET_CON01, reg_val |
				ACCDET_EINT1_SEQ_INIT_EN_B5);
			mdelay(2);
			reg_val = pmic_pwrap_read(ACCDET_CON01);
			pr_info("[accdet1]high:2.[0x%x]=0x%x\n",
				ACCDET_CON01, reg_val);
			reg_val = pmic_pwrap_read(ACCDET_DEFAULT_EINT_STS);
			pr_info("[accdet1]high:1.[0x%x]=0x%x\n",
				ACCDET_DEFAULT_EINT_STS,
	...
	/* pwrap_write(ACCDET_CON02,
	 * pmic_pwrap_read(ACCDET_CON02)|ACCDET_EINT1_PWM_EN);
	 */
#elif defined CONFIG_ACCDET_SUPPORT_BI_EINT
	if (g_cur_eint_state == EINT_PIN_PLUG_OUT) {
		reg_val = pmic_pwrap_read(ACCDET_CON12);
		if (headset_dts_data.eint_level_pol == IRQ_TYPE_LEVEL_HIGH) {
			g_accdet_eint_type = IRQ_TYPE_LEVEL_HIGH;
			pwrap_write(ACCDET_CON12, reg_val |
				ACCDET_EINT_IRQ_POL_HIGH);
			pr_info("[accdet2]high:[0x%x]=0x%x\n",
				ACCDET_CON12, reg_val);

			/* set pmic eint default value */
			reg_val = pmic_pwrap_read(ACCDET_CON01);
			pr_info("[accdet2]high:1.[0x%x]=0x%x\n",
				ACCDET_CON01, reg_val);
			/* set bit3&bit5 to enable default EINT init status */
			pmic_pwrap_write(ACCDET_CON01, reg_val |
				ACCDET_EINT_SEQ_INIT_EN);
			mdelay(2);
			reg_val = pmic_pwrap_read(ACCDET_CON01);
			pr_info("[accdet2]high:2.[0x%x]=0x%x\n",
				ACCDET_CON01, reg_val);
			reg_val = pmic_pwrap_read(ACCDET_DEFAULT_EINT_STS);
			pr_info("[accdet2]high:1.[0x%x]=0x%x\n",
				ACCDET_DEFAULT_EINT_STS,
			...
			g_accdet_eint_type = IRQ_TYPE_LEVEL_LOW;
			pwrap_write(ACCDET_CON12, reg_val &
				(~ACCDET_EINT_IRQ_POL_LOW));
		}
		/* maybe need judge the bit3 is clear actually??? */
		reg_val = pmic_pwrap_read(ACCDET_CON12);
		pmic_pwrap_write(ACCDET_CON12,
			reg_val & (~ACCDET_EINT_IRQ_CLR_B10_11));
...
void accdet_init_once(int init_flag)
{
	...
	if (init_flag == ACCDET_INIT0_ONCE) {
...
#else /* SW MODE */
		pr_info(
			"[%s]sw mode setting,old Accdet_con24:0x%x,AUDENC_ANA_CON6:0x%x\n",
			__func__,
			    pmic_pwrap_read(ACCDET_CON24),
			    pmic_pwrap_read(AUDENC_ANA_CON6));

		reg_val = pmic_pwrap_read(ACCDET_CON24);
		pmic_pwrap_write(ACCDET_CON24,
				 (reg_val & (~ACCDET_HWMODE_SEL)) |
				     ACCDET_FAST_DISCAHRGE);
		...
		pr_info(
			"[%s]sw mode setting,new Accdet_con24:0x%x,AUDENC_ANA_CON6:0x%x\n",
			    __func__, pmic_pwrap_read(ACCDET_CON24),
			    pmic_pwrap_read(AUDENC_ANA_CON6));
#endif
...
/* pmic_pwrap_write(ACCDET_CON24, ACCDET_DISABLE_CMPC); */
		...
		if (headset_dts_data.accdet_mic_mode ==
			...
			pmic_pwrap_write(
			    AUDENC_ANA_CON10,
			    reg_val | RG_ACCDET_MODE_ANA10_MODE1); /* 0x07 */
		else if (headset_dts_data.accdet_mic_mode ==
			 HEADSET_MODE_2) {
			/* Low cost mode without internal bias*/
			/* for test discharge quickly */
			pmic_pwrap_write(
			    AUDENC_ANA_CON10,
			    reg_val | RG_ACCDET_MODE_ANA10_MODE2); /* 0x887 */
		} else if (headset_dts_data.accdet_mic_mode ==
			   HEADSET_MODE_6) {
			...
			pmic_pwrap_write(
			    AUDENC_ANA_CON9,
			    reg_val |
				0x100); /* Set bit8 = 1 to use internal bias */
		}			/* end HEADSET_MODE_6 */

/* ACCDET AUXADC AUTO Setting  */
/* pmic_pwrap_write(AUXADC_ACCDET,
 * pmic_pwrap_read(AUXADC_ACCDET)|AUXADC_ACCDET_AUTO_SPL_EN);
 */
...
/* =========interrupt enable and eint pwm set========== */
		...
		/* pmic_pwrap_write(ACCDET_CON2,
		 * reg_val|ACCDET_EINT0_PWM_EN_B3); */ /* discard */
		pr_debug("[%s]-1_0[0x%x]=0x%x\n",
		...
		pwrap_write(ACCDET_CON01,
			    pmic_pwrap_read(ACCDET_CON01) | ACCDET_EINT0_EN_B2);
#elif defined CONFIG_ACCDET_SUPPORT_EINT1
		...
		/* pmic_pwrap_write(ACCDET_CON2,
		 * reg_val|ACCDET_EINT1_PWM_EN_B4); */ /* discard */
		pr_debug("[%s]-1_1[0x%x]=0x%x\n",
			__func__, ACCDET_CON02,
			pmic_pwrap_read(ACCDET_CON02));
		...
		pwrap_write(ACCDET_CON01,
			    pmic_pwrap_read(ACCDET_CON01) | ACCDET_EINT1_EN_B4);
#elif defined CONFIG_ACCDET_SUPPORT_BI_EINT
		pmic_pwrap_write(ACCDET_CON15, pmic_pwrap_read(ACCDET_CON15) &
						   (~(0x1F << 8)));
		reg_val = pmic_pwrap_read(ACCDET_CON15);
		...
		reg_val = pmic_pwrap_read(ACCDET_CON02);
		pmic_pwrap_write(ACCDET_CON02, reg_val |
						   ACCDET_EINT_PWM_IDLE_B11_12 |
						   ACCDET_EINT_PWM_EN_B3_4);
		/* pmic_pwrap_write(ACCDET_CON02,
		 * reg_val|ACCDET_EINT_PWM_EN_B3_4); */ /* discard */
		pr_debug("[%s]-1_2[0x%x]=0x%x\n",
			__func__, ACCDET_CON02,
			pmic_pwrap_read(ACCDET_CON02));
...
#else
		pr_info(
		    "[%s]CONFIG_ACCDET_EINT_IRQ defined error\n", __func__);
#endif
#endif /* end ifdef CONFIG_ACCDET_EINT_IRQ  */
		pr_info("[%s] ACCDET_INIT1_ONCE done--->\n", __func__);
	} else {
		pr_info("[%s] error: do nothing of %s\n",
			__func__, __func__);
	}
}
...
static inline void accdet_init(void)
{
	...
	pmic_pwrap_write(ACCDET_CON01,
			 pmic_pwrap_read(ACCDET_CON01) | ACCDET_SEQ_INIT_EN_B1);
	mdelay(2); /* 2ms */
	 ...
	 *			       switch
	 */
	/* pwrap_write(AUDENC_ANA_CON10,
	 * pmic_pwrap_read(AUDENC_ANA_CON10)|RG_AUD_ACCDET_MBIAS1_EN);
	 */
	...
	/* |
	 * RG_AUD_PW_MBIAS1);
	 *|RG_AUD_MICBIAS1_LOWP_EN|RG_AUD_MICBIAS1_BYPASS_EN);
	 */

	/* maybe needn't run at the most case  */
	/* accdet_eint_high_level_support(); */
	/* set high level trigger of eint */
}
...
static int dump_register(void)
{
...
#ifdef CONFIG_HEADSET_SUPPORT_FIVE_POLE
	pr_info("[CONFIG_HEADSET_SUPPORT_FIVE_POLE] support 5-pole.\n");
#endif

#ifdef CONFIG_ACCDET_EINT_IRQ
#ifdef CONFIG_ACCDET_SUPPORT_EINT0
	pr_info("[CONFIG_ACCDET_SUPPORT_EINT0][MODE_%d]dump_regs:\n",
		    headset_dts_data.accdet_mic_mode);
#elif defined CONFIG_ACCDET_SUPPORT_EINT1
	pr_info("[CONFIG_ACCDET_SUPPORT_EINT1][MODE_%d]dump_regs:\n",
		    headset_dts_data.accdet_mic_mode);
#elif defined CONFIG_ACCDET_SUPPORT_BI_EINT
	pr_info("[CONFIG_ACCDET_SUPPORT_BI_EINT][MODE_%d]dump_regs:\n",
		    headset_dts_data.accdet_mic_mode);
#else
	pr_info("[NO_CONFIG_ACCDET_EINT_0_1]Error1.\n");
#endif
#elif defined CONFIG_ACCDET_EINT
	pr_info("[CONFIG_ACCDET_EINT][MODE_%d]dump_regs:\n",
		    headset_dts_data.accdet_mic_mode);
#else
	pr_info("[NO_CONFIG_ACCDET_EINT]Error2.\n");
#endif
	...
	pr_info("(0x%x)=0x%x\n", AUDENC_ANA_CON6,
		    pmic_pwrap_read(AUDENC_ANA_CON6));
	pr_info("(0x%x)=0x%x\n", AUDENC_ANA_CON9,
		    pmic_pwrap_read(AUDENC_ANA_CON9));
	pr_info("(0x%x)=0x%x\n", AUDENC_ANA_CON10,
		    pmic_pwrap_read(AUDENC_ANA_CON10));
	pr_info("(0x%x)=0x%x\n", AUDENC_ANA_CON11,
				       ...
				       const char *buf, size_t count)
{
	...
	case CALL_IDLE:
		pr_debug("[%s]accdet call: Idle state!\n", __func__);
		break;
	case CALL_RINGING:
		pr_debug("[%s]accdet call: ringing state!\n", __func__);
		break;
	case CALL_ACTIVE:
		pr_debug("[%s]accdet call: active or hold state!\n",
			     __func__);
		break;
	default:
		pr_debug("[%s]accdet call: Invalid value=%d\n", __func__,
			     s_call_status);
		break;
	}
	return count;
...
{
	if (buf == NULL) {
		pr_info("[%s] *buf is NULL Pointer\n", __func__);
		return -EINVAL;
	}

	pr_info("[CONFIG_ACCDET_PIN_RECOGNIZATION]No defined,no support\n");
					     ...
					     const char *buf, size_t count)
{
	...
	pr_info("[%s]get accdet mode: %d\n", __func__, tmp_headset_mode);
	switch (tmp_headset_mode & 0x0F) {
	case HEADSET_MODE_1:
		pr_info("[%s]Don't support accdet mode_1 to configure\n",
			    __func__);
	...
	case HEADSET_MODE_6:
		...
		break;
	default:
		pr_info("[%s]Not support accdet mode: %d\n", __func__,
			     tmp_headset_mode);
		break;
	}
					       ...
					       const char *buf, size_t count)
{
	...
	if (ret) {
		...
		if (IS_ERR(s_thread)) {
			...
			pr_info("[%s]failed to create kernel thread: %d\n",
				     __func__, error);
		} else {
					...
					const char *buf, size_t count)
{
	...
	if ((rw_flag == 'r') || (rw_flag == 'R')) {
		...
		if (ret < 2) {
			pr_info("[%s] read Invalid input!!\n", __func__);
			return ret;
		}
		pr_info("[%s] read addr[0x%x]\n", __func__, addr_temp);
		/* comfirm PMIC addr is legal */
		if ((addr_temp < PMIC_REG_BASE_START) ||
		    (addr_temp > PMIC_REG_BASE_END)) {
			pr_info("[%s] Can't set illegal addr[0x%x]!!\n",
				    __func__, addr_temp);
		} else if (addr_temp & 0x01) {
			pr_info("[%s] No set illegal addr[0x%x]!!\n",
				     __func__, addr_temp);
		} else {
		...
		ret = sscanf(buf, "%c,0x%x,0x%x", &rw_flag, &addr_temp,
			     &value_temp);
		if (ret < 3) {
			pr_info("[%s] write Invalid input!!\n", __func__);
			return ret;
		}
		pr_info("[%s] write addr[0x%x]=0x%x\n", __func__, addr_temp,
			    value_temp);
		/* comfirm PMIC addr is legal */
		if ((addr_temp < PMIC_REG_BASE_START) ||
		    (addr_temp > PMIC_REG_BASE_END)) {
		...
		pr_info("[%s] error handle register\n", __func__);
	}
...
static int accdet_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		...
		if (err) {
			pr_info("[%s]driver_create_file (%s) = %d\n",
				__func__, accdet_attr_list[idx]->attr.name,
				err);
			break;
		}
	}
...
void accdet_late_init(unsigned long a)
{
	if (atomic_cmpxchg(&s_accdet_first, 1, 0)) {
		...
		pr_info("[%s]err: accdet have been done or get dts failed!\n",
			__func__);
	}
}
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	if (ret)
		pr_info("[%s]alloc_chrdev_region: Get Major number error!\n",
			__func__);
	...
	if (ret)
		pr_info("[%s]accdet error: cdev_add\n", __func__);
	...
	if (ret != 0)
		pr_info("[%s]create attribute err = %d\n", __func__,
			     ret);
#endif
	...
	pmic_register_interrupt_callback(
	    INT_ACCDET_EINT0, accdet_eint_int_handler); /* accdet eint0 */
	pr_info("[%s]CONFIG_ACCDET_SUPPORT_EINT0 opened!\n", __func__);
#elif defined CONFIG_ACCDET_SUPPORT_EINT1
	pmic_register_interrupt_callback(
	    INT_ACCDET_EINT1, accdet_eint_int_handler); /* accdet eint1 */
	pr_info("[%s]CONFIG_ACCDET_SUPPORT_EINT1 opened!\n", __func__);
/* #elif defined CONFIG_ACCDET_SUPPORT_BI_EINT */
#else
	pmic_register_interrupt_callback(
	    INT_ACCDET_EINT0, accdet_eint_int_handler); /* accdet eint0 */
	pmic_register_interrupt_callback(
	    INT_ACCDET_EINT1, accdet_eint_int_handler); /* accdet eint1 */
	pr_info("[%s]CONFIG_ACCDET_SUPPORT_BI_EINT opened!\n", __func__);
#endif
#endif
...
/* close top eint and AB interrupt before accdet_init in the first anyhow*/
	...
	pr_info("[%s](0x%x)=0x%x\n", __func__, INT_CON1_ACCDET,
				pmic_pwrap_read(INT_CON1_ACCDET));
	pr_info("[%s](0x%x)=0x%x\n", __func__, INT_STATUS_ACCDET,
	...
	if (ret == 0) {
#ifdef CONFIG_ACCDET_EINT
		pr_info("[%s]CONFIG_ACCDET_EINT opened!\n", __func__);
		...
		pr_info("[%s]accdet_get_dts_data Failed\n", __func__);
	}
	accdet_pmic_Read_Efuse_HPOffset();
...
void mt_accdet_suspend(void) /* only one suspend mode */
{
	...
	pr_debug("[accdet]suspend:[0x%x]=0x%x\n", ACCDET_CON12,
		     pmic_pwrap_read(ACCDET_CON12));
#else
	pr_debug("[accdet]suspend:[0x%x]=0x%x,[0x%x]=0x%x(0x%x)\n",
...
{
#if defined CONFIG_ACCDET_EINT || defined CONFIG_ACCDET_EINT_IRQ
	pr_debug("[accdet]resume:[0x%x]=0x%x\n", ACCDET_CON12,
		     pmic_pwrap_read(ACCDET_CON12));
#else
	pr_debug("[accdet]resume:[0x%x]=0x%x, [0x%x]=0x%x(0x%x)\n",
		     ACCDET_CON01, pmic_pwrap_read(ACCDET_CON01), ACCDET_CON02,
		     pmic_pwrap_read(ACCDET_CON02), s_pre_state_swctrl);
#endif
}
 ...
 */
	...
	pr_debug("[Accdet]%s:current ABC:0x%x\n", __func__,
		     current_status_restore);
	...
	case 3: /* AB=3 */
		...
		break;
	default:
		pr_debug("[Accdet]%s:error current status:%d!\n",
			__func__, current_status_restore);
		break;
	}
