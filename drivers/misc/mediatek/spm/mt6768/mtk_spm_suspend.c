...
#if IS_ENABLED(CONFIG_MTK_EIC) || IS_ENABLED(CONFIG_PINCTRL_MTK)
...
{
	return SMC_CALL(GET_PWR_CTRL_ARGS, SPM_PWR_CTRL_SUSPEND, PW_WAKE_SRC, 0);
}
...
EXPORT_SYMBOL(__spm_suspend);
//module_init(spm_logger_init);
//module_exit(spm_logger_exit);
