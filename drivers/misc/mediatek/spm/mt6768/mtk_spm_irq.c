	...
	for (i = 0; i < IRQ_NUMBER; i++) {
		if (edge_trig_irqs[i]) {
			...
			pr_info("[SPM] start to unmask edge trigger irq: %d\n",
					edge_trig_irqs[i]);
			arm_smccc_smc(MTK_SIP_KERNEL_SPM_CIRQ_CONTROL,
				MTK_SPM_IRQ_SMC_UNMASK_FOR_SLEEP_EX,
...
//#if IS_ENABLED(CONFIG_MTK_GIC_V3_EXT)
	...
	arm_smccc_smc(MTK_SIP_KERNEL_SPM_CIRQ_CONTROL, MTK_SPM_IRQ_SMC_MASK_ALL,
		0, 0, 0, 0, 0, 0, &smc_res);
	if (smc_res.a0)
	...
	arm_smccc_smc(MTK_SIP_KERNEL_SPM_CIRQ_CONTROL, MTK_SPM_IRQ_SMC_UNMASK_FOR_SLEEP_EX,
...
//#if IS_ENABLED(CONFIG_MTK_SYS_CIRQ)
	...
	arm_smccc_smc(MTK_SIP_KERNEL_SPM_CIRQ_CONTROL, MTK_SPM_CIRQ_SMC_CLONE_GIC,
		0, 0, 0, 0, 0, 0, &smc_res);
	if (smc_res.a0)
		pr_info("[SPM] fail to clone gic, ret=0x%lx\n", smc_res.a0);
	//mt_cirq_enable();
	pr_info("[SPM] start to enable cirq\n");
	arm_smccc_smc(MTK_SIP_KERNEL_SPM_CIRQ_CONTROL, MTK_SPM_CIRQ_SMC_ENABLE,
		0, 0, 0, 0, 0, 0, &smc_res);
	if (smc_res.a0)
...
//#if IS_ENABLED(CONFIG_MTK_SYS_CIRQ)
	...
	arm_smccc_smc(MTK_SIP_KERNEL_SPM_CIRQ_CONTROL, MTK_SPM_CIRQ_SMC_FLUSH,
		0, 0, 0, 0, 0, 0, &smc_res);
	if (smc_res.a0)
		pr_info("[SPM] fail to flush cirq, ret=0x%lx\n", smc_res.a0);
	//mt_cirq_disable();
	pr_info("[SPM] start to disable cirq\n");
	arm_smccc_smc(MTK_SIP_KERNEL_SPM_CIRQ_CONTROL, MTK_SPM_CIRQ_SMC_DISABLE,
		0, 0, 0, 0, 0, 0, &smc_res);
	if (smc_res.a0)
...
//#if IS_ENABLED(CONFIG_MTK_GIC_V3_EXT)
	...
	arm_smccc_smc(MTK_SIP_KERNEL_SPM_CIRQ_CONTROL, MTK_SPM_IRQ_SMC_MASK_RESTORE,
		0, 0, 0, 0, 0, 0, &smc_res);
	if (smc_res.a0)
