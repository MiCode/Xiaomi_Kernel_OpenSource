...
int mtk_lp_sysfs_debugfs_entry_create_plat(const char *name
...
int mtk_lp_sysfs_debugfs_entry_node_add_plat(const char *name
...
int mtk_lp_sysfs_debugfs_entry_node_remove_plat(
		struct mtk_lp_sysfs_handle *node)
{
	int bRet = 0;
...
int mtk_lp_sysfs_debugfs_entry_group_create_plat(const char *name
		...
		, struct mtk_lp_sysfs_handle *handle)
{
	...
	mtk_lp_sysfs_debugfs_entry_create_plat(name, mode, parent, pGrouper);
	...
	if (_group && IS_MTK_LP_SYS_HANDLE_VALID(pGrouper)) {
		for (idx = 0;; ++idx) {
			...
			mtk_lp_sysfs_debugfs_entry_node_add_plat(
