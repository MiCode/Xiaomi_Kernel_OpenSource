...
static int __maybe_unused gt9772b_vcm_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct gt9772b_device *gt9772b = sd_to_gt9772b_vcm(sd);

	return gt9772b_power_off(gt9772b);
}

static int __maybe_unused gt9772b_vcm_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct gt9772b_device *gt9772b = sd_to_gt9772b_vcm(sd);

	return gt9772b_power_on(gt9772b);
}
...
static const struct dev_pm_ops gt9772b_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
				pm_runtime_force_resume)
	SET_RUNTIME_PM_OPS(gt9772b_vcm_suspend, gt9772b_vcm_resume, NULL)
};
