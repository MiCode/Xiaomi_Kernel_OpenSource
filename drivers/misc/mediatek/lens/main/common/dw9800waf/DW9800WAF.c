...
static int s4AF_ReadReg(void)
{
	...
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, addr, 1);
	i4RetValue = i2c_master_recv(g_pstAF_I2Cclient, pBuff, 1);
	if (i4RetValue < 0) {
		LOG_INF("I2C read failed!!\n");
		return -1;
	}
	LOG_INF ("Jesse+ 0x00:%x\n",pBuff[0]);
...
void DW9800WAF_EnbleLowPower(bool enable)
{
	if(enable){
		s4AF_WriteReg_Directly(0x0201);//pwoer down mode
	}else{
		s4AF_WriteReg_Directly(0x0200);//power active
	}
}
static int DW9800WAF_setSACMode(void)
{
    s4AF_WriteReg_Directly(0x0201);
    s4AF_WriteReg_Directly(0x0200);
    mdelay(1);
    s4AF_WriteReg_Directly(0x0202);
    s4AF_WriteReg_Directly(0x0680);
    s4AF_WriteReg_Directly(0x0701);
    return 0;
}
		    ...
		    unsigned long a_u4Param)
{
	...
	case AFIOC_T_SETMACROPOS:
		...
		break;
/* ITD: Modify Enable LPM By Jesse 20190513 Start */
	case AFIOC_S_SETPOWERDOWN:
		DW9800WAF_EnbleLowPower((bool)a_u4Param);
		break;
/* ITD: Modify Enable LPM By Jesse 20190513 End */
	default:
		LOG_INF("No CMD\n");
		i4RetValue = -EPERM;
		break;
...
//	unsigned long currPosition = g_u4CurrPosition;
	...
	mdelay(1);
	s4AF_WriteReg_Directly(0x0201);
	LOG_INF("DW9714 Power down mode!\n");
	LOG_INF("End\n");
