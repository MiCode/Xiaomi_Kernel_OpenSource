...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		char puSendCmd2[2] = { 0xED, 0xAB };
		char puSendCmd3[2] = { 0x02, 0x01 };
		char puSendCmd4[2] = { 0x02, 0x00 };

		char puSendCmd11[2] = { 0x03, 0x00 };
		char puSendCmd12[2] = { 0x04, 0xfb };

		char puSendCmd5[2] = { 0x06, 0x84 };
		char puSendCmd6[2] = { 0x07, 0x01 };
		char puSendCmd7[2] = { 0x08, 0x41 };
		...
		i2c_master_send(g_pstAF_I2Cclient, puSendCmd11, 2);
		i2c_master_send(g_pstAF_I2Cclient, puSendCmd12, 2);
...
int DW9714AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (*g_pAF_Opened == 2) {
		...
		g_pstAF_I2Cclient->addr = AF_I2C_SLAVE_ADDR;
        	g_pstAF_I2Cclient->addr = g_pstAF_I2Cclient->addr >> 1;
                i2c_master_send(g_pstAF_I2Cclient, puSendCmdMoveRelease, 2);
		s4AF_WriteReg(250);
		msleep(20);
		s4AF_WriteReg(200);
		msleep(20);
		...
		s4AF_WriteReg(100);
		msleep(20);
                i2c_master_send(g_pstAF_I2Cclient, puSendCmdRelease, 2);
	}
