...
int LC898217AFA_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (*g_pAF_Opened == 2) {
		...
		start_ms = ((unsigned long long)g_TSAFOpen.tv_sec *
				NSEC_PER_SEC +
				(unsigned long long)g_TSAFOpen.tv_nsec)
				/ 1000000;
		end_ms = ((unsigned long long)g_TSAFClose.tv_sec *
				NSEC_PER_SEC +
				(unsigned long long)g_TSAFClose.tv_nsec)
				/ 1000000;
