 ...
 */
/*
 * GT9764BAF voice coil motor driver
 *
 *
 */
#include <linux/i2c.h>
...
#include <linux/fs.h>
#include "lens_info.h"
#define AF_DRVNAME "GT9764BAF_DRV"
...
static int s4GT9764BAF_ReadReg(unsigned short *a_pu2Result)
{
...
static int initdrv(void)
{
	...
	{0x02, 0x02}, {0x06, 0xC0}, {0x07, 0x5C}, {0xFE, 0xFE},
	};
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		initdrv();
		ret = s4GT9764BAF_ReadReg(&InitPos);
		if (ret == 0) {
			LOG_INF("Init Pos %6d\n", InitPos);
...
/* ////////////////////////////////////////////////////////////// */
long GT9764BAF_Ioctl(struct file *a_pstFile,
...
int GT9764BAF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	int ret = 0;
	...
	ret = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2); //af power down
	LOG_INF("End\n");
	return ret;
}
int GT9764BAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int GT9764BAF_GetFileName(unsigned char *pFileName)
{
