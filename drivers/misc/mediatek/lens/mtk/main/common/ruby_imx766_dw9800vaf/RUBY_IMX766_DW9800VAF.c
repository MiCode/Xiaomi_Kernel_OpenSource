...
#define AF_DRVNAME "RUBY_IMX766_DW9800VAF_DRV"
...
static int s4RUBY_IMX766_DW9800VAF_ReadReg(unsigned short *a_pu2Result)
{
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (g_u4setinitpos > 0) {
		if ((g_u4CurrPosition - a_u4Position > 150) || (a_u4Position - g_u4CurrPosition > 150)) {
			int CntStep = g_u4CurrPosition - a_u4Position;
			...
			while (CntStep > 50) {
				...
				s4AF_WriteReg(Position);
				mdelay(5);//add for af stable
				Position -= PerStep;
				if (PerStep < 0) {
					CntStep += PerStep;
				} else {
...
/* ////////////////////////////////////////////////////////////// */
long RUBY_IMX766_DW9800VAF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int RUBY_IMX766_DW9800VAF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
...
int RUBY_IMX766_DW9800VAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
	spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
	...
	if (*g_pAF_Opened == 1) {
		initdrv();
		ret = s4RUBY_IMX766_DW9800VAF_ReadReg(&InitPos);
...
int RUBY_IMX766_DW9800VAF_GetFileName(unsigned char *pFileName)
{
