...
short int func_PROGRAM_DOWNLOAD(void)
{
	...
	printk("jianlong func_PROGRAM_DOWNLOAD 84F7 = 0x%4x", sts);
...
#define DWNLD_TRNS_SIZE (254)
	...
	if (0x01 == g_module_id_OIS) {
		if (u16_type == 0)
			n = DOWNLOAD_BIN_SUNNY_LEN;
		else
			n = DOWNLOAD_COEF_SUNNY_LEN;
	} else {
		if (u16_type == 0)
			n = DOWNLOAD_BIN_OFILM_LEN;
		else
			n = DOWNLOAD_COEF_OFILM_LEN;
	}
		...
		if (lp != 0) {
			if (0x01 == g_module_id_OIS) {
				if (u16_type == 0) {
					...
					for (u16_i = 1; u16_i <= lp; u16_i += 1)
						temp[u16_i] = DOWNLOAD_COEF_SUNNY
							[(total_cnt - block_cnt) *
								 DWNLD_TRNS_SIZE +
							 u16_i - 1];
				}
				...
				if (u16_type == 0) {
					...
					temp[0] = _OP_COEF_DWNLD;
					for (u16_i = 1; u16_i <= lp; u16_i += 1)
						temp[u16_i] = DOWNLOAD_COEF_OFILM
							[(total_cnt - block_cnt) *
								 DWNLD_TRNS_SIZE +
							 u16_i - 1];
				}
...
/* ***************************************************** */
#define ANGLE_LIMIT                                                            \
	(unsigned short int)(GYRSNS * 7 / 10) /* GYRSNS * limit[deg] */
#define G_SENSE 114			       /* [LSB/dps] */
			       ...
			       const struct _FACT_ADJ *param)
{
	...
	u16_dat_SCENE_NIGHT_1[1] = 0x3FF0;
	u16_dat_SCENE_NIGHT_2[1] = 0x3FF0;
	u16_dat_SCENE_NIGHT_3[1] = 0x3FF0;
	u16_dat_SCENE_D_A_Y_1[1] = 0x3FF0;
	u16_dat_SCENE_D_A_Y_2[1] = 0x3FF0;
	u16_dat_SCENE_D_A_Y_3[1] = 0x3FF0;
	u16_dat_SCENE_SPORT_1[1] = 0x3FF0;
	u16_dat_SCENE_SPORT_2[1] = 0x3FF0;
	u16_dat_SCENE_SPORT_3[1] = 0x3FF0;
	u16_dat_SCENE_SPORT_3[2] = 0x0080;
					       ...
					       const struct _FACT_ADJ *param)
{
	...
	/* Disable OIS ( position Servo is not disable ) */
	u16_dat = I2C_OIS_mem__read(_M_EQCTL);
	printk("jianlong func_SET_SCENE_PARAM_for_NewGYRO_Fil 847f = 0x%4x\n", u16_dat);
	...
	if (u16_mode == 1) { /* OIS ON */
		...
		I2C_OIS_mem_write(_M_EQCTL, u16_dat);
		DEBUG_printf(("SET : EQCTL:%.4x\n", u16_dat));
		printk("SET : u16_mode(1) EQCTL:%.4x\n", u16_dat);
	} else if (u16_mode ==
