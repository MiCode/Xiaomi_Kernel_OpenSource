...
//#define SLAVE_ID_DEBUG
#ifdef SLAVE_ID_DEBUG
unsigned short slave_id[4] = {0x18, 0x1E, 0xE8, 0x5A};
#else
...
static int s4AF_WriteReg(u16 a_u2Addr, u16 a_u2Data)
{
	...
	for (i = 0; i < 100; i++) {
		...
		g_pstAF_I2Cclient->addr = slave_id[i] >> 1;

		i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);
		printk("jianlong slave_id = 0x%x", slave_id[i]);
		if (i4RetValue < 0) {
			LOG_INF("I2C write failed!!\n");
			printk("jianlong slave_id = 0x%x", slave_id[i]);
			//return -1;
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	printk("jianlong moveAF a_u4Position = %lu", a_u4Position);
