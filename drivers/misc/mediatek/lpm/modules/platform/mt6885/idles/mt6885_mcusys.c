...
static u64 mt6885_mcusysoff_last_ns;
static unsigned int mt6885_mcusys_status;

int mt6885_mcusys_prompt(int cpu, const struct mtk_lpm_issuer *issuer)
{
	int bRet = 0;
	...
	if (mtk_lp_plat_is_mcusys_off()) {
		...
		mt6885_mcusys_status = mcusys_status;
		if (mt6885_mcusys_status != 0)
			mt6885_do_mcusys_prepare_pdn(mt6885_mcusys_status,
						     &smc_res);
	}
...
void mt6885_mcusys_reflect(int cpu, const struct mtk_lpm_issuer *issuer)
{
	if (mtk_lp_plat_is_mcusys_off()) {
		if (mt6885_mcusys_status != 0) {
			mt6885_do_mcusys_prepare_on();
			mt6885_mcusys_status = 0;
		}
		if (issuer) {
			u64 delta_ns = sched_clock() - mt6885_mcusysoff_last_ns;

			if (IS_NEED_ISSUER(delta_ns)) {
				issuer->log(MT_LPM_ISSUER_CPUIDLE,
						"MCUSYSOFF", NULL);
				mt6885_mcusysoff_last_ns = sched_clock();
			}
		}
	}
...
struct mtk_lpm_model mt6885_model_mcusys = {
	.flag = MTK_LP_REQ_NONE,
	.op = {
		.prompt = mt6885_mcusys_prompt,
		.reflect = mt6885_mcusys_reflect,
		.prepare_resume = NULL,
		.prepare_enter = NULL,
	}
};

int __init mt6885_model_mcusys_init(void)
{
	mtk_lp_model_register("mcusysoff", &mt6885_model_mcusys);
	return 0;
}
