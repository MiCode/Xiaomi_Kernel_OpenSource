...
static u64 mt6833_mcusysoff_last_ns;
static unsigned int mt6833_mcusys_status;

int mt6833_mcusys_prompt(int cpu, const struct mtk_lpm_issuer *issuer)
{
	int bRet = 0;
	...
	if (mtk_lp_plat_is_mcusys_off()) {
		...
		mt6833_mcusys_status = mcusys_status;
		if (mt6833_mcusys_status != 0)
			mt6833_do_mcusys_prepare_pdn(mt6833_mcusys_status,
						     &smc_res);
	}
...
void mt6833_mcusys_reflect(int cpu, const struct mtk_lpm_issuer *issuer)
{
	if (mtk_lp_plat_is_mcusys_off()) {
		if (mt6833_mcusys_status != 0) {
			mt6833_do_mcusys_prepare_on();
			mt6833_mcusys_status = 0;
		}
		if (issuer) {
			u64 delta_ns = sched_clock() - mt6833_mcusysoff_last_ns;

			if (IS_NEED_ISSUER(delta_ns)) {
				issuer->log(MT_LPM_ISSUER_CPUIDLE,
						"MCUSYSOFF", NULL);
				mt6833_mcusysoff_last_ns = sched_clock();
			}
		}
	}
...
struct mtk_lpm_model mt6833_model_mcusys = {
	.flag = MTK_LP_REQ_NONE,
	.op = {
		.prompt = mt6833_mcusys_prompt,
		.reflect = mt6833_mcusys_reflect,
		.prepare_resume = NULL,
		.prepare_enter = NULL,
	}
};

int __init mt6833_model_mcusys_init(void)
{
	mtk_lp_model_register("mcusysoff", &mt6833_model_mcusys);
	return 0;
}
