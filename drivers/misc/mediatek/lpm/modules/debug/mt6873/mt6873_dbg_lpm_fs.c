...
#define MT6873_GENERIC_TRACE_NODE_INIT(_n, _r, _w) ({\
		_n.op.fs_read = _r;\
		_n.op.fs_write = _w;\
		_n.op.priv = &_n; })


#define mt6873_dbg_lpm_trace_log(buf, sz, len, fmt, args...) ({\
	if (len < sz)\
		len += scnprintf(buf + len, sz - len,\
				 fmt, ##args); })

/* Common section */
#define MT6873_DGB_TRACE_COMMON_BASE		(0x0)
#define MT6873_DGB_TRACE_COMMON_SZ		(0x20)
...
/* Suspend section */
#define MT6873_DGB_TRACE_SUSPEND_BASE \
	(MT6873_DGB_TRACE_COMMON_BASE + MT6873_DGB_TRACE_COMMON_SZ)
#define MT6873_DGB_TRACE_SUSPEND_SZ	(0xe0)


/* Low power section */
#define MT6873_DGB_TRACE_LP_BASE \
	(MT6873_DGB_TRACE_SUSPEND_BASE + MT6873_DGB_TRACE_SUSPEND_SZ)
#define MT6873_DGB_TRACE_LP_SZ		(0x200)


struct MT6873_LPM_TRACE_NODE {
	struct mtk_lp_sysfs_handle handle;
	struct mtk_lp_sysfs_op op;
};

struct mtk_lp_sysfs_handle mt6873_entry_lpm_trace;

struct MT6873_LPM_TRACE_NODE mt6873_lpm_trace_node_comm;
struct MT6873_LPM_TRACE_NODE mt6873_lpm_trace_node_lp;
struct MT6873_LPM_TRACE_NODE mt6873_lpm_trace_node_suspend;

struct MT6873_TRACE_COMM {
	unsigned int magic;
...
static ssize_t mt6873_dbg_lpm_trace_suspend_show(char *ToUserBuf,
							size_t sz, void *priv)
{
	...
	while (((offset + sizeof(dump)) < MT6873_DGB_TRACE_SUSPEND_SZ)) {
		...
		trace.read((MT6873_DGB_TRACE_SUSPEND_BASE + offset),
			   dump, sizeof(dump));
		mt6873_dbg_lpm_trace_log(ToUserBuf, sz, len,
...
static ssize_t mt6873_dbg_lpm_trace_lp_show(char *ToUserBuf,
						      size_t sz, void *priv)
{
	struct MTK_LPM_PLAT_TRACE trace;
	unsigned int dump[4], offset = 0;
...
static ssize_t mt6873_dbg_lpm_trace_comm_show(char *ToUserBuf,
							size_t sz, void *priv)
{
	struct MTK_LPM_PLAT_TRACE trace;
	struct MT6873_TRACE_COMM *data = NULL;
	size_t len = 0, rSz = 0;
	int ret = 0;
	...
	data = kzalloc(sizeof(struct MT6873_TRACE_COMM),
			GFP_KERNEL);

	if (!data)
		return -ENOMEM;

	rSz = (sizeof(struct MT6873_TRACE_COMM) > MT6873_DGB_TRACE_COMMON_SZ) ?
		MT6873_DGB_TRACE_COMMON_SZ : sizeof(struct MT6873_TRACE_COMM);

	len = trace.read(MT6873_DGB_TRACE_COMMON_BASE, data, rSz);
...
int mt6873_dbg_lpm_init(void)
{
	...
	MT6873_GENERIC_TRACE_NODE_INIT(mt6873_lpm_trace_node_comm,
	...
	MT6873_GENERIC_TRACE_NODE_INIT(mt6873_lpm_trace_node_lp,
	...
	MT6873_GENERIC_TRACE_NODE_INIT(mt6873_lpm_trace_node_suspend,
...
{
	mtk_lpm_sysfs_entry_node_remove(&mt6873_lpm_trace_node_suspend.handle);
	mtk_lpm_sysfs_entry_node_remove(&mt6873_lpm_trace_node_lp.handle);
	mtk_lpm_sysfs_entry_node_remove(&mt6873_lpm_trace_node_comm.handle);
	mtk_lpm_sysfs_entry_node_remove(&mt6873_entry_lpm_trace);
	return 0;
}
