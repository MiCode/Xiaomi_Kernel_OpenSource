...
static char *mt6983_pwr_ctrl_str[PW_MAX_COUNT] = {
...
static ssize_t mt6983_show_pwr_ctrl(int id, char *buf, size_t buf_sz)
{
	char *p = buf;
	size_t mSize = 0;
	int i;
 ...
 **************************************/
/* code gen by spm_pwr_ctrl_atf.pl, need struct pwr_ctrl */
static ssize_t mt6983_store_pwr_ctrl(int id,	const char *buf, size_t count)
{
	u32 val;
	char cmd[64];
	int i;
	...
	for (i = 0 ; i < PW_MAX_COUNT; i++) {
		if (!strcmp(cmd, mt6983_pwr_ctrl_str[i])) {
...
static ssize_t
mt6983_generic_spm_read(char *ToUserBuf, size_t sz, void *priv);

static ssize_t
mt6983_generic_spm_write(char *FromUserBuf, size_t sz, void *priv);

struct mt6983_SPM_ENTERY {
	const char *name;
	int mode;
	struct mtk_lp_sysfs_handle handle;
};

struct mt6983_SPM_NODE {
	const char *name;
	int mode;
	struct mtk_lp_sysfs_handle handle;
...
struct mt6983_SPM_ENTERY mt6983_spm_root = {
	.name = "power",
	.mode = 0644,
};

struct mt6983_SPM_NODE mt6983_spm_idle = {
	.name = "idle_ctrl",
	.mode = 0644,
	.op = {
		.fs_read = mt6983_generic_spm_read,
		.fs_write = mt6983_generic_spm_write,
		.priv = (void *)&mt6983_spm_idle,
	},
};

struct mt6983_SPM_NODE mt6983_spm_suspend = {
	.name = "suspend_ctrl",
	.mode = 0644,
	.op = {
		.fs_read = mt6983_generic_spm_read,
		.fs_write = mt6983_generic_spm_write,
		.priv = (void *)&mt6983_spm_suspend,
	},
};
...
mt6983_generic_spm_read(char *ToUserBuf, size_t sz, void *priv)
{
	...
	if (priv == &mt6983_spm_idle)
		id = MT_SPM_DBG_SMC_UID_IDLE_PWR_CTRL;
	return mt6983_show_pwr_ctrl(id, ToUserBuf, sz);
}
...
mt6983_generic_spm_write(char *FromUserBuf, size_t sz, void *priv)
{
	...
	return mt6983_store_pwr_ctrl(id, FromUserBuf, sz);
}

static char *mt6983_spm_resource_str[MT_SPM_RES_MAX] = {
...
static ssize_t mt6983_spm_res_rq_read(char *ToUserBuf, size_t sz, void *priv)
{
	char *p = ToUserBuf;
	int i, s, u;
...
static ssize_t mt6983_spm_res_rq_write(char *FromUserBuf, size_t sz, void *priv)
{
	char cmd[128];
	int parm;
...
static const struct mtk_lp_sysfs_op mt6983_spm_res_rq_fops = {
	.fs_read = mt6983_spm_res_rq_read,
	.fs_write = mt6983_spm_res_rq_write,
};
...
int lpm_spm_fs_init(void)
{
	...
	mtk_spm_sysfs_entry_node_add("spm_resource_req", 0444
			, &mt6983_spm_res_rq_fops, NULL);

	r = mtk_lp_sysfs_entry_func_create(mt6983_spm_root.name,
					   mt6983_spm_root.mode, NULL,
					   &mt6983_spm_root.handle);
	if (!r) {
		mtk_lp_sysfs_entry_func_node_add(mt6983_spm_suspend.name,
						mt6983_spm_suspend.mode,
						&mt6983_spm_suspend.op,
						&mt6983_spm_root.handle,
						&mt6983_spm_suspend.handle);

		mtk_lp_sysfs_entry_func_node_add(mt6983_spm_idle.name,
						 mt6983_spm_idle.mode,
						 &mt6983_spm_idle.op,
						 &mt6983_spm_root.handle,
						 &mt6983_spm_idle.handle);
	}
