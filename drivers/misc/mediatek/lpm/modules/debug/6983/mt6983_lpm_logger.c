...
#include <mt6983_spm_reg.h>
#include <mt6983_pwr_ctrl.h>
#include <mt-plat/mtk_ccci_common.h>
#include <lpm_timer.h>
#include <mtk_lpm_sysfs.h>
#include <mt6983_cond.h>
...
/*FIXME*/
const char *mt6983_wakesrc_str[32] = {
	[0] = " R12_PCM_TIMER",
...
/*FIXME*/
struct spm_wakesrc_irq_list mt6983_spm_wakesrc_irqs[] = {
	/* mtk-kpd */
...
struct mt6983_logger_timer {
	struct lpm_timer tm;
	unsigned int fired;
};
#define	STATE_NUM	10
#define	STATE_NAME_SIZE	15
struct mt6983_logger_fired_info {
	unsigned int fired;
...
static struct lpm_spm_wake_status mt6983_wakesrc;

static struct lpm_log_helper mt6983_log_help = {
	.wakesrc = &mt6983_wakesrc,
	.cur = 0,
	.prev = 0,
};
...
#define IRQ_NUMBER	\
	(sizeof(mt6983_spm_wakesrc_irqs)/sizeof(struct spm_wakesrc_irq_list))
static void lpm_get_spm_wakesrc_irq(void)
{
	...
	for (i = 0; i < IRQ_NUMBER; i++) {
		if (mt6983_spm_wakesrc_irqs[i].name == NULL)
			continue;

		node = of_find_compatible_node(NULL, NULL,
			mt6983_spm_wakesrc_irqs[i].name);
		if (!node) {
		...
		mt6983_spm_wakesrc_irqs[i].irq_no =
			irq_of_parse_and_map(node,
				mt6983_spm_wakesrc_irqs[i].order);

		if (!mt6983_spm_wakesrc_irqs[i].irq_no) {
...
{
	struct lpm_log_helper *help = &mt6983_log_help;
...
static void mt6983_suspend_show_detailed_wakeup_reason
	...
	for (i = 1 ; i < PLAT_SPM_COND_MAX ; i++) {
		...
		if (blkcg != 0)
			pr_info("suspend warning: CG: %6s = 0x%08lx\n"
				, mt6983_spm_cond_cg_str[i], blkcg);
...
{
	struct lpm_spm_wake_status *wakesrc = mt6983_log_help.wakesrc;
...
#undef LOG_BUF_SIZE
	...
	if (wakesrc->is_abort != 0) {
		...
		for (i = 1; i < 32; i++) {
			if (wakesrc->r12 & (1U << i)) {
				if (IS_LOGBUF(buf, mt6983_wakesrc_str[i]))
					strncat(buf, mt6983_wakesrc_str[i],
						strlen(mt6983_wakesrc_str[i]));
	...
	if (type == LPM_ISSUER_SUSPEND) {
		...
		mt6983_suspend_show_detailed_wakeup_reason(wakesrc);
		mt6983_suspend_spm_rsc_req_check(wakesrc);
...
static struct lpm_dbg_plat_ops mt6983_dbg_ops = {
...
int mt6983_dbg_ops_register(void)
{
	int ret;

	ret = lpm_dbg_plat_ops_register(&mt6983_dbg_ops);
