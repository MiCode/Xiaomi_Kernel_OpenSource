...
int mt6781_pwr_gs_set(unsigned int type, const struct mtk_lpm_data *val)
{
	int ret = 0;
...
struct mtk_lpm_callee mt6781_pwr_gs_callee = {
	.uid = MTK_LPM_CALLEE_PWR_GS,
	.i.simple = {
		.set = mt6781_pwr_gs_set,
	},
};

#ifdef MTK_LPM_GS_PLAT_CLK_DUMP_SUPPORT
struct mtk_lpm_gs_clk mt6781_clk_cg = {
	.type = GS_CG,
	.name = "CG",
};

struct mtk_lpm_gs_clk mt6781_clk_dcm = {
	.type = GS_DCM,
	.name = "DCM",
};

struct mtk_lpm_gs_clk *mt6781_clks[] = {
	&mt6781_clk_dcm,
	&mt6781_clk_cg,
	NULL,
};

int mt6781_power_gs_clk_user_attach(struct mtk_lpm_gs_clk *p)
{
...
struct mtk_lpm_gs_clk_info mt6781_clk_infos = {
	.attach = mt6781_power_gs_clk_user_attach,
	.dcm = mt6781_clks,
};
#endif

/* PMIC */
struct mtk_lpm_gs_pmic mt6781_pmic6366 = {
	.type = GS_PMIC,
	.regulator = "vcore",
	.pwr_domain = "6366",
};
...
int mt6781_power_gs_pmic_user_attach(struct mtk_lpm_gs_pmic *p)
{
	if (!p || !p->regulator)
		return -EINVAL;
...
struct mtk_lpm_gs_pmic_info mt6781_pmic_infos = {
	.pmic = mt6781_pmic,
	.attach = mt6781_power_gs_pmic_user_attach,
};

int mt6781_power_gs_init(void)
{
	mtk_lpm_callee_registry(&mt6781_pwr_gs_callee);

	/* initial gs compare method */
	mtk_lpm_pwr_gs_common_init();

	/* initial gs pmic information */
	mtk_lpm_pwr_gs_compare_init(MTK_LPM_GS_CMP_PMIC, &mt6781_pmic_infos);

#ifdef MTK_LPM_GS_PLAT_CLK_DUMP_SUPPORT
	/* initial gs dcm information */
	mtk_lpm_pwr_gs_compare_init(MTK_LPM_GS_CMP_CLK, &mt6781_clk_infos);
#endif
	return 0;
}

int mt6781_power_gs_deinit(void)
{
