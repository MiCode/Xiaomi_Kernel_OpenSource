...
#define MT6853_CPUIDLE_PROFILE_OP(op, _priv) ({\
	op.fs_read = mt6853_cpuidle_profile_read;\
	op.fs_write = mt6853_cpuidle_profile_write;\
	op.priv = _priv; })

#define MT6853_CPUIDLE_PROFILE_NODE_INIT(_n, _name, _type) ({\
	_n.name = _name;\
	_n.type = _type;\
	MT6853_CPUIDLE_PROFILE_OP(_n.op, &_n); })
...
struct mtk_lp_sysfs_handle mt6853_entry_cpuidle_profile;

struct MT6853_CPUIDLE_NODE profile_cpc;
struct MT6853_CPUIDLE_NODE profile_kernel;
struct MT6853_CPUIDLE_NODE profile_ratio;
struct MT6853_CPUIDLE_NODE profile_target_state;
...
static ssize_t mt6853_cpuidle_profile_read(char *ToUserBuf,
					    size_t sz, void *priv)
{
	char *p = ToUserBuf;
...
static ssize_t mt6853_cpuidle_profile_write(char *FromUserBuf,
					  size_t sz, void *priv)
{
	struct MT6853_CPUIDLE_NODE *node =
				(struct MT6853_CPUIDLE_NODE *)priv;
...
void mtk_cpuidle_profile_init(void)
{
	...
	mtk_cpuidle_sysfs_sub_entry_add("profile", MTK_CPUIDLE_SYS_FS_MODE,
				NULL, &mt6853_entry_cpuidle_profile);

	MT6853_CPUIDLE_PROFILE_NODE_INIT(profile_cpc, "cpc",
				    TYPE_CPC);
	mtk_cpuidle_sysfs_sub_entry_node_add(profile_cpc.name,
					MTK_CPUIDLE_SYS_FS_MODE,
					&profile_cpc.op,
					&mt6853_entry_cpuidle_profile,
					&profile_cpc.handle);

	MT6853_CPUIDLE_PROFILE_NODE_INIT(profile_kernel, "kernel",
				    TYPE_KERNEL);
	...
	MT6853_CPUIDLE_PROFILE_NODE_INIT(profile_ratio, "ratio",
				    TYPE_RATIO);
	...
	MT6853_CPUIDLE_PROFILE_NODE_INIT(profile_target_state, "target_state",
				    TYPE_TARGET_STATE);
