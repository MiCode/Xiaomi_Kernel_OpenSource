...
#define MT6853_CPUIDLE_CONTROL_OP(op, _priv) ({\
	op.fs_read = mt6853_cpuidle_control_read;\
	op.fs_write = mt6853_cpuidle_control_write;\
	op.priv = _priv; })

#define MT6853_CPUIDLE_CONTROL_NODE_INIT(_n, _name, _type) ({\
	_n.name = _name;\
	_n.type = _type;\
	MT6853_CPUIDLE_CONTROL_OP(_n.op, &_n); })
...
struct mtk_lp_sysfs_handle mt6853_entry_cpuidle_control;

struct MT6853_CPUIDLE_NODE armpll_mode;
struct MT6853_CPUIDLE_NODE buck_mode;
struct MT6853_CPUIDLE_NODE log_enable;
struct MT6853_CPUIDLE_NODE notify_cm;
struct MT6853_CPUIDLE_NODE stress_enable;
struct MT6853_CPUIDLE_NODE stress_time;
struct MT6853_CPUIDLE_NODE stress_timer;
...
static ssize_t mt6853_cpuidle_control_read(char *ToUserBuf,
					    size_t sz, void *priv)
{
	char *p = ToUserBuf;
	struct MT6853_CPUIDLE_NODE *node =
			(struct MT6853_CPUIDLE_NODE *)priv;
	int mode;
...
static ssize_t mt6853_cpuidle_control_write(char *FromUserBuf,
					  size_t sz, void *priv)
{
	struct MT6853_CPUIDLE_NODE *node =
				(struct MT6853_CPUIDLE_NODE *)priv;
...
void mtk_cpuidle_control_init(void)
{
	...
	mtk_cpuidle_sysfs_sub_entry_add("control", MTK_CPUIDLE_SYS_FS_MODE,
				NULL, &mt6853_entry_cpuidle_control);

	MT6853_CPUIDLE_CONTROL_NODE_INIT(armpll_mode, "armpll_mode",
				    TYPE_ARMPLL_MODE);
	mtk_cpuidle_sysfs_sub_entry_node_add(armpll_mode.name,
					MTK_CPUIDLE_SYS_FS_MODE,
					&armpll_mode.op,
					&mt6853_entry_cpuidle_control,
					&armpll_mode.handle);

	MT6853_CPUIDLE_CONTROL_NODE_INIT(buck_mode, "buck_mode",
				    TYPE_BUCK_MODE);
	...
	MT6853_CPUIDLE_CONTROL_NODE_INIT(log_enable, "log",
				    TYPE_LOG_EN);
	...
	MT6853_CPUIDLE_CONTROL_NODE_INIT(notify_cm, "notify_cm",
				    TYPE_NOTIFY_CM);
	...
	MT6853_CPUIDLE_CONTROL_NODE_INIT(stress_enable, "stress",
				    TYPE_STRESS_EN);
	...
	MT6853_CPUIDLE_CONTROL_NODE_INIT(stress_time, "stress_time",
				    TYPE_STRESS_TIME);
	...
	MT6853_CPUIDLE_CONTROL_NODE_INIT(stress_timer, "timer",
				    TYPE_STRESS_TIMER);
