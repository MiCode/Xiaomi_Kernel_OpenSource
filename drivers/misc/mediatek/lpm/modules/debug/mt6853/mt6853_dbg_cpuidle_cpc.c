...
#define MT6853_CPUIDLE_CPC_OP(op, _priv) ({\
	op.fs_read = mt6853_cpuidle_cpc_read;\
	op.fs_write = mt6853_cpuidle_cpc_write;\
	op.priv = _priv; })

#define MT6853_CPUIDLE_CPC_NODE_INIT(_n, _name, _type) ({\
	_n.name = _name;\
	_n.type = _type;\
	MT6853_CPUIDLE_CPC_OP(_n.op, &_n); })
...
struct mtk_lp_sysfs_handle mt6853_entry_cpuidle_cpc;

struct MT6853_CPUIDLE_NODE auto_off_mode;
struct MT6853_CPUIDLE_NODE auto_off_thres;
...
static ssize_t mt6853_cpuidle_cpc_read(char *ToUserBuf,
					    size_t sz, void *priv)
{
	char *p = ToUserBuf;
...
static ssize_t mt6853_cpuidle_cpc_write(char *FromUserBuf,
					  size_t sz, void *priv)
{
	struct MT6853_CPUIDLE_NODE *node =
				(struct MT6853_CPUIDLE_NODE *)priv;
...
void mtk_cpuidle_cpc_init(void)
{
	...
	mtk_cpuidle_sysfs_sub_entry_add("cpc", MTK_CPUIDLE_SYS_FS_MODE,
				NULL, &mt6853_entry_cpuidle_cpc);

	MT6853_CPUIDLE_CPC_NODE_INIT(auto_off_mode, "auto_off",
				    TYPE_AUTO_OFF_MODE);
	mtk_cpuidle_sysfs_sub_entry_node_add(auto_off_mode.name,
					MTK_CPUIDLE_SYS_FS_MODE,
					&auto_off_mode.op,
					&mt6853_entry_cpuidle_cpc,
					&auto_off_mode.handle);

	MT6853_CPUIDLE_CPC_NODE_INIT(auto_off_thres, "auto_off_thres",
				    TYPE_AUTO_OFF_THRES);
