...
#define MT6853_CPUIDLE_STATE_OP(op, _priv) ({\
	op.fs_read = mt6853_cpuidle_state_read;\
	op.fs_write = mt6853_cpuidle_state_write;\
	op.priv = _priv; })

#define MT6853_CPUIDLE_STATE_NODE_INIT(_n, _name, _type) ({\
	_n.name = _name;\
	_n.type = _type;\
	MT6853_CPUIDLE_STATE_OP(_n.op, &_n); })

enum MT6853_CPUIDLE_STATE_NODE_TYPE {
	MT6853_CPUIDLE_STATE_NODE_ENABLED,
	MT6853_CPUIDLE_STATE_NODE_MAX
};

struct MT6853_CPUIDLE_STATE_NODE {
	const char *name;
	int type;
...
struct mtk_lp_sysfs_handle mt6853_entry_cpuidle_state;

struct MT6853_CPUIDLE_STATE_NODE state_enabled;
struct MT6853_CPUIDLE_STATE_NODE state_latency;
struct MT6853_CPUIDLE_STATE_NODE state_residency;
...
static ssize_t mt6853_cpuidle_state_read(char *ToUserBuf,
					    size_t sz, void *priv)
{
	char *p = ToUserBuf;
	struct MT6853_CPUIDLE_STATE_NODE *node = (struct MT6853_CPUIDLE_STATE_NODE *)priv;
...
static ssize_t mt6853_cpuidle_state_write(char *FromUserBuf,
					  size_t sz, void *priv)
{
	struct MT6853_CPUIDLE_STATE_NODE *node = (struct MT6853_CPUIDLE_STATE_NODE *)priv;
	char cmd[128];
...
void mtk_cpuidle_state_init(void)
{
	...
	mtk_cpuidle_sysfs_sub_entry_add("state", MTK_CPUIDLE_SYS_FS_MODE,
				NULL, &mt6853_entry_cpuidle_state);

	MT6853_CPUIDLE_STATE_NODE_INIT(state_enabled, "enabled",
				    IDLE_PARAM_EN);
	mtk_cpuidle_sysfs_sub_entry_node_add(state_enabled.name,
					MTK_CPUIDLE_SYS_FS_MODE,
					&state_enabled.op,
					&mt6853_entry_cpuidle_state,
					&state_enabled.handle);

	MT6853_CPUIDLE_STATE_NODE_INIT(state_latency, "latency",
				    IDLE_PARAM_LAT);
	...
	MT6853_CPUIDLE_STATE_NODE_INIT(state_residency, "residency",
				    IDLE_PARAM_RES);
