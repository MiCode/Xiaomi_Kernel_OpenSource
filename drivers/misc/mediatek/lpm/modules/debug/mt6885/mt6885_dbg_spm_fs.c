...
static ssize_t mt6885_store_pwr_ctrl(int id,	const char *buf, size_t count)
{
	...
	if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_PCM_FLAGS])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_PCM_FLAGS, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_PCM_FLAGS_CUST])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_PCM_FLAGS_CUST, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_PCM_FLAGS_CUST_SET])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_PCM_FLAGS_CUST_SET, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_PCM_FLAGS_CUST_CLR])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_PCM_FLAGS_CUST_CLR, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_PCM_FLAGS1])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_PCM_FLAGS1, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_PCM_FLAGS1_CUST])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_PCM_FLAGS1_CUST, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_PCM_FLAGS1_CUST_SET])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_PCM_FLAGS1_CUST_SET, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_PCM_FLAGS1_CUST_CLR])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_PCM_FLAGS1_CUST_CLR, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_TIMER_VAL])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_TIMER_VAL, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_TIMER_VAL_CUST])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_TIMER_VAL_CUST, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_TIMER_VAL_RAMP_EN])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_TIMER_VAL_RAMP_EN, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_TIMER_VAL_RAMP_EN_SEC])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_TIMER_VAL_RAMP_EN_SEC, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_WAKE_SRC])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_WAKE_SRC, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_WAKE_SRC_CUST])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_WAKE_SRC_CUST, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_WAKELOCK_TIMER_VAL])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_WAKELOCK_TIMER_VAL, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_WDT_DISABLE])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_WDT_DISABLE, val);
	/* SPM_CLK_CON */
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SRCCLKEN0_CTL])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SRCCLKEN0_CTL, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SRCCLKEN1_CTL])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SRCCLKEN1_CTL, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_LOCK_INFRA_DCM])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_LOCK_INFRA_DCM, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SRCCLKEN_MASK])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SRCCLKEN_MASK, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD1_C32RM_EN])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD1_C32RM_EN, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD2_C32RM_EN])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD2_C32RM_EN, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CLKSQ0_SEL_CTRL])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CLKSQ0_SEL_CTRL, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CLKSQ1_SEL_CTRL])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CLKSQ1_SEL_CTRL, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SRCCLKEN0_EN])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SRCCLKEN0_EN, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SRCCLKEN1_EN])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SRCCLKEN1_EN, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SYSCLK0_SRC_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SYSCLK0_SRC_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SYSCLK1_SRC_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SYSCLK1_SRC_MASK_B, val);
	/* SPM_AP_STANDBY_CON */
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_WFI_OP])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_WFI_OP, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_WFI_TYPE])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_WFI_TYPE, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MP0_CPUTOP_IDLE_MASK])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MP0_CPUTOP_IDLE_MASK, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MP1_CPUTOP_IDLE_MASK])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MP1_CPUTOP_IDLE_MASK, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MCUSYS_IDLE_MASK])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MCUSYS_IDLE_MASK, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_APSRC_1_SEL])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_APSRC_1_SEL, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_APSRC_0_SEL])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_APSRC_0_SEL, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CONN_APSRC_SEL])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CONN_APSRC_SEL, val);
	/* SPM_SRC_REQ */
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_APSRC_REQ])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_APSRC_REQ, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_F26M_REQ])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_F26M_REQ, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_INFRA_REQ])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_INFRA_REQ, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_VRF18_REQ])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_VRF18_REQ, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_DDR_EN_REQ])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_DDR_EN_REQ, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_DVFS_REQ])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_DVFS_REQ, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_SW_MAILBOX_REQ])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_SW_MAILBOX_REQ, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_SSPM_MAILBOX_REQ])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_SSPM_MAILBOX_REQ, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_ADSP_MAILBOX_REQ])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_ADSP_MAILBOX_REQ, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_SCP_MAILBOX_REQ])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_SCP_MAILBOX_REQ, val);
	/* SPM_SRC_MASK */
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_SRCCLKENA_0_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_SRCCLKENA_0_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_SRCCLKENA2INFRA_REQ_0_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_SRCCLKENA2INFRA_REQ_0_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_APSRC2INFRA_REQ_0_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_APSRC2INFRA_REQ_0_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_APSRC_REQ_0_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_APSRC_REQ_0_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_VRF18_REQ_0_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_VRF18_REQ_0_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_DDR_EN_0_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_DDR_EN_0_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_SRCCLKENA_1_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_SRCCLKENA_1_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_SRCCLKENA2INFRA_REQ_1_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_SRCCLKENA2INFRA_REQ_1_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_APSRC2INFRA_REQ_1_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_APSRC2INFRA_REQ_1_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_APSRC_REQ_1_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_APSRC_REQ_1_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_VRF18_REQ_1_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_VRF18_REQ_1_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD_DDR_EN_1_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD_DDR_EN_1_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CONN_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CONN_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CONN_SRCCLKENB_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CONN_SRCCLKENB_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CONN_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CONN_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CONN_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CONN_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CONN_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CONN_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CONN_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CONN_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CONN_VFE28_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CONN_VFE28_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SRCCLKENI0_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SRCCLKENI0_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SRCCLKENI0_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SRCCLKENI0_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SRCCLKENI1_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SRCCLKENI1_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SRCCLKENI1_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SRCCLKENI1_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SRCCLKENI2_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SRCCLKENI2_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SRCCLKENI2_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SRCCLKENI2_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_INFRASYS_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_INFRASYS_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_INFRASYS_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_INFRASYS_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD32_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD32_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD32_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD32_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD32_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD32_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD32_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD32_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MD32_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MD32_DDR_EN_MASK_B, val);
	/* SPM_SRC2_MASK */
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SCP_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SCP_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SCP_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SCP_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SCP_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SCP_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SCP_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SCP_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SCP_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SCP_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_AUDIO_DSP_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_AUDIO_DSP_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_AUDIO_DSP_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_AUDIO_DSP_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_AUDIO_DSP_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_AUDIO_DSP_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_AUDIO_DSP_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_AUDIO_DSP_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_AUDIO_DSP_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_AUDIO_DSP_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_UFS_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_UFS_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_UFS_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_UFS_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_UFS_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_UFS_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_UFS_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_UFS_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_UFS_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_UFS_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_DISP0_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_DISP0_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_DISP0_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_DISP0_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_DISP1_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_DISP1_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_DISP1_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_DISP1_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_GCE_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_GCE_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_GCE_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_GCE_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_GCE_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_GCE_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_GCE_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_GCE_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_APU_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_APU_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_APU_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_APU_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_APU_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_APU_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_APU_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_APU_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_APU_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_APU_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CG_CHECK_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CG_CHECK_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CG_CHECK_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CG_CHECK_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CG_CHECK_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CG_CHECK_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CG_CHECK_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CG_CHECK_DDR_EN_MASK_B, val);
	/* SPM_SRC3_MASK */
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_DVFSRC_EVENT_TRIGGER_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_DVFSRC_EVENT_TRIGGER_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SW2SPM_INT0_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SW2SPM_INT0_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SW2SPM_INT1_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SW2SPM_INT1_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SW2SPM_INT2_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SW2SPM_INT2_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SW2SPM_INT3_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SW2SPM_INT3_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SC_ADSP2SPM_WAKEUP_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SC_ADSP2SPM_WAKEUP_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SC_SSPM2SPM_WAKEUP_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SC_SSPM2SPM_WAKEUP_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SC_SCP2SPM_WAKEUP_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SC_SCP2SPM_WAKEUP_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CSYSPWRREQ_MASK])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CSYSPWRREQ_MASK, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_SRCCLKENA_RESERVED_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_SRCCLKENA_RESERVED_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_INFRA_REQ_RESERVED_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_INFRA_REQ_RESERVED_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_APSRC_REQ_RESERVED_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_APSRC_REQ_RESERVED_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_VRF18_REQ_RESERVED_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_VRF18_REQ_RESERVED_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_SPM_DDR_EN_RESERVED_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_SPM_DDR_EN_RESERVED_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MCUPM_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MCUPM_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MCUPM_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MCUPM_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MCUPM_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MCUPM_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MCUPM_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MCUPM_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MCUPM_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MCUPM_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MSDC0_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MSDC0_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MSDC0_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MSDC0_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MSDC0_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MSDC0_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MSDC0_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MSDC0_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MSDC0_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MSDC0_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MSDC1_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MSDC1_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MSDC1_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MSDC1_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MSDC1_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MSDC1_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MSDC1_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MSDC1_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MSDC1_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MSDC1_DDR_EN_MASK_B, val);
	/* SPM_SRC4_MASK */
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_CCIF_EVENT_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
					PW_CCIF_EVENT_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_BAK_PSRI_SRCCLKENA_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_BAK_PSRI_SRCCLKENA_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_BAK_PSRI_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_BAK_PSRI_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_BAK_PSRI_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_BAK_PSRI_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_BAK_PSRI_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_BAK_PSRI_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_BAK_PSRI_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_BAK_PSRI_DDR_EN_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_DRAMC0_MD32_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_DRAMC0_MD32_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_DRAMC0_MD32_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_DRAMC0_MD32_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_DRAMC1_MD32_INFRA_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_DRAMC1_MD32_INFRA_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_DRAMC1_MD32_VRF18_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_DRAMC1_MD32_VRF18_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_CONN_SRCCLKENB2PWRAP_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_CONN_SRCCLKENB2PWRAP_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_DRAMC0_MD32_WAKEUP_MASK])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_DRAMC0_MD32_WAKEUP_MASK, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_DRAMC1_MD32_WAKEUP_MASK])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_DRAMC1_MD32_WAKEUP_MASK, val);
	/* SPM_SRC5_MASK */
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MCUSYS_MERGE_APSRC_REQ_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MCUSYS_MERGE_APSRC_REQ_MASK_B, val);
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_MCUSYS_MERGE_DDR_EN_MASK_B])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_MCUSYS_MERGE_DDR_EN_MASK_B, val);
	/* SPM_WAKEUP_EVENT_MASK */
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_WAKEUP_EVENT_MASK])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
				PW_REG_WAKEUP_EVENT_MASK, val);
	/* SPM_WAKEUP_EVENT_EXT_MASK */
	} else if (!strcmp(cmd,
		mt6885_pwr_ctrl_str[PW_REG_EXT_WAKEUP_EVENT_MASK])) {
		mtk_lpm_smc_spm_dbg(id, MT_LPM_SMC_ACT_SET,
...
static const struct mtk_spm_sysfs_op mt6885_spm_res_rq_fops = {
