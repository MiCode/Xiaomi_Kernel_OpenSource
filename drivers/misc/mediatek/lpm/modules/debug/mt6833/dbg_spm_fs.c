...
static ssize_t
mt6833_generic_spm_read(char *ToUserBuf, size_t sz, void *priv);

static ssize_t
mt6833_generic_spm_write(char *FromUserBuf, size_t sz, void *priv);

struct mt6833_SPM_ENTERY {
	const char *name;
	int mode;
	struct mtk_lp_sysfs_handle handle;
};

struct mt6833_SPM_NODE {
	const char *name;
	int mode;
	struct mtk_lp_sysfs_handle handle;
...
struct mt6833_SPM_ENTERY mt6833_spm_root = {
	.name = "power",
	.mode = 0644,
};

struct mt6833_SPM_NODE mt6833_spm_idle = {
	.name = "idle_ctrl",
	.mode = 0644,
	.op = {
		.fs_read = mt6833_generic_spm_read,
		.fs_write = mt6833_generic_spm_write,
		.priv = (void *)&mt6833_spm_idle,
	},
};

struct mt6833_SPM_NODE mt6833_spm_suspend = {
	.name = "suspend_ctrl",
	.mode = 0644,
	.op = {
		.fs_read = mt6833_generic_spm_read,
		.fs_write = mt6833_generic_spm_write,
		.priv = (void *)&mt6833_spm_suspend,
	},
};
...
mt6833_generic_spm_read(char *ToUserBuf, size_t sz, void *priv)
{
	...
	if (priv == &mt6833_spm_idle)
		id = MT_SPM_DBG_SMC_UID_IDLE_PWR_CTRL;
	return mt6833_show_pwr_ctrl(id, ToUserBuf, sz);
}
...
mt6833_generic_spm_write(char *FromUserBuf, size_t sz, void *priv)
{
	...
	return mt6833_store_pwr_ctrl(id, FromUserBuf, sz);
}
...
int lpm_spm_fs_init(void)
{
	...
	r = mtk_lp_sysfs_entry_func_create(mt6833_spm_root.name,
					   mt6833_spm_root.mode, NULL,
					   &mt6833_spm_root.handle);
	if (!r) {
		mtk_lp_sysfs_entry_func_node_add(mt6833_spm_suspend.name,
						mt6833_spm_suspend.mode,
						&mt6833_spm_suspend.op,
						&mt6833_spm_root.handle,
						&mt6833_spm_suspend.handle);

		mtk_lp_sysfs_entry_func_node_add(mt6833_spm_idle.name,
						 mt6833_spm_idle.mode,
						 &mt6833_spm_idle.op,
						 &mt6833_spm_root.handle,
						 &mt6833_spm_idle.handle);
	}
