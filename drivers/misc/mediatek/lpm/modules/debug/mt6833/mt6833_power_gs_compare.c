...
int mt6833_pwr_gs_set(unsigned int type, const struct mtk_lpm_data *val)
{
	int ret = 0;
...
struct mtk_lpm_callee mt6833_pwr_gs_callee = {
...
#ifdef MTK_LPM_GS_PLAT_CLK_DUMP_SUPPORT
struct mtk_lpm_gs_clk mt6833_clk_cg = {
	.type = GS_CG,
	.name = "CG",
};

struct mtk_lpm_gs_clk mt6833_clk_dcm = {
	.type = GS_DCM,
	.name = "DCM",
};

struct mtk_lpm_gs_clk *mt6833_clks[] = {
	&mt6833_clk_dcm,
	&mt6833_clk_cg,
	NULL,
};

int mt6833_power_gs_clk_user_attach(struct mtk_lpm_gs_clk *p)
{
...
struct mtk_lpm_gs_clk_info mt6833_clk_infos = {
...
/* PMIC */
struct mtk_lpm_gs_pmic mt6833_pmic6365 = {
	.type = GS_PMIC,
	.regulator = "vcore",
	.pwr_domain = "6365",
};

struct mtk_lpm_gs_pmic mt6833_pmic6315 = {
	.type = GS_PMIC,
	.regulator = "3_vbuck1",
	.pwr_domain = "6315",
};

struct mtk_lpm_gs_pmic *mt6833_pmic[] = {
	&mt6833_pmic6365,
	&mt6833_pmic6315,
	NULL,
};

int mt6833_power_gs_pmic_user_attach(struct mtk_lpm_gs_pmic *p)
{
	if (!p || !p->regulator)
		return -EINVAL;
...
struct mtk_lpm_gs_pmic_info mt6833_pmic_infos = {
	.pmic = mt6833_pmic,
	.attach = mt6833_power_gs_pmic_user_attach,
};

int mt6833_power_gs_init(void)
{
	mtk_lpm_callee_registry(&mt6833_pwr_gs_callee);

	/* initial gs compare method */
	mtk_lpm_pwr_gs_common_init();

	/* initial gs pmic information */
	mtk_lpm_pwr_gs_compare_init(MTK_LPM_GS_CMP_PMIC, &mt6833_pmic_infos);

#ifdef MTK_LPM_GS_PLAT_CLK_DUMP_SUPPORT
	/* initial gs dcm information */
	mtk_lpm_pwr_gs_compare_init(MTK_LPM_GS_CMP_CLK, &mt6833_clk_infos);
#endif
	return 0;
}

int mt6833_power_gs_deinit(void)
{
