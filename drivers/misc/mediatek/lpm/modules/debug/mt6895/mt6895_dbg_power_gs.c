...
struct lpm_gs_clk mt6895_clk_dcm = {
	.type = GS_DCM,
	.name = "DCM",
};
struct lpm_gs_clk *mt6895_clks[] = {
	&mt6895_clk_dcm,
	&mt6895_clk_cg,
	NULL,
};
int mt6895_power_gs_clk_user_attach(struct lpm_gs_clk *p)
{
...
struct lpm_gs_clk_info mt6895_clk_infos = {
...
struct lpm_gs_pmic mt6895_pmic6368 = {
	.type = GS_PMIC,
...
struct lpm_gs_pmic *mt6895_pmic[] = {
	&mt6895_pmic6363,
	&mt6895_pmic6368,
	NULL,
};
int mt6895_power_gs_pmic_user_attach(struct lpm_gs_pmic *p)
{
	if (!p || !p->regulator)
		return -EINVAL;
	/* Set compare golden setting for scenario */
...
struct lpm_gs_pmic_info mt6895_pmic_infos = {
	.pmic = mt6895_pmic,
	.attach = mt6895_power_gs_pmic_user_attach,
};

int mt6895_power_gs_init(void)
{
	lpm_callee_registry(&mt6895_pwr_gs_callee);
	/* initial gs compare method */
	lpm_pwr_gs_common_init();
	/* initial gs pmic information */
	lpm_pwr_gs_compare_init(LPM_GS_CMP_PMIC, &mt6895_pmic_infos);
#ifdef MTK_LPM_GS_PLAT_CLK_DUMP_SUPPORT
	/* initial gs dcm information */
	lpm_pwr_gs_compare_init(LPM_GS_CMP_CLK, &mt6895_clk_infos);
#endif
	return 0;
}

void mt6895_power_gs_deinit(void)
{
