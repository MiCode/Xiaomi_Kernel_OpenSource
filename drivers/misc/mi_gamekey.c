...
static int power_init(struct gamekey_core *core, bool enable)
{
	int ret = -1;
	struct regulator *vreg;
	struct device *dev = core->dev;
	static bool regulator_xm_vote = false;
	...
	if (enable) {
		if (regulator_is_enabled(vreg)) {
			...
			regulator_xm_vote = true;
		} else {
			pr_err("gamekey_vreg has been turned on once!\n");
		}
		ret = regulator_get_voltage(vreg);
		pr_info("%s regulator_value %d!\n", __func__,ret);
	} else {
		if (regulator_xm_vote) {
			ret = regulator_disable(vreg);
			if (ret < 0) {
				pr_err("error disabling gamekey_vreg!\n");
				goto rg_err;
			}
			regulator_xm_vote = false;
		}
	}
