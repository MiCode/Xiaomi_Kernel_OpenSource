...
static struct audio_service_info	audio_service;
							...
							void __user *arg)
{
	...
	case AUDIO_SERVICE_PRESET_SET:
		...
		break;
	case AUDIO_SERVICE_ULTRASOUND_SET:
	{
		uint32_t value = 0;
		...
		pr_info("AUDIO_SERVICE_ULTRASOUND_SET, value:%d\n", value);
		break;
	}
	case AUDIO_SERVICE_ULTRASOUND_GET:
		break;
	default:
...
#define AUDIO_SERVICE_PRESET_SET32 _IOWR(AUDIO_SERVICE_IOCTL_MAGIC, \
							3, compat_uptr_t)
#define AUDIO_SERVICE_ULTRASOUND_SET32 _IOWR(AUDIO_SERVICE_IOCTL_MAGIC, \
							4, compat_uptr_t)
#define AUDIO_SERVICE_ULTRASOUND_GET32 _IOWR(AUDIO_SERVICE_IOCTL_MAGIC, \
							5, compat_uptr_t)
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_SERVICE_PRESET_SET32:
		...
		break;
	case AUDIO_SERVICE_ULTRASOUND_SET32:
		cmd64 = AUDIO_SERVICE_ULTRASOUND_SET;
		break;
	case AUDIO_SERVICE_ULTRASOUND_GET32:
		cmd64 = AUDIO_SERVICE_ULTRASOUND_GET;
		break;
	default:
		pr_err("%s: ioctl not found!\n", __func__);
		ret = -EFAULT;
...
static int __init audio_service_init(void)
{
	...
	mutex_init(&audio_service.common_lock);
