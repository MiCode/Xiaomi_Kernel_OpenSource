...
DECLARE_PER_CPU(unsigned long[PID_MAX_DEFAULT], task_hist_nivcsw);
DECLARE_PER_CPU(int, sched_load_usf);
			    ...
			    unsigned long val, void *data)
{
	...
	if (evdata->data && val == MI_DRM_EVENT_BLANK) {
		...
		if (is_set_sched_dbg)
			trace_printk("%s : usf_is_screen_on:%d\n",
				 __func__, usf_is_screen_on);
	}
	return NOTIFY_OK;
}
#elif CONFIG_FB
...
#ifdef CONFIG_DRM
	res = mi_drm_register_client(&usf_lcd_nb);
#elif CONFIG_FB
...
{
#ifdef CONFIG_DRM
	mi_drm_unregister_client(&usf_lcd_nb);
#elif CONFIG_FB
