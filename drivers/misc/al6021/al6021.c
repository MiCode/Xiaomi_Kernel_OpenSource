			...
			struct device_attribute *attr, char *buf)
{
	...
	reinit_completion(&data->completion_irq_cause);
...
static ssize_t al6021_event_set(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t size)
{
	struct al6021_data_info *data = dev_get_drvdata(dev);
	...
	ret = kstrtoint(buf, 0, &value);
	data->event_id = value;
	pr_i("al6021 set event_id = %d", value);
	complete_all(&data->completion_irq_cause);
	mutex_unlock(&(data->mutex_devfile));

	return size;
}
static DEVICE_ATTR(event, 0444, al6021_event_get, al6021_event_set);
			...
			const char *buf, size_t size)
{
	...
	if (!strncmp(buf, "fake-intr", size-1))
...
static int al6021_devfile_open(struct inode *inode, struct file *filp)
{
	int ret = 0;
...
static int al6021_devfile_release(struct inode *inode, struct file *filp)
{
	int ret = 0;
...
static ssize_t al6021_devfile_read(struct file *filp, char *buf, size_t len, loff_t *fseek)
{
	struct al6021_data_info *data = filp->private_data;
	uint32_t event_id;
	int count = 0;

	mutex_lock(&data->mutex_devfile);
	event_id = data->event_id;
	pr_i("al6021_read, event_id = %d\n", event_id);
	data->event_id = 0;
	if (copy_to_user(buf, &event_id, sizeof(uint32_t)) != 0) {
            pr_e("Copy to user space failed!\n");
            mutex_unlock(&data->mutex_devfile);
            return -EFAULT;
	}
...
static ssize_t al6021_devfile_write(struct file *filp, const char *buf, size_t len, loff_t *fseek)
{
	int ret = 0;
...
static unsigned int al6021_devfile_poll(struct file *filp, poll_table *wait)
{
	int ret = 0;
	unsigned int mask = 0;
	struct al6021_data_info *data = filp->private_data;

	pr_i("al6021 poll enter");

	reinit_completion(&data->completion_irq_cause);
	ret = wait_for_completion_interruptible(&data->completion_irq_cause);
	if (ret) {
		pr_e("al6021 poll interrupted");
		return -ERESTARTSYS;
	}
	pr_i("al6021 poll have new event");
...
static const struct file_operations al6021_devfile_ops = {
	...
	.read = al6021_devfile_read,
	.write = al6021_devfile_write,
	.poll = al6021_devfile_poll,
...
static void al6021_irq_control_process(struct al6021_data_info *data)
{
	...
	iop_basic.size = AL6021_SIZEOF_TXN_BASIC + sizeof(iop_basic.preamble);
	//check txlm_b1 frame done status

        // read rx status
        iop_basic.op = AL6021_OP_SPI_READ_REG;
        iop_basic.addr = 0xfff92188;
        iop_basic.data_out[0] = 0x0;
        do_spi_operation(data,&iop_basic,iop_basic.size,&data_in);
        value = *((u32*)data_in);
        pr_i("read reg=0x%08x , value=%x (spi)", iop_basic.addr, value);

        if (value & 0x1FF00000) {
            data->event_id = ISPV2EventMipiRXErr;

            //disable zisp mrx interrupt status
            iop_basic.size = AL6021_SIZEOF_TXN_BASIC;
            iop_basic.op = AL6021_OP_SPI_WRITE_REG;
            iop_basic.addr = 0xffef0028;
            iop_basic.data_out[0] = 0x0;
            do_spi_operation(data,&iop_basic,iop_basic.size,&data_in);
        }

        //clean zisp mrx interrupt status
        iop_basic.size = AL6021_SIZEOF_TXN_BASIC;
        iop_basic.op = AL6021_OP_SPI_WRITE_REG;
        iop_basic.addr = 0xfff92188;
        iop_basic.data_out[0] = 0x1ff6afbd;
        do_spi_operation(data,&iop_basic,iop_basic.size,&data_in);
}
...
irqreturn_t al6021_irq_thread(int irq_num, void *_data)
{
        ...
        data->event_id = ISPV2EventMax;

        al6021_irq_control_process(data);

        if (data->event_id >= ISPV2EventMax) {
            mutex_unlock(&(data->mutex_devfile));
            return IRQ_HANDLED;
        }

        // Trigger all the thread which wait completion_irq_cause
        complete_all(&data->completion_irq_cause);
