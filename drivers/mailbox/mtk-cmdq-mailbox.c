...
{
	struct mbox_chan *c = chan;
	struct cmdq *cmdq = container_of(c->mbox, struct cmdq, mbox);

	return cmdq->addr_shift;
}
	...
	for (i = 0; i < CMDQ_NUM_CMD(task->pkt); i++)
		if (cmdq_command_is_wfe(base[i]))
			base[i] = (u64)CMDQ_JUMP_BY_OFFSET << 32 |
				  (CMDQ_JUMP_PASS >> task->cmdq->addr_shift);
...
static void cmdq_task_handle_error(struct cmdq_task *task)
{
	...
	if (next_task)
		writel(next_task->pa_base >> task->cmdq->addr_shift,
		       thread->base + CMDQ_THR_CURR_ADDR);
				    ...
				    struct cmdq_thread *thread)
{
	...
	curr_pa = readl(thread->base + CMDQ_THR_CURR_ADDR) << cmdq->addr_shift;
...
static int cmdq_mbox_send_data(struct mbox_chan *chan, void *data)
{
	...
	if (list_empty(&thread->task_busy_list)) {
		...
		writel(task->pa_base >> cmdq->addr_shift,
		       thread->base + CMDQ_THR_CURR_ADDR);
		writel((task->pa_base + pkt->cmd_buf_size) >> cmdq->addr_shift,
...
static int cmdq_probe(struct platform_device *pdev)
{
	...
	dev_dbg(dev, "cmdq thread:%u shift:%u\n",
		plat_data->thread_nr, plat_data->shift);

	cmdq->thread_nr = plat_data->thread_nr;
	cmdq->addr_shift = plat_data->shift;
