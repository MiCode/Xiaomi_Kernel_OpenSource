/*
 * STMicroelectronics st_sensor_hub gyro_uncalibscope driver
 *
...
#define ST_HUB_GYRO_UNCALIB_NUM_DATA_CH		3
#define ST_HUB_GYRO_UNCALIB_DATA_BYTE		2

static const struct iio_chan_spec st_hub_gyro_uncalib_ch[] = {
	ST_HUB_DEVICE_CHANNEL(IIO_ANGL_VEL, 0, 1, IIO_MOD_X, IIO_LE, 32, 32,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE)
				| BIT(IIO_CHAN_INFO_OFFSET), 0, 0, 's'),
	ST_HUB_DEVICE_CHANNEL(IIO_ANGL_VEL, 1, 1, IIO_MOD_Y, IIO_LE, 32, 32,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE)
				| BIT(IIO_CHAN_INFO_OFFSET), 0, 0, 's'),
	ST_HUB_DEVICE_CHANNEL(IIO_ANGL_VEL, 2, 1, IIO_MOD_Z, IIO_LE, 32, 32,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE)
				| BIT(IIO_CHAN_INFO_OFFSET), 0, 0, 's'),
...
static void st_hub_gyro_uncalib_push_data(struct platform_device *pdev,
						u8 *data, int64_t timestamp)
{
	...
	struct st_hub_sensor_data *gudata = iio_priv(indio_dev);

	for (i = 0; i < ST_HUB_GYRO_UNCALIB_NUM_DATA_CH; i++) {
		byte_for_channel =
			indio_dev->channels[i].scan_type.storagebits >> 3;
		if (test_bit(i, indio_dev->active_scan_mask)) {
			memcpy(&gudata->buffer[init_copy],
				...
				unsigned int index, int *data, long mask)
{
	...
	case IIO_CHAN_INFO_OFFSET:
		*data = (s16)get_unaligned_le16(&outdata[byte_for_channel *
					index + ST_HUB_GYRO_UNCALIB_DATA_BYTE]);
		break;
	default:
		break;
	}
...
static int st_hub_gyro_uncalib_read_raw(struct iio_dev *indio_dev,
...
static struct attribute *st_hub_gyro_uncalib_attributes[] = {
...
static const struct attribute_group st_hub_gyro_uncalib_attribute_group = {
	.attrs = st_hub_gyro_uncalib_attributes,
};

static const struct iio_info st_hub_gyro_uncalib_info = {
	.driver_module = THIS_MODULE,
	.attrs = &st_hub_gyro_uncalib_attribute_group,
	.read_raw = &st_hub_gyro_uncalib_read_raw,
};
...
static int st_hub_gyro_uncalib_probe(struct platform_device *pdev)
{
	int err;
	...
	indio_dev->channels = st_hub_gyro_uncalib_ch;
	indio_dev->num_channels = ARRAY_SIZE(st_hub_gyro_uncalib_ch);
	indio_dev->dev.parent = &pdev->dev;
	indio_dev->info = &st_hub_gyro_uncalib_info;
	...
	callback.pdev = pdev;
	callback.push_data = &st_hub_gyro_uncalib_push_data;
...
static int st_hub_gyro_uncalib_remove(struct platform_device *pdev)
{
	struct iio_dev *indio_dev = platform_get_drvdata(pdev);
	struct st_hub_sensor_data *gdata = iio_priv(indio_dev);
...
static struct platform_device_id st_hub_gyro_uncalib_ids[] = {
	{ CONCATENATE_STRING(LIS331EB_DEV_NAME, "gyro_u") },
	{ CONCATENATE_STRING(LSM6DB0_DEV_NAME, "gyro_u") },
	{ CONCATENATE_STRING(LIS331EB_DEV_NAME, "gyro_u_wk") },
	{ CONCATENATE_STRING(LSM6DB0_DEV_NAME, "gyro_u_wk") },
	{},
};
MODULE_DEVICE_TABLE(platform, st_hub_gyro_uncalib_ids);

static struct platform_driver st_hub_gyro_uncalib_platform_driver = {
	.id_table = st_hub_gyro_uncalib_ids,
	.driver = {
		.name	= KBUILD_MODNAME,
		.owner	= THIS_MODULE,
	},
	.probe		= st_hub_gyro_uncalib_probe,
	.remove		= st_hub_gyro_uncalib_remove,
};
module_platform_driver(st_hub_gyro_uncalib_platform_driver);

MODULE_AUTHOR("Denis Ciocca <denis.ciocca@st.com>");
MODULE_DESCRIPTION("STMicroelectronics sensor-hub gyro uncalibrated driver");
