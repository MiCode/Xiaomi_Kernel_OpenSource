				...
				unsigned int index, int *data, long mask)
{
	...
	err = st_hub_set_enable(info->client, info->index, true, true, 0);
	if (err < 0)
		goto st_hub_read_axis_free_memory;

	err = st_hub_read_axis_data_asincronous(info->client, info->index,
					outdata, gdata->cdata->payload_byte);
	if (err < 0)
		goto st_hub_read_axis_free_memory;

	err = st_hub_set_enable(info->client, info->index, false, true, 0);
	if (err < 0)
		goto st_hub_read_axis_free_memory;
...
static int st_hub_magn_uncalib_probe(struct platform_device *pdev)
{
	...
	st_hub_get_common_data(info->client, info->index, &gdata->cdata);
