/*
 * STMicroelectronics st_sensor_hub accelerometer driver
 *
...
#define ST_HUB_ACCEL_NUM_DATA_CH		3

static const struct iio_chan_spec st_hub_accel_ch[] = {
	ST_HUB_DEVICE_CHANNEL(IIO_ACCEL, 0, 1, IIO_MOD_X, IIO_LE, 16, 16,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE), 0, 0, 's'),
	ST_HUB_DEVICE_CHANNEL(IIO_ACCEL, 1, 1, IIO_MOD_Y, IIO_LE, 16, 16,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE), 0, 0, 's'),
	ST_HUB_DEVICE_CHANNEL(IIO_ACCEL, 2, 1, IIO_MOD_Z, IIO_LE, 16, 16,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE), 0, 0, 's'),
...
static void st_hub_accel_push_data(struct platform_device *pdev,
						u8 *data, int64_t timestamp)
{
	...
	for (i = 0; i < ST_HUB_ACCEL_NUM_DATA_CH; i++) {
		byte_for_channel =
			indio_dev->channels[i].scan_type.storagebits >> 3;
		if (test_bit(i, indio_dev->active_scan_mask)) {
			memcpy(&adata->buffer[init_copy],
						...
						unsigned int index, int *data)
{
	...
	*data = (s16)get_unaligned_le16(&outdata[byte_for_channel * index]);
...
static int st_hub_accel_read_raw(struct iio_dev *indio_dev,
...
static struct attribute *st_hub_accel_attributes[] = {
...
static const struct attribute_group st_hub_accel_attribute_group = {
	.attrs = st_hub_accel_attributes,
};

static const struct iio_info st_hub_accel_info = {
	.driver_module = THIS_MODULE,
	.attrs = &st_hub_accel_attribute_group,
	.read_raw = &st_hub_accel_read_raw,
};
...
static int st_hub_accel_probe(struct platform_device *pdev)
{
	int err;
	...
	indio_dev->channels = st_hub_accel_ch;
	indio_dev->num_channels = ARRAY_SIZE(st_hub_accel_ch);
	indio_dev->dev.parent = &pdev->dev;
	indio_dev->info = &st_hub_accel_info;
	...
	callback.pdev = pdev;
	callback.push_data = &st_hub_accel_push_data;
...
static int st_hub_accel_remove(struct platform_device *pdev)
{
	struct iio_dev *indio_dev = platform_get_drvdata(pdev);
	struct st_hub_sensor_data *adata = iio_priv(indio_dev);
...
static struct platform_device_id st_hub_accel_ids[] = {
	{ CONCATENATE_STRING(LIS331EB_DEV_NAME, "accel") },
	{ CONCATENATE_STRING(LSM6DB0_DEV_NAME, "accel") },
	{ CONCATENATE_STRING(LIS331EB_DEV_NAME, "accel_wk") },
	{ CONCATENATE_STRING(LSM6DB0_DEV_NAME, "accel_wk") },
	{},
};
MODULE_DEVICE_TABLE(platform, st_hub_accel_ids);

static struct platform_driver st_hub_accel_platform_driver = {
	.id_table = st_hub_accel_ids,
	.driver = {
		.name	= KBUILD_MODNAME,
		.owner	= THIS_MODULE,
	},
	.probe		= st_hub_accel_probe,
	.remove		= st_hub_accel_remove,
};
module_platform_driver(st_hub_accel_platform_driver);

MODULE_AUTHOR("Denis Ciocca <denis.ciocca@st.com>");
MODULE_DESCRIPTION("STMicroelectronics sensor-hub accelerometers driver");
