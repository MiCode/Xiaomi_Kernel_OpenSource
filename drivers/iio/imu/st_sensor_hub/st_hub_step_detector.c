...
static int st_hub_step_detector_buffer_postenable(struct iio_dev *indio_dev)
{
	int err;
...
static int st_hub_step_detector_buffer_predisable(struct iio_dev *indio_dev)
{
	int err;
	struct st_hub_sensor_data *adata = iio_priv(indio_dev);
	struct st_hub_pdata_info *info = indio_dev->dev.parent->platform_data;
...
static const struct iio_buffer_setup_ops st_hub_buffer_setup_ops = {
	...
	.postenable = &st_hub_step_detector_buffer_postenable,
	.predisable = &st_hub_step_detector_buffer_predisable,
};
