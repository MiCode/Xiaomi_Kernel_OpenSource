...
#define ST_HUB_STEP_COUNTER_NUM_DATA_CH		1
						...
						u8 *data, int64_t timestamp)
{
	...
	memcpy(adata->buffer, data, byte_for_channel);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	err = st_hub_send(info->client, command, ARRAY_SIZE(command), true);
	if (err < 0)
		return err;
...
static IIO_DEVICE_ATTR(set_max_delivery_rate, S_IWUSR | S_IRUGO,
...
static struct attribute *st_hub_step_counter_attributes[] = {
	&iio_dev_attr_set_max_delivery_rate.dev_attr.attr,
