		       ...
		       struct tee_shm *device_shm, u32 *shm_size)
{
	...
	ret = mitee_client_invoke_func(ctx, &inv_arg, param);
...
int optee_enumerate_devices(void)
{
	...
	ctx = mitee_client_open_context(NULL, optee_ctx_match, NULL, NULL);
	if (IS_ERR(ctx))
		return -ENODEV;
	...
	rc = mitee_client_open_session(ctx, &sess_arg, NULL);
...
out_sess:
	mitee_client_close_session(ctx, sess_arg.session);
out_ctx:
	mitee_client_close_context(ctx);
