		      ...
		      struct optee_call_ctx *call_ctx)
{
	...
	if (DEBUG_RPC)
		pr_err("optee_handle_rpc in\n");
	switch (OPTEE_SMC_RETURN_GET_RPC_FUNC(param->a0)) {
	case OPTEE_SMC_RPC_FUNC_ALLOC:
		if (DEBUG_RPC)
			pr_err("alloc shm rpc call size=%08x\n", param->a1);
		...
		if (!IS_ERR(shm) && !tee_shm_get_pa(shm, 0, &pa)) {
			...
			reg_pair_from_64(&param->a4, &param->a5,
					 (unsigned long)shm);
			if (DEBUG_RPC)
				pr_err("alloc shm success rpc call shmptr=%x a4=%x a5=%x\n", (unsigned long)shm, param->a4, param->a5);
		} else {
			param->a1 = 0;
			param->a2 = 0;
			param->a4 = 0;
			param->a5 = 0;
			if (DEBUG_RPC)
				pr_err("alloc shm failed rpc call\n");
		}
		break;
