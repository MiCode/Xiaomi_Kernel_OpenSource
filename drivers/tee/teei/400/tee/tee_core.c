...
static int tee_k_ioctl_shm_alloc(struct tee_context *ctx,
...
long tee_k_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case TEE_IOC_SHM_ALLOC:
		return tee_k_ioctl_shm_alloc(ctx, (void *)arg);
	case TEE_IOC_OPEN_SESSION:
		return tee_k_ioctl_open_session(ctx, (void *)arg);
	case TEE_IOC_INVOKE:
		return tee_k_ioctl_invoke(ctx, (void *)arg);
	case TEE_IOC_CANCEL:
		return tee_k_ioctl_cancel(ctx, (void *)arg);
	case TEE_IOC_CLOSE_SESSION:
		return tee_k_ioctl_close_session(ctx, (void *)arg);
	case TEE_IOC_SET_HOSTNAME:
		return tee_k_ioctl_set_hostname(ctx, (void *)arg);
	default:
