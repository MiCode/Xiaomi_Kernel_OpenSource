...
struct clk_hw *gpu_cc_sdm855_hws[] = {
...
static struct clk_regmap *gpu_cc_sdm855_clocks[] = {
...
static const struct qcom_reset_map gpu_cc_sdm855_resets[] = {
...
static const struct regmap_config gpu_cc_sdm855_regmap_config = {
	.reg_bits	= 32,
	.reg_stride	= 4,
	.val_bits	= 32,
	.max_register	= 0x8008,
...
static const struct qcom_cc_desc gpu_cc_sdm855_desc = {
	.config = &gpu_cc_sdm855_regmap_config,
	.clks = gpu_cc_sdm855_clocks,
	.num_clks = ARRAY_SIZE(gpu_cc_sdm855_clocks),
	.resets = gpu_cc_sdm855_resets,
	.num_resets = ARRAY_SIZE(gpu_cc_sdm855_resets),
};
...
static int gpu_cc_sdm855_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &gpu_cc_sdm855_desc);
	if (IS_ERR(regmap))
	...
	for (i = 0; i < ARRAY_SIZE(gpu_cc_sdm855_hws); i++) {
		clk = devm_clk_register(&pdev->dev, gpu_cc_sdm855_hws[i]);
		if (IS_ERR(clk))
			return PTR_ERR(clk);
	}
...
static struct platform_driver gpu_cc_sdm855_driver = {
	.probe		= gpu_cc_sdm855_probe,
	.driver		= {
		.name	= "gpu_cc-sdm855",
		.of_match_table = gpu_cc_sdm855_match_table,
	},
};

static int __init gpu_cc_sdm855_init(void)
{
	return platform_driver_register(&gpu_cc_sdm855_driver);
}
subsys_initcall(gpu_cc_sdm855_init);

static void __exit gpu_cc_sdm855_exit(void)
{
	platform_driver_unregister(&gpu_cc_sdm855_driver);
}
module_exit(gpu_cc_sdm855_exit);

MODULE_DESCRIPTION("QTI GPU_CC SDM855 Driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:gpu_cc-sdm855");
