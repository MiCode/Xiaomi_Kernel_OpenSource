...
static struct clk_rcg2 gcc_emac0_ptp_clk_src = {
	...
	.clkr.vdd_data = {
		...
		.num_vdd_classes = ARRAY_SIZE(gcc_sdxpinn_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 75000000,
			[VDD_LOW] = 125000000,
...
static struct clk_branch gcc_mvm_ahb_clk = {
	.halt_reg = 0x6f008,
	.halt_check = BRANCH_HALT,
	.clkr = {
		.enable_reg = 0x6f008,
		.enable_mask = BIT(0),
		.hw.init = &(const struct clk_init_data){
			.name = "gcc_mvm_ahb_clk",
			.ops = &clk_branch2_ops,
		},
	},
};

static struct clk_branch gcc_mvm_master_axi_clk = {
	.halt_reg = 0x6f004,
	.halt_check = BRANCH_HALT_VOTED,
	.hwcg_reg = 0x6f004,
	.hwcg_bit = 1,
	.clkr = {
		...
		.enable_mask = BIT(0),
		.hw.init = &(const struct clk_init_data){
			.name = "gcc_mvm_master_axi_clk",
			.ops = &clk_branch2_ops,
		},
	},
};

static struct clk_branch gcc_mvmss_nts_clk = {
	.halt_reg = 0x6f010,
	.halt_check = BRANCH_HALT_VOTED,
	.hwcg_reg = 0x6f010,
	.hwcg_bit = 1,
...
static struct clk_branch gcc_sys_noc_mvmss_clk = {
	.halt_reg = 0x6f018,
	.halt_check = BRANCH_HALT_VOTED,
	.hwcg_reg = 0x6f018,
	.hwcg_bit = 1,
...
static struct clk_regmap *gcc_sdxpinn_clocks[] = {
	...
	[GCC_MVM_AHB_CLK] = &gcc_mvm_ahb_clk.clkr,
	[GCC_MVM_MASTER_AXI_CLK] = &gcc_mvm_master_axi_clk.clkr,
	[GCC_MVMSS_NTS_CLK] = &gcc_mvmss_nts_clk.clkr,
...
static const struct regmap_config gcc_sdxpinn_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_sdxpinn_desc = {
	.config = &gcc_sdxpinn_regmap_config,
	.clks = gcc_sdxpinn_clocks,
	.num_clks = ARRAY_SIZE(gcc_sdxpinn_clocks),
	.resets = gcc_sdxpinn_resets,
	.num_resets = ARRAY_SIZE(gcc_sdxpinn_resets),
	.clk_regulators = gcc_sdxpinn_regulators,
	.num_clk_regulators = ARRAY_SIZE(gcc_sdxpinn_regulators),
};

static const struct of_device_id gcc_sdxpinn_match_table[] = {
	{ .compatible = "qcom,sdxpinn-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_sdxpinn_match_table);

static int gcc_sdxpinn_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_sdxpinn_desc);
	if (IS_ERR(regmap))
	...
	regmap_update_bits(regmap, 0x45008, BIT(0), BIT(0));
	regmap_update_bits(regmap, 0x3e008, BIT(0), BIT(0));

	ret = qcom_cc_really_probe(pdev, &gcc_sdxpinn_desc, regmap);
	if (ret) {
...
static void gcc_sdxpinn_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_sdxpinn_desc);
}

static struct platform_driver gcc_sdxpinn_driver = {
	.probe = gcc_sdxpinn_probe,
	.driver = {
		.name = "gcc-sdxpinn",
		.of_match_table = gcc_sdxpinn_match_table,
		.sync_state = gcc_sdxpinn_sync_state,
	},
};

static int __init gcc_sdxpinn_init(void)
{
...
{
	platform_driver_unregister(&gcc_sdxpinn_driver);
}
module_exit(gcc_sdxpinn_exit);

MODULE_DESCRIPTION("QTI GCC SDXPINN Driver");
