...
static struct clk_vdd_class *gcc_yupik_regulators[] = {
	&vdd_cx,
};
...
static const struct regmap_config gcc_yupik_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct of_device_id gcc_yupik_match_table[] = {
	{ .compatible = "qcom,yupik-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_yupik_match_table);

static int gcc_yupik_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_yupik_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gcc_yupik_desc, regmap);
	if (ret) {
...
static void gcc_yupik_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_yupik_desc);
}

static struct platform_driver gcc_yupik_driver = {
	.probe = gcc_yupik_probe,
	.driver = {
		.name = "gcc-yupik",
...
static int __init gcc_yupik_init(void)
{
	return platform_driver_register(&gcc_yupik_driver);
}
subsys_initcall(gcc_yupik_init);

static void __exit gcc_yupik_exit(void)
{
	platform_driver_unregister(&gcc_yupik_driver);
}
module_exit(gcc_yupik_exit);

MODULE_DESCRIPTION("QTI GCC YUPIK Driver");
