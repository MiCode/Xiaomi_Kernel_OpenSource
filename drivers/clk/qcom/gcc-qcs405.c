...
static struct clk_regmap *gcc_qcs405_clocks[] = {
...
static const struct qcom_cc_desc gcc_qcs405_desc = {
	.config = &gcc_qcs405_regmap_config,
	.clks = gcc_qcs405_clocks,
	.num_clks = ARRAY_SIZE(gcc_qcs405_clocks),
	.resets = gcc_qcs405_resets,
	.num_resets = ARRAY_SIZE(gcc_qcs405_resets),
};

static const struct of_device_id gcc_qcs405_match_table[] = {
	{ .compatible = "qcom,gcc-qcs405" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_qcs405_match_table);

static int gcc_qcs405_probe(struct platform_device *pdev)
{
	struct clk *clk;
	struct regmap *regmap;
	int ret = 0;

	regmap = qcom_cc_map(pdev, &gcc_qcs405_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gcc_qcs405_desc, regmap);
	if (ret) {
...
static struct platform_driver gcc_qcs405_driver = {
	.probe		= gcc_qcs405_probe,
	.driver		= {
		.name	= "gcc-qcs405",
		.of_match_table = gcc_qcs405_match_table,
	},
};

static int __init gcc_qcs405_init(void)
{
	return platform_driver_register(&gcc_qcs405_driver);
}
subsys_initcall(gcc_qcs405_init);

static void __exit gcc_qcs405_exit(void)
{
	platform_driver_unregister(&gcc_qcs405_driver);
}
module_exit(gcc_qcs405_exit);

struct clk_hw *mdss_qcs405_hws[] = {
	[MDSS_MDP_VOTE_CLK] = &mdss_mdp_vote_clk.hw,
	[MDSS_ROTATOR_VOTE_CLK] = &mdss_rotator_vote_clk.hw,
};

static struct clk_regmap *mdss_qcs405_clocks[] = {
...
static const struct qcom_cc_desc mdss_qcs405_desc = {
	.config = &gcc_qcs405_regmap_config,
...
MODULE_DEVICE_TABLE(of, mdss_qcs405_match_table);
...
static int mdss_qcs405_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "Registered MDSS clocks\n");

	return ret;
}

static struct platform_driver mdss_qcs405_driver = {
	.probe		= mdss_qcs405_probe,
	.driver		= {
		.name	= "gcc-mdss-qcs405",
		.of_match_table = mdss_qcs405_match_table,
	},
};

static int __init mdss_qcs405_init(void)
{
	return platform_driver_register(&mdss_qcs405_driver);
}
subsys_initcall(mdss_qcs405_init);

static void __exit mdss_qcs405_exit(void)
{
	platform_driver_unregister(&mdss_qcs405_driver);
}
module_exit(mdss_qcs405_exit);

MODULE_DESCRIPTION("QTI GCC QCS405 Driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:gcc-qcs405");
