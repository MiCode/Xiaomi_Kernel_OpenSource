...
static struct clk_regmap *npu_cc_sdm855_clocks[] = {
...
static const struct qcom_reset_map npu_cc_sdm855_resets[] = {
	[NPU_CC_CAL_DP_BCR] = { 0x1000 },
	[NPU_CC_NPU_CORE_BCR] = { 0x1024 },
};

static const struct regmap_config npu_cc_sdm855_regmap_config = {
	.reg_bits	= 32,
	.reg_stride	= 4,
	.val_bits	= 32,
	.max_register	= 0x8000,
...
static const struct qcom_cc_desc npu_cc_sdm855_desc = {
	.config = &npu_cc_sdm855_regmap_config,
	.clks = npu_cc_sdm855_clocks,
	.num_clks = ARRAY_SIZE(npu_cc_sdm855_clocks),
	.resets = npu_cc_sdm855_resets,
	.num_resets = ARRAY_SIZE(npu_cc_sdm855_resets),
};

static const struct of_device_id npu_cc_sdm855_match_table[] = {
...
static int npu_cc_sdm855_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &npu_cc_sdm855_desc);
	if (IS_ERR(regmap)) {
...
static struct platform_driver npu_cc_sdm855_driver = {
	.probe		= npu_cc_sdm855_probe,
	.driver		= {
		.name	= "npu_cc-sdm855",
		.of_match_table = npu_cc_sdm855_match_table,
	},
};

static int __init npu_cc_sdm855_init(void)
{
	return platform_driver_register(&npu_cc_sdm855_driver);
}
subsys_initcall(npu_cc_sdm855_init);

static void __exit npu_cc_sdm855_exit(void)
{
	platform_driver_unregister(&npu_cc_sdm855_driver);
}
module_exit(npu_cc_sdm855_exit);

MODULE_DESCRIPTION("QTI NPU_CC SDM855 Driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:npu_cc-sdm855");
