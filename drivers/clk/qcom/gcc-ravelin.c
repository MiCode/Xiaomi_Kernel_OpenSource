...
static struct clk_vdd_class *gcc_ravelin_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 gcc_pcie_0_aux_clk_src = {
	...
	.clkr.vdd_data = {
		...
		.num_vdd_classes = ARRAY_SIZE(gcc_ravelin_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 19200000},
	},
...
static struct clk_regmap *gcc_ravelin_clocks[] = {
...
static const struct qcom_reset_map gcc_ravelin_resets[] = {
...
static const struct regmap_config gcc_ravelin_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_ravelin_desc = {
	.config = &gcc_ravelin_regmap_config,
	.clks = gcc_ravelin_clocks,
...
static const struct of_device_id gcc_ravelin_match_table[] = {
	{ .compatible = "qcom,ravelin-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_ravelin_match_table);

static int gcc_ravelin_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_ravelin_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gcc_ravelin_desc, regmap);
	if (ret) {
...
{
	return platform_driver_register(&gcc_ravelin_driver);
}
subsys_initcall(gcc_ravelin_init);

static void __exit gcc_ravelin_exit(void)
{
	platform_driver_unregister(&gcc_ravelin_driver);
}
module_exit(gcc_ravelin_exit);

MODULE_DESCRIPTION("QTI GCC RAVELIN Driver");
