...
static struct clk_vdd_class *gcc_parrot_regulators[] = {
	&vdd_cx,
	&vdd_mxa,
};
...
static struct clk_rcg2 gcc_pcie_0_aux_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = gcc_parrot_regulators,
		.num_vdd_classes = ARRAY_SIZE(gcc_parrot_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 19200000},
	},
...
static struct clk_regmap *gcc_parrot_clocks[] = {
...
static const struct qcom_reset_map gcc_parrot_resets[] = {
...
static const struct regmap_config gcc_parrot_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_parrot_desc = {
	.config = &gcc_parrot_regmap_config,
	.clks = gcc_parrot_clocks,
	.num_clks = ARRAY_SIZE(gcc_parrot_clocks),
	.resets = gcc_parrot_resets,
	.num_resets = ARRAY_SIZE(gcc_parrot_resets),
	.clk_regulators = gcc_parrot_regulators,
	.num_clk_regulators = ARRAY_SIZE(gcc_parrot_regulators),
};

static const struct of_device_id gcc_parrot_match_table[] = {
	{ .compatible = "qcom,parrot-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_parrot_match_table);

static int gcc_parrot_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_parrot_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gcc_parrot_desc, regmap);
	if (ret) {
...
static void gcc_parrot_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_parrot_desc);
}

static struct platform_driver gcc_parrot_driver = {
	.probe = gcc_parrot_probe,
	.driver = {
		.name = "gcc-parrot",
		.of_match_table = gcc_parrot_match_table,
		.sync_state = gcc_parrot_sync_state,
	},
};

static int __init gcc_parrot_init(void)
{
	return platform_driver_register(&gcc_parrot_driver);
}
subsys_initcall(gcc_parrot_init);

static void __exit gcc_parrot_exit(void)
{
	platform_driver_unregister(&gcc_parrot_driver);
}
