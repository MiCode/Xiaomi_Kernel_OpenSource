...
static struct clk_regmap *disp_cc_atoll_clocks[] = {
...
static const struct regmap_config disp_cc_atoll_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc disp_cc_atoll_desc = {
	.config = &disp_cc_atoll_regmap_config,
	.clks = disp_cc_atoll_clocks,
	.num_clks = ARRAY_SIZE(disp_cc_atoll_clocks),
};

static const struct of_device_id disp_cc_atoll_match_table[] = {
	{ .compatible = "qcom,atoll-dispcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, disp_cc_atoll_match_table);

static int disp_cc_atoll_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &disp_cc_atoll_desc);
	if (IS_ERR(regmap)) {
	...
	ret = qcom_cc_really_probe(pdev, &disp_cc_atoll_desc, regmap);
	if (ret) {
...
static struct platform_driver disp_cc_atoll_driver = {
	.probe = disp_cc_atoll_probe,
	.driver = {
		.name = "atoll-dispcc",
...
static int __init disp_cc_atoll_init(void)
{
	return platform_driver_register(&disp_cc_atoll_driver);
}
subsys_initcall(disp_cc_atoll_init);

static void __exit disp_cc_atoll_exit(void)
{
	platform_driver_unregister(&disp_cc_atoll_driver);
}
module_exit(disp_cc_atoll_exit);

MODULE_DESCRIPTION("QTI DISP_CC ATOLL Driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:disp_cc-atoll");
