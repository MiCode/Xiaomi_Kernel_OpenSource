...
static const struct qcom_reset_map gcc_sdm855_resets[] = {
	[GCC_EMAC_BCR] = { 0x6000 },
	[GCC_GPU_BCR] = { 0x71000 },
	[GCC_MMSS_BCR] = { 0xb000 },
	[GCC_NPU_BCR] = { 0x4d000 },
...
static const struct qcom_cc_dfs_desc gcc_sdm855_dfs_desc = {
	.clks = gcc_dfs_clocks,
	.num_clks = ARRAY_SIZE(gcc_dfs_clocks),
};

static const struct regmap_config gcc_sdm855_regmap_config = {
	.reg_bits	= 32,
	.reg_stride	= 4,
	.val_bits	= 32,
...
static const struct qcom_cc_desc gcc_sdm855_desc = {
	.config = &gcc_sdm855_regmap_config,
	.clks = gcc_sdm855_clocks,
	.num_clks = ARRAY_SIZE(gcc_sdm855_clocks),
	.resets = gcc_sdm855_resets,
	.num_resets = ARRAY_SIZE(gcc_sdm855_resets),
};

static const struct of_device_id gcc_sdm855_match_table[] = {
	{ .compatible = "qcom,gcc-sdm855" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_sdm855_match_table);

static int gcc_sdm855_probe(struct platform_device *pdev)
{
	struct clk *clk;
	struct regmap *regmap;
	int i, ret = 0;

	regmap = qcom_cc_map(pdev, &gcc_sdm855_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gcc_sdm855_desc, regmap);
	if (ret) {
	...
	ret = qcom_cc_register_rcg_dfs(pdev, &gcc_sdm855_dfs_desc);
	if (ret)
...
static struct platform_driver gcc_sdm855_driver = {
	.probe		= gcc_sdm855_probe,
	.driver		= {
		.name	= "gcc-sdm855",
		.of_match_table = gcc_sdm855_match_table,
	},
};

static int __init gcc_sdm855_init(void)
{
	return platform_driver_register(&gcc_sdm855_driver);
}
subsys_initcall(gcc_sdm855_init);

static void __exit gcc_sdm855_exit(void)
{
	platform_driver_unregister(&gcc_sdm855_driver);
}
module_exit(gcc_sdm855_exit);

MODULE_DESCRIPTION("QTI GCC SDM855 Driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:gcc-sdm855");
