...
static struct clk_vdd_class *gcc_lemans_regulators[] = {
	&vdd_cx,
	&vdd_mxa,
};
...
static struct clk_rcg2 gcc_emac0_rgmii_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = gcc_lemans_regulators,
		.num_vdd_classes = ARRAY_SIZE(gcc_lemans_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOW_L1] = 125000000,
...
static const struct regmap_config gcc_lemans_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
MODULE_DEVICE_TABLE(of, gcc_lemans_match_table);

static int gcc_lemans_probe(struct platform_device *pdev)
{
...
static void gcc_lemans_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_lemans_desc);
}

static struct platform_driver gcc_lemans_driver = {
	.probe = gcc_lemans_probe,
	.driver = {
		.name = "gcc-lemans",
		.of_match_table = gcc_lemans_match_table,
		.sync_state = gcc_lemans_sync_state,
	},
};

static int __init gcc_lemans_init(void)
{
	return platform_driver_register(&gcc_lemans_driver);
}
subsys_initcall(gcc_lemans_init);

static void __exit gcc_lemans_exit(void)
{
	platform_driver_unregister(&gcc_lemans_driver);
}
module_exit(gcc_lemans_exit);

MODULE_DESCRIPTION("QTI GCC LEMANS Driver");
