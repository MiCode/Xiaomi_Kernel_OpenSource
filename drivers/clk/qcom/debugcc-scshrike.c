...
static struct clk_hw *debugcc_scshrike_hws[] = {
...
static int clk_debug_scshrike_probe(struct platform_device *pdev)
{
	struct clk *clk;
	int ret, i;
	...
	for (i = 0; i < ARRAY_SIZE(debugcc_scshrike_hws); i++) {
		clk = devm_clk_register(&pdev->dev, debugcc_scshrike_hws[i]);
		if (IS_ERR(clk)) {
			dev_err(&pdev->dev, "Unable to register %s, err:(%d)\n",
				clk_hw_get_name(debugcc_scshrike_hws[i]),
				PTR_ERR(clk));
			return PTR_ERR(clk);
		}
	}
...
static struct platform_driver clk_debug_driver = {
	.probe = clk_debug_scshrike_probe,
	.driver = {
		.name = "scshrike-debugcc",
		.of_match_table = clk_debug_match_table,
	},
};

static int __init clk_debug_scshrike_init(void)
{
	return platform_driver_register(&clk_debug_driver);
}
fs_initcall(clk_debug_scshrike_init);

MODULE_DESCRIPTION("QTI DEBUG CC SCSHRIKE Driver");
