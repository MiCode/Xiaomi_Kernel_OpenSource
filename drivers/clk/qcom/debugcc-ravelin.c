...
static int apss_cc_debug_mux_sels[] = {
	0x45,		/* measure_only_apcs_gold_post_acd_clk */
	0x21,		/* measure_only_apcs_gold_pre_acd_clk */
	0x25,		/* measure_only_apcs_l3_post_acd_clk */
	0x22,		/* measure_only_apcs_l3_pre_acd_clk */
	0x41,		/* measure_only_apcs_silver_post_acd_clk */
	0x20,		/* measure_only_apcs_silver_pre_acd_clk */
};
...
static struct clk_hw *debugcc_ravelin_hws[] = {
...
static int clk_debug_ravelin_probe(struct platform_device *pdev)
{
	struct clk *clk;
	int ret = 0, i;
	...
	for (i = 0; i < ARRAY_SIZE(debugcc_ravelin_hws); i++) {
		clk = devm_clk_register(&pdev->dev, debugcc_ravelin_hws[i]);
		if (IS_ERR(clk)) {
			dev_err(&pdev->dev, "Unable to register %s, err:(%d)\n",
				clk_hw_get_name(debugcc_ravelin_hws[i]),
				PTR_ERR(clk));
			return PTR_ERR(clk);
		}
	}
...
static struct platform_driver clk_debug_driver = {
	.probe = clk_debug_ravelin_probe,
	.driver = {
		.name = "ravelin-debugcc",
		.of_match_table = clk_debug_match_table,
	},
};

static int __init clk_debug_ravelin_init(void)
{
	return platform_driver_register(&clk_debug_driver);
}
fs_initcall(clk_debug_ravelin_init);

MODULE_DESCRIPTION("QTI DEBUG CC RAVELIN Driver");
