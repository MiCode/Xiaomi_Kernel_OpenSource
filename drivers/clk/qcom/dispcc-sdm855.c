...
static struct clk_regmap *disp_cc_sdm855_clocks[] = {
...
static const struct qcom_reset_map disp_cc_sdm855_resets[] = {
	[DISP_CC_MDSS_CORE_BCR] = { 0x2000 },
	[DISP_CC_MDSS_RSCC_BCR] = { 0x4000 },
	[DISP_CC_MDSS_SPDM_BCR] = { 0x6000 },
};

static const struct regmap_config disp_cc_sdm855_regmap_config = {
	.reg_bits	= 32,
	.reg_stride	= 4,
	.val_bits	= 32,
...
static const struct qcom_cc_desc disp_cc_sdm855_desc = {
	.config = &disp_cc_sdm855_regmap_config,
	.clks = disp_cc_sdm855_clocks,
	.num_clks = ARRAY_SIZE(disp_cc_sdm855_clocks),
	.resets = disp_cc_sdm855_resets,
	.num_resets = ARRAY_SIZE(disp_cc_sdm855_resets),
};

static const struct of_device_id disp_cc_sdm855_match_table[] = {
	{ .compatible = "qcom,dispcc-sdm855" },
	{ }
};
MODULE_DEVICE_TABLE(of, disp_cc_sdm855_match_table);

static int disp_cc_sdm855_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	struct clk *clk;
	int ret = 0;

	regmap = qcom_cc_map(pdev, &disp_cc_sdm855_desc);
	if (IS_ERR(regmap)) {
	...
	ret = qcom_cc_really_probe(pdev, &disp_cc_sdm855_desc, regmap);
	if (ret) {
...
static struct platform_driver disp_cc_sdm855_driver = {
	.probe		= disp_cc_sdm855_probe,
	.driver		= {
		.name	= "disp_cc-sdm855",
		.of_match_table = disp_cc_sdm855_match_table,
	},
};

static int __init disp_cc_sdm855_init(void)
{
	return platform_driver_register(&disp_cc_sdm855_driver);
}
subsys_initcall(disp_cc_sdm855_init);

static void __exit disp_cc_sdm855_exit(void)
{
	platform_driver_unregister(&disp_cc_sdm855_driver);
}
module_exit(disp_cc_sdm855_exit);

MODULE_DESCRIPTION("QTI DISP_CC SDM855 Driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:disp_cc-sdm855");
