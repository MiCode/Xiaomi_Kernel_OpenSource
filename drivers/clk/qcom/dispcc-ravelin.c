...
static struct clk_vdd_class *disp_cc_ravelin_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 disp_cc_mdss_mdp_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = disp_cc_ravelin_regulators,
		.num_vdd_classes = ARRAY_SIZE(disp_cc_ravelin_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 200000000,
			[VDD_LOW] = 325000000,
...
static const struct qcom_reset_map disp_cc_ravelin_resets[] = {
	[DISP_CC_MDSS_CORE_BCR] = { 0x8000 },
	[DISP_CC_MDSS_CORE_INT2_BCR] = { 0xa000 },
	[DISP_CC_MDSS_RSCC_BCR] = { 0xc000 },
};

static const struct regmap_config disp_cc_ravelin_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc disp_cc_ravelin_desc = {
	.config = &disp_cc_ravelin_regmap_config,
	.clks = disp_cc_ravelin_clocks,
...
MODULE_DEVICE_TABLE(of, disp_cc_ravelin_match_table);

static int disp_cc_ravelin_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &disp_cc_ravelin_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &disp_cc_ravelin_desc, regmap);
	if (ret) {
...
{
	return platform_driver_register(&disp_cc_ravelin_driver);
}
subsys_initcall(disp_cc_ravelin_init);

static void __exit disp_cc_ravelin_exit(void)
{
	platform_driver_unregister(&disp_cc_ravelin_driver);
}
module_exit(disp_cc_ravelin_exit);

MODULE_DESCRIPTION("QTI DISP_CC RAVELIN Driver");
