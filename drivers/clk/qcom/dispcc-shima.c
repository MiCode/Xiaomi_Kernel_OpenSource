...
static struct clk_regmap *disp_cc_shima_clocks[] = {
...
static const struct regmap_config disp_cc_shima_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc disp_cc_shima_desc = {
	.config = &disp_cc_shima_regmap_config,
	.clks = disp_cc_shima_clocks,
	.num_clks = ARRAY_SIZE(disp_cc_shima_clocks),
};

static const struct of_device_id disp_cc_shima_match_table[] = {
	{ .compatible = "qcom,shima-dispcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, disp_cc_shima_match_table);

static int disp_cc_shima_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &disp_cc_shima_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &disp_cc_shima_desc, regmap);
	if (ret) {
...
static const struct dev_pm_ops disp_cc_shima_pm_ops = {
	SET_RUNTIME_PM_OPS(pm_clk_suspend, pm_clk_resume, NULL)
};

static void disp_cc_shima_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &disp_cc_shima_desc);
}

static struct platform_driver disp_cc_shima_driver = {
	.probe = disp_cc_shima_probe,
	.driver = {
		.name = "disp_cc-shima",
		.of_match_table = disp_cc_shima_match_table,
		.sync_state = disp_cc_shima_sync_state,
		.pm = &disp_cc_shima_pm_ops,
	},
};

static int __init disp_cc_shima_init(void)
{
	return platform_driver_register(&disp_cc_shima_driver);
}
subsys_initcall(disp_cc_shima_init);

static void __exit disp_cc_shima_exit(void)
{
	platform_driver_unregister(&disp_cc_shima_driver);
}
module_exit(disp_cc_shima_exit);

MODULE_DESCRIPTION("QTI DISP_CC SHIMA Driver");
