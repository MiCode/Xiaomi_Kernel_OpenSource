...
static struct clk_regmap *gcc_sdmshrike_clocks[] = {
...
static const struct qcom_cc_dfs_desc gcc_sdmshrike_dfs_desc = {
	.clks = gcc_dfs_clocks,
	.num_clks = ARRAY_SIZE(gcc_dfs_clocks),
};

static const struct regmap_config gcc_sdmshrike_regmap_config = {
	.reg_bits	= 32,
	.reg_stride	= 4,
	.val_bits	= 32,
...
static const struct qcom_cc_desc gcc_sdmshrike_desc = {
	.config = &gcc_sdmshrike_regmap_config,
	.clks = gcc_sdmshrike_clocks,
	.num_clks = ARRAY_SIZE(gcc_sdmshrike_clocks),
	.resets = gcc_sdmshrike_resets,
	.num_resets = ARRAY_SIZE(gcc_sdmshrike_resets),
};

static const struct of_device_id gcc_sdmshrike_match_table[] = {
	{ .compatible = "qcom,gcc-sdmshrike" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_sdmshrike_match_table);
...
static int gcc_sdmshrike_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &gcc_sdmshrike_desc);
	if (IS_ERR(regmap))
	...
	for (i = 0; i < ARRAY_SIZE(gcc_sdmshrike_hws); i++) {
		clk = devm_clk_register(&pdev->dev, gcc_sdmshrike_hws[i]);
		if (IS_ERR(clk))
			return PTR_ERR(clk);
	}

	ret = qcom_cc_really_probe(pdev, &gcc_sdmshrike_desc, regmap);
	if (ret) {
	...
	ret = qcom_cc_register_rcg_dfs(pdev, &gcc_sdmshrike_dfs_desc);
	if (ret)
...
static struct platform_driver gcc_sdmshrike_driver = {
	.probe		= gcc_sdmshrike_probe,
	.driver		= {
		.name	= "gcc-sdmshrike",
		.of_match_table = gcc_sdmshrike_match_table,
	},
};

static int __init gcc_sdmshrike_init(void)
{
...
{
	platform_driver_unregister(&gcc_sdmshrike_driver);
}
module_exit(gcc_sdmshrike_exit);

MODULE_DESCRIPTION("QTI GCC SDMSHRIKE Driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:gcc-sdmshrike");
