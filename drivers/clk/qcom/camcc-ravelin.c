...
static struct clk_vdd_class *cam_cc_ravelin_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 cam_cc_bps_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = cam_cc_ravelin_regulators,
		.num_vdd_classes = ARRAY_SIZE(cam_cc_ravelin_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 300000000,
			[VDD_LOW] = 410000000,
...
static struct clk_regmap *cam_cc_ravelin_clocks[] = {
...
static const struct qcom_reset_map cam_cc_ravelin_resets[] = {
...
static const struct regmap_config cam_cc_ravelin_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static struct qcom_cc_desc cam_cc_ravelin_desc = {
	.config = &cam_cc_ravelin_regmap_config,
	.clks = cam_cc_ravelin_clocks,
	.num_clks = ARRAY_SIZE(cam_cc_ravelin_clocks),
	.resets = cam_cc_ravelin_resets,
	.num_resets = ARRAY_SIZE(cam_cc_ravelin_resets),
	.clk_regulators = cam_cc_ravelin_regulators,
	.num_clk_regulators = ARRAY_SIZE(cam_cc_ravelin_regulators),
};

static const struct of_device_id cam_cc_ravelin_match_table[] = {
	{ .compatible = "qcom,ravelin-camcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, cam_cc_ravelin_match_table);

static int cam_cc_ravelin_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &cam_cc_ravelin_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &cam_cc_ravelin_desc, regmap);
	if (ret) {
...
static void cam_cc_ravelin_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &cam_cc_ravelin_desc);
}
...
{
	return platform_driver_register(&cam_cc_ravelin_driver);
}
subsys_initcall(cam_cc_ravelin_init);

static void __exit cam_cc_ravelin_exit(void)
{
	platform_driver_unregister(&cam_cc_ravelin_driver);
}
module_exit(cam_cc_ravelin_exit);

MODULE_DESCRIPTION("QTI CAM_CC RAVELIN Driver");
