...
static const struct of_device_id of_match_clk_mt6895_adsp[] = {
	{
		.compatible = "mediatek,mt6895-afe",
		.data = &afe_mcd,
	}, {
		/* sentinel */
	}
...
static int clk_mt6895_adsp_grp_probe(struct platform_device *pdev)
{
	int r;
...
static struct platform_driver clk_mt6895_adsp_drv = {
	.probe = clk_mt6895_adsp_grp_probe,
	.driver = {
		.name = "clk-mt6895-adsp",
		.of_match_table = of_match_clk_mt6895_adsp,
	},
};

static int __init clk_mt6895_adsp_init(void)
{
	return platform_driver_register(&clk_mt6895_adsp_drv);
}

static void __exit clk_mt6895_adsp_exit(void)
{
	platform_driver_unregister(&clk_mt6895_adsp_drv);
}

arch_initcall(clk_mt6895_adsp_init);
module_exit(clk_mt6895_adsp_exit);
MODULE_LICENSE("GPL");
