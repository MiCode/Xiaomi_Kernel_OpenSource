...
static int clk_mt6893_mfgcfg_probe(struct platform_device *pdev)
{
	...
	mtk_clk_register_gates_with_dev(node, mfgcfg_clks, ARRAY_SIZE(mfgcfg_clks),
			clk_data, &pdev->dev);
...
builtin_platform_driver(clk_mt6893_mfgcfg_drv);
...
arch_initcall(clk_mt6893_mfgcfg_init);
