...
static const struct fmeter_clk fclks[] = {
	...
	FMCLK(CKGEN,  hf_fscp_ck, "hf_fscp_ck", 1),
	FMCLK(CKGEN,  hf_fmfg_ck, "hf_fmfg_ck", 1),
	FMCLK(CKGEN,  hf_fatb_ck, "hf_fatb_ck", 1),
	FMCLK(CKGEN,  f_fcamtg_ck, "f_fcamtg_ck", 1),
	FMCLK(CKGEN,  f_fcamtg1_ck, "f_fcamtg1_ck", 1),
	FMCLK(CKGEN,  f_fcamtg2_ck, "f_fcamtg2_ck", 1),
	FMCLK(CKGEN,  f_fcamtg3_ck, "f_fcamtg3_ck", 1),
	...
	FMCLK(CKGEN,  hf_faud_engen1_ck, "hf_faud_engen1_ck", 1),
	FMCLK(CKGEN,  f_fdisp_pwm_ck, "f_fdisp_pwm_ck", 1),
	FMCLK(CKGEN,  hf_fsspm_ck, "hf_fsspm_ck", 1),
	FMCLK(CKGEN,  hf_fdxcc_ck, "hf_fdxcc_ck", 1),
	FMCLK(CKGEN,  f_fusb_top_ck, "f_fusb_top_ck", 1),
	FMCLK(CKGEN,  hf_fspm_ck, "hf_fspm_ck", 1),
	FMCLK(CKGEN,  hf_fi2c_ck, "hf_fi2c_ck", 1),
	FMCLK(CKGEN,  f_fpwm_ck, "f_fpwm_ck", 1),
	FMCLK(CKGEN,  f_fseninf_ck, "f_fseninf_ck", 1),
	FMCLK(CKGEN,  hf_faes_fde_ck, "hf_faes_ufsfde_ck", 1),
	FMCLK(CKGEN,  f_fpwrap_ulposc_ck, "f_fpwrap_ulposc_ck", 1),
	FMCLK(CKGEN,  f_fcamtm_ck, "f_fcamtm_ck", 1),
	...
	FMCLK(CKGEN,  fmem_ck_bfe_dcm_ch1, "fmem_ck_bfe_dcm_ch1", 1),
	FMCLK(CKGEN,  fmem_ck_aft_dcm_ch1, "fmem_ck_aft_dcm_ch1", 1),
	//ABIST
	...
	FMCLK(ABIST,  AD_PLLGP2_TST_CK, "AD_PLLGP2_TST_CK", 1),
...
const struct fmeter_clk *mt6761_get_fmeter_clks(void)
{
	return fclks;
}
...
/* implement done */
static unsigned int mt6761_get_ckgen_freq(unsigned int ID)
{
	int output = 0, i = 0;
...
static unsigned int mt6761_get_abist_freq(unsigned int ID)
{
	int output = 0, i = 0;
	unsigned int temp, clk_dbg_cfg, clk_misc_cfg_0;
	unsigned int clk26cali_0, clk26cali_1;
...
static unsigned int mt6761_get_abist2_freq(unsigned int ID)
{
	pr_notice("mt6761 not support abist2 func\n");
	return 0;
}

static unsigned int mt6761_get_fmeter_freq(unsigned int id, enum  FMETER_TYPE type)
{
	if (type == CKGEN)
		return mt6761_get_ckgen_freq(id);
	else if (type == ABIST)
		return mt6761_get_abist_freq(id);
	else if (type == ABIST_2)
		return mt6761_get_abist2_freq(id);

	return FT_NULL;
}

static int mt6761_get_fmeter_id(enum FMETER_ID fid)
{
	if (fid == FID_DISP_PWM)
...
static struct fmeter_ops fm_ops = {
	.get_fmeter_clks = mt6761_get_fmeter_clks,
	.get_ckgen_freq = mt6761_get_ckgen_freq,
	.get_abist_freq = mt6761_get_abist_freq,
	.get_abist2_freq = mt6761_get_abist2_freq,
	.get_fmeter_freq = mt6761_get_fmeter_freq,
	.get_fmeter_id = mt6761_get_fmeter_id,
};


static int clk_fmeter_mt6761_probe(struct platform_device *pdev)
{
	topck_base = get_base_from_comp("mediatek,mt6761-topckgen");
	if (IS_ERR(topck_base))
		goto ERR;

	apmixed_base = get_base_from_comp("mediatek,mt6761-apmixedsys");
	if (IS_ERR(apmixed_base))
		goto ERR;

	spm_base = get_base_from_comp("mediatek,mt6761-scpsys");
	if (IS_ERR(spm_base))
		goto ERR;
...
static struct platform_driver clk_fmeter_mt6761_drv = {
	.probe = clk_fmeter_mt6761_probe,
	.driver = {
		.name = "clk-fmeter-mt6761",
		.owner = THIS_MODULE,
	},
};
...
static int __init clk_fmeter_init(void)
{
	...
	clk_fmeter_dev = platform_device_register_simple("clk-fmeter-mt6761", -1, NULL, 0);
	if (IS_ERR(clk_fmeter_dev))
		pr_warn("unable to register clk-fmeter device");

	return platform_driver_register(&clk_fmeter_mt6761_drv);
}

static void __exit clk_fmeter_exit(void)
{
	platform_driver_unregister(&clk_fmeter_mt6761_drv);
}
