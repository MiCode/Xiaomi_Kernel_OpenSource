					...
					struct clk_onecell_data *clk_data)
{
	...
	for (i = 0; i < ARRAY_SIZE(scp_clks); i++) {
		...
		pre_clk = pg->pre_clk_name ? devm_clk_get(&pdev->dev, pg->pre_clk_name) : NULL;
		pre_clk2 = pg->pre_clk2_name ? devm_clk_get(&pdev->dev, pg->pre_clk2_name) : NULL;
...
*/
	...
	if (!clk_data)
		return -ENOMEM;
	init_clk_scpsys(pdev, infracfg_reg, spm_reg, clk_data);
