...
static int clk_mt6893_vde2_probe(struct platform_device *pdev)
{
	...
	mtk_clk_register_gates_with_dev(node, vde2_clks, ARRAY_SIZE(vde2_clks),
			clk_data, &pdev->dev);
...
static const struct of_device_id of_match_clk_mt6893_vde2[] = {
...
static struct platform_driver clk_mt6893_vde2_drv = {
	.probe = clk_mt6893_vde2_probe,
	.driver = {
		.name = "clk-mt6893-vde2",
		.of_match_table = of_match_clk_mt6893_vde2,
	},
};

builtin_platform_driver(clk_mt6893_vde2_drv);
...
static int __init clk_mt6893_vde2_init(void)
{
	return platform_driver_register(&clk_mt6893_vde2_drv);
}

static void __exit clk_mt6893_vde2_exit(void)
{
	platform_driver_unregister(&clk_mt6893_vde2_drv);
}

arch_initcall(clk_mt6893_vde2_init);
module_exit(clk_mt6893_vde2_exit);
MODULE_LICENSE("GPL");
