...
#define CAM_PROT_STEP2_1_ACK_MASK        ((0x1 << 1) \
					  |(0x1 << 3) \
					  |(0x1 << 5))
#define CAM_PROT_STEP2_2_MASK            ((0x1 << 28))
#define CAM_PROT_STEP2_2_ACK_MASK        ((0x1 << 28))
...
static unsigned int check_cg_state(const char *clkname)
{
...
static const char * const *get_vde_clk_names(size_t *num)
{
	static const char * const clks[] = {
		/* VDE */
		/* VDEC_SOC_GCON */
...
static const char * const *get_vde2_clk_names(size_t *num)
{
	static const char * const clks[] = {
		/* VDE2 */
		/* VDEC_GCON */
...
static const char * const *get_venc_c1_clk_names(size_t *num)
{
	static const char * const clks[] = {
		/* VENC_C1 */
...
static const char * const *get_cam_rawa_clk_names(size_t *num)
{
	static const char * const clks[] = {
		/* CAM RAWA */
...
static const char * const *get_cam_rawb_clk_names(size_t *num)
{
	static const char * const clks[] = {
		/* CAM RAWB */
...
static const char * const *get_cam_rawc_clk_names(size_t *num)
{
	static const char * const clks[] = {
		/* CAM RAWC */
...
static const char * const *get_img1_clk_names(size_t *num)
{
	static const char * const clks[] = {
		/* IMG1 */
		"imgsys1_larb9_cgpdn",
...
void check_subsys_swcg(enum subsys_id id)
{
	unsigned int ret = 0;
	int i = 0;
	size_t mm_num, mdp_num = 0;
	size_t vde_num, vde2_num = 0;
	size_t venc_c1_num, venc_num = 0;
	size_t cam_num, cam_rawa_num, cam_rawb_num, cam_rawc_num = 0;
	size_t img1_num, img2_num, ipe_num = 0;

	const char * const *mm_clks = get_mm_clk_names(&mm_num);
	const char * const *mdp_clks = get_mdp_clk_names(&mdp_num);

	const char * const *vde_clks = get_vde_clk_names(&vde_num);
	const char * const *vde2_clks = get_vde2_clk_names(&vde2_num);

	const char * const *venc_c1_clks = get_venc_c1_clk_names(&venc_c1_num);
	const char * const *venc_clks = get_venc_clk_names(&venc_num);

	const char * const *cam_clks = get_cam_clk_names(&cam_num);
	const char * const *cam_rawa_clks =
		get_cam_rawa_clk_names(&cam_rawa_num);
	const char * const *cam_rawb_clks =
		get_cam_rawb_clk_names(&cam_rawb_num);
	const char * const *cam_rawc_clks =
		get_cam_rawc_clk_names(&cam_rawc_num);

	const char * const *img1_clks = get_img1_clk_names(&img1_num);
	...
	if (id == SYS_MDP) {
		for (i = 0; i < mdp_num; i++)
			ret += check_cg_state(mdp_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_MDP(%d) warning!\r\n", __func__, id);
			print_subsys_reg("mdpsys");
	}
	} else if (id == SYS_DIS) {
		for (i = 0; i < mm_num; i++)
			ret += check_cg_state(mm_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_DIS(%d) warning!\r\n", __func__, id);
			print_subsys_reg("mmsys");
	}
	} else if (id == SYS_ISP) {
		for (i = 0; i < img1_num; i++)
			ret += check_cg_state(img1_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_ISP(%d) warning!\r\n", __func__, id);
			print_subsys_reg("img1sys");
	}
	} else if (id == SYS_ISP2) {
		for (i = 0; i < img2_num; i++)
			ret += check_cg_state(img2_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_ISP2(%d) warning!\r\n", __func__, id);
			print_subsys_reg("img2sys");
		}
	} else if (id == SYS_IPE) {
		for (i = 0; i < ipe_num; i++)
			ret += check_cg_state(ipe_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_IPE(%d) warning!\r\n", __func__, id);
			print_subsys_reg("ipesys");
		}
	} else if (id == SYS_VDE) {
		for (i = 0; i < vde_num; i++)
			ret += check_cg_state(vde_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_VDE(%d) warning!\r\n", __func__, id);
			print_subsys_reg("mmsys");
			print_subsys_reg("vdec_soc_sys");
		}
	} else if (id == SYS_VDE2) {
		for (i = 0; i < vde2_num; i++)
			ret += check_cg_state(vde2_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_VDE2(%d) warning!\r\n", __func__, id);
			print_subsys_reg("vdecsys");
		}
	} else if (id == SYS_VEN) {
		for (i = 0; i < venc_num; i++)
			ret += check_cg_state(venc_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_VEN(%d) warning!\r\n", __func__, id);
			print_subsys_reg("vencsys");
		}
	} else if (id == SYS_VEN_CORE1) {
		for (i = 0; i < venc_c1_num; i++)
			ret += check_cg_state(venc_c1_clks[i]);
		if (ret) {
		...
		for (i = 0; i < cam_num; i++)
			ret += check_cg_state(cam_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_CAM(%d) warning!\r\n", __func__, id);
			print_subsys_reg("camsys");
		}
	} else if (id == SYS_CAM_RAWA) {
		for (i = 0; i < cam_rawa_num; i++)
			ret += check_cg_state(cam_rawa_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_CAM_RAWA(%d) warning!\r\n",
				__func__, id);
			print_subsys_reg("cam_rawa_sys");
		}
	} else if (id == SYS_CAM_RAWB) {
		for (i = 0; i < cam_rawb_num; i++)
			ret += check_cg_state(cam_rawb_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_CAM_RAWB(%d) warning!\r\n",
				__func__, id);
			print_subsys_reg("cam_rawb_sys");
		}
	} else if (id == SYS_CAM_RAWC) {
		for (i = 0; i < cam_rawc_num; i++)
			ret += check_cg_state(cam_rawc_clks[i]);
		if (ret) {
			pr_notice("%s:SYS_CAM_RAWC(%d) warning!\r\n",
				__func__, id);
			print_subsys_reg("cam_rawc_sys");
		}
	}
