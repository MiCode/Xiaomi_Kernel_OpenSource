...
static const struct mtk_gate ifrao_clks[] = {
	...
	GATE_IFRAO3(CLK_IFRAO_CCIF4_MD, "ifrao_ccif4_md",
			"axi_ck"/* parent */, 29),
	/* IFRAO4 */
	GATE_IFRAO4(CLK_IFRAO_RG_MMW_DPMAIF26M_CK, "ifrao_dpmaif_26m_set",
			"f26m_ck"/* parent */, 17),
...
static const struct of_device_id of_match_clk_mt6855_bus[] = {
	{
		.compatible = "mediatek,mt6855-infracfg_ao",
		.data = &ifrao_mcd,
	}, {
		/* sentinel */
	}
...
static int clk_mt6855_bus_grp_probe(struct platform_device *pdev)
{
	int r;
...
static struct platform_driver clk_mt6855_bus_drv = {
	.probe = clk_mt6855_bus_grp_probe,
	.driver = {
		.name = "clk-mt6855-bus",
		.of_match_table = of_match_clk_mt6855_bus,
	},
};

static int __init clk_mt6855_bus_init(void)
{
	return platform_driver_register(&clk_mt6855_bus_drv);
}

static void __exit clk_mt6855_bus_exit(void)
{
	platform_driver_unregister(&clk_mt6855_bus_drv);
}

arch_initcall(clk_mt6855_bus_init);
module_exit(clk_mt6855_bus_exit);
MODULE_LICENSE("GPL");
