...
static int clk_mt6853_apu1_probe(struct platform_device *pdev)
{
	...
	mtk_clk_register_gates_with_dev(node, apu1_clks, ARRAY_SIZE(apu1_clks),
			clk_data, &pdev->dev);
...
static int __init clk_mt6853_apu1_init(void)
{
	return platform_driver_register(&clk_mt6853_apu1_drv);
}

static void __exit clk_mt6853_apu1_exit(void)
{
	platform_driver_unregister(&clk_mt6853_apu1_drv);
}

arch_initcall(clk_mt6853_apu1_init);
module_exit(clk_mt6853_apu1_exit);
MODULE_LICENSE("GPL");
