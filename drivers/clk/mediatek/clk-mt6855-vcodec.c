...
static const struct of_device_id of_match_clk_mt6855_vcodec[] = {
	{
		.compatible = "mediatek,mt6855-vdec_gcon_base",
		.data = &vde2_mcd,
	}, {
		.compatible = "mediatek,mt6855-vencsys",
		.data = &ven1_mcd,
	}, {
		/* sentinel */
	}
...
static int clk_mt6855_vcodec_grp_probe(struct platform_device *pdev)
{
	int r;
...
static struct platform_driver clk_mt6855_vcodec_drv = {
	.probe = clk_mt6855_vcodec_grp_probe,
	.driver = {
		.name = "clk-mt6855-vcodec",
		.of_match_table = of_match_clk_mt6855_vcodec,
	},
};

static int __init clk_mt6855_vcodec_init(void)
{
	return platform_driver_register(&clk_mt6855_vcodec_drv);
}

static void __exit clk_mt6855_vcodec_exit(void)
{
	platform_driver_unregister(&clk_mt6855_vcodec_drv);
}

arch_initcall(clk_mt6855_vcodec_init);
module_exit(clk_mt6855_vcodec_exit);
MODULE_LICENSE("GPL");
