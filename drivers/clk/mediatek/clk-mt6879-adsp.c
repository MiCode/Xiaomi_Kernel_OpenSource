...
static const struct mtk_gate_regs adsp0_cg_regs = {
	.set_ofs = 0x1000,
	.clr_ofs = 0x1000,
	.sta_ofs = 0x1000,
};

static const struct mtk_gate_regs adsp1_cg_regs = {
	.set_ofs = 0x1010,
	.clr_ofs = 0x1010,
	.sta_ofs = 0x1010,
};

#define GATE_ADSP0(_id, _name, _parent, _shift) {	\
		.id = _id,				\
		.name = _name,				\
		.parent_name = _parent,			\
		.regs = &adsp0_cg_regs,			\
...
#define GATE_ADSP1(_id, _name, _parent, _shift) {	\
		.id = _id,				\
		.name = _name,				\
		.parent_name = _parent,			\
		.regs = &adsp1_cg_regs,			\
...
static const struct mtk_gate adsp_clks[] = {
	/* ADSP0 */
	GATE_ADSP0(CLK_ADSP_CORE0_EN, "adsp_core0",
			"adsp_ck"/* parent */, 0),
	GATE_ADSP0(CLK_ADSP_CORE0_DBG_EN, "adsp_core0_dbg_en",
			"adsp_ck"/* parent */, 1),
	GATE_ADSP0(CLK_ADSP_TIMER_EN, "adsp_timer_en",
			"adsp_ck"/* parent */, 3),
	GATE_ADSP0(CLK_ADSP_DMA0_EN, "adsp_dma0_en",
			"adsp_ck"/* parent */, 4),
	GATE_ADSP0(CLK_ADSP_UART_EN, "adsp_uart_en",
			"adsp_ck"/* parent */, 5),
	/* ADSP1 */
	GATE_ADSP1(CLK_ADSP_UART_BCLK, "adsp_uart_bclk",
			"adsp_ck"/* parent */, 0),
};
...
static int clk_mt6879_adsp_probe(struct platform_device *pdev)
{
	...
	mtk_clk_register_gates(node, adsp_clks, ARRAY_SIZE(adsp_clks),
			clk_data);
...
static int clk_mt6879_afe_probe(struct platform_device *pdev)
{
	struct device_node *node = pdev->dev.of_node;
	struct clk_onecell_data *clk_data;
	int r;
	...
	mtk_clk_register_gates(node, afe_clks, ARRAY_SIZE(afe_clks),
			clk_data);
...
static const struct of_device_id of_match_clk_mt6879_adsp[] = {
	{
		.compatible = "mediatek,mt6879-adsp",
		.data = clk_mt6879_adsp_probe,
	}, {
		.compatible = "mediatek,mt6879-afe",
		.data = clk_mt6879_afe_probe,
	}, {
		/* sentinel */
	}
};
...
static int clk_mt6879_adsp_grp_probe(struct platform_device *pdev)
{
...
static struct platform_driver clk_mt6879_adsp_drv = {
	.probe = clk_mt6879_adsp_grp_probe,
	.driver = {
		.name = "clk-mt6879-adsp",
		.of_match_table = of_match_clk_mt6879_adsp,
	},
};

static int __init clk_mt6879_adsp_init(void)
{
	return platform_driver_register(&clk_mt6879_adsp_drv);
}

static void __exit clk_mt6879_adsp_exit(void)
{
	platform_driver_unregister(&clk_mt6879_adsp_drv);
}

arch_initcall(clk_mt6879_adsp_init);
module_exit(clk_mt6879_adsp_exit);
MODULE_LICENSE("GPL");
