...
static int get_hw_sem(void)
{
	unsigned int i;
	void __iomem *ap_sema_reg;

	ap_sema_reg = g_spm_base + 0x428;
...
static void release_hw_sem(void)
{
	void __iomem *ap_sema_reg;
...
static int __ssc_v2(struct fh_pll_regs *regs,
		struct fh_pll_data *data,
		int fh_id, int rate)
{
	unsigned int updnlmt_val;
	...
	if (rate > 0) {
		...
		if (fh_id == FH_ID_MEM) {
			...
			writel(val, g_spm_base + 0x4D0);
			val = mempll_ssc_pmap1344[rate];
			writel(val, g_spm_base + 0x4D4);
		} else if (fh_id == FH_ID_MM) {
			unsigned int val;

			val = mmpll_ssc_pmap180[rate];
			writel(val, g_spm_base + 0x4D8);
			val = mmpll_ssc_pmap300[rate];
			writel(val, g_spm_base + 0x4DC);
		}
...
static int ap_hopping_v2(void *priv_data, char *domain_name, int fh_id,
		unsigned int new_dds, int postdiv)
{
	struct fh_pll_domain *domain;
	struct fh_pll_regs *regs;
	...
	if (array->ssc_rate)
		__ssc_v2(regs, data, fh_id, array->ssc_rate);

	spin_unlock_irqrestore(lock, flags);
	return ret;
}

static int ap_ssc_enable_v2(void *priv_data, char *domain_name, int fh_id,
		int rate)
{
	struct fh_pll_domain *domain;
	struct fh_pll_regs *regs;
...
static int ap_ssc_disable_v2(void *priv_data, char *domain_name, int fh_id)
{
	struct fh_pll_domain *domain;
	struct fh_pll_regs *regs;
	struct fh_pll_data *data;
...
static int ap_init_v2(struct pll_dts *array, struct match *match)
{
	static DEFINE_SPINLOCK(lock);
	struct hdlr_data_v1 *priv_data;
	struct fh_hdlr *hdlr;
	...
	if (array->fh_id == FH_ID_MEM) {
		struct device_node *node;

		node = of_find_compatible_node(NULL, NULL, "mediatek,sleep");
		g_spm_base = of_iomap(node, 0);
		node = of_find_compatible_node(NULL, NULL, "mediatek,ddrphy");
		g_ddrphy_base = of_iomap(node, 0);

		regs->reg_con_pcw = g_ddrphy_base + 0x624;
	}
...
static struct fh_operation ap_ops_v2 = {
	.hopping = ap_hopping_v2,
	.ssc_enable = ap_ssc_enable_v2,
	.ssc_disable = ap_ssc_disable_v2,
};
static struct fh_hdlr ap_hdlr_v2 = {
	.ops = &ap_ops_v2,
};
