...
static const struct fmeter_clk fclks[] = {
	...
	FMCLK(ABIST_2, FM_R0_OUT_FM, "fm_r0_out_fm", 1),
	...
	FMCLK(ABIST_2, FM_AUD_ETDM_OUT1_M_CK, "fm_aud_etdm_out1_m_ck", 1),
	FMCLK(ABIST_2, FM_AUD_ETDM_IN1_M_CK, "fm_aud_etdm_in1_m_ck", 1),
...
const struct fmeter_clk *mt6879_get_fmeter_clks(void)
{
	return fm_all_clks;
}
...
static unsigned int mt6879_get_ckgen_freq(unsigned int ID)
{
	return __mt_get_freq(ID, CKGEN);
}

static unsigned int mt6879_get_abist_freq(unsigned int ID)
{
	return __mt_get_freq(ID, ABIST);
}

static unsigned int mt6879_get_abist2_freq(unsigned int ID)
{
	return __mt_get_freq(ID, ABIST_2);
}

static unsigned int mt6879_get_vlpck_freq(unsigned int ID)
{
	return __mt_get_freq2(VLP_CK_CON0, VLP_CK_CON1, ID, VLPCK);
}

static unsigned int mt6879_get_subsys_freq(unsigned int ID)
{
	int output = 0;
	unsigned long flags;
...
static unsigned int mt6879_get_fmeter_freq(unsigned int id, enum  FMETER_TYPE type)
{
	if (type == CKGEN)
		return mt6879_get_ckgen_freq(id);
	else if (type == ABIST)
		return mt6879_get_abist_freq(id);
	else if (type == ABIST_2)
		return mt6879_get_abist2_freq(id);
	else if (type == SUBSYS)
		return mt6879_get_subsys_freq(id);
	else if (type == VLPCK)
		return mt6879_get_vlpck_freq(id);

	return FT_NULL;
}

static int mt6879_subsys_freq_register(struct fm_subsys *fm, unsigned int size)
{
...
static int mt6879_get_fmeter_id(enum FMETER_ID fid)
{
	if (fid == FID_DISP_PWM)
...
static struct fmeter_ops fm_ops = {
	.get_fmeter_clks = mt6879_get_fmeter_clks,
	.get_fmeter_freq = mt6879_get_fmeter_freq,
	.subsys_freq_register = mt6879_subsys_freq_register,
	.get_fmeter_id = mt6879_get_fmeter_id,
};

static int clk_fmeter_mt6879_probe(struct platform_device *pdev)
{
	int i;

	topck_base = get_base_from_comp("mediatek,mt6879-topckgen");
	if (IS_ERR(topck_base))
		goto ERR;

	apmixed_base = get_base_from_comp("mediatek,mt6879-apmixedsys");
	if (IS_ERR(apmixed_base))
		goto ERR;

	spm_base = get_base_from_comp("mediatek,mt6879-scpsys");
	if (IS_ERR(spm_base))
		goto ERR;

	vlp_ck_base = get_base_from_comp("mediatek,mt6879-vlp_cksys");
...
static struct platform_driver clk_fmeter_mt6879_drv = {
	.probe = clk_fmeter_mt6879_probe,
	.driver = {
		.name = "clk-fmeter-mt6879",
		.owner = THIS_MODULE,
	},
};
...
static int __init clk_fmeter_init(void)
{
	...
	clk_fmeter_dev = platform_device_register_simple("clk-fmeter-mt6879", -1, NULL, 0);
	if (IS_ERR(clk_fmeter_dev))
		pr_warn("unable to register clk-fmeter device");

	return platform_driver_register(&clk_fmeter_mt6879_drv);
}

static void __exit clk_fmeter_exit(void)
{
	platform_driver_unregister(&clk_fmeter_mt6879_drv);
}
