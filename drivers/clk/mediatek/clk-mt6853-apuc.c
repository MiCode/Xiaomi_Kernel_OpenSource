...
static int clk_mt6853_apuc_probe(struct platform_device *pdev)
{
	...
	mtk_clk_register_gates_with_dev(node, apuc_clks, ARRAY_SIZE(apuc_clks),
			clk_data, &pdev->dev);
...
static int __init clk_mt6853_apuc_init(void)
{
	return platform_driver_register(&clk_mt6853_apuc_drv);
}

static void __exit clk_mt6853_apuc_exit(void)
{
	platform_driver_unregister(&clk_mt6853_apuc_drv);
}

arch_initcall(clk_mt6853_apuc_init);
module_exit(clk_mt6853_apuc_exit);
MODULE_LICENSE("GPL");
