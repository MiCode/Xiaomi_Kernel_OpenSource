...
#define CONFIG_TMS_GUIDE_DEVICE 1
...
int nfc_probe(struct i2c_client *client,
...
int nfc_remove(struct i2c_client *client)
{
	int ret = 0;
	struct pn544_dev *pn544_dev;
 ...
 *
 * Description      This function is called during nfc_probe to retrieve
 *                  HW info.
...
EXPORT_SYMBOL(nfc_probe);
EXPORT_SYMBOL(nfc_remove);
#else
