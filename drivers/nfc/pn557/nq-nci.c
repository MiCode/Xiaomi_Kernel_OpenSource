...
/*
 * Inside pnnfc_ioctl_nfcc_info
 *
 * @brief   pnnfc_ioctl_nfcc_info
 *
 * Check the NQ Chipset and firmware version details
 */
unsigned int pnnfc_ioctl_nfcc_info(struct file *filp, unsigned long arg)
{
	unsigned int r = 0;
	struct nqx_dev *nqx_dev = filp->private_data;
...
static int nfc_parse_dt(struct device *dev, struct nqx_platform_data *pdata)
{
	...
	pdata->irq_gpio = of_get_named_gpio(np, "qcom,sn-irq", 0);
	if ((!gpio_is_valid(pdata->irq_gpio)))
		return -EINVAL;

	pdata->firm_gpio = of_get_named_gpio(np, "qcom,sn-firm", 0);
	...
	pdata->clkreq_gpio = of_get_named_gpio(np, "qcom,sn-clkreq", 0);
			...
			const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev,
	"%s: fhg probing NFCC NQxxx exited successfully\n",
		 __func__);
	return 0;
...
{
#ifdef CONFIG_WT_QGKI
  if((strstr(saved_command_line,"board_id=S88006AA1")||strstr(saved_command_line,"board_id=S88106BA1"))){
	pr_info("Loading pn557 nfc driver\n");
	return i2c_add_driver(&nqx);
  }else{
	printk("Not support  NFC pn557");
	return -1;
  }
#else
	pr_info("Loading pn557 nfc driver\n");
