...
#include "pn553.h"
/* N17 code remove nq-nci if sku is cn modify by zhangyize start */
#include "../../misc/hqsysfs/hqsys_pcba.h"
/* N17 code remove nq-nci if sku is cn modify by zhangyize end */
...
/* N17 code remove nq-nci if sku is cn modify by zhangyize start */
	...
	pr_err("sku = %s\n", pcba_msg->sku);
	if (!(strnstr(pcba_msg->sku,"iron_gl", strlen(pcba_msg->sku)) ||
            strnstr(pcba_msg->sku,"iron_p_gl", strlen(pcba_msg->sku))))  {
