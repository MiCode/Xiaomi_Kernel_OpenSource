                          ...
                          int timeout)
{
    ...
    if (!gpio_get_value(nfc->hw_res.irq_gpio)) {
        while (1) {
            ...
            if (!gpio_get_value(nfc->hw_res.irq_gpio)) {
                if (timeout) {
                    ret = wait_event_interruptible_timeout(nfc->read_wq, !nfc->irq_enable || nfc->release_read,
                    ...
                    ret = wait_event_interruptible(nfc->read_wq, !nfc->irq_enable || nfc->release_read);
                        ...
                        loff_t *offset)
{
    ...
    last_count = count;
    TMS_DUMP(LOG_LEVEL_DUMP, "Rx <-", read_buf, count);
err_release_read:
                                ...
                                size_t count, loff_t *offset)
{
    ...
    TMS_DUMP(LOG_LEVEL_DUMP, "Tx ->", write_buf, count);
err_release_write:
...
#endif
{
    ...
    nfc_disable_irq(nfc);
#if !IS_ENABLED(CONFIG_TMS_GUIDE_DEVICE)
    nfc_hw_check(nfc->client, nfc->tms);
#endif
    nfc_hard_reset(nfc->tms);
