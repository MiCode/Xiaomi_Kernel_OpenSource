...
static int st21nfc_clock_select(struct st21nfc_device *st21nfc_dev) {
  ...
  if (IS_ERR_OR_NULL(st21nfc_dev->s_clk)) {
	pr_err("LQ_NFC_DEBUG:%s : st21nfc_dev->s_clk IS_ERR_OR_NULL\n", __func__);
	return 0;
  }
  pr_err("LQ_NFC_DEBUG:%s : st21nfc_dev->s_clk Get OK\n", __func__);
...
static void st21nfc_power_stats_switch(struct st21nfc_device *st21nfc_dev,
  ...
  pr_err("%s Switching from %s to %s: %llx, ntf=%d\n", __func__,
           st21nfc_power_state_name[old_state],
           st21nfc_power_state_name[new_state], current_time_ms, is_ntf);
...
static ssize_t st21nfc_dev_read(struct file *filp, char __user *buf,
  ...
  if (enable_debug_log) pr_err("%s : reading %zu bytes.\n", __func__, count);
  ...
  if (idle < HEADER_LENGTH) {
    ...
    if ((st21nfc_dev->r_state_current == ST21NFC_HEADER) &&
      ...
      if (enable_debug_log)
        pr_err("%s : new state = ST21NFC_PAYLOAD\n", __func__);
    } else {
      st21nfc_dev->r_state_current = ST21NFC_HEADER;
      if (enable_debug_log)
        pr_err("%s : new state = ST21NFC_HEADER\n", __func__);
    }
  }
#endif  // ST54J_PWRSTATS
...
static int st21nfc_release(struct inode *inode, struct file *file) {
  ...
  if (enable_debug_log) pr_err("%s : device_open  = false\n", __func__);
  return 0;
}
...
static long st21nfc_dev_ioctl(struct file *filp, unsigned int cmd,
  ...
  switch (cmd) {
    ...
    case ST21NFC_LEGACY_GET_WAKEUP:
      ...
      if (enable_debug_log) pr_err("%s get gpio result %d\n", __func__, ret);
      break;
    case ST21NFC_GET_POLARITY:
    case ST21NFC_LEGACY_GET_POLARITY:
      ret = st21nfc_dev->polarity_mode;
      if (enable_debug_log) pr_err("%s get polarity %d\n", __func__, ret);
      break;
    ...
    case ST21NFC_USE_ESE:
      ...
      if (enable_debug_log)
        pr_err("%s use ESE %d : %d\n", __func__, ret, tmp);
      break;
#ifdef USE_NODE  //will not use
    case ST21NFC_SET_ESEPWR:
  //      ret = __get_user(tmp, (u32 __user *)arg);
        pr_err("%s set ESE PWR arg =  %d\n", __func__, arg);

        if (arg == 0) {
        pr_err("%spower on eSE set ESE PWR \n", __func__);
            // Set NSS pin for the SPI function.

            usleep_range(4000, 5000);

            gpiod_set_value(st21nfc_dev->gpiod_esepwr, 1);
            usleep_range(1500, 2000);

        } else {
        pr_err("%spower off eSE set ESE PWR arg \n", __func__);
	    // Set NSS pin as highZ (ST54H and ST54J).

	    // Change NSS polarity to have NSS low.


            gpiod_set_value(st21nfc_dev->gpiod_esepwr, 0);
            usleep_range(2000, 4500);
        }

        pr_err("%s : st21nfc set ese pwr\n", __func__);
    break;
    case ST21NFC_GET_ESEPWR:
        ret = gpiod_get_value(st21nfc_dev->gpiod_esepwr);
        pr_err("%s : st21nfc get ese pwr\n", __func__);
    break;
#endif 	//end will not use
    default:
...
static unsigned int st21nfc_poll(struct file *file, poll_table *wait) {
  ...
  if (pinlev != 0) {
    if (enable_debug_log) pr_err("%s return ready\n", __func__);
    mask = POLLIN | POLLRDNORM; /* signal data avail */
    st21nfc_disable_irq(st21nfc_dev);
  } else {
    /* Wake_up_pin is low. Activate ISR  */
    if (!st21nfc_dev->irq_enabled) {
      if (enable_debug_log) pr_err("%s enable irq\n", __func__);
      st21nfc_enable_irq(st21nfc_dev);
    } else {
      if (enable_debug_log) pr_err("%s irq already enabled\n", __func__);
    }
  }
  return mask;
}
...
#ifndef USE_NODE 
static ssize_t se_power_show(struct device *dev, struct device_attribute *attr,
                             char *buf) {
  struct st21nfc_device *data = dev_get_drvdata(dev);

  if (data != NULL)
    return scnprintf(buf, PAGE_SIZE, "%d\n", gpiod_get_value(data->gpiod_esepwr));
  return -ENODEV;
}

static ssize_t se_power_store(struct device *dev, struct device_attribute *attr,
                              const char *buf, size_t count) {
  struct st21nfc_device *data = dev_get_drvdata(dev);
  long new_se_pwr = 0;

  if (data != NULL && data->client != NULL) {
    if (!kstrtol(buf, 10, &new_se_pwr)) {
	  if (new_se_pwr == 0) {
		  /*power off */
		  
        pr_err("%s power off eSE set ESE PWR arg \n", __func__);

        gpiod_set_value(data->gpiod_esepwr, 0);
        usleep_range(2000, 4500);		  
	  } else if (new_se_pwr == 1) {
		pr_err("%s power on eSE set ESE PWR \n", __func__);
		/*power on */
        usleep_range(4000, 5000);

        gpiod_set_value(data->gpiod_esepwr, 1);
        usleep_range(1500, 2000);		  
	  } else {
		  return -EINVAL;
	  }
...
static DEVICE_ATTR_RW(se_power);
#endif
...
static const struct acpi_gpio_params esepwr_gpios = {4, 0, false};
...
static int st21nfc_probe(struct i2c_client *client,
...
#endif  // ST21NFCD_MTK
#ifdef CHECK_NFC_NONE_NFC
	if (strnstr(saved_command_line, "androidboot.hwc=GLOBAL_NFC_K7AN", strlen(saved_command_line)) != NULL) {
		pr_err("%s:CHECK_NFC_NONE_NFC androidboot.hwc=GLOBAL_NFC_K7AN : st21nfc_probe\n", __func__);
	}else if (strnstr(saved_command_line, "androidboot.hwc=GLOBAL_UKR_NFC_K7AN", strlen(saved_command_line)) != NULL) {
		pr_err("%s:CHECK_NFC_NONE_NFC androidboot.hwc=GLOBAL_UKR_NFC_K7AN : st21nfc_probe\n", __func__);
	}else {
		pr_err("%s:CHECK_NFC_NONE_NFC androidboot.hwc=GLOBAL_UKR_NFC_K7AN : EXIT st21nfc_probe\n", __func__);
		return -ENODEV;
	}
#endif
  ...
  if (I2CDMAReadBuf == NULL)
    pr_err("%s : failed to allocate dma buffer\n", __func__);
  pr_err("%s :I2CDMAWriteBuf_pa %d, I2CDMAReadBuf_pa,%d\n", __func__,
  ...
  if (ret) pr_err("Unable to add GPIO mapping table\n");
#else   // ST21NFCD_MTK
  ...
  pr_err("LQ_NFC_DEBUG:%s : request irq-gpios success\n", __func__);
  ...
  pr_err("LQ_NFC_DEBUG:%s : request reset-gpios success\n", __func__);

 st21nfc_dev->gpiod_esepwr = devm_gpiod_get(dev, "st,esepwr", GPIOD_OUT_HIGH);
 if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_esepwr)) {
    pr_err("%s : st21nfc get ese pwr gpio fail\n", __func__);
 } else {
    pr_err("%s : st21nfc get ese pwr gpio sucess\n", __func__);
 }
 //pr_err("LQ_NFC_DEBUG:%s : st21nfc not need get ese pwr gpio\n", __func__);
  ...
  if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_pidle)) {
	...
	pr_warn("LQ_NFC_DEBUG:[OPTIONAL] %s: not use request pidle-gpio\n", __func__);
    ret = 0;
  } else {
  ...
  if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_clkreq)) {
	...
	pr_err("LQ_NFC_DEBUG:[OPTIONAL] %s :request clkreq-gpios success\n", __func__);
    if (!device_property_read_bool(dev, "st,clk_pinctrl")) {
      pr_err("[dsc]%s:[OPTIONAL] clk_pinctrl not set\n", __func__);
	  pr_err("LQ_NFC_DEBUG:[dsc]%s:[OPTIONAL] clk_pinctrl not set\n", __func__);
      st21nfc_dev->pinctrl_en = 0;
    } else {
      pr_err("[dsc]%s:[OPTIONAL] clk_pinctrl set\n", __func__);
	  pr_err("LQ_NFC_DEBUG:[dsc]%s:[OPTIONAL] clk_pinctrl set\n", __func__);
    ...
    pr_err("LQ_NFC_DEBUG:[dsc]%s:[OPTIONAL] st21nfc_dev->clk_run = %d\n", __func__, st21nfc_dev->clk_run);
...
static int __init st21nfc_dev_init(void) {
  ...
  if (enable_debug_log) pr_err("Loading st21nfc i2c driver\n");
#endif
#endif
