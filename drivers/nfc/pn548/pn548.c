...
static bool insuspend;
static bool inabsorbing;
struct pn548_dev	{
	wait_queue_head_t	absorb_wq;
	wait_queue_head_t	read_wq;
	struct mutex		read_mutex;
	struct device		*dev;
	struct i2c_client	*client;
	struct miscdevice	pn548_device;
	struct task_struct *irq_resume_task;
...
static __ref int pn548irqCEresume_kthread(void *arg)
{
	struct pn548_dev *pn548_dev = (struct pn548_dev *)arg;
	struct sched_param param = {.sched_priority = MAX_RT_PRIO-1};
	char tmp[260] = {0};
	int ret, i, numByteToRead, retryCnt, noreadErrCnt;

	sched_setscheduler(current, SCHED_BATCH, &param);
	while (!kthread_should_stop()) {

		pn548_dev->do_absorbing = 0;
		pn548_enable_irq(pn548_dev);
		ret = wait_event_interruptible(pn548_dev->absorb_wq, pn548_dev->do_absorbing);
		...
		if (ret < 0) {
			...
			do {
				...
				for (i = 0; i < 60; i++) {
					...
					noreadErrCnt = 0;
					ret = i2c_master_recv(pn548_dev->client, tmp, 3);

					if (ret <= 0)
						break;
					numByteToRead = 0;
					numByteToRead = tmp[2];
					ret = i2c_master_recv(pn548_dev->client, tmp, numByteToRead);
				...
				pn548_dev->do_absorbing = 0;
				pn548_enable_irq(pn548_dev);
				ret = wait_event_timeout(pn548_dev->absorb_wq, pn548_dev->do_absorbing, 10);
				retryCnt--;
			} while (retryCnt > 0);
			inabsorbing = false;
...
static irqreturn_t pn548_dev_irq_handler(int irq, void *dev_id)
{
	...
	if ((true == insuspend) || (true == inabsorbing)) {
		pn548_disable_irq(pn548_dev);

		pn548_dev->do_absorbing = 1;

		/* Wake up waiting readers */
		wake_up(&pn548_dev->absorb_wq);

		if (false == inabsorbing) {
		...
		const struct i2c_device_id *id)
{
	...
	init_waitqueue_head(&pn548_dev->absorb_wq);
	pn548_dev->irq_resume_task = kthread_run(pn548irqCEresume_kthread, pn548_dev, "irqCEresume_thread");
...
static int pn548_remove(struct i2c_client *client)
{
	...
	kthread_stop(pn548_dev->irq_resume_task);
