...
int ntag5_read_data(u8* buf, int count, int flags) {
	...
	pr_info("%s count = %d, flags = %d, buf0 = %d enter \n", __func__, count, flags, buf[0]);
	...
	pr_info("%s ret = %d exit Happy ending\n", __func__, ret);
	return ret;
}
...
{
	int ret = 0;
	u8 kernelBuf[MAX_BUFFER_SIZE];
	pr_info("%s count  = %d\n", __func__,count);
	if (ntag5_read_data(kernelBuf, count, 0)) {
			  ...
			  size_t count, loff_t *offset)
{
	...
	if (copy_from_user(kernelBuf, buf, count)) {
		pr_err( "%s Failed to copy user buffer to kernel\n", __func__);
	}

	if (count == 5 || count == 6 || count == 2) {
		if(ntag5_write_data(kernelBuf, count , 0) >= 0) return count;
	}
	return ret;
}
...
int nfc_i2c_dev_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = nfc_ldo_config(&client->dev, ntag_dev);
	if (ret) {
