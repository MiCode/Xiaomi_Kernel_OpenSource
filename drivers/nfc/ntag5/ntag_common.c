...
int nfc_parse_dt(struct device *dev, struct platform_configs *nfc_configs)
{
	...
	// optional property
	ret = of_property_read_u32_array(np, NTAG5_LDO_VOL_DT_NAME,
			(u32 *) ldo->vdd_levels,
	...
	// optional property
	ret = of_property_read_u32(np, NTAG5_LDO_CUR_DT_NAME, &ldo->max_current);
	if (ret) {
...
int nfc_ldo_config(struct device *dev, struct ntag_dev *nfc_dev)
{
	...
	if (of_get_property(dev->of_node, NTAG5_LDO_SUPPLY_NAME, NULL)) {
		// Get the regulator handle
		nfc_dev->reg = regulator_get(dev, NTAG5_LDO_SUPPLY_DT_NAME);
