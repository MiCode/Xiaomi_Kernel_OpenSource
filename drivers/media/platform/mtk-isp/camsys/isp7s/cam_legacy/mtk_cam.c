...
static bool finish_sv_cq_buf(struct mtk_cam_request_stream_data *req_stream_data)
{
	...
	if (ctx->sv_dev == NULL || ctx->sv_dev->used_tag_cnt == 0)
		return false;
				...
				struct mtk_cam_scen *scen, s64 hw_mode)
{
	...
	int (*func_ptr_arr[3])(struct mtk_cam_ctx *, int);
	...
	func_ptr_arr[0] = get_main_sv_tag_id;
	func_ptr_arr[1] = get_sub_sv_tag_id;
	func_ptr_arr[2] = get_last_sv_tag_id;
	...
	for (rawi_idx = 0; rawi_idx < rawi_port_num; rawi_idx++) {
		if (is_otf || is_dc) {
			tag_idx = (is_dc && ((rawi_idx + 1) == rawi_port_num)) ?
				(*func_ptr_arr[2])(ctx,
...
static void check_buffer_mem_saving(
		...
		int rawi_port_num)
{
	if (req->feature.scen && mtk_cam_scen_is_hdr_save_mem(req->feature.scen)) {
		if (rawi_port_num == 2) {
				 ...
				 struct mtk_cam_request *cam_req)
{
	...
	for (rawi_idx = 0; rawi_idx < rawi_port_num; rawi_idx++) {
		tag_idx = (is_dc && ((rawi_idx + 1) == rawi_port_num)) ?
			(*func_ptr_arr[2])(ctx, ctx->sv_dev->enabled_tags) :
			(*func_ptr_arr[rawi_idx])(ctx, ctx->sv_dev->enabled_tags);
		input_node = ptr_rawi[rawi_idx];
				   ...
				   struct mtk_cam_request_stream_data *s_data)
{
	...
	dev_dbg(raw_pipe->subdev.v4l2_dev->dev,
		"%s:%s:%s:raw_scen(%s), prev_scen(%s), switch_feature_type(0x%0x), sensor_mode_update(0x%0x)\n",
		__func__, raw_pipe->subdev.name, debug_str,
		s_data->feature.scen->dbg_str,
	...
	for (i = 0 ; i < vb->num_planes; i++) {
		...
		if (mtk_cam_scen_is_mstream_2exp_types(scen)) {
			mtk_cam_mstream_buf_update(req, pipe_id, scen,
						desc_id, i, vb, f);
		} else if (mtk_cam_scen_get_exp_num(scen) == 3) {
			if (i == 0) { /* camsv1*/
				int in_node = MTKCAM_IPI_RAW_RAWI_2;
		...
		} else if (mtk_cam_scen_get_exp_num(scen) == 2) {
			if (i == 0) { /* camsv1*/
				int in_node = MTKCAM_IPI_RAW_RAWI_2;
			      ...
			      struct mtk_cam_request *req)
{
	...
	list_for_each_entry_safe(obj, obj_prev, &req->req.objects, list) {
		...
		/*
		 * TBC: Ryan, James
		 * Do camsv and mraw s_data need it?
		 */
		if (mtk_cam_scen_is_mstream_2exp_types(&scen)) {
		...
		case MTKCAM_IPI_CAMSV_MAIN_OUT:
			ret = mtk_cam_config_sv_img_out_imgo(req_stream_data, buf);
			if (ret)
				return ret;
			ret = mtk_cam_camsv_update_fparam(req_stream_data, buf);
			if (ret)
				return ret;
			break;
...
int get_main_sv_tag_id(struct mtk_cam_ctx *ctx,
							int used_tags)
{
	unsigned int i;
	...
	for (i = SVTAG_IMG_START; i < SVTAG_IMG_END; i++) {
		if (used_tags & (1 << i))
			if (ctx->sv_dev->tag_info[i].tag_order == MTKCAM_IPI_ORDER_FIRST_TAG)
				return i;
	}
	return -1;
}

int get_sub_sv_tag_id(struct mtk_cam_ctx *ctx,
							int used_tags)
{
	unsigned int i;
	...
	for (i = SVTAG_IMG_START; i < SVTAG_IMG_END; i++) {
		if (used_tags & (1 << i))
			if (ctx->sv_dev->tag_info[i].tag_order == MTKCAM_IPI_ORDER_NORMAL_TAG)
				return i;
	}
	return -1;
}

int get_last_sv_tag_id(struct mtk_cam_ctx *ctx,
							int used_tags)
{
	unsigned int i;
	...
	for (i = SVTAG_IMG_START; i < SVTAG_IMG_END; i++) {
		if (used_tags & (1 << i))
			if (ctx->sv_dev->tag_info[i].tag_order == MTKCAM_IPI_ORDER_LAST_TAG)
				return i;
	}
	return -1;
}
...
void mtk_cam_sv_reset_tag_info(struct mtk_cam_ctx *ctx)
{
	int i;

	for (i = SVTAG_START; i < SVTAG_END; i++) {
		ctx->sv_dev->tag_info[i].sv_pipe = NULL;
		ctx->sv_dev->tag_info[i].seninf_padidx = 0;
		ctx->sv_dev->tag_info[i].hw_scen = 0;
		ctx->sv_dev->tag_info[i].tag_order = MTKCAM_IPI_ORDER_FIRST_TAG;
		/* camsv todo: stride may not be necessary */
		ctx->sv_dev->tag_info[i].stride = 0;
		/*
		 * camsv todo: check that all params in
		 * cfg_in_param is covered when next dev_config
		 */
		atomic_set(&ctx->sv_dev->tag_info[i].is_config_done, 0);
		atomic_set(&ctx->sv_dev->tag_info[i].is_stream_on, 0);
	}
...
unsigned int mtk_cam_get_sv_idle_tags(
	struct mtk_cam_ctx *ctx, int hw_scen, int exp_no, int req_amount)
{
	...
	if (hw_scen & (1 << HWPATH_ID(MTKCAM_IPI_HW_PATH_STAGGER))) {
		...
		if (req_amount == 1)
			used_tags = (1 << SVTAG_3);
		else if (req_amount == 2)
			used_tags = (1 << SVTAG_2) | (1 << SVTAG_3);
	...
	if (camsv_dev->enabled_tags & used_tags) {
		used_tags = 0;
		dev_info(camsv_dev->dev, "no available sv tags: hw_scen:%d exp_no:%d req_amount:%d enabled_tags:0x%x",
			hw_scen, exp_no, req_amount, camsv_dev->enabled_tags);
	}
	...
	struct v4l2_mbus_framefmt *mf, struct v4l2_format *img_fmt)
{
	...
	unsigned int seninf_padidx, sv_cammux_id;
	...
	for (i = SVTAG_IMG_START; i < SVTAG_IMG_END; i++) {
		if (ctx->pipe->enabled_sv_tags & (1 << i)) {
			...
			mtk_cam_call_sv_pipeline_config(ctx, i, seninf_padidx,
	...
	bool streaming, bool config_pipe)
{
	...
	if (config_pipe && mtk_cam_scen_is_stagger(scen)) {
		...
		idle_tags = mtk_cam_get_sv_idle_tags(ctx, hw_scen, exp_no, req_amount);
		/* cached used sv tags */
	...
	for (i = 0; i < ctx->used_sv_num; i++) {
		...
		idle_tags = mtk_cam_get_sv_idle_tags(ctx, 1, 1, 1);
		if (idle_tags == 0) {
		...
		mtk_cam_call_sv_pipeline_config(ctx, tag_idx,
			ctx->sv_pipe[i]->seninf_padidx, 1, MTKCAM_IPI_ORDER_NORMAL_TAG,
			3, cfg_in_param->subsample,
			ctx->sv_pipe[i], mf, img_fmt);
	}

	/* camsv's group setup */
	mtk_cam_sv_set_group_info(ctx->sv_dev);
...
int mtk_cam_dev_config(struct mtk_cam_ctx *ctx, bool streaming, bool config_pipe)
{
	...
	if (config_pipe && mtk_cam_scen_is_stagger(scen_active)) {
		...
		mtk_cam_sv_img_tag_update(ctx, hw_scen, exp_no,
			cfg_in_param->subsample, mf, img_fmt);
	} else if (config_pipe && mtk_cam_scen_is_with_w_channel(scen_active)) {
		unsigned int hw_scen, exp_no, req_amount, idle_tags;
...
#ifdef CAMSV_TODO /* camsv todo: preisp */
...
static void mtk_cam_ctx_watchdog_worker(struct work_struct *work)
{
	...
	if (atomic_read(&ctx->watchdog_dumped)) {
		...
		if (timeout) {
			...
			mtk_cam_seninf_dump(seninf, dequeued_frame_seq_no, true);
...
void mtk_cam_resource_to_v2(struct mtk_cam_resource_v2 *res_v2,
			    ...
			    struct v4l2_mbus_framefmt *sink_fmt)
{
	res_v2->sensor_res.width = sink_fmt->width;
	res_v2->sensor_res.height = sink_fmt->height;
	res_v2->sensor_res.code = sink_fmt->code;
	res_v2->sensor_res.interval = res->sensor_res.interval;
	res_v2->sensor_res.hblank =  res->sensor_res.hblank;
	res_v2->sensor_res.vblank = res->sensor_res.vblank;
	res_v2->sensor_res.pixel_rate = res->sensor_res.pixel_rate;
	/* cust_pixel_rate should be phased-out */
	if (res->sensor_res.cust_pixel_rate > 0) {
		res_v2->sensor_res.pixel_rate =
			res->sensor_res.cust_pixel_rate;
		res_v2->sensor_res.no_bufferd_prate_calc = true;
	}

	/**
	 * sensor_res' driver_buffered_pixel_rate should be phased-out since
	 * the use can simply use the (width * height) to select the
	 * sensor mode which required the max throughput among
	 * the seamless switch sensor modes (because their mipi pixel
	 * rates are the same.)
	 */

	mtk_cam_feature_to_scen(&res_v2->raw_res.scen,
				res->raw_res.feature, res->raw_res.feature);
	/* can't be convert raw_max/raw_min to raws/raws_must (twin case) */
	res_v2->raw_res.raws_must = 0;
	res_v2->raw_res.raws = 0; /* use current available raws */

	res_v2->raw_res.bin = res->raw_res.bin;
	res_v2->raw_res.hw_mode = res->raw_res.hw_mode;
}
			    ...
			    struct mtk_cam_resource_v2 *res_v2)
{
	...
	if (sink_fmt) {
		sink_fmt->width = res_v2->sensor_res.width;
		sink_fmt->height =  res_v2->sensor_res.height;
		sink_fmt->code = res_v2->sensor_res.code;
	}

	res->sensor_res.interval = res_v2->sensor_res.interval;
	res->sensor_res.hblank = res_v2->sensor_res.hblank;
	res->sensor_res.vblank = res_v2->sensor_res.vblank;

	/* cust_pixel_rate should be phased-out */
	if (res_v2->sensor_res.no_bufferd_prate_calc) {
		res->sensor_res.cust_pixel_rate = res_v2->sensor_res.pixel_rate;
		res->sensor_res.pixel_rate = 0;
	} else {
		res->sensor_res.pixel_rate = res_v2->sensor_res.pixel_rate;
	}
	res->sensor_res.driver_buffered_pixel_rate = res_v2->sensor_res.driver_buffered_pixel_rate;

	mtk_cam_scen_to_feature(&feature, &res_v2->raw_res.scen);
	res->raw_res.feature = feature;

	/* can't be convert raw_max/raw_min to raws/raws_must (twin case) */
	res->raw_res.raw_used = 1;
	res->raw_res.bin = res_v2->raw_res.bin;
	res->raw_res.hw_mode = res_v2->raw_res.hw_mode;

	/* TODO: return img_buf_sz and max_img_buf_sz */
