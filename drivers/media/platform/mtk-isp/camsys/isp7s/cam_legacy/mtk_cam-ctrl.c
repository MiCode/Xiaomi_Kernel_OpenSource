	...
	list_for_each_entry(state_entry, &sensor_ctrl->camsys_state_list,
		...
		if (req_stream_data->frame_seq_no ==
			...
			if (state_entry->estate == E_STATE_CQ && USINGSCQ &&
				...
				return;
			} else if (mtk_cam_scen_is_ext_isp(&ctx->pipe->scen_active) &&
				       ...
				       struct mtk_camsys_irq_info *irq_info)
{
	...
	/* update sv/mraw's ts */
	if (0/*mtk_cam_sv_update_all_buffer_ts(ctx, irq_info->ts_ns) == 0*/)
