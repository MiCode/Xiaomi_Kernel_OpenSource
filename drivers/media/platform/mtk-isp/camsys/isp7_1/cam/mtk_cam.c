	...
	list_for_each_entry_safe(s_data, s_data_prev, &s_data_clean_list,
		...
		if (need_clean_s_data) {
			dev_info(cam->dev,
				 ...
				 __func__, req->req.debug_str, pipe_id,
				 s_data->frame_seq_no);
			mtk_cam_req_s_data_clean(s_data);
				...
				const struct v4l2_format *cfg_fmt)
{
	...
	in_fmt = &req_stream_data->frame_params.img_ins[input_node - MTKCAM_IPI_RAW_RAWI_2];
				...
				int sd_width, int sd_height)
{
	...
	config_img_in_fmt_mstream(cam, req_stream_data, node, cfg_fmt);
				...
				struct mtk_cam_request *req)
{
	...
	if (in_fmt->buf[0].iova == 0x0) {
		...
		config_img_fmt_mstream(cam, ctx, req, cfg_fmt, vdev,
...
{
	int raw_feature =
		ctx->pipe->feature_pending & MTK_CAM_FEATURE_HDR_MASK;
			      ...
			      struct mtk_cam_request *req)
{
	...
	list_for_each_entry_safe(obj, obj_prev, &req->req.objects, list) {
		...
		cfg_fmt = mtk_cam_s_data_get_vfmt(req_stream_data, node->desc.id);
		cfg_selection = mtk_cam_s_data_get_vsel(req_stream_data, node->desc.id);
		...
		case MTKCAM_IPI_RAW_DRZS4NO_3:
			pfmt = mtk_cam_s_data_get_pfmt(req_stream_data, MTK_RAW_SINK);
			sd_width = pfmt->width;
			...
			out_fmt->crop.p.x = cfg_selection->r.left;
			out_fmt->crop.p.y = cfg_selection->r.top;
			out_fmt->crop.s.w = cfg_selection->r.width;
			out_fmt->crop.s.h = cfg_selection->r.height;

			ret = config_img_fmt(cam, node, cfg_fmt, out_fmt,
					     sd_width, sd_height);
			if (ret)
				return ret;
			if (mtk_cam_is_stagger(ctx))
...
static void isp_tx_frame_worker(struct work_struct *work)
{
	...
	if (ctx->used_mraw_num > 0) {
		...
		mtk_cam_mraw_wbuf_set_s_data(mraw_buf_entry, req_stream_data);
	...
	for (i = 0; i < ctx->used_mraw_num; i++) {
		/* Copy mraw's frame_param to raw's */
		mraw_subdev_id = ctx->mraw_pipe[i]->id;
		req_stream_data_mraw = mtk_cam_req_get_s_data(req, mraw_subdev_id, 0);
		req_stream_data->frame_params.mraw_param[i]
			= req_stream_data_mraw->frame_params.mraw_param[0];

		/* Assign mraw's pixel_mode to raw's frame_param */
		req_stream_data->frame_params.mraw_param[i].pixel_mode
			= ctx->mraw_pipe[i]->res_config.pixel_mode;

		dev_dbg(cam->dev, "%s: mraw subdev_id:%d tg_size_w:%d pixel_mode:%d\n",
			__func__,
			mraw_subdev_id,
...
bool mtk_cam_sv_req_enqueue(struct mtk_cam_ctx *ctx, struct mtk_cam_request *req)
{
	...
	for (i = 0 ; i < ctx->used_sv_num ; i++) {
		...
		req->p_data[pipe_id].s_data_num = 1;
...
void mtk_cam_stop_ctx(struct mtk_cam_ctx *ctx, struct media_entity *entity)
{
	...
	INIT_LIST_HEAD(&ctx->mraw_using_buffer_list.list);
	INIT_LIST_HEAD(&ctx->mraw_composed_buffer_list.list);
	INIT_LIST_HEAD(&ctx->mraw_processing_buffer_list.list);
			     ...
			     unsigned int stream_id)
{
	...
	spin_lock_init(&ctx->mraw_using_buffer_list.lock);
