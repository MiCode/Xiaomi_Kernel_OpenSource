...
#define MAP_SHMEM_MM_CACHEABLE_END                                             \
	(MAP_SHMEM_MM_CACHEABLE_BASE + MAP_SHMEM_MM_RANGE)
...
struct mtk_vcu {
	...
	wait_queue_head_t gce_wq[2];
	atomic_t gce_flush_done[2];
};
		 ...
		 unsigned int len)
{
	...
	if (id <= IPI_VCU_INIT || id >= IPI_MAX ||
		...
		dev_err(&pdev->dev,
			"[VCU] failed to send ipi message (Invalid arg.)\n");
		return -EINVAL;
	}
	...
	if (!vcu->fuse_bypass) {
		...
		ret = vcu->file->f_op->compat_ioctl(
			vcu->file, (unsigned int)VCU_SET_OBJECT,
			(unsigned long)&send_obj);
#else
		ret = vcu->file->f_op->unlocked_ioctl(
			vcu->file, (unsigned int)VCU_SET_OBJECT,
			(unsigned long)&send_obj);
#endif
		set_fs(old_fs);
	...
	if (ret == 0) {
		...
		dev_err(&pdev->dev,
			"vcu ipi %d ack wait interrupted by a signal", id);
		ret = -ERESTARTSYS;
		goto end;
	} else
		ret = 0;
...
static void vcu_set_gce_cmd(struct cmdq_pkt *pkt, struct mtk_vcu *vcu,
			    unsigned char cmd, u64 addr, u64 data, u32 mask)
{
	switch (cmd) {
	case CMD_READ:
	...
	case CMD_WAIT_EVENT:
		...
		break;
	case CMD_MEM_MV:
		cmdq_pkt_mem_move(pkt, vcu->clt_base, addr, data,
				  CMDQ_THR_SPR_IDX1);
		break;
	default:
...
static void vcu_gce_flush_callback(struct cmdq_cb_data data)
{
	...
	i = buff->cmdq_buff.codec_type ? 1 : 0;

	vcu = buff->vcu_ptr;
	atomic_inc(&vcu->gce_flush_done[i]);
	wake_up(&vcu->gce_wq[i]);

	pr_info("[VCU] %s: buff %p type %d cnt %d order %d handle %llx\n",
		__func__, buff, buff->cmdq_buff.codec_type,
		buff->cmdq_buff.cmd_cnt, buff->cmdq_buff.flush_order,
...
static int vcu_gce_cmd_flush(struct mtk_vcu *vcu, unsigned long arg)
{
	...
	buff = kmalloc(sizeof(struct gce_callback_data), GFP_KERNEL);
	...
	for (i = 0; i < GCE_EVENT_MAX; i++) {
		if (vcu->gce_codec_eid[i] != -1)
			cmdq_pkt_clear_event(pkt_ptr, vcu->gce_codec_eid[i]);
	}

	for (i = 0; i < buff->cmdq_buff.cmd_cnt; i++) {
		vcu_set_gce_cmd(pkt_ptr, vcu, buff->cmdq_buff.cmd[i],
			buff->cmdq_buff.addr[i],
			buff->cmdq_buff.data[i],
			buff->cmdq_buff.mask[i]);
	}
...
static int vcu_wait_gce_callback(struct mtk_vcu *vcu, unsigned long arg)
{
	...
	i = obj.codec_type ? 1 : 0;
	pr_info("[VCU] %s: type %d handle %llx\n", __func__, obj.codec_type,
		obj.gce_handle);

	ret = wait_event_freezable(vcu->gce_wq[i],
			...
			const char *expected_version)
{
	...
	cur_ver_num = sscanf(cur_version, "%d.%d.%d-rc%d", &cur_major,
			     &cur_minor, &cur_build, &cur_rel);
	if (cur_ver_num < 3)
		return -1;
	ver_num = sscanf(expected_version, "%d.%d.%d-rc%d", &major, &minor,
			 &build, &rel);
	if (ver_num < 3)
		return -1;
...
static int vcu_ipi_handler(struct mtk_vcu *vcu, struct share_obj *rcv_obj)
{
	...
	if (rcv_obj->id < (int)IPI_MAX &&
		...
		non_ack = ipi_desc[rcv_obj->id].handler(
			rcv_obj->share_buf, rcv_obj->len,
...
static int vcu_alloc_d_ext_mem(struct mtk_vcu *vcu, unsigned long len)
{
	...
	dev_dbg(vcu->dev,
		...
		VCU_DMEM0_VIRT(vcu), (unsigned long long)VCU_DMEM0_IOVA(vcu));
	return 0;
}
				   ...
				   unsigned long arg)
{
	...
	case VCU_SET_OBJECT:
		...
		ret = (long)copy_from_user(
			&share_buff_data, user_data_addr,
			(unsigned long)sizeof(struct share_obj));
		...
		ret = (long)copy_to_user(
			user_data_addr, &share_buff_data,
			(unsigned long)sizeof(struct share_obj));
		if (ret != 0L) {
	...
	case VCU_GET_OBJECT:
		ret = vcu_ipi_get(vcu_dev, arg);
			pr_debug("[VCU] %s(%d) VCU_GET_OBJECT!\n",
				 __func__, __LINE__);
		break;
	...
	case VCU_PA_ALLOCATION:
		...
		if (cmd == VCU_MVA_ALLOCATION) {
			...
			mem_priv = cmdq_mbox_buf_alloc(dev,
				(dma_addr_t *)&mem_buff_data.pa);
			//mem_buff_data.va = (u64)mem_priv;
			//mem_buff_data.pa = (u64)virt_to_phys(mem_priv);
	...
	case VCU_PA_FREE:
		...
		ret = (long)copy_from_user(
			...
			(unsigned long)sizeof(struct mem_obj));
		if ((ret != 0L) ||
		    (mem_buff_data.iova == 0UL && mem_buff_data.va == 0UL)) {
		...
		//else
			//cmdq_mbox_buf_free(dev, (void *)mem_buff_data.va,
					   //(dma_addr_t)mem_buff_data.pa);
	...
	case VCU_CACHE_INVALIDATE_BUFF:
		...
		vcu_buffer_cache_sync(
			...
			(cmd == VCU_CACHE_FLUSH_BUFF) ? DMA_TO_DEVICE
						      : DMA_FROM_DEVICE);
...
#if IS_ENABLED(CONFIG_COMPAT)
static int compat_get_vpud_allocation_data(struct compat_mem_obj __user *data32,
...
static int compat_put_vpud_allocation_data(struct compat_mem_obj __user *data32,
					   struct mem_obj __user *data)
{
	compat_ulong_t l;
	compat_u64 u;
					  ...
					  unsigned long arg)
{
	...
	case COMPAT_VCU_PA_ALLOCATION:
		...
		if (cmd == COMPAT_VCU_MVA_ALLOCATION)
			ret = file->f_op->unlocked_ioctl(
				file, (uint32_t)VCU_MVA_ALLOCATION,
				(unsigned long)data);
		else
			ret = file->f_op->unlocked_ioctl(
				file, (uint32_t)VCU_PA_ALLOCATION,
				(unsigned long)data);
	...
	case COMPAT_VCU_PA_FREE:
		...
		if (cmd == COMPAT_VCU_MVA_FREE)
			ret = file->f_op->unlocked_ioctl(file,
							 (uint32_t)VCU_MVA_FREE,
							 (unsigned long)data);
		else
			ret = file->f_op->unlocked_ioctl(file,
							 (uint32_t)VCU_PA_FREE,
							 (unsigned long)data);
		err = compat_put_vpud_allocation_data(data32, data);
		if (err != 0)
			return err;
	...
	case COMPAT_VCU_CACHE_FLUSH_ALL:
		ret = file->f_op->unlocked_ioctl(
			file, (uint32_t)VCU_CACHE_FLUSH_ALL, 0);
		break;
	...
	case COMPAT_VCU_CACHE_INVALIDATE_BUFF:
		...
		if (cmd == COMPAT_VCU_CACHE_FLUSH_BUFF) {
			ret = file->f_op->unlocked_ioctl(
				file, (uint32_t)VCU_CACHE_FLUSH_BUFF,
				(unsigned long)data);
		} else {
			ret = file->f_op->unlocked_ioctl(
				file, (uint32_t)VCU_CACHE_INVALIDATE_BUFF,
				(unsigned long)data);
		}
...
static int mtk_vcu_write(const char *val, const struct kernel_param *kp)
{
	...
	if (vcu_ptr->vdec_log_info != NULL) {
		ret = param_set_charp(val, kp);
		if (ret != 0)
			return -EINVAL;
		memcpy(vcu_ptr->vdec_log_info->log_info, vcodec_param_string,
		       LOG_INFO_SIZE);
	} else
		return -EFAULT;
	...
	pr_debug("[log wakeup VPUD] %s %s %d\n",
		 (char *)vcu_ptr->vdec_log_info->log_info, vcodec_param_string,
		 (int)strlen(vcodec_param_string));

	return 0;
}

static struct kernel_param_ops log_param_ops = {
	.set = mtk_vcu_write, .get = param_get_charp,
};
...
static int mtk_vcu_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_VIDEO_MEDIATEK_VCU_WO_FUSE
		vcu_mtkdev[vcuid]->vdec_log_info = devm_kzalloc(
			dev, sizeof(struct log_test_nofuse), GFP_KERNEL);
#endif
	...
	if (vcuid == 0) {
		for (i = 0; i < (int)VCU_MAP_HW_REG_NUM; i++) {
			...
			dev_dbg(dev, "[VCU] base[%d]: 0x%lx 0x%lx", i,
				vcu->map_base[i].base, vcu->map_base[i].len);
		}
	}
	...
	init_waitqueue_head(&vcu->gce_wq[0]);
	init_waitqueue_head(&vcu->gce_wq[1]);
	...
	atomic_set(&vcu->vdec_log_got, 0);
	atomic_set(&vcu->gce_flush_done[0], 0);
	atomic_set(&vcu->gce_flush_done[1], 0);
	...
	vcu_mtkdev[vcuid]->vcu_class =
		class_create(THIS_MODULE, vcu_mtkdev[vcuid]->vcuname);
	...
	vcu_mtkdev[vcuid]->vcu_device = device_create(
		vcu_mtkdev[vcuid]->vcu_class, NULL,
	...
	vcu->clt_vdec = cmdq_mbox_create(dev, 0);
	vcu->clt_venc = cmdq_mbox_create(dev, 1);
	...
	vcu->gce_codec_eid[VENC_EOF] = cmdq_dev_get_event(dev, "venc_eof");
