...
static void get_pic_info(struct vdec_vp8_inst *inst, struct vdec_pic_info *pic)
{
	*pic = inst->vsi->pic;
	...
	mtk_vcodec_debug(inst, "pic(%d, %d), buf(%d, %d)", pic->pic_w,
			 pic->pic_h, pic->buf_w, pic->buf_h);
...
static void get_free_fb(struct vdec_vp8_inst *inst, struct vdec_fb **out_fb)
{
	...
	if (node) {
		...
		mtk_vcodec_debug(inst, "[FB] get free fb %p st=%d", node->fb,
				 fb->status);
	} else {
		fb = NULL;
...
static void get_crop_info(struct vdec_vp8_inst *inst, struct v4l2_rect *cr)
{
	...
	mtk_vcodec_debug(inst, "get crop info l=%d, t=%d, w=%d, h=%d", cr->left,
			 cr->top, cr->width, cr->height);
}
