...
static struct mtk_video_fmt *mtk_find_fmt_by_pixel(unsigned int pixelformat)
{
	...
	mtk_v4l2_err("Error!! Cannot find fourcc: %d", pixelformat);
...
static struct vb2_buffer *get_display_buffer(struct mtk_vcodec_ctx *ctx)
{
	...
	if (dstbuf->used) {
		...
		mtk_v4l2_debug(2, "[%d]status=%x queue id=%d to done_list %d",
			       ...
			       dstbuf->vb.vb2_buf.index, dstbuf->queued_in_vb2);
...
static void mtk_vdec_pic_info_update(struct mtk_vcodec_ctx *ctx)
{
	...
	mtk_v4l2_debug(1, "[%d]-> new(%d,%d), old(%d,%d), bit(%d) real(%d,%d)",
		       ctx->id, ctx->last_decoded_picinfo.pic_w,
		       ctx->last_decoded_picinfo.pic_h, ctx->picinfo.pic_w,
		       ctx->picinfo.pic_h, ctx->picinfo.bitdepth,
...
static void mtk_vdec_worker(struct work_struct *work)
{
	...
	mtk_v4l2_debug(
		...
		dst_buf->index, pfb, pfb->fb_base[0].va,
		&pfb->fb_base[0].dma_addr, &pfb->fb_base[1].dma_addr,
		pfb->fb_base[0].dmabuf, pfb->fb_base[1].dmabuf,
		pfb->fb_base[0].size);
	...
	mtk_v4l2_debug(
		...
		ctx->id, buf->va, &buf->dma_addr, buf->size, buf->length,
		buf->dmabuf, src_buf, src_buf_info->lastframe,
		src_buf_info->vb.vb2_buf.timestamp);
	dst_buf_info->vb.vb2_buf.timestamp = src_buf_info->vb.vb2_buf.timestamp;
	...
	if (ret < 0 || mtk_vcodec_unsupport) {
		...
		/*
		 * Getting early eos frame buffer, after decode this
		 * buffer, need to flush decode. Change the last
		 * frame buffer status from EOS_WITH_DATA to
		 * EOS_WITH_DATA_DRAIN, and flush decode.
		 */
...
static const struct v4l2_ctrl_ops mtk_vcodec_dec_ctrl_ops = {
	.g_volatile_ctrl = mtk_vdec_g_v_ctrl, .s_ctrl = mtk_vdec_s_ctrl,
};
