...
static ssize_t ispv4_probe_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct ispv4_spi_dev *sdev = NULL;
	struct ispv4_v4l2_dev *priv;

	sdev = dev_get_drvdata(dev);
	if (!sdev || !sdev->priv) {
		dev_err(dev, "ispv4 spi dev:0x%lx or v4l2 dev are NULL!!\n",
			(unsigned long)sdev);
		return -EINVAL;
	}

	priv = (struct ispv4_v4l2_dev *)sdev->priv;
	return sysfs_emit(buf, "%d\n", priv->probe_finish);
}
static DEVICE_ATTR_RO(ispv4_probe);
