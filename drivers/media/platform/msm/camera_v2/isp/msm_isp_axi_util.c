	...
	enum msm_vfe_input_src frame_src, struct msm_isp_timestamp *ts)
{
	...
	if ((dual_hw_type == DUAL_HW_MASTER_SLAVE) &&
		 ...
		 */
		prev_frame_id = vfe_dev->axi_data.src_info[frame_src].frame_id ;
		vfe_dev->axi_data.src_info[frame_src].frame_id =
			master_sof_info->frame_id + sof_incr;
		if (vfe_dev->axi_data.src_info[frame_src].frame_id - prev_frame_id > 1 ||
			vfe_dev->axi_data.src_info[frame_src].frame_id < 15) {
			pr_err("%s: Slave frame ID jump from %d to %d\n", __func__, prev_frame_id,
				vfe_dev->axi_data.src_info[frame_src].frame_id);

			pr_err("%s: vfe %d frame_src %d frame %d Slave time %d Master time %d delta %d\n",
				__func__, vfe_dev->pdev->id, frame_src,
				vfe_dev->axi_data.src_info[frame_src].frame_id,
				time, master_time, time - master_time);
		}
	...
	if (dual_hw_type == DUAL_HW_MASTER_SLAVE &&
		...
		sof_info->mono_timestamp_ms = ts->buf_time.tv_sec * 1000 +
			ts->buf_time.tv_usec / 1000;
		pr_err("%s: vfe %d frame_src %d  Slave/Master %d frame %d time %d\n",
			__func__, vfe_dev->pdev->id, frame_src, ms_type,
			vfe_dev->axi_data.src_info[frame_src].frame_id,
	...
	struct msm_vfe_axi_stream *stream_info, uint32_t pingpong_bit)
{
	...
	pr_err_ratelimited("%s:%d session_id:%d, stream_id:%d,undelivered_request_cnt:%d\n",
			__func__, __LINE__,
			temp_stream_info->session_id,
			temp_stream_info->stream_id,
	...
	uint32_t frame_id, uint32_t buf_index)
{
	...
	pr_err_ratelimited("%s:%d session_id:%d, stream_id:%d,undelivered_request_cnt:%d, stream_info->sw_ping_pong_bit:%d, dual_vfe:%d\n",
			__func__, __LINE__,
