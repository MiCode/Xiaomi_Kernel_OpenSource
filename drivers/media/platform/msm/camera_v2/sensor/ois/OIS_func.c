...
ADJ_STS	func_SET_SCENE_PARAM_for_NewGYRO_Fil(OIS_UBYTE u16_scene, OIS_UBYTE u16_mode, OIS_UBYTE filter, OIS_UBYTE range, const _FACT_ADJ *param)	// RHM_HT 2013/04/15	Change "typedef" of return value
{
	...
	OIS_UWORD u16_dat_SCENE_NIGHT_3[4] = { 0x7FE0,   ANGLE_LIMIT,   G_SENSE * 16,   0x0200,  };	// 16dps	RHM_HT 2014/11/27	Changed Kgxdr at Xiaomi
	...
	OIS_UWORD u16_dat_SCENE_D_A_Y_3[4] = { 0x7FE0,   ANGLE_LIMIT,   G_SENSE * 40,   0x0300,  };	// 40dps	RHM_HT 2014/11/27	Changed Kgxdr at Xiaomi
	...
	//OIS_UWORD u16_dat_SCENE_SPORT_3[4] = { 0x7FE0,   ANGLE_LIMIT,   G_SENSE * 60,   0x0300,  };	// 60dps	RHM_HT 2015/12/08	Changed for Xiaomi A1
	OIS_UWORD u16_dat_SCENE_SPORT_2[4] = { 0x7FFF,   ANGLE_LIMIT,   G_SENSE * 60,   0x0080,  };	// 60dps	RHM_HT 2016/02/25 //for video
	OIS_UWORD u16_dat_SCENE_SPORT_3[4] = { 0x7FC0,   ANGLE_LIMIT,   G_SENSE * 60,   0x0080,  };	// 60dps	RHM_HT 2016/02/25 //for capture
...
// ==> RHM_HT 2015/09/12	Added for Shift OIS
void	EnableShiftOIS( void )					// Shift OIS gain at 10cm
{
	I2C_OIS_per_write( 0x8B, DEFAULT_10CM_GAIN );			// RHM_HT 2015/10/12	Modified from 0xAD0
	I2C_OIS_per_write( 0xCB, DEFAULT_10CM_GAIN );			// RHM_HT 2015/10/12	Modified from 0xAD0
	I2C_OIS_mem_write( 0x10, 0x0000 );			// Disable Gyro OIS
}

void	DisableShiftOIS( void )
{
	I2C_OIS_mem_write( 0x10, 0x7FFF );			// Enable Gyro OIS
	I2C_OIS_per_write( 0x8B, 0x0000 );
	I2C_OIS_per_write( 0xCB, 0x0000 );
}
// ==> RHM_HT 2015/09/12	Added for Shift OIS

// ==> RHM_HT 2015/10/24	Added to change Shift OIS gain
void	ChangeShiftOISGain( int distance )			// Unit [cm]
{
	int shift_gain = DEFAULT_10CM_GAIN * 10 / distance;

	// pr_info("[OIS] set shift gain for distance %d to %d \n",
	// 	distance, shift_gain);
