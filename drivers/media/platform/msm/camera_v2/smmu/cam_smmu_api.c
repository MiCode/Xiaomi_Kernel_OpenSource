...
static int cam_smmu_attach(int idx)
{
	...
	if (iommu_cb_set.cb_info[idx].state == CAM_SMMU_ATTACH) {
		pr_err("%s: Error: index %d got attached before\n",
				__func__, idx);
		ret = -EINVAL;
...
static int cam_smmu_detach(int idx)
{
	...
	if (iommu_cb_set.cb_info[idx].state == CAM_SMMU_DETACH) {
		pr_err("%s: Error: Index %d got detached before\n",
				__func__, idx);
		ret = -EINVAL;
		 ...
		 unsigned long *len_ptr)
{
	...
	mapping_info->len = (unsigned long)sg_dma_len(table->sgl);
	...
	*len_ptr = (unsigned long)sg_dma_len(table->sgl);
	...
	struct device *dev)
{
	...
	/*
	 * set the domain attributes disable L2 redirect
	 * since it decreases performance
	*/
