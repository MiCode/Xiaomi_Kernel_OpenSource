			...
			const struct v4l2_event_subscription *sub)
{
	...
	if (sub->type == V4L2_EVENT_ALL) {
		rc = 0;
		s = *sub;
		s.type = V4L2_EVENT_PRIVATE_START +
				VCAP_GENERIC_NOTIFY_EVENT;
		s.id = 0;
		do {
			rc = v4l2_event_subscribe(fh, &s, 16, NULL);
			if (rc < 0) {
				s.type = V4L2_EVENT_ALL;
				v4l2_event_unsubscribe(fh, &s);
				return rc;
			}
			s.type++;
		} while (s.type !=
		...
		rc = v4l2_event_subscribe(fh, sub, 16, NULL);
	}
	return rc;
}
