					...
					size_t length, loff_t *offset)
{
	...
	printk(KERN_ERR "N19 IR-SPI chardev write start\n");
...
out_unlock:
	...
	printk(KERN_ERR "N19 IR-SPI chardev write end\n");
...
static int ir_spi_chardev_open(struct inode *inode, struct file *file)
{
	...
	printk(KERN_ERR "N19 IR-SPI chardev open start\n");
	...
	printk(KERN_ERR "N19 IR-SPI chardev open end\n");
	return 0;
}
...
static int ir_spi_chardev_close(struct inode *inode, struct file *file)
{
	...
	printk(KERN_ERR "N19 IR-SPI chardev close start\n");
	mutex_lock(&idata->mutex);
	idata->nusers--;
	...
	printk(KERN_ERR "N19 IR-SPI chardev close end\n");
	return 0;
}
						...
						unsigned long arg)
{
	...
	printk(KERN_ERR "N19 IR-SPI chardev ioctl start\n");
	switch (cmd) {
	...
	printk(KERN_ERR "N19 IR-SPI chardev ioctl end\n");
	return -EINVAL;
}
...
static int ir_spi_probe(struct spi_device *spi)
{
	struct ir_spi_data *idata;
	u8 *buffer = NULL;
	printk(KERN_ERR "N19 IR-SPI probe start\n");
	idata = devm_kzalloc(&spi->dev, sizeof(*idata), GFP_KERNEL);
	if (!idata)
		return -ENOMEM;

	printk(KERN_ERR "N19 IR-SPI mutex init\n");
	mutex_init(&idata->mutex);

	idata->spi = spi;
	printk(KERN_ERR "N19 IR-SPI set drvdata\n");
	spi_set_drvdata(spi, idata);
	ir_spi_data_g = idata;

	idata->xfer.bits_per_word = IR_SPI_BIT_PER_WORD;
	idata->xfer.speed_hz = IR_SPI_DEFAULT_FREQUENCY;
	printk(KERN_ERR "N19 IR-SPI kmalloc\n");
	...
	printk(KERN_ERR "N19 IR-SPI register\n");
	misc_register(&ir_spi_dev_drv);
	printk(KERN_ERR "N19 IR-SPI probe end\n");
	return 0;
}
