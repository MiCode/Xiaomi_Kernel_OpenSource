...
extern bool of_obtain_ebuffphyha_info(void);
#endif
...
static bool paser_ebuff_phy_arg_to_name_and_arg(char *input, u32 *name,
						u32 *arg)
{
	char *ptr = input;
	int i = 0;
...
static void ufs_phy_ebuff_write_arg(struct ufs_qcom_phy *ufs_qcom_phy,
				    int index, char *buff)
{
	...
	for (i = 0; i < strlen(ptr); i += size) {
		...
		writel_relaxed(arg, (ufs_qcom_phy->mmio +
				     ebuffphy_data[index].reg_offset + name));
...
static bool obtain_ebuffphy_info(struct ufs_qcom_phy *ufs_qcom_phy)
{
	...
	for (i = 0; i < sizeof(ebuffphy_data) / sizeof(struct ebuffphy); i++) {
		ret = of_property_read_string(ebuff_node, ebuffphy_data[i].name,
					      (const char **)&buff);
		if (ret || !buff)
			continue;
...
static int ufs_qcom_phy_qmp_v4_phy_calibrate(struct phy *generic_phy)
{
	...
	if (!of_obtain_ebuffphyha_info())
		dev_info(dev, "Failed to set ebuff to PHY Host Adapter\n");
#endif
