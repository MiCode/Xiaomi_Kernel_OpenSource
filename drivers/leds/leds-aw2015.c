			   ...
			   const struct i2c_device_id *id)
{
	...
	/* p2 has double aw2015s, one for left, the other for right */
	if (right && aw2015_led_get_board_version() < 2) {
		pr_info("%s: P0.1 and P1 hardware don't have right aw2015\n", __func__);
		return -EINVAL;
	}
	...
	/* p1 as4 and as5 use gpio to switch right leds, */
