...
static int msm_hsic_phy_clk_reset(struct msm_hsic_per *mhsic)
{
	...
	ret = reset_control_assert(mhsic->core_reset);
	if (ret) {
	...
	ret = reset_control_deassert(mhsic->core_reset);
	if (ret)
				...
				struct msm_hsic_per *mhsic, bool enable)
{
	...
	mhsic->core_reset = devm_reset_control_get(&pdev->dev, "core_reset");
	if (IS_ERR(mhsic->core_reset)) {
		dev_err(&pdev->dev, "failed to get core_reset\n");
		ret = PTR_ERR(mhsic->core_reset);
		goto put_core_clk;
	}
...
static DEVICE_ATTR(hsic_init, 0200, NULL, store_hsic_init);

static void ci13xxx_msm_hsic_notify_event(struct ci13xxx *udc,
							unsigned int event)
{
