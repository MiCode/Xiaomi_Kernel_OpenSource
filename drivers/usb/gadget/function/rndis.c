...
static struct md_ep0_data md_ep0_data_par;
...
void rndis_get_pkt_info(struct rndis_params *params, u32 *maxPacketsPerTransfer,
			u32 *maxTransferSize)
{
...
void rndis_get_net_stats(struct rndis_params *params, tethering_net_stats_t
			*net_stats)
{
...
void rndis_set_net_stats(struct rndis_params *params, tethering_net_stats_t
			*net_stats)
{
	struct net_device *net;
	struct rtnl_link_stats64 temp;
...
void rndis_deregister(struct rndis_params *params)
{
	...
	while (!list_empty(&md_ep0_data_par.msg_queue)) {
		struct md_ep0_msg *ep0_msg =
				list_first_entry(&md_ep0_data_par.msg_queue,
				md_ep0_msg, list);
		list_del(&ep0_msg->list);
		kfree(ep0_msg->msg);
		kfree(ep0_msg);
