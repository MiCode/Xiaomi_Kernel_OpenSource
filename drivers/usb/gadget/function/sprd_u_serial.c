 ...
 */
struct usb_request *
sprd_gs_alloc_req(struct usb_ep *ep, unsigned len, gfp_t kmalloc_flags)
{
	struct usb_request *req;
...
EXPORT_SYMBOL_GPL(sprd_gs_alloc_req);
 ...
 */
void sprd_gs_free_req(struct usb_ep *ep, struct usb_request *req)
{
	kfree(req->buf);
	usb_ep_free_request(ep, req);
}
EXPORT_SYMBOL_GPL(sprd_gs_free_req);
		...
		int *allocated)
{
	 ...
	 */
	for (i = 0; i < n; i++) {
		req = sprd_gs_alloc_req(ep, ep->maxpacket, GFP_ATOMIC);
		if (!req)
...
EXPORT_SYMBOL_GPL(sprd_gserial_set_console);
...
EXPORT_SYMBOL_GPL(sprd_gserial_get_console);
...
void sprd_gserial_free_line(unsigned char port_num)
{
	struct gs_port	*port;
...
EXPORT_SYMBOL_GPL(sprd_gserial_free_line);

int sprd_gserial_alloc_line_no_console(unsigned char *line_num)
{
...
EXPORT_SYMBOL_GPL(sprd_gserial_alloc_line_no_console);

int sprd_gserial_alloc_line(unsigned char *line_num)
{
	int ret = sprd_gserial_alloc_line_no_console(line_num);
...
EXPORT_SYMBOL_GPL(sprd_gserial_alloc_line);
...
int sprd_gserial_connect(struct gserial *gser, u8 port_num)
{
	...
	gser->ioport = port;
	port->port_usb = gser;
	/*reset request in case callback after release*/
...
EXPORT_SYMBOL_GPL(sprd_gserial_connect);
/**
...
void sprd_gserial_suspend(struct gserial *gser)
{
	struct gs_port	*port = gser->ioport;
	unsigned long	flags;
...
EXPORT_SYMBOL_GPL(sprd_gserial_suspend);
...
EXPORT_SYMBOL_GPL(sprd_gserial_resume);
