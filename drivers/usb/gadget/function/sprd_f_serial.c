...
static int gser_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (gser->port.in->enabled) {
		...
		sprd_gserial_disconnect(&gser->port);
	}
	...
	sprd_gserial_connect(&gser->port, gser->port_num);
	return 0;
}
...
{
	return sprd_gserial_set_console(to_f_serial_opts(item)->port_num,
				   page, count);
}

static ssize_t f_serial_console_show(struct config_item *item, char *page)
{
	return sprd_gserial_get_console(to_f_serial_opts(item)->port_num, page);
}
...
static void gser_free_inst(struct usb_function_instance *f)
{
	...
	sprd_gserial_free_line(opts->port_num);
	kfree(opts);
}
...
static struct usb_function_instance *gser_alloc_inst(void)
{
	...
	ret = sprd_gserial_alloc_line(&opts->port_num);
	if (ret) {
		kfree(opts);
		return ERR_PTR(ret);
	}
...
static void gser_suspend(struct usb_function *f)
{
	...
	sprd_gserial_suspend(&gser->port);
}
...
DECLARE_USB_FUNCTION_INIT(sprdgser, gser_alloc_inst, gser_alloc);
MODULE_LICENSE("GPL");
