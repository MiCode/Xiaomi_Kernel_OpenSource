...
static void ps5169_set_config(struct ps5169_info *info)
{
	...
	if(info->ps5169_redriver.usb_mode != HOST) {
		ps5169_err("%s: Device.\n", __func__);
...
static void ps5169_enable_work(struct ps5169_info *info, int enable)
{
	...
	if (enable) {
		...
		msleep(50);
		ps5169_info("%s: true.\n", __func__);
		info->ps5169_enable = true;
		ps5169_set_config(info);
	} else {
		if (info->ps5169_enable == false)
			return;
		...
		ps5169_info("%s: false.\n", __func__);
		info->ps5169_enable = false;
	}
}

// gpio_get_value(info->cc_gpio): only 0 or 1 ?
...
void ps5169_get_chipcfg_and_modeselection(struct ps5169_info *info)
{
...
static void ps5169_remove_usb_dp(struct ps5169_info *info)
{
	...
	if (ret < 0)
		ps5169_err("%s: crc err.\n", __func__);
	ps5169_info("%s: remove_usb_dp done.\n", __func__);
}
...
static void ps5169_channel_update(struct ps5169_info *info)
{
	...
	ps5169_info("%s: config channel success: %s.\n", __func__, OPMODESTR(info->op_mode));
	return;
}

static int ps5169_get_orientation(struct usb_redriver *ps5169_redriver)
{
	struct ps5169_info *info =
		container_of(ps5169_redriver, struct ps5169_info, ps5169_redriver);

	if (!ps5169_present_check())
		return 0;

	return gpio_get_value(info->cc_gpio);
}

static int ps5169_notify_connect(struct usb_redriver *ps5169_redriver, int ort)
{
	struct ps5169_info *info =
		container_of(ps5169_redriver, struct ps5169_info, ps5169_redriver);
	...
	ps5169_info("%s: op_mode %s.\n", __func__, OPMODESTR(info->op_mode));
	ps5169_info("%s: orientation %s.\n", __func__, ort == ORIENTATION_CC1 ? "CC1" : "CC2");
	if (info->op_mode == OP_MODE_NONE)
		info->op_mode = OP_MODE_USB;

	ps5169_enable_work(info, 1);
	ps5169_channel_update(info);

	return 0;
}

static int ps5169_notify_disconnect(struct usb_redriver *ps5169_redriver)
{
	struct ps5169_info *info =
		container_of(ps5169_redriver, struct ps5169_info, ps5169_redriver);
	...
	ps5169_info("%s: op_mode:%s\n", __func__, OPMODESTR(info->op_mode));
	if ((info->op_mode == OP_MODE_DEFAULT) || (info->op_mode == OP_MODE_NONE))
		return 0;

	ps5169_get_has_dp();	//dbg

	switch (info->op_mode) {
	case OP_MODE_USB:
		ps5169_remove_usb(info);
		info->op_mode = OP_MODE_NONE;
		ps5169_enable_work(info, 0);
		break;
	case OP_MODE_USB_AND_DP:
	...
	case OP_MODE_DP:
		if(ps5169_get_CCorientation()) {
			ps5169_info("%s: dp_only_mode and cc, 4lane DP flow, keep ps5169.\n", __func__);
		} else {
			ps5169_info("%s: dp_only_mode but no cc, disable ps5169.\n", __func__);
...
static int ps5169_release_usb_lanes(struct usb_redriver *ps5169_redriver, int num)
{
	struct ps5169_info *info =
		container_of(ps5169_redriver, struct ps5169_info, ps5169_redriver);
	...
	ps5169_info("%s: display notify lane mode: %d\n", __func__, num);

	if ((num > 0) && (info->ps5169_enable == false)) {
		ps5169_info("%s: restart enable ps5169.\n", __func__);
		ps5169_enable_work(info, 1);
	}

	if (num == 2)
		info->op_mode = OP_MODE_USB_AND_DP;
	else if (num == 4)
		info->op_mode = OP_MODE_DP;
	else
		info->op_mode = OP_MODE_USB;
...
static int ps5169_dump(struct ps5169_info *info)
{
	int ret = 0;
	int i = 0;
	u8 data;
	...
	for (i = 0; i < 256; i++)
	{
		ret |= ps5169_read_reg(info, i, &data);
		ps5169_info("%s: addr:%02x data:%02x.\n", __func__, i, data);
	}

	return 0;
}

static ssize_t ps5169_dump_show(struct class *c,
					struct class_attribute *attr, char *buf)
{
	struct ps5169_info *info = container_of(c, struct ps5169_info,
						ps5169_class);
	int ret = 0;
	ret = ps5169_dump(info);
	return scnprintf(buf, PAGE_SIZE, "%u\n", ret);
}
static CLASS_ATTR_RO(ps5169_dump);
...
static int ps5169_parse_dt(struct ps5169_info *info)
{
	...
	if ((!gpio_is_valid(info->cc_gpio))){
		ps5169_err("%s: Failed to get gpio\n", __func__);
		return -EINVAL;
	}
	return 0;
}
...
static int ps5169_gpio_init(struct ps5169_info *info)
{
	...
	info->ps5169_gpio_active = pinctrl_lookup_state(info->ps5169_pinctrl, "ps5169_active");
	...
	ret = devm_gpio_request(dev, info->cc_gpio, "redriver");
	if (ret < 0) {
		dev_err(dev, "Failed to request gpio\n");
		info->cc_gpio = -EINVAL;
		return ret;
	}
	info->ps5169_redriver.has_orientation = true;
	return ret;
}
...
{
	struct ps5169_info *info =
		container_of(w, struct ps5169_info, pullup_work);
	...
	ps5169_enable_work(info, 1);
	ps5169_channel_update(info);
	info->work_ongoing = false;
}

static int ps5169_gadget_pullup_enter( struct usb_redriver *ps5169_redriver, int is_on)
{
	struct ps5169_info *info =
		container_of(ps5169_redriver, struct ps5169_info, ps5169_redriver);
	u64 time = 0;
	...
	OPMODESTR(info->op_mode), is_on, info->work_ongoing);
	if (info->op_mode != OP_MODE_USB)
		return -EINVAL;
	if (!is_on)
		return 0;
	while (info->work_ongoing) {
		udelay(1);
		if (time++ > PULLUP_WORKER_DELAY_US) {
			dev_err(info->dev, "pullup timeout\n");
			break;
		}
	}
...
static int ps5169_gadget_pullup_exit(struct usb_redriver *ps5169_redriver, int is_on)
{
	struct ps5169_info *info =
		container_of(ps5169_redriver, struct ps5169_info, ps5169_redriver);
	if (!ps5169_present_check())
	...
	if (is_on)
		return 0;
	info->work_ongoing = true;
	queue_work(info->pullup_wq, &info->pullup_work);
	return 0;
}
		...
		const struct i2c_device_id *id)
{
	...
	info->ps5169_class.name = "qcom-ps5169";
	info->ps5169_class.class_groups = ps5169_class_groups;
	ret = class_register(&info->ps5169_class);
	if (ret < 0) {
		ps5169_err("%s: Failed to create battery_class rc=%d.\n", __func__, ret);
		goto cleanup;
	}

	info->ps5169_redriver.of_node = info->dev->of_node;
	info->ps5169_redriver.usb_mode = false ;
	info->ps5169_nb.notifier_call = ps5169_ucsi_notifier;
	// redriver interface
	info->ps5169_redriver.release_usb_lanes = ps5169_release_usb_lanes;
	info->ps5169_redriver.notify_connect = ps5169_notify_connect;
	info->ps5169_redriver.notify_disconnect = ps5169_notify_disconnect;
	info->ps5169_redriver.get_orientation = ps5169_get_orientation ;
	info->ps5169_redriver.gadget_pullup_enter = ps5169_gadget_pullup_enter;
	info->ps5169_redriver.gadget_pullup_exit = ps5169_gadget_pullup_exit;
	info->ps5169_redriver.host_powercycle = NULL ;
	usb_add_redriver(&info->ps5169_redriver);
...
MODULE_SOFTDEP("post: redriver dwc3_msm");
