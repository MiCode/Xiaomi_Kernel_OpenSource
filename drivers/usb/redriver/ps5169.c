...
static void ps5169_set_config(struct ps5169_info *info)
{
	...
	if(info->ps_redriver.usb_mode != HOST) {
...
static int ps5169_get_orientation(struct usb_redriver *ps_redriver)
{
	struct ps5169_info *info =
		container_of(ps_redriver, struct ps5169_info, ps_redriver);
	ps5169_info( "%s: mode %s\n", __func__,
		OPMODESTR(info->op_mode));

	return gpio_get_value(info->cc_gpio);
}

static int  ps5169_notify_connect(struct usb_redriver *ps_redriver, int ort)
{
	struct ps5169_info *info =
		container_of(ps_redriver, struct ps5169_info, ps_redriver);

	if (!ps5169_present_check() || (info->op_mode == OP_MODE_DEFAULT) ||
	 ...
	 */
	ps5169_info("%s: mode %s, orientation %s\n", __func__, OPMODESTR(info->op_mode), ort == ORIENTATION_CC1 ? "CC1" : "CC2");
	if (info->op_mode != OP_MODE_DP &&  info->op_mode != OP_MODE_USB_AND_DP) {
		info->op_mode = OP_MODE_USB;
	}
	if (ort == ORIENTATION_CC1)
		info->flip = 1;
	else
	    info->flip = 2;
...
static int ps5169_notify_disconnect(struct usb_redriver *ps_redriver)
{
...
static int  ps5169_release_usb_lanes(struct usb_redriver *ps_redriver, int num)
{
	struct ps5169_info *info =
		container_of(ps_redriver, struct ps5169_info, ps_redriver);

	if (!ps5169_present_check() || info->op_mode == OP_MODE_DP||info->op_mode == OP_MODE_USB_AND_DP)
		return 0;
...
static int ps5169_gpio_init(struct ps5169_info *info)
{
	...
	info->ps_redriver.has_orientation = true;
	return ret;
}
...
static int ps5169_gadget_pullup_enter( struct usb_redriver *ps_redriver, int is_on)
{
	struct ps5169_info *info =
		container_of(ps_redriver, struct ps5169_info, ps_redriver);
	u64 time = 0;
...
static int ps5169_gadget_pullup_exit(struct usb_redriver *ps_redriver, int is_on)
{
	struct ps5169_info *info =
		container_of(ps_redriver, struct ps5169_info, ps_redriver);
	if (!ps5169_present_check())
		...
		const struct i2c_device_id *id)
{
	...
	info->ps_redriver.of_node = info->dev->of_node;
	info->ps_redriver.usb_mode = false ;
	info->ps5169_nb.notifier_call = ps5169_ucsi_notifier;
	// redriver interface
	info->ps_redriver.release_usb_lanes = ps5169_release_usb_lanes;
	info->ps_redriver.notify_connect = ps5169_notify_connect;
	info->ps_redriver.notify_disconnect = ps5169_notify_disconnect;
	info->ps_redriver.get_orientation = ps5169_get_orientation ;
	info->ps_redriver.gadget_pullup_enter = ps5169_gadget_pullup_enter;
	info->ps_redriver.gadget_pullup_exit = ps5169_gadget_pullup_exit;
	info->ps_redriver.host_powercycle = NULL ;
	usb_add_redriver(&info->ps_redriver);
