...
static int msm_ssphy_qmp_init(struct usb_phy *uphy)
{
	...
	if(phy->usb3_eye) {
		...
		if (phy->TXA_DRV_LVL) {
			...
			if(reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset, 0xFF, phy->TXA_DRV_LVL);
			else
		...
		if (phy->TXB_DRV_LVL) {
			...
			if(reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset, 0xFF, phy->TXB_DRV_LVL);
			else
		...
		if (phy->TXA_PRE_EMPH) {
			...
			if(reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset, 0xFF, phy->TXA_PRE_EMPH);
			else
				dev_err(uphy->dev, "write TXA_PRE_EMPH fail.\n");
		}
		reg++;
		if (phy->TXB_PRE_EMPH) {
			dev_err(uphy->dev, "TXB_PRE_EMPH:0x%02x.\n", phy->TXB_PRE_EMPH);
			if(reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset, 0xFF, phy->TXB_PRE_EMPH);
			else
				dev_err(uphy->dev, "write TXB_PRE_EMPH fail.\n");
		}
		reg++;
		if (phy->TXA_POST1_LVL) {
			dev_err(uphy->dev, "TXA_POST1_LVL:0x%02x.\n", phy->TXA_POST1_LVL);
			if(reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset, 0xFF, phy->TXA_POST1_LVL);
			else
				dev_err(uphy->dev, "write TXA_POST1_LVL fail.\n");
		}
		reg++;
		if (phy->TXB_POST1_LVL) {
			dev_err(uphy->dev, "TXB_POST1_LVL:0x%02x.\n", phy->TXB_POST1_LVL);
			if(reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset, 0xFF, phy->TXB_POST1_LVL);
			else
				dev_err(uphy->dev, "write TXB_POST1_LVL fail.\n");
		}
		reg++;
		if (phy->TXMGN_V0) {
		...
		if (phy->G3S2_TXMGN_MAIN) {
			...
			if(reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset, 0xFF, phy->G3S2_TXMGN_MAIN);
			else
				dev_err(uphy->dev, "write G3S2_TXMGN_MAIN fail.\n");
		}
		reg++;
...
static int msm_ssphy_qmp_set_suspend(struct usb_phy *uphy, int suspend)
{
	...
	if (suspend) {
		...
		if (phy->phy_type == USB3_AND_DP && ((uphy->flags & PHY_DP_MODE) || (uphy->flags & PHY_USB_DP_CONCURRENT_MODE))) {
			msm_ssphy_qmp_setmode(phy, USB3_DP_COMBO_MODE);
		}
...
static void msm_ssphy_create_debugfs(struct msm_ssphy_qmp *phy)
{
	...
	debugfs_create_x8("g3s2_txmgn_main", 0644, phy->root, &phy->G3S2_TXMGN_MAIN);
}
