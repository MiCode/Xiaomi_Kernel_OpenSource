		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* Begin: fix bug LC-1306, 2022/11/24 */
	if ((adapter_dev->adapter_id == 0) && (adapter_dev->adapter_svid == 0) && (adapter_dev->verify_process == 0)) {
		class_info("adapter maybe removed, force set usbpd verifyed :0\n");
		val = 0;
	}
	/* End: fix bug LC-1306, 2022/11/24 */
		...
		struct device_attribute *attr, char *buf)
{
	...
	class_info("%s: Invalid PDO index\n", __func__);
	return -EINVAL;
}
