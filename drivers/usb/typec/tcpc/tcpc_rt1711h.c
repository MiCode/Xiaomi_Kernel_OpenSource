...
#define pr_err(_fmt, ...) \
	{ \
		if(!charger_ipc_log_context){   \
			...
			ipc_log_string(charger_ipc_log_context, "tcpc_rt1711h: %s %d "_fmt, __func__, __LINE__, ##__VA_ARGS__); \
		}\
	}
				...
				const struct i2c_device_id *id)
{
	...
	if (i2c_check_functionality(client->adapter,
			...
			tcpc_err("I2C functionality : OK...\n");
			}
	else{
		pr_info("I2C functionality check : failuare...\n");
		tcpc_err("I2C functionality check : failuare...\n");
	}
	...
	if (ret < 0) {
		...
		tcpc_err("rt1711 init alert fail\n");
		goto err_irq_init;
	}
	chip->probed = 1;
	pr_info("%s probe OK!\n", __func__);
