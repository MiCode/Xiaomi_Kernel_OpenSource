...
struct sprd_dp_altmode {
	struct typec_displayport_data data;

	enum sprd_dp_state state;
...
static int sprd_dp_altmode_notify(struct sprd_dp_altmode *dp)
{
...
static int sprd_dp_altmode_configure(struct sprd_dp_altmode *dp, u8 con)
{
...
static int sprd_dp_altmode_status_update(struct sprd_dp_altmode *dp)
{
	...
	if (configured && (dp->data.status & DP_STATUS_SWITCH_TO_USB)) {
		...
		ret = sprd_dp_altmode_configure(dp, con);
		if (!ret)
			dp->state = DP_STATE_CONFIGURE;
	}
...
static int sprd_dp_altmode_configured(struct sprd_dp_altmode *dp)
{
	int ret;
...
static int sprd_dp_altmode_configure_vdm(struct sprd_dp_altmode *dp, u32 conf)
{
...
static void sprd_dp_altmode_work(struct work_struct *work)
{
	struct sprd_dp_altmode *dp = container_of(work, struct sprd_dp_altmode, work);
	int svdm_version;
	u32 header;
	u32 vdo;
	int ret;
	...
	case DP_STATE_CONFIGURE:
		ret = sprd_dp_altmode_configure_vdm(dp, dp->data.conf);
		if (ret)
			dev_err(&dp->alt->dev,
...
static void sprd_dp_altmode_attention(struct typec_altmode *alt, const u32 vdo)
{
	struct sprd_dp_altmode *dp = typec_altmode_get_drvdata(alt);
	u8 old_state;
	...
	if (sprd_dp_altmode_status_update(dp))
		dev_warn(&alt->dev, "%s: status update failed\n", __func__);

	if (sprd_dp_altmode_notify(dp))
...
static int sprd_dp_altmode_vdm(struct typec_altmode *alt,
			  const u32 hdr, const u32 *vdo, int count)
{
	...
	case CMDT_RSP_ACK:
		...
		case DP_CMD_STATUS_UPDATE:
			dp->data.status = *vdo;
			ret = sprd_dp_altmode_status_update(dp);
			break;
		case DP_CMD_CONFIGURE:
			ret = sprd_dp_altmode_configured(dp);
			break;
		default:
			break;
		}
...
static int sprd_dp_altmode_activate(struct typec_altmode *alt, int activate)
{
	return activate ? typec_altmode_enter(alt, NULL) :
			  typec_altmode_exit(alt);
}

static const struct typec_altmode_ops sprd_dp_altmode_ops = {
	.attention = sprd_dp_altmode_attention,
	.vdm = sprd_dp_altmode_vdm,
	.activate = sprd_dp_altmode_activate,
};
...
{
	struct sprd_dp_altmode *dp = dev_get_drvdata(dev);
	u32 conf;
	u32 cap;
	int con;
	int ret = 0;
	...
	if (dp->alt->active) {
		ret = sprd_dp_altmode_configure_vdm(dp, conf);
		if (ret)
			goto err_unlock;
	}
...
static struct attribute *sprd_dp_altmode_attrs[] = {
	&dev_attr_configuration.attr,
	&dev_attr_pin_assignment.attr,
	NULL
};

static const struct attribute_group sprd_dp_altmode_group = {
	.name = "displayport",
	.attrs = sprd_dp_altmode_attrs,
};

int sprd_dp_altmode_probe(struct typec_altmode *alt)
{
	const struct typec_altmode *port = typec_altmode_get_partner(alt);
	struct sprd_dp_altmode *dp;
	int ret;
	...
	ret = sysfs_create_group(&alt->dev.kobj, &sprd_dp_altmode_group);
	if (ret)
		return ret;
	...
	INIT_WORK(&dp->work, sprd_dp_altmode_work);
	mutex_init(&dp->lock);
	dp->port = port;
	dp->alt = alt;

	alt->desc = "DisplayPort";
	alt->ops = &sprd_dp_altmode_ops;
...
EXPORT_SYMBOL_GPL(sprd_dp_altmode_probe);

void sprd_dp_altmode_remove(struct typec_altmode *alt)
{
	struct sprd_dp_altmode *dp = typec_altmode_get_drvdata(alt);

	sysfs_remove_group(&alt->dev.kobj, &sprd_dp_altmode_group);
	cancel_work_sync(&dp->work);
}
EXPORT_SYMBOL_GPL(sprd_dp_altmode_remove);

static const struct typec_device_id sprd_dp_typec_id[] = {
	{ USB_TYPEC_DP_SID, USB_TYPEC_DP_MODE },
	{ },
};
MODULE_DEVICE_TABLE(typec, sprd_dp_typec_id);

static struct typec_altmode_driver sprd_dp_altmode_driver = {
	.id_table = sprd_dp_typec_id,
	.probe = sprd_dp_altmode_probe,
	.remove = sprd_dp_altmode_remove,
	.driver = {
		.name = "typec_displayport",
		.owner = THIS_MODULE,
	},
};
module_typec_altmode_driver(sprd_dp_altmode_driver);
