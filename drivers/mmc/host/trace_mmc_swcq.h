...
#undef TRACE_SYSTEM
#define TRACE_SYSTEM mmc_swcq

#if !defined(_TRACE_MMC_SWCQ) || defined(TRACE_HEADER_MULTI_READ)
#define _TRACE_MMC_SWCQ
...
TRACE_EVENT(mmc_request_begin,
	TP_PROTO(struct mmc_host *host, struct gendisk *disk, struct mmc_request *mrq),
	TP_ARGS(host, disk, mrq),
	TP_STRUCT__entry(
		__field(dev_t,                  dev)
	...
	TP_printk("mmc_request_begin %s dev=%d mmc_request[%p]: cmd_opcode=%u cmd_arg=0x%x cmd_flags=0x%x tag=%d blocks=%d block_size=%d blk_addr=%d",
		  __get_str(name), __entry->dev,  __entry->mrq, __entry->cmd_opcode,
		  __entry->cmd_arg, __entry->cmd_flags, __entry->tag,
		  __entry->blocks, __entry->blksz, __entry->blk_addr)
);

TRACE_EVENT(mmc_request_end,
	...
	TP_printk("mmc_request_end %s mmc_request[%p]: cmd_opcode=%u cmd_err=%d cmd_resp=%d cmd_retries=%u tag=%d",
...
#define TRACE_INCLUDE_FILE trace_mmc_swcq
