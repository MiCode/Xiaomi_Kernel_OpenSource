...
#define MSM_MMC_LATENCY_TBL_SIZE		3
...
struct sdhci_msm_pm_qos_mngmt {
	...
	s32				prev_latency;
	int				type;
#define PM_QOS_MNGMT_IRQ_TYPE		0
#define PM_QOS_MNGMT_AFFINITY_TYPE	1
	struct pm_qos_request		dma_req;
};

struct sdhci_msm_pm_qos {
	struct sdhci_msm_pm_qos_mngmt	irq;
	struct sdhci_msm_pm_qos_mngmt	affine;
...
static ssize_t
show_msm_sdhci_pm_qos_tout(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
...
static ssize_t
store_msm_sdhci_pm_qos_tout(struct device *dev, struct device_attribute *attr,
			  ...
			  struct sdhci_msm_pm_qos_mngmt *mngmt)
{
	...
	struct sdhci_msm_pm_qos_config *qos_config, *config = NULL;
	bool changed_cpu = false;
	s32 latency;
	enum sdhci_msm_pm_qos_rw_policy search_policy = policy;
...
search_list:
	list_for_each_entry(qos_config, &mngmt->head, list) {
		if ((search_policy == PM_QOS_READ_WRITE ||
		     search_policy == qos_config->rw_policy) &&
	...
	if (!config && ((policy == PM_QOS_READ &&
		...
		search_policy = PM_QOS_READ_WRITE;
		goto search_list;
	}
	...
	if (!config) {
		 ...
		 */
		if ((mngmt->type == PM_QOS_MNGMT_AFFINITY_TYPE &&
	...
	if (type == SDHCI_PM_QOS_UNVOTE &&
		...
		latency = config->cpu_dma_latency_us[host->power_policy];
	else
		latency = config->cpu_dma_latency_us[0];

	if (unlikely(!mngmt->prev)) {
		/*
		 * ensure that:
		 * 1. mngmt->prev is not NULL the next time pm_qos is updated
		 * 2. mngmt->prev_latency has a bogus value
		 */
		latency = -1;
		goto out_ok;
	}
	...
	if (config != mngmt->prev &&
	    (config->cpu_affinity_type == PM_QOS_REQ_ALL_CORES ||
	     config->cpu_affinity_type == PM_QOS_REQ_AFFINE_CORES))
	...
	pm_qos_renew_request(mngmt, changed_cpu, latency);

	if (type == SDHCI_PM_QOS_UNVOTE &&
	    host->power_policy != SDHCI_POWER_SAVE_MODE) {
		/*
		 * In performance mode, release QoS vote after a timeout
		 * to make sure back-to-back requests don't suffer from
		 * latencies that are involved to wake CPU from low
		 * power modes in cases where the CPU goes into low
		 * power mode as soon as QoS vote is released.
		 */
				   ...
				   enum sdhci_pm_qos_update type)
{
	...
	pm_qos_update(host, rw_policy, type, &pdata->pm_qos.affine);
...
out:
	for (qos_config = &pm_qos->qos_configs[0];
		...
		if (qos_config->cpu_affinity_type == PM_QOS_REQ_AFFINE_IRQ)
			...
			list_add(&qos_config->list, &pm_qos->affine.head);
	}
...
static int sdhci_msm_copy_cpu_latency(struct device *dev, u32 *values,
	...
	qos_config->cpu_dma_latency_us = devm_kzalloc(dev,
		qos_config->cpu_dma_latency_tbl_sz*
		sizeof(*qos_config->cpu_dma_latency_us),
		GFP_KERNEL);
	if (!qos_config->cpu_dma_latency_us) {
		dev_err(dev, "no memory for latency table\n");
		return -ENOMEM;
	}

	for (i = 0; i < qos_config->cpu_dma_latency_tbl_sz ; i++)
		qos_config->cpu_dma_latency_us[i] = be_to_cpu ?
					...
					struct sdhci_host *host)
{
	...
	INIT_LIST_HEAD(&pm_qos->affine.head);
	pm_qos->affine.dma_req.type = PM_QOS_REQ_ALL_CORES;
	pm_qos->affine.type = PM_QOS_MNGMT_AFFINITY_TYPE;
	...
	latencies = (u32 *)of_get_property(np, "qcom,cpu-dma-latency-us",
					  &tbl_sz);
	if (latencies) {
		tbl_sz /= sizeof(*pm_qos->qos_configs[0].cpu_dma_latency_us);

		if (!((pm_qos->num_qos_configs == 1 && tbl_sz == 1) ||
		     (tbl_sz / pm_qos->num_qos_configs == 3))) {
			dev_err(dev,
	...
	pm_qos_add_request(&pm_qos->affine.dma_req,
				 ...
				 struct sdhci_msm_pm_qos *pm_qos)
{
	...
	pm_qos_remove_request(&pm_qos->affine.dma_req);
