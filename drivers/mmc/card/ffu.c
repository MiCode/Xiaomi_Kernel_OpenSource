...
#include "samsung_64GB_emmc_p05.h"
#include "hynix_64GB_emmc_p162.h"
...
#define HYNIX_64GB_CID_PROD_NAME "HCG8a4"

#define SAMSUNG_CID_MANFID 0x15
#define SAMSUNG_64GB_CID_PROD_NAME "RH64MB"
...
static int mmc_ffu_restart (struct mmc_card *card)
{
	...
	if (err) {
		pr_warn ("%s: mmc_hw_reset_check failed (%d)!!!\n", __func__, err);
	}
...
int mmc_ffu_install (struct mmc_card *card, u8 new_fw_ver)
{
	...
	if (err) {
		...
		if (ext_csd[EXT_CSD_FW_VERSION] == new_fw_ver) {
			card->ext_csd.fw_version = new_fw_ver;
...
#if 0
	{SAMSUNG_CID_MANFID, SAMSUNG_16GB_CID_PROD_NAME, 7, 13,
	 ss_emmc_fw_binary_16_p13, sizeof (ss_emmc_fw_binary_16_p13)}
#endif

	{HYNIX_CID_MANFID, HYNIX_64GB_CID_PROD_NAME, 161, 162, /*from 0xa1 to 0xa2*/
			hynix_emmc_fw_64GB_p162, sizeof (hynix_emmc_fw_64GB_p162)},
	{SAMSUNG_CID_MANFID, SAMSUNG_64GB_CID_PROD_NAME, 2, 5, /*from 0xa1 to 0xa2*/
			samsung_emmc_fw_64GB_p05, sizeof (samsung_emmc_fw_64GB_p05)}
#if 0
	{HYNIX_CID_MANFID, HYNIX_128GB_CID_PROD_NAME, 84, 162 , 0 , 0 , /*from test version to 0xa2*/
			sk_emmc_fw_binary_128_p162, sizeof (sk_emmc_fw_binary_128_p162)},
		{SAMSUNG_CID_MANFID, SAMSUNG_32GB_CID_PROD_NAME, 7, 18, 2015 , 7 ,
				ss_emmc_fw_binary_32_64_p13, sizeof (ss_emmc_fw_binary_32_64_p13)},
#endif
};
...
#if 0
				if ((!ffu_table[i].year && !ffu_table[i].month) ||
						 (ffu_table[i].year == card->cid.year && ffu_table[i].month == card->cid.month)) {
#endif
					err = update_fw (card, ffu_table + i);

			} else if (card->ext_csd.fw_version == ffu_table[i].new_fw_ver) {
				pr_info ("the emmc fw_version has updated to %d \n",
