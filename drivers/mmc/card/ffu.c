...
#include "Jadeplus_2D_PRV_0x03_EXT_CSD_0006_FFU.h"
#include "Jadeplus_S76_PRV_0x01_EXT_CSD_0002_FFU.h"
#include <linux/bug.h>
...
#define SAMSUNG_64GB_CID_PROD_NAME "DH6DAB"
#define Jadeplus_CID_MANFID 0x90
#define Jadeplus_32GB_CID_PROD_NAME "hB8aP>"
#define Jadeplus_16GB_CID_PROD_NAME "HAG4a2"
#define Jadeplus_64GB_CID_PROD_NAME "hC9aP3"
...
static int mmc_ffu_restart(struct mmc_card *card)
{
		...
		if (err) {
		pr_err("%s: hw_reset failed%d\n", __func__, err);
		return err;
		}
		pr_info("mmc_ffu_restart done\n");
...
int mmc_ffu_install(struct mmc_card *card, __u64 new_fw_ver)
{
	u8 ext_csd[CARD_BLOCK_SIZE];
	int err;
	u32 ffu_data_len;
	...
	if (!FFU_FEATURES(ext_csd[EXT_CSD_FFU_FEATURES])) {
		...
		pr_info("mmc_ffu_restart err:%d line:%d\n", err, __LINE__);
		if (err) {
	...
	if (err) {
		pr_err("FFU: %s: error %d FFU install: LINE:%d\n",
		       mmc_hostname(card->host), err, __LINE__);
		err = -EINVAL;
		goto exit;
	} else {
		if (card->ext_csd.fw_version == new_fw_ver) {
			pr_info("install finished ,fw_version has updated to  0x%llx\n", new_fw_ver);
		} else {
			pr_info
			("fw install status is  ok,but the version is not right,the version : 0x%llx  %s  %s  %d\n",
			 card->ext_csd.fw_version, __FILE__,
...
int update_fw(struct mmc_card *card, struct fw_update_info *fw_info)
{
	...
	pr_info("emmc ffu_download update_fw\n");
	...
	pr_info("emmc ffu_download update_fw err:%d\n", err);
	if (err) {
...
static struct fw_update_info ffu_table[] = {
	{Jadeplus_CID_MANFID, Jadeplus_32GB_CID_PROD_NAME, 0x3030303200000000, 0x3030303600000000,
			Jadeplus_emmc_fw_binary_32_p02, sizeof(Jadeplus_emmc_fw_binary_32_p02)},//3+32
	{Jadeplus_CID_MANFID, Jadeplus_32GB_CID_PROD_NAME, 0x3030303400000000, 0x3030303600000000,
			Jadeplus_emmc_fw_binary_32_p02, sizeof(Jadeplus_emmc_fw_binary_32_p02)},//2+32
	{Jadeplus_CID_MANFID, Jadeplus_64GB_CID_PROD_NAME, 0x3030303100000000, 0x3030303200000000,
			Jadeplus_emmc_fw_binary_32_p02_464, sizeof(Jadeplus_emmc_fw_binary_32_p02_464)},//4+64
...
int mmc_ffu(struct mmc_card *card)
{
	int i;
	int err = 0;
	pr_info("mmc_ffu start,mid:%d,name:%s,fw_ver:0x%llx prv:%d\n",
		card->cid.manfid,
		card->cid.prod_name, card->ext_csd.fw_version, card->cid.prv);
	for (i = 0; i < sizeof(ffu_table) / sizeof(ffu_table[0]); i++) {
		if (card->cid.manfid == ffu_table[i].manfid &&
			...
			if (card->ext_csd.fw_version == ffu_table[i].old_fw_ver) {
				...
				pr_info
				    ("the emmc fw_version has updated to 0x%llx\n",
				     ffu_table[i].new_fw_ver);
			else {
				pr_err("emmc fw_version misstach 0x%llx\n",
