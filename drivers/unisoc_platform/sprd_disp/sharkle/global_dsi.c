				...
				struct device_node *np)
{
	...
	if (IS_ERR(ctx_reset.regmap)) {
		...
		ctx_reset.ctrl_reg = args[0];
		ctx_reset.ctrl_mask = args[1];
	}
...
const struct dsi_glb_ops sharkle_dsi_glb_ops = {
...
MODULE_DESCRIPTION("Unisoc Sharkle DSI global APB regs low-level config");
