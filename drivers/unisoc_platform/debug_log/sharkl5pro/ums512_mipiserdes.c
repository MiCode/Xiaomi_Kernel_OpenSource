...
extern int serdes_enable(struct serdes_drv_data *serdes, int enable);
static int L5proSwitchFlag;
...
{
	regmap_write_bits(base, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_CSI_2P2L_TEST_DB, 0xFFFFFFFF, (address << 11) | (0x1 << 2));
	regmap_write_bits(base, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_CSI_2P2L_TEST_DB, 0xFFFFFFFF, (address << 11) | (0x1 << 2) | (0x1 << 1));
	regmap_write_bits(base, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_CSI_2P2L_TEST_DB, 0xFFFFFFFF, (address << 11) | (0x1 << 2));
	regmap_write_bits(base, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_CSI_2P2L_TEST_DB, 0xFFFFFFFF, (data << 11));
	regmap_write_bits(base, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_CSI_2P2L_TEST_DB, 0xFFFFFFFF, (data << 11) | (0x1 << 1));
	regmap_write_bits(base, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_CSI_2P2L_TEST_DB, 0xFFFFFFFF, (data << 11));
}
...
static void inter_dbg_log_init(struct dbg_log_device *dbg)
{
	...
	if (L5proSwitchFlag) {
		...
		DEBUG_LOG_PRINT("L5 mipiserdes init\n");
	}
...
/* 	DEBUG_LOG_PRINT("dbg->mm = %d", dbg->mm);
	...
	regmap_update_bits(dbg->phy->pmu_apb, REG_PMU_APB_RF_PD_MM_TOP_CFG/*0x0024*/, 0x2000000, 0x0);

	/* power on */
	regmap_update_bits(dbg->phy->pmu_apb, REG_PMU_APB_RF_PD_MM_TOP_CFG/*0x0024*/, 0x1000000, 0x0);

	DEBUG_LOG_PRINT("power on ok!\n");

	/* here need wait */
	fsleep(1000); /* Wait for 1mS */

	/* enable cphy cfg clk in mm */
	regmap_update_bits(dbg->phy->mm_ahb, REG_MM_AHB_RF_GEN_CLK_CFG/*0x8*/, 0x100, 0x100);

	/* here need wait */
	fsleep(1000); /* Wait for 1mS */

	/* set 2P2L DSI TEST_CLR */
	regmap_update_bits(dbg->phy->dsi_apb, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_CSI_2P2L_TEST_DB/*0x7C*/, 0x1, 0x1);

	/* disable 2P2L DSI SHUTDOWNZ_DB[15] and RSTZ_DB[14] */
	regmap_update_bits(dbg->phy->dsi_apb, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_CSI_2P2L_CTRL_DB/*0x78*/, 0xC000, 0xC000);

	/* set dbg_sel 2P2L DSI SHUTDOWNZ_DB */
	regmap_update_bits(dbg->phy->dsi_apb, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_REG_SEL_CFG_0/*0x8C*/, 0x40, 0x40);

	/* set MIPI CSI SEL CTRL[2:0] = 3'B001 */
	regmap_update_bits(dbg->phy->mm_ahb, REG_MM_AHB_RF_MIPI_CSI_SEL_CTRL/*0x30*/, 0x1, 0x1);
	regmap_update_bits(dbg->phy->mm_ahb, REG_MM_AHB_RF_MIPI_CSI_SEL_CTRL/*0x30*/, 0x6, 0x6);

	/* set csi_2p2l_test_clr_m_sel and csi_2p2l_test_clr_m */
	regmap_update_bits(dbg->phy->dsi_apb, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_COMBO_CSI_4L_BIST_TEST /*0xb4*/, 0x300000, 0x300000);
	...
	regmap_update_bits(dbg->phy->mm_ahb, REG_MM_AHB_RF_MIPI_CSI_SEL_CTRL/*0x30*/, 0x7, ~0x7);

	/* set csi_2p2l_test_clr_s_sel and csi_2p2l_test_clr_s */
	regmap_update_bits(dbg->phy->dsi_apb, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_COMBO_CSI_4L_BIST_TEST /*0xb4*/, 0x60000, 0x60000);
	...
	regmap_update_bits(dbg->phy->mm_ahb, REG_MM_AHB_RF_MIPI_CSI_SEL_CTRL/*0x30*/, 0x2, 0x2);

	/* set dbg_sel_csi_2p2l_dsi_if_sel_db */
	regmap_update_bits(dbg->phy->dsi_apb, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_REG_SEL_CFG_0 /*0x8C*/, 0x20, 0x20);

	/* release 2P2L DSI TEST_CLR */
	regmap_update_bits(dbg->phy->dsi_apb, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_CSI_2P2L_TEST_DB/*0x7C*/, 0x1, ~0x1);

	/* enable 2P2L DSI CLK DB */
	regmap_update_bits(dbg->phy->dsi_apb, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_CSI_2P2L_CTRL_DB/*0x78*/, 0x200, 0x200);

	/* enable 2P2L DSI CLK EN */
	regmap_update_bits(dbg->phy->dsi_apb, REG_ANLG_PHY_G10_RF_ANALOG_MIPI_CSI_2P2LANE_CSI_2P2L_CTRL_DB/*0x78*/, 0x3C00, 0x3C00);
...
static int dbg_log_probe(struct platform_device *pdev)
{
	...
	if (0 == of_property_read_u32(pdev->dev.of_node, "sprd,mipi-l5pro", &l5Proswitch)) {
...
/* 	pm_runtime_set_active(&pdev->dev);
	...
	DEBUG_LOG_PRINT("Finish mipi serdes probe\n");
	return 0;
}

static const struct of_device_id dt_ids[] = {
	{.compatible = "sprd,dbg-log-sharkl5",},
	{.compatible = "sprd,dbg-log-sharkl5pro",},
	{},
};
...
MODULE_AUTHOR("Sitao Chens <sitao.chen@unisoc.com>");
