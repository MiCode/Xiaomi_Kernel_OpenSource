...
/*
 * sprd_coresight_claim_device_unlocked : Claim the device for self-hosted usage
 ...
 */
int sprd_coresight_claim_device_unlocked(struct coresight_device *csdev)
{
	if (WARN_ON(!csdev))
		return -EINVAL;
...
EXPORT_SYMBOL_GPL(sprd_coresight_claim_device_unlocked);

int sprd_coresight_claim_device(struct coresight_device *csdev)
{
	int rc;
...
EXPORT_SYMBOL_GPL(sprd_coresight_claim_device);

/*
 * sprd_coresight_disclaim_device_unlocked : Clear the claim tags for the device.
 * Called with CS_UNLOCKed for the component.
 */
void sprd_coresight_disclaim_device_unlocked(struct coresight_device *csdev)
{
...
EXPORT_SYMBOL_GPL(sprd_coresight_disclaim_device_unlocked);

void sprd_coresight_disclaim_device(struct coresight_device *csdev)
{
	if (WARN_ON(!csdev))
		return;
...
EXPORT_SYMBOL_GPL(sprd_coresight_disclaim_device);
 ...
 * to avoid a race with sprd_coresight_enable that may try to use this value.
 */
					...
					struct coresight_node *nd)
{
	...
	list_for_each_entry_continue(nd, path, link) {
		...
		if (type == CORESIGHT_DEV_TYPE_LINKSINK)
			type = (csdev == sprd_coresight_get_sink(path)) ?
...
EXPORT_SYMBOL_GPL(sprd_coresight_disable_path);
...
/**
 * sprd_coresight_release_path - release a previously built path.
...
int sprd_coresight_enable(struct coresight_device *csdev)
{
	int cpu, ret = 0;
	struct coresight_device *sink;
	struct list_head *path;
	...
	ret = sprd_coresight_enable_path(path, CS_MODE_SYSFS, NULL);
	if (ret)
		goto err_path;
	...
	case CORESIGHT_DEV_SUBTYPE_SOURCE_PROC:
		 ...
		 * undone in 'sprd_coresight_disable()'.  Since there can only
...
err_path:
	sprd_coresight_release_path(path);
	goto out;
}
EXPORT_SYMBOL_GPL(sprd_coresight_enable);

void sprd_coresight_disable(struct coresight_device *csdev)
{
	int cpu, ret;
...
EXPORT_SYMBOL_GPL(sprd_coresight_disable);
...
EXPORT_SYMBOL_GPL(sprd_coresight_enable_sink_show_export);
...
EXPORT_SYMBOL_GPL(sprd_coresight_enable_sink_store_export);
				   ...
				   const char *buf, size_t size)
{
	...
	if (val) {
		ret = sprd_coresight_enable(csdev);
		if (ret)
			return ret;
	} else {
		sprd_coresight_disable(csdev);
	}
...
EXPORT_SYMBOL_GPL(sprd_coresight_enable_source_show_export);
...
EXPORT_SYMBOL_GPL(sprd_coresight_enable_source_store_export);
...
static int coresight_fixup_device_conns(struct coresight_device *csdev)
{
	...
	for (i = 0; i < csdev->pdata->nr_outport; i++) {
		...
		if (!conn->child_fwnode)
			continue;
		conn->child_dev =
			sprd_coresight_find_csdev_by_fwnode(conn->child_fwnode);
		if (conn->child_dev) {
...
/**
 * sprd_coresight_timeout - loop until a bit has changed to a specific register
 *			state.
...
EXPORT_SYMBOL_GPL(sprd_coresight_timeout);
...
/*
 * sprd_coresight_release_platform_data: Release references to the devices connected
...
struct coresight_device *sprd_coresight_register(struct coresight_desc *desc)
{
	int ret;
	int link_subtype;
	int nr_refcnts = 1;
	...
	mutex_unlock(&coresight_mutex);
	if (ret) {
		sprd_coresight_unregister(csdev);
		return ERR_PTR(ret);
	}
...
err_out:
	...
	sprd_coresight_release_platform_data(NULL, desc->pdata);
	return ERR_PTR(ret);
}
EXPORT_SYMBOL_GPL(sprd_coresight_register);

void sprd_coresight_unregister(struct coresight_device *csdev)
{
	...
	sprd_coresight_release_platform_data(csdev, csdev->pdata);
	device_unregister(&csdev->dev);
}
EXPORT_SYMBOL_GPL(sprd_coresight_unregister);
...
bool sprd_coresight_loses_context_with_cpu(struct device *dev)
{
	return fwnode_property_present(dev_fwnode(dev),
				       "arm,coresight-loses-context-with-cpu");
}
EXPORT_SYMBOL_GPL(sprd_coresight_loses_context_with_cpu);

/*
 * sprd_coresight_alloc_device_name - Get an index for a given device in the
 * device index list specific to a driver. An index is allocated for a
 * device and is tracked with the fwnode_handle to prevent allocating
 * duplicate indices for the same device (e.g, if we defer probing of
 * a device due to dependencies), in case the index is requested again.
 */
char *sprd_coresight_alloc_device_name(struct coresight_dev_list *dict,
				  struct device *dev)
{
	int idx;
	char *name = NULL;
...
EXPORT_SYMBOL_GPL(sprd_coresight_alloc_device_name);
...
MODULE_DESCRIPTION("Spreadtrum SoC CoreSight Driver");
