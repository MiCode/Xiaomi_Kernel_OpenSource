...
#include "sprd_coresight-etm-perf.h"
...
static void free_sink_buffer(struct etm_event_data *event_data)
{
	...
	sink = sprd_coresight_get_sink(etm_event_cpu_path(event_data, cpu));
...
static void free_event_data(struct work_struct *work)
{
	...
	for_each_cpu(cpu, mask) {
		...
		if (!(IS_ERR_OR_NULL(*ppath)))
			sprd_coresight_release_path(*ppath);
		*ppath = NULL;
	}
...
static void etm_event_start(struct perf_event *event, int flags)
{
	...
	sink = sprd_coresight_get_sink(path);
	if (WARN_ON_ONCE(!sink))
		goto fail_end_stop;

	/* Nothing will happen without a path */
	if (sprd_coresight_enable_path(path, CS_MODE_PERF, handle))
		goto fail_end_stop;
...
fail_disable_path:
	sprd_coresight_disable_path(path);
fail_end_stop:
	/*
