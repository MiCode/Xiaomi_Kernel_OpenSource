...
{
	int rc = sprd_coresight_claim_device(drvdata->csdev);
...
static void tmc_etb_dump_hw(struct tmc_drvdata *drvdata)
{
	...
	bufp = drvdata->buf;
	drvdata->len = 0;
#ifdef CONFIG_UNISOC_CORESIGHT_TMC_GROUP_CTRL
...
static void __tmc_etb_disable_hw(struct tmc_drvdata *drvdata)
{
	...
	sprd_tmc_flush_and_stop(drvdata);
	/*
...
static void tmc_etb_disable_hw(struct tmc_drvdata *drvdata)
{
	...
	sprd_coresight_disclaim_device(drvdata->csdev);
}
...
EXPORT_SYMBOL_GPL(sprd_tmc_etb_get_sysfs_trace);
			       ...
			       u32 mode, void *data)
{
	...
	dev_dbg(&csdev->dev, "SPRD_TMC-ETB/ETF enabled\n");
	return 0;
}
...
static int tmc_disable_etf_sink(struct coresight_device *csdev)
{
	...
	dev_dbg(&csdev->dev, "SPRD_TMC-ETB/ETF disabled\n");
	return 0;
}
			       ...
			       int inport, int outport)
{
	...
	if (first_enable)
		dev_dbg(&csdev->dev, "SPRD_TMC-ETF enabled\n");
	return ret;
}
				 ...
				 int inport, int outport)
{
	...
	if (last_disable)
		dev_dbg(&csdev->dev, "SPRD_TMC-ETF disabled\n");
}
				  ...
				  void *sink_config)
{
	...
	if (!buf->snapshot && to_read > handle->size) {
		u32 mask = sprd_tmc_get_memwidth_mask(drvdata);

		/*
		 * Make sure the new size is aligned in accordance with the
		 * requirement explained in function sprd_tmc_get_memwidth_mask().
		 */
...
int sprd_tmc_read_prepare_etb(struct tmc_drvdata *drvdata)
{
	...
	if (drvdata->reading) {
		pr_err("sprd_tmc_read_prepare_etb : %s: is reading!\n", __func__);
		ret = -EBUSY;
		goto out;
	}
	...
	if (drvdata->mode == CS_MODE_PERF) {
		pr_err("sprd_tmc_read_prepare_etb : %s: drvdata->mode == CS_MODE_PERF!\n", __func__);
		ret = -EINVAL;
		goto out;
	}
	...
	if (drvdata->buf == NULL) {
		pr_err("sprd_tmc_read_prepare_etb : %s: drvdata->buf == NULL!\n", __func__);
		ret = -EINVAL;
		goto out;
	}
	...
	if (drvdata->mode == CS_MODE_SYSFS) {
		...
		if (mode != TMC_MODE_CIRCULAR_BUFFER) {
			pr_err("sprd_tmc_read_prepare_etb : %s: mode wrong!\n", __func__);
			ret = -EINVAL;
			goto out;
		}
		__tmc_etb_disable_hw(drvdata);
...
EXPORT_SYMBOL_GPL(sprd_tmc_read_prepare_etb);
...
EXPORT_SYMBOL_GPL(sprd_tmc_read_unprepare_etb);
