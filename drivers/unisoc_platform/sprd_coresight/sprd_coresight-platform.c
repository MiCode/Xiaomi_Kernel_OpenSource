...
EXPORT_SYMBOL_GPL(sprd_coresight_find_csdev_by_fwnode);
...
struct device *
sprd_of_coresight_get_device_by_node(struct device_node *endpoint)
{
...
EXPORT_SYMBOL_GPL(sprd_of_coresight_get_device_by_node);
				       ...
				       struct coresight_platform_data *pdata)
{
	...
	do {
		 ...
		 * released via:
		 * 1) sprd_coresight_release_platform_data() if the probe fails or
...
int sprd_coresight_get_cpu(struct device *dev)
{
...
EXPORT_SYMBOL_GPL(sprd_coresight_get_cpu);

struct coresight_platform_data *
sprd_coresight_get_platform_data(struct device *dev)
{
	int ret = -ENOENT;
...
error:
	if (!IS_ERR_OR_NULL(pdata))
		...
		sprd_coresight_release_platform_data(NULL, pdata);
	return ERR_PTR(ret);
}
EXPORT_SYMBOL_GPL(sprd_coresight_get_platform_data);