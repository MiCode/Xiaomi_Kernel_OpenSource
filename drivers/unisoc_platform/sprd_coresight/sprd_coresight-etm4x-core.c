...
#include "sprd_coresight-self-hosted-trace.h"
...
static int etm4_enable_hw(struct etmv4_drvdata *drvdata)
{
	...
	if (sprd_coresight_timeout(drvdata->base/*csa*/, TRCSTATR, TRCSTATR_IDLE_BIT, 1))
		dev_err(etm_dev,
	...
	if (sprd_coresight_timeout(drvdata->base/*csa*/, TRCSTATR, TRCSTATR_IDLE_BIT, 0))
		dev_err(etm_dev,
			"timeout while waiting for Idle Trace Status\n");
...
static void etm4_disable_hw(void *info)
{
	...
	if (sprd_coresight_timeout(drvdata->base/*csa*/, TRCSTATR, TRCSTATR_PMSTABLE_BIT, 1))
		dev_err(etm_dev,
				    ...
				    struct csdev_access *csa)
{
	...
	case ETM_DEVARCH_ETMv4x_ARCH:
		*csa = (struct csdev_access) {
			.io_mem	= false,
			.read	= sprd_etm4x_sysreg_read,
			.write	= sprd_etm4x_sysreg_write,
		};
		break;
...
static int etm4_online_cpu(unsigned int cpu)
{
	...
	if (etmdrvdata[cpu]->boot_enable && !etmdrvdata[cpu]->sticky_enable)
		sprd_coresight_enable(etmdrvdata[cpu]->csdev);
	return 0;
}
...
{
	drvdata->trcid = sprd_coresight_get_trace_id(drvdata->cpu);
}
...
int sprd_etm4_enable_source_show(struct device *dev)
{
	...
	val = sprd_coresight_enable_source_show_export(drvdata->csdev);
...
int sprd_etm4_enable_source_store(struct device *dev, int val)
{
	...
	if (val)
		ret = sprd_coresight_enable_source_store_export(drvdata->csdev, true);
	else
		ret = sprd_coresight_enable_source_store_export(drvdata->csdev, false);

	return ret;
}
EXPORT_SYMBOL_GPL(sprd_etm4_enable_source_store);

static int sprd_etm4_probe(struct device *dev, void __iomem *base, u32 etm_pid)
{
	int ret;
	...
	if (pm_save_enable == PARAM_PM_SAVE_FIRMWARE)
		pm_save_enable = sprd_coresight_loses_context_with_cpu(dev) ?
	...
	drvdata->cpu = sprd_coresight_get_cpu(dev);
	if (drvdata->cpu < 0)
		return drvdata->cpu;
	...
	if (boot_enable) {
		sprd_coresight_enable(drvdata->csdev);
...
static int sprd_etm4_probe_amba(struct amba_device *adev, const struct amba_id *id)
{
	void __iomem *base;
	...
	ret = sprd_etm4_probe(dev, base, id->id);
	if (!ret)
...
static int etm4_probe_platform_dev(struct platform_device *pdev)
{
	 ...
	 */
	ret = sprd_etm4_probe(&pdev->dev, NULL, 0);
...
static void __exit sprd_etm4_remove_amba(struct amba_device *adev)
{
...
static const struct amba_id sprd_etm4_ids[] = {
	...
	CS_AMBA_UCI_ID(0x000bbd0b, uci_id_etm4),/* Cortex-A76 Enyo */
	{},
};

MODULE_DEVICE_TABLE(amba, sprd_etm4_ids);

static struct amba_driver sprd_etm4x_amba_driver = {
	.drv = {
		.name   = "coresight-etm4x",
		.owner  = THIS_MODULE,
		.suppress_bind_attrs = true,
	},
	.probe		= sprd_etm4_probe_amba,
	.remove         = sprd_etm4_remove_amba,
	.id_table	= sprd_etm4_ids,
};

static const struct of_device_id sprd_etm4_sysreg_match[] = {
	{ .compatible	= "arm,coresight-etm4x-sysreg" },
	{ .compatible	= "arm,embedded-trace-extension" },
	{}
};

static struct platform_driver sprd_etm4_platform_driver = {
	.probe		= etm4_probe_platform_dev,
	.remove		= etm4_remove_platform_dev,
	.driver			= {
	.name			= "coresight-etm4x",
	.of_match_table		= sprd_etm4_sysreg_match,
	.suppress_bind_attrs	= true,
	},
};

static int __init sprd_etm4x_init(void)
{
	int ret;
	...
	ret = amba_driver_register(&sprd_etm4x_amba_driver);
	if (ret) {
		pr_err("Error registering etm4x AMBA driver\n");
		goto clear_pm;
	}

	ret = platform_driver_register(&sprd_etm4_platform_driver);
	if (!ret)
		return 0;

	pr_err("Error registering etm4x platform driver\n");
	amba_driver_unregister(&sprd_etm4x_amba_driver);
...
static void __exit sprd_etm4x_exit(void)
{
	amba_driver_unregister(&sprd_etm4x_amba_driver);
	platform_driver_unregister(&sprd_etm4_platform_driver);
	etm4_pm_clear();
}

module_init(sprd_etm4x_init);
module_exit(sprd_etm4x_exit);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Spreadtrum SoC Coresight ETM4X Driver");
