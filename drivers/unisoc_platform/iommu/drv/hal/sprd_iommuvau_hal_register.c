...
void mmu_vau_default_ppn(ulong ctrl_base_addr, u32 iommu_id, ulong pp_addr)
{
	...
	if (iommu_id == IOMMU_EX_ISP) {
        ...
        /*
		*default rd reg offset: 0x14
        *default wr reg offset: 0x18
		*/
        reg_addr += DEFAULT_PPN_OFFSET;
        reg_write_dword(reg_addr + 0x04, (pp_addr >> MMU_MAPING_PAGESIZE_SHIFFT));
	}
	reg_write_dword(reg_addr, (pp_addr >> MMU_MAPING_PAGESIZE_SHIFFT));

	if (iommu_id != IOMMU_EX_ISP) {
		/*
		 * The ISP IOMMU read/write register offset is always 0x34
		 * only set the default write register offset for other IOMMUs to 0x18
		 */
		reg_write_dword(reg_addr + 0x04, (pp_addr >> MMU_MAPING_PAGESIZE_SHIFFT));
	}
}
