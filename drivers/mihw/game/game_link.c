...
void game_link_init(void)
{
	...
	register_mi_dequeue_task_fair_hook(GT_TASK, game_dequeue_hooks);
	...
	register_rvh_mi_try_to_wake_up(GT_TASK, mi_mutex_wait_finish_hook);
}

void game_link_exit(void)
{
	unregister_rvh_mi_try_to_wake_up(GT_TASK);
	...
	unregister_mi_dequeue_task_fair_hook(GT_TASK);
