...
{
	int ret = 0;
	ret |= i2c_bkl_write(drvdata->client,0x02, 0x20);     //pwm pin disable LSB 4   & BIT[7,5] Backlight OVP 25V
	ret |= i2c_bkl_write(drvdata->client,0x04, 0x07);     //LSB 3  bkl
	ret |= i2c_bkl_write(drvdata->client,0x05, 0xff);	   // MSB 8  bkl
	ret |= i2c_bkl_write(drvdata->client,0x08, 0x1f);		// BACKLIGHT enable
	ret |= i2c_bkl_write(drvdata->client,0x11, 0x00);		// Bias  config 
	ret |= i2c_bkl_write(drvdata->client,0x09, 0x9E);       //VOPS and VNEG enabled via I2C
	ret |= i2c_bkl_write(drvdata->client,0x0D, 0x1E);
	ret |= i2c_bkl_write(drvdata->client,0x0E, 0x1E);	
	ret |= i2c_bkl_write(drvdata->client,0x14, 0x00);	
	ret |= i2c_bkl_write(drvdata->client,0x0F, 0x07);	
	ret |= i2c_bkl_write(drvdata->client,0x10, 0x03);	
	ret |= i2c_bkl_write(drvdata->client,0x1a, 0x04);	
	ret |= i2c_bkl_write(drvdata->client,0x20, 0x07);	
	ret |= i2c_bkl_write(drvdata->client,0x21, 0xff);	
	ret |= i2c_bkl_write(drvdata->client,0x22, 0x07);	
	ret |= i2c_bkl_write(drvdata->client,0x23, 0xff);	
	ret |= i2c_bkl_write(drvdata->client,0x24, 0x02);
  	ret |= i2c_bkl_write(drvdata->client,0x16, 0x00);
  	ret |= i2c_bkl_write(drvdata->client,0x1C, 0x0f);
	return ret;
}
			...
			const struct i2c_device_id *id)
{
	...
	int err = 0;
  	pr_err("%s :  bkl: enter leds_bkl probe\n", __func__);
