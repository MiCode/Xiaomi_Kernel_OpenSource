...
static unsigned int LP855X_GPIO_EN;
#define GPIO_OUT_ONE 1
#define GPIO_OUT_ZERO 0

static struct lp855x *lp855x_device;
...
#define LP8556_EPROM_START		0x90 /* 0x98 0xA0 */
...
#define AAL_MAX_BRIGHTNESS	1023
...
static void lp855x_set_gpio_output(unsigned int GPIO, unsigned int output)
{
...
static int lp855x_read_byte(struct lp855x *lp, u8 reg)
{
	...
	dev_info(lp->dev, "dump registers  0x%.2x -value- 0x%.2x \n", reg, ret);
 ...
 *
 */
static int lp855x_configure(struct lp855x *lp, int first_boot)
{
	u8 val, addr;
	int i, ret = 0;
	...
	if (!first_boot) {
		lp855x_set_gpio_output(LP855X_GPIO_EN, GPIO_OUT_ONE);
	...
	if (!first_boot) {
		ret = lp855x_write_byte(lp, lp->cfg->reg_devicectrl, 0x00);
		if (ret)
			goto err;
		...
		val = pd->led_enable;
		ret = lp855x_write_byte(lp, lp->cfg->reg_ledenable, val);
		if (ret)
			goto err;
	...
	dev_info(lp->dev, " %s - [%d]\n", __func__, pd->initial_brightness);
...
static int lp855x_parse_dt(struct device *dev, struct device_node *node)
{
	...
	LP855X_GPIO_EN = of_get_named_gpio(node, "lp8556_gpio_enable", 0);
...
{
	lp855x_set_gpio_output(LP855X_GPIO_EN, GPIO_OUT_ZERO);
	return;
}
...
int lp855x_set_backlight_level(unsigned int level)
{
	int ret = 0;
	unsigned int data = 0;
	static u8 last_level = 0x1f;

	if (lp855x_device == NULL) {
		pr_err("%s, client is NULL!\n", __func__);
		ret = -1;
		goto err;;
	}

	if (level > AAL_MAX_BRIGHTNESS) {
		pr_warn("%s, level:%d is too large!\n", __func__, level);
		level = AAL_MAX_BRIGHTNESS;
	}

	if ((last_level == 0) && (level != 0)) {
		pr_info("lp8556 backlight on");
		ret = lp855x_configure(lp855x_device, 0);
		if (ret != 0)
			goto err;
	...
	/*
	* lp855x support 256 steps, but aal is support 1024 steps
	* so it  need remap from 1024 to 256 steps
	*/
	data = 2 * MAX_BRIGHTNESS * level  + AAL_MAX_BRIGHTNESS;
	do_div(data, 2 * AAL_MAX_BRIGHTNESS);

	if (data == last_level)
		goto err;

	if (lp855x_device->mode == PWM_BASED) {
		int max_br = MAX_BRIGHTNESS;

		lp855x_pwm_ctrl(lp855x_device, (int)data, max_br);
	} else if (lp855x_device->mode == REGISTER_BASED) {
		ret = lp855x_write_byte(lp855x_device, lp855x_device->cfg->reg_brightness, (u8)data);
		if (ret != 0) {
			dev_err(lp855x_device->dev, "failed to write %d to reg_brightness\n", data);
			goto err;
		}
	}
	...
	if (level == 0) {
		...
		ret = lp855x_update_bit(lp855x_device, LP855X_DEVICE_CTRL, LP8556_BL_MASK,
			LP8556_BL_OFF);
		if (ret != 0) {
			dev_err(lp855x_device->dev, "failed to update ctrl register with LP8556_BL_OFF\n");
			goto err;
		}
		lp855x_set_gpio_output(LP855X_GPIO_EN, GPIO_OUT_ZERO);
	}
...
EXPORT_SYMBOL(lp855x_set_backlight_level);
