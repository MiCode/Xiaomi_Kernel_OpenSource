					 ...
					 int brightness)
{
	...
	if (bl->lm3533_edp_client) {
		...
		edp_state = i;
		ret = edp_update_client_request(bl->lm3533_edp_client,
						edp_state, &approved);
		if (ret) {
			dev_err(dev, "LM3533 E state transition failed\n");
			return ret;
		}
		if (approved != edp_state) {
			edp_state = approved;
			pr_info
			    ("jnv LM3533 backlight set with edp:%d (replace the anticipant value %d)",
			     bl->edp_brightness_states[edp_state], brightness);
			brightness = bl->edp_brightness_states[edp_state];
		}
	}
...
static int __devinit lm3533_bl_probe(struct platform_device *pdev)
{
	...
	bl->lm3533_edp_client = devm_kzalloc(&pdev->dev,
					     sizeof(struct edp_client),
					     GFP_KERNEL);
	if (IS_ERR_OR_NULL(bl->lm3533_edp_client)) {
		dev_err(&pdev->dev, "could not allocate edp client\n");
		return PTR_ERR(bl->lm3533_edp_client);
	}

	sprintf(bl->lm3533_edp_client->name, "%s%u", "backlight", bl->id);
	bl->lm3533_edp_client->name[EDP_NAME_LEN - 1] = '\0';
	bl->lm3533_edp_client->states = pdata->edp_states;
	bl->lm3533_edp_client->num_states = LM3533_EDP_NUM_STATES;
	bl->lm3533_edp_client->e0_index = LM3533_EDP_ZERO;
	bl->lm3533_edp_client->private_data = bd;
	bl->lm3533_edp_client->priority = EDP_MAX_PRIO + 2;
	bl->lm3533_edp_client->throttle = lm3533_backlight_throttle;
	bl->edp_brightness_states = pdata->edp_brightness;
	...
	if (!battery_manager) {
		...
		ret = edp_register_client(battery_manager,
					  bl->lm3533_edp_client);
		if (ret)
			dev_err(&pdev->dev, "unable to register edp client\n");
		else {
			ret = edp_update_client_request(bl->lm3533_edp_client,
							LM3533_EDP_ZERO, NULL);
			if (ret) {
				dev_err(&pdev->dev,
					"unable to set E0 EDP state\n");
				edp_unregister_client(bl->lm3533_edp_client);
			} else
				goto edp_success;
		}
	}

	devm_kfree(&pdev->dev, bl->lm3533_edp_client);
	bl->lm3533_edp_client = NULL;
