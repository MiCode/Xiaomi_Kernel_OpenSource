...
static inline void mdss_panel_disparam_set(struct mdss_dsi_ctrl_pdata *ctrl, uint32_t param)
{
	...
	case 0x20000:
		...
		break;
	case 0x30000:
		if (ctrl->dispparam_aodon_cmds.cmd_cnt) {
			pr_info("aod on\n");
			mdss_dsi_panel_cmds_send(ctrl, &ctrl->dispparam_aodon_cmds, CMD_REQ_COMMIT);
		}
		break;
	case 0x40000:
		if (ctrl->dispparam_aodoff_cmds.cmd_cnt) {
			pr_info("aod off\n");
			mdss_dsi_panel_cmds_send(ctrl, &ctrl->dispparam_aodoff_cmds, CMD_REQ_COMMIT);
		}
		break;
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->dispparam_aodon_cmds,
		"qcom,mdss-dsi-aodon-command", "qcom,mdss-dsi-aodon-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->dispparam_aodoff_cmds,
		"qcom,mdss-dsi-aodoff-command", "qcom,mdss-dsi-aodoff-command-state");
	...
	int ndx)
{
	...
	if (panel_model) {
		register_hw_component_info(HWCOMPONENT_NAME);
		add_hw_component_info(HWCOMPONENT_NAME, HWCOMPONENT_KEY_LCD, (char *)panel_model);
	} else
		pr_info("%s:%d, Panel model not specified\n", __func__, __LINE__);
