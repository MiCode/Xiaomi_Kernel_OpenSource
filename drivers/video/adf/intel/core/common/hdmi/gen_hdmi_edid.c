 ...
 * A lot of code has been copied from drm_edid.c and hdmi_edid.c
 * Created by: Shashank Sharma <shashank.sharma@intel.com>
 *
 */
...
u32 edid_get_quirks(struct edid *edid)
{
...
/**
 * mode_set_name - set the name on a mode
...
static void do_detailed_mode(struct detailed_timing *timing, void *c)
{
	...
	if (timing->pixel_clock) {
		...
		if (mode_detailed(&newmode->drm_mode, closure->edid,
			...
			pr_debug("vfresh=%d, flags=%x, type=%x, name=%s, clock=%d\n",
...
static void cea_for_each_detailed_block(u8 *ext, detailed_cb *cb,
				void *closure)
{
	int i, n = 0;
	u8 d = ext[0x02];
...
static void vtb_for_each_detailed_block(u8 *ext, detailed_cb *cb,
				void *closure)
{
...
int add_detailed_modes(struct hdmi_monitor *monitor, struct edid *edid,
		   u32 quirks)
{
		...
		struct edid *edid, struct std_timing *t, int revision)
{
	 ...
	 * reduced blanking.
	 */
	list_for_each_entry(m, &monitor->probed_modes, head)
...
add_standard_modes(struct hdmi_monitor *monitor, struct edid *edid)
{
	...
	for (i = 0; i < EDID_STD_TIMINGS; i++) {
		...
		res = mode_std(monitor, &newmode->drm_mode, edid,
				       ...
				       edid->revision);
		if (!res) {
			list_add_tail(&newmode->head, &monitor->probed_modes);
			pr_debug("Add a new mode in %s\n", __func__);
			pr_debug("vdisplay=%d, hdisplay=%d, vtotal=%d, htotal=%d,",
				newmode->drm_mode.vdisplay,
				newmode->drm_mode.hdisplay,
...
int add_cvt_modes(struct hdmi_monitor *monitor, struct edid *edid)
{
	struct detailed_mode_closure closure = {
		monitor, edid, 0, 0, 0
	};
...
static int fill_est3_modes(struct hdmi_monitor *monitor,
				struct detailed_timing *timing)
{
	int i, j, m, modes = 0;
	struct hdmi_mode_info *newmode;
...
int add_established_modes(struct hdmi_monitor *monitor, struct edid *edid)
{
...
static bool mode_in_hsync_range(const struct drm_mode_modeinfo *mode,
...
static bool mode_in_vsync_range(const struct drm_mode_modeinfo *mode,
		    struct edid *edid, u8 *t)
{
	int vsync, vmin, vmax;
...
add_alternate_cea_modes(struct hdmi_monitor *monitor, struct edid *edid)
{
	 ...
	 */
	list_for_each_entry(mode, &monitor->probed_modes, head) {
		const struct drm_mode_modeinfo *cea_mode;
		struct hdmi_mode_info *newmode;
	...
	list_for_each_entry_safe(mode, tmp, &list, head) {
		list_del(&mode->head);
		list_add_tail(&mode->head, &monitor->probed_modes);
		modes++;
	}
	return modes;
}
...
void edid_fixup_preferred(struct hdmi_monitor *monitor,
				 u32 quirks)
{
	struct hdmi_mode_info *t, *cur_mode, *preferred_mode;
	int target_refresh = 0;

	if (list_empty(&monitor->probed_modes))
		return;
	...
	preferred_mode = list_first_entry(&monitor->probed_modes,
