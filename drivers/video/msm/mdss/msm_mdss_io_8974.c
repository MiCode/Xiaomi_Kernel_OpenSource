...
void mdss_dsi_phy_disable(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (left_ctrl &&
			(ctrl->panel_data.panel_info.pdest
			 ==
			 DISPLAY_2)) {
		MIPI_OUTP(left_ctrl->phy_base + 0x0170,
				0x000);
		MIPI_OUTP(left_ctrl->phy_base + 0x0298,
				0x000);
	}

	MIPI_OUTP(ctrl->phy_base + 0x0170, 0x000);
	MIPI_OUTP(ctrl->phy_base + 0x0298, 0x000);
...
void mdss_dsi_phy_init(struct mdss_panel_data *pdata)
{
	...
	/* Strength ctrl 0 */
	MIPI_OUTP((ctrl_pdata->phy_base) + 0x0184, pd->strength[0]);
	...
	/* Regulator ctrl 0 */
	MIPI_OUTP((ctrl_pdata->phy_base) + off + (4 * 0), 0x0);
	/* Regulator ctrl - CAL_PWR_CFG */
	MIPI_OUTP((ctrl_pdata->phy_base) + off + (4 * 6), pd->regulator[6]);

	/* Regulator ctrl - TEST */
	MIPI_OUTP((ctrl_pdata->phy_base) + off + (4 * 5), pd->regulator[5]);
	/* Regulator ctrl 3 */
	MIPI_OUTP((ctrl_pdata->phy_base) + off + (4 * 3), pd->regulator[3]);
	/* Regulator ctrl 2 */
	MIPI_OUTP((ctrl_pdata->phy_base) + off + (4 * 2), pd->regulator[2]);
	/* Regulator ctrl 1 */
	MIPI_OUTP((ctrl_pdata->phy_base) + off + (4 * 1), pd->regulator[1]);
	/* Regulator ctrl 0 */
	MIPI_OUTP((ctrl_pdata->phy_base) + off + (4 * 0), pd->regulator[0]);
	/* Regulator ctrl 4 */
	MIPI_OUTP((ctrl_pdata->phy_base) + off + (4 * 4), pd->regulator[4]);

	/* LDO ctrl 0 */
	if ((ctrl_pdata->panel_data).panel_info.pdest == DISPLAY_1)
		MIPI_OUTP((ctrl_pdata->phy_base) + 0x1dc, 0x00);
	else
		MIPI_OUTP((ctrl_pdata->phy_base) + 0x1dc, 0x00);

	off = 0x0140;	/* phy timing ctrl 0 - 11 */
	for (i = 0; i < 12; i++) {
		MIPI_OUTP((ctrl_pdata->phy_base) + off, pd->timing[i]);
		wmb();
		off += 4;
	}

	/* MMSS_DSI_0_PHY_DSIPHY_CTRL_1 */
	MIPI_OUTP((ctrl_pdata->phy_base) + 0x0174, 0x00);
	/* MMSS_DSI_0_PHY_DSIPHY_CTRL_0 */
	MIPI_OUTP((ctrl_pdata->phy_base) + 0x0170, 0x5f);
	wmb();

	/* Strength ctrl 1 */
	MIPI_OUTP((ctrl_pdata->phy_base) + 0x0188, pd->strength[1]);
	wmb();
	...
	for (ln = 0; ln < 5; ln++) {
		...
		for (i = 0; i < 9; i++) {
			offset = i + (ln * 9);
			MIPI_OUTP((ctrl_pdata->phy_base) + off,
							pd->lanecfg[offset]);
			wmb();
			off += 4;
		}
	...
	if ((ctrl_pdata->panel_data).panel_info.pdest == DISPLAY_1)
		MIPI_OUTP((ctrl_pdata->phy_base) + 0x01d4, 0x01);
	else
		MIPI_OUTP((ctrl_pdata->phy_base) + 0x01d4, 0x00);
	wmb();

	off = 0x01b4;	/* phy BIST ctrl 0 - 5 */
	for (i = 0; i < 6; i++) {
		MIPI_OUTP((ctrl_pdata->phy_base) + off, pd->bistctrl[i]);
		wmb();
		off += 4;
	}
