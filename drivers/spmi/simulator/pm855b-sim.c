...
static const struct spmi_sim_register_init pm855b_regs[] = {
...
static int pm855b_sim_probe(struct platform_device *pdev)
{
	struct pmic_sim *sim;
	u32 sid_offset = 0;
	int rc;
	...
	rc = spmi_sim_init_register(sim->spmi_sim, pm855b_regs,
				ARRAY_SIZE(pm855b_regs), sim->base_addr);
	if (rc) {
		dev_err(&pdev->dev, "could not initialize PM855B register values, rc=%d\n",
			rc);
		return rc;
	}

	dev_info(&pdev->dev, "PMIC PM855B simulator registered at base addr=0x%05X\n",
		sim->base_addr);

	return 0;
}

static const struct of_device_id pm855b_sim_match_table[] = {
	{ .compatible = "qcom,pm855b-sim", },
	{},
};
MODULE_DEVICE_TABLE(of, pm855b_sim_match_table);

static struct platform_driver pm855b_sim_driver = {
	.probe		= pm855b_sim_probe,
	.driver		= {
		.name	= "pm855b_sim",
		.of_match_table = pm855b_sim_match_table,
	},
};

static int __init pm855b_sim_init(void)
{
	return platform_driver_register(&pm855b_sim_driver);
}
postcore_initcall(pm855b_sim_init);

static void __exit pm855b_sim_exit(void)
{
	platform_driver_unregister(&pm855b_sim_driver);
}
module_exit(pm855b_sim_exit);

MODULE_DESCRIPTION("PM855B Simulator Driver");
