...
int ktrace_q_single_show(struct seq_file *m, void *v)
{
	...
	while (!QUEUE_EMPTY(q->head, tail)) {
		ret = q->show_entry(m, q->buf + tail * q->entry_size,
				    q->enable_debug);
...
void __init ktrace_init_queue(struct ktrace_queue *q, struct dentry *dir,
			      void *priv, void *buf, int entry_size,
			      int max, int items_per_read,
			      int (*show_entry)(struct seq_file *, void *,
						bool))
{
