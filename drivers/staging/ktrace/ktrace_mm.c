...
static int mm_show_entry(struct seq_file *m, void *entry, bool debug)
{
	...
	/*
	 *if (debug) {
	 *	ktime_t time = ns_to_ktime(mm->time_stamp_ns);
	 *	struct timespec ts;
	 *	struct rtc_time tm;
	 ...
	 *	seq_printf(m, "%02d:%02d:%02d.%03lu ms %03lu us %03lu
	 *		   ns %5d <%s> %d(tail %d, head %d)\n",
	 *		   tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
	 *		   tm.tm_hour, tm.tm_min, tm.tm_sec,
	 *		   ts.tv_nsec / 1000000L,  (ts.tv_nsec / 1000L) % 1000L,
	 *		   ts.tv_nsec % 1000000L,
	 *		   mm->pid,
	 *		   mm_event_name[mm->type], mm->time_ns,
	 *		   kt->mm_queue_tail, kt->mm_queue_head);
	 *	}
	 */
...
int __init ktrace_mm_init(struct dentry *dir, struct ktrace_queue *q)
{
	...
	BUILD_BUG_ON(ARRAY_SIZE(mm_event_name) / sizeof(mm_event_name[0])
		!= KTRACE_MM_TYPE_NR);
