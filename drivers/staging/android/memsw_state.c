...
#define memsw_filepage     \
(global_page_state(NR_FILE_PAGES) - total_swapcache_pages())

#define MEMSW_FREEMEM_KB   \
((memsw_freeram + memsw_filepage) << (PAGE_SHIFT - 10))
#define MEMSW_FREESWAP_KB  \
(get_nr_swap_pages() << (PAGE_SHIFT - 10))

#define MEMSW_EXTRA_KB     \
(16 << (PAGE_SHIFT - 10)) /*16 pages extra*/
...
static long memsw_dev_ioctl(struct file *file, unsigned int cmd,
							unsigned long arg)
{
	long ret = -EINVAL;
	struct memsw_reader *reader = file->private_data;
