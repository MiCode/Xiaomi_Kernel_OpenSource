...
#define GLOBAL_RECLAIM_MAX_SIZE 200 /* MB */
...
struct rtmm_reclaim {
	...
	spinlock_t lock;  /*lock for reclaim*/
...
static void __release_reclaim_cmd(struct rtmm_reclaim *reclaim,
				  struct reclaim_cmd *cmd)
{
	spin_lock(&reclaim->lock);
...
static void __enqueue_reclaim_cmd(struct rtmm_reclaim *reclaim,
				  struct reclaim_cmd *cmd)
{
	spin_lock(&reclaim->lock);
...
static int mem_reclaim_thread(void *data)
{
	...
	while (true) {
		...
		wait_event_freezable(reclaim->waitqueue,
				     (atomic_read(&reclaim->cmd_nr) > 0));
...
static ssize_t sys_proc_reclaim_write(struct file *file,
				      const char __user *user_buf,
				      size_t size, loff_t *ppos)
{
	struct rtmm_reclaim *reclaim = ((struct seq_file *)
...
static ssize_t sys_global_reclaim_write(struct file *file,
					const char __user *user_buf,
					size_t size, loff_t *ppos)
{
	struct rtmm_reclaim *reclaim = ((struct seq_file *)
...
static ssize_t sys_auto_reclaim_write(struct file *file,
				      const char __user *user_buf,
				      size_t size, loff_t *ppos)
{
	struct rtmm_reclaim *reclaim = ((struct seq_file *)
