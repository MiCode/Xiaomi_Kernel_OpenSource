				   ...
				   const char *buf, size_t count)
{
	...
	pr_info("extend_reclaim: current->comm = %s, reclaim type = %s, nr_to_reclaim = %ld, nr_reclaimed = %ld", current->comm, reclaim_type_str[reclaim_type], nr_to_reclaim, nr_reclaimed);
	return count;
}
