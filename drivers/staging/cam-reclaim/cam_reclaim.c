...
static inline void do_reclaim()
{
	...
	force_reclaim_type = pg_cam_reclaim->page_reclaim.reclaim_type;
...
static int cam_reclaim_thread(void *unused)
{
	...
	while (!kthread_should_stop()) {
		...
		if (force_reclaim_type == 1) {
			do_reclaim();
			spin_lock(&pg_cam_reclaim->page_reclaim.reclaim_lock);
			pg_cam_reclaim->page_reclaim.reclaim_type = 3;
			spin_unlock(&pg_cam_reclaim->page_reclaim.reclaim_lock);
			do_reclaim();
		} else {
			do_reclaim();
...
static void cam_reclaim_setup(void)
{
	...
	pg_cam_reclaim->page_reclaim.reclaim_swappiness = 0;
	pg_cam_reclaim->page_reclaim.reclaim_anonprivate = 0;
	pg_cam_reclaim->page_reclaim.reclaim_type = 3;
	pg_cam_reclaim->page_reclaim.nr_reclaim = 100000;
	pg_cam_reclaim->page_reclaim.per_reclaim = 100000;
