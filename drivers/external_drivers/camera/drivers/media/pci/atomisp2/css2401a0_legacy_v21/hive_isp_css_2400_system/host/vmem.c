	...
	unsigned src_end)
{
	...
	if (subword_width + start_bit > uedge_bits) { /* overlap */
		...
		hive_uedge  old_val1 = INV_SUBWORD(target[start_elem+1], 0, subword_width + start_bit - uedge_bits);
...
#if !defined(HRT_MEMORY_ACCESS)
	ia_css_device_load(ISP_BAMEM_BASE[ID] + (unsigned)from, &v[0][0], size);
#else
	hrt_master_port_load(ISP_BAMEM_BASE[ID] + (unsigned)from, &v[0][0], size);
#endif
	...
	const t_vmem_elem	*from)
{
	...
	assert(ISP_BAMEM_BASE[ID] != (hrt_address)-1);
#if !defined(HRT_MEMORY_ACCESS)
	ia_css_device_store(ISP_BAMEM_BASE[ID] + (unsigned)to, &v, size);
#else
	//hrt_mem_store (ISP, VMEM, (unsigned)to, &v, siz); /* This will overwrite the next vector as well */
	hrt_master_port_store(ISP_BAMEM_BASE[ID] + (unsigned)to, &v, size);
#endif
	hrt_sleep(); /* Spend at least 1 cycles per vector */
#endif
}
	...
	unsigned		elems) /* In t_vmem_elem */
{
	...
	assert(ID < N_ISP_ID);
#ifndef C_RUN
	assert((unsigned)from % ISP_VEC_ALIGN == 0);
#endif
	...
	unsigned		elems) /* In t_vmem_elem */
{
	...
	assert(ID < N_ISP_ID);
#ifndef C_RUN
	assert((unsigned)to % ISP_VEC_ALIGN == 0);
#endif
	assert(elems % ISP_NWAY == 0);
	for (c = 0; c < elems; c += ISP_NWAY) {
