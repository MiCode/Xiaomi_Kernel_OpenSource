...
static int epm_get_temp(void *data, int *temp)
{
	struct epm_tz_device *epm_tz = data;
	struct epm_priv *epm = epm_tz->priv;

	return epm->ops->get_temp(epm_tz, temp);
}

static const struct thermal_zone_of_device_ops epm_thermal_of_ops = {
...
static int thermal_zone_register(struct epm_priv *epm)
{
	...
	for (idx = 0; idx < epm->dt_tz_cnt; idx++) {
		...
		epm_tz->tz = devm_thermal_zone_of_sensor_register(
