...
void dsi_panel_fps_count_lock(struct lcm *lcm, u32 fps, u32 enable)
{
	...
	mutex_lock(&lcm->panel_lock);
	dsi_panel_fps_count(lcm, fps, enable);
	mutex_unlock(&lcm->panel_lock);
}
...
void dsi_panel_count_init(struct lcm *lcm)
{
	...
	dsi_panel_state_count(lcm, 1);
	dsi_panel_fps_count(lcm, 0, 1);
