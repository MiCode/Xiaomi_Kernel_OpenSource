			...
			u32 param_type)
{
	...
	ret = dsi_display_set_disp_param(connector, param_type);
...
ssize_t mi_drm_sysfs_get_disp_param(struct drm_connector *connector,
			char *buf)
{
	return dsi_display_get_disp_param(connector, buf);
}
			...
			char *buf, size_t count)
{
	...
	return dsi_display_write_mipi_reg(connector, buf, count);
}
