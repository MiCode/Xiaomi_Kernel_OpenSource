...
static int dsi_panel_get_lockdown_from_cmdline(unsigned char *plockdowninfo)
{
	...
	if (sscanf(lockdown_info, "0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx",
		...
		if (plockdowninfo[1] == 0x42 || plockdowninfo[1] == 0x35) {
			ret = 0;
		} else {
			ret = -1;
		}
	...
	if (!(panel_ext && panel_ext->funcs && panel_ext->funcs->set_doze_brightness)) {
		...
		if (panel_ext && panel_ext->params && panel_ext->params->aod_delay_enable) {
			if (doze_brightness && !(dsi->mi_cfg.aod_wait_frame)) {
				ret = wait_for_completion_timeout(&dsi->aod_wait_completion, msecs_to_jiffies(100));
...
static void mi_disp_set_dimming_delayed_work_handler(struct kthread_work *work)
{
	...
	struct mtk_dsi *dsi = (struct mtk_dsi *)(cur_work->data);
	struct disp_feature_ctl ctl;
	struct mi_dsi_panel_cfg *mi_cfg = NULL;

	if (get_panel_dead_flag()) {
		DISP_ERROR("panel is in esd process, cancal!\n");
		return;
	}
	...
	mi_cfg = &dsi->mi_cfg;
	if (mi_cfg->feature_val[DISP_FEATURE_DIMMING] == FEATURE_OFF) {
		DISP_DEBUG("Dimming status off, do not need enable dimming\n");
		return;
	}
	...
	mi_dsi_panel_set_disp_param(dsi, &ctl);
			...
			struct mtk_dsi *dsi)
{
	...
	kthread_queue_delayed_work(&dd_ptr->d_thread.worker,
...
void mi_dsi_panel_tigger_dimming_work(struct mtk_dsi *dsi)
{
	...
	disp_id = mi_get_disp_id("primary");
	mi_disp_set_dimming_queue_delayed_work(&df->d_display[disp_id], dsi);
}
EXPORT_SYMBOL(mi_dsi_panel_tigger_dimming_work);
...
int mi_dsi_panel_set_disp_param(struct mtk_dsi *dsi, struct disp_feature_ctl *ctl)
{
...
#ifndef CONFIG_DRM_PANEL_M16_42_0D_0A_DSC_VDO
	...
	case DISP_FEATURE_GAMUT:
		if (mi_cfg->crc_state == ctl->feature_val) {
			DISP_INFO("CRC state is the same(%d), return\n", mi_cfg->crc_state );
			break;
		}
		...
		default:
			DISP_INFO(" unknow ctl->feature_val = %d\n", ctl->feature_val);
			break;
		}
	...
	case DISP_FEATURE_HBM_BACKLIGHT:
		DISP_INFO("hbm brightness:%d\n", ctl->feature_val);
		mi_cfg->feature_val[DISP_FEATURE_HBM_BACKLIGHT] = ctl->feature_val;
		mtkfb_set_backlight_level(ctl->feature_val);
		break;
	default:
