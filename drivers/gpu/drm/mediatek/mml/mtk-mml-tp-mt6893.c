...
#define engine_wrot(e)	(e == MML_WROT0 || e == MML_WROT1)

struct path_node path_map[PATH_MML_MAX][MML_MAX_PATH_CACHES] = {
	[PATH_MML_DC_NOPQ_P0] = {
		{MML_MMLSYS,},
		{MML_MUTEX,},
		{MML_RDMA0, MML_WROT0,},
...
static u8 clt_dispatch[PATH_MML_MAX] = {
...
static void tp_dump_path(struct mml_topology_path *path)
{
	...
	for (i = 0; i < path->engine_cnt; i++) {
		mml_log("engine %hhu (%p) prev %p next %p %p comp %p tile idx %hhu",
			path->engines[i].id, &path->engines[i],
			path->engines[i].prev,
			path->engines[i].next[0], path->engines[i].next[1],
			path->engines[i].comp,
			path->engines[i].tile_eng_idx);
	}
}

static void tp_parse_path(struct mml_dev *mml, struct mml_topology_path *path,
	struct path_node *route)
{
	struct mml_engine *prev[2] = {0};
	u8 connect_eng[2] = {0};
	...
	for (i = 0; i < MML_MAX_PATH_CACHES; i++) {
		...
		path->engines[i].id = route[i].eng;
		path->engines[i].comp = mml_dev_get_comp_by_id(mml,
							       route[i].eng);

		if (!path->engines[i].comp)
			mml_err("no comp idx:%hhu engine:%hhu",
				i, route[i].eng);

		if (route[i].eng == MML_MMLSYS) {
			path->mmlsys = path->engines[i].comp;
			path->mmlsys_eid = i;
			continue;
		} else if (route[i].eng == MML_MUTEX) {
			path->mutex = path->engines[i].comp;
			path->mutex_eid = i;
			continue;
		}

		/* check cursor for 2 out and link if id match */
		if (!connect_eng[0] && route[i].next0) {
			/* First engine case, set current out 0 to this engine.
			 * And config next engines
			 */
			connect_eng[0] = route[i].next0;
			prev[0] = &path->engines[i];

		} else if (connect_eng[0] == route[i].eng) {
			/* connect out 0 */
			prev[0]->next[0] = &path->engines[i];

			/* replace current out 0 to this engine */
			path->engines[i].prev = prev[0];
			prev[0] = &path->engines[i];
			connect_eng[0] = route[i].next0;
			 ...
			 */
			if (!connect_eng[1] && route[i].next1) {
				prev[1] = &path->engines[i];
				connect_eng[1] = route[i].next1;
			}

		} else if (connect_eng[1] == route[i].eng) {
			/* connect out 1 */
			prev[1]->next[0] = &path->engines[i];
			/* replace current out 1 to this engine */
			path->engines[i].prev = prev[1];
			prev[1] = &path->engines[i];
			connect_eng[1] = route[i].next0;

			if (route[i].next1)
				mml_err("%s wrong path index %hhu engine %hhu",
					i, route[i].eng);
		} else {
			mml_err("connect fail idx:%hhu engine:%hhu next0:%hhu next1:%hhu from:%hhu %hhu",
				i, route[i].eng,
				route[i].next0, route[i].next1,
	...
	tile_idx = 0;
	for (i = 0; i < path->engine_cnt; i++) {
		if (!path->engines[i].prev && !path->engines[i].next[0]) {
			path->engines[i].tile_eng_idx = ~0;
			continue;
		}
		path->engines[i].tile_eng_idx = tile_idx;
	...
	/* scan out engines */
	out_eng_idx = 0;
	for (i = 0; i < path->engine_cnt && out_eng_idx < MML_MAX_OUTPUTS;
		i++) {
		if (!engine_wrot(path->engines[i].id))
			continue;
		path->out_engine_id[out_eng_idx++] = path->engines[i].id;
	}
...
const static struct mml_topology_ops topology_ops_mt6893 = {
...
{
	mml_topology_register_ip(TOPOLOGY_PLATFORM, &topology_ops_mt6893);
