...
static int mtk_drm_crtc_hbm_wait(struct drm_crtc *crtc, bool en, bool aod_en, bool pre, bool icon)
{
	struct mtk_panel_params *panel_ext = mtk_drm_get_lcm_ext_params(crtc);
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
	...
	if ((!factory_version)
		...
		if (!post_flush) {
			...
			if (hbm_en != last_hbm_en) {
				...
				if(need_delay)
					mtk_drm_crtc_hbm_wait(crtc, hbm_en,aod_en, false, false);
				if (hbm_en)
					hbm_en_wait = true;
			}
			last_hbm_en = hbm_en;
		...
		if (post_flush) {
			if (hbm_en) {
				if ((icon_en != last_icon_en) && (!hbm_en_wait)) {
					...
					mtk_drm_crtc_hbm_wait(crtc, icon_en,aod_en, false, true);
				}
