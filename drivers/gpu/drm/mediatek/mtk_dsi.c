...
#define DC_BACKLIGHT_THREHOLD  564
...
void mtk_notify_backlight_node(unsigned int backlight)
{
	...
	if((!g_mtk_dsi->fod_backlight_flag) && g_mtk_dsi->dc_flag && backlight < DC_BACKLIGHT_THREHOLD) {
...
static int mtk_dsi_set_touch_fps(struct mtk_dsi *dsi,
...
static int mtk_dsi_set_idle_fps(struct mtk_dsi *dsi,
		unsigned int cur_mode, unsigned int dst_mode)
{
	int ret = 0;

	if(dsi->ext->funcs->mode_switch) {
		ret = dsi->ext->funcs->mode_switch(dsi->panel, cur_mode,
				dst_mode, AFTER_DSI_POWERON);
	}
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case DSI_SET_IDLE_FPS:
	{
    		...
    		if(*src_mode != 0) {
        		mtk_dsi_set_idle_fps(dsi, *src_mode, 0);
    		}
	}
    		break;
	case DSI_SET_NON_IDLE_FPS:
	{
    		...
    		if(*src_mode != 0) {
        		mtk_dsi_set_touch_fps(dsi, 0, *src_mode);
    		}
	}
    		break;
	case GET_PANEL_NAME:
		...
		case DISPPARAM_DOZE_STATE:
		{
			...
			if (!(panel_ext && panel_ext->funcs && panel_ext->funcs->aod_set_state)) {
				...
				/* AOD->normal */
							...
							&& (panel_power_mode == DRM_BLANK_LP1 || panel_power_mode == DRM_BLANK_LP2)) {
						in_aod = false;
						panel_power_mode = doze_state;
						panel_ext->funcs->aod_set_state(dsi->panel, &in_aod);
