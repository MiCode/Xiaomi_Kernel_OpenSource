	...
	unsigned int cmd, void *data)
{
	...
	{
		int *value = data;
		mtk_aal_setbypass(comp, *value, handle);
	}
		break;
	default:
		AALERR("error cmd: %d\n", cmd);
...
void disp_aal_set_bypass(struct drm_crtc *crtc, int bypass)
{
	...
	DDPFUNC("ret = %d, bypass = %d", ret, bypass);
}
