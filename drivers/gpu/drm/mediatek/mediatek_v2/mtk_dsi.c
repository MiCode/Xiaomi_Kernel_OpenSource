	...
	if (!(mtk_crtc->mode_change_index & MODE_DSI_RES)) {
		...
		if (mtk_drm_lcm_is_connect())
			cmdq_pkt_wfe(cmdq_handle2,
				mtk_crtc->gce_obj.event[EVENT_TE]);
	#endif
#if IS_ENABLED(CONFIG_DRM_PANEL_L12A_36_02_0B_DSC_CMD) || IS_ENABLED(CONFIG_DRM_PANEL_L12A_42_02_0A_DSC_CMD)
	if (fps_src > fps_dst) {
		cmdq_pkt_clear_event(cmdq_handle2,
			mtk_crtc->gce_obj.event[EVENT_TE]);
		if (mtk_drm_lcm_is_connect())
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	/* add for display vdo panel fps count */
  	dsi_panel_fps_count(ctx, drm_mode_vrefresh(&adjusted_mode), 1);
#endif
}
