...
{
	if(en){
	DDPPR_ERR("tp disable lcd esd\n");
	}else{
	DDPPR_ERR("tp enable lcd esd\n");
	}
        esd_status = en;
}
...
static int mtk_drm_esd_recover(struct drm_crtc *crtc)
{
	...
	DDPPR_ERR("hsc mtk_drm_esd_recover enter\n");
...
static irqreturn_t _mi_esd_check_err_flag_irq_handler(int irq, void *data)
{
	...
	if (((mi_esd_ctx->err_flag_irq_flags & IRQF_TRIGGER_FALLING
		...
		&& !gpio_get_value(mi_esd_ctx->err_flag_irq_gpio))) {
		DDPPR_ERR("[ESD] err flag pin level is normal\n");
		return IRQ_HANDLED;
	}
	DDPPR_ERR("%s: [ESD]panel_init: %d\n", __func__, mi_esd_ctx->panel_init);
	if (mi_esd_ctx->panel_init) {
		atomic_set(&mi_esd_ctx->err_flag_event, 1);
		wake_up_interruptible(&mi_esd_ctx->err_flag_wq);
		DDPPR_ERR("[ESD]_esd_check_err_flag_irq_handler is comming\n");
	}
	else {
		DDPPR_ERR("[ESD]_esd_check_err_flag_irq_handler is comming, but ignore\n");
	}
	return IRQ_HANDLED;
}
...
static int mi_esd_err_flag_irq_check_worker_kthread(void *data)
{
	...
	while (1) {
		...
		if (ret < 0) {
			DDPPR_ERR("[ESD]check thread waked up accidently\n");
			continue;
		}
		DDPPR_ERR("ESD waked up\n");
		...
		DDPPR_ERR("[ESD]check thread is over\n");
		if(nvt_ts_esd_resume_probe == true){
			if(lcd_nvt_ts_esd_resume!=NULL){
				(*lcd_nvt_ts_esd_resume)();
			}
		}
		...
		if (mi_esd_ctx->err_flag_irq) {
			if (enable) {
				...
				if (mi_esd_ctx->err_flag_enabled) {
					...
					DDPPR_ERR("panel esd irq is disable\n");
				}
			}
		}
	} else {
		DDPPR_ERR("panel esd irq gpio invalid\n");
	}
	return 0;
}
void mi_disp_err_flag_esd_check_switch(struct drm_crtc *crtc, bool enable)
{
	...
	if (!mtk_drm_helper_get_opt(priv->helper_opt,
		...
		return;
	if (unlikely(!mi_esd_ctx)) {
		DDPPR_ERR("%s:invalid ESD context, crtc id:%d\n",
...
void mi_disp_esd_chk_init(struct drm_crtc *crtc)
{
	...
	if (!mi_disp_request_err_flag(crtc)) {
        DDPPR_ERR("%s mi_disp_request_err_flag\n", __func__);
