...
#define pr_fmt(fmt) "mi_disp_core: " fmt
			...
			struct cdev **cdevp)
{
	...
	if (ret < 0) {
		pr_err("allocate chrdev region failed for %s\n", name);
		goto err_exit;
	}

	cdev = cdev_alloc();
	if (!cdev) {
		pr_err("cdev alloc failed for %s\n", name);
		ret = -ENOMEM;
...
int mi_disp_core_init(void)
{
	...
	if (IS_ERR(disp_core->class)) {
		pr_err("class_create failed for %s\n", MI_DISPLAY_CLASS);
	...
	if (!disp_core->procfs_dir) {
		pr_err("proc_mkdir failed for %s\n", MI_DISPLAY_PROCFS_DIR);
		ret = -ENOMEM;
	...
	if (!disp_core->debugfs_dir) {
		pr_err("debugfs_create_dir failed for %s\n", MI_DISPLAY_DEBUGFS_DIR);
		ret = -ENOMEM;
