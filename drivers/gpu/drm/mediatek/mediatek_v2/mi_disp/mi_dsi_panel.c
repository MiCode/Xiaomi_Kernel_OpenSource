...
int mi_dsi_panel_get_cabc_mode(struct mtk_dsi *dsi, int *cabc_mode)
{
	...
	comp = &dsi->ddp_comp;
	panel_ext = mtk_dsi_get_panel_ext(comp);
	if (panel_ext && panel_ext->funcs && panel_ext->funcs->get_cabc_mode) {
		panel_ext->funcs->get_cabc_mode(dsi->panel, cabc_mode);
		dsi->mi_cfg.cabc_state = *cabc_mode;
		dsi->mi_cfg.feature_val[DISP_FEATURE_CABC] = *cabc_mode;
		return 0;
	} else {
		*cabc_mode = DDIC_CABC_OFF;
		DISP_ERROR("panel_get_cabc_mode func not defined\n");
	}
	return -EINVAL;
}
			...
			int doze_brightness)
{
	...
	if (!(panel_ext && panel_ext->funcs && panel_ext->funcs->set_doze_brightness)) {
		...
		if (!panel_ext->funcs->set_doze_brightness(dsi->panel, doze_brightness)) {
			mi_disp_feature_event_notify_by_type(mi_get_disp_id("primary"), MI_DISP_EVENT_DOZE, sizeof(doze_brightness), doze_brightness + DOZE_BRIGHTNESS_MAX);
...
unsigned int mi_get_panel_id(void)
{
	struct device_node *chosen;
	unsigned int vendor_id = 0xFF;
	int ret = 0;

	chosen = of_find_node_by_path("/chosen");
	if (chosen) {
		ret = of_property_read_u32(chosen, "panel_id", &vendor_id);
		if (ret == 0)
...
EXPORT_SYMBOL_GPL(mi_get_panel_id);
