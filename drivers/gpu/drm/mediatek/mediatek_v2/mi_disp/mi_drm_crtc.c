...
int mi_drm_bl_wait_for_completion(struct drm_crtc *crtc, unsigned int level)
{
	...
	if (is_backlight_set_block(dsi, level)) {
		...
		ret = wait_for_completion_timeout(&dsi->bl_wait_completion, msecs_to_jiffies(600));
...
static int mi_drm_update_fod_anim_status(struct mtk_dsi *dsi, bool fod_anim_flag)
{
	...
	if (mi_cfg->fod_anim_flag == false && fod_anim_flag == true && dsi->mi_cfg.doze_state) {
		...
		DISP_INFO("exit aod\n");
		mi_disp_feature_event_notify_by_type(mi_get_disp_id("primary"), MI_DISP_EVENT_DOZE, sizeof(dsi->mi_cfg.mdoze_brightness), DOZE_TO_NORMAL);
		...
		mi_disp_feature_event_notify_by_type(mi_get_disp_id("primary"), MI_DISP_EVENT_DOZE,
			sizeof(dsi->mi_cfg.mdoze_brightness), dsi->mi_cfg.mdoze_brightness + DOZE_BRIGHTNESS_MAX);
		dsi->mi_cfg.feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] = dsi->mi_cfg.mdoze_brightness;
	}
#endif
