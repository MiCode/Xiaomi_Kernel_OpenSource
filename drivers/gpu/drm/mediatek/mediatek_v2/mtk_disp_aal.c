...
void disp_aal_notify_backlight_changed(int trans_backlight, int max_backlight)
{
	...
	AALAPI_LOG("%d backup relay brightness : %d(%d), \n",
		__LINE__, trans_backlight, g_aal_hist.backlight);
...
static int record_brightness_node_value = 0;
static int pq_max_51backlight_threshold = 0;
	...
	void *v)
{
	...
	case LED_BRIGHTNESS_CHANGED:
		...
		if (m_new_pq_persist_property[DISP_PQ_GAMMA_SILKY_BRIGHTNESS] ||
			...
			record_brightness_node_value = led_conf->cdev.brightness;
			pq_max_51backlight_threshold = led_conf->pq_max_51backlight_threshold;
	...
	struct drm_file *file_priv)
{
	...
	if (m_new_pq_persist_property[DISP_PQ_CCORR_SILKY_BRIGHTNESS]) {
		...
		//if (pre_bl != cur_bl)
		AALAPI_LOG("gain = %u, backlight = %d, ELVSSPN = %u, flag = %u, silky_bl = %d",
			g_aal_param.silky_bright_gain[0], g_aal_backlight_set,
			g_aal_ess20_spect_param.ELVSSPN, g_aal_ess20_spect_param.flag,
			g_aal_hist.backlight);

		mtk_trans_gain_to_gamma(crtc, &g_aal_param.silky_bright_gain[0],
			g_aal_backlight_set, (void *)&g_aal_ess20_spect_param);
	} else {

		AALAPI_LOG("pre_bl=%d, bl=%d, pn=%u, flag=%u, record_brightness_node_value = %d", prev_backlight, g_aal_backlight_set,
			g_aal_ess20_spect_param.ELVSSPN, g_aal_ess20_spect_param.flag, record_brightness_node_value);
#if IS_ENABLED(CONFIG_DRM_PANEL_N16U_42_02_0A_DSC_VDO)
		//if(record_brightness_node_value > pq_max_51backlight_threshold) {
			if (prev_backlight == 0)
				g_aal_ess20_spect_param.flag |= (1 << SET_BACKLIGHT_LEVEL);
			if(g_aal_backlight_set != record_brightness_node_value) {
				AALERR("%s: pq_max_51backlight_threshold=%d, brightness node %d mismatch AAL set level %d\n",
					__func__, pq_max_51backlight_threshold, record_brightness_node_value, g_aal_backlight_set);
				g_aal_backlight_set = record_brightness_node_value;
				g_aal_ess20_spect_param.flag |= (1 << SET_BACKLIGHT_LEVEL);
                        }
		//}
#endif
	...
	unsigned int cmd, void *data)
{
	...
	case BYPASS_AAL:
	{
		...
		if (g_aal_fo->mtk_dre30_support) {
			...
			struct drm_crtc *crtc = &mtk_crtc->base;
			struct mtk_drm_private *priv = crtc->dev->dev_private;
			struct mtk_ddp_comp *comp_dmdp_aal0 = priv->ddp_comp[DDP_COMPONENT_DMDP_AAL0];
		...
		if (comp->mtk_crtc->is_dual_pipe) {
			...
			if (g_aal_fo->mtk_dre30_support) {
				...
				struct drm_crtc *crtc = &mtk_crtc->base;
				struct mtk_drm_private *priv = crtc->dev->dev_private;
				struct mtk_ddp_comp *comp_dmdp_aal1 = priv->ddp_comp[DDP_COMPONENT_DMDP_AAL1];
