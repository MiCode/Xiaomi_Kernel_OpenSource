...
static struct blocking_notifier_head mi_disp_feature_head;
/**
 * mi_disp_feature_register_notifier - get notified of mi_disp_feature
 * @nb: notifier block with the notifier to call on mi_disp_feature
 *
 * @return 0 on success, otherwise a negative error code
 *
 * Register a notifier to get notified of mi_disp_feature.
 */
int mi_disp_feature_register_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&mi_disp_feature_head, nb);
}
EXPORT_SYMBOL(mi_disp_feature_register_notifier);

/**
 * mi_disp_feature_unregister_notifier - unregister a mi_disp_feature notifier
 * @nb: notifier block to unregister
 *
 * @return 0 on success, otherwise a negative error code
 *
 * Unregister a notifier to mi_disp_feature.
 */
int mi_disp_feature_unregister_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&mi_disp_feature_head, nb);
}
EXPORT_SYMBOL(mi_disp_feature_unregister_notifier);

/**
 * mi_disp_feature_notifier_call_chain - notify clients of fb_events
 *
 */
int mi_disp_feature_notifier_call_chain(unsigned long val, void *v)
{
	return blocking_notifier_call_chain(&mi_disp_feature_head, val, v);
}
EXPORT_SYMBOL(mi_disp_feature_notifier_call_chain);
...
static int mi_esd_err_flag_irq_check_worker_kthread(void *data)
{
	...
	struct mi_esd_ctx *mi_esd_ctx = mtk_crtc->mi_esd_ctx;
	int ret = 0;
	unsigned long mi_disp_event_val;
	...
	while (1) {
		...
		/* 1.esd recovery */
		//mi_disp_event_val = MI_DISP_FEATURE_PANEL_DEAD;
		//mi_disp_feature_notifier_call_chain(mi_disp_event_val, NULL);

		mi_drm_esd_recover(crtc);

		mi_disp_event_val = MI_DISP_FEATURE_PANEL_RECOVERED;
		mi_disp_feature_notifier_call_chain(mi_disp_event_val, NULL);
