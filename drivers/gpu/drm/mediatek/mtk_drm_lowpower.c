...
void mtk_drm_crtc_touch_notify(int value)
{
	...
	if(value != old_value) {
		DDPINFO("%s-touch notify %d and src_idx = %d\n", __func__, value, src_idx);
		output_comp = mtk_ddp_comp_request_output(mtk_crtc);
		if(output_comp && mtk_show_brightness_clone(output_comp) > MAX_TOUCH_FPS_BRIGHTNESS) {
			if (value) {
				if(mtk_drm_is_idle_fps(crtc)) {
					...
					idlemgr->idlemgr_ctx->idle_fps_en = 0;
				}
			} else {
				idlemgr->idlemgr_ctx->idle_fps_en = 1;
			}
		}
		old_value = value;
	}
...
bool mtk_drm_enable_idle_fps(struct drm_crtc *crtc)
{
	...
	return idlemgr->idlemgr_ctx->idle_fps_en;
}
...
static void mtk_drm_idlemgr_disable_crtc(struct drm_crtc *crtc)
{
	...
	output_comp = mtk_ddp_comp_request_output(mtk_crtc);
	if (output_comp) {
		if (mtk_show_brightness_clone(output_comp) > MAX_TOUCH_FPS_BRIGHTNESS
						&& mtk_drm_enable_idle_fps(crtc)) {
			mtk_ddp_comp_io_cmd(output_comp, NULL, DSI_SET_IDLE_FPS,
				&src_idx);
			idlemgr->idlemgr_ctx->idle_fps_en = 0;
