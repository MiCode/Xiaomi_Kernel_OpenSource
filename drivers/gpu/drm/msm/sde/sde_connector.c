...
static int sde_backlight_device_update_status(struct backlight_device *bd)
{
	...
	if (brightness < 0) {
		SDE_ERROR("brightness is not invalid!\n");
		return rc;
	}
...
static int _sde_connector_update_bl_scale(struct sde_connector *c_conn)
{
	...
	SDE_DEBUG("bl_scale = %u, bl_scale_ad = %u, bl_level = %u dsi_display->panel->in_aod = %d bl_config->xiaomi_f10_flag = %d\n",
		bl_config->bl_scale, bl_config->bl_scale_ad,
		bl_config->bl_level, dsi_display->panel->in_aod,bl_config->xiaomi_f10_flag);
	if(bl_config->xiaomi_f10_flag && dsi_display->panel->in_aod){
		SDE_ERROR("display power mode is Doze ,set backlight failed.bl_scale = %u, bl_scale_ad = %u, bl_level = %u\n",
			bl_config->bl_scale, bl_config->bl_scale_ad,bl_config->bl_level);
	} else {
		rc = c_conn->ops.set_backlight(&c_conn->base,
	...
	if (!hbm_overlay) {
		if (dsi_display->panel->fod_dimlayer_hbm_enabled) {
			...
			if (dsi_display->drm_dev
				&& ((dsi_display->drm_dev->doze_brightness == DOZE_BRIGHTNESS_HBM)|| (dsi_display->drm_dev->doze_brightness == DOZE_BRIGHTNESS_LBM)) && !dsi_display->panel->sysfs_fod_unlock_success) {
				dsi_display->panel->doze_need_nolp = true;
...
void sde_connector_helper_bridge_enable(struct drm_connector *connector)
{
	...
	if (c_conn->bl_device) {
		...
		c_conn->bl_device->props.state &= ~BL_CORE_FBBLANK;
		/*if (strstr(display->name,"dsi_k6_38_0e_0b"))
			c_conn->bl_device->props.brightness = -1;*/
		backlight_update_status(c_conn->bl_device);
	}
