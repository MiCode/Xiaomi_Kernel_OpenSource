	...
	u32 bl_lvl)
{
	...
	if (panel->dc_enable && panel->dc_demura_threshold) {
		if (bl_lvl > panel->dc_demura_threshold
			...
			panel->backlight_demura_level = DEMURA_STATUS_DC_L1;
		} else if (panel->backlight_demura_level != DEMURA_STATUS_DC_L2) {
	...
	case DSI_BACKLIGHT_DCS:
		if (panel->fod_backlight_flag) {
			...
			if (panel->f4_51_ctrl_flag &&
				(panel->fod_hbm_enabled || panel->hbm_enabled)) {
				pr_info("if use 51 register control hbm and hbm on(%d,%d), skip set backlight\n",
					panel->fod_hbm_enabled, panel->hbm_enabled);
			} else {
				rc = dsi_panel_update_backlight(panel, bl_lvl);
			}
