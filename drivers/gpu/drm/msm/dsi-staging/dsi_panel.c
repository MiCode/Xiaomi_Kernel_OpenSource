...
int dsi_panel_set_doze_backlight(struct dsi_display *display)
{
	...
	if (drm_dev->doze_brightness == DOZE_BRIGHTNESS_LBM ||drm_dev->doze_brightness == DOZE_BRIGHTNESS_HBM){
		...
		panel->in_aod = true;
		pr_info("doze_state = %d, doze_brightness = %d panel->in_aod = %d panel->bl_config.xiaomi_f10_flag = %d\n", drm_dev->doze_state, drm_dev->doze_brightness,panel->in_aod,panel->bl_config.xiaomi_f10_flag);
	}
...
bool dc_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	float mDCBLCoeff1[2] = {0.5911, 4.2837};
	float mDCBLCoeff2[2] = {0.42045, 49.81818};
	...
	if (panel->k6_dc_flag && !panel->dc_enable && !panel->in_aod
		&& panel->doze_brightness == DOZE_BRIGHTNESS_INVALID) {
		/* 400 -  dc_threshold loggical */
		if (bl_lvl < panel->dc_threshold && bl_lvl >= 400) {
			crcValue = 0.5 + mDCBLCoeff2[0] * bl_lvl + mDCBLCoeff2[1]; //0.5 is for roundin
			pr_info("dc bl_lvl:%d crcValue:%d,", bl_lvl, crcValue);
			if (crcValue > 255)
				crcValue = 255;
			...
			if (panel->cur_mode->timing.refresh_rate == 60) {
				...
				if (cmds) {
					...
					tx_buf = (u8 *)cmds[6].msg.tx_buf;
					tx_buf[1] = (panel->dc_threshold >> 8) & 0xFF;
					tx_buf[2] = panel->dc_threshold & 0xFF;
				}
		...
		/* thredhold loggical */
		if  (0 != last_bl_lvl && ((last_bl_lvl >= 400 && bl_lvl < 400) || (last_bl_lvl <= panel->dc_threshold && bl_lvl > panel->dc_threshold))) {
			last_bl_lvl = bl_lvl;
			crcValue = 255;
			if (panel->cur_mode->timing.refresh_rate == 60) {
				cmds = priv_info->cmd_sets[DSI_CMD_SET_DISP_DC_CRC_SETTING_60HZ].cmds;
	...
	if (panel->k6_dc_flag && panel->dc_enable && bl_lvl < panel->dc_threshold && bl_lvl != 0
		...
		if (bl_lvl <= 80)
			crcValue = 0.5 + mDCBLCoeff1[0] * bl_lvl + mDCBLCoeff1[1]; //0.5 is for roundin
		else
			crcValue = 0.5 + mDCBLCoeff2[0] * bl_lvl + mDCBLCoeff2[1]; //0.5 is for roundin
		pr_info("dc bl_lvl:%d crcValue:%d,", bl_lvl, crcValue);
		if (crcValue > 255)
...
static int dsi_panel_parse_bl_config(struct dsi_panel *panel)
{
	...
	panel->bl_config.xiaomi_f10_flag = utils->read_bool(utils->data,
								"qcom,mdss-dsi-bl-xiaomi-f10-flag");
				     ...
				     struct device_node *of_node)
{
	...
	if (panel->fod_dimlayer_enabled) {
        ...
        if (panel->fod_dimlayer_enabled) {
		panel->fod_skip_nolp = utils->read_bool(of_node,
			"qcom,mdss-dsi-panel-fod-skip-nolp");
		if (panel->fod_skip_nolp) {
			pr_info("fod skip nolp\n");
		}
	}
...
#define BOOT_MODE_STR_LEN	8
static int dsi_panel_parse_boot_mode(struct dsi_panel *panel)
{
	char *boot_mode_ptr = NULL;
	char boot_mode_array[BOOT_MODE_STR_LEN] = {'\0'};

	boot_mode_ptr = (char *)strnstr(saved_command_line, "androidboot.mode=", strlen(saved_command_line));
	if (boot_mode_ptr) {
		memcpy(boot_mode_array, (boot_mode_ptr + strlen("androidboot.mode=")), BOOT_MODE_STR_LEN - 1);
		if (strnstr(boot_mode_array, "charger", strlen(boot_mode_array)))
			panel->boot_mode_charger = true;
		else
			panel->boot_mode_charger = false;
	} else {
		panel->boot_mode_charger = false;
		pr_err("can not find androidboot.mode=  from cmdline\n");

		return -1;
	}

	pr_info("boot_mode_charger = %d, boot_mode_array = %s\n", panel->boot_mode_charger, boot_mode_array);
				...
				int topology_override)
{
	...
	rc = dsi_panel_parse_boot_mode(panel);
	if (rc)
		pr_err("failed to parse boot mode, rc=%d\n", rc);
...
int dsi_panel_set_lp1(struct dsi_panel *panel)
{
	...
	if (panel->bl_config.xiaomi_f10_flag && (panel->fod_hbm_enabled || panel->fod_dimlayer_hbm_enabled || panel->fod_backlight_flag)) {
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	if (!panel->oled_panel_video_mode && !panel->fod_hbm_enabled && !panel->fod_dimlayer_hbm_enabled && (panel->doze_need_nolp || !(panel->fod_skip_nolp && panel->sysfs_fod_unlock_success))) {
		pr_info("%s set nolp cmd ,doze_need_nolp = %d , sysfs_fod_unlock_success = %d\n", __func__, panel->doze_need_nolp, panel->sysfs_fod_unlock_success);
...
int panel_disp_param_send_lock(struct dsi_panel *panel, int param)
{
	...
	case DISPPARAM_DIMMING:
		...
		if (panel->skip_dimmingon != STATE_DIM_BLOCK) {
			if (ktime_after(ktime_get(), panel->fod_hbm_off_time)
				...
				pr_info("set display dimming on cmd success.\n");
			} else {
	...
	case DISPPARAM_LCD_HBM_L1_ON:
		...
		pr_info("panel->fod_hbm_enabled = %d, thermal_hbm_disabled = %d\n", panel->fod_hbm_enabled, panel->thermal_hbm_disabled);
		if (!panel->fod_hbm_enabled && !panel->thermal_hbm_disabled)
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DISP_LCD_HBM_L1_ON);
		if (is_thermal_call) {
			pr_info("thermal clear hbm limit, restore previous lcd hbm L1 on\n");
		} else {
			panel->skip_dimmingon = STATE_DIM_BLOCK;
	...
	case DISPPARAM_LCD_HBM_L2_ON:
		...
		if (is_thermal_call) {
			pr_info("thermal clear hbm limit, restore previous lcd hbm L2 on\n");
		} else {
			panel->skip_dimmingon = STATE_DIM_BLOCK;
			panel->hbm_enabled = true;
		}
	...
	case DISPPARAM_LCD_HBM_L3_ON:
		...
		if (is_thermal_call) {
			pr_info("thermal clear hbm limit, restore previous lcd hbm L3 on\n");
		} else {
			panel->skip_dimmingon = STATE_DIM_BLOCK;
			panel->hbm_enabled = true;
		}
	...
	case DISPPARAM_LCD_HBM_OFF:
		...
		if (is_thermal_call) {
			pr_info("thermal set hbm limit, lcd hbm off\n");
		} else {
			panel->hbm_enabled = false;
			dsi_panel_set_backlight(panel, panel->last_bl_lvl);
		}
	...
	case DISPPARAM_FOD_UNLOCK_SUCCESS:
		...
		panel->sysfs_fod_unlock_success = true;

		break;
        case DISPPARAM_FOD_UNLOCK_FAIL:
                pr_info("Fod fingerprint unlock fail\n");
                panel->sysfs_fod_unlock_success = false;
...
int dsi_panel_enable(struct dsi_panel *panel)
{
	...
	if (panel->boot_mode_charger == true) {
		pr_info("[SDE] %s: boot_mode_charger is %d, DSI_CMD_SET_DISP_DIMMINGON\n", __func__, panel->boot_mode_charger);

		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DISP_DIMMINGON);
		if (rc) {
			pr_warn_ratelimited("[%s] failed to send DSI_CMD_SET_DISP_DIMMINGON cmds, rc=%d\n",
					panel->name, rc);
			rc = 0;
		}
	}
