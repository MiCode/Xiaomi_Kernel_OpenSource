	...
	struct drm_file *file_priv)
{
	...
	bool aod_state =  state->prop_val[CRTC_PROP_DOZE_ACTIVE];
	...
	if (backlight_value == 0) {
		if (aod_state){
			g_aal_param.cabc_fltgain_force = 0x3FF;
			pr_err("%s g_aal_param.cabc_fltgain_force = %d\n",
				__func__, g_aal_param.cabc_fltgain_force);
		} else {
