...
static int lcm_panel_bias_regulator_init(void)
{
	...
	if (IS_ERR(disp_bias_pos)) { /* handle return value */
		...
		pr_info("[error]get dsv_pos fail, error: %d\n", ret);
		return ret;
	}
	...
	if (IS_ERR(disp_bias_neg)) { /* handle return value */
		...
		pr_info("[error]get dsv_neg fail, error: %d\n", ret);
		return ret;
	}
...
static int lcm_panel_bias_enable(void)
{
	...
	if (ret < 0)
		pr_info("[error]set voltage disp_bias_pos fail, ret = %d\n", ret);
	retval |= ret;

	ret = regulator_set_voltage(disp_bias_neg, 5400000, 5400000);
	if (ret < 0)
		pr_info("[error]set voltage disp_bias_neg fail, ret = %d\n", ret);
	retval |= ret;

	/* enable regulator */
	ret = regulator_enable(disp_bias_pos);
	if (ret < 0)
		pr_info("[error]enable regulator disp_bias_pos fail, ret = %d\n", ret);
	retval |= ret;

	ret = regulator_enable(disp_bias_neg);
	if (ret < 0)
		pr_info("[error]enable regulator disp_bias_neg fail, ret = %d\n", ret);
	retval |= ret;
...
static int lcm_panel_bias_disable(void)
{
	...
	if (ret < 0)
		pr_info("[error]disable regulator disp_bias_neg fail, ret = %d\n", ret);
	retval |= ret;

	ret = regulator_disable(disp_bias_pos);
	if (ret < 0)
		pr_info("[error]disable regulator disp_bias_pos fail, ret = %d\n", ret);
	retval |= ret;
