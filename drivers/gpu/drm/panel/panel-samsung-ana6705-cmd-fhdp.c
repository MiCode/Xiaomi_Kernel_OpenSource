...
static void lcm_panel_fhd90_init(struct lcm *ctx)
{
	/* DSC Setting */
	/* Dsc 10bit input */
	lcm_dcs_write_seq_static(ctx, 0x9E, 0x11,
...
static void lcm_panel_fhd72_init(struct lcm *ctx)
{
	/* DSC Setting */
	/* Dsc 10bit input */
	lcm_dcs_write_seq_static(ctx, 0x9E, 0x11,
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	case 0:
		...
		break;
	case 1:
		lcm_panel_fhd72_init(ctx);
		break;
	case 2:
		lcm_panel_fhd90_init(ctx);
		break;
	case 3:
...
#define PLL_CLOCK_FHD_90 (478)
#define PLL_CLOCK_FHD_72 (478)
#define PLL_CLOCK_FHD_60 (478)
...
static struct drm_display_mode fhd_72_mode = {
	.clock		= 201983, //72Hz
	.hdisplay	= HACT_FHD,
	.hsync_start	= HACT_FHD + HFP,
	.hsync_end	= HACT_FHD + HFP + HSA,
...
static void mode_switch_to_fhd72(struct drm_panel *panel,
			enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	struct lcm *ctx = panel_to_lcm(panel);
		...
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	case 0:
		...
		break;
	case 1:
		mode_switch_to_fhd72(panel, stage);
		break;
	case 2:
...
struct mtk_panel_para_table msync_level_60[] = {
		...
		{5, {0x2B, 0x00, 0x00, 0x09, 0x6B}},

		/* Scaler Setting */
		{3, {0xF0, 0x5A, 0x5A}},
		{2, {0xC3, 0x89}},
		{4, {0xB0, 0x00, 0x24, 0xC3}},
		{18, {0xC3, 0xFD, 0x00, 0xFD, 0x00, 0xFD, 0x00,
				0xFD, 0x00, 0xFD, 0x00, 0xFD, 0x00, 0xFD, 0x00, 0xFD, 0x00,
				0xFD}},
		{4, {0xB0, 0x00, 0x3A, 0xC3}},
		{18, {0xC3, 0xB0, 0x00, 0xB0, 0x00, 0xB0, 0x00,
				0xB0, 0x00, 0xB0, 0x00, 0xB0, 0x00, 0xB0, 0x00, 0xB0, 0x00,
				0xB0}},
		{4, {0xB0, 0x00, 0x50, 0xC3}},
		{18, {0xC3, 0x43, 0x00, 0x43, 0x00, 0x43, 0x00,
				0x43, 0x00, 0x43, 0x00, 0x43, 0x00, 0x43, 0x00, 0x43, 0x00,
				0x43}},
		{4, {0xB0, 0x00, 0x61, 0xC3}},
		{23, {0xC3, 0xCC, 0xCC, 0xCC, 0xCC, 0xC0, 0xFE,
				0x00, 0xFE, 0x00, 0xFE, 0x00, 0xFE, 0x00, 0xFE, 0x00, 0xFE,
				0x00, 0xFE, 0x00, 0xFE, 0x00, 0xFE}},
		{3, {0xF0, 0xA5, 0xA5}},
			...
			struct drm_connector *connector)
{
	...
	mode1 = drm_mode_duplicate(connector->dev, &fhd_72_mode);
	if (!mode1) {
		dev_info(connector->dev->dev, "failed to add mode %ux%ux@%u\n",
			fhd_72_mode.hdisplay,
			fhd_72_mode.vdisplay,
			drm_mode_vrefresh(&fhd_72_mode));
		return -ENOMEM;
	}
	...
	list_add_tail(&fhd_72_mode.head, &ctx->probed_modes);
	...
	mode3 = drm_mode_duplicate(connector->dev, &fhd_120_mode);
	if (!mode3) {
		dev_info(connector->dev->dev, "failed to add mode %ux%ux@%u\n",
