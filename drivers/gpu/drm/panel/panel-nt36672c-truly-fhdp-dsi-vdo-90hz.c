...
static void truly_panel_init(struct truly *ctx)
{
	...
	if (current_fps == 60)
		truly_dcs_write_seq_static(ctx, 0x18, 0x21);
	else if (current_fps == 90)
		truly_dcs_write_seq_static(ctx, 0x18, 0x20);
	else
		truly_dcs_write_seq_static(ctx, 0x18, 0x21);
#endif
    ...
    truly_dcs_write_seq_static(ctx, 0xC0, 0x00);

    truly_dcs_write_seq_static(ctx, 0xFF, 0xF0);
    truly_dcs_write_seq_static(ctx, 0xFB, 0x01);
    truly_dcs_write_seq_static(ctx, 0x1C, 0x01);
    truly_dcs_write_seq_static(ctx, 0x33, 0x01);
    truly_dcs_write_seq_static(ctx, 0x5A, 0x00);
    truly_dcs_write_seq_static(ctx, 0xD2, 0x52);

    truly_dcs_write_seq_static(ctx, 0xFF, 0xD0);
    truly_dcs_write_seq_static(ctx, 0xFB, 0x01);
    truly_dcs_write_seq_static(ctx, 0x53, 0x22);
    truly_dcs_write_seq_static(ctx, 0x54, 0x02);

    truly_dcs_write_seq_static(ctx, 0xFF, 0xC0);
    truly_dcs_write_seq_static(ctx, 0xFB, 0x01);
    truly_dcs_write_seq_static(ctx, 0x9C, 0x11);
    truly_dcs_write_seq_static(ctx, 0x9D, 0x11);

    truly_dcs_write_seq_static(ctx, 0xFF, 0x10);
    truly_dcs_write_seq_static(ctx, 0xFB, 0x01);
    truly_dcs_write_seq_static(ctx, 0x35, 0x00);
    truly_dcs_write_seq_static(ctx, 0x51, 0xFF);
    truly_dcs_write_seq_static(ctx, 0x53, 0x0C);
...
static int truly_unprepare(struct drm_panel *panel)
{
	...
	truly_dcs_write_seq_static(ctx, MIPI_DCS_UPDATE_PAGE, MIPI_DCS_PAGE_ZERO);
...
static int truly_panel_ata_check(struct drm_panel *panel)
{
...
struct drm_display_mode *truly_get_mode_by_id_hfp(struct drm_panel *panel,
	unsigned int mode)
{
...
static int truly_mtk_panel_ext_param_set(struct drm_panel *panel, unsigned int mode)
{
	struct mtk_panel_ext *ext = find_panel_ext(panel);
	int ret = 0;
	struct drm_display_mode *m = truly_get_mode_by_id_hfp(panel, mode);
	printk("zzz+\n");
	if (m->vrefresh == 60) {
...
static int truly_mtk_panel_ext_param_get(struct mtk_panel_params *ext_para,
			 unsigned int mode)
{
	int ret = 0;
...
static void truly_mode_switch_to_90(struct drm_panel *panel)
{
	struct truly *ctx = panel_to_truly(panel);

	truly_dcs_write_seq_static(ctx, 0xFF, 0x25);
	truly_dcs_write_seq_static(ctx, 0xFB, 0x01);
	truly_dcs_write_seq_static(ctx, 0x18, 0x20);//90hz

}

static void truly_mode_switch_to_60(struct drm_panel *panel)
{
	struct truly *ctx = panel_to_truly(panel);
...
static int truly_mode_switch(struct drm_panel *panel, unsigned int cur_mode,
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	if (dst_mode == 60) { /* 60 switch to 120 */
		truly_mode_switch_to_60(panel);
	} else if (dst_mode == 90) { /* 1200 switch to 60 */
		truly_mode_switch_to_90(panel);
	} else
		ret = 1;
...
static int truly_panel_ext_reset(struct drm_panel *panel, int on)
{
	struct truly *ctx = panel_to_truly(panel);
...
static struct mtk_panel_funcs ext_funcs = {
	.reset = truly_panel_ext_reset,
	.set_backlight_cmdq = truly_setbacklight_cmdq,
	.ext_param_set = truly_mtk_panel_ext_param_set,
	.ext_param_get = truly_mtk_panel_ext_param_get,
	.mode_switch = truly_mode_switch,
	.ata_check = truly_panel_ata_check,
};
#endif
...
static int truly_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("[LCM][KERNEL]dsi_panel_K19_43_02_0b_vdo probe!!!\n");

	if (strstr(saved_command_line, "dsi_panel_K19_43_02_0b_vdo")) {
		pr_err("dsi_panel_K19_43_02_0b_vdo truly probe match");
	} else {
		pr_err("not match dsi_panel_K19_43_02_0b_vdo !!!\n");
		return -ENODEV;
	}
	...
	pr_info("%s- dsi_panel_K19_43_02_0b_vdo,rt4801\n", __func__);
...
static struct mipi_dsi_driver truly_driver = {
	...
	.remove = truly_remove,
	.driver = {
		.name = "dsi_panel_K19_43_02_0b_vdo",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("truly nt36672c VDO Panel Driver");
MODULE_LICENSE("GPL v2");
/* BSP.lcm - 2020.12.02 - end */
