...
static bool elvss_control_pending = false;
	...
	if (stage == BEFORE_DSI_POWERDOWN) {
		switch (fps) {
			...
			case 120:
				...
				break;
			case 90:	
				if ((ctx->dynamic_fps == 60) || (ctx->dynamic_fps == 40) ||(ctx->dynamic_fps == 30) || (ctx->dynamic_fps == 24)) {
...
static int panel_set_doze_brightness(struct drm_panel *panel, int doze_brightness)
{
	int ret;
	struct lcm *ctx;
	char AOD_setting_mode_cmd[] = {0x53, 0x20};
	char AOD_setting_hbm_cmd[] = {0x51, 0x00, 0xE5};
	char AOD_setting_lbm_cmd[] = {0x51, 0x00, 0x10};
	...
	cmd_msg.type[0] = ARRAY_SIZE(AOD_setting_mode_cmd) > 2 ? 0x39 : 0x15;
	cmd_msg.tx_buf[0] = AOD_setting_mode_cmd;
	cmd_msg.tx_len[0] = ARRAY_SIZE(AOD_setting_mode_cmd);

	if (doze_brightness == DOZE_BRIGHTNESS_HBM) {
		cmd_msg.type[1] = ARRAY_SIZE(AOD_setting_hbm_cmd) > 2 ? 0x39 : 0x15;
		cmd_msg.tx_buf[1] = AOD_setting_hbm_cmd;
		cmd_msg.tx_len[1] = ARRAY_SIZE(AOD_setting_hbm_cmd);
	} else {
		cmd_msg.type[1] = ARRAY_SIZE(AOD_setting_lbm_cmd) > 2 ? 0x39 : 0x15;
		cmd_msg.tx_buf[1] = AOD_setting_lbm_cmd;
		cmd_msg.tx_len[1] = ARRAY_SIZE(AOD_setting_lbm_cmd);
	}
...
static int local_hbm_off_to_normal(struct mtk_dsi *dsi, enum local_hbm_state lhbm_state)
{
	...
	char AOD_setting_hbm_cmd3[] = {0x53, 0x20};
	char AOD_setting_hbm_cmd4[] = {0x51, 0x00, 0xE5};
	...
	char AOD_setting_lbm_cmd3[] = {0x53, 0x20};
	char AOD_setting_lbm_cmd4[] = {0x51, 0x00, 0x10};
...
static void panel_elvss_control(struct drm_panel *panel, bool en)
{
	...
	if (en && atomic_read(&lhbm_enable)) {
		elvss_control_pending = true;
		pr_err("lhbm_enable skip elvss control\n");
		return;
	}
	pr_info("%s dimming = %d\n", __func__, en);
	ctx = panel_to_lcm(panel);
...
static int panel_set_lhbm_fod(struct mtk_dsi *dsi, enum local_hbm_state lhbm_state)
{
	...
	switch (lhbm_state) {
		...
		case LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT_RESTORE: {
			...
			atomic_set(&lhbm_enable, 0);
			if (elvss_control_pending) {
				pr_err("%s: reset elvss status\n", __func__);
				panel_elvss_control(dsi->panel, true);
				elvss_control_pending = false;
			}
		} break;
...
static void lcm_esd_restore_backlight(struct drm_panel *panel)
{
	...
	if (ctx->hbm_en) {
		lcm_dcs_write(ctx, hbm_on_write_cmd0, ARRAY_SIZE(hbm_on_write_cmd0));
	}
...
static int panel_doze_suspend(struct drm_panel *panel,
	void *dsi, dcs_write_gce cb, void *handle)
{
	char level2_key_enable_tb[] = {0xF0, 0x5A, 0x5A};
	char AOD_setting_cmd0[] = {0xBB, 0x31};
	char AOD_setting_cmd1[] = {0xF7, 0x0F};
	char AOD_setting_cmd2[] = {0x53, 0x24}; // aod mode
	char AOD_setting_hbm[] = {0x51, 0x07, 0xFE};
	char AOD_setting_lbm[] = {0x51, 0x00, 0xF6};
	...
	cb(dsi, handle, AOD_setting_cmd0, ARRAY_SIZE(AOD_setting_cmd0));
	cb(dsi, handle, AOD_setting_cmd1, ARRAY_SIZE(AOD_setting_cmd1));
	cb(dsi, handle, AOD_setting_cmd2, ARRAY_SIZE(AOD_setting_cmd2));

	if (ctx->doze_brightness_state == DOZE_BRIGHTNESS_HBM) {
		cb(dsi, handle, AOD_setting_hbm, ARRAY_SIZE(AOD_setting_hbm));
	} else if (ctx->doze_brightness_state == DOZE_BRIGHTNESS_LBM) {
		cb(dsi, handle, AOD_setting_lbm, ARRAY_SIZE(AOD_setting_lbm));
	}
