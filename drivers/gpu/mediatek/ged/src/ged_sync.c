...
static const struct file_operations ged_sync_fops = {
	...
	struct GED_BRIDGE_OUT_CREATE_TIMELINE *out)
{
	...
	if (!obj) {
		GED_PDEBUG("sync_timeline_create() fail\n");
		goto err_obj;
	}

	timeline_fd = anon_inode_getfd(in->name,	&ged_sync_fops, obj, 0);

	if (timeline_fd < 0) {
		GED_PDEBUG("anon_inode_getfd() fail\n");
		goto err_fd;
	}

	out->timeline_fd = timeline_fd;
