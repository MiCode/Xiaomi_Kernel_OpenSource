...
unsigned int gpudfd_init(struct platform_device *pdev)
{
	...
	/* 0x1000d060 */
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "drm_debug_base");
	if (unlikely(!res)) {
		GPUFREQ_LOGE("fail to get resource DRM_DEBUG_BASE");
		goto done;
	}
	g_drm_debug_base = devm_ioremap(gpufreq_dev, res->start, resource_size(res));
	if (unlikely(!g_drm_debug_base)) {
		GPUFREQ_LOGE("fail to ioremap DRM_DEBUG_BASE: 0x%llx", res->start);
		goto done;
	}
