...
#include <platform/mt2712/mali_kbase_devfreq_mediatek.h>
...
#ifdef ENABLE_MTK_GPU_DEBUG
#include "platform/mtk_platform_common.h"
#endif /* ENABLE_MTK_GPU_DEBUG */
...
static ssize_t show_gpu_loading(struct device *dev,
		struct device_attribute *attr, char * const buf)
{
	int loading = mtk_get_gpu_loading_fp();
...
static DEVICE_ATTR(gpu_loading, 0644, show_gpu_loading, NULL);
...
static int power_control_init(struct platform_device *pdev)
{
	...
	/* MTK: Follow the previous flow to avoid enable mfg clk */
