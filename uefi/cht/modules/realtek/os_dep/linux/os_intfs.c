...
static int rtw_ndev_notifier_call(struct notifier_block * nb, unsigned long state, void *ndev)
{
	...
	if (dev == NULL || dev->netdev_ops == NULL) {
		pr_warn("%s: struct net_device passed in has NULL elements, returning early "
			"(no state change)\n", __func__);
		return NOTIFY_DONE;
	}
	...
	DBG_871X_LEVEL(_drv_always_, FUNC_NDEV_FMT" state:%lu\n", FUNC_NDEV_ARG(dev), state);
...
struct net_device *rtw_init_netdev(_adapter *old_padapter)
{
	...
	DBG_871X_LEVEL(_drv_always_,
		"%s: pnetdev->netdev_ops=%p, rtw_netdev_ops=%p\n",
		 __func__, pnetdev->netdev_ops, &rtw_netdev_ops);
#else
...
#ifdef CONFIG_BT_COEXIST
        rtw_btcoex_IpsNotify(padapter, IPS_NONE);  //for fix coexisting issue with BT from Realtek
#endif
	}
	padapter->net_closed = _FALSE;
