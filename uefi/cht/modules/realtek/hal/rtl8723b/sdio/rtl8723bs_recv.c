	...
	do {
		...
		while (ptr < precvbuf->ptail)
		{
			...
			else
			{
				...
				{
					DBG_871X_LEVEL(_drv_always_, "pkt_copy: alloc_sz=%d, head=%p, data=%p, tail=%d, end=%d, len=%d\n", alloc_sz,
							pkt_copy->head, pkt_copy->data, pkt_copy->tail, pkt_copy->end, pkt_copy->len);

					DBG_871X_LEVEL(_drv_always_, "alloc_skb fail!, free recvframe!\n");

					rtw_free_recvframe(precvframe, &precvpriv->free_recv_queue);
					break;
				}

				if(recvframe_put(precvframe, skb_len)==NULL)
				{
					DBG_871X_LEVEL(_drv_always_, "abnormal skb_len=%d\n", skb_len);
					rtw_free_recvframe(precvframe, &precvpriv->free_recv_queue);
					break;
				}
				...
				else if(pattrib->pkt_rpt_type == C2H_PACKET)
				{
					...
					DBG_871X_LEVEL(_drv_always_, "%s: [WARNNING] RX type(%d) not be handled!\n",
