			...
			struct cpu_stop_work *work_buf)
{
	...
	if (work_buf->queue_stack[0] != 0) {
#ifdef CONFIG_KALLSYMS
		for (i = 0; i < 7; i++) {
			sprint_symbol(sym, work_buf->queue_stack[i]);
			pr_info("last queue statck[%d]:%s\n", i, sym);
		}
#endif
	}
...
#if IS_ENABLED(CONFIG_MTK_IRQ_MONITOR_DEBUG)
		memset(work->queue_stack, 0, sizeof(work->queue_stack));
#endif
