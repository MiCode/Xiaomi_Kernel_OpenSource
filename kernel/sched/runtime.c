...
static u64 direct_reclaim_count;
static u64 direct_reclaim_count_back;
...
static struct reclaimed_time global_reclaimed_time;
static struct reclaimed_time global_reclaimed_time_back;
/*static u64 top_app_load[TOP_TASKS] = {0};*/
static DECLARE_BITMAP(package_uid_bit, (TRACE_UID_MAX+1));
static int develop_mode;

static __inline__ int init_reclaimed_stat(struct reclaimed_time *rt);
...
print_user_zero:
		if (user) {
			...
			seq_printf(m, "%5d: <b>%16llu [%3d.%02d] <l>: %16llu [%3d.%02d] bias: [%3d.%02d]\
				total:%16llu max:%8llu anon_max:%8llu other:%8llu\n", bit,
				(user->big_cluster_runtime[prev_index] -
				user->big_cluster_runtime[pprev_index]),
				...
				user->reclaimed_time.total - 	user->reclaimed_time_back.total,
				user->reclaimed_time.max - user->reclaimed_time_back.max,
				user->reclaimed_time.anon_max - user->reclaimed_time_back.anon_max,
				user->reclaimed_time.max_file_slab - user->reclaimed_time_back.max_file_slab
			);
	...
	seq_printf(m, "\nusage--- big cluster:[%d.%d] little cluster :[%d.%d]\
			reclaimed total:%16llu, max:%8llu, anon_max:%8llu, other:%8llu, count %llu\n",
			bcluster_usage/100, bcluster_usage%100,
			lcluster_usage/100, lcluster_usage%100,
			global_reclaimed_time.total - global_reclaimed_time_back.total,
			global_reclaimed_time.max - global_reclaimed_time_back.max,
			global_reclaimed_time.anon_max - global_reclaimed_time_back.anon_max,
			global_reclaimed_time.max_file_slab - global_reclaimed_time_back.max_file_slab,
			direct_reclaim_count - direct_reclaim_count_back
	);
...
static int package_runtime_show(struct seq_file *m, void *v)
{
	...
	for_each_process(tsk) {
		...
		for_each_thread (tsk, p) {
			...
			if (bcluster_bias < 0)
				bcluster_bias = 0;
			seq_printf(m, "%5d,%16s,b:%16llu [%3d.%02d],l:%16llu [%3d.%02d], b/b+l:[%3d.%02d]\
					total:%16llu max:%8llu anon_max:%8llu other:%8llu\n", p->pid,
					...
					bcluster_bias%100,
					p->reclaimed_time.total - p->reclaimed_time_back.total,
					p->reclaimed_time.max - p->reclaimed_time_back.max,
					p->reclaimed_time.anon_max - p->reclaimed_time_back.anon_max,
					p->reclaimed_time.max_file_slab - p->reclaimed_time_back.max_file_slab);
		}
	}
...
static void set_traced_window(int window_idx)
{
	...
	direct_reclaim_count_back = direct_reclaim_count;
	memcpy(&global_reclaimed_time_back, &global_reclaimed_time,
			sizeof(struct reclaimed_time));
	for_each_set_bit(bit, package_uid_bit,
		...
		if (user) {
			...
			free_uid(user);
			memcpy(&user->reclaimed_time_back, &user->reclaimed_time,
					sizeof(struct reclaimed_time));
		} else {
			...
			memcpy(&user_zero.reclaimed_time_back,
					&user_zero.reclaimed_time,
					sizeof(struct reclaimed_time));
		}
	...
	for_each_process(ptsk) {
		for_each_thread(ptsk, tsk) {
			memcpy(&tsk->reclaimed_time_back, &tsk->reclaimed_time,
					sizeof(struct reclaimed_time));
...
{
	if (rt)
		memset(rt, 0, sizeof(struct reclaimed_time));
...
static int __init runtime_init(void)
{
	...
	init_reclaimed_stat(&global_reclaimed_time);
	runtime_enable = 1;
	return 0;
}
...
void reclaimed_time_trace_begin(u64 time_stamp)
{
	direct_reclaim_count++;
	current->reclaimed_time.anon_cur = 0;
}
EXPORT_SYMBOL(reclaimed_time_trace_begin);

static void update_reclaimed_time(struct reclaimed_time *rt, u64 delta)
{
	if (!rt || delta <= 0)
		return;
	...
	if (current->reclaimed_time.anon_max > rt->anon_max)
		rt->anon_max = current->reclaimed_time.anon_max;

	if (delta > rt->max)
		rt->max = delta;

	if (delta > current->reclaimed_time.anon_cur + rt->max_file_slab)
		rt->max_file_slab = delta - current->reclaimed_time.anon_cur;

}

void update_task_anon_cur (u64 delta)
{
	if (current->flags & PF_MEMALLOC)
		current->reclaimed_time.anon_cur += delta;
}
EXPORT_SYMBOL(update_task_anon_cur);

void reclaimed_time_trace_end(u64 delta)
{
	current->reclaimed_time.total += delta;

	if (current->reclaimed_time.anon_cur > current->reclaimed_time.anon_max)
		current->reclaimed_time.anon_max = current->reclaimed_time.anon_cur;

	if (delta > current->reclaimed_time.max)
		current->reclaimed_time.max = delta;

	if (delta > current->reclaimed_time.anon_cur + current->reclaimed_time.max_file_slab)
		current->reclaimed_time.max_file_slab = delta - current->reclaimed_time.anon_cur;

	update_reclaimed_time(&global_reclaimed_time, delta);
	update_reclaimed_time(&task_user(current)->reclaimed_time, delta);
}
EXPORT_SYMBOL(reclaimed_time_trace_end);
...
void init_task_runtime_info(struct task_struct *tsk)
{
	...
	if (tsk) {
		...
		init_reclaimed_stat(&tsk->reclaimed_time);
	}
}
...
void init_package_runtime_info(struct user_struct *user)
{
	...
	init_reclaimed_stat(&user->reclaimed_time);
