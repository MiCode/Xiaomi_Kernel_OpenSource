...
static int package_runtime_disable = 1;
...
print_user_zero:
		if (user) {
			...
			seq_printf(m,
				"%5d: <b>%16llu [%2d.%02d]\t<l>: %16llu [%2d.%02d]\t<b/b+l>: [%2d.%02d],\n", bit,
				(user->big_cluster_runtime[prev_index] -
				user->big_cluster_runtime[pprev_index]),
...
static int package_runtime_show(struct seq_file *m, void *v)
{
	...
	for_each_process(tsk) {
		...
		for_each_thread (tsk, p) {
			...
			if (bcluster_bias < 0)
				bcluster_bias = 0;
			seq_printf(m, "%5d,%16s,b:%16llu [%2d.%02d],l:%16llu [%2d.%02d], \
					b/b+l:[%2d.%02d],\n", p->pid,
...
static int package_trace_stat(struct seq_file *m, void *v)
{
	...
	/*seq_printf(m, "top app load info %llu %llu %llu:\n",
	top_app_load[0], top_app_load[1], top_app_load[2]);*/
	for_each_process(ptsk) {
		for_each_thread(ptsk, tsk) {
			tsk->big_cluster_runtime[HISTORY_ITMES+1]
				= tsk->big_cluster_runtime[HISTORY_ITMES];
			tsk->little_cluster_runtime[HISTORY_ITMES+1]
				= tsk->little_cluster_runtime[HISTORY_ITMES];
			/*if (is_top_app(tsk))
				seq_printf(m, "%s %d %u %u\n", tsk->comm, tsk->pid,
				task_uid(tsk).val, tsk->ravg.pred_demand);*/
		}
	}
	rcu_read_unlock();
...
static void create_runtime_proc(void)
{
	...
	if (package_rootdir) {
		if (!package_showall_entry)
			package_showall_entry = proc_create("show_all", 0, package_rootdir,
					&package_show_all_fops);

		if (!package_tracestat_entry)
			package_tracestat_entry = proc_create("show_tracestat", 0, package_rootdir,
						&package_tracestat_fops);

		if (!package_top_bcore_entry)
			package_top_bcore_entry = proc_create("top_package_on_bcore", 0, package_rootdir,
						&package_top_bcore_fops);

		if (!package_top_lcore_entry)
			package_top_lcore_entry = proc_create("top_package_on_lcore", 0, package_rootdir,
					       &package_top_lcore_fops);

		if (!package_top_entry)
			package_top_entry = proc_create("top_package", 0, package_rootdir,
					&package_top_fops);

		if (!package_trace_entry)
			package_trace_entry = proc_create("show_traced_package", 0, package_rootdir,
					&package_trace_fops);

		if (!package_window_size_entry)
			package_window_size_entry = proc_create("show_windowsize", 0, package_rootdir,
					&package_windowsize_fops);

		if (!package_runtime_entry)
			package_runtime_entry = proc_create("package_runtime_disable", 0,
					package_rootdir, &package_runtime);

		if (!package_develop_mode_entry)
			package_develop_mode_entry = proc_create("develop_mode", 0,
