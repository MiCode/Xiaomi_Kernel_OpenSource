...
static u64 penalize_yield(u64 delta, struct walt_task_struct *wts)
{
	if (opts_init && opts.penalize_yield)
...
{
	if (opts_init && opts.update_yield_util)
				 ...
				 int event, u64 wallclock, u64 irqtime)
{
	...
	if (!is_idle_task(p)) {
		if (new_window && opts_init && opts.rollover_task_window)
                                  ...
                                  u64 wallclock, u64 irqtime)
{
	...
	/* sched_opt */
	if (old_window_start != wrq->window_start) {
		if(opts_init && opts.rollover_cpu_window)
...
//MIUI ADD: Performance_BoostFramework
	if (rows == 4)
		rows = sizeof(cpu_arrays_init_x54) / sizeof(cpu_arrays_init_x54[0]);
//END Performance_BoostFramework
...
//MIUI ADD: Performance_BoostFramework
	...
	case 1:
		...
		size = sizeof(cpu_arrays_init_x11) / sizeof(cpu_arrays_init_x11[0]);
		break;
	case 2:
		select_init_list = (u8 *)cpu_arrays_init_x22;
		size = sizeof(cpu_arrays_init_x22) / sizeof(cpu_arrays_init_x22[0]);
		break;
	case 3:
		select_init_list = (u8 *)cpu_arrays_init_x33;
		size = sizeof(cpu_arrays_init_x33) / sizeof(cpu_arrays_init_x33[0]);
		break;
	case 4:
		select_init_list = (u8 *)cpu_arrays_init_x54;
		size = sizeof(cpu_arrays_init_x54) / sizeof(cpu_arrays_init_x54[0]);
		break;
	default:
...
bool find_heaviest_topapp(u64 window_start)
{
	...
	/* sched_opt */
	if(opts_init && opts.update_heavy && have_heavy_list > 0)
				...
				bool is_shared_rail_migration, struct cpumask *lock_cpus)
{
	...
	for_each_sched_cluster(cluster) {
		u64 aggr_grp_load = 0;
		/* sched_opt */
		if(opts_init && opts.cluster_init)
...
{
	if(opts_init && opts.account_wakeup)
		...
		struct task_struct *next, struct rq *rq)
{
	...
	if (likely(prev != next)) {
		...
		if (opts_init && opts.update_yield_ts)
